...
export class ScTasksPage extends LitElement {
      ...
      <div>
        <div class="page-header">
          ...
          <div class="view-toggle">
            <button class=${this.viewMode === 'list' ? 'active' : ''} @click=${() => this.viewMode = 'list'}>📋 列表</sc-button>
            <button class=${this.viewMode === 'kanban' ? 'active' : ''} @click=${() => this.viewMode = 'kanban'}>📊 看板</sc-button>
          </div>
        </div>
        <div class="stats-row">
        ...
        <div class="toolbar">
          <div class="toolbar-left">
            <sc-button size="sm" variant="primary" @click=${this.openCreateModal}>➕ 新建任务</sc-button>
            ${this.selectedTasks.size > 0 ? html`<sc-button size="sm" variant="secondary" @click=${() => this.batchUpdateStatus('in_progress')}>▶ 批量开始</sc-button><sc-button size="sm" variant="secondary" @click=${() => this.batchUpdateStatus('completed')}>✓ 批量完成</sc-button><sc-button size="sm" variant="danger" @click=${() => this.batchUpdateStatus('cancelled')}>✕ 批量取消</sc-button><span style="color:var(--sc-text-secondary);font-size:13px;">已选 ${this.selectedTasks.size} 项</span>` : ''}
  ...
  private renderListView(tasks: Task[]) {
    if (tasks.length === 0) return html`<div class="empty-state"><div class="empty-icon">📋</div><div>暂无任务</div><sc-button variant="primary" style="margin-top:16px;" @click=${this.openCreateModal}>创建第一个任务</sc-button></div>`;
    return html`<div class="table-wrap"><table><thead><tr><th style="width:40px;"><input type="checkbox" class="checkbox" .checked=${this.selectedTasks.size === tasks.length && tasks.length > 0} @change=${this.toggleSelectAll} /></th><th @click=${() => this.handleSort('id')}>ID ${this.sortBy === 'id' ? (this.sortOrder === 'asc' ? '↑' : '↓') : ''}</th><th @click=${() => this.handleSort('title')}>任务 ${this.sortBy === 'title' ? (this.sortOrder === 'asc' ? '↑' : '↓') : ''}</th><th>类型</th><th>优先级</th><th @click=${() => this.handleSort('status')}>状态 ${this.sortBy === 'status' ? (this.sortOrder === 'asc' ? '↑' : '↓') : ''}</th><th>操作</th></tr></thead><tbody>${tasks.map(t => html`<tr class=${this.selectedTasks.has(t.id) ? 'selected' : ''}><td><input type="checkbox" class="checkbox" .checked=${this.selectedTasks.has(t.id)} @change=${() => this.toggleTaskSelection(t.id)} /></td><td><code style="font-size:12px;color:var(--sc-text-secondary);">${t.id}</code></td><td><span style="cursor:pointer;color:#60a5fa;" @click=${() => this.openDetailModal(t)}>${t.title}</span></td><td>${t.type ? this.getTypeText(t.type) : '-'}</td><td><span class="badge ${this.getPriorityBadgeClass(t.priority || 'medium')}">${t.priority || '中'}</span></td><td><span class="badge ${this.getStatusBadgeClass(t.status)}">${this.getStatusText(t.status)}</span></td><td><sc-button variant="secondary" size="sm" style="margin-right:4px;" @click=${() => this.openEditModal(t)}>编辑</sc-button><sc-button variant="danger" size="sm" @click=${() => this.deleteTask(t.id)}>删除</sc-button></td></tr>`)}</tbody></table></div>`;
  }
