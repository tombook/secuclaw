...
`;
  ...
  @state() private _businessFrauCourses: Array<{id:string;title:string;category:string;duration:number;enrolled:number;completed:number;difficulty:string;rating:number}> = [
  ...
  @state() private _businessFrauLearningPaths: Array<{id:string;name:string;courseIds:string[];progress:number;enrolled:number}> = [
  ...
  @state() private _businessFrauDeptCompliance: Array<{dept:string;trainedPct:number;targetPct:number;avgScore:number;certCount:number}> = [
  ...
  @state() private _businessFrauSkillsGaps: Array<{skill:string;current:number;required:number;gap:number;priority:string}> = [
  ...
  private _renderBusinessfrauTraining(): TemplateResult {
    const courses = this._businessFrauCourses;
    const deptComp = this._businessFrauDeptCompliance;
    return html`
  ...
  @state() private _businessFrauCorrRules: Array<{id:string;name:string;sources:string[];logic:string;severity:string;active:boolean;lastTriggered:string}> = [
  ...
  @state() private _businessFrauEventTimeline: Array<{timestamp:string;source:string;eventType:string;details:string;correlated:boolean}> = [
  ...
  @state() private _businessFrauFalsePosMetrics: {totalEvents:number;correlatedEvents:number;falsePositives:number;fpRate:number;topFpRules:string[]} = {
  ...
  @state() private _businessFrauEventPatterns: Array<{id:string;pattern:string;frequency:number;firstSeen:string;lastSeen:string;status:string}> = [
  ...
  private _renderBusinessfrauEventCorr(): TemplateResult {
    const rules = this._businessFrauCorrRules;
    const timeline = this._businessFrauEventTimeline;
    const fpMetrics = this._businessFrauFalsePosMetrics;
    return html`
  ...
  @state() private _businessFrauZones: Array<{id:string;name:string;trustLevel:number;subnet:string;devices:number;policy:string;lastAudit:string}> = [
  ...
  @state() private _businessFrauSegRules: Array<{id:string;source:string;dest:string;action:string;protocol:string;port:string;status:string;hits:number}> = [
  ...
  @state() private _businessFrauCrossZoneTraffic: Array<{source:string;dest:string;bytes:number;sessions:number;violations:number}> = [
  ...
  @state() private _businessFrauMicroSegGaps: Array<{id:string;zone:string;gapType:string;severity:string;recommendation:string}> = [
  ...
  private _renderBusinessfrauNetworkSeg(): TemplateResult {
    const zones = this._businessFrauZones;
    const gaps = this._businessFrauMicroSegGaps;
    return html`
  ...
  @state() private _businessFrauRiskTopTen: Array<{id:string;name:string;score:number;trend:string;owner:string;category:string;lastAssessed:string}> = [
  ...
  @state() private _businessFrauRiskCategories: Array<{category:string;count:number;avgScore:number;color:string}> = [
  ...
  @state() private _businessFrauRiskVelocity: Array<{week:string;newRisks:number;closedRisks:number;netChange:number}> = [
  ...
  @state() private _businessFrauRiskAppetite: {current:number;threshold:number;maxTolerated:number;status:string} = {
    current: 68, threshold: 55, maxTolerated: 80, status: 'Warning'
  };
  @state() private _businessFrauRiskOwnerMatrix: Array<{owner:string;criticalCount:number;highCount:number;mediumCount:number;complianceRate:number}> = [
  ...
  private _renderBusinessfrauRiskDash(): TemplateResult {
    const riskItems = this._businessFrauRiskTopTen;
    const velocity = this._businessFrauRiskVelocity;
    const appetite = this._businessFrauRiskAppetite;
    return html`
  ...
  @state() private _complianceCalendarEvents: Array<{id: string; title: string; deadline: string; category: string; status: string; assignee: string; notes: string}> = [
    {id: 'cc-business-fraud-001', title: 'SOX Section 404 Compliance Review', deadline: '2026-06-30', category: 'Regulatory', status: 'In Progress', assignee: 'Compliance Team', notes: 'Annual internal control assessment for financial reporting'},
    {id: 'cc-business-fraud-002', title: 'GDPR Data Protection Impact Assessment', deadline: '2026-05-15', category: 'Privacy', status: 'Pending', assignee: 'DPO Office', notes: 'Quarterly DPIA for high-risk processing activities'},
    {id: 'cc-business-fraud-003', title: 'ISO 27001 Surveillance Audit', deadline: '2026-07-20', category: 'Certification', status: 'Scheduled', assignee: 'QA Lead', notes: 'Stage 2 surveillance audit for ISMS certification renewal'},
    {id: 'cc-business-fraud-004', title: 'PCI DSS v4.0 Gap Analysis', deadline: '2026-05-30', category: 'Payment', status: 'Not Started', assignee: 'Security Architect', notes: 'Assess current state against PCI DSS v4.0 requirements'},
    {id: 'cc-business-fraud-005', title: 'NIST CSF Self-Assessment', deadline: '2026-08-15', category: 'Framework', status: 'Pending', assignee: 'CISO', notes: 'Biannual self-assessment against NIST Cybersecurity Framework'},
    {id: 'cc-business-fraud-006', title: 'HIPAA Security Rule Audit', deadline: '2026-06-15', category: 'Healthcare', status: 'In Progress', assignee: 'Compliance Analyst', notes: 'Annual audit of administrative, physical, and technical safeguards'},
    {id: 'cc-business-fraud-007', title: 'SOC 2 Type II Report Renewal', deadline: '2026-09-30', category: 'Audit', status: 'Scheduled', assignee: 'External Auditor', notes: 'Engage auditor for SOC 2 Type II examination period'},
    {id: 'cc-business-fraud-008', title: 'FedRAMP Authorization Review', deadline: '2026-07-01', category: 'Government', status: 'Pending', assignee: 'FedRAMP PMO', notes: 'Review and update security authorization package'},
    {id: 'cc-business-fraud-009', title: 'Annual Penetration Test', deadline: '2026-10-15', category: 'Testing', status: 'Not Started', assignee: 'Red Team Lead', notes: 'Comprehensive external and internal penetration testing engagement'},
    {id: 'cc-business-fraud-010', title: 'Security Awareness Training Rollout', deadline: '2026-05-01', category: 'Training', status: 'Completed', assignee: 'Security Awareness Manager', notes: 'Q2 organization-wide security awareness training program'},
    {id: 'cc-business-fraud-011', title: 'Vendor Security Review Cycle', deadline: '2026-06-01', category: 'Third Party', status: 'In Progress', assignee: 'Vendor Manager', notes: 'Quarterly review of critical vendor security posture and SLA compliance'},
    {id: 'cc-business-fraud-012', title: 'Incident Response Plan Update', deadline: '2026-05-20', category: 'Operations', status: 'Pending', assignee: 'IR Team Lead', notes: 'Update IR procedures based on lessons learned from recent incidents'},
  ];
  ...
  @state() private _dataQualityMetrics: Array<{id: string; dataSource: string; completenessScore: number; accuracyScore: number; timelinessScore: number; consistencyScore: number; reliabilityRank: number; lastUpdated: string; issues: string[]; trend: string}> = [
    {id: 'dq-business-fraud-001', dataSource: 'Vulnerability Scanner Feed', completenessScore: 94, accuracyScore: 91, timelinessScore: 88, consistencyScore: 95, reliabilityRank: 1, lastUpdated: '2026-04-23T08:00:00Z', issues: ['Minor delay in Nessus plugin updates'], trend: 'improving'},
    {id: 'dq-business-fraud-002', dataSource: 'SIEM Event Logs', completenessScore: 87, accuracyScore: 93, timelinessScore: 96, consistencyScore: 89, reliabilityRank: 2, lastUpdated: '2026-04-23T07:30:00Z', issues: ['Some log sources showing gaps in off-hours'], trend: 'stable'},
    {id: 'dq-business-fraud-003', dataSource: 'Asset Inventory Database', completenessScore: 82, accuracyScore: 78, timelinessScore: 75, consistencyScore: 84, reliabilityRank: 4, lastUpdated: '2026-04-22T18:00:00Z', issues: ['Shadow IT devices not fully cataloged', 'Decommissioned assets still listed'], trend: 'declining'},
    {id: 'dq-business-fraud-004', dataSource: 'Threat Intelligence Platform', completenessScore: 91, accuracyScore: 89, timelinessScore: 92, consistencyScore: 87, reliabilityRank: 3, lastUpdated: '2026-04-23T06:00:00Z', issues: ['Some IOC sources lack confidence scoring'], trend: 'improving'},
    {id: 'dq-business-fraud-005', dataSource: 'Identity Provider Logs', completenessScore: 96, accuracyScore: 97, timelinessScore: 98, consistencyScore: 96, reliabilityRank: 1, lastUpdated: '2026-04-23T09:00:00Z', issues: [], trend: 'stable'},
    {id: 'dq-business-fraud-006', dataSource: 'Cloud Security Posture Data', completenessScore: 79, accuracyScore: 85, timelinessScore: 82, consistencyScore: 81, reliabilityRank: 5, lastUpdated: '2026-04-23T05:00:00Z', issues: ['Multi-cloud coverage gaps', 'API rate limiting affects scan frequency'], trend: 'declining'},
    {id: 'dq-business-fraud-007', dataSource: 'Compliance Evidence Store', completenessScore: 88, accuracyScore: 86, timelinessScore: 80, consistencyScore: 83, reliabilityRank: 4, lastUpdated: '2026-04-21T14:00:00Z', issues: ['Manual evidence collection delays', 'Inconsistent formatting across frameworks'], trend: 'stable'},
    {id: 'dq-business-fraud-008', dataSource: 'Network Flow Data', completenessScore: 92, accuracyScore: 90, timelinessScore: 94, consistencyScore: 91, reliabilityRank: 2, lastUpdated: '2026-04-23T08:30:00Z', issues: ['Encrypted traffic classification accuracy needs improvement'], trend: 'improving'},
  ];
  ...
  @state() private _workflowTemplates: Array<{id: string; name: string; description: string; category: string; steps: number; avgDuration: string; successRate: number; lastRun: string; manualInterventions: number; optimizationScore: number}> = [
    {id: 'wf-business-fraud-001', name: 'Vulnerability Triage Pipeline', description: 'Automated vulnerability assessment, prioritization, and assignment workflow', category: 'Vulnerability Management', steps: 8, avgDuration: '12 min', successRate: 94.5, lastRun: '2026-04-23T06:00:00Z', manualInterventions: 2, optimizationScore: 87},
    {id: 'wf-business-fraud-002', name: 'Incident Response Orchestration', description: 'End-to-end IR workflow from detection to containment and recovery', category: 'Incident Response', steps: 15, avgDuration: '45 min', successRate: 89.2, lastRun: '2026-04-22T14:30:00Z', manualInterventions: 5, optimizationScore: 72},
    {id: 'wf-business-fraud-003', name: 'Access Request Approval Chain', description: 'Multi-level approval workflow for privileged access requests', category: 'Identity & Access', steps: 6, avgDuration: '4 hours', successRate: 97.1, lastRun: '2026-04-23T09:15:00Z', manualInterventions: 1, optimizationScore: 92},
    {id: 'wf-business-fraud-004', name: 'Compliance Evidence Collection', description: 'Automated gathering and packaging of audit evidence artifacts', category: 'Compliance', steps: 10, avgDuration: '30 min', successRate: 91.8, lastRun: '2026-04-21T10:00:00Z', manualInterventions: 3, optimizationScore: 78},
    {id: 'wf-business-fraud-005', name: 'Security Patch Deployment', description: 'Staged patch rollout with validation and rollback capability', category: 'Patch Management', steps: 12, avgDuration: '2 hours', successRate: 96.3, lastRun: '2026-04-20T22:00:00Z', manualInterventions: 1, optimizationScore: 85},
    {id: 'wf-business-fraud-006', name: 'Threat Intelligence Enrichment', description: 'IOC enrichment and correlation with internal threat data', category: 'Threat Intelligence', steps: 7, avgDuration: '8 min', successRate: 93.7, lastRun: '2026-04-23T07:00:00Z', manualInterventions: 0, optimizationScore: 95},
    {id: 'wf-business-fraud-007', name: 'Security Reporting Pipeline', description: 'Automated generation and distribution of security metrics reports', category: 'Reporting', steps: 5, avgDuration: '15 min', successRate: 98.9, lastRun: '2026-04-23T08:00:00Z', manualInterventions: 0, optimizationScore: 97},
    {id: 'wf-business-fraud-008', name: 'Vendor Risk Assessment', description: 'Automated vendor security questionnaire distribution and scoring', category: 'Third Party', steps: 9, avgDuration: '3 days', successRate: 85.4, lastRun: '2026-04-19T11:00:00Z', manualInterventions: 4, optimizationScore: 65},
  ];
  ...
  @state() private _workflowExecutionHistory: Array<{workflowId: string; runDate: string; duration: string; status: string; trigger: string}> = [
    {workflowId: 'wf-business-fraud-001', runDate: '2026-04-23T06:00:00Z', duration: '11 min 23 sec', status: 'Success', trigger: 'Scheduled'},
    {workflowId: 'wf-business-fraud-002', runDate: '2026-04-22T14:30:00Z', duration: '42 min 15 sec', status: 'Success', trigger: 'Alert'},
    {workflowId: 'wf-business-fraud-003', runDate: '2026-04-23T09:15:00Z', duration: '3h 45 min', status: 'Success', trigger: 'User Request'},
    {workflowId: 'wf-business-fraud-001', runDate: '2026-04-22T06:00:00Z', duration: '13 min 08 sec', status: 'Partial', trigger: 'Scheduled'},
    {workflowId: 'wf-business-fraud-004', runDate: '2026-04-21T10:00:00Z', duration: '28 min 42 sec', status: 'Success', trigger: 'Scheduled'},
  ];
  ...
  @state() private _knowledgeArticles: Array<{id: string; title: string; category: string; author: string; publishDate: string; views: number; rating: number; tags: string[]; summary: string; readTime: string}> = [
    {id: 'kb-business-fraud-001', title: 'Zero Trust Architecture Implementation Guide', category: 'Architecture', author: 'Security Architecture Team', publishDate: '2026-03-15', views: 1842, rating: 4.8, tags: ['zero-trust', 'network', 'identity'], summary: 'Comprehensive guide for implementing zero trust principles across enterprise infrastructure', readTime: '15 min'},
    {id: 'kb-business-fraud-002', title: 'Ransomware Defense Playbook 2026', category: 'Threats', author: 'Threat Intelligence Team', publishDate: '2026-04-01', views: 2341, rating: 4.9, tags: ['ransomware', 'incident-response', 'defense'], summary: 'Updated playbook with latest ransomware tactics, techniques, and defensive measures', readTime: '12 min'},
    {id: 'kb-business-fraud-003', title: 'Cloud Security Best Practices for Multi-Cloud', category: 'Cloud', author: 'Cloud Security Team', publishDate: '2026-02-28', views: 1567, rating: 4.5, tags: ['cloud', 'aws', 'azure', 'gcp'], summary: 'Best practices for securing workloads across multiple cloud service providers', readTime: '18 min'},
    {id: 'kb-business-fraud-004', title: 'API Security Testing Methodology', category: 'Testing', author: 'Red Team', publishDate: '2026-03-22', views: 987, rating: 4.6, tags: ['api', 'testing', 'owasp'], summary: 'Structured methodology for identifying and testing API security vulnerabilities', readTime: '10 min'},
    {id: 'kb-business-fraud-005', title: 'Supply Chain Security Risk Management', category: 'Third Party', author: 'GRC Team', publishDate: '2026-01-20', views: 1234, rating: 4.3, tags: ['supply-chain', 'vendor', 'risk'], summary: 'Framework for managing security risks in the software supply chain', readTime: '14 min'},
    {id: 'kb-business-fraud-006', title: 'Kubernetes Security Hardening Checklist', category: 'Container', author: 'Platform Security', publishDate: '2026-04-10', views: 2103, rating: 4.7, tags: ['kubernetes', 'container', 'hardening'], summary: 'Step-by-step hardening checklist for production Kubernetes clusters', readTime: '11 min'},
    {id: 'kb-business-fraud-007', title: 'Security Metrics That Matter for Executives', category: 'Metrics', author: 'CISO Office', publishDate: '2026-03-05', views: 876, rating: 4.4, tags: ['metrics', 'kpi', 'reporting'], summary: 'Key security metrics and KPIs that resonate with board-level stakeholders', readTime: '8 min'},
    {id: 'kb-business-fraud-008', title: 'Phishing Simulation Campaign Design', category: 'Awareness', author: 'Security Awareness Team', publishDate: '2026-02-15', views: 1456, rating: 4.5, tags: ['phishing', 'simulation', 'training'], summary: 'Designing effective phishing simulations that drive real behavioral change', readTime: '9 min'},
    {id: 'kb-business-fraud-009', title: 'Data Loss Prevention Strategy and Implementation', category: 'Data Protection', author: 'Data Security Team', publishDate: '2026-03-28', views: 1678, rating: 4.6, tags: ['dlp', 'data-protection', 'compliance'], summary: 'Strategic approach to DLP covering people, process, and technology layers', readTime: '16 min'},
    {id: 'kb-business-fraud-010', title: 'Security Automation with SOAR Platforms', category: 'Automation', author: 'SOC Engineering', publishDate: '2026-04-05', views: 1098, rating: 4.7, tags: ['soar', 'automation', 'soc'], summary: 'Building effective security automation playbooks using SOAR technology', readTime: '13 min'},
    {id: 'kb-business-fraud-011', title: 'Third-Party Risk Assessment Framework', category: 'GRC', author: 'Vendor Management', publishDate: '2026-01-30', views: 1345, rating: 4.4, tags: ['vendor', 'risk', 'assessment'], summary: 'Structured framework for evaluating and monitoring third-party security risks', readTime: '12 min'},
    {id: 'kb-business-fraud-012', title: 'Network Detection and Response Best Practices', category: 'Network', author: 'Network Security Team', publishDate: '2026-03-18', views: 1567, rating: 4.5, tags: ['ndr', 'network', 'detection'], summary: 'Implementing effective network detection and response capabilities', readTime: '14 min'},
    {id: 'kb-business-fraud-013', title: 'Security Chaos Engineering Guide', category: 'Resilience', author: 'SRE Security', publishDate: '2026-04-12', views: 789, rating: 4.3, tags: ['chaos-engineering', 'resilience', 'testing'], summary: 'Applying chaos engineering principles to validate security controls', readTime: '10 min'},
    {id: 'kb-business-fraud-014', title: 'Incident Communication Templates', category: 'Incident Response', author: 'IR Team', publishDate: '2026-02-22', views: 2234, rating: 4.8, tags: ['incident', 'communication', 'templates'], summary: 'Ready-to-use communication templates for security incident scenarios', readTime: '7 min'},
    {id: 'kb-business-fraud-015', title: 'Endpoint Detection and Response Configuration', category: 'Endpoint', author: 'Endpoint Security', publishDate: '2026-03-08', views: 1345, rating: 4.5, tags: ['edr', 'endpoint', 'configuration'], summary: 'Optimal EDR configuration for maximum detection with minimal false positives', readTime: '11 min'},
    {id: 'kb-business-fraud-016', title: 'Security Code Review Standards', category: 'Development', author: 'AppSec Team', publishDate: '2026-02-10', views: 1890, rating: 4.6, tags: ['code-review', 'secure-sdlc', 'standards'], summary: 'Standards and checklists for security-focused code reviews', readTime: '13 min'},
    {id: 'kb-business-fraud-017', title: 'IoT Security Assessment Methodology', category: 'IoT', author: 'IoT Security Lab', publishDate: '2026-04-08', views: 654, rating: 4.2, tags: ['iot', 'embedded', 'assessment'], summary: 'Methodology for assessing security posture of IoT devices and ecosystems', readTime: '15 min'},
    {id: 'kb-business-fraud-018', title: 'Passwordless Authentication Migration Guide', category: 'Identity', author: 'IAM Team', publishDate: '2026-03-25', views: 1678, rating: 4.7, tags: ['passwordless', 'authentication', 'mfa'], summary: 'Step-by-step migration plan from password-based to passwordless authentication', readTime: '12 min'},
    {id: 'kb-business-fraud-019', title: 'Security Awareness Program Metrics', category: 'Awareness', author: 'Security Culture Team', publishDate: '2026-01-25', views: 1123, rating: 4.4, tags: ['awareness', 'metrics', 'culture'], summary: 'Measuring the effectiveness of your security awareness and training programs', readTime: '9 min'},
    {id: 'kb-business-fraud-020', title: 'Deception Technology Deployment Guide', category: 'Detection', author: 'Blue Team', publishDate: '2026-04-15', views: 876, rating: 4.5, tags: ['deception', 'honeypot', 'detection'], summary: 'Planning and deploying deception technology for advanced threat detection', readTime: '11 min'},
  ];
  ...
  // --- Security Scenario Planning Methods (BUFR) ---
  private _initBufrScenarios(): void {
    const worstCaseScenarios = [
      { id: "bufr-sc-01", name: "Ransomware Attack on Critical Infrastructure", category: "Malware", impact: 9500000, probability: 0.12, recoveryDays: 21, affectedSystems: 47, dataLossGb: 320, businessImpact: "critical" },
      { id: "bufr-sc-02", name: "Insider Data Exfiltration via Authorized Channels", category: "Insider Threat", impact: 7200000, probability: 0.18, recoveryDays: 14, affectedSystems: 12, dataLossGb: 85, businessImpact: "high" },
      { id: "bufr-sc-03", name: "Supply Chain Compromise via Third-Party Library", category: "Supply Chain", impact: 6800000, probability: 0.09, recoveryDays: 30, affectedSystems: 63, dataLossGb: 0, businessImpact: "critical" },
      { id: "bufr-sc-04", name: "Cloud Misconfiguration Leading to Public Exposure", category: "Cloud", impact: 4100000, probability: 0.22, recoveryDays: 7, affectedSystems: 8, dataLossGb: 450, businessImpact: "high" },
      { id: "bufr-sc-05", name: "Zero-Day Exploit in Core Application Framework", category: "Vulnerability", impact: 8900000, probability: 0.05, recoveryDays: 45, affectedSystems: 120, dataLossGb: 0, businessImpact: "critical" },
      { id: "bufr-sc-06", name: "Distributed Denial of Service on Customer-Facing Services", category: "Availability", impact: 3400000, probability: 0.25, recoveryDays: 3, affectedSystems: 15, dataLossGb: 0, businessImpact: "medium" },
      { id: "bufr-sc-07", name: "Credential Stuffing and Account Takeover Wave", category: "Identity", impact: 2800000, probability: 0.30, recoveryDays: 5, affectedSystems: 6, dataLossGb: 12, businessImpact: "medium" },
      { id: "bufr-sc-08", name: "Physical Security Breach at Data Center Facility", category: "Physical", impact: 12000000, probability: 0.02, recoveryDays: 60, affectedSystems: 200, dataLossGb: 5000, businessImpact: "critical" },
    ];
    this._bufrScenarios = worstCaseScenarios.map(s => ({ ...s, mitigationCost: Math.round(s.impact * 0.08), drillScheduled: false, lastDrillDate: null, drillScore: null, controlGaps: Math.floor(Math.random() * 5) + 1 }));
  }

  private _calculateBufrScenarioRisk(): void {
    this._bufrScenarios.forEach(s => {
  ...
  private _scheduleBufrDrills(): void {
  ...
  private _bufrScenarioComparison(): Record<string, unknown>[] {
    return this._bufrScenarios.map(s => ({
  ...
  // --- Security Control Effectiveness Analytics (BUFR) ---
  private _initBufrControls(): void {
    const controls = [
      { id: "bufr-ct-01", name: "Network Segmentation", type: "preventive", maturity: "defined", score: 82, target: 90, failures: 3, lastTest: "2026-03-15" },
      { id: "bufr-ct-02", name: "Endpoint Detection and Response", type: "detective", maturity: "managed", score: 88, target: 92, failures: 1, lastTest: "2026-04-01" },
      { id: "bufr-ct-03", name: "Data Loss Prevention", type: "preventive", maturity: "repeatable", score: 71, target: 85, failures: 7, lastTest: "2026-03-22" },
      { id: "bufr-ct-04", name: "Security Information and Event Management", type: "detective", maturity: "managed", score: 85, target: 90, failures: 2, lastTest: "2026-04-05" },
      { id: "bufr-ct-05", name: "Identity and Access Management", type: "preventive", maturity: "defined", score: 76, target: 88, failures: 5, lastTest: "2026-03-28" },
      { id: "bufr-ct-06", name: "Vulnerability Management", type: "corrective", maturity: "managed", score: 80, target: 90, failures: 4, lastTest: "2026-04-02" },
      { id: "bufr-ct-07", name: "Email Security Gateway", type: "preventive", maturity: "managed", score: 90, target: 95, failures: 1, lastTest: "2026-04-08" },
      { id: "bufr-ct-08", name: "Web Application Firewall", type: "preventive", maturity: "managed", score: 87, target: 92, failures: 2, lastTest: "2026-03-30" },
      { id: "bufr-ct-09", name: "Patch Management", type: "corrective", maturity: "defined", score: 68, target: 85, failures: 9, lastTest: "2026-03-18" },
      { id: "bufr-ct-10", name: "Encryption at Rest", type: "preventive", maturity: "managed", score: 93, target: 95, failures: 0, lastTest: "2026-04-10" },
      { id: "bufr-ct-11", name: "Encryption in Transit", type: "preventive", maturity: "optimized", score: 96, target: 98, failures: 0, lastTest: "2026-04-12" },
      { id: "bufr-ct-12", name: "Privileged Access Management", type: "preventive", maturity: "defined", score: 74, target: 88, failures: 6, lastTest: "2026-03-25" },
      { id: "bufr-ct-13", name: "Security Awareness Training", type: "preventive", maturity: "repeatable", score: 65, target: 80, failures: 11, lastTest: "2026-03-20" },
      { id: "bufr-ct-14", name: "Incident Response Plan", type: "corrective", maturity: "managed", score: 78, target: 88, failures: 4, lastTest: "2026-04-03" },
      { id: "bufr-ct-15", name: "Backup and Recovery", type: "corrective", maturity: "managed", score: 84, target: 92, failures: 3, lastTest: "2026-04-06" },
      { id: "bufr-ct-16", name: "Multi-Factor Authentication", type: "preventive", maturity: "managed", score: 91, target: 95, failures: 1, lastTest: "2026-04-09" },
      { id: "bufr-ct-17", name: "Network Traffic Analysis", type: "detective", maturity: "defined", score: 72, target: 85, failures: 5, lastTest: "2026-03-27" },
      { id: "bufr-ct-18", name: "Cloud Security Posture Management", type: "detective", maturity: "repeatable", score: 69, target: 82, failures: 8, lastTest: "2026-03-24" },
      { id: "bufr-ct-19", name: "Container Security Scanning", type: "preventive", maturity: "defined", score: 75, target: 85, failures: 5, lastTest: "2026-04-04" },
      { id: "bufr-ct-20", name: "API Security Gateway", type: "preventive", maturity: "repeatable", score: 70, target: 82, failures: 7, lastTest: "2026-03-29" },
      { id: "bufr-ct-21", name: "Threat Intelligence Platform", type: "detective", maturity: "managed", score: 83, target: 90, failures: 2, lastTest: "2026-04-07" },
      { id: "bufr-ct-22", name: "Database Activity Monitoring", type: "detective", maturity: "defined", score: 67, target: 80, failures: 8, lastTest: "2026-03-21" },
      { id: "bufr-ct-23", name: "Deception Technology", type: "detective", maturity: "initial", score: 55, target: 75, failures: 12, lastTest: "2026-03-16" },
      { id: "bufr-ct-24", name: "Security Orchestration Automation", type: "corrective", maturity: "repeatable", score: 73, target: 85, failures: 6, lastTest: "2026-04-01" },
      { id: "bufr-ct-25", name: "Third-Party Risk Assessment", type: "preventive", maturity: "defined", score: 64, target: 78, failures: 9, lastTest: "2026-03-19" },
    ];
    this._bufrControls = controls;
  }

  private _analyzeBufrControlEffectiveness(): void {
    const typeBreakdown: Record<string, { total: number; avgScore: number; avgFailures: number }> = {};
    this._bufrControls.forEach(c => {
    ...
    this._bufrControlTypeBreakdown = typeBreakdown;
  }

  private _bufrControlOptimization(): Record<string, unknown>[] {
    return this._bufrControls.filter(c => c.score < c.target).map(c => ({
  ...
  // --- Security Data Pipeline Health (BUFR) ---
  private _initBufrPipelines(): void {
    const pipelines = [
      { id: "bufr-pl-01", name: "SIEM Log Ingestion", status: "healthy", healthScore: 94, latencyMs: 120, freshnessMin: 2, errorRate: 0.02, throughputMbps: 450, recordsPerSec: 12000, backPressure: 0.05, uptime: 99.97 },
      { id: "bufr-pl-02", name: "Threat Feed Aggregation", status: "healthy", healthScore: 91, latencyMs: 300, freshnessMin: 15, errorRate: 0.05, throughputMbps: 85, recordsPerSec: 3200, backPressure: 0.08, uptime: 99.92 },
      { id: "bufr-pl-03", name: "Vulnerability Scan Results", status: "degraded", healthScore: 72, latencyMs: 2500, freshnessMin: 60, errorRate: 0.15, throughputMbps: 25, recordsPerSec: 800, backPressure: 0.35, uptime: 98.50 },
      { id: "bufr-pl-04", name: "Endpoint Telemetry Stream", status: "healthy", healthScore: 88, latencyMs: 450, freshnessMin: 5, errorRate: 0.08, throughputMbps: 280, recordsPerSec: 8500, backPressure: 0.12, uptime: 99.85 },
      { id: "bufr-pl-05", name: "Cloud Audit Log Pipeline", status: "healthy", healthScore: 86, latencyMs: 600, freshnessMin: 10, errorRate: 0.06, throughputMbps: 150, recordsPerSec: 4500, backPressure: 0.10, uptime: 99.80 },
      { id: "bufr-pl-06", name: "Identity Event Stream", status: "warning", healthScore: 78, latencyMs: 1200, freshnessMin: 20, errorRate: 0.12, throughputMbps: 45, recordsPerSec: 1500, backPressure: 0.22, uptime: 99.40 },
      { id: "bufr-pl-07", name: "Network Flow Collection", status: "healthy", healthScore: 82, latencyMs: 800, freshnessMin: 8, errorRate: 0.09, throughputMbps: 650, recordsPerSec: 18000, backPressure: 0.15, uptime: 99.70 },
      { id: "bufr-pl-08", name: "DLP Incident Pipeline", status: "degraded", healthScore: 68, latencyMs: 3500, freshnessMin: 45, errorRate: 0.20, throughputMbps: 18, recordsPerSec: 500, backPressure: 0.42, uptime: 97.80 },
    ];
    this._bufrPipelines = pipelines.map(p => ({ ...p, dependencies: [], slaBreached: p.healthScore < 75, alertThreshold: p.errorRate > 0.15 }));
    this._bufrPipelines[0].dependencies = ["bufr-pl-04", "bufr-pl-07"];
    this._bufrPipelines[4].dependencies = ["bufr-pl-01"];
    this._bufrPipelines[5].dependencies = ["bufr-pl-02"];
    this._bufrPipelines[7].dependencies = ["bufr-pl-04", "bufr-pl-05"];
  }

  private _bufrPipelineTrend(): Record<string, unknown>[] {
    const hours = Array.from({ length: 24 }, (_, i) => `H${String(i).padStart(2, "0")}`);
    return this._bufrPipelines.map(p => ({
  ...
  // --- Security Stakeholder Report Generator (BUFR) ---
  private _initBufrReportTemplates(): void {
    this._bufrReportTemplates = [
      { id: "bufr-rp-01", name: "Board Security Report", audience: "board", frequency: "quarterly", sections: 6, autoGenerate: true, lastGenerated: "2026-03-31", pages: 12 },
      { id: "bufr-rp-02", name: "Executive Risk Summary", audience: "executive", frequency: "monthly", sections: 8, autoGenerate: true, lastGenerated: "2026-04-01", pages: 8 },
      { id: "bufr-rp-03", name: "Technical Security Review", audience: "technical", frequency: "weekly", sections: 12, autoGenerate: true, lastGenerated: "2026-04-21", pages: 25 },
      { id: "bufr-rp-04", name: "Audit Compliance Report", audience: "audit", frequency: "quarterly", sections: 10, autoGenerate: false, lastGenerated: "2026-03-15", pages: 35 },
      { id: "bufr-rp-05", name: "Regulatory Filing Package", audience: "regulatory", frequency: "annual", sections: 15, autoGenerate: false, lastGenerated: "2025-12-31", pages: 48 },
    ];
  }

  private _generateBufrExecSummary(): string {
    const totalRisk = this._bufrScenarios.reduce((sum: number, s: any) => sum + s.riskScore, 0);
    const avgControl = Math.round(this._bufrControls.reduce((sum: number, c: any) => sum + c.score, 0) / this._bufrControls.length);
    const degradedPipes = this._bufrPipelines.filter((p: any) => p.status !== "healthy").length;
    return `Overall risk exposure: ${(totalRisk / 1000000).toFixed(1)}M. Average control effectiveness: {avgControl}%. {degradedPipes} pipeline(s) need attention. {this._bufrScenarios.filter((s: any) => s.mitigationPriority === "immediate").length} scenarios require immediate action.`;
  }

  // --- Security Technology Radar (BUFR) ---
  private _initBufrTechRadar(): void {
    this._bufrTechRadar = [
      { id: "bufr-tr-01", name: "AI-Powered Threat Detection", ring: "adopt", category: "Detection", maturity: 4, trend: "up", investment: "high", roi: 3.2, vendorCount: 8 },
      { id: "bufr-tr-02", name: "Zero Trust Architecture", ring: "adopt", category: "Architecture", maturity: 4, trend: "up", investment: "high", roi: 2.8, vendorCount: 12 },
      { id: "bufr-tr-03", name: "SASE/SSE Platform", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.4, vendorCount: 10 },
      { id: "bufr-tr-04", name: "CNAPP Cloud Security", ring: "trial", category: "Cloud", maturity: 3, trend: "up", investment: "medium", roi: 2.1, vendorCount: 7 },
      { id: "bufr-tr-05", name: "Extended Detection and Response", ring: "adopt", category: "Detection", maturity: 4, trend: "stable", investment: "high", roi: 2.9, vendorCount: 9 },
      { id: "bufr-tr-06", name: "Quantum-Safe Cryptography", ring: "assess", category: "Crypto", maturity: 2, trend: "up", investment: "low", roi: 0.8, vendorCount: 4 },
      { id: "bufr-tr-07", name: "Security Service Edge", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.0, vendorCount: 6 },
      { id: "bufr-tr-08", name: "SOAR 2.0 Autonomous SOC", ring: "assess", category: "Operations", maturity: 2, trend: "up", investment: "low", roi: 1.2, vendorCount: 5 },
      { id: "bufr-tr-09", name: "Software Supply Chain Security", ring: "trial", category: "DevSecOps", maturity: 3, trend: "up", investment: "medium", roi: 2.3, vendorCount: 8 },
      { id: "bufr-tr-10", name: "Identity Threat Detection", ring: "adopt", category: "Identity", maturity: 3, trend: "up", investment: "high", roi: 2.7, vendorCount: 7 },
      { id: "bufr-tr-11", name: "Data Security Posture Management", ring: "trial", category: "Data", maturity: 3, trend: "up", investment: "medium", roi: 2.2, vendorCount: 9 },
      { id: "bufr-tr-12", name: "Decentralized Identity Standards", ring: "hold", category: "Identity", maturity: 1, trend: "stable", investment: "low", roi: 0.5, vendorCount: 3 },
    ];
  }

  private _bufrRadarSummary(): Record<string, unknown> {
    const rings: Record<string, number> = { adopt: 0, trial: 0, assess: 0, hold: 0 };
    this._bufrTechRadar.forEach(t => { if (rings[t.ring as keyof typeof rings] !== undefined) rings[t.ring as keyof typeof rings]++; });
    return { adopt: rings.adopt, trial: rings.trial, assess: rings.assess, hold: rings.hold, total: this._bufrTechRadar.length, avgMaturity: 2.8, topInvestment: this._bufrTechRadar.filter(t => t.investment === "high").map(t => t.name) };
  }

  // --- BUFR Security Compliance Mapping Matrix ---
  private _initBufrComplianceMatrix(): void {
    ...
    this._bufrComplianceMatrix = frameworks.map(fw => ({
  ...
  private _bufrComplianceTrend(): Record<string, unknown>[] {
    const quarters = ["Q1-2025", "Q2-2025", "Q3-2025", "Q4-2025", "Q1-2026", "Q2-2026"];
    return this._bufrComplianceMatrix.map(fw => ({
  ...
  // --- BUFR Threat Intelligence Correlation Engine ---
  private _initBufrThreatIntel(): void {
    ...
    this._bufrThreatIntel = threatActors.map((actor, i) => ({
  ...
  private _bufrThreatCorrelation(): Record<string, unknown>[] {
    return this._bufrThreatIntel.filter(t => t.riskScore > 70).map(t => ({
  ...
  // --- BUFR Security Metrics Deep Dive ---
  private _initBufrMetricsDeep(): void {
    ...
    this._bufrMetricsDeep = metricCategories.map(cat => ({
  ...
  private _bufrMetricsHeatmap(): number[][] {
    return this._bufrMetricsDeep.map(cat =>
      cat.metrics.map(m => Math.round((m.value / m.target) * 100))
    );
  }

  // --- BUFR Incident Cost Modeling ---
  private _initBufrCostModel(): void {
    const incidentTypes = ["Data Breach", "Ransomware", "DDoS", "Insider Threat", "Phishing", "Supply Chain", "Cloud Misconfig", "Zero-Day"];
    this._bufrCostModel = incidentTypes.map(inc => ({
    ...
    this._bufrCostModel.forEach(m => { m.totalAnnualExposure = m.avgCost * m.frequency; });
  }

  private _bufrCostProjection(): Record<string, unknown>[] {
    return this._bufrCostModel.map(m => ({
  ...
  // --- BUFR Security Architecture Pattern Library ---
  private _initBufrArchPatterns(): void {
    ...
    this._bufrArchPatterns = patterns.map((p, i) => ({
  ...
  private _bufrArchPatternAnalysis(): Record<string, unknown> {
    const implemented = this._bufrArchPatterns.filter(p => p.implementation > 75).length;
    const avgCoverage = Math.round(this._bufrArchPatterns.reduce((s: number, p: any) => s + p.coverage, 0) / this._bufrArchPatterns.length);
    return {
      totalPatterns: this._bufrArchPatterns.length, fullyImplemented: implemented, avgCoverage,
      avgRiskReduction: Math.round(this._bufrArchPatterns.reduce((s: number, p: any) => s + p.riskReduction, 0) / this._bufrArchPatterns.length),
      gaps: this._bufrArchPatterns.filter(p => p.implementation < 60).map(p => p.pattern),
      recommendations: this._bufrArchPatterns.filter(p => p.implementation < 70).map(p => ({ pattern: p.pattern, current: p.implementation, target: 85, effort: p.costComplexity }))
    };
  }
  ...
  private _32bRiskAssessmentWorkflow = [
  ...
  private _32bTreatmentOptions = [
    {id: '32b-T-001', riskId: 'RSK-042', risk: 'Ransomware Attack on Critical Infrastructure', before: 20, after: 6, strategy: 'Mitigate', controls: ['Next-gen EDR with behavioral detection and automated response', 'Zero-trust network micro-segmentation isolating critical assets', 'Immutable offline backup rotation with quarterly recovery testing', 'Incident response retainer with 1-hour response SLA'], cost: '$450K/yr', roi: '78% reduction vs $2.8M potential loss', timeline: 'Q3 2026', status: 'approved' as const},
  ...
  private _32bTalentPositions = [
    ...
    {id: '32b-H-002', title: 'Cloud Security Engineer', dept: 'Cloud Security', level: 'Mid-Senior', loc: 'SF/Remote', salary: '$155K-$180K', posted: '2026-03-20', applicants: 45, pipeline: 12, interviews: 6, offers: 2, urgency: 'high' as const, manager: 'Mike Johnson', skills: ['AWS/Azure/GCP', 'Terraform', 'Kubernetes', 'IAM', 'CSPM'], nice: ['CCSP', 'CKS', 'Serverless']},
    {id: '32b-H-003', title: 'SOC Analyst L2', dept: 'Security Operations', level: 'Mid', loc: 'Austin TX', salary: '$105K-$130K', posted: '2026-04-01', applicants: 67, pipeline: 18, interviews: 8, offers: 0, urgency: 'medium' as const, manager: 'Lisa Park', skills: ['SIEM', 'Incident triage', 'Malware analysis', 'TCP/IP'], nice: ['GCIA', 'SOAR', 'Kill chain']},
    {id: '32b-H-004', title: 'Staff Security Architect', dept: 'Architecture', level: 'Staff', loc: 'Remote US', salary: '$200K-$240K', posted: '2026-02-28', applicants: 12, pipeline: 3, interviews: 2, offers: 0, urgency: 'critical' as const, manager: 'David Lee', skills: ['Enterprise arch', 'Zero trust', 'Risk frameworks', 'Board comms'], nice: ['CISSP+SABSA', 'Team building', 'Speaker']},
    {id: '32b-H-005', title: 'GRC Analyst', dept: 'GRC', level: 'Mid', loc: 'NY/Remote', salary: '$110K-$135K', posted: '2026-04-05', applicants: 34, pipeline: 9, interviews: 3, offers: 1, urgency: 'medium' as const, manager: 'Rachel Green', skills: ['ISO 27001', 'SOC 2', 'Risk assessment', 'Audit'], nice: ['CRISC', 'CISA', 'GDPR']},
    {id: '32b-H-006', title: 'Threat Intel Analyst', dept: 'Threat Intel', level: 'Mid-Senior', loc: 'Remote US', salary: '$140K-$170K', posted: '2026-03-25', applicants: 19, pipeline: 5, interviews: 2, offers: 0, urgency: 'low' as const, manager: 'James Wilson', skills: ['MITRE ATT&CK', 'OSINT', 'Threat modeling', 'Reverse eng'], nice: ['CTIA', 'Nation-state', 'Dark web']},
    {id: '32b-H-007', title: 'AppSec Engineer', dept: 'AppSec', level: 'Senior', loc: 'Seattle/Remote', salary: '$160K-$190K', posted: '2026-04-10', applicants: 28, pipeline: 7, interviews: 3, offers: 0, urgency: 'medium' as const, manager: 'Emily Zhang', skills: ['SAST/DAST/SCA', 'Secure SDLC', 'Code review', 'CI/CD'], nice: ['CSSLP', 'Mobile security']},
    {id: '32b-H-008', title: 'IAM Engineer', dept: 'IAM', level: 'Mid', loc: 'Remote US', salary: '$135K-$160K', posted: '2026-04-08', applicants: 31, pipeline: 10, interviews: 5, offers: 1, urgency: 'medium' as const, manager: 'Chris Martinez', skills: ['Okta/Azure AD', 'SAML/OIDC', 'PAM', 'RBAC'], nice: ['CISSP', 'Zero trust IAM']}
  ];

  private _32bInterviewScorecard = [
  ...
  private _32bOnboardingSteps = [
    {id: '32b-OB-01', item: 'Provision security-hardened laptop, monitors, peripherals, and configured workstation image with all required security tools', cat: 'IT Setup', day: 'Day 1', owner: 'IT Operations'},
    {id: '32b-OB-02', item: 'Create AD account, email, Slack, VPN, SSO enrollment, and all required application access accounts', cat: 'Access', day: 'Day 1', owner: 'IAM Team'},
    {id: '32b-OB-03', item: 'Grant security tooling access: SIEM console, EDR dashboard, vuln scanner, pentest platforms, SOAR', cat: 'Tools', day: 'Day 1-2', owner: 'SOC Manager'},
    {id: '32b-OB-04', item: 'Complete MFA enrollment, security awareness onboarding course, and acceptable use policy acknowledgment', cat: 'Security', day: 'Day 1', owner: 'Awareness Team'},
    {id: '32b-OB-05', item: 'Sign NDA, employment agreement, IP assignment, and code of conduct documents with HR', cat: 'HR', day: 'Day 1', owner: 'Human Resources'},
    {id: '32b-OB-06', item: 'Introduction meetings with direct team members and key cross-functional stakeholders', cat: 'Integration', day: 'Week 1', owner: 'Hiring Manager'},
    {id: '32b-OB-07', item: 'Security architecture overview, tooling walkthrough, and operational runbook review sessions', cat: 'Training', day: 'Week 1-2', owner: 'Security Architect'},
    {id: '32b-OB-08', item: 'Shadow experienced team member on active incident response and threat hunting investigations', cat: 'Mentoring', day: 'Week 2-3', owner: 'SOC Lead'},
    {id: '32b-OB-09', item: 'Set up individual certification study plan aligned with role requirements and career goals', cat: 'Development', day: 'Week 2', owner: 'Hiring Manager'},
    {id: '32b-OB-10', item: 'Establish 30-60-90 day goals, define success metrics, and schedule first performance checkpoint', cat: 'Goals', day: 'Week 1', owner: 'Hiring Manager'}
  ];

  private _32bMetricsDashboard = [
    {id: '32b-MTD-001', name: 'Mean Time to Detect (MTTD)', value: '4.2 hours', trend: 'improving' as const, target: '< 4 hours', source: 'SIEM - Splunk', method: 'Median time from first alert to analyst acknowledgment', owner: 'SOC Manager', sla: '4 hours', prev: '5.1 hours', spark: [5.8, 5.4, 5.1, 4.9, 4.7, 4.5, 4.3, 4.2], breakdown: [{src: 'Network IDS', pct: '35%', avg: '3.8h'}, {src: 'Endpoint EDR', pct: '28%', avg: '4.1h'}, {src: 'Email Gateway', pct: '22%', avg: '3.5h'}, {src: 'Cloud Security', pct: '10%', avg: '5.2h'}, {src: 'User Reports', pct: '5%', avg: '6.8h'}]},
    {id: '32b-MTD-002', name: 'Mean Time to Respond (MTTR)', value: '2.8 hours', trend: 'stable' as const, target: '< 3 hours', source: 'SOAR - XSOAR', method: 'Median time from acknowledgment to first containment action', owner: 'IR Lead', sla: '3 hours', prev: '2.7 hours', spark: [3.2, 3.0, 2.9, 2.8, 2.8, 2.9, 2.8, 2.8], breakdown: [{src: 'Auto Playbooks', pct: '42%', avg: '1.2h'}, {src: 'Semi-Auto', pct: '33%', avg: '2.5h'}, {src: 'Manual', pct: '20%', avg: '5.1h'}, {src: 'L3 Escalation', pct: '5%', avg: '8.3h'}]},
    {id: '32b-MTD-003', name: 'Patch Compliance Rate', value: '97.6%', trend: 'improving' as const, target: '> 95%', source: 'Tenable.io', method: 'Pct of critical/high vulns patched within SLA windows', owner: 'Vuln Manager', sla: '95%', prev: '96.1%', spark: [93.2, 94.1, 94.8, 95.3, 95.9, 96.4, 97.0, 97.6], breakdown: [{src: 'Critical (7d)', pct: '40%', rate: '99.2%'}, {src: 'High (30d)', pct: '35%', rate: '97.8%'}, {src: 'Medium (90d)', pct: '20%', rate: '95.1%'}, {src: 'Low (180d)', pct: '5%', rate: '89.4%'}]},
    {id: '32b-MTD-004', name: 'Phishing Click Rate', value: '3.2%', trend: 'improving' as const, target: '< 5%', source: 'KnowBe4 + Proofpoint', method: 'Pct of users clicking simulated phishing links monthly', owner: 'Awareness Lead', sla: '5%', prev: '4.1%', spark: [6.8, 6.2, 5.5, 5.1, 4.6, 4.1, 3.6, 3.2], breakdown: [{src: 'Spear Phishing', pct: '35%', rate: '5.1%'}, {src: 'Generic Phishing', pct: '30%', rate: '2.8%'}, {src: 'BEC Sim', pct: '20%', rate: '3.5%'}, {src: 'SMS Phishing', pct: '15%', rate: '1.9%'}]},
    {id: '32b-MTD-005', name: 'Security Posture Score', value: '784', trend: 'improving' as const, target: '> 750', source: 'SecurityScorecard', method: 'Composite: external 30%, compliance 25%, vuln 25%, config 20%', owner: 'CISO', sla: '> 750', prev: '771', spark: [748, 752, 758, 762, 768, 773, 778, 784], breakdown: [{src: 'External Rating', pct: '30%', score: '791'}, {src: 'Compliance', pct: '25%', score: '812'}, {src: 'Vuln Mgmt', pct: '25%', score: '745'}, {src: 'Config', pct: '20%', score: '769'}]},
    {id: '32b-MTD-006', name: 'Vulnerability Backlog', value: '142', trend: 'improving' as const, target: '< 150', source: 'Tenable + Qualys', method: 'Total open vulns across all scanned assets tracked weekly', owner: 'Vuln Manager', sla: '< 150', prev: '168', spark: [210, 198, 185, 175, 165, 158, 150, 142], breakdown: [{src: 'Critical', pct: '8%', count: '11'}, {src: 'High', pct: '22%', count: '31'}, {src: 'Medium', pct: '45%', count: '64'}, {src: 'Low', pct: '25%', count: '36'}]}
  ];

  private _32bArchReview = [
    {id: '32b-AR-001', cat: 'Network', item: 'Network segmentation between trust zones with dedicated firewalls and granular ACLs', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Micro-segmentation via NSX across all production zones'},
    {id: '32b-AR-002', cat: 'Network', item: 'DMZ architecture isolating public services from internal network', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Dual-firewall DMZ with WAF and DDoS mitigation'},
    {id: '32b-AR-003', cat: 'Network', item: 'Least-privilege traffic flows with deny-by-default and quarterly review', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Explicit allow rules with automated drift detection'},
    {id: '32b-AR-004', cat: 'Network', item: 'Encrypted external communications with IPSec/TLS 1.3 and PFS', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'IKEv2 with PFS, TLS 1.3 for all external APIs'},
    {id: '32b-AR-005', cat: 'Identity', item: 'Centralized IdP with SSO and automated SCIM provisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Okta SSO SAML 2.0 for 247 apps with SCIM'},
    {id: '32b-AR-006', cat: 'Identity', item: 'RBAC with quarterly reviews and automated deprovisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Quarterly recertification with JIT admin access'},
    {id: '32b-AR-007', cat: 'Identity', item: 'PAM controlling all admin and service accounts with session recording', sev: 'high' as const, status: 'fail' as const, reviewer: 'IAM Lead', notes: '3 legacy accounts pending CyberArk enrollment'},
    {id: '32b-AR-008', cat: 'Identity', item: 'MFA enforced for all users, hardware tokens for privileged access', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'FIDO2 for admins, push MFA for users, 99.8% enrollment'},
    {id: '32b-AR-009', cat: 'Data', item: 'Data classification enforced with automated labeling on all repos', sev: 'high' as const, status: 'partial' as const, reviewer: 'Data Protection Lead', notes: 'Framework deployed, gaps on 4 legacy file shares'},
    {id: '32b-AR-010', cat: 'Data', item: 'AES-256 encryption at rest for all sensitive data stores', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'Full-disk, TDE on DBs, SSE-KMS on cloud storage'},
    {id: '32b-AR-011', cat: 'Data', item: 'DLP monitoring and preventing unauthorized data exfiltration', sev: 'high' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'DLP active on email, endpoint, cloud, and network'},
    {id: '32b-AR-012', cat: 'Data', item: 'Encrypted off-site backups with quarterly restore testing', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Infrastructure Lead', notes: '3-2-1 strategy with immutable backups'},
    {id: '32b-AR-013', cat: 'AppSec', item: 'Secure SDLC with SAST, DAST, SCA in all CI/CD pipelines', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: '34 pipelines with security gates and PR blocking'},
    {id: '32b-AR-014', cat: 'AppSec', item: 'API gateway with auth, rate limiting, schema validation', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: 'Kong with OAuth2, rate limits, OpenAPI validation'},
    {id: '32b-AR-015', cat: 'AppSec', item: 'Container security with image scanning and runtime protection', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Trivy scanning, Falco runtime in 80% of clusters'},
    {id: '32b-AR-016', cat: 'Monitoring', item: 'SIEM collecting from all critical systems with correlation rules', sev: 'critical' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'Splunk 98% coverage with 1200+ correlation rules'},
    {id: '32b-AR-017', cat: 'Monitoring', item: 'EDR with behavioral detection on all managed endpoints', sev: 'high' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'CrowdStrike on 99.2% endpoints with 24/7 MDR'},
    {id: '32b-AR-018', cat: 'Monitoring', item: 'Executive dashboards with real-time posture visibility', sev: 'medium' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'PowerBI dashboards with weekly exec summaries'},
    {id: '32b-AR-019', cat: 'Cloud', item: 'CSPM monitoring and remediating cloud misconfigurations', sev: 'high' as const, status: 'pass' as const, reviewer: 'Cloud Security Lead', notes: 'Prisma Cloud 450+ checks with auto-remediation'},
    {id: '32b-AR-020', cat: 'Cloud', item: 'IaC templates scanned before deployment to production', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Checkov for Terraform, cfn-nag for CloudFormation pending'}
  ];

  private _get32bRiskProgress(): number {
    return Math.round(this._32bRiskAssessmentWorkflow.filter(s => s.status === 'completed').length / this._32bRiskAssessmentWorkflow.length * 100);
  }

  private _get32bTreatmentSummary(): {total: number; approved: number; avgReduction: number; investment: string} {
    const a = this._32bTreatmentOptions.filter(r => r.status === 'approved').length;
    const avg = this._32bTreatmentOptions.reduce((s, r) => s + ((r.before - r.after) / r.before * 100), 0) / this._32bTreatmentOptions.length;
    return {total: this._32bTreatmentOptions.length, approved: a, avgReduction: Math.round(avg), investment: '$1.125M annually'};
  }

  private _get32bRuleSummary(): {total: number; passing: number; failing: number; avgRate: number; violations: number} {
    const p = this._32bComplianceRules.filter(r => r.result === 'pass').length;
    const f = this._32bComplianceRules.filter(r => r.result === 'fail').length;
    const avg = this._32bComplianceRules.reduce((s, r) => s + r.passRate, 0) / this._32bComplianceRules.length;
    const v = this._32bComplianceRules.reduce((s, r) => s + r.violations, 0);
    return {total: this._32bComplianceRules.length, passing: p, failing: f, avgRate: Math.round(avg * 10) / 10, violations: v};
  }

  private _get32bPipelineStats(): {positions: number; applicants: number; interviewing: number; offers: number; critical: number} {
    const t = this._32bTalentPositions.length;
    const a = this._32bTalentPositions.reduce((s, p) => s + p.applicants, 0);
    const i = this._32bTalentPositions.reduce((s, p) => s + p.interviews, 0);
    const o = this._32bTalentPositions.reduce((s, p) => s + p.offers, 0);
    const c = this._32bTalentPositions.filter(x => x.urgency === 'critical').length;
    return {positions: t, applicants: a, interviewing: i, offers: o, critical: c};
  }

  private _get32bMetricTrends(): {improving: number; stable: number; total: number} {
    const imp = this._32bMetricsDashboard.filter(m => m.trend === 'improving').length;
    const stb = this._32bMetricsDashboard.filter(m => m.trend === 'stable').length;
    return {improving: imp, stable: stb, total: this._32bMetricsDashboard.length};
  }

  private _get32bArchScore(): {total: number; passed: number; failed: number; partial: number; score: number} {
    const p = this._32bArchReview.filter(c => c.status === 'pass').length;
    const f = this._32bArchReview.filter(c => c.status === 'fail').length;
    const pa = this._32bArchReview.filter(c => c.status === 'partial').length;
    return {total: this._32bArchReview.length, passed: p, failed: f, partial: pa, score: Math.round((p / this._32bArchReview.length) * 100)};
  }


  private _z32bIncidentScenarios = [
    {id: 'z32b-IS-001', name: 'Ransomware Outbreak in Production Environment', category: 'Malware', likelihood: 'Medium', impact: 'Critical', riskScore: 15, playbooks: ['IR-Ransomware-Isolate', 'IR-Ransomware-Eradicate', 'IR-Ransomway-Recover'], estimatedMTTD: '15min', estimatedMTTR: '4h', involvedTeams: ['SOC', 'IR', 'Infrastructure', 'Legal', 'Comms'], keyIndicators: ['Mass file encryption events', 'Ransom note deployment', 'Lateral movement via SMB/RDP', 'Backup deletion attempts'], mitigationControls: ['EDR behavioral detection', 'Network segmentation', 'Immutable backups', 'Email filtering'], lessonsLearned: 'Ensure offline backups are tested quarterly and Ransomware-specific playbooks are updated with latest threat intelligence', lastDrillDate: '2026-03-15', nextDrillDate: '2026-06-15'},
    {id: 'z32b-IS-002', name: 'Cloud Misconfiguration Leading to Data Exposure', category: 'Cloud', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Cloud-Misconfig-Assess', 'IR-Cloud-Misconfig-Remediate', 'IR-Cloud-Misconfig-Notify'], estimatedMTTD: '2h', estimatedMTTR: '6h', involvedTeams: ['Cloud Security', 'SOC', 'Data Protection', 'Legal'], keyIndicators: ['Public S3 bucket exposure', 'Overly permissive IAM policies', 'Unencrypted database snapshots', 'Public API endpoint discovery'], mitigationControls: ['CSPM monitoring', 'IAM policy analyzer', 'Automated remediation', 'Cloud audit logging'], lessonsLearned: 'Implement CSPM with auto-remediation for common misconfigurations and conduct monthly cloud configuration reviews', lastDrillDate: '2026-02-20', nextDrillDate: '2026-05-20'},
    {id: 'z32b-IS-003', name: 'Insider Threat Data Exfiltration', category: 'Insider', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-Insider-Investigate', 'IR-Insider-Contain', 'IR-Insider-Escalate'], estimatedMTTD: '8h', estimatedMTTR: '24h', involvedTeams: ['SOC', 'IR', 'HR', 'Legal', 'IAM'], keyIndicators: ['Anomalous data access patterns', 'Large file downloads to USB or cloud', 'Off-hours access to sensitive systems', 'Accessing data outside normal job scope'], mitigationControls: ['UEBA analytics', 'DLP policies', 'PAM controls', 'Access reviews'], lessonsLearned: 'Strengthen UEBA baselines and implement automated alerts for data access pattern anomalies', lastDrillDate: '2026-01-10', nextDrillDate: '2026-04-10'},
    {id: 'z32b-IS-004', name: 'Supply Chain Attack via Compromised Vendor Software', category: 'Supply Chain', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-SupplyChain-Detect', 'IR-SupplyChain-Isolate', 'IR-SupplyChain-Remediate'], estimatedMTTD: '24h', estimatedMTTR: '72h', involvedTeams: ['SOC', 'IR', 'Procurement', 'Vendor Mgmt', 'Legal'], keyIndicators: ['Unexpected network connections from vendor software', 'Modified binaries or dependencies', 'Anomalous behavior from trusted applications', 'SBOM discrepancy alerts'], mitigationControls: ['SCA scanning', 'SBOM monitoring', 'Vendor security assessments', 'Network segmentation'], lessonsLearned: 'Require SBOM from all vendors and implement continuous monitoring of third-party component behavior', lastDrillDate: '2026-03-01', nextDrillDate: '2026-06-01'},
    {id: 'z32b-IS-005', name: 'Credential Stuffing Attack on Customer Portal', category: 'External Attack', likelihood: 'High', impact: 'Medium', riskScore: 12, playbooks: ['IR-CredentialStuffing-Detect', 'IR-CredentialStuffing-Block', 'IR-CredentialStuffing-Notify'], estimatedMTTD: '30min', estimatedMTTR: '2h', involvedTeams: ['SOC', 'AppSec', 'Customer Support', 'Comms'], keyIndicators: ['High volume of failed login attempts', 'Login attempts from known compromised credential lists', 'Unusual geographic distribution of login attempts', 'Account takeover indicators'], mitigationControls: ['WAF rate limiting', 'CAPTCHA on login', 'MFA enforcement', 'Credential monitoring service'], lessonsLearned: 'Deploy advanced bot detection on login endpoints and implement proactive credential monitoring for customer accounts', lastDrillDate: '2026-04-01', nextDrillDate: '2026-07-01'},
    {id: 'z32b-IS-006', name: 'Phishing Campaign Targeting Executive Team', category: 'Social Engineering', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Phishing-Contain', 'IR-Phishing-Investigate', 'IR-Phishing-Notify'], estimatedMTTD: '1h', estimatedMTTR: '4h', involvedTeams: ['SOC', 'Awareness', 'Executive Office', 'IT'], keyIndicators: ['Sophisticated spear-phishing emails to C-suite', 'Business email compromise indicators', 'Redirected MFA approval requests', 'Unauthorized wire transfer attempts'], mitigationControls: ['AI email filtering', 'Executive awareness training', 'Out-of-band verification policy', 'DMARC/DKIM/SPF'], lessonsLearned: 'Implement additional email security controls for executive mailboxes and conduct monthly spear-phishing simulations', lastDrillDate: '2026-02-15', nextDrillDate: '2026-05-15'}
  ];

  private _z32bSecurityFrameworks = [
  ...
  private _getz32bScenarioSummary(): {total: number; critical: number; high: number; drilled: number; avgRiskScore: number} {
    const crit = this._z32bIncidentScenarios.filter(s => s.impact === 'Critical').length;
    const high = this._z32bIncidentScenarios.filter(s => s.impact === 'High').length;
    const avg = this._z32bIncidentScenarios.reduce((s, x) => s + x.riskScore, 0) / this._z32bIncidentScenarios.length;
    return {total: this._z32bIncidentScenarios.length, critical: crit, high: high, drilled: this._z32bIncidentScenarios.length, avgRiskScore: Math.round(avg)};
  }

  private _getz32bVendorRiskSummary(): {total: number; approved: number; conditional: number; remediation: number; avgScore: number} {
    const approved = this._z32bThirdPartyRiskData.filter(v => v.status === 'approved').length;
    const cond = this._z32bThirdPartyRiskData.filter(v => v.status === 'conditional').length;
    const rem = this._z32bThirdPartyRiskData.filter(v => v.status === 'remediation_required').length;
    const avg = this._z32bThirdPartyRiskData.reduce((s, v) => s + v.riskScore, 0) / this._z32bThirdPartyRiskData.length;
    return {total: this._z32bThirdPartyRiskData.length, approved, conditional: cond, remediation: rem, avgScore: Math.round(avg)};
  }

  private _getz32bTrainingSummary(): {total: number; avgCompletion: number; avgScore: number; overdueUpdates: number} {
    const avgComp = this._z32bSecurityTrainingCatalog.reduce((s, t) => s + t.completionRate, 0) / this._z32bSecurityTrainingCatalog.length;
    const avgScore = this._z32bSecurityTrainingCatalog.filter(t => t.avgScore > 0).reduce((s, t) => s + t.avgScore, 0) / this._z32bSecurityTrainingCatalog.filter(t => t.avgScore > 0).length;
    return {total: this._z32bSecurityTrainingCatalog.length, avgCompletion: Math.round(avgComp * 10) / 10, avgScore: Math.round(avgScore), overdueUpdates: 0};
  }

  private _getz32bToolHealthSummary(): {total: number; healthy: number; degraded: number; annualCost: string} {
    const healthy = this._z32bSecurityToolInventory.filter(t => t.health === 'healthy').length;
    const degraded = this._z32bSecurityToolInventory.filter(t => t.health === 'degraded').length;
    const cost = this._z32bSecurityToolInventory.reduce((s, t) => s + parseInt(t.annualCost.replace(/[^0-9]/g, '')), 0);
    return {total: this._z32bSecurityToolInventory.length, healthy, degraded, annualCost: '$' + (cost / 1000).toFixed(0) + 'K'};
  }


  private _x32bSecurityZones = [
    {zoneId: 'x32b-Z-001', name: 'Internet DMZ', trustLevel: 'Untrusted', subnet: '10.0.0.0/24', segmentation: 'Dual-firewall', firewallRules: 45, monitoring: 'Full packet capture', assets: ['Web servers', 'Load balancers', 'WAF appliances', 'Reverse proxy'], riskLevel: 'high' as const, lastAssessment: '2026-04-15', nextAssessment: '2026-07-15'},
    {zoneId: 'x32b-Z-002', name: 'Corporate Network', trustLevel: 'Trusted', subnet: '10.1.0.0/16', segmentation: 'VLAN + micro-seg', firewallRules: 128, monitoring: 'NetFlow + IDS', assets: ['Workstations', 'Printers', 'Meeting room systems', 'VoIP phones'], riskLevel: 'medium' as const, lastAssessment: '2026-04-10', nextAssessment: '2026-07-10'},
    {zoneId: 'x32b-Z-003', name: 'Production Data Center', trustLevel: 'Restricted', subnet: '10.2.0.0/16', segmentation: 'Full micro-segmentation', firewallRules: 256, monitoring: 'Full packet capture + EDR', assets: ['Application servers', 'Database clusters', 'Message queues', 'API gateways'], riskLevel: 'low' as const, lastAssessment: '2026-04-12', nextAssessment: '2026-07-12'},
    {zoneId: 'x32b-Z-004', name: 'Management Network', trustLevel: 'Restricted', subnet: '10.3.0.0/24', segmentation: 'Isolated VLAN', firewallRules: 32, monitoring: 'Syslog + SNMP', assets: ['Hypervisors', 'Switch management', 'Storage controllers', 'Backup appliances'], riskLevel: 'low' as const, lastAssessment: '2026-04-08', nextAssessment: '2026-07-08'},
    {zoneId: 'x32b-Z-005', name: 'Cloud Production', trustLevel: 'Restricted', subnet: 'VPC: 172.16.0.0/12', segmentation: 'Security groups + NACLs', firewallRules: 189, monitoring: 'VPC Flow Logs + GuardDuty', assets: ['Kubernetes clusters', 'Managed databases', 'Object storage', 'Serverless functions'], riskLevel: 'medium' as const, lastAssessment: '2026-04-18', nextAssessment: '2026-07-18'}
  ];

  private _x32bSecurityAlerts = [
    {id: 'x32b-AL-001', severity: 'critical', title: 'Active ransomware encryption detected on PROD-DB-01', source: 'CrowdStrike', timestamp: '2026-04-22T13:45:00Z', status: 'investigating', assignee: 'SOC Analyst J. Smith', asset: 'PROD-DB-01 (10.2.1.15)', mitreTechnique: 'T1486 - Data Encrypted for Impact', playbook: 'IR-Ransomware-Isolate', sla: '15 minutes', elapsed: '12 minutes', notes: 'EDR detected mass file encryption event pattern. Automated isolation triggered. IR team paged.'},
    {id: 'x32b-AL-002', severity: 'high', title: 'Impossible travel detected for user admin@company.com', source: 'Okta', timestamp: '2026-04-22T12:30:00Z', status: 'triaged', assignee: 'SOC Analyst M. Johnson', asset: 'Okta - admin@company.com', mitreTechnique: 'T1078 - Valid Accounts', playbook: 'IR-ImpossibleTravel', sla: '30 minutes', elapsed: '87 minutes', notes: 'Login from Tokyo followed by login from New York within 30 minutes. Account temporarily locked. User confirmed via out-of-band channel they are in New York.'},
    {id: 'x32b-AL-003', severity: 'high', title: 'Public S3 bucket discovered with customer PII exposure', source: 'Prisma Cloud', timestamp: '2026-04-22T11:15:00Z', status: 'remediating', assignee: 'Cloud Security Lead A. Chen', asset: 'AWS S3: customer-data-archive-2024', mitreTechnique: 'N/A - Misconfiguration', playbook: 'IR-Cloud-Misconfig-Remediate', sla: '1 hour', elapsed: '2.5 hours', notes: 'Bucket containing archived customer records was publicly accessible. Bucket blocked and encrypted. Data exposure assessment in progress.'},
    {id: 'x32b-AL-004', severity: 'medium', title: 'Anomalous data download by user jane.developer@company.com', source: 'Microsoft DLP', timestamp: '2026-04-22T10:00:00Z', status: 'investigating', assignee: 'SOC Analyst R. Davis', asset: 'Endpoint: WS-DEV-042', mitreTechnique: 'T1567 - Exfil Over Web Service', playbook: 'IR-DLP-Investigation', sla: '4 hours', elapsed: '3.5 hours', notes: 'User downloaded 4.2GB of source code files to personal Google Drive. Interview scheduled with user and manager.'},
    {id: 'x32b-AL-005', severity: 'medium', title: 'Credential stuffing attack detected on customer portal', source: 'Cloudflare WAF', timestamp: '2026-04-22T09:30:00Z', status: 'contained', assignee: 'SOC Analyst K. Wilson', asset: 'Customer Portal (portal.company.com)', mitreTechnique: 'T1110 - Brute Force', playbook: 'IR-CredentialStuffing-Block', sla: '2 hours', elapsed: '4 hours', notes: 'Rate limiting and CAPTCHA automatically deployed. 12,000 failed login attempts from 800 unique IPs blocked. No successful account takeovers confirmed.'},
    {id: 'x32b-AL-006', severity: 'low', title: 'SSL certificate expiring in 14 days for api.company.com', source: 'Certificate monitoring', timestamp: '2026-04-22T08:00:00Z', status: 'remediated', assignee: 'Infrastructure Lead T. Brown', asset: 'api.company.com', mitreTechnique: 'N/A', playbook: 'Certificate Renewal', sla: '30 days', elapsed: '16 days', notes: 'Automated renewal triggered via ACME. New certificate deployed successfully. No service interruption.'}
  ];

  private _x32bComplianceFrameworks = [
  ...
  private _x32bDataClassificationPolicy = [
  ...
  private _getx32bZoneSummary(): {total: number; highRisk: number; mediumRisk: number; lowRisk: number; totalRules: number} {
    const high = this._x32bSecurityZones.filter(z => z.riskLevel === 'high').length;
    const med = this._x32bSecurityZones.filter(z => z.riskLevel === 'medium').length;
    const low = this._x32bSecurityZones.filter(z => z.riskLevel === 'low').length;
    const rules = this._x32bSecurityZones.reduce((s, z) => s + z.firewallRules, 0);
    return {total: this._x32bSecurityZones.length, highRisk: high, mediumRisk: med, lowRisk: low, totalRules: rules};
  }

  private _getx32bAlertSummary(): {total: number; critical: number; high: number; medium: number; low: number; open: number; contained: number} {
    const crit = this._x32bSecurityAlerts.filter(a => a.severity === 'critical').length;
    const high = this._x32bSecurityAlerts.filter(a => a.severity === 'high').length;
    const med = this._x32bSecurityAlerts.filter(a => a.severity === 'medium').length;
    const low = this._x32bSecurityAlerts.filter(a => a.severity === 'low').length;
    const open = this._x32bSecurityAlerts.filter(a => a.status === 'investigating' || a.status === 'triaged').length;
    const contained = this._x32bSecurityAlerts.filter(a => a.status === 'contained' || a.status === 'remediated').length;
    return {total: this._x32bSecurityAlerts.length, critical: crit, high, medium: med, low, open, contained};
  }

  private _getx32bComplianceSummary(): {total: number; fullyCompliant: number; mostlyCompliant: number; avgComplianceRate: number} {
    const full = this._x32bComplianceFrameworks.filter(f => f.status === 'compliant').length;
    const mostly = this._x32bComplianceFrameworks.filter(f => f.status === 'mostly_compliant').length;
    const avgRate = this._x32bComplianceFrameworks.reduce((s, f) => s + (f.compliant / f.requirements * 100), 0) / this._x32bComplianceFrameworks.length;
    return {total: this._x32bComplianceFrameworks.length, fullyCompliant: full, mostlyCompliant: mostly, avgComplianceRate: Math.round(avgRate * 10) / 10};
  }
  ...
  // === Security Risk Register (business_fra) ===
  private _business_fraRiskRegister = [
    { id: "RSK-BUS-0001", title: "Data breach from unpatched systems", owner: "CTO", category: "Financial", status: "mitigating", trend: "stable", severity: "low", likelihood: 7, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-15", nextReview: "2026-07-06" },
    { id: "RSK-BUS-0002", title: "Ransomware attack on critical infrastructure", owner: "CRO", category: "Strategic", status: "closed", trend: "decreasing", severity: "high", likelihood: 8, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-14", nextReview: "2026-07-04" },
    { id: "RSK-BUS-0003", title: "Insider threat data exfiltration", owner: "VP Eng", category: "Compliance", status: "accepted", trend: "increasing", severity: "high", likelihood: 4, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-01", nextReview: "2026-07-27" },
    { id: "RSK-BUS-0004", title: "Supply chain compromise", owner: "Security Lead", category: "Technical", status: "open", trend: "stable", severity: "medium", likelihood: 4, impact: 4, treatment: "Monitor and review quarterly", lastReview: "2026-04-15", nextReview: "2026-07-11" },
    { id: "RSK-BUS-0005", title: "Cloud misconfiguration exposure", owner: "Risk Mgr", category: "Reputational", status: "mitigating", trend: "decreasing", severity: "medium", likelihood: 6, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-20", nextReview: "2026-07-03" },
    { id: "RSK-BUS-0006", title: "Phishing campaign success rate", owner: "Compliance Dir", category: "Legal", status: "closed", trend: "increasing", severity: "medium", likelihood: 9, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-16", nextReview: "2026-07-19" },
    { id: "RSK-BUS-0007", title: "Third-party vendor data breach", owner: "IT Dir", category: "Regulatory", status: "accepted", trend: "stable", severity: "medium", likelihood: 4, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-22", nextReview: "2026-07-22" },
    { id: "RSK-BUS-0008", title: "Regulatory non-compliance penalty", owner: "DevOps Lead", category: "Third-Party", status: "open", trend: "decreasing", severity: "critical", likelihood: 6, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-23", nextReview: "2026-07-03" },
    { id: "RSK-BUS-0009", title: "Zero-day exploit in production", owner: "Architect", category: "Human Capital", status: "mitigating", trend: "increasing", severity: "critical", likelihood: 1, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-18", nextReview: "2026-07-26" },
    { id: "RSK-BUS-0010", title: "Insufficient access controls", owner: "CISO", category: "Operational", status: "closed", trend: "stable", severity: "low", likelihood: 8, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-10", nextReview: "2026-07-06" },
    { id: "RSK-BUS-0011", title: "DDoS attack on services", owner: "CTO", category: "Financial", status: "accepted", trend: "decreasing", severity: "critical", likelihood: 5, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-20", nextReview: "2026-07-10" },
    { id: "RSK-BUS-0012", title: "Social engineering attack", owner: "CRO", category: "Strategic", status: "open", trend: "increasing", severity: "low", likelihood: 3, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-09", nextReview: "2026-07-13" },
    { id: "RSK-BUS-0013", title: "API security vulnerability", owner: "VP Eng", category: "Compliance", status: "mitigating", trend: "stable", severity: "critical", likelihood: 8, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-15", nextReview: "2026-07-09" },
    { id: "RSK-BUS-0014", title: "Mobile device compromise", owner: "Security Lead", category: "Technical", status: "closed", trend: "decreasing", severity: "low", likelihood: 2, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-02", nextReview: "2026-07-03" },
    { id: "RSK-BUS-0015", title: "Physical security breach", owner: "Risk Mgr", category: "Reputational", status: "accepted", trend: "increasing", severity: "high", likelihood: 5, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-16", nextReview: "2026-07-06" },
    { id: "RSK-BUS-0016", title: "Password policy weakness", owner: "Compliance Dir", category: "Legal", status: "open", trend: "stable", severity: "medium", likelihood: 5, impact: 9, treatment: "Monitor and review quarterly", lastReview: "2026-04-23", nextReview: "2026-07-05" },
    { id: "RSK-BUS-0017", title: "Network segmentation gap", owner: "IT Dir", category: "Regulatory", status: "mitigating", trend: "decreasing", severity: "low", likelihood: 3, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-05", nextReview: "2026-07-16" },
    { id: "RSK-BUS-0018", title: "Encryption key management failure", owner: "DevOps Lead", category: "Third-Party", status: "closed", trend: "increasing", severity: "critical", likelihood: 5, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-11", nextReview: "2026-07-20" },
    { id: "RSK-BUS-0019", title: "Audit trail tampering", owner: "Architect", category: "Human Capital", status: "accepted", trend: "stable", severity: "medium", likelihood: 5, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-18", nextReview: "2026-07-03" },
    { id: "RSK-BUS-0020", title: "Business email compromise", owner: "CISO", category: "Operational", status: "open", trend: "decreasing", severity: "low", likelihood: 7, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-08", nextReview: "2026-07-15" },
  ];
  private _business_fraRiskFilter: string = 'all';
  private _business_fraRiskSeverity: string = 'all';
  private _business_fraRiskStatus: string = 'all';
  private _business_fraExpandedRisk: string = '';
  private _business_fraFilterRisks() {
    const reg = this._business_fraRiskRegister;
    return reg.filter(r => {
      if (this._business_fraRiskFilter !== "all" && r.category !== this._business_fraRiskFilter) return false;
      if (this._business_fraRiskSeverity !== "all" && r.severity !== this._business_fraRiskSeverity) return false;
      if (this._business_fraRiskStatus !== "all" && r.status !== this._business_fraRiskStatus) return false;
      return true;
    });
  }
  private _business_fraGetRiskScore(r: any) { return Math.round(r.likelihood * r.impact * 1.5); }
  private _business_fraGetRiskTrendIcon(trend: string) {
    if (trend === "increasing") return "\u2191";
    if (trend === "decreasing") return "\u2193";
    return "\u2192";
  }
  private _business_fraGetRiskColor(sev: string) {
    if (sev === "critical") return "#dc2626";
    if (sev === "high") return "#ea580c";
    if (sev === "medium") return "#d97706";
    return "#16a34a";
  }
  private _business_fraGetRiskCounts() {
    const reg = this._business_fraRiskRegister;
  ...
  private _business_fraGetTreatmentProgress() {
    const reg = this._business_fraRiskRegister;
    const treated = reg.filter(r => r.status === "mitigating" || r.status === "closed").length;
    return Math.round((treated / Math.max(reg.length, 1)) * 100);
  }

  // === Security Metrics API Gateway (business_fra) ===
  private _business_fraApiEndpoints = [
    { method: "GET", path: "/api/v1/threats", name: "Threat Intelligence Feed", status: "active", avgLatency: 181.7, reqPerMin: 1381.0, errorRate: 2.24, uptime: round(96.55152641768751,2), version: "v1.7.5" },
    { method: "POST", path: "/api/v1/scans/start", name: "Vulnerability Scanner", status: "active", avgLatency: 262.2, reqPerMin: 1949.0, errorRate: 1.17, uptime: round(96.41131401221715,2), version: "v2.4.11" },
    { method: "GET", path: "/api/v1/assets", name: "Asset Inventory", status: "active", avgLatency: 302.5, reqPerMin: 93.0, errorRate: 2.07, uptime: round(95.08758304339759,2), version: "v2.6.5" },
    { method: "POST", path: "/api/v1/alerts", name: "Alert Management", status: "active", avgLatency: 423.4, reqPerMin: 1179.0, errorRate: 0.62, uptime: round(98.20070731156736,2), version: "v2.9.1" },
    { method: "GET", path: "/api/v1/compliance", name: "Compliance Status", status: "active", avgLatency: 369.2, reqPerMin: 1078.0, errorRate: 0.69, uptime: round(96.3943285154973,2), version: "v2.6.0" },
    { method: "PUT", path: "/api/v1/policies", name: "Policy Engine", status: "active", avgLatency: 256.5, reqPerMin: 352.0, errorRate: 1.75, uptime: round(98.9226360000928,2), version: "v1.9.6" },
    { method: "GET", path: "/api/v1/incidents", name: "Incident Tracker", status: "active", avgLatency: 417.3, reqPerMin: 365.0, errorRate: 1.05, uptime: round(99.71023016556504,2), version: "v3.1.8" },
    { method: "POST", path: "/api/v1/forensics", name: "Forensics Collector", status: "degraded", avgLatency: 91.4, reqPerMin: 248.0, errorRate: 0.92, uptime: round(99.89621529349019,2), version: "v2.3.14" },
    { method: "GET", path: "/api/v1/risk/assess", name: "Risk Assessment", status: "active", avgLatency: 79.8, reqPerMin: 2389.0, errorRate: 2.18, uptime: round(98.38019493937982,2), version: "v2.8.6" },
    { method: "POST", path: "/api/v1/auth/verify", name: "Authentication", status: "active", avgLatency: 84.9, reqPerMin: 2291.0, errorRate: 1.59, uptime: round(95.25708353484085,2), version: "v3.6.13" },
    { method: "GET", path: "/api/v1/logs/audit", name: "Audit Log Query", status: "active", avgLatency: 242.2, reqPerMin: 1339.0, errorRate: 1.94, uptime: round(97.42147460020713,2), version: "v1.5.7" },
    { method: "PUT", path: "/api/v1/users/roles", name: "Role Management", status: "active", avgLatency: 400.3, reqPerMin: 155.0, errorRate: 0.16, uptime: round(98.79618851150971,2), version: "v3.5.19" },
    { method: "POST", path: "/api/v1/encrypt", name: "Encryption Service", status: "active", avgLatency: 382.8, reqPerMin: 796.0, errorRate: 1.73, uptime: round(95.93095685767227,2), version: "v1.0.12" },
    { method: "GET", path: "/api/v1/network/topo", name: "Network Topology", status: "maintenance", avgLatency: 16.8, reqPerMin: 2223.0, errorRate: 1.59, uptime: round(96.55201232157597,2), version: "v1.6.19" },
    { method: "DELETE", path: "/api/v1/sessions", name: "Session Manager", status: "active", avgLatency: 391.4, reqPerMin: 2200.0, errorRate: 1.46, uptime: round(98.55092033379371,2), version: "v1.9.1" },
  ];
  private _business_fraApiKeys = [
    { id: "ak-000001", name: "key-busi-001", created: "2026-08-26", lastUsed: "2026-04-23", status: "active", calls: 39702, rateLimit: 100 },
    { id: "ak-000002", name: "key-busi-002", created: "2026-03-20", lastUsed: "2026-04-17", status: "active", calls: 7146, rateLimit: 100 },
    { id: "ak-000003", name: "key-busi-003", created: "2026-08-22", lastUsed: "2026-04-05", status: "active", calls: 42424, rateLimit: 100 },
    { id: "ak-000004", name: "key-busi-004", created: "2026-08-11", lastUsed: "2026-04-02", status: "active", calls: 11720, rateLimit: 5000 },
    { id: "ak-000005", name: "key-busi-005", created: "2026-02-27", lastUsed: "2026-04-18", status: "active", calls: 37158, rateLimit: 100 },
    { id: "ak-000006", name: "key-busi-006", created: "2026-06-18", lastUsed: "2026-04-02", status: "active", calls: 25244, rateLimit: 5000 },
    { id: "ak-000007", name: "key-busi-007", created: "2026-07-10", lastUsed: "2026-04-13", status: "active", calls: 2642, rateLimit: 1000 },
    { id: "ak-000008", name: "key-busi-008", created: "2026-08-24", lastUsed: "2026-04-15", status: "active", calls: 24791, rateLimit: 1000 },
  ];
  private _business_fraApiHealthSummary() {
    const eps = this._business_fraApiEndpoints;
  ...
  private _business_fraGetApiByMethod(method: string) { return this._business_fraApiEndpoints.filter(e=>e.method===method); }
  private _business_fraGetSlowEndpoints() { return this._business_fraApiEndpoints.filter(e=>e.avgLatency>200).sort((a,b)=>b.avgLatency-a.avgLatency); }
  private _business_fraGetHighErrorEndpoints() { return this._business_fraApiEndpoints.filter(e=>e.errorRate>1.0).sort((a,b)=>b.errorRate-a.errorRate); }

  // === Security Training Effectiveness (business_fra) ===
  private _business_fraTrainingModules = [
    { id: "TRN-001", name: "Security Fundamentals", completionRate: 76.9, avgScore: 57.4, behaviorChange: 83.1, enrolled: 258, completed: 46, duration: "28min", category: "mandatory" },
    { id: "TRN-002", name: "Phishing Awareness", completionRate: 49.6, avgScore: 74.3, behaviorChange: 51.3, enrolled: 272, completed: 254, duration: "66min", category: "mandatory" },
    { id: "TRN-003", name: "Social Engineering Defense", completionRate: 69.6, avgScore: 61.1, behaviorChange: 63.3, enrolled: 284, completed: 64, duration: "69min", category: "optional" },
    { id: "TRN-004", name: "Password Hygiene", completionRate: 67.5, avgScore: 64.3, behaviorChange: 82.7, enrolled: 373, completed: 31, duration: "41min", category: "mandatory" },
    { id: "TRN-005", name: "Data Classification", completionRate: 56.5, avgScore: 91.3, behaviorChange: 35.3, enrolled: 357, completed: 334, duration: "86min", category: "mandatory" },
    { id: "TRN-006", name: "Incident Response Basics", completionRate: 70.1, avgScore: 77.3, behaviorChange: 54.5, enrolled: 164, completed: 421, duration: "32min", category: "mandatory" },
    { id: "TRN-007", name: "Secure Coding", completionRate: 91.6, avgScore: 63.1, behaviorChange: 31.1, enrolled: 170, completed: 147, duration: "18min", category: "optional" },
    { id: "TRN-008", name: "Cloud Security", completionRate: 88.5, avgScore: 92.1, behaviorChange: 54.1, enrolled: 260, completed: 105, duration: "20min", category: "mandatory" },
    { id: "TRN-009", name: "Mobile Device Security", completionRate: 75.4, avgScore: 70.1, behaviorChange: 82.5, enrolled: 487, completed: 434, duration: "95min", category: "mandatory" },
    { id: "TRN-010", name: "Network Security", completionRate: 79.0, avgScore: 94.2, behaviorChange: 42.6, enrolled: 62, completed: 269, duration: "72min", category: "mandatory" },
    { id: "TRN-011", name: "Physical Security", completionRate: 94.4, avgScore: 55.9, behaviorChange: 80.3, enrolled: 158, completed: 186, duration: "53min", category: "optional" },
    { id: "TRN-012", name: "Regulatory Compliance", completionRate: 94.9, avgScore: 55.0, behaviorChange: 83.3, enrolled: 132, completed: 363, duration: "30min", category: "mandatory" },
    { id: "TRN-013", name: "Risk Management", completionRate: 71.6, avgScore: 76.3, behaviorChange: 37.1, enrolled: 126, completed: 214, duration: "88min", category: "mandatory" },
    { id: "TRN-014", name: "Cryptography Basics", completionRate: 52.6, avgScore: 56.5, behaviorChange: 55.5, enrolled: 436, completed: 447, duration: "92min", category: "mandatory" },
    { id: "TRN-015", name: "Access Control", completionRate: 91.9, avgScore: 89.3, behaviorChange: 47.3, enrolled: 223, completed: 248, duration: "19min", category: "optional" },
    { id: "TRN-016", name: "Vendor Management", completionRate: 72.7, avgScore: 80.5, behaviorChange: 80.6, enrolled: 134, completed: 188, duration: "92min", category: "mandatory" },
  ];
  private _business_fraPhishingResults = [
    { month: "2026-01", sent: 695, clicked: 106, reported: 386, clickRate: round(15.251798561151078,1), reportRate: round(55.539568345323744,1) },
    { month: "2026-02", sent: 675, clicked: 73, reported: 119, clickRate: round(10.814814814814815,1), reportRate: round(17.62962962962963,1) },
    { month: "2026-03", sent: 275, clicked: 39, reported: 79, clickRate: round(14.181818181818182,1), reportRate: round(28.72727272727273,1) },
    { month: "2026-04", sent: 685, clicked: 83, reported: 358, clickRate: round(12.116788321167883,1), reportRate: round(52.26277372262774,1) },
    { month: "2026-05", sent: 702, clicked: 195, reported: 338, clickRate: round(27.77777777777778,1), reportRate: round(48.148148148148145,1) },
    { month: "2026-06", sent: 760, clicked: 160, reported: 386, clickRate: round(21.052631578947366,1), reportRate: round(50.78947368421053,1) },
  ];
  private _business_fraTrainingROI = { totalInvestment: 69780, avgCostPerEmployee: 378, riskReductionPct: round(random.uniform(15,45),1), incidentReductionPct: round(random.uniform(10,35),1), complianceScoreGain: round(random.uniform(5,25),1) };
  private _business_fraLearningPaths = [
    { name: "Beginner Security Analyst", totalModules: 16, completedModules: 4, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 39, enrolled: 29 },
    { name: "Advanced Threat Hunter", totalModules: 19, completedModules: 7, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 24, enrolled: 108 },
    { name: "Security Architect", totalModules: 19, completedModules: 6, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 80, enrolled: 72 },
    { name: "Incident Responder", totalModules: 13, completedModules: 2, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 43, enrolled: 83 },
    { name: "Compliance Specialist", totalModules: 12, completedModules: 2, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 20, enrolled: 123 },
    { name: "DevSecOps Engineer", totalModules: 13, completedModules: 3, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 58, enrolled: 115 },
  ];
  private _business_fraGetOverallCompletion() {
    const mods = this._business_fraTrainingModules;
    return round(mods.reduce((s,m)=>s+m.completionRate,0)/mods.length,1);
  }
  private _business_fraGetTopPerformers() { return [...this._business_fraTrainingModules].sort((a,b)=>b.avgScore-a.avgScore).slice(0,5); }
  private _business_fraGetModulesNeedingAttention() { return this._business_fraTrainingModules.filter(m=>m.completionRate<70||m.avgScore<65); }

  // === Security Governance Framework (business_fra) ===
  private _business_fraGovBodies = [
    { name: "Security Steering Committee", members: 15, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-22", nextMeeting: "2026-05-20", quorum: 7 },
    { name: "Risk Management Board", members: 10, chair: "CTO", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-04", nextMeeting: "2026-05-21", quorum: 7 },
    { name: "Data Governance Council", members: 7, chair: "CRO", meetingFreq: "Monthly", lastMeeting: "2026-04-15", nextMeeting: "2026-05-12", quorum: 5 },
    { name: "Compliance Oversight Board", members: 12, chair: "CDO", meetingFreq: "Monthly", lastMeeting: "2026-04-10", nextMeeting: "2026-05-10", quorum: 3 },
    { name: "Architecture Review Board", members: 6, chair: "VP Eng", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-16", nextMeeting: "2026-05-08", quorum: 3 },
    { name: "Change Advisory Board", members: 6, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-19", nextMeeting: "2026-05-24", quorum: 6 },
    { name: "Incident Review Board", members: 13, chair: "CIO", meetingFreq: "As needed", lastMeeting: "2026-04-09", nextMeeting: "2026-05-21", quorum: 3 },
    { name: "Vendor Risk Committee", members: 5, chair: "CFO", meetingFreq: "Quarterly", lastMeeting: "2026-04-17", nextMeeting: "2026-05-15", quorum: 8 },
  ];
  private _business_fraDecisions = [
    ...
    { id: "DEC-0004", title: "Mandate MFA for all external access", date: "2026-04-10", status: "pending", owner: "CRO" },
    ...
    { id: "DEC-0009", title: "Establish security champion program", date: "2026-04-05", status: "pending", owner: "CRO" },
    { id: "DEC-0010", title: "Migrate to passwordless authentication", date: "2026-01-18", status: "approved", owner: "VP Eng" },
  ];
  private _business_fraPolicyLifecycle = [
    { name: "Information Security Policy", version: "v4.3", status: "active", lastUpdated: "2026-02-11", nextReview: "2026-12-17", owner: "CISO" },
    { name: "Acceptable Use Policy", version: "v2.1", status: "active", lastUpdated: "2026-03-14", nextReview: "2026-9-13", owner: "Legal" },
    { name: "Data Retention Policy", version: "v2.1", status: "under-review", lastUpdated: "2026-01-17", nextReview: "2026-7-23", owner: "DPO" },
    { name: "Access Control Policy", version: "v3.2", status: "active", lastUpdated: "2026-02-03", nextReview: "2026-12-16", owner: "IAM Lead" },
    { name: "Incident Response Policy", version: "v3.5", status: "draft", lastUpdated: "2026-03-03", nextReview: "2026-8-09", owner: "IR Lead" },
    { name: "Business Continuity Plan", version: "v3.2", status: "active", lastUpdated: "2026-04-22", nextReview: "2026-12-20", owner: "BCP Mgr" },
    { name: "Vendor Management Policy", version: "v3.2", status: "active", lastUpdated: "2026-02-28", nextReview: "2026-11-27", owner: "Procurement" },
    { name: "Encryption Standard", version: "v4.2", status: "under-review", lastUpdated: "2026-03-28", nextReview: "2026-11-08", owner: "Security Arch" },
  ];
  private _business_fraGovMaturityScore = { overall: 2.1, riskManagement: 3.7, compliance: 4.4, incidentResponse: 3.8, awareness: 4.0, technology: 4.3 };
  private _business_fraGetPendingDecisions() { return this._business_fraDecisions.filter(d=>d.status==='pending'||d.status==='in-progress'); }
  private _business_fraGetActivePolicies() { return this._business_fraPolicyLifecycle.filter(p=>p.status==='active'); }
  private _business_fraGetEscalationPath() { return ["L1 Analyst","L2 Senior","Security Lead","CISO","Board"]; }

  // === Security Innovation Lab (business_fra) ===
  private _business_fraInnoProjects = [
    { id: "INN-001", name: "AI-Powered Threat Detection", description: "Machine learning models for real-time threat identification", status: "active", progress: 72, startDate: "2026-03-26", teamSize: 3, budget: 112841, milestones: 8, completedMilestones: 4 },
    { id: "INN-002", name: "Quantum-Resistant Cryptography", description: "Post-quantum encryption algorithm prototyping", status: "research", progress: 35, startDate: "2026-01-18", teamSize: 3, budget: 60320, milestones: 5, completedMilestones: 6 },
    { id: "INN-003", name: "Automated Red Teaming", description: "Autonomous penetration testing framework", status: "active", progress: 58, startDate: "2026-02-27", teamSize: 6, budget: 134806, milestones: 6, completedMilestones: 5 },
    { id: "INN-004", name: "Zero-Knowledge Authentication", description: "Privacy-preserving identity verification", status: "poc", progress: 88, startDate: "2026-01-19", teamSize: 4, budget: 165014, milestones: 7, completedMilestones: 1 },
    { id: "INN-005", name: "Blockchain Audit Trail", description: "Immutable security event logging", status: "active", progress: 45, startDate: "2026-02-07", teamSize: 2, budget: 156655, milestones: 7, completedMilestones: 3 },
    { id: "INN-006", name: "Behavioral Biometrics", description: "Continuous authentication via user behavior patterns", status: "research", progress: 22, startDate: "2026-03-15", teamSize: 6, budget: 173581, milestones: 9, completedMilestones: 4 },
    { id: "INN-007", name: "Deception Grid 2.0", description: "Advanced honeypot network with adaptive responses", status: "active", progress: 65, startDate: "2026-02-20", teamSize: 3, budget: 39398, milestones: 6, completedMilestones: 2 },
    { id: "INN-008", name: "Secure Enclave Integration", description: "Hardware-backed security for critical workloads", status: "poc", progress: 40, startDate: "2026-02-04", teamSize: 7, budget: 58013, milestones: 7, completedMilestones: 2 },
  ];
  private _business_fraTechEvaluations = [
    ...
    { name: "Confidential Computing", status: "planned", score: 8.8, recommendation: "Investigate", vendor: "Azure" },
  ...
  private _business_fraCollaborationPartners = [
  ...
  private _business_fraInnoMetrics = { totalProjects: 8, activeProjects: 4, avgTimeToValue: "134 days", pocSuccessRate: round(random.uniform(55,85),1), researchToProduction: round(random.uniform(20,50),1), innovationIndex: round(random.uniform(6.0,9.5),1) };
  private _business_fraGetProjectByStatus(status: string) { return this._business_fraInnoProjects.filter(p=>p.status===status); }
  private _business_fraGetTopEvaluations() { return [...this._business_fraTechEvaluations].sort((a,b)=>b.score-a.score).slice(0,3); }



  // === Compliance Dashboard Extension (business_fra) ===
  private _business_fraComplianceFrameworks = [
    { name: "SOC 2 Type II", description: "Trust Services Criteria", totalControls: 5, implementedControls: 2, status: "compliant", lastAudit: "2026-03-01", nextAudit: "2026-7-15", evidenceCount: 270 },
    { name: "ISO 27001", description: "Information Security Management", totalControls: 114, implementedControls: 85, status: "compliant", lastAudit: "2026-04-13", nextAudit: "2026-10-28", evidenceCount: 346 },
    { name: "PCI DSS 4.0", description: "Payment Card Industry", totalControls: 12, implementedControls: 11, status: "in-review", lastAudit: "2026-01-18", nextAudit: "2026-11-08", evidenceCount: 111 },
    { name: "HIPAA", description: "Health Insurance Portability", totalControls: 18, implementedControls: 18, status: "compliant", lastAudit: "2026-04-03", nextAudit: "2026-11-02", evidenceCount: 497 },
    { name: "GDPR", description: "Data Protection Regulation", totalControls: 99, implementedControls: 67, status: "non-compliant", lastAudit: "2026-03-21", nextAudit: "2026-8-28", evidenceCount: 189 },
    { name: "NIST CSF 2.0", description: "Cybersecurity Framework", totalControls: 6, implementedControls: 5, status: "partially-compliant", lastAudit: "2026-03-19", nextAudit: "2026-12-01", evidenceCount: 310 },
    { name: "FedRAMP", description: "Federal Risk Authorization", totalControls: 15, implementedControls: 11, status: "in-review", lastAudit: "2026-04-11", nextAudit: "2026-9-24", evidenceCount: 280 },
    { name: "SOX", description: "Sarbanes-Oxley Compliance", totalControls: 8, implementedControls: 4, status: "in-review", lastAudit: "2026-01-06", nextAudit: "2026-9-13", evidenceCount: 379 },
    { name: "CIS Controls v8", description: "Center for Internet Security", totalControls: 18, implementedControls: 14, status: "compliant", lastAudit: "2026-02-06", nextAudit: "2026-7-19", evidenceCount: 433 },
    { name: "COBIT 2019", description: "IT Governance Framework", totalControls: 40, implementedControls: 32, status: "in-review", lastAudit: "2026-02-08", nextAudit: "2026-8-10", evidenceCount: 116 },
  ];
  private _business_fraGetComplianceScore() {
    const fw = this._business_fraComplianceFrameworks;
    return round(fw.reduce((s,f)=>s + (f.implementedControls/Math.max(f.totalControls,1))*100, 0) / fw.length, 1);
  }
  private _business_fraGetGaps() {
    return this._business_fraComplianceFrameworks.filter(f => f.status !== "compliant");
  }
  private _business_fraAuditTrail = [
    ...
    { id: "AUD-0003", action: "Gap identified", auditor: "Security Team", date: "2026-04-10", result: "fail", findings: 5 },
    ...
    { id: "AUD-0006", action: "Training verified", auditor: "Risk Team", date: "2026-04-13", result: "pass", findings: 5 },
    ...
    { id: "AUD-0009", action: "Risk accepted", auditor: "Board Audit", date: "2026-04-13", result: "pass", findings: 2 },
    { id: "AUD-0010", action: "Control enhanced", auditor: "Third Party", date: "2026-04-18", result: "pass", findings: 0 },
  ];

  // === Threat Intelligence Feed Extension (business_fra) ===
  private _business_fraThreatActors = [
    { name: "APT-29", alias: "Cozy Bear", origin: "Russia", type: "Nation-State", severity: "high", lastActivity: "2026-04-18", targets: "Energy", indicators: 343, ttps: 19 },
    { name: "APT-41", alias: "Double Dragon", origin: "China", type: "Nation-State", severity: "critical", lastActivity: "2026-04-20", targets: "Energy", indicators: 487, ttps: 50 },
    { name: "Lazarus Group", alias: "Hidden Cobra", origin: "North Korea", type: "Nation-State", severity: "critical", lastActivity: "2026-04-11", targets: "Finance", indicators: 474, ttps: 9 },
    { name: "FIN7", alias: "Carbanak", origin: "Eastern Europe", type: "Financial", severity: "high", lastActivity: "2026-04-06", targets: "Manufacturing", indicators: 226, ttps: 26 },
    { name: "Conti", alias: "Wizard Spider", origin: "Russia", type: "Ransomware", severity: "critical", lastActivity: "2026-04-01", targets: "Manufacturing", indicators: 213, ttps: 19 },
    { name: "LockBit", alias: "LockBit Gang", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-15", targets: "Energy", indicators: 264, ttps: 19 },
    { name: "Cl0p", alias: "Cl0p Team", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-08", targets: "Manufacturing", indicators: 167, ttps: 35 },
    { name: "Sandworm", alias: "Unit 74455", origin: "Russia", type: "Nation-State", severity: "critical", lastActivity: "2026-04-16", targets: "Energy", indicators: 419, ttps: 49 },
  ];
  private _business_fraIoCFeed = [
    { id: "ioc-000001", type: "ip", value: "18.121.96.175", confidence: 78, source: "VirusTotal", firstSeen: "2026-04-11", lastSeen: "2026-04-15" },
    { id: "ioc-000002", type: "ip", value: "61.47.184.74", confidence: 96, source: "MISP", firstSeen: "2026-04-15", lastSeen: "2026-04-12" },
    { id: "ioc-000003", type: "ip", value: "142.37.236.181", confidence: 84, source: "Mandiant", firstSeen: "2026-04-03", lastSeen: "2026-04-03" },
    { id: "ioc-000004", type: "ip", value: "246.125.12.17", confidence: 78, source: "MISP", firstSeen: "2026-04-19", lastSeen: "2026-04-14" },
    { id: "ioc-000005", type: "ip", value: "62.166.204.107", confidence: 87, source: "CrowdStrike", firstSeen: "2026-04-22", lastSeen: "2026-04-15" },
    { id: "ioc-000006", type: "ip", value: "84.207.29.245", confidence: 66, source: "STIX", firstSeen: "2026-04-13", lastSeen: "2026-04-03" },
    { id: "ioc-000007", type: "ip", value: "35.202.41.147", confidence: 92, source: "AlienVault", firstSeen: "2026-04-07", lastSeen: "2026-04-16" },
    { id: "ioc-000008", type: "ip", value: "222.99.53.160", confidence: 46, source: "VirusTotal", firstSeen: "2026-04-04", lastSeen: "2026-04-07" },
    { id: "ioc-000009", type: "ip", value: "47.88.230.164", confidence: 72, source: "STIX", firstSeen: "2026-04-22", lastSeen: "2026-04-21" },
    { id: "ioc-000010", type: "ip", value: "82.112.154.50", confidence: 45, source: "CrowdStrike", firstSeen: "2026-04-14", lastSeen: "2026-04-13" },
    { id: "ioc-000011", type: "ip", value: "197.120.123.142", confidence: 81, source: "Mandiant", firstSeen: "2026-04-18", lastSeen: "2026-04-07" },
    { id: "ioc-000012", type: "ip", value: "140.57.35.10", confidence: 68, source: "STIX", firstSeen: "2026-04-10", lastSeen: "2026-04-14" },
    { id: "ioc-000013", type: "ip", value: "166.238.201.36", confidence: 76, source: "AlienVault", firstSeen: "2026-04-01", lastSeen: "2026-04-12" },
    { id: "ioc-000014", type: "ip", value: "136.241.223.248", confidence: 47, source: "VirusTotal", firstSeen: "2026-04-12", lastSeen: "2026-04-03" },
    { id: "ioc-000015", type: "ip", value: "66.209.162.25", confidence: 87, source: "CrowdStrike", firstSeen: "2026-04-16", lastSeen: "2026-04-10" },
  ];
  private _business_fraGetActiveThreats() { return this._business_fraThreatActors.filter(a => a.severity === 'critical'); }
  private _business_fraGetThreatSummary() {
    const actors = this._business_fraThreatActors;
  ...
  // === Incident Management Extension (business_fra) ===
  private _business_fraIncidents = [
    { id: "INC-20260001", title: "Unauthorized access detected", severity: "critical", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-10T19:44", affectedAssets: 15, rootCause: "Misconfiguration" },
    { id: "INC-20260002", title: "Malware outbreak on workstation", severity: "high", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-05T01:33", affectedAssets: 50, rootCause: "Credential compromise" },
    { id: "INC-20260003", title: "Data leak from S3 bucket", severity: "medium", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-18T07:35", affectedAssets: 4, rootCause: "Zero-day" },
    { id: "INC-20260004", title: "Phishing campaign targeting finance", severity: "low", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-03T08:46", affectedAssets: 31, rootCause: "Human error" },
    { id: "INC-20260005", title: "DDoS attack on web services", severity: "critical", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-22T20:37", affectedAssets: 48, rootCause: "Policy violation" },
    { id: "INC-20260006", title: "Ransomware encryption attempt", severity: "high", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-17T05:42", affectedAssets: 31, rootCause: "Unknown" },
    { id: "INC-20260007", title: "Insider data exfiltration", severity: "medium", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-12T22:27", affectedAssets: 39, rootCause: "Misconfiguration" },
    { id: "INC-20260008", title: "API key exposure in repo", severity: "low", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-04T00:16", affectedAssets: 37, rootCause: "Credential compromise" },
    { id: "INC-20260009", title: "SQL injection on portal", severity: "critical", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-12T13:26", affectedAssets: 16, rootCause: "Zero-day" },
    { id: "INC-20260010", title: "Brute force on VPN", severity: "high", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-15T07:42", affectedAssets: 32, rootCause: "Human error" },
    { id: "INC-20260011", title: "Supply chain alert from vendor", severity: "medium", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-17T16:06", affectedAssets: 44, rootCause: "Policy violation" },
    { id: "INC-20260012", title: "Suspicious lateral movement", severity: "low", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-03T05:20", affectedAssets: 35, rootCause: "Unknown" },
  ];
  private _business_fraGetIncidentStats() {
    const inc = this._business_fraIncidents;
    return { total: inc.length, open: inc.filter(i=>i.status==="open").length, investigating: inc.filter(i=>i.status==="investigating").length, mttd: 37, mttr: 81 };
  }
  private _business_fraGetSeverityDistribution() {
    const inc = this._business_fraIncidents;
  ...
  private _initThreatModel() {
    const categories = [
      ...
      { id: 'tc2', category: 'Tampering', threats: [
        { id: 't4', name: 'Data injection in business fraud detection', stride: 'T', likelihood: 7, impact: 8, dreadScore: 7.0, status: 'open', mitigations: ['Input validation', 'WAF rules'], assignedTo: 'Dev Team', discoveredDate: '2024-01-08', lastReviewed: '2024-02-10' },
  ...
  @state() private _soarPlaybooks: Array<{
    ...
    {
      id: 'pb-soar-001', name: 'Business Fraud Auto Response', status: 'active',
      steps: [
    ...
    {
      id: 'pb-soar-002', name: 'Business Fraud Investigation Workflow', status: 'active',
      steps: [
    ...
    {
      id: 'pb-soar-003', name: 'Business Fraud Compliance Scan', status: 'draft',
      steps: [
  ...
  @state() private _bfAle: number = 0;
  @state() private _bfSroi: number = 0;
  @state() private _bfCpi: number = 0;
  @state() private _bfBudgetAlloc: number = 0;
  @state() private _bfCostBenefit: number = 0;

  // Security Economics Calculator
  private bfInitEconomics() {
    this._bfAle = Math.round(2850000 + Math.random() * 4500000);
    this._bfSroi = Math.round(180 + Math.random() * 320);
    this._bfCpi = Math.round(45000 + Math.random() * 120000);
    this._bfBudgetAlloc = Math.round(2500000 + Math.random() * 3000000);
    this._bfCostBenefit = Math.round(220 + Math.random() * 180);
  }

  private _bfCalcAle(): { annual: number; perIncident: number; byCategory: Array<{name: string; value: number}> } {
    const base = this._bfAle;
    const categories = [
  ...
  private _bfCalcSroi(): Array<{year: number; investment: number; savings: number; roi: number}> {
    const baseInv = this._bfSroi * 10000;
  ...
  private _bfGetBudgetAlloc(): Array<{category: string; amount: number; pct: number; trend: string}> {
    const total = this._bfBudgetAlloc;
    const items = [
  ...
  private _bfGetCostBenefit(): Array<{control: string; cost: number; benefit: number; ratio: number; priority: string}> {
    const base = this._bfCostBenefit;
    const controls = [
  ...
  private _bfRenderEconomics() {
    const ale = this._bfCalcAle();
    const roi = this._bfCalcSroi();
    const budget = this._bfGetBudgetAlloc();
    const cb = this._bfGetCostBenefit();
    const cpi = this._bfCpi;
  ...
  @state() private _bfThreatLevel: any = null;
  @state() private _bfEmergingThreats: any = null;
  @state() private _bfThreatTrends: any = null;
  @state() private _bfSectorRadar: any = null;
  @state() private _bfActorActivity: any = null;

  // Threat Landscape Intelligence
  private bfInitThreatIntel() {
    this._bfThreatLevel = {
  ...
  private _bfRenderThreatIntel() {
    const tl = this._bfThreatLevel;
    const et = this._bfEmergingThreats;
    const sr = this._bfSectorRadar;
    const aa = this._bfActorActivity;
  ...
  @state() private _bfPolicies: any = null;
  @state() private _bfExceptions: any = null;
  @state() private _bfRiskRegister: any = null;
  @state() private _bfMeetings: any = null;
  @state() private _bfDeadlines: any = null;

  // Security Governance Dashboard
  private bfInitGovernance() {
    this._bfPolicies = [
  ...
  private _bfRenderGovernance() {
    const policies = this._bfPolicies;
    const risks = this._bfRiskRegister;
    const deadlines = this._bfDeadlines;
  ...
  @state() private _bfCriticalAssets: any = null;
  @state() private _bfAssetDeps: any = null;
  @state() private _bfEolAssets: any = null;
  @state() private _bfAssetRisk: any = null;

  // Asset Intelligence
  private bfInitAssetIntel() {
    this._bfCriticalAssets = [
    ...
    this._bfAssetRisk = { critical: 7, high: 23, medium: 45, low: 128, total: 203 };
  }

  private _bfRenderAssetIntel() {
    const assets = this._bfCriticalAssets;
    const eol = this._bfEolAssets;
    const ar = this._bfAssetRisk;
  ...
  @state() private _bfUserBaseline: any = null;
  @state() private _bfAnomalyRules: any = null;
  @state() private _bfDataAccess: any = null;
  @state() private _bfInsiderRisk: any = null;
  @state() private _businer15Timeline: Array<{id:string;time:string;event:string;severity:string;actor:string}> = [
  ...
  @state() private _businer15RootCauses: Array<{why:string;answer:string}> = [
  ...
  @state() private _businer15ImpactMatrix: Array<{system:string;users:number;data:string;revenue:string;status:string}> = [
  ...
  @state() private _businer15Actions: Array<{id:string;item:string;owner:string;deadline:string;priority:string;status:string}> = [
  ...
  @state() private _businer15Lessons: Array<{id:string;lesson:string;category:string;severity:string;applies_to:string}> = [
  ...
  @state() private _businer15ActiveTab: string = 'timeline';
  @state() private _businer15Benchmarks: Array<{metric:string;current:number;industry:number;target:number;unit:string;source:string}> = [
  ...
  @state() private _businer15MaturityLevels: Array<{domain:string;current:number;target:number;description:string}> = [
  ...
  @state() private _businer15QuarterlyData: Array<{quarter:string;score:number;improvement:number}> = [
  ...
  @state() private _businer15SelectedDomain: string = 'all';
  @state() private _businer15Alerts: Array<{id:string;name:string;severity:number;confidence:number;assetCrit:number;score:number;enriched:boolean;group:string;status:string;enrichData:Array<{key:string;value:string}>}> = [
  ...
  @state() private _businer15QualityMetrics: {fpRate:number;enrichSuccess:number;avgTriageTime:number;enrichedCount:number;totalCount:number} = {fpRate:0.12, enrichSuccess:0.78, avgTriageTime:4.5, enrichedCount:3, totalCount:5};
  @state() private _businer15RoutingRules: Array<{name:string;condition:string;channel:string;active:boolean}> = [
  ...
  @state() private _businer15Shapes: Array<{id:string;type:string;label:string;controls:string[]}> = [
  ...
  @state() private _businer15TrustBoundaries: Array<{from:string;to:string;label:string;strength:string}> = [
  ...
  @state() private _businer15ADRs: Array<{id:string;title:string;status:string;date:string;decision:string}> = [
  ...
  @state() private _businer15SelectedShape: string = '';
  @state() private _businer15Gauges: Array<{name:string;value:number;max:number;unit:string;status:string;color:string}> = [
  ...
  @state() private _businer15Anomalies: Array<{id:string;time:string;description:string;severity:string;acknowledged:boolean}> = [
  ...
  @state() private _businer15Integrations: Array<{name:string;status:string;uptime:number;lastCheck:string;latency:number}> = [
  ...
  @state() private _businer15AlertFatigue: Array<{analyst:string;alertsPerDay:number;escalated:number;dismissed:number;avgResponseMin:number}> = [
  ...
  @state() private _businer15SlaTarget: number = 99.9;

  // Insider Threat Detection
  private bfInitInsiderThreat() {
    this._bfUserBaseline = [
    ...
    this._bfInsiderRisk = { totalUsers: 2847, monitored: 342, flagged: 18, investigated: 5, confirmed: 1 };
  }

  private _bfRenderInsiderThreat() {
    const baseline = this._bfUserBaseline;
    const rules = this._bfAnomalyRules;
    const ir = this._bfInsiderRisk;
    ...
    return html`<div class="card" style="padding:14px;margin-bottom:8px">
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:4px;margin-bottom:10px;flex-wrap:wrap">
            ${['timeline','rootcause','impact','actions','lessons'].map(t => html`
              <button class="btn ${this._businer15ActiveTab === t ? 'btn-primary' : ''}" style="font-size:10px;padding:3px 8px" @click=${() => { this._businer15ActiveTab = t; }}>${t.charAt(0).toUpperCase() + t.slice(1)}</button>
            `)}
          </div>
          ${this._businer15ActiveTab === 'timeline' ? html`
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._businer15Timeline.map(e => html`
          ...
          ` : this._businer15ActiveTab === 'rootcause' ? html`
            <div style="margin-bottom:8px">${{__html: this._businer15RenderFishbone()}}</div>
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._businer15RootCauses.map((rc, i) => html`
          ...
          ` : this._businer15ActiveTab === 'impact' ? html`
            <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:10px">
              ...
              <tbody>${this._businer15ImpactMatrix.map(imp => html`
          ...
          ` : this._businer15ActiveTab === 'actions' ? html`
            <div style="display:flex;gap:12px;margin-bottom:8px;font-size:10px;color:#888">
              ${Object.entries(this._businer15GetActionStats()).map(([k,v]) => html`<span>${k}: <b style="color:#ddd">${v}</b></span>`)}
            </div>
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._businer15Actions.map(a => html`
                <div style="display:flex;gap:8px;align-items:center;padding:6px;background:#1a1d27;border-radius:4px;cursor:pointer" @click=${() => this._businer15ToggleAction(a.id)}>
          ...
          ` : html`
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._businer15Lessons.map(l => html`
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:4px;margin-bottom:10px;flex-wrap:wrap">
            ${['all', ...this._businer15MaturityLevels.map(m => m.domain)].map(d => html`
              <button class="btn ${this._businer15SelectedDomain === d ? 'btn-primary' : ''}" style="font-size:9px;padding:2px 6px" @click=${() => { this._businer15SelectedDomain = d; }}>${d}</button>
            `)}
          </div>
          <div style="display:flex;gap:16px;margin-bottom:10px">
            <div style="text-align:center"><div style="color:#e0e0e0;font-size:20px;font-weight:bold">${this._businer15GetOverallMaturity()}/5</div><div style="color:#888;font-size:10px">Maturity Level</div></div>
            <div style="text-align:center"><div style="color:#4f4;font-size:20px;font-weight:bold">${this._businer15GetGapAnalysis().filter(g => g.isAbove).length}/${this._businer15Benchmarks.length}</div><div style="color:#888;font-size:10px">Above Industry</div></div>
          </div>
          <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:10px">
            ...
            <tbody>${this._businer15GetGapAnalysis().map(b => html`
          ...
          <div style="margin-top:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Maturity by Domain</div>
            ${this._businer15MaturityLevels.filter(m => this._businer15SelectedDomain === 'all' || m.domain === this._businer15SelectedDomain).map(m => html`
          ...
          <div style="margin-top:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Quarterly Trend</div>
            <div style="display:flex;height:40px;align-items:flex-end;gap:4px">
              ${this._businer15QuarterlyData.map(q => html`<div style="flex:1;text-align:center"><div style="background:#4a9eff;height:${q.score * 0.5}px;border-radius:2px 2px 0 0" title="${q.score}"></div><div style="color:#666;font-size:8px;margin-top:2px">${q.quarter}</div></div>`)}
            </div>
          </div>
        </div>
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:12px;margin-bottom:8px;font-size:10px">
            <div style="display:flex;gap:4px;flex-wrap:wrap">${this._businer15GroupAlerts().map(g => html`<span style="color:#888;padding:2px 6px;background:#1a1d27;border-radius:3px">${g.group}: ${g.count}</span>`)}</div>
          </div>
          <div style="display:flex;gap:8px;margin-bottom:8px;font-size:10px;color:#888">
            <span>FP Rate: <b style="color:${this._businer15QualityMetrics.fpRate > 0.15 ? '#f44' : '#4f4'}">${(this._businer15QualityMetrics.fpRate * 100).toFixed(1)}%</b></span>
            <span>Enrich: <b style="color:#4a9eff">${(this._businer15QualityMetrics.enrichSuccess * 100).toFixed(0)}%</b></span>
            <span>Avg Triage: <b style="color:#ddd">${this._businer15QualityMetrics.avgTriageTime}m</b></span>
          </div>
          <div style="display:flex;flex-direction:column;gap:4px;max-height:200px;overflow-y:auto">
            ${this._businer15Alerts.sort((a, b) => this._businer15CalcScore(b) - this._businer15CalcScore(a)).map(a => {
              const score = this._businer15CalcScore(a);
              return html`
                <div style="padding:6px;background:#1a1d27;border-radius:4px;border-left:3px solid ${a.severity >= 4 ? '#f44' : a.severity >= 3 ? '#fa0' : '#4a9eff'}">
                  <div style="display:flex;justify-content:space-between;align-items:center">
                    ...
                    <button class="btn" style="font-size:9px;padding:1px 6px;margin-left:4px" @click=${() => this._businer15EnrichAlert(a.id)}>${a.enriched ? 'Re-enrich' : 'Enrich'}</button>
                  </div>
          ...
          <div style="margin-top:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Routing Rules</div>
            ${this._businer15RoutingRules.map(r => html`
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap">
            ${this._businer15Shapes.map(s => {
              const v = this._businer15ValidateControl(s.id);
              return html`<div style="padding:4px 8px;background:#1a1d27;border-radius:3px;cursor:pointer;border:1px solid ${this._businer15SelectedShape === s.id ? '#4a9eff' : '#2a2d37'}" @click=${() => { this._businer15SelectedShape = this._businer15SelectedShape === s.id ? '' : s.id; }}>
          ...
          ${this._businer15SelectedShape ? html`
            ${(() => {
              const shape = this._businer15Shapes.find(s => s.id === this._businer15SelectedShape);
              const v = this._businer15ValidateControl(this._businer15SelectedShape);
              return shape ? html`
          ...
          <div style="margin-bottom:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Trust Boundaries</div>
            ${this._businer15TrustBoundaries.map(tb => html`
              <div style="display:flex;gap:6px;align-items:center;padding:3px;background:#1a1d27;border-radius:3px;margin-bottom:2px;font-size:9px">
                <span style="color:#ddd">${this._businer15Shapes.find(s => s.id === tb.from)?.label || tb.from}</span>
                <span style="color:#4a9eff">\u2194</span>
                <span style="color:#ddd">${this._businer15Shapes.find(s => s.id === tb.to)?.label || tb.to}</span>
          ...
          <div><div style="color:#888;font-size:10px;margin-bottom:4px">Architecture Decision Records</div>
            ${this._businer15ADRs.map(adr => html`
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap">
            ${this._businer15Gauges.map(g => html`
          ...
          <div style="display:flex;gap:8px;margin-bottom:8px;align-items:center">
            ...
            <span style="color:${this._businer15GetOverallHealth().score >= 99 ? '#4f4' : '#fa0'};font-size:12px;font-weight:bold">${this._businer15GetOverallHealth().score}%</span>
            <span style="color:#888;font-size:10px">(Target: ${this._businer15SlaTarget}%)</span>
            <span style="color:#4f4;font-size:10px">${this._businer15GetOverallHealth().healthy} healthy</span>
            <span style="color:#fa0;font-size:10px">${this._businer15GetOverallHealth().degraded} degraded</span>
          </div>
          <div style="margin-bottom:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Anomaly Stream</div>
            ${this._businer15Anomalies.slice(0, 3).map(a => html`
              <div style="display:flex;gap:6px;align-items:center;padding:4px;background:#1a1d27;border-radius:3px;margin-bottom:2px;opacity:${a.acknowledged ? 0.5 : 1}">
                ...
                ${!a.acknowledged ? html`<button class="btn" style="font-size:8px;padding:1px 4px" @click=${() => this._businer15AckAnomaly(a.id)}>ACK</button>` : html`<span style="color:#4f4;font-size:9px">ACK'd</span>`}
              </div>
            `)}
          </div>
          <div style="margin-bottom:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Integration Health</div>
            ${this._businer15Integrations.map(i => html`
              <div style="display:flex;gap:6px;align-items:center;padding:3px;background:#1a1d27;border-radius:3px;margin-bottom:2px;font-size:9px">
          ...
          <div><div style="color:#888;font-size:10px;margin-bottom:4px">Alert Fatigue Analysis</div>
            <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:9px">
              ...
              <tbody>${this._businer15AlertFatigue.map(af => html`
  ...
  private _businer15RenderFishbone(): string {
  ...
  private _businer15ToggleAction(id: string) {
    this._businer15Actions = this._businer15Actions.map(a => a.id === id ? {...a, status: a.status === 'pending' ? 'in_progress' : a.status === 'in_progress' ? 'completed' : 'pending'} : a);
  }

  private _businer15GetActionStats() {
    const total = this._businer15Actions.length;
    const done = this._businer15Actions.filter(a => a.status === 'completed').length;
    const inProg = this._businer15Actions.filter(a => a.status === 'in_progress').length;
    return { total, done, inProg, pending: total - done - inProg };
  }

  private _businer15GetOverallMaturity(): number {
    if (this._businer15SelectedDomain === 'all') {
      return Math.round(this._businer15MaturityLevels.reduce((s, m) => s + m.current, 0) / this._businer15MaturityLevels.length * 10) / 10;
    }
    const d = this._businer15MaturityLevels.find(m => m.domain === this._businer15SelectedDomain);
    return d ? d.current : 0;
  }

  private _businer15GetGapAnalysis() {
    return this._businer15Benchmarks.map(b => {
  ...
  private _businer15CalcScore(alert: any): number {
    return Math.round(alert.severity * alert.confidence * alert.assetCrit * 100) / 100;
  }

  private _businer15EnrichAlert(id: string) {
    this._businer15Alerts = this._businer15Alerts.map(a => {
      if (a.id !== id) return a;
      const score = this._businer15CalcScore(a);
  ...
  private _businer15GroupAlerts() {
    const groups: Record<string, number> = {};
    this._businer15Alerts.forEach(a => { groups[a.group] = (groups[a.group] || 0) + 1; });
    return Object.entries(groups).map(([g, c]) => ({group: g, count: c}));
  }

  private _businer15ValidateControl(shapeId: string): {valid:boolean;missing:string[]} {
    const shape = this._businer15Shapes.find(s => s.id === shapeId);
  ...
  private _businer15GetOverallHealth(): {healthy:number;degraded:number;down:number;score:number} {
    const online = this._businer15Integrations.filter(i => i.status === 'online').length;
    const degraded = this._businer15Integrations.filter(i => i.status === 'degraded').length;
    const total = this._businer15Integrations.length;
    return { healthy: online, degraded, down: total - online - degraded, score: Math.round(online / total * 100) };
  }

  private _businer15AckAnomaly(id: string) {
    this._businer15Anomalies = this._businer15Anomalies.map(a => a.id === id ? {...a, acknowledged: true} : a);
  }
...
// ===== DATA SOVEREIGNTY COMPLIANCE (R22) =====
  ...
  private _renderBudgetPlanning() {
    const budgetData = [
      { category: "Personnel & Training", planned: 3797000, actual: 1405000, utilization: 37.0, q1: "18%", q2: "23%", q3: "21%", q4: "13%" },
      { category: "Tooling & Licensing", planned: 3804000, actual: 1418000, utilization: 37.3, q1: "21%", q2: "28%", q3: "32%", q4: "14%" },
      { category: "Infrastructure Security", planned: 3811000, actual: 1431000, utilization: 37.5, q1: "24%", q2: "33%", q3: "27%", q4: "15%" },
      { category: "Compliance & Audit", planned: 3818000, actual: 1444000, utilization: 37.8, q1: "27%", q2: "22%", q3: "22%", q4: "16%" },
      { category: "Incident Response", planned: 3825000, actual: 1457000, utilization: 38.1, q1: "30%", q2: "27%", q3: "33%", q4: "17%" },
      { category: "Third-Party Assessments", planned: 3832000, actual: 1470000, utilization: 38.4, q1: "17%", q2: "32%", q3: "28%", q4: "18%" },
      { category: "Security Awareness", planned: 3839000, actual: 1483000, utilization: 38.6, q1: "20%", q2: "21%", q3: "23%", q4: "19%" },
      { category: "Research & Innovation", planned: 3846000, actual: 1496000, utilization: 38.9, q1: "23%", q2: "26%", q3: "18%", q4: "20%" },
    ];
    ...
    const headcount = [
      { team: "SOC Tier 1", current: 7, target: 18, gap: 3, avgSalary: "88k" },
      { team: "SOC Tier 2", current: 16, target: 17, gap: 2, avgSalary: "117k" },
      { team: "Threat Intel", current: 8, target: 16, gap: 1, avgSalary: "146k" },
      { team: "Red Team", current: 17, target: 15, gap: 0, avgSalary: "175k" },
      { team: "GRC", current: 9, target: 14, gap: 5, avgSalary: "93k" },
      { team: "AppSec", current: 18, target: 13, gap: 4, avgSalary: "122k" },
      { team: "Cloud Sec", current: 10, target: 12, gap: 3, avgSalary: "151k" },
      { team: "Identity & Access", current: 2, target: 11, gap: 2, avgSalary: "180k" },
    ];
    const vendorSpend = [
      { vendor: "CrowdStrike", annual: "334k", contractEnd: "2026-04", renewalRisk: "Low", satisfaction: 3 },
      { vendor: "Palo Alto", annual: "365k", contractEnd: "2026-05", renewalRisk: "Medium", satisfaction: 5 },
      { vendor: "Splunk", annual: "396k", contractEnd: "2026-06", renewalRisk: "High", satisfaction: 4 },
      { vendor: "Qualys", annual: "427k", contractEnd: "2026-07", renewalRisk: "Low", satisfaction: 3 },
      { vendor: "Rapid7", annual: "458k", contractEnd: "2026-08", renewalRisk: "Medium", satisfaction: 5 },
      { vendor: "Mandiant", annual: "489k", contractEnd: "2026-09", renewalRisk: "High", satisfaction: 4 },
      { vendor: "Zscaler", annual: "520k", contractEnd: "2026-10", renewalRisk: "Low", satisfaction: 3 },
      { vendor: "Duo Security", annual: "551k", contractEnd: "2026-11", renewalRisk: "Medium", satisfaction: 5 },
    ];
    const roiProjections = [
      { area: "Threat Detection", investment: "865k", projectedReturn: "902k", roiMultiple: "2.2x", confidence: 87 },
      { area: "Incident Reduction", investment: "107k", projectedReturn: "949k", roiMultiple: "2.1x", confidence: 68 },
      { area: "Compliance Savings", investment: "150k", projectedReturn: "996k", roiMultiple: "2.0x", confidence: 85 },
      { area: "Automation Gains", investment: "193k", projectedReturn: "1043k", roiMultiple: "1.9x", confidence: 66 },
      { area: "Risk Avoidance", investment: "236k", projectedReturn: "1090k", roiMultiple: "1.8x", confidence: 83 },
    ];
    return html`
  ...
  private _renderMetricsNormalization() {
    const kpiCatalog = [
      { id: "kpi-1", name: "MTTD", owner: "SOC", unit: "minutes", target: 94, current: 63, benchmark: 83, collection: "auto", frequency: "realtime" },
      { id: "kpi-2", name: "MTTR", owner: "GRC", unit: "%", target: 97, current: 70, benchmark: 94, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-3", name: "MTTC", owner: "AppSec", unit: "score", target: 70, current: 77, benchmark: 71, collection: "manual", frequency: "weekly" },
      { id: "kpi-4", name: "Vuln SLA Compliance", owner: "Cloud Sec", unit: "count", target: 73, current: 84, benchmark: 82, collection: "auto", frequency: "monthly" },
      { id: "kpi-5", name: "Patch Coverage", owner: "Identity", unit: "days", target: 76, current: 91, benchmark: 93, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-6", name: "Phishing Click Rate", owner: "Threat Intel", unit: "minutes", target: 79, current: 98, benchmark: 70, collection: "manual", frequency: "daily" },
      { id: "kpi-7", name: "Training Completion", owner: "Security Ops", unit: "%", target: 82, current: 59, benchmark: 81, collection: "auto", frequency: "weekly" },
      { id: "kpi-8", name: "Escalation Rate", owner: "Risk Mgmt", unit: "score", target: 85, current: 66, benchmark: 92, collection: "semi-auto", frequency: "monthly" },
      { id: "kpi-9", name: "False Positive Rate", owner: "SOC", unit: "count", target: 88, current: 73, benchmark: 69, collection: "manual", frequency: "realtime" },
      { id: "kpi-10", name: "Threat Intel Actionability", owner: "GRC", unit: "days", target: 91, current: 80, benchmark: 80, collection: "auto", frequency: "daily" },
      { id: "kpi-11", name: "Endpoint Compliance", owner: "AppSec", unit: "minutes", target: 94, current: 87, benchmark: 91, collection: "semi-auto", frequency: "weekly" },
      { id: "kpi-12", name: "Cloud Misconfig Score", owner: "Cloud Sec", unit: "%", target: 97, current: 94, benchmark: 68, collection: "manual", frequency: "monthly" },
      { id: "kpi-13", name: "Identity Anomaly Rate", owner: "Identity", unit: "score", target: 70, current: 55, benchmark: 79, collection: "auto", frequency: "realtime" },
      { id: "kpi-14", name: "DLP Events", owner: "Threat Intel", unit: "count", target: 73, current: 62, benchmark: 90, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-15", name: "Vendor Risk Avg", owner: "Security Ops", unit: "days", target: 76, current: 69, benchmark: 67, collection: "manual", frequency: "weekly" },
      { id: "kpi-16", name: "Compliance Audit Pass Rate", owner: "Risk Mgmt", unit: "minutes", target: 79, current: 76, benchmark: 78, collection: "auto", frequency: "monthly" },
      { id: "kpi-17", name: "Awareness Score", owner: "SOC", unit: "%", target: 82, current: 83, benchmark: 89, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-18", name: "SOC Utilization", owner: "GRC", unit: "score", target: 85, current: 90, benchmark: 66, collection: "manual", frequency: "daily" },
      { id: "kpi-19", name: "Automation Coverage", owner: "AppSec", unit: "count", target: 88, current: 97, benchmark: 77, collection: "auto", frequency: "weekly" },
      { id: "kpi-20", name: "Risk Register Currency", owner: "Cloud Sec", unit: "days", target: 91, current: 58, benchmark: 88, collection: "semi-auto", frequency: "monthly" },
    ];
    const benchmarkSources = [
      { source: "NIST CSF", mappedKPIs: 3, alignment: 78, lastReview: "2026-01-27", status: "aligned" },
      { source: "CIS Controls v8", mappedKPIs: 4, alignment: 95, lastReview: "2026-02-22", status: "partial" },
      { source: "ISO 27001:2022", mappedKPIs: 5, alignment: 73, lastReview: "2026-03-17", status: "reviewing" },
      { source: "PCI DSS 4.0", mappedKPIs: 6, alignment: 90, lastReview: "2026-04-12", status: "aligned" },
      { source: "SOC 2 Type II", mappedKPIs: 7, alignment: 68, lastReview: "2026-05-07", status: "partial" },
      { source: "MITRE ATT&CK", mappedKPIs: 8, alignment: 85, lastReview: "2026-06-02", status: "reviewing" },
      { source: "SANS Top 20", mappedKPIs: 3, alignment: 63, lastReview: "2026-01-25", status: "aligned" },
      { source: "OWASP Top 10", mappedKPIs: 4, alignment: 80, lastReview: "2026-02-20", status: "partial" },
    ];
  ...
  private _renderThreatHuntingCampaigns() {
    const campaigns = [
      { id: "HC-1001", name: "Lateral Movement Sweep", status: "active", hypothesis: "H1: Actors using pass-the-hash for lateral movement", leadHunter: "Alice Chen", findings: 34, startDate: "2026-03-19", endDate: null, effectiveness: 85 },
      { id: "HC-1002", name: "Credential Harvesting Hunt", status: "completed", hypothesis: "H2: Actors using web shells for persistence", leadHunter: "Bob Martinez", findings: 37, startDate: "2026-02-02", endDate: "2026-06-08", effectiveness: 45 },
      { id: "HC-1003", name: "Persistence Mechanism Audit", status: "planned", hypothesis: "H3: Actors using scheduled tasks for data theft", leadHunter: "Carol Wu", findings: 40, startDate: "2026-01-13", endDate: null, effectiveness: 64 },
      { id: "HC-1004", name: "C2 Beacon Detection", status: "in-review", hypothesis: "H4: Actors using DNS tunneling for C2 communication", leadHunter: "Dave Kim", findings: 43, startDate: "2026-04-24", endDate: null, effectiveness: 83 },
      { id: "HC-1005", name: "Data Exfiltration Patterns", status: "active", hypothesis: "H5: Actors using encrypted channels for privilege escalation", leadHunter: "Eve Johnson", findings: 46, startDate: "2026-03-07", endDate: null, effectiveness: 43 },
      { id: "HC-1006", name: "Privilege Escalation Scan", status: "completed", hypothesis: "H6: Actors using token impersonation for defense evasion", leadHunter: "Frank Liu", findings: 1, startDate: "2026-02-18", endDate: "2026-04-20", effectiveness: 62 },
      { id: "HC-1007", name: "Supply Chain Implant Hunt", status: "planned", hypothesis: "H7: Actors using poisoned images for initial access", leadHunter: "Grace Park", findings: 4, startDate: "2026-01-01", endDate: null, effectiveness: 81 },
      { id: "HC-1008", name: "Insider Threat Indicators", status: "in-review", hypothesis: "H8: Actors using legitimate tools for credential access", leadHunter: "Hector Silva", findings: 7, startDate: "2026-04-12", endDate: null, effectiveness: 41 },
      { id: "HC-1009", name: "Cloud Metadata Analysis", status: "active", hypothesis: "H9: Actors using API keys for command execution", leadHunter: "Alice Chen", findings: 10, startDate: "2026-03-23", endDate: null, effectiveness: 60 },
      { id: "HC-1010", name: "DNS Tunnel Detection", status: "completed", hypothesis: "H10: Actors using encoded subdomains for exfiltration", leadHunter: "Bob Martinez", findings: 13, startDate: "2026-02-06", endDate: "2026-05-04", effectiveness: 79 },
      { id: "HC-1011", name: "Fileless Malware Search", status: "planned", hypothesis: "H11: Actors using WMI providers for discovery", leadHunter: "Carol Wu", findings: 16, startDate: "2026-01-17", endDate: null, effectiveness: 98 },
      { id: "HC-1012", name: "Zero-Day Exploit Traces", status: "in-review", hypothesis: "H12: Actors using exploit kits for collection", leadHunter: "Dave Kim", findings: 19, startDate: "2026-04-28", endDate: null, effectiveness: 58 },
    ];
    const hunterLeaderboard = [
      { hunter: "Alice Chen", campaigns: 7, findings: 35, highSeverity: 4, avgScore: 65, streak: 3 },
      { hunter: "Bob Martinez", campaigns: 4, findings: 64, highSeverity: 9, avgScore: 58, streak: 4 },
      { hunter: "Carol Wu", campaigns: 14, findings: 93, highSeverity: 14, avgScore: 95, streak: 5 },
      { hunter: "Dave Kim", campaigns: 11, findings: 6, highSeverity: 19, avgScore: 88, streak: 6 },
      { hunter: "Eve Johnson", campaigns: 8, findings: 35, highSeverity: 24, avgScore: 81, streak: 7 },
      { hunter: "Frank Liu", campaigns: 5, findings: 64, highSeverity: 3, avgScore: 74, streak: 8 },
      { hunter: "Grace Park", campaigns: 15, findings: 93, highSeverity: 8, avgScore: 67, streak: 1 },
      { hunter: "Hector Silva", campaigns: 12, findings: 6, highSeverity: 13, avgScore: 60, streak: 2 },
    ];
    const mitreMapping = [
      { tactic: "Initial Access", techniques: 12, campaigns: 5, coverage: 86 },
      { tactic: "Execution", techniques: 11, campaigns: 4, coverage: 66 },
      { tactic: "Persistence", techniques: 10, campaigns: 3, coverage: 46 },
      { tactic: "Privilege Escalation", techniques: 9, campaigns: 2, coverage: 97 },
      { tactic: "Defense Evasion", techniques: 8, campaigns: 1, coverage: 77 },
      { tactic: "Credential Access", techniques: 7, campaigns: 6, coverage: 57 },
      { tactic: "Discovery", techniques: 6, campaigns: 5, coverage: 37 },
      { tactic: "Lateral Movement", techniques: 5, campaigns: 4, coverage: 88 },
      { tactic: "Collection", techniques: 4, campaigns: 3, coverage: 68 },
      { tactic: "Exfiltration", techniques: 3, campaigns: 2, coverage: 48 },
      { tactic: "Command & Control", techniques: 2, campaigns: 1, coverage: 99 },
      { tactic: "Impact", techniques: 12, campaigns: 6, coverage: 79 },
    ];
    return html`
  ...
  private _renderControlInventory() {
    const controls = [
      { id: "CTL-2001", name: "MFA Enforcement", domain: "Access Control", status: "implemented", effectiveness: 82, lastTest: "2026-03-11", nextReview: "2026-07-11", owner: "SOC", risk: "Low" },
      { id: "CTL-2002", name: "Network Segmentation", domain: "Network Security", status: "partial", effectiveness: 36, lastTest: "2026-02-24", nextReview: "2026-08-02", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2003", name: "EDR Deployment", domain: "Endpoint Protection", status: "planned", effectiveness: 43, lastTest: "2026-01-09", nextReview: "2026-09-21", owner: "IT Ops", risk: "High" },
      { id: "CTL-2004", name: "DLP Policy", domain: "Data Protection", status: "gap", effectiveness: 50, lastTest: "2026-04-22", nextReview: "2026-10-12", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2005", name: "SSO Integration", domain: "Identity Management", status: "implemented", effectiveness: 94, lastTest: "2026-03-07", nextReview: "2026-11-03", owner: "IAM", risk: "Low" },
      { id: "CTL-2006", name: "SAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 13, lastTest: "2026-02-20", nextReview: "2026-12-22", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2007", name: "CSPM Scanning", domain: "Cloud Security", status: "planned", effectiveness: 20, lastTest: "2026-01-05", nextReview: "2026-05-13", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2008", name: "Badge Access", domain: "Physical Security", status: "gap", effectiveness: 27, lastTest: "2026-04-18", nextReview: "2026-06-04", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2009", name: "Least Privilege", domain: "Access Control", status: "implemented", effectiveness: 47, lastTest: "2026-03-03", nextReview: "2026-07-23", owner: "SOC", risk: "Low" },
      { id: "CTL-2010", name: "Firewall Rules", domain: "Network Security", status: "partial", effectiveness: 41, lastTest: "2026-02-16", nextReview: "2026-08-14", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2011", name: "Disk Encryption", domain: "Endpoint Protection", status: "planned", effectiveness: 48, lastTest: "2026-01-01", nextReview: "2026-09-05", owner: "IT Ops", risk: "High" },
      { id: "CTL-2012", name: "Data Classification", domain: "Data Protection", status: "gap", effectiveness: 4, lastTest: "2026-04-14", nextReview: "2026-10-24", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2013", name: "PAM Implementation", domain: "Identity Management", status: "implemented", effectiveness: 59, lastTest: "2026-03-27", nextReview: "2026-11-15", owner: "IAM", risk: "Low" },
      { id: "CTL-2014", name: "DAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 18, lastTest: "2026-02-12", nextReview: "2026-12-06", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2015", name: "IAM Policy Review", domain: "Cloud Security", status: "planned", effectiveness: 25, lastTest: "2026-01-25", nextReview: "2026-05-25", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2016", name: "Visitor Management", domain: "Physical Security", status: "gap", effectiveness: 32, lastTest: "2026-04-10", nextReview: "2026-06-16", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2017", name: "Access Reviews", domain: "Access Control", status: "implemented", effectiveness: 71, lastTest: "2026-03-23", nextReview: "2026-07-07", owner: "SOC", risk: "Low" },
      { id: "CTL-2018", name: "IDS/IPS Tuning", domain: "Network Security", status: "partial", effectiveness: 46, lastTest: "2026-02-08", nextReview: "2026-08-26", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2019", name: "Patch Management", domain: "Endpoint Protection", status: "planned", effectiveness: 2, lastTest: "2026-01-21", nextReview: "2026-09-17", owner: "IT Ops", risk: "High" },
      { id: "CTL-2020", name: "Backup Encryption", domain: "Data Protection", status: "gap", effectiveness: 9, lastTest: "2026-04-06", nextReview: "2026-10-08", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2021", name: "Password Policy", domain: "Identity Management", status: "implemented", effectiveness: 83, lastTest: "2026-03-19", nextReview: "2026-11-27", owner: "IAM", risk: "Low" },
      { id: "CTL-2022", name: "Container Scanning", domain: "Application Security", status: "partial", effectiveness: 23, lastTest: "2026-02-04", nextReview: "2026-12-18", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2023", name: "WAF Configuration", domain: "Cloud Security", status: "planned", effectiveness: 30, lastTest: "2026-01-17", nextReview: "2026-05-09", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2024", name: "CCTV Coverage", domain: "Physical Security", status: "gap", effectiveness: 37, lastTest: "2026-04-02", nextReview: "2026-06-28", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2025", name: "RBAC Enforcement", domain: "Access Control", status: "implemented", effectiveness: 95, lastTest: "2026-03-15", nextReview: "2026-07-19", owner: "SOC", risk: "Low" },
      { id: "CTL-2026", name: "VPN Management", domain: "Network Security", status: "partial", effectiveness: 0, lastTest: "2026-02-28", nextReview: "2026-08-10", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2027", name: "App Whitelisting", domain: "Endpoint Protection", status: "planned", effectiveness: 7, lastTest: "2026-01-13", nextReview: "2026-09-01", owner: "IT Ops", risk: "High" },
      { id: "CTL-2028", name: "Key Management", domain: "Data Protection", status: "gap", effectiveness: 14, lastTest: "2026-04-26", nextReview: "2026-10-20", owner: "Data Gov", risk: "Critical" },
    ];
    const gapAnalysis = [
  ...
  private _renderIncidentCostTracker() {
    const incidents = [
      { id: "INC-7001", name: "Security Incident 1", severity: "Critical", totalCost: 1743000, responseCost: 418320, recoveryCost: 505470, legalCost: 122010, regulatoryCost: 226590, insuranceClaim: 0, avoidedCost: 471000, date: "2026-03-03" },
      { id: "INC-7002", name: "Security Incident 2", severity: "High", totalCost: 1746000, responseCost: 541260, recoveryCost: 698400, legalCost: 349200, regulatoryCost: 139680, insuranceClaim: 1082520, avoidedCost: 500000, date: "2026-02-12" },
      { id: "INC-7003", name: "Security Incident 3", severity: "Medium", totalCost: 1749000, responseCost: 297330, recoveryCost: 524700, legalCost: 297330, regulatoryCost: 244860, insuranceClaim: 0, avoidedCost: 33000, date: "2026-01-21" },
      { id: "INC-7004", name: "Security Incident 4", severity: "Low", totalCost: 1752000, responseCost: 420480, recoveryCost: 350400, legalCost: 245280, regulatoryCost: 157680, insuranceClaim: 998640, avoidedCost: 62000, date: "2026-04-02" },
      { id: "INC-7005", name: "Security Incident 5", severity: "Critical", totalCost: 1755000, responseCost: 544050, recoveryCost: 544050, legalCost: 193050, regulatoryCost: 263250, insuranceClaim: 0, avoidedCost: 91000, date: "2026-03-11" },
      { id: "INC-7006", name: "Security Incident 6", severity: "High", totalCost: 1758000, responseCost: 298860, recoveryCost: 369180, legalCost: 140640, regulatoryCost: 175800, insuranceClaim: 914160, avoidedCost: 120000, date: "2026-02-20" },
      { id: "INC-7007", name: "Security Incident 7", severity: "Medium", totalCost: 1761000, responseCost: 422640, recoveryCost: 563520, legalCost: 88050, regulatoryCost: 88050, insuranceClaim: 0, avoidedCost: 149000, date: "2026-01-01" },
      { id: "INC-7008", name: "Security Incident 8", severity: "Low", totalCost: 1764000, responseCost: 546840, recoveryCost: 388080, legalCost: 317520, regulatoryCost: 194040, insuranceClaim: 829080, avoidedCost: 178000, date: "2026-04-10" },
      { id: "INC-7009", name: "Security Incident 9", severity: "Critical", totalCost: 1767000, responseCost: 300390, recoveryCost: 583110, legalCost: 265050, regulatoryCost: 106020, insuranceClaim: 0, avoidedCost: 207000, date: "2026-03-19" },
      { id: "INC-7010", name: "Security Incident 10", severity: "High", totalCost: 1770000, responseCost: 424800, recoveryCost: 407100, legalCost: 212400, regulatoryCost: 212400, insuranceClaim: 743400, avoidedCost: 236000, date: "2026-02-28" },
      { id: "INC-7011", name: "Security Incident 11", severity: "Medium", totalCost: 1773000, responseCost: 549630, recoveryCost: 602820, legalCost: 159570, regulatoryCost: 124110, insuranceClaim: 0, avoidedCost: 265000, date: "2026-01-09" },
      { id: "INC-7012", name: "Security Incident 12", severity: "Low", totalCost: 1776000, responseCost: 301920, recoveryCost: 426240, legalCost: 106560, regulatoryCost: 230880, insuranceClaim: 657120, avoidedCost: 294000, date: "2026-04-18" },
    ];
    const yearlyTrend = [
      { month: "Jan", incidents: 10, totalCost: "578k", avgCost: "156k", insured: 29 },
      { month: "Feb", incidents: 7, totalCost: "621k", avgCost: "22k", insured: 29 },
      { month: "Mar", incidents: 4, totalCost: "664k", avgCost: "69k", insured: 29 },
      { month: "Apr", incidents: 12, totalCost: "707k", avgCost: "116k", insured: 29 },
      { month: "May", incidents: 9, totalCost: "750k", avgCost: "163k", insured: 29 },
      { month: "Jun", incidents: 6, totalCost: "793k", avgCost: "29k", insured: 29 },
    ];
