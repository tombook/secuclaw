...
@customElement('sc-log-query')
export class ScLogQuery extends LitElement {
  static styles = css`
...
@state() private _tablePage = 1;
  ...
  @state() private _comments: CommentEntry[] = [
    { id: 'c1', author: 'soc-tier1', timestamp: '2026-04-23 04:00', text: 'Initial assessment started for Log Query finding #1' },
  ...
  private _items: PanelItem[] = [
      {
            ...
            "category": "Security",
            "source": "SIEM",
            "description": "Query: source=auth AND action=failed | Results: 1,247 entries | Top source: 45.33.32.1 (547)",
      ...
      },
      {
            ...
            "category": "Network",
            "source": "EDR",
            "description": "Query: event_id=4672 OR event_id=4673 | Results: 89 entries | Anomaly: 3 off-hours escalations",
      ...
      {
            "id": "log-query-3",
            "title": "Data Access Anomalies",
            "severity": "medium",
            "status": "resolved",
            "category": "Access",
            "source": "Scanner",
            "description": "Query: resource=customer_db AND volume>1GB | Results: 12 entries | 2 after-hours bulk exports",
      ...
      },
      {
            ...
            "category": "Compliance",
            "source": "Audit",
            "description": "Query: proto=SMB AND dst!=fileserver | Results: 34 entries | 4 cross-segment connections flagged",
      ...
      {
            "id": "log-query-5",
            "title": "Log Query Finding #5",
            "severity": "medium",
            "status": "open",
            "category": "Operations",
            "source": "Manual",
            "description": "Automated log query assessment finding detected during routine scan of infrastructure component. Review and remediation recommended.",
  ...
  private _history: HistoryEntry[] = [
    { timestamp: '2026-04-23 04:15', action: 'Auto-correlated', user: 'system', details: 'Grouped related findings from Log Query analysis' },
    { timestamp: '2026-04-23 03:00', action: 'Created', user: 'scanner', details: 'New Log Query finding detected by automated assessment' },
    ...
    { timestamp: '2026-04-22 22:00', action: 'Resolved', user: 'security-eng', details: 'Remediation applied and verified for Log Query finding' },
    { timestamp: '2026-04-22 18:00', action: 'Created', user: 'audit', details: 'Compliance audit identified Log Query gap requiring remediation' },
    ...
    { timestamp: '2026-04-21 16:00', action: 'Resolved', user: 'soc-tier1', details: 'False positive confirmed - benign activity flagged by Log Query scan' },
    { timestamp: '2026-04-21 12:00', action: 'Exported', user: 'compliance', details: 'Exported Log Query findings for Q1 compliance report' },
  ];
...
private _workflowTasks: WorkflowTask[] = [
    { id: 'wt-1', title: 'Review Log Query finding #1', status: 'active', assignee: 'soc-tier1', blockedBy: [], slaDeadline: '2026-04-23T12:00:00Z', priority: 'p1', createdAt: '2026-04-23T00:00:00Z', completedAt: null },
    ...
    { id: 'wt-3', title: 'Update Log Query detection rules', status: 'blocked', assignee: 'soc-tier2', blockedBy: ['wt-2'], slaDeadline: '2026-04-24T00:00:00Z', priority: 'p2', createdAt: '2026-04-23T03:00:00Z', completedAt: null },
    ...
    { id: 'wt-8', title: 'Archive resolved Log Query findings', status: 'failed', assignee: 'soc-tier1', blockedBy: [], slaDeadline: '2026-04-23T08:00:00Z', priority: 'p4', createdAt: '2026-04-22T22:00:00Z', completedAt: null },
  ];

private _executionHistory: ExecutionRecord[] = [
    {
      id: 'exec-1', name: 'Log Query Full Assessment Run', startedAt: '2026-04-22T10:00:00Z', completedAt: '2026-04-22T10:12:00Z', status: 'success',
      steps: [
    ...
    {
      id: 'exec-2', name: 'Log Query Delta Scan', startedAt: '2026-04-23T02:00:00Z', completedAt: '2026-04-23T02:05:00Z', status: 'failed',
      steps: [
  ...
  private _runExecution() {
    ...
    const exec: ExecutionRecord = {
      id: 'exec-' + Date.now(), name: 'Log Query Assessment ' + new Date().toISOString().slice(0, 10), startedAt: new Date().toISOString(), completedAt: null, status: 'running', steps,
    };
  ...
  private _renderExecutionPanel(): unknown {
    ...
    return html`
      <div class="form-section">
        ...
        ` : html`<div class="empty-state">Click "Run Assessment" to start the Log Query analysis pipeline</div>`}
      </div>
      <div class="form-section">
        ...
        <div class="settings-card">
          ...
          <div style="display:flex;gap:8px;margin-top:4px;">
            <button class="btn primary" @click=${() => {
              ...
              const a = document.createElement('a'); a.href = url; a.download = 'log-query-config.json'; a.click();
    ...
    return html`
      <div style="margin-top:12px;">
        ...
        <div style="display:flex;gap:6px;margin-top:8px;">
          <input class="search-box" type="text" placeholder="Add a comment..." style="min-width:auto;flex:1;" id="log-query-comment-input" />
          <button class="btn primary" @click=${() => {
            const input = this.shadowRoot?.querySelector('#log-query-comment-input') as HTMLInputElement;
  ...
  private _exportJSON() {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'sc-log-query-data.json'; a.click();
    URL.revokeObjectURL(url);
  }
  ...
  private _exportCSV() {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'sc-log-query-data.csv'; a.click();
    URL.revokeObjectURL(url);
  }


  // --- Domain Rules Engine ---
  @state() private _lqRules: { id: string; name: string; category: string; severity: Severity; enabled: boolean; lastEval: string; passRate: number }[] = [];
  private _initLqRules() {
    const rules = [
  ...
  private _evaluateLqRules(): { passed: number; failed: number; skipped: number; total: number } {
    let passed = 0, failed = 0, skipped = 0;
    this._lqRules.forEach(r => { if (!r.enabled) { skipped++; } else if (r.passRate >= 80) { passed++; } else { failed++; } });
    return { passed, failed, skipped, total: this._lqRules.length };
  }

  // --- CVSS Scoring ---
  @state() private _lqcvssData: { itemId: string; vector: string; base: number; temporal: number; environmental: number; overall: number }[] = [];
  private _initLqCvss() {
    ...
    this._lqcvssData = vectors.map((v, i) => {
  ...
  // --- Anomaly Detection ---
  @state() private _lqanomalies: { id: string; type: string; severity: Severity; description: string; detected: string; confidence: number; affected: string[] }[] = [];
  private _runLqAnomalyDetection() {
    const types = [
    ...
    this._lqanomalies = types.map((a, i) => ({
  ...
  // --- Trend Prediction ---
  @state() private _lqpredictions: { horizon: string; metric: string; current: number; predicted: number; direction: 'up' | 'down' | 'stable'; confidence: number }[] = [];
  private _generateLqPredictions() {
    this._lqpredictions = [
  ...
  // --- Approval Workflow ---
  @state() private _lqApprovals: { id: string; title: string; requester: string; status: 'pending' | 'approved' | 'rejected' | 'expired'; createdAt: string; priority: Priority; type: string }[] = [];
  private _initLqApprovals() {
    this._lqApprovals = [
  ...
  private _approveLqItem(id: string) { const item = this._lqApprovals.find(a => a.id === id); if (item) item.status = 'approved'; this.requestUpdate(); }
  private _rejectLqItem(id: string) { const item = this._lqApprovals.find(a => a.id === id); if (item) item.status = 'rejected'; this.requestUpdate(); }

  // --- Activity Feed ---
  @state() private _lqActivity: { id: string; action: string; user: string; target: string; timestamp: string }[] = [];
  private _initLqActivity() {
    const actions = [
    ...
    this._lqActivity = actions.map((a, i) => ({ id: 'ACT-' + String(i + 1).padStart(3, '0'), ...a, timestamp: new Date(Date.now() - i * 3600000).toISOString() }));
  }

  // --- Notification System ---
  @state() private _lqNotifications: { id: string; message: string; type: 'info' | 'warning' | 'error' | 'success'; read: boolean; timestamp: string }[] = [];
  private _initLqNotifications() {
    this._lqNotifications = [
  ...
  private _markLqNotifRead(id: string) { const n = this._lqNotifications.find(x => x.id === id); if (n) n.read = true; this.requestUpdate(); }

  // --- Panel Configuration ---
  @state() private _lqConfig: { layout: 'compact' | 'default' | 'expanded'; theme: 'dark' | 'midnight' | 'slate'; showAnomalies: boolean; showPredictions: boolean; autoRefresh: boolean; refreshInterval: number } = {
    layout: 'default', theme: 'dark', showAnomalies: true, showPredictions: true, autoRefresh: true, refreshInterval: 60,
  };
  private _lqPresets: { name: string; config: typeof this._lqConfig }[] = [
  ...
  private _applyLqPreset(preset: typeof this._lqPresets[0]) { this._lqConfig = { ...preset.config }; this.requestUpdate(); }

  private _renderLqTreemapSVG(): string {
    const categories = [
  ...
  private _renderLqSankeySVG(): string {
  ...
  // --- Render: Rules Engine ---
  private _renderLqRules(): any {
    const ev = this._evaluateLqRules();
    return html`
  ...
  private _renderLqAnomalies(): any {
  ...
  // --- Render: Approvals ---
  private _renderLqApprovals(): any {
          ...
          $${this._lqApprovals.map(a => html`
            <div style="padding:6px 8px;background:#1f2937;border-radius:4px">
              ...
              $${a.status === 'pending' ? html`
                <div style="display:flex;gap:4px;margin-top:4px">
                  <button class="btn success" style="padding:2px 8px;font-size:9px" @click=$${() => this._approveLqItem(a.id)}>Approve</button>
                  <button class="btn error" style="padding:2px 8px;font-size:9px" @click=$${() => this._rejectLqItem(a.id)}>Reject</button>
                </div>` : nothing}
            </div>`)}
        </div>
      </div>`;
  ...
  private _renderLqNotifications(): any {
    const tc = (t: string) => t === 'error' ? '#ef4444' : t === 'warning' ? '#eab308' : t === 'success' ? '#22c55e' : '#3b82f6';
    const unread = this._lqNotifications.filter(n => !n.read).length;
    return html`
      <div style="background:#1a1d27;border-radius:8px;padding:12px;margin-top:12px">
        ...
        <div style="display:flex;flex-direction:column;gap:4px;max-height:120px;overflow-y:auto">
          $${this._lqNotifications.map(n => html`
            <div style="display:flex;align-items:center;gap:8px;padding:4px 8px;background:$${n.read ? '#1f2937' : '#252a36'};border-radius:4px;font-size:10px;opacity:$${n.read ? '0.6' : '1'};cursor:pointer" @click=$${() => this._markLqNotifRead(n.id)}>
      ...
      <div style="background:#1a1d27;border-radius:8px;padding:12px;margin-top:12px">
        ...
        <div style="display:flex;flex-direction:column;gap:6px;font-size:10px">
          <div style="display:flex;align-items:center;gap:8px">
            ...
            <select class="form-input" style="flex:1" @change=$${(e: Event) => { this._lqConfig.layout = (e.target as HTMLSelectElement).value as any; this.requestUpdate(); }}>
              <option value="compact" ?selected=$${this._lqConfig.layout === 'compact'}>Compact</option>
              <option value="default" ?selected=$${this._lqConfig.layout === 'default'}>Default</option>
              <option value="expanded" ?selected=$${this._lqConfig.layout === 'expanded'}>Expanded</option>
            </select>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:100px">Theme</span>
            <select class="form-input" style="flex:1" @change=$${(e: Event) => { this._lqConfig.theme = (e.target as HTMLSelectElement).value as any; this.requestUpdate(); }}>
              <option value="dark" ?selected=$${this._lqConfig.theme === 'dark'}>Dark</option>
              <option value="midnight" ?selected=$${this._lqConfig.theme === 'midnight'}>Midnight</option>
              <option value="slate" ?selected=$${this._lqConfig.theme === 'slate'}>Slate</option>
            </select>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:100px">Auto Refresh</span>
            <input type="checkbox" ?checked=$${this._lqConfig.autoRefresh} @change=$${() => { this._lqConfig.autoRefresh = !this._lqConfig.autoRefresh; this.requestUpdate(); }}/>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:100px">Show Anomalies</span>
            <input type="checkbox" ?checked=$${this._lqConfig.showAnomalies} @change=$${() => { this._lqConfig.showAnomalies = !this._lqConfig.showAnomalies; this.requestUpdate(); }}/>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:100px">Show Predictions</span>
            <input type="checkbox" ?checked=$${this._lqConfig.showPredictions} @change=$${() => { this._lqConfig.showPredictions = !this._lqConfig.showPredictions; this.requestUpdate(); }}/>
          </div>
          <div style="margin-top:6px;font-weight:600">Presets</div>
          <div style="display:flex;gap:4px">
            $${this._lqPresets.map(ps => html`<button class="btn" style="padding:2px 8px;font-size:9px" @click=$${() => this._applyLqPreset(ps)}>$${ps.name}</button>`)}
          </div>
        </div>
      </div>`;
  }
  ...
  // === Awareness Simulation Data ===
  @state() private _awarenessHistoricalData = [
    { quarter: 'Q3 2025', phishingCTR: 12.5, reportRate: 15.2, completionRate: 72, avgScore: 75 },
    { quarter: 'Q4 2025', phishingCTR: 10.8, reportRate: 18.5, completionRate: 78, avgScore: 79 },
    { quarter: 'Q1 2026', phishingCTR: 7.6, reportRate: 22.8, completionRate: 84, avgScore: 83 },
  ] as any[];
  @state() private _awarenessTemplates = [
    { id: 'TPL-001', name: 'CEO Wire Transfer', type: 'phishing', difficulty: 'medium',
      successRate: 0.08, description: 'Urgent request from CEO to transfer funds to unknown account',
      indicators: ['Urgent tone', 'Direct money request', 'New vendor account', 'Bypass normal process'] },
    { id: 'TPL-002', name: 'IT Password Reset', type: 'vishing', difficulty: 'hard',
      successRate: 0.05, description: 'IT support calls claiming password needs immediate reset',
      indicators: ['Caller ID spoofing', 'Asks for password', 'Creates urgency', 'Refuses callback'] },
    { id: 'TPL-003', name: 'Package Delivery', type: 'smishing', difficulty: 'easy',
      successRate: 0.15, description: 'SMS about undelivered package with tracking link',
      indicators: ['Shortened URL', 'Generic greeting', 'Wrong carrier name', 'Spelling errors'] },
    { id: 'TPL-004', name: 'USB Reward Drive', type: 'physical', difficulty: 'medium',
      successRate: 0.16, description: 'Labeled USB drive left in common area with tempting label',
      indicators: ['Unknown origin', 'Promising label', 'Left in high-traffic area', 'No owner info'] },
    { id: 'TPL-005', name: 'HR Survey Phish', type: 'phishing', difficulty: 'easy',
      successRate: 0.12, description: 'Fake employee satisfaction survey with credential harvest',
      indicators: ['Survey request', 'External link', 'Asks for credentials', 'Non-HR sender'] },
    { id: 'TPL-006', name: 'Badge Scanner Clone', type: 'physical', difficulty: 'hard',
      successRate: 0.04, description: 'Fake badge scanner at building entrance to clone access cards',
      indicators: ['Unfamiliar device', 'No security personnel', 'Unusual location', 'Missing branding'] },
  ] as any[];
  @state() private _awarenessBenchmark = { industryAvgCTR: 15.0, topPerformerCTR: 3.0, targetCTR: 5.0, currentCTR: 7.6 };

  private _calculateAwarenessTrend(): { direction: string; improvement: number } {
    const data = this._awarenessHistoricalData;
    if (data.length < 2) return { direction: 'insufficient-data', improvement: 0 };
    const latest = data[data.length - 1].phishingCTR;
    const previous = data[data.length - 2].phishingCTR;
    const improvement = Math.round((previous - latest) / previous * 10000) / 100;
    return { direction: improvement > 0 ? 'improving' : 'declining', improvement };
  }

  private _assessTrainingGaps(): string[] {
    const gaps: string[] = [];
    for (const mod of this._awarenessTrainingModules) {
      if (mod.completionRate < 0.7) gaps.push(mod.name + ': completion rate ' + Math.round(mod.completionRate * 100) + '% below 70% threshold');
      if (mod.avgScore < 75) gaps.push(mod.name + ': average score ' + mod.avgScore + ' below 75 threshold');
    }
    return gaps;
  }

  private _predictNextQuarterCTR(): number {
    const data = this._awarenessHistoricalData;
    if (data.length < 2) return this._awarenessBenchmark.currentCTR;
    const avgDecline = (data[0].phishingCTR - data[data.length - 1].phishingCTR) / (data.length - 1);
    const predicted = data[data.length - 1].phishingCTR - avgDecline * 0.8;
    return Math.round(Math.max(2, predicted) * 10) / 10;
  }
...
@state() private _spmComplianceCheck6 = { id: "CHK-006", control: "Control 6", framework: "NIST 800-53", status: "pass", evidenceCount: 4, lastAudit: "2026-01-11", nextAudit: "2026-04-11", findings: 2 } as any;
  ...
  @state() private _logQueryCourses: Array<{id:string;title:string;category:string;duration:number;enrolled:number;completed:number;difficulty:string;rating:number}> = [
  ...
  @state() private _logQueryLearningPaths: Array<{id:string;name:string;courseIds:string[];progress:number;enrolled:number}> = [
  ...
  @state() private _logQueryDeptCompliance: Array<{dept:string;trainedPct:number;targetPct:number;avgScore:number;certCount:number}> = [
  ...
  @state() private _logQuerySkillsGaps: Array<{skill:string;current:number;required:number;gap:number;priority:string}> = [
  ...
  private _renderLogqueryTraining(): TemplateResult {
    const courses = this._logQueryCourses;
    const deptComp = this._logQueryDeptCompliance;
    return html`
  ...
  @state() private _logQueryContainerScans: Array<{image:string;registry:string;critical:number;high:number;medium:number;scanDate:string;status:string}> = [
  ...
  @state() private _logQueryK8sPods: Array<{namespace:string;pod:string;securityContext:string;hostIPC:boolean;hostPID:boolean;privileged:boolean;riskLevel:string}> = [
  ...
  @state() private _logQueryServerlessRisk: Array<{function:string;runtime:string;timeout:number;iamPerms:string;externalCalls:number;riskScore:number}> = [
  ...
  @state() private _logQueryRuntimeAlerts: Array<{id:string;workload:string;alertType:string;severity:string;description:string;timestamp:string}> = [
  ...
  private _renderLogqueryCloudWl(): TemplateResult {
    const containers = this._logQueryContainerScans;
    const pods = this._logQueryK8sPods;
    const alerts = this._logQueryRuntimeAlerts;
    return html`
  ...
  @state() private _logQueryCorrRules: Array<{id:string;name:string;sources:string[];logic:string;severity:string;active:boolean;lastTriggered:string}> = [
  ...
  @state() private _logQueryEventTimeline: Array<{timestamp:string;source:string;eventType:string;details:string;correlated:boolean}> = [
  ...
  @state() private _logQueryFalsePosMetrics: {totalEvents:number;correlatedEvents:number;falsePositives:number;fpRate:number;topFpRules:string[]} = {
  ...
  @state() private _logQueryEventPatterns: Array<{id:string;pattern:string;frequency:number;firstSeen:string;lastSeen:string;status:string}> = [
  ...
  private _renderLogqueryEventCorr(): TemplateResult {
    const rules = this._logQueryCorrRules;
    const timeline = this._logQueryEventTimeline;
    const fpMetrics = this._logQueryFalsePosMetrics;
    return html`
  ...
  @state() private _logQueryZones: Array<{id:string;name:string;trustLevel:number;subnet:string;devices:number;policy:string;lastAudit:string}> = [
  ...
  @state() private _logQuerySegRules: Array<{id:string;source:string;dest:string;action:string;protocol:string;port:string;status:string;hits:number}> = [
  ...
  @state() private _logQueryCrossZoneTraffic: Array<{source:string;dest:string;bytes:number;sessions:number;violations:number}> = [
  ...
  @state() private _logQueryMicroSegGaps: Array<{id:string;zone:string;gapType:string;severity:string;recommendation:string}> = [
  ...
  private _renderLogqueryNetworkSeg(): TemplateResult {
    const zones = this._logQueryZones;
    const gaps = this._logQueryMicroSegGaps;
    return html`
  ...
  @state() private _logQueryThreatIntel: Array<{source:string;indicator:string;type:string;confidence:number;severity:string;observed:string;description:string}> = [
  ...
  @state() private _logQueryIntelMetrics: {totalIndicators:number;criticalCount:number;highCount:number;mediumCount:number;staleIndicators:number;feedUptime:number} = {
    totalIndicators: 45230, criticalCount: 234, highCount: 1892, mediumCount: 8765, staleIndicators: 567, feedUptime: 99.7
  };
  @state() private _logQueryIntelTrending: Array<{category:string;thisWeek:number;lastWeek:number;change:number}> = [
  ...
  private _renderLogqueryThreatIntel(): TemplateResult {
    const intel = this._logQueryThreatIntel;
    const trending = this._logQueryIntelTrending;
    const metrics = this._logQueryIntelMetrics;
    return html`
  ...
  @state() private _complianceCalendarEvents: Array<{id: string; title: string; deadline: string; category: string; status: string; assignee: string; notes: string}> = [
    {id: 'cc-log-query-001', title: 'SOX Section 404 Compliance Review', deadline: '2026-06-30', category: 'Regulatory', status: 'In Progress', assignee: 'Compliance Team', notes: 'Annual internal control assessment for financial reporting'},
    {id: 'cc-log-query-002', title: 'GDPR Data Protection Impact Assessment', deadline: '2026-05-15', category: 'Privacy', status: 'Pending', assignee: 'DPO Office', notes: 'Quarterly DPIA for high-risk processing activities'},
    {id: 'cc-log-query-003', title: 'ISO 27001 Surveillance Audit', deadline: '2026-07-20', category: 'Certification', status: 'Scheduled', assignee: 'QA Lead', notes: 'Stage 2 surveillance audit for ISMS certification renewal'},
    {id: 'cc-log-query-004', title: 'PCI DSS v4.0 Gap Analysis', deadline: '2026-05-30', category: 'Payment', status: 'Not Started', assignee: 'Security Architect', notes: 'Assess current state against PCI DSS v4.0 requirements'},
    {id: 'cc-log-query-005', title: 'NIST CSF Self-Assessment', deadline: '2026-08-15', category: 'Framework', status: 'Pending', assignee: 'CISO', notes: 'Biannual self-assessment against NIST Cybersecurity Framework'},
    {id: 'cc-log-query-006', title: 'HIPAA Security Rule Audit', deadline: '2026-06-15', category: 'Healthcare', status: 'In Progress', assignee: 'Compliance Analyst', notes: 'Annual audit of administrative, physical, and technical safeguards'},
    {id: 'cc-log-query-007', title: 'SOC 2 Type II Report Renewal', deadline: '2026-09-30', category: 'Audit', status: 'Scheduled', assignee: 'External Auditor', notes: 'Engage auditor for SOC 2 Type II examination period'},
    {id: 'cc-log-query-008', title: 'FedRAMP Authorization Review', deadline: '2026-07-01', category: 'Government', status: 'Pending', assignee: 'FedRAMP PMO', notes: 'Review and update security authorization package'},
    {id: 'cc-log-query-009', title: 'Annual Penetration Test', deadline: '2026-10-15', category: 'Testing', status: 'Not Started', assignee: 'Red Team Lead', notes: 'Comprehensive external and internal penetration testing engagement'},
    {id: 'cc-log-query-010', title: 'Security Awareness Training Rollout', deadline: '2026-05-01', category: 'Training', status: 'Completed', assignee: 'Security Awareness Manager', notes: 'Q2 organization-wide security awareness training program'},
    {id: 'cc-log-query-011', title: 'Vendor Security Review Cycle', deadline: '2026-06-01', category: 'Third Party', status: 'In Progress', assignee: 'Vendor Manager', notes: 'Quarterly review of critical vendor security posture and SLA compliance'},
    {id: 'cc-log-query-012', title: 'Incident Response Plan Update', deadline: '2026-05-20', category: 'Operations', status: 'Pending', assignee: 'IR Team Lead', notes: 'Update IR procedures based on lessons learned from recent incidents'},
  ];
  ...
  @state() private _dataQualityMetrics: Array<{id: string; dataSource: string; completenessScore: number; accuracyScore: number; timelinessScore: number; consistencyScore: number; reliabilityRank: number; lastUpdated: string; issues: string[]; trend: string}> = [
    {id: 'dq-log-query-001', dataSource: 'Vulnerability Scanner Feed', completenessScore: 94, accuracyScore: 91, timelinessScore: 88, consistencyScore: 95, reliabilityRank: 1, lastUpdated: '2026-04-23T08:00:00Z', issues: ['Minor delay in Nessus plugin updates'], trend: 'improving'},
    {id: 'dq-log-query-002', dataSource: 'SIEM Event Logs', completenessScore: 87, accuracyScore: 93, timelinessScore: 96, consistencyScore: 89, reliabilityRank: 2, lastUpdated: '2026-04-23T07:30:00Z', issues: ['Some log sources showing gaps in off-hours'], trend: 'stable'},
    {id: 'dq-log-query-003', dataSource: 'Asset Inventory Database', completenessScore: 82, accuracyScore: 78, timelinessScore: 75, consistencyScore: 84, reliabilityRank: 4, lastUpdated: '2026-04-22T18:00:00Z', issues: ['Shadow IT devices not fully cataloged', 'Decommissioned assets still listed'], trend: 'declining'},
    {id: 'dq-log-query-004', dataSource: 'Threat Intelligence Platform', completenessScore: 91, accuracyScore: 89, timelinessScore: 92, consistencyScore: 87, reliabilityRank: 3, lastUpdated: '2026-04-23T06:00:00Z', issues: ['Some IOC sources lack confidence scoring'], trend: 'improving'},
    {id: 'dq-log-query-005', dataSource: 'Identity Provider Logs', completenessScore: 96, accuracyScore: 97, timelinessScore: 98, consistencyScore: 96, reliabilityRank: 1, lastUpdated: '2026-04-23T09:00:00Z', issues: [], trend: 'stable'},
    {id: 'dq-log-query-006', dataSource: 'Cloud Security Posture Data', completenessScore: 79, accuracyScore: 85, timelinessScore: 82, consistencyScore: 81, reliabilityRank: 5, lastUpdated: '2026-04-23T05:00:00Z', issues: ['Multi-cloud coverage gaps', 'API rate limiting affects scan frequency'], trend: 'declining'},
    {id: 'dq-log-query-007', dataSource: 'Compliance Evidence Store', completenessScore: 88, accuracyScore: 86, timelinessScore: 80, consistencyScore: 83, reliabilityRank: 4, lastUpdated: '2026-04-21T14:00:00Z', issues: ['Manual evidence collection delays', 'Inconsistent formatting across frameworks'], trend: 'stable'},
    {id: 'dq-log-query-008', dataSource: 'Network Flow Data', completenessScore: 92, accuracyScore: 90, timelinessScore: 94, consistencyScore: 91, reliabilityRank: 2, lastUpdated: '2026-04-23T08:30:00Z', issues: ['Encrypted traffic classification accuracy needs improvement'], trend: 'improving'},
  ];
  ...
  @state() private _workflowTemplates: Array<{id: string; name: string; description: string; category: string; steps: number; avgDuration: string; successRate: number; lastRun: string; manualInterventions: number; optimizationScore: number}> = [
    {id: 'wf-log-query-001', name: 'Vulnerability Triage Pipeline', description: 'Automated vulnerability assessment, prioritization, and assignment workflow', category: 'Vulnerability Management', steps: 8, avgDuration: '12 min', successRate: 94.5, lastRun: '2026-04-23T06:00:00Z', manualInterventions: 2, optimizationScore: 87},
    {id: 'wf-log-query-002', name: 'Incident Response Orchestration', description: 'End-to-end IR workflow from detection to containment and recovery', category: 'Incident Response', steps: 15, avgDuration: '45 min', successRate: 89.2, lastRun: '2026-04-22T14:30:00Z', manualInterventions: 5, optimizationScore: 72},
    {id: 'wf-log-query-003', name: 'Access Request Approval Chain', description: 'Multi-level approval workflow for privileged access requests', category: 'Identity & Access', steps: 6, avgDuration: '4 hours', successRate: 97.1, lastRun: '2026-04-23T09:15:00Z', manualInterventions: 1, optimizationScore: 92},
    {id: 'wf-log-query-004', name: 'Compliance Evidence Collection', description: 'Automated gathering and packaging of audit evidence artifacts', category: 'Compliance', steps: 10, avgDuration: '30 min', successRate: 91.8, lastRun: '2026-04-21T10:00:00Z', manualInterventions: 3, optimizationScore: 78},
    {id: 'wf-log-query-005', name: 'Security Patch Deployment', description: 'Staged patch rollout with validation and rollback capability', category: 'Patch Management', steps: 12, avgDuration: '2 hours', successRate: 96.3, lastRun: '2026-04-20T22:00:00Z', manualInterventions: 1, optimizationScore: 85},
    {id: 'wf-log-query-006', name: 'Threat Intelligence Enrichment', description: 'IOC enrichment and correlation with internal threat data', category: 'Threat Intelligence', steps: 7, avgDuration: '8 min', successRate: 93.7, lastRun: '2026-04-23T07:00:00Z', manualInterventions: 0, optimizationScore: 95},
    {id: 'wf-log-query-007', name: 'Security Reporting Pipeline', description: 'Automated generation and distribution of security metrics reports', category: 'Reporting', steps: 5, avgDuration: '15 min', successRate: 98.9, lastRun: '2026-04-23T08:00:00Z', manualInterventions: 0, optimizationScore: 97},
    {id: 'wf-log-query-008', name: 'Vendor Risk Assessment', description: 'Automated vendor security questionnaire distribution and scoring', category: 'Third Party', steps: 9, avgDuration: '3 days', successRate: 85.4, lastRun: '2026-04-19T11:00:00Z', manualInterventions: 4, optimizationScore: 65},
  ];
  ...
  @state() private _workflowExecutionHistory: Array<{workflowId: string; runDate: string; duration: string; status: string; trigger: string}> = [
    {workflowId: 'wf-log-query-001', runDate: '2026-04-23T06:00:00Z', duration: '11 min 23 sec', status: 'Success', trigger: 'Scheduled'},
    {workflowId: 'wf-log-query-002', runDate: '2026-04-22T14:30:00Z', duration: '42 min 15 sec', status: 'Success', trigger: 'Alert'},
    {workflowId: 'wf-log-query-003', runDate: '2026-04-23T09:15:00Z', duration: '3h 45 min', status: 'Success', trigger: 'User Request'},
    {workflowId: 'wf-log-query-001', runDate: '2026-04-22T06:00:00Z', duration: '13 min 08 sec', status: 'Partial', trigger: 'Scheduled'},
    {workflowId: 'wf-log-query-004', runDate: '2026-04-21T10:00:00Z', duration: '28 min 42 sec', status: 'Success', trigger: 'Scheduled'},
  ];
  ...
  @state() private _knowledgeArticles: Array<{id: string; title: string; category: string; author: string; publishDate: string; views: number; rating: number; tags: string[]; summary: string; readTime: string}> = [
    {id: 'kb-log-query-001', title: 'Zero Trust Architecture Implementation Guide', category: 'Architecture', author: 'Security Architecture Team', publishDate: '2026-03-15', views: 1842, rating: 4.8, tags: ['zero-trust', 'network', 'identity'], summary: 'Comprehensive guide for implementing zero trust principles across enterprise infrastructure', readTime: '15 min'},
    {id: 'kb-log-query-002', title: 'Ransomware Defense Playbook 2026', category: 'Threats', author: 'Threat Intelligence Team', publishDate: '2026-04-01', views: 2341, rating: 4.9, tags: ['ransomware', 'incident-response', 'defense'], summary: 'Updated playbook with latest ransomware tactics, techniques, and defensive measures', readTime: '12 min'},
    {id: 'kb-log-query-003', title: 'Cloud Security Best Practices for Multi-Cloud', category: 'Cloud', author: 'Cloud Security Team', publishDate: '2026-02-28', views: 1567, rating: 4.5, tags: ['cloud', 'aws', 'azure', 'gcp'], summary: 'Best practices for securing workloads across multiple cloud service providers', readTime: '18 min'},
    {id: 'kb-log-query-004', title: 'API Security Testing Methodology', category: 'Testing', author: 'Red Team', publishDate: '2026-03-22', views: 987, rating: 4.6, tags: ['api', 'testing', 'owasp'], summary: 'Structured methodology for identifying and testing API security vulnerabilities', readTime: '10 min'},
    {id: 'kb-log-query-005', title: 'Supply Chain Security Risk Management', category: 'Third Party', author: 'GRC Team', publishDate: '2026-01-20', views: 1234, rating: 4.3, tags: ['supply-chain', 'vendor', 'risk'], summary: 'Framework for managing security risks in the software supply chain', readTime: '14 min'},
    {id: 'kb-log-query-006', title: 'Kubernetes Security Hardening Checklist', category: 'Container', author: 'Platform Security', publishDate: '2026-04-10', views: 2103, rating: 4.7, tags: ['kubernetes', 'container', 'hardening'], summary: 'Step-by-step hardening checklist for production Kubernetes clusters', readTime: '11 min'},
    {id: 'kb-log-query-007', title: 'Security Metrics That Matter for Executives', category: 'Metrics', author: 'CISO Office', publishDate: '2026-03-05', views: 876, rating: 4.4, tags: ['metrics', 'kpi', 'reporting'], summary: 'Key security metrics and KPIs that resonate with board-level stakeholders', readTime: '8 min'},
    {id: 'kb-log-query-008', title: 'Phishing Simulation Campaign Design', category: 'Awareness', author: 'Security Awareness Team', publishDate: '2026-02-15', views: 1456, rating: 4.5, tags: ['phishing', 'simulation', 'training'], summary: 'Designing effective phishing simulations that drive real behavioral change', readTime: '9 min'},
    {id: 'kb-log-query-009', title: 'Data Loss Prevention Strategy and Implementation', category: 'Data Protection', author: 'Data Security Team', publishDate: '2026-03-28', views: 1678, rating: 4.6, tags: ['dlp', 'data-protection', 'compliance'], summary: 'Strategic approach to DLP covering people, process, and technology layers', readTime: '16 min'},
    {id: 'kb-log-query-010', title: 'Security Automation with SOAR Platforms', category: 'Automation', author: 'SOC Engineering', publishDate: '2026-04-05', views: 1098, rating: 4.7, tags: ['soar', 'automation', 'soc'], summary: 'Building effective security automation playbooks using SOAR technology', readTime: '13 min'},
    {id: 'kb-log-query-011', title: 'Third-Party Risk Assessment Framework', category: 'GRC', author: 'Vendor Management', publishDate: '2026-01-30', views: 1345, rating: 4.4, tags: ['vendor', 'risk', 'assessment'], summary: 'Structured framework for evaluating and monitoring third-party security risks', readTime: '12 min'},
    {id: 'kb-log-query-012', title: 'Network Detection and Response Best Practices', category: 'Network', author: 'Network Security Team', publishDate: '2026-03-18', views: 1567, rating: 4.5, tags: ['ndr', 'network', 'detection'], summary: 'Implementing effective network detection and response capabilities', readTime: '14 min'},
    {id: 'kb-log-query-013', title: 'Security Chaos Engineering Guide', category: 'Resilience', author: 'SRE Security', publishDate: '2026-04-12', views: 789, rating: 4.3, tags: ['chaos-engineering', 'resilience', 'testing'], summary: 'Applying chaos engineering principles to validate security controls', readTime: '10 min'},
    {id: 'kb-log-query-014', title: 'Incident Communication Templates', category: 'Incident Response', author: 'IR Team', publishDate: '2026-02-22', views: 2234, rating: 4.8, tags: ['incident', 'communication', 'templates'], summary: 'Ready-to-use communication templates for security incident scenarios', readTime: '7 min'},
    {id: 'kb-log-query-015', title: 'Endpoint Detection and Response Configuration', category: 'Endpoint', author: 'Endpoint Security', publishDate: '2026-03-08', views: 1345, rating: 4.5, tags: ['edr', 'endpoint', 'configuration'], summary: 'Optimal EDR configuration for maximum detection with minimal false positives', readTime: '11 min'},
    {id: 'kb-log-query-016', title: 'Security Code Review Standards', category: 'Development', author: 'AppSec Team', publishDate: '2026-02-10', views: 1890, rating: 4.6, tags: ['code-review', 'secure-sdlc', 'standards'], summary: 'Standards and checklists for security-focused code reviews', readTime: '13 min'},
    {id: 'kb-log-query-017', title: 'IoT Security Assessment Methodology', category: 'IoT', author: 'IoT Security Lab', publishDate: '2026-04-08', views: 654, rating: 4.2, tags: ['iot', 'embedded', 'assessment'], summary: 'Methodology for assessing security posture of IoT devices and ecosystems', readTime: '15 min'},
    {id: 'kb-log-query-018', title: 'Passwordless Authentication Migration Guide', category: 'Identity', author: 'IAM Team', publishDate: '2026-03-25', views: 1678, rating: 4.7, tags: ['passwordless', 'authentication', 'mfa'], summary: 'Step-by-step migration plan from password-based to passwordless authentication', readTime: '12 min'},
    {id: 'kb-log-query-019', title: 'Security Awareness Program Metrics', category: 'Awareness', author: 'Security Culture Team', publishDate: '2026-01-25', views: 1123, rating: 4.4, tags: ['awareness', 'metrics', 'culture'], summary: 'Measuring the effectiveness of your security awareness and training programs', readTime: '9 min'},
    {id: 'kb-log-query-020', title: 'Deception Technology Deployment Guide', category: 'Detection', author: 'Blue Team', publishDate: '2026-04-15', views: 876, rating: 4.5, tags: ['deception', 'honeypot', 'detection'], summary: 'Planning and deploying deception technology for advanced threat detection', readTime: '11 min'},
  ];
  ...
  // --- Security Scenario Planning Methods (LOQU) ---
  private _initLoquScenarios(): void {
    const worstCaseScenarios = [
      { id: "loqu-sc-01", name: "Ransomware Attack on Critical Infrastructure", category: "Malware", impact: 9500000, probability: 0.12, recoveryDays: 21, affectedSystems: 47, dataLossGb: 320, businessImpact: "critical" },
      { id: "loqu-sc-02", name: "Insider Data Exfiltration via Authorized Channels", category: "Insider Threat", impact: 7200000, probability: 0.18, recoveryDays: 14, affectedSystems: 12, dataLossGb: 85, businessImpact: "high" },
      { id: "loqu-sc-03", name: "Supply Chain Compromise via Third-Party Library", category: "Supply Chain", impact: 6800000, probability: 0.09, recoveryDays: 30, affectedSystems: 63, dataLossGb: 0, businessImpact: "critical" },
      { id: "loqu-sc-04", name: "Cloud Misconfiguration Leading to Public Exposure", category: "Cloud", impact: 4100000, probability: 0.22, recoveryDays: 7, affectedSystems: 8, dataLossGb: 450, businessImpact: "high" },
      { id: "loqu-sc-05", name: "Zero-Day Exploit in Core Application Framework", category: "Vulnerability", impact: 8900000, probability: 0.05, recoveryDays: 45, affectedSystems: 120, dataLossGb: 0, businessImpact: "critical" },
      { id: "loqu-sc-06", name: "Distributed Denial of Service on Customer-Facing Services", category: "Availability", impact: 3400000, probability: 0.25, recoveryDays: 3, affectedSystems: 15, dataLossGb: 0, businessImpact: "medium" },
      { id: "loqu-sc-07", name: "Credential Stuffing and Account Takeover Wave", category: "Identity", impact: 2800000, probability: 0.30, recoveryDays: 5, affectedSystems: 6, dataLossGb: 12, businessImpact: "medium" },
      { id: "loqu-sc-08", name: "Physical Security Breach at Data Center Facility", category: "Physical", impact: 12000000, probability: 0.02, recoveryDays: 60, affectedSystems: 200, dataLossGb: 5000, businessImpact: "critical" },
    ];
    this._loquScenarios = worstCaseScenarios.map(s => ({ ...s, mitigationCost: Math.round(s.impact * 0.08), drillScheduled: false, lastDrillDate: null, drillScore: null, controlGaps: Math.floor(Math.random() * 5) + 1 }));
  }

  private _calculateLoquScenarioRisk(): void {
    this._loquScenarios.forEach(s => {
  ...
  private _scheduleLoquDrills(): void {
  ...
  private _loquScenarioComparison(): Record<string, unknown>[] {
    return this._loquScenarios.map(s => ({
  ...
  // --- Security Control Effectiveness Analytics (LOQU) ---
  private _initLoquControls(): void {
    const controls = [
      { id: "loqu-ct-01", name: "Network Segmentation", type: "preventive", maturity: "defined", score: 82, target: 90, failures: 3, lastTest: "2026-03-15" },
      { id: "loqu-ct-02", name: "Endpoint Detection and Response", type: "detective", maturity: "managed", score: 88, target: 92, failures: 1, lastTest: "2026-04-01" },
      { id: "loqu-ct-03", name: "Data Loss Prevention", type: "preventive", maturity: "repeatable", score: 71, target: 85, failures: 7, lastTest: "2026-03-22" },
      { id: "loqu-ct-04", name: "Security Information and Event Management", type: "detective", maturity: "managed", score: 85, target: 90, failures: 2, lastTest: "2026-04-05" },
      { id: "loqu-ct-05", name: "Identity and Access Management", type: "preventive", maturity: "defined", score: 76, target: 88, failures: 5, lastTest: "2026-03-28" },
      { id: "loqu-ct-06", name: "Vulnerability Management", type: "corrective", maturity: "managed", score: 80, target: 90, failures: 4, lastTest: "2026-04-02" },
      { id: "loqu-ct-07", name: "Email Security Gateway", type: "preventive", maturity: "managed", score: 90, target: 95, failures: 1, lastTest: "2026-04-08" },
      { id: "loqu-ct-08", name: "Web Application Firewall", type: "preventive", maturity: "managed", score: 87, target: 92, failures: 2, lastTest: "2026-03-30" },
      { id: "loqu-ct-09", name: "Patch Management", type: "corrective", maturity: "defined", score: 68, target: 85, failures: 9, lastTest: "2026-03-18" },
      { id: "loqu-ct-10", name: "Encryption at Rest", type: "preventive", maturity: "managed", score: 93, target: 95, failures: 0, lastTest: "2026-04-10" },
      { id: "loqu-ct-11", name: "Encryption in Transit", type: "preventive", maturity: "optimized", score: 96, target: 98, failures: 0, lastTest: "2026-04-12" },
      { id: "loqu-ct-12", name: "Privileged Access Management", type: "preventive", maturity: "defined", score: 74, target: 88, failures: 6, lastTest: "2026-03-25" },
      { id: "loqu-ct-13", name: "Security Awareness Training", type: "preventive", maturity: "repeatable", score: 65, target: 80, failures: 11, lastTest: "2026-03-20" },
      { id: "loqu-ct-14", name: "Incident Response Plan", type: "corrective", maturity: "managed", score: 78, target: 88, failures: 4, lastTest: "2026-04-03" },
      { id: "loqu-ct-15", name: "Backup and Recovery", type: "corrective", maturity: "managed", score: 84, target: 92, failures: 3, lastTest: "2026-04-06" },
      { id: "loqu-ct-16", name: "Multi-Factor Authentication", type: "preventive", maturity: "managed", score: 91, target: 95, failures: 1, lastTest: "2026-04-09" },
      { id: "loqu-ct-17", name: "Network Traffic Analysis", type: "detective", maturity: "defined", score: 72, target: 85, failures: 5, lastTest: "2026-03-27" },
      { id: "loqu-ct-18", name: "Cloud Security Posture Management", type: "detective", maturity: "repeatable", score: 69, target: 82, failures: 8, lastTest: "2026-03-24" },
      { id: "loqu-ct-19", name: "Container Security Scanning", type: "preventive", maturity: "defined", score: 75, target: 85, failures: 5, lastTest: "2026-04-04" },
      { id: "loqu-ct-20", name: "API Security Gateway", type: "preventive", maturity: "repeatable", score: 70, target: 82, failures: 7, lastTest: "2026-03-29" },
      { id: "loqu-ct-21", name: "Threat Intelligence Platform", type: "detective", maturity: "managed", score: 83, target: 90, failures: 2, lastTest: "2026-04-07" },
      { id: "loqu-ct-22", name: "Database Activity Monitoring", type: "detective", maturity: "defined", score: 67, target: 80, failures: 8, lastTest: "2026-03-21" },
      { id: "loqu-ct-23", name: "Deception Technology", type: "detective", maturity: "initial", score: 55, target: 75, failures: 12, lastTest: "2026-03-16" },
      { id: "loqu-ct-24", name: "Security Orchestration Automation", type: "corrective", maturity: "repeatable", score: 73, target: 85, failures: 6, lastTest: "2026-04-01" },
      { id: "loqu-ct-25", name: "Third-Party Risk Assessment", type: "preventive", maturity: "defined", score: 64, target: 78, failures: 9, lastTest: "2026-03-19" },
    ];
    this._loquControls = controls;
  }

  private _analyzeLoquControlEffectiveness(): void {
    const typeBreakdown: Record<string, { total: number; avgScore: number; avgFailures: number }> = {};
    this._loquControls.forEach(c => {
    ...
    this._loquControlTypeBreakdown = typeBreakdown;
  }

  private _loquControlOptimization(): Record<string, unknown>[] {
    return this._loquControls.filter(c => c.score < c.target).map(c => ({
  ...
  // --- Security Data Pipeline Health (LOQU) ---
  private _initLoquPipelines(): void {
    const pipelines = [
      { id: "loqu-pl-01", name: "SIEM Log Ingestion", status: "healthy", healthScore: 94, latencyMs: 120, freshnessMin: 2, errorRate: 0.02, throughputMbps: 450, recordsPerSec: 12000, backPressure: 0.05, uptime: 99.97 },
      { id: "loqu-pl-02", name: "Threat Feed Aggregation", status: "healthy", healthScore: 91, latencyMs: 300, freshnessMin: 15, errorRate: 0.05, throughputMbps: 85, recordsPerSec: 3200, backPressure: 0.08, uptime: 99.92 },
      { id: "loqu-pl-03", name: "Vulnerability Scan Results", status: "degraded", healthScore: 72, latencyMs: 2500, freshnessMin: 60, errorRate: 0.15, throughputMbps: 25, recordsPerSec: 800, backPressure: 0.35, uptime: 98.50 },
      { id: "loqu-pl-04", name: "Endpoint Telemetry Stream", status: "healthy", healthScore: 88, latencyMs: 450, freshnessMin: 5, errorRate: 0.08, throughputMbps: 280, recordsPerSec: 8500, backPressure: 0.12, uptime: 99.85 },
      { id: "loqu-pl-05", name: "Cloud Audit Log Pipeline", status: "healthy", healthScore: 86, latencyMs: 600, freshnessMin: 10, errorRate: 0.06, throughputMbps: 150, recordsPerSec: 4500, backPressure: 0.10, uptime: 99.80 },
      { id: "loqu-pl-06", name: "Identity Event Stream", status: "warning", healthScore: 78, latencyMs: 1200, freshnessMin: 20, errorRate: 0.12, throughputMbps: 45, recordsPerSec: 1500, backPressure: 0.22, uptime: 99.40 },
      { id: "loqu-pl-07", name: "Network Flow Collection", status: "healthy", healthScore: 82, latencyMs: 800, freshnessMin: 8, errorRate: 0.09, throughputMbps: 650, recordsPerSec: 18000, backPressure: 0.15, uptime: 99.70 },
      { id: "loqu-pl-08", name: "DLP Incident Pipeline", status: "degraded", healthScore: 68, latencyMs: 3500, freshnessMin: 45, errorRate: 0.20, throughputMbps: 18, recordsPerSec: 500, backPressure: 0.42, uptime: 97.80 },
    ];
    this._loquPipelines = pipelines.map(p => ({ ...p, dependencies: [], slaBreached: p.healthScore < 75, alertThreshold: p.errorRate > 0.15 }));
    this._loquPipelines[0].dependencies = ["loqu-pl-04", "loqu-pl-07"];
    this._loquPipelines[4].dependencies = ["loqu-pl-01"];
    this._loquPipelines[5].dependencies = ["loqu-pl-02"];
    this._loquPipelines[7].dependencies = ["loqu-pl-04", "loqu-pl-05"];
  }

  private _loquPipelineTrend(): Record<string, unknown>[] {
    const hours = Array.from({ length: 24 }, (_, i) => `H${String(i).padStart(2, "0")}`);
    return this._loquPipelines.map(p => ({
  ...
  // --- Security Stakeholder Report Generator (LOQU) ---
  private _initLoquReportTemplates(): void {
    this._loquReportTemplates = [
      { id: "loqu-rp-01", name: "Board Security Report", audience: "board", frequency: "quarterly", sections: 6, autoGenerate: true, lastGenerated: "2026-03-31", pages: 12 },
      { id: "loqu-rp-02", name: "Executive Risk Summary", audience: "executive", frequency: "monthly", sections: 8, autoGenerate: true, lastGenerated: "2026-04-01", pages: 8 },
      { id: "loqu-rp-03", name: "Technical Security Review", audience: "technical", frequency: "weekly", sections: 12, autoGenerate: true, lastGenerated: "2026-04-21", pages: 25 },
      { id: "loqu-rp-04", name: "Audit Compliance Report", audience: "audit", frequency: "quarterly", sections: 10, autoGenerate: false, lastGenerated: "2026-03-15", pages: 35 },
      { id: "loqu-rp-05", name: "Regulatory Filing Package", audience: "regulatory", frequency: "annual", sections: 15, autoGenerate: false, lastGenerated: "2025-12-31", pages: 48 },
    ];
  }

  private _generateLoquExecSummary(): string {
    const totalRisk = this._loquScenarios.reduce((sum: number, s: any) => sum + s.riskScore, 0);
    const avgControl = Math.round(this._loquControls.reduce((sum: number, c: any) => sum + c.score, 0) / this._loquControls.length);
    const degradedPipes = this._loquPipelines.filter((p: any) => p.status !== "healthy").length;
    return `Overall risk exposure: ${(totalRisk / 1000000).toFixed(1)}M. Average control effectiveness: {avgControl}%. {degradedPipes} pipeline(s) need attention. {this._loquScenarios.filter((s: any) => s.mitigationPriority === "immediate").length} scenarios require immediate action.`;
  }

  // --- Security Technology Radar (LOQU) ---
  private _initLoquTechRadar(): void {
    this._loquTechRadar = [
      { id: "loqu-tr-01", name: "AI-Powered Threat Detection", ring: "adopt", category: "Detection", maturity: 4, trend: "up", investment: "high", roi: 3.2, vendorCount: 8 },
      { id: "loqu-tr-02", name: "Zero Trust Architecture", ring: "adopt", category: "Architecture", maturity: 4, trend: "up", investment: "high", roi: 2.8, vendorCount: 12 },
      { id: "loqu-tr-03", name: "SASE/SSE Platform", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.4, vendorCount: 10 },
      { id: "loqu-tr-04", name: "CNAPP Cloud Security", ring: "trial", category: "Cloud", maturity: 3, trend: "up", investment: "medium", roi: 2.1, vendorCount: 7 },
      { id: "loqu-tr-05", name: "Extended Detection and Response", ring: "adopt", category: "Detection", maturity: 4, trend: "stable", investment: "high", roi: 2.9, vendorCount: 9 },
      { id: "loqu-tr-06", name: "Quantum-Safe Cryptography", ring: "assess", category: "Crypto", maturity: 2, trend: "up", investment: "low", roi: 0.8, vendorCount: 4 },
      { id: "loqu-tr-07", name: "Security Service Edge", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.0, vendorCount: 6 },
      { id: "loqu-tr-08", name: "SOAR 2.0 Autonomous SOC", ring: "assess", category: "Operations", maturity: 2, trend: "up", investment: "low", roi: 1.2, vendorCount: 5 },
      { id: "loqu-tr-09", name: "Software Supply Chain Security", ring: "trial", category: "DevSecOps", maturity: 3, trend: "up", investment: "medium", roi: 2.3, vendorCount: 8 },
      { id: "loqu-tr-10", name: "Identity Threat Detection", ring: "adopt", category: "Identity", maturity: 3, trend: "up", investment: "high", roi: 2.7, vendorCount: 7 },
      { id: "loqu-tr-11", name: "Data Security Posture Management", ring: "trial", category: "Data", maturity: 3, trend: "up", investment: "medium", roi: 2.2, vendorCount: 9 },
      { id: "loqu-tr-12", name: "Decentralized Identity Standards", ring: "hold", category: "Identity", maturity: 1, trend: "stable", investment: "low", roi: 0.5, vendorCount: 3 },
    ];
  }

  private _loquRadarSummary(): Record<string, unknown> {
    const rings: Record<string, number> = { adopt: 0, trial: 0, assess: 0, hold: 0 };
    this._loquTechRadar.forEach(t => { if (rings[t.ring as keyof typeof rings] !== undefined) rings[t.ring as keyof typeof rings]++; });
    return { adopt: rings.adopt, trial: rings.trial, assess: rings.assess, hold: rings.hold, total: this._loquTechRadar.length, avgMaturity: 2.8, topInvestment: this._loquTechRadar.filter(t => t.investment === "high").map(t => t.name) };
  }

  // --- LOQU Security Compliance Mapping Matrix ---
  private _initLoquComplianceMatrix(): void {
    ...
    this._loquComplianceMatrix = frameworks.map(fw => ({
  ...
  private _loquComplianceTrend(): Record<string, unknown>[] {
    const quarters = ["Q1-2025", "Q2-2025", "Q3-2025", "Q4-2025", "Q1-2026", "Q2-2026"];
    return this._loquComplianceMatrix.map(fw => ({
  ...
  // --- LOQU Threat Intelligence Correlation Engine ---
  private _initLoquThreatIntel(): void {
    ...
    this._loquThreatIntel = threatActors.map((actor, i) => ({
  ...
  private _loquThreatCorrelation(): Record<string, unknown>[] {
    return this._loquThreatIntel.filter(t => t.riskScore > 70).map(t => ({
  ...
  // --- LOQU Security Metrics Deep Dive ---
  private _initLoquMetricsDeep(): void {
    ...
    this._loquMetricsDeep = metricCategories.map(cat => ({
  ...
  private _loquMetricsHeatmap(): number[][] {
    return this._loquMetricsDeep.map(cat =>
      cat.metrics.map(m => Math.round((m.value / m.target) * 100))
    );
  }

  // --- LOQU Incident Cost Modeling ---
  private _initLoquCostModel(): void {
    const incidentTypes = ["Data Breach", "Ransomware", "DDoS", "Insider Threat", "Phishing", "Supply Chain", "Cloud Misconfig", "Zero-Day"];
    this._loquCostModel = incidentTypes.map(inc => ({
    ...
    this._loquCostModel.forEach(m => { m.totalAnnualExposure = m.avgCost * m.frequency; });
  }

  private _loquCostProjection(): Record<string, unknown>[] {
    return this._loquCostModel.map(m => ({
  ...
  // --- LOQU Security Architecture Pattern Library ---
  private _initLoquArchPatterns(): void {
    ...
    this._loquArchPatterns = patterns.map((p, i) => ({
  ...
  private _loquArchPatternAnalysis(): Record<string, unknown> {
    const implemented = this._loquArchPatterns.filter(p => p.implementation > 75).length;
    const avgCoverage = Math.round(this._loquArchPatterns.reduce((s: number, p: any) => s + p.coverage, 0) / this._loquArchPatterns.length);
    return {
      totalPatterns: this._loquArchPatterns.length, fullyImplemented: implemented, avgCoverage,
      avgRiskReduction: Math.round(this._loquArchPatterns.reduce((s: number, p: any) => s + p.riskReduction, 0) / this._loquArchPatterns.length),
      gaps: this._loquArchPatterns.filter(p => p.implementation < 60).map(p => p.pattern),
      recommendations: this._loquArchPatterns.filter(p => p.implementation < 70).map(p => ({ pattern: p.pattern, current: p.implementation, target: 85, effort: p.costComplexity }))
    };
  }
  ...
  private _9adRiskAssessmentWorkflow = [
  ...
  private _9adTreatmentOptions = [
    {id: '9ad-T-001', riskId: 'RSK-042', risk: 'Ransomware Attack on Critical Infrastructure', before: 20, after: 6, strategy: 'Mitigate', controls: ['Next-gen EDR with behavioral detection and automated response', 'Zero-trust network micro-segmentation isolating critical assets', 'Immutable offline backup rotation with quarterly recovery testing', 'Incident response retainer with 1-hour response SLA'], cost: '$450K/yr', roi: '78% reduction vs $2.8M potential loss', timeline: 'Q3 2026', status: 'approved' as const},
  ...
  private _9adTalentPositions = [
    ...
    {id: '9ad-H-002', title: 'Cloud Security Engineer', dept: 'Cloud Security', level: 'Mid-Senior', loc: 'SF/Remote', salary: '$155K-$180K', posted: '2026-03-20', applicants: 45, pipeline: 12, interviews: 6, offers: 2, urgency: 'high' as const, manager: 'Mike Johnson', skills: ['AWS/Azure/GCP', 'Terraform', 'Kubernetes', 'IAM', 'CSPM'], nice: ['CCSP', 'CKS', 'Serverless']},
    {id: '9ad-H-003', title: 'SOC Analyst L2', dept: 'Security Operations', level: 'Mid', loc: 'Austin TX', salary: '$105K-$130K', posted: '2026-04-01', applicants: 67, pipeline: 18, interviews: 8, offers: 0, urgency: 'medium' as const, manager: 'Lisa Park', skills: ['SIEM', 'Incident triage', 'Malware analysis', 'TCP/IP'], nice: ['GCIA', 'SOAR', 'Kill chain']},
    {id: '9ad-H-004', title: 'Staff Security Architect', dept: 'Architecture', level: 'Staff', loc: 'Remote US', salary: '$200K-$240K', posted: '2026-02-28', applicants: 12, pipeline: 3, interviews: 2, offers: 0, urgency: 'critical' as const, manager: 'David Lee', skills: ['Enterprise arch', 'Zero trust', 'Risk frameworks', 'Board comms'], nice: ['CISSP+SABSA', 'Team building', 'Speaker']},
    {id: '9ad-H-005', title: 'GRC Analyst', dept: 'GRC', level: 'Mid', loc: 'NY/Remote', salary: '$110K-$135K', posted: '2026-04-05', applicants: 34, pipeline: 9, interviews: 3, offers: 1, urgency: 'medium' as const, manager: 'Rachel Green', skills: ['ISO 27001', 'SOC 2', 'Risk assessment', 'Audit'], nice: ['CRISC', 'CISA', 'GDPR']},
    {id: '9ad-H-006', title: 'Threat Intel Analyst', dept: 'Threat Intel', level: 'Mid-Senior', loc: 'Remote US', salary: '$140K-$170K', posted: '2026-03-25', applicants: 19, pipeline: 5, interviews: 2, offers: 0, urgency: 'low' as const, manager: 'James Wilson', skills: ['MITRE ATT&CK', 'OSINT', 'Threat modeling', 'Reverse eng'], nice: ['CTIA', 'Nation-state', 'Dark web']},
    {id: '9ad-H-007', title: 'AppSec Engineer', dept: 'AppSec', level: 'Senior', loc: 'Seattle/Remote', salary: '$160K-$190K', posted: '2026-04-10', applicants: 28, pipeline: 7, interviews: 3, offers: 0, urgency: 'medium' as const, manager: 'Emily Zhang', skills: ['SAST/DAST/SCA', 'Secure SDLC', 'Code review', 'CI/CD'], nice: ['CSSLP', 'Mobile security']},
    {id: '9ad-H-008', title: 'IAM Engineer', dept: 'IAM', level: 'Mid', loc: 'Remote US', salary: '$135K-$160K', posted: '2026-04-08', applicants: 31, pipeline: 10, interviews: 5, offers: 1, urgency: 'medium' as const, manager: 'Chris Martinez', skills: ['Okta/Azure AD', 'SAML/OIDC', 'PAM', 'RBAC'], nice: ['CISSP', 'Zero trust IAM']}
  ];

  private _9adInterviewScorecard = [
  ...
  private _9adOnboardingSteps = [
    {id: '9ad-OB-01', item: 'Provision security-hardened laptop, monitors, peripherals, and configured workstation image with all required security tools', cat: 'IT Setup', day: 'Day 1', owner: 'IT Operations'},
    {id: '9ad-OB-02', item: 'Create AD account, email, Slack, VPN, SSO enrollment, and all required application access accounts', cat: 'Access', day: 'Day 1', owner: 'IAM Team'},
    {id: '9ad-OB-03', item: 'Grant security tooling access: SIEM console, EDR dashboard, vuln scanner, pentest platforms, SOAR', cat: 'Tools', day: 'Day 1-2', owner: 'SOC Manager'},
    {id: '9ad-OB-04', item: 'Complete MFA enrollment, security awareness onboarding course, and acceptable use policy acknowledgment', cat: 'Security', day: 'Day 1', owner: 'Awareness Team'},
    {id: '9ad-OB-05', item: 'Sign NDA, employment agreement, IP assignment, and code of conduct documents with HR', cat: 'HR', day: 'Day 1', owner: 'Human Resources'},
    {id: '9ad-OB-06', item: 'Introduction meetings with direct team members and key cross-functional stakeholders', cat: 'Integration', day: 'Week 1', owner: 'Hiring Manager'},
    {id: '9ad-OB-07', item: 'Security architecture overview, tooling walkthrough, and operational runbook review sessions', cat: 'Training', day: 'Week 1-2', owner: 'Security Architect'},
    {id: '9ad-OB-08', item: 'Shadow experienced team member on active incident response and threat hunting investigations', cat: 'Mentoring', day: 'Week 2-3', owner: 'SOC Lead'},
    {id: '9ad-OB-09', item: 'Set up individual certification study plan aligned with role requirements and career goals', cat: 'Development', day: 'Week 2', owner: 'Hiring Manager'},
    {id: '9ad-OB-10', item: 'Establish 30-60-90 day goals, define success metrics, and schedule first performance checkpoint', cat: 'Goals', day: 'Week 1', owner: 'Hiring Manager'}
  ];

  private _9adMetricsDashboard = [
    {id: '9ad-MTD-001', name: 'Mean Time to Detect (MTTD)', value: '4.2 hours', trend: 'improving' as const, target: '< 4 hours', source: 'SIEM - Splunk', method: 'Median time from first alert to analyst acknowledgment', owner: 'SOC Manager', sla: '4 hours', prev: '5.1 hours', spark: [5.8, 5.4, 5.1, 4.9, 4.7, 4.5, 4.3, 4.2], breakdown: [{src: 'Network IDS', pct: '35%', avg: '3.8h'}, {src: 'Endpoint EDR', pct: '28%', avg: '4.1h'}, {src: 'Email Gateway', pct: '22%', avg: '3.5h'}, {src: 'Cloud Security', pct: '10%', avg: '5.2h'}, {src: 'User Reports', pct: '5%', avg: '6.8h'}]},
    {id: '9ad-MTD-002', name: 'Mean Time to Respond (MTTR)', value: '2.8 hours', trend: 'stable' as const, target: '< 3 hours', source: 'SOAR - XSOAR', method: 'Median time from acknowledgment to first containment action', owner: 'IR Lead', sla: '3 hours', prev: '2.7 hours', spark: [3.2, 3.0, 2.9, 2.8, 2.8, 2.9, 2.8, 2.8], breakdown: [{src: 'Auto Playbooks', pct: '42%', avg: '1.2h'}, {src: 'Semi-Auto', pct: '33%', avg: '2.5h'}, {src: 'Manual', pct: '20%', avg: '5.1h'}, {src: 'L3 Escalation', pct: '5%', avg: '8.3h'}]},
    {id: '9ad-MTD-003', name: 'Patch Compliance Rate', value: '97.6%', trend: 'improving' as const, target: '> 95%', source: 'Tenable.io', method: 'Pct of critical/high vulns patched within SLA windows', owner: 'Vuln Manager', sla: '95%', prev: '96.1%', spark: [93.2, 94.1, 94.8, 95.3, 95.9, 96.4, 97.0, 97.6], breakdown: [{src: 'Critical (7d)', pct: '40%', rate: '99.2%'}, {src: 'High (30d)', pct: '35%', rate: '97.8%'}, {src: 'Medium (90d)', pct: '20%', rate: '95.1%'}, {src: 'Low (180d)', pct: '5%', rate: '89.4%'}]},
    {id: '9ad-MTD-004', name: 'Phishing Click Rate', value: '3.2%', trend: 'improving' as const, target: '< 5%', source: 'KnowBe4 + Proofpoint', method: 'Pct of users clicking simulated phishing links monthly', owner: 'Awareness Lead', sla: '5%', prev: '4.1%', spark: [6.8, 6.2, 5.5, 5.1, 4.6, 4.1, 3.6, 3.2], breakdown: [{src: 'Spear Phishing', pct: '35%', rate: '5.1%'}, {src: 'Generic Phishing', pct: '30%', rate: '2.8%'}, {src: 'BEC Sim', pct: '20%', rate: '3.5%'}, {src: 'SMS Phishing', pct: '15%', rate: '1.9%'}]},
    {id: '9ad-MTD-005', name: 'Security Posture Score', value: '784', trend: 'improving' as const, target: '> 750', source: 'SecurityScorecard', method: 'Composite: external 30%, compliance 25%, vuln 25%, config 20%', owner: 'CISO', sla: '> 750', prev: '771', spark: [748, 752, 758, 762, 768, 773, 778, 784], breakdown: [{src: 'External Rating', pct: '30%', score: '791'}, {src: 'Compliance', pct: '25%', score: '812'}, {src: 'Vuln Mgmt', pct: '25%', score: '745'}, {src: 'Config', pct: '20%', score: '769'}]},
    {id: '9ad-MTD-006', name: 'Vulnerability Backlog', value: '142', trend: 'improving' as const, target: '< 150', source: 'Tenable + Qualys', method: 'Total open vulns across all scanned assets tracked weekly', owner: 'Vuln Manager', sla: '< 150', prev: '168', spark: [210, 198, 185, 175, 165, 158, 150, 142], breakdown: [{src: 'Critical', pct: '8%', count: '11'}, {src: 'High', pct: '22%', count: '31'}, {src: 'Medium', pct: '45%', count: '64'}, {src: 'Low', pct: '25%', count: '36'}]}
  ];

  private _9adArchReview = [
    {id: '9ad-AR-001', cat: 'Network', item: 'Network segmentation between trust zones with dedicated firewalls and granular ACLs', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Micro-segmentation via NSX across all production zones'},
    {id: '9ad-AR-002', cat: 'Network', item: 'DMZ architecture isolating public services from internal network', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Dual-firewall DMZ with WAF and DDoS mitigation'},
    {id: '9ad-AR-003', cat: 'Network', item: 'Least-privilege traffic flows with deny-by-default and quarterly review', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Explicit allow rules with automated drift detection'},
    {id: '9ad-AR-004', cat: 'Network', item: 'Encrypted external communications with IPSec/TLS 1.3 and PFS', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'IKEv2 with PFS, TLS 1.3 for all external APIs'},
    {id: '9ad-AR-005', cat: 'Identity', item: 'Centralized IdP with SSO and automated SCIM provisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Okta SSO SAML 2.0 for 247 apps with SCIM'},
    {id: '9ad-AR-006', cat: 'Identity', item: 'RBAC with quarterly reviews and automated deprovisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Quarterly recertification with JIT admin access'},
    {id: '9ad-AR-007', cat: 'Identity', item: 'PAM controlling all admin and service accounts with session recording', sev: 'high' as const, status: 'fail' as const, reviewer: 'IAM Lead', notes: '3 legacy accounts pending CyberArk enrollment'},
    {id: '9ad-AR-008', cat: 'Identity', item: 'MFA enforced for all users, hardware tokens for privileged access', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'FIDO2 for admins, push MFA for users, 99.8% enrollment'},
    {id: '9ad-AR-009', cat: 'Data', item: 'Data classification enforced with automated labeling on all repos', sev: 'high' as const, status: 'partial' as const, reviewer: 'Data Protection Lead', notes: 'Framework deployed, gaps on 4 legacy file shares'},
    {id: '9ad-AR-010', cat: 'Data', item: 'AES-256 encryption at rest for all sensitive data stores', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'Full-disk, TDE on DBs, SSE-KMS on cloud storage'},
    {id: '9ad-AR-011', cat: 'Data', item: 'DLP monitoring and preventing unauthorized data exfiltration', sev: 'high' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'DLP active on email, endpoint, cloud, and network'},
    {id: '9ad-AR-012', cat: 'Data', item: 'Encrypted off-site backups with quarterly restore testing', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Infrastructure Lead', notes: '3-2-1 strategy with immutable backups'},
    {id: '9ad-AR-013', cat: 'AppSec', item: 'Secure SDLC with SAST, DAST, SCA in all CI/CD pipelines', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: '34 pipelines with security gates and PR blocking'},
    {id: '9ad-AR-014', cat: 'AppSec', item: 'API gateway with auth, rate limiting, schema validation', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: 'Kong with OAuth2, rate limits, OpenAPI validation'},
    {id: '9ad-AR-015', cat: 'AppSec', item: 'Container security with image scanning and runtime protection', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Trivy scanning, Falco runtime in 80% of clusters'},
    {id: '9ad-AR-016', cat: 'Monitoring', item: 'SIEM collecting from all critical systems with correlation rules', sev: 'critical' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'Splunk 98% coverage with 1200+ correlation rules'},
    {id: '9ad-AR-017', cat: 'Monitoring', item: 'EDR with behavioral detection on all managed endpoints', sev: 'high' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'CrowdStrike on 99.2% endpoints with 24/7 MDR'},
    {id: '9ad-AR-018', cat: 'Monitoring', item: 'Executive dashboards with real-time posture visibility', sev: 'medium' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'PowerBI dashboards with weekly exec summaries'},
    {id: '9ad-AR-019', cat: 'Cloud', item: 'CSPM monitoring and remediating cloud misconfigurations', sev: 'high' as const, status: 'pass' as const, reviewer: 'Cloud Security Lead', notes: 'Prisma Cloud 450+ checks with auto-remediation'},
    {id: '9ad-AR-020', cat: 'Cloud', item: 'IaC templates scanned before deployment to production', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Checkov for Terraform, cfn-nag for CloudFormation pending'}
  ];

  private _get9adRiskProgress(): number {
    return Math.round(this._9adRiskAssessmentWorkflow.filter(s => s.status === 'completed').length / this._9adRiskAssessmentWorkflow.length * 100);
  }

  private _get9adTreatmentSummary(): {total: number; approved: number; avgReduction: number; investment: string} {
    const a = this._9adTreatmentOptions.filter(r => r.status === 'approved').length;
    const avg = this._9adTreatmentOptions.reduce((s, r) => s + ((r.before - r.after) / r.before * 100), 0) / this._9adTreatmentOptions.length;
    return {total: this._9adTreatmentOptions.length, approved: a, avgReduction: Math.round(avg), investment: '$1.125M annually'};
  }

  private _get9adRuleSummary(): {total: number; passing: number; failing: number; avgRate: number; violations: number} {
    const p = this._9adComplianceRules.filter(r => r.result === 'pass').length;
    const f = this._9adComplianceRules.filter(r => r.result === 'fail').length;
    const avg = this._9adComplianceRules.reduce((s, r) => s + r.passRate, 0) / this._9adComplianceRules.length;
    const v = this._9adComplianceRules.reduce((s, r) => s + r.violations, 0);
    return {total: this._9adComplianceRules.length, passing: p, failing: f, avgRate: Math.round(avg * 10) / 10, violations: v};
  }

  private _get9adPipelineStats(): {positions: number; applicants: number; interviewing: number; offers: number; critical: number} {
    const t = this._9adTalentPositions.length;
    const a = this._9adTalentPositions.reduce((s, p) => s + p.applicants, 0);
    const i = this._9adTalentPositions.reduce((s, p) => s + p.interviews, 0);
    const o = this._9adTalentPositions.reduce((s, p) => s + p.offers, 0);
    const c = this._9adTalentPositions.filter(x => x.urgency === 'critical').length;
    return {positions: t, applicants: a, interviewing: i, offers: o, critical: c};
  }

  private _get9adMetricTrends(): {improving: number; stable: number; total: number} {
    const imp = this._9adMetricsDashboard.filter(m => m.trend === 'improving').length;
    const stb = this._9adMetricsDashboard.filter(m => m.trend === 'stable').length;
    return {improving: imp, stable: stb, total: this._9adMetricsDashboard.length};
  }

  private _get9adArchScore(): {total: number; passed: number; failed: number; partial: number; score: number} {
    const p = this._9adArchReview.filter(c => c.status === 'pass').length;
    const f = this._9adArchReview.filter(c => c.status === 'fail').length;
    const pa = this._9adArchReview.filter(c => c.status === 'partial').length;
    return {total: this._9adArchReview.length, passed: p, failed: f, partial: pa, score: Math.round((p / this._9adArchReview.length) * 100)};
  }


  private _z9adIncidentScenarios = [
    {id: 'z9ad-IS-001', name: 'Ransomware Outbreak in Production Environment', category: 'Malware', likelihood: 'Medium', impact: 'Critical', riskScore: 15, playbooks: ['IR-Ransomware-Isolate', 'IR-Ransomware-Eradicate', 'IR-Ransomway-Recover'], estimatedMTTD: '15min', estimatedMTTR: '4h', involvedTeams: ['SOC', 'IR', 'Infrastructure', 'Legal', 'Comms'], keyIndicators: ['Mass file encryption events', 'Ransom note deployment', 'Lateral movement via SMB/RDP', 'Backup deletion attempts'], mitigationControls: ['EDR behavioral detection', 'Network segmentation', 'Immutable backups', 'Email filtering'], lessonsLearned: 'Ensure offline backups are tested quarterly and Ransomware-specific playbooks are updated with latest threat intelligence', lastDrillDate: '2026-03-15', nextDrillDate: '2026-06-15'},
    {id: 'z9ad-IS-002', name: 'Cloud Misconfiguration Leading to Data Exposure', category: 'Cloud', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Cloud-Misconfig-Assess', 'IR-Cloud-Misconfig-Remediate', 'IR-Cloud-Misconfig-Notify'], estimatedMTTD: '2h', estimatedMTTR: '6h', involvedTeams: ['Cloud Security', 'SOC', 'Data Protection', 'Legal'], keyIndicators: ['Public S3 bucket exposure', 'Overly permissive IAM policies', 'Unencrypted database snapshots', 'Public API endpoint discovery'], mitigationControls: ['CSPM monitoring', 'IAM policy analyzer', 'Automated remediation', 'Cloud audit logging'], lessonsLearned: 'Implement CSPM with auto-remediation for common misconfigurations and conduct monthly cloud configuration reviews', lastDrillDate: '2026-02-20', nextDrillDate: '2026-05-20'},
    {id: 'z9ad-IS-003', name: 'Insider Threat Data Exfiltration', category: 'Insider', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-Insider-Investigate', 'IR-Insider-Contain', 'IR-Insider-Escalate'], estimatedMTTD: '8h', estimatedMTTR: '24h', involvedTeams: ['SOC', 'IR', 'HR', 'Legal', 'IAM'], keyIndicators: ['Anomalous data access patterns', 'Large file downloads to USB or cloud', 'Off-hours access to sensitive systems', 'Accessing data outside normal job scope'], mitigationControls: ['UEBA analytics', 'DLP policies', 'PAM controls', 'Access reviews'], lessonsLearned: 'Strengthen UEBA baselines and implement automated alerts for data access pattern anomalies', lastDrillDate: '2026-01-10', nextDrillDate: '2026-04-10'},
    {id: 'z9ad-IS-004', name: 'Supply Chain Attack via Compromised Vendor Software', category: 'Supply Chain', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-SupplyChain-Detect', 'IR-SupplyChain-Isolate', 'IR-SupplyChain-Remediate'], estimatedMTTD: '24h', estimatedMTTR: '72h', involvedTeams: ['SOC', 'IR', 'Procurement', 'Vendor Mgmt', 'Legal'], keyIndicators: ['Unexpected network connections from vendor software', 'Modified binaries or dependencies', 'Anomalous behavior from trusted applications', 'SBOM discrepancy alerts'], mitigationControls: ['SCA scanning', 'SBOM monitoring', 'Vendor security assessments', 'Network segmentation'], lessonsLearned: 'Require SBOM from all vendors and implement continuous monitoring of third-party component behavior', lastDrillDate: '2026-03-01', nextDrillDate: '2026-06-01'},
    {id: 'z9ad-IS-005', name: 'Credential Stuffing Attack on Customer Portal', category: 'External Attack', likelihood: 'High', impact: 'Medium', riskScore: 12, playbooks: ['IR-CredentialStuffing-Detect', 'IR-CredentialStuffing-Block', 'IR-CredentialStuffing-Notify'], estimatedMTTD: '30min', estimatedMTTR: '2h', involvedTeams: ['SOC', 'AppSec', 'Customer Support', 'Comms'], keyIndicators: ['High volume of failed login attempts', 'Login attempts from known compromised credential lists', 'Unusual geographic distribution of login attempts', 'Account takeover indicators'], mitigationControls: ['WAF rate limiting', 'CAPTCHA on login', 'MFA enforcement', 'Credential monitoring service'], lessonsLearned: 'Deploy advanced bot detection on login endpoints and implement proactive credential monitoring for customer accounts', lastDrillDate: '2026-04-01', nextDrillDate: '2026-07-01'},
    {id: 'z9ad-IS-006', name: 'Phishing Campaign Targeting Executive Team', category: 'Social Engineering', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Phishing-Contain', 'IR-Phishing-Investigate', 'IR-Phishing-Notify'], estimatedMTTD: '1h', estimatedMTTR: '4h', involvedTeams: ['SOC', 'Awareness', 'Executive Office', 'IT'], keyIndicators: ['Sophisticated spear-phishing emails to C-suite', 'Business email compromise indicators', 'Redirected MFA approval requests', 'Unauthorized wire transfer attempts'], mitigationControls: ['AI email filtering', 'Executive awareness training', 'Out-of-band verification policy', 'DMARC/DKIM/SPF'], lessonsLearned: 'Implement additional email security controls for executive mailboxes and conduct monthly spear-phishing simulations', lastDrillDate: '2026-02-15', nextDrillDate: '2026-05-15'}
  ];

  private _z9adSecurityFrameworks = [
  ...
  private _getz9adScenarioSummary(): {total: number; critical: number; high: number; drilled: number; avgRiskScore: number} {
    const crit = this._z9adIncidentScenarios.filter(s => s.impact === 'Critical').length;
    const high = this._z9adIncidentScenarios.filter(s => s.impact === 'High').length;
    const avg = this._z9adIncidentScenarios.reduce((s, x) => s + x.riskScore, 0) / this._z9adIncidentScenarios.length;
    return {total: this._z9adIncidentScenarios.length, critical: crit, high: high, drilled: this._z9adIncidentScenarios.length, avgRiskScore: Math.round(avg)};
  }

  private _getz9adVendorRiskSummary(): {total: number; approved: number; conditional: number; remediation: number; avgScore: number} {
    const approved = this._z9adThirdPartyRiskData.filter(v => v.status === 'approved').length;
    const cond = this._z9adThirdPartyRiskData.filter(v => v.status === 'conditional').length;
    const rem = this._z9adThirdPartyRiskData.filter(v => v.status === 'remediation_required').length;
    const avg = this._z9adThirdPartyRiskData.reduce((s, v) => s + v.riskScore, 0) / this._z9adThirdPartyRiskData.length;
    return {total: this._z9adThirdPartyRiskData.length, approved, conditional: cond, remediation: rem, avgScore: Math.round(avg)};
  }

  private _getz9adTrainingSummary(): {total: number; avgCompletion: number; avgScore: number; overdueUpdates: number} {
    const avgComp = this._z9adSecurityTrainingCatalog.reduce((s, t) => s + t.completionRate, 0) / this._z9adSecurityTrainingCatalog.length;
    const avgScore = this._z9adSecurityTrainingCatalog.filter(t => t.avgScore > 0).reduce((s, t) => s + t.avgScore, 0) / this._z9adSecurityTrainingCatalog.filter(t => t.avgScore > 0).length;
    return {total: this._z9adSecurityTrainingCatalog.length, avgCompletion: Math.round(avgComp * 10) / 10, avgScore: Math.round(avgScore), overdueUpdates: 0};
  }

  private _getz9adToolHealthSummary(): {total: number; healthy: number; degraded: number; annualCost: string} {
    const healthy = this._z9adSecurityToolInventory.filter(t => t.health === 'healthy').length;
    const degraded = this._z9adSecurityToolInventory.filter(t => t.health === 'degraded').length;
    const cost = this._z9adSecurityToolInventory.reduce((s, t) => s + parseInt(t.annualCost.replace(/[^0-9]/g, '')), 0);
    return {total: this._z9adSecurityToolInventory.length, healthy, degraded, annualCost: '$' + (cost / 1000).toFixed(0) + 'K'};
  }


  private _x9adSecurityZones = [
    {zoneId: 'x9ad-Z-001', name: 'Internet DMZ', trustLevel: 'Untrusted', subnet: '10.0.0.0/24', segmentation: 'Dual-firewall', firewallRules: 45, monitoring: 'Full packet capture', assets: ['Web servers', 'Load balancers', 'WAF appliances', 'Reverse proxy'], riskLevel: 'high' as const, lastAssessment: '2026-04-15', nextAssessment: '2026-07-15'},
    {zoneId: 'x9ad-Z-002', name: 'Corporate Network', trustLevel: 'Trusted', subnet: '10.1.0.0/16', segmentation: 'VLAN + micro-seg', firewallRules: 128, monitoring: 'NetFlow + IDS', assets: ['Workstations', 'Printers', 'Meeting room systems', 'VoIP phones'], riskLevel: 'medium' as const, lastAssessment: '2026-04-10', nextAssessment: '2026-07-10'},
    {zoneId: 'x9ad-Z-003', name: 'Production Data Center', trustLevel: 'Restricted', subnet: '10.2.0.0/16', segmentation: 'Full micro-segmentation', firewallRules: 256, monitoring: 'Full packet capture + EDR', assets: ['Application servers', 'Database clusters', 'Message queues', 'API gateways'], riskLevel: 'low' as const, lastAssessment: '2026-04-12', nextAssessment: '2026-07-12'},
    {zoneId: 'x9ad-Z-004', name: 'Management Network', trustLevel: 'Restricted', subnet: '10.3.0.0/24', segmentation: 'Isolated VLAN', firewallRules: 32, monitoring: 'Syslog + SNMP', assets: ['Hypervisors', 'Switch management', 'Storage controllers', 'Backup appliances'], riskLevel: 'low' as const, lastAssessment: '2026-04-08', nextAssessment: '2026-07-08'},
    {zoneId: 'x9ad-Z-005', name: 'Cloud Production', trustLevel: 'Restricted', subnet: 'VPC: 172.16.0.0/12', segmentation: 'Security groups + NACLs', firewallRules: 189, monitoring: 'VPC Flow Logs + GuardDuty', assets: ['Kubernetes clusters', 'Managed databases', 'Object storage', 'Serverless functions'], riskLevel: 'medium' as const, lastAssessment: '2026-04-18', nextAssessment: '2026-07-18'}
  ];

  private _x9adSecurityAlerts = [
    {id: 'x9ad-AL-001', severity: 'critical', title: 'Active ransomware encryption detected on PROD-DB-01', source: 'CrowdStrike', timestamp: '2026-04-22T13:45:00Z', status: 'investigating', assignee: 'SOC Analyst J. Smith', asset: 'PROD-DB-01 (10.2.1.15)', mitreTechnique: 'T1486 - Data Encrypted for Impact', playbook: 'IR-Ransomware-Isolate', sla: '15 minutes', elapsed: '12 minutes', notes: 'EDR detected mass file encryption event pattern. Automated isolation triggered. IR team paged.'},
    {id: 'x9ad-AL-002', severity: 'high', title: 'Impossible travel detected for user admin@company.com', source: 'Okta', timestamp: '2026-04-22T12:30:00Z', status: 'triaged', assignee: 'SOC Analyst M. Johnson', asset: 'Okta - admin@company.com', mitreTechnique: 'T1078 - Valid Accounts', playbook: 'IR-ImpossibleTravel', sla: '30 minutes', elapsed: '87 minutes', notes: 'Login from Tokyo followed by login from New York within 30 minutes. Account temporarily locked. User confirmed via out-of-band channel they are in New York.'},
    {id: 'x9ad-AL-003', severity: 'high', title: 'Public S3 bucket discovered with customer PII exposure', source: 'Prisma Cloud', timestamp: '2026-04-22T11:15:00Z', status: 'remediating', assignee: 'Cloud Security Lead A. Chen', asset: 'AWS S3: customer-data-archive-2024', mitreTechnique: 'N/A - Misconfiguration', playbook: 'IR-Cloud-Misconfig-Remediate', sla: '1 hour', elapsed: '2.5 hours', notes: 'Bucket containing archived customer records was publicly accessible. Bucket blocked and encrypted. Data exposure assessment in progress.'},
    {id: 'x9ad-AL-004', severity: 'medium', title: 'Anomalous data download by user jane.developer@company.com', source: 'Microsoft DLP', timestamp: '2026-04-22T10:00:00Z', status: 'investigating', assignee: 'SOC Analyst R. Davis', asset: 'Endpoint: WS-DEV-042', mitreTechnique: 'T1567 - Exfil Over Web Service', playbook: 'IR-DLP-Investigation', sla: '4 hours', elapsed: '3.5 hours', notes: 'User downloaded 4.2GB of source code files to personal Google Drive. Interview scheduled with user and manager.'},
    {id: 'x9ad-AL-005', severity: 'medium', title: 'Credential stuffing attack detected on customer portal', source: 'Cloudflare WAF', timestamp: '2026-04-22T09:30:00Z', status: 'contained', assignee: 'SOC Analyst K. Wilson', asset: 'Customer Portal (portal.company.com)', mitreTechnique: 'T1110 - Brute Force', playbook: 'IR-CredentialStuffing-Block', sla: '2 hours', elapsed: '4 hours', notes: 'Rate limiting and CAPTCHA automatically deployed. 12,000 failed login attempts from 800 unique IPs blocked. No successful account takeovers confirmed.'},
    {id: 'x9ad-AL-006', severity: 'low', title: 'SSL certificate expiring in 14 days for api.company.com', source: 'Certificate monitoring', timestamp: '2026-04-22T08:00:00Z', status: 'remediated', assignee: 'Infrastructure Lead T. Brown', asset: 'api.company.com', mitreTechnique: 'N/A', playbook: 'Certificate Renewal', sla: '30 days', elapsed: '16 days', notes: 'Automated renewal triggered via ACME. New certificate deployed successfully. No service interruption.'}
  ];

  private _x9adComplianceFrameworks = [
  ...
  private _x9adDataClassificationPolicy = [
  ...
  private _getx9adZoneSummary(): {total: number; highRisk: number; mediumRisk: number; lowRisk: number; totalRules: number} {
    const high = this._x9adSecurityZones.filter(z => z.riskLevel === 'high').length;
    const med = this._x9adSecurityZones.filter(z => z.riskLevel === 'medium').length;
    const low = this._x9adSecurityZones.filter(z => z.riskLevel === 'low').length;
    const rules = this._x9adSecurityZones.reduce((s, z) => s + z.firewallRules, 0);
    return {total: this._x9adSecurityZones.length, highRisk: high, mediumRisk: med, lowRisk: low, totalRules: rules};
  }

  private _getx9adAlertSummary(): {total: number; critical: number; high: number; medium: number; low: number; open: number; contained: number} {
    const crit = this._x9adSecurityAlerts.filter(a => a.severity === 'critical').length;
    const high = this._x9adSecurityAlerts.filter(a => a.severity === 'high').length;
    const med = this._x9adSecurityAlerts.filter(a => a.severity === 'medium').length;
    const low = this._x9adSecurityAlerts.filter(a => a.severity === 'low').length;
    const open = this._x9adSecurityAlerts.filter(a => a.status === 'investigating' || a.status === 'triaged').length;
    const contained = this._x9adSecurityAlerts.filter(a => a.status === 'contained' || a.status === 'remediated').length;
    return {total: this._x9adSecurityAlerts.length, critical: crit, high, medium: med, low, open, contained};
  }

  private _getx9adComplianceSummary(): {total: number; fullyCompliant: number; mostlyCompliant: number; avgComplianceRate: number} {
    const full = this._x9adComplianceFrameworks.filter(f => f.status === 'compliant').length;
    const mostly = this._x9adComplianceFrameworks.filter(f => f.status === 'mostly_compliant').length;
    const avgRate = this._x9adComplianceFrameworks.reduce((s, f) => s + (f.compliant / f.requirements * 100), 0) / this._x9adComplianceFrameworks.length;
    return {total: this._x9adComplianceFrameworks.length, fullyCompliant: full, mostlyCompliant: mostly, avgComplianceRate: Math.round(avgRate * 10) / 10};
  }
  ...
  // === Security Risk Register (log_query) ===
  private _log_queryRiskRegister = [
    { id: "RSK-LOG-0001", title: "Data breach from unpatched systems", owner: "CTO", category: "Financial", status: "mitigating", trend: "stable", severity: "critical", likelihood: 8, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-08", nextReview: "2026-07-15" },
    { id: "RSK-LOG-0002", title: "Ransomware attack on critical infrastructure", owner: "CRO", category: "Strategic", status: "closed", trend: "decreasing", severity: "low", likelihood: 10, impact: 9, treatment: "Monitor and review quarterly", lastReview: "2026-04-11", nextReview: "2026-07-08" },
    { id: "RSK-LOG-0003", title: "Insider threat data exfiltration", owner: "VP Eng", category: "Compliance", status: "accepted", trend: "increasing", severity: "medium", likelihood: 3, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-21", nextReview: "2026-07-04" },
    { id: "RSK-LOG-0004", title: "Supply chain compromise", owner: "Security Lead", category: "Technical", status: "open", trend: "stable", severity: "low", likelihood: 10, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-13", nextReview: "2026-07-18" },
    { id: "RSK-LOG-0005", title: "Cloud misconfiguration exposure", owner: "Risk Mgr", category: "Reputational", status: "mitigating", trend: "decreasing", severity: "medium", likelihood: 4, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-08", nextReview: "2026-07-05" },
    { id: "RSK-LOG-0006", title: "Phishing campaign success rate", owner: "Compliance Dir", category: "Legal", status: "closed", trend: "increasing", severity: "critical", likelihood: 6, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-15", nextReview: "2026-07-03" },
    { id: "RSK-LOG-0007", title: "Third-party vendor data breach", owner: "IT Dir", category: "Regulatory", status: "accepted", trend: "stable", severity: "low", likelihood: 5, impact: 9, treatment: "Monitor and review quarterly", lastReview: "2026-04-16", nextReview: "2026-07-20" },
    { id: "RSK-LOG-0008", title: "Regulatory non-compliance penalty", owner: "DevOps Lead", category: "Third-Party", status: "open", trend: "decreasing", severity: "high", likelihood: 1, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-12", nextReview: "2026-07-13" },
    { id: "RSK-LOG-0009", title: "Zero-day exploit in production", owner: "Architect", category: "Human Capital", status: "mitigating", trend: "increasing", severity: "low", likelihood: 9, impact: 9, treatment: "Monitor and review quarterly", lastReview: "2026-04-09", nextReview: "2026-07-14" },
    { id: "RSK-LOG-0010", title: "Insufficient access controls", owner: "CISO", category: "Operational", status: "closed", trend: "stable", severity: "critical", likelihood: 2, impact: 4, treatment: "Monitor and review quarterly", lastReview: "2026-04-19", nextReview: "2026-07-23" },
    { id: "RSK-LOG-0011", title: "DDoS attack on services", owner: "CTO", category: "Financial", status: "accepted", trend: "decreasing", severity: "critical", likelihood: 7, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-14", nextReview: "2026-07-24" },
    { id: "RSK-LOG-0012", title: "Social engineering attack", owner: "CRO", category: "Strategic", status: "open", trend: "increasing", severity: "critical", likelihood: 6, impact: 9, treatment: "Monitor and review quarterly", lastReview: "2026-04-01", nextReview: "2026-07-07" },
    { id: "RSK-LOG-0013", title: "API security vulnerability", owner: "VP Eng", category: "Compliance", status: "mitigating", trend: "stable", severity: "low", likelihood: 6, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-15", nextReview: "2026-07-17" },
    { id: "RSK-LOG-0014", title: "Mobile device compromise", owner: "Security Lead", category: "Technical", status: "closed", trend: "decreasing", severity: "low", likelihood: 5, impact: 4, treatment: "Monitor and review quarterly", lastReview: "2026-04-01", nextReview: "2026-07-12" },
    { id: "RSK-LOG-0015", title: "Physical security breach", owner: "Risk Mgr", category: "Reputational", status: "accepted", trend: "increasing", severity: "low", likelihood: 2, impact: 9, treatment: "Monitor and review quarterly", lastReview: "2026-04-20", nextReview: "2026-07-03" },
    { id: "RSK-LOG-0016", title: "Password policy weakness", owner: "Compliance Dir", category: "Legal", status: "open", trend: "stable", severity: "medium", likelihood: 7, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-19", nextReview: "2026-07-18" },
    { id: "RSK-LOG-0017", title: "Network segmentation gap", owner: "IT Dir", category: "Regulatory", status: "mitigating", trend: "decreasing", severity: "low", likelihood: 9, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-09", nextReview: "2026-07-02" },
    { id: "RSK-LOG-0018", title: "Encryption key management failure", owner: "DevOps Lead", category: "Third-Party", status: "closed", trend: "increasing", severity: "critical", likelihood: 7, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-13", nextReview: "2026-07-16" },
    { id: "RSK-LOG-0019", title: "Audit trail tampering", owner: "Architect", category: "Human Capital", status: "accepted", trend: "stable", severity: "medium", likelihood: 8, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-05", nextReview: "2026-07-25" },
    { id: "RSK-LOG-0020", title: "Business email compromise", owner: "CISO", category: "Operational", status: "open", trend: "decreasing", severity: "critical", likelihood: 9, impact: 4, treatment: "Monitor and review quarterly", lastReview: "2026-04-22", nextReview: "2026-07-13" },
  ];
  private _log_queryRiskFilter: string = 'all';
  private _log_queryRiskSeverity: string = 'all';
  private _log_queryRiskStatus: string = 'all';
  private _log_queryExpandedRisk: string = '';
  private _log_queryFilterRisks() {
    const reg = this._log_queryRiskRegister;
    return reg.filter(r => {
      if (this._log_queryRiskFilter !== "all" && r.category !== this._log_queryRiskFilter) return false;
      if (this._log_queryRiskSeverity !== "all" && r.severity !== this._log_queryRiskSeverity) return false;
      if (this._log_queryRiskStatus !== "all" && r.status !== this._log_queryRiskStatus) return false;
      return true;
    });
  }
  private _log_queryGetRiskScore(r: any) { return Math.round(r.likelihood * r.impact * 1.5); }
  private _log_queryGetRiskTrendIcon(trend: string) {
    if (trend === "increasing") return "\u2191";
    if (trend === "decreasing") return "\u2193";
    return "\u2192";
  }
  private _log_queryGetRiskColor(sev: string) {
    if (sev === "critical") return "#dc2626";
    if (sev === "high") return "#ea580c";
    if (sev === "medium") return "#d97706";
    return "#16a34a";
  }
  private _log_queryGetRiskCounts() {
    const reg = this._log_queryRiskRegister;
  ...
  private _log_queryGetTreatmentProgress() {
    const reg = this._log_queryRiskRegister;
    const treated = reg.filter(r => r.status === "mitigating" || r.status === "closed").length;
    return Math.round((treated / Math.max(reg.length, 1)) * 100);
  }

  // === Security Metrics API Gateway (log_query) ===
  private _log_queryApiEndpoints = [
    { method: "GET", path: "/api/v1/threats", name: "Threat Intelligence Feed", status: "active", avgLatency: 309.6, reqPerMin: 2046.0, errorRate: 0.08, uptime: round(98.26759023979646,2), version: "v3.5.16" },
    { method: "POST", path: "/api/v1/scans/start", name: "Vulnerability Scanner", status: "active", avgLatency: 44.8, reqPerMin: 2297.0, errorRate: 2.37, uptime: round(97.54748232885606,2), version: "v2.7.5" },
    { method: "GET", path: "/api/v1/assets", name: "Asset Inventory", status: "active", avgLatency: 225.0, reqPerMin: 731.0, errorRate: 1.3, uptime: round(98.56569338786642,2), version: "v3.5.10" },
    { method: "POST", path: "/api/v1/alerts", name: "Alert Management", status: "active", avgLatency: 394.8, reqPerMin: 1717.0, errorRate: 1.13, uptime: round(98.3740039128873,2), version: "v1.4.10" },
    { method: "GET", path: "/api/v1/compliance", name: "Compliance Status", status: "active", avgLatency: 210.5, reqPerMin: 1520.0, errorRate: 0.42, uptime: round(99.8075420745043,2), version: "v3.2.6" },
    { method: "PUT", path: "/api/v1/policies", name: "Policy Engine", status: "active", avgLatency: 360.9, reqPerMin: 231.0, errorRate: 0.65, uptime: round(98.85215518282749,2), version: "v1.2.3" },
    { method: "GET", path: "/api/v1/incidents", name: "Incident Tracker", status: "active", avgLatency: 298.0, reqPerMin: 910.0, errorRate: 0.28, uptime: round(98.54919497114504,2), version: "v1.7.14" },
    { method: "POST", path: "/api/v1/forensics", name: "Forensics Collector", status: "degraded", avgLatency: 321.3, reqPerMin: 1672.0, errorRate: 0.52, uptime: round(97.67993321575412,2), version: "v3.4.6" },
    { method: "GET", path: "/api/v1/risk/assess", name: "Risk Assessment", status: "active", avgLatency: 424.4, reqPerMin: 1936.0, errorRate: 1.84, uptime: round(95.25422953883049,2), version: "v3.5.18" },
    { method: "POST", path: "/api/v1/auth/verify", name: "Authentication", status: "active", avgLatency: 24.5, reqPerMin: 1037.0, errorRate: 0.37, uptime: round(99.35537908232288,2), version: "v1.1.11" },
    { method: "GET", path: "/api/v1/logs/audit", name: "Audit Log Query", status: "active", avgLatency: 142.3, reqPerMin: 526.0, errorRate: 1.71, uptime: round(97.3755505927844,2), version: "v2.5.9" },
    { method: "PUT", path: "/api/v1/users/roles", name: "Role Management", status: "active", avgLatency: 291.9, reqPerMin: 1548.0, errorRate: 2.0, uptime: round(95.32661522381602,2), version: "v3.5.16" },
    { method: "POST", path: "/api/v1/encrypt", name: "Encryption Service", status: "active", avgLatency: 16.4, reqPerMin: 198.0, errorRate: 0.39, uptime: round(97.44945548913829,2), version: "v2.9.8" },
    { method: "GET", path: "/api/v1/network/topo", name: "Network Topology", status: "maintenance", avgLatency: 445.8, reqPerMin: 2225.0, errorRate: 2.45, uptime: round(97.04975685676693,2), version: "v2.1.18" },
    { method: "DELETE", path: "/api/v1/sessions", name: "Session Manager", status: "active", avgLatency: 301.3, reqPerMin: 1638.0, errorRate: 1.6, uptime: round(97.25079388041483,2), version: "v3.6.0" },
  ];
  private _log_queryApiKeys = [
    { id: "ak-000001", name: "key-log_-001", created: "2026-08-09", lastUsed: "2026-04-02", status: "active", calls: 38600, rateLimit: 500 },
    { id: "ak-000002", name: "key-log_-002", created: "2026-02-27", lastUsed: "2026-04-02", status: "active", calls: 5844, rateLimit: 500 },
    { id: "ak-000003", name: "key-log_-003", created: "2026-01-09", lastUsed: "2026-04-10", status: "active", calls: 40709, rateLimit: 5000 },
    { id: "ak-000004", name: "key-log_-004", created: "2026-09-16", lastUsed: "2026-04-03", status: "active", calls: 23549, rateLimit: 1000 },
    { id: "ak-000005", name: "key-log_-005", created: "2026-08-12", lastUsed: "2026-04-22", status: "active", calls: 11659, rateLimit: 1000 },
    { id: "ak-000006", name: "key-log_-006", created: "2026-05-18", lastUsed: "2026-04-12", status: "active", calls: 46928, rateLimit: 100 },
    { id: "ak-000007", name: "key-log_-007", created: "2026-02-20", lastUsed: "2026-04-07", status: "active", calls: 19324, rateLimit: 500 },
    { id: "ak-000008", name: "key-log_-008", created: "2026-06-24", lastUsed: "2026-04-04", status: "revoked", calls: 7301, rateLimit: 100 },
  ];
  private _log_queryApiHealthSummary() {
    const eps = this._log_queryApiEndpoints;
  ...
  private _log_queryGetApiByMethod(method: string) { return this._log_queryApiEndpoints.filter(e=>e.method===method); }
  private _log_queryGetSlowEndpoints() { return this._log_queryApiEndpoints.filter(e=>e.avgLatency>200).sort((a,b)=>b.avgLatency-a.avgLatency); }
  private _log_queryGetHighErrorEndpoints() { return this._log_queryApiEndpoints.filter(e=>e.errorRate>1.0).sort((a,b)=>b.errorRate-a.errorRate); }

  // === Security Training Effectiveness (log_query) ===
  private _log_queryTrainingModules = [
    { id: "TRN-001", name: "Security Fundamentals", completionRate: 69.2, avgScore: 67.4, behaviorChange: 31.7, enrolled: 458, completed: 296, duration: "81min", category: "mandatory" },
    { id: "TRN-002", name: "Phishing Awareness", completionRate: 68.7, avgScore: 88.9, behaviorChange: 34.0, enrolled: 479, completed: 234, duration: "42min", category: "mandatory" },
    { id: "TRN-003", name: "Social Engineering Defense", completionRate: 67.9, avgScore: 80.7, behaviorChange: 41.0, enrolled: 242, completed: 347, duration: "35min", category: "optional" },
    { id: "TRN-004", name: "Password Hygiene", completionRate: 88.0, avgScore: 90.4, behaviorChange: 71.8, enrolled: 439, completed: 27, duration: "69min", category: "mandatory" },
    { id: "TRN-005", name: "Data Classification", completionRate: 93.9, avgScore: 77.6, behaviorChange: 47.0, enrolled: 205, completed: 162, duration: "86min", category: "mandatory" },
    { id: "TRN-006", name: "Incident Response Basics", completionRate: 65.4, avgScore: 73.3, behaviorChange: 67.6, enrolled: 414, completed: 164, duration: "48min", category: "mandatory" },
    { id: "TRN-007", name: "Secure Coding", completionRate: 74.5, avgScore: 57.6, behaviorChange: 64.0, enrolled: 128, completed: 332, duration: "45min", category: "optional" },
    { id: "TRN-008", name: "Cloud Security", completionRate: 94.6, avgScore: 71.6, behaviorChange: 37.5, enrolled: 364, completed: 248, duration: "85min", category: "mandatory" },
    { id: "TRN-009", name: "Mobile Device Security", completionRate: 53.3, avgScore: 94.9, behaviorChange: 55.8, enrolled: 157, completed: 245, duration: "102min", category: "mandatory" },
    { id: "TRN-010", name: "Network Security", completionRate: 74.1, avgScore: 56.9, behaviorChange: 45.9, enrolled: 317, completed: 295, duration: "46min", category: "mandatory" },
    { id: "TRN-011", name: "Physical Security", completionRate: 72.6, avgScore: 80.4, behaviorChange: 80.4, enrolled: 348, completed: 30, duration: "36min", category: "optional" },
    { id: "TRN-012", name: "Regulatory Compliance", completionRate: 57.3, avgScore: 71.8, behaviorChange: 82.0, enrolled: 446, completed: 371, duration: "70min", category: "mandatory" },
    { id: "TRN-013", name: "Risk Management", completionRate: 60.9, avgScore: 59.7, behaviorChange: 73.8, enrolled: 372, completed: 332, duration: "49min", category: "mandatory" },
    { id: "TRN-014", name: "Cryptography Basics", completionRate: 54.8, avgScore: 88.5, behaviorChange: 71.9, enrolled: 144, completed: 55, duration: "41min", category: "mandatory" },
    { id: "TRN-015", name: "Access Control", completionRate: 70.6, avgScore: 77.5, behaviorChange: 51.5, enrolled: 212, completed: 381, duration: "33min", category: "optional" },
    { id: "TRN-016", name: "Vendor Management", completionRate: 50.9, avgScore: 80.6, behaviorChange: 66.1, enrolled: 230, completed: 265, duration: "110min", category: "mandatory" },
  ];
  private _log_queryPhishingResults = [
    { month: "2026-01", sent: 403, clicked: 52, reported: 127, clickRate: round(12.903225806451612,1), reportRate: round(31.5136476426799,1) },
    { month: "2026-02", sent: 226, clicked: 9, reported: 131, clickRate: round(3.982300884955752,1), reportRate: round(57.9646017699115,1) },
    { month: "2026-03", sent: 850, clicked: 174, reported: 420, clickRate: round(20.47058823529412,1), reportRate: round(49.411764705882355,1) },
    { month: "2026-04", sent: 413, clicked: 31, reported: 176, clickRate: round(7.506053268765134,1), reportRate: round(42.615012106537534,1) },
    { month: "2026-05", sent: 910, clicked: 185, reported: 56, clickRate: round(20.32967032967033,1), reportRate: round(6.153846153846154,1) },
    { month: "2026-06", sent: 664, clicked: 117, reported: 326, clickRate: round(17.620481927710845,1), reportRate: round(49.096385542168676,1) },
  ];
  private _log_queryTrainingROI = { totalInvestment: 95191, avgCostPerEmployee: 421, riskReductionPct: round(random.uniform(15,45),1), incidentReductionPct: round(random.uniform(10,35),1), complianceScoreGain: round(random.uniform(5,25),1) };
  private _log_queryLearningPaths = [
    { name: "Beginner Security Analyst", totalModules: 12, completedModules: 12, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 63, enrolled: 184 },
    { name: "Advanced Threat Hunter", totalModules: 19, completedModules: 14, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 14, enrolled: 34 },
    { name: "Security Architect", totalModules: 18, completedModules: 5, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 62, enrolled: 95 },
    { name: "Incident Responder", totalModules: 11, completedModules: 9, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 17, enrolled: 190 },
    { name: "Compliance Specialist", totalModules: 20, completedModules: 14, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 62, enrolled: 51 },
    { name: "DevSecOps Engineer", totalModules: 16, completedModules: 18, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 21, enrolled: 157 },
  ];
  private _log_queryGetOverallCompletion() {
    const mods = this._log_queryTrainingModules;
    return round(mods.reduce((s,m)=>s+m.completionRate,0)/mods.length,1);
  }
  private _log_queryGetTopPerformers() { return [...this._log_queryTrainingModules].sort((a,b)=>b.avgScore-a.avgScore).slice(0,5); }
  private _log_queryGetModulesNeedingAttention() { return this._log_queryTrainingModules.filter(m=>m.completionRate<70||m.avgScore<65); }

  // === Security Governance Framework (log_query) ===
  private _log_queryGovBodies = [
    { name: "Security Steering Committee", members: 15, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-18", nextMeeting: "2026-05-09", quorum: 6 },
    { name: "Risk Management Board", members: 8, chair: "CTO", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-03", nextMeeting: "2026-05-15", quorum: 5 },
    { name: "Data Governance Council", members: 7, chair: "CRO", meetingFreq: "Monthly", lastMeeting: "2026-04-01", nextMeeting: "2026-05-20", quorum: 4 },
    { name: "Compliance Oversight Board", members: 10, chair: "CDO", meetingFreq: "Monthly", lastMeeting: "2026-04-18", nextMeeting: "2026-05-22", quorum: 7 },
    { name: "Architecture Review Board", members: 11, chair: "VP Eng", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-13", nextMeeting: "2026-05-18", quorum: 7 },
    { name: "Change Advisory Board", members: 15, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-10", nextMeeting: "2026-05-28", quorum: 5 },
    { name: "Incident Review Board", members: 12, chair: "CIO", meetingFreq: "As needed", lastMeeting: "2026-04-12", nextMeeting: "2026-05-02", quorum: 3 },
    { name: "Vendor Risk Committee", members: 15, chair: "CFO", meetingFreq: "Quarterly", lastMeeting: "2026-04-21", nextMeeting: "2026-05-28", quorum: 3 },
  ];
  private _log_queryDecisions = [
    { id: "DEC-0001", title: "Adopt zero-trust architecture framework", date: "2026-03-10", status: "approved", owner: "CISO" },
    { id: "DEC-0002", title: "Migrate to SIEM 2.0 platform", date: "2026-04-15", status: "implemented", owner: "CTO" },
    { id: "DEC-0003", title: "Implement DLP across all endpoints", date: "2026-01-18", status: "in-progress", owner: "Security Lead" },
    { id: "DEC-0004", title: "Mandate MFA for all external access", date: "2026-01-15", status: "pending", owner: "CRO" },
    { id: "DEC-0005", title: "Establish bug bounty program", date: "2026-01-07", status: "approved", owner: "VP Eng" },
    ...
    { id: "DEC-0009", title: "Establish security champion program", date: "2026-01-12", status: "pending", owner: "CRO" },
    { id: "DEC-0010", title: "Migrate to passwordless authentication", date: "2026-04-21", status: "approved", owner: "VP Eng" },
  ];
  private _log_queryPolicyLifecycle = [
    { name: "Information Security Policy", version: "v1.3", status: "active", lastUpdated: "2026-02-07", nextReview: "2026-9-09", owner: "CISO" },
    { name: "Acceptable Use Policy", version: "v4.4", status: "active", lastUpdated: "2026-04-23", nextReview: "2026-9-06", owner: "Legal" },
    { name: "Data Retention Policy", version: "v1.4", status: "under-review", lastUpdated: "2026-02-13", nextReview: "2026-8-26", owner: "DPO" },
    { name: "Access Control Policy", version: "v3.2", status: "active", lastUpdated: "2026-01-22", nextReview: "2026-7-02", owner: "IAM Lead" },
    { name: "Incident Response Policy", version: "v3.2", status: "draft", lastUpdated: "2026-04-27", nextReview: "2026-7-28", owner: "IR Lead" },
    { name: "Business Continuity Plan", version: "v3.5", status: "active", lastUpdated: "2026-01-19", nextReview: "2026-8-05", owner: "BCP Mgr" },
    { name: "Vendor Management Policy", version: "v4.2", status: "active", lastUpdated: "2026-03-26", nextReview: "2026-9-10", owner: "Procurement" },
    { name: "Encryption Standard", version: "v3.2", status: "under-review", lastUpdated: "2026-03-25", nextReview: "2026-11-17", owner: "Security Arch" },
  ];
  private _log_queryGovMaturityScore = { overall: 4.1, riskManagement: 2.5, compliance: 2.6, incidentResponse: 2.7, awareness: 4.4, technology: 5.9 };
  private _log_queryGetPendingDecisions() { return this._log_queryDecisions.filter(d=>d.status==='pending'||d.status==='in-progress'); }
  private _log_queryGetActivePolicies() { return this._log_queryPolicyLifecycle.filter(p=>p.status==='active'); }
  private _log_queryGetEscalationPath() { return ["L1 Analyst","L2 Senior","Security Lead","CISO","Board"]; }

  // === Security Innovation Lab (log_query) ===
  private _log_queryInnoProjects = [
    { id: "INN-001", name: "AI-Powered Threat Detection", description: "Machine learning models for real-time threat identification", status: "active", progress: 72, startDate: "2026-02-24", teamSize: 7, budget: 52246, milestones: 7, completedMilestones: 1 },
    { id: "INN-002", name: "Quantum-Resistant Cryptography", description: "Post-quantum encryption algorithm prototyping", status: "research", progress: 35, startDate: "2026-03-25", teamSize: 2, budget: 51013, milestones: 10, completedMilestones: 3 },
    { id: "INN-003", name: "Automated Red Teaming", description: "Autonomous penetration testing framework", status: "active", progress: 58, startDate: "2026-01-06", teamSize: 8, budget: 115707, milestones: 6, completedMilestones: 6 },
    { id: "INN-004", name: "Zero-Knowledge Authentication", description: "Privacy-preserving identity verification", status: "poc", progress: 88, startDate: "2026-01-13", teamSize: 5, budget: 71768, milestones: 7, completedMilestones: 7 },
    { id: "INN-005", name: "Blockchain Audit Trail", description: "Immutable security event logging", status: "active", progress: 45, startDate: "2026-02-27", teamSize: 2, budget: 139131, milestones: 3, completedMilestones: 3 },
    { id: "INN-006", name: "Behavioral Biometrics", description: "Continuous authentication via user behavior patterns", status: "research", progress: 22, startDate: "2026-03-17", teamSize: 8, budget: 19807, milestones: 4, completedMilestones: 6 },
    { id: "INN-007", name: "Deception Grid 2.0", description: "Advanced honeypot network with adaptive responses", status: "active", progress: 65, startDate: "2026-01-19", teamSize: 7, budget: 42367, milestones: 8, completedMilestones: 9 },
    { id: "INN-008", name: "Secure Enclave Integration", description: "Hardware-backed security for critical workloads", status: "poc", progress: 40, startDate: "2026-02-12", teamSize: 4, budget: 132557, milestones: 9, completedMilestones: 6 },
  ];
  private _log_queryTechEvaluations = [
    { name: "Rust for Security Tools", status: "evaluating", score: 10.9, recommendation: "Adopt", vendor: "Open Source" },
    { name: "eBPF for Runtime Detection", status: "completed", score: 5.8, recommendation: "Adopt", vendor: "AWS" },
    { name: "Confidential Computing", status: "planned", score: 9.1, recommendation: "Investigate", vendor: "Azure" },
    { name: "Homomorphic Encryption", status: "evaluating", score: 8.9, recommendation: "Pilot", vendor: "GCP" },
  ...
  private _log_queryCollaborationPartners = [
  ...
  private _log_queryInnoMetrics = { totalProjects: 8, activeProjects: 4, avgTimeToValue: "86 days", pocSuccessRate: round(random.uniform(55,85),1), researchToProduction: round(random.uniform(20,50),1), innovationIndex: round(random.uniform(6.0,9.5),1) };
  private _log_queryGetProjectByStatus(status: string) { return this._log_queryInnoProjects.filter(p=>p.status===status); }
  private _log_queryGetTopEvaluations() { return [...this._log_queryTechEvaluations].sort((a,b)=>b.score-a.score).slice(0,3); }



  // === Compliance Dashboard Extension (log_query) ===
  private _log_queryComplianceFrameworks = [
    { name: "SOC 2 Type II", description: "Trust Services Criteria", totalControls: 5, implementedControls: 5, status: "in-review", lastAudit: "2026-01-14", nextAudit: "2026-9-24", evidenceCount: 94 },
    { name: "ISO 27001", description: "Information Security Management", totalControls: 114, implementedControls: 92, status: "in-review", lastAudit: "2026-03-20", nextAudit: "2026-8-03", evidenceCount: 364 },
    { name: "PCI DSS 4.0", description: "Payment Card Industry", totalControls: 12, implementedControls: 7, status: "partially-compliant", lastAudit: "2026-02-05", nextAudit: "2026-7-22", evidenceCount: 99 },
    { name: "HIPAA", description: "Health Insurance Portability", totalControls: 18, implementedControls: 13, status: "compliant", lastAudit: "2026-01-20", nextAudit: "2026-9-11", evidenceCount: 185 },
    { name: "GDPR", description: "Data Protection Regulation", totalControls: 99, implementedControls: 99, status: "non-compliant", lastAudit: "2026-03-16", nextAudit: "2026-11-26", evidenceCount: 260 },
    { name: "NIST CSF 2.0", description: "Cybersecurity Framework", totalControls: 6, implementedControls: 5, status: "in-review", lastAudit: "2026-04-15", nextAudit: "2026-9-23", evidenceCount: 442 },
    { name: "FedRAMP", description: "Federal Risk Authorization", totalControls: 15, implementedControls: 11, status: "in-review", lastAudit: "2026-03-28", nextAudit: "2026-8-13", evidenceCount: 343 },
    { name: "SOX", description: "Sarbanes-Oxley Compliance", totalControls: 8, implementedControls: 4, status: "compliant", lastAudit: "2026-01-24", nextAudit: "2026-8-23", evidenceCount: 491 },
    { name: "CIS Controls v8", description: "Center for Internet Security", totalControls: 18, implementedControls: 10, status: "non-compliant", lastAudit: "2026-04-20", nextAudit: "2026-10-13", evidenceCount: 386 },
    { name: "COBIT 2019", description: "IT Governance Framework", totalControls: 40, implementedControls: 29, status: "non-compliant", lastAudit: "2026-04-27", nextAudit: "2026-9-09", evidenceCount: 261 },
  ];
  private _log_queryGetComplianceScore() {
    const fw = this._log_queryComplianceFrameworks;
    return round(fw.reduce((s,f)=>s + (f.implementedControls/Math.max(f.totalControls,1))*100, 0) / fw.length, 1);
  }
  private _log_queryGetGaps() {
    return this._log_queryComplianceFrameworks.filter(f => f.status !== "compliant");
  }
  private _log_queryAuditTrail = [
    { id: "AUD-0001", action: "Control tested", auditor: "Internal Audit", date: "2026-04-17", result: "pass", findings: 1 },
    { id: "AUD-0002", action: "Evidence collected", auditor: "External Auditor", date: "2026-04-01", result: "pass", findings: 2 },
    { id: "AUD-0003", action: "Gap identified", auditor: "Security Team", date: "2026-04-13", result: "fail", findings: 1 },
    { id: "AUD-0004", action: "Remediation completed", auditor: "Compliance Officer", date: "2026-04-23", result: "pass", findings: 1 },
    { id: "AUD-0005", action: "Policy updated", auditor: "IT Audit", date: "2026-04-13", result: "pass", findings: 1 },
    { id: "AUD-0006", action: "Training verified", auditor: "Risk Team", date: "2026-04-12", result: "pass", findings: 0 },
    { id: "AUD-0007", action: "Access reviewed", auditor: "QA Team", date: "2026-04-22", result: "pass", findings: 2 },
    { id: "AUD-0008", action: "Exception approved", auditor: "CISO Office", date: "2026-04-08", result: "conditional", findings: 5 },
    { id: "AUD-0009", action: "Risk accepted", auditor: "Board Audit", date: "2026-04-08", result: "pass", findings: 4 },
    { id: "AUD-0010", action: "Control enhanced", auditor: "Third Party", date: "2026-04-06", result: "pass", findings: 1 },
  ];

  // === Threat Intelligence Feed Extension (log_query) ===
  private _log_queryThreatActors = [
    { name: "APT-29", alias: "Cozy Bear", origin: "Russia", type: "Nation-State", severity: "high", lastActivity: "2026-04-19", targets: "Technology", indicators: 67, ttps: 14 },
    { name: "APT-41", alias: "Double Dragon", origin: "China", type: "Nation-State", severity: "critical", lastActivity: "2026-04-16", targets: "Government", indicators: 356, ttps: 30 },
    { name: "Lazarus Group", alias: "Hidden Cobra", origin: "North Korea", type: "Nation-State", severity: "critical", lastActivity: "2026-04-11", targets: "Finance", indicators: 303, ttps: 10 },
    { name: "FIN7", alias: "Carbanak", origin: "Eastern Europe", type: "Financial", severity: "high", lastActivity: "2026-04-16", targets: "Healthcare", indicators: 93, ttps: 15 },
    { name: "Conti", alias: "Wizard Spider", origin: "Russia", type: "Ransomware", severity: "critical", lastActivity: "2026-04-06", targets: "Finance", indicators: 179, ttps: 37 },
    { name: "LockBit", alias: "LockBit Gang", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-14", targets: "Finance", indicators: 92, ttps: 7 },
    { name: "Cl0p", alias: "Cl0p Team", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-02", targets: "Technology", indicators: 352, ttps: 39 },
    { name: "Sandworm", alias: "Unit 74455", origin: "Russia", type: "Nation-State", severity: "critical", lastActivity: "2026-04-15", targets: "Energy", indicators: 427, ttps: 46 },
  ];
  private _log_queryIoCFeed = [
    { id: "ioc-000001", type: "ip", value: "103.254.216.105", confidence: 47, source: "AlienVault", firstSeen: "2026-04-14", lastSeen: "2026-04-14" },
    { id: "ioc-000002", type: "ip", value: "182.127.196.189", confidence: 49, source: "AlienVault", firstSeen: "2026-04-03", lastSeen: "2026-04-04" },
    { id: "ioc-000003", type: "ip", value: "108.147.48.135", confidence: 74, source: "VirusTotal", firstSeen: "2026-04-03", lastSeen: "2026-04-12" },
    { id: "ioc-000004", type: "ip", value: "9.184.220.139", confidence: 59, source: "CrowdStrike", firstSeen: "2026-04-19", lastSeen: "2026-04-06" },
    { id: "ioc-000005", type: "ip", value: "183.204.198.42", confidence: 60, source: "AlienVault", firstSeen: "2026-04-05", lastSeen: "2026-04-02" },
    { id: "ioc-000006", type: "ip", value: "101.202.38.33", confidence: 51, source: "MISP", firstSeen: "2026-04-06", lastSeen: "2026-04-18" },
    { id: "ioc-000007", type: "ip", value: "227.51.254.227", confidence: 80, source: "AlienVault", firstSeen: "2026-04-04", lastSeen: "2026-04-09" },
    { id: "ioc-000008", type: "ip", value: "234.120.41.216", confidence: 92, source: "STIX", firstSeen: "2026-04-11", lastSeen: "2026-04-17" },
    { id: "ioc-000009", type: "ip", value: "21.52.44.112", confidence: 45, source: "MISP", firstSeen: "2026-04-06", lastSeen: "2026-04-10" },
    { id: "ioc-000010", type: "ip", value: "134.162.99.83", confidence: 42, source: "CrowdStrike", firstSeen: "2026-04-03", lastSeen: "2026-04-09" },
    { id: "ioc-000011", type: "ip", value: "214.39.89.32", confidence: 75, source: "STIX", firstSeen: "2026-04-04", lastSeen: "2026-04-18" },
    { id: "ioc-000012", type: "ip", value: "111.148.114.244", confidence: 53, source: "VirusTotal", firstSeen: "2026-04-23", lastSeen: "2026-04-10" },
    { id: "ioc-000013", type: "ip", value: "230.162.183.154", confidence: 44, source: "VirusTotal", firstSeen: "2026-04-04", lastSeen: "2026-04-17" },
    { id: "ioc-000014", type: "ip", value: "23.118.67.104", confidence: 78, source: "STIX", firstSeen: "2026-04-06", lastSeen: "2026-04-15" },
    { id: "ioc-000015", type: "ip", value: "114.57.153.192", confidence: 98, source: "Mandiant", firstSeen: "2026-04-10", lastSeen: "2026-04-22" },
  ];
  private _log_queryGetActiveThreats() { return this._log_queryThreatActors.filter(a => a.severity === 'critical'); }
  private _log_queryGetThreatSummary() {
    const actors = this._log_queryThreatActors;
  ...
  // === Incident Management Extension (log_query) ===
  private _log_queryIncidents = [
    { id: "INC-20260001", title: "Unauthorized access detected", severity: "critical", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-13T01:40", affectedAssets: 26, rootCause: "Misconfiguration" },
    { id: "INC-20260002", title: "Malware outbreak on workstation", severity: "high", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-03T10:16", affectedAssets: 1, rootCause: "Credential compromise" },
    { id: "INC-20260003", title: "Data leak from S3 bucket", severity: "medium", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-16T10:16", affectedAssets: 25, rootCause: "Zero-day" },
    { id: "INC-20260004", title: "Phishing campaign targeting finance", severity: "low", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-17T02:25", affectedAssets: 30, rootCause: "Human error" },
    { id: "INC-20260005", title: "DDoS attack on web services", severity: "critical", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-19T03:06", affectedAssets: 20, rootCause: "Policy violation" },
    { id: "INC-20260006", title: "Ransomware encryption attempt", severity: "high", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-08T00:39", affectedAssets: 18, rootCause: "Unknown" },
    { id: "INC-20260007", title: "Insider data exfiltration", severity: "medium", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-20T08:02", affectedAssets: 1, rootCause: "Misconfiguration" },
    { id: "INC-20260008", title: "API key exposure in repo", severity: "low", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-16T05:27", affectedAssets: 6, rootCause: "Credential compromise" },
    { id: "INC-20260009", title: "SQL injection on portal", severity: "critical", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-19T09:40", affectedAssets: 46, rootCause: "Zero-day" },
    { id: "INC-20260010", title: "Brute force on VPN", severity: "high", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-12T17:54", affectedAssets: 19, rootCause: "Human error" },
    { id: "INC-20260011", title: "Supply chain alert from vendor", severity: "medium", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-09T20:09", affectedAssets: 9, rootCause: "Policy violation" },
    { id: "INC-20260012", title: "Suspicious lateral movement", severity: "low", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-05T20:25", affectedAssets: 25, rootCause: "Unknown" },
  ];
  private _log_queryGetIncidentStats() {
    const inc = this._log_queryIncidents;
    return { total: inc.length, open: inc.filter(i=>i.status==="open").length, investigating: inc.filter(i=>i.status==="investigating").length, mttd: 30, mttr: 37 };
  }
  private _log_queryGetSeverityDistribution() {
    const inc = this._log_queryIncidents;
...
<div class="tabs">
        ...
        ${this._activeTab === 'rules' ? html`
          ...
          <div style="background:#1a1d27;border-radius:8px;padding:12px">${this._renderLqTreemapSVG()}</div>
          <div style="margin-top:12px;font-size:11px;font-weight:600;margin-bottom:6px">Data Flow (Sankey)</div>
          <div style="background:#1a1d27;border-radius:8px;padding:12px">${this._renderLqSankeySVG()}</div>
          ${this._renderLqActivity()}
        ` : nothing}
        ${this._activeTab === 'anomalies' ? html`
          ${this._renderLqAnomalies()}
          ${this._renderLqNotifications()}
        ` : nothing}
        ${this._activeTab === 'predictions' ? html`
          ${this._renderLqPredictions()}
          ...
          <div style="background:#1a1d27;border-radius:8px;padding:12px">
            <table class="history-table"><thead><tr><th>ID</th><th>Base</th><th>Temporal</th><th>Environmental</th><th>Overall</th></tr></thead><tbody>
              ${this._lqcvssData.map(c => html`<tr><td>${c.itemId}</td><td style="color:${c.base >= 7 ? '#ef4444' : c.base >= 4 ? '#eab308' : '#22c55e'}">${c.base}</td><td>${c.temporal}</td><td>${c.environmental}</td><td style="font-weight:700;color:${c.overall >= 7 ? '#ef4444' : c.overall >= 4 ? '#eab308' : '#22c55e'}">${c.overall}</td></tr>`)}
            </tbody></table>
          </div>
        ` : nothing}
        ${this._activeTab === 'approvals' ? html`
          ${this._renderLqApprovals()}
        ` : nothing}
        ${this._activeTab === 'config' ? html`
          ${this._renderLqConfig()}
        ` : nothing}
        ${this._activeTab === 'new' ? html`
          <div class="form-section">
            <div class="form-title">Create New Log Query Entry</div>
            <div class="form-grid">
  ...
  @state() private _lqScenarios: {id:string;name:string;attackType:string;target:string;method:string;impactLow:number;impactHigh:number;confidence:number;mitigation:string;status:string}[] = [];
  @state() private _lqScenarioForm: {attackType:string;target:string;method:string} = {attackType:'',target:'',method:''};
  @state() private _lqScenarioCompare: boolean = false;
  @state() private _lqScenarioSelected: string[] = [];

  private _lqInitScenarios(): void {
    const saved = localStorage.getItem('lq_scenarios');
    if (saved) { try { this._lqScenarios = JSON.parse(saved); } catch { /* ignore */ } }
    if (this._lqScenarios.length === 0) {
      this._lqScenarios = [
        {id:'lq-s1',name:'Baseline Threat',attackType:'Phishing',target:'Employees',method:'Spear Email',impactLow:45,impactHigh:78,confidence:72,mitigation:'Security awareness training + email filtering',status:'active'},
        {id:'lq-s2',name:'Escalated Attack',attackType:'Ransomware',target:'Endpoints',method:'Drive-by Download',impactLow:65,impactHigh:95,confidence:58,mitigation:'EDR deployment + network segmentation',status:'saved'},
        {id:'lq-s3',name:'Insider Threat',attackType:'Data Exfiltration',target:'Databases',method:'SQL Injection',impactLow:55,impactHigh:88,confidence:65,mitigation:'DLP policies + query monitoring',status:'draft'},
      ];
    }
  }

  private _lqSaveScenarios(): void {
    localStorage.setItem('lq_scenarios', JSON.stringify(this._lqScenarios));
  }

  private _lqAddScenario(): void {
    const f = this._lqScenarioForm;
    if (!f.attackType || !f.target) return;
    this._lqScenarios = [...this._lqScenarios, {
      id: 'lq-s' + (this._lqScenarios.length + 1),
    ...
    this._lqScenarioForm = {attackType:'',target:'',method:''};
    this._lqSaveScenarios();
  }

  private _lqRenderScenarioEngine(): any {
    ...
    return html`
      <div style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          ...
          <button class="tab" @click=${() => { this._lqScenarioCompare = !this._lqScenarioCompare; }}>${this._lqScenarioCompare ? 'List View' : 'Compare'}</button>
        </div>
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:10px">
          <select style="background:#1a1d2e;border:1px solid #2a2d3a;border-radius:4px;padding:4px 6px;color:#e2e8f0;font-size:11px" @change=${(e: any) => { this._lqScenarioForm = {...this._lqScenarioForm, attackType: (e.target as HTMLSelectElement).value}; }}>
            <option value="">Attack Type</option>
            ${attackTypes.map(a => html`<option value=${a} ${this._lqScenarioForm.attackType === a ? 'selected' : ''}>${a}</option>`)}
          </select>
          <select style="background:#1a1d2e;border:1px solid #2a2d3a;border-radius:4px;padding:4px 6px;color:#e2e8f0;font-size:11px" @change=${(e: any) => { this._lqScenarioForm = {...this._lqScenarioForm, target: (e.target as HTMLSelectElement).value}; }}>
            <option value="">Target</option>
            ${targets.map(t => html`<option value=${t} ${this._lqScenarioForm.target === t ? 'selected' : ''}>${t}</option>`)}
          </select>
          <select style="background:#1a1d2e;border:1px solid #2a2d3a;border-radius:4px;padding:4px 6px;color:#e2e8f0;font-size:11px" @change=${(e: any) => { this._lqScenarioForm = {...this._lqScenarioForm, method: (e.target as HTMLSelectElement).value}; }}>
            <option value="">Method</option>
            ${methods.map(m => html`<option value=${m} ${this._lqScenarioForm.method === m ? 'selected' : ''}>${m}</option>`)}
          </select>
        </div>
        <button style="background:#3b82f6;border:none;border-radius:4px;padding:4px 12px;color:#fff;font-size:11px;cursor:pointer" @click=${this._lqAddScenario}>Run Simulation</button>
      </div>
      ${this._lqScenarioCompare && this._lqScenarios.length >= 2 ? html`
        <div style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
          <div style="font-weight:600;font-size:12px;color:#e2e8f0;margin-bottom:8px">Side-by-Side Comparison</div>
          <div style="display:grid;grid-template-columns:repeat(${Math.min(3, this._lqScenarios.length)},1fr);gap:8px">
            ${this._lqScenarios.slice(0,3).map(s => html`
      ...
      <div style="background:#0f1117;border-radius:8px;padding:12px">
        <div style="font-weight:600;font-size:12px;color:#e2e8f0;margin-bottom:8px">Saved Scenarios (${this._lqScenarios.length})</div>
        ${this._lqScenarios.map(s => html`
  ...
  @state() private _lqTrendData: {day:number;value:number;anomaly:boolean}[] = [];
  @state() private _lqTrendZoom: {start:number;end:number} | null = null;
  @state() private _lqTrendMA: number = 7;

  private _lqInitTrendData(): void {
    ...
    this._lqTrendData = data;
  }

  private _lqCalcMA(window: number): number[] {
    const result: number[] = [];
    for (let i = 0; i < this._lqTrendData.length; i++) {
      const start = Math.max(0, i - window + 1);
      const slice = this._lqTrendData.slice(start, i + 1);
      result.push(slice.reduce((s, d) => s + d.value, 0) / slice.length);
    }
    return result;
  }

  private _lqGetStats(): {mean:number;median:number;stddev:number;trend:string} {
    const vals = this._lqTrendData.map(d => d.value);
    const n = vals.length;
  ...
  private _lqRenderTimeSeries(): any {
    const stats = this._lqGetStats();
    const filtered = this._lqTrendZoom ? this._lqTrendData.filter(d => d.day >= this._lqTrendZoom.start && d.day <= this._lqTrendZoom.end) : this._lqTrendData;
    ...
    return html`
      <div style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          ...
          <div style="display:flex;gap:4px">
            <button class="tab ${this._lqTrendMA === 7 ? 'active' : ''}" @click=${() => { this._lqTrendMA = 7; }}>7D MA</button>
            <button class="tab ${this._lqTrendMA === 30 ? 'active' : ''}" @click=${() => { this._lqTrendMA = 30; }}>30D MA</button>
            <button class="tab" @click=${() => { this._lqTrendZoom = null; }}>Reset Zoom</button>
          </div>
        </div>
        <div style="position:relative;height:120px;background:#1a1d2e;border-radius:6px;overflow:hidden;margin-bottom:8px;cursor:crosshair" @click=${(e: any) => {
          ...
          this._lqTrendZoom = { start: Math.max(0, center - 10), end: Math.min(89, center + 10) };
        }}>
  ...
  @state() private _lqRoles: string[] = ['Admin','Analyst','Operator','Viewer','Auditor'];
  @state() private _lqActions: string[] = ['Create','Read','Update','Delete','Export','Approve'];
  @state() private _lqPermissions: { [role: string]: { [action: string]: boolean } } = {};
  @state() private _lqPermAudit: {role:string;action:string;changedBy:string;timestamp:string;oldVal:boolean;newVal:boolean}[] = [];
  @state() private _lqPermCompare: string[] = [];

  private _lqInitPermissions(): void {
    ...
    for (const role of this._lqRoles) {
      perms[role] = {};
      this._lqActions.forEach((a, i) => { perms[role][a] = defaults[role]?.[i] ?? false; });
    }
    this._lqPermissions = perms;
  }

  private _lqTogglePermission(role: string, action: string): void {
    const old = this._lqPermissions[role][action];
    this._lqPermissions = {...this._lqPermissions, [role]: {...this._lqPermissions[role], [action]: !old}};
    this._lqPermAudit.unshift({role,action,changedBy:'current_user',timestamp:new Date().toISOString(),oldVal:old,newVal:!old});
  }

  private _lqRenderRBAC(): any {
    const compareRoles = this._lqPermCompare.map(r => this._lqPermissions[r]).filter(Boolean);
    return html`
      <div style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          <span style="font-weight:700;font-size:13px;color:#e2e8f0">RBAC Permission Matrix</span>
          <div style="display:flex;gap:4px">
            ${this._lqRoles.map(r => html`
              <button class="tab ${this._lqPermCompare.includes(r) ? 'active' : ''}" @click=${() => {
                this._lqPermCompare = this._lqPermCompare.includes(r) ? this._lqPermCompare.filter(x => x !== r) : [...this._lqPermCompare, r];
              }}>${r}</button>
            `)}
          </div>
        </div>
          ...
          <table style="width:100%;border-collapse:collapse;font-size:10px">
            <thead>
              <tr>
                ...
                ${this._lqActions.map(a => html`<th style="padding:6px;color:#6b7280;border-bottom:1px solid #2a2d3a;text-align:center">${a}</th>`)}
              </tr>
            </thead>
            <tbody>
              ${this._lqRoles.map(role => html`
                <tr style="border-bottom:1px solid #1a1d2e">
                  <td style="padding:6px;color:#e2e8f0;font-weight:600">${role}</td>
                  ${this._lqActions.map(action => html`
                    <td style="text-align:center;padding:6px">
                      <button style="width:28px;height:20px;border-radius:3px;border:1px solid #2a2d3a;background:${this._lqPermissions[role][action] ? '#22c55e' : '#1a1d2e'};cursor:pointer;color:#fff;font-size:10px" @click=${() => this._lqTogglePermission(role, action)}>${this._lqPermissions[role][action] ? 'Y' : 'N'}</button>
                    </td>
                  `)}
                </tr>
              `)}
        ...
        ${compareRoles.length >= 2 ? html`
          <div style="margin-top:10px;padding:8px;background:#1a1d2e;border-radius:6px">
            <div style="font-size:11px;font-weight:600;color:#e2e8f0;margin-bottom:6px">Role Diff: ${this._lqPermCompare.join(' vs ')}</div>
            ${this._lqActions.map(action => {
              ...
              return allSame ? nothing : html`
                <div style="display:flex;gap:8px;padding:3px 0;font-size:10px">
                  ...
                  ${compareRoles.map((r, i) => html`<span style="color:${r[action] ? '#22c55e' : '#ef4444'}">${this._lqPermCompare[i]}=${r[action] ? 'Y' : 'N'}</span>`)}
                </div>
              `;
            })}
          </div>
        ` : nothing}
        ${this._lqPermAudit.length > 0 ? html`
          <div style="margin-top:8px;font-size:9px;color:#6b7280">Recent: ${this._lqPermAudit.slice(0,3).map(a => html`<span style="margin-right:8px">${a.role}.${a.action}: ${a.oldVal ? 'Y' : 'N'}->${a.newVal ? 'Y' : 'N'}</span>`)}
        ` : nothing}
      </div>
    `;
  }
  ...
  // === REPORTING SUITE ===
  @state() private _lqReportTemplate: string = 'executive';
  @state() private _lqReportSchedule: string = 'weekly';
  @state() private _lqReportDistList: string[] = ['security-team@company.com','ciso@company.com'];
  @state() private _lqReportHistory: {id:string;template:string;generatedAt:string;status:string}[] = [];

  private _lqGenerateReport(): void {
    const id = 'rpt-' + Date.now();
    this._lqReportHistory.unshift({id,template:this._lqReportTemplate,generatedAt:new Date().toISOString(),status:'sent'});
  }

  private _lqRenderReporting(): any {
    ...
    return html`
      <div style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">
          <div>
            ...
            ${templates.map(t => html`
              <div style="padding:6px;background:${this._lqReportTemplate === t.key ? '#1e3a5f' : '#1a1d2e'};border:1px solid ${this._lqReportTemplate === t.key ? '#3b82f6' : '#2a2d3a'};border-radius:4px;margin-bottom:4px;cursor:pointer" @click=${() => { this._lqReportTemplate = t.key; }}>
          ...
          <div>
            ...
            <div style="display:flex;gap:4px;margin-bottom:8px">
              ${schedules.map(s => html`<button class="tab ${this._lqReportSchedule === s.key ? 'active' : ''}" @click=${() => { this._lqReportSchedule = s.key; }}>${s.label}</button>`)}
            </div>
            <div style="font-size:10px;color:#6b7280;margin-bottom:4px">Distribution</div>
            <div style="font-size:10px;color:#9ca3af">${this._lqReportDistList.map(d => html`<div>${d}</div>`)}</div>
            <div style="margin-top:6px;display:flex;gap:4px">
              <button style="background:#3b82f6;border:none;border-radius:4px;padding:4px 10px;color:#fff;font-size:10px;cursor:pointer" @click=${this._lqGenerateReport}>Generate</button>
        ...
        ${this._lqReportHistory.length > 0 ? html`
          <div style="border-top:1px solid #2a2d3a;padding-top:8px">
            ...
            ${this._lqReportHistory.slice(0,3).map(r => html`
  ...
  @state() private _lqHighContrast: boolean = false;
  @state() private _lqA11yAnnounce: string = '';
  @state() private _lqShortcutsVisible: boolean = false;
  @state() private _lqFocusTrap: boolean = false;

  private _lqShortcuts: Record<string, string> = {
  ...
  private _lqHandleKeydown(e: KeyboardEvent): void {
    if (e.key === 'Escape' && this._lqFocusTrap) { this._lqFocusTrap = false; this._lqAnnounce('Dialog closed'); }
    if (e.ctrlKey && e.shiftKey && e.key === 'H') { e.preventDefault(); this._lqHighContrast = !this._lqHighContrast; this._lqAnnounce('High contrast ' + (this._lqHighContrast ? 'enabled' : 'disabled')); }
    if (e.ctrlKey && e.shiftKey && e.key === 'A') { e.preventDefault(); this._lqShortcutsVisible = !this._lqShortcutsVisible; }
  }

  private _lqAnnounce(msg: string): void {
    this._lqA11yAnnounce = msg;
    setTimeout(() => { this._lqA11yAnnounce = ''; }, 2000);
  }

  private _lqRenderAccessibility(): any {
    return html`
      <div role="region" aria-label="Accessibility Controls" style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          ...
          <button class="tab ${this._lqShortcutsVisible ? 'active' : ''}" @click=${() => { this._lqShortcutsVisible = !this._lqShortcutsVisible; }} aria-expanded=${this._lqShortcutsVisible}>Shortcuts</button>
        </div>
        <div style="display:flex;gap:8px;margin-bottom:8px">
          <label style="display:flex;align-items:center;gap:6px;font-size:11px;color:#e2e8f0;cursor:pointer">
            <input type="checkbox" .checked=${this._lqHighContrast} @change=${() => { this._lqHighContrast = !this._lqHighContrast; }} aria-label="Toggle high contrast mode">
            High Contrast
          </label>
        </div>
        ${this._lqShortcutsVisible ? html`
          <div role="list" aria-label="Keyboard shortcuts" style="background:#1a1d2e;border-radius:6px;padding:8px">
            ${Object.entries(this._lqShortcuts).map(([key, desc]) => html`
        ...
        ` : nothing}
        <div role="status" aria-live="polite" aria-atomic="true" style="position:absolute;left:-9999px">${this._lqA11yAnnounce}</div>
      </div>
    `;
  }
  ...
  connectedCallback(): void {
    ...
    this._lqInitScenarios();
    this._lqInitTrendData();
    this._lqInitPermissions();
    document.addEventListener('keydown', this._lqHandleKeydown.bind(this));
  }

  disconnectedCallback(): void {
    super.disconnectedCallback();
    document.removeEventListener('keydown', this._lqHandleKeydown.bind(this));
  }

  // === TAB INTEGRATION FOR EXTENDED FEATURES ===
  @state() private _lqActiveSubTab: string = 'scenario';
  ...
  private _initThreatModel() {
    const categories = [
      ...
      { id: 'tc2', category: 'Tampering', threats: [
        { id: 't4', name: 'Data injection in log analysis', stride: 'T', likelihood: 7, impact: 8, dreadScore: 7.0, status: 'open', mitigations: ['Input validation', 'WAF rules'], assignedTo: 'Dev Team', discoveredDate: '2024-01-08', lastReviewed: '2024-02-10' },
  ...
  private _lqGetAllSubTabs(): {key:string;label:string}[] {
    return [
  ...
  private _lqRenderSubPanel(): any {
    switch (this._lqActiveSubTab) {
      case 'scenario': return this._lqRenderScenarioEngine();
      case 'timeseries': return this._lqRenderTimeSeries();
      case 'rbac': return this._lqRenderRBAC();
      case 'reporting': return this._lqRenderReporting();
      case 'a11y': return this._lqRenderAccessibility();
      default: return nothing;
    }
  }

  private _lqRenderTabBar(): any {
    return html`
      <div style="display:flex;gap:4px;margin-bottom:12px;border-bottom:1px solid #2a2d3a;padding-bottom:8px;flex-wrap:wrap" role="tablist" aria-label="Extended panel features">
        ${this._lqGetAllSubTabs().map(t => html`
          <button class="tab ${this._lqActiveSubTab === t.key ? 'active' : ''}" @click=${() => { this._lqActiveSubTab = t.key; }} role="tab" aria-selected=${this._lqActiveSubTab === t.key}>${t.label}</button>
        `)}
      </div>
      <div role="tabpanel" aria-labelledby="lq-tab-${this._lqActiveSubTab}">
        ${this._lqRenderSubPanel()}
      </div>
    `;
  }
  ...
  @state() private _soarPlaybooks: Array<{
    ...
    {
      id: 'pb-soar-001', name: 'Log Query Auto Response', status: 'active',
      steps: [
    ...
    {
      id: 'pb-soar-002', name: 'Log Query Investigation Workflow', status: 'active',
      steps: [
    ...
    {
      id: 'pb-soar-003', name: 'Log Query Compliance Scan', status: 'draft',
      steps: [
  ...
  @state() private _lqAle: number = 0;
  @state() private _lqSroi: number = 0;
  @state() private _lqCpi: number = 0;
  @state() private _lqBudgetAlloc: number = 0;
  @state() private _lqCostBenefit: number = 0;

  // Security Economics Calculator
  private lqInitEconomics() {
    this._lqAle = Math.round(2850000 + Math.random() * 4500000);
    this._lqSroi = Math.round(180 + Math.random() * 320);
    this._lqCpi = Math.round(45000 + Math.random() * 120000);
    this._lqBudgetAlloc = Math.round(2500000 + Math.random() * 3000000);
    this._lqCostBenefit = Math.round(220 + Math.random() * 180);
  }

  private _lqCalcAle(): { annual: number; perIncident: number; byCategory: Array<{name: string; value: number}> } {
    const base = this._lqAle;
    const categories = [
  ...
  private _lqCalcSroi(): Array<{year: number; investment: number; savings: number; roi: number}> {
    const baseInv = this._lqSroi * 10000;
  ...
  private _lqGetBudgetAlloc(): Array<{category: string; amount: number; pct: number; trend: string}> {
    const total = this._lqBudgetAlloc;
    const items = [
  ...
  private _lqGetCostBenefit(): Array<{control: string; cost: number; benefit: number; ratio: number; priority: string}> {
    const base = this._lqCostBenefit;
    const controls = [
  ...
  private _lqRenderEconomics() {
    const ale = this._lqCalcAle();
    const roi = this._lqCalcSroi();
    const budget = this._lqGetBudgetAlloc();
    const cb = this._lqGetCostBenefit();
    const cpi = this._lqCpi;
  ...
  @state() private _lqThreatLevel: any = null;
  @state() private _lqEmergingThreats: any = null;
  @state() private _lqThreatTrends: any = null;
  @state() private _lqSectorRadar: any = null;
  @state() private _lqActorActivity: any = null;

  // Threat Landscape Intelligence
  private lqInitThreatIntel() {
    this._lqThreatLevel = {
  ...
  private _lqRenderThreatIntel() {
    const tl = this._lqThreatLevel;
    const et = this._lqEmergingThreats;
    const sr = this._lqSectorRadar;
    const aa = this._lqActorActivity;
  ...
  @state() private _lqPolicies: any = null;
  @state() private _lqExceptions: any = null;
  @state() private _lqRiskRegister: any = null;
  @state() private _lqMeetings: any = null;
  @state() private _lqDeadlines: any = null;

  // Security Governance Dashboard
  private lqInitGovernance() {
    this._lqPolicies = [
  ...
  private _lqRenderGovernance() {
    const policies = this._lqPolicies;
    const risks = this._lqRiskRegister;
    const deadlines = this._lqDeadlines;
  ...
  @state() private _lqCriticalAssets: any = null;
  @state() private _lqAssetDeps: any = null;
  @state() private _lqEolAssets: any = null;
  @state() private _lqAssetRisk: any = null;

  // Asset Intelligence
  private lqInitAssetIntel() {
    this._lqCriticalAssets = [
    ...
    this._lqAssetRisk = { critical: 7, high: 23, medium: 45, low: 128, total: 203 };
  }

  private _lqRenderAssetIntel() {
    const assets = this._lqCriticalAssets;
    const eol = this._lqEolAssets;
    const ar = this._lqAssetRisk;
  ...
  @state() private _lqUserBaseline: any = null;
  @state() private _lqAnomalyRules: any = null;
  @state() private _lqDataAccess: any = null;
  @state() private _lqInsiderRisk: any = null;
  @state() private _logquer15Timeline: Array<{id:string;time:string;event:string;severity:string;actor:string}> = [
  ...
  @state() private _logquer15RootCauses: Array<{why:string;answer:string}> = [
  ...
  @state() private _logquer15ImpactMatrix: Array<{system:string;users:number;data:string;revenue:string;status:string}> = [
  ...
  @state() private _logquer15Actions: Array<{id:string;item:string;owner:string;deadline:string;priority:string;status:string}> = [
  ...
  @state() private _logquer15Lessons: Array<{id:string;lesson:string;category:string;severity:string;applies_to:string}> = [
  ...
  @state() private _logquer15ActiveTab: string = 'timeline';
  @state() private _logquer15Benchmarks: Array<{metric:string;current:number;industry:number;target:number;unit:string;source:string}> = [
  ...
  @state() private _logquer15MaturityLevels: Array<{domain:string;current:number;target:number;description:string}> = [
  ...
  @state() private _logquer15QuarterlyData: Array<{quarter:string;score:number;improvement:number}> = [
  ...
  @state() private _logquer15SelectedDomain: string = 'all';
  @state() private _logquer15Alerts: Array<{id:string;name:string;severity:number;confidence:number;assetCrit:number;score:number;enriched:boolean;group:string;status:string;enrichData:Array<{key:string;value:string}>}> = [
  ...
  @state() private _logquer15QualityMetrics: {fpRate:number;enrichSuccess:number;avgTriageTime:number;enrichedCount:number;totalCount:number} = {fpRate:0.12, enrichSuccess:0.78, avgTriageTime:4.5, enrichedCount:3, totalCount:5};
  @state() private _logquer15RoutingRules: Array<{name:string;condition:string;channel:string;active:boolean}> = [
  ...
  @state() private _logquer15Shapes: Array<{id:string;type:string;label:string;controls:string[]}> = [
  ...
  @state() private _logquer15TrustBoundaries: Array<{from:string;to:string;label:string;strength:string}> = [
  ...
  @state() private _logquer15ADRs: Array<{id:string;title:string;status:string;date:string;decision:string}> = [
  ...
  @state() private _logquer15SelectedShape: string = '';
  @state() private _logquer15Gauges: Array<{name:string;value:number;max:number;unit:string;status:string;color:string}> = [
  ...
  @state() private _logquer15Anomalies: Array<{id:string;time:string;description:string;severity:string;acknowledged:boolean}> = [
  ...
  @state() private _logquer15Integrations: Array<{name:string;status:string;uptime:number;lastCheck:string;latency:number}> = [
  ...
  @state() private _logquer15AlertFatigue: Array<{analyst:string;alertsPerDay:number;escalated:number;dismissed:number;avgResponseMin:number}> = [
  ...
  @state() private _logquer15SlaTarget: number = 99.9;

  // Insider Threat Detection
  private lqInitInsiderThreat() {
    this._lqUserBaseline = [
    ...
    this._lqInsiderRisk = { totalUsers: 2847, monitored: 342, flagged: 18, investigated: 5, confirmed: 1 };
  }

  private _lqRenderInsiderThreat() {
    const baseline = this._lqUserBaseline;
    const rules = this._lqAnomalyRules;
    const ir = this._lqInsiderRisk;
    ...
    return html`<div class="card" style="padding:14px;margin-bottom:8px">
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:4px;margin-bottom:10px;flex-wrap:wrap">
            ${['timeline','rootcause','impact','actions','lessons'].map(t => html`
              <button class="btn ${this._logquer15ActiveTab === t ? 'btn-primary' : ''}" style="font-size:10px;padding:3px 8px" @click=${() => { this._logquer15ActiveTab = t; }}>${t.charAt(0).toUpperCase() + t.slice(1)}</button>
            `)}
          </div>
          ${this._logquer15ActiveTab === 'timeline' ? html`
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._logquer15Timeline.map(e => html`
          ...
          ` : this._logquer15ActiveTab === 'rootcause' ? html`
            <div style="margin-bottom:8px">${{__html: this._logquer15RenderFishbone()}}</div>
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._logquer15RootCauses.map((rc, i) => html`
          ...
          ` : this._logquer15ActiveTab === 'impact' ? html`
            <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:10px">
              ...
              <tbody>${this._logquer15ImpactMatrix.map(imp => html`
          ...
          ` : this._logquer15ActiveTab === 'actions' ? html`
            <div style="display:flex;gap:12px;margin-bottom:8px;font-size:10px;color:#888">
              ${Object.entries(this._logquer15GetActionStats()).map(([k,v]) => html`<span>${k}: <b style="color:#ddd">${v}</b></span>`)}
            </div>
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._logquer15Actions.map(a => html`
                <div style="display:flex;gap:8px;align-items:center;padding:6px;background:#1a1d27;border-radius:4px;cursor:pointer" @click=${() => this._logquer15ToggleAction(a.id)}>
          ...
          ` : html`
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._logquer15Lessons.map(l => html`
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:4px;margin-bottom:10px;flex-wrap:wrap">
            ${['all', ...this._logquer15MaturityLevels.map(m => m.domain)].map(d => html`
              <button class="btn ${this._logquer15SelectedDomain === d ? 'btn-primary' : ''}" style="font-size:9px;padding:2px 6px" @click=${() => { this._logquer15SelectedDomain = d; }}>${d}</button>
            `)}
          </div>
          <div style="display:flex;gap:16px;margin-bottom:10px">
            <div style="text-align:center"><div style="color:#e0e0e0;font-size:20px;font-weight:bold">${this._logquer15GetOverallMaturity()}/5</div><div style="color:#888;font-size:10px">Maturity Level</div></div>
            <div style="text-align:center"><div style="color:#4f4;font-size:20px;font-weight:bold">${this._logquer15GetGapAnalysis().filter(g => g.isAbove).length}/${this._logquer15Benchmarks.length}</div><div style="color:#888;font-size:10px">Above Industry</div></div>
          </div>
          <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:10px">
            ...
            <tbody>${this._logquer15GetGapAnalysis().map(b => html`
          ...
          <div style="margin-top:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Maturity by Domain</div>
            ${this._logquer15MaturityLevels.filter(m => this._logquer15SelectedDomain === 'all' || m.domain === this._logquer15SelectedDomain).map(m => html`
          ...
          <div style="margin-top:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Quarterly Trend</div>
            <div style="display:flex;height:40px;align-items:flex-end;gap:4px">
              ${this._logquer15QuarterlyData.map(q => html`<div style="flex:1;text-align:center"><div style="background:#4a9eff;height:${q.score * 0.5}px;border-radius:2px 2px 0 0" title="${q.score}"></div><div style="color:#666;font-size:8px;margin-top:2px">${q.quarter}</div></div>`)}
            </div>
          </div>
        </div>
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:12px;margin-bottom:8px;font-size:10px">
            <div style="display:flex;gap:4px;flex-wrap:wrap">${this._logquer15GroupAlerts().map(g => html`<span style="color:#888;padding:2px 6px;background:#1a1d27;border-radius:3px">${g.group}: ${g.count}</span>`)}</div>
          </div>
          <div style="display:flex;gap:8px;margin-bottom:8px;font-size:10px;color:#888">
            <span>FP Rate: <b style="color:${this._logquer15QualityMetrics.fpRate > 0.15 ? '#f44' : '#4f4'}">${(this._logquer15QualityMetrics.fpRate * 100).toFixed(1)}%</b></span>
            <span>Enrich: <b style="color:#4a9eff">${(this._logquer15QualityMetrics.enrichSuccess * 100).toFixed(0)}%</b></span>
            <span>Avg Triage: <b style="color:#ddd">${this._logquer15QualityMetrics.avgTriageTime}m</b></span>
          </div>
          <div style="display:flex;flex-direction:column;gap:4px;max-height:200px;overflow-y:auto">
            ${this._logquer15Alerts.sort((a, b) => this._logquer15CalcScore(b) - this._logquer15CalcScore(a)).map(a => {
              const score = this._logquer15CalcScore(a);
              return html`
                <div style="padding:6px;background:#1a1d27;border-radius:4px;border-left:3px solid ${a.severity >= 4 ? '#f44' : a.severity >= 3 ? '#fa0' : '#4a9eff'}">
                  <div style="display:flex;justify-content:space-between;align-items:center">
                    ...
                    <button class="btn" style="font-size:9px;padding:1px 6px;margin-left:4px" @click=${() => this._logquer15EnrichAlert(a.id)}>${a.enriched ? 'Re-enrich' : 'Enrich'}</button>
                  </div>
          ...
          <div style="margin-top:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Routing Rules</div>
            ${this._logquer15RoutingRules.map(r => html`
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap">
            ${this._logquer15Shapes.map(s => {
              const v = this._logquer15ValidateControl(s.id);
              return html`<div style="padding:4px 8px;background:#1a1d27;border-radius:3px;cursor:pointer;border:1px solid ${this._logquer15SelectedShape === s.id ? '#4a9eff' : '#2a2d37'}" @click=${() => { this._logquer15SelectedShape = this._logquer15SelectedShape === s.id ? '' : s.id; }}>
          ...
          ${this._logquer15SelectedShape ? html`
            ${(() => {
              const shape = this._logquer15Shapes.find(s => s.id === this._logquer15SelectedShape);
              const v = this._logquer15ValidateControl(this._logquer15SelectedShape);
              return shape ? html`
          ...
          <div style="margin-bottom:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Trust Boundaries</div>
            ${this._logquer15TrustBoundaries.map(tb => html`
              <div style="display:flex;gap:6px;align-items:center;padding:3px;background:#1a1d27;border-radius:3px;margin-bottom:2px;font-size:9px">
                <span style="color:#ddd">${this._logquer15Shapes.find(s => s.id === tb.from)?.label || tb.from}</span>
                <span style="color:#4a9eff">\u2194</span>
                <span style="color:#ddd">${this._logquer15Shapes.find(s => s.id === tb.to)?.label || tb.to}</span>
          ...
          <div><div style="color:#888;font-size:10px;margin-bottom:4px">Architecture Decision Records</div>
            ${this._logquer15ADRs.map(adr => html`
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap">
            ${this._logquer15Gauges.map(g => html`
          ...
          <div style="display:flex;gap:8px;margin-bottom:8px;align-items:center">
            ...
            <span style="color:${this._logquer15GetOverallHealth().score >= 99 ? '#4f4' : '#fa0'};font-size:12px;font-weight:bold">${this._logquer15GetOverallHealth().score}%</span>
            <span style="color:#888;font-size:10px">(Target: ${this._logquer15SlaTarget}%)</span>
            <span style="color:#4f4;font-size:10px">${this._logquer15GetOverallHealth().healthy} healthy</span>
            <span style="color:#fa0;font-size:10px">${this._logquer15GetOverallHealth().degraded} degraded</span>
          </div>
          <div style="margin-bottom:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Anomaly Stream</div>
            ${this._logquer15Anomalies.slice(0, 3).map(a => html`
              <div style="display:flex;gap:6px;align-items:center;padding:4px;background:#1a1d27;border-radius:3px;margin-bottom:2px;opacity:${a.acknowledged ? 0.5 : 1}">
                ...
                ${!a.acknowledged ? html`<button class="btn" style="font-size:8px;padding:1px 4px" @click=${() => this._logquer15AckAnomaly(a.id)}>ACK</button>` : html`<span style="color:#4f4;font-size:9px">ACK'd</span>`}
              </div>
            `)}
          </div>
          <div style="margin-bottom:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Integration Health</div>
            ${this._logquer15Integrations.map(i => html`
              <div style="display:flex;gap:6px;align-items:center;padding:3px;background:#1a1d27;border-radius:3px;margin-bottom:2px;font-size:9px">
          ...
          <div><div style="color:#888;font-size:10px;margin-bottom:4px">Alert Fatigue Analysis</div>
            <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:9px">
              ...
              <tbody>${this._logquer15AlertFatigue.map(af => html`
  ...
  private _logquer15RenderFishbone(): string {
  ...
  private _logquer15ToggleAction(id: string) {
    this._logquer15Actions = this._logquer15Actions.map(a => a.id === id ? {...a, status: a.status === 'pending' ? 'in_progress' : a.status === 'in_progress' ? 'completed' : 'pending'} : a);
  }

  private _logquer15GetActionStats() {
    const total = this._logquer15Actions.length;
    const done = this._logquer15Actions.filter(a => a.status === 'completed').length;
    const inProg = this._logquer15Actions.filter(a => a.status === 'in_progress').length;
    return { total, done, inProg, pending: total - done - inProg };
  }

  private _logquer15GetOverallMaturity(): number {
    if (this._logquer15SelectedDomain === 'all') {
      return Math.round(this._logquer15MaturityLevels.reduce((s, m) => s + m.current, 0) / this._logquer15MaturityLevels.length * 10) / 10;
    }
    const d = this._logquer15MaturityLevels.find(m => m.domain === this._logquer15SelectedDomain);
    return d ? d.current : 0;
  }

  private _logquer15GetGapAnalysis() {
    return this._logquer15Benchmarks.map(b => {
  ...
  private _logquer15CalcScore(alert: any): number {
    return Math.round(alert.severity * alert.confidence * alert.assetCrit * 100) / 100;
  }

  private _logquer15EnrichAlert(id: string) {
    this._logquer15Alerts = this._logquer15Alerts.map(a => {
      if (a.id !== id) return a;
      const score = this._logquer15CalcScore(a);
  ...
  private _logquer15GroupAlerts() {
    const groups: Record<string, number> = {};
    this._logquer15Alerts.forEach(a => { groups[a.group] = (groups[a.group] || 0) + 1; });
    return Object.entries(groups).map(([g, c]) => ({group: g, count: c}));
  }

  private _logquer15ValidateControl(shapeId: string): {valid:boolean;missing:string[]} {
    const shape = this._logquer15Shapes.find(s => s.id === shapeId);
  ...
  private _logquer15GetOverallHealth(): {healthy:number;degraded:number;down:number;score:number} {
    const online = this._logquer15Integrations.filter(i => i.status === 'online').length;
    const degraded = this._logquer15Integrations.filter(i => i.status === 'degraded').length;
    const total = this._logquer15Integrations.length;
    return { healthy: online, degraded, down: total - online - degraded, score: Math.round(online / total * 100) };
  }

  private _logquer15AckAnomaly(id: string) {
    this._logquer15Anomalies = this._logquer15Anomalies.map(a => a.id === id ? {...a, acknowledged: true} : a);
  }
...
// ===== SECURITY ROI DASHBOARD (R22) =====
  ...
  private _renderBudgetPlanning() {
    const budgetData = [
      { category: "Personnel & Training", planned: 4359000, actual: 2772000, utilization: 63.6, q1: "26%", q2: "31%", q3: "29%", q4: "21%" },
      { category: "Tooling & Licensing", planned: 4366000, actual: 2785000, utilization: 63.8, q1: "29%", q2: "20%", q3: "24%", q4: "22%" },
      { category: "Infrastructure Security", planned: 4373000, actual: 2798000, utilization: 64.0, q1: "16%", q2: "25%", q3: "19%", q4: "23%" },
      { category: "Compliance & Audit", planned: 4380000, actual: 2811000, utilization: 64.2, q1: "19%", q2: "30%", q3: "30%", q4: "24%" },
      { category: "Incident Response", planned: 4387000, actual: 2824000, utilization: 64.4, q1: "22%", q2: "35%", q3: "25%", q4: "25%" },
      { category: "Third-Party Assessments", planned: 4394000, actual: 2837000, utilization: 64.6, q1: "25%", q2: "24%", q3: "20%", q4: "10%" },
      { category: "Security Awareness", planned: 4401000, actual: 2850000, utilization: 64.8, q1: "28%", q2: "29%", q3: "31%", q4: "11%" },
      { category: "Research & Innovation", planned: 4408000, actual: 2863000, utilization: 65.0, q1: "15%", q2: "34%", q3: "26%", q4: "12%" },
    ];
    ...
    const headcount = [
      { team: "SOC Tier 1", current: 10, target: 10, gap: 5, avgSalary: "192k" },
      { team: "SOC Tier 2", current: 2, target: 9, gap: 4, avgSalary: "110k" },
      { team: "Threat Intel", current: 11, target: 8, gap: 3, avgSalary: "139k" },
      { team: "Red Team", current: 3, target: 7, gap: 2, avgSalary: "168k" },
      { team: "GRC", current: 12, target: 6, gap: 1, avgSalary: "86k" },
      { team: "AppSec", current: 4, target: 5, gap: 0, avgSalary: "115k" },
      { team: "Cloud Sec", current: 13, target: 4, gap: 5, avgSalary: "144k" },
      { team: "Identity & Access", current: 5, target: 3, gap: 4, avgSalary: "173k" },
    ];
    const vendorSpend = [
      { vendor: "CrowdStrike", annual: "304k", contractEnd: "2026-12", renewalRisk: "Low", satisfaction: 5 },
      { vendor: "Palo Alto", annual: "335k", contractEnd: "2026-01", renewalRisk: "Medium", satisfaction: 4 },
      { vendor: "Splunk", annual: "366k", contractEnd: "2026-02", renewalRisk: "High", satisfaction: 3 },
      { vendor: "Qualys", annual: "397k", contractEnd: "2026-03", renewalRisk: "Low", satisfaction: 5 },
      { vendor: "Rapid7", annual: "428k", contractEnd: "2026-04", renewalRisk: "Medium", satisfaction: 4 },
      { vendor: "Mandiant", annual: "459k", contractEnd: "2026-05", renewalRisk: "High", satisfaction: 3 },
      { vendor: "Zscaler", annual: "490k", contractEnd: "2026-06", renewalRisk: "Low", satisfaction: 5 },
      { vendor: "Duo Security", annual: "521k", contractEnd: "2026-07", renewalRisk: "Medium", satisfaction: 4 },
    ];
    const roiProjections = [
      { area: "Threat Detection", investment: "279k", projectedReturn: "486k", roiMultiple: "2.4x", confidence: 95 },
      { area: "Incident Reduction", investment: "322k", projectedReturn: "533k", roiMultiple: "2.3x", confidence: 76 },
      { area: "Compliance Savings", investment: "365k", projectedReturn: "580k", roiMultiple: "2.2x", confidence: 93 },
      { area: "Automation Gains", investment: "408k", projectedReturn: "627k", roiMultiple: "2.1x", confidence: 74 },
      { area: "Risk Avoidance", investment: "451k", projectedReturn: "674k", roiMultiple: "2.0x", confidence: 91 },
    ];
    return html`
  ...
  private _renderMetricsNormalization() {
    const kpiCatalog = [
      { id: "kpi-1", name: "MTTD", owner: "SOC", unit: "minutes", target: 96, current: 71, benchmark: 69, collection: "auto", frequency: "realtime" },
      { id: "kpi-2", name: "MTTR", owner: "GRC", unit: "%", target: 99, current: 78, benchmark: 80, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-3", name: "MTTC", owner: "AppSec", unit: "score", target: 72, current: 85, benchmark: 91, collection: "manual", frequency: "weekly" },
      { id: "kpi-4", name: "Vuln SLA Compliance", owner: "Cloud Sec", unit: "count", target: 75, current: 92, benchmark: 68, collection: "auto", frequency: "monthly" },
      { id: "kpi-5", name: "Patch Coverage", owner: "Identity", unit: "days", target: 78, current: 99, benchmark: 79, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-6", name: "Phishing Click Rate", owner: "Threat Intel", unit: "minutes", target: 81, current: 60, benchmark: 90, collection: "manual", frequency: "daily" },
      { id: "kpi-7", name: "Training Completion", owner: "Security Ops", unit: "%", target: 84, current: 67, benchmark: 67, collection: "auto", frequency: "weekly" },
      { id: "kpi-8", name: "Escalation Rate", owner: "Risk Mgmt", unit: "score", target: 87, current: 74, benchmark: 78, collection: "semi-auto", frequency: "monthly" },
      { id: "kpi-9", name: "False Positive Rate", owner: "SOC", unit: "count", target: 90, current: 81, benchmark: 89, collection: "manual", frequency: "realtime" },
      { id: "kpi-10", name: "Threat Intel Actionability", owner: "GRC", unit: "days", target: 93, current: 88, benchmark: 66, collection: "auto", frequency: "daily" },
      { id: "kpi-11", name: "Endpoint Compliance", owner: "AppSec", unit: "minutes", target: 96, current: 95, benchmark: 77, collection: "semi-auto", frequency: "weekly" },
      { id: "kpi-12", name: "Cloud Misconfig Score", owner: "Cloud Sec", unit: "%", target: 99, current: 56, benchmark: 88, collection: "manual", frequency: "monthly" },
      { id: "kpi-13", name: "Identity Anomaly Rate", owner: "Identity", unit: "score", target: 72, current: 63, benchmark: 65, collection: "auto", frequency: "realtime" },
      { id: "kpi-14", name: "DLP Events", owner: "Threat Intel", unit: "count", target: 75, current: 70, benchmark: 76, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-15", name: "Vendor Risk Avg", owner: "Security Ops", unit: "days", target: 78, current: 77, benchmark: 87, collection: "manual", frequency: "weekly" },
      { id: "kpi-16", name: "Compliance Audit Pass Rate", owner: "Risk Mgmt", unit: "minutes", target: 81, current: 84, benchmark: 98, collection: "auto", frequency: "monthly" },
      { id: "kpi-17", name: "Awareness Score", owner: "SOC", unit: "%", target: 84, current: 91, benchmark: 75, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-18", name: "SOC Utilization", owner: "GRC", unit: "score", target: 87, current: 98, benchmark: 86, collection: "manual", frequency: "daily" },
      { id: "kpi-19", name: "Automation Coverage", owner: "AppSec", unit: "count", target: 90, current: 59, benchmark: 97, collection: "auto", frequency: "weekly" },
      { id: "kpi-20", name: "Risk Register Currency", owner: "Cloud Sec", unit: "days", target: 93, current: 66, benchmark: 74, collection: "semi-auto", frequency: "monthly" },
    ];
    const benchmarkSources = [
      { source: "NIST CSF", mappedKPIs: 5, alignment: 80, lastReview: "2026-03-27", status: "aligned" },
      { source: "CIS Controls v8", mappedKPIs: 6, alignment: 97, lastReview: "2026-04-22", status: "partial" },
      { source: "ISO 27001:2022", mappedKPIs: 7, alignment: 75, lastReview: "2026-05-17", status: "reviewing" },
      { source: "PCI DSS 4.0", mappedKPIs: 8, alignment: 92, lastReview: "2026-06-12", status: "aligned" },
      { source: "SOC 2 Type II", mappedKPIs: 3, alignment: 70, lastReview: "2026-01-07", status: "partial" },
      { source: "MITRE ATT&CK", mappedKPIs: 4, alignment: 87, lastReview: "2026-02-02", status: "reviewing" },
      { source: "SANS Top 20", mappedKPIs: 5, alignment: 65, lastReview: "2026-03-25", status: "aligned" },
      { source: "OWASP Top 10", mappedKPIs: 6, alignment: 82, lastReview: "2026-04-20", status: "partial" },
    ];
  ...
  private _renderThreatHuntingCampaigns() {
    const campaigns = [
      { id: "HC-1001", name: "Lateral Movement Sweep", status: "active", hypothesis: "H1: Actors using pass-the-hash for lateral movement", leadHunter: "Alice Chen", findings: 42, startDate: "2026-03-19", endDate: null, effectiveness: 40 },
      { id: "HC-1002", name: "Credential Harvesting Hunt", status: "completed", hypothesis: "H2: Actors using web shells for persistence", leadHunter: "Bob Martinez", findings: 45, startDate: "2026-02-02", endDate: "2026-05-08", effectiveness: 59 },
      { id: "HC-1003", name: "Persistence Mechanism Audit", status: "planned", hypothesis: "H3: Actors using scheduled tasks for data theft", leadHunter: "Carol Wu", findings: 0, startDate: "2026-01-13", endDate: null, effectiveness: 78 },
      { id: "HC-1004", name: "C2 Beacon Detection", status: "in-review", hypothesis: "H4: Actors using DNS tunneling for C2 communication", leadHunter: "Dave Kim", findings: 3, startDate: "2026-04-24", endDate: null, effectiveness: 97 },
      { id: "HC-1005", name: "Data Exfiltration Patterns", status: "active", hypothesis: "H5: Actors using encrypted channels for privilege escalation", leadHunter: "Eve Johnson", findings: 6, startDate: "2026-03-07", endDate: null, effectiveness: 57 },
      { id: "HC-1006", name: "Privilege Escalation Scan", status: "completed", hypothesis: "H6: Actors using token impersonation for defense evasion", leadHunter: "Frank Liu", findings: 9, startDate: "2026-02-18", endDate: "2026-06-20", effectiveness: 76 },
      { id: "HC-1007", name: "Supply Chain Implant Hunt", status: "planned", hypothesis: "H7: Actors using poisoned images for initial access", leadHunter: "Grace Park", findings: 12, startDate: "2026-01-01", endDate: null, effectiveness: 95 },
      { id: "HC-1008", name: "Insider Threat Indicators", status: "in-review", hypothesis: "H8: Actors using legitimate tools for credential access", leadHunter: "Hector Silva", findings: 15, startDate: "2026-04-12", endDate: null, effectiveness: 55 },
      { id: "HC-1009", name: "Cloud Metadata Analysis", status: "active", hypothesis: "H9: Actors using API keys for command execution", leadHunter: "Alice Chen", findings: 18, startDate: "2026-03-23", endDate: null, effectiveness: 74 },
      { id: "HC-1010", name: "DNS Tunnel Detection", status: "completed", hypothesis: "H10: Actors using encoded subdomains for exfiltration", leadHunter: "Bob Martinez", findings: 21, startDate: "2026-02-06", endDate: "2026-04-04", effectiveness: 93 },
      { id: "HC-1011", name: "Fileless Malware Search", status: "planned", hypothesis: "H11: Actors using WMI providers for discovery", leadHunter: "Carol Wu", findings: 24, startDate: "2026-01-17", endDate: null, effectiveness: 53 },
      { id: "HC-1012", name: "Zero-Day Exploit Traces", status: "in-review", hypothesis: "H12: Actors using exploit kits for collection", leadHunter: "Dave Kim", findings: 27, startDate: "2026-04-28", endDate: null, effectiveness: 72 },
    ];
    const hunterLeaderboard = [
      { hunter: "Alice Chen", campaigns: 9, findings: 47, highSeverity: 6, avgScore: 73, streak: 3 },
      { hunter: "Bob Martinez", campaigns: 6, findings: 76, highSeverity: 11, avgScore: 66, streak: 4 },
      { hunter: "Carol Wu", campaigns: 3, findings: 105, highSeverity: 16, avgScore: 59, streak: 5 },
      { hunter: "Dave Kim", campaigns: 13, findings: 18, highSeverity: 21, avgScore: 96, streak: 6 },
      { hunter: "Eve Johnson", campaigns: 10, findings: 47, highSeverity: 0, avgScore: 89, streak: 7 },
      { hunter: "Frank Liu", campaigns: 7, findings: 76, highSeverity: 5, avgScore: 82, streak: 8 },
      { hunter: "Grace Park", campaigns: 4, findings: 105, highSeverity: 10, avgScore: 75, streak: 1 },
      { hunter: "Hector Silva", campaigns: 14, findings: 18, highSeverity: 15, avgScore: 68, streak: 2 },
    ];
    const mitreMapping = [
      { tactic: "Initial Access", techniques: 9, campaigns: 1, coverage: 62 },
      { tactic: "Execution", techniques: 8, campaigns: 6, coverage: 42 },
      { tactic: "Persistence", techniques: 7, campaigns: 5, coverage: 93 },
      { tactic: "Privilege Escalation", techniques: 6, campaigns: 4, coverage: 73 },
      { tactic: "Defense Evasion", techniques: 5, campaigns: 3, coverage: 53 },
      { tactic: "Credential Access", techniques: 4, campaigns: 2, coverage: 33 },
      { tactic: "Discovery", techniques: 3, campaigns: 1, coverage: 84 },
      { tactic: "Lateral Movement", techniques: 2, campaigns: 6, coverage: 64 },
      { tactic: "Collection", techniques: 12, campaigns: 5, coverage: 44 },
      { tactic: "Exfiltration", techniques: 11, campaigns: 4, coverage: 95 },
      { tactic: "Command & Control", techniques: 10, campaigns: 3, coverage: 75 },
      { tactic: "Impact", techniques: 9, campaigns: 2, coverage: 55 },
    ];
    return html`
  ...
  private _renderControlInventory() {
    const controls = [
      { id: "CTL-2001", name: "MFA Enforcement", domain: "Access Control", status: "implemented", effectiveness: 96, lastTest: "2026-03-11", nextReview: "2026-07-11", owner: "SOC", risk: "Low" },
      { id: "CTL-2002", name: "Network Segmentation", domain: "Network Security", status: "partial", effectiveness: 5, lastTest: "2026-02-24", nextReview: "2026-08-02", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2003", name: "EDR Deployment", domain: "Endpoint Protection", status: "planned", effectiveness: 12, lastTest: "2026-01-09", nextReview: "2026-09-21", owner: "IT Ops", risk: "High" },
      { id: "CTL-2004", name: "DLP Policy", domain: "Data Protection", status: "gap", effectiveness: 19, lastTest: "2026-04-22", nextReview: "2026-10-12", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2005", name: "SSO Integration", domain: "Identity Management", status: "implemented", effectiveness: 49, lastTest: "2026-03-07", nextReview: "2026-11-03", owner: "IAM", risk: "Low" },
      { id: "CTL-2006", name: "SAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 33, lastTest: "2026-02-20", nextReview: "2026-12-22", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2007", name: "CSPM Scanning", domain: "Cloud Security", status: "planned", effectiveness: 40, lastTest: "2026-01-05", nextReview: "2026-05-13", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2008", name: "Badge Access", domain: "Physical Security", status: "gap", effectiveness: 47, lastTest: "2026-04-18", nextReview: "2026-06-04", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2009", name: "Least Privilege", domain: "Access Control", status: "implemented", effectiveness: 61, lastTest: "2026-03-03", nextReview: "2026-07-23", owner: "SOC", risk: "Low" },
      { id: "CTL-2010", name: "Firewall Rules", domain: "Network Security", status: "partial", effectiveness: 10, lastTest: "2026-02-16", nextReview: "2026-08-14", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2011", name: "Disk Encryption", domain: "Endpoint Protection", status: "planned", effectiveness: 17, lastTest: "2026-01-01", nextReview: "2026-09-05", owner: "IT Ops", risk: "High" },
      { id: "CTL-2012", name: "Data Classification", domain: "Data Protection", status: "gap", effectiveness: 24, lastTest: "2026-04-14", nextReview: "2026-10-24", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2013", name: "PAM Implementation", domain: "Identity Management", status: "implemented", effectiveness: 73, lastTest: "2026-03-27", nextReview: "2026-11-15", owner: "IAM", risk: "Low" },
      { id: "CTL-2014", name: "DAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 38, lastTest: "2026-02-12", nextReview: "2026-12-06", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2015", name: "IAM Policy Review", domain: "Cloud Security", status: "planned", effectiveness: 45, lastTest: "2026-01-25", nextReview: "2026-05-25", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2016", name: "Visitor Management", domain: "Physical Security", status: "gap", effectiveness: 1, lastTest: "2026-04-10", nextReview: "2026-06-16", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2017", name: "Access Reviews", domain: "Access Control", status: "implemented", effectiveness: 85, lastTest: "2026-03-23", nextReview: "2026-07-07", owner: "SOC", risk: "Low" },
      { id: "CTL-2018", name: "IDS/IPS Tuning", domain: "Network Security", status: "partial", effectiveness: 15, lastTest: "2026-02-08", nextReview: "2026-08-26", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2019", name: "Patch Management", domain: "Endpoint Protection", status: "planned", effectiveness: 22, lastTest: "2026-01-21", nextReview: "2026-09-17", owner: "IT Ops", risk: "High" },
      { id: "CTL-2020", name: "Backup Encryption", domain: "Data Protection", status: "gap", effectiveness: 29, lastTest: "2026-04-06", nextReview: "2026-10-08", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2021", name: "Password Policy", domain: "Identity Management", status: "implemented", effectiveness: 97, lastTest: "2026-03-19", nextReview: "2026-11-27", owner: "IAM", risk: "Low" },
      { id: "CTL-2022", name: "Container Scanning", domain: "Application Security", status: "partial", effectiveness: 43, lastTest: "2026-02-04", nextReview: "2026-12-18", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2023", name: "WAF Configuration", domain: "Cloud Security", status: "planned", effectiveness: 50, lastTest: "2026-01-17", nextReview: "2026-05-09", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2024", name: "CCTV Coverage", domain: "Physical Security", status: "gap", effectiveness: 6, lastTest: "2026-04-02", nextReview: "2026-06-28", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2025", name: "RBAC Enforcement", domain: "Access Control", status: "implemented", effectiveness: 50, lastTest: "2026-03-15", nextReview: "2026-07-19", owner: "SOC", risk: "Low" },
      { id: "CTL-2026", name: "VPN Management", domain: "Network Security", status: "partial", effectiveness: 20, lastTest: "2026-02-28", nextReview: "2026-08-10", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2027", name: "App Whitelisting", domain: "Endpoint Protection", status: "planned", effectiveness: 27, lastTest: "2026-01-13", nextReview: "2026-09-01", owner: "IT Ops", risk: "High" },
      { id: "CTL-2028", name: "Key Management", domain: "Data Protection", status: "gap", effectiveness: 34, lastTest: "2026-04-26", nextReview: "2026-10-20", owner: "Data Gov", risk: "Critical" },
    ];
    const gapAnalysis = [
      { gap: "Insufficient MFA coverage for legacy apps", severity: "High", remediationPlan: "Plan R3001", eta: "2026-Q3", estimatedCost: "156k" },
      { gap: "Missing network micro-segmentation", severity: "Medium", remediationPlan: "Plan R3002", eta: "2026-Q2", estimatedCost: "185k" },
      { gap: "Inconsistent EDR deployment", severity: "Medium", remediationPlan: "Plan R3003", eta: "2026-Q4", estimatedCost: "33k" },
      { gap: "DLP not covering cloud storage", severity: "Low", remediationPlan: "Plan R3004", eta: "2026-Q3", estimatedCost: "62k" },
      { gap: "SSO not integrated with all SaaS", severity: "High", remediationPlan: "Plan R3005", eta: "2026-Q2", estimatedCost: "91k" },
    ];
    return html`
  ...
  private _renderIncidentCostTracker() {
    const incidents = [
      { id: "INC-7001", name: "Security Incident 1", severity: "Critical", totalCost: 1831000, responseCost: 311270, recoveryCost: 402820, legalCost: 274650, regulatoryCost: 183100, insuranceClaim: 0, avoidedCost: 239000, date: "2026-03-03" },
      { id: "INC-7002", name: "Security Incident 2", severity: "High", totalCost: 1834000, responseCost: 440160, recoveryCost: 605220, legalCost: 220080, regulatoryCost: 91700, insuranceClaim: 568540, avoidedCost: 268000, date: "2026-02-12" },
      { id: "INC-7003", name: "Security Incident 3", severity: "Medium", totalCost: 1837000, responseCost: 569470, recoveryCost: 422510, legalCost: 165330, regulatoryCost: 202070, insuranceClaim: 0, avoidedCost: 297000, date: "2026-01-21" },
      { id: "INC-7004", name: "Security Incident 4", severity: "Low", totalCost: 1840000, responseCost: 312800, recoveryCost: 625600, legalCost: 110400, regulatoryCost: 110400, insuranceClaim: 1416800, avoidedCost: 326000, date: "2026-04-02" },
      { id: "INC-7005", name: "Security Incident 5", severity: "Critical", totalCost: 1843000, responseCost: 442320, recoveryCost: 442320, legalCost: 350170, regulatoryCost: 221160, insuranceClaim: 0, avoidedCost: 355000, date: "2026-03-11" },
      { id: "INC-7006", name: "Security Incident 6", severity: "High", totalCost: 1846000, responseCost: 572260, recoveryCost: 646100, legalCost: 295360, regulatoryCost: 129220, insuranceClaim: 1329120, avoidedCost: 384000, date: "2026-02-20" },
      { id: "INC-7007", name: "Security Incident 7", severity: "Medium", totalCost: 1849000, responseCost: 314330, recoveryCost: 462250, legalCost: 240370, regulatoryCost: 240370, insuranceClaim: 0, avoidedCost: 413000, date: "2026-01-01" },
      { id: "INC-7008", name: "Security Incident 8", severity: "Low", totalCost: 1852000, responseCost: 444480, recoveryCost: 666720, legalCost: 185200, regulatoryCost: 148160, insuranceClaim: 1240840, avoidedCost: 442000, date: "2026-04-10" },
      { id: "INC-7009", name: "Security Incident 9", severity: "Critical", totalCost: 1855000, responseCost: 575050, recoveryCost: 482300, legalCost: 129850, regulatoryCost: 259700, insuranceClaim: 0, avoidedCost: 471000, date: "2026-03-19" },
      { id: "INC-7010", name: "Security Incident 10", severity: "High", totalCost: 1858000, responseCost: 315860, recoveryCost: 687460, legalCost: 371600, regulatoryCost: 167220, insuranceClaim: 1151960, avoidedCost: 500000, date: "2026-02-28" },
      { id: "INC-7011", name: "Security Incident 11", severity: "Medium", totalCost: 1861000, responseCost: 446640, recoveryCost: 502470, legalCost: 316370, regulatoryCost: 279150, insuranceClaim: 0, avoidedCost: 33000, date: "2026-01-09" },
      { id: "INC-7012", name: "Security Incident 12", severity: "Low", totalCost: 1864000, responseCost: 577840, recoveryCost: 708320, legalCost: 260960, regulatoryCost: 186400, insuranceClaim: 1062480, avoidedCost: 62000, date: "2026-04-18" },
    ];
    const yearlyTrend = [
      { month: "Jan", incidents: 7, totalCost: "548k", avgCost: "70k", insured: 49 },
      { month: "Feb", incidents: 4, totalCost: "591k", avgCost: "117k", insured: 49 },
      { month: "Mar", incidents: 12, totalCost: "634k", avgCost: "164k", insured: 49 },
      { month: "Apr", incidents: 9, totalCost: "677k", avgCost: "30k", insured: 49 },
      { month: "May", incidents: 6, totalCost: "720k", avgCost: "77k", insured: 49 },
      { month: "Jun", incidents: 3, totalCost: "763k", avgCost: "124k", insured: 49 },
    ];
...
declare global { interface HTMLElementTagNameMap { 'sc-log-query': ScLogQuery; } }
