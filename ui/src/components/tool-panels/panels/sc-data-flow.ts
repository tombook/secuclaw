...
@customElement('sc-data-flow')
export class ScDataFlow extends LitElement {
  static styles = css`
...
@state() private _tablePage = 1;
  ...
  @state() private _comments: CommentEntry[] = [
    { id: 'c1', author: 'soc-tier1', timestamp: '2026-04-23 04:00', text: 'Initial assessment started for Data Flow finding #1' },
  ...
  private _items: PanelItem[] = [
      {
            "id": "data-flow-1",
            "title": "User -> Web App (HTTPS)",
            "severity": "low",
            "status": "open",
            "category": "Security",
            "source": "SIEM",
            "description": "TLS 1.3 | Data: Login creds, PII | Direction: Inbound | Risk: Low | Classification: Confidential",
      ...
      {
            "id": "data-flow-2",
            "title": "Web App -> API Gateway (mTLS)",
            "severity": "low",
            "status": "in-progress",
            "category": "Network",
            "source": "EDR",
            "description": "mTLS with service mesh | Data: Business requests | Direction: Internal | Classification: Internal",
      ...
      {
            "id": "data-flow-3",
            "title": "API -> Database (TLS)",
            "severity": "medium",
            "status": "resolved",
            "category": "Access",
            "source": "Scanner",
            "description": "MySQL TLS connection | Data: All business data | Direction: Internal | Classification: Restricted",
      ...
      {
            "id": "data-flow-4",
            "title": "Database -> Analytics (ETL)",
            "severity": "high",
            "status": "open",
            "category": "Compliance",
            "source": "Audit",
            "description": "S3 transfer (encrypted) | Data: PII partially masked | Direction: Outbound | Classification: Confidential",
      ...
      },
      {
            ...
            "category": "Operations",
            "source": "Manual",
            "description": "CSV export via email | Data: Aggregated PII | DLP: Enabled but bypassed | Risk: HIGH",
      ...
      {
            "id": "data-flow-6",
            "title": "Data Flow Finding #6",
            "severity": "high",
            "status": "open",
            "category": "Security",
            "source": "SIEM",
            "description": "Automated data flow assessment finding detected during routine scan of infrastructure component. Review and remediation recommended.",
  ...
  private _history: HistoryEntry[] = [
    { timestamp: '2026-04-23 04:15', action: 'Auto-correlated', user: 'system', details: 'Grouped related findings from Data Flow analysis' },
    { timestamp: '2026-04-23 03:00', action: 'Created', user: 'scanner', details: 'New Data Flow finding detected by automated assessment' },
    ...
    { timestamp: '2026-04-22 22:00', action: 'Resolved', user: 'security-eng', details: 'Remediation applied and verified for Data Flow finding' },
    { timestamp: '2026-04-22 18:00', action: 'Created', user: 'audit', details: 'Compliance audit identified Data Flow gap requiring remediation' },
    ...
    { timestamp: '2026-04-21 16:00', action: 'Resolved', user: 'soc-tier1', details: 'False positive confirmed - benign activity flagged by Data Flow scan' },
    { timestamp: '2026-04-21 12:00', action: 'Exported', user: 'compliance', details: 'Exported Data Flow findings for Q1 compliance report' },
  ];
...
private _workflowTasks: WorkflowTask[] = [
    { id: 'wt-1', title: 'Review Data Flow finding #1', status: 'active', assignee: 'soc-tier1', blockedBy: [], slaDeadline: '2026-04-23T12:00:00Z', priority: 'p1', createdAt: '2026-04-23T00:00:00Z', completedAt: null },
    ...
    { id: 'wt-3', title: 'Update Data Flow detection rules', status: 'blocked', assignee: 'soc-tier2', blockedBy: ['wt-2'], slaDeadline: '2026-04-24T00:00:00Z', priority: 'p2', createdAt: '2026-04-23T03:00:00Z', completedAt: null },
    ...
    { id: 'wt-8', title: 'Archive resolved Data Flow findings', status: 'failed', assignee: 'soc-tier1', blockedBy: [], slaDeadline: '2026-04-23T08:00:00Z', priority: 'p4', createdAt: '2026-04-22T22:00:00Z', completedAt: null },
  ];

private _executionHistory: ExecutionRecord[] = [
    {
      id: 'exec-1', name: 'Data Flow Full Assessment Run', startedAt: '2026-04-22T10:00:00Z', completedAt: '2026-04-22T10:12:00Z', status: 'success',
      steps: [
    ...
    {
      id: 'exec-2', name: 'Data Flow Delta Scan', startedAt: '2026-04-23T02:00:00Z', completedAt: '2026-04-23T02:05:00Z', status: 'failed',
      steps: [
  ...
  private _runExecution() {
    ...
    const exec: ExecutionRecord = {
      id: 'exec-' + Date.now(), name: 'Data Flow Assessment ' + new Date().toISOString().slice(0, 10), startedAt: new Date().toISOString(), completedAt: null, status: 'running', steps,
    };
  ...
  private _renderExecutionPanel(): unknown {
    ...
    return html`
      <div class="form-section">
        ...
        ` : html`<div class="empty-state">Click "Run Assessment" to start the Data Flow analysis pipeline</div>`}
      </div>
      <div class="form-section">
        ...
        <div class="settings-card">
          ...
          <div style="display:flex;gap:8px;margin-top:4px;">
            <button class="btn primary" @click=${() => {
              ...
              const a = document.createElement('a'); a.href = url; a.download = 'data-flow-config.json'; a.click();
    ...
    return html`
      <div style="margin-top:12px;">
        ...
        <div style="display:flex;gap:6px;margin-top:8px;">
          <input class="search-box" type="text" placeholder="Add a comment..." style="min-width:auto;flex:1;" id="data-flow-comment-input" />
          <button class="btn primary" @click=${() => {
            const input = this.shadowRoot?.querySelector('#data-flow-comment-input') as HTMLInputElement;
  ...
  private _exportJSON() {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'sc-data-flow-data.json'; a.click();
    URL.revokeObjectURL(url);
  }
  ...
  private _exportCSV() {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'sc-data-flow-data.csv'; a.click();
    URL.revokeObjectURL(url);
  }


  // --- Domain Rules Engine ---
  @state() private _dfRules: { id: string; name: string; category: string; severity: Severity; enabled: boolean; lastEval: string; passRate: number }[] = [];
  private _initDfRules() {
    const rules = [
  ...
  private _evaluateDfRules(): { passed: number; failed: number; skipped: number; total: number } {
    let passed = 0, failed = 0, skipped = 0;
    this._dfRules.forEach(r => { if (!r.enabled) { skipped++; } else if (r.passRate >= 80) { passed++; } else { failed++; } });
    return { passed, failed, skipped, total: this._dfRules.length };
  }

  // --- CVSS Scoring ---
  @state() private _dfcvssData: { itemId: string; vector: string; base: number; temporal: number; environmental: number; overall: number }[] = [];
  private _initDfCvss() {
    ...
    this._dfcvssData = vectors.map((v, i) => {
  ...
  // --- Anomaly Detection ---
  @state() private _dfanomalies: { id: string; type: string; severity: Severity; description: string; detected: string; confidence: number; affected: string[] }[] = [];
  private _runDfAnomalyDetection() {
    const types = [
    ...
    this._dfanomalies = types.map((a, i) => ({
  ...
  // --- Trend Prediction ---
  @state() private _dfpredictions: { horizon: string; metric: string; current: number; predicted: number; direction: 'up' | 'down' | 'stable'; confidence: number }[] = [];
  private _generateDfPredictions() {
    this._dfpredictions = [
  ...
  // --- Approval Workflow ---
  @state() private _dfApprovals: { id: string; title: string; requester: string; status: 'pending' | 'approved' | 'rejected' | 'expired'; createdAt: string; priority: Priority; type: string }[] = [];
  private _initDfApprovals() {
    this._dfApprovals = [
  ...
  private _approveDfItem(id: string) { const item = this._dfApprovals.find(a => a.id === id); if (item) item.status = 'approved'; this.requestUpdate(); }
  private _rejectDfItem(id: string) { const item = this._dfApprovals.find(a => a.id === id); if (item) item.status = 'rejected'; this.requestUpdate(); }

  // --- Activity Feed ---
  @state() private _dfActivity: { id: string; action: string; user: string; target: string; timestamp: string }[] = [];
  private _initDfActivity() {
    const actions = [
    ...
    this._dfActivity = actions.map((a, i) => ({ id: 'ACT-' + String(i + 1).padStart(3, '0'), ...a, timestamp: new Date(Date.now() - i * 3600000).toISOString() }));
  }

  // --- Notification System ---
  @state() private _dfNotifications: { id: string; message: string; type: 'info' | 'warning' | 'error' | 'success'; read: boolean; timestamp: string }[] = [];
  private _initDfNotifications() {
    this._dfNotifications = [
  ...
  private _markDfNotifRead(id: string) { const n = this._dfNotifications.find(x => x.id === id); if (n) n.read = true; this.requestUpdate(); }

  // --- Panel Configuration ---
  @state() private _dfConfig: { layout: 'compact' | 'default' | 'expanded'; theme: 'dark' | 'midnight' | 'slate'; showAnomalies: boolean; showPredictions: boolean; autoRefresh: boolean; refreshInterval: number } = {
    layout: 'default', theme: 'dark', showAnomalies: true, showPredictions: true, autoRefresh: true, refreshInterval: 60,
  };
  private _dfPresets: { name: string; config: typeof this._dfConfig }[] = [
  ...
  private _applyDfPreset(preset: typeof this._dfPresets[0]) { this._dfConfig = { ...preset.config }; this.requestUpdate(); }

  private _renderDfTreemapSVG(): string {
    const categories = [
  ...
  private _renderDfSankeySVG(): string {
  ...
  // --- Render: Rules Engine ---
  private _renderDfRules(): any {
    const ev = this._evaluateDfRules();
    return html`
  ...
  private _renderDfAnomalies(): any {
  ...
  // --- Render: Approvals ---
  private _renderDfApprovals(): any {
          ...
          $${this._dfApprovals.map(a => html`
            <div style="padding:6px 8px;background:#1f2937;border-radius:4px">
              ...
              $${a.status === 'pending' ? html`
                <div style="display:flex;gap:4px;margin-top:4px">
                  <button class="btn success" style="padding:2px 8px;font-size:9px" @click=$${() => this._approveDfItem(a.id)}>Approve</button>
                  <button class="btn error" style="padding:2px 8px;font-size:9px" @click=$${() => this._rejectDfItem(a.id)}>Reject</button>
                </div>` : nothing}
            </div>`)}
        </div>
      </div>`;
  ...
  private _renderDfNotifications(): any {
    const tc = (t: string) => t === 'error' ? '#ef4444' : t === 'warning' ? '#eab308' : t === 'success' ? '#22c55e' : '#3b82f6';
    const unread = this._dfNotifications.filter(n => !n.read).length;
    return html`
      <div style="background:#1a1d27;border-radius:8px;padding:12px;margin-top:12px">
        ...
        <div style="display:flex;flex-direction:column;gap:4px;max-height:120px;overflow-y:auto">
          $${this._dfNotifications.map(n => html`
            <div style="display:flex;align-items:center;gap:8px;padding:4px 8px;background:$${n.read ? '#1f2937' : '#252a36'};border-radius:4px;font-size:10px;opacity:$${n.read ? '0.6' : '1'};cursor:pointer" @click=$${() => this._markDfNotifRead(n.id)}>
      ...
      <div style="background:#1a1d27;border-radius:8px;padding:12px;margin-top:12px">
        ...
        <div style="display:flex;flex-direction:column;gap:6px;font-size:10px">
          <div style="display:flex;align-items:center;gap:8px">
            ...
            <select class="form-input" style="flex:1" @change=$${(e: Event) => { this._dfConfig.layout = (e.target as HTMLSelectElement).value as any; this.requestUpdate(); }}>
              <option value="compact" ?selected=$${this._dfConfig.layout === 'compact'}>Compact</option>
              <option value="default" ?selected=$${this._dfConfig.layout === 'default'}>Default</option>
              <option value="expanded" ?selected=$${this._dfConfig.layout === 'expanded'}>Expanded</option>
            </select>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:100px">Theme</span>
            <select class="form-input" style="flex:1" @change=$${(e: Event) => { this._dfConfig.theme = (e.target as HTMLSelectElement).value as any; this.requestUpdate(); }}>
              <option value="dark" ?selected=$${this._dfConfig.theme === 'dark'}>Dark</option>
              <option value="midnight" ?selected=$${this._dfConfig.theme === 'midnight'}>Midnight</option>
              <option value="slate" ?selected=$${this._dfConfig.theme === 'slate'}>Slate</option>
            </select>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:100px">Auto Refresh</span>
            <input type="checkbox" ?checked=$${this._dfConfig.autoRefresh} @change=$${() => { this._dfConfig.autoRefresh = !this._dfConfig.autoRefresh; this.requestUpdate(); }}/>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:100px">Show Anomalies</span>
            <input type="checkbox" ?checked=$${this._dfConfig.showAnomalies} @change=$${() => { this._dfConfig.showAnomalies = !this._dfConfig.showAnomalies; this.requestUpdate(); }}/>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:100px">Show Predictions</span>
            <input type="checkbox" ?checked=$${this._dfConfig.showPredictions} @change=$${() => { this._dfConfig.showPredictions = !this._dfConfig.showPredictions; this.requestUpdate(); }}/>
          </div>
          <div style="margin-top:6px;font-weight:600">Presets</div>
          <div style="display:flex;gap:4px">
            $${this._dfPresets.map(ps => html`<button class="btn" style="padding:2px 8px;font-size:9px" @click=$${() => this._applyDfPreset(ps)}>$${ps.name}</button>`)}
          </div>
        </div>
      </div>`;
  }
  ...
  @state() private _dataFlowRiskTopTen: Array<{id:string;name:string;score:number;trend:string;owner:string;category:string;lastAssessed:string}> = [
  ...
  @state() private _dataFlowRiskCategories: Array<{category:string;count:number;avgScore:number;color:string}> = [
  ...
  @state() private _dataFlowRiskVelocity: Array<{week:string;newRisks:number;closedRisks:number;netChange:number}> = [
  ...
  @state() private _dataFlowRiskAppetite: {current:number;threshold:number;maxTolerated:number;status:string} = {
    current: 68, threshold: 55, maxTolerated: 80, status: 'Warning'
  };
  @state() private _dataFlowRiskOwnerMatrix: Array<{owner:string;criticalCount:number;highCount:number;mediumCount:number;complianceRate:number}> = [
  ...
  private _renderDataflowRiskDash(): TemplateResult {
    const riskItems = this._dataFlowRiskTopTen;
    const velocity = this._dataFlowRiskVelocity;
    const appetite = this._dataFlowRiskAppetite;
    return html`
  ...
  @state() private _dataFlowCorrRules: Array<{id:string;name:string;sources:string[];logic:string;severity:string;active:boolean;lastTriggered:string}> = [
  ...
  @state() private _dataFlowEventTimeline: Array<{timestamp:string;source:string;eventType:string;details:string;correlated:boolean}> = [
  ...
  @state() private _dataFlowFalsePosMetrics: {totalEvents:number;correlatedEvents:number;falsePositives:number;fpRate:number;topFpRules:string[]} = {
  ...
  @state() private _dataFlowEventPatterns: Array<{id:string;pattern:string;frequency:number;firstSeen:string;lastSeen:string;status:string}> = [
  ...
  private _renderDataflowEventCorr(): TemplateResult {
    const rules = this._dataFlowCorrRules;
    const timeline = this._dataFlowEventTimeline;
    const fpMetrics = this._dataFlowFalsePosMetrics;
    return html`
  ...
  @state() private _dataFlowCourses: Array<{id:string;title:string;category:string;duration:number;enrolled:number;completed:number;difficulty:string;rating:number}> = [
  ...
  @state() private _dataFlowLearningPaths: Array<{id:string;name:string;courseIds:string[];progress:number;enrolled:number}> = [
  ...
  @state() private _dataFlowDeptCompliance: Array<{dept:string;trainedPct:number;targetPct:number;avgScore:number;certCount:number}> = [
  ...
  @state() private _dataFlowSkillsGaps: Array<{skill:string;current:number;required:number;gap:number;priority:string}> = [
  ...
  private _renderDataflowTraining(): TemplateResult {
    const courses = this._dataFlowCourses;
    const deptComp = this._dataFlowDeptCompliance;
    return html`
  ...
  @state() private _dataFlowContainerScans: Array<{image:string;registry:string;critical:number;high:number;medium:number;scanDate:string;status:string}> = [
  ...
  @state() private _dataFlowK8sPods: Array<{namespace:string;pod:string;securityContext:string;hostIPC:boolean;hostPID:boolean;privileged:boolean;riskLevel:string}> = [
  ...
  @state() private _dataFlowServerlessRisk: Array<{function:string;runtime:string;timeout:number;iamPerms:string;externalCalls:number;riskScore:number}> = [
  ...
  @state() private _dataFlowRuntimeAlerts: Array<{id:string;workload:string;alertType:string;severity:string;description:string;timestamp:string}> = [
  ...
  private _renderDataflowCloudWl(): TemplateResult {
    const containers = this._dataFlowContainerScans;
    const pods = this._dataFlowK8sPods;
    const alerts = this._dataFlowRuntimeAlerts;
    return html`
  ...
  @state() private _complianceCalendarEvents: Array<{id: string; title: string; deadline: string; category: string; status: string; assignee: string; notes: string}> = [
    {id: 'cc-data-flow-001', title: 'SOX Section 404 Compliance Review', deadline: '2026-06-30', category: 'Regulatory', status: 'In Progress', assignee: 'Compliance Team', notes: 'Annual internal control assessment for financial reporting'},
    {id: 'cc-data-flow-002', title: 'GDPR Data Protection Impact Assessment', deadline: '2026-05-15', category: 'Privacy', status: 'Pending', assignee: 'DPO Office', notes: 'Quarterly DPIA for high-risk processing activities'},
    {id: 'cc-data-flow-003', title: 'ISO 27001 Surveillance Audit', deadline: '2026-07-20', category: 'Certification', status: 'Scheduled', assignee: 'QA Lead', notes: 'Stage 2 surveillance audit for ISMS certification renewal'},
    {id: 'cc-data-flow-004', title: 'PCI DSS v4.0 Gap Analysis', deadline: '2026-05-30', category: 'Payment', status: 'Not Started', assignee: 'Security Architect', notes: 'Assess current state against PCI DSS v4.0 requirements'},
    {id: 'cc-data-flow-005', title: 'NIST CSF Self-Assessment', deadline: '2026-08-15', category: 'Framework', status: 'Pending', assignee: 'CISO', notes: 'Biannual self-assessment against NIST Cybersecurity Framework'},
    {id: 'cc-data-flow-006', title: 'HIPAA Security Rule Audit', deadline: '2026-06-15', category: 'Healthcare', status: 'In Progress', assignee: 'Compliance Analyst', notes: 'Annual audit of administrative, physical, and technical safeguards'},
    {id: 'cc-data-flow-007', title: 'SOC 2 Type II Report Renewal', deadline: '2026-09-30', category: 'Audit', status: 'Scheduled', assignee: 'External Auditor', notes: 'Engage auditor for SOC 2 Type II examination period'},
    {id: 'cc-data-flow-008', title: 'FedRAMP Authorization Review', deadline: '2026-07-01', category: 'Government', status: 'Pending', assignee: 'FedRAMP PMO', notes: 'Review and update security authorization package'},
    {id: 'cc-data-flow-009', title: 'Annual Penetration Test', deadline: '2026-10-15', category: 'Testing', status: 'Not Started', assignee: 'Red Team Lead', notes: 'Comprehensive external and internal penetration testing engagement'},
    {id: 'cc-data-flow-010', title: 'Security Awareness Training Rollout', deadline: '2026-05-01', category: 'Training', status: 'Completed', assignee: 'Security Awareness Manager', notes: 'Q2 organization-wide security awareness training program'},
    {id: 'cc-data-flow-011', title: 'Vendor Security Review Cycle', deadline: '2026-06-01', category: 'Third Party', status: 'In Progress', assignee: 'Vendor Manager', notes: 'Quarterly review of critical vendor security posture and SLA compliance'},
    {id: 'cc-data-flow-012', title: 'Incident Response Plan Update', deadline: '2026-05-20', category: 'Operations', status: 'Pending', assignee: 'IR Team Lead', notes: 'Update IR procedures based on lessons learned from recent incidents'},
  ];
  ...
  @state() private _dataQualityMetrics: Array<{id: string; dataSource: string; completenessScore: number; accuracyScore: number; timelinessScore: number; consistencyScore: number; reliabilityRank: number; lastUpdated: string; issues: string[]; trend: string}> = [
    {id: 'dq-data-flow-001', dataSource: 'Vulnerability Scanner Feed', completenessScore: 94, accuracyScore: 91, timelinessScore: 88, consistencyScore: 95, reliabilityRank: 1, lastUpdated: '2026-04-23T08:00:00Z', issues: ['Minor delay in Nessus plugin updates'], trend: 'improving'},
    {id: 'dq-data-flow-002', dataSource: 'SIEM Event Logs', completenessScore: 87, accuracyScore: 93, timelinessScore: 96, consistencyScore: 89, reliabilityRank: 2, lastUpdated: '2026-04-23T07:30:00Z', issues: ['Some log sources showing gaps in off-hours'], trend: 'stable'},
    {id: 'dq-data-flow-003', dataSource: 'Asset Inventory Database', completenessScore: 82, accuracyScore: 78, timelinessScore: 75, consistencyScore: 84, reliabilityRank: 4, lastUpdated: '2026-04-22T18:00:00Z', issues: ['Shadow IT devices not fully cataloged', 'Decommissioned assets still listed'], trend: 'declining'},
    {id: 'dq-data-flow-004', dataSource: 'Threat Intelligence Platform', completenessScore: 91, accuracyScore: 89, timelinessScore: 92, consistencyScore: 87, reliabilityRank: 3, lastUpdated: '2026-04-23T06:00:00Z', issues: ['Some IOC sources lack confidence scoring'], trend: 'improving'},
    {id: 'dq-data-flow-005', dataSource: 'Identity Provider Logs', completenessScore: 96, accuracyScore: 97, timelinessScore: 98, consistencyScore: 96, reliabilityRank: 1, lastUpdated: '2026-04-23T09:00:00Z', issues: [], trend: 'stable'},
    {id: 'dq-data-flow-006', dataSource: 'Cloud Security Posture Data', completenessScore: 79, accuracyScore: 85, timelinessScore: 82, consistencyScore: 81, reliabilityRank: 5, lastUpdated: '2026-04-23T05:00:00Z', issues: ['Multi-cloud coverage gaps', 'API rate limiting affects scan frequency'], trend: 'declining'},
    {id: 'dq-data-flow-007', dataSource: 'Compliance Evidence Store', completenessScore: 88, accuracyScore: 86, timelinessScore: 80, consistencyScore: 83, reliabilityRank: 4, lastUpdated: '2026-04-21T14:00:00Z', issues: ['Manual evidence collection delays', 'Inconsistent formatting across frameworks'], trend: 'stable'},
    {id: 'dq-data-flow-008', dataSource: 'Network Flow Data', completenessScore: 92, accuracyScore: 90, timelinessScore: 94, consistencyScore: 91, reliabilityRank: 2, lastUpdated: '2026-04-23T08:30:00Z', issues: ['Encrypted traffic classification accuracy needs improvement'], trend: 'improving'},
  ];
  ...
  @state() private _workflowTemplates: Array<{id: string; name: string; description: string; category: string; steps: number; avgDuration: string; successRate: number; lastRun: string; manualInterventions: number; optimizationScore: number}> = [
    {id: 'wf-data-flow-001', name: 'Vulnerability Triage Pipeline', description: 'Automated vulnerability assessment, prioritization, and assignment workflow', category: 'Vulnerability Management', steps: 8, avgDuration: '12 min', successRate: 94.5, lastRun: '2026-04-23T06:00:00Z', manualInterventions: 2, optimizationScore: 87},
    {id: 'wf-data-flow-002', name: 'Incident Response Orchestration', description: 'End-to-end IR workflow from detection to containment and recovery', category: 'Incident Response', steps: 15, avgDuration: '45 min', successRate: 89.2, lastRun: '2026-04-22T14:30:00Z', manualInterventions: 5, optimizationScore: 72},
    {id: 'wf-data-flow-003', name: 'Access Request Approval Chain', description: 'Multi-level approval workflow for privileged access requests', category: 'Identity & Access', steps: 6, avgDuration: '4 hours', successRate: 97.1, lastRun: '2026-04-23T09:15:00Z', manualInterventions: 1, optimizationScore: 92},
    {id: 'wf-data-flow-004', name: 'Compliance Evidence Collection', description: 'Automated gathering and packaging of audit evidence artifacts', category: 'Compliance', steps: 10, avgDuration: '30 min', successRate: 91.8, lastRun: '2026-04-21T10:00:00Z', manualInterventions: 3, optimizationScore: 78},
    {id: 'wf-data-flow-005', name: 'Security Patch Deployment', description: 'Staged patch rollout with validation and rollback capability', category: 'Patch Management', steps: 12, avgDuration: '2 hours', successRate: 96.3, lastRun: '2026-04-20T22:00:00Z', manualInterventions: 1, optimizationScore: 85},
    {id: 'wf-data-flow-006', name: 'Threat Intelligence Enrichment', description: 'IOC enrichment and correlation with internal threat data', category: 'Threat Intelligence', steps: 7, avgDuration: '8 min', successRate: 93.7, lastRun: '2026-04-23T07:00:00Z', manualInterventions: 0, optimizationScore: 95},
    {id: 'wf-data-flow-007', name: 'Security Reporting Pipeline', description: 'Automated generation and distribution of security metrics reports', category: 'Reporting', steps: 5, avgDuration: '15 min', successRate: 98.9, lastRun: '2026-04-23T08:00:00Z', manualInterventions: 0, optimizationScore: 97},
    {id: 'wf-data-flow-008', name: 'Vendor Risk Assessment', description: 'Automated vendor security questionnaire distribution and scoring', category: 'Third Party', steps: 9, avgDuration: '3 days', successRate: 85.4, lastRun: '2026-04-19T11:00:00Z', manualInterventions: 4, optimizationScore: 65},
  ];
  ...
  @state() private _workflowExecutionHistory: Array<{workflowId: string; runDate: string; duration: string; status: string; trigger: string}> = [
    {workflowId: 'wf-data-flow-001', runDate: '2026-04-23T06:00:00Z', duration: '11 min 23 sec', status: 'Success', trigger: 'Scheduled'},
    {workflowId: 'wf-data-flow-002', runDate: '2026-04-22T14:30:00Z', duration: '42 min 15 sec', status: 'Success', trigger: 'Alert'},
    {workflowId: 'wf-data-flow-003', runDate: '2026-04-23T09:15:00Z', duration: '3h 45 min', status: 'Success', trigger: 'User Request'},
    {workflowId: 'wf-data-flow-001', runDate: '2026-04-22T06:00:00Z', duration: '13 min 08 sec', status: 'Partial', trigger: 'Scheduled'},
    {workflowId: 'wf-data-flow-004', runDate: '2026-04-21T10:00:00Z', duration: '28 min 42 sec', status: 'Success', trigger: 'Scheduled'},
  ];
  ...
  @state() private _knowledgeArticles: Array<{id: string; title: string; category: string; author: string; publishDate: string; views: number; rating: number; tags: string[]; summary: string; readTime: string}> = [
    ...
    {id: 'kb-data-flow-002', title: 'Ransomware Defense Playbook 2026', category: 'Threats', author: 'Threat Intelligence Team', publishDate: '2026-04-01', views: 2341, rating: 4.9, tags: ['ransomware', 'incident-response', 'defense'], summary: 'Updated playbook with latest ransomware tactics, techniques, and defensive measures', readTime: '12 min'},
    {id: 'kb-data-flow-003', title: 'Cloud Security Best Practices for Multi-Cloud', category: 'Cloud', author: 'Cloud Security Team', publishDate: '2026-02-28', views: 1567, rating: 4.5, tags: ['cloud', 'aws', 'azure', 'gcp'], summary: 'Best practices for securing workloads across multiple cloud service providers', readTime: '18 min'},
    {id: 'kb-data-flow-004', title: 'API Security Testing Methodology', category: 'Testing', author: 'Red Team', publishDate: '2026-03-22', views: 987, rating: 4.6, tags: ['api', 'testing', 'owasp'], summary: 'Structured methodology for identifying and testing API security vulnerabilities', readTime: '10 min'},
    {id: 'kb-data-flow-005', title: 'Supply Chain Security Risk Management', category: 'Third Party', author: 'GRC Team', publishDate: '2026-01-20', views: 1234, rating: 4.3, tags: ['supply-chain', 'vendor', 'risk'], summary: 'Framework for managing security risks in the software supply chain', readTime: '14 min'},
    {id: 'kb-data-flow-006', title: 'Kubernetes Security Hardening Checklist', category: 'Container', author: 'Platform Security', publishDate: '2026-04-10', views: 2103, rating: 4.7, tags: ['kubernetes', 'container', 'hardening'], summary: 'Step-by-step hardening checklist for production Kubernetes clusters', readTime: '11 min'},
    {id: 'kb-data-flow-007', title: 'Security Metrics That Matter for Executives', category: 'Metrics', author: 'CISO Office', publishDate: '2026-03-05', views: 876, rating: 4.4, tags: ['metrics', 'kpi', 'reporting'], summary: 'Key security metrics and KPIs that resonate with board-level stakeholders', readTime: '8 min'},
    {id: 'kb-data-flow-008', title: 'Phishing Simulation Campaign Design', category: 'Awareness', author: 'Security Awareness Team', publishDate: '2026-02-15', views: 1456, rating: 4.5, tags: ['phishing', 'simulation', 'training'], summary: 'Designing effective phishing simulations that drive real behavioral change', readTime: '9 min'},
    ...
    {id: 'kb-data-flow-010', title: 'Security Automation with SOAR Platforms', category: 'Automation', author: 'SOC Engineering', publishDate: '2026-04-05', views: 1098, rating: 4.7, tags: ['soar', 'automation', 'soc'], summary: 'Building effective security automation playbooks using SOAR technology', readTime: '13 min'},
    {id: 'kb-data-flow-011', title: 'Third-Party Risk Assessment Framework', category: 'GRC', author: 'Vendor Management', publishDate: '2026-01-30', views: 1345, rating: 4.4, tags: ['vendor', 'risk', 'assessment'], summary: 'Structured framework for evaluating and monitoring third-party security risks', readTime: '12 min'},
    {id: 'kb-data-flow-012', title: 'Network Detection and Response Best Practices', category: 'Network', author: 'Network Security Team', publishDate: '2026-03-18', views: 1567, rating: 4.5, tags: ['ndr', 'network', 'detection'], summary: 'Implementing effective network detection and response capabilities', readTime: '14 min'},
    {id: 'kb-data-flow-013', title: 'Security Chaos Engineering Guide', category: 'Resilience', author: 'SRE Security', publishDate: '2026-04-12', views: 789, rating: 4.3, tags: ['chaos-engineering', 'resilience', 'testing'], summary: 'Applying chaos engineering principles to validate security controls', readTime: '10 min'},
    {id: 'kb-data-flow-014', title: 'Incident Communication Templates', category: 'Incident Response', author: 'IR Team', publishDate: '2026-02-22', views: 2234, rating: 4.8, tags: ['incident', 'communication', 'templates'], summary: 'Ready-to-use communication templates for security incident scenarios', readTime: '7 min'},
    {id: 'kb-data-flow-015', title: 'Endpoint Detection and Response Configuration', category: 'Endpoint', author: 'Endpoint Security', publishDate: '2026-03-08', views: 1345, rating: 4.5, tags: ['edr', 'endpoint', 'configuration'], summary: 'Optimal EDR configuration for maximum detection with minimal false positives', readTime: '11 min'},
    {id: 'kb-data-flow-016', title: 'Security Code Review Standards', category: 'Development', author: 'AppSec Team', publishDate: '2026-02-10', views: 1890, rating: 4.6, tags: ['code-review', 'secure-sdlc', 'standards'], summary: 'Standards and checklists for security-focused code reviews', readTime: '13 min'},
    {id: 'kb-data-flow-017', title: 'IoT Security Assessment Methodology', category: 'IoT', author: 'IoT Security Lab', publishDate: '2026-04-08', views: 654, rating: 4.2, tags: ['iot', 'embedded', 'assessment'], summary: 'Methodology for assessing security posture of IoT devices and ecosystems', readTime: '15 min'},
    {id: 'kb-data-flow-018', title: 'Passwordless Authentication Migration Guide', category: 'Identity', author: 'IAM Team', publishDate: '2026-03-25', views: 1678, rating: 4.7, tags: ['passwordless', 'authentication', 'mfa'], summary: 'Step-by-step migration plan from password-based to passwordless authentication', readTime: '12 min'},
    {id: 'kb-data-flow-019', title: 'Security Awareness Program Metrics', category: 'Awareness', author: 'Security Culture Team', publishDate: '2026-01-25', views: 1123, rating: 4.4, tags: ['awareness', 'metrics', 'culture'], summary: 'Measuring the effectiveness of your security awareness and training programs', readTime: '9 min'},
    {id: 'kb-data-flow-020', title: 'Deception Technology Deployment Guide', category: 'Detection', author: 'Blue Team', publishDate: '2026-04-15', views: 876, rating: 4.5, tags: ['deception', 'honeypot', 'detection'], summary: 'Planning and deploying deception technology for advanced threat detection', readTime: '11 min'},
  ];
  ...
  // --- Security Scenario Planning Methods (DAFL) ---
  private _initDaflScenarios(): void {
    const worstCaseScenarios = [
      { id: "dafl-sc-01", name: "Ransomware Attack on Critical Infrastructure", category: "Malware", impact: 9500000, probability: 0.12, recoveryDays: 21, affectedSystems: 47, dataLossGb: 320, businessImpact: "critical" },
      { id: "dafl-sc-02", name: "Insider Data Exfiltration via Authorized Channels", category: "Insider Threat", impact: 7200000, probability: 0.18, recoveryDays: 14, affectedSystems: 12, dataLossGb: 85, businessImpact: "high" },
      { id: "dafl-sc-03", name: "Supply Chain Compromise via Third-Party Library", category: "Supply Chain", impact: 6800000, probability: 0.09, recoveryDays: 30, affectedSystems: 63, dataLossGb: 0, businessImpact: "critical" },
      { id: "dafl-sc-04", name: "Cloud Misconfiguration Leading to Public Exposure", category: "Cloud", impact: 4100000, probability: 0.22, recoveryDays: 7, affectedSystems: 8, dataLossGb: 450, businessImpact: "high" },
      { id: "dafl-sc-05", name: "Zero-Day Exploit in Core Application Framework", category: "Vulnerability", impact: 8900000, probability: 0.05, recoveryDays: 45, affectedSystems: 120, dataLossGb: 0, businessImpact: "critical" },
      { id: "dafl-sc-06", name: "Distributed Denial of Service on Customer-Facing Services", category: "Availability", impact: 3400000, probability: 0.25, recoveryDays: 3, affectedSystems: 15, dataLossGb: 0, businessImpact: "medium" },
      { id: "dafl-sc-07", name: "Credential Stuffing and Account Takeover Wave", category: "Identity", impact: 2800000, probability: 0.30, recoveryDays: 5, affectedSystems: 6, dataLossGb: 12, businessImpact: "medium" },
      { id: "dafl-sc-08", name: "Physical Security Breach at Data Center Facility", category: "Physical", impact: 12000000, probability: 0.02, recoveryDays: 60, affectedSystems: 200, dataLossGb: 5000, businessImpact: "critical" },
    ];
    this._daflScenarios = worstCaseScenarios.map(s => ({ ...s, mitigationCost: Math.round(s.impact * 0.08), drillScheduled: false, lastDrillDate: null, drillScore: null, controlGaps: Math.floor(Math.random() * 5) + 1 }));
  }

  private _calculateDaflScenarioRisk(): void {
    this._daflScenarios.forEach(s => {
  ...
  private _scheduleDaflDrills(): void {
  ...
  private _daflScenarioComparison(): Record<string, unknown>[] {
    return this._daflScenarios.map(s => ({
  ...
  // --- Security Control Effectiveness Analytics (DAFL) ---
  private _initDaflControls(): void {
    const controls = [
      { id: "dafl-ct-01", name: "Network Segmentation", type: "preventive", maturity: "defined", score: 82, target: 90, failures: 3, lastTest: "2026-03-15" },
      { id: "dafl-ct-02", name: "Endpoint Detection and Response", type: "detective", maturity: "managed", score: 88, target: 92, failures: 1, lastTest: "2026-04-01" },
      { id: "dafl-ct-03", name: "Data Loss Prevention", type: "preventive", maturity: "repeatable", score: 71, target: 85, failures: 7, lastTest: "2026-03-22" },
      { id: "dafl-ct-04", name: "Security Information and Event Management", type: "detective", maturity: "managed", score: 85, target: 90, failures: 2, lastTest: "2026-04-05" },
      { id: "dafl-ct-05", name: "Identity and Access Management", type: "preventive", maturity: "defined", score: 76, target: 88, failures: 5, lastTest: "2026-03-28" },
      { id: "dafl-ct-06", name: "Vulnerability Management", type: "corrective", maturity: "managed", score: 80, target: 90, failures: 4, lastTest: "2026-04-02" },
      { id: "dafl-ct-07", name: "Email Security Gateway", type: "preventive", maturity: "managed", score: 90, target: 95, failures: 1, lastTest: "2026-04-08" },
      { id: "dafl-ct-08", name: "Web Application Firewall", type: "preventive", maturity: "managed", score: 87, target: 92, failures: 2, lastTest: "2026-03-30" },
      { id: "dafl-ct-09", name: "Patch Management", type: "corrective", maturity: "defined", score: 68, target: 85, failures: 9, lastTest: "2026-03-18" },
      { id: "dafl-ct-10", name: "Encryption at Rest", type: "preventive", maturity: "managed", score: 93, target: 95, failures: 0, lastTest: "2026-04-10" },
      { id: "dafl-ct-11", name: "Encryption in Transit", type: "preventive", maturity: "optimized", score: 96, target: 98, failures: 0, lastTest: "2026-04-12" },
      { id: "dafl-ct-12", name: "Privileged Access Management", type: "preventive", maturity: "defined", score: 74, target: 88, failures: 6, lastTest: "2026-03-25" },
      { id: "dafl-ct-13", name: "Security Awareness Training", type: "preventive", maturity: "repeatable", score: 65, target: 80, failures: 11, lastTest: "2026-03-20" },
      { id: "dafl-ct-14", name: "Incident Response Plan", type: "corrective", maturity: "managed", score: 78, target: 88, failures: 4, lastTest: "2026-04-03" },
      { id: "dafl-ct-15", name: "Backup and Recovery", type: "corrective", maturity: "managed", score: 84, target: 92, failures: 3, lastTest: "2026-04-06" },
      { id: "dafl-ct-16", name: "Multi-Factor Authentication", type: "preventive", maturity: "managed", score: 91, target: 95, failures: 1, lastTest: "2026-04-09" },
      { id: "dafl-ct-17", name: "Network Traffic Analysis", type: "detective", maturity: "defined", score: 72, target: 85, failures: 5, lastTest: "2026-03-27" },
      { id: "dafl-ct-18", name: "Cloud Security Posture Management", type: "detective", maturity: "repeatable", score: 69, target: 82, failures: 8, lastTest: "2026-03-24" },
      { id: "dafl-ct-19", name: "Container Security Scanning", type: "preventive", maturity: "defined", score: 75, target: 85, failures: 5, lastTest: "2026-04-04" },
      { id: "dafl-ct-20", name: "API Security Gateway", type: "preventive", maturity: "repeatable", score: 70, target: 82, failures: 7, lastTest: "2026-03-29" },
      { id: "dafl-ct-21", name: "Threat Intelligence Platform", type: "detective", maturity: "managed", score: 83, target: 90, failures: 2, lastTest: "2026-04-07" },
      { id: "dafl-ct-22", name: "Database Activity Monitoring", type: "detective", maturity: "defined", score: 67, target: 80, failures: 8, lastTest: "2026-03-21" },
      { id: "dafl-ct-23", name: "Deception Technology", type: "detective", maturity: "initial", score: 55, target: 75, failures: 12, lastTest: "2026-03-16" },
      { id: "dafl-ct-24", name: "Security Orchestration Automation", type: "corrective", maturity: "repeatable", score: 73, target: 85, failures: 6, lastTest: "2026-04-01" },
      { id: "dafl-ct-25", name: "Third-Party Risk Assessment", type: "preventive", maturity: "defined", score: 64, target: 78, failures: 9, lastTest: "2026-03-19" },
    ];
    this._daflControls = controls;
  }

  private _analyzeDaflControlEffectiveness(): void {
    const typeBreakdown: Record<string, { total: number; avgScore: number; avgFailures: number }> = {};
    this._daflControls.forEach(c => {
    ...
    this._daflControlTypeBreakdown = typeBreakdown;
  }

  private _daflControlOptimization(): Record<string, unknown>[] {
    return this._daflControls.filter(c => c.score < c.target).map(c => ({
  ...
  // --- Security Data Pipeline Health (DAFL) ---
  private _initDaflPipelines(): void {
    const pipelines = [
      { id: "dafl-pl-01", name: "SIEM Log Ingestion", status: "healthy", healthScore: 94, latencyMs: 120, freshnessMin: 2, errorRate: 0.02, throughputMbps: 450, recordsPerSec: 12000, backPressure: 0.05, uptime: 99.97 },
      { id: "dafl-pl-02", name: "Threat Feed Aggregation", status: "healthy", healthScore: 91, latencyMs: 300, freshnessMin: 15, errorRate: 0.05, throughputMbps: 85, recordsPerSec: 3200, backPressure: 0.08, uptime: 99.92 },
      { id: "dafl-pl-03", name: "Vulnerability Scan Results", status: "degraded", healthScore: 72, latencyMs: 2500, freshnessMin: 60, errorRate: 0.15, throughputMbps: 25, recordsPerSec: 800, backPressure: 0.35, uptime: 98.50 },
      { id: "dafl-pl-04", name: "Endpoint Telemetry Stream", status: "healthy", healthScore: 88, latencyMs: 450, freshnessMin: 5, errorRate: 0.08, throughputMbps: 280, recordsPerSec: 8500, backPressure: 0.12, uptime: 99.85 },
      { id: "dafl-pl-05", name: "Cloud Audit Log Pipeline", status: "healthy", healthScore: 86, latencyMs: 600, freshnessMin: 10, errorRate: 0.06, throughputMbps: 150, recordsPerSec: 4500, backPressure: 0.10, uptime: 99.80 },
      { id: "dafl-pl-06", name: "Identity Event Stream", status: "warning", healthScore: 78, latencyMs: 1200, freshnessMin: 20, errorRate: 0.12, throughputMbps: 45, recordsPerSec: 1500, backPressure: 0.22, uptime: 99.40 },
      { id: "dafl-pl-07", name: "Network Flow Collection", status: "healthy", healthScore: 82, latencyMs: 800, freshnessMin: 8, errorRate: 0.09, throughputMbps: 650, recordsPerSec: 18000, backPressure: 0.15, uptime: 99.70 },
      { id: "dafl-pl-08", name: "DLP Incident Pipeline", status: "degraded", healthScore: 68, latencyMs: 3500, freshnessMin: 45, errorRate: 0.20, throughputMbps: 18, recordsPerSec: 500, backPressure: 0.42, uptime: 97.80 },
    ];
    this._daflPipelines = pipelines.map(p => ({ ...p, dependencies: [], slaBreached: p.healthScore < 75, alertThreshold: p.errorRate > 0.15 }));
    this._daflPipelines[0].dependencies = ["dafl-pl-04", "dafl-pl-07"];
    this._daflPipelines[4].dependencies = ["dafl-pl-01"];
    this._daflPipelines[5].dependencies = ["dafl-pl-02"];
    this._daflPipelines[7].dependencies = ["dafl-pl-04", "dafl-pl-05"];
  }

  private _daflPipelineTrend(): Record<string, unknown>[] {
    const hours = Array.from({ length: 24 }, (_, i) => `H${String(i).padStart(2, "0")}`);
    return this._daflPipelines.map(p => ({
  ...
  // --- Security Stakeholder Report Generator (DAFL) ---
  private _initDaflReportTemplates(): void {
    this._daflReportTemplates = [
      { id: "dafl-rp-01", name: "Board Security Report", audience: "board", frequency: "quarterly", sections: 6, autoGenerate: true, lastGenerated: "2026-03-31", pages: 12 },
      { id: "dafl-rp-02", name: "Executive Risk Summary", audience: "executive", frequency: "monthly", sections: 8, autoGenerate: true, lastGenerated: "2026-04-01", pages: 8 },
      { id: "dafl-rp-03", name: "Technical Security Review", audience: "technical", frequency: "weekly", sections: 12, autoGenerate: true, lastGenerated: "2026-04-21", pages: 25 },
      { id: "dafl-rp-04", name: "Audit Compliance Report", audience: "audit", frequency: "quarterly", sections: 10, autoGenerate: false, lastGenerated: "2026-03-15", pages: 35 },
      { id: "dafl-rp-05", name: "Regulatory Filing Package", audience: "regulatory", frequency: "annual", sections: 15, autoGenerate: false, lastGenerated: "2025-12-31", pages: 48 },
    ];
  }

  private _generateDaflExecSummary(): string {
    const totalRisk = this._daflScenarios.reduce((sum: number, s: any) => sum + s.riskScore, 0);
    const avgControl = Math.round(this._daflControls.reduce((sum: number, c: any) => sum + c.score, 0) / this._daflControls.length);
    const degradedPipes = this._daflPipelines.filter((p: any) => p.status !== "healthy").length;
    return `Overall risk exposure: ${(totalRisk / 1000000).toFixed(1)}M. Average control effectiveness: {avgControl}%. {degradedPipes} pipeline(s) need attention. {this._daflScenarios.filter((s: any) => s.mitigationPriority === "immediate").length} scenarios require immediate action.`;
  }

  // --- Security Technology Radar (DAFL) ---
  private _initDaflTechRadar(): void {
    this._daflTechRadar = [
      { id: "dafl-tr-01", name: "AI-Powered Threat Detection", ring: "adopt", category: "Detection", maturity: 4, trend: "up", investment: "high", roi: 3.2, vendorCount: 8 },
      { id: "dafl-tr-02", name: "Zero Trust Architecture", ring: "adopt", category: "Architecture", maturity: 4, trend: "up", investment: "high", roi: 2.8, vendorCount: 12 },
      { id: "dafl-tr-03", name: "SASE/SSE Platform", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.4, vendorCount: 10 },
      { id: "dafl-tr-04", name: "CNAPP Cloud Security", ring: "trial", category: "Cloud", maturity: 3, trend: "up", investment: "medium", roi: 2.1, vendorCount: 7 },
      { id: "dafl-tr-05", name: "Extended Detection and Response", ring: "adopt", category: "Detection", maturity: 4, trend: "stable", investment: "high", roi: 2.9, vendorCount: 9 },
      { id: "dafl-tr-06", name: "Quantum-Safe Cryptography", ring: "assess", category: "Crypto", maturity: 2, trend: "up", investment: "low", roi: 0.8, vendorCount: 4 },
      { id: "dafl-tr-07", name: "Security Service Edge", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.0, vendorCount: 6 },
      { id: "dafl-tr-08", name: "SOAR 2.0 Autonomous SOC", ring: "assess", category: "Operations", maturity: 2, trend: "up", investment: "low", roi: 1.2, vendorCount: 5 },
      { id: "dafl-tr-09", name: "Software Supply Chain Security", ring: "trial", category: "DevSecOps", maturity: 3, trend: "up", investment: "medium", roi: 2.3, vendorCount: 8 },
      { id: "dafl-tr-10", name: "Identity Threat Detection", ring: "adopt", category: "Identity", maturity: 3, trend: "up", investment: "high", roi: 2.7, vendorCount: 7 },
      { id: "dafl-tr-11", name: "Data Security Posture Management", ring: "trial", category: "Data", maturity: 3, trend: "up", investment: "medium", roi: 2.2, vendorCount: 9 },
      { id: "dafl-tr-12", name: "Decentralized Identity Standards", ring: "hold", category: "Identity", maturity: 1, trend: "stable", investment: "low", roi: 0.5, vendorCount: 3 },
    ];
  }

  private _daflRadarSummary(): Record<string, unknown> {
    const rings: Record<string, number> = { adopt: 0, trial: 0, assess: 0, hold: 0 };
    this._daflTechRadar.forEach(t => { if (rings[t.ring as keyof typeof rings] !== undefined) rings[t.ring as keyof typeof rings]++; });
    return { adopt: rings.adopt, trial: rings.trial, assess: rings.assess, hold: rings.hold, total: this._daflTechRadar.length, avgMaturity: 2.8, topInvestment: this._daflTechRadar.filter(t => t.investment === "high").map(t => t.name) };
  }

  // --- DAFL Security Compliance Mapping Matrix ---
  private _initDaflComplianceMatrix(): void {
    ...
    this._daflComplianceMatrix = frameworks.map(fw => ({
  ...
  private _daflComplianceTrend(): Record<string, unknown>[] {
    const quarters = ["Q1-2025", "Q2-2025", "Q3-2025", "Q4-2025", "Q1-2026", "Q2-2026"];
    return this._daflComplianceMatrix.map(fw => ({
  ...
  // --- DAFL Threat Intelligence Correlation Engine ---
  private _initDaflThreatIntel(): void {
    ...
    this._daflThreatIntel = threatActors.map((actor, i) => ({
  ...
  private _daflThreatCorrelation(): Record<string, unknown>[] {
    return this._daflThreatIntel.filter(t => t.riskScore > 70).map(t => ({
  ...
  // --- DAFL Security Metrics Deep Dive ---
  private _initDaflMetricsDeep(): void {
    ...
    this._daflMetricsDeep = metricCategories.map(cat => ({
  ...
  private _daflMetricsHeatmap(): number[][] {
    return this._daflMetricsDeep.map(cat =>
      cat.metrics.map(m => Math.round((m.value / m.target) * 100))
    );
  }

  // --- DAFL Incident Cost Modeling ---
  private _initDaflCostModel(): void {
    const incidentTypes = ["Data Breach", "Ransomware", "DDoS", "Insider Threat", "Phishing", "Supply Chain", "Cloud Misconfig", "Zero-Day"];
    this._daflCostModel = incidentTypes.map(inc => ({
    ...
    this._daflCostModel.forEach(m => { m.totalAnnualExposure = m.avgCost * m.frequency; });
  }

  private _daflCostProjection(): Record<string, unknown>[] {
    return this._daflCostModel.map(m => ({
  ...
  // --- DAFL Security Architecture Pattern Library ---
  private _initDaflArchPatterns(): void {
    ...
    this._daflArchPatterns = patterns.map((p, i) => ({
  ...
  private _daflArchPatternAnalysis(): Record<string, unknown> {
    const implemented = this._daflArchPatterns.filter(p => p.implementation > 75).length;
    const avgCoverage = Math.round(this._daflArchPatterns.reduce((s: number, p: any) => s + p.coverage, 0) / this._daflArchPatterns.length);
    return {
      totalPatterns: this._daflArchPatterns.length, fullyImplemented: implemented, avgCoverage,
      avgRiskReduction: Math.round(this._daflArchPatterns.reduce((s: number, p: any) => s + p.riskReduction, 0) / this._daflArchPatterns.length),
      gaps: this._daflArchPatterns.filter(p => p.implementation < 60).map(p => p.pattern),
      recommendations: this._daflArchPatterns.filter(p => p.implementation < 70).map(p => ({ pattern: p.pattern, current: p.implementation, target: 85, effort: p.costComplexity }))
    };
  }
  ...
  private _6fcRiskAssessmentWorkflow = [
  ...
  private _6fcTreatmentOptions = [
    {id: '6fc-T-001', riskId: 'RSK-042', risk: 'Ransomware Attack on Critical Infrastructure', before: 20, after: 6, strategy: 'Mitigate', controls: ['Next-gen EDR with behavioral detection and automated response', 'Zero-trust network micro-segmentation isolating critical assets', 'Immutable offline backup rotation with quarterly recovery testing', 'Incident response retainer with 1-hour response SLA'], cost: '$450K/yr', roi: '78% reduction vs $2.8M potential loss', timeline: 'Q3 2026', status: 'approved' as const},
  ...
  private _6fcTalentPositions = [
    ...
    {id: '6fc-H-002', title: 'Cloud Security Engineer', dept: 'Cloud Security', level: 'Mid-Senior', loc: 'SF/Remote', salary: '$155K-$180K', posted: '2026-03-20', applicants: 45, pipeline: 12, interviews: 6, offers: 2, urgency: 'high' as const, manager: 'Mike Johnson', skills: ['AWS/Azure/GCP', 'Terraform', 'Kubernetes', 'IAM', 'CSPM'], nice: ['CCSP', 'CKS', 'Serverless']},
    {id: '6fc-H-003', title: 'SOC Analyst L2', dept: 'Security Operations', level: 'Mid', loc: 'Austin TX', salary: '$105K-$130K', posted: '2026-04-01', applicants: 67, pipeline: 18, interviews: 8, offers: 0, urgency: 'medium' as const, manager: 'Lisa Park', skills: ['SIEM', 'Incident triage', 'Malware analysis', 'TCP/IP'], nice: ['GCIA', 'SOAR', 'Kill chain']},
    {id: '6fc-H-004', title: 'Staff Security Architect', dept: 'Architecture', level: 'Staff', loc: 'Remote US', salary: '$200K-$240K', posted: '2026-02-28', applicants: 12, pipeline: 3, interviews: 2, offers: 0, urgency: 'critical' as const, manager: 'David Lee', skills: ['Enterprise arch', 'Zero trust', 'Risk frameworks', 'Board comms'], nice: ['CISSP+SABSA', 'Team building', 'Speaker']},
    {id: '6fc-H-005', title: 'GRC Analyst', dept: 'GRC', level: 'Mid', loc: 'NY/Remote', salary: '$110K-$135K', posted: '2026-04-05', applicants: 34, pipeline: 9, interviews: 3, offers: 1, urgency: 'medium' as const, manager: 'Rachel Green', skills: ['ISO 27001', 'SOC 2', 'Risk assessment', 'Audit'], nice: ['CRISC', 'CISA', 'GDPR']},
    {id: '6fc-H-006', title: 'Threat Intel Analyst', dept: 'Threat Intel', level: 'Mid-Senior', loc: 'Remote US', salary: '$140K-$170K', posted: '2026-03-25', applicants: 19, pipeline: 5, interviews: 2, offers: 0, urgency: 'low' as const, manager: 'James Wilson', skills: ['MITRE ATT&CK', 'OSINT', 'Threat modeling', 'Reverse eng'], nice: ['CTIA', 'Nation-state', 'Dark web']},
    {id: '6fc-H-007', title: 'AppSec Engineer', dept: 'AppSec', level: 'Senior', loc: 'Seattle/Remote', salary: '$160K-$190K', posted: '2026-04-10', applicants: 28, pipeline: 7, interviews: 3, offers: 0, urgency: 'medium' as const, manager: 'Emily Zhang', skills: ['SAST/DAST/SCA', 'Secure SDLC', 'Code review', 'CI/CD'], nice: ['CSSLP', 'Mobile security']},
    {id: '6fc-H-008', title: 'IAM Engineer', dept: 'IAM', level: 'Mid', loc: 'Remote US', salary: '$135K-$160K', posted: '2026-04-08', applicants: 31, pipeline: 10, interviews: 5, offers: 1, urgency: 'medium' as const, manager: 'Chris Martinez', skills: ['Okta/Azure AD', 'SAML/OIDC', 'PAM', 'RBAC'], nice: ['CISSP', 'Zero trust IAM']}
  ];

  private _6fcInterviewScorecard = [
  ...
  private _6fcOnboardingSteps = [
    {id: '6fc-OB-01', item: 'Provision security-hardened laptop, monitors, peripherals, and configured workstation image with all required security tools', cat: 'IT Setup', day: 'Day 1', owner: 'IT Operations'},
    {id: '6fc-OB-02', item: 'Create AD account, email, Slack, VPN, SSO enrollment, and all required application access accounts', cat: 'Access', day: 'Day 1', owner: 'IAM Team'},
    {id: '6fc-OB-03', item: 'Grant security tooling access: SIEM console, EDR dashboard, vuln scanner, pentest platforms, SOAR', cat: 'Tools', day: 'Day 1-2', owner: 'SOC Manager'},
    {id: '6fc-OB-04', item: 'Complete MFA enrollment, security awareness onboarding course, and acceptable use policy acknowledgment', cat: 'Security', day: 'Day 1', owner: 'Awareness Team'},
    {id: '6fc-OB-05', item: 'Sign NDA, employment agreement, IP assignment, and code of conduct documents with HR', cat: 'HR', day: 'Day 1', owner: 'Human Resources'},
    {id: '6fc-OB-06', item: 'Introduction meetings with direct team members and key cross-functional stakeholders', cat: 'Integration', day: 'Week 1', owner: 'Hiring Manager'},
    {id: '6fc-OB-07', item: 'Security architecture overview, tooling walkthrough, and operational runbook review sessions', cat: 'Training', day: 'Week 1-2', owner: 'Security Architect'},
    {id: '6fc-OB-08', item: 'Shadow experienced team member on active incident response and threat hunting investigations', cat: 'Mentoring', day: 'Week 2-3', owner: 'SOC Lead'},
    {id: '6fc-OB-09', item: 'Set up individual certification study plan aligned with role requirements and career goals', cat: 'Development', day: 'Week 2', owner: 'Hiring Manager'},
    {id: '6fc-OB-10', item: 'Establish 30-60-90 day goals, define success metrics, and schedule first performance checkpoint', cat: 'Goals', day: 'Week 1', owner: 'Hiring Manager'}
  ];

  private _6fcMetricsDashboard = [
    {id: '6fc-MTD-001', name: 'Mean Time to Detect (MTTD)', value: '4.2 hours', trend: 'improving' as const, target: '< 4 hours', source: 'SIEM - Splunk', method: 'Median time from first alert to analyst acknowledgment', owner: 'SOC Manager', sla: '4 hours', prev: '5.1 hours', spark: [5.8, 5.4, 5.1, 4.9, 4.7, 4.5, 4.3, 4.2], breakdown: [{src: 'Network IDS', pct: '35%', avg: '3.8h'}, {src: 'Endpoint EDR', pct: '28%', avg: '4.1h'}, {src: 'Email Gateway', pct: '22%', avg: '3.5h'}, {src: 'Cloud Security', pct: '10%', avg: '5.2h'}, {src: 'User Reports', pct: '5%', avg: '6.8h'}]},
    {id: '6fc-MTD-002', name: 'Mean Time to Respond (MTTR)', value: '2.8 hours', trend: 'stable' as const, target: '< 3 hours', source: 'SOAR - XSOAR', method: 'Median time from acknowledgment to first containment action', owner: 'IR Lead', sla: '3 hours', prev: '2.7 hours', spark: [3.2, 3.0, 2.9, 2.8, 2.8, 2.9, 2.8, 2.8], breakdown: [{src: 'Auto Playbooks', pct: '42%', avg: '1.2h'}, {src: 'Semi-Auto', pct: '33%', avg: '2.5h'}, {src: 'Manual', pct: '20%', avg: '5.1h'}, {src: 'L3 Escalation', pct: '5%', avg: '8.3h'}]},
    {id: '6fc-MTD-003', name: 'Patch Compliance Rate', value: '97.6%', trend: 'improving' as const, target: '> 95%', source: 'Tenable.io', method: 'Pct of critical/high vulns patched within SLA windows', owner: 'Vuln Manager', sla: '95%', prev: '96.1%', spark: [93.2, 94.1, 94.8, 95.3, 95.9, 96.4, 97.0, 97.6], breakdown: [{src: 'Critical (7d)', pct: '40%', rate: '99.2%'}, {src: 'High (30d)', pct: '35%', rate: '97.8%'}, {src: 'Medium (90d)', pct: '20%', rate: '95.1%'}, {src: 'Low (180d)', pct: '5%', rate: '89.4%'}]},
    {id: '6fc-MTD-004', name: 'Phishing Click Rate', value: '3.2%', trend: 'improving' as const, target: '< 5%', source: 'KnowBe4 + Proofpoint', method: 'Pct of users clicking simulated phishing links monthly', owner: 'Awareness Lead', sla: '5%', prev: '4.1%', spark: [6.8, 6.2, 5.5, 5.1, 4.6, 4.1, 3.6, 3.2], breakdown: [{src: 'Spear Phishing', pct: '35%', rate: '5.1%'}, {src: 'Generic Phishing', pct: '30%', rate: '2.8%'}, {src: 'BEC Sim', pct: '20%', rate: '3.5%'}, {src: 'SMS Phishing', pct: '15%', rate: '1.9%'}]},
    {id: '6fc-MTD-005', name: 'Security Posture Score', value: '784', trend: 'improving' as const, target: '> 750', source: 'SecurityScorecard', method: 'Composite: external 30%, compliance 25%, vuln 25%, config 20%', owner: 'CISO', sla: '> 750', prev: '771', spark: [748, 752, 758, 762, 768, 773, 778, 784], breakdown: [{src: 'External Rating', pct: '30%', score: '791'}, {src: 'Compliance', pct: '25%', score: '812'}, {src: 'Vuln Mgmt', pct: '25%', score: '745'}, {src: 'Config', pct: '20%', score: '769'}]},
    {id: '6fc-MTD-006', name: 'Vulnerability Backlog', value: '142', trend: 'improving' as const, target: '< 150', source: 'Tenable + Qualys', method: 'Total open vulns across all scanned assets tracked weekly', owner: 'Vuln Manager', sla: '< 150', prev: '168', spark: [210, 198, 185, 175, 165, 158, 150, 142], breakdown: [{src: 'Critical', pct: '8%', count: '11'}, {src: 'High', pct: '22%', count: '31'}, {src: 'Medium', pct: '45%', count: '64'}, {src: 'Low', pct: '25%', count: '36'}]}
  ];

  private _6fcArchReview = [
    {id: '6fc-AR-001', cat: 'Network', item: 'Network segmentation between trust zones with dedicated firewalls and granular ACLs', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Micro-segmentation via NSX across all production zones'},
    {id: '6fc-AR-002', cat: 'Network', item: 'DMZ architecture isolating public services from internal network', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Dual-firewall DMZ with WAF and DDoS mitigation'},
    {id: '6fc-AR-003', cat: 'Network', item: 'Least-privilege traffic flows with deny-by-default and quarterly review', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Explicit allow rules with automated drift detection'},
    {id: '6fc-AR-004', cat: 'Network', item: 'Encrypted external communications with IPSec/TLS 1.3 and PFS', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'IKEv2 with PFS, TLS 1.3 for all external APIs'},
    {id: '6fc-AR-005', cat: 'Identity', item: 'Centralized IdP with SSO and automated SCIM provisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Okta SSO SAML 2.0 for 247 apps with SCIM'},
    {id: '6fc-AR-006', cat: 'Identity', item: 'RBAC with quarterly reviews and automated deprovisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Quarterly recertification with JIT admin access'},
    {id: '6fc-AR-007', cat: 'Identity', item: 'PAM controlling all admin and service accounts with session recording', sev: 'high' as const, status: 'fail' as const, reviewer: 'IAM Lead', notes: '3 legacy accounts pending CyberArk enrollment'},
    {id: '6fc-AR-008', cat: 'Identity', item: 'MFA enforced for all users, hardware tokens for privileged access', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'FIDO2 for admins, push MFA for users, 99.8% enrollment'},
    {id: '6fc-AR-009', cat: 'Data', item: 'Data classification enforced with automated labeling on all repos', sev: 'high' as const, status: 'partial' as const, reviewer: 'Data Protection Lead', notes: 'Framework deployed, gaps on 4 legacy file shares'},
    {id: '6fc-AR-010', cat: 'Data', item: 'AES-256 encryption at rest for all sensitive data stores', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'Full-disk, TDE on DBs, SSE-KMS on cloud storage'},
    {id: '6fc-AR-011', cat: 'Data', item: 'DLP monitoring and preventing unauthorized data exfiltration', sev: 'high' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'DLP active on email, endpoint, cloud, and network'},
    {id: '6fc-AR-012', cat: 'Data', item: 'Encrypted off-site backups with quarterly restore testing', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Infrastructure Lead', notes: '3-2-1 strategy with immutable backups'},
    {id: '6fc-AR-013', cat: 'AppSec', item: 'Secure SDLC with SAST, DAST, SCA in all CI/CD pipelines', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: '34 pipelines with security gates and PR blocking'},
    {id: '6fc-AR-014', cat: 'AppSec', item: 'API gateway with auth, rate limiting, schema validation', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: 'Kong with OAuth2, rate limits, OpenAPI validation'},
    {id: '6fc-AR-015', cat: 'AppSec', item: 'Container security with image scanning and runtime protection', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Trivy scanning, Falco runtime in 80% of clusters'},
    {id: '6fc-AR-016', cat: 'Monitoring', item: 'SIEM collecting from all critical systems with correlation rules', sev: 'critical' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'Splunk 98% coverage with 1200+ correlation rules'},
    {id: '6fc-AR-017', cat: 'Monitoring', item: 'EDR with behavioral detection on all managed endpoints', sev: 'high' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'CrowdStrike on 99.2% endpoints with 24/7 MDR'},
    {id: '6fc-AR-018', cat: 'Monitoring', item: 'Executive dashboards with real-time posture visibility', sev: 'medium' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'PowerBI dashboards with weekly exec summaries'},
    {id: '6fc-AR-019', cat: 'Cloud', item: 'CSPM monitoring and remediating cloud misconfigurations', sev: 'high' as const, status: 'pass' as const, reviewer: 'Cloud Security Lead', notes: 'Prisma Cloud 450+ checks with auto-remediation'},
    {id: '6fc-AR-020', cat: 'Cloud', item: 'IaC templates scanned before deployment to production', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Checkov for Terraform, cfn-nag for CloudFormation pending'}
  ];

  private _get6fcRiskProgress(): number {
    return Math.round(this._6fcRiskAssessmentWorkflow.filter(s => s.status === 'completed').length / this._6fcRiskAssessmentWorkflow.length * 100);
  }

  private _get6fcTreatmentSummary(): {total: number; approved: number; avgReduction: number; investment: string} {
    const a = this._6fcTreatmentOptions.filter(r => r.status === 'approved').length;
    const avg = this._6fcTreatmentOptions.reduce((s, r) => s + ((r.before - r.after) / r.before * 100), 0) / this._6fcTreatmentOptions.length;
    return {total: this._6fcTreatmentOptions.length, approved: a, avgReduction: Math.round(avg), investment: '$1.125M annually'};
  }

  private _get6fcRuleSummary(): {total: number; passing: number; failing: number; avgRate: number; violations: number} {
    const p = this._6fcComplianceRules.filter(r => r.result === 'pass').length;
    const f = this._6fcComplianceRules.filter(r => r.result === 'fail').length;
    const avg = this._6fcComplianceRules.reduce((s, r) => s + r.passRate, 0) / this._6fcComplianceRules.length;
    const v = this._6fcComplianceRules.reduce((s, r) => s + r.violations, 0);
    return {total: this._6fcComplianceRules.length, passing: p, failing: f, avgRate: Math.round(avg * 10) / 10, violations: v};
  }

  private _get6fcPipelineStats(): {positions: number; applicants: number; interviewing: number; offers: number; critical: number} {
    const t = this._6fcTalentPositions.length;
    const a = this._6fcTalentPositions.reduce((s, p) => s + p.applicants, 0);
    const i = this._6fcTalentPositions.reduce((s, p) => s + p.interviews, 0);
    const o = this._6fcTalentPositions.reduce((s, p) => s + p.offers, 0);
    const c = this._6fcTalentPositions.filter(x => x.urgency === 'critical').length;
    return {positions: t, applicants: a, interviewing: i, offers: o, critical: c};
  }

  private _get6fcMetricTrends(): {improving: number; stable: number; total: number} {
    const imp = this._6fcMetricsDashboard.filter(m => m.trend === 'improving').length;
    const stb = this._6fcMetricsDashboard.filter(m => m.trend === 'stable').length;
    return {improving: imp, stable: stb, total: this._6fcMetricsDashboard.length};
  }

  private _get6fcArchScore(): {total: number; passed: number; failed: number; partial: number; score: number} {
    const p = this._6fcArchReview.filter(c => c.status === 'pass').length;
    const f = this._6fcArchReview.filter(c => c.status === 'fail').length;
    const pa = this._6fcArchReview.filter(c => c.status === 'partial').length;
    return {total: this._6fcArchReview.length, passed: p, failed: f, partial: pa, score: Math.round((p / this._6fcArchReview.length) * 100)};
  }


  private _z6fcIncidentScenarios = [
    {id: 'z6fc-IS-001', name: 'Ransomware Outbreak in Production Environment', category: 'Malware', likelihood: 'Medium', impact: 'Critical', riskScore: 15, playbooks: ['IR-Ransomware-Isolate', 'IR-Ransomware-Eradicate', 'IR-Ransomway-Recover'], estimatedMTTD: '15min', estimatedMTTR: '4h', involvedTeams: ['SOC', 'IR', 'Infrastructure', 'Legal', 'Comms'], keyIndicators: ['Mass file encryption events', 'Ransom note deployment', 'Lateral movement via SMB/RDP', 'Backup deletion attempts'], mitigationControls: ['EDR behavioral detection', 'Network segmentation', 'Immutable backups', 'Email filtering'], lessonsLearned: 'Ensure offline backups are tested quarterly and Ransomware-specific playbooks are updated with latest threat intelligence', lastDrillDate: '2026-03-15', nextDrillDate: '2026-06-15'},
    {id: 'z6fc-IS-002', name: 'Cloud Misconfiguration Leading to Data Exposure', category: 'Cloud', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Cloud-Misconfig-Assess', 'IR-Cloud-Misconfig-Remediate', 'IR-Cloud-Misconfig-Notify'], estimatedMTTD: '2h', estimatedMTTR: '6h', involvedTeams: ['Cloud Security', 'SOC', 'Data Protection', 'Legal'], keyIndicators: ['Public S3 bucket exposure', 'Overly permissive IAM policies', 'Unencrypted database snapshots', 'Public API endpoint discovery'], mitigationControls: ['CSPM monitoring', 'IAM policy analyzer', 'Automated remediation', 'Cloud audit logging'], lessonsLearned: 'Implement CSPM with auto-remediation for common misconfigurations and conduct monthly cloud configuration reviews', lastDrillDate: '2026-02-20', nextDrillDate: '2026-05-20'},
    {id: 'z6fc-IS-003', name: 'Insider Threat Data Exfiltration', category: 'Insider', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-Insider-Investigate', 'IR-Insider-Contain', 'IR-Insider-Escalate'], estimatedMTTD: '8h', estimatedMTTR: '24h', involvedTeams: ['SOC', 'IR', 'HR', 'Legal', 'IAM'], keyIndicators: ['Anomalous data access patterns', 'Large file downloads to USB or cloud', 'Off-hours access to sensitive systems', 'Accessing data outside normal job scope'], mitigationControls: ['UEBA analytics', 'DLP policies', 'PAM controls', 'Access reviews'], lessonsLearned: 'Strengthen UEBA baselines and implement automated alerts for data access pattern anomalies', lastDrillDate: '2026-01-10', nextDrillDate: '2026-04-10'},
    {id: 'z6fc-IS-004', name: 'Supply Chain Attack via Compromised Vendor Software', category: 'Supply Chain', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-SupplyChain-Detect', 'IR-SupplyChain-Isolate', 'IR-SupplyChain-Remediate'], estimatedMTTD: '24h', estimatedMTTR: '72h', involvedTeams: ['SOC', 'IR', 'Procurement', 'Vendor Mgmt', 'Legal'], keyIndicators: ['Unexpected network connections from vendor software', 'Modified binaries or dependencies', 'Anomalous behavior from trusted applications', 'SBOM discrepancy alerts'], mitigationControls: ['SCA scanning', 'SBOM monitoring', 'Vendor security assessments', 'Network segmentation'], lessonsLearned: 'Require SBOM from all vendors and implement continuous monitoring of third-party component behavior', lastDrillDate: '2026-03-01', nextDrillDate: '2026-06-01'},
    {id: 'z6fc-IS-005', name: 'Credential Stuffing Attack on Customer Portal', category: 'External Attack', likelihood: 'High', impact: 'Medium', riskScore: 12, playbooks: ['IR-CredentialStuffing-Detect', 'IR-CredentialStuffing-Block', 'IR-CredentialStuffing-Notify'], estimatedMTTD: '30min', estimatedMTTR: '2h', involvedTeams: ['SOC', 'AppSec', 'Customer Support', 'Comms'], keyIndicators: ['High volume of failed login attempts', 'Login attempts from known compromised credential lists', 'Unusual geographic distribution of login attempts', 'Account takeover indicators'], mitigationControls: ['WAF rate limiting', 'CAPTCHA on login', 'MFA enforcement', 'Credential monitoring service'], lessonsLearned: 'Deploy advanced bot detection on login endpoints and implement proactive credential monitoring for customer accounts', lastDrillDate: '2026-04-01', nextDrillDate: '2026-07-01'},
    {id: 'z6fc-IS-006', name: 'Phishing Campaign Targeting Executive Team', category: 'Social Engineering', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Phishing-Contain', 'IR-Phishing-Investigate', 'IR-Phishing-Notify'], estimatedMTTD: '1h', estimatedMTTR: '4h', involvedTeams: ['SOC', 'Awareness', 'Executive Office', 'IT'], keyIndicators: ['Sophisticated spear-phishing emails to C-suite', 'Business email compromise indicators', 'Redirected MFA approval requests', 'Unauthorized wire transfer attempts'], mitigationControls: ['AI email filtering', 'Executive awareness training', 'Out-of-band verification policy', 'DMARC/DKIM/SPF'], lessonsLearned: 'Implement additional email security controls for executive mailboxes and conduct monthly spear-phishing simulations', lastDrillDate: '2026-02-15', nextDrillDate: '2026-05-15'}
  ];

  private _z6fcSecurityFrameworks = [
  ...
  private _getz6fcScenarioSummary(): {total: number; critical: number; high: number; drilled: number; avgRiskScore: number} {
    const crit = this._z6fcIncidentScenarios.filter(s => s.impact === 'Critical').length;
    const high = this._z6fcIncidentScenarios.filter(s => s.impact === 'High').length;
    const avg = this._z6fcIncidentScenarios.reduce((s, x) => s + x.riskScore, 0) / this._z6fcIncidentScenarios.length;
    return {total: this._z6fcIncidentScenarios.length, critical: crit, high: high, drilled: this._z6fcIncidentScenarios.length, avgRiskScore: Math.round(avg)};
  }

  private _getz6fcVendorRiskSummary(): {total: number; approved: number; conditional: number; remediation: number; avgScore: number} {
    const approved = this._z6fcThirdPartyRiskData.filter(v => v.status === 'approved').length;
    const cond = this._z6fcThirdPartyRiskData.filter(v => v.status === 'conditional').length;
    const rem = this._z6fcThirdPartyRiskData.filter(v => v.status === 'remediation_required').length;
    const avg = this._z6fcThirdPartyRiskData.reduce((s, v) => s + v.riskScore, 0) / this._z6fcThirdPartyRiskData.length;
    return {total: this._z6fcThirdPartyRiskData.length, approved, conditional: cond, remediation: rem, avgScore: Math.round(avg)};
  }

  private _getz6fcTrainingSummary(): {total: number; avgCompletion: number; avgScore: number; overdueUpdates: number} {
    const avgComp = this._z6fcSecurityTrainingCatalog.reduce((s, t) => s + t.completionRate, 0) / this._z6fcSecurityTrainingCatalog.length;
    const avgScore = this._z6fcSecurityTrainingCatalog.filter(t => t.avgScore > 0).reduce((s, t) => s + t.avgScore, 0) / this._z6fcSecurityTrainingCatalog.filter(t => t.avgScore > 0).length;
    return {total: this._z6fcSecurityTrainingCatalog.length, avgCompletion: Math.round(avgComp * 10) / 10, avgScore: Math.round(avgScore), overdueUpdates: 0};
  }

  private _getz6fcToolHealthSummary(): {total: number; healthy: number; degraded: number; annualCost: string} {
    const healthy = this._z6fcSecurityToolInventory.filter(t => t.health === 'healthy').length;
    const degraded = this._z6fcSecurityToolInventory.filter(t => t.health === 'degraded').length;
    const cost = this._z6fcSecurityToolInventory.reduce((s, t) => s + parseInt(t.annualCost.replace(/[^0-9]/g, '')), 0);
    return {total: this._z6fcSecurityToolInventory.length, healthy, degraded, annualCost: '$' + (cost / 1000).toFixed(0) + 'K'};
  }


  private _x6fcSecurityZones = [
    {zoneId: 'x6fc-Z-001', name: 'Internet DMZ', trustLevel: 'Untrusted', subnet: '10.0.0.0/24', segmentation: 'Dual-firewall', firewallRules: 45, monitoring: 'Full packet capture', assets: ['Web servers', 'Load balancers', 'WAF appliances', 'Reverse proxy'], riskLevel: 'high' as const, lastAssessment: '2026-04-15', nextAssessment: '2026-07-15'},
    {zoneId: 'x6fc-Z-002', name: 'Corporate Network', trustLevel: 'Trusted', subnet: '10.1.0.0/16', segmentation: 'VLAN + micro-seg', firewallRules: 128, monitoring: 'NetFlow + IDS', assets: ['Workstations', 'Printers', 'Meeting room systems', 'VoIP phones'], riskLevel: 'medium' as const, lastAssessment: '2026-04-10', nextAssessment: '2026-07-10'},
    {zoneId: 'x6fc-Z-003', name: 'Production Data Center', trustLevel: 'Restricted', subnet: '10.2.0.0/16', segmentation: 'Full micro-segmentation', firewallRules: 256, monitoring: 'Full packet capture + EDR', assets: ['Application servers', 'Database clusters', 'Message queues', 'API gateways'], riskLevel: 'low' as const, lastAssessment: '2026-04-12', nextAssessment: '2026-07-12'},
    {zoneId: 'x6fc-Z-004', name: 'Management Network', trustLevel: 'Restricted', subnet: '10.3.0.0/24', segmentation: 'Isolated VLAN', firewallRules: 32, monitoring: 'Syslog + SNMP', assets: ['Hypervisors', 'Switch management', 'Storage controllers', 'Backup appliances'], riskLevel: 'low' as const, lastAssessment: '2026-04-08', nextAssessment: '2026-07-08'},
    {zoneId: 'x6fc-Z-005', name: 'Cloud Production', trustLevel: 'Restricted', subnet: 'VPC: 172.16.0.0/12', segmentation: 'Security groups + NACLs', firewallRules: 189, monitoring: 'VPC Flow Logs + GuardDuty', assets: ['Kubernetes clusters', 'Managed databases', 'Object storage', 'Serverless functions'], riskLevel: 'medium' as const, lastAssessment: '2026-04-18', nextAssessment: '2026-07-18'}
  ];

  private _x6fcSecurityAlerts = [
    {id: 'x6fc-AL-001', severity: 'critical', title: 'Active ransomware encryption detected on PROD-DB-01', source: 'CrowdStrike', timestamp: '2026-04-22T13:45:00Z', status: 'investigating', assignee: 'SOC Analyst J. Smith', asset: 'PROD-DB-01 (10.2.1.15)', mitreTechnique: 'T1486 - Data Encrypted for Impact', playbook: 'IR-Ransomware-Isolate', sla: '15 minutes', elapsed: '12 minutes', notes: 'EDR detected mass file encryption event pattern. Automated isolation triggered. IR team paged.'},
    {id: 'x6fc-AL-002', severity: 'high', title: 'Impossible travel detected for user admin@company.com', source: 'Okta', timestamp: '2026-04-22T12:30:00Z', status: 'triaged', assignee: 'SOC Analyst M. Johnson', asset: 'Okta - admin@company.com', mitreTechnique: 'T1078 - Valid Accounts', playbook: 'IR-ImpossibleTravel', sla: '30 minutes', elapsed: '87 minutes', notes: 'Login from Tokyo followed by login from New York within 30 minutes. Account temporarily locked. User confirmed via out-of-band channel they are in New York.'},
    {id: 'x6fc-AL-003', severity: 'high', title: 'Public S3 bucket discovered with customer PII exposure', source: 'Prisma Cloud', timestamp: '2026-04-22T11:15:00Z', status: 'remediating', assignee: 'Cloud Security Lead A. Chen', asset: 'AWS S3: customer-data-archive-2024', mitreTechnique: 'N/A - Misconfiguration', playbook: 'IR-Cloud-Misconfig-Remediate', sla: '1 hour', elapsed: '2.5 hours', notes: 'Bucket containing archived customer records was publicly accessible. Bucket blocked and encrypted. Data exposure assessment in progress.'},
    {id: 'x6fc-AL-004', severity: 'medium', title: 'Anomalous data download by user jane.developer@company.com', source: 'Microsoft DLP', timestamp: '2026-04-22T10:00:00Z', status: 'investigating', assignee: 'SOC Analyst R. Davis', asset: 'Endpoint: WS-DEV-042', mitreTechnique: 'T1567 - Exfil Over Web Service', playbook: 'IR-DLP-Investigation', sla: '4 hours', elapsed: '3.5 hours', notes: 'User downloaded 4.2GB of source code files to personal Google Drive. Interview scheduled with user and manager.'},
    {id: 'x6fc-AL-005', severity: 'medium', title: 'Credential stuffing attack detected on customer portal', source: 'Cloudflare WAF', timestamp: '2026-04-22T09:30:00Z', status: 'contained', assignee: 'SOC Analyst K. Wilson', asset: 'Customer Portal (portal.company.com)', mitreTechnique: 'T1110 - Brute Force', playbook: 'IR-CredentialStuffing-Block', sla: '2 hours', elapsed: '4 hours', notes: 'Rate limiting and CAPTCHA automatically deployed. 12,000 failed login attempts from 800 unique IPs blocked. No successful account takeovers confirmed.'},
    {id: 'x6fc-AL-006', severity: 'low', title: 'SSL certificate expiring in 14 days for api.company.com', source: 'Certificate monitoring', timestamp: '2026-04-22T08:00:00Z', status: 'remediated', assignee: 'Infrastructure Lead T. Brown', asset: 'api.company.com', mitreTechnique: 'N/A', playbook: 'Certificate Renewal', sla: '30 days', elapsed: '16 days', notes: 'Automated renewal triggered via ACME. New certificate deployed successfully. No service interruption.'}
  ];

  private _x6fcComplianceFrameworks = [
  ...
  private _x6fcDataClassificationPolicy = [
  ...
  private _getx6fcZoneSummary(): {total: number; highRisk: number; mediumRisk: number; lowRisk: number; totalRules: number} {
    const high = this._x6fcSecurityZones.filter(z => z.riskLevel === 'high').length;
    const med = this._x6fcSecurityZones.filter(z => z.riskLevel === 'medium').length;
    const low = this._x6fcSecurityZones.filter(z => z.riskLevel === 'low').length;
    const rules = this._x6fcSecurityZones.reduce((s, z) => s + z.firewallRules, 0);
    return {total: this._x6fcSecurityZones.length, highRisk: high, mediumRisk: med, lowRisk: low, totalRules: rules};
  }

  private _getx6fcAlertSummary(): {total: number; critical: number; high: number; medium: number; low: number; open: number; contained: number} {
    const crit = this._x6fcSecurityAlerts.filter(a => a.severity === 'critical').length;
    const high = this._x6fcSecurityAlerts.filter(a => a.severity === 'high').length;
    const med = this._x6fcSecurityAlerts.filter(a => a.severity === 'medium').length;
    const low = this._x6fcSecurityAlerts.filter(a => a.severity === 'low').length;
    const open = this._x6fcSecurityAlerts.filter(a => a.status === 'investigating' || a.status === 'triaged').length;
    const contained = this._x6fcSecurityAlerts.filter(a => a.status === 'contained' || a.status === 'remediated').length;
    return {total: this._x6fcSecurityAlerts.length, critical: crit, high, medium: med, low, open, contained};
  }

  private _getx6fcComplianceSummary(): {total: number; fullyCompliant: number; mostlyCompliant: number; avgComplianceRate: number} {
    const full = this._x6fcComplianceFrameworks.filter(f => f.status === 'compliant').length;
    const mostly = this._x6fcComplianceFrameworks.filter(f => f.status === 'mostly_compliant').length;
    const avgRate = this._x6fcComplianceFrameworks.reduce((s, f) => s + (f.compliant / f.requirements * 100), 0) / this._x6fcComplianceFrameworks.length;
    return {total: this._x6fcComplianceFrameworks.length, fullyCompliant: full, mostlyCompliant: mostly, avgComplianceRate: Math.round(avgRate * 10) / 10};
  }


  private _w6fcVulnerabilityTrends = [
  ...
  private _w6fcAccessReviewHistory = [
    {reviewId: 'w6fc-AR-001', period: 'Q1 2026', reviewer: 'IAM Team', scope: 'All privileged accounts (350)', completedDate: '2026-03-31', findings: 22, revoked: 18, modified: 4, newExceptions: 0, overdue: false, nextReview: 'Q2 2026', notes: '18 accounts revoked due to role changes, 4 access levels reduced. No new exceptions approved.'},
    {reviewId: 'w6fc-AR-002', period: 'Q4 2025', reviewer: 'IAM Team', scope: 'All privileged accounts (342)', completedDate: '2025-12-31', findings: 28, revoked: 22, modified: 6, newExceptions: 2, overdue: false, nextReview: 'Q1 2026', notes: '22 accounts revoked, 6 modified. 2 new exceptions approved for contractor admin access with 90-day expiry.'},
    {reviewId: 'w6fc-AR-003', period: 'Q3 2025', reviewer: 'IAM Team', scope: 'All privileged accounts (335)', completedDate: '2025-09-30', findings: 25, revoked: 20, modified: 5, newExceptions: 1, overdue: false, nextReview: 'Q4 2025', notes: 'Trend of decreasing findings continues. New exception for emergency break-glass account.'},
    {reviewId: 'w6fc-AR-004', period: 'Q2 2025', reviewer: 'IAM Team', scope: 'All privileged accounts (328)', completedDate: '2025-06-30', findings: 30, revoked: 25, modified: 5, newExceptions: 0, overdue: false, nextReview: 'Q3 2025', notes: '30 findings identified. Process improvement implemented to reduce orphaned account creation.'}
  ];

  private _w6fcSecurityBudgetAllocation = [
  ...
  private _getw6fcPatchSummary(): {total: number; critical: number; completed: number; inProgress: number; waiting: number; avgSLACompliance: number} {
    const crit = this._w6fcPatchDeploymentSchedule.filter(p => p.severity === 'Critical').length;
    const comp = this._w6fcPatchDeploymentSchedule.filter(p => p.status === 'completed').length;
    const inProg = this._w6fcPatchDeploymentSchedule.filter(p => p.status === 'in_progress').length;
    const wait = this._w6fcPatchDeploymentSchedule.filter(p => p.status === 'waiting').length;
    return {total: this._w6fcPatchDeploymentSchedule.length, critical: crit, completed: comp, inProgress: inProg, waiting: wait, avgSLACompliance: 85};
  }

  private _getw6fcBudgetSummary(): {totalBudget: string; totalActual: string; overallUtilization: number; categories: number} {
    return {totalBudget: '$7.3M', totalActual: '$6.31M', overallUtilization: 86.4, categories: this._w6fcSecurityBudgetAllocation.length};
  }
  ...
  // === Security Risk Register (data_flow) ===
  private _data_flowRiskRegister = [
    { id: "RSK-DAT-0001", title: "Data breach from unpatched systems", owner: "CTO", category: "Financial", status: "mitigating", trend: "stable", severity: "high", likelihood: 8, impact: 9, treatment: "Monitor and review quarterly", lastReview: "2026-04-21", nextReview: "2026-07-19" },
    { id: "RSK-DAT-0002", title: "Ransomware attack on critical infrastructure", owner: "CRO", category: "Strategic", status: "closed", trend: "decreasing", severity: "high", likelihood: 4, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-03", nextReview: "2026-07-26" },
    { id: "RSK-DAT-0003", title: "Insider threat data exfiltration", owner: "VP Eng", category: "Compliance", status: "accepted", trend: "increasing", severity: "medium", likelihood: 2, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-03", nextReview: "2026-07-19" },
    { id: "RSK-DAT-0004", title: "Supply chain compromise", owner: "Security Lead", category: "Technical", status: "open", trend: "stable", severity: "critical", likelihood: 7, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-03", nextReview: "2026-07-07" },
    { id: "RSK-DAT-0005", title: "Cloud misconfiguration exposure", owner: "Risk Mgr", category: "Reputational", status: "mitigating", trend: "decreasing", severity: "high", likelihood: 3, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-01", nextReview: "2026-07-03" },
    { id: "RSK-DAT-0006", title: "Phishing campaign success rate", owner: "Compliance Dir", category: "Legal", status: "closed", trend: "increasing", severity: "critical", likelihood: 6, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-02", nextReview: "2026-07-02" },
    { id: "RSK-DAT-0007", title: "Third-party vendor data breach", owner: "IT Dir", category: "Regulatory", status: "accepted", trend: "stable", severity: "low", likelihood: 9, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-07", nextReview: "2026-07-10" },
    { id: "RSK-DAT-0008", title: "Regulatory non-compliance penalty", owner: "DevOps Lead", category: "Third-Party", status: "open", trend: "decreasing", severity: "critical", likelihood: 2, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-13", nextReview: "2026-07-17" },
    { id: "RSK-DAT-0009", title: "Zero-day exploit in production", owner: "Architect", category: "Human Capital", status: "mitigating", trend: "increasing", severity: "medium", likelihood: 8, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-23", nextReview: "2026-07-01" },
    { id: "RSK-DAT-0010", title: "Insufficient access controls", owner: "CISO", category: "Operational", status: "closed", trend: "stable", severity: "medium", likelihood: 3, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-13", nextReview: "2026-07-12" },
    { id: "RSK-DAT-0011", title: "DDoS attack on services", owner: "CTO", category: "Financial", status: "accepted", trend: "decreasing", severity: "critical", likelihood: 4, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-03", nextReview: "2026-07-24" },
    { id: "RSK-DAT-0012", title: "Social engineering attack", owner: "CRO", category: "Strategic", status: "open", trend: "increasing", severity: "high", likelihood: 1, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-16", nextReview: "2026-07-24" },
    { id: "RSK-DAT-0013", title: "API security vulnerability", owner: "VP Eng", category: "Compliance", status: "mitigating", trend: "stable", severity: "medium", likelihood: 9, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-19", nextReview: "2026-07-28" },
    { id: "RSK-DAT-0014", title: "Mobile device compromise", owner: "Security Lead", category: "Technical", status: "closed", trend: "decreasing", severity: "low", likelihood: 1, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-23", nextReview: "2026-07-13" },
    { id: "RSK-DAT-0015", title: "Physical security breach", owner: "Risk Mgr", category: "Reputational", status: "accepted", trend: "increasing", severity: "medium", likelihood: 4, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-11", nextReview: "2026-07-05" },
    { id: "RSK-DAT-0016", title: "Password policy weakness", owner: "Compliance Dir", category: "Legal", status: "open", trend: "stable", severity: "critical", likelihood: 5, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-07", nextReview: "2026-07-03" },
    { id: "RSK-DAT-0017", title: "Network segmentation gap", owner: "IT Dir", category: "Regulatory", status: "mitigating", trend: "decreasing", severity: "medium", likelihood: 1, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-19", nextReview: "2026-07-13" },
    { id: "RSK-DAT-0018", title: "Encryption key management failure", owner: "DevOps Lead", category: "Third-Party", status: "closed", trend: "increasing", severity: "high", likelihood: 2, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-07", nextReview: "2026-07-14" },
    { id: "RSK-DAT-0019", title: "Audit trail tampering", owner: "Architect", category: "Human Capital", status: "accepted", trend: "stable", severity: "low", likelihood: 5, impact: 4, treatment: "Monitor and review quarterly", lastReview: "2026-04-07", nextReview: "2026-07-27" },
    { id: "RSK-DAT-0020", title: "Business email compromise", owner: "CISO", category: "Operational", status: "open", trend: "decreasing", severity: "critical", likelihood: 7, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-11", nextReview: "2026-07-03" },
  ];
  private _data_flowRiskFilter: string = 'all';
  private _data_flowRiskSeverity: string = 'all';
  private _data_flowRiskStatus: string = 'all';
  private _data_flowExpandedRisk: string = '';
  private _data_flowFilterRisks() {
    const reg = this._data_flowRiskRegister;
    return reg.filter(r => {
      if (this._data_flowRiskFilter !== "all" && r.category !== this._data_flowRiskFilter) return false;
      if (this._data_flowRiskSeverity !== "all" && r.severity !== this._data_flowRiskSeverity) return false;
      if (this._data_flowRiskStatus !== "all" && r.status !== this._data_flowRiskStatus) return false;
      return true;
    });
  }
  private _data_flowGetRiskScore(r: any) { return Math.round(r.likelihood * r.impact * 1.5); }
  private _data_flowGetRiskTrendIcon(trend: string) {
    if (trend === "increasing") return "\u2191";
    if (trend === "decreasing") return "\u2193";
    return "\u2192";
  }
  private _data_flowGetRiskColor(sev: string) {
    if (sev === "critical") return "#dc2626";
    if (sev === "high") return "#ea580c";
    if (sev === "medium") return "#d97706";
    return "#16a34a";
  }
  private _data_flowGetRiskCounts() {
    const reg = this._data_flowRiskRegister;
  ...
  private _data_flowGetTreatmentProgress() {
    const reg = this._data_flowRiskRegister;
    const treated = reg.filter(r => r.status === "mitigating" || r.status === "closed").length;
    return Math.round((treated / Math.max(reg.length, 1)) * 100);
  }

  // === Security Metrics API Gateway (data_flow) ===
  private _data_flowApiEndpoints = [
    { method: "GET", path: "/api/v1/threats", name: "Threat Intelligence Feed", status: "active", avgLatency: 82.9, reqPerMin: 1465.0, errorRate: 1.12, uptime: round(96.28091317659073,2), version: "v2.9.2" },
    { method: "POST", path: "/api/v1/scans/start", name: "Vulnerability Scanner", status: "active", avgLatency: 315.7, reqPerMin: 2380.0, errorRate: 1.19, uptime: round(97.72868907260859,2), version: "v3.2.13" },
    { method: "GET", path: "/api/v1/assets", name: "Asset Inventory", status: "active", avgLatency: 297.8, reqPerMin: 2125.0, errorRate: 2.32, uptime: round(99.61260817863125,2), version: "v1.0.19" },
    { method: "POST", path: "/api/v1/alerts", name: "Alert Management", status: "active", avgLatency: 198.5, reqPerMin: 2418.0, errorRate: 2.3, uptime: round(96.22336274217065,2), version: "v3.1.20" },
    { method: "GET", path: "/api/v1/compliance", name: "Compliance Status", status: "active", avgLatency: 225.6, reqPerMin: 16.0, errorRate: 1.2, uptime: round(95.47281154730649,2), version: "v1.1.17" },
    { method: "PUT", path: "/api/v1/policies", name: "Policy Engine", status: "active", avgLatency: 394.5, reqPerMin: 1061.0, errorRate: 1.69, uptime: round(98.77381392272078,2), version: "v1.8.1" },
    { method: "GET", path: "/api/v1/incidents", name: "Incident Tracker", status: "active", avgLatency: 393.5, reqPerMin: 537.0, errorRate: 2.06, uptime: round(99.81653205500379,2), version: "v2.1.7" },
    { method: "POST", path: "/api/v1/forensics", name: "Forensics Collector", status: "degraded", avgLatency: 52.9, reqPerMin: 2153.0, errorRate: 1.32, uptime: round(99.43848215047603,2), version: "v2.8.15" },
    { method: "GET", path: "/api/v1/risk/assess", name: "Risk Assessment", status: "active", avgLatency: 178.0, reqPerMin: 55.0, errorRate: 1.31, uptime: round(97.3218967502594,2), version: "v3.1.4" },
    { method: "POST", path: "/api/v1/auth/verify", name: "Authentication", status: "active", avgLatency: 332.9, reqPerMin: 2481.0, errorRate: 1.23, uptime: round(95.69902664537938,2), version: "v1.5.13" },
    { method: "GET", path: "/api/v1/logs/audit", name: "Audit Log Query", status: "active", avgLatency: 79.6, reqPerMin: 720.0, errorRate: 1.02, uptime: round(97.52237840214626,2), version: "v3.1.12" },
    { method: "PUT", path: "/api/v1/users/roles", name: "Role Management", status: "active", avgLatency: 248.3, reqPerMin: 2053.0, errorRate: 2.09, uptime: round(97.13900709339376,2), version: "v1.2.4" },
    { method: "POST", path: "/api/v1/encrypt", name: "Encryption Service", status: "active", avgLatency: 223.0, reqPerMin: 2290.0, errorRate: 1.48, uptime: round(97.37781689379555,2), version: "v1.2.17" },
    { method: "GET", path: "/api/v1/network/topo", name: "Network Topology", status: "maintenance", avgLatency: 316.6, reqPerMin: 260.0, errorRate: 1.56, uptime: round(97.24970829742324,2), version: "v1.8.9" },
    { method: "DELETE", path: "/api/v1/sessions", name: "Session Manager", status: "active", avgLatency: 272.3, reqPerMin: 875.0, errorRate: 0.77, uptime: round(96.1458012869261,2), version: "v2.9.7" },
  ];
  private _data_flowApiKeys = [
    { id: "ak-000001", name: "key-data-001", created: "2026-06-09", lastUsed: "2026-04-18", status: "active", calls: 14341, rateLimit: 500 },
    { id: "ak-000002", name: "key-data-002", created: "2026-01-09", lastUsed: "2026-04-15", status: "revoked", calls: 19065, rateLimit: 500 },
    { id: "ak-000003", name: "key-data-003", created: "2026-01-05", lastUsed: "2026-04-06", status: "active", calls: 922, rateLimit: 1000 },
    { id: "ak-000004", name: "key-data-004", created: "2026-06-23", lastUsed: "2026-04-13", status: "revoked", calls: 23246, rateLimit: 1000 },
    { id: "ak-000005", name: "key-data-005", created: "2026-03-21", lastUsed: "2026-04-15", status: "revoked", calls: 27890, rateLimit: 100 },
    { id: "ak-000006", name: "key-data-006", created: "2026-09-10", lastUsed: "2026-04-11", status: "active", calls: 28015, rateLimit: 1000 },
    { id: "ak-000007", name: "key-data-007", created: "2026-02-06", lastUsed: "2026-04-03", status: "revoked", calls: 37024, rateLimit: 5000 },
    { id: "ak-000008", name: "key-data-008", created: "2026-03-15", lastUsed: "2026-04-03", status: "active", calls: 31387, rateLimit: 100 },
  ];
  private _data_flowApiHealthSummary() {
    const eps = this._data_flowApiEndpoints;
  ...
  private _data_flowGetApiByMethod(method: string) { return this._data_flowApiEndpoints.filter(e=>e.method===method); }
  private _data_flowGetSlowEndpoints() { return this._data_flowApiEndpoints.filter(e=>e.avgLatency>200).sort((a,b)=>b.avgLatency-a.avgLatency); }
  private _data_flowGetHighErrorEndpoints() { return this._data_flowApiEndpoints.filter(e=>e.errorRate>1.0).sort((a,b)=>b.errorRate-a.errorRate); }

  // === Security Training Effectiveness (data_flow) ===
  private _data_flowTrainingModules = [
    { id: "TRN-001", name: "Security Fundamentals", completionRate: 90.6, avgScore: 80.7, behaviorChange: 63.7, enrolled: 443, completed: 275, duration: "108min", category: "mandatory" },
    { id: "TRN-002", name: "Phishing Awareness", completionRate: 85.7, avgScore: 67.2, behaviorChange: 43.9, enrolled: 109, completed: 398, duration: "100min", category: "mandatory" },
    { id: "TRN-003", name: "Social Engineering Defense", completionRate: 50.8, avgScore: 76.7, behaviorChange: 44.5, enrolled: 375, completed: 294, duration: "71min", category: "optional" },
    { id: "TRN-004", name: "Password Hygiene", completionRate: 46.9, avgScore: 57.2, behaviorChange: 77.4, enrolled: 212, completed: 205, duration: "117min", category: "mandatory" },
    { id: "TRN-005", name: "Data Classification", completionRate: 87.9, avgScore: 83.5, behaviorChange: 44.6, enrolled: 203, completed: 402, duration: "87min", category: "mandatory" },
    { id: "TRN-006", name: "Incident Response Basics", completionRate: 47.5, avgScore: 56.3, behaviorChange: 69.3, enrolled: 415, completed: 372, duration: "73min", category: "mandatory" },
    { id: "TRN-007", name: "Secure Coding", completionRate: 65.0, avgScore: 76.6, behaviorChange: 45.8, enrolled: 291, completed: 184, duration: "29min", category: "optional" },
    { id: "TRN-008", name: "Cloud Security", completionRate: 77.3, avgScore: 93.9, behaviorChange: 87.6, enrolled: 258, completed: 208, duration: "54min", category: "mandatory" },
    { id: "TRN-009", name: "Mobile Device Security", completionRate: 62.2, avgScore: 56.5, behaviorChange: 83.2, enrolled: 476, completed: 83, duration: "20min", category: "mandatory" },
    { id: "TRN-010", name: "Network Security", completionRate: 81.1, avgScore: 67.9, behaviorChange: 77.5, enrolled: 346, completed: 278, duration: "113min", category: "mandatory" },
    { id: "TRN-011", name: "Physical Security", completionRate: 78.8, avgScore: 62.2, behaviorChange: 70.5, enrolled: 351, completed: 200, duration: "113min", category: "optional" },
    { id: "TRN-012", name: "Regulatory Compliance", completionRate: 66.4, avgScore: 64.0, behaviorChange: 85.8, enrolled: 429, completed: 270, duration: "87min", category: "mandatory" },
    { id: "TRN-013", name: "Risk Management", completionRate: 59.3, avgScore: 82.3, behaviorChange: 61.3, enrolled: 65, completed: 323, duration: "96min", category: "mandatory" },
    { id: "TRN-014", name: "Cryptography Basics", completionRate: 67.9, avgScore: 85.8, behaviorChange: 33.0, enrolled: 147, completed: 284, duration: "97min", category: "mandatory" },
    { id: "TRN-015", name: "Access Control", completionRate: 73.0, avgScore: 89.2, behaviorChange: 42.5, enrolled: 486, completed: 426, duration: "108min", category: "optional" },
    { id: "TRN-016", name: "Vendor Management", completionRate: 87.4, avgScore: 55.9, behaviorChange: 72.5, enrolled: 429, completed: 56, duration: "83min", category: "mandatory" },
  ];
  private _data_flowPhishingResults = [
    { month: "2026-01", sent: 876, clicked: 10, reported: 366, clickRate: round(1.141552511415525,1), reportRate: round(41.78082191780822,1) },
    { month: "2026-02", sent: 985, clicked: 126, reported: 100, clickRate: round(12.791878172588833,1), reportRate: round(10.152284263959391,1) },
    { month: "2026-03", sent: 525, clicked: 55, reported: 303, clickRate: round(10.476190476190476,1), reportRate: round(57.714285714285715,1) },
    { month: "2026-04", sent: 295, clicked: 75, reported: 165, clickRate: round(25.423728813559322,1), reportRate: round(55.932203389830505,1) },
    { month: "2026-05", sent: 402, clicked: 103, reported: 88, clickRate: round(25.621890547263682,1), reportRate: round(21.890547263681594,1) },
    { month: "2026-06", sent: 524, clicked: 48, reported: 248, clickRate: round(9.16030534351145,1), reportRate: round(47.32824427480916,1) },
  ];
  private _data_flowTrainingROI = { totalInvestment: 125934, avgCostPerEmployee: 415, riskReductionPct: round(random.uniform(15,45),1), incidentReductionPct: round(random.uniform(10,35),1), complianceScoreGain: round(random.uniform(5,25),1) };
  private _data_flowLearningPaths = [
    { name: "Beginner Security Analyst", totalModules: 10, completedModules: 6, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 40, enrolled: 115 },
    { name: "Advanced Threat Hunter", totalModules: 17, completedModules: 14, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 21, enrolled: 102 },
    { name: "Security Architect", totalModules: 11, completedModules: 18, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 41, enrolled: 181 },
    { name: "Incident Responder", totalModules: 14, completedModules: 5, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 76, enrolled: 98 },
    { name: "Compliance Specialist", totalModules: 13, completedModules: 2, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 71, enrolled: 122 },
    { name: "DevSecOps Engineer", totalModules: 11, completedModules: 3, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 34, enrolled: 180 },
  ];
  private _data_flowGetOverallCompletion() {
    const mods = this._data_flowTrainingModules;
    return round(mods.reduce((s,m)=>s+m.completionRate,0)/mods.length,1);
  }
  private _data_flowGetTopPerformers() { return [...this._data_flowTrainingModules].sort((a,b)=>b.avgScore-a.avgScore).slice(0,5); }
  private _data_flowGetModulesNeedingAttention() { return this._data_flowTrainingModules.filter(m=>m.completionRate<70||m.avgScore<65); }

  // === Security Governance Framework (data_flow) ===
  private _data_flowGovBodies = [
    { name: "Security Steering Committee", members: 13, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-03", nextMeeting: "2026-05-21", quorum: 3 },
    { name: "Risk Management Board", members: 7, chair: "CTO", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-02", nextMeeting: "2026-05-06", quorum: 8 },
    { name: "Data Governance Council", members: 5, chair: "CRO", meetingFreq: "Monthly", lastMeeting: "2026-04-10", nextMeeting: "2026-05-17", quorum: 8 },
    { name: "Compliance Oversight Board", members: 9, chair: "CDO", meetingFreq: "Monthly", lastMeeting: "2026-04-01", nextMeeting: "2026-05-09", quorum: 6 },
    { name: "Architecture Review Board", members: 11, chair: "VP Eng", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-22", nextMeeting: "2026-05-09", quorum: 6 },
    { name: "Change Advisory Board", members: 8, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-22", nextMeeting: "2026-05-02", quorum: 7 },
    { name: "Incident Review Board", members: 5, chair: "CIO", meetingFreq: "As needed", lastMeeting: "2026-04-13", nextMeeting: "2026-05-25", quorum: 8 },
    { name: "Vendor Risk Committee", members: 5, chair: "CFO", meetingFreq: "Quarterly", lastMeeting: "2026-04-06", nextMeeting: "2026-05-21", quorum: 5 },
  ];
  private _data_flowDecisions = [
    { id: "DEC-0001", title: "Adopt zero-trust architecture framework", date: "2026-02-06", status: "approved", owner: "CISO" },
    { id: "DEC-0002", title: "Migrate to SIEM 2.0 platform", date: "2026-01-11", status: "implemented", owner: "CTO" },
    { id: "DEC-0003", title: "Implement DLP across all endpoints", date: "2026-03-15", status: "in-progress", owner: "Security Lead" },
    { id: "DEC-0004", title: "Mandate MFA for all external access", date: "2026-02-25", status: "pending", owner: "CRO" },
    { id: "DEC-0005", title: "Establish bug bounty program", date: "2026-02-24", status: "approved", owner: "VP Eng" },
    { id: "DEC-0006", title: "Deploy EDR solution enterprise-wide", date: "2026-02-11", status: "approved", owner: "CISO" },
    { id: "DEC-0007", title: "Conduct annual penetration testing", date: "2026-04-18", status: "implemented", owner: "CTO" },
    { id: "DEC-0008", title: "Implement network micro-segmentation", date: "2026-04-20", status: "in-progress", owner: "Security Lead" },
    { id: "DEC-0009", title: "Establish security champion program", date: "2026-03-12", status: "pending", owner: "CRO" },
    { id: "DEC-0010", title: "Migrate to passwordless authentication", date: "2026-03-16", status: "approved", owner: "VP Eng" },
  ];
  private _data_flowPolicyLifecycle = [
    { name: "Information Security Policy", version: "v1.4", status: "active", lastUpdated: "2026-01-19", nextReview: "2026-10-13", owner: "CISO" },
    { name: "Acceptable Use Policy", version: "v1.0", status: "active", lastUpdated: "2026-03-12", nextReview: "2026-9-01", owner: "Legal" },
    { name: "Data Retention Policy", version: "v1.4", status: "under-review", lastUpdated: "2026-03-15", nextReview: "2026-8-22", owner: "DPO" },
    { name: "Access Control Policy", version: "v4.3", status: "active", lastUpdated: "2026-04-20", nextReview: "2026-12-20", owner: "IAM Lead" },
    { name: "Incident Response Policy", version: "v1.2", status: "draft", lastUpdated: "2026-01-05", nextReview: "2026-7-07", owner: "IR Lead" },
    { name: "Business Continuity Plan", version: "v3.4", status: "active", lastUpdated: "2026-03-01", nextReview: "2026-12-26", owner: "BCP Mgr" },
    { name: "Vendor Management Policy", version: "v4.5", status: "active", lastUpdated: "2026-02-07", nextReview: "2026-7-02", owner: "Procurement" },
    { name: "Encryption Standard", version: "v2.3", status: "under-review", lastUpdated: "2026-02-19", nextReview: "2026-8-11", owner: "Security Arch" },
  ];
  private _data_flowGovMaturityScore = { overall: 2.2, riskManagement: 5.1, compliance: 5.7, incidentResponse: 3.9, awareness: 3.8, technology: 2.4 };
  private _data_flowGetPendingDecisions() { return this._data_flowDecisions.filter(d=>d.status==='pending'||d.status==='in-progress'); }
  private _data_flowGetActivePolicies() { return this._data_flowPolicyLifecycle.filter(p=>p.status==='active'); }
  private _data_flowGetEscalationPath() { return ["L1 Analyst","L2 Senior","Security Lead","CISO","Board"]; }

  // === Security Innovation Lab (data_flow) ===
  private _data_flowInnoProjects = [
    { id: "INN-001", name: "AI-Powered Threat Detection", description: "Machine learning models for real-time threat identification", status: "active", progress: 72, startDate: "2026-01-04", teamSize: 7, budget: 54591, milestones: 6, completedMilestones: 8 },
    { id: "INN-002", name: "Quantum-Resistant Cryptography", description: "Post-quantum encryption algorithm prototyping", status: "research", progress: 35, startDate: "2026-01-01", teamSize: 7, budget: 42253, milestones: 10, completedMilestones: 5 },
    { id: "INN-003", name: "Automated Red Teaming", description: "Autonomous penetration testing framework", status: "active", progress: 58, startDate: "2026-03-05", teamSize: 2, budget: 50762, milestones: 8, completedMilestones: 8 },
    { id: "INN-004", name: "Zero-Knowledge Authentication", description: "Privacy-preserving identity verification", status: "poc", progress: 88, startDate: "2026-02-23", teamSize: 3, budget: 113613, milestones: 10, completedMilestones: 9 },
    { id: "INN-005", name: "Blockchain Audit Trail", description: "Immutable security event logging", status: "active", progress: 45, startDate: "2026-02-23", teamSize: 4, budget: 184754, milestones: 6, completedMilestones: 8 },
    { id: "INN-006", name: "Behavioral Biometrics", description: "Continuous authentication via user behavior patterns", status: "research", progress: 22, startDate: "2026-01-23", teamSize: 2, budget: 119959, milestones: 3, completedMilestones: 4 },
    { id: "INN-007", name: "Deception Grid 2.0", description: "Advanced honeypot network with adaptive responses", status: "active", progress: 65, startDate: "2026-01-23", teamSize: 5, budget: 65731, milestones: 9, completedMilestones: 4 },
    { id: "INN-008", name: "Secure Enclave Integration", description: "Hardware-backed security for critical workloads", status: "poc", progress: 40, startDate: "2026-02-05", teamSize: 8, budget: 100466, milestones: 4, completedMilestones: 7 },
  ];
  private _data_flowTechEvaluations = [
    { name: "Rust for Security Tools", status: "evaluating", score: 6.0, recommendation: "Adopt", vendor: "Open Source" },
    { name: "eBPF for Runtime Detection", status: "completed", score: 8.3, recommendation: "Adopt", vendor: "AWS" },
    { name: "Confidential Computing", status: "planned", score: 9.4, recommendation: "Investigate", vendor: "Azure" },
    { name: "Homomorphic Encryption", status: "evaluating", score: 6.0, recommendation: "Pilot", vendor: "GCP" },
    { name: "SASE Architecture", status: "completed", score: 4.9, recommendation: "Adopt", vendor: "Multiple" },
    { name: "SOAR Platform 3.0", status: "planned", score: 5.3, recommendation: "Monitor", vendor: "Splunk" },
  ];
  private _data_flowCollaborationPartners = [
  ...
  private _data_flowInnoMetrics = { totalProjects: 8, activeProjects: 4, avgTimeToValue: "86 days", pocSuccessRate: round(random.uniform(55,85),1), researchToProduction: round(random.uniform(20,50),1), innovationIndex: round(random.uniform(6.0,9.5),1) };
  private _data_flowGetProjectByStatus(status: string) { return this._data_flowInnoProjects.filter(p=>p.status===status); }
  private _data_flowGetTopEvaluations() { return [...this._data_flowTechEvaluations].sort((a,b)=>b.score-a.score).slice(0,3); }



  // === Compliance Dashboard Extension (data_flow) ===
  private _data_flowComplianceFrameworks = [
    { name: "SOC 2 Type II", description: "Trust Services Criteria", totalControls: 5, implementedControls: 5, status: "compliant", lastAudit: "2026-02-16", nextAudit: "2026-9-27", evidenceCount: 72 },
    { name: "ISO 27001", description: "Information Security Management", totalControls: 114, implementedControls: 96, status: "partially-compliant", lastAudit: "2026-03-01", nextAudit: "2026-8-11", evidenceCount: 89 },
    { name: "PCI DSS 4.0", description: "Payment Card Industry", totalControls: 12, implementedControls: 10, status: "in-review", lastAudit: "2026-01-27", nextAudit: "2026-9-26", evidenceCount: 169 },
    { name: "HIPAA", description: "Health Insurance Portability", totalControls: 18, implementedControls: 16, status: "non-compliant", lastAudit: "2026-03-14", nextAudit: "2026-7-20", evidenceCount: 57 },
    { name: "GDPR", description: "Data Protection Regulation", totalControls: 99, implementedControls: 72, status: "partially-compliant", lastAudit: "2026-01-02", nextAudit: "2026-12-26", evidenceCount: 500 },
    { name: "NIST CSF 2.0", description: "Cybersecurity Framework", totalControls: 6, implementedControls: 6, status: "non-compliant", lastAudit: "2026-02-01", nextAudit: "2026-8-14", evidenceCount: 116 },
    { name: "FedRAMP", description: "Federal Risk Authorization", totalControls: 15, implementedControls: 7, status: "non-compliant", lastAudit: "2026-04-08", nextAudit: "2026-8-03", evidenceCount: 494 },
    { name: "SOX", description: "Sarbanes-Oxley Compliance", totalControls: 8, implementedControls: 7, status: "in-review", lastAudit: "2026-03-25", nextAudit: "2026-10-28", evidenceCount: 328 },
    { name: "CIS Controls v8", description: "Center for Internet Security", totalControls: 18, implementedControls: 17, status: "partially-compliant", lastAudit: "2026-02-08", nextAudit: "2026-9-22", evidenceCount: 473 },
    { name: "COBIT 2019", description: "IT Governance Framework", totalControls: 40, implementedControls: 28, status: "in-review", lastAudit: "2026-01-16", nextAudit: "2026-11-22", evidenceCount: 98 },
  ];
  private _data_flowGetComplianceScore() {
    const fw = this._data_flowComplianceFrameworks;
    return round(fw.reduce((s,f)=>s + (f.implementedControls/Math.max(f.totalControls,1))*100, 0) / fw.length, 1);
  }
  private _data_flowGetGaps() {
    return this._data_flowComplianceFrameworks.filter(f => f.status !== "compliant");
  }
  private _data_flowAuditTrail = [
    { id: "AUD-0001", action: "Control tested", auditor: "Internal Audit", date: "2026-04-19", result: "pass", findings: 5 },
    { id: "AUD-0002", action: "Evidence collected", auditor: "External Auditor", date: "2026-04-09", result: "pass", findings: 4 },
    { id: "AUD-0003", action: "Gap identified", auditor: "Security Team", date: "2026-04-05", result: "fail", findings: 1 },
    { id: "AUD-0004", action: "Remediation completed", auditor: "Compliance Officer", date: "2026-04-20", result: "pass", findings: 2 },
    { id: "AUD-0005", action: "Policy updated", auditor: "IT Audit", date: "2026-04-09", result: "pass", findings: 2 },
    { id: "AUD-0006", action: "Training verified", auditor: "Risk Team", date: "2026-04-08", result: "pass", findings: 4 },
    { id: "AUD-0007", action: "Access reviewed", auditor: "QA Team", date: "2026-04-14", result: "pass", findings: 5 },
    { id: "AUD-0008", action: "Exception approved", auditor: "CISO Office", date: "2026-04-01", result: "conditional", findings: 0 },
    { id: "AUD-0009", action: "Risk accepted", auditor: "Board Audit", date: "2026-04-17", result: "pass", findings: 3 },
    { id: "AUD-0010", action: "Control enhanced", auditor: "Third Party", date: "2026-04-06", result: "pass", findings: 4 },
  ];

  // === Threat Intelligence Feed Extension (data_flow) ===
  private _data_flowThreatActors = [
    { name: "APT-29", alias: "Cozy Bear", origin: "Russia", type: "Nation-State", severity: "high", lastActivity: "2026-04-01", targets: "Manufacturing", indicators: 366, ttps: 37 },
    { name: "APT-41", alias: "Double Dragon", origin: "China", type: "Nation-State", severity: "critical", lastActivity: "2026-04-12", targets: "Technology", indicators: 411, ttps: 35 },
    { name: "Lazarus Group", alias: "Hidden Cobra", origin: "North Korea", type: "Nation-State", severity: "critical", lastActivity: "2026-04-19", targets: "Healthcare", indicators: 348, ttps: 13 },
    { name: "FIN7", alias: "Carbanak", origin: "Eastern Europe", type: "Financial", severity: "high", lastActivity: "2026-04-23", targets: "Manufacturing", indicators: 222, ttps: 9 },
    { name: "Conti", alias: "Wizard Spider", origin: "Russia", type: "Ransomware", severity: "critical", lastActivity: "2026-04-09", targets: "Healthcare", indicators: 69, ttps: 42 },
    { name: "LockBit", alias: "LockBit Gang", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-10", targets: "Healthcare", indicators: 405, ttps: 23 },
    { name: "Cl0p", alias: "Cl0p Team", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-15", targets: "Technology", indicators: 468, ttps: 32 },
    { name: "Sandworm", alias: "Unit 74455", origin: "Russia", type: "Nation-State", severity: "critical", lastActivity: "2026-04-04", targets: "Government", indicators: 475, ttps: 12 },
  ];
  private _data_flowIoCFeed = [
    { id: "ioc-000001", type: "ip", value: "186.62.71.210", confidence: 48, source: "AlienVault", firstSeen: "2026-04-18", lastSeen: "2026-04-09" },
    { id: "ioc-000002", type: "ip", value: "206.141.101.73", confidence: 59, source: "MISP", firstSeen: "2026-04-14", lastSeen: "2026-04-13" },
    { id: "ioc-000003", type: "ip", value: "106.185.194.143", confidence: 85, source: "STIX", firstSeen: "2026-04-09", lastSeen: "2026-04-09" },
    { id: "ioc-000004", type: "ip", value: "88.207.20.5", confidence: 47, source: "AlienVault", firstSeen: "2026-04-21", lastSeen: "2026-04-17" },
    { id: "ioc-000005", type: "ip", value: "250.68.227.190", confidence: 64, source: "AlienVault", firstSeen: "2026-04-20", lastSeen: "2026-04-13" },
    { id: "ioc-000006", type: "ip", value: "146.160.235.248", confidence: 80, source: "AlienVault", firstSeen: "2026-04-08", lastSeen: "2026-04-15" },
    { id: "ioc-000007", type: "ip", value: "58.22.129.1", confidence: 91, source: "MISP", firstSeen: "2026-04-13", lastSeen: "2026-04-21" },
    { id: "ioc-000008", type: "ip", value: "133.173.77.216", confidence: 59, source: "STIX", firstSeen: "2026-04-11", lastSeen: "2026-04-03" },
    { id: "ioc-000009", type: "ip", value: "203.198.7.113", confidence: 66, source: "CrowdStrike", firstSeen: "2026-04-03", lastSeen: "2026-04-09" },
    { id: "ioc-000010", type: "ip", value: "3.158.155.9", confidence: 46, source: "MISP", firstSeen: "2026-04-17", lastSeen: "2026-04-09" },
    { id: "ioc-000011", type: "ip", value: "25.20.44.116", confidence: 63, source: "VirusTotal", firstSeen: "2026-04-11", lastSeen: "2026-04-22" },
    { id: "ioc-000012", type: "ip", value: "162.194.96.33", confidence: 74, source: "MISP", firstSeen: "2026-04-17", lastSeen: "2026-04-05" },
    { id: "ioc-000013", type: "ip", value: "175.66.108.168", confidence: 76, source: "Mandiant", firstSeen: "2026-04-16", lastSeen: "2026-04-01" },
    { id: "ioc-000014", type: "ip", value: "144.163.148.105", confidence: 84, source: "CrowdStrike", firstSeen: "2026-04-13", lastSeen: "2026-04-05" },
    { id: "ioc-000015", type: "ip", value: "57.219.100.54", confidence: 54, source: "VirusTotal", firstSeen: "2026-04-23", lastSeen: "2026-04-11" },
  ];
  private _data_flowGetActiveThreats() { return this._data_flowThreatActors.filter(a => a.severity === 'critical'); }
  private _data_flowGetThreatSummary() {
    const actors = this._data_flowThreatActors;
  ...
  // === Incident Management Extension (data_flow) ===
  private _data_flowIncidents = [
    { id: "INC-20260001", title: "Unauthorized access detected", severity: "critical", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-08T04:00", affectedAssets: 1, rootCause: "Misconfiguration" },
    { id: "INC-20260002", title: "Malware outbreak on workstation", severity: "high", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-14T00:09", affectedAssets: 15, rootCause: "Credential compromise" },
    { id: "INC-20260003", title: "Data leak from S3 bucket", severity: "medium", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-13T18:53", affectedAssets: 18, rootCause: "Zero-day" },
    { id: "INC-20260004", title: "Phishing campaign targeting finance", severity: "low", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-05T15:11", affectedAssets: 25, rootCause: "Human error" },
    { id: "INC-20260005", title: "DDoS attack on web services", severity: "critical", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-14T22:44", affectedAssets: 38, rootCause: "Policy violation" },
    { id: "INC-20260006", title: "Ransomware encryption attempt", severity: "high", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-06T06:55", affectedAssets: 43, rootCause: "Unknown" },
    { id: "INC-20260007", title: "Insider data exfiltration", severity: "medium", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-07T02:14", affectedAssets: 27, rootCause: "Misconfiguration" },
    { id: "INC-20260008", title: "API key exposure in repo", severity: "low", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-05T11:12", affectedAssets: 16, rootCause: "Credential compromise" },
    { id: "INC-20260009", title: "SQL injection on portal", severity: "critical", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-09T10:01", affectedAssets: 17, rootCause: "Zero-day" },
    { id: "INC-20260010", title: "Brute force on VPN", severity: "high", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-18T08:53", affectedAssets: 39, rootCause: "Human error" },
    { id: "INC-20260011", title: "Supply chain alert from vendor", severity: "medium", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-09T18:28", affectedAssets: 49, rootCause: "Policy violation" },
    { id: "INC-20260012", title: "Suspicious lateral movement", severity: "low", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-07T10:30", affectedAssets: 38, rootCause: "Unknown" },
  ];
  private _data_flowGetIncidentStats() {
    const inc = this._data_flowIncidents;
    return { total: inc.length, open: inc.filter(i=>i.status==="open").length, investigating: inc.filter(i=>i.status==="investigating").length, mttd: 41, mttr: 40 };
  }
  private _data_flowGetSeverityDistribution() {
    const inc = this._data_flowIncidents;
  ...
  render() {    if (this._dfRules.length === 0) { this._initDfRules(); this._initDfCvss(); this._runDfAnomalyDetection(); this._generateDfPredictions(); this._initDfApprovals(); this._initDfActivity(); this._initDfNotifications(); }
...
<div class="tabs">
        ...
        ${this._activeTab === 'rules' ? html`
          ...
          <div style="background:#1a1d27;border-radius:8px;padding:12px">${this._renderDfTreemapSVG()}</div>
          <div style="margin-top:12px;font-size:11px;font-weight:600;margin-bottom:6px">Data Flow (Sankey)</div>
          <div style="background:#1a1d27;border-radius:8px;padding:12px">${this._renderDfSankeySVG()}</div>
          ${this._renderDfActivity()}
        ` : nothing}
        ${this._activeTab === 'anomalies' ? html`
          ${this._renderDfAnomalies()}
          ${this._renderDfNotifications()}
        ` : nothing}
        ${this._activeTab === 'predictions' ? html`
          ${this._renderDfPredictions()}
          ...
          <div style="background:#1a1d27;border-radius:8px;padding:12px">
            <table class="history-table"><thead><tr><th>ID</th><th>Base</th><th>Temporal</th><th>Environmental</th><th>Overall</th></tr></thead><tbody>
              ${this._dfcvssData.map(c => html`<tr><td>${c.itemId}</td><td style="color:${c.base >= 7 ? '#ef4444' : c.base >= 4 ? '#eab308' : '#22c55e'}">${c.base}</td><td>${c.temporal}</td><td>${c.environmental}</td><td style="font-weight:700;color:${c.overall >= 7 ? '#ef4444' : c.overall >= 4 ? '#eab308' : '#22c55e'}">${c.overall}</td></tr>`)}
            </tbody></table>
          </div>
        ` : nothing}
        ${this._activeTab === 'approvals' ? html`
          ${this._renderDfApprovals()}
        ` : nothing}
        ${this._activeTab === 'config' ? html`
          ${this._renderDfConfig()}
        ` : nothing}
        ${this._activeTab === 'new' ? html`
          <div class="form-section">
            <div class="form-title">Create New Data Flow Entry</div>
            <div class="form-grid">
  ...
  @state() private _dfScenarios: {id:string;name:string;attackType:string;target:string;method:string;impactLow:number;impactHigh:number;confidence:number;mitigation:string;status:string}[] = [];
  @state() private _dfScenarioForm: {attackType:string;target:string;method:string} = {attackType:'',target:'',method:''};
  @state() private _dfScenarioCompare: boolean = false;
  @state() private _dfScenarioSelected: string[] = [];

  private _dfInitScenarios(): void {
    const saved = localStorage.getItem('df_scenarios');
    if (saved) { try { this._dfScenarios = JSON.parse(saved); } catch { /* ignore */ } }
    if (this._dfScenarios.length === 0) {
      this._dfScenarios = [
        {id:'df-s1',name:'Baseline Threat',attackType:'Phishing',target:'Employees',method:'Spear Email',impactLow:45,impactHigh:78,confidence:72,mitigation:'Security awareness training + email filtering',status:'active'},
        {id:'df-s2',name:'Escalated Attack',attackType:'Ransomware',target:'Endpoints',method:'Drive-by Download',impactLow:65,impactHigh:95,confidence:58,mitigation:'EDR deployment + network segmentation',status:'saved'},
        {id:'df-s3',name:'Insider Threat',attackType:'Data Exfiltration',target:'Databases',method:'SQL Injection',impactLow:55,impactHigh:88,confidence:65,mitigation:'DLP policies + query monitoring',status:'draft'},
      ];
    }
  }

  private _dfSaveScenarios(): void {
    localStorage.setItem('df_scenarios', JSON.stringify(this._dfScenarios));
  }

  private _dfAddScenario(): void {
    const f = this._dfScenarioForm;
    if (!f.attackType || !f.target) return;
    this._dfScenarios = [...this._dfScenarios, {
      id: 'df-s' + (this._dfScenarios.length + 1),
    ...
    this._dfScenarioForm = {attackType:'',target:'',method:''};
    this._dfSaveScenarios();
  }

  private _dfRenderScenarioEngine(): any {
    ...
    return html`
      <div style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          ...
          <button class="tab" @click=${() => { this._dfScenarioCompare = !this._dfScenarioCompare; }}>${this._dfScenarioCompare ? 'List View' : 'Compare'}</button>
        </div>
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:10px">
          <select style="background:#1a1d2e;border:1px solid #2a2d3a;border-radius:4px;padding:4px 6px;color:#e2e8f0;font-size:11px" @change=${(e: any) => { this._dfScenarioForm = {...this._dfScenarioForm, attackType: (e.target as HTMLSelectElement).value}; }}>
            <option value="">Attack Type</option>
            ${attackTypes.map(a => html`<option value=${a} ${this._dfScenarioForm.attackType === a ? 'selected' : ''}>${a}</option>`)}
          </select>
          <select style="background:#1a1d2e;border:1px solid #2a2d3a;border-radius:4px;padding:4px 6px;color:#e2e8f0;font-size:11px" @change=${(e: any) => { this._dfScenarioForm = {...this._dfScenarioForm, target: (e.target as HTMLSelectElement).value}; }}>
            <option value="">Target</option>
            ${targets.map(t => html`<option value=${t} ${this._dfScenarioForm.target === t ? 'selected' : ''}>${t}</option>`)}
          </select>
          <select style="background:#1a1d2e;border:1px solid #2a2d3a;border-radius:4px;padding:4px 6px;color:#e2e8f0;font-size:11px" @change=${(e: any) => { this._dfScenarioForm = {...this._dfScenarioForm, method: (e.target as HTMLSelectElement).value}; }}>
            <option value="">Method</option>
            ${methods.map(m => html`<option value=${m} ${this._dfScenarioForm.method === m ? 'selected' : ''}>${m}</option>`)}
          </select>
        </div>
        <button style="background:#3b82f6;border:none;border-radius:4px;padding:4px 12px;color:#fff;font-size:11px;cursor:pointer" @click=${this._dfAddScenario}>Run Simulation</button>
      </div>
      ${this._dfScenarioCompare && this._dfScenarios.length >= 2 ? html`
        <div style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
          <div style="font-weight:600;font-size:12px;color:#e2e8f0;margin-bottom:8px">Side-by-Side Comparison</div>
          <div style="display:grid;grid-template-columns:repeat(${Math.min(3, this._dfScenarios.length)},1fr);gap:8px">
            ${this._dfScenarios.slice(0,3).map(s => html`
      ...
      <div style="background:#0f1117;border-radius:8px;padding:12px">
        <div style="font-weight:600;font-size:12px;color:#e2e8f0;margin-bottom:8px">Saved Scenarios (${this._dfScenarios.length})</div>
        ${this._dfScenarios.map(s => html`
  ...
  @state() private _dfTrendData: {day:number;value:number;anomaly:boolean}[] = [];
  @state() private _dfTrendZoom: {start:number;end:number} | null = null;
  @state() private _dfTrendMA: number = 7;

  private _dfInitTrendData(): void {
    ...
    this._dfTrendData = data;
  }

  private _dfCalcMA(window: number): number[] {
    const result: number[] = [];
    for (let i = 0; i < this._dfTrendData.length; i++) {
      const start = Math.max(0, i - window + 1);
      const slice = this._dfTrendData.slice(start, i + 1);
      result.push(slice.reduce((s, d) => s + d.value, 0) / slice.length);
    }
    return result;
  }

  private _dfGetStats(): {mean:number;median:number;stddev:number;trend:string} {
    const vals = this._dfTrendData.map(d => d.value);
    const n = vals.length;
  ...
  private _dfRenderTimeSeries(): any {
    const stats = this._dfGetStats();
    const filtered = this._dfTrendZoom ? this._dfTrendData.filter(d => d.day >= this._dfTrendZoom.start && d.day <= this._dfTrendZoom.end) : this._dfTrendData;
    ...
    return html`
      <div style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          ...
          <div style="display:flex;gap:4px">
            <button class="tab ${this._dfTrendMA === 7 ? 'active' : ''}" @click=${() => { this._dfTrendMA = 7; }}>7D MA</button>
            <button class="tab ${this._dfTrendMA === 30 ? 'active' : ''}" @click=${() => { this._dfTrendMA = 30; }}>30D MA</button>
            <button class="tab" @click=${() => { this._dfTrendZoom = null; }}>Reset Zoom</button>
          </div>
        </div>
        <div style="position:relative;height:120px;background:#1a1d2e;border-radius:6px;overflow:hidden;margin-bottom:8px;cursor:crosshair" @click=${(e: any) => {
          ...
          this._dfTrendZoom = { start: Math.max(0, center - 10), end: Math.min(89, center + 10) };
        }}>
  ...
  @state() private _dfRoles: string[] = ['Admin','Analyst','Operator','Viewer','Auditor'];
  @state() private _dfActions: string[] = ['Create','Read','Update','Delete','Export','Approve'];
  @state() private _dfPermissions: { [role: string]: { [action: string]: boolean } } = {};
  @state() private _dfPermAudit: {role:string;action:string;changedBy:string;timestamp:string;oldVal:boolean;newVal:boolean}[] = [];
  @state() private _dfPermCompare: string[] = [];

  private _dfInitPermissions(): void {
    ...
    for (const role of this._dfRoles) {
      perms[role] = {};
      this._dfActions.forEach((a, i) => { perms[role][a] = defaults[role]?.[i] ?? false; });
    }
    this._dfPermissions = perms;
  }

  private _dfTogglePermission(role: string, action: string): void {
    const old = this._dfPermissions[role][action];
    this._dfPermissions = {...this._dfPermissions, [role]: {...this._dfPermissions[role], [action]: !old}};
    this._dfPermAudit.unshift({role,action,changedBy:'current_user',timestamp:new Date().toISOString(),oldVal:old,newVal:!old});
  }

  private _dfRenderRBAC(): any {
    const compareRoles = this._dfPermCompare.map(r => this._dfPermissions[r]).filter(Boolean);
    return html`
      <div style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          <span style="font-weight:700;font-size:13px;color:#e2e8f0">RBAC Permission Matrix</span>
          <div style="display:flex;gap:4px">
            ${this._dfRoles.map(r => html`
              <button class="tab ${this._dfPermCompare.includes(r) ? 'active' : ''}" @click=${() => {
                this._dfPermCompare = this._dfPermCompare.includes(r) ? this._dfPermCompare.filter(x => x !== r) : [...this._dfPermCompare, r];
              }}>${r}</button>
            `)}
          </div>
        </div>
          ...
          <table style="width:100%;border-collapse:collapse;font-size:10px">
            <thead>
              <tr>
                ...
                ${this._dfActions.map(a => html`<th style="padding:6px;color:#6b7280;border-bottom:1px solid #2a2d3a;text-align:center">${a}</th>`)}
              </tr>
            </thead>
            <tbody>
              ${this._dfRoles.map(role => html`
                <tr style="border-bottom:1px solid #1a1d2e">
                  <td style="padding:6px;color:#e2e8f0;font-weight:600">${role}</td>
                  ${this._dfActions.map(action => html`
                    <td style="text-align:center;padding:6px">
                      <button style="width:28px;height:20px;border-radius:3px;border:1px solid #2a2d3a;background:${this._dfPermissions[role][action] ? '#22c55e' : '#1a1d2e'};cursor:pointer;color:#fff;font-size:10px" @click=${() => this._dfTogglePermission(role, action)}>${this._dfPermissions[role][action] ? 'Y' : 'N'}</button>
                    </td>
                  `)}
                </tr>
              `)}
        ...
        ${compareRoles.length >= 2 ? html`
          <div style="margin-top:10px;padding:8px;background:#1a1d2e;border-radius:6px">
            <div style="font-size:11px;font-weight:600;color:#e2e8f0;margin-bottom:6px">Role Diff: ${this._dfPermCompare.join(' vs ')}</div>
            ${this._dfActions.map(action => {
              ...
              return allSame ? nothing : html`
                <div style="display:flex;gap:8px;padding:3px 0;font-size:10px">
                  ...
                  ${compareRoles.map((r, i) => html`<span style="color:${r[action] ? '#22c55e' : '#ef4444'}">${this._dfPermCompare[i]}=${r[action] ? 'Y' : 'N'}</span>`)}
                </div>
              `;
            })}
          </div>
        ` : nothing}
        ${this._dfPermAudit.length > 0 ? html`
          <div style="margin-top:8px;font-size:9px;color:#6b7280">Recent: ${this._dfPermAudit.slice(0,3).map(a => html`<span style="margin-right:8px">${a.role}.${a.action}: ${a.oldVal ? 'Y' : 'N'}->${a.newVal ? 'Y' : 'N'}</span>`)}
        ` : nothing}
      </div>
    `;
  }
  ...
  // === REPORTING SUITE ===
  @state() private _dfReportTemplate: string = 'executive';
  @state() private _dfReportSchedule: string = 'weekly';
  @state() private _dfReportDistList: string[] = ['security-team@company.com','ciso@company.com'];
  @state() private _dfReportHistory: {id:string;template:string;generatedAt:string;status:string}[] = [];

  private _dfGenerateReport(): void {
    const id = 'rpt-' + Date.now();
    this._dfReportHistory.unshift({id,template:this._dfReportTemplate,generatedAt:new Date().toISOString(),status:'sent'});
  }

  private _dfRenderReporting(): any {
    ...
    return html`
      <div style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">
          <div>
            ...
            ${templates.map(t => html`
              <div style="padding:6px;background:${this._dfReportTemplate === t.key ? '#1e3a5f' : '#1a1d2e'};border:1px solid ${this._dfReportTemplate === t.key ? '#3b82f6' : '#2a2d3a'};border-radius:4px;margin-bottom:4px;cursor:pointer" @click=${() => { this._dfReportTemplate = t.key; }}>
          ...
          <div>
            ...
            <div style="display:flex;gap:4px;margin-bottom:8px">
              ${schedules.map(s => html`<button class="tab ${this._dfReportSchedule === s.key ? 'active' : ''}" @click=${() => { this._dfReportSchedule = s.key; }}>${s.label}</button>`)}
            </div>
            <div style="font-size:10px;color:#6b7280;margin-bottom:4px">Distribution</div>
            <div style="font-size:10px;color:#9ca3af">${this._dfReportDistList.map(d => html`<div>${d}</div>`)}</div>
            <div style="margin-top:6px;display:flex;gap:4px">
              <button style="background:#3b82f6;border:none;border-radius:4px;padding:4px 10px;color:#fff;font-size:10px;cursor:pointer" @click=${this._dfGenerateReport}>Generate</button>
        ...
        ${this._dfReportHistory.length > 0 ? html`
          <div style="border-top:1px solid #2a2d3a;padding-top:8px">
            ...
            ${this._dfReportHistory.slice(0,3).map(r => html`
  ...
  @state() private _dfHighContrast: boolean = false;
  @state() private _dfA11yAnnounce: string = '';
  @state() private _dfShortcutsVisible: boolean = false;
  @state() private _dfFocusTrap: boolean = false;

  private _dfShortcuts: Record<string, string> = {
  ...
  private _dfHandleKeydown(e: KeyboardEvent): void {
    if (e.key === 'Escape' && this._dfFocusTrap) { this._dfFocusTrap = false; this._dfAnnounce('Dialog closed'); }
    if (e.ctrlKey && e.shiftKey && e.key === 'H') { e.preventDefault(); this._dfHighContrast = !this._dfHighContrast; this._dfAnnounce('High contrast ' + (this._dfHighContrast ? 'enabled' : 'disabled')); }
    if (e.ctrlKey && e.shiftKey && e.key === 'A') { e.preventDefault(); this._dfShortcutsVisible = !this._dfShortcutsVisible; }
  }

  private _dfAnnounce(msg: string): void {
    this._dfA11yAnnounce = msg;
    setTimeout(() => { this._dfA11yAnnounce = ''; }, 2000);
  }

  private _dfRenderAccessibility(): any {
    return html`
      <div role="region" aria-label="Accessibility Controls" style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          ...
          <button class="tab ${this._dfShortcutsVisible ? 'active' : ''}" @click=${() => { this._dfShortcutsVisible = !this._dfShortcutsVisible; }} aria-expanded=${this._dfShortcutsVisible}>Shortcuts</button>
        </div>
        <div style="display:flex;gap:8px;margin-bottom:8px">
          <label style="display:flex;align-items:center;gap:6px;font-size:11px;color:#e2e8f0;cursor:pointer">
            <input type="checkbox" .checked=${this._dfHighContrast} @change=${() => { this._dfHighContrast = !this._dfHighContrast; }} aria-label="Toggle high contrast mode">
            High Contrast
          </label>
        </div>
        ${this._dfShortcutsVisible ? html`
          <div role="list" aria-label="Keyboard shortcuts" style="background:#1a1d2e;border-radius:6px;padding:8px">
            ${Object.entries(this._dfShortcuts).map(([key, desc]) => html`
        ...
        ` : nothing}
        <div role="status" aria-live="polite" aria-atomic="true" style="position:absolute;left:-9999px">${this._dfA11yAnnounce}</div>
      </div>
    `;
  }
  ...
  connectedCallback(): void {
    ...
    this._dfInitScenarios();
    this._dfInitTrendData();
    this._dfInitPermissions();
    document.addEventListener('keydown', this._dfHandleKeydown.bind(this));
  }

  disconnectedCallback(): void {
    super.disconnectedCallback();
    document.removeEventListener('keydown', this._dfHandleKeydown.bind(this));
  }

  // === TAB INTEGRATION FOR EXTENDED FEATURES ===
  @state() private _dfActiveSubTab: string = 'scenario';
  ...
  private _dfGetAllSubTabs(): {key:string;label:string}[] {
    return [
  ...
  private _dfRenderSubPanel(): any {
    switch (this._dfActiveSubTab) {
      case 'scenario': return this._dfRenderScenarioEngine();
      case 'timeseries': return this._dfRenderTimeSeries();
      case 'rbac': return this._dfRenderRBAC();
      case 'reporting': return this._dfRenderReporting();
      case 'a11y': return this._dfRenderAccessibility();
      default: return nothing;
    }
  }

  private _dfRenderTabBar(): any {
    return html`
      <div style="display:flex;gap:4px;margin-bottom:12px;border-bottom:1px solid #2a2d3a;padding-bottom:8px;flex-wrap:wrap" role="tablist" aria-label="Extended panel features">
        ${this._dfGetAllSubTabs().map(t => html`
          <button class="tab ${this._dfActiveSubTab === t.key ? 'active' : ''}" @click=${() => { this._dfActiveSubTab = t.key; }} role="tab" aria-selected=${this._dfActiveSubTab === t.key}>${t.label}</button>
        `)}
      </div>
      <div role="tabpanel" aria-labelledby="df-tab-${this._dfActiveSubTab}">
        ${this._dfRenderSubPanel()}
      </div>
    `;
  }
  ...
  @state() private _soarPlaybooks: Array<{
    ...
    {
      id: 'pb-soar-001', name: 'Data Flow Auto Response', status: 'active',
      steps: [
    ...
    {
      id: 'pb-soar-002', name: 'Data Flow Investigation Workflow', status: 'active',
      steps: [
    ...
    {
      id: 'pb-soar-003', name: 'Data Flow Compliance Scan', status: 'draft',
      steps: [
  ...
  @state() private _dfAle: number = 0;
  @state() private _dfSroi: number = 0;
  @state() private _dfCpi: number = 0;
  @state() private _dfBudgetAlloc: number = 0;
  @state() private _dfCostBenefit: number = 0;

  // Security Economics Calculator
  private dfInitEconomics() {
    this._dfAle = Math.round(2850000 + Math.random() * 4500000);
    this._dfSroi = Math.round(180 + Math.random() * 320);
    this._dfCpi = Math.round(45000 + Math.random() * 120000);
    this._dfBudgetAlloc = Math.round(2500000 + Math.random() * 3000000);
    this._dfCostBenefit = Math.round(220 + Math.random() * 180);
  }

  private _dfCalcAle(): { annual: number; perIncident: number; byCategory: Array<{name: string; value: number}> } {
    const base = this._dfAle;
    const categories = [
  ...
  private _dfCalcSroi(): Array<{year: number; investment: number; savings: number; roi: number}> {
    const baseInv = this._dfSroi * 10000;
  ...
  private _dfGetBudgetAlloc(): Array<{category: string; amount: number; pct: number; trend: string}> {
    const total = this._dfBudgetAlloc;
    const items = [
  ...
  private _dfGetCostBenefit(): Array<{control: string; cost: number; benefit: number; ratio: number; priority: string}> {
    const base = this._dfCostBenefit;
    const controls = [
  ...
  private _dfRenderEconomics() {
    const ale = this._dfCalcAle();
    const roi = this._dfCalcSroi();
    const budget = this._dfGetBudgetAlloc();
    const cb = this._dfGetCostBenefit();
    const cpi = this._dfCpi;
  ...
  @state() private _dfThreatLevel: any = null;
  @state() private _dfEmergingThreats: any = null;
  @state() private _dfThreatTrends: any = null;
  @state() private _dfSectorRadar: any = null;
  @state() private _dfActorActivity: any = null;

  // Threat Landscape Intelligence
  private dfInitThreatIntel() {
    this._dfThreatLevel = {
  ...
  private _dfRenderThreatIntel() {
    const tl = this._dfThreatLevel;
    const et = this._dfEmergingThreats;
    const sr = this._dfSectorRadar;
    const aa = this._dfActorActivity;
  ...
  @state() private _dfPolicies: any = null;
  @state() private _dfExceptions: any = null;
  @state() private _dfRiskRegister: any = null;
  @state() private _dfMeetings: any = null;
  @state() private _dfDeadlines: any = null;

  // Security Governance Dashboard
  private dfInitGovernance() {
    this._dfPolicies = [
  ...
  private _dfRenderGovernance() {
    const policies = this._dfPolicies;
    const risks = this._dfRiskRegister;
    const deadlines = this._dfDeadlines;
  ...
  @state() private _dfCriticalAssets: any = null;
  @state() private _dfAssetDeps: any = null;
  @state() private _dfEolAssets: any = null;
  @state() private _dfAssetRisk: any = null;

  // Asset Intelligence
  private dfInitAssetIntel() {
    this._dfCriticalAssets = [
    ...
    this._dfAssetRisk = { critical: 7, high: 23, medium: 45, low: 128, total: 203 };
  }

  private _dfRenderAssetIntel() {
    const assets = this._dfCriticalAssets;
    const eol = this._dfEolAssets;
    const ar = this._dfAssetRisk;
  ...
  @state() private _dfUserBaseline: any = null;
  @state() private _dfAnomalyRules: any = null;
  @state() private _dfDataAccess: any = null;
  @state() private _dfInsiderRisk: any = null;
  @state() private _dataflr15Timeline: Array<{id:string;time:string;event:string;severity:string;actor:string}> = [
  ...
  @state() private _dataflr15RootCauses: Array<{why:string;answer:string}> = [
  ...
  @state() private _dataflr15ImpactMatrix: Array<{system:string;users:number;data:string;revenue:string;status:string}> = [
  ...
  @state() private _dataflr15Actions: Array<{id:string;item:string;owner:string;deadline:string;priority:string;status:string}> = [
  ...
  @state() private _dataflr15Lessons: Array<{id:string;lesson:string;category:string;severity:string;applies_to:string}> = [
  ...
  @state() private _dataflr15ActiveTab: string = 'timeline';
  @state() private _dataflr15Benchmarks: Array<{metric:string;current:number;industry:number;target:number;unit:string;source:string}> = [
  ...
  @state() private _dataflr15MaturityLevels: Array<{domain:string;current:number;target:number;description:string}> = [
  ...
  @state() private _dataflr15QuarterlyData: Array<{quarter:string;score:number;improvement:number}> = [
  ...
  @state() private _dataflr15SelectedDomain: string = 'all';
  @state() private _dataflr15Alerts: Array<{id:string;name:string;severity:number;confidence:number;assetCrit:number;score:number;enriched:boolean;group:string;status:string;enrichData:Array<{key:string;value:string}>}> = [
  ...
  @state() private _dataflr15QualityMetrics: {fpRate:number;enrichSuccess:number;avgTriageTime:number;enrichedCount:number;totalCount:number} = {fpRate:0.12, enrichSuccess:0.78, avgTriageTime:4.5, enrichedCount:3, totalCount:5};
  @state() private _dataflr15RoutingRules: Array<{name:string;condition:string;channel:string;active:boolean}> = [
  ...
  @state() private _dataflr15Shapes: Array<{id:string;type:string;label:string;controls:string[]}> = [
  ...
  @state() private _dataflr15TrustBoundaries: Array<{from:string;to:string;label:string;strength:string}> = [
  ...
  @state() private _dataflr15ADRs: Array<{id:string;title:string;status:string;date:string;decision:string}> = [
  ...
  @state() private _dataflr15SelectedShape: string = '';
  @state() private _dataflr15Gauges: Array<{name:string;value:number;max:number;unit:string;status:string;color:string}> = [
  ...
  @state() private _dataflr15Anomalies: Array<{id:string;time:string;description:string;severity:string;acknowledged:boolean}> = [
  ...
  @state() private _dataflr15Integrations: Array<{name:string;status:string;uptime:number;lastCheck:string;latency:number}> = [
  ...
  @state() private _dataflr15AlertFatigue: Array<{analyst:string;alertsPerDay:number;escalated:number;dismissed:number;avgResponseMin:number}> = [
  ...
  @state() private _dataflr15SlaTarget: number = 99.9;

  // Insider Threat Detection
  private dfInitInsiderThreat() {
    this._dfUserBaseline = [
    ...
    this._dfInsiderRisk = { totalUsers: 2847, monitored: 342, flagged: 18, investigated: 5, confirmed: 1 };
  }

  private _dfRenderInsiderThreat() {
    const baseline = this._dfUserBaseline;
    const rules = this._dfAnomalyRules;
    const ir = this._dfInsiderRisk;
    ...
    return html`<div class="card" style="padding:14px;margin-bottom:8px">
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:4px;margin-bottom:10px;flex-wrap:wrap">
            ${['timeline','rootcause','impact','actions','lessons'].map(t => html`
              <button class="btn ${this._dataflr15ActiveTab === t ? 'btn-primary' : ''}" style="font-size:10px;padding:3px 8px" @click=${() => { this._dataflr15ActiveTab = t; }}>${t.charAt(0).toUpperCase() + t.slice(1)}</button>
            `)}
          </div>
          ${this._dataflr15ActiveTab === 'timeline' ? html`
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._dataflr15Timeline.map(e => html`
          ...
          ` : this._dataflr15ActiveTab === 'rootcause' ? html`
            <div style="margin-bottom:8px">${{__html: this._dataflr15RenderFishbone()}}</div>
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._dataflr15RootCauses.map((rc, i) => html`
          ...
          ` : this._dataflr15ActiveTab === 'impact' ? html`
            <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:10px">
              ...
              <tbody>${this._dataflr15ImpactMatrix.map(imp => html`
          ...
          ` : this._dataflr15ActiveTab === 'actions' ? html`
            <div style="display:flex;gap:12px;margin-bottom:8px;font-size:10px;color:#888">
              ${Object.entries(this._dataflr15GetActionStats()).map(([k,v]) => html`<span>${k}: <b style="color:#ddd">${v}</b></span>`)}
            </div>
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._dataflr15Actions.map(a => html`
                <div style="display:flex;gap:8px;align-items:center;padding:6px;background:#1a1d27;border-radius:4px;cursor:pointer" @click=${() => this._dataflr15ToggleAction(a.id)}>
          ...
          ` : html`
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._dataflr15Lessons.map(l => html`
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:4px;margin-bottom:10px;flex-wrap:wrap">
            ${['all', ...this._dataflr15MaturityLevels.map(m => m.domain)].map(d => html`
              <button class="btn ${this._dataflr15SelectedDomain === d ? 'btn-primary' : ''}" style="font-size:9px;padding:2px 6px" @click=${() => { this._dataflr15SelectedDomain = d; }}>${d}</button>
            `)}
          </div>
          <div style="display:flex;gap:16px;margin-bottom:10px">
            <div style="text-align:center"><div style="color:#e0e0e0;font-size:20px;font-weight:bold">${this._dataflr15GetOverallMaturity()}/5</div><div style="color:#888;font-size:10px">Maturity Level</div></div>
            <div style="text-align:center"><div style="color:#4f4;font-size:20px;font-weight:bold">${this._dataflr15GetGapAnalysis().filter(g => g.isAbove).length}/${this._dataflr15Benchmarks.length}</div><div style="color:#888;font-size:10px">Above Industry</div></div>
          </div>
          <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:10px">
            ...
            <tbody>${this._dataflr15GetGapAnalysis().map(b => html`
          ...
          <div style="margin-top:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Maturity by Domain</div>
            ${this._dataflr15MaturityLevels.filter(m => this._dataflr15SelectedDomain === 'all' || m.domain === this._dataflr15SelectedDomain).map(m => html`
          ...
          <div style="margin-top:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Quarterly Trend</div>
            <div style="display:flex;height:40px;align-items:flex-end;gap:4px">
              ${this._dataflr15QuarterlyData.map(q => html`<div style="flex:1;text-align:center"><div style="background:#4a9eff;height:${q.score * 0.5}px;border-radius:2px 2px 0 0" title="${q.score}"></div><div style="color:#666;font-size:8px;margin-top:2px">${q.quarter}</div></div>`)}
            </div>
          </div>
        </div>
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:12px;margin-bottom:8px;font-size:10px">
            <div style="display:flex;gap:4px;flex-wrap:wrap">${this._dataflr15GroupAlerts().map(g => html`<span style="color:#888;padding:2px 6px;background:#1a1d27;border-radius:3px">${g.group}: ${g.count}</span>`)}</div>
          </div>
          <div style="display:flex;gap:8px;margin-bottom:8px;font-size:10px;color:#888">
            <span>FP Rate: <b style="color:${this._dataflr15QualityMetrics.fpRate > 0.15 ? '#f44' : '#4f4'}">${(this._dataflr15QualityMetrics.fpRate * 100).toFixed(1)}%</b></span>
            <span>Enrich: <b style="color:#4a9eff">${(this._dataflr15QualityMetrics.enrichSuccess * 100).toFixed(0)}%</b></span>
            <span>Avg Triage: <b style="color:#ddd">${this._dataflr15QualityMetrics.avgTriageTime}m</b></span>
          </div>
          <div style="display:flex;flex-direction:column;gap:4px;max-height:200px;overflow-y:auto">
            ${this._dataflr15Alerts.sort((a, b) => this._dataflr15CalcScore(b) - this._dataflr15CalcScore(a)).map(a => {
              const score = this._dataflr15CalcScore(a);
              return html`
                <div style="padding:6px;background:#1a1d27;border-radius:4px;border-left:3px solid ${a.severity >= 4 ? '#f44' : a.severity >= 3 ? '#fa0' : '#4a9eff'}">
                  <div style="display:flex;justify-content:space-between;align-items:center">
                    ...
                    <button class="btn" style="font-size:9px;padding:1px 6px;margin-left:4px" @click=${() => this._dataflr15EnrichAlert(a.id)}>${a.enriched ? 'Re-enrich' : 'Enrich'}</button>
                  </div>
          ...
          <div style="margin-top:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Routing Rules</div>
            ${this._dataflr15RoutingRules.map(r => html`
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap">
            ${this._dataflr15Shapes.map(s => {
              const v = this._dataflr15ValidateControl(s.id);
              return html`<div style="padding:4px 8px;background:#1a1d27;border-radius:3px;cursor:pointer;border:1px solid ${this._dataflr15SelectedShape === s.id ? '#4a9eff' : '#2a2d37'}" @click=${() => { this._dataflr15SelectedShape = this._dataflr15SelectedShape === s.id ? '' : s.id; }}>
          ...
          ${this._dataflr15SelectedShape ? html`
            ${(() => {
              const shape = this._dataflr15Shapes.find(s => s.id === this._dataflr15SelectedShape);
              const v = this._dataflr15ValidateControl(this._dataflr15SelectedShape);
              return shape ? html`
          ...
          <div style="margin-bottom:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Trust Boundaries</div>
            ${this._dataflr15TrustBoundaries.map(tb => html`
              <div style="display:flex;gap:6px;align-items:center;padding:3px;background:#1a1d27;border-radius:3px;margin-bottom:2px;font-size:9px">
                <span style="color:#ddd">${this._dataflr15Shapes.find(s => s.id === tb.from)?.label || tb.from}</span>
                <span style="color:#4a9eff">\u2194</span>
                <span style="color:#ddd">${this._dataflr15Shapes.find(s => s.id === tb.to)?.label || tb.to}</span>
          ...
          <div><div style="color:#888;font-size:10px;margin-bottom:4px">Architecture Decision Records</div>
            ${this._dataflr15ADRs.map(adr => html`
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap">
            ${this._dataflr15Gauges.map(g => html`
          ...
          <div style="display:flex;gap:8px;margin-bottom:8px;align-items:center">
            ...
            <span style="color:${this._dataflr15GetOverallHealth().score >= 99 ? '#4f4' : '#fa0'};font-size:12px;font-weight:bold">${this._dataflr15GetOverallHealth().score}%</span>
            <span style="color:#888;font-size:10px">(Target: ${this._dataflr15SlaTarget}%)</span>
            <span style="color:#4f4;font-size:10px">${this._dataflr15GetOverallHealth().healthy} healthy</span>
            <span style="color:#fa0;font-size:10px">${this._dataflr15GetOverallHealth().degraded} degraded</span>
          </div>
          <div style="margin-bottom:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Anomaly Stream</div>
            ${this._dataflr15Anomalies.slice(0, 3).map(a => html`
              <div style="display:flex;gap:6px;align-items:center;padding:4px;background:#1a1d27;border-radius:3px;margin-bottom:2px;opacity:${a.acknowledged ? 0.5 : 1}">
                ...
                ${!a.acknowledged ? html`<button class="btn" style="font-size:8px;padding:1px 4px" @click=${() => this._dataflr15AckAnomaly(a.id)}>ACK</button>` : html`<span style="color:#4f4;font-size:9px">ACK'd</span>`}
              </div>
            `)}
          </div>
          <div style="margin-bottom:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Integration Health</div>
            ${this._dataflr15Integrations.map(i => html`
              <div style="display:flex;gap:6px;align-items:center;padding:3px;background:#1a1d27;border-radius:3px;margin-bottom:2px;font-size:9px">
          ...
          <div><div style="color:#888;font-size:10px;margin-bottom:4px">Alert Fatigue Analysis</div>
            <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:9px">
              ...
              <tbody>${this._dataflr15AlertFatigue.map(af => html`
  ...
  private _dataflr15RenderFishbone(): string {
  ...
  private _dataflr15ToggleAction(id: string) {
    this._dataflr15Actions = this._dataflr15Actions.map(a => a.id === id ? {...a, status: a.status === 'pending' ? 'in_progress' : a.status === 'in_progress' ? 'completed' : 'pending'} : a);
  }

  private _dataflr15GetActionStats() {
    const total = this._dataflr15Actions.length;
    const done = this._dataflr15Actions.filter(a => a.status === 'completed').length;
    const inProg = this._dataflr15Actions.filter(a => a.status === 'in_progress').length;
    return { total, done, inProg, pending: total - done - inProg };
  }

  private _dataflr15GetOverallMaturity(): number {
    if (this._dataflr15SelectedDomain === 'all') {
      return Math.round(this._dataflr15MaturityLevels.reduce((s, m) => s + m.current, 0) / this._dataflr15MaturityLevels.length * 10) / 10;
    }
    const d = this._dataflr15MaturityLevels.find(m => m.domain === this._dataflr15SelectedDomain);
    return d ? d.current : 0;
  }

  private _dataflr15GetGapAnalysis() {
    return this._dataflr15Benchmarks.map(b => {
  ...
  private _dataflr15CalcScore(alert: any): number {
    return Math.round(alert.severity * alert.confidence * alert.assetCrit * 100) / 100;
  }

  private _dataflr15EnrichAlert(id: string) {
    this._dataflr15Alerts = this._dataflr15Alerts.map(a => {
      if (a.id !== id) return a;
      const score = this._dataflr15CalcScore(a);
  ...
  private _dataflr15GroupAlerts() {
    const groups: Record<string, number> = {};
    this._dataflr15Alerts.forEach(a => { groups[a.group] = (groups[a.group] || 0) + 1; });
    return Object.entries(groups).map(([g, c]) => ({group: g, count: c}));
  }

  private _dataflr15ValidateControl(shapeId: string): {valid:boolean;missing:string[]} {
    const shape = this._dataflr15Shapes.find(s => s.id === shapeId);
  ...
  private _dataflr15GetOverallHealth(): {healthy:number;degraded:number;down:number;score:number} {
    const online = this._dataflr15Integrations.filter(i => i.status === 'online').length;
    const degraded = this._dataflr15Integrations.filter(i => i.status === 'degraded').length;
    const total = this._dataflr15Integrations.length;
    return { healthy: online, degraded, down: total - online - degraded, score: Math.round(online / total * 100) };
  }

  private _dataflr15AckAnomaly(id: string) {
    this._dataflr15Anomalies = this._dataflr15Anomalies.map(a => a.id === id ? {...a, acknowledged: true} : a);
  }
...
// ===== SECURITY ROI DASHBOARD (R22) =====
  ...
  private _renderBudgetPlanning() {
    const budgetData = [
      { category: "Personnel & Training", planned: 4381000, actual: 1973000, utilization: 45.0, q1: "17%", q2: "22%", q3: "20%", q4: "12%" },
      { category: "Tooling & Licensing", planned: 4388000, actual: 1986000, utilization: 45.3, q1: "20%", q2: "27%", q3: "31%", q4: "13%" },
      { category: "Infrastructure Security", planned: 4395000, actual: 1999000, utilization: 45.5, q1: "23%", q2: "32%", q3: "26%", q4: "14%" },
      { category: "Compliance & Audit", planned: 4402000, actual: 2012000, utilization: 45.7, q1: "26%", q2: "21%", q3: "21%", q4: "15%" },
      { category: "Incident Response", planned: 4409000, actual: 2025000, utilization: 45.9, q1: "29%", q2: "26%", q3: "32%", q4: "16%" },
      { category: "Third-Party Assessments", planned: 4416000, actual: 2038000, utilization: 46.2, q1: "16%", q2: "31%", q3: "27%", q4: "17%" },
      { category: "Security Awareness", planned: 4423000, actual: 2051000, utilization: 46.4, q1: "19%", q2: "20%", q3: "22%", q4: "18%" },
      { category: "Research & Innovation", planned: 4430000, actual: 2064000, utilization: 46.6, q1: "22%", q2: "25%", q3: "33%", q4: "19%" },
    ];
    ...
    const headcount = [
      { team: "SOC Tier 1", current: 13, target: 17, gap: 4, avgSalary: "134k" },
      { team: "SOC Tier 2", current: 5, target: 16, gap: 3, avgSalary: "163k" },
      { team: "Threat Intel", current: 14, target: 15, gap: 2, avgSalary: "192k" },
      { team: "Red Team", current: 6, target: 14, gap: 1, avgSalary: "110k" },
      { team: "GRC", current: 15, target: 13, gap: 0, avgSalary: "139k" },
      { team: "AppSec", current: 7, target: 12, gap: 5, avgSalary: "168k" },
      { team: "Cloud Sec", current: 16, target: 11, gap: 4, avgSalary: "86k" },
      { team: "Identity & Access", current: 8, target: 10, gap: 3, avgSalary: "115k" },
    ];
    const vendorSpend = [
      { vendor: "CrowdStrike", annual: "468k", contractEnd: "2026-11", renewalRisk: "Low", satisfaction: 4 },
      { vendor: "Palo Alto", annual: "499k", contractEnd: "2026-12", renewalRisk: "Medium", satisfaction: 3 },
      { vendor: "Splunk", annual: "530k", contractEnd: "2026-01", renewalRisk: "High", satisfaction: 5 },
      { vendor: "Qualys", annual: "561k", contractEnd: "2026-02", renewalRisk: "Low", satisfaction: 4 },
      { vendor: "Rapid7", annual: "592k", contractEnd: "2026-03", renewalRisk: "Medium", satisfaction: 3 },
      { vendor: "Mandiant", annual: "623k", contractEnd: "2026-04", renewalRisk: "High", satisfaction: 5 },
      { vendor: "Zscaler", annual: "654k", contractEnd: "2026-05", renewalRisk: "Low", satisfaction: 4 },
      { vendor: "Duo Security", annual: "685k", contractEnd: "2026-06", renewalRisk: "Medium", satisfaction: 3 },
    ];
    const roiProjections = [
      { area: "Threat Detection", investment: "443k", projectedReturn: "178k", roiMultiple: "3.5x", confidence: 70 },
      { area: "Incident Reduction", investment: "486k", projectedReturn: "225k", roiMultiple: "3.4x", confidence: 87 },
      { area: "Compliance Savings", investment: "529k", projectedReturn: "272k", roiMultiple: "3.3x", confidence: 68 },
      { area: "Automation Gains", investment: "572k", projectedReturn: "319k", roiMultiple: "3.2x", confidence: 85 },
      { area: "Risk Avoidance", investment: "615k", projectedReturn: "366k", roiMultiple: "3.1x", confidence: 66 },
    ];
    return html`
  ...
  private _renderMetricsNormalization() {
    const kpiCatalog = [
      { id: "kpi-1", name: "MTTD", owner: "SOC", unit: "minutes", target: 83, current: 58, benchmark: 72, collection: "auto", frequency: "realtime" },
      { id: "kpi-2", name: "MTTR", owner: "GRC", unit: "%", target: 86, current: 65, benchmark: 83, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-3", name: "MTTC", owner: "AppSec", unit: "score", target: 89, current: 72, benchmark: 94, collection: "manual", frequency: "weekly" },
      { id: "kpi-4", name: "Vuln SLA Compliance", owner: "Cloud Sec", unit: "count", target: 92, current: 79, benchmark: 71, collection: "auto", frequency: "monthly" },
      { id: "kpi-5", name: "Patch Coverage", owner: "Identity", unit: "days", target: 95, current: 86, benchmark: 82, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-6", name: "Phishing Click Rate", owner: "Threat Intel", unit: "minutes", target: 98, current: 93, benchmark: 93, collection: "manual", frequency: "daily" },
      { id: "kpi-7", name: "Training Completion", owner: "Security Ops", unit: "%", target: 71, current: 100, benchmark: 70, collection: "auto", frequency: "weekly" },
      { id: "kpi-8", name: "Escalation Rate", owner: "Risk Mgmt", unit: "score", target: 74, current: 61, benchmark: 81, collection: "semi-auto", frequency: "monthly" },
      { id: "kpi-9", name: "False Positive Rate", owner: "SOC", unit: "count", target: 77, current: 68, benchmark: 92, collection: "manual", frequency: "realtime" },
      { id: "kpi-10", name: "Threat Intel Actionability", owner: "GRC", unit: "days", target: 80, current: 75, benchmark: 69, collection: "auto", frequency: "daily" },
      { id: "kpi-11", name: "Endpoint Compliance", owner: "AppSec", unit: "minutes", target: 83, current: 82, benchmark: 80, collection: "semi-auto", frequency: "weekly" },
      { id: "kpi-12", name: "Cloud Misconfig Score", owner: "Cloud Sec", unit: "%", target: 86, current: 89, benchmark: 91, collection: "manual", frequency: "monthly" },
      { id: "kpi-13", name: "Identity Anomaly Rate", owner: "Identity", unit: "score", target: 89, current: 96, benchmark: 68, collection: "auto", frequency: "realtime" },
      { id: "kpi-14", name: "DLP Events", owner: "Threat Intel", unit: "count", target: 92, current: 57, benchmark: 79, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-15", name: "Vendor Risk Avg", owner: "Security Ops", unit: "days", target: 95, current: 64, benchmark: 90, collection: "manual", frequency: "weekly" },
      { id: "kpi-16", name: "Compliance Audit Pass Rate", owner: "Risk Mgmt", unit: "minutes", target: 98, current: 71, benchmark: 67, collection: "auto", frequency: "monthly" },
      { id: "kpi-17", name: "Awareness Score", owner: "SOC", unit: "%", target: 71, current: 78, benchmark: 78, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-18", name: "SOC Utilization", owner: "GRC", unit: "score", target: 74, current: 85, benchmark: 89, collection: "manual", frequency: "daily" },
      { id: "kpi-19", name: "Automation Coverage", owner: "AppSec", unit: "count", target: 77, current: 92, benchmark: 66, collection: "auto", frequency: "weekly" },
      { id: "kpi-20", name: "Risk Register Currency", owner: "Cloud Sec", unit: "days", target: 80, current: 99, benchmark: 77, collection: "semi-auto", frequency: "monthly" },
    ];
    const benchmarkSources = [
      { source: "NIST CSF", mappedKPIs: 4, alignment: 91, lastReview: "2026-02-26", status: "aligned" },
      { source: "CIS Controls v8", mappedKPIs: 5, alignment: 69, lastReview: "2026-03-21", status: "partial" },
      { source: "ISO 27001:2022", mappedKPIs: 6, alignment: 86, lastReview: "2026-04-16", status: "reviewing" },
      { source: "PCI DSS 4.0", mappedKPIs: 7, alignment: 64, lastReview: "2026-05-11", status: "aligned" },
      { source: "SOC 2 Type II", mappedKPIs: 8, alignment: 81, lastReview: "2026-06-06", status: "partial" },
      { source: "MITRE ATT&CK", mappedKPIs: 3, alignment: 98, lastReview: "2026-01-01", status: "reviewing" },
      { source: "SANS Top 20", mappedKPIs: 4, alignment: 76, lastReview: "2026-02-24", status: "aligned" },
      { source: "OWASP Top 10", mappedKPIs: 5, alignment: 93, lastReview: "2026-03-19", status: "partial" },
    ];
  ...
  private _renderThreatHuntingCampaigns() {
    const campaigns = [
      { id: "HC-1001", name: "Lateral Movement Sweep", status: "active", hypothesis: "H1: Actors using pass-the-hash for lateral movement", leadHunter: "Alice Chen", findings: 17, startDate: "2026-02-18", endDate: null, effectiveness: 64 },
      { id: "HC-1002", name: "Credential Harvesting Hunt", status: "completed", hypothesis: "H2: Actors using web shells for persistence", leadHunter: "Bob Martinez", findings: 20, startDate: "2026-01-01", endDate: "2026-04-07", effectiveness: 83 },
      { id: "HC-1003", name: "Persistence Mechanism Audit", status: "planned", hypothesis: "H3: Actors using scheduled tasks for data theft", leadHunter: "Carol Wu", findings: 23, startDate: "2026-04-12", endDate: null, effectiveness: 43 },
      { id: "HC-1004", name: "C2 Beacon Detection", status: "in-review", hypothesis: "H4: Actors using DNS tunneling for C2 communication", leadHunter: "Dave Kim", findings: 26, startDate: "2026-03-23", endDate: null, effectiveness: 62 },
      { id: "HC-1005", name: "Data Exfiltration Patterns", status: "active", hypothesis: "H5: Actors using encrypted channels for privilege escalation", leadHunter: "Eve Johnson", findings: 29, startDate: "2026-02-06", endDate: null, effectiveness: 81 },
      { id: "HC-1006", name: "Privilege Escalation Scan", status: "completed", hypothesis: "H6: Actors using token impersonation for defense evasion", leadHunter: "Frank Liu", findings: 32, startDate: "2026-01-17", endDate: "2026-05-19", effectiveness: 41 },
      { id: "HC-1007", name: "Supply Chain Implant Hunt", status: "planned", hypothesis: "H7: Actors using poisoned images for initial access", leadHunter: "Grace Park", findings: 35, startDate: "2026-04-28", endDate: null, effectiveness: 60 },
      { id: "HC-1008", name: "Insider Threat Indicators", status: "in-review", hypothesis: "H8: Actors using legitimate tools for credential access", leadHunter: "Hector Silva", findings: 38, startDate: "2026-03-11", endDate: null, effectiveness: 79 },
      { id: "HC-1009", name: "Cloud Metadata Analysis", status: "active", hypothesis: "H9: Actors using API keys for command execution", leadHunter: "Alice Chen", findings: 41, startDate: "2026-02-22", endDate: null, effectiveness: 98 },
      { id: "HC-1010", name: "DNS Tunnel Detection", status: "completed", hypothesis: "H10: Actors using encoded subdomains for exfiltration", leadHunter: "Bob Martinez", findings: 44, startDate: "2026-01-05", endDate: "2026-06-03", effectiveness: 58 },
      { id: "HC-1011", name: "Fileless Malware Search", status: "planned", hypothesis: "H11: Actors using WMI providers for discovery", leadHunter: "Carol Wu", findings: 47, startDate: "2026-04-16", endDate: null, effectiveness: 77 },
      { id: "HC-1012", name: "Zero-Day Exploit Traces", status: "in-review", hypothesis: "H12: Actors using exploit kits for collection", leadHunter: "Dave Kim", findings: 2, startDate: "2026-03-27", endDate: null, effectiveness: 96 },
    ];
    const hunterLeaderboard = [
      { hunter: "Alice Chen", campaigns: 7, findings: 6, highSeverity: 17, avgScore: 76, streak: 2 },
      { hunter: "Bob Martinez", campaigns: 4, findings: 35, highSeverity: 22, avgScore: 69, streak: 3 },
      { hunter: "Carol Wu", campaigns: 14, findings: 64, highSeverity: 1, avgScore: 62, streak: 4 },
      { hunter: "Dave Kim", campaigns: 11, findings: 93, highSeverity: 6, avgScore: 55, streak: 5 },
      { hunter: "Eve Johnson", campaigns: 8, findings: 6, highSeverity: 11, avgScore: 92, streak: 6 },
      { hunter: "Frank Liu", campaigns: 5, findings: 35, highSeverity: 16, avgScore: 85, streak: 7 },
      { hunter: "Grace Park", campaigns: 15, findings: 64, highSeverity: 21, avgScore: 78, streak: 8 },
      { hunter: "Hector Silva", campaigns: 12, findings: 93, highSeverity: 0, avgScore: 71, streak: 1 },
    ];
    const mitreMapping = [
      { tactic: "Initial Access", techniques: 12, campaigns: 6, coverage: 78 },
      { tactic: "Execution", techniques: 11, campaigns: 5, coverage: 58 },
      { tactic: "Persistence", techniques: 10, campaigns: 4, coverage: 38 },
      { tactic: "Privilege Escalation", techniques: 9, campaigns: 3, coverage: 89 },
      { tactic: "Defense Evasion", techniques: 8, campaigns: 2, coverage: 69 },
      { tactic: "Credential Access", techniques: 7, campaigns: 1, coverage: 49 },
      { tactic: "Discovery", techniques: 6, campaigns: 6, coverage: 100 },
      { tactic: "Lateral Movement", techniques: 5, campaigns: 5, coverage: 80 },
      { tactic: "Collection", techniques: 4, campaigns: 4, coverage: 60 },
      { tactic: "Exfiltration", techniques: 3, campaigns: 3, coverage: 40 },
      { tactic: "Command & Control", techniques: 2, campaigns: 2, coverage: 91 },
      { tactic: "Impact", techniques: 12, campaigns: 1, coverage: 71 },
    ];
    return html`
  ...
  private _renderControlInventory() {
    const controls = [
      { id: "CTL-2001", name: "MFA Enforcement", domain: "Access Control", status: "implemented", effectiveness: 61, lastTest: "2026-02-10", nextReview: "2026-06-10", owner: "SOC", risk: "Low" },
      { id: "CTL-2002", name: "Network Segmentation", domain: "Network Security", status: "partial", effectiveness: 25, lastTest: "2026-01-23", nextReview: "2026-07-01", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2003", name: "EDR Deployment", domain: "Endpoint Protection", status: "planned", effectiveness: 32, lastTest: "2026-04-08", nextReview: "2026-08-20", owner: "IT Ops", risk: "High" },
      { id: "CTL-2004", name: "DLP Policy", domain: "Data Protection", status: "gap", effectiveness: 39, lastTest: "2026-03-21", nextReview: "2026-09-11", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2005", name: "SSO Integration", domain: "Identity Management", status: "implemented", effectiveness: 73, lastTest: "2026-02-06", nextReview: "2026-10-02", owner: "IAM", risk: "Low" },
      { id: "CTL-2006", name: "SAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 2, lastTest: "2026-01-19", nextReview: "2026-11-21", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2007", name: "CSPM Scanning", domain: "Cloud Security", status: "planned", effectiveness: 9, lastTest: "2026-04-04", nextReview: "2026-12-12", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2008", name: "Badge Access", domain: "Physical Security", status: "gap", effectiveness: 16, lastTest: "2026-03-17", nextReview: "2026-05-03", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2009", name: "Least Privilege", domain: "Access Control", status: "implemented", effectiveness: 85, lastTest: "2026-02-02", nextReview: "2026-06-22", owner: "SOC", risk: "Low" },
      { id: "CTL-2010", name: "Firewall Rules", domain: "Network Security", status: "partial", effectiveness: 30, lastTest: "2026-01-15", nextReview: "2026-07-13", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2011", name: "Disk Encryption", domain: "Endpoint Protection", status: "planned", effectiveness: 37, lastTest: "2026-04-28", nextReview: "2026-08-04", owner: "IT Ops", risk: "High" },
      { id: "CTL-2012", name: "Data Classification", domain: "Data Protection", status: "gap", effectiveness: 44, lastTest: "2026-03-13", nextReview: "2026-09-23", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2013", name: "PAM Implementation", domain: "Identity Management", status: "implemented", effectiveness: 97, lastTest: "2026-02-26", nextReview: "2026-10-14", owner: "IAM", risk: "Low" },
      { id: "CTL-2014", name: "DAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 7, lastTest: "2026-01-11", nextReview: "2026-11-05", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2015", name: "IAM Policy Review", domain: "Cloud Security", status: "planned", effectiveness: 14, lastTest: "2026-04-24", nextReview: "2026-12-24", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2016", name: "Visitor Management", domain: "Physical Security", status: "gap", effectiveness: 21, lastTest: "2026-03-09", nextReview: "2026-05-15", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2017", name: "Access Reviews", domain: "Access Control", status: "implemented", effectiveness: 50, lastTest: "2026-02-22", nextReview: "2026-06-06", owner: "SOC", risk: "Low" },
      { id: "CTL-2018", name: "IDS/IPS Tuning", domain: "Network Security", status: "partial", effectiveness: 35, lastTest: "2026-01-07", nextReview: "2026-07-25", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2019", name: "Patch Management", domain: "Endpoint Protection", status: "planned", effectiveness: 42, lastTest: "2026-04-20", nextReview: "2026-08-16", owner: "IT Ops", risk: "High" },
      { id: "CTL-2020", name: "Backup Encryption", domain: "Data Protection", status: "gap", effectiveness: 49, lastTest: "2026-03-05", nextReview: "2026-09-07", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2021", name: "Password Policy", domain: "Identity Management", status: "implemented", effectiveness: 62, lastTest: "2026-02-18", nextReview: "2026-10-26", owner: "IAM", risk: "Low" },
      { id: "CTL-2022", name: "Container Scanning", domain: "Application Security", status: "partial", effectiveness: 12, lastTest: "2026-01-03", nextReview: "2026-11-17", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2023", name: "WAF Configuration", domain: "Cloud Security", status: "planned", effectiveness: 19, lastTest: "2026-04-16", nextReview: "2026-12-08", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2024", name: "CCTV Coverage", domain: "Physical Security", status: "gap", effectiveness: 26, lastTest: "2026-03-01", nextReview: "2026-05-27", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2025", name: "RBAC Enforcement", domain: "Access Control", status: "implemented", effectiveness: 74, lastTest: "2026-02-14", nextReview: "2026-06-18", owner: "SOC", risk: "Low" },
      { id: "CTL-2026", name: "VPN Management", domain: "Network Security", status: "partial", effectiveness: 40, lastTest: "2026-01-27", nextReview: "2026-07-09", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2027", name: "App Whitelisting", domain: "Endpoint Protection", status: "planned", effectiveness: 47, lastTest: "2026-04-12", nextReview: "2026-08-28", owner: "IT Ops", risk: "High" },
      { id: "CTL-2028", name: "Key Management", domain: "Data Protection", status: "gap", effectiveness: 3, lastTest: "2026-03-25", nextReview: "2026-09-19", owner: "Data Gov", risk: "Critical" },
    ];
    const gapAnalysis = [
      { gap: "Insufficient MFA coverage for legacy apps", severity: "High", remediationPlan: "Plan R3001", eta: "2026-Q2", estimatedCost: "121k" },
      { gap: "Missing network micro-segmentation", severity: "Medium", remediationPlan: "Plan R3002", eta: "2026-Q4", estimatedCost: "150k" },
      { gap: "Inconsistent EDR deployment", severity: "Medium", remediationPlan: "Plan R3003", eta: "2026-Q3", estimatedCost: "179k" },
      { gap: "DLP not covering cloud storage", severity: "Low", remediationPlan: "Plan R3004", eta: "2026-Q2", estimatedCost: "27k" },
      { gap: "SSO not integrated with all SaaS", severity: "High", remediationPlan: "Plan R3005", eta: "2026-Q4", estimatedCost: "56k" },
    ];
    return html`
  ...
  private _renderIncidentCostTracker() {
    const incidents = [
      { id: "INC-7001", name: "Security Incident 1", severity: "Critical", totalCost: 2222000, responseCost: 355520, recoveryCost: 466620, legalCost: 133320, regulatoryCost: 288860, insuranceClaim: 1755380, avoidedCost: 342000, date: "2026-02-02" },
      { id: "INC-7002", name: "Security Incident 2", severity: "High", totalCost: 2225000, responseCost: 511750, recoveryCost: 712000, legalCost: 422750, regulatoryCost: 178000, insuranceClaim: 0, avoidedCost: 371000, date: "2026-01-11" },
      { id: "INC-7003", name: "Security Incident 3", severity: "Medium", totalCost: 2228000, responseCost: 668400, recoveryCost: 490160, legalCost: 356480, regulatoryCost: 311920, insuranceClaim: 1648720, avoidedCost: 400000, date: "2026-04-20" },
      { id: "INC-7004", name: "Security Incident 4", severity: "Low", totalCost: 2231000, responseCost: 356960, recoveryCost: 736230, legalCost: 290030, regulatoryCost: 200790, insuranceClaim: 0, avoidedCost: 429000, date: "2026-03-01" },
      { id: "INC-7005", name: "Security Incident 5", severity: "Critical", totalCost: 2234000, responseCost: 513820, recoveryCost: 513820, legalCost: 223400, regulatoryCost: 335100, insuranceClaim: 1541460, avoidedCost: 458000, date: "2026-02-10" },
      { id: "INC-7006", name: "Security Incident 6", severity: "High", totalCost: 2237000, responseCost: 671100, recoveryCost: 760580, legalCost: 156590, regulatoryCost: 223700, insuranceClaim: 0, avoidedCost: 487000, date: "2026-01-19" },
      { id: "INC-7007", name: "Security Incident 7", severity: "Medium", totalCost: 2240000, responseCost: 358400, recoveryCost: 537600, legalCost: 448000, regulatoryCost: 112000, insuranceClaim: 1433600, avoidedCost: 20000, date: "2026-04-28" },
      { id: "INC-7008", name: "Security Incident 8", severity: "Low", totalCost: 2243000, responseCost: 515890, recoveryCost: 785050, legalCost: 381310, regulatoryCost: 246730, insuranceClaim: 0, avoidedCost: 49000, date: "2026-03-09" },
      { id: "INC-7009", name: "Security Incident 9", severity: "Critical", totalCost: 2246000, responseCost: 673800, recoveryCost: 561500, legalCost: 314440, regulatoryCost: 134760, insuranceClaim: 1325140, avoidedCost: 78000, date: "2026-02-18" },
      { id: "INC-7010", name: "Security Incident 10", severity: "High", totalCost: 2249000, responseCost: 359840, recoveryCost: 809640, legalCost: 247390, regulatoryCost: 269880, insuranceClaim: 0, avoidedCost: 107000, date: "2026-01-27" },
      { id: "INC-7011", name: "Security Incident 11", severity: "Medium", totalCost: 2252000, responseCost: 517960, recoveryCost: 585520, legalCost: 180160, regulatoryCost: 157640, insuranceClaim: 1216080, avoidedCost: 136000, date: "2026-04-08" },
      { id: "INC-7012", name: "Security Incident 12", severity: "Low", totalCost: 2255000, responseCost: 676500, recoveryCost: 834350, legalCost: 112750, regulatoryCost: 293150, insuranceClaim: 0, avoidedCost: 165000, date: "2026-03-17" },
    ];
    const yearlyTrend = [
      { month: "Jan", incidents: 10, totalCost: "712k", avgCost: "35k", insured: 69 },
      { month: "Feb", incidents: 7, totalCost: "755k", avgCost: "82k", insured: 69 },
      { month: "Mar", incidents: 4, totalCost: "798k", avgCost: "129k", insured: 69 },
      { month: "Apr", incidents: 12, totalCost: "90k", avgCost: "176k", insured: 69 },
      { month: "May", incidents: 9, totalCost: "133k", avgCost: "42k", insured: 69 },
      { month: "Jun", incidents: 6, totalCost: "176k", avgCost: "89k", insured: 69 },
    ];
...
declare global { interface HTMLElementTagNameMap { 'sc-data-flow': ScDataFlow; } }
