...
export class ScThreatHuntingWorkspace extends LitElement {
  static styles = css`
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .hunt-card { background: #0f172a; border: 1px solid #374151; border-radius: 6px; padding: 12px; margin-bottom: 8px; cursor: pointer; transition: all 0.2s; }
    .hunt-card:hover { border-color: #f59e0b; }
    .hunt-card.active { border-color: #f59e0b; background: #1c1f2e; }
    .hunt-header { display: flex; justify-content: space-between; margin-bottom: 6px; }
    .hunt-name { font-weight: 600; font-size: 13px; }
    .hunt-status { font-size: 10px; padding: 2px 8px; border-radius: 10px; font-weight: 600; }
    .status-active { background: #22c55e20; color: #22c55e; }
    .status-completed { background: #3b82f620; color: #3b82f6; }
    .status-archived { background: #374151; color: #6b7280; }
    .hunt-hypothesis { font-size: 11px; color: #94a3b8; margin-bottom: 8px; font-style: italic; }
    .hunt-meta { display: flex; gap: 12px; font-size: 10px; color: #6b7280; }
    .mitre-chips { display: flex; flex-wrap: wrap; gap: 4px; margin-top: 6px; }
    .mitre-chip { font-size: 9px; padding: 2px 6px; background: #3b82f620; color: #60a5fa; border-radius: 3px; }
    ...
    .finding-meta { display: flex; gap: 10px; font-size: 10px; color: #6b7280; }
    .ioc-tag { font-size: 9px; background: #7f1d1d; color: #fca5a5; padding: 2px 6px; border-radius: 3px; margin: 2px; display: inline-block; }
    .ds-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; }
    .ds-item { background: #0f172a; border-radius: 6px; padding: 10px; text-align: center; }
    .ds-name { font-size: 10px; color: #94a3b8; margin-bottom: 4px; }
    .ds-cov { font-size: 18px; font-weight: 700; }
    .ds-status { font-size: 9px; margin-top: 2px; }
    .mitre-matrix { display: grid; grid-template-columns: repeat(6, 1fr); gap: 4px; margin-top: 12px; }
    .mitre-cell { background: #0f172a; border-radius: 4px; padding: 6px; text-align: center; font-size: 9px; }
    .mitre-cell.active { background: #3b82f630; border: 1px solid #3b82f6; }
    .mitre-cell.suspect { background: #f59e0b30; border: 1px solid #f59e0b; }
    .playbook-btn { background: #3b82f6; color: white; border: none; padding: 8px 14px; border-radius: 6px; font-size: 11px; font-weight: 600; cursor: pointer; margin-right: 6px; margin-bottom: 6px; }
    .playbook-btn:hover { background: #2563eb; }
    ...
    .new-hunt-btn { background: #22c55e; color: white; border: none; padding: 8px 16px; border-radius: 6px; font-size: 12px; font-weight: 600; cursor: pointer; }
    .new-hunt-btn:hover { background: #16a34a; }
    textarea { width: 100%; padding: 10px; background: #0f172a; border: 1px solid #374151; border-radius: 6px; color: #e2e8f0; font-size: 12px; font-family: inherit; outline: none; resize: vertical; min-height: 60px; }
...
`;
  ...
  @state() private _tab: 'sessions' | 'active' | 'playbooks' | 'datasources' = 'sessions';
  @state() private _selectedHunt: string | null = 'h1';
  @state() private _newHypothesis = '';
  ...
  private _sessions: HuntSession[] = [
    { id: 'h1', name: 'Ransomware Precursor Activity', hypothesis: 'Adversaries may be using credential dumping tools before deploying ransomware', status: 'active', startDate: '2026-04-20', analyst: 'Analyst: Chen Li', findings: 3, mitreTactics: ['TA0006', 'TA0005', 'TA0011'] },
    { id: 'h2', name: 'Lateral Movement Detection', hypothesis: 'Suspicious lateral movement patterns may indicate compromise', status: 'active', startDate: '2026-04-18', analyst: 'Analyst: Wang Wei', findings: 1, mitreTactics: ['TA0008', 'TA0011'] },
    { id: 'h3', name: 'Data Exfiltration via DNS', hypothesis: 'DNS tunneling may be used to exfiltrate sensitive data', status: 'completed', startDate: '2026-04-10', analyst: 'Analyst: Zhang San', findings: 0, mitreTactics: ['TA0010', 'TA0011'] },
    { id: 'h4', name: 'Living-off-the-Land Attacks', hypothesis: 'LOLBins may be abused for execution and defense evasion', status: 'completed', startDate: '2026-04-05', analyst: 'Analyst: Li Si', findings: 2, mitreTactics: ['TA0002', 'TA0005'] },
  ];

  private _findings: HuntFinding[] = [
    { id: 'f1', title: 'Suspicious PowerShell Execution with Encoded Command', severity: 'high', technique: 'T1059.001', tactic: 'Execution', evidence: 'Process: powershell.exe -enc <base64>... Parent: winword.exe', ioc: ['10.0.1.45', 'malicious-domain.com'] },
    { id: 'f2', title: 'Mimikatz-style LSASS Access', severity: 'critical', technique: 'T1003.001', tactic: 'Credential Access', evidence: 'Handle opened to LSASS with PROCESS_QUERY_INFORMATION', ioc: ['lsass.exe access', 'mimikatz.exe'] },
    { id: 'f3', title: 'Unusual SMB Traffic Pattern', severity: 'medium', technique: 'T1021.002', tactic: 'Lateral Movement', evidence: 'SMB connection to file server outside business hours', ioc: ['10.0.5.100', '445/tcp'] },
    { id: 'f4', title: 'Rundll32 Spawning CMD', severity: 'low', technique: 'T1218.011', tactic: 'Defense Evasion', evidence: 'rundll32.exe spawning cmd.exe with suspicious arguments', ioc: [] },
  ];

  private _dataSources: DataSource[] = [
    { name: 'Windows Event Logs', coverage: 92, status: 'active' },
    { name: 'Network Traffic (Zeek)', coverage: 88, status: 'active' },
    { name: 'DNS Logs', coverage: 85, status: 'active' },
    { name: 'EDR Telemetry', coverage: 95, status: 'active' },
    { name: 'Proxy Logs', coverage: 78, status: 'partial' },
    { name: 'CloudTrail', coverage: 90, status: 'active' },
    { name: 'Auth Logs', coverage: 82, status: 'active' },
    { name: 'Email Gateway', coverage: 70, status: 'partial' },
  ];

  private _mitreTactics = [
    { id: 'TA0043', name: 'Recon' }, { id: 'TA0042', name: 'Resource' }, { id: 'TA0001', name: 'Initial' },
    { id: 'TA0002', name: 'Exec' }, { id: 'TA0003', name: 'Persist' }, { id: 'TA0004', name: 'PrivEsc' },
  ];

  private _playbooks = [
    { name: 'Ransomware Hunt Playbook', steps: 12, lastRun: '2026-04-19' },
    { name: 'Insider Threat Detection', steps: 8, lastRun: '2026-04-15' },
    { name: 'APT Detection Suite', steps: 15, lastRun: '2026-04-12' },
    { name: 'Living-off-Land Hunting', steps: 10, lastRun: '2026-04-08' },
    { name: 'Data Exfil Detection', steps: 9, lastRun: '2026-04-05' },
  ];
  ...
  private _getStatusClass(status: string): string {
    const m: Record<string, string> = { active: 'status-active', completed: 'status-completed', archived: 'status-archived' };
    return m[status] ?? '';
  }
  ...
  private _renderSessionsTab() {
    return html`
      <div style="margin-bottom:12px;display:flex;gap:8px;align-items:center">
        <input style="flex:1;padding:8px 12px;background:#1f2937;border:1px solid #374151;border-radius:6px;color:#e2e8f0;font-size:12px" placeholder="Search hunts..."/>
        <button class="new-hunt-btn" @click=${() => alert('Create new hunt wizard...')}>+ New Hunt</button>
      </div>
      <div style="max-height:400px;overflow-y:auto">
        ${this._sessions.map(s => html`
          <div class="hunt-card ${this._selectedHunt === s.id ? 'active' : ''}" @click=${() => { this._selectedHunt = s.id; this.requestUpdate(); }}>
            <div class="hunt-header">
              <span class="hunt-name">${s.name}</span>
              <span class="hunt-status ${this._getStatusClass(s.status)}">${s.status}</span>
            </div>
            <div class="hunt-hypothesis">"${s.hypothesis}"</div>
            <div class="hunt-meta">
              <span>📅 ${s.startDate}</span>
              <span>👤 ${s.analyst}</span>
              <span>🔍 ${s.findings} findings</span>
            </div>
            <div class="mitre-chips">${s.mitreTactics.map(t => html`<span class="mitre-chip">${t}</span>`)}</div>
          </div>
        `)}
      </div>
    `;
  ...
  private _renderActiveTab() {
    const hunt = this._sessions.find(h => h.id === this._selectedHunt);
    if (!hunt) return nothing;

    return html`
      <div class="grid-2">
        <div class="section">
          <div class="stitle">Active Hunt Details</div>
          <div style="font-weight:600;margin-bottom:8px">${hunt.name}</div>
          <div style="font-size:11px;color:#94a3b8;margin-bottom:12px">Hypothesis: ${hunt.hypothesis}</div>
          <div style="font-size:11px;color:#94a3b8;margin-bottom:12px">Started: ${hunt.startDate} | Analyst: ${hunt.analyst}</div>
          <textarea placeholder="Enter hypothesis for this hunt..." .value=${this._newHypothesis} @input=${(e: Event) => { this._newHypothesis = (e.target as HTMLTextAreaElement).value; }}></textarea>
          <div style="margin-top:8px">
            <button class="playbook-btn" @click=${() => alert('Search logs...')}>🔍 Search Logs</button>
            <button class="playbook-btn" @click=${() => alert('Query EDR...')}>📡 Query EDR</button>
            <button class="playbook-btn" @click=${() => alert('Cross-reference...')}>🔗 Correlate</button>
          </div>
        </div>
        <div class="section">
          <div class="stitle">MITRE ATT&CK Coverage</div>
          <div class="mitre-matrix">
            ${this._mitreTactics.map(t => html`<div class="mitre-cell ${hunt.mitreTactics.includes(t.id) ? 'active' : ''}">${t.name}<br/><small>${t.id}</small></div>`)}
          </div>
          <div style="margin-top:12px;font-size:10px;color:#6b7280">Legend: <span style="color:#3b82f6">Active Hunt</span> | <span style="color:#f59e0b">Suspicious</span></div>
        </div>
        <div class="kpi-grid">
      ...
      <div class="section">
        <div class="stitle">Hunt Findings (${this._findings.length})</div>
        ${this._findings.map(f => html`
          <div class="finding-row ${f.severity}">
            <div class="finding-title">${f.title} <span style="font-size:10px;color:#6b7280">${f.technique} | ${f.tactic}</span></div>
            <div style="font-size:11px;color:#94a3b8;margin:4px 0">${f.evidence}</div>
            ${f.ioc.length > 0 ? html`<div>IOCs: ${f.ioc.map(i => html`<span class="ioc-tag">${i}</span>`)}</div>` : nothing}
          </div>
        `)}
      </div>
    `;
  ...
  private _renderPlaybooksTab() {
    return html`
      <div class="section">
        <div class="stitle">Hunt Playbooks</div>
        ${this._playbooks.map(p => html`
          <div style="background:#0f172a;border-radius:6px;padding:12px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center">
            <div>
              <div style="font-weight:600;font-size:13px">${p.name}</div>
              <div style="font-size:10px;color:#6b7280;margin-top:2px">${p.steps} steps | Last run: ${p.lastRun}</div>
            </div>
            <button class="playbook-btn" @click=${() => alert(`Launching ${p.name}...`)}>▶ Run</button>
          </div>
        `)}
      </div>
    `;
  ...
  private _renderDatasourcesTab() {
    return html`
      <div class="section">
        <div class="stitle">Data Source Coverage Matrix</div>
        <div class="ds-grid">
          ${this._dataSources.map(ds => html`
            <div class="ds-item">
              <div class="ds-name">${ds.name}</div>
              <div class="ds-cov" style="color:${ds.coverage > 80 ? '#22c55e' : ds.coverage > 60 ? '#f97316' : '#ef4444'}">${ds.coverage}%</div>
              <div class="ds-status" style="color:${ds.status === 'active' ? '#22c55e' : ds.status === 'partial' ? '#f97316' : '#ef4444'}">${ds.status}</div>
            </div>
          `)}
        </div>
      </div>
  ...
  private _recommendations = [
    ...
    { priority: '#f97316', text: 'Complete semi-annual review for all Threat Hunting Workspace items', meta: 'Due: 2026-05-15 | Owner: Compliance' },
  ...
  private _exportData(format: string) {
    const blob = new Blob(['threat-hunting-workspace export'], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'threat-hunting-workspace-export.' + (format === 'markdown' ? 'md' : format); a.click();
  ...
  private _renderPlaybook() {
    ...
    return html`<div style="background:#1f2937;border-radius:8px;padding:14px;margin-bottom:12px">
      <div style="font-size:12px;font-weight:600;color:#94a3b8;margin-bottom:10px;text-transform:uppercase;letter-spacing:0.5px">Threat Hunting Workspace Playbook</div>
  ...
  private _workflowTasks: WorkflowTask[] = [
    { id: 'wt-1', title: 'Review Threat Hunting Workspace finding #1', status: 'active', assignee: 'soc-tier1', blockedBy: [], slaDeadline: '2026-04-23T12:00:00Z', priority: 'p1', createdAt: '2026-04-23T00:00:00Z', completedAt: null },
    ...
    { id: 'wt-3', title: 'Update Threat Hunting Workspace detection rules', status: 'blocked', assignee: 'soc-tier2', blockedBy: ['wt-2'], slaDeadline: '2026-04-24T00:00:00Z', priority: 'p2', createdAt: '2026-04-23T03:00:00Z', completedAt: null },
    ...
    { id: 'wt-8', title: 'Archive resolved Threat Hunting Workspace findings', status: 'failed', assignee: 'soc-tier1', blockedBy: [], slaDeadline: '2026-04-23T08:00:00Z', priority: 'p4', createdAt: '2026-04-22T22:00:00Z', completedAt: null },
  ];

  private _executionHistory: ExecutionRecord[] = [
    {
      id: 'exec-1', name: 'Threat Hunting Workspace Full Assessment Run', startedAt: '2026-04-22T10:00:00Z', completedAt: '2026-04-22T10:12:00Z', status: 'success',
      steps: [
    ...
    {
      id: 'exec-2', name: 'Threat Hunting Workspace Delta Scan', startedAt: '2026-04-23T02:00:00Z', completedAt: '2026-04-23T02:05:00Z', status: 'failed',
      steps: [
  ...
  private _runExecution() {
    ...
    const exec: ExecutionRecord = {
      id: 'exec-' + Date.now(), name: 'Threat Hunting Workspace Assessment ' + new Date().toISOString().slice(0, 10), startedAt: new Date().toISOString(), completedAt: null, status: 'running', steps,
    };
  ...
  private _renderExecutionPanel(): unknown {
    ...
    return html`
      <div class="form-section">
        ...
        ` : html`<div class="empty-state">Click "Run Assessment" to start the Threat Hunting Workspace analysis pipeline</div>`}
      </div>
      <div class="form-section">
        ...
        <div class="settings-card">
          ...
          <div style="display:flex;gap:8px;margin-top:4px;">
            <button class="btn primary" @click=${() => {
              ...
              const url = URL.createObjectURL(blob);
              const a = document.createElement('a'); a.href = url; a.download = 'threat-hunting-workspace-config.json'; a.click();
              URL.revokeObjectURL(url);
            }}>Export</button>
    ...
    return html`
      <div style="margin-top:12px;">
        ...
        <div style="display:flex;gap:6px;margin-top:8px;">
          <input class="search-box" type="text" placeholder="Add a comment..." style="min-width:auto;flex:1;" id="threat-hunting-workspace-comment-input" />
          <button class="btn primary" @click=${() => {
            const input = this.shadowRoot?.querySelector('#threat-hunting-workspace-comment-input') as HTMLInputElement;
  ...
  @state() private _thFairModel: any = null;
  @state() private _thRiskHeatMap: any = null;
  @state() private _thRiskAppetite: any = null;
  @state() private _thMonteCarlo: any = null;
  @state() private _thRiskRegister: any = null;
  @state() private _thRiskTrend: any = null;

  private thInitRiskQuant() {
    this._thFairModel = {
      scenarios: [
  ...
  private thRenderRiskQuant() {
    const fm = this._thFairModel;
    ...
    return html`<div class="card" style="padding:14px;margin-bottom:8px">
        ...
        <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:2px;font-size:8px">
          ${this._thRiskHeatMap.map((c: any) => html`<div style="background:${c.color}22;border:1px solid ${c.color}44;border-radius:2px;padding:3px;text-align:center;color:${c.color}">
              <div>${c.score}</div>
            </div>`)}
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px">
          <div>
            ...
            ${this._thMonteCarlo.slice(0, 10).map((r: any) => html`<div style="display:flex;gap:4px;font-size:8px;margin-bottom:1px">
          ...
          <div>
            ...
            ${this._thRiskRegister.map((r: any) => html`<div style="display:flex;align-items:center;gap:4px;margin-bottom:2px;font-size:8px">
        ...
        <div style="display:flex;gap:4px;align-items:flex-end;height:50px">
          ${this._thRiskTrend.map((t: any) => html`<div style="flex:1;display:flex;flex-direction:column;gap:1px;align-items:center">
  ...
  @state() private _thOkrs: any = null;
  @state() private _thInitiatives: any = null;
  @state() private _thResourceAlloc: any = null;
  @state() private _thHeadcount: any = null;
  @state() private _thMilestones: any = null;
  @state() private _thBudget: any = null;

  private thInitSecProgram() {
    this._thOkrs = [
  ...
  private thRenderSecProgram() {
    const okrs = this._thOkrs;
    ...
    return html`<div class="card" style="padding:14px;margin-bottom:8px">
        ...
        <div style="max-height:120px;overflow-y:auto">
          ${this._thInitiatives.map((i: any) => html`<div style="display:flex;align-items:center;gap:4px;margin-bottom:2px;font-size:8px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px">
          <div>
            ...
            ${this._thResourceAlloc.slice(0, 5).map((r: any) => html`<div style="display:flex;align-items:center;gap:4px;font-size:8px;margin-bottom:2px">
          ...
          <div>
            ...
            <div style="font-size:9px;color:#ccc;margin-bottom:4px">Total: ${fmt(this._thBudget.total)} | Spent: ${fmt(this._thBudget.spent)}</div>
            ${this._thBudget.quarterly.map((q: any) => html`<div style="display:flex;align-items:center;gap:4px;font-size:8px;margin-bottom:2px">
        ...
        ${this._thMilestones.map((m: any) => html`<div style="display:flex;align-items:center;gap:4px;margin-bottom:2px;font-size:8px">
  ...
  @state() private _thVendorTiers: any = null;
  @state() private _thDueDiligence: any = null;
  @state() private _thContractClauses: any = null;
  @state() private _thVendorScorecard: any = null;
  @state() private _thSubProcessors: any = null;
  @state() private _thVendorIncidents: any = null;

  private thInitThirdParty() {
    this._thVendorTiers = [
  ...
  private thRenderThirdParty() {
    const tiers = this._thVendorTiers;
    ...
    return html`<div class="card" style="padding:14px;margin-bottom:8px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">
          <div>
            ...
            ${this._thDueDiligence.slice(0, 8).map((d: any) => html`<div style="display:flex;align-items:center;gap:4px;font-size:8px;margin-bottom:2px">
          ...
          <div>
            ...
            ${this._thContractClauses.map((c: any) => html`<div style="display:flex;align-items:center;gap:4px;font-size:8px;margin-bottom:2px">
        ...
        ${this._thVendorScorecard.map((v: any) => html`<div style="display:flex;align-items:center;gap:6px;margin-bottom:3px;font-size:8px">
        ...
        ${this._thVendorIncidents.map((i: any) => html`<div style="display:flex;align-items:center;gap:4px;margin-bottom:2px;font-size:8px">
  ...
  @state() private _thDlpRules: any = null;
  @state() private _thDataMovement: any = null;
  @state() private _thDataDiscovery: any = null;
  @state() private _thDlpIncidents: any = null;
  @state() private _thEncryptionMatrix: any = null;
  ...
  private thRenderDLP() {
    ...
    return html`<div class="card" style="padding:14px;margin-bottom:8px">
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-bottom:10px">
          ${[["Monitored",this._thDataMovement.monitored.toLocaleString(),"#48f"],["Blocked",this._thDataMovement.blocked.toLocaleString(),"#f44"],["Rules",rules.length,"#ff8"],["Incidents",this._thDlpIncidents.length,"#f84"]].map(([l,v,c]) => html`<div style="background:#1a1d27;border-radius:4px;padding:6px;text-align:center">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px">
          <div>
            ...
            ${this._thDataMovement.byChannel.map((c: any) => html`<div style="display:flex;align-items:center;gap:4px;font-size:8px;margin-bottom:2px">
          ...
          <div>
            ...
            ${this._thEncryptionMatrix.map((e: any) => html`<div style="display:flex;align-items:center;gap:4px;font-size:8px;margin-bottom:2px">
        ...
        ${this._thDlpIncidents.map((i: any) => html`<div style="display:flex;align-items:center;gap:4px;margin-bottom:2px;font-size:8px">
  ...
  @state() private _thAutoMetrics: any = null;
  @state() private _thAutoTimeSaved: any = null;
  @state() private _thAutoReliability: any = null;
  @state() private _thAutoComparison: any = null;
  @state() private _thAutoROI: any = null;
  @state() private _thAutoCandidates: any = null;

  private thInitAutomation() {
    this._thAutoMetrics = {
  ...
  private thRenderAutomation() {
    const m = this._thAutoMetrics;
    if (!m) return nothing;
    return html`<div class="card" style="padding:14px;margin-bottom:8px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">
          <div>
            ...
            ${this._thAutoTimeSaved.slice(0, 5).map((t: any) => html`<div style="display:flex;align-items:center;gap:4px;font-size:8px;margin-bottom:2px">
          ...
          <div>
            ...
            ${this._thAutoReliability.map((p: any) => html`<div style="display:flex;align-items:center;gap:4px;font-size:8px;margin-bottom:2px">
        ...
        <div style="background:#1a1d27;border-radius:4px;padding:8px;margin-bottom:8px">
          <div style="display:flex;justify-content:space-between;align-items:center">
            <div>
              ...
              <div style="color:#4caf50;font-size:16px;font-weight:bold">${this._thAutoROI.roi}%</div>
            </div>
            <div style="text-align:center">
              <div style="color:#aaa;font-size:9px">Annual Savings</div>
              <div style="color:#48f;font-size:16px;font-weight:bold">${(this._thAutoROI.annualSavings / 1000000).toFixed(1)}M</div>
            </div>
            <div style="text-align:center">
              <div style="color:#aaa;font-size:9px">Headcount Saved</div>
              <div style="color:#ff8;font-size:16px;font-weight:bold">${this._thAutoROI.headcountSaved}</div>
            </div>
            <div style="text-align:right">
              <div style="color:#aaa;font-size:9px">Investment</div>
              <div style="color:#ccc;font-size:16px;font-weight:bold">${(this._thAutoROI.investment / 1000).toFixed(0)}K</div>
            </div>
          </div>
        </div>
        <div style="color:#aaa;font-size:10px;margin-bottom:4px">Manual vs Automated (hours/process)</div>
        ${this._thAutoComparison.map((c: any) => html`<div style="display:flex;align-items:center;gap:4px;font-size:8px;margin-bottom:2px">
        ...
        ${this._thAutoCandidates.map((c: any) => html`<div style="display:flex;align-items:center;gap:4px;margin-bottom:2px;font-size:8px">
  ...
  private thInitRound17() {
    this.thInitRiskQuant();
    this.thInitSecProgram();
    this.thInitThirdParty();
    this.thInitDLP();
    this.thInitAutomation();
  }

  private thRenderRound17() {
    return html`${this.thRenderRiskQuant()}${this.thRenderSecProgram()}${this.thRenderThirdParty()}${this.thRenderDLP()}${this.thRenderAutomation()}`;
  }
  ...
  @state() private _threatHuntinCorrRules: Array<{id:string;name:string;sources:string[];logic:string;severity:string;active:boolean;lastTriggered:string}> = [
  ...
  @state() private _threatHuntinEventTimeline: Array<{timestamp:string;source:string;eventType:string;details:string;correlated:boolean}> = [
  ...
  @state() private _threatHuntinFalsePosMetrics: {totalEvents:number;correlatedEvents:number;falsePositives:number;fpRate:number;topFpRules:string[]} = {
  ...
  @state() private _threatHuntinEventPatterns: Array<{id:string;pattern:string;frequency:number;firstSeen:string;lastSeen:string;status:string}> = [
  ...
  private _renderThreathuntinEventCorr(): TemplateResult {
    const rules = this._threatHuntinCorrRules;
    const timeline = this._threatHuntinEventTimeline;
    const fpMetrics = this._threatHuntinFalsePosMetrics;
    return html`
  ...
  @state() private _threatHuntinZones: Array<{id:string;name:string;trustLevel:number;subnet:string;devices:number;policy:string;lastAudit:string}> = [
  ...
  @state() private _threatHuntinSegRules: Array<{id:string;source:string;dest:string;action:string;protocol:string;port:string;status:string;hits:number}> = [
  ...
  @state() private _threatHuntinCrossZoneTraffic: Array<{source:string;dest:string;bytes:number;sessions:number;violations:number}> = [
  ...
  @state() private _threatHuntinMicroSegGaps: Array<{id:string;zone:string;gapType:string;severity:string;recommendation:string}> = [
  ...
  private _renderThreathuntinNetworkSeg(): TemplateResult {
    const zones = this._threatHuntinZones;
    const gaps = this._threatHuntinMicroSegGaps;
    return html`
  ...
  @state() private _threatHuntinCourses: Array<{id:string;title:string;category:string;duration:number;enrolled:number;completed:number;difficulty:string;rating:number}> = [
  ...
  @state() private _threatHuntinLearningPaths: Array<{id:string;name:string;courseIds:string[];progress:number;enrolled:number}> = [
  ...
  @state() private _threatHuntinDeptCompliance: Array<{dept:string;trainedPct:number;targetPct:number;avgScore:number;certCount:number}> = [
  ...
  @state() private _threatHuntinSkillsGaps: Array<{skill:string;current:number;required:number;gap:number;priority:string}> = [
  ...
  private _renderThreathuntinTraining(): TemplateResult {
    const courses = this._threatHuntinCourses;
    const deptComp = this._threatHuntinDeptCompliance;
    return html`
  ...
  @state() private _threatHuntinRiskTopTen: Array<{id:string;name:string;score:number;trend:string;owner:string;category:string;lastAssessed:string}> = [
  ...
  @state() private _threatHuntinRiskCategories: Array<{category:string;count:number;avgScore:number;color:string}> = [
  ...
  @state() private _threatHuntinRiskVelocity: Array<{week:string;newRisks:number;closedRisks:number;netChange:number}> = [
  ...
  @state() private _threatHuntinRiskAppetite: {current:number;threshold:number;maxTolerated:number;status:string} = {
    current: 68, threshold: 55, maxTolerated: 80, status: 'Warning'
  };
  @state() private _threatHuntinRiskOwnerMatrix: Array<{owner:string;criticalCount:number;highCount:number;mediumCount:number;complianceRate:number}> = [
  ...
  private _renderThreathuntinRiskDash(): TemplateResult {
    const riskItems = this._threatHuntinRiskTopTen;
    const velocity = this._threatHuntinRiskVelocity;
    const appetite = this._threatHuntinRiskAppetite;
    return html`
  ...
  @state() private _complianceCalendarEvents: Array<{id: string; title: string; deadline: string; category: string; status: string; assignee: string; notes: string}> = [
    {id: 'cc-threat-hunting-workspace-001', title: 'SOX Section 404 Compliance Review', deadline: '2026-06-30', category: 'Regulatory', status: 'In Progress', assignee: 'Compliance Team', notes: 'Annual internal control assessment for financial reporting'},
    {id: 'cc-threat-hunting-workspace-002', title: 'GDPR Data Protection Impact Assessment', deadline: '2026-05-15', category: 'Privacy', status: 'Pending', assignee: 'DPO Office', notes: 'Quarterly DPIA for high-risk processing activities'},
    {id: 'cc-threat-hunting-workspace-003', title: 'ISO 27001 Surveillance Audit', deadline: '2026-07-20', category: 'Certification', status: 'Scheduled', assignee: 'QA Lead', notes: 'Stage 2 surveillance audit for ISMS certification renewal'},
    {id: 'cc-threat-hunting-workspace-004', title: 'PCI DSS v4.0 Gap Analysis', deadline: '2026-05-30', category: 'Payment', status: 'Not Started', assignee: 'Security Architect', notes: 'Assess current state against PCI DSS v4.0 requirements'},
    {id: 'cc-threat-hunting-workspace-005', title: 'NIST CSF Self-Assessment', deadline: '2026-08-15', category: 'Framework', status: 'Pending', assignee: 'CISO', notes: 'Biannual self-assessment against NIST Cybersecurity Framework'},
    {id: 'cc-threat-hunting-workspace-006', title: 'HIPAA Security Rule Audit', deadline: '2026-06-15', category: 'Healthcare', status: 'In Progress', assignee: 'Compliance Analyst', notes: 'Annual audit of administrative, physical, and technical safeguards'},
    {id: 'cc-threat-hunting-workspace-007', title: 'SOC 2 Type II Report Renewal', deadline: '2026-09-30', category: 'Audit', status: 'Scheduled', assignee: 'External Auditor', notes: 'Engage auditor for SOC 2 Type II examination period'},
    {id: 'cc-threat-hunting-workspace-008', title: 'FedRAMP Authorization Review', deadline: '2026-07-01', category: 'Government', status: 'Pending', assignee: 'FedRAMP PMO', notes: 'Review and update security authorization package'},
    {id: 'cc-threat-hunting-workspace-009', title: 'Annual Penetration Test', deadline: '2026-10-15', category: 'Testing', status: 'Not Started', assignee: 'Red Team Lead', notes: 'Comprehensive external and internal penetration testing engagement'},
    {id: 'cc-threat-hunting-workspace-010', title: 'Security Awareness Training Rollout', deadline: '2026-05-01', category: 'Training', status: 'Completed', assignee: 'Security Awareness Manager', notes: 'Q2 organization-wide security awareness training program'},
    {id: 'cc-threat-hunting-workspace-011', title: 'Vendor Security Review Cycle', deadline: '2026-06-01', category: 'Third Party', status: 'In Progress', assignee: 'Vendor Manager', notes: 'Quarterly review of critical vendor security posture and SLA compliance'},
    {id: 'cc-threat-hunting-workspace-012', title: 'Incident Response Plan Update', deadline: '2026-05-20', category: 'Operations', status: 'Pending', assignee: 'IR Team Lead', notes: 'Update IR procedures based on lessons learned from recent incidents'},
  ];
  ...
  @state() private _dataQualityMetrics: Array<{id: string; dataSource: string; completenessScore: number; accuracyScore: number; timelinessScore: number; consistencyScore: number; reliabilityRank: number; lastUpdated: string; issues: string[]; trend: string}> = [
    {id: 'dq-threat-hunting-workspace-001', dataSource: 'Vulnerability Scanner Feed', completenessScore: 94, accuracyScore: 91, timelinessScore: 88, consistencyScore: 95, reliabilityRank: 1, lastUpdated: '2026-04-23T08:00:00Z', issues: ['Minor delay in Nessus plugin updates'], trend: 'improving'},
    {id: 'dq-threat-hunting-workspace-002', dataSource: 'SIEM Event Logs', completenessScore: 87, accuracyScore: 93, timelinessScore: 96, consistencyScore: 89, reliabilityRank: 2, lastUpdated: '2026-04-23T07:30:00Z', issues: ['Some log sources showing gaps in off-hours'], trend: 'stable'},
    {id: 'dq-threat-hunting-workspace-003', dataSource: 'Asset Inventory Database', completenessScore: 82, accuracyScore: 78, timelinessScore: 75, consistencyScore: 84, reliabilityRank: 4, lastUpdated: '2026-04-22T18:00:00Z', issues: ['Shadow IT devices not fully cataloged', 'Decommissioned assets still listed'], trend: 'declining'},
    {id: 'dq-threat-hunting-workspace-004', dataSource: 'Threat Intelligence Platform', completenessScore: 91, accuracyScore: 89, timelinessScore: 92, consistencyScore: 87, reliabilityRank: 3, lastUpdated: '2026-04-23T06:00:00Z', issues: ['Some IOC sources lack confidence scoring'], trend: 'improving'},
    {id: 'dq-threat-hunting-workspace-005', dataSource: 'Identity Provider Logs', completenessScore: 96, accuracyScore: 97, timelinessScore: 98, consistencyScore: 96, reliabilityRank: 1, lastUpdated: '2026-04-23T09:00:00Z', issues: [], trend: 'stable'},
    {id: 'dq-threat-hunting-workspace-006', dataSource: 'Cloud Security Posture Data', completenessScore: 79, accuracyScore: 85, timelinessScore: 82, consistencyScore: 81, reliabilityRank: 5, lastUpdated: '2026-04-23T05:00:00Z', issues: ['Multi-cloud coverage gaps', 'API rate limiting affects scan frequency'], trend: 'declining'},
    {id: 'dq-threat-hunting-workspace-007', dataSource: 'Compliance Evidence Store', completenessScore: 88, accuracyScore: 86, timelinessScore: 80, consistencyScore: 83, reliabilityRank: 4, lastUpdated: '2026-04-21T14:00:00Z', issues: ['Manual evidence collection delays', 'Inconsistent formatting across frameworks'], trend: 'stable'},
    {id: 'dq-threat-hunting-workspace-008', dataSource: 'Network Flow Data', completenessScore: 92, accuracyScore: 90, timelinessScore: 94, consistencyScore: 91, reliabilityRank: 2, lastUpdated: '2026-04-23T08:30:00Z', issues: ['Encrypted traffic classification accuracy needs improvement'], trend: 'improving'},
  ];
  ...
  @state() private _workflowTemplates: Array<{id: string; name: string; description: string; category: string; steps: number; avgDuration: string; successRate: number; lastRun: string; manualInterventions: number; optimizationScore: number}> = [
    {id: 'wf-threat-hunting-workspace-001', name: 'Vulnerability Triage Pipeline', description: 'Automated vulnerability assessment, prioritization, and assignment workflow', category: 'Vulnerability Management', steps: 8, avgDuration: '12 min', successRate: 94.5, lastRun: '2026-04-23T06:00:00Z', manualInterventions: 2, optimizationScore: 87},
    {id: 'wf-threat-hunting-workspace-002', name: 'Incident Response Orchestration', description: 'End-to-end IR workflow from detection to containment and recovery', category: 'Incident Response', steps: 15, avgDuration: '45 min', successRate: 89.2, lastRun: '2026-04-22T14:30:00Z', manualInterventions: 5, optimizationScore: 72},
    {id: 'wf-threat-hunting-workspace-003', name: 'Access Request Approval Chain', description: 'Multi-level approval workflow for privileged access requests', category: 'Identity & Access', steps: 6, avgDuration: '4 hours', successRate: 97.1, lastRun: '2026-04-23T09:15:00Z', manualInterventions: 1, optimizationScore: 92},
    {id: 'wf-threat-hunting-workspace-004', name: 'Compliance Evidence Collection', description: 'Automated gathering and packaging of audit evidence artifacts', category: 'Compliance', steps: 10, avgDuration: '30 min', successRate: 91.8, lastRun: '2026-04-21T10:00:00Z', manualInterventions: 3, optimizationScore: 78},
    {id: 'wf-threat-hunting-workspace-005', name: 'Security Patch Deployment', description: 'Staged patch rollout with validation and rollback capability', category: 'Patch Management', steps: 12, avgDuration: '2 hours', successRate: 96.3, lastRun: '2026-04-20T22:00:00Z', manualInterventions: 1, optimizationScore: 85},
    {id: 'wf-threat-hunting-workspace-006', name: 'Threat Intelligence Enrichment', description: 'IOC enrichment and correlation with internal threat data', category: 'Threat Intelligence', steps: 7, avgDuration: '8 min', successRate: 93.7, lastRun: '2026-04-23T07:00:00Z', manualInterventions: 0, optimizationScore: 95},
    {id: 'wf-threat-hunting-workspace-007', name: 'Security Reporting Pipeline', description: 'Automated generation and distribution of security metrics reports', category: 'Reporting', steps: 5, avgDuration: '15 min', successRate: 98.9, lastRun: '2026-04-23T08:00:00Z', manualInterventions: 0, optimizationScore: 97},
    {id: 'wf-threat-hunting-workspace-008', name: 'Vendor Risk Assessment', description: 'Automated vendor security questionnaire distribution and scoring', category: 'Third Party', steps: 9, avgDuration: '3 days', successRate: 85.4, lastRun: '2026-04-19T11:00:00Z', manualInterventions: 4, optimizationScore: 65},
  ];
  ...
  @state() private _workflowExecutionHistory: Array<{workflowId: string; runDate: string; duration: string; status: string; trigger: string}> = [
    {workflowId: 'wf-threat-hunting-workspace-001', runDate: '2026-04-23T06:00:00Z', duration: '11 min 23 sec', status: 'Success', trigger: 'Scheduled'},
    {workflowId: 'wf-threat-hunting-workspace-002', runDate: '2026-04-22T14:30:00Z', duration: '42 min 15 sec', status: 'Success', trigger: 'Alert'},
    {workflowId: 'wf-threat-hunting-workspace-003', runDate: '2026-04-23T09:15:00Z', duration: '3h 45 min', status: 'Success', trigger: 'User Request'},
    {workflowId: 'wf-threat-hunting-workspace-001', runDate: '2026-04-22T06:00:00Z', duration: '13 min 08 sec', status: 'Partial', trigger: 'Scheduled'},
    {workflowId: 'wf-threat-hunting-workspace-004', runDate: '2026-04-21T10:00:00Z', duration: '28 min 42 sec', status: 'Success', trigger: 'Scheduled'},
  ];
  ...
  @state() private _knowledgeArticles: Array<{id: string; title: string; category: string; author: string; publishDate: string; views: number; rating: number; tags: string[]; summary: string; readTime: string}> = [
    {id: 'kb-threat-hunting-workspace-001', title: 'Zero Trust Architecture Implementation Guide', category: 'Architecture', author: 'Security Architecture Team', publishDate: '2026-03-15', views: 1842, rating: 4.8, tags: ['zero-trust', 'network', 'identity'], summary: 'Comprehensive guide for implementing zero trust principles across enterprise infrastructure', readTime: '15 min'},
    {id: 'kb-threat-hunting-workspace-002', title: 'Ransomware Defense Playbook 2026', category: 'Threats', author: 'Threat Intelligence Team', publishDate: '2026-04-01', views: 2341, rating: 4.9, tags: ['ransomware', 'incident-response', 'defense'], summary: 'Updated playbook with latest ransomware tactics, techniques, and defensive measures', readTime: '12 min'},
    {id: 'kb-threat-hunting-workspace-003', title: 'Cloud Security Best Practices for Multi-Cloud', category: 'Cloud', author: 'Cloud Security Team', publishDate: '2026-02-28', views: 1567, rating: 4.5, tags: ['cloud', 'aws', 'azure', 'gcp'], summary: 'Best practices for securing workloads across multiple cloud service providers', readTime: '18 min'},
    {id: 'kb-threat-hunting-workspace-004', title: 'API Security Testing Methodology', category: 'Testing', author: 'Red Team', publishDate: '2026-03-22', views: 987, rating: 4.6, tags: ['api', 'testing', 'owasp'], summary: 'Structured methodology for identifying and testing API security vulnerabilities', readTime: '10 min'},
    {id: 'kb-threat-hunting-workspace-005', title: 'Supply Chain Security Risk Management', category: 'Third Party', author: 'GRC Team', publishDate: '2026-01-20', views: 1234, rating: 4.3, tags: ['supply-chain', 'vendor', 'risk'], summary: 'Framework for managing security risks in the software supply chain', readTime: '14 min'},
    {id: 'kb-threat-hunting-workspace-006', title: 'Kubernetes Security Hardening Checklist', category: 'Container', author: 'Platform Security', publishDate: '2026-04-10', views: 2103, rating: 4.7, tags: ['kubernetes', 'container', 'hardening'], summary: 'Step-by-step hardening checklist for production Kubernetes clusters', readTime: '11 min'},
    {id: 'kb-threat-hunting-workspace-007', title: 'Security Metrics That Matter for Executives', category: 'Metrics', author: 'CISO Office', publishDate: '2026-03-05', views: 876, rating: 4.4, tags: ['metrics', 'kpi', 'reporting'], summary: 'Key security metrics and KPIs that resonate with board-level stakeholders', readTime: '8 min'},
    {id: 'kb-threat-hunting-workspace-008', title: 'Phishing Simulation Campaign Design', category: 'Awareness', author: 'Security Awareness Team', publishDate: '2026-02-15', views: 1456, rating: 4.5, tags: ['phishing', 'simulation', 'training'], summary: 'Designing effective phishing simulations that drive real behavioral change', readTime: '9 min'},
    {id: 'kb-threat-hunting-workspace-009', title: 'Data Loss Prevention Strategy and Implementation', category: 'Data Protection', author: 'Data Security Team', publishDate: '2026-03-28', views: 1678, rating: 4.6, tags: ['dlp', 'data-protection', 'compliance'], summary: 'Strategic approach to DLP covering people, process, and technology layers', readTime: '16 min'},
    {id: 'kb-threat-hunting-workspace-010', title: 'Security Automation with SOAR Platforms', category: 'Automation', author: 'SOC Engineering', publishDate: '2026-04-05', views: 1098, rating: 4.7, tags: ['soar', 'automation', 'soc'], summary: 'Building effective security automation playbooks using SOAR technology', readTime: '13 min'},
    {id: 'kb-threat-hunting-workspace-011', title: 'Third-Party Risk Assessment Framework', category: 'GRC', author: 'Vendor Management', publishDate: '2026-01-30', views: 1345, rating: 4.4, tags: ['vendor', 'risk', 'assessment'], summary: 'Structured framework for evaluating and monitoring third-party security risks', readTime: '12 min'},
    {id: 'kb-threat-hunting-workspace-012', title: 'Network Detection and Response Best Practices', category: 'Network', author: 'Network Security Team', publishDate: '2026-03-18', views: 1567, rating: 4.5, tags: ['ndr', 'network', 'detection'], summary: 'Implementing effective network detection and response capabilities', readTime: '14 min'},
    {id: 'kb-threat-hunting-workspace-013', title: 'Security Chaos Engineering Guide', category: 'Resilience', author: 'SRE Security', publishDate: '2026-04-12', views: 789, rating: 4.3, tags: ['chaos-engineering', 'resilience', 'testing'], summary: 'Applying chaos engineering principles to validate security controls', readTime: '10 min'},
    {id: 'kb-threat-hunting-workspace-014', title: 'Incident Communication Templates', category: 'Incident Response', author: 'IR Team', publishDate: '2026-02-22', views: 2234, rating: 4.8, tags: ['incident', 'communication', 'templates'], summary: 'Ready-to-use communication templates for security incident scenarios', readTime: '7 min'},
    {id: 'kb-threat-hunting-workspace-015', title: 'Endpoint Detection and Response Configuration', category: 'Endpoint', author: 'Endpoint Security', publishDate: '2026-03-08', views: 1345, rating: 4.5, tags: ['edr', 'endpoint', 'configuration'], summary: 'Optimal EDR configuration for maximum detection with minimal false positives', readTime: '11 min'},
    {id: 'kb-threat-hunting-workspace-016', title: 'Security Code Review Standards', category: 'Development', author: 'AppSec Team', publishDate: '2026-02-10', views: 1890, rating: 4.6, tags: ['code-review', 'secure-sdlc', 'standards'], summary: 'Standards and checklists for security-focused code reviews', readTime: '13 min'},
    {id: 'kb-threat-hunting-workspace-017', title: 'IoT Security Assessment Methodology', category: 'IoT', author: 'IoT Security Lab', publishDate: '2026-04-08', views: 654, rating: 4.2, tags: ['iot', 'embedded', 'assessment'], summary: 'Methodology for assessing security posture of IoT devices and ecosystems', readTime: '15 min'},
    {id: 'kb-threat-hunting-workspace-018', title: 'Passwordless Authentication Migration Guide', category: 'Identity', author: 'IAM Team', publishDate: '2026-03-25', views: 1678, rating: 4.7, tags: ['passwordless', 'authentication', 'mfa'], summary: 'Step-by-step migration plan from password-based to passwordless authentication', readTime: '12 min'},
    {id: 'kb-threat-hunting-workspace-019', title: 'Security Awareness Program Metrics', category: 'Awareness', author: 'Security Culture Team', publishDate: '2026-01-25', views: 1123, rating: 4.4, tags: ['awareness', 'metrics', 'culture'], summary: 'Measuring the effectiveness of your security awareness and training programs', readTime: '9 min'},
    {id: 'kb-threat-hunting-workspace-020', title: 'Deception Technology Deployment Guide', category: 'Detection', author: 'Blue Team', publishDate: '2026-04-15', views: 876, rating: 4.5, tags: ['deception', 'honeypot', 'detection'], summary: 'Planning and deploying deception technology for advanced threat detection', readTime: '11 min'},
  ];
  ...
  // --- Security Scenario Planning Methods (THHUWO) ---
  private _initThhuwoScenarios(): void {
    const worstCaseScenarios = [
      { id: "thhuwo-sc-01", name: "Ransomware Attack on Critical Infrastructure", category: "Malware", impact: 9500000, probability: 0.12, recoveryDays: 21, affectedSystems: 47, dataLossGb: 320, businessImpact: "critical" },
      { id: "thhuwo-sc-02", name: "Insider Data Exfiltration via Authorized Channels", category: "Insider Threat", impact: 7200000, probability: 0.18, recoveryDays: 14, affectedSystems: 12, dataLossGb: 85, businessImpact: "high" },
      { id: "thhuwo-sc-03", name: "Supply Chain Compromise via Third-Party Library", category: "Supply Chain", impact: 6800000, probability: 0.09, recoveryDays: 30, affectedSystems: 63, dataLossGb: 0, businessImpact: "critical" },
      { id: "thhuwo-sc-04", name: "Cloud Misconfiguration Leading to Public Exposure", category: "Cloud", impact: 4100000, probability: 0.22, recoveryDays: 7, affectedSystems: 8, dataLossGb: 450, businessImpact: "high" },
      { id: "thhuwo-sc-05", name: "Zero-Day Exploit in Core Application Framework", category: "Vulnerability", impact: 8900000, probability: 0.05, recoveryDays: 45, affectedSystems: 120, dataLossGb: 0, businessImpact: "critical" },
      { id: "thhuwo-sc-06", name: "Distributed Denial of Service on Customer-Facing Services", category: "Availability", impact: 3400000, probability: 0.25, recoveryDays: 3, affectedSystems: 15, dataLossGb: 0, businessImpact: "medium" },
      { id: "thhuwo-sc-07", name: "Credential Stuffing and Account Takeover Wave", category: "Identity", impact: 2800000, probability: 0.30, recoveryDays: 5, affectedSystems: 6, dataLossGb: 12, businessImpact: "medium" },
      { id: "thhuwo-sc-08", name: "Physical Security Breach at Data Center Facility", category: "Physical", impact: 12000000, probability: 0.02, recoveryDays: 60, affectedSystems: 200, dataLossGb: 5000, businessImpact: "critical" },
    ];
    this._thhuwoScenarios = worstCaseScenarios.map(s => ({ ...s, mitigationCost: Math.round(s.impact * 0.08), drillScheduled: false, lastDrillDate: null, drillScore: null, controlGaps: Math.floor(Math.random() * 5) + 1 }));
  }

  private _calculateThhuwoScenarioRisk(): void {
    this._thhuwoScenarios.forEach(s => {
  ...
  private _scheduleThhuwoDrills(): void {
  ...
  private _thhuwoScenarioComparison(): Record<string, unknown>[] {
    return this._thhuwoScenarios.map(s => ({
  ...
  // --- Security Control Effectiveness Analytics (THHUWO) ---
  private _initThhuwoControls(): void {
    const controls = [
      { id: "thhuwo-ct-01", name: "Network Segmentation", type: "preventive", maturity: "defined", score: 82, target: 90, failures: 3, lastTest: "2026-03-15" },
      { id: "thhuwo-ct-02", name: "Endpoint Detection and Response", type: "detective", maturity: "managed", score: 88, target: 92, failures: 1, lastTest: "2026-04-01" },
      { id: "thhuwo-ct-03", name: "Data Loss Prevention", type: "preventive", maturity: "repeatable", score: 71, target: 85, failures: 7, lastTest: "2026-03-22" },
      { id: "thhuwo-ct-04", name: "Security Information and Event Management", type: "detective", maturity: "managed", score: 85, target: 90, failures: 2, lastTest: "2026-04-05" },
      { id: "thhuwo-ct-05", name: "Identity and Access Management", type: "preventive", maturity: "defined", score: 76, target: 88, failures: 5, lastTest: "2026-03-28" },
      { id: "thhuwo-ct-06", name: "Vulnerability Management", type: "corrective", maturity: "managed", score: 80, target: 90, failures: 4, lastTest: "2026-04-02" },
      { id: "thhuwo-ct-07", name: "Email Security Gateway", type: "preventive", maturity: "managed", score: 90, target: 95, failures: 1, lastTest: "2026-04-08" },
      { id: "thhuwo-ct-08", name: "Web Application Firewall", type: "preventive", maturity: "managed", score: 87, target: 92, failures: 2, lastTest: "2026-03-30" },
      { id: "thhuwo-ct-09", name: "Patch Management", type: "corrective", maturity: "defined", score: 68, target: 85, failures: 9, lastTest: "2026-03-18" },
      { id: "thhuwo-ct-10", name: "Encryption at Rest", type: "preventive", maturity: "managed", score: 93, target: 95, failures: 0, lastTest: "2026-04-10" },
      { id: "thhuwo-ct-11", name: "Encryption in Transit", type: "preventive", maturity: "optimized", score: 96, target: 98, failures: 0, lastTest: "2026-04-12" },
      { id: "thhuwo-ct-12", name: "Privileged Access Management", type: "preventive", maturity: "defined", score: 74, target: 88, failures: 6, lastTest: "2026-03-25" },
      { id: "thhuwo-ct-13", name: "Security Awareness Training", type: "preventive", maturity: "repeatable", score: 65, target: 80, failures: 11, lastTest: "2026-03-20" },
      { id: "thhuwo-ct-14", name: "Incident Response Plan", type: "corrective", maturity: "managed", score: 78, target: 88, failures: 4, lastTest: "2026-04-03" },
      { id: "thhuwo-ct-15", name: "Backup and Recovery", type: "corrective", maturity: "managed", score: 84, target: 92, failures: 3, lastTest: "2026-04-06" },
      { id: "thhuwo-ct-16", name: "Multi-Factor Authentication", type: "preventive", maturity: "managed", score: 91, target: 95, failures: 1, lastTest: "2026-04-09" },
      { id: "thhuwo-ct-17", name: "Network Traffic Analysis", type: "detective", maturity: "defined", score: 72, target: 85, failures: 5, lastTest: "2026-03-27" },
      { id: "thhuwo-ct-18", name: "Cloud Security Posture Management", type: "detective", maturity: "repeatable", score: 69, target: 82, failures: 8, lastTest: "2026-03-24" },
      { id: "thhuwo-ct-19", name: "Container Security Scanning", type: "preventive", maturity: "defined", score: 75, target: 85, failures: 5, lastTest: "2026-04-04" },
      { id: "thhuwo-ct-20", name: "API Security Gateway", type: "preventive", maturity: "repeatable", score: 70, target: 82, failures: 7, lastTest: "2026-03-29" },
      { id: "thhuwo-ct-21", name: "Threat Intelligence Platform", type: "detective", maturity: "managed", score: 83, target: 90, failures: 2, lastTest: "2026-04-07" },
      { id: "thhuwo-ct-22", name: "Database Activity Monitoring", type: "detective", maturity: "defined", score: 67, target: 80, failures: 8, lastTest: "2026-03-21" },
      { id: "thhuwo-ct-23", name: "Deception Technology", type: "detective", maturity: "initial", score: 55, target: 75, failures: 12, lastTest: "2026-03-16" },
      { id: "thhuwo-ct-24", name: "Security Orchestration Automation", type: "corrective", maturity: "repeatable", score: 73, target: 85, failures: 6, lastTest: "2026-04-01" },
      { id: "thhuwo-ct-25", name: "Third-Party Risk Assessment", type: "preventive", maturity: "defined", score: 64, target: 78, failures: 9, lastTest: "2026-03-19" },
    ];
    this._thhuwoControls = controls;
  }

  private _analyzeThhuwoControlEffectiveness(): void {
    const typeBreakdown: Record<string, { total: number; avgScore: number; avgFailures: number }> = {};
    this._thhuwoControls.forEach(c => {
    ...
    this._thhuwoControlTypeBreakdown = typeBreakdown;
  }

  private _thhuwoControlOptimization(): Record<string, unknown>[] {
    return this._thhuwoControls.filter(c => c.score < c.target).map(c => ({
  ...
  // --- Security Data Pipeline Health (THHUWO) ---
  private _initThhuwoPipelines(): void {
    const pipelines = [
      { id: "thhuwo-pl-01", name: "SIEM Log Ingestion", status: "healthy", healthScore: 94, latencyMs: 120, freshnessMin: 2, errorRate: 0.02, throughputMbps: 450, recordsPerSec: 12000, backPressure: 0.05, uptime: 99.97 },
      { id: "thhuwo-pl-02", name: "Threat Feed Aggregation", status: "healthy", healthScore: 91, latencyMs: 300, freshnessMin: 15, errorRate: 0.05, throughputMbps: 85, recordsPerSec: 3200, backPressure: 0.08, uptime: 99.92 },
      { id: "thhuwo-pl-03", name: "Vulnerability Scan Results", status: "degraded", healthScore: 72, latencyMs: 2500, freshnessMin: 60, errorRate: 0.15, throughputMbps: 25, recordsPerSec: 800, backPressure: 0.35, uptime: 98.50 },
      { id: "thhuwo-pl-04", name: "Endpoint Telemetry Stream", status: "healthy", healthScore: 88, latencyMs: 450, freshnessMin: 5, errorRate: 0.08, throughputMbps: 280, recordsPerSec: 8500, backPressure: 0.12, uptime: 99.85 },
      { id: "thhuwo-pl-05", name: "Cloud Audit Log Pipeline", status: "healthy", healthScore: 86, latencyMs: 600, freshnessMin: 10, errorRate: 0.06, throughputMbps: 150, recordsPerSec: 4500, backPressure: 0.10, uptime: 99.80 },
      { id: "thhuwo-pl-06", name: "Identity Event Stream", status: "warning", healthScore: 78, latencyMs: 1200, freshnessMin: 20, errorRate: 0.12, throughputMbps: 45, recordsPerSec: 1500, backPressure: 0.22, uptime: 99.40 },
      { id: "thhuwo-pl-07", name: "Network Flow Collection", status: "healthy", healthScore: 82, latencyMs: 800, freshnessMin: 8, errorRate: 0.09, throughputMbps: 650, recordsPerSec: 18000, backPressure: 0.15, uptime: 99.70 },
      { id: "thhuwo-pl-08", name: "DLP Incident Pipeline", status: "degraded", healthScore: 68, latencyMs: 3500, freshnessMin: 45, errorRate: 0.20, throughputMbps: 18, recordsPerSec: 500, backPressure: 0.42, uptime: 97.80 },
    ];
    this._thhuwoPipelines = pipelines.map(p => ({ ...p, dependencies: [], slaBreached: p.healthScore < 75, alertThreshold: p.errorRate > 0.15 }));
    this._thhuwoPipelines[0].dependencies = ["thhuwo-pl-04", "thhuwo-pl-07"];
    this._thhuwoPipelines[4].dependencies = ["thhuwo-pl-01"];
    this._thhuwoPipelines[5].dependencies = ["thhuwo-pl-02"];
    this._thhuwoPipelines[7].dependencies = ["thhuwo-pl-04", "thhuwo-pl-05"];
  }

  private _thhuwoPipelineTrend(): Record<string, unknown>[] {
    const hours = Array.from({ length: 24 }, (_, i) => `H${String(i).padStart(2, "0")}`);
    return this._thhuwoPipelines.map(p => ({
  ...
  // --- Security Stakeholder Report Generator (THHUWO) ---
  private _initThhuwoReportTemplates(): void {
    this._thhuwoReportTemplates = [
      { id: "thhuwo-rp-01", name: "Board Security Report", audience: "board", frequency: "quarterly", sections: 6, autoGenerate: true, lastGenerated: "2026-03-31", pages: 12 },
      { id: "thhuwo-rp-02", name: "Executive Risk Summary", audience: "executive", frequency: "monthly", sections: 8, autoGenerate: true, lastGenerated: "2026-04-01", pages: 8 },
      { id: "thhuwo-rp-03", name: "Technical Security Review", audience: "technical", frequency: "weekly", sections: 12, autoGenerate: true, lastGenerated: "2026-04-21", pages: 25 },
      { id: "thhuwo-rp-04", name: "Audit Compliance Report", audience: "audit", frequency: "quarterly", sections: 10, autoGenerate: false, lastGenerated: "2026-03-15", pages: 35 },
      { id: "thhuwo-rp-05", name: "Regulatory Filing Package", audience: "regulatory", frequency: "annual", sections: 15, autoGenerate: false, lastGenerated: "2025-12-31", pages: 48 },
    ];
  }

  private _generateThhuwoExecSummary(): string {
    const totalRisk = this._thhuwoScenarios.reduce((sum: number, s: any) => sum + s.riskScore, 0);
    const avgControl = Math.round(this._thhuwoControls.reduce((sum: number, c: any) => sum + c.score, 0) / this._thhuwoControls.length);
    const degradedPipes = this._thhuwoPipelines.filter((p: any) => p.status !== "healthy").length;
    return `Overall risk exposure: ${(totalRisk / 1000000).toFixed(1)}M. Average control effectiveness: {avgControl}%. {degradedPipes} pipeline(s) need attention. {this._thhuwoScenarios.filter((s: any) => s.mitigationPriority === "immediate").length} scenarios require immediate action.`;
  }

  // --- Security Technology Radar (THHUWO) ---
  private _initThhuwoTechRadar(): void {
    this._thhuwoTechRadar = [
      { id: "thhuwo-tr-01", name: "AI-Powered Threat Detection", ring: "adopt", category: "Detection", maturity: 4, trend: "up", investment: "high", roi: 3.2, vendorCount: 8 },
      { id: "thhuwo-tr-02", name: "Zero Trust Architecture", ring: "adopt", category: "Architecture", maturity: 4, trend: "up", investment: "high", roi: 2.8, vendorCount: 12 },
      { id: "thhuwo-tr-03", name: "SASE/SSE Platform", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.4, vendorCount: 10 },
      { id: "thhuwo-tr-04", name: "CNAPP Cloud Security", ring: "trial", category: "Cloud", maturity: 3, trend: "up", investment: "medium", roi: 2.1, vendorCount: 7 },
      { id: "thhuwo-tr-05", name: "Extended Detection and Response", ring: "adopt", category: "Detection", maturity: 4, trend: "stable", investment: "high", roi: 2.9, vendorCount: 9 },
      { id: "thhuwo-tr-06", name: "Quantum-Safe Cryptography", ring: "assess", category: "Crypto", maturity: 2, trend: "up", investment: "low", roi: 0.8, vendorCount: 4 },
      { id: "thhuwo-tr-07", name: "Security Service Edge", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.0, vendorCount: 6 },
      { id: "thhuwo-tr-08", name: "SOAR 2.0 Autonomous SOC", ring: "assess", category: "Operations", maturity: 2, trend: "up", investment: "low", roi: 1.2, vendorCount: 5 },
      { id: "thhuwo-tr-09", name: "Software Supply Chain Security", ring: "trial", category: "DevSecOps", maturity: 3, trend: "up", investment: "medium", roi: 2.3, vendorCount: 8 },
      { id: "thhuwo-tr-10", name: "Identity Threat Detection", ring: "adopt", category: "Identity", maturity: 3, trend: "up", investment: "high", roi: 2.7, vendorCount: 7 },
      { id: "thhuwo-tr-11", name: "Data Security Posture Management", ring: "trial", category: "Data", maturity: 3, trend: "up", investment: "medium", roi: 2.2, vendorCount: 9 },
      { id: "thhuwo-tr-12", name: "Decentralized Identity Standards", ring: "hold", category: "Identity", maturity: 1, trend: "stable", investment: "low", roi: 0.5, vendorCount: 3 },
    ];
  }

  private _thhuwoRadarSummary(): Record<string, unknown> {
    const rings: Record<string, number> = { adopt: 0, trial: 0, assess: 0, hold: 0 };
    this._thhuwoTechRadar.forEach(t => { if (rings[t.ring as keyof typeof rings] !== undefined) rings[t.ring as keyof typeof rings]++; });
    return { adopt: rings.adopt, trial: rings.trial, assess: rings.assess, hold: rings.hold, total: this._thhuwoTechRadar.length, avgMaturity: 2.8, topInvestment: this._thhuwoTechRadar.filter(t => t.investment === "high").map(t => t.name) };
  }

  // --- THHUWO Security Compliance Mapping Matrix ---
  private _initThhuwoComplianceMatrix(): void {
    ...
    this._thhuwoComplianceMatrix = frameworks.map(fw => ({
  ...
  private _thhuwoComplianceTrend(): Record<string, unknown>[] {
    const quarters = ["Q1-2025", "Q2-2025", "Q3-2025", "Q4-2025", "Q1-2026", "Q2-2026"];
    return this._thhuwoComplianceMatrix.map(fw => ({
  ...
  // --- THHUWO Threat Intelligence Correlation Engine ---
  private _initThhuwoThreatIntel(): void {
    ...
    this._thhuwoThreatIntel = threatActors.map((actor, i) => ({
  ...
  private _thhuwoThreatCorrelation(): Record<string, unknown>[] {
    return this._thhuwoThreatIntel.filter(t => t.riskScore > 70).map(t => ({
  ...
  // --- THHUWO Security Metrics Deep Dive ---
  private _initThhuwoMetricsDeep(): void {
    ...
    this._thhuwoMetricsDeep = metricCategories.map(cat => ({
  ...
  private _thhuwoMetricsHeatmap(): number[][] {
    return this._thhuwoMetricsDeep.map(cat =>
      cat.metrics.map(m => Math.round((m.value / m.target) * 100))
    );
  }

  // --- THHUWO Incident Cost Modeling ---
  private _initThhuwoCostModel(): void {
    const incidentTypes = ["Data Breach", "Ransomware", "DDoS", "Insider Threat", "Phishing", "Supply Chain", "Cloud Misconfig", "Zero-Day"];
    this._thhuwoCostModel = incidentTypes.map(inc => ({
    ...
    this._thhuwoCostModel.forEach(m => { m.totalAnnualExposure = m.avgCost * m.frequency; });
  }

  private _thhuwoCostProjection(): Record<string, unknown>[] {
    return this._thhuwoCostModel.map(m => ({
  ...
  // --- THHUWO Security Architecture Pattern Library ---
  private _initThhuwoArchPatterns(): void {
    ...
    this._thhuwoArchPatterns = patterns.map((p, i) => ({
  ...
  private _thhuwoArchPatternAnalysis(): Record<string, unknown> {
    const implemented = this._thhuwoArchPatterns.filter(p => p.implementation > 75).length;
    const avgCoverage = Math.round(this._thhuwoArchPatterns.reduce((s: number, p: any) => s + p.coverage, 0) / this._thhuwoArchPatterns.length);
    return {
      totalPatterns: this._thhuwoArchPatterns.length, fullyImplemented: implemented, avgCoverage,
      avgRiskReduction: Math.round(this._thhuwoArchPatterns.reduce((s: number, p: any) => s + p.riskReduction, 0) / this._thhuwoArchPatterns.length),
      gaps: this._thhuwoArchPatterns.filter(p => p.implementation < 60).map(p => p.pattern),
      recommendations: this._thhuwoArchPatterns.filter(p => p.implementation < 70).map(p => ({ pattern: p.pattern, current: p.implementation, target: 85, effort: p.costComplexity }))
    };
  }
  ...
  private _b5aRiskAssessmentWorkflow = [
  ...
  private _b5aTreatmentOptions = [
    {id: 'b5a-T-001', riskId: 'RSK-042', risk: 'Ransomware Attack on Critical Infrastructure', before: 20, after: 6, strategy: 'Mitigate', controls: ['Next-gen EDR with behavioral detection and automated response', 'Zero-trust network micro-segmentation isolating critical assets', 'Immutable offline backup rotation with quarterly recovery testing', 'Incident response retainer with 1-hour response SLA'], cost: '$450K/yr', roi: '78% reduction vs $2.8M potential loss', timeline: 'Q3 2026', status: 'approved' as const},
  ...
  private _b5aTalentPositions = [
    ...
    {id: 'b5a-H-002', title: 'Cloud Security Engineer', dept: 'Cloud Security', level: 'Mid-Senior', loc: 'SF/Remote', salary: '$155K-$180K', posted: '2026-03-20', applicants: 45, pipeline: 12, interviews: 6, offers: 2, urgency: 'high' as const, manager: 'Mike Johnson', skills: ['AWS/Azure/GCP', 'Terraform', 'Kubernetes', 'IAM', 'CSPM'], nice: ['CCSP', 'CKS', 'Serverless']},
    {id: 'b5a-H-003', title: 'SOC Analyst L2', dept: 'Security Operations', level: 'Mid', loc: 'Austin TX', salary: '$105K-$130K', posted: '2026-04-01', applicants: 67, pipeline: 18, interviews: 8, offers: 0, urgency: 'medium' as const, manager: 'Lisa Park', skills: ['SIEM', 'Incident triage', 'Malware analysis', 'TCP/IP'], nice: ['GCIA', 'SOAR', 'Kill chain']},
    {id: 'b5a-H-004', title: 'Staff Security Architect', dept: 'Architecture', level: 'Staff', loc: 'Remote US', salary: '$200K-$240K', posted: '2026-02-28', applicants: 12, pipeline: 3, interviews: 2, offers: 0, urgency: 'critical' as const, manager: 'David Lee', skills: ['Enterprise arch', 'Zero trust', 'Risk frameworks', 'Board comms'], nice: ['CISSP+SABSA', 'Team building', 'Speaker']},
    {id: 'b5a-H-005', title: 'GRC Analyst', dept: 'GRC', level: 'Mid', loc: 'NY/Remote', salary: '$110K-$135K', posted: '2026-04-05', applicants: 34, pipeline: 9, interviews: 3, offers: 1, urgency: 'medium' as const, manager: 'Rachel Green', skills: ['ISO 27001', 'SOC 2', 'Risk assessment', 'Audit'], nice: ['CRISC', 'CISA', 'GDPR']},
    {id: 'b5a-H-006', title: 'Threat Intel Analyst', dept: 'Threat Intel', level: 'Mid-Senior', loc: 'Remote US', salary: '$140K-$170K', posted: '2026-03-25', applicants: 19, pipeline: 5, interviews: 2, offers: 0, urgency: 'low' as const, manager: 'James Wilson', skills: ['MITRE ATT&CK', 'OSINT', 'Threat modeling', 'Reverse eng'], nice: ['CTIA', 'Nation-state', 'Dark web']},
    {id: 'b5a-H-007', title: 'AppSec Engineer', dept: 'AppSec', level: 'Senior', loc: 'Seattle/Remote', salary: '$160K-$190K', posted: '2026-04-10', applicants: 28, pipeline: 7, interviews: 3, offers: 0, urgency: 'medium' as const, manager: 'Emily Zhang', skills: ['SAST/DAST/SCA', 'Secure SDLC', 'Code review', 'CI/CD'], nice: ['CSSLP', 'Mobile security']},
    {id: 'b5a-H-008', title: 'IAM Engineer', dept: 'IAM', level: 'Mid', loc: 'Remote US', salary: '$135K-$160K', posted: '2026-04-08', applicants: 31, pipeline: 10, interviews: 5, offers: 1, urgency: 'medium' as const, manager: 'Chris Martinez', skills: ['Okta/Azure AD', 'SAML/OIDC', 'PAM', 'RBAC'], nice: ['CISSP', 'Zero trust IAM']}
  ];

  private _b5aInterviewScorecard = [
  ...
  private _b5aOnboardingSteps = [
    {id: 'b5a-OB-01', item: 'Provision security-hardened laptop, monitors, peripherals, and configured workstation image with all required security tools', cat: 'IT Setup', day: 'Day 1', owner: 'IT Operations'},
    {id: 'b5a-OB-02', item: 'Create AD account, email, Slack, VPN, SSO enrollment, and all required application access accounts', cat: 'Access', day: 'Day 1', owner: 'IAM Team'},
    {id: 'b5a-OB-03', item: 'Grant security tooling access: SIEM console, EDR dashboard, vuln scanner, pentest platforms, SOAR', cat: 'Tools', day: 'Day 1-2', owner: 'SOC Manager'},
    {id: 'b5a-OB-04', item: 'Complete MFA enrollment, security awareness onboarding course, and acceptable use policy acknowledgment', cat: 'Security', day: 'Day 1', owner: 'Awareness Team'},
    {id: 'b5a-OB-05', item: 'Sign NDA, employment agreement, IP assignment, and code of conduct documents with HR', cat: 'HR', day: 'Day 1', owner: 'Human Resources'},
    {id: 'b5a-OB-06', item: 'Introduction meetings with direct team members and key cross-functional stakeholders', cat: 'Integration', day: 'Week 1', owner: 'Hiring Manager'},
    {id: 'b5a-OB-07', item: 'Security architecture overview, tooling walkthrough, and operational runbook review sessions', cat: 'Training', day: 'Week 1-2', owner: 'Security Architect'},
    {id: 'b5a-OB-08', item: 'Shadow experienced team member on active incident response and threat hunting investigations', cat: 'Mentoring', day: 'Week 2-3', owner: 'SOC Lead'},
    {id: 'b5a-OB-09', item: 'Set up individual certification study plan aligned with role requirements and career goals', cat: 'Development', day: 'Week 2', owner: 'Hiring Manager'},
    {id: 'b5a-OB-10', item: 'Establish 30-60-90 day goals, define success metrics, and schedule first performance checkpoint', cat: 'Goals', day: 'Week 1', owner: 'Hiring Manager'}
  ];

  private _b5aMetricsDashboard = [
    {id: 'b5a-MTD-001', name: 'Mean Time to Detect (MTTD)', value: '4.2 hours', trend: 'improving' as const, target: '< 4 hours', source: 'SIEM - Splunk', method: 'Median time from first alert to analyst acknowledgment', owner: 'SOC Manager', sla: '4 hours', prev: '5.1 hours', spark: [5.8, 5.4, 5.1, 4.9, 4.7, 4.5, 4.3, 4.2], breakdown: [{src: 'Network IDS', pct: '35%', avg: '3.8h'}, {src: 'Endpoint EDR', pct: '28%', avg: '4.1h'}, {src: 'Email Gateway', pct: '22%', avg: '3.5h'}, {src: 'Cloud Security', pct: '10%', avg: '5.2h'}, {src: 'User Reports', pct: '5%', avg: '6.8h'}]},
    {id: 'b5a-MTD-002', name: 'Mean Time to Respond (MTTR)', value: '2.8 hours', trend: 'stable' as const, target: '< 3 hours', source: 'SOAR - XSOAR', method: 'Median time from acknowledgment to first containment action', owner: 'IR Lead', sla: '3 hours', prev: '2.7 hours', spark: [3.2, 3.0, 2.9, 2.8, 2.8, 2.9, 2.8, 2.8], breakdown: [{src: 'Auto Playbooks', pct: '42%', avg: '1.2h'}, {src: 'Semi-Auto', pct: '33%', avg: '2.5h'}, {src: 'Manual', pct: '20%', avg: '5.1h'}, {src: 'L3 Escalation', pct: '5%', avg: '8.3h'}]},
    {id: 'b5a-MTD-003', name: 'Patch Compliance Rate', value: '97.6%', trend: 'improving' as const, target: '> 95%', source: 'Tenable.io', method: 'Pct of critical/high vulns patched within SLA windows', owner: 'Vuln Manager', sla: '95%', prev: '96.1%', spark: [93.2, 94.1, 94.8, 95.3, 95.9, 96.4, 97.0, 97.6], breakdown: [{src: 'Critical (7d)', pct: '40%', rate: '99.2%'}, {src: 'High (30d)', pct: '35%', rate: '97.8%'}, {src: 'Medium (90d)', pct: '20%', rate: '95.1%'}, {src: 'Low (180d)', pct: '5%', rate: '89.4%'}]},
    {id: 'b5a-MTD-004', name: 'Phishing Click Rate', value: '3.2%', trend: 'improving' as const, target: '< 5%', source: 'KnowBe4 + Proofpoint', method: 'Pct of users clicking simulated phishing links monthly', owner: 'Awareness Lead', sla: '5%', prev: '4.1%', spark: [6.8, 6.2, 5.5, 5.1, 4.6, 4.1, 3.6, 3.2], breakdown: [{src: 'Spear Phishing', pct: '35%', rate: '5.1%'}, {src: 'Generic Phishing', pct: '30%', rate: '2.8%'}, {src: 'BEC Sim', pct: '20%', rate: '3.5%'}, {src: 'SMS Phishing', pct: '15%', rate: '1.9%'}]},
    {id: 'b5a-MTD-005', name: 'Security Posture Score', value: '784', trend: 'improving' as const, target: '> 750', source: 'SecurityScorecard', method: 'Composite: external 30%, compliance 25%, vuln 25%, config 20%', owner: 'CISO', sla: '> 750', prev: '771', spark: [748, 752, 758, 762, 768, 773, 778, 784], breakdown: [{src: 'External Rating', pct: '30%', score: '791'}, {src: 'Compliance', pct: '25%', score: '812'}, {src: 'Vuln Mgmt', pct: '25%', score: '745'}, {src: 'Config', pct: '20%', score: '769'}]},
    {id: 'b5a-MTD-006', name: 'Vulnerability Backlog', value: '142', trend: 'improving' as const, target: '< 150', source: 'Tenable + Qualys', method: 'Total open vulns across all scanned assets tracked weekly', owner: 'Vuln Manager', sla: '< 150', prev: '168', spark: [210, 198, 185, 175, 165, 158, 150, 142], breakdown: [{src: 'Critical', pct: '8%', count: '11'}, {src: 'High', pct: '22%', count: '31'}, {src: 'Medium', pct: '45%', count: '64'}, {src: 'Low', pct: '25%', count: '36'}]}
  ];

  private _b5aArchReview = [
    {id: 'b5a-AR-001', cat: 'Network', item: 'Network segmentation between trust zones with dedicated firewalls and granular ACLs', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Micro-segmentation via NSX across all production zones'},
    {id: 'b5a-AR-002', cat: 'Network', item: 'DMZ architecture isolating public services from internal network', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Dual-firewall DMZ with WAF and DDoS mitigation'},
    {id: 'b5a-AR-003', cat: 'Network', item: 'Least-privilege traffic flows with deny-by-default and quarterly review', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Explicit allow rules with automated drift detection'},
    {id: 'b5a-AR-004', cat: 'Network', item: 'Encrypted external communications with IPSec/TLS 1.3 and PFS', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'IKEv2 with PFS, TLS 1.3 for all external APIs'},
    {id: 'b5a-AR-005', cat: 'Identity', item: 'Centralized IdP with SSO and automated SCIM provisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Okta SSO SAML 2.0 for 247 apps with SCIM'},
    {id: 'b5a-AR-006', cat: 'Identity', item: 'RBAC with quarterly reviews and automated deprovisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Quarterly recertification with JIT admin access'},
    {id: 'b5a-AR-007', cat: 'Identity', item: 'PAM controlling all admin and service accounts with session recording', sev: 'high' as const, status: 'fail' as const, reviewer: 'IAM Lead', notes: '3 legacy accounts pending CyberArk enrollment'},
    {id: 'b5a-AR-008', cat: 'Identity', item: 'MFA enforced for all users, hardware tokens for privileged access', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'FIDO2 for admins, push MFA for users, 99.8% enrollment'},
    {id: 'b5a-AR-009', cat: 'Data', item: 'Data classification enforced with automated labeling on all repos', sev: 'high' as const, status: 'partial' as const, reviewer: 'Data Protection Lead', notes: 'Framework deployed, gaps on 4 legacy file shares'},
    {id: 'b5a-AR-010', cat: 'Data', item: 'AES-256 encryption at rest for all sensitive data stores', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'Full-disk, TDE on DBs, SSE-KMS on cloud storage'},
    {id: 'b5a-AR-011', cat: 'Data', item: 'DLP monitoring and preventing unauthorized data exfiltration', sev: 'high' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'DLP active on email, endpoint, cloud, and network'},
    {id: 'b5a-AR-012', cat: 'Data', item: 'Encrypted off-site backups with quarterly restore testing', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Infrastructure Lead', notes: '3-2-1 strategy with immutable backups'},
    {id: 'b5a-AR-013', cat: 'AppSec', item: 'Secure SDLC with SAST, DAST, SCA in all CI/CD pipelines', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: '34 pipelines with security gates and PR blocking'},
    {id: 'b5a-AR-014', cat: 'AppSec', item: 'API gateway with auth, rate limiting, schema validation', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: 'Kong with OAuth2, rate limits, OpenAPI validation'},
    {id: 'b5a-AR-015', cat: 'AppSec', item: 'Container security with image scanning and runtime protection', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Trivy scanning, Falco runtime in 80% of clusters'},
    {id: 'b5a-AR-016', cat: 'Monitoring', item: 'SIEM collecting from all critical systems with correlation rules', sev: 'critical' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'Splunk 98% coverage with 1200+ correlation rules'},
    {id: 'b5a-AR-017', cat: 'Monitoring', item: 'EDR with behavioral detection on all managed endpoints', sev: 'high' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'CrowdStrike on 99.2% endpoints with 24/7 MDR'},
    {id: 'b5a-AR-018', cat: 'Monitoring', item: 'Executive dashboards with real-time posture visibility', sev: 'medium' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'PowerBI dashboards with weekly exec summaries'},
    {id: 'b5a-AR-019', cat: 'Cloud', item: 'CSPM monitoring and remediating cloud misconfigurations', sev: 'high' as const, status: 'pass' as const, reviewer: 'Cloud Security Lead', notes: 'Prisma Cloud 450+ checks with auto-remediation'},
    {id: 'b5a-AR-020', cat: 'Cloud', item: 'IaC templates scanned before deployment to production', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Checkov for Terraform, cfn-nag for CloudFormation pending'}
  ];

  private _getb5aRiskProgress(): number {
    return Math.round(this._b5aRiskAssessmentWorkflow.filter(s => s.status === 'completed').length / this._b5aRiskAssessmentWorkflow.length * 100);
  }

  private _getb5aTreatmentSummary(): {total: number; approved: number; avgReduction: number; investment: string} {
    const a = this._b5aTreatmentOptions.filter(r => r.status === 'approved').length;
    const avg = this._b5aTreatmentOptions.reduce((s, r) => s + ((r.before - r.after) / r.before * 100), 0) / this._b5aTreatmentOptions.length;
    return {total: this._b5aTreatmentOptions.length, approved: a, avgReduction: Math.round(avg), investment: '$1.125M annually'};
  }

  private _getb5aRuleSummary(): {total: number; passing: number; failing: number; avgRate: number; violations: number} {
    const p = this._b5aComplianceRules.filter(r => r.result === 'pass').length;
    const f = this._b5aComplianceRules.filter(r => r.result === 'fail').length;
    const avg = this._b5aComplianceRules.reduce((s, r) => s + r.passRate, 0) / this._b5aComplianceRules.length;
    const v = this._b5aComplianceRules.reduce((s, r) => s + r.violations, 0);
    return {total: this._b5aComplianceRules.length, passing: p, failing: f, avgRate: Math.round(avg * 10) / 10, violations: v};
  }

  private _getb5aPipelineStats(): {positions: number; applicants: number; interviewing: number; offers: number; critical: number} {
    const t = this._b5aTalentPositions.length;
    const a = this._b5aTalentPositions.reduce((s, p) => s + p.applicants, 0);
    const i = this._b5aTalentPositions.reduce((s, p) => s + p.interviews, 0);
    const o = this._b5aTalentPositions.reduce((s, p) => s + p.offers, 0);
    const c = this._b5aTalentPositions.filter(x => x.urgency === 'critical').length;
    return {positions: t, applicants: a, interviewing: i, offers: o, critical: c};
  }

  private _getb5aMetricTrends(): {improving: number; stable: number; total: number} {
    const imp = this._b5aMetricsDashboard.filter(m => m.trend === 'improving').length;
    const stb = this._b5aMetricsDashboard.filter(m => m.trend === 'stable').length;
    return {improving: imp, stable: stb, total: this._b5aMetricsDashboard.length};
  }

  private _getb5aArchScore(): {total: number; passed: number; failed: number; partial: number; score: number} {
    const p = this._b5aArchReview.filter(c => c.status === 'pass').length;
    const f = this._b5aArchReview.filter(c => c.status === 'fail').length;
    const pa = this._b5aArchReview.filter(c => c.status === 'partial').length;
    return {total: this._b5aArchReview.length, passed: p, failed: f, partial: pa, score: Math.round((p / this._b5aArchReview.length) * 100)};
  }


  private _zb5aIncidentScenarios = [
    {id: 'zb5a-IS-001', name: 'Ransomware Outbreak in Production Environment', category: 'Malware', likelihood: 'Medium', impact: 'Critical', riskScore: 15, playbooks: ['IR-Ransomware-Isolate', 'IR-Ransomware-Eradicate', 'IR-Ransomway-Recover'], estimatedMTTD: '15min', estimatedMTTR: '4h', involvedTeams: ['SOC', 'IR', 'Infrastructure', 'Legal', 'Comms'], keyIndicators: ['Mass file encryption events', 'Ransom note deployment', 'Lateral movement via SMB/RDP', 'Backup deletion attempts'], mitigationControls: ['EDR behavioral detection', 'Network segmentation', 'Immutable backups', 'Email filtering'], lessonsLearned: 'Ensure offline backups are tested quarterly and Ransomware-specific playbooks are updated with latest threat intelligence', lastDrillDate: '2026-03-15', nextDrillDate: '2026-06-15'},
    {id: 'zb5a-IS-002', name: 'Cloud Misconfiguration Leading to Data Exposure', category: 'Cloud', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Cloud-Misconfig-Assess', 'IR-Cloud-Misconfig-Remediate', 'IR-Cloud-Misconfig-Notify'], estimatedMTTD: '2h', estimatedMTTR: '6h', involvedTeams: ['Cloud Security', 'SOC', 'Data Protection', 'Legal'], keyIndicators: ['Public S3 bucket exposure', 'Overly permissive IAM policies', 'Unencrypted database snapshots', 'Public API endpoint discovery'], mitigationControls: ['CSPM monitoring', 'IAM policy analyzer', 'Automated remediation', 'Cloud audit logging'], lessonsLearned: 'Implement CSPM with auto-remediation for common misconfigurations and conduct monthly cloud configuration reviews', lastDrillDate: '2026-02-20', nextDrillDate: '2026-05-20'},
    {id: 'zb5a-IS-003', name: 'Insider Threat Data Exfiltration', category: 'Insider', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-Insider-Investigate', 'IR-Insider-Contain', 'IR-Insider-Escalate'], estimatedMTTD: '8h', estimatedMTTR: '24h', involvedTeams: ['SOC', 'IR', 'HR', 'Legal', 'IAM'], keyIndicators: ['Anomalous data access patterns', 'Large file downloads to USB or cloud', 'Off-hours access to sensitive systems', 'Accessing data outside normal job scope'], mitigationControls: ['UEBA analytics', 'DLP policies', 'PAM controls', 'Access reviews'], lessonsLearned: 'Strengthen UEBA baselines and implement automated alerts for data access pattern anomalies', lastDrillDate: '2026-01-10', nextDrillDate: '2026-04-10'},
    {id: 'zb5a-IS-004', name: 'Supply Chain Attack via Compromised Vendor Software', category: 'Supply Chain', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-SupplyChain-Detect', 'IR-SupplyChain-Isolate', 'IR-SupplyChain-Remediate'], estimatedMTTD: '24h', estimatedMTTR: '72h', involvedTeams: ['SOC', 'IR', 'Procurement', 'Vendor Mgmt', 'Legal'], keyIndicators: ['Unexpected network connections from vendor software', 'Modified binaries or dependencies', 'Anomalous behavior from trusted applications', 'SBOM discrepancy alerts'], mitigationControls: ['SCA scanning', 'SBOM monitoring', 'Vendor security assessments', 'Network segmentation'], lessonsLearned: 'Require SBOM from all vendors and implement continuous monitoring of third-party component behavior', lastDrillDate: '2026-03-01', nextDrillDate: '2026-06-01'},
    {id: 'zb5a-IS-005', name: 'Credential Stuffing Attack on Customer Portal', category: 'External Attack', likelihood: 'High', impact: 'Medium', riskScore: 12, playbooks: ['IR-CredentialStuffing-Detect', 'IR-CredentialStuffing-Block', 'IR-CredentialStuffing-Notify'], estimatedMTTD: '30min', estimatedMTTR: '2h', involvedTeams: ['SOC', 'AppSec', 'Customer Support', 'Comms'], keyIndicators: ['High volume of failed login attempts', 'Login attempts from known compromised credential lists', 'Unusual geographic distribution of login attempts', 'Account takeover indicators'], mitigationControls: ['WAF rate limiting', 'CAPTCHA on login', 'MFA enforcement', 'Credential monitoring service'], lessonsLearned: 'Deploy advanced bot detection on login endpoints and implement proactive credential monitoring for customer accounts', lastDrillDate: '2026-04-01', nextDrillDate: '2026-07-01'},
    {id: 'zb5a-IS-006', name: 'Phishing Campaign Targeting Executive Team', category: 'Social Engineering', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Phishing-Contain', 'IR-Phishing-Investigate', 'IR-Phishing-Notify'], estimatedMTTD: '1h', estimatedMTTR: '4h', involvedTeams: ['SOC', 'Awareness', 'Executive Office', 'IT'], keyIndicators: ['Sophisticated spear-phishing emails to C-suite', 'Business email compromise indicators', 'Redirected MFA approval requests', 'Unauthorized wire transfer attempts'], mitigationControls: ['AI email filtering', 'Executive awareness training', 'Out-of-band verification policy', 'DMARC/DKIM/SPF'], lessonsLearned: 'Implement additional email security controls for executive mailboxes and conduct monthly spear-phishing simulations', lastDrillDate: '2026-02-15', nextDrillDate: '2026-05-15'}
  ];

  private _zb5aSecurityFrameworks = [
  ...
  private _getzb5aScenarioSummary(): {total: number; critical: number; high: number; drilled: number; avgRiskScore: number} {
    const crit = this._zb5aIncidentScenarios.filter(s => s.impact === 'Critical').length;
    const high = this._zb5aIncidentScenarios.filter(s => s.impact === 'High').length;
    const avg = this._zb5aIncidentScenarios.reduce((s, x) => s + x.riskScore, 0) / this._zb5aIncidentScenarios.length;
    return {total: this._zb5aIncidentScenarios.length, critical: crit, high: high, drilled: this._zb5aIncidentScenarios.length, avgRiskScore: Math.round(avg)};
  }

  private _getzb5aVendorRiskSummary(): {total: number; approved: number; conditional: number; remediation: number; avgScore: number} {
    const approved = this._zb5aThirdPartyRiskData.filter(v => v.status === 'approved').length;
    const cond = this._zb5aThirdPartyRiskData.filter(v => v.status === 'conditional').length;
    const rem = this._zb5aThirdPartyRiskData.filter(v => v.status === 'remediation_required').length;
    const avg = this._zb5aThirdPartyRiskData.reduce((s, v) => s + v.riskScore, 0) / this._zb5aThirdPartyRiskData.length;
    return {total: this._zb5aThirdPartyRiskData.length, approved, conditional: cond, remediation: rem, avgScore: Math.round(avg)};
  }

  private _getzb5aTrainingSummary(): {total: number; avgCompletion: number; avgScore: number; overdueUpdates: number} {
    const avgComp = this._zb5aSecurityTrainingCatalog.reduce((s, t) => s + t.completionRate, 0) / this._zb5aSecurityTrainingCatalog.length;
    const avgScore = this._zb5aSecurityTrainingCatalog.filter(t => t.avgScore > 0).reduce((s, t) => s + t.avgScore, 0) / this._zb5aSecurityTrainingCatalog.filter(t => t.avgScore > 0).length;
    return {total: this._zb5aSecurityTrainingCatalog.length, avgCompletion: Math.round(avgComp * 10) / 10, avgScore: Math.round(avgScore), overdueUpdates: 0};
  }

  private _getzb5aToolHealthSummary(): {total: number; healthy: number; degraded: number; annualCost: string} {
    const healthy = this._zb5aSecurityToolInventory.filter(t => t.health === 'healthy').length;
    const degraded = this._zb5aSecurityToolInventory.filter(t => t.health === 'degraded').length;
    const cost = this._zb5aSecurityToolInventory.reduce((s, t) => s + parseInt(t.annualCost.replace(/[^0-9]/g, '')), 0);
    return {total: this._zb5aSecurityToolInventory.length, healthy, degraded, annualCost: '$' + (cost / 1000).toFixed(0) + 'K'};
  }


  private _xb5aSecurityZones = [
    {zoneId: 'xb5a-Z-001', name: 'Internet DMZ', trustLevel: 'Untrusted', subnet: '10.0.0.0/24', segmentation: 'Dual-firewall', firewallRules: 45, monitoring: 'Full packet capture', assets: ['Web servers', 'Load balancers', 'WAF appliances', 'Reverse proxy'], riskLevel: 'high' as const, lastAssessment: '2026-04-15', nextAssessment: '2026-07-15'},
    {zoneId: 'xb5a-Z-002', name: 'Corporate Network', trustLevel: 'Trusted', subnet: '10.1.0.0/16', segmentation: 'VLAN + micro-seg', firewallRules: 128, monitoring: 'NetFlow + IDS', assets: ['Workstations', 'Printers', 'Meeting room systems', 'VoIP phones'], riskLevel: 'medium' as const, lastAssessment: '2026-04-10', nextAssessment: '2026-07-10'},
    {zoneId: 'xb5a-Z-003', name: 'Production Data Center', trustLevel: 'Restricted', subnet: '10.2.0.0/16', segmentation: 'Full micro-segmentation', firewallRules: 256, monitoring: 'Full packet capture + EDR', assets: ['Application servers', 'Database clusters', 'Message queues', 'API gateways'], riskLevel: 'low' as const, lastAssessment: '2026-04-12', nextAssessment: '2026-07-12'},
    {zoneId: 'xb5a-Z-004', name: 'Management Network', trustLevel: 'Restricted', subnet: '10.3.0.0/24', segmentation: 'Isolated VLAN', firewallRules: 32, monitoring: 'Syslog + SNMP', assets: ['Hypervisors', 'Switch management', 'Storage controllers', 'Backup appliances'], riskLevel: 'low' as const, lastAssessment: '2026-04-08', nextAssessment: '2026-07-08'},
    {zoneId: 'xb5a-Z-005', name: 'Cloud Production', trustLevel: 'Restricted', subnet: 'VPC: 172.16.0.0/12', segmentation: 'Security groups + NACLs', firewallRules: 189, monitoring: 'VPC Flow Logs + GuardDuty', assets: ['Kubernetes clusters', 'Managed databases', 'Object storage', 'Serverless functions'], riskLevel: 'medium' as const, lastAssessment: '2026-04-18', nextAssessment: '2026-07-18'}
  ];

  private _xb5aSecurityAlerts = [
    {id: 'xb5a-AL-001', severity: 'critical', title: 'Active ransomware encryption detected on PROD-DB-01', source: 'CrowdStrike', timestamp: '2026-04-22T13:45:00Z', status: 'investigating', assignee: 'SOC Analyst J. Smith', asset: 'PROD-DB-01 (10.2.1.15)', mitreTechnique: 'T1486 - Data Encrypted for Impact', playbook: 'IR-Ransomware-Isolate', sla: '15 minutes', elapsed: '12 minutes', notes: 'EDR detected mass file encryption event pattern. Automated isolation triggered. IR team paged.'},
    {id: 'xb5a-AL-002', severity: 'high', title: 'Impossible travel detected for user admin@company.com', source: 'Okta', timestamp: '2026-04-22T12:30:00Z', status: 'triaged', assignee: 'SOC Analyst M. Johnson', asset: 'Okta - admin@company.com', mitreTechnique: 'T1078 - Valid Accounts', playbook: 'IR-ImpossibleTravel', sla: '30 minutes', elapsed: '87 minutes', notes: 'Login from Tokyo followed by login from New York within 30 minutes. Account temporarily locked. User confirmed via out-of-band channel they are in New York.'},
    {id: 'xb5a-AL-003', severity: 'high', title: 'Public S3 bucket discovered with customer PII exposure', source: 'Prisma Cloud', timestamp: '2026-04-22T11:15:00Z', status: 'remediating', assignee: 'Cloud Security Lead A. Chen', asset: 'AWS S3: customer-data-archive-2024', mitreTechnique: 'N/A - Misconfiguration', playbook: 'IR-Cloud-Misconfig-Remediate', sla: '1 hour', elapsed: '2.5 hours', notes: 'Bucket containing archived customer records was publicly accessible. Bucket blocked and encrypted. Data exposure assessment in progress.'},
    {id: 'xb5a-AL-004', severity: 'medium', title: 'Anomalous data download by user jane.developer@company.com', source: 'Microsoft DLP', timestamp: '2026-04-22T10:00:00Z', status: 'investigating', assignee: 'SOC Analyst R. Davis', asset: 'Endpoint: WS-DEV-042', mitreTechnique: 'T1567 - Exfil Over Web Service', playbook: 'IR-DLP-Investigation', sla: '4 hours', elapsed: '3.5 hours', notes: 'User downloaded 4.2GB of source code files to personal Google Drive. Interview scheduled with user and manager.'},
    {id: 'xb5a-AL-005', severity: 'medium', title: 'Credential stuffing attack detected on customer portal', source: 'Cloudflare WAF', timestamp: '2026-04-22T09:30:00Z', status: 'contained', assignee: 'SOC Analyst K. Wilson', asset: 'Customer Portal (portal.company.com)', mitreTechnique: 'T1110 - Brute Force', playbook: 'IR-CredentialStuffing-Block', sla: '2 hours', elapsed: '4 hours', notes: 'Rate limiting and CAPTCHA automatically deployed. 12,000 failed login attempts from 800 unique IPs blocked. No successful account takeovers confirmed.'},
    {id: 'xb5a-AL-006', severity: 'low', title: 'SSL certificate expiring in 14 days for api.company.com', source: 'Certificate monitoring', timestamp: '2026-04-22T08:00:00Z', status: 'remediated', assignee: 'Infrastructure Lead T. Brown', asset: 'api.company.com', mitreTechnique: 'N/A', playbook: 'Certificate Renewal', sla: '30 days', elapsed: '16 days', notes: 'Automated renewal triggered via ACME. New certificate deployed successfully. No service interruption.'}
  ];

  private _xb5aComplianceFrameworks = [
  ...
  private _xb5aDataClassificationPolicy = [
  ...
  private _getxb5aZoneSummary(): {total: number; highRisk: number; mediumRisk: number; lowRisk: number; totalRules: number} {
    const high = this._xb5aSecurityZones.filter(z => z.riskLevel === 'high').length;
    const med = this._xb5aSecurityZones.filter(z => z.riskLevel === 'medium').length;
    const low = this._xb5aSecurityZones.filter(z => z.riskLevel === 'low').length;
    const rules = this._xb5aSecurityZones.reduce((s, z) => s + z.firewallRules, 0);
    return {total: this._xb5aSecurityZones.length, highRisk: high, mediumRisk: med, lowRisk: low, totalRules: rules};
  }

  private _getxb5aAlertSummary(): {total: number; critical: number; high: number; medium: number; low: number; open: number; contained: number} {
    const crit = this._xb5aSecurityAlerts.filter(a => a.severity === 'critical').length;
    const high = this._xb5aSecurityAlerts.filter(a => a.severity === 'high').length;
    const med = this._xb5aSecurityAlerts.filter(a => a.severity === 'medium').length;
    const low = this._xb5aSecurityAlerts.filter(a => a.severity === 'low').length;
    const open = this._xb5aSecurityAlerts.filter(a => a.status === 'investigating' || a.status === 'triaged').length;
    const contained = this._xb5aSecurityAlerts.filter(a => a.status === 'contained' || a.status === 'remediated').length;
    return {total: this._xb5aSecurityAlerts.length, critical: crit, high, medium: med, low, open, contained};
  }

  private _getxb5aComplianceSummary(): {total: number; fullyCompliant: number; mostlyCompliant: number; avgComplianceRate: number} {
    const full = this._xb5aComplianceFrameworks.filter(f => f.status === 'compliant').length;
    const mostly = this._xb5aComplianceFrameworks.filter(f => f.status === 'mostly_compliant').length;
    const avgRate = this._xb5aComplianceFrameworks.reduce((s, f) => s + (f.compliant / f.requirements * 100), 0) / this._xb5aComplianceFrameworks.length;
    return {total: this._xb5aComplianceFrameworks.length, fullyCompliant: full, mostlyCompliant: mostly, avgComplianceRate: Math.round(avgRate * 10) / 10};
  }
  ...
  // === Security Risk Register (threat_hunti) ===
  private _threat_huntiRiskRegister = [
    { id: "RSK-THR-0001", title: "Data breach from unpatched systems", owner: "CTO", category: "Financial", status: "mitigating", trend: "stable", severity: "high", likelihood: 3, impact: 4, treatment: "Monitor and review quarterly", lastReview: "2026-04-21", nextReview: "2026-07-05" },
    { id: "RSK-THR-0002", title: "Ransomware attack on critical infrastructure", owner: "CRO", category: "Strategic", status: "closed", trend: "decreasing", severity: "high", likelihood: 3, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-06", nextReview: "2026-07-10" },
    { id: "RSK-THR-0003", title: "Insider threat data exfiltration", owner: "VP Eng", category: "Compliance", status: "accepted", trend: "increasing", severity: "high", likelihood: 10, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-17", nextReview: "2026-07-04" },
    { id: "RSK-THR-0004", title: "Supply chain compromise", owner: "Security Lead", category: "Technical", status: "open", trend: "stable", severity: "medium", likelihood: 8, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-01", nextReview: "2026-07-11" },
    { id: "RSK-THR-0005", title: "Cloud misconfiguration exposure", owner: "Risk Mgr", category: "Reputational", status: "mitigating", trend: "decreasing", severity: "high", likelihood: 8, impact: 4, treatment: "Monitor and review quarterly", lastReview: "2026-04-11", nextReview: "2026-07-25" },
    { id: "RSK-THR-0006", title: "Phishing campaign success rate", owner: "Compliance Dir", category: "Legal", status: "closed", trend: "increasing", severity: "critical", likelihood: 5, impact: 4, treatment: "Monitor and review quarterly", lastReview: "2026-04-01", nextReview: "2026-07-06" },
    { id: "RSK-THR-0007", title: "Third-party vendor data breach", owner: "IT Dir", category: "Regulatory", status: "accepted", trend: "stable", severity: "medium", likelihood: 9, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-20", nextReview: "2026-07-12" },
    { id: "RSK-THR-0008", title: "Regulatory non-compliance penalty", owner: "DevOps Lead", category: "Third-Party", status: "open", trend: "decreasing", severity: "medium", likelihood: 10, impact: 4, treatment: "Monitor and review quarterly", lastReview: "2026-04-09", nextReview: "2026-07-16" },
    { id: "RSK-THR-0009", title: "Zero-day exploit in production", owner: "Architect", category: "Human Capital", status: "mitigating", trend: "increasing", severity: "medium", likelihood: 1, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-11", nextReview: "2026-07-12" },
    { id: "RSK-THR-0010", title: "Insufficient access controls", owner: "CISO", category: "Operational", status: "closed", trend: "stable", severity: "critical", likelihood: 3, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-22", nextReview: "2026-07-01" },
    { id: "RSK-THR-0011", title: "DDoS attack on services", owner: "CTO", category: "Financial", status: "accepted", trend: "decreasing", severity: "low", likelihood: 5, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-09", nextReview: "2026-07-06" },
    { id: "RSK-THR-0012", title: "Social engineering attack", owner: "CRO", category: "Strategic", status: "open", trend: "increasing", severity: "high", likelihood: 1, impact: 9, treatment: "Monitor and review quarterly", lastReview: "2026-04-12", nextReview: "2026-07-07" },
    { id: "RSK-THR-0013", title: "API security vulnerability", owner: "VP Eng", category: "Compliance", status: "mitigating", trend: "stable", severity: "high", likelihood: 1, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-03", nextReview: "2026-07-25" },
    { id: "RSK-THR-0014", title: "Mobile device compromise", owner: "Security Lead", category: "Technical", status: "closed", trend: "decreasing", severity: "medium", likelihood: 3, impact: 5, treatment: "Monitor and review quarterly", lastReview: "2026-04-15", nextReview: "2026-07-07" },
    { id: "RSK-THR-0015", title: "Physical security breach", owner: "Risk Mgr", category: "Reputational", status: "accepted", trend: "increasing", severity: "low", likelihood: 9, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-23", nextReview: "2026-07-16" },
    { id: "RSK-THR-0016", title: "Password policy weakness", owner: "Compliance Dir", category: "Legal", status: "open", trend: "stable", severity: "medium", likelihood: 10, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-18", nextReview: "2026-07-18" },
    { id: "RSK-THR-0017", title: "Network segmentation gap", owner: "IT Dir", category: "Regulatory", status: "mitigating", trend: "decreasing", severity: "critical", likelihood: 10, impact: 5, treatment: "Monitor and review quarterly", lastReview: "2026-04-16", nextReview: "2026-07-01" },
    { id: "RSK-THR-0018", title: "Encryption key management failure", owner: "DevOps Lead", category: "Third-Party", status: "closed", trend: "increasing", severity: "critical", likelihood: 7, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-09", nextReview: "2026-07-06" },
    { id: "RSK-THR-0019", title: "Audit trail tampering", owner: "Architect", category: "Human Capital", status: "accepted", trend: "stable", severity: "critical", likelihood: 1, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-23", nextReview: "2026-07-28" },
    { id: "RSK-THR-0020", title: "Business email compromise", owner: "CISO", category: "Operational", status: "open", trend: "decreasing", severity: "medium", likelihood: 2, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-05", nextReview: "2026-07-18" },
  ];
  private _threat_huntiRiskFilter: string = 'all';
  private _threat_huntiRiskSeverity: string = 'all';
  private _threat_huntiRiskStatus: string = 'all';
  private _threat_huntiExpandedRisk: string = '';
  private _threat_huntiFilterRisks() {
    const reg = this._threat_huntiRiskRegister;
    return reg.filter(r => {
      if (this._threat_huntiRiskFilter !== "all" && r.category !== this._threat_huntiRiskFilter) return false;
      if (this._threat_huntiRiskSeverity !== "all" && r.severity !== this._threat_huntiRiskSeverity) return false;
      if (this._threat_huntiRiskStatus !== "all" && r.status !== this._threat_huntiRiskStatus) return false;
      return true;
    });
  }
  private _threat_huntiGetRiskScore(r: any) { return Math.round(r.likelihood * r.impact * 1.5); }
  private _threat_huntiGetRiskTrendIcon(trend: string) {
    if (trend === "increasing") return "\u2191";
    if (trend === "decreasing") return "\u2193";
    return "\u2192";
  }
  private _threat_huntiGetRiskColor(sev: string) {
    if (sev === "critical") return "#dc2626";
    if (sev === "high") return "#ea580c";
    if (sev === "medium") return "#d97706";
    return "#16a34a";
  }
  private _threat_huntiGetRiskCounts() {
    const reg = this._threat_huntiRiskRegister;
  ...
  private _threat_huntiGetTreatmentProgress() {
    const reg = this._threat_huntiRiskRegister;
    const treated = reg.filter(r => r.status === "mitigating" || r.status === "closed").length;
    return Math.round((treated / Math.max(reg.length, 1)) * 100);
  }

  // === Security Metrics API Gateway (threat_hunti) ===
  private _threat_huntiApiEndpoints = [
    { method: "GET", path: "/api/v1/threats", name: "Threat Intelligence Feed", status: "active", avgLatency: 377.1, reqPerMin: 40.0, errorRate: 0.77, uptime: round(98.72935067572693,2), version: "v3.5.14" },
    { method: "POST", path: "/api/v1/scans/start", name: "Vulnerability Scanner", status: "active", avgLatency: 257.8, reqPerMin: 2041.0, errorRate: 1.33, uptime: round(95.01965670918253,2), version: "v2.5.0" },
    { method: "GET", path: "/api/v1/assets", name: "Asset Inventory", status: "active", avgLatency: 384.1, reqPerMin: 1122.0, errorRate: 0.23, uptime: round(99.6713583258446,2), version: "v3.5.15" },
    { method: "POST", path: "/api/v1/alerts", name: "Alert Management", status: "active", avgLatency: 180.9, reqPerMin: 975.0, errorRate: 0.69, uptime: round(99.52578440262207,2), version: "v1.7.18" },
    { method: "GET", path: "/api/v1/compliance", name: "Compliance Status", status: "active", avgLatency: 75.1, reqPerMin: 1725.0, errorRate: 1.78, uptime: round(96.65815897252514,2), version: "v2.4.9" },
    { method: "PUT", path: "/api/v1/policies", name: "Policy Engine", status: "active", avgLatency: 384.6, reqPerMin: 180.0, errorRate: 0.11, uptime: round(98.34434083617653,2), version: "v1.7.14" },
    { method: "GET", path: "/api/v1/incidents", name: "Incident Tracker", status: "active", avgLatency: 408.9, reqPerMin: 585.0, errorRate: 0.66, uptime: round(96.8067619119801,2), version: "v3.6.13" },
    { method: "POST", path: "/api/v1/forensics", name: "Forensics Collector", status: "degraded", avgLatency: 375.3, reqPerMin: 153.0, errorRate: 0.56, uptime: round(96.49330065608699,2), version: "v2.5.10" },
    { method: "GET", path: "/api/v1/risk/assess", name: "Risk Assessment", status: "active", avgLatency: 88.7, reqPerMin: 817.0, errorRate: 1.81, uptime: round(99.77949908957049,2), version: "v2.6.2" },
    { method: "POST", path: "/api/v1/auth/verify", name: "Authentication", status: "active", avgLatency: 58.0, reqPerMin: 759.0, errorRate: 0.13, uptime: round(96.77140422235833,2), version: "v3.4.3" },
    { method: "GET", path: "/api/v1/logs/audit", name: "Audit Log Query", status: "active", avgLatency: 279.0, reqPerMin: 2044.0, errorRate: 0.11, uptime: round(99.81819797442272,2), version: "v1.0.7" },
    { method: "PUT", path: "/api/v1/users/roles", name: "Role Management", status: "active", avgLatency: 374.8, reqPerMin: 498.0, errorRate: 1.6, uptime: round(99.69008288640197,2), version: "v3.8.11" },
    { method: "POST", path: "/api/v1/encrypt", name: "Encryption Service", status: "active", avgLatency: 85.1, reqPerMin: 445.0, errorRate: 2.38, uptime: round(97.22112254268815,2), version: "v2.8.20" },
    { method: "GET", path: "/api/v1/network/topo", name: "Network Topology", status: "maintenance", avgLatency: 213.9, reqPerMin: 1633.0, errorRate: 0.8, uptime: round(99.90624429554033,2), version: "v2.9.6" },
    { method: "DELETE", path: "/api/v1/sessions", name: "Session Manager", status: "active", avgLatency: 371.9, reqPerMin: 112.0, errorRate: 2.01, uptime: round(95.61549982792725,2), version: "v1.3.10" },
  ];
  private _threat_huntiApiKeys = [
    { id: "ak-000001", name: "key-thre-001", created: "2026-07-14", lastUsed: "2026-04-13", status: "active", calls: 15108, rateLimit: 5000 },
    { id: "ak-000002", name: "key-thre-002", created: "2026-06-04", lastUsed: "2026-04-05", status: "revoked", calls: 5831, rateLimit: 5000 },
    { id: "ak-000003", name: "key-thre-003", created: "2026-04-23", lastUsed: "2026-04-16", status: "active", calls: 32822, rateLimit: 1000 },
    { id: "ak-000004", name: "key-thre-004", created: "2026-04-19", lastUsed: "2026-04-21", status: "active", calls: 985, rateLimit: 500 },
    { id: "ak-000005", name: "key-thre-005", created: "2026-01-06", lastUsed: "2026-04-19", status: "active", calls: 43534, rateLimit: 500 },
    { id: "ak-000006", name: "key-thre-006", created: "2026-05-07", lastUsed: "2026-04-18", status: "active", calls: 32046, rateLimit: 5000 },
    { id: "ak-000007", name: "key-thre-007", created: "2026-04-22", lastUsed: "2026-04-21", status: "active", calls: 49808, rateLimit: 5000 },
    { id: "ak-000008", name: "key-thre-008", created: "2026-01-14", lastUsed: "2026-04-22", status: "active", calls: 2472, rateLimit: 100 },
  ];
  private _threat_huntiApiHealthSummary() {
    const eps = this._threat_huntiApiEndpoints;
  ...
  private _threat_huntiGetApiByMethod(method: string) { return this._threat_huntiApiEndpoints.filter(e=>e.method===method); }
  private _threat_huntiGetSlowEndpoints() { return this._threat_huntiApiEndpoints.filter(e=>e.avgLatency>200).sort((a,b)=>b.avgLatency-a.avgLatency); }
  private _threat_huntiGetHighErrorEndpoints() { return this._threat_huntiApiEndpoints.filter(e=>e.errorRate>1.0).sort((a,b)=>b.errorRate-a.errorRate); }

  // === Security Training Effectiveness (threat_hunti) ===
  private _threat_huntiTrainingModules = [
    { id: "TRN-001", name: "Security Fundamentals", completionRate: 90.1, avgScore: 55.7, behaviorChange: 67.4, enrolled: 70, completed: 205, duration: "58min", category: "mandatory" },
    { id: "TRN-002", name: "Phishing Awareness", completionRate: 45.6, avgScore: 85.8, behaviorChange: 81.8, enrolled: 252, completed: 440, duration: "37min", category: "mandatory" },
    { id: "TRN-003", name: "Social Engineering Defense", completionRate: 45.5, avgScore: 68.8, behaviorChange: 64.0, enrolled: 328, completed: 267, duration: "107min", category: "optional" },
    { id: "TRN-004", name: "Password Hygiene", completionRate: 69.9, avgScore: 58.7, behaviorChange: 50.1, enrolled: 103, completed: 57, duration: "113min", category: "mandatory" },
    { id: "TRN-005", name: "Data Classification", completionRate: 76.9, avgScore: 80.7, behaviorChange: 49.2, enrolled: 255, completed: 23, duration: "15min", category: "mandatory" },
    { id: "TRN-006", name: "Incident Response Basics", completionRate: 51.2, avgScore: 89.8, behaviorChange: 85.7, enrolled: 133, completed: 143, duration: "82min", category: "mandatory" },
    { id: "TRN-007", name: "Secure Coding", completionRate: 49.2, avgScore: 65.3, behaviorChange: 89.3, enrolled: 67, completed: 238, duration: "21min", category: "optional" },
    { id: "TRN-008", name: "Cloud Security", completionRate: 79.8, avgScore: 81.4, behaviorChange: 48.5, enrolled: 334, completed: 230, duration: "82min", category: "mandatory" },
    { id: "TRN-009", name: "Mobile Device Security", completionRate: 82.4, avgScore: 65.8, behaviorChange: 61.5, enrolled: 264, completed: 166, duration: "94min", category: "mandatory" },
    { id: "TRN-010", name: "Network Security", completionRate: 57.3, avgScore: 60.3, behaviorChange: 79.6, enrolled: 148, completed: 133, duration: "112min", category: "mandatory" },
    { id: "TRN-011", name: "Physical Security", completionRate: 79.5, avgScore: 89.5, behaviorChange: 55.1, enrolled: 88, completed: 280, duration: "98min", category: "optional" },
    { id: "TRN-012", name: "Regulatory Compliance", completionRate: 47.2, avgScore: 57.3, behaviorChange: 70.5, enrolled: 263, completed: 381, duration: "55min", category: "mandatory" },
    { id: "TRN-013", name: "Risk Management", completionRate: 76.1, avgScore: 68.4, behaviorChange: 63.5, enrolled: 230, completed: 196, duration: "61min", category: "mandatory" },
    { id: "TRN-014", name: "Cryptography Basics", completionRate: 95.3, avgScore: 85.7, behaviorChange: 85.5, enrolled: 494, completed: 296, duration: "86min", category: "mandatory" },
    { id: "TRN-015", name: "Access Control", completionRate: 72.8, avgScore: 83.4, behaviorChange: 84.0, enrolled: 436, completed: 130, duration: "48min", category: "optional" },
    { id: "TRN-016", name: "Vendor Management", completionRate: 83.5, avgScore: 63.2, behaviorChange: 55.3, enrolled: 444, completed: 427, duration: "79min", category: "mandatory" },
  ];
  private _threat_huntiPhishingResults = [
    { month: "2026-01", sent: 561, clicked: 12, reported: 154, clickRate: round(2.13903743315508,1), reportRate: round(27.450980392156865,1) },
    { month: "2026-02", sent: 905, clicked: 203, reported: 137, clickRate: round(22.430939226519335,1), reportRate: round(15.138121546961326,1) },
    { month: "2026-03", sent: 729, clicked: 199, reported: 379, clickRate: round(27.29766803840878,1), reportRate: round(51.98902606310014,1) },
    { month: "2026-04", sent: 282, clicked: 67, reported: 28, clickRate: round(23.75886524822695,1), reportRate: round(9.929078014184398,1) },
    { month: "2026-05", sent: 667, clicked: 193, reported: 122, clickRate: round(28.93553223388306,1), reportRate: round(18.290854572713645,1) },
    { month: "2026-06", sent: 372, clicked: 7, reported: 117, clickRate: round(1.881720430107527,1), reportRate: round(31.451612903225808,1) },
  ];
  private _threat_huntiTrainingROI = { totalInvestment: 145783, avgCostPerEmployee: 457, riskReductionPct: round(random.uniform(15,45),1), incidentReductionPct: round(random.uniform(10,35),1), complianceScoreGain: round(random.uniform(5,25),1) };
  private _threat_huntiLearningPaths = [
    { name: "Beginner Security Analyst", totalModules: 14, completedModules: 17, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 67, enrolled: 24 },
    { name: "Advanced Threat Hunter", totalModules: 15, completedModules: 4, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 16, enrolled: 194 },
    { name: "Security Architect", totalModules: 12, completedModules: 4, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 65, enrolled: 196 },
    { name: "Incident Responder", totalModules: 8, completedModules: 17, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 62, enrolled: 115 },
    { name: "Compliance Specialist", totalModules: 16, completedModules: 16, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 56, enrolled: 171 },
    { name: "DevSecOps Engineer", totalModules: 20, completedModules: 16, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 66, enrolled: 111 },
  ];
  private _threat_huntiGetOverallCompletion() {
    const mods = this._threat_huntiTrainingModules;
    return round(mods.reduce((s,m)=>s+m.completionRate,0)/mods.length,1);
  }
  private _threat_huntiGetTopPerformers() { return [...this._threat_huntiTrainingModules].sort((a,b)=>b.avgScore-a.avgScore).slice(0,5); }
  private _threat_huntiGetModulesNeedingAttention() { return this._threat_huntiTrainingModules.filter(m=>m.completionRate<70||m.avgScore<65); }

  // === Security Governance Framework (threat_hunti) ===
  private _threat_huntiGovBodies = [
    { name: "Security Steering Committee", members: 9, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-03", nextMeeting: "2026-05-18", quorum: 5 },
    { name: "Risk Management Board", members: 5, chair: "CTO", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-03", nextMeeting: "2026-05-10", quorum: 8 },
    { name: "Data Governance Council", members: 6, chair: "CRO", meetingFreq: "Monthly", lastMeeting: "2026-04-08", nextMeeting: "2026-05-27", quorum: 4 },
    { name: "Compliance Oversight Board", members: 6, chair: "CDO", meetingFreq: "Monthly", lastMeeting: "2026-04-21", nextMeeting: "2026-05-08", quorum: 6 },
    { name: "Architecture Review Board", members: 15, chair: "VP Eng", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-06", nextMeeting: "2026-05-18", quorum: 3 },
    { name: "Change Advisory Board", members: 11, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-16", nextMeeting: "2026-05-04", quorum: 5 },
    { name: "Incident Review Board", members: 10, chair: "CIO", meetingFreq: "As needed", lastMeeting: "2026-04-17", nextMeeting: "2026-05-25", quorum: 7 },
    { name: "Vendor Risk Committee", members: 15, chair: "CFO", meetingFreq: "Quarterly", lastMeeting: "2026-04-10", nextMeeting: "2026-05-06", quorum: 3 },
  ];
  private _threat_huntiDecisions = [
    { id: "DEC-0001", title: "Adopt zero-trust architecture framework", date: "2026-02-13", status: "approved", owner: "CISO" },
    { id: "DEC-0002", title: "Migrate to SIEM 2.0 platform", date: "2026-04-18", status: "implemented", owner: "CTO" },
    { id: "DEC-0003", title: "Implement DLP across all endpoints", date: "2026-01-22", status: "in-progress", owner: "Security Lead" },
    { id: "DEC-0004", title: "Mandate MFA for all external access", date: "2026-03-09", status: "pending", owner: "CRO" },
    { id: "DEC-0005", title: "Establish bug bounty program", date: "2026-02-05", status: "approved", owner: "VP Eng" },
    { id: "DEC-0006", title: "Deploy EDR solution enterprise-wide", date: "2026-01-01", status: "approved", owner: "CISO" },
    { id: "DEC-0007", title: "Conduct annual penetration testing", date: "2026-04-07", status: "implemented", owner: "CTO" },
    { id: "DEC-0008", title: "Implement network micro-segmentation", date: "2026-01-03", status: "in-progress", owner: "Security Lead" },
    { id: "DEC-0009", title: "Establish security champion program", date: "2026-01-15", status: "pending", owner: "CRO" },
    { id: "DEC-0010", title: "Migrate to passwordless authentication", date: "2026-04-08", status: "approved", owner: "VP Eng" },
  ];
  private _threat_huntiPolicyLifecycle = [
    { name: "Information Security Policy", version: "v4.4", status: "active", lastUpdated: "2026-02-10", nextReview: "2026-8-18", owner: "CISO" },
    { name: "Acceptable Use Policy", version: "v3.2", status: "active", lastUpdated: "2026-02-28", nextReview: "2026-9-12", owner: "Legal" },
    { name: "Data Retention Policy", version: "v2.4", status: "under-review", lastUpdated: "2026-01-16", nextReview: "2026-11-19", owner: "DPO" },
    { name: "Access Control Policy", version: "v1.2", status: "active", lastUpdated: "2026-02-26", nextReview: "2026-9-26", owner: "IAM Lead" },
    { name: "Incident Response Policy", version: "v2.1", status: "draft", lastUpdated: "2026-01-01", nextReview: "2026-7-01", owner: "IR Lead" },
    { name: "Business Continuity Plan", version: "v1.2", status: "active", lastUpdated: "2026-03-15", nextReview: "2026-8-12", owner: "BCP Mgr" },
    { name: "Vendor Management Policy", version: "v3.3", status: "active", lastUpdated: "2026-03-14", nextReview: "2026-8-11", owner: "Procurement" },
    { name: "Encryption Standard", version: "v3.0", status: "under-review", lastUpdated: "2026-02-07", nextReview: "2026-8-24", owner: "Security Arch" },
  ];
  private _threat_huntiGovMaturityScore = { overall: 3.3, riskManagement: 3.0, compliance: 4.6, incidentResponse: 2.7, awareness: 4.7, technology: 2.2 };
  private _threat_huntiGetPendingDecisions() { return this._threat_huntiDecisions.filter(d=>d.status==='pending'||d.status==='in-progress'); }
  private _threat_huntiGetActivePolicies() { return this._threat_huntiPolicyLifecycle.filter(p=>p.status==='active'); }
  private _threat_huntiGetEscalationPath() { return ["L1 Analyst","L2 Senior","Security Lead","CISO","Board"]; }

  // === Security Innovation Lab (threat_hunti) ===
  private _threat_huntiInnoProjects = [
    { id: "INN-001", name: "AI-Powered Threat Detection", description: "Machine learning models for real-time threat identification", status: "active", progress: 72, startDate: "2026-02-05", teamSize: 8, budget: 80004, milestones: 10, completedMilestones: 6 },
    { id: "INN-002", name: "Quantum-Resistant Cryptography", description: "Post-quantum encryption algorithm prototyping", status: "research", progress: 35, startDate: "2026-03-26", teamSize: 4, budget: 98183, milestones: 7, completedMilestones: 3 },
    { id: "INN-003", name: "Automated Red Teaming", description: "Autonomous penetration testing framework", status: "active", progress: 58, startDate: "2026-03-05", teamSize: 3, budget: 103217, milestones: 6, completedMilestones: 1 },
    { id: "INN-004", name: "Zero-Knowledge Authentication", description: "Privacy-preserving identity verification", status: "poc", progress: 88, startDate: "2026-02-05", teamSize: 3, budget: 193527, milestones: 4, completedMilestones: 6 },
    { id: "INN-005", name: "Blockchain Audit Trail", description: "Immutable security event logging", status: "active", progress: 45, startDate: "2026-02-20", teamSize: 5, budget: 96596, milestones: 9, completedMilestones: 5 },
    { id: "INN-006", name: "Behavioral Biometrics", description: "Continuous authentication via user behavior patterns", status: "research", progress: 22, startDate: "2026-01-15", teamSize: 7, budget: 110050, milestones: 9, completedMilestones: 7 },
    { id: "INN-007", name: "Deception Grid 2.0", description: "Advanced honeypot network with adaptive responses", status: "active", progress: 65, startDate: "2026-03-03", teamSize: 4, budget: 21395, milestones: 4, completedMilestones: 9 },
    { id: "INN-008", name: "Secure Enclave Integration", description: "Hardware-backed security for critical workloads", status: "poc", progress: 40, startDate: "2026-01-20", teamSize: 5, budget: 179962, milestones: 8, completedMilestones: 4 },
  ];
  private _threat_huntiTechEvaluations = [
    { name: "Rust for Security Tools", status: "evaluating", score: 8.7, recommendation: "Adopt", vendor: "Open Source" },
    { name: "eBPF for Runtime Detection", status: "completed", score: 6.7, recommendation: "Adopt", vendor: "AWS" },
    { name: "Confidential Computing", status: "planned", score: 5.8, recommendation: "Investigate", vendor: "Azure" },
    { name: "Homomorphic Encryption", status: "evaluating", score: 4.8, recommendation: "Pilot", vendor: "GCP" },
    { name: "SASE Architecture", status: "completed", score: 6.4, recommendation: "Adopt", vendor: "Multiple" },
    { name: "SOAR Platform 3.0", status: "planned", score: 5.5, recommendation: "Monitor", vendor: "Splunk" },
  ];
  private _threat_huntiCollaborationPartners = [
  ...
  private _threat_huntiInnoMetrics = { totalProjects: 8, activeProjects: 4, avgTimeToValue: "89 days", pocSuccessRate: round(random.uniform(55,85),1), researchToProduction: round(random.uniform(20,50),1), innovationIndex: round(random.uniform(6.0,9.5),1) };
  private _threat_huntiGetProjectByStatus(status: string) { return this._threat_huntiInnoProjects.filter(p=>p.status===status); }
  private _threat_huntiGetTopEvaluations() { return [...this._threat_huntiTechEvaluations].sort((a,b)=>b.score-a.score).slice(0,3); }



  // === Compliance Dashboard Extension (threat_hunti) ===
  private _threat_huntiComplianceFrameworks = [
    { name: "SOC 2 Type II", description: "Trust Services Criteria", totalControls: 5, implementedControls: 2, status: "partially-compliant", lastAudit: "2026-01-28", nextAudit: "2026-12-13", evidenceCount: 233 },
    { name: "ISO 27001", description: "Information Security Management", totalControls: 114, implementedControls: 66, status: "partially-compliant", lastAudit: "2026-03-21", nextAudit: "2026-11-15", evidenceCount: 150 },
    { name: "PCI DSS 4.0", description: "Payment Card Industry", totalControls: 12, implementedControls: 11, status: "non-compliant", lastAudit: "2026-02-09", nextAudit: "2026-8-12", evidenceCount: 144 },
    { name: "HIPAA", description: "Health Insurance Portability", totalControls: 18, implementedControls: 14, status: "partially-compliant", lastAudit: "2026-02-23", nextAudit: "2026-11-23", evidenceCount: 143 },
    { name: "GDPR", description: "Data Protection Regulation", totalControls: 99, implementedControls: 77, status: "partially-compliant", lastAudit: "2026-04-17", nextAudit: "2026-11-19", evidenceCount: 323 },
    { name: "NIST CSF 2.0", description: "Cybersecurity Framework", totalControls: 6, implementedControls: 6, status: "non-compliant", lastAudit: "2026-02-22", nextAudit: "2026-12-18", evidenceCount: 198 },
    { name: "FedRAMP", description: "Federal Risk Authorization", totalControls: 15, implementedControls: 7, status: "non-compliant", lastAudit: "2026-03-17", nextAudit: "2026-7-05", evidenceCount: 463 },
    { name: "SOX", description: "Sarbanes-Oxley Compliance", totalControls: 8, implementedControls: 4, status: "compliant", lastAudit: "2026-03-01", nextAudit: "2026-10-21", evidenceCount: 69 },
    { name: "CIS Controls v8", description: "Center for Internet Security", totalControls: 18, implementedControls: 18, status: "partially-compliant", lastAudit: "2026-03-13", nextAudit: "2026-9-22", evidenceCount: 132 },
    { name: "COBIT 2019", description: "IT Governance Framework", totalControls: 40, implementedControls: 35, status: "non-compliant", lastAudit: "2026-03-13", nextAudit: "2026-12-06", evidenceCount: 54 },
  ];
  private _threat_huntiGetComplianceScore() {
    const fw = this._threat_huntiComplianceFrameworks;
    return round(fw.reduce((s,f)=>s + (f.implementedControls/Math.max(f.totalControls,1))*100, 0) / fw.length, 1);
  }
  private _threat_huntiGetGaps() {
    return this._threat_huntiComplianceFrameworks.filter(f => f.status !== "compliant");
  }
  private _threat_huntiAuditTrail = [
    { id: "AUD-0001", action: "Control tested", auditor: "Internal Audit", date: "2026-04-07", result: "pass", findings: 2 },
    { id: "AUD-0002", action: "Evidence collected", auditor: "External Auditor", date: "2026-04-14", result: "pass", findings: 4 },
    { id: "AUD-0003", action: "Gap identified", auditor: "Security Team", date: "2026-04-07", result: "fail", findings: 2 },
    { id: "AUD-0004", action: "Remediation completed", auditor: "Compliance Officer", date: "2026-04-07", result: "pass", findings: 3 },
    { id: "AUD-0005", action: "Policy updated", auditor: "IT Audit", date: "2026-04-12", result: "pass", findings: 4 },
    { id: "AUD-0006", action: "Training verified", auditor: "Risk Team", date: "2026-04-12", result: "pass", findings: 2 },
    { id: "AUD-0007", action: "Access reviewed", auditor: "QA Team", date: "2026-04-23", result: "pass", findings: 0 },
    { id: "AUD-0008", action: "Exception approved", auditor: "CISO Office", date: "2026-04-07", result: "conditional", findings: 5 },
    { id: "AUD-0009", action: "Risk accepted", auditor: "Board Audit", date: "2026-04-15", result: "pass", findings: 1 },
    { id: "AUD-0010", action: "Control enhanced", auditor: "Third Party", date: "2026-04-08", result: "pass", findings: 2 },
  ];

  // === Threat Intelligence Feed Extension (threat_hunti) ===
  private _threat_huntiThreatActors = [
    { name: "APT-29", alias: "Cozy Bear", origin: "Russia", type: "Nation-State", severity: "high", lastActivity: "2026-04-06", targets: "Technology", indicators: 444, ttps: 21 },
    { name: "APT-41", alias: "Double Dragon", origin: "China", type: "Nation-State", severity: "critical", lastActivity: "2026-04-20", targets: "Finance", indicators: 80, ttps: 28 },
    { name: "Lazarus Group", alias: "Hidden Cobra", origin: "North Korea", type: "Nation-State", severity: "critical", lastActivity: "2026-04-04", targets: "Technology", indicators: 32, ttps: 5 },
    { name: "FIN7", alias: "Carbanak", origin: "Eastern Europe", type: "Financial", severity: "high", lastActivity: "2026-04-23", targets: "Government", indicators: 430, ttps: 40 },
    { name: "Conti", alias: "Wizard Spider", origin: "Russia", type: "Ransomware", severity: "critical", lastActivity: "2026-04-08", targets: "Finance", indicators: 222, ttps: 37 },
    { name: "LockBit", alias: "LockBit Gang", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-02", targets: "Finance", indicators: 427, ttps: 50 },
    { name: "Cl0p", alias: "Cl0p Team", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-03", targets: "Finance", indicators: 103, ttps: 40 },
    { name: "Sandworm", alias: "Unit 74455", origin: "Russia", type: "Nation-State", severity: "critical", lastActivity: "2026-04-19", targets: "Technology", indicators: 53, ttps: 22 },
  ];
  private _threat_huntiIoCFeed = [
    { id: "ioc-000001", type: "ip", value: "37.190.253.70", confidence: 81, source: "VirusTotal", firstSeen: "2026-04-03", lastSeen: "2026-04-02" },
    { id: "ioc-000002", type: "ip", value: "129.101.180.114", confidence: 84, source: "Mandiant", firstSeen: "2026-04-04", lastSeen: "2026-04-09" },
    { id: "ioc-000003", type: "ip", value: "34.78.244.203", confidence: 70, source: "AlienVault", firstSeen: "2026-04-14", lastSeen: "2026-04-13" },
    { id: "ioc-000004", type: "ip", value: "197.222.46.12", confidence: 85, source: "AlienVault", firstSeen: "2026-04-13", lastSeen: "2026-04-17" },
    { id: "ioc-000005", type: "ip", value: "112.32.186.197", confidence: 75, source: "VirusTotal", firstSeen: "2026-04-21", lastSeen: "2026-04-21" },
    { id: "ioc-000006", type: "ip", value: "210.224.170.109", confidence: 65, source: "MISP", firstSeen: "2026-04-23", lastSeen: "2026-04-04" },
    { id: "ioc-000007", type: "ip", value: "214.67.191.25", confidence: 71, source: "CrowdStrike", firstSeen: "2026-04-11", lastSeen: "2026-04-06" },
    { id: "ioc-000008", type: "ip", value: "35.161.241.195", confidence: 93, source: "STIX", firstSeen: "2026-04-03", lastSeen: "2026-04-11" },
    { id: "ioc-000009", type: "ip", value: "138.91.3.235", confidence: 56, source: "STIX", firstSeen: "2026-04-18", lastSeen: "2026-04-22" },
    { id: "ioc-000010", type: "ip", value: "205.110.156.118", confidence: 50, source: "CrowdStrike", firstSeen: "2026-04-06", lastSeen: "2026-04-19" },
    { id: "ioc-000011", type: "ip", value: "202.143.227.156", confidence: 53, source: "CrowdStrike", firstSeen: "2026-04-22", lastSeen: "2026-04-17" },
    { id: "ioc-000012", type: "ip", value: "36.40.132.82", confidence: 99, source: "CrowdStrike", firstSeen: "2026-04-13", lastSeen: "2026-04-20" },
    { id: "ioc-000013", type: "ip", value: "237.126.182.190", confidence: 73, source: "STIX", firstSeen: "2026-04-22", lastSeen: "2026-04-09" },
    { id: "ioc-000014", type: "ip", value: "185.87.255.156", confidence: 62, source: "AlienVault", firstSeen: "2026-04-05", lastSeen: "2026-04-08" },
    { id: "ioc-000015", type: "ip", value: "74.139.7.228", confidence: 70, source: "AlienVault", firstSeen: "2026-04-14", lastSeen: "2026-04-06" },
  ];
  private _threat_huntiGetActiveThreats() { return this._threat_huntiThreatActors.filter(a => a.severity === 'critical'); }
  private _threat_huntiGetThreatSummary() {
    const actors = this._threat_huntiThreatActors;
  ...
  // === Incident Management Extension (threat_hunti) ===
  private _threat_huntiIncidents = [
    { id: "INC-20260001", title: "Unauthorized access detected", severity: "critical", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-21T08:26", affectedAssets: 36, rootCause: "Misconfiguration" },
    { id: "INC-20260002", title: "Malware outbreak on workstation", severity: "high", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-18T18:25", affectedAssets: 34, rootCause: "Credential compromise" },
    { id: "INC-20260003", title: "Data leak from S3 bucket", severity: "medium", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-03T10:14", affectedAssets: 47, rootCause: "Zero-day" },
    { id: "INC-20260004", title: "Phishing campaign targeting finance", severity: "low", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-17T04:14", affectedAssets: 15, rootCause: "Human error" },
    { id: "INC-20260005", title: "DDoS attack on web services", severity: "critical", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-11T13:29", affectedAssets: 11, rootCause: "Policy violation" },
    { id: "INC-20260006", title: "Ransomware encryption attempt", severity: "high", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-12T14:03", affectedAssets: 36, rootCause: "Unknown" },
    { id: "INC-20260007", title: "Insider data exfiltration", severity: "medium", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-07T00:06", affectedAssets: 27, rootCause: "Misconfiguration" },
    { id: "INC-20260008", title: "API key exposure in repo", severity: "low", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-18T02:58", affectedAssets: 19, rootCause: "Credential compromise" },
    { id: "INC-20260009", title: "SQL injection on portal", severity: "critical", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-09T07:43", affectedAssets: 33, rootCause: "Zero-day" },
    { id: "INC-20260010", title: "Brute force on VPN", severity: "high", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-03T08:09", affectedAssets: 34, rootCause: "Human error" },
    { id: "INC-20260011", title: "Supply chain alert from vendor", severity: "medium", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-16T21:49", affectedAssets: 35, rootCause: "Policy violation" },
    { id: "INC-20260012", title: "Suspicious lateral movement", severity: "low", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-16T21:06", affectedAssets: 32, rootCause: "Unknown" },
  ];
  private _threat_huntiGetIncidentStats() {
    const inc = this._threat_huntiIncidents;
    return { total: inc.length, open: inc.filter(i=>i.status==="open").length, investigating: inc.filter(i=>i.status==="investigating").length, mttd: 19, mttr: 49 };
  }
  private _threat_huntiGetSeverityDistribution() {
    const inc = this._threat_huntiIncidents;
  ...
  render() {
    return html`${this.thRenderRound17()}
      <div class="panel">
        <div class="pt">🎯 Threat Hunting Workspace</div>
      ...
      <div style="background:#0f172a;border-radius:8px;padding:12px;margin-bottom:12px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px">
          <div style="background:#1f2937;border-radius:8px;padding:14px">
            ...
            <div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid #1f2937;font-size:11px">              <div style="width:140px;font-weight:600;color:#94a3b8">Active Hunts</div>              <div style="width:60px;font-weight:700;font-size:14px;color:#06b6d4">5</div>              <div style="flex:1;font-size:10px;color:#6b7280">2 high priority</div>            </div>
            <div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid #1f2937;font-size:11px">              <div style="width:140px;font-weight:600;color:#94a3b8">IOCs Identified</div>              <div style="width:60px;font-weight:700;font-size:14px;color:#ef4444">47</div>              <div style="flex:1;font-size:10px;color:#6b7280">12 confirmed malicious</div>            </div>
            <div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid #1f2937;font-size:11px">              <div style="width:140px;font-weight:600;color:#94a3b8">Hypotheses Tested</div>              <div style="width:60px;font-weight:700;font-size:14px;color:#3b82f6">23</div>              <div style="flex:1;font-size:10px;color:#6b7280">8 confirmed, 15 refuted</div>            </div>
            <div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid #1f2937;font-size:11px">              <div style="width:140px;font-weight:600;color:#94a3b8">MITRE Coverage</div>              <div style="width:60px;font-weight:700;font-size:14px;color:#22c55e">78%</div>              <div style="flex:1;font-size:10px;color:#6b7280">Up from 72%</div>            </div>
            <div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid #1f2937;font-size:11px">              <div style="width:140px;font-weight:600;color:#94a3b8">Intel Sources</div>              <div style="width:60px;font-weight:700;font-size:14px;color:#a855f7">12</div>              <div style="flex:1;font-size:10px;color:#6b7280">4 new this quarter</div>            </div>
            <div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid #1f2937;font-size:11px">              <div style="width:140px;font-weight:600;color:#94a3b8">Avg Hunt Duration</div>              <div style="width:60px;font-weight:700;font-size:14px;color:#eab308">3.2d</div>              <div style="flex:1;font-size:10px;color:#6b7280">Within target</div>            </div>
          </div>
          <div style="background:#1f2937;border-radius:8px;padding:14px">
            ...
            <div style="display:flex;align-items:flex-start;gap:8px;padding:6px 0;border-bottom:1px solid #1f2937;font-size:11px">              <span style="color:#06b6d4;flex-shrink:0;margin-top:1px">&#8226;</span>              <span style="color:#94a3b8">Active hunt identified 3 IOC matches related to APT29 campaign.</span>            </div>
            <div style="display:flex;align-items:flex-start;gap:8px;padding:6px 0;border-bottom:1px solid #1f2937;font-size:11px">              <span style="color:#06b6d4;flex-shrink:0;margin-top:1px">&#8226;</span>              <span style="color:#94a3b8">New YARA rule set deployed for detecting Cobalt Strike beacons.</span>            </div>
            <div style="display:flex;align-items:flex-start;gap:8px;padding:6px 0;border-bottom:1px solid #1f2937;font-size:11px">              <span style="color:#06b6d4;flex-shrink:0;margin-top:1px">&#8226;</span>              <span style="color:#94a3b8">Threat intelligence feed integration expanded with 4 new sources.</span>            </div>
            <div style="display:flex;align-items:flex-start;gap:8px;padding:6px 0;border-bottom:1px solid #1f2937;font-size:11px">              <span style="color:#06b6d4;flex-shrink:0;margin-top:1px">&#8226;</span>              <span style="color:#94a3b8">MITRE ATT&CK coverage improved to 78% across all techniques.</span>            </div>
          </div>
        </div>

        <div class="tabs">
          <span class="tab ${this._tab === 'sessions' ? 'active' : ''}" @click=${() => { this._tab = 'sessions'; this.requestUpdate(); }}>Sessions</span>
          <span class="tab ${this._tab === 'active' ? 'active' : ''}" @click=${() => { this._tab = 'active'; this.requestUpdate(); }}>Active Hunt</span>
          <span class="tab ${this._tab === 'playbooks' ? 'active' : ''}" @click=${() => { this._tab = 'playbooks'; this.requestUpdate(); }}>Playbooks</span>
          <span class="tab ${this._tab === 'datasources' ? 'active' : ''}" @click=${() => { this._tab = 'datasources'; this.requestUpdate(); }}>Data Sources</span>
        </div>
        ${this._tab === 'sessions' ? this._renderSessionsTab() : ''}
        ${this._tab === 'active' ? this._renderActiveTab() : ''}
        ${this._tab === 'playbooks' ? this._renderPlaybooksTab() : ''}
        ${this._tab === 'datasources' ? this._renderDatasourcesTab() : ''}
      </div>
  ...
  private _renderAdvancedGrid(): any {
    ...
    return html`
      <div style="background:#1f2937;border-radius:8px;padding:14px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
          <span style="font-weight:600;font-size:12px;color:#9ca3af;text-transform:uppercase">Threat Hunting Workspace Findings Grid</span>
          <div style="display:flex;gap:4px">
  ...
  @state() private _thrScenarios: {id:string;name:string;attackType:string;target:string;method:string;impactLow:number;impactHigh:number;confidence:number;mitigation:string;status:string}[] = [];
  @state() private _thrScenarioForm: {attackType:string;target:string;method:string} = {attackType:'',target:'',method:''};
  @state() private _thrScenarioCompare: boolean = false;
  @state() private _thrScenarioSelected: string[] = [];

  private _thrInitScenarios(): void {
    const saved = localStorage.getItem('thr_scenarios');
    if (saved) { try { this._thrScenarios = JSON.parse(saved); } catch { /* ignore */ } }
    if (this._thrScenarios.length === 0) {
      this._thrScenarios = [
        {id:'thr-s1',name:'Baseline Threat',attackType:'Phishing',target:'Employees',method:'Spear Email',impactLow:45,impactHigh:78,confidence:72,mitigation:'Security awareness training + email filtering',status:'active'},
        {id:'thr-s2',name:'Escalated Attack',attackType:'Ransomware',target:'Endpoints',method:'Drive-by Download',impactLow:65,impactHigh:95,confidence:58,mitigation:'EDR deployment + network segmentation',status:'saved'},
        {id:'thr-s3',name:'Insider Threat',attackType:'Data Exfiltration',target:'Databases',method:'SQL Injection',impactLow:55,impactHigh:88,confidence:65,mitigation:'DLP policies + query monitoring',status:'draft'},
      ];
    }
  }

  private _thrSaveScenarios(): void {
    localStorage.setItem('thr_scenarios', JSON.stringify(this._thrScenarios));
  }

  private _thrAddScenario(): void {
    const f = this._thrScenarioForm;
    if (!f.attackType || !f.target) return;
    this._thrScenarios = [...this._thrScenarios, {
      id: 'thr-s' + (this._thrScenarios.length + 1),
    ...
    this._thrScenarioForm = {attackType:'',target:'',method:''};
    this._thrSaveScenarios();
  }

  private _thrRenderScenarioEngine(): any {
    ...
    return html`
      <div style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          ...
          <button class="tab" @click=${() => { this._thrScenarioCompare = !this._thrScenarioCompare; }}>${this._thrScenarioCompare ? 'List View' : 'Compare'}</button>
        </div>
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:10px">
          <select style="background:#1a1d2e;border:1px solid #2a2d3a;border-radius:4px;padding:4px 6px;color:#e2e8f0;font-size:11px" @change=${(e: any) => { this._thrScenarioForm = {...this._thrScenarioForm, attackType: (e.target as HTMLSelectElement).value}; }}>
            <option value="">Attack Type</option>
            ${attackTypes.map(a => html`<option value=${a} ${this._thrScenarioForm.attackType === a ? 'selected' : ''}>${a}</option>`)}
          </select>
          <select style="background:#1a1d2e;border:1px solid #2a2d3a;border-radius:4px;padding:4px 6px;color:#e2e8f0;font-size:11px" @change=${(e: any) => { this._thrScenarioForm = {...this._thrScenarioForm, target: (e.target as HTMLSelectElement).value}; }}>
            <option value="">Target</option>
            ${targets.map(t => html`<option value=${t} ${this._thrScenarioForm.target === t ? 'selected' : ''}>${t}</option>`)}
          </select>
          <select style="background:#1a1d2e;border:1px solid #2a2d3a;border-radius:4px;padding:4px 6px;color:#e2e8f0;font-size:11px" @change=${(e: any) => { this._thrScenarioForm = {...this._thrScenarioForm, method: (e.target as HTMLSelectElement).value}; }}>
            <option value="">Method</option>
            ${methods.map(m => html`<option value=${m} ${this._thrScenarioForm.method === m ? 'selected' : ''}>${m}</option>`)}
          </select>
        </div>
        <button style="background:#3b82f6;border:none;border-radius:4px;padding:4px 12px;color:#fff;font-size:11px;cursor:pointer" @click=${this._thrAddScenario}>Run Simulation</button>
      </div>
      ${this._thrScenarioCompare && this._thrScenarios.length >= 2 ? html`
        <div style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
          <div style="font-weight:600;font-size:12px;color:#e2e8f0;margin-bottom:8px">Side-by-Side Comparison</div>
          <div style="display:grid;grid-template-columns:repeat(${Math.min(3, this._thrScenarios.length)},1fr);gap:8px">
            ${this._thrScenarios.slice(0,3).map(s => html`
      ...
      <div style="background:#0f1117;border-radius:8px;padding:12px">
        <div style="font-weight:600;font-size:12px;color:#e2e8f0;margin-bottom:8px">Saved Scenarios (${this._thrScenarios.length})</div>
        ${this._thrScenarios.map(s => html`
  ...
  @state() private _thrTrendData: {day:number;value:number;anomaly:boolean}[] = [];
  @state() private _thrTrendZoom: {start:number;end:number} | null = null;
  @state() private _thrTrendMA: number = 7;

  private _thrInitTrendData(): void {
    ...
    this._thrTrendData = data;
  }

  private _thrCalcMA(window: number): number[] {
    const result: number[] = [];
    for (let i = 0; i < this._thrTrendData.length; i++) {
      const start = Math.max(0, i - window + 1);
      const slice = this._thrTrendData.slice(start, i + 1);
      result.push(slice.reduce((s, d) => s + d.value, 0) / slice.length);
    }
    return result;
  }

  private _thrGetStats(): {mean:number;median:number;stddev:number;trend:string} {
    const vals = this._thrTrendData.map(d => d.value);
    const n = vals.length;
  ...
  private _thrRenderTimeSeries(): any {
    const stats = this._thrGetStats();
    const filtered = this._thrTrendZoom ? this._thrTrendData.filter(d => d.day >= this._thrTrendZoom.start && d.day <= this._thrTrendZoom.end) : this._thrTrendData;
    ...
    return html`
      <div style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          ...
          <div style="display:flex;gap:4px">
            <button class="tab ${this._thrTrendMA === 7 ? 'active' : ''}" @click=${() => { this._thrTrendMA = 7; }}>7D MA</button>
            <button class="tab ${this._thrTrendMA === 30 ? 'active' : ''}" @click=${() => { this._thrTrendMA = 30; }}>30D MA</button>
            <button class="tab" @click=${() => { this._thrTrendZoom = null; }}>Reset Zoom</button>
          </div>
        </div>
        <div style="position:relative;height:120px;background:#1a1d2e;border-radius:6px;overflow:hidden;margin-bottom:8px;cursor:crosshair" @click=${(e: any) => {
          ...
          this._thrTrendZoom = { start: Math.max(0, center - 10), end: Math.min(89, center + 10) };
        }}>
  ...
  @state() private _thrRoles: string[] = ['Admin','Analyst','Operator','Viewer','Auditor'];
  @state() private _thrActions: string[] = ['Create','Read','Update','Delete','Export','Approve'];
  @state() private _thrPermissions: { [role: string]: { [action: string]: boolean } } = {};
  @state() private _thrPermAudit: {role:string;action:string;changedBy:string;timestamp:string;oldVal:boolean;newVal:boolean}[] = [];
  @state() private _thrPermCompare: string[] = [];

  private _thrInitPermissions(): void {
    ...
    for (const role of this._thrRoles) {
      perms[role] = {};
      this._thrActions.forEach((a, i) => { perms[role][a] = defaults[role]?.[i] ?? false; });
    }
    this._thrPermissions = perms;
  }

  private _thrTogglePermission(role: string, action: string): void {
    const old = this._thrPermissions[role][action];
    this._thrPermissions = {...this._thrPermissions, [role]: {...this._thrPermissions[role], [action]: !old}};
    this._thrPermAudit.unshift({role,action,changedBy:'current_user',timestamp:new Date().toISOString(),oldVal:old,newVal:!old});
  }

  private _thrRenderRBAC(): any {
    const compareRoles = this._thrPermCompare.map(r => this._thrPermissions[r]).filter(Boolean);
    return html`
      <div style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          <span style="font-weight:700;font-size:13px;color:#e2e8f0">RBAC Permission Matrix</span>
          <div style="display:flex;gap:4px">
            ${this._thrRoles.map(r => html`
              <button class="tab ${this._thrPermCompare.includes(r) ? 'active' : ''}" @click=${() => {
                this._thrPermCompare = this._thrPermCompare.includes(r) ? this._thrPermCompare.filter(x => x !== r) : [...this._thrPermCompare, r];
              }}>${r}</button>
            `)}
          </div>
        </div>
          ...
          <table style="width:100%;border-collapse:collapse;font-size:10px">
            <thead>
              <tr>
                ...
                ${this._thrActions.map(a => html`<th style="padding:6px;color:#6b7280;border-bottom:1px solid #2a2d3a;text-align:center">${a}</th>`)}
              </tr>
            </thead>
            <tbody>
              ${this._thrRoles.map(role => html`
                <tr style="border-bottom:1px solid #1a1d2e">
                  <td style="padding:6px;color:#e2e8f0;font-weight:600">${role}</td>
                  ${this._thrActions.map(action => html`
                    <td style="text-align:center;padding:6px">
                      <button style="width:28px;height:20px;border-radius:3px;border:1px solid #2a2d3a;background:${this._thrPermissions[role][action] ? '#22c55e' : '#1a1d2e'};cursor:pointer;color:#fff;font-size:10px" @click=${() => this._thrTogglePermission(role, action)}>${this._thrPermissions[role][action] ? 'Y' : 'N'}</button>
                    </td>
                  `)}
                </tr>
              `)}
        ...
        ${compareRoles.length >= 2 ? html`
          <div style="margin-top:10px;padding:8px;background:#1a1d2e;border-radius:6px">
            <div style="font-size:11px;font-weight:600;color:#e2e8f0;margin-bottom:6px">Role Diff: ${this._thrPermCompare.join(' vs ')}</div>
            ${this._thrActions.map(action => {
              ...
              return allSame ? nothing : html`
                <div style="display:flex;gap:8px;padding:3px 0;font-size:10px">
                  ...
                  ${compareRoles.map((r, i) => html`<span style="color:${r[action] ? '#22c55e' : '#ef4444'}">${this._thrPermCompare[i]}=${r[action] ? 'Y' : 'N'}</span>`)}
                </div>
              `;
            })}
          </div>
        ` : nothing}
        ${this._thrPermAudit.length > 0 ? html`
          <div style="margin-top:8px;font-size:9px;color:#6b7280">Recent: ${this._thrPermAudit.slice(0,3).map(a => html`<span style="margin-right:8px">${a.role}.${a.action}: ${a.oldVal ? 'Y' : 'N'}->${a.newVal ? 'Y' : 'N'}</span>`)}
        ` : nothing}
      </div>
    `;
  }
  ...
  // === REPORTING SUITE ===
  @state() private _thrReportTemplate: string = 'executive';
  @state() private _thrReportSchedule: string = 'weekly';
  @state() private _thrReportDistList: string[] = ['security-team@company.com','ciso@company.com'];
  @state() private _thrReportHistory: {id:string;template:string;generatedAt:string;status:string}[] = [];

  private _thrGenerateReport(): void {
    const id = 'rpt-' + Date.now();
    this._thrReportHistory.unshift({id,template:this._thrReportTemplate,generatedAt:new Date().toISOString(),status:'sent'});
  }

  private _thrRenderReporting(): any {
    ...
    return html`
      <div style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">
          <div>
            ...
            ${templates.map(t => html`
              <div style="padding:6px;background:${this._thrReportTemplate === t.key ? '#1e3a5f' : '#1a1d2e'};border:1px solid ${this._thrReportTemplate === t.key ? '#3b82f6' : '#2a2d3a'};border-radius:4px;margin-bottom:4px;cursor:pointer" @click=${() => { this._thrReportTemplate = t.key; }}>
          ...
          <div>
            ...
            <div style="display:flex;gap:4px;margin-bottom:8px">
              ${schedules.map(s => html`<button class="tab ${this._thrReportSchedule === s.key ? 'active' : ''}" @click=${() => { this._thrReportSchedule = s.key; }}>${s.label}</button>`)}
            </div>
            <div style="font-size:10px;color:#6b7280;margin-bottom:4px">Distribution</div>
            <div style="font-size:10px;color:#9ca3af">${this._thrReportDistList.map(d => html`<div>${d}</div>`)}</div>
            <div style="margin-top:6px;display:flex;gap:4px">
              <button style="background:#3b82f6;border:none;border-radius:4px;padding:4px 10px;color:#fff;font-size:10px;cursor:pointer" @click=${this._thrGenerateReport}>Generate</button>
        ...
        ${this._thrReportHistory.length > 0 ? html`
          <div style="border-top:1px solid #2a2d3a;padding-top:8px">
            ...
            ${this._thrReportHistory.slice(0,3).map(r => html`
  ...
  @state() private _thrHighContrast: boolean = false;
  @state() private _thrA11yAnnounce: string = '';
  @state() private _thrShortcutsVisible: boolean = false;
  @state() private _thrFocusTrap: boolean = false;

  private _thrShortcuts: Record<string, string> = {
  ...
  private _thrHandleKeydown(e: KeyboardEvent): void {
    if (e.key === 'Escape' && this._thrFocusTrap) { this._thrFocusTrap = false; this._thrAnnounce('Dialog closed'); }
    if (e.ctrlKey && e.shiftKey && e.key === 'H') { e.preventDefault(); this._thrHighContrast = !this._thrHighContrast; this._thrAnnounce('High contrast ' + (this._thrHighContrast ? 'enabled' : 'disabled')); }
    if (e.ctrlKey && e.shiftKey && e.key === 'A') { e.preventDefault(); this._thrShortcutsVisible = !this._thrShortcutsVisible; }
  }

  private _thrAnnounce(msg: string): void {
    this._thrA11yAnnounce = msg;
    setTimeout(() => { this._thrA11yAnnounce = ''; }, 2000);
  }

  private _thrRenderAccessibility(): any {
    return html`
      <div role="region" aria-label="Accessibility Controls" style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          ...
          <button class="tab ${this._thrShortcutsVisible ? 'active' : ''}" @click=${() => { this._thrShortcutsVisible = !this._thrShortcutsVisible; }} aria-expanded=${this._thrShortcutsVisible}>Shortcuts</button>
        </div>
        <div style="display:flex;gap:8px;margin-bottom:8px">
          <label style="display:flex;align-items:center;gap:6px;font-size:11px;color:#e2e8f0;cursor:pointer">
            <input type="checkbox" .checked=${this._thrHighContrast} @change=${() => { this._thrHighContrast = !this._thrHighContrast; }} aria-label="Toggle high contrast mode">
            High Contrast
          </label>
        </div>
        ${this._thrShortcutsVisible ? html`
          <div role="list" aria-label="Keyboard shortcuts" style="background:#1a1d2e;border-radius:6px;padding:8px">
            ${Object.entries(this._thrShortcuts).map(([key, desc]) => html`
        ...
        ` : nothing}
        <div role="status" aria-live="polite" aria-atomic="true" style="position:absolute;left:-9999px">${this._thrA11yAnnounce}</div>
      </div>
    `;
  }
  ...
  connectedCallback(): void {
    ...
    this._thrInitScenarios();
    this._thrInitTrendData();
    this._thrInitPermissions();
    document.addEventListener('keydown', this._thrHandleKeydown.bind(this));
  }

  disconnectedCallback(): void {
    super.disconnectedCallback();
    document.removeEventListener('keydown', this._thrHandleKeydown.bind(this));
  }

  // === TAB INTEGRATION FOR EXTENDED FEATURES ===
  @state() private _thrActiveSubTab: string = 'scenario';
  ...
  private _thrGetAllSubTabs(): {key:string;label:string}[] {
    return [
  ...
  private _thrRenderSubPanel(): any {
    switch (this._thrActiveSubTab) {
      case 'scenario': return this._thrRenderScenarioEngine();
      case 'timeseries': return this._thrRenderTimeSeries();
      case 'rbac': return this._thrRenderRBAC();
      case 'reporting': return this._thrRenderReporting();
      case 'a11y': return this._thrRenderAccessibility();
      default: return nothing;
    }
  }

  private _thrRenderTabBar(): any {
    return html`
      <div style="display:flex;gap:4px;margin-bottom:12px;border-bottom:1px solid #2a2d3a;padding-bottom:8px;flex-wrap:wrap" role="tablist" aria-label="Extended panel features">
        ${this._thrGetAllSubTabs().map(t => html`
          <button class="tab ${this._thrActiveSubTab === t.key ? 'active' : ''}" @click=${() => { this._thrActiveSubTab = t.key; }} role="tab" aria-selected=${this._thrActiveSubTab === t.key}>${t.label}</button>
        `)}
      </div>
      <div role="tabpanel" aria-labelledby="thr-tab-${this._thrActiveSubTab}">
        ${this._thrRenderSubPanel()}
      </div>
    `;
  }
  ...
  private _thwGetNewSubTabs(): {key:string;label:string}[] {
    return [
  ...
  @state() private _thwBayesianPrior: number = 0.5;
  @state() private _thwBayesianLikelihood: number = 0.7;
  @state() private _thwMonteCarloResults: number[] = [];
  @state() private _thwCorrelationMatrix: number[][] = [];
  @state() private _thwOutlierIndices: number[] = [];
  @state() private _thwTrendComponents: {trend:number;seasonal:number;residual:number}[] = [];
  @state() private _thwAnalyticsView: string = 'bayesian';
  @state() private _thwConfidenceLevel: number = 95;
  @state() private _thwMonteCarloIterations: number = 100;

  private _thwCalculateBayesianPosterior(): number {
    const prior = this._thwBayesianPrior;
    const likelihood = this._thwBayesianLikelihood;
  ...
  private _thwRunMonteCarloSimulation(): number[] {
    const results: number[] = [];
    const baseRisk = 0.35;
    const volatility = 0.15;
    for (let i = 0; i < this._thwMonteCarloIterations; i++) {
    ...
    this._thwMonteCarloResults = results;
    return results;
  }

  private _thwComputeCorrelationMatrix(): number[][] {
    const events = this._thwGenerateMockTimeSeries(6, 50);
    const n = events.length;
    const matrix: number[][] = [];
    for (let i = 0; i < n; i++) {
      const row: number[] = [];
      for (let j = 0; j < n; j++) {
        row.push(this._thwPearsonCorrelation(events[i], events[j]));
      }
      matrix.push(row);
    }
    this._thwCorrelationMatrix = matrix;
    return matrix;
  }

  private _thwPearsonCorrelation(x: number[], y: number[]): number {
  ...
  private _thwDetectOutliersZScore(data: number[]): number[] {
  ...
  private _thwDetectOutliersIQR(data: number[]): number[] {
  ...
  private _thwDecomposeTrend(data: number[]): {trend:number;seasonal:number;residual:number}[] {
    ...
    this._thwTrendComponents = result;
    return result;
  }

  private _thwPredictiveScoreWithCI(data: number[]): {score:number;low:number;high:number} {
    const recent = data.slice(-10);
    const avg = recent.reduce((a, b) => a + b, 0) / recent.length;
    const std = Math.sqrt(recent.reduce((a, b) => a + (b - avg) ** 2, 0) / recent.length);
    const zScore = this._thwConfidenceLevel === 99 ? 2.576 : this._thwConfidenceLevel === 90 ? 1.645 : 1.96;
    return { score: avg, low: avg - zScore * std, high: avg + zScore * std };
  }

  private _thwGenerateMockTimeSeries(count: number, length: number): number[][] {
    const series: number[][] = [];
  ...
  private _thwRenderAnalyticsEngine(): any {
    const posterior = this._thwCalculateBayesianPosterior();
    const mcResults = this._thwMonteCarloResults.length > 0 ? this._thwMonteCarloResults : this._thwRunMonteCarloSimulation();
    ...
    const matrix = this._thwCorrelationMatrix.length > 0 ? this._thwCorrelationMatrix : this._thwComputeCorrelationMatrix();
    ...
    return html`
      <div class="analytics-engine" style="padding:12px">
        <div style="display:flex;gap:8px;margin-bottom:12px;flex-wrap:wrap">
          <button class="tab ${this._thwAnalyticsView === 'bayesian' ? 'active' : ''}" @click=${() => { this._thwAnalyticsView = 'bayesian'; }}>Bayesian</button>
          <button class="tab ${this._thwAnalyticsView === 'montecarlo' ? 'active' : ''}" @click=${() => { this._thwAnalyticsView = 'montecarlo'; }}>Monte Carlo</button>
          <button class="tab ${this._thwAnalyticsView === 'correlation' ? 'active' : ''}" @click=${() => { this._thwAnalyticsView = 'correlation'; }}>Correlation</button>
          <button class="tab ${this._thwAnalyticsView === 'outliers' ? 'active' : ''}" @click=${() => { this._thwAnalyticsView = 'outliers'; }}>Outliers</button>
          <button class="tab ${this._thwAnalyticsView === 'trend' ? 'active' : ''}" @click=${() => { this._thwAnalyticsView = 'trend'; }}>Trend</button>
          <button class="tab ${this._thwAnalyticsView === 'predictive' ? 'active' : ''}" @click=${() => { this._thwAnalyticsView = 'predictive'; }}>Predictive</button>
        </div>
        ${this._thwAnalyticsView === 'bayesian' ? html`
          <div class="card" style="padding:12px;margin-bottom:8px">
            <h4 style="margin:0 0 8px;color:#e0e0e0">Bayesian Risk Probability</h4>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
              <div><span style="color:#888">Prior:</span> ${this._thwBayesianPrior.toFixed(2)}</div>
              <div><span style="color:#888">Likelihood:</span> ${this._thwBayesianLikelihood.toFixed(2)}</div>
            ...
            <div style="margin-top:8px;display:flex;gap:8px;align-items:center">
              ...
              <input type="range" min="0" max="100" .value=${String(this._thwBayesianPrior * 100)} @input=${(e: any) => { this._thwBayesianPrior = Number(e.target.value) / 100; }} style="flex:1" />
              <label style="color:#888;font-size:12px">Likelihood:</label>
              <input type="range" min="0" max="100" .value=${String(this._thwBayesianLikelihood * 100)} @input=${(e: any) => { this._thwBayesianLikelihood = Number(e.target.value) / 100; }} style="flex:1" />
            </div>
          </div>
        ` : nothing}
        ${this._thwAnalyticsView === 'montecarlo' ? html`
          <div class="card" style="padding:12px;margin-bottom:8px">
            ...
            <button class="btn" style="margin-top:8px" @click=${() => { this._thwRunMonteCarloSimulation(); }}>Re-run Simulation</button>
          </div>
        ` : nothing}
        ${this._thwAnalyticsView === 'correlation' ? html`
          <div class="card" style="padding:12px;margin-bottom:8px">
        ...
        ` : nothing}
        ${this._thwAnalyticsView === 'outliers' ? html`
          <div class="card" style="padding:12px;margin-bottom:8px">
            <h4 style="margin:0 0 8px;color:#e0e0e0">Statistical Outlier Detection</h4>
            ${['zscore', 'iqr'].map(method => {
              const data = this._thwGenerateMockTimeSeries(1, 30)[0];
              const outliers = method === 'zscore' ? this._thwDetectOutliersZScore(data) : this._thwDetectOutliersIQR(data);
        ...
        ` : nothing}
        ${this._thwAnalyticsView === 'trend' ? html`
          <div class="card" style="padding:12px;margin-bottom:8px">
            ...
            ${['Trend', 'Seasonal', 'Residual'].map((comp, ci) => html`
              <div style="margin-bottom:8px">
                ...
                <div style="display:flex;gap:1px;height:25px;align-items:center">${this._thwDecomposeTrend(this._thwGenerateMockTimeSeries(1, 24)[0]).map(p => {
        ...
        ` : nothing}
        ${this._thwAnalyticsView === 'predictive' ? html`
          <div class="card" style="padding:12px;margin-bottom:8px">
            ...
            <div style="display:flex;gap:8px;align-items:center;margin-bottom:8px">
              ...
              <select .value=${String(this._thwConfidenceLevel)} @change=${(e: any) => { this._thwConfidenceLevel = Number(e.target.value); }}>
            ...
            ${['Risk Score', 'Compliance', 'Threat Index'].map(label => {
              const data = this._thwGenerateMockTimeSeries(1, 20)[0];
              const pred = this._thwPredictiveScoreWithCI(data);
  ...
  @state() private _thwWarRoomActive: boolean = false;
  @state() private _thwWarRoomParticipants: string[] = ['SOC Lead', 'IR Manager', 'CISO', 'Legal', 'PR'];
  @state() private _thwIncidentSeverity: string = 'P3';
  @state() private _thwEscalationLevel: number = 0;
  @state() private _thwCommTemplate: string = 'initial';
  @state() private _thwLessonsLearned: string = '';
  @state() private _thwPostIncidentAnswers: Record<string, string> = {};
  @state() private _thwWarRoomMessages: {sender:string;time:string;text:string}[] = [];

  private _thwGetSeverityMatrix(): {severity:string;responseTime:string;escalation:string;notify:string}[] {
    return [
  ...
  private _thwGetCommunicationTemplates(): {key:string;subject:string;body:string}[] {
    return [
  ...
  private _thwGetStakeholderMatrix(): {role:string;notifyP1:boolean;notifyP2:boolean;notifyP3:boolean;channel:string}[] {
    return [
  ...
  private _thwGetPostIncidentQuestions(): {id:string;question:string;type:string;options:string[]}[] {
    return [
  ...
  private _thwToggleWarRoom(): void {
    this._thwWarRoomActive = !this._thwWarRoomActive;
    if (this._thwWarRoomActive) {
      this._thwWarRoomMessages = [
  ...
  private _thwSendWarRoomMessage(text: string): void {
    this._thwWarRoomMessages = [...this._thwWarRoomMessages, {
      sender: 'You', time: new Date().toLocaleTimeString(), text
    }];
  }

  private _thwEscalateIncident(): void {
    const levels = ['P4', 'P3', 'P2', 'P1'];
    const idx = levels.indexOf(this._thwIncidentSeverity);
    if (idx < levels.length - 1) {
      this._thwIncidentSeverity = levels[idx + 1];
      this._thwEscalationLevel = idx + 1;
    }
  }

  private _thwRenderIncidentCoordination(): any {
    const templates = this._thwGetCommunicationTemplates();
    const currentTemplate = templates.find(t => t.key === this._thwCommTemplate) || templates[0];
    const questions = this._thwGetPostIncidentQuestions();
    const severityMatrix = this._thwGetSeverityMatrix();
    const stakeholders = this._thwGetStakeholderMatrix();
    return html`
      <div class="incident-coordination" style="padding:12px">
        <div style="display:flex;gap:8px;margin-bottom:12px;flex-wrap:wrap">
          <button class="tab ${this._thwWarRoomActive ? 'active' : ''}" @click=${() => { this._thwToggleWarRoom(); }}>War Room ${this._thwWarRoomActive ? '(Active)' : ''}</button>
          <button class="tab" @click=${() => { this._thwEscalateIncident(); }}>Escalate (${this._thwIncidentSeverity})</button>
        </div>
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <table style="width:100%;border-collapse:collapse;font-size:11px">
            ...
            <tbody>${severityMatrix.map(r => html`
              <tr style="${r.severity.startsWith(this._thwIncidentSeverity) ? 'background:rgba(255,68,68,0.15)' : ''}">
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:4px;margin-bottom:8px;flex-wrap:wrap">
            ${templates.map(t => html`<button class="tab ${this._thwCommTemplate === t.key ? 'active' : ''}" @click=${() => { this._thwCommTemplate = t.key; }}>${t.subject.split(' - ')[0]}</button>`)}
          </div>
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          ${questions.map(q => html`
            <div style="margin-bottom:8px">
              ...
              ${q.type === 'select' ? html`
                <select style="background:#1a1d27;color:#ddd;border:1px solid #333;padding:4px;border-radius:3px;width:100%;font-size:12px" @change=${(e: any) => { this._thwPostIncidentAnswers = {...this._thwPostIncidentAnswers, [q.id]: e.target.value}; }}>
                  <option value="">Select...</option>
                  ${q.options.map(o => html`<option value="${o}" ${this._thwPostIncidentAnswers[q.id] === o ? 'selected' : ''}>${o}</option>`)}
                </select>
              ` : html`
                <textarea style="background:#1a1d27;color:#ddd;border:1px solid #333;padding:4px;border-radius:3px;width:100%;font-size:12px;min-height:40px" placeholder="Enter details..." @input=${(e: any) => { this._thwPostIncidentAnswers = {...this._thwPostIncidentAnswers, [q.id]: e.target.value}; }}></textarea>
              `}
            </div>
          `)}
          <div style="margin-top:8px">
            <label style="color:#aaa;font-size:12px;display:block;margin-bottom:4px">Lessons Learned</label>
            <textarea style="background:#1a1d27;color:#ddd;border:1px solid #333;padding:4px;border-radius:3px;width:100%;font-size:12px;min-height:60px" .value=${this._thwLessonsLearned} @input=${(e: any) => { this._thwLessonsLearned = e.target.value; }}></textarea>
          </div>
        </div>
      </div>
    `;
  ...
  @state() private _thwMetricData: Record<string, number[]> = {};
  @state() private _thwCompositeScore: number = 72;
  @state() private _thwMetricAlerts: string[] = [];
  @state() private _thwLeadingIndicators: string[] = ['Phishing Click Rate', 'Patch Compliance', 'Training Completion', 'Access Review Age'];
  @state() private _thwLaggingIndicators: string[] = ['Incident Count', 'MTTR', 'Data Breach Cost', 'Compliance Failures'];
  @state() private _thwExecutiveSummary: string = '';

  private _thwInitializeMetricData(): void {
    const metrics = ['Vulnerability Count', 'Incident Rate', 'Patch Coverage', 'Training Score', 'Compliance Pct', 'Access Anomalies'];
    metrics.forEach(m => {
      this._thwMetricData[m] = this._thwGenerateMockTimeSeries(1, 30)[0];
    });
  }

  private _thwCalculateCompositeScore(): number {
    ...
    for (const [metric, weight] of Object.entries(weights)) {
      const data = this._thwMetricData[metric];
    ...
    this._thwCompositeScore = Math.max(0, Math.min(100, score));
    return this._thwCompositeScore;
  }

  private _thwDetectMetricAnomalies(): string[] {
    const alerts: string[] = [];
    for (const [metric, data] of Object.entries(this._thwMetricData)) {
      if (!data || data.length < 10) continue;
      const outliers = this._thwDetectOutliersZScore(data);
    ...
    this._thwMetricAlerts = alerts;
    return alerts;
  }

  private _thwGenerateExecutiveSummary(): string {
    const score = this._thwCalculateCompositeScore();
    const alerts = this._thwDetectMetricAnomalies();
    ...
    summary += 'Leading indicators show ' + (this._thwLeadingIndicators.length > 2 ? 'generally positive' : 'mixed') + ' trends. ';
    if (alerts.length > 0) {
    ...
    this._thwExecutiveSummary = summary;
    return summary;
  }

  private _thwRenderMetricsCorrelation(): any {
    if (Object.keys(this._thwMetricData).length === 0) this._thwInitializeMetricData();
    const score = this._thwCalculateCompositeScore();
    const alerts = this._thwDetectMetricAnomalies();
    const metricNames = Object.keys(this._thwMetricData);
    const corrMatrix = metricNames.map((m1, i) =>
      metricNames.map((m2, j) => this._thwPearsonCorrelation(this._thwMetricData[m1] || [], this._thwMetricData[m2] || []))
    );
    ...
    return html`
      <div class="metrics-correlation" style="padding:12px">
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
            <div>
              ...
              ${this._thwLeadingIndicators.map(ind => html`<div style="color:#aaa;font-size:11px;padding:2px 0">- ${ind}</div>`)}
            </div>
            <div>
              <div style="color:#a4a;font-size:12px;font-weight:bold;margin-bottom:4px">Lagging (Reactive)</div>
              ${this._thwLaggingIndicators.map(ind => html`<div style="color:#aaa;font-size:11px;padding:2px 0">- ${ind}</div>`)}
            </div>
          </div>
        </div>
        <div class="card" style="padding:12px;margin-bottom:8px">
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="background:#1a1d27;padding:8px;border-radius:4px;color:#bbb;font-size:12px;line-height:1.6">${this._thwGenerateExecutiveSummary()}</div>
          <button class="btn" style="margin-top:8px;font-size:11px" @click=${() => { this._thwGenerateExecutiveSummary(); }}>Regenerate Summary</button>
        </div>
      </div>
    `;
  }
  ...
  @state() private _thwApiEndpoints: {id:string;path:string;method:string;status:string;latency:number;rateLimit:number}[] = [];
  @state() private _thwRateLimitPolicy: {endpoint:string;requestsPerMin:number;burstLimit:number;windowSec:number}[] = [];
  @state() private _thwApiKeys: {id:string;name:string;created:string;expires:string;status:string;lastUsed:string}[] = [];
  @state() private _thwWebhookStatuses: {id:string;url:string;events:string;lastDelivery:string;status:string;retryCount:number}[] = [];

  private _thwInitializeApiData(): void {
    this._thwApiEndpoints = [
  ...
  private _thwUpdateRateLimit(endpoint: string, field: string, value: number): void {
    this._thwRateLimitPolicy = this._thwRateLimitPolicy.map(p =>
      p.endpoint === endpoint ? { ...p, [field]: value } : p
    );
  }

  private _thwRenderApiGateway(): any {
    if (this._thwApiEndpoints.length === 0) this._thwInitializeApiData();
    const totalRpm = this._thwApiEndpoints.reduce((a, e) => a + (Math.random() * e.rateLimit * 0.5), 0);
      ...
      <div class="api-gateway" style="padding:12px">
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px">
            ...
            <div style="background:#1a1d27;padding:8px;border-radius:4px;text-align:center">
              <div style="color:#e0e0e0;font-size:20px;font-weight:bold">${this._thwApiEndpoints.length}</div>
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <table style="width:100%;border-collapse:collapse;font-size:11px">
            ...
            <tbody>${this._thwApiEndpoints.map(e => html`
              <tr>
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          ${this._thwRateLimitPolicy.map(p => html`
            <div style="display:flex;gap:8px;align-items:center;margin-bottom:6px;padding:4px;background:#1a1d27;border-radius:4px">
              ...
              <div style="display:flex;align-items:center;gap:4px">
                ...
                <input type="number" style="background:#0d0f17;color:#ddd;border:1px solid #333;padding:2px 4px;border-radius:3px;width:60px;font-size:11px" .value=${String(p.requestsPerMin)} @change=${(e: any) => { this._thwUpdateRateLimit(p.endpoint, 'requestsPerMin', Number(e.target.value)); }} />
              </div>
              <div style="display:flex;align-items:center;gap:4px">
                <span style="color:#888;font-size:10px">Burst:</span>
                <input type="number" style="background:#0d0f17;color:#ddd;border:1px solid #333;padding:2px 4px;border-radius:3px;width:60px;font-size:11px" .value=${String(p.burstLimit)} @change=${(e: any) => { this._thwUpdateRateLimit(p.endpoint, 'burstLimit', Number(e.target.value)); }} />
              </div>
              <div style="display:flex;align-items:center;gap:4px">
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <table style="width:100%;border-collapse:collapse;font-size:11px">
            ...
            <tbody>${this._thwApiKeys.map(k => html`
              <tr>
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          ${this._thwWebhookStatuses.map(w => html`
  ...
  @state() private _thwRenderTime: number = 0;
  @state() private _thwMemoryEstimate: number = 0;
  @state() private _thwCacheHits: number = 0;
  @state() private _thwCacheMisses: number = 0;
  @state() private _thwLazyLoadingEnabled: boolean = true;
  @state() private _thwVirtualScrollEnabled: boolean = false;
  @state() private _thwPerfHistory: {timestamp:number;renderMs:number;memoryKb:number;cacheRatio:number}[] = [];
  @state() private _thwDataSetSize: number = 1000;

  private _thwMeasurePerformance(): void {
    const start = performance.now();
    const data = Array.from({ length: this._thwDataSetSize }, (_, i) => ({
    ...
    this._thwRenderTime = Math.round((end - start) * 100) / 100;
    this._thwMemoryEstimate = Math.round((this._thwDataSetSize * 0.15) * 100) / 100;
    this._thwCacheHits = Math.floor(Math.random() * 80 + 60);
    this._thwCacheMisses = Math.floor(Math.random() * 30 + 10);
    this._thwPerfHistory.push({
      timestamp: Date.now(), renderMs: this._thwRenderTime,
      memoryKb: this._thwMemoryEstimate,
      cacheRatio: this._thwCacheHits / (this._thwCacheHits + this._thwCacheMisses)
    });
    if (this._thwPerfHistory.length > 20) this._thwPerfHistory = this._thwPerfHistory.slice(-20);
  }

  private _thwGetCacheRatio(): number {
    const total = this._thwCacheHits + this._thwCacheMisses;
    return total > 0 ? this._thwCacheHits / total : 0;
  }

  private _thwGetPerfRecommendation(): string {
    if (this._thwRenderTime > 50) return 'High render time detected. Consider enabling virtual scrolling and reducing data set size.';
    if (this._thwGetCacheRatio() < 0.7) return 'Cache hit ratio is low. Review cache invalidation strategy and increase cache TTL.';
    if (this._thwMemoryEstimate > 500) return 'High memory usage. Enable lazy loading and consider pagination for large datasets.';
    if (this._thwDataSetSize > 500 && !this._thwVirtualScrollEnabled) return 'Large dataset detected. Enable virtual scrolling for optimal performance.';
    return 'Performance is within acceptable parameters. Continue monitoring.';
  }

  private _thwRenderPerformancePanel(): any {
    if (this._thwPerfHistory.length === 0) this._thwMeasurePerformance();
    const cacheRatio = this._thwGetCacheRatio();
    const recommendation = this._thwGetPerfRecommendation();
    ...
    return html`
      <div class="perf-panel" style="padding:12px">
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px">
            <div style="background:#1a1d27;padding:8px;border-radius:4px;text-align:center">
              <div style="color:${this._thwRenderTime > 50 ? '#f44' : '#4f4'};font-size:18px;font-weight:bold">${this._thwRenderTime}ms</div>
              <div style="color:#888;font-size:10px">Render Time</div>
            </div>
            <div style="background:#1a1d27;padding:8px;border-radius:4px;text-align:center">
              <div style="color:${this._thwMemoryEstimate > 500 ? '#fa0' : '#4a9'};font-size:18px;font-weight:bold">${this._thwMemoryEstimate}KB</div>
            ...
            <div style="background:#1a1d27;padding:8px;border-radius:4px;text-align:center">
              <div style="color:#e0e0e0;font-size:18px;font-weight:bold">${this._thwDataSetSize}</div>
          ...
          <div style="margin-top:8px">
            <div style="display:flex;gap:8px;align-items:center">
              ...
              <input type="range" min="100" max="10000" step="100" .value=${String(this._thwDataSetSize)} @input=${(e: any) => { this._thwDataSetSize = Number(e.target.value); }} style="flex:1" />
              <button class="btn" style="font-size:11px" @click=${() => { this._thwMeasurePerformance(); }}>Benchmark</button>
            </div>
          </div>
        </div>
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:16px;flex-wrap:wrap">
            <label style="display:flex;align-items:center;gap:6px;color:#ddd;font-size:12px;cursor:pointer">
              <input type="checkbox" .checked=${this._thwLazyLoadingEnabled} @change=${(e: any) => { this._thwLazyLoadingEnabled = e.target.checked; }} />
              Lazy Loading
            </label>
            <label style="display:flex;align-items:center;gap:6px;color:#ddd;font-size:12px;cursor:pointer">
              <input type="checkbox" .checked=${this._thwVirtualScrollEnabled} @change=${(e: any) => { this._thwVirtualScrollEnabled = e.target.checked; }} />
              Virtual Scrolling
            </label>
          </div>
          ...
          <div style="display:flex;gap:12px;align-items:center;margin-bottom:8px">
            <div style="flex:1">
              <div style="display:flex;justify-content:space-between;color:#888;font-size:10px;margin-bottom:2px">
                <span>Hits: ${this._thwCacheHits}</span><span>Misses: ${this._thwCacheMisses}</span>
              </div>
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;height:40px;align-items:flex-end;gap:1px">
            ${this._thwPerfHistory.slice(-15).map(h => html`
...
// ===== SECURITY ROI DASHBOARD (R22) =====
  ...
  private _renderBudgetPlanning() {
    const budgetData = [
      { category: "Personnel & Training", planned: 2757000, actual: 1000000, utilization: 36.3, q1: "15%", q2: "20%", q3: "18%", q4: "10%" },
      { category: "Tooling & Licensing", planned: 2764000, actual: 1013000, utilization: 36.6, q1: "18%", q2: "25%", q3: "29%", q4: "11%" },
      { category: "Infrastructure Security", planned: 2771000, actual: 1026000, utilization: 37.0, q1: "21%", q2: "30%", q3: "24%", q4: "12%" },
      { category: "Compliance & Audit", planned: 2778000, actual: 1039000, utilization: 37.4, q1: "24%", q2: "35%", q3: "19%", q4: "13%" },
      { category: "Incident Response", planned: 2785000, actual: 1052000, utilization: 37.8, q1: "27%", q2: "24%", q3: "30%", q4: "14%" },
      { category: "Third-Party Assessments", planned: 2792000, actual: 1065000, utilization: 38.1, q1: "30%", q2: "29%", q3: "25%", q4: "15%" },
      { category: "Security Awareness", planned: 2799000, actual: 1078000, utilization: 38.5, q1: "17%", q2: "34%", q3: "20%", q4: "16%" },
      { category: "Research & Innovation", planned: 2806000, actual: 1091000, utilization: 38.9, q1: "20%", q2: "23%", q3: "31%", q4: "17%" },
    ];
    ...
    const headcount = [
      { team: "SOC Tier 1", current: 11, target: 11, gap: 0, avgSalary: "112k" },
      { team: "SOC Tier 2", current: 3, target: 10, gap: 5, avgSalary: "141k" },
      { team: "Threat Intel", current: 12, target: 9, gap: 4, avgSalary: "170k" },
      { team: "Red Team", current: 4, target: 8, gap: 3, avgSalary: "88k" },
      { team: "GRC", current: 13, target: 7, gap: 2, avgSalary: "117k" },
      { team: "AppSec", current: 5, target: 6, gap: 1, avgSalary: "146k" },
      { team: "Cloud Sec", current: 14, target: 5, gap: 0, avgSalary: "175k" },
      { team: "Identity & Access", current: 6, target: 4, gap: 5, avgSalary: "93k" },
    ];
    const vendorSpend = [
      { vendor: "CrowdStrike", annual: "390k", contractEnd: "2026-01", renewalRisk: "Low", satisfaction: 3 },
      { vendor: "Palo Alto", annual: "421k", contractEnd: "2026-02", renewalRisk: "Medium", satisfaction: 5 },
      { vendor: "Splunk", annual: "452k", contractEnd: "2026-03", renewalRisk: "High", satisfaction: 4 },
      { vendor: "Qualys", annual: "483k", contractEnd: "2026-04", renewalRisk: "Low", satisfaction: 3 },
      { vendor: "Rapid7", annual: "514k", contractEnd: "2026-05", renewalRisk: "Medium", satisfaction: 5 },
      { vendor: "Mandiant", annual: "545k", contractEnd: "2026-06", renewalRisk: "High", satisfaction: 4 },
      { vendor: "Zscaler", annual: "576k", contractEnd: "2026-07", renewalRisk: "Low", satisfaction: 3 },
      { vendor: "Duo Security", annual: "607k", contractEnd: "2026-08", renewalRisk: "Medium", satisfaction: 5 },
    ];
    const roiProjections = [
      { area: "Threat Detection", investment: "502k", projectedReturn: "2183k", roiMultiple: "2.7x", confidence: 84 },
      { area: "Incident Reduction", investment: "545k", projectedReturn: "2230k", roiMultiple: "2.6x", confidence: 65 },
      { area: "Compliance Savings", investment: "588k", projectedReturn: "2277k", roiMultiple: "2.5x", confidence: 82 },
      { area: "Automation Gains", investment: "631k", projectedReturn: "2324k", roiMultiple: "2.4x", confidence: 63 },
      { area: "Risk Avoidance", investment: "674k", projectedReturn: "2371k", roiMultiple: "2.3x", confidence: 80 },
    ];
    return html`
  ...
  private _renderMetricsNormalization() {
    const kpiCatalog = [
      { id: "kpi-1", name: "MTTD", owner: "SOC", unit: "minutes", target: 97, current: 62, benchmark: 70, collection: "auto", frequency: "realtime" },
      { id: "kpi-2", name: "MTTR", owner: "GRC", unit: "%", target: 70, current: 69, benchmark: 81, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-3", name: "MTTC", owner: "AppSec", unit: "score", target: 73, current: 76, benchmark: 92, collection: "manual", frequency: "weekly" },
      { id: "kpi-4", name: "Vuln SLA Compliance", owner: "Cloud Sec", unit: "count", target: 76, current: 83, benchmark: 69, collection: "auto", frequency: "monthly" },
      { id: "kpi-5", name: "Patch Coverage", owner: "Identity", unit: "days", target: 79, current: 90, benchmark: 80, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-6", name: "Phishing Click Rate", owner: "Threat Intel", unit: "minutes", target: 82, current: 97, benchmark: 91, collection: "manual", frequency: "daily" },
      { id: "kpi-7", name: "Training Completion", owner: "Security Ops", unit: "%", target: 85, current: 58, benchmark: 68, collection: "auto", frequency: "weekly" },
      { id: "kpi-8", name: "Escalation Rate", owner: "Risk Mgmt", unit: "score", target: 88, current: 65, benchmark: 79, collection: "semi-auto", frequency: "monthly" },
      { id: "kpi-9", name: "False Positive Rate", owner: "SOC", unit: "count", target: 91, current: 72, benchmark: 90, collection: "manual", frequency: "realtime" },
      { id: "kpi-10", name: "Threat Intel Actionability", owner: "GRC", unit: "days", target: 94, current: 79, benchmark: 67, collection: "auto", frequency: "daily" },
      { id: "kpi-11", name: "Endpoint Compliance", owner: "AppSec", unit: "minutes", target: 97, current: 86, benchmark: 78, collection: "semi-auto", frequency: "weekly" },
      { id: "kpi-12", name: "Cloud Misconfig Score", owner: "Cloud Sec", unit: "%", target: 70, current: 93, benchmark: 89, collection: "manual", frequency: "monthly" },
      { id: "kpi-13", name: "Identity Anomaly Rate", owner: "Identity", unit: "score", target: 73, current: 100, benchmark: 66, collection: "auto", frequency: "realtime" },
      { id: "kpi-14", name: "DLP Events", owner: "Threat Intel", unit: "count", target: 76, current: 61, benchmark: 77, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-15", name: "Vendor Risk Avg", owner: "Security Ops", unit: "days", target: 79, current: 68, benchmark: 88, collection: "manual", frequency: "weekly" },
      { id: "kpi-16", name: "Compliance Audit Pass Rate", owner: "Risk Mgmt", unit: "minutes", target: 82, current: 75, benchmark: 65, collection: "auto", frequency: "monthly" },
      { id: "kpi-17", name: "Awareness Score", owner: "SOC", unit: "%", target: 85, current: 82, benchmark: 76, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-18", name: "SOC Utilization", owner: "GRC", unit: "score", target: 88, current: 89, benchmark: 87, collection: "manual", frequency: "daily" },
      { id: "kpi-19", name: "Automation Coverage", owner: "AppSec", unit: "count", target: 91, current: 96, benchmark: 98, collection: "auto", frequency: "weekly" },
      { id: "kpi-20", name: "Risk Register Currency", owner: "Cloud Sec", unit: "days", target: 94, current: 57, benchmark: 75, collection: "semi-auto", frequency: "monthly" },
    ];
    const benchmarkSources = [
      { source: "NIST CSF", mappedKPIs: 6, alignment: 96, lastReview: "2026-04-04", status: "aligned" },
      { source: "CIS Controls v8", mappedKPIs: 7, alignment: 74, lastReview: "2026-05-27", status: "partial" },
      { source: "ISO 27001:2022", mappedKPIs: 8, alignment: 91, lastReview: "2026-06-22", status: "reviewing" },
      { source: "PCI DSS 4.0", mappedKPIs: 3, alignment: 69, lastReview: "2026-01-17", status: "aligned" },
      { source: "SOC 2 Type II", mappedKPIs: 4, alignment: 86, lastReview: "2026-02-12", status: "partial" },
      { source: "MITRE ATT&CK", mappedKPIs: 5, alignment: 64, lastReview: "2026-03-07", status: "reviewing" },
      { source: "SANS Top 20", mappedKPIs: 6, alignment: 81, lastReview: "2026-04-02", status: "aligned" },
      { source: "OWASP Top 10", mappedKPIs: 7, alignment: 98, lastReview: "2026-05-25", status: "partial" },
    ];
  ...
  private _renderThreatHuntingCampaigns() {
    const campaigns = [
      { id: "HC-1001", name: "Lateral Movement Sweep", status: "active", hypothesis: "H1: Actors using pass-the-hash for lateral movement", leadHunter: "Alice Chen", findings: 31, startDate: "2026-04-24", endDate: null, effectiveness: 40 },
      { id: "HC-1002", name: "Credential Harvesting Hunt", status: "completed", hypothesis: "H2: Actors using web shells for persistence", leadHunter: "Bob Martinez", findings: 34, startDate: "2026-03-07", endDate: "2026-06-13", effectiveness: 59 },
      { id: "HC-1003", name: "Persistence Mechanism Audit", status: "planned", hypothesis: "H3: Actors using scheduled tasks for data theft", leadHunter: "Carol Wu", findings: 37, startDate: "2026-02-18", endDate: null, effectiveness: 78 },
      { id: "HC-1004", name: "C2 Beacon Detection", status: "in-review", hypothesis: "H4: Actors using DNS tunneling for C2 communication", leadHunter: "Dave Kim", findings: 40, startDate: "2026-01-01", endDate: null, effectiveness: 97 },
      { id: "HC-1005", name: "Data Exfiltration Patterns", status: "active", hypothesis: "H5: Actors using encrypted channels for privilege escalation", leadHunter: "Eve Johnson", findings: 43, startDate: "2026-04-12", endDate: null, effectiveness: 57 },
      { id: "HC-1006", name: "Privilege Escalation Scan", status: "completed", hypothesis: "H6: Actors using token impersonation for defense evasion", leadHunter: "Frank Liu", findings: 46, startDate: "2026-03-23", endDate: "2026-04-25", effectiveness: 76 },
      { id: "HC-1007", name: "Supply Chain Implant Hunt", status: "planned", hypothesis: "H7: Actors using poisoned images for initial access", leadHunter: "Grace Park", findings: 1, startDate: "2026-02-06", endDate: null, effectiveness: 95 },
      { id: "HC-1008", name: "Insider Threat Indicators", status: "in-review", hypothesis: "H8: Actors using legitimate tools for credential access", leadHunter: "Hector Silva", findings: 4, startDate: "2026-01-17", endDate: null, effectiveness: 55 },
      { id: "HC-1009", name: "Cloud Metadata Analysis", status: "active", hypothesis: "H9: Actors using API keys for command execution", leadHunter: "Alice Chen", findings: 7, startDate: "2026-04-28", endDate: null, effectiveness: 74 },
      { id: "HC-1010", name: "DNS Tunnel Detection", status: "completed", hypothesis: "H10: Actors using encoded subdomains for exfiltration", leadHunter: "Bob Martinez", findings: 10, startDate: "2026-03-11", endDate: "2026-05-09", effectiveness: 93 },
      { id: "HC-1011", name: "Fileless Malware Search", status: "planned", hypothesis: "H11: Actors using WMI providers for discovery", leadHunter: "Carol Wu", findings: 13, startDate: "2026-02-22", endDate: null, effectiveness: 53 },
      { id: "HC-1012", name: "Zero-Day Exploit Traces", status: "in-review", hypothesis: "H12: Actors using exploit kits for collection", leadHunter: "Dave Kim", findings: 16, startDate: "2026-01-05", endDate: null, effectiveness: 72 },
    ];
    const hunterLeaderboard = [
      { hunter: "Alice Chen", campaigns: 12, findings: 96, highSeverity: 9, avgScore: 94, streak: 8 },
      { hunter: "Bob Martinez", campaigns: 9, findings: 9, highSeverity: 14, avgScore: 87, streak: 1 },
      { hunter: "Carol Wu", campaigns: 6, findings: 38, highSeverity: 19, avgScore: 80, streak: 2 },
      { hunter: "Dave Kim", campaigns: 3, findings: 67, highSeverity: 24, avgScore: 73, streak: 3 },
      { hunter: "Eve Johnson", campaigns: 13, findings: 96, highSeverity: 3, avgScore: 66, streak: 4 },
      { hunter: "Frank Liu", campaigns: 10, findings: 9, highSeverity: 8, avgScore: 59, streak: 5 },
      { hunter: "Grace Park", campaigns: 7, findings: 38, highSeverity: 13, avgScore: 96, streak: 6 },
      { hunter: "Hector Silva", campaigns: 4, findings: 67, highSeverity: 18, avgScore: 89, streak: 7 },
    ];
    const mitreMapping = [
      { tactic: "Initial Access", techniques: 8, campaigns: 2, coverage: 80 },
      { tactic: "Execution", techniques: 7, campaigns: 1, coverage: 60 },
      { tactic: "Persistence", techniques: 6, campaigns: 6, coverage: 40 },
      { tactic: "Privilege Escalation", techniques: 5, campaigns: 5, coverage: 91 },
      { tactic: "Defense Evasion", techniques: 4, campaigns: 4, coverage: 71 },
      { tactic: "Credential Access", techniques: 3, campaigns: 3, coverage: 51 },
      { tactic: "Discovery", techniques: 2, campaigns: 2, coverage: 31 },
      { tactic: "Lateral Movement", techniques: 12, campaigns: 1, coverage: 82 },
      { tactic: "Collection", techniques: 11, campaigns: 6, coverage: 62 },
      { tactic: "Exfiltration", techniques: 10, campaigns: 5, coverage: 42 },
      { tactic: "Command & Control", techniques: 9, campaigns: 4, coverage: 93 },
      { tactic: "Impact", techniques: 8, campaigns: 3, coverage: 73 },
    ];
    return html`
  ...
  private _renderControlInventory() {
    const controls = [
      { id: "CTL-2001", name: "MFA Enforcement", domain: "Access Control", status: "implemented", effectiveness: 96, lastTest: "2026-04-16", nextReview: "2026-12-16", owner: "SOC", risk: "Low" },
      { id: "CTL-2002", name: "Network Segmentation", domain: "Network Security", status: "partial", effectiveness: 6, lastTest: "2026-03-01", nextReview: "2026-05-07", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2003", name: "EDR Deployment", domain: "Endpoint Protection", status: "planned", effectiveness: 13, lastTest: "2026-02-14", nextReview: "2026-06-26", owner: "IT Ops", risk: "High" },
      { id: "CTL-2004", name: "DLP Policy", domain: "Data Protection", status: "gap", effectiveness: 20, lastTest: "2026-01-27", nextReview: "2026-07-17", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2005", name: "SSO Integration", domain: "Identity Management", status: "implemented", effectiveness: 49, lastTest: "2026-04-12", nextReview: "2026-08-08", owner: "IAM", risk: "Low" },
      { id: "CTL-2006", name: "SAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 34, lastTest: "2026-03-25", nextReview: "2026-09-27", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2007", name: "CSPM Scanning", domain: "Cloud Security", status: "planned", effectiveness: 41, lastTest: "2026-02-10", nextReview: "2026-10-18", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2008", name: "Badge Access", domain: "Physical Security", status: "gap", effectiveness: 48, lastTest: "2026-01-23", nextReview: "2026-11-09", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2009", name: "Least Privilege", domain: "Access Control", status: "implemented", effectiveness: 61, lastTest: "2026-04-08", nextReview: "2026-12-28", owner: "SOC", risk: "Low" },
      { id: "CTL-2010", name: "Firewall Rules", domain: "Network Security", status: "partial", effectiveness: 11, lastTest: "2026-03-21", nextReview: "2026-05-19", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2011", name: "Disk Encryption", domain: "Endpoint Protection", status: "planned", effectiveness: 18, lastTest: "2026-02-06", nextReview: "2026-06-10", owner: "IT Ops", risk: "High" },
      { id: "CTL-2012", name: "Data Classification", domain: "Data Protection", status: "gap", effectiveness: 25, lastTest: "2026-01-19", nextReview: "2026-07-01", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2013", name: "PAM Implementation", domain: "Identity Management", status: "implemented", effectiveness: 73, lastTest: "2026-04-04", nextReview: "2026-08-20", owner: "IAM", risk: "Low" },
      { id: "CTL-2014", name: "DAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 39, lastTest: "2026-03-17", nextReview: "2026-09-11", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2015", name: "IAM Policy Review", domain: "Cloud Security", status: "planned", effectiveness: 46, lastTest: "2026-02-02", nextReview: "2026-10-02", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2016", name: "Visitor Management", domain: "Physical Security", status: "gap", effectiveness: 2, lastTest: "2026-01-15", nextReview: "2026-11-21", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2017", name: "Access Reviews", domain: "Access Control", status: "implemented", effectiveness: 85, lastTest: "2026-04-28", nextReview: "2026-12-12", owner: "SOC", risk: "Low" },
      { id: "CTL-2018", name: "IDS/IPS Tuning", domain: "Network Security", status: "partial", effectiveness: 16, lastTest: "2026-03-13", nextReview: "2026-05-03", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2019", name: "Patch Management", domain: "Endpoint Protection", status: "planned", effectiveness: 23, lastTest: "2026-02-26", nextReview: "2026-06-22", owner: "IT Ops", risk: "High" },
      { id: "CTL-2020", name: "Backup Encryption", domain: "Data Protection", status: "gap", effectiveness: 30, lastTest: "2026-01-11", nextReview: "2026-07-13", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2021", name: "Password Policy", domain: "Identity Management", status: "implemented", effectiveness: 97, lastTest: "2026-04-24", nextReview: "2026-08-04", owner: "IAM", risk: "Low" },
      { id: "CTL-2022", name: "Container Scanning", domain: "Application Security", status: "partial", effectiveness: 44, lastTest: "2026-03-09", nextReview: "2026-09-23", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2023", name: "WAF Configuration", domain: "Cloud Security", status: "planned", effectiveness: 0, lastTest: "2026-02-22", nextReview: "2026-10-14", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2024", name: "CCTV Coverage", domain: "Physical Security", status: "gap", effectiveness: 7, lastTest: "2026-01-07", nextReview: "2026-11-05", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2025", name: "RBAC Enforcement", domain: "Access Control", status: "implemented", effectiveness: 50, lastTest: "2026-04-20", nextReview: "2026-12-24", owner: "SOC", risk: "Low" },
      { id: "CTL-2026", name: "VPN Management", domain: "Network Security", status: "partial", effectiveness: 21, lastTest: "2026-03-05", nextReview: "2026-05-15", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2027", name: "App Whitelisting", domain: "Endpoint Protection", status: "planned", effectiveness: 28, lastTest: "2026-02-18", nextReview: "2026-06-06", owner: "IT Ops", risk: "High" },
      { id: "CTL-2028", name: "Key Management", domain: "Data Protection", status: "gap", effectiveness: 35, lastTest: "2026-01-03", nextReview: "2026-07-25", owner: "Data Gov", risk: "Critical" },
    ];
    const gapAnalysis = [
      { gap: "Insufficient MFA coverage for legacy apps", severity: "High", remediationPlan: "Plan R3001", eta: "2026-Q4", estimatedCost: "104k" },
      { gap: "Missing network micro-segmentation", severity: "Medium", remediationPlan: "Plan R3002", eta: "2026-Q3", estimatedCost: "133k" },
      { gap: "Inconsistent EDR deployment", severity: "Medium", remediationPlan: "Plan R3003", eta: "2026-Q2", estimatedCost: "162k" },
      { gap: "DLP not covering cloud storage", severity: "Low", remediationPlan: "Plan R3004", eta: "2026-Q4", estimatedCost: "191k" },
      { gap: "SSO not integrated with all SaaS", severity: "High", remediationPlan: "Plan R3005", eta: "2026-Q3", estimatedCost: "39k" },
    ];
    return html`
  ...
  private _renderIncidentCostTracker() {
    const incidents = [
      { id: "INC-7001", name: "Security Incident 1", severity: "Critical", totalCost: 1060000, responseCost: 159000, recoveryCost: 212000, legalCost: 212000, regulatoryCost: 95400, insuranceClaim: 636000, avoidedCost: 212000, date: "2026-04-08" },
      { id: "INC-7002", name: "Security Incident 2", severity: "High", totalCost: 1063000, responseCost: 233860, recoveryCost: 329530, legalCost: 180710, regulatoryCost: 159450, insuranceClaim: 0, avoidedCost: 241000, date: "2026-03-17" },
      { id: "INC-7003", name: "Security Incident 3", severity: "Medium", totalCost: 1066000, responseCost: 309140, recoveryCost: 223860, legalCost: 149240, regulatoryCost: 106600, insuranceClaim: 586300, avoidedCost: 270000, date: "2026-02-26" },
      { id: "INC-7004", name: "Security Incident 4", severity: "Low", totalCost: 1069000, responseCost: 160350, recoveryCost: 342080, legalCost: 117590, regulatoryCost: 53450, insuranceClaim: 0, avoidedCost: 299000, date: "2026-01-07" },
      { id: "INC-7005", name: "Security Incident 5", severity: "Critical", totalCost: 1072000, responseCost: 235840, recoveryCost: 235840, legalCost: 85760, regulatoryCost: 117920, insuranceClaim: 536000, avoidedCost: 328000, date: "2026-04-16" },
      { id: "INC-7006", name: "Security Incident 6", severity: "High", totalCost: 1075000, responseCost: 311750, recoveryCost: 354750, legalCost: 53750, regulatoryCost: 64500, insuranceClaim: 0, avoidedCost: 357000, date: "2026-03-25" },
      { id: "INC-7007", name: "Security Incident 7", severity: "Medium", totalCost: 1078000, responseCost: 161700, recoveryCost: 247940, legalCost: 194040, regulatoryCost: 129360, insuranceClaim: 485100, avoidedCost: 386000, date: "2026-02-06" },
      { id: "INC-7008", name: "Security Incident 8", severity: "Low", totalCost: 1081000, responseCost: 237820, recoveryCost: 367540, legalCost: 162150, regulatoryCost: 75670, insuranceClaim: 0, avoidedCost: 415000, date: "2026-01-15" },
      { id: "INC-7009", name: "Security Incident 9", severity: "Critical", totalCost: 1084000, responseCost: 314360, recoveryCost: 260160, legalCost: 130080, regulatoryCost: 140920, insuranceClaim: 433600, avoidedCost: 444000, date: "2026-04-24" },
      { id: "INC-7010", name: "Security Incident 10", severity: "High", totalCost: 1087000, responseCost: 163050, recoveryCost: 380450, legalCost: 97830, regulatoryCost: 86960, insuranceClaim: 0, avoidedCost: 473000, date: "2026-03-05" },
      { id: "INC-7011", name: "Security Incident 11", severity: "Medium", totalCost: 1090000, responseCost: 239800, recoveryCost: 272500, legalCost: 65400, regulatoryCost: 152600, insuranceClaim: 381500, avoidedCost: 6000, date: "2026-02-14" },
      { id: "INC-7012", name: "Security Incident 12", severity: "Low", totalCost: 1093000, responseCost: 316970, recoveryCost: 393480, legalCost: 207670, regulatoryCost: 98370, insuranceClaim: 0, avoidedCost: 35000, date: "2026-01-23" },
    ];
    const yearlyTrend = [
      { month: "Jan", incidents: 6, totalCost: "634k", avgCost: "199k", insured: 50 },
      { month: "Feb", incidents: 3, totalCost: "677k", avgCost: "65k", insured: 50 },
      { month: "Mar", incidents: 11, totalCost: "720k", avgCost: "112k", insured: 50 },
      { month: "Apr", incidents: 8, totalCost: "763k", avgCost: "159k", insured: 50 },
      { month: "May", incidents: 5, totalCost: "55k", avgCost: "25k", insured: 50 },
      { month: "Jun", incidents: 2, totalCost: "98k", avgCost: "72k", insured: 50 },
    ];
