...
`;
  ...
  @state() private _mitreNavigatZones: Array<{id:string;name:string;trustLevel:number;subnet:string;devices:number;policy:string;lastAudit:string}> = [
  ...
  @state() private _mitreNavigatSegRules: Array<{id:string;source:string;dest:string;action:string;protocol:string;port:string;status:string;hits:number}> = [
  ...
  @state() private _mitreNavigatCrossZoneTraffic: Array<{source:string;dest:string;bytes:number;sessions:number;violations:number}> = [
  ...
  @state() private _mitreNavigatMicroSegGaps: Array<{id:string;zone:string;gapType:string;severity:string;recommendation:string}> = [
  ...
  private _renderMitrenavigatNetworkSeg(): TemplateResult {
    const zones = this._mitreNavigatZones;
    const gaps = this._mitreNavigatMicroSegGaps;
    return html`
  ...
  @state() private _mitreNavigatContainerScans: Array<{image:string;registry:string;critical:number;high:number;medium:number;scanDate:string;status:string}> = [
  ...
  @state() private _mitreNavigatK8sPods: Array<{namespace:string;pod:string;securityContext:string;hostIPC:boolean;hostPID:boolean;privileged:boolean;riskLevel:string}> = [
  ...
  @state() private _mitreNavigatServerlessRisk: Array<{function:string;runtime:string;timeout:number;iamPerms:string;externalCalls:number;riskScore:number}> = [
  ...
  @state() private _mitreNavigatRuntimeAlerts: Array<{id:string;workload:string;alertType:string;severity:string;description:string;timestamp:string}> = [
  ...
  private _renderMitrenavigatCloudWl(): TemplateResult {
    const containers = this._mitreNavigatContainerScans;
    const pods = this._mitreNavigatK8sPods;
    const alerts = this._mitreNavigatRuntimeAlerts;
    return html`
  ...
  @state() private _mitreNavigatCourses: Array<{id:string;title:string;category:string;duration:number;enrolled:number;completed:number;difficulty:string;rating:number}> = [
  ...
  @state() private _mitreNavigatLearningPaths: Array<{id:string;name:string;courseIds:string[];progress:number;enrolled:number}> = [
  ...
  @state() private _mitreNavigatDeptCompliance: Array<{dept:string;trainedPct:number;targetPct:number;avgScore:number;certCount:number}> = [
  ...
  @state() private _mitreNavigatSkillsGaps: Array<{skill:string;current:number;required:number;gap:number;priority:string}> = [
  ...
  private _renderMitrenavigatTraining(): TemplateResult {
    const courses = this._mitreNavigatCourses;
    const deptComp = this._mitreNavigatDeptCompliance;
    return html`
  ...
  @state() private _mitreNavigatCorrRules: Array<{id:string;name:string;sources:string[];logic:string;severity:string;active:boolean;lastTriggered:string}> = [
  ...
  @state() private _mitreNavigatEventTimeline: Array<{timestamp:string;source:string;eventType:string;details:string;correlated:boolean}> = [
  ...
  @state() private _mitreNavigatFalsePosMetrics: {totalEvents:number;correlatedEvents:number;falsePositives:number;fpRate:number;topFpRules:string[]} = {
  ...
  @state() private _mitreNavigatEventPatterns: Array<{id:string;pattern:string;frequency:number;firstSeen:string;lastSeen:string;status:string}> = [
  ...
  private _renderMitrenavigatEventCorr(): TemplateResult {
    const rules = this._mitreNavigatCorrRules;
    const timeline = this._mitreNavigatEventTimeline;
    const fpMetrics = this._mitreNavigatFalsePosMetrics;
    return html`
  ...
  @state() private _complianceCalendarEvents: Array<{id: string; title: string; deadline: string; category: string; status: string; assignee: string; notes: string}> = [
    {id: 'cc-mitre-navigator-001', title: 'SOX Section 404 Compliance Review', deadline: '2026-06-30', category: 'Regulatory', status: 'In Progress', assignee: 'Compliance Team', notes: 'Annual internal control assessment for financial reporting'},
    {id: 'cc-mitre-navigator-002', title: 'GDPR Data Protection Impact Assessment', deadline: '2026-05-15', category: 'Privacy', status: 'Pending', assignee: 'DPO Office', notes: 'Quarterly DPIA for high-risk processing activities'},
    {id: 'cc-mitre-navigator-003', title: 'ISO 27001 Surveillance Audit', deadline: '2026-07-20', category: 'Certification', status: 'Scheduled', assignee: 'QA Lead', notes: 'Stage 2 surveillance audit for ISMS certification renewal'},
    {id: 'cc-mitre-navigator-004', title: 'PCI DSS v4.0 Gap Analysis', deadline: '2026-05-30', category: 'Payment', status: 'Not Started', assignee: 'Security Architect', notes: 'Assess current state against PCI DSS v4.0 requirements'},
    {id: 'cc-mitre-navigator-005', title: 'NIST CSF Self-Assessment', deadline: '2026-08-15', category: 'Framework', status: 'Pending', assignee: 'CISO', notes: 'Biannual self-assessment against NIST Cybersecurity Framework'},
    {id: 'cc-mitre-navigator-006', title: 'HIPAA Security Rule Audit', deadline: '2026-06-15', category: 'Healthcare', status: 'In Progress', assignee: 'Compliance Analyst', notes: 'Annual audit of administrative, physical, and technical safeguards'},
    {id: 'cc-mitre-navigator-007', title: 'SOC 2 Type II Report Renewal', deadline: '2026-09-30', category: 'Audit', status: 'Scheduled', assignee: 'External Auditor', notes: 'Engage auditor for SOC 2 Type II examination period'},
    {id: 'cc-mitre-navigator-008', title: 'FedRAMP Authorization Review', deadline: '2026-07-01', category: 'Government', status: 'Pending', assignee: 'FedRAMP PMO', notes: 'Review and update security authorization package'},
    {id: 'cc-mitre-navigator-009', title: 'Annual Penetration Test', deadline: '2026-10-15', category: 'Testing', status: 'Not Started', assignee: 'Red Team Lead', notes: 'Comprehensive external and internal penetration testing engagement'},
    {id: 'cc-mitre-navigator-010', title: 'Security Awareness Training Rollout', deadline: '2026-05-01', category: 'Training', status: 'Completed', assignee: 'Security Awareness Manager', notes: 'Q2 organization-wide security awareness training program'},
    {id: 'cc-mitre-navigator-011', title: 'Vendor Security Review Cycle', deadline: '2026-06-01', category: 'Third Party', status: 'In Progress', assignee: 'Vendor Manager', notes: 'Quarterly review of critical vendor security posture and SLA compliance'},
    {id: 'cc-mitre-navigator-012', title: 'Incident Response Plan Update', deadline: '2026-05-20', category: 'Operations', status: 'Pending', assignee: 'IR Team Lead', notes: 'Update IR procedures based on lessons learned from recent incidents'},
  ];
  ...
  @state() private _dataQualityMetrics: Array<{id: string; dataSource: string; completenessScore: number; accuracyScore: number; timelinessScore: number; consistencyScore: number; reliabilityRank: number; lastUpdated: string; issues: string[]; trend: string}> = [
    {id: 'dq-mitre-navigator-001', dataSource: 'Vulnerability Scanner Feed', completenessScore: 94, accuracyScore: 91, timelinessScore: 88, consistencyScore: 95, reliabilityRank: 1, lastUpdated: '2026-04-23T08:00:00Z', issues: ['Minor delay in Nessus plugin updates'], trend: 'improving'},
    {id: 'dq-mitre-navigator-002', dataSource: 'SIEM Event Logs', completenessScore: 87, accuracyScore: 93, timelinessScore: 96, consistencyScore: 89, reliabilityRank: 2, lastUpdated: '2026-04-23T07:30:00Z', issues: ['Some log sources showing gaps in off-hours'], trend: 'stable'},
    {id: 'dq-mitre-navigator-003', dataSource: 'Asset Inventory Database', completenessScore: 82, accuracyScore: 78, timelinessScore: 75, consistencyScore: 84, reliabilityRank: 4, lastUpdated: '2026-04-22T18:00:00Z', issues: ['Shadow IT devices not fully cataloged', 'Decommissioned assets still listed'], trend: 'declining'},
    {id: 'dq-mitre-navigator-004', dataSource: 'Threat Intelligence Platform', completenessScore: 91, accuracyScore: 89, timelinessScore: 92, consistencyScore: 87, reliabilityRank: 3, lastUpdated: '2026-04-23T06:00:00Z', issues: ['Some IOC sources lack confidence scoring'], trend: 'improving'},
    {id: 'dq-mitre-navigator-005', dataSource: 'Identity Provider Logs', completenessScore: 96, accuracyScore: 97, timelinessScore: 98, consistencyScore: 96, reliabilityRank: 1, lastUpdated: '2026-04-23T09:00:00Z', issues: [], trend: 'stable'},
    {id: 'dq-mitre-navigator-006', dataSource: 'Cloud Security Posture Data', completenessScore: 79, accuracyScore: 85, timelinessScore: 82, consistencyScore: 81, reliabilityRank: 5, lastUpdated: '2026-04-23T05:00:00Z', issues: ['Multi-cloud coverage gaps', 'API rate limiting affects scan frequency'], trend: 'declining'},
    {id: 'dq-mitre-navigator-007', dataSource: 'Compliance Evidence Store', completenessScore: 88, accuracyScore: 86, timelinessScore: 80, consistencyScore: 83, reliabilityRank: 4, lastUpdated: '2026-04-21T14:00:00Z', issues: ['Manual evidence collection delays', 'Inconsistent formatting across frameworks'], trend: 'stable'},
    {id: 'dq-mitre-navigator-008', dataSource: 'Network Flow Data', completenessScore: 92, accuracyScore: 90, timelinessScore: 94, consistencyScore: 91, reliabilityRank: 2, lastUpdated: '2026-04-23T08:30:00Z', issues: ['Encrypted traffic classification accuracy needs improvement'], trend: 'improving'},
  ];
  ...
  @state() private _workflowTemplates: Array<{id: string; name: string; description: string; category: string; steps: number; avgDuration: string; successRate: number; lastRun: string; manualInterventions: number; optimizationScore: number}> = [
    {id: 'wf-mitre-navigator-001', name: 'Vulnerability Triage Pipeline', description: 'Automated vulnerability assessment, prioritization, and assignment workflow', category: 'Vulnerability Management', steps: 8, avgDuration: '12 min', successRate: 94.5, lastRun: '2026-04-23T06:00:00Z', manualInterventions: 2, optimizationScore: 87},
    {id: 'wf-mitre-navigator-002', name: 'Incident Response Orchestration', description: 'End-to-end IR workflow from detection to containment and recovery', category: 'Incident Response', steps: 15, avgDuration: '45 min', successRate: 89.2, lastRun: '2026-04-22T14:30:00Z', manualInterventions: 5, optimizationScore: 72},
    {id: 'wf-mitre-navigator-003', name: 'Access Request Approval Chain', description: 'Multi-level approval workflow for privileged access requests', category: 'Identity & Access', steps: 6, avgDuration: '4 hours', successRate: 97.1, lastRun: '2026-04-23T09:15:00Z', manualInterventions: 1, optimizationScore: 92},
    {id: 'wf-mitre-navigator-004', name: 'Compliance Evidence Collection', description: 'Automated gathering and packaging of audit evidence artifacts', category: 'Compliance', steps: 10, avgDuration: '30 min', successRate: 91.8, lastRun: '2026-04-21T10:00:00Z', manualInterventions: 3, optimizationScore: 78},
    {id: 'wf-mitre-navigator-005', name: 'Security Patch Deployment', description: 'Staged patch rollout with validation and rollback capability', category: 'Patch Management', steps: 12, avgDuration: '2 hours', successRate: 96.3, lastRun: '2026-04-20T22:00:00Z', manualInterventions: 1, optimizationScore: 85},
    {id: 'wf-mitre-navigator-006', name: 'Threat Intelligence Enrichment', description: 'IOC enrichment and correlation with internal threat data', category: 'Threat Intelligence', steps: 7, avgDuration: '8 min', successRate: 93.7, lastRun: '2026-04-23T07:00:00Z', manualInterventions: 0, optimizationScore: 95},
    {id: 'wf-mitre-navigator-007', name: 'Security Reporting Pipeline', description: 'Automated generation and distribution of security metrics reports', category: 'Reporting', steps: 5, avgDuration: '15 min', successRate: 98.9, lastRun: '2026-04-23T08:00:00Z', manualInterventions: 0, optimizationScore: 97},
    {id: 'wf-mitre-navigator-008', name: 'Vendor Risk Assessment', description: 'Automated vendor security questionnaire distribution and scoring', category: 'Third Party', steps: 9, avgDuration: '3 days', successRate: 85.4, lastRun: '2026-04-19T11:00:00Z', manualInterventions: 4, optimizationScore: 65},
  ];
  ...
  @state() private _workflowExecutionHistory: Array<{workflowId: string; runDate: string; duration: string; status: string; trigger: string}> = [
    {workflowId: 'wf-mitre-navigator-001', runDate: '2026-04-23T06:00:00Z', duration: '11 min 23 sec', status: 'Success', trigger: 'Scheduled'},
    {workflowId: 'wf-mitre-navigator-002', runDate: '2026-04-22T14:30:00Z', duration: '42 min 15 sec', status: 'Success', trigger: 'Alert'},
    {workflowId: 'wf-mitre-navigator-003', runDate: '2026-04-23T09:15:00Z', duration: '3h 45 min', status: 'Success', trigger: 'User Request'},
    {workflowId: 'wf-mitre-navigator-001', runDate: '2026-04-22T06:00:00Z', duration: '13 min 08 sec', status: 'Partial', trigger: 'Scheduled'},
    {workflowId: 'wf-mitre-navigator-004', runDate: '2026-04-21T10:00:00Z', duration: '28 min 42 sec', status: 'Success', trigger: 'Scheduled'},
  ];
  ...
  @state() private _knowledgeArticles: Array<{id: string; title: string; category: string; author: string; publishDate: string; views: number; rating: number; tags: string[]; summary: string; readTime: string}> = [
    {id: 'kb-mitre-navigator-001', title: 'Zero Trust Architecture Implementation Guide', category: 'Architecture', author: 'Security Architecture Team', publishDate: '2026-03-15', views: 1842, rating: 4.8, tags: ['zero-trust', 'network', 'identity'], summary: 'Comprehensive guide for implementing zero trust principles across enterprise infrastructure', readTime: '15 min'},
    {id: 'kb-mitre-navigator-002', title: 'Ransomware Defense Playbook 2026', category: 'Threats', author: 'Threat Intelligence Team', publishDate: '2026-04-01', views: 2341, rating: 4.9, tags: ['ransomware', 'incident-response', 'defense'], summary: 'Updated playbook with latest ransomware tactics, techniques, and defensive measures', readTime: '12 min'},
    {id: 'kb-mitre-navigator-003', title: 'Cloud Security Best Practices for Multi-Cloud', category: 'Cloud', author: 'Cloud Security Team', publishDate: '2026-02-28', views: 1567, rating: 4.5, tags: ['cloud', 'aws', 'azure', 'gcp'], summary: 'Best practices for securing workloads across multiple cloud service providers', readTime: '18 min'},
    {id: 'kb-mitre-navigator-004', title: 'API Security Testing Methodology', category: 'Testing', author: 'Red Team', publishDate: '2026-03-22', views: 987, rating: 4.6, tags: ['api', 'testing', 'owasp'], summary: 'Structured methodology for identifying and testing API security vulnerabilities', readTime: '10 min'},
    {id: 'kb-mitre-navigator-005', title: 'Supply Chain Security Risk Management', category: 'Third Party', author: 'GRC Team', publishDate: '2026-01-20', views: 1234, rating: 4.3, tags: ['supply-chain', 'vendor', 'risk'], summary: 'Framework for managing security risks in the software supply chain', readTime: '14 min'},
    {id: 'kb-mitre-navigator-006', title: 'Kubernetes Security Hardening Checklist', category: 'Container', author: 'Platform Security', publishDate: '2026-04-10', views: 2103, rating: 4.7, tags: ['kubernetes', 'container', 'hardening'], summary: 'Step-by-step hardening checklist for production Kubernetes clusters', readTime: '11 min'},
    {id: 'kb-mitre-navigator-007', title: 'Security Metrics That Matter for Executives', category: 'Metrics', author: 'CISO Office', publishDate: '2026-03-05', views: 876, rating: 4.4, tags: ['metrics', 'kpi', 'reporting'], summary: 'Key security metrics and KPIs that resonate with board-level stakeholders', readTime: '8 min'},
    {id: 'kb-mitre-navigator-008', title: 'Phishing Simulation Campaign Design', category: 'Awareness', author: 'Security Awareness Team', publishDate: '2026-02-15', views: 1456, rating: 4.5, tags: ['phishing', 'simulation', 'training'], summary: 'Designing effective phishing simulations that drive real behavioral change', readTime: '9 min'},
    {id: 'kb-mitre-navigator-009', title: 'Data Loss Prevention Strategy and Implementation', category: 'Data Protection', author: 'Data Security Team', publishDate: '2026-03-28', views: 1678, rating: 4.6, tags: ['dlp', 'data-protection', 'compliance'], summary: 'Strategic approach to DLP covering people, process, and technology layers', readTime: '16 min'},
    {id: 'kb-mitre-navigator-010', title: 'Security Automation with SOAR Platforms', category: 'Automation', author: 'SOC Engineering', publishDate: '2026-04-05', views: 1098, rating: 4.7, tags: ['soar', 'automation', 'soc'], summary: 'Building effective security automation playbooks using SOAR technology', readTime: '13 min'},
    {id: 'kb-mitre-navigator-011', title: 'Third-Party Risk Assessment Framework', category: 'GRC', author: 'Vendor Management', publishDate: '2026-01-30', views: 1345, rating: 4.4, tags: ['vendor', 'risk', 'assessment'], summary: 'Structured framework for evaluating and monitoring third-party security risks', readTime: '12 min'},
    {id: 'kb-mitre-navigator-012', title: 'Network Detection and Response Best Practices', category: 'Network', author: 'Network Security Team', publishDate: '2026-03-18', views: 1567, rating: 4.5, tags: ['ndr', 'network', 'detection'], summary: 'Implementing effective network detection and response capabilities', readTime: '14 min'},
    {id: 'kb-mitre-navigator-013', title: 'Security Chaos Engineering Guide', category: 'Resilience', author: 'SRE Security', publishDate: '2026-04-12', views: 789, rating: 4.3, tags: ['chaos-engineering', 'resilience', 'testing'], summary: 'Applying chaos engineering principles to validate security controls', readTime: '10 min'},
    {id: 'kb-mitre-navigator-014', title: 'Incident Communication Templates', category: 'Incident Response', author: 'IR Team', publishDate: '2026-02-22', views: 2234, rating: 4.8, tags: ['incident', 'communication', 'templates'], summary: 'Ready-to-use communication templates for security incident scenarios', readTime: '7 min'},
    {id: 'kb-mitre-navigator-015', title: 'Endpoint Detection and Response Configuration', category: 'Endpoint', author: 'Endpoint Security', publishDate: '2026-03-08', views: 1345, rating: 4.5, tags: ['edr', 'endpoint', 'configuration'], summary: 'Optimal EDR configuration for maximum detection with minimal false positives', readTime: '11 min'},
    {id: 'kb-mitre-navigator-016', title: 'Security Code Review Standards', category: 'Development', author: 'AppSec Team', publishDate: '2026-02-10', views: 1890, rating: 4.6, tags: ['code-review', 'secure-sdlc', 'standards'], summary: 'Standards and checklists for security-focused code reviews', readTime: '13 min'},
    {id: 'kb-mitre-navigator-017', title: 'IoT Security Assessment Methodology', category: 'IoT', author: 'IoT Security Lab', publishDate: '2026-04-08', views: 654, rating: 4.2, tags: ['iot', 'embedded', 'assessment'], summary: 'Methodology for assessing security posture of IoT devices and ecosystems', readTime: '15 min'},
    {id: 'kb-mitre-navigator-018', title: 'Passwordless Authentication Migration Guide', category: 'Identity', author: 'IAM Team', publishDate: '2026-03-25', views: 1678, rating: 4.7, tags: ['passwordless', 'authentication', 'mfa'], summary: 'Step-by-step migration plan from password-based to passwordless authentication', readTime: '12 min'},
    {id: 'kb-mitre-navigator-019', title: 'Security Awareness Program Metrics', category: 'Awareness', author: 'Security Culture Team', publishDate: '2026-01-25', views: 1123, rating: 4.4, tags: ['awareness', 'metrics', 'culture'], summary: 'Measuring the effectiveness of your security awareness and training programs', readTime: '9 min'},
    {id: 'kb-mitre-navigator-020', title: 'Deception Technology Deployment Guide', category: 'Detection', author: 'Blue Team', publishDate: '2026-04-15', views: 876, rating: 4.5, tags: ['deception', 'honeypot', 'detection'], summary: 'Planning and deploying deception technology for advanced threat detection', readTime: '11 min'},
  ];
  ...
  // --- Security Scenario Planning Methods (MINA) ---
  private _initMinaScenarios(): void {
    const worstCaseScenarios = [
      { id: "mina-sc-01", name: "Ransomware Attack on Critical Infrastructure", category: "Malware", impact: 9500000, probability: 0.12, recoveryDays: 21, affectedSystems: 47, dataLossGb: 320, businessImpact: "critical" },
      { id: "mina-sc-02", name: "Insider Data Exfiltration via Authorized Channels", category: "Insider Threat", impact: 7200000, probability: 0.18, recoveryDays: 14, affectedSystems: 12, dataLossGb: 85, businessImpact: "high" },
      { id: "mina-sc-03", name: "Supply Chain Compromise via Third-Party Library", category: "Supply Chain", impact: 6800000, probability: 0.09, recoveryDays: 30, affectedSystems: 63, dataLossGb: 0, businessImpact: "critical" },
      { id: "mina-sc-04", name: "Cloud Misconfiguration Leading to Public Exposure", category: "Cloud", impact: 4100000, probability: 0.22, recoveryDays: 7, affectedSystems: 8, dataLossGb: 450, businessImpact: "high" },
      { id: "mina-sc-05", name: "Zero-Day Exploit in Core Application Framework", category: "Vulnerability", impact: 8900000, probability: 0.05, recoveryDays: 45, affectedSystems: 120, dataLossGb: 0, businessImpact: "critical" },
      { id: "mina-sc-06", name: "Distributed Denial of Service on Customer-Facing Services", category: "Availability", impact: 3400000, probability: 0.25, recoveryDays: 3, affectedSystems: 15, dataLossGb: 0, businessImpact: "medium" },
      { id: "mina-sc-07", name: "Credential Stuffing and Account Takeover Wave", category: "Identity", impact: 2800000, probability: 0.30, recoveryDays: 5, affectedSystems: 6, dataLossGb: 12, businessImpact: "medium" },
      { id: "mina-sc-08", name: "Physical Security Breach at Data Center Facility", category: "Physical", impact: 12000000, probability: 0.02, recoveryDays: 60, affectedSystems: 200, dataLossGb: 5000, businessImpact: "critical" },
    ];
    this._minaScenarios = worstCaseScenarios.map(s => ({ ...s, mitigationCost: Math.round(s.impact * 0.08), drillScheduled: false, lastDrillDate: null, drillScore: null, controlGaps: Math.floor(Math.random() * 5) + 1 }));
  }

  private _calculateMinaScenarioRisk(): void {
    this._minaScenarios.forEach(s => {
  ...
  private _scheduleMinaDrills(): void {
  ...
  private _minaScenarioComparison(): Record<string, unknown>[] {
    return this._minaScenarios.map(s => ({
  ...
  // --- Security Control Effectiveness Analytics (MINA) ---
  private _initMinaControls(): void {
    const controls = [
      { id: "mina-ct-01", name: "Network Segmentation", type: "preventive", maturity: "defined", score: 82, target: 90, failures: 3, lastTest: "2026-03-15" },
      { id: "mina-ct-02", name: "Endpoint Detection and Response", type: "detective", maturity: "managed", score: 88, target: 92, failures: 1, lastTest: "2026-04-01" },
      { id: "mina-ct-03", name: "Data Loss Prevention", type: "preventive", maturity: "repeatable", score: 71, target: 85, failures: 7, lastTest: "2026-03-22" },
      { id: "mina-ct-04", name: "Security Information and Event Management", type: "detective", maturity: "managed", score: 85, target: 90, failures: 2, lastTest: "2026-04-05" },
      { id: "mina-ct-05", name: "Identity and Access Management", type: "preventive", maturity: "defined", score: 76, target: 88, failures: 5, lastTest: "2026-03-28" },
      { id: "mina-ct-06", name: "Vulnerability Management", type: "corrective", maturity: "managed", score: 80, target: 90, failures: 4, lastTest: "2026-04-02" },
      { id: "mina-ct-07", name: "Email Security Gateway", type: "preventive", maturity: "managed", score: 90, target: 95, failures: 1, lastTest: "2026-04-08" },
      { id: "mina-ct-08", name: "Web Application Firewall", type: "preventive", maturity: "managed", score: 87, target: 92, failures: 2, lastTest: "2026-03-30" },
      { id: "mina-ct-09", name: "Patch Management", type: "corrective", maturity: "defined", score: 68, target: 85, failures: 9, lastTest: "2026-03-18" },
      { id: "mina-ct-10", name: "Encryption at Rest", type: "preventive", maturity: "managed", score: 93, target: 95, failures: 0, lastTest: "2026-04-10" },
      { id: "mina-ct-11", name: "Encryption in Transit", type: "preventive", maturity: "optimized", score: 96, target: 98, failures: 0, lastTest: "2026-04-12" },
      { id: "mina-ct-12", name: "Privileged Access Management", type: "preventive", maturity: "defined", score: 74, target: 88, failures: 6, lastTest: "2026-03-25" },
      { id: "mina-ct-13", name: "Security Awareness Training", type: "preventive", maturity: "repeatable", score: 65, target: 80, failures: 11, lastTest: "2026-03-20" },
      { id: "mina-ct-14", name: "Incident Response Plan", type: "corrective", maturity: "managed", score: 78, target: 88, failures: 4, lastTest: "2026-04-03" },
      { id: "mina-ct-15", name: "Backup and Recovery", type: "corrective", maturity: "managed", score: 84, target: 92, failures: 3, lastTest: "2026-04-06" },
      { id: "mina-ct-16", name: "Multi-Factor Authentication", type: "preventive", maturity: "managed", score: 91, target: 95, failures: 1, lastTest: "2026-04-09" },
      { id: "mina-ct-17", name: "Network Traffic Analysis", type: "detective", maturity: "defined", score: 72, target: 85, failures: 5, lastTest: "2026-03-27" },
      { id: "mina-ct-18", name: "Cloud Security Posture Management", type: "detective", maturity: "repeatable", score: 69, target: 82, failures: 8, lastTest: "2026-03-24" },
      { id: "mina-ct-19", name: "Container Security Scanning", type: "preventive", maturity: "defined", score: 75, target: 85, failures: 5, lastTest: "2026-04-04" },
      { id: "mina-ct-20", name: "API Security Gateway", type: "preventive", maturity: "repeatable", score: 70, target: 82, failures: 7, lastTest: "2026-03-29" },
      { id: "mina-ct-21", name: "Threat Intelligence Platform", type: "detective", maturity: "managed", score: 83, target: 90, failures: 2, lastTest: "2026-04-07" },
      { id: "mina-ct-22", name: "Database Activity Monitoring", type: "detective", maturity: "defined", score: 67, target: 80, failures: 8, lastTest: "2026-03-21" },
      { id: "mina-ct-23", name: "Deception Technology", type: "detective", maturity: "initial", score: 55, target: 75, failures: 12, lastTest: "2026-03-16" },
      { id: "mina-ct-24", name: "Security Orchestration Automation", type: "corrective", maturity: "repeatable", score: 73, target: 85, failures: 6, lastTest: "2026-04-01" },
      { id: "mina-ct-25", name: "Third-Party Risk Assessment", type: "preventive", maturity: "defined", score: 64, target: 78, failures: 9, lastTest: "2026-03-19" },
    ];
    this._minaControls = controls;
  }

  private _analyzeMinaControlEffectiveness(): void {
    const typeBreakdown: Record<string, { total: number; avgScore: number; avgFailures: number }> = {};
    this._minaControls.forEach(c => {
    ...
    this._minaControlTypeBreakdown = typeBreakdown;
  }

  private _minaControlOptimization(): Record<string, unknown>[] {
    return this._minaControls.filter(c => c.score < c.target).map(c => ({
  ...
  // --- Security Data Pipeline Health (MINA) ---
  private _initMinaPipelines(): void {
    const pipelines = [
      { id: "mina-pl-01", name: "SIEM Log Ingestion", status: "healthy", healthScore: 94, latencyMs: 120, freshnessMin: 2, errorRate: 0.02, throughputMbps: 450, recordsPerSec: 12000, backPressure: 0.05, uptime: 99.97 },
      { id: "mina-pl-02", name: "Threat Feed Aggregation", status: "healthy", healthScore: 91, latencyMs: 300, freshnessMin: 15, errorRate: 0.05, throughputMbps: 85, recordsPerSec: 3200, backPressure: 0.08, uptime: 99.92 },
      { id: "mina-pl-03", name: "Vulnerability Scan Results", status: "degraded", healthScore: 72, latencyMs: 2500, freshnessMin: 60, errorRate: 0.15, throughputMbps: 25, recordsPerSec: 800, backPressure: 0.35, uptime: 98.50 },
      { id: "mina-pl-04", name: "Endpoint Telemetry Stream", status: "healthy", healthScore: 88, latencyMs: 450, freshnessMin: 5, errorRate: 0.08, throughputMbps: 280, recordsPerSec: 8500, backPressure: 0.12, uptime: 99.85 },
      { id: "mina-pl-05", name: "Cloud Audit Log Pipeline", status: "healthy", healthScore: 86, latencyMs: 600, freshnessMin: 10, errorRate: 0.06, throughputMbps: 150, recordsPerSec: 4500, backPressure: 0.10, uptime: 99.80 },
      { id: "mina-pl-06", name: "Identity Event Stream", status: "warning", healthScore: 78, latencyMs: 1200, freshnessMin: 20, errorRate: 0.12, throughputMbps: 45, recordsPerSec: 1500, backPressure: 0.22, uptime: 99.40 },
      { id: "mina-pl-07", name: "Network Flow Collection", status: "healthy", healthScore: 82, latencyMs: 800, freshnessMin: 8, errorRate: 0.09, throughputMbps: 650, recordsPerSec: 18000, backPressure: 0.15, uptime: 99.70 },
      { id: "mina-pl-08", name: "DLP Incident Pipeline", status: "degraded", healthScore: 68, latencyMs: 3500, freshnessMin: 45, errorRate: 0.20, throughputMbps: 18, recordsPerSec: 500, backPressure: 0.42, uptime: 97.80 },
    ];
    this._minaPipelines = pipelines.map(p => ({ ...p, dependencies: [], slaBreached: p.healthScore < 75, alertThreshold: p.errorRate > 0.15 }));
    this._minaPipelines[0].dependencies = ["mina-pl-04", "mina-pl-07"];
    this._minaPipelines[4].dependencies = ["mina-pl-01"];
    this._minaPipelines[5].dependencies = ["mina-pl-02"];
    this._minaPipelines[7].dependencies = ["mina-pl-04", "mina-pl-05"];
  }

  private _minaPipelineTrend(): Record<string, unknown>[] {
    const hours = Array.from({ length: 24 }, (_, i) => `H${String(i).padStart(2, "0")}`);
    return this._minaPipelines.map(p => ({
  ...
  // --- Security Stakeholder Report Generator (MINA) ---
  private _initMinaReportTemplates(): void {
    this._minaReportTemplates = [
      { id: "mina-rp-01", name: "Board Security Report", audience: "board", frequency: "quarterly", sections: 6, autoGenerate: true, lastGenerated: "2026-03-31", pages: 12 },
      { id: "mina-rp-02", name: "Executive Risk Summary", audience: "executive", frequency: "monthly", sections: 8, autoGenerate: true, lastGenerated: "2026-04-01", pages: 8 },
      { id: "mina-rp-03", name: "Technical Security Review", audience: "technical", frequency: "weekly", sections: 12, autoGenerate: true, lastGenerated: "2026-04-21", pages: 25 },
      { id: "mina-rp-04", name: "Audit Compliance Report", audience: "audit", frequency: "quarterly", sections: 10, autoGenerate: false, lastGenerated: "2026-03-15", pages: 35 },
      { id: "mina-rp-05", name: "Regulatory Filing Package", audience: "regulatory", frequency: "annual", sections: 15, autoGenerate: false, lastGenerated: "2025-12-31", pages: 48 },
    ];
  }

  private _generateMinaExecSummary(): string {
    const totalRisk = this._minaScenarios.reduce((sum: number, s: any) => sum + s.riskScore, 0);
    const avgControl = Math.round(this._minaControls.reduce((sum: number, c: any) => sum + c.score, 0) / this._minaControls.length);
    const degradedPipes = this._minaPipelines.filter((p: any) => p.status !== "healthy").length;
    return `Overall risk exposure: ${(totalRisk / 1000000).toFixed(1)}M. Average control effectiveness: {avgControl}%. {degradedPipes} pipeline(s) need attention. {this._minaScenarios.filter((s: any) => s.mitigationPriority === "immediate").length} scenarios require immediate action.`;
  }

  // --- Security Technology Radar (MINA) ---
  private _initMinaTechRadar(): void {
    this._minaTechRadar = [
      { id: "mina-tr-01", name: "AI-Powered Threat Detection", ring: "adopt", category: "Detection", maturity: 4, trend: "up", investment: "high", roi: 3.2, vendorCount: 8 },
      { id: "mina-tr-02", name: "Zero Trust Architecture", ring: "adopt", category: "Architecture", maturity: 4, trend: "up", investment: "high", roi: 2.8, vendorCount: 12 },
      { id: "mina-tr-03", name: "SASE/SSE Platform", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.4, vendorCount: 10 },
      { id: "mina-tr-04", name: "CNAPP Cloud Security", ring: "trial", category: "Cloud", maturity: 3, trend: "up", investment: "medium", roi: 2.1, vendorCount: 7 },
      { id: "mina-tr-05", name: "Extended Detection and Response", ring: "adopt", category: "Detection", maturity: 4, trend: "stable", investment: "high", roi: 2.9, vendorCount: 9 },
      { id: "mina-tr-06", name: "Quantum-Safe Cryptography", ring: "assess", category: "Crypto", maturity: 2, trend: "up", investment: "low", roi: 0.8, vendorCount: 4 },
      { id: "mina-tr-07", name: "Security Service Edge", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.0, vendorCount: 6 },
      { id: "mina-tr-08", name: "SOAR 2.0 Autonomous SOC", ring: "assess", category: "Operations", maturity: 2, trend: "up", investment: "low", roi: 1.2, vendorCount: 5 },
      { id: "mina-tr-09", name: "Software Supply Chain Security", ring: "trial", category: "DevSecOps", maturity: 3, trend: "up", investment: "medium", roi: 2.3, vendorCount: 8 },
      { id: "mina-tr-10", name: "Identity Threat Detection", ring: "adopt", category: "Identity", maturity: 3, trend: "up", investment: "high", roi: 2.7, vendorCount: 7 },
      { id: "mina-tr-11", name: "Data Security Posture Management", ring: "trial", category: "Data", maturity: 3, trend: "up", investment: "medium", roi: 2.2, vendorCount: 9 },
      { id: "mina-tr-12", name: "Decentralized Identity Standards", ring: "hold", category: "Identity", maturity: 1, trend: "stable", investment: "low", roi: 0.5, vendorCount: 3 },
    ];
  }

  private _minaRadarSummary(): Record<string, unknown> {
    const rings: Record<string, number> = { adopt: 0, trial: 0, assess: 0, hold: 0 };
    this._minaTechRadar.forEach(t => { if (rings[t.ring as keyof typeof rings] !== undefined) rings[t.ring as keyof typeof rings]++; });
    return { adopt: rings.adopt, trial: rings.trial, assess: rings.assess, hold: rings.hold, total: this._minaTechRadar.length, avgMaturity: 2.8, topInvestment: this._minaTechRadar.filter(t => t.investment === "high").map(t => t.name) };
  }

  // --- MINA Security Compliance Mapping Matrix ---
  private _initMinaComplianceMatrix(): void {
    ...
    this._minaComplianceMatrix = frameworks.map(fw => ({
  ...
  private _minaComplianceTrend(): Record<string, unknown>[] {
    const quarters = ["Q1-2025", "Q2-2025", "Q3-2025", "Q4-2025", "Q1-2026", "Q2-2026"];
    return this._minaComplianceMatrix.map(fw => ({
  ...
  // --- MINA Threat Intelligence Correlation Engine ---
  private _initMinaThreatIntel(): void {
    ...
    this._minaThreatIntel = threatActors.map((actor, i) => ({
  ...
  private _minaThreatCorrelation(): Record<string, unknown>[] {
    return this._minaThreatIntel.filter(t => t.riskScore > 70).map(t => ({
  ...
  // --- MINA Security Metrics Deep Dive ---
  private _initMinaMetricsDeep(): void {
    ...
    this._minaMetricsDeep = metricCategories.map(cat => ({
  ...
  private _minaMetricsHeatmap(): number[][] {
    return this._minaMetricsDeep.map(cat =>
      cat.metrics.map(m => Math.round((m.value / m.target) * 100))
    );
  }

  // --- MINA Incident Cost Modeling ---
  private _initMinaCostModel(): void {
    const incidentTypes = ["Data Breach", "Ransomware", "DDoS", "Insider Threat", "Phishing", "Supply Chain", "Cloud Misconfig", "Zero-Day"];
    this._minaCostModel = incidentTypes.map(inc => ({
    ...
    this._minaCostModel.forEach(m => { m.totalAnnualExposure = m.avgCost * m.frequency; });
  }

  private _minaCostProjection(): Record<string, unknown>[] {
    return this._minaCostModel.map(m => ({
  ...
  // --- MINA Security Architecture Pattern Library ---
  private _initMinaArchPatterns(): void {
    ...
    this._minaArchPatterns = patterns.map((p, i) => ({
  ...
  private _minaArchPatternAnalysis(): Record<string, unknown> {
    const implemented = this._minaArchPatterns.filter(p => p.implementation > 75).length;
    const avgCoverage = Math.round(this._minaArchPatterns.reduce((s: number, p: any) => s + p.coverage, 0) / this._minaArchPatterns.length);
    return {
      totalPatterns: this._minaArchPatterns.length, fullyImplemented: implemented, avgCoverage,
      avgRiskReduction: Math.round(this._minaArchPatterns.reduce((s: number, p: any) => s + p.riskReduction, 0) / this._minaArchPatterns.length),
      gaps: this._minaArchPatterns.filter(p => p.implementation < 60).map(p => p.pattern),
      recommendations: this._minaArchPatterns.filter(p => p.implementation < 70).map(p => ({ pattern: p.pattern, current: p.implementation, target: 85, effort: p.costComplexity }))
    };
  }
  ...
  private _66bRiskAssessmentWorkflow = [
  ...
  private _66bTreatmentOptions = [
    {id: '66b-T-001', riskId: 'RSK-042', risk: 'Ransomware Attack on Critical Infrastructure', before: 20, after: 6, strategy: 'Mitigate', controls: ['Next-gen EDR with behavioral detection and automated response', 'Zero-trust network micro-segmentation isolating critical assets', 'Immutable offline backup rotation with quarterly recovery testing', 'Incident response retainer with 1-hour response SLA'], cost: '$450K/yr', roi: '78% reduction vs $2.8M potential loss', timeline: 'Q3 2026', status: 'approved' as const},
  ...
  private _66bTalentPositions = [
    ...
    {id: '66b-H-002', title: 'Cloud Security Engineer', dept: 'Cloud Security', level: 'Mid-Senior', loc: 'SF/Remote', salary: '$155K-$180K', posted: '2026-03-20', applicants: 45, pipeline: 12, interviews: 6, offers: 2, urgency: 'high' as const, manager: 'Mike Johnson', skills: ['AWS/Azure/GCP', 'Terraform', 'Kubernetes', 'IAM', 'CSPM'], nice: ['CCSP', 'CKS', 'Serverless']},
    {id: '66b-H-003', title: 'SOC Analyst L2', dept: 'Security Operations', level: 'Mid', loc: 'Austin TX', salary: '$105K-$130K', posted: '2026-04-01', applicants: 67, pipeline: 18, interviews: 8, offers: 0, urgency: 'medium' as const, manager: 'Lisa Park', skills: ['SIEM', 'Incident triage', 'Malware analysis', 'TCP/IP'], nice: ['GCIA', 'SOAR', 'Kill chain']},
    {id: '66b-H-004', title: 'Staff Security Architect', dept: 'Architecture', level: 'Staff', loc: 'Remote US', salary: '$200K-$240K', posted: '2026-02-28', applicants: 12, pipeline: 3, interviews: 2, offers: 0, urgency: 'critical' as const, manager: 'David Lee', skills: ['Enterprise arch', 'Zero trust', 'Risk frameworks', 'Board comms'], nice: ['CISSP+SABSA', 'Team building', 'Speaker']},
    {id: '66b-H-005', title: 'GRC Analyst', dept: 'GRC', level: 'Mid', loc: 'NY/Remote', salary: '$110K-$135K', posted: '2026-04-05', applicants: 34, pipeline: 9, interviews: 3, offers: 1, urgency: 'medium' as const, manager: 'Rachel Green', skills: ['ISO 27001', 'SOC 2', 'Risk assessment', 'Audit'], nice: ['CRISC', 'CISA', 'GDPR']},
    {id: '66b-H-006', title: 'Threat Intel Analyst', dept: 'Threat Intel', level: 'Mid-Senior', loc: 'Remote US', salary: '$140K-$170K', posted: '2026-03-25', applicants: 19, pipeline: 5, interviews: 2, offers: 0, urgency: 'low' as const, manager: 'James Wilson', skills: ['MITRE ATT&CK', 'OSINT', 'Threat modeling', 'Reverse eng'], nice: ['CTIA', 'Nation-state', 'Dark web']},
    {id: '66b-H-007', title: 'AppSec Engineer', dept: 'AppSec', level: 'Senior', loc: 'Seattle/Remote', salary: '$160K-$190K', posted: '2026-04-10', applicants: 28, pipeline: 7, interviews: 3, offers: 0, urgency: 'medium' as const, manager: 'Emily Zhang', skills: ['SAST/DAST/SCA', 'Secure SDLC', 'Code review', 'CI/CD'], nice: ['CSSLP', 'Mobile security']},
    {id: '66b-H-008', title: 'IAM Engineer', dept: 'IAM', level: 'Mid', loc: 'Remote US', salary: '$135K-$160K', posted: '2026-04-08', applicants: 31, pipeline: 10, interviews: 5, offers: 1, urgency: 'medium' as const, manager: 'Chris Martinez', skills: ['Okta/Azure AD', 'SAML/OIDC', 'PAM', 'RBAC'], nice: ['CISSP', 'Zero trust IAM']}
  ];

  private _66bInterviewScorecard = [
  ...
  private _66bOnboardingSteps = [
    {id: '66b-OB-01', item: 'Provision security-hardened laptop, monitors, peripherals, and configured workstation image with all required security tools', cat: 'IT Setup', day: 'Day 1', owner: 'IT Operations'},
    {id: '66b-OB-02', item: 'Create AD account, email, Slack, VPN, SSO enrollment, and all required application access accounts', cat: 'Access', day: 'Day 1', owner: 'IAM Team'},
    {id: '66b-OB-03', item: 'Grant security tooling access: SIEM console, EDR dashboard, vuln scanner, pentest platforms, SOAR', cat: 'Tools', day: 'Day 1-2', owner: 'SOC Manager'},
    {id: '66b-OB-04', item: 'Complete MFA enrollment, security awareness onboarding course, and acceptable use policy acknowledgment', cat: 'Security', day: 'Day 1', owner: 'Awareness Team'},
    {id: '66b-OB-05', item: 'Sign NDA, employment agreement, IP assignment, and code of conduct documents with HR', cat: 'HR', day: 'Day 1', owner: 'Human Resources'},
    {id: '66b-OB-06', item: 'Introduction meetings with direct team members and key cross-functional stakeholders', cat: 'Integration', day: 'Week 1', owner: 'Hiring Manager'},
    {id: '66b-OB-07', item: 'Security architecture overview, tooling walkthrough, and operational runbook review sessions', cat: 'Training', day: 'Week 1-2', owner: 'Security Architect'},
    {id: '66b-OB-08', item: 'Shadow experienced team member on active incident response and threat hunting investigations', cat: 'Mentoring', day: 'Week 2-3', owner: 'SOC Lead'},
    {id: '66b-OB-09', item: 'Set up individual certification study plan aligned with role requirements and career goals', cat: 'Development', day: 'Week 2', owner: 'Hiring Manager'},
    {id: '66b-OB-10', item: 'Establish 30-60-90 day goals, define success metrics, and schedule first performance checkpoint', cat: 'Goals', day: 'Week 1', owner: 'Hiring Manager'}
  ];

  private _66bMetricsDashboard = [
    {id: '66b-MTD-001', name: 'Mean Time to Detect (MTTD)', value: '4.2 hours', trend: 'improving' as const, target: '< 4 hours', source: 'SIEM - Splunk', method: 'Median time from first alert to analyst acknowledgment', owner: 'SOC Manager', sla: '4 hours', prev: '5.1 hours', spark: [5.8, 5.4, 5.1, 4.9, 4.7, 4.5, 4.3, 4.2], breakdown: [{src: 'Network IDS', pct: '35%', avg: '3.8h'}, {src: 'Endpoint EDR', pct: '28%', avg: '4.1h'}, {src: 'Email Gateway', pct: '22%', avg: '3.5h'}, {src: 'Cloud Security', pct: '10%', avg: '5.2h'}, {src: 'User Reports', pct: '5%', avg: '6.8h'}]},
    {id: '66b-MTD-002', name: 'Mean Time to Respond (MTTR)', value: '2.8 hours', trend: 'stable' as const, target: '< 3 hours', source: 'SOAR - XSOAR', method: 'Median time from acknowledgment to first containment action', owner: 'IR Lead', sla: '3 hours', prev: '2.7 hours', spark: [3.2, 3.0, 2.9, 2.8, 2.8, 2.9, 2.8, 2.8], breakdown: [{src: 'Auto Playbooks', pct: '42%', avg: '1.2h'}, {src: 'Semi-Auto', pct: '33%', avg: '2.5h'}, {src: 'Manual', pct: '20%', avg: '5.1h'}, {src: 'L3 Escalation', pct: '5%', avg: '8.3h'}]},
    {id: '66b-MTD-003', name: 'Patch Compliance Rate', value: '97.6%', trend: 'improving' as const, target: '> 95%', source: 'Tenable.io', method: 'Pct of critical/high vulns patched within SLA windows', owner: 'Vuln Manager', sla: '95%', prev: '96.1%', spark: [93.2, 94.1, 94.8, 95.3, 95.9, 96.4, 97.0, 97.6], breakdown: [{src: 'Critical (7d)', pct: '40%', rate: '99.2%'}, {src: 'High (30d)', pct: '35%', rate: '97.8%'}, {src: 'Medium (90d)', pct: '20%', rate: '95.1%'}, {src: 'Low (180d)', pct: '5%', rate: '89.4%'}]},
    {id: '66b-MTD-004', name: 'Phishing Click Rate', value: '3.2%', trend: 'improving' as const, target: '< 5%', source: 'KnowBe4 + Proofpoint', method: 'Pct of users clicking simulated phishing links monthly', owner: 'Awareness Lead', sla: '5%', prev: '4.1%', spark: [6.8, 6.2, 5.5, 5.1, 4.6, 4.1, 3.6, 3.2], breakdown: [{src: 'Spear Phishing', pct: '35%', rate: '5.1%'}, {src: 'Generic Phishing', pct: '30%', rate: '2.8%'}, {src: 'BEC Sim', pct: '20%', rate: '3.5%'}, {src: 'SMS Phishing', pct: '15%', rate: '1.9%'}]},
    {id: '66b-MTD-005', name: 'Security Posture Score', value: '784', trend: 'improving' as const, target: '> 750', source: 'SecurityScorecard', method: 'Composite: external 30%, compliance 25%, vuln 25%, config 20%', owner: 'CISO', sla: '> 750', prev: '771', spark: [748, 752, 758, 762, 768, 773, 778, 784], breakdown: [{src: 'External Rating', pct: '30%', score: '791'}, {src: 'Compliance', pct: '25%', score: '812'}, {src: 'Vuln Mgmt', pct: '25%', score: '745'}, {src: 'Config', pct: '20%', score: '769'}]},
    {id: '66b-MTD-006', name: 'Vulnerability Backlog', value: '142', trend: 'improving' as const, target: '< 150', source: 'Tenable + Qualys', method: 'Total open vulns across all scanned assets tracked weekly', owner: 'Vuln Manager', sla: '< 150', prev: '168', spark: [210, 198, 185, 175, 165, 158, 150, 142], breakdown: [{src: 'Critical', pct: '8%', count: '11'}, {src: 'High', pct: '22%', count: '31'}, {src: 'Medium', pct: '45%', count: '64'}, {src: 'Low', pct: '25%', count: '36'}]}
  ];

  private _66bArchReview = [
    {id: '66b-AR-001', cat: 'Network', item: 'Network segmentation between trust zones with dedicated firewalls and granular ACLs', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Micro-segmentation via NSX across all production zones'},
    {id: '66b-AR-002', cat: 'Network', item: 'DMZ architecture isolating public services from internal network', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Dual-firewall DMZ with WAF and DDoS mitigation'},
    {id: '66b-AR-003', cat: 'Network', item: 'Least-privilege traffic flows with deny-by-default and quarterly review', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Explicit allow rules with automated drift detection'},
    {id: '66b-AR-004', cat: 'Network', item: 'Encrypted external communications with IPSec/TLS 1.3 and PFS', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'IKEv2 with PFS, TLS 1.3 for all external APIs'},
    {id: '66b-AR-005', cat: 'Identity', item: 'Centralized IdP with SSO and automated SCIM provisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Okta SSO SAML 2.0 for 247 apps with SCIM'},
    {id: '66b-AR-006', cat: 'Identity', item: 'RBAC with quarterly reviews and automated deprovisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Quarterly recertification with JIT admin access'},
    {id: '66b-AR-007', cat: 'Identity', item: 'PAM controlling all admin and service accounts with session recording', sev: 'high' as const, status: 'fail' as const, reviewer: 'IAM Lead', notes: '3 legacy accounts pending CyberArk enrollment'},
    {id: '66b-AR-008', cat: 'Identity', item: 'MFA enforced for all users, hardware tokens for privileged access', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'FIDO2 for admins, push MFA for users, 99.8% enrollment'},
    {id: '66b-AR-009', cat: 'Data', item: 'Data classification enforced with automated labeling on all repos', sev: 'high' as const, status: 'partial' as const, reviewer: 'Data Protection Lead', notes: 'Framework deployed, gaps on 4 legacy file shares'},
    {id: '66b-AR-010', cat: 'Data', item: 'AES-256 encryption at rest for all sensitive data stores', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'Full-disk, TDE on DBs, SSE-KMS on cloud storage'},
    {id: '66b-AR-011', cat: 'Data', item: 'DLP monitoring and preventing unauthorized data exfiltration', sev: 'high' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'DLP active on email, endpoint, cloud, and network'},
    {id: '66b-AR-012', cat: 'Data', item: 'Encrypted off-site backups with quarterly restore testing', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Infrastructure Lead', notes: '3-2-1 strategy with immutable backups'},
    {id: '66b-AR-013', cat: 'AppSec', item: 'Secure SDLC with SAST, DAST, SCA in all CI/CD pipelines', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: '34 pipelines with security gates and PR blocking'},
    {id: '66b-AR-014', cat: 'AppSec', item: 'API gateway with auth, rate limiting, schema validation', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: 'Kong with OAuth2, rate limits, OpenAPI validation'},
    {id: '66b-AR-015', cat: 'AppSec', item: 'Container security with image scanning and runtime protection', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Trivy scanning, Falco runtime in 80% of clusters'},
    {id: '66b-AR-016', cat: 'Monitoring', item: 'SIEM collecting from all critical systems with correlation rules', sev: 'critical' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'Splunk 98% coverage with 1200+ correlation rules'},
    {id: '66b-AR-017', cat: 'Monitoring', item: 'EDR with behavioral detection on all managed endpoints', sev: 'high' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'CrowdStrike on 99.2% endpoints with 24/7 MDR'},
    {id: '66b-AR-018', cat: 'Monitoring', item: 'Executive dashboards with real-time posture visibility', sev: 'medium' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'PowerBI dashboards with weekly exec summaries'},
    {id: '66b-AR-019', cat: 'Cloud', item: 'CSPM monitoring and remediating cloud misconfigurations', sev: 'high' as const, status: 'pass' as const, reviewer: 'Cloud Security Lead', notes: 'Prisma Cloud 450+ checks with auto-remediation'},
    {id: '66b-AR-020', cat: 'Cloud', item: 'IaC templates scanned before deployment to production', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Checkov for Terraform, cfn-nag for CloudFormation pending'}
  ];

  private _get66bRiskProgress(): number {
    return Math.round(this._66bRiskAssessmentWorkflow.filter(s => s.status === 'completed').length / this._66bRiskAssessmentWorkflow.length * 100);
  }

  private _get66bTreatmentSummary(): {total: number; approved: number; avgReduction: number; investment: string} {
    const a = this._66bTreatmentOptions.filter(r => r.status === 'approved').length;
    const avg = this._66bTreatmentOptions.reduce((s, r) => s + ((r.before - r.after) / r.before * 100), 0) / this._66bTreatmentOptions.length;
    return {total: this._66bTreatmentOptions.length, approved: a, avgReduction: Math.round(avg), investment: '$1.125M annually'};
  }

  private _get66bRuleSummary(): {total: number; passing: number; failing: number; avgRate: number; violations: number} {
    const p = this._66bComplianceRules.filter(r => r.result === 'pass').length;
    const f = this._66bComplianceRules.filter(r => r.result === 'fail').length;
    const avg = this._66bComplianceRules.reduce((s, r) => s + r.passRate, 0) / this._66bComplianceRules.length;
    const v = this._66bComplianceRules.reduce((s, r) => s + r.violations, 0);
    return {total: this._66bComplianceRules.length, passing: p, failing: f, avgRate: Math.round(avg * 10) / 10, violations: v};
  }

  private _get66bPipelineStats(): {positions: number; applicants: number; interviewing: number; offers: number; critical: number} {
    const t = this._66bTalentPositions.length;
    const a = this._66bTalentPositions.reduce((s, p) => s + p.applicants, 0);
    const i = this._66bTalentPositions.reduce((s, p) => s + p.interviews, 0);
    const o = this._66bTalentPositions.reduce((s, p) => s + p.offers, 0);
    const c = this._66bTalentPositions.filter(x => x.urgency === 'critical').length;
    return {positions: t, applicants: a, interviewing: i, offers: o, critical: c};
  }

  private _get66bMetricTrends(): {improving: number; stable: number; total: number} {
    const imp = this._66bMetricsDashboard.filter(m => m.trend === 'improving').length;
    const stb = this._66bMetricsDashboard.filter(m => m.trend === 'stable').length;
    return {improving: imp, stable: stb, total: this._66bMetricsDashboard.length};
  }

  private _get66bArchScore(): {total: number; passed: number; failed: number; partial: number; score: number} {
    const p = this._66bArchReview.filter(c => c.status === 'pass').length;
    const f = this._66bArchReview.filter(c => c.status === 'fail').length;
    const pa = this._66bArchReview.filter(c => c.status === 'partial').length;
    return {total: this._66bArchReview.length, passed: p, failed: f, partial: pa, score: Math.round((p / this._66bArchReview.length) * 100)};
  }


  private _z66bIncidentScenarios = [
    {id: 'z66b-IS-001', name: 'Ransomware Outbreak in Production Environment', category: 'Malware', likelihood: 'Medium', impact: 'Critical', riskScore: 15, playbooks: ['IR-Ransomware-Isolate', 'IR-Ransomware-Eradicate', 'IR-Ransomway-Recover'], estimatedMTTD: '15min', estimatedMTTR: '4h', involvedTeams: ['SOC', 'IR', 'Infrastructure', 'Legal', 'Comms'], keyIndicators: ['Mass file encryption events', 'Ransom note deployment', 'Lateral movement via SMB/RDP', 'Backup deletion attempts'], mitigationControls: ['EDR behavioral detection', 'Network segmentation', 'Immutable backups', 'Email filtering'], lessonsLearned: 'Ensure offline backups are tested quarterly and Ransomware-specific playbooks are updated with latest threat intelligence', lastDrillDate: '2026-03-15', nextDrillDate: '2026-06-15'},
    {id: 'z66b-IS-002', name: 'Cloud Misconfiguration Leading to Data Exposure', category: 'Cloud', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Cloud-Misconfig-Assess', 'IR-Cloud-Misconfig-Remediate', 'IR-Cloud-Misconfig-Notify'], estimatedMTTD: '2h', estimatedMTTR: '6h', involvedTeams: ['Cloud Security', 'SOC', 'Data Protection', 'Legal'], keyIndicators: ['Public S3 bucket exposure', 'Overly permissive IAM policies', 'Unencrypted database snapshots', 'Public API endpoint discovery'], mitigationControls: ['CSPM monitoring', 'IAM policy analyzer', 'Automated remediation', 'Cloud audit logging'], lessonsLearned: 'Implement CSPM with auto-remediation for common misconfigurations and conduct monthly cloud configuration reviews', lastDrillDate: '2026-02-20', nextDrillDate: '2026-05-20'},
    {id: 'z66b-IS-003', name: 'Insider Threat Data Exfiltration', category: 'Insider', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-Insider-Investigate', 'IR-Insider-Contain', 'IR-Insider-Escalate'], estimatedMTTD: '8h', estimatedMTTR: '24h', involvedTeams: ['SOC', 'IR', 'HR', 'Legal', 'IAM'], keyIndicators: ['Anomalous data access patterns', 'Large file downloads to USB or cloud', 'Off-hours access to sensitive systems', 'Accessing data outside normal job scope'], mitigationControls: ['UEBA analytics', 'DLP policies', 'PAM controls', 'Access reviews'], lessonsLearned: 'Strengthen UEBA baselines and implement automated alerts for data access pattern anomalies', lastDrillDate: '2026-01-10', nextDrillDate: '2026-04-10'},
    {id: 'z66b-IS-004', name: 'Supply Chain Attack via Compromised Vendor Software', category: 'Supply Chain', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-SupplyChain-Detect', 'IR-SupplyChain-Isolate', 'IR-SupplyChain-Remediate'], estimatedMTTD: '24h', estimatedMTTR: '72h', involvedTeams: ['SOC', 'IR', 'Procurement', 'Vendor Mgmt', 'Legal'], keyIndicators: ['Unexpected network connections from vendor software', 'Modified binaries or dependencies', 'Anomalous behavior from trusted applications', 'SBOM discrepancy alerts'], mitigationControls: ['SCA scanning', 'SBOM monitoring', 'Vendor security assessments', 'Network segmentation'], lessonsLearned: 'Require SBOM from all vendors and implement continuous monitoring of third-party component behavior', lastDrillDate: '2026-03-01', nextDrillDate: '2026-06-01'},
    {id: 'z66b-IS-005', name: 'Credential Stuffing Attack on Customer Portal', category: 'External Attack', likelihood: 'High', impact: 'Medium', riskScore: 12, playbooks: ['IR-CredentialStuffing-Detect', 'IR-CredentialStuffing-Block', 'IR-CredentialStuffing-Notify'], estimatedMTTD: '30min', estimatedMTTR: '2h', involvedTeams: ['SOC', 'AppSec', 'Customer Support', 'Comms'], keyIndicators: ['High volume of failed login attempts', 'Login attempts from known compromised credential lists', 'Unusual geographic distribution of login attempts', 'Account takeover indicators'], mitigationControls: ['WAF rate limiting', 'CAPTCHA on login', 'MFA enforcement', 'Credential monitoring service'], lessonsLearned: 'Deploy advanced bot detection on login endpoints and implement proactive credential monitoring for customer accounts', lastDrillDate: '2026-04-01', nextDrillDate: '2026-07-01'},
    {id: 'z66b-IS-006', name: 'Phishing Campaign Targeting Executive Team', category: 'Social Engineering', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Phishing-Contain', 'IR-Phishing-Investigate', 'IR-Phishing-Notify'], estimatedMTTD: '1h', estimatedMTTR: '4h', involvedTeams: ['SOC', 'Awareness', 'Executive Office', 'IT'], keyIndicators: ['Sophisticated spear-phishing emails to C-suite', 'Business email compromise indicators', 'Redirected MFA approval requests', 'Unauthorized wire transfer attempts'], mitigationControls: ['AI email filtering', 'Executive awareness training', 'Out-of-band verification policy', 'DMARC/DKIM/SPF'], lessonsLearned: 'Implement additional email security controls for executive mailboxes and conduct monthly spear-phishing simulations', lastDrillDate: '2026-02-15', nextDrillDate: '2026-05-15'}
  ];

  private _z66bSecurityFrameworks = [
  ...
  private _getz66bScenarioSummary(): {total: number; critical: number; high: number; drilled: number; avgRiskScore: number} {
    const crit = this._z66bIncidentScenarios.filter(s => s.impact === 'Critical').length;
    const high = this._z66bIncidentScenarios.filter(s => s.impact === 'High').length;
    const avg = this._z66bIncidentScenarios.reduce((s, x) => s + x.riskScore, 0) / this._z66bIncidentScenarios.length;
    return {total: this._z66bIncidentScenarios.length, critical: crit, high: high, drilled: this._z66bIncidentScenarios.length, avgRiskScore: Math.round(avg)};
  }

  private _getz66bVendorRiskSummary(): {total: number; approved: number; conditional: number; remediation: number; avgScore: number} {
    const approved = this._z66bThirdPartyRiskData.filter(v => v.status === 'approved').length;
    const cond = this._z66bThirdPartyRiskData.filter(v => v.status === 'conditional').length;
    const rem = this._z66bThirdPartyRiskData.filter(v => v.status === 'remediation_required').length;
    const avg = this._z66bThirdPartyRiskData.reduce((s, v) => s + v.riskScore, 0) / this._z66bThirdPartyRiskData.length;
    return {total: this._z66bThirdPartyRiskData.length, approved, conditional: cond, remediation: rem, avgScore: Math.round(avg)};
  }

  private _getz66bTrainingSummary(): {total: number; avgCompletion: number; avgScore: number; overdueUpdates: number} {
    const avgComp = this._z66bSecurityTrainingCatalog.reduce((s, t) => s + t.completionRate, 0) / this._z66bSecurityTrainingCatalog.length;
    const avgScore = this._z66bSecurityTrainingCatalog.filter(t => t.avgScore > 0).reduce((s, t) => s + t.avgScore, 0) / this._z66bSecurityTrainingCatalog.filter(t => t.avgScore > 0).length;
    return {total: this._z66bSecurityTrainingCatalog.length, avgCompletion: Math.round(avgComp * 10) / 10, avgScore: Math.round(avgScore), overdueUpdates: 0};
  }

  private _getz66bToolHealthSummary(): {total: number; healthy: number; degraded: number; annualCost: string} {
    const healthy = this._z66bSecurityToolInventory.filter(t => t.health === 'healthy').length;
    const degraded = this._z66bSecurityToolInventory.filter(t => t.health === 'degraded').length;
    const cost = this._z66bSecurityToolInventory.reduce((s, t) => s + parseInt(t.annualCost.replace(/[^0-9]/g, '')), 0);
    return {total: this._z66bSecurityToolInventory.length, healthy, degraded, annualCost: '$' + (cost / 1000).toFixed(0) + 'K'};
  }


  private _x66bSecurityZones = [
    {zoneId: 'x66b-Z-001', name: 'Internet DMZ', trustLevel: 'Untrusted', subnet: '10.0.0.0/24', segmentation: 'Dual-firewall', firewallRules: 45, monitoring: 'Full packet capture', assets: ['Web servers', 'Load balancers', 'WAF appliances', 'Reverse proxy'], riskLevel: 'high' as const, lastAssessment: '2026-04-15', nextAssessment: '2026-07-15'},
    {zoneId: 'x66b-Z-002', name: 'Corporate Network', trustLevel: 'Trusted', subnet: '10.1.0.0/16', segmentation: 'VLAN + micro-seg', firewallRules: 128, monitoring: 'NetFlow + IDS', assets: ['Workstations', 'Printers', 'Meeting room systems', 'VoIP phones'], riskLevel: 'medium' as const, lastAssessment: '2026-04-10', nextAssessment: '2026-07-10'},
    {zoneId: 'x66b-Z-003', name: 'Production Data Center', trustLevel: 'Restricted', subnet: '10.2.0.0/16', segmentation: 'Full micro-segmentation', firewallRules: 256, monitoring: 'Full packet capture + EDR', assets: ['Application servers', 'Database clusters', 'Message queues', 'API gateways'], riskLevel: 'low' as const, lastAssessment: '2026-04-12', nextAssessment: '2026-07-12'},
    {zoneId: 'x66b-Z-004', name: 'Management Network', trustLevel: 'Restricted', subnet: '10.3.0.0/24', segmentation: 'Isolated VLAN', firewallRules: 32, monitoring: 'Syslog + SNMP', assets: ['Hypervisors', 'Switch management', 'Storage controllers', 'Backup appliances'], riskLevel: 'low' as const, lastAssessment: '2026-04-08', nextAssessment: '2026-07-08'},
    {zoneId: 'x66b-Z-005', name: 'Cloud Production', trustLevel: 'Restricted', subnet: 'VPC: 172.16.0.0/12', segmentation: 'Security groups + NACLs', firewallRules: 189, monitoring: 'VPC Flow Logs + GuardDuty', assets: ['Kubernetes clusters', 'Managed databases', 'Object storage', 'Serverless functions'], riskLevel: 'medium' as const, lastAssessment: '2026-04-18', nextAssessment: '2026-07-18'}
  ];

  private _x66bSecurityAlerts = [
    {id: 'x66b-AL-001', severity: 'critical', title: 'Active ransomware encryption detected on PROD-DB-01', source: 'CrowdStrike', timestamp: '2026-04-22T13:45:00Z', status: 'investigating', assignee: 'SOC Analyst J. Smith', asset: 'PROD-DB-01 (10.2.1.15)', mitreTechnique: 'T1486 - Data Encrypted for Impact', playbook: 'IR-Ransomware-Isolate', sla: '15 minutes', elapsed: '12 minutes', notes: 'EDR detected mass file encryption event pattern. Automated isolation triggered. IR team paged.'},
    {id: 'x66b-AL-002', severity: 'high', title: 'Impossible travel detected for user admin@company.com', source: 'Okta', timestamp: '2026-04-22T12:30:00Z', status: 'triaged', assignee: 'SOC Analyst M. Johnson', asset: 'Okta - admin@company.com', mitreTechnique: 'T1078 - Valid Accounts', playbook: 'IR-ImpossibleTravel', sla: '30 minutes', elapsed: '87 minutes', notes: 'Login from Tokyo followed by login from New York within 30 minutes. Account temporarily locked. User confirmed via out-of-band channel they are in New York.'},
    {id: 'x66b-AL-003', severity: 'high', title: 'Public S3 bucket discovered with customer PII exposure', source: 'Prisma Cloud', timestamp: '2026-04-22T11:15:00Z', status: 'remediating', assignee: 'Cloud Security Lead A. Chen', asset: 'AWS S3: customer-data-archive-2024', mitreTechnique: 'N/A - Misconfiguration', playbook: 'IR-Cloud-Misconfig-Remediate', sla: '1 hour', elapsed: '2.5 hours', notes: 'Bucket containing archived customer records was publicly accessible. Bucket blocked and encrypted. Data exposure assessment in progress.'},
    {id: 'x66b-AL-004', severity: 'medium', title: 'Anomalous data download by user jane.developer@company.com', source: 'Microsoft DLP', timestamp: '2026-04-22T10:00:00Z', status: 'investigating', assignee: 'SOC Analyst R. Davis', asset: 'Endpoint: WS-DEV-042', mitreTechnique: 'T1567 - Exfil Over Web Service', playbook: 'IR-DLP-Investigation', sla: '4 hours', elapsed: '3.5 hours', notes: 'User downloaded 4.2GB of source code files to personal Google Drive. Interview scheduled with user and manager.'},
    {id: 'x66b-AL-005', severity: 'medium', title: 'Credential stuffing attack detected on customer portal', source: 'Cloudflare WAF', timestamp: '2026-04-22T09:30:00Z', status: 'contained', assignee: 'SOC Analyst K. Wilson', asset: 'Customer Portal (portal.company.com)', mitreTechnique: 'T1110 - Brute Force', playbook: 'IR-CredentialStuffing-Block', sla: '2 hours', elapsed: '4 hours', notes: 'Rate limiting and CAPTCHA automatically deployed. 12,000 failed login attempts from 800 unique IPs blocked. No successful account takeovers confirmed.'},
    {id: 'x66b-AL-006', severity: 'low', title: 'SSL certificate expiring in 14 days for api.company.com', source: 'Certificate monitoring', timestamp: '2026-04-22T08:00:00Z', status: 'remediated', assignee: 'Infrastructure Lead T. Brown', asset: 'api.company.com', mitreTechnique: 'N/A', playbook: 'Certificate Renewal', sla: '30 days', elapsed: '16 days', notes: 'Automated renewal triggered via ACME. New certificate deployed successfully. No service interruption.'}
  ];

  private _x66bComplianceFrameworks = [
  ...
  private _x66bDataClassificationPolicy = [
  ...
  private _getx66bZoneSummary(): {total: number; highRisk: number; mediumRisk: number; lowRisk: number; totalRules: number} {
    const high = this._x66bSecurityZones.filter(z => z.riskLevel === 'high').length;
    const med = this._x66bSecurityZones.filter(z => z.riskLevel === 'medium').length;
    const low = this._x66bSecurityZones.filter(z => z.riskLevel === 'low').length;
    const rules = this._x66bSecurityZones.reduce((s, z) => s + z.firewallRules, 0);
    return {total: this._x66bSecurityZones.length, highRisk: high, mediumRisk: med, lowRisk: low, totalRules: rules};
  }

  private _getx66bAlertSummary(): {total: number; critical: number; high: number; medium: number; low: number; open: number; contained: number} {
    const crit = this._x66bSecurityAlerts.filter(a => a.severity === 'critical').length;
    const high = this._x66bSecurityAlerts.filter(a => a.severity === 'high').length;
    const med = this._x66bSecurityAlerts.filter(a => a.severity === 'medium').length;
    const low = this._x66bSecurityAlerts.filter(a => a.severity === 'low').length;
    const open = this._x66bSecurityAlerts.filter(a => a.status === 'investigating' || a.status === 'triaged').length;
    const contained = this._x66bSecurityAlerts.filter(a => a.status === 'contained' || a.status === 'remediated').length;
    return {total: this._x66bSecurityAlerts.length, critical: crit, high, medium: med, low, open, contained};
  }

  private _getx66bComplianceSummary(): {total: number; fullyCompliant: number; mostlyCompliant: number; avgComplianceRate: number} {
    const full = this._x66bComplianceFrameworks.filter(f => f.status === 'compliant').length;
    const mostly = this._x66bComplianceFrameworks.filter(f => f.status === 'mostly_compliant').length;
    const avgRate = this._x66bComplianceFrameworks.reduce((s, f) => s + (f.compliant / f.requirements * 100), 0) / this._x66bComplianceFrameworks.length;
    return {total: this._x66bComplianceFrameworks.length, fullyCompliant: full, mostlyCompliant: mostly, avgComplianceRate: Math.round(avgRate * 10) / 10};
  }
  ...
  // === Security Risk Register (mitre_naviga) ===
  private _mitre_navigaRiskRegister = [
    { id: "RSK-MIT-0001", title: "Data breach from unpatched systems", owner: "CTO", category: "Financial", status: "mitigating", trend: "stable", severity: "medium", likelihood: 2, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-17", nextReview: "2026-07-12" },
    { id: "RSK-MIT-0002", title: "Ransomware attack on critical infrastructure", owner: "CRO", category: "Strategic", status: "closed", trend: "decreasing", severity: "low", likelihood: 5, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-21", nextReview: "2026-07-06" },
    { id: "RSK-MIT-0003", title: "Insider threat data exfiltration", owner: "VP Eng", category: "Compliance", status: "accepted", trend: "increasing", severity: "medium", likelihood: 9, impact: 9, treatment: "Monitor and review quarterly", lastReview: "2026-04-01", nextReview: "2026-07-22" },
    { id: "RSK-MIT-0004", title: "Supply chain compromise", owner: "Security Lead", category: "Technical", status: "open", trend: "stable", severity: "medium", likelihood: 2, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-13", nextReview: "2026-07-27" },
    { id: "RSK-MIT-0005", title: "Cloud misconfiguration exposure", owner: "Risk Mgr", category: "Reputational", status: "mitigating", trend: "decreasing", severity: "high", likelihood: 7, impact: 4, treatment: "Monitor and review quarterly", lastReview: "2026-04-06", nextReview: "2026-07-03" },
    { id: "RSK-MIT-0006", title: "Phishing campaign success rate", owner: "Compliance Dir", category: "Legal", status: "closed", trend: "increasing", severity: "high", likelihood: 3, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-11", nextReview: "2026-07-15" },
    { id: "RSK-MIT-0007", title: "Third-party vendor data breach", owner: "IT Dir", category: "Regulatory", status: "accepted", trend: "stable", severity: "medium", likelihood: 3, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-07", nextReview: "2026-07-13" },
    { id: "RSK-MIT-0008", title: "Regulatory non-compliance penalty", owner: "DevOps Lead", category: "Third-Party", status: "open", trend: "decreasing", severity: "high", likelihood: 6, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-03", nextReview: "2026-07-24" },
    { id: "RSK-MIT-0009", title: "Zero-day exploit in production", owner: "Architect", category: "Human Capital", status: "mitigating", trend: "increasing", severity: "high", likelihood: 10, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-05", nextReview: "2026-07-25" },
    { id: "RSK-MIT-0010", title: "Insufficient access controls", owner: "CISO", category: "Operational", status: "closed", trend: "stable", severity: "critical", likelihood: 5, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-11", nextReview: "2026-07-15" },
    { id: "RSK-MIT-0011", title: "DDoS attack on services", owner: "CTO", category: "Financial", status: "accepted", trend: "decreasing", severity: "critical", likelihood: 5, impact: 5, treatment: "Monitor and review quarterly", lastReview: "2026-04-09", nextReview: "2026-07-27" },
    { id: "RSK-MIT-0012", title: "Social engineering attack", owner: "CRO", category: "Strategic", status: "open", trend: "increasing", severity: "medium", likelihood: 10, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-07", nextReview: "2026-07-14" },
    { id: "RSK-MIT-0013", title: "API security vulnerability", owner: "VP Eng", category: "Compliance", status: "mitigating", trend: "stable", severity: "low", likelihood: 5, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-15", nextReview: "2026-07-02" },
    { id: "RSK-MIT-0014", title: "Mobile device compromise", owner: "Security Lead", category: "Technical", status: "closed", trend: "decreasing", severity: "critical", likelihood: 5, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-01", nextReview: "2026-07-28" },
    { id: "RSK-MIT-0015", title: "Physical security breach", owner: "Risk Mgr", category: "Reputational", status: "accepted", trend: "increasing", severity: "critical", likelihood: 3, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-17", nextReview: "2026-07-22" },
    { id: "RSK-MIT-0016", title: "Password policy weakness", owner: "Compliance Dir", category: "Legal", status: "open", trend: "stable", severity: "critical", likelihood: 8, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-19", nextReview: "2026-07-13" },
    { id: "RSK-MIT-0017", title: "Network segmentation gap", owner: "IT Dir", category: "Regulatory", status: "mitigating", trend: "decreasing", severity: "medium", likelihood: 1, impact: 5, treatment: "Monitor and review quarterly", lastReview: "2026-04-14", nextReview: "2026-07-02" },
    { id: "RSK-MIT-0018", title: "Encryption key management failure", owner: "DevOps Lead", category: "Third-Party", status: "closed", trend: "increasing", severity: "critical", likelihood: 1, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-18", nextReview: "2026-07-20" },
    { id: "RSK-MIT-0019", title: "Audit trail tampering", owner: "Architect", category: "Human Capital", status: "accepted", trend: "stable", severity: "low", likelihood: 10, impact: 5, treatment: "Monitor and review quarterly", lastReview: "2026-04-21", nextReview: "2026-07-03" },
    { id: "RSK-MIT-0020", title: "Business email compromise", owner: "CISO", category: "Operational", status: "open", trend: "decreasing", severity: "critical", likelihood: 9, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-15", nextReview: "2026-07-13" },
  ];
  private _mitre_navigaRiskFilter: string = 'all';
  private _mitre_navigaRiskSeverity: string = 'all';
  private _mitre_navigaRiskStatus: string = 'all';
  private _mitre_navigaExpandedRisk: string = '';
  private _mitre_navigaFilterRisks() {
    const reg = this._mitre_navigaRiskRegister;
    return reg.filter(r => {
      if (this._mitre_navigaRiskFilter !== "all" && r.category !== this._mitre_navigaRiskFilter) return false;
      if (this._mitre_navigaRiskSeverity !== "all" && r.severity !== this._mitre_navigaRiskSeverity) return false;
      if (this._mitre_navigaRiskStatus !== "all" && r.status !== this._mitre_navigaRiskStatus) return false;
      return true;
    });
  }
  private _mitre_navigaGetRiskScore(r: any) { return Math.round(r.likelihood * r.impact * 1.5); }
  private _mitre_navigaGetRiskTrendIcon(trend: string) {
    if (trend === "increasing") return "\u2191";
    if (trend === "decreasing") return "\u2193";
    return "\u2192";
  }
  private _mitre_navigaGetRiskColor(sev: string) {
    if (sev === "critical") return "#dc2626";
    if (sev === "high") return "#ea580c";
    if (sev === "medium") return "#d97706";
    return "#16a34a";
  }
  private _mitre_navigaGetRiskCounts() {
    const reg = this._mitre_navigaRiskRegister;
  ...
  private _mitre_navigaGetTreatmentProgress() {
    const reg = this._mitre_navigaRiskRegister;
    const treated = reg.filter(r => r.status === "mitigating" || r.status === "closed").length;
    return Math.round((treated / Math.max(reg.length, 1)) * 100);
  }

  // === Security Metrics API Gateway (mitre_naviga) ===
  private _mitre_navigaApiEndpoints = [
    { method: "GET", path: "/api/v1/threats", name: "Threat Intelligence Feed", status: "active", avgLatency: 17.1, reqPerMin: 1316.0, errorRate: 1.99, uptime: round(97.25455255051587,2), version: "v3.0.19" },
    { method: "POST", path: "/api/v1/scans/start", name: "Vulnerability Scanner", status: "active", avgLatency: 30.7, reqPerMin: 1676.0, errorRate: 0.4, uptime: round(95.69971214238612,2), version: "v3.5.13" },
    { method: "GET", path: "/api/v1/assets", name: "Asset Inventory", status: "active", avgLatency: 73.7, reqPerMin: 74.0, errorRate: 0.13, uptime: round(96.19934808856048,2), version: "v2.1.11" },
    { method: "POST", path: "/api/v1/alerts", name: "Alert Management", status: "active", avgLatency: 22.7, reqPerMin: 2157.0, errorRate: 2.32, uptime: round(97.81462432752396,2), version: "v1.9.17" },
    { method: "GET", path: "/api/v1/compliance", name: "Compliance Status", status: "active", avgLatency: 116.4, reqPerMin: 412.0, errorRate: 1.74, uptime: round(98.97994915594349,2), version: "v1.1.1" },
    { method: "PUT", path: "/api/v1/policies", name: "Policy Engine", status: "active", avgLatency: 24.1, reqPerMin: 57.0, errorRate: 1.01, uptime: round(98.65561476115832,2), version: "v1.7.3" },
    { method: "GET", path: "/api/v1/incidents", name: "Incident Tracker", status: "active", avgLatency: 380.6, reqPerMin: 678.0, errorRate: 1.16, uptime: round(95.44340202596686,2), version: "v1.5.20" },
    { method: "POST", path: "/api/v1/forensics", name: "Forensics Collector", status: "degraded", avgLatency: 276.6, reqPerMin: 1661.0, errorRate: 2.04, uptime: round(96.35988268055758,2), version: "v1.5.18" },
    { method: "GET", path: "/api/v1/risk/assess", name: "Risk Assessment", status: "active", avgLatency: 189.9, reqPerMin: 33.0, errorRate: 1.35, uptime: round(95.28071069180832,2), version: "v1.7.7" },
    { method: "POST", path: "/api/v1/auth/verify", name: "Authentication", status: "active", avgLatency: 298.4, reqPerMin: 802.0, errorRate: 0.3, uptime: round(98.67933388440659,2), version: "v2.5.12" },
    { method: "GET", path: "/api/v1/logs/audit", name: "Audit Log Query", status: "active", avgLatency: 70.0, reqPerMin: 247.0, errorRate: 1.13, uptime: round(99.90618833980969,2), version: "v2.0.4" },
    { method: "PUT", path: "/api/v1/users/roles", name: "Role Management", status: "active", avgLatency: 270.0, reqPerMin: 539.0, errorRate: 0.14, uptime: round(95.75932759991528,2), version: "v1.1.15" },
    { method: "POST", path: "/api/v1/encrypt", name: "Encryption Service", status: "active", avgLatency: 201.5, reqPerMin: 2245.0, errorRate: 0.24, uptime: round(99.13529667835071,2), version: "v3.8.20" },
    { method: "GET", path: "/api/v1/network/topo", name: "Network Topology", status: "maintenance", avgLatency: 148.9, reqPerMin: 654.0, errorRate: 0.52, uptime: round(98.74657808814305,2), version: "v3.6.8" },
    { method: "DELETE", path: "/api/v1/sessions", name: "Session Manager", status: "active", avgLatency: 299.2, reqPerMin: 1364.0, errorRate: 0.62, uptime: round(96.61423954358428,2), version: "v3.3.11" },
  ];
  private _mitre_navigaApiKeys = [
    { id: "ak-000001", name: "key-mitr-001", created: "2026-06-21", lastUsed: "2026-04-09", status: "active", calls: 22239, rateLimit: 500 },
    { id: "ak-000002", name: "key-mitr-002", created: "2026-02-04", lastUsed: "2026-04-14", status: "active", calls: 8714, rateLimit: 500 },
    { id: "ak-000003", name: "key-mitr-003", created: "2026-08-24", lastUsed: "2026-04-15", status: "active", calls: 39274, rateLimit: 5000 },
    { id: "ak-000004", name: "key-mitr-004", created: "2026-08-24", lastUsed: "2026-04-06", status: "revoked", calls: 41091, rateLimit: 500 },
    { id: "ak-000005", name: "key-mitr-005", created: "2026-01-21", lastUsed: "2026-04-11", status: "active", calls: 23529, rateLimit: 500 },
    { id: "ak-000006", name: "key-mitr-006", created: "2026-05-11", lastUsed: "2026-04-07", status: "active", calls: 44776, rateLimit: 500 },
    { id: "ak-000007", name: "key-mitr-007", created: "2026-08-13", lastUsed: "2026-04-20", status: "revoked", calls: 42822, rateLimit: 1000 },
    { id: "ak-000008", name: "key-mitr-008", created: "2026-03-05", lastUsed: "2026-04-06", status: "revoked", calls: 14837, rateLimit: 5000 },
  ];
  private _mitre_navigaApiHealthSummary() {
    const eps = this._mitre_navigaApiEndpoints;
  ...
  private _mitre_navigaGetApiByMethod(method: string) { return this._mitre_navigaApiEndpoints.filter(e=>e.method===method); }
  private _mitre_navigaGetSlowEndpoints() { return this._mitre_navigaApiEndpoints.filter(e=>e.avgLatency>200).sort((a,b)=>b.avgLatency-a.avgLatency); }
  private _mitre_navigaGetHighErrorEndpoints() { return this._mitre_navigaApiEndpoints.filter(e=>e.errorRate>1.0).sort((a,b)=>b.errorRate-a.errorRate); }

  // === Security Training Effectiveness (mitre_naviga) ===
  private _mitre_navigaTrainingModules = [
    { id: "TRN-001", name: "Security Fundamentals", completionRate: 96.8, avgScore: 55.3, behaviorChange: 65.5, enrolled: 399, completed: 340, duration: "62min", category: "mandatory" },
    { id: "TRN-002", name: "Phishing Awareness", completionRate: 60.0, avgScore: 74.1, behaviorChange: 78.6, enrolled: 436, completed: 399, duration: "99min", category: "mandatory" },
    { id: "TRN-003", name: "Social Engineering Defense", completionRate: 97.2, avgScore: 57.1, behaviorChange: 38.3, enrolled: 380, completed: 144, duration: "110min", category: "optional" },
    { id: "TRN-004", name: "Password Hygiene", completionRate: 80.5, avgScore: 87.5, behaviorChange: 41.4, enrolled: 353, completed: 426, duration: "98min", category: "mandatory" },
    { id: "TRN-005", name: "Data Classification", completionRate: 82.3, avgScore: 70.0, behaviorChange: 49.2, enrolled: 218, completed: 83, duration: "108min", category: "mandatory" },
    { id: "TRN-006", name: "Incident Response Basics", completionRate: 96.5, avgScore: 67.3, behaviorChange: 81.0, enrolled: 327, completed: 439, duration: "40min", category: "mandatory" },
    { id: "TRN-007", name: "Secure Coding", completionRate: 87.7, avgScore: 77.0, behaviorChange: 33.8, enrolled: 178, completed: 145, duration: "115min", category: "optional" },
    { id: "TRN-008", name: "Cloud Security", completionRate: 88.3, avgScore: 94.9, behaviorChange: 89.5, enrolled: 145, completed: 264, duration: "87min", category: "mandatory" },
    { id: "TRN-009", name: "Mobile Device Security", completionRate: 73.9, avgScore: 75.2, behaviorChange: 60.2, enrolled: 182, completed: 307, duration: "61min", category: "mandatory" },
    { id: "TRN-010", name: "Network Security", completionRate: 47.8, avgScore: 56.7, behaviorChange: 46.5, enrolled: 434, completed: 178, duration: "99min", category: "mandatory" },
    { id: "TRN-011", name: "Physical Security", completionRate: 78.4, avgScore: 62.8, behaviorChange: 86.9, enrolled: 252, completed: 428, duration: "51min", category: "optional" },
    { id: "TRN-012", name: "Regulatory Compliance", completionRate: 67.0, avgScore: 73.9, behaviorChange: 86.2, enrolled: 407, completed: 163, duration: "91min", category: "mandatory" },
    { id: "TRN-013", name: "Risk Management", completionRate: 96.2, avgScore: 79.9, behaviorChange: 32.4, enrolled: 150, completed: 214, duration: "38min", category: "mandatory" },
    { id: "TRN-014", name: "Cryptography Basics", completionRate: 76.8, avgScore: 93.1, behaviorChange: 32.4, enrolled: 100, completed: 300, duration: "111min", category: "mandatory" },
    { id: "TRN-015", name: "Access Control", completionRate: 55.4, avgScore: 76.9, behaviorChange: 75.3, enrolled: 198, completed: 255, duration: "50min", category: "optional" },
    { id: "TRN-016", name: "Vendor Management", completionRate: 81.2, avgScore: 86.4, behaviorChange: 68.8, enrolled: 133, completed: 169, duration: "80min", category: "mandatory" },
  ];
  private _mitre_navigaPhishingResults = [
    { month: "2026-01", sent: 202, clicked: 32, reported: 114, clickRate: round(15.841584158415841,1), reportRate: round(56.43564356435643,1) },
    { month: "2026-02", sent: 412, clicked: 78, reported: 167, clickRate: round(18.932038834951456,1), reportRate: round(40.53398058252427,1) },
    { month: "2026-03", sent: 622, clicked: 164, reported: 53, clickRate: round(26.366559485530544,1), reportRate: round(8.520900321543408,1) },
    { month: "2026-04", sent: 360, clicked: 72, reported: 146, clickRate: round(20.0,1), reportRate: round(40.55555555555556,1) },
    { month: "2026-05", sent: 604, clicked: 27, reported: 328, clickRate: round(4.470198675496689,1), reportRate: round(54.3046357615894,1) },
    { month: "2026-06", sent: 608, clicked: 121, reported: 96, clickRate: round(19.901315789473685,1), reportRate: round(15.789473684210526,1) },
  ];
  private _mitre_navigaTrainingROI = { totalInvestment: 198962, avgCostPerEmployee: 342, riskReductionPct: round(random.uniform(15,45),1), incidentReductionPct: round(random.uniform(10,35),1), complianceScoreGain: round(random.uniform(5,25),1) };
  private _mitre_navigaLearningPaths = [
    { name: "Beginner Security Analyst", totalModules: 11, completedModules: 3, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 69, enrolled: 37 },
    { name: "Advanced Threat Hunter", totalModules: 14, completedModules: 10, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 25, enrolled: 72 },
    { name: "Security Architect", totalModules: 15, completedModules: 4, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 54, enrolled: 193 },
    { name: "Incident Responder", totalModules: 14, completedModules: 2, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 41, enrolled: 146 },
    { name: "Compliance Specialist", totalModules: 10, completedModules: 7, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 77, enrolled: 94 },
    { name: "DevSecOps Engineer", totalModules: 8, completedModules: 8, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 57, enrolled: 59 },
  ];
  private _mitre_navigaGetOverallCompletion() {
    const mods = this._mitre_navigaTrainingModules;
    return round(mods.reduce((s,m)=>s+m.completionRate,0)/mods.length,1);
  }
  private _mitre_navigaGetTopPerformers() { return [...this._mitre_navigaTrainingModules].sort((a,b)=>b.avgScore-a.avgScore).slice(0,5); }
  private _mitre_navigaGetModulesNeedingAttention() { return this._mitre_navigaTrainingModules.filter(m=>m.completionRate<70||m.avgScore<65); }

  // === Security Governance Framework (mitre_naviga) ===
  private _mitre_navigaGovBodies = [
    { name: "Security Steering Committee", members: 15, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-04", nextMeeting: "2026-05-16", quorum: 4 },
    { name: "Risk Management Board", members: 11, chair: "CTO", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-10", nextMeeting: "2026-05-11", quorum: 5 },
    { name: "Data Governance Council", members: 6, chair: "CRO", meetingFreq: "Monthly", lastMeeting: "2026-04-09", nextMeeting: "2026-05-05", quorum: 7 },
    { name: "Compliance Oversight Board", members: 11, chair: "CDO", meetingFreq: "Monthly", lastMeeting: "2026-04-08", nextMeeting: "2026-05-23", quorum: 3 },
    { name: "Architecture Review Board", members: 10, chair: "VP Eng", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-03", nextMeeting: "2026-05-02", quorum: 7 },
    { name: "Change Advisory Board", members: 13, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-06", nextMeeting: "2026-05-24", quorum: 8 },
    { name: "Incident Review Board", members: 12, chair: "CIO", meetingFreq: "As needed", lastMeeting: "2026-04-16", nextMeeting: "2026-05-21", quorum: 4 },
    { name: "Vendor Risk Committee", members: 14, chair: "CFO", meetingFreq: "Quarterly", lastMeeting: "2026-04-10", nextMeeting: "2026-05-04", quorum: 7 },
  ];
  private _mitre_navigaDecisions = [
    { id: "DEC-0001", title: "Adopt zero-trust architecture framework", date: "2026-03-24", status: "approved", owner: "CISO" },
  ...
  private _mitre_navigaPolicyLifecycle = [
    { name: "Information Security Policy", version: "v1.3", status: "active", lastUpdated: "2026-04-27", nextReview: "2026-7-14", owner: "CISO" },
    { name: "Acceptable Use Policy", version: "v3.3", status: "active", lastUpdated: "2026-01-08", nextReview: "2026-8-24", owner: "Legal" },
    { name: "Data Retention Policy", version: "v3.2", status: "under-review", lastUpdated: "2026-04-12", nextReview: "2026-8-01", owner: "DPO" },
    { name: "Access Control Policy", version: "v2.0", status: "active", lastUpdated: "2026-01-24", nextReview: "2026-10-08", owner: "IAM Lead" },
    { name: "Incident Response Policy", version: "v2.3", status: "draft", lastUpdated: "2026-01-16", nextReview: "2026-11-01", owner: "IR Lead" },
    { name: "Business Continuity Plan", version: "v2.5", status: "active", lastUpdated: "2026-04-25", nextReview: "2026-10-06", owner: "BCP Mgr" },
    { name: "Vendor Management Policy", version: "v4.3", status: "active", lastUpdated: "2026-04-27", nextReview: "2026-9-28", owner: "Procurement" },
    { name: "Encryption Standard", version: "v2.1", status: "under-review", lastUpdated: "2026-02-28", nextReview: "2026-8-09", owner: "Security Arch" },
  ];
  private _mitre_navigaGovMaturityScore = { overall: 3.0, riskManagement: 4.8, compliance: 4.6, incidentResponse: 3.8, awareness: 3.6, technology: 4.4 };
  private _mitre_navigaGetPendingDecisions() { return this._mitre_navigaDecisions.filter(d=>d.status==='pending'||d.status==='in-progress'); }
  private _mitre_navigaGetActivePolicies() { return this._mitre_navigaPolicyLifecycle.filter(p=>p.status==='active'); }
  private _mitre_navigaGetEscalationPath() { return ["L1 Analyst","L2 Senior","Security Lead","CISO","Board"]; }

  // === Security Innovation Lab (mitre_naviga) ===
  private _mitre_navigaInnoProjects = [
    { id: "INN-001", name: "AI-Powered Threat Detection", description: "Machine learning models for real-time threat identification", status: "active", progress: 72, startDate: "2026-03-04", teamSize: 8, budget: 90041, milestones: 9, completedMilestones: 3 },
    { id: "INN-002", name: "Quantum-Resistant Cryptography", description: "Post-quantum encryption algorithm prototyping", status: "research", progress: 35, startDate: "2026-03-26", teamSize: 4, budget: 77485, milestones: 7, completedMilestones: 3 },
    { id: "INN-003", name: "Automated Red Teaming", description: "Autonomous penetration testing framework", status: "active", progress: 58, startDate: "2026-02-06", teamSize: 4, budget: 57073, milestones: 8, completedMilestones: 4 },
    { id: "INN-004", name: "Zero-Knowledge Authentication", description: "Privacy-preserving identity verification", status: "poc", progress: 88, startDate: "2026-02-28", teamSize: 3, budget: 55157, milestones: 6, completedMilestones: 7 },
    { id: "INN-005", name: "Blockchain Audit Trail", description: "Immutable security event logging", status: "active", progress: 45, startDate: "2026-01-07", teamSize: 8, budget: 154831, milestones: 8, completedMilestones: 7 },
    { id: "INN-006", name: "Behavioral Biometrics", description: "Continuous authentication via user behavior patterns", status: "research", progress: 22, startDate: "2026-02-25", teamSize: 3, budget: 173440, milestones: 8, completedMilestones: 2 },
    { id: "INN-007", name: "Deception Grid 2.0", description: "Advanced honeypot network with adaptive responses", status: "active", progress: 65, startDate: "2026-01-11", teamSize: 8, budget: 64555, milestones: 9, completedMilestones: 5 },
    { id: "INN-008", name: "Secure Enclave Integration", description: "Hardware-backed security for critical workloads", status: "poc", progress: 40, startDate: "2026-03-26", teamSize: 2, budget: 55148, milestones: 7, completedMilestones: 5 },
  ];
  private _mitre_navigaTechEvaluations = [
    { name: "Rust for Security Tools", status: "evaluating", score: 8.0, recommendation: "Adopt", vendor: "Open Source" },
    { name: "eBPF for Runtime Detection", status: "completed", score: 9.3, recommendation: "Adopt", vendor: "AWS" },
    { name: "Confidential Computing", status: "planned", score: 5.3, recommendation: "Investigate", vendor: "Azure" },
    { name: "Homomorphic Encryption", status: "evaluating", score: 3.0, recommendation: "Pilot", vendor: "GCP" },
    { name: "SASE Architecture", status: "completed", score: 8.2, recommendation: "Adopt", vendor: "Multiple" },
    { name: "SOAR Platform 3.0", status: "planned", score: 8.3, recommendation: "Monitor", vendor: "Splunk" },
  ];
  private _mitre_navigaCollaborationPartners = [
  ...
  private _mitre_navigaInnoMetrics = { totalProjects: 8, activeProjects: 4, avgTimeToValue: "164 days", pocSuccessRate: round(random.uniform(55,85),1), researchToProduction: round(random.uniform(20,50),1), innovationIndex: round(random.uniform(6.0,9.5),1) };
  private _mitre_navigaGetProjectByStatus(status: string) { return this._mitre_navigaInnoProjects.filter(p=>p.status===status); }
  private _mitre_navigaGetTopEvaluations() { return [...this._mitre_navigaTechEvaluations].sort((a,b)=>b.score-a.score).slice(0,3); }



  // === Compliance Dashboard Extension (mitre_naviga) ===
  private _mitre_navigaComplianceFrameworks = [
    { name: "SOC 2 Type II", description: "Trust Services Criteria", totalControls: 5, implementedControls: 2, status: "compliant", lastAudit: "2026-04-17", nextAudit: "2026-7-06", evidenceCount: 282 },
    { name: "ISO 27001", description: "Information Security Management", totalControls: 114, implementedControls: 58, status: "in-review", lastAudit: "2026-01-01", nextAudit: "2026-10-18", evidenceCount: 129 },
    { name: "PCI DSS 4.0", description: "Payment Card Industry", totalControls: 12, implementedControls: 11, status: "non-compliant", lastAudit: "2026-03-25", nextAudit: "2026-7-03", evidenceCount: 188 },
    { name: "HIPAA", description: "Health Insurance Portability", totalControls: 18, implementedControls: 18, status: "compliant", lastAudit: "2026-04-01", nextAudit: "2026-12-17", evidenceCount: 97 },
    { name: "GDPR", description: "Data Protection Regulation", totalControls: 99, implementedControls: 77, status: "partially-compliant", lastAudit: "2026-01-07", nextAudit: "2026-10-01", evidenceCount: 499 },
    { name: "NIST CSF 2.0", description: "Cybersecurity Framework", totalControls: 6, implementedControls: 5, status: "non-compliant", lastAudit: "2026-03-01", nextAudit: "2026-12-25", evidenceCount: 330 },
    { name: "FedRAMP", description: "Federal Risk Authorization", totalControls: 15, implementedControls: 9, status: "in-review", lastAudit: "2026-03-24", nextAudit: "2026-7-07", evidenceCount: 471 },
    { name: "SOX", description: "Sarbanes-Oxley Compliance", totalControls: 8, implementedControls: 7, status: "partially-compliant", lastAudit: "2026-02-11", nextAudit: "2026-10-24", evidenceCount: 408 },
    { name: "CIS Controls v8", description: "Center for Internet Security", totalControls: 18, implementedControls: 10, status: "non-compliant", lastAudit: "2026-03-09", nextAudit: "2026-12-06", evidenceCount: 209 },
    { name: "COBIT 2019", description: "IT Governance Framework", totalControls: 40, implementedControls: 33, status: "in-review", lastAudit: "2026-02-10", nextAudit: "2026-9-04", evidenceCount: 339 },
  ];
  private _mitre_navigaGetComplianceScore() {
    const fw = this._mitre_navigaComplianceFrameworks;
    return round(fw.reduce((s,f)=>s + (f.implementedControls/Math.max(f.totalControls,1))*100, 0) / fw.length, 1);
  }
  private _mitre_navigaGetGaps() {
    return this._mitre_navigaComplianceFrameworks.filter(f => f.status !== "compliant");
  }
  private _mitre_navigaAuditTrail = [
    ...
    { id: "AUD-0004", action: "Remediation completed", auditor: "Compliance Officer", date: "2026-04-09", result: "pass", findings: 4 },
    { id: "AUD-0005", action: "Policy updated", auditor: "IT Audit", date: "2026-04-03", result: "pass", findings: 1 },
    ...
    { id: "AUD-0008", action: "Exception approved", auditor: "CISO Office", date: "2026-04-06", result: "conditional", findings: 3 },
    { id: "AUD-0009", action: "Risk accepted", auditor: "Board Audit", date: "2026-04-21", result: "pass", findings: 2 },
    { id: "AUD-0010", action: "Control enhanced", auditor: "Third Party", date: "2026-04-10", result: "pass", findings: 4 },
  ];

  // === Threat Intelligence Feed Extension (mitre_naviga) ===
  private _mitre_navigaThreatActors = [
    { name: "APT-29", alias: "Cozy Bear", origin: "Russia", type: "Nation-State", severity: "high", lastActivity: "2026-04-01", targets: "Technology", indicators: 203, ttps: 49 },
    { name: "APT-41", alias: "Double Dragon", origin: "China", type: "Nation-State", severity: "critical", lastActivity: "2026-04-10", targets: "Technology", indicators: 183, ttps: 16 },
    { name: "Lazarus Group", alias: "Hidden Cobra", origin: "North Korea", type: "Nation-State", severity: "critical", lastActivity: "2026-04-17", targets: "Manufacturing", indicators: 417, ttps: 29 },
    { name: "FIN7", alias: "Carbanak", origin: "Eastern Europe", type: "Financial", severity: "high", lastActivity: "2026-04-13", targets: "Manufacturing", indicators: 350, ttps: 24 },
    { name: "Conti", alias: "Wizard Spider", origin: "Russia", type: "Ransomware", severity: "critical", lastActivity: "2026-04-18", targets: "Healthcare", indicators: 291, ttps: 14 },
    { name: "LockBit", alias: "LockBit Gang", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-08", targets: "Finance", indicators: 291, ttps: 27 },
    { name: "Cl0p", alias: "Cl0p Team", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-03", targets: "Energy", indicators: 248, ttps: 47 },
    { name: "Sandworm", alias: "Unit 74455", origin: "Russia", type: "Nation-State", severity: "critical", lastActivity: "2026-04-02", targets: "Energy", indicators: 24, ttps: 15 },
  ];
  private _mitre_navigaIoCFeed = [
    { id: "ioc-000001", type: "ip", value: "156.43.116.28", confidence: 97, source: "VirusTotal", firstSeen: "2026-04-13", lastSeen: "2026-04-03" },
    { id: "ioc-000002", type: "ip", value: "5.148.128.176", confidence: 95, source: "AlienVault", firstSeen: "2026-04-22", lastSeen: "2026-04-15" },
    { id: "ioc-000003", type: "ip", value: "115.2.139.200", confidence: 89, source: "Mandiant", firstSeen: "2026-04-06", lastSeen: "2026-04-03" },
    { id: "ioc-000004", type: "ip", value: "70.199.114.186", confidence: 78, source: "Mandiant", firstSeen: "2026-04-20", lastSeen: "2026-04-02" },
    { id: "ioc-000005", type: "ip", value: "191.147.227.1", confidence: 96, source: "AlienVault", firstSeen: "2026-04-01", lastSeen: "2026-04-23" },
    { id: "ioc-000006", type: "ip", value: "77.92.143.80", confidence: 71, source: "Mandiant", firstSeen: "2026-04-09", lastSeen: "2026-04-07" },
    { id: "ioc-000007", type: "ip", value: "71.149.59.63", confidence: 73, source: "VirusTotal", firstSeen: "2026-04-08", lastSeen: "2026-04-17" },
    { id: "ioc-000008", type: "ip", value: "203.22.48.64", confidence: 66, source: "MISP", firstSeen: "2026-04-20", lastSeen: "2026-04-12" },
    { id: "ioc-000009", type: "ip", value: "252.58.50.46", confidence: 96, source: "Mandiant", firstSeen: "2026-04-01", lastSeen: "2026-04-15" },
    { id: "ioc-000010", type: "ip", value: "92.215.153.61", confidence: 60, source: "STIX", firstSeen: "2026-04-06", lastSeen: "2026-04-20" },
    { id: "ioc-000011", type: "ip", value: "219.34.37.214", confidence: 95, source: "AlienVault", firstSeen: "2026-04-01", lastSeen: "2026-04-23" },
    { id: "ioc-000012", type: "ip", value: "56.214.115.235", confidence: 68, source: "AlienVault", firstSeen: "2026-04-17", lastSeen: "2026-04-07" },
    { id: "ioc-000013", type: "ip", value: "75.105.169.145", confidence: 73, source: "AlienVault", firstSeen: "2026-04-22", lastSeen: "2026-04-09" },
    { id: "ioc-000014", type: "ip", value: "219.147.29.4", confidence: 50, source: "Mandiant", firstSeen: "2026-04-21", lastSeen: "2026-04-17" },
    { id: "ioc-000015", type: "ip", value: "133.17.224.253", confidence: 78, source: "STIX", firstSeen: "2026-04-20", lastSeen: "2026-04-15" },
  ];
  private _mitre_navigaGetActiveThreats() { return this._mitre_navigaThreatActors.filter(a => a.severity === 'critical'); }
  private _mitre_navigaGetThreatSummary() {
    const actors = this._mitre_navigaThreatActors;
  ...
  // === Incident Management Extension (mitre_naviga) ===
  private _mitre_navigaIncidents = [
    { id: "INC-20260001", title: "Unauthorized access detected", severity: "critical", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-21T14:00", affectedAssets: 41, rootCause: "Misconfiguration" },
    { id: "INC-20260002", title: "Malware outbreak on workstation", severity: "high", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-15T08:28", affectedAssets: 45, rootCause: "Credential compromise" },
    { id: "INC-20260003", title: "Data leak from S3 bucket", severity: "medium", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-14T14:47", affectedAssets: 3, rootCause: "Zero-day" },
    { id: "INC-20260004", title: "Phishing campaign targeting finance", severity: "low", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-15T16:06", affectedAssets: 4, rootCause: "Human error" },
    { id: "INC-20260005", title: "DDoS attack on web services", severity: "critical", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-23T20:35", affectedAssets: 13, rootCause: "Policy violation" },
    { id: "INC-20260006", title: "Ransomware encryption attempt", severity: "high", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-23T19:36", affectedAssets: 40, rootCause: "Unknown" },
    { id: "INC-20260007", title: "Insider data exfiltration", severity: "medium", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-07T19:34", affectedAssets: 41, rootCause: "Misconfiguration" },
    { id: "INC-20260008", title: "API key exposure in repo", severity: "low", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-18T05:48", affectedAssets: 10, rootCause: "Credential compromise" },
    { id: "INC-20260009", title: "SQL injection on portal", severity: "critical", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-11T20:00", affectedAssets: 25, rootCause: "Zero-day" },
    { id: "INC-20260010", title: "Brute force on VPN", severity: "high", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-15T19:11", affectedAssets: 3, rootCause: "Human error" },
    { id: "INC-20260011", title: "Supply chain alert from vendor", severity: "medium", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-14T12:23", affectedAssets: 27, rootCause: "Policy violation" },
    { id: "INC-20260012", title: "Suspicious lateral movement", severity: "low", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-06T15:20", affectedAssets: 30, rootCause: "Unknown" },
  ];
  private _mitre_navigaGetIncidentStats() {
    const inc = this._mitre_navigaIncidents;
    return { total: inc.length, open: inc.filter(i=>i.status==="open").length, investigating: inc.filter(i=>i.status==="investigating").length, mttd: 16, mttr: 144 };
  }
  private _mitre_navigaGetSeverityDistribution() {
    const inc = this._mitre_navigaIncidents;
  ...
  private _initMitreNavigatorControlsMatrix() {
    this._mitre_navigatorControlEffectiveness = [
    ...
    this._mitre_navigatorControlGaps = this._identifyMitreNavigatorControlGaps();
    this._mitre_navigatorControlInvestmentPriority = this._prioritizeMitreNavigatorControlInvestment();
  }

  private _identifyMitreNavigatorControlGaps(): Array<Record<string, unknown>> {
    return this._mitre_navigatorControlEffectiveness
  ...
  private _prioritizeMitreNavigatorControlInvestment(): Array<Record<string, unknown>> {
    return this._mitre_navigatorControlEffectiveness
      .map((c: Record<string, unknown>) => ({
  ...
  private _initMitreNavigatorSocIntelHub() {
    this._mitre_navigatorThreatActors = [
    ...
    this._mitre_navigatorTtpMapping = this._buildMitreNavigatorTtpMatrix();
    this._mitre_navigatorThreatCampaigns = this._trackMitreNavigatorActiveCampaigns();
    this._mitre_navigatorIntelFeeds = this._configureMitreNavigatorIntelFeeds();
    this._mitre_navigatorThreatScores = this._calcMitreNavigatorThreatScores();
    this._mitre_navigatorPredictiveAnalysis = this._runMitreNavigatorPredictiveThreatModel();
  }

  private _buildMitreNavigatorTtpMatrix(): Array<Record<string, unknown>> {
    const techniques = [
  ...
  private _trackMitreNavigatorActiveCampaigns(): Array<Record<string, unknown>> {
    return [
  ...
  private _configureMitreNavigatorIntelFeeds(): Array<Record<string, unknown>> {
    return [
  ...
  private _calcMitreNavigatorThreatScores(): Array<Record<string, unknown>> {
    return this._mitre_navigatorThreatActors.map((actor: Record<string, unknown>) => ({
  ...
  private _runMitreNavigatorPredictiveThreatModel(): Array<Record<string, unknown>> {
    return [
  ...
  private _initMitreNavigatorWorkflowEngine() {
    this._mitre_navigatorWorkflows = [
    ...
    this._mitre_navigatorWorkflowMetrics = this._aggregateMitreNavigatorWorkflowMetrics();
    this._mitre_navigatorWorkflowBottlenecks = this._identifyMitreNavigatorBottlenecks();
    this._mitre_navigatorAutomationROI = this._calcMitreNavigatorAutomationROI();
  }

  private _aggregateMitreNavigatorWorkflowMetrics(): Record<string, unknown> {
    const totalRuns = this._mitre_navigatorWorkflows.reduce((s: number, w: Record<string, unknown>) => s + Number(w.totalRuns), 0);
    const avgSuccess = this._mitre_navigatorWorkflows.reduce((s: number, w: Record<string, unknown>) => s + Number(w.successRate), 0) / this._mitre_navigatorWorkflows.length;
    const totalAutoSteps = this._mitre_navigatorWorkflows.reduce((s: number, w: Record<string, unknown>) => s + Number(w.autoSteps), 0);
    const totalManualSteps = this._mitre_navigatorWorkflows.reduce((s: number, w: Record<string, unknown>) => s + Number(w.manualSteps), 0);
    return {
      ...
      activeWorkflows: this._mitre_navigatorWorkflows.filter((w: Record<string, unknown>) => w.active).length,
      avgStepsPerWorkflow: Math.round((totalAutoSteps + totalManualSteps) / this._mitre_navigatorWorkflows.length)
    };
  }

  private _identifyMitreNavigatorBottlenecks(): Array<Record<string, unknown>> {
    return [
  ...
  private _calcMitreNavigatorAutomationROI(): Array<Record<string, unknown>> {
    return this._mitre_navigatorWorkflows.map((w: Record<string, unknown>) => ({
  ...
  private _runMitreNavigatorPostureAssessment() {
    this._mitre_navigatorPostureDimensions = [
    ...
    this._mitre_navigatorOverallPostureScore = this._mitre_navigatorPostureDimensions.reduce(
      (s: number, d: Record<string, unknown>) => s + Number(d.score) * Number(d.weight), 0
    );
    this._mitre_navigatorPostureHistory = Array.from({ length: 30 }, (_, i) => ({
    ...
    this._mitre_navigatorPostureRecommendations = this._generateMitreNavigatorPostureRecommendations();
  }

  private _generateMitreNavigatorPostureRecommendations(): Array<Record<string, unknown>> {
    return [
  ...
  private _initMitreNavigatorRiskQuantification() {
    this._mitre_navigatorRiskScenarios = [
    ...
    this._mitre_navigatorRiskQuantification = this._calcMitreNavigatorFinancialRisk();
    this._mitre_navigatorScenarioHeatmap = this._buildMitreNavigatorScenarioHeatmap();
    this._mitre_navigatorRiskMitigationPlans = this._generateMitreNavigatorMitigationPlans();
    this._mitre_navigatorMonteCarloResults = this._runMitreNavigatorMonteCarloSimulation();
  }

  private _calcMitreNavigatorFinancialRisk(): Array<Record<string, unknown>> {
    return this._mitre_navigatorRiskScenarios.map((s: Record<string, unknown>) => ({
  ...
  private _buildMitreNavigatorScenarioHeatmap(): Array<Record<string, unknown>> {
    return this._mitre_navigatorRiskScenarios.map((s: Record<string, unknown>) => ({
      scenarioId: s.scenarioId, scenarioName: s.name,
  ...
  private _generateMitreNavigatorMitigationPlans(): Array<Record<string, unknown>> {
    return this._mitre_navigatorRiskScenarios.filter((s: Record<string, unknown>) => String(s.residualRisk) !== 'low').map((s: Record<string, unknown>) => ({
      scenarioId: s.scenarioId, scenarioName: s.name,
  ...
  private _runMitreNavigatorMonteCarloSimulation(): Record<string, unknown> {
    ...
    for (let i = 0; i < iterations; i++) {
      let annualLoss = 0;
      for (const s of this._mitre_navigatorRiskScenarios) {
    ...
    return {
      ...
      probabilityOfBreach: Math.round(this._mitre_navigatorRiskScenarios.reduce((s: number, r: Record<string, unknown>) => s + Number(r.likelihood), 0) * 100) / this._mitre_navigatorRiskScenarios.length
    };
  }

  // === Security Incident Cost Modeling ===
  private _initMitreNavigatorIncidentCostModel() {
    this._mitre_navigatorIncidentCostCategories = [
    ...
    this._mitre_navigatorTotalIncidentCost = this._mitre_navigatorIncidentCostCategories.reduce((s: number, c: Record<string, unknown>) => s + Number(c.avgCost), 0);
    this._mitre_navigatorCostBySeverity = this._modelMitreNavigatorCostBySeverity();
  }

  private _modelMitreNavigatorCostBySeverity(): Array<Record<string, unknown>> {
    return [
...
// ===== SECURITY ROI DASHBOARD (R22) =====
  ...
  private _renderBudgetPlanning() {
    const budgetData = [
      { category: "Personnel & Training", planned: 2314000, actual: 3693000, utilization: 159.6, q1: "15%", q2: "20%", q3: "18%", q4: "10%" },
      { category: "Tooling & Licensing", planned: 2321000, actual: 3706000, utilization: 159.7, q1: "18%", q2: "25%", q3: "29%", q4: "11%" },
      { category: "Infrastructure Security", planned: 2328000, actual: 3719000, utilization: 159.8, q1: "21%", q2: "30%", q3: "24%", q4: "12%" },
      { category: "Compliance & Audit", planned: 2335000, actual: 3732000, utilization: 159.8, q1: "24%", q2: "35%", q3: "19%", q4: "13%" },
      { category: "Incident Response", planned: 2342000, actual: 3745000, utilization: 159.9, q1: "27%", q2: "24%", q3: "30%", q4: "14%" },
      { category: "Third-Party Assessments", planned: 2349000, actual: 3758000, utilization: 160.0, q1: "30%", q2: "29%", q3: "25%", q4: "15%" },
      { category: "Security Awareness", planned: 2356000, actual: 3771000, utilization: 160.1, q1: "17%", q2: "34%", q3: "20%", q4: "16%" },
      { category: "Research & Innovation", planned: 2363000, actual: 3784000, utilization: 160.1, q1: "20%", q2: "23%", q3: "31%", q4: "17%" },
    ];
    ...
    const headcount = [
      { team: "SOC Tier 1", current: 16, target: 15, gap: 2, avgSalary: "123k" },
      { team: "SOC Tier 2", current: 8, target: 14, gap: 1, avgSalary: "152k" },
      { team: "Threat Intel", current: 17, target: 13, gap: 0, avgSalary: "181k" },
      { team: "Red Team", current: 9, target: 12, gap: 5, avgSalary: "99k" },
      { team: "GRC", current: 18, target: 11, gap: 4, avgSalary: "128k" },
      { team: "AppSec", current: 10, target: 10, gap: 3, avgSalary: "157k" },
      { team: "Cloud Sec", current: 2, target: 9, gap: 2, avgSalary: "186k" },
      { team: "Identity & Access", current: 11, target: 8, gap: 1, avgSalary: "104k" },
    ];
    const vendorSpend = [
      { vendor: "CrowdStrike", annual: "756k", contractEnd: "2026-09", renewalRisk: "Low", satisfaction: 5 },
      { vendor: "Palo Alto", annual: "787k", contractEnd: "2026-10", renewalRisk: "Medium", satisfaction: 4 },
      { vendor: "Splunk", annual: "67k", contractEnd: "2026-11", renewalRisk: "High", satisfaction: 3 },
      { vendor: "Qualys", annual: "98k", contractEnd: "2026-12", renewalRisk: "Low", satisfaction: 5 },
      { vendor: "Rapid7", annual: "129k", contractEnd: "2026-01", renewalRisk: "Medium", satisfaction: 4 },
      { vendor: "Mandiant", annual: "160k", contractEnd: "2026-02", renewalRisk: "High", satisfaction: 3 },
      { vendor: "Zscaler", annual: "191k", contractEnd: "2026-03", renewalRisk: "Low", satisfaction: 5 },
      { vendor: "Duo Security", annual: "222k", contractEnd: "2026-04", renewalRisk: "Medium", satisfaction: 4 },
    ];
    const roiProjections = [
      { area: "Threat Detection", investment: "240k", projectedReturn: "617k", roiMultiple: "3.9x", confidence: 92 },
      { area: "Incident Reduction", investment: "283k", projectedReturn: "664k", roiMultiple: "3.8x", confidence: 73 },
      { area: "Compliance Savings", investment: "326k", projectedReturn: "711k", roiMultiple: "3.7x", confidence: 90 },
      { area: "Automation Gains", investment: "369k", projectedReturn: "758k", roiMultiple: "3.6x", confidence: 71 },
      { area: "Risk Avoidance", investment: "412k", projectedReturn: "805k", roiMultiple: "3.5x", confidence: 88 },
    ];
    return html`
  ...
  private _renderMetricsNormalization() {
    const kpiCatalog = [
      { id: "kpi-1", name: "MTTD", owner: "SOC", unit: "minutes", target: 81, current: 80, benchmark: 92, collection: "auto", frequency: "realtime" },
      { id: "kpi-2", name: "MTTR", owner: "GRC", unit: "%", target: 84, current: 87, benchmark: 69, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-3", name: "MTTC", owner: "AppSec", unit: "score", target: 87, current: 94, benchmark: 80, collection: "manual", frequency: "weekly" },
      { id: "kpi-4", name: "Vuln SLA Compliance", owner: "Cloud Sec", unit: "count", target: 90, current: 55, benchmark: 91, collection: "auto", frequency: "monthly" },
      { id: "kpi-5", name: "Patch Coverage", owner: "Identity", unit: "days", target: 93, current: 62, benchmark: 68, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-6", name: "Phishing Click Rate", owner: "Threat Intel", unit: "minutes", target: 96, current: 69, benchmark: 79, collection: "manual", frequency: "daily" },
      { id: "kpi-7", name: "Training Completion", owner: "Security Ops", unit: "%", target: 99, current: 76, benchmark: 90, collection: "auto", frequency: "weekly" },
      { id: "kpi-8", name: "Escalation Rate", owner: "Risk Mgmt", unit: "score", target: 72, current: 83, benchmark: 67, collection: "semi-auto", frequency: "monthly" },
      { id: "kpi-9", name: "False Positive Rate", owner: "SOC", unit: "count", target: 75, current: 90, benchmark: 78, collection: "manual", frequency: "realtime" },
      { id: "kpi-10", name: "Threat Intel Actionability", owner: "GRC", unit: "days", target: 78, current: 97, benchmark: 89, collection: "auto", frequency: "daily" },
      { id: "kpi-11", name: "Endpoint Compliance", owner: "AppSec", unit: "minutes", target: 81, current: 58, benchmark: 66, collection: "semi-auto", frequency: "weekly" },
      { id: "kpi-12", name: "Cloud Misconfig Score", owner: "Cloud Sec", unit: "%", target: 84, current: 65, benchmark: 77, collection: "manual", frequency: "monthly" },
      { id: "kpi-13", name: "Identity Anomaly Rate", owner: "Identity", unit: "score", target: 87, current: 72, benchmark: 88, collection: "auto", frequency: "realtime" },
      { id: "kpi-14", name: "DLP Events", owner: "Threat Intel", unit: "count", target: 90, current: 79, benchmark: 65, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-15", name: "Vendor Risk Avg", owner: "Security Ops", unit: "days", target: 93, current: 86, benchmark: 76, collection: "manual", frequency: "weekly" },
      { id: "kpi-16", name: "Compliance Audit Pass Rate", owner: "Risk Mgmt", unit: "minutes", target: 96, current: 93, benchmark: 87, collection: "auto", frequency: "monthly" },
      { id: "kpi-17", name: "Awareness Score", owner: "SOC", unit: "%", target: 99, current: 100, benchmark: 98, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-18", name: "SOC Utilization", owner: "GRC", unit: "score", target: 72, current: 61, benchmark: 75, collection: "manual", frequency: "daily" },
      { id: "kpi-19", name: "Automation Coverage", owner: "AppSec", unit: "count", target: 75, current: 68, benchmark: 86, collection: "auto", frequency: "weekly" },
      { id: "kpi-20", name: "Risk Register Currency", owner: "Cloud Sec", unit: "days", target: 78, current: 75, benchmark: 97, collection: "semi-auto", frequency: "monthly" },
    ];
    const benchmarkSources = [
      { source: "NIST CSF", mappedKPIs: 8, alignment: 95, lastReview: "2026-06-08", status: "aligned" },
      { source: "CIS Controls v8", mappedKPIs: 3, alignment: 73, lastReview: "2026-01-03", status: "partial" },
      { source: "ISO 27001:2022", mappedKPIs: 4, alignment: 90, lastReview: "2026-02-26", status: "reviewing" },
      { source: "PCI DSS 4.0", mappedKPIs: 5, alignment: 68, lastReview: "2026-03-21", status: "aligned" },
      { source: "SOC 2 Type II", mappedKPIs: 6, alignment: 85, lastReview: "2026-04-16", status: "partial" },
      { source: "MITRE ATT&CK", mappedKPIs: 7, alignment: 63, lastReview: "2026-05-11", status: "reviewing" },
      { source: "SANS Top 20", mappedKPIs: 8, alignment: 80, lastReview: "2026-06-06", status: "aligned" },
      { source: "OWASP Top 10", mappedKPIs: 3, alignment: 97, lastReview: "2026-01-01", status: "partial" },
    ];
  ...
  private _renderThreatHuntingCampaigns() {
    const campaigns = [
      { id: "HC-1001", name: "Lateral Movement Sweep", status: "active", hypothesis: "H1: Actors using pass-the-hash for lateral movement", leadHunter: "Alice Chen", findings: 15, startDate: "2026-04-28", endDate: null, effectiveness: 44 },
      { id: "HC-1002", name: "Credential Harvesting Hunt", status: "completed", hypothesis: "H2: Actors using web shells for persistence", leadHunter: "Bob Martinez", findings: 18, startDate: "2026-03-11", endDate: "2026-05-17", effectiveness: 63 },
      { id: "HC-1003", name: "Persistence Mechanism Audit", status: "planned", hypothesis: "H3: Actors using scheduled tasks for data theft", leadHunter: "Carol Wu", findings: 21, startDate: "2026-02-22", endDate: null, effectiveness: 82 },
      { id: "HC-1004", name: "C2 Beacon Detection", status: "in-review", hypothesis: "H4: Actors using DNS tunneling for C2 communication", leadHunter: "Dave Kim", findings: 24, startDate: "2026-01-05", endDate: null, effectiveness: 42 },
      { id: "HC-1005", name: "Data Exfiltration Patterns", status: "active", hypothesis: "H5: Actors using encrypted channels for privilege escalation", leadHunter: "Eve Johnson", findings: 27, startDate: "2026-04-16", endDate: null, effectiveness: 61 },
      { id: "HC-1006", name: "Privilege Escalation Scan", status: "completed", hypothesis: "H6: Actors using token impersonation for defense evasion", leadHunter: "Frank Liu", findings: 30, startDate: "2026-03-27", endDate: "2026-06-01", effectiveness: 80 },
      { id: "HC-1007", name: "Supply Chain Implant Hunt", status: "planned", hypothesis: "H7: Actors using poisoned images for initial access", leadHunter: "Grace Park", findings: 33, startDate: "2026-02-10", endDate: null, effectiveness: 40 },
      { id: "HC-1008", name: "Insider Threat Indicators", status: "in-review", hypothesis: "H8: Actors using legitimate tools for credential access", leadHunter: "Hector Silva", findings: 36, startDate: "2026-01-21", endDate: null, effectiveness: 59 },
      { id: "HC-1009", name: "Cloud Metadata Analysis", status: "active", hypothesis: "H9: Actors using API keys for command execution", leadHunter: "Alice Chen", findings: 39, startDate: "2026-04-04", endDate: null, effectiveness: 78 },
      { id: "HC-1010", name: "DNS Tunnel Detection", status: "completed", hypothesis: "H10: Actors using encoded subdomains for exfiltration", leadHunter: "Bob Martinez", findings: 42, startDate: "2026-03-15", endDate: "2026-04-13", effectiveness: 97 },
      { id: "HC-1011", name: "Fileless Malware Search", status: "planned", hypothesis: "H11: Actors using WMI providers for discovery", leadHunter: "Carol Wu", findings: 45, startDate: "2026-02-26", endDate: null, effectiveness: 57 },
      { id: "HC-1012", name: "Zero-Day Exploit Traces", status: "in-review", hypothesis: "H12: Actors using exploit kits for collection", leadHunter: "Dave Kim", findings: 0, startDate: "2026-01-09", endDate: null, effectiveness: 76 },
    ];
    const hunterLeaderboard = [
      { hunter: "Alice Chen", campaigns: 11, findings: 24, highSeverity: 21, avgScore: 82, streak: 8 },
      { hunter: "Bob Martinez", campaigns: 8, findings: 53, highSeverity: 0, avgScore: 75, streak: 1 },
      { hunter: "Carol Wu", campaigns: 5, findings: 82, highSeverity: 5, avgScore: 68, streak: 2 },
      { hunter: "Dave Kim", campaigns: 15, findings: 111, highSeverity: 10, avgScore: 61, streak: 3 },
      { hunter: "Eve Johnson", campaigns: 12, findings: 24, highSeverity: 15, avgScore: 98, streak: 4 },
      { hunter: "Frank Liu", campaigns: 9, findings: 53, highSeverity: 20, avgScore: 91, streak: 5 },
      { hunter: "Grace Park", campaigns: 6, findings: 82, highSeverity: 25, avgScore: 84, streak: 6 },
      { hunter: "Hector Silva", campaigns: 3, findings: 111, highSeverity: 4, avgScore: 77, streak: 7 },
    ];
    const mitreMapping = [
      { tactic: "Initial Access", techniques: 7, campaigns: 4, coverage: 72 },
      { tactic: "Execution", techniques: 6, campaigns: 3, coverage: 52 },
      { tactic: "Persistence", techniques: 5, campaigns: 2, coverage: 32 },
      { tactic: "Privilege Escalation", techniques: 4, campaigns: 1, coverage: 83 },
      { tactic: "Defense Evasion", techniques: 3, campaigns: 6, coverage: 63 },
      { tactic: "Credential Access", techniques: 2, campaigns: 5, coverage: 43 },
      { tactic: "Discovery", techniques: 12, campaigns: 4, coverage: 94 },
      { tactic: "Lateral Movement", techniques: 11, campaigns: 3, coverage: 74 },
      { tactic: "Collection", techniques: 10, campaigns: 2, coverage: 54 },
      { tactic: "Exfiltration", techniques: 9, campaigns: 1, coverage: 34 },
      { tactic: "Command & Control", techniques: 8, campaigns: 6, coverage: 85 },
      { tactic: "Impact", techniques: 7, campaigns: 5, coverage: 65 },
    ];
    return html`
  ...
  private _renderControlInventory() {
    const controls = [
      { id: "CTL-2001", name: "MFA Enforcement", domain: "Access Control", status: "implemented", effectiveness: 41, lastTest: "2026-04-20", nextReview: "2026-12-20", owner: "SOC", risk: "Low" },
      { id: "CTL-2002", name: "Network Segmentation", domain: "Network Security", status: "partial", effectiveness: 11, lastTest: "2026-03-05", nextReview: "2026-05-11", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2003", name: "EDR Deployment", domain: "Endpoint Protection", status: "planned", effectiveness: 18, lastTest: "2026-02-18", nextReview: "2026-06-02", owner: "IT Ops", risk: "High" },
      { id: "CTL-2004", name: "DLP Policy", domain: "Data Protection", status: "gap", effectiveness: 25, lastTest: "2026-01-03", nextReview: "2026-07-21", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2005", name: "SSO Integration", domain: "Identity Management", status: "implemented", effectiveness: 53, lastTest: "2026-04-16", nextReview: "2026-08-12", owner: "IAM", risk: "Low" },
      { id: "CTL-2006", name: "SAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 39, lastTest: "2026-03-01", nextReview: "2026-09-03", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2007", name: "CSPM Scanning", domain: "Cloud Security", status: "planned", effectiveness: 46, lastTest: "2026-02-14", nextReview: "2026-10-22", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2008", name: "Badge Access", domain: "Physical Security", status: "gap", effectiveness: 2, lastTest: "2026-01-27", nextReview: "2026-11-13", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2009", name: "Least Privilege", domain: "Access Control", status: "implemented", effectiveness: 65, lastTest: "2026-04-12", nextReview: "2026-12-04", owner: "SOC", risk: "Low" },
      { id: "CTL-2010", name: "Firewall Rules", domain: "Network Security", status: "partial", effectiveness: 16, lastTest: "2026-03-25", nextReview: "2026-05-23", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2011", name: "Disk Encryption", domain: "Endpoint Protection", status: "planned", effectiveness: 23, lastTest: "2026-02-10", nextReview: "2026-06-14", owner: "IT Ops", risk: "High" },
      { id: "CTL-2012", name: "Data Classification", domain: "Data Protection", status: "gap", effectiveness: 30, lastTest: "2026-01-23", nextReview: "2026-07-05", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2013", name: "PAM Implementation", domain: "Identity Management", status: "implemented", effectiveness: 77, lastTest: "2026-04-08", nextReview: "2026-08-24", owner: "IAM", risk: "Low" },
      { id: "CTL-2014", name: "DAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 44, lastTest: "2026-03-21", nextReview: "2026-09-15", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2015", name: "IAM Policy Review", domain: "Cloud Security", status: "planned", effectiveness: 0, lastTest: "2026-02-06", nextReview: "2026-10-06", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2016", name: "Visitor Management", domain: "Physical Security", status: "gap", effectiveness: 7, lastTest: "2026-01-19", nextReview: "2026-11-25", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2017", name: "Access Reviews", domain: "Access Control", status: "implemented", effectiveness: 89, lastTest: "2026-04-04", nextReview: "2026-12-16", owner: "SOC", risk: "Low" },
      { id: "CTL-2018", name: "IDS/IPS Tuning", domain: "Network Security", status: "partial", effectiveness: 21, lastTest: "2026-03-17", nextReview: "2026-05-07", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2019", name: "Patch Management", domain: "Endpoint Protection", status: "planned", effectiveness: 28, lastTest: "2026-02-02", nextReview: "2026-06-26", owner: "IT Ops", risk: "High" },
      { id: "CTL-2020", name: "Backup Encryption", domain: "Data Protection", status: "gap", effectiveness: 35, lastTest: "2026-01-15", nextReview: "2026-07-17", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2021", name: "Password Policy", domain: "Identity Management", status: "implemented", effectiveness: 42, lastTest: "2026-04-28", nextReview: "2026-08-08", owner: "IAM", risk: "Low" },
      { id: "CTL-2022", name: "Container Scanning", domain: "Application Security", status: "partial", effectiveness: 49, lastTest: "2026-03-13", nextReview: "2026-09-27", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2023", name: "WAF Configuration", domain: "Cloud Security", status: "planned", effectiveness: 5, lastTest: "2026-02-26", nextReview: "2026-10-18", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2024", name: "CCTV Coverage", domain: "Physical Security", status: "gap", effectiveness: 12, lastTest: "2026-01-11", nextReview: "2026-11-09", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2025", name: "RBAC Enforcement", domain: "Access Control", status: "implemented", effectiveness: 54, lastTest: "2026-04-24", nextReview: "2026-12-28", owner: "SOC", risk: "Low" },
      { id: "CTL-2026", name: "VPN Management", domain: "Network Security", status: "partial", effectiveness: 26, lastTest: "2026-03-09", nextReview: "2026-05-19", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2027", name: "App Whitelisting", domain: "Endpoint Protection", status: "planned", effectiveness: 33, lastTest: "2026-02-22", nextReview: "2026-06-10", owner: "IT Ops", risk: "High" },
      { id: "CTL-2028", name: "Key Management", domain: "Data Protection", status: "gap", effectiveness: 40, lastTest: "2026-01-07", nextReview: "2026-07-01", owner: "Data Gov", risk: "Critical" },
    ];
    const gapAnalysis = [
  ...
  private _renderIncidentCostTracker() {
    const incidents = [
      { id: "INC-7001", name: "Security Incident 1", severity: "Critical", totalCost: 797000, responseCost: 207220, recoveryCost: 247070, legalCost: 159400, regulatoryCost: 63760, insuranceClaim: 518050, avoidedCost: 212000, date: "2026-04-12" },
      { id: "INC-7002", name: "Security Incident 2", severity: "High", totalCost: 800000, responseCost: 264000, recoveryCost: 168000, legalCost: 136000, regulatoryCost: 112000, insuranceClaim: 0, avoidedCost: 241000, date: "2026-03-21" },
      { id: "INC-7003", name: "Security Incident 3", severity: "Medium", totalCost: 803000, responseCost: 152570, recoveryCost: 256960, legalCost: 112420, regulatoryCost: 72270, insuranceClaim: 481800, avoidedCost: 270000, date: "2026-02-02" },
      { id: "INC-7004", name: "Security Incident 4", severity: "Low", totalCost: 806000, responseCost: 209560, recoveryCost: 177320, legalCost: 88660, regulatoryCost: 120900, insuranceClaim: 0, avoidedCost: 299000, date: "2026-01-11" },
      { id: "INC-7005", name: "Security Incident 5", severity: "Critical", totalCost: 809000, responseCost: 266970, recoveryCost: 266970, legalCost: 64720, regulatoryCost: 80900, insuranceClaim: 444950, avoidedCost: 328000, date: "2026-04-20" },
      { id: "INC-7006", name: "Security Incident 6", severity: "High", totalCost: 812000, responseCost: 154280, recoveryCost: 186760, legalCost: 40600, regulatoryCost: 40600, insuranceClaim: 0, avoidedCost: 357000, date: "2026-03-01" },
      { id: "INC-7007", name: "Security Incident 7", severity: "Medium", totalCost: 815000, responseCost: 211900, recoveryCost: 277100, legalCost: 146700, regulatoryCost: 89650, insuranceClaim: 407500, avoidedCost: 386000, date: "2026-02-10" },
      { id: "INC-7008", name: "Security Incident 8", severity: "Low", totalCost: 818000, responseCost: 269940, recoveryCost: 196320, legalCost: 122700, regulatoryCost: 49080, insuranceClaim: 0, avoidedCost: 415000, date: "2026-01-19" },
      { id: "INC-7009", name: "Security Incident 9", severity: "Critical", totalCost: 821000, responseCost: 155990, recoveryCost: 287350, legalCost: 98520, regulatoryCost: 98520, insuranceClaim: 369450, avoidedCost: 444000, date: "2026-04-28" },
      { id: "INC-7010", name: "Security Incident 10", severity: "High", totalCost: 824000, responseCost: 214240, recoveryCost: 206000, legalCost: 74160, regulatoryCost: 57680, insuranceClaim: 0, avoidedCost: 473000, date: "2026-03-09" },
      { id: "INC-7011", name: "Security Incident 11", severity: "Medium", totalCost: 827000, responseCost: 272910, recoveryCost: 297720, legalCost: 49620, regulatoryCost: 107510, insuranceClaim: 330800, avoidedCost: 6000, date: "2026-02-18" },
      { id: "INC-7012", name: "Security Incident 12", severity: "Low", totalCost: 830000, responseCost: 157700, recoveryCost: 215800, legalCost: 157700, regulatoryCost: 66400, insuranceClaim: 0, avoidedCost: 35000, date: "2026-01-27" },
    ];
    const yearlyTrend = [
      { month: "Jan", incidents: 5, totalCost: "249k", avgCost: "54k", insured: 55 },
      { month: "Feb", incidents: 2, totalCost: "292k", avgCost: "101k", insured: 55 },
      { month: "Mar", incidents: 10, totalCost: "335k", avgCost: "148k", insured: 55 },
      { month: "Apr", incidents: 7, totalCost: "378k", avgCost: "195k", insured: 55 },
      { month: "May", incidents: 4, totalCost: "421k", avgCost: "61k", insured: 55 },
      { month: "Jun", incidents: 12, totalCost: "464k", avgCost: "108k", insured: 55 },
    ];
