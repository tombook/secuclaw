/**
 * sc-severity-calc - Severity Calc
...
@customElement('sc-severity-calc')
export class ScSeverityCalc extends LitElement {
  static styles = css`
...
@state() private _tablePage = 1;
  ...
  @state() private _comments: CommentEntry[] = [
    { id: 'c1', author: 'soc-tier1', timestamp: '2026-04-23 04:00', text: 'Initial assessment started for Severity Calc finding #1' },
  ...
  private _items: PanelItem[] = [
      {
            "id": "severity-calc-1",
            "title": "P1 Critical | SLA: 1h",
            "severity": "critical",
            "status": "open",
            "category": "Security",
            "source": "SIEM",
            "description": "Affected: Production DB + 50K users | Data: PII + Financial | Business: Revenue impact $500K/hr",
      ...
      {
            "id": "severity-calc-2",
            "title": "P2 High | SLA: 24h",
            "severity": "high",
            "status": "in-progress",
            "category": "Network",
            "source": "EDR",
            "description": "Affected: Staging + 0 users | Data: Test data | Business: Delayed deployment",
      ...
      {
            "id": "severity-calc-3",
            "title": "P3 Medium | SLA: 7d",
            "severity": "medium",
            "status": "resolved",
            "category": "Access",
            "source": "Scanner",
            "description": "Affected: Internal wiki + 200 users | Data: Internal docs | Business: Informational impact",
      ...
      {
            "id": "severity-calc-4",
            "title": "P4 Low | SLA: 30d",
            "severity": "low",
            "status": "open",
            "category": "Compliance",
            "source": "Audit",
            "description": "Affected: Dev tools + 5 users | Data: Public repos | Business: Minimal impact",
      ...
      {
            "id": "severity-calc-5",
            "title": "Severity Calc Finding #5",
            "severity": "medium",
            "status": "open",
            "category": "Operations",
            "source": "Manual",
            "description": "Automated severity calc assessment finding detected during routine scan of infrastructure component. Review and remediation recommended.",
      ...
      {
            "id": "severity-calc-10",
            "title": "Severity Calc Finding #10",
            "severity": "low",
            "status": "open",
            "category": "Operations",
            "source": "Manual",
  ...
  private _history: HistoryEntry[] = [
    { timestamp: '2026-04-23 04:15', action: 'Auto-correlated', user: 'system', details: 'Grouped related findings from Severity Calc analysis' },
    { timestamp: '2026-04-23 03:00', action: 'Created', user: 'scanner', details: 'New Severity Calc finding detected by automated assessment' },
    ...
    { timestamp: '2026-04-22 22:00', action: 'Resolved', user: 'security-eng', details: 'Remediation applied and verified for Severity Calc finding' },
    { timestamp: '2026-04-22 18:00', action: 'Created', user: 'audit', details: 'Compliance audit identified Severity Calc gap requiring remediation' },
    ...
    { timestamp: '2026-04-21 16:00', action: 'Resolved', user: 'soc-tier1', details: 'False positive confirmed - benign activity flagged by Severity Calc scan' },
    { timestamp: '2026-04-21 12:00', action: 'Exported', user: 'compliance', details: 'Exported Severity Calc findings for Q1 compliance report' },
  ];
...
private _workflowTasks: WorkflowTask[] = [
    { id: 'wt-1', title: 'Review Severity Calc finding #1', status: 'active', assignee: 'soc-tier1', blockedBy: [], slaDeadline: '2026-04-23T12:00:00Z', priority: 'p1', createdAt: '2026-04-23T00:00:00Z', completedAt: null },
    ...
    { id: 'wt-3', title: 'Update Severity Calc detection rules', status: 'blocked', assignee: 'soc-tier2', blockedBy: ['wt-2'], slaDeadline: '2026-04-24T00:00:00Z', priority: 'p2', createdAt: '2026-04-23T03:00:00Z', completedAt: null },
    ...
    { id: 'wt-8', title: 'Archive resolved Severity Calc findings', status: 'failed', assignee: 'soc-tier1', blockedBy: [], slaDeadline: '2026-04-23T08:00:00Z', priority: 'p4', createdAt: '2026-04-22T22:00:00Z', completedAt: null },
  ];

private _executionHistory: ExecutionRecord[] = [
    {
      id: 'exec-1', name: 'Severity Calc Full Assessment Run', startedAt: '2026-04-22T10:00:00Z', completedAt: '2026-04-22T10:12:00Z', status: 'success',
      steps: [
    ...
    {
      id: 'exec-2', name: 'Severity Calc Delta Scan', startedAt: '2026-04-23T02:00:00Z', completedAt: '2026-04-23T02:05:00Z', status: 'failed',
      steps: [
  ...
  private _runExecution() {
    ...
    const exec: ExecutionRecord = {
      id: 'exec-' + Date.now(), name: 'Severity Calc Assessment ' + new Date().toISOString().slice(0, 10), startedAt: new Date().toISOString(), completedAt: null, status: 'running', steps,
    };
  ...
  private _renderExecutionPanel(): unknown {
    ...
    return html`
      <div class="form-section">
        ...
        ` : html`<div class="empty-state">Click "Run Assessment" to start the Severity Calc analysis pipeline</div>`}
      </div>
      <div class="form-section">
        ...
        <div class="settings-card">
          ...
          <div style="display:flex;gap:8px;margin-top:4px;">
            <button class="btn primary" @click=${() => {
              ...
              const a = document.createElement('a'); a.href = url; a.download = 'severity-calc-config.json'; a.click();
    ...
    return html`
      <div style="margin-top:12px;">
        ...
        <div style="display:flex;gap:6px;margin-top:8px;">
          <input class="search-box" type="text" placeholder="Add a comment..." style="min-width:auto;flex:1;" id="severity-calc-comment-input" />
          <button class="btn primary" @click=${() => {
            const input = this.shadowRoot?.querySelector('#severity-calc-comment-input') as HTMLInputElement;
  ...
  private _exportJSON() {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'sc-severity-calc-data.json'; a.click();
    URL.revokeObjectURL(url);
  }
  ...
  private _exportCSV() {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'sc-severity-calc-data.csv'; a.click();
    URL.revokeObjectURL(url);
  }


  // --- Domain Rules Engine ---
  @state() private _svcRules: { id: string; name: string; category: string; severity: Severity; enabled: boolean; lastEval: string; passRate: number }[] = [];
  private _initSvcRules() {
    const rules = [
    ...
    this._svcRules = rules;
  }
  private _evaluateSvcRules(): { passed: number; failed: number; skipped: number; total: number } {
    let passed = 0, failed = 0, skipped = 0;
    this._svcRules.forEach(r => { if (!r.enabled) { skipped++; } else if (r.passRate >= 80) { passed++; } else { failed++; } });
    return { passed, failed, skipped, total: this._svcRules.length };
  }

  // --- CVSS Scoring ---
  @state() private _svccvssData: { itemId: string; vector: string; base: number; temporal: number; environmental: number; overall: number }[] = [];
  private _initSvcCvss() {
    ...
    this._svccvssData = vectors.map((v, i) => {
  ...
  // --- Anomaly Detection ---
  @state() private _svcanomalies: { id: string; type: string; severity: Severity; description: string; detected: string; confidence: number; affected: string[] }[] = [];
  private _runSvcAnomalyDetection() {
    const types = [
    ...
    this._svcanomalies = types.map((a, i) => ({
  ...
  // --- Trend Prediction ---
  @state() private _svcpredictions: { horizon: string; metric: string; current: number; predicted: number; direction: 'up' | 'down' | 'stable'; confidence: number }[] = [];
  private _generateSvcPredictions() {
    this._svcpredictions = [
  ...
  // --- Approval Workflow ---
  @state() private _svcApprovals: { id: string; title: string; requester: string; status: 'pending' | 'approved' | 'rejected' | 'expired'; createdAt: string; priority: Priority; type: string }[] = [];
  private _initSvcApprovals() {
    this._svcApprovals = [
  ...
  private _approveSvcItem(id: string) { const item = this._svcApprovals.find(a => a.id === id); if (item) item.status = 'approved'; this.requestUpdate(); }
  private _rejectSvcItem(id: string) { const item = this._svcApprovals.find(a => a.id === id); if (item) item.status = 'rejected'; this.requestUpdate(); }

  // --- Activity Feed ---
  @state() private _svcActivity: { id: string; action: string; user: string; target: string; timestamp: string }[] = [];
  private _initSvcActivity() {
    const actions = [
    ...
    this._svcActivity = actions.map((a, i) => ({ id: 'ACT-' + String(i + 1).padStart(3, '0'), ...a, timestamp: new Date(Date.now() - i * 3600000).toISOString() }));
  }

  // --- Notification System ---
  @state() private _svcNotifications: { id: string; message: string; type: 'info' | 'warning' | 'error' | 'success'; read: boolean; timestamp: string }[] = [];
  private _initSvcNotifications() {
    this._svcNotifications = [
  ...
  private _markSvcNotifRead(id: string) { const n = this._svcNotifications.find(x => x.id === id); if (n) n.read = true; this.requestUpdate(); }

  // --- Panel Configuration ---
  @state() private _svcConfig: { layout: 'compact' | 'default' | 'expanded'; theme: 'dark' | 'midnight' | 'slate'; showAnomalies: boolean; showPredictions: boolean; autoRefresh: boolean; refreshInterval: number } = {
    layout: 'default', theme: 'dark', showAnomalies: true, showPredictions: true, autoRefresh: true, refreshInterval: 60,
  };
  private _svcPresets: { name: string; config: typeof this._svcConfig }[] = [
  ...
  private _applySvcPreset(preset: typeof this._svcPresets[0]) { this._svcConfig = { ...preset.config }; this.requestUpdate(); }

  private _renderSvcTreemapSVG(): string {
    const categories = [
  ...
  private _renderSvcSankeySVG(): string {
  ...
  // --- Render: Rules Engine ---
  private _renderSvcRules(): any {
    const ev = this._evaluateSvcRules();
    return html`
  ...
  private _renderSvcAnomalies(): any {
  ...
  // --- Render: Approvals ---
  private _renderSvcApprovals(): any {
          ...
          $${this._svcApprovals.map(a => html`
            <div style="padding:6px 8px;background:#1f2937;border-radius:4px">
              ...
              $${a.status === 'pending' ? html`
                <div style="display:flex;gap:4px;margin-top:4px">
                  <button class="btn success" style="padding:2px 8px;font-size:9px" @click=$${() => this._approveSvcItem(a.id)}>Approve</button>
                  <button class="btn error" style="padding:2px 8px;font-size:9px" @click=$${() => this._rejectSvcItem(a.id)}>Reject</button>
                </div>` : nothing}
            </div>`)}
        </div>
      </div>`;
  ...
  private _renderSvcNotifications(): any {
    const tc = (t: string) => t === 'error' ? '#ef4444' : t === 'warning' ? '#eab308' : t === 'success' ? '#22c55e' : '#3b82f6';
    const unread = this._svcNotifications.filter(n => !n.read).length;
    return html`
      <div style="background:#1a1d27;border-radius:8px;padding:12px;margin-top:12px">
        ...
        <div style="display:flex;flex-direction:column;gap:4px;max-height:120px;overflow-y:auto">
          $${this._svcNotifications.map(n => html`
            <div style="display:flex;align-items:center;gap:8px;padding:4px 8px;background:$${n.read ? '#1f2937' : '#252a36'};border-radius:4px;font-size:10px;opacity:$${n.read ? '0.6' : '1'};cursor:pointer" @click=$${() => this._markSvcNotifRead(n.id)}>
      ...
      <div style="background:#1a1d27;border-radius:8px;padding:12px;margin-top:12px">
        ...
        <div style="display:flex;flex-direction:column;gap:6px;font-size:10px">
          <div style="display:flex;align-items:center;gap:8px">
            ...
            <select class="form-input" style="flex:1" @change=$${(e: Event) => { this._svcConfig.layout = (e.target as HTMLSelectElement).value as any; this.requestUpdate(); }}>
              <option value="compact" ?selected=$${this._svcConfig.layout === 'compact'}>Compact</option>
              <option value="default" ?selected=$${this._svcConfig.layout === 'default'}>Default</option>
              <option value="expanded" ?selected=$${this._svcConfig.layout === 'expanded'}>Expanded</option>
            </select>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:100px">Theme</span>
            <select class="form-input" style="flex:1" @change=$${(e: Event) => { this._svcConfig.theme = (e.target as HTMLSelectElement).value as any; this.requestUpdate(); }}>
              <option value="dark" ?selected=$${this._svcConfig.theme === 'dark'}>Dark</option>
              <option value="midnight" ?selected=$${this._svcConfig.theme === 'midnight'}>Midnight</option>
              <option value="slate" ?selected=$${this._svcConfig.theme === 'slate'}>Slate</option>
            </select>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:100px">Auto Refresh</span>
            <input type="checkbox" ?checked=$${this._svcConfig.autoRefresh} @change=$${() => { this._svcConfig.autoRefresh = !this._svcConfig.autoRefresh; this.requestUpdate(); }}/>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:100px">Show Anomalies</span>
            <input type="checkbox" ?checked=$${this._svcConfig.showAnomalies} @change=$${() => { this._svcConfig.showAnomalies = !this._svcConfig.showAnomalies; this.requestUpdate(); }}/>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:100px">Show Predictions</span>
            <input type="checkbox" ?checked=$${this._svcConfig.showPredictions} @change=$${() => { this._svcConfig.showPredictions = !this._svcConfig.showPredictions; this.requestUpdate(); }}/>
          </div>
          <div style="margin-top:6px;font-weight:600">Presets</div>
          <div style="display:flex;gap:4px">
            $${this._svcPresets.map(ps => html`<button class="btn" style="padding:2px 8px;font-size:9px" @click=$${() => this._applySvcPreset(ps)}>$${ps.name}</button>`)}
          </div>
        </div>
      </div>`;
  }
  ...
  @state() private _severityCalcContainerScans: Array<{image:string;registry:string;critical:number;high:number;medium:number;scanDate:string;status:string}> = [
  ...
  @state() private _severityCalcK8sPods: Array<{namespace:string;pod:string;securityContext:string;hostIPC:boolean;hostPID:boolean;privileged:boolean;riskLevel:string}> = [
  ...
  @state() private _severityCalcServerlessRisk: Array<{function:string;runtime:string;timeout:number;iamPerms:string;externalCalls:number;riskScore:number}> = [
  ...
  @state() private _severityCalcRuntimeAlerts: Array<{id:string;workload:string;alertType:string;severity:string;description:string;timestamp:string}> = [
  ...
  private _renderSeveritycalcCloudWl(): TemplateResult {
    const containers = this._severityCalcContainerScans;
    const pods = this._severityCalcK8sPods;
    const alerts = this._severityCalcRuntimeAlerts;
    return html`
  ...
  @state() private _severityCalcRiskTopTen: Array<{id:string;name:string;score:number;trend:string;owner:string;category:string;lastAssessed:string}> = [
  ...
  @state() private _severityCalcRiskCategories: Array<{category:string;count:number;avgScore:number;color:string}> = [
  ...
  @state() private _severityCalcRiskVelocity: Array<{week:string;newRisks:number;closedRisks:number;netChange:number}> = [
  ...
  @state() private _severityCalcRiskAppetite: {current:number;threshold:number;maxTolerated:number;status:string} = {
    current: 68, threshold: 55, maxTolerated: 80, status: 'Warning'
  };
  @state() private _severityCalcRiskOwnerMatrix: Array<{owner:string;criticalCount:number;highCount:number;mediumCount:number;complianceRate:number}> = [
  ...
  private _renderSeveritycalcRiskDash(): TemplateResult {
    const riskItems = this._severityCalcRiskTopTen;
    const velocity = this._severityCalcRiskVelocity;
    const appetite = this._severityCalcRiskAppetite;
    return html`
  ...
  @state() private _severityCalcCorrRules: Array<{id:string;name:string;sources:string[];logic:string;severity:string;active:boolean;lastTriggered:string}> = [
  ...
  @state() private _severityCalcEventTimeline: Array<{timestamp:string;source:string;eventType:string;details:string;correlated:boolean}> = [
  ...
  @state() private _severityCalcFalsePosMetrics: {totalEvents:number;correlatedEvents:number;falsePositives:number;fpRate:number;topFpRules:string[]} = {
  ...
  @state() private _severityCalcEventPatterns: Array<{id:string;pattern:string;frequency:number;firstSeen:string;lastSeen:string;status:string}> = [
  ...
  private _renderSeveritycalcEventCorr(): TemplateResult {
    const rules = this._severityCalcCorrRules;
    const timeline = this._severityCalcEventTimeline;
    const fpMetrics = this._severityCalcFalsePosMetrics;
    return html`
  ...
  @state() private _severityCalcCourses: Array<{id:string;title:string;category:string;duration:number;enrolled:number;completed:number;difficulty:string;rating:number}> = [
  ...
  @state() private _severityCalcLearningPaths: Array<{id:string;name:string;courseIds:string[];progress:number;enrolled:number}> = [
  ...
  @state() private _severityCalcDeptCompliance: Array<{dept:string;trainedPct:number;targetPct:number;avgScore:number;certCount:number}> = [
  ...
  @state() private _severityCalcSkillsGaps: Array<{skill:string;current:number;required:number;gap:number;priority:string}> = [
  ...
  private _renderSeveritycalcTraining(): TemplateResult {
    const courses = this._severityCalcCourses;
    const deptComp = this._severityCalcDeptCompliance;
    return html`
  ...
  @state() private _complianceCalendarEvents: Array<{id: string; title: string; deadline: string; category: string; status: string; assignee: string; notes: string}> = [
    {id: 'cc-severity-calc-001', title: 'SOX Section 404 Compliance Review', deadline: '2026-06-30', category: 'Regulatory', status: 'In Progress', assignee: 'Compliance Team', notes: 'Annual internal control assessment for financial reporting'},
    {id: 'cc-severity-calc-002', title: 'GDPR Data Protection Impact Assessment', deadline: '2026-05-15', category: 'Privacy', status: 'Pending', assignee: 'DPO Office', notes: 'Quarterly DPIA for high-risk processing activities'},
    {id: 'cc-severity-calc-003', title: 'ISO 27001 Surveillance Audit', deadline: '2026-07-20', category: 'Certification', status: 'Scheduled', assignee: 'QA Lead', notes: 'Stage 2 surveillance audit for ISMS certification renewal'},
    {id: 'cc-severity-calc-004', title: 'PCI DSS v4.0 Gap Analysis', deadline: '2026-05-30', category: 'Payment', status: 'Not Started', assignee: 'Security Architect', notes: 'Assess current state against PCI DSS v4.0 requirements'},
    {id: 'cc-severity-calc-005', title: 'NIST CSF Self-Assessment', deadline: '2026-08-15', category: 'Framework', status: 'Pending', assignee: 'CISO', notes: 'Biannual self-assessment against NIST Cybersecurity Framework'},
    {id: 'cc-severity-calc-006', title: 'HIPAA Security Rule Audit', deadline: '2026-06-15', category: 'Healthcare', status: 'In Progress', assignee: 'Compliance Analyst', notes: 'Annual audit of administrative, physical, and technical safeguards'},
    {id: 'cc-severity-calc-007', title: 'SOC 2 Type II Report Renewal', deadline: '2026-09-30', category: 'Audit', status: 'Scheduled', assignee: 'External Auditor', notes: 'Engage auditor for SOC 2 Type II examination period'},
    {id: 'cc-severity-calc-008', title: 'FedRAMP Authorization Review', deadline: '2026-07-01', category: 'Government', status: 'Pending', assignee: 'FedRAMP PMO', notes: 'Review and update security authorization package'},
    {id: 'cc-severity-calc-009', title: 'Annual Penetration Test', deadline: '2026-10-15', category: 'Testing', status: 'Not Started', assignee: 'Red Team Lead', notes: 'Comprehensive external and internal penetration testing engagement'},
    {id: 'cc-severity-calc-010', title: 'Security Awareness Training Rollout', deadline: '2026-05-01', category: 'Training', status: 'Completed', assignee: 'Security Awareness Manager', notes: 'Q2 organization-wide security awareness training program'},
    {id: 'cc-severity-calc-011', title: 'Vendor Security Review Cycle', deadline: '2026-06-01', category: 'Third Party', status: 'In Progress', assignee: 'Vendor Manager', notes: 'Quarterly review of critical vendor security posture and SLA compliance'},
    {id: 'cc-severity-calc-012', title: 'Incident Response Plan Update', deadline: '2026-05-20', category: 'Operations', status: 'Pending', assignee: 'IR Team Lead', notes: 'Update IR procedures based on lessons learned from recent incidents'},
  ];
  ...
  @state() private _dataQualityMetrics: Array<{id: string; dataSource: string; completenessScore: number; accuracyScore: number; timelinessScore: number; consistencyScore: number; reliabilityRank: number; lastUpdated: string; issues: string[]; trend: string}> = [
    {id: 'dq-severity-calc-001', dataSource: 'Vulnerability Scanner Feed', completenessScore: 94, accuracyScore: 91, timelinessScore: 88, consistencyScore: 95, reliabilityRank: 1, lastUpdated: '2026-04-23T08:00:00Z', issues: ['Minor delay in Nessus plugin updates'], trend: 'improving'},
    {id: 'dq-severity-calc-002', dataSource: 'SIEM Event Logs', completenessScore: 87, accuracyScore: 93, timelinessScore: 96, consistencyScore: 89, reliabilityRank: 2, lastUpdated: '2026-04-23T07:30:00Z', issues: ['Some log sources showing gaps in off-hours'], trend: 'stable'},
    {id: 'dq-severity-calc-003', dataSource: 'Asset Inventory Database', completenessScore: 82, accuracyScore: 78, timelinessScore: 75, consistencyScore: 84, reliabilityRank: 4, lastUpdated: '2026-04-22T18:00:00Z', issues: ['Shadow IT devices not fully cataloged', 'Decommissioned assets still listed'], trend: 'declining'},
    {id: 'dq-severity-calc-004', dataSource: 'Threat Intelligence Platform', completenessScore: 91, accuracyScore: 89, timelinessScore: 92, consistencyScore: 87, reliabilityRank: 3, lastUpdated: '2026-04-23T06:00:00Z', issues: ['Some IOC sources lack confidence scoring'], trend: 'improving'},
    {id: 'dq-severity-calc-005', dataSource: 'Identity Provider Logs', completenessScore: 96, accuracyScore: 97, timelinessScore: 98, consistencyScore: 96, reliabilityRank: 1, lastUpdated: '2026-04-23T09:00:00Z', issues: [], trend: 'stable'},
    {id: 'dq-severity-calc-006', dataSource: 'Cloud Security Posture Data', completenessScore: 79, accuracyScore: 85, timelinessScore: 82, consistencyScore: 81, reliabilityRank: 5, lastUpdated: '2026-04-23T05:00:00Z', issues: ['Multi-cloud coverage gaps', 'API rate limiting affects scan frequency'], trend: 'declining'},
    {id: 'dq-severity-calc-007', dataSource: 'Compliance Evidence Store', completenessScore: 88, accuracyScore: 86, timelinessScore: 80, consistencyScore: 83, reliabilityRank: 4, lastUpdated: '2026-04-21T14:00:00Z', issues: ['Manual evidence collection delays', 'Inconsistent formatting across frameworks'], trend: 'stable'},
    {id: 'dq-severity-calc-008', dataSource: 'Network Flow Data', completenessScore: 92, accuracyScore: 90, timelinessScore: 94, consistencyScore: 91, reliabilityRank: 2, lastUpdated: '2026-04-23T08:30:00Z', issues: ['Encrypted traffic classification accuracy needs improvement'], trend: 'improving'},
  ];
  ...
  @state() private _workflowTemplates: Array<{id: string; name: string; description: string; category: string; steps: number; avgDuration: string; successRate: number; lastRun: string; manualInterventions: number; optimizationScore: number}> = [
    {id: 'wf-severity-calc-001', name: 'Vulnerability Triage Pipeline', description: 'Automated vulnerability assessment, prioritization, and assignment workflow', category: 'Vulnerability Management', steps: 8, avgDuration: '12 min', successRate: 94.5, lastRun: '2026-04-23T06:00:00Z', manualInterventions: 2, optimizationScore: 87},
    {id: 'wf-severity-calc-002', name: 'Incident Response Orchestration', description: 'End-to-end IR workflow from detection to containment and recovery', category: 'Incident Response', steps: 15, avgDuration: '45 min', successRate: 89.2, lastRun: '2026-04-22T14:30:00Z', manualInterventions: 5, optimizationScore: 72},
    {id: 'wf-severity-calc-003', name: 'Access Request Approval Chain', description: 'Multi-level approval workflow for privileged access requests', category: 'Identity & Access', steps: 6, avgDuration: '4 hours', successRate: 97.1, lastRun: '2026-04-23T09:15:00Z', manualInterventions: 1, optimizationScore: 92},
    {id: 'wf-severity-calc-004', name: 'Compliance Evidence Collection', description: 'Automated gathering and packaging of audit evidence artifacts', category: 'Compliance', steps: 10, avgDuration: '30 min', successRate: 91.8, lastRun: '2026-04-21T10:00:00Z', manualInterventions: 3, optimizationScore: 78},
    {id: 'wf-severity-calc-005', name: 'Security Patch Deployment', description: 'Staged patch rollout with validation and rollback capability', category: 'Patch Management', steps: 12, avgDuration: '2 hours', successRate: 96.3, lastRun: '2026-04-20T22:00:00Z', manualInterventions: 1, optimizationScore: 85},
    {id: 'wf-severity-calc-006', name: 'Threat Intelligence Enrichment', description: 'IOC enrichment and correlation with internal threat data', category: 'Threat Intelligence', steps: 7, avgDuration: '8 min', successRate: 93.7, lastRun: '2026-04-23T07:00:00Z', manualInterventions: 0, optimizationScore: 95},
    {id: 'wf-severity-calc-007', name: 'Security Reporting Pipeline', description: 'Automated generation and distribution of security metrics reports', category: 'Reporting', steps: 5, avgDuration: '15 min', successRate: 98.9, lastRun: '2026-04-23T08:00:00Z', manualInterventions: 0, optimizationScore: 97},
    {id: 'wf-severity-calc-008', name: 'Vendor Risk Assessment', description: 'Automated vendor security questionnaire distribution and scoring', category: 'Third Party', steps: 9, avgDuration: '3 days', successRate: 85.4, lastRun: '2026-04-19T11:00:00Z', manualInterventions: 4, optimizationScore: 65},
  ];
  ...
  @state() private _workflowExecutionHistory: Array<{workflowId: string; runDate: string; duration: string; status: string; trigger: string}> = [
    {workflowId: 'wf-severity-calc-001', runDate: '2026-04-23T06:00:00Z', duration: '11 min 23 sec', status: 'Success', trigger: 'Scheduled'},
    {workflowId: 'wf-severity-calc-002', runDate: '2026-04-22T14:30:00Z', duration: '42 min 15 sec', status: 'Success', trigger: 'Alert'},
    {workflowId: 'wf-severity-calc-003', runDate: '2026-04-23T09:15:00Z', duration: '3h 45 min', status: 'Success', trigger: 'User Request'},
    {workflowId: 'wf-severity-calc-001', runDate: '2026-04-22T06:00:00Z', duration: '13 min 08 sec', status: 'Partial', trigger: 'Scheduled'},
    {workflowId: 'wf-severity-calc-004', runDate: '2026-04-21T10:00:00Z', duration: '28 min 42 sec', status: 'Success', trigger: 'Scheduled'},
  ];
  ...
  @state() private _knowledgeArticles: Array<{id: string; title: string; category: string; author: string; publishDate: string; views: number; rating: number; tags: string[]; summary: string; readTime: string}> = [
    {id: 'kb-severity-calc-001', title: 'Zero Trust Architecture Implementation Guide', category: 'Architecture', author: 'Security Architecture Team', publishDate: '2026-03-15', views: 1842, rating: 4.8, tags: ['zero-trust', 'network', 'identity'], summary: 'Comprehensive guide for implementing zero trust principles across enterprise infrastructure', readTime: '15 min'},
    {id: 'kb-severity-calc-002', title: 'Ransomware Defense Playbook 2026', category: 'Threats', author: 'Threat Intelligence Team', publishDate: '2026-04-01', views: 2341, rating: 4.9, tags: ['ransomware', 'incident-response', 'defense'], summary: 'Updated playbook with latest ransomware tactics, techniques, and defensive measures', readTime: '12 min'},
    {id: 'kb-severity-calc-003', title: 'Cloud Security Best Practices for Multi-Cloud', category: 'Cloud', author: 'Cloud Security Team', publishDate: '2026-02-28', views: 1567, rating: 4.5, tags: ['cloud', 'aws', 'azure', 'gcp'], summary: 'Best practices for securing workloads across multiple cloud service providers', readTime: '18 min'},
    {id: 'kb-severity-calc-004', title: 'API Security Testing Methodology', category: 'Testing', author: 'Red Team', publishDate: '2026-03-22', views: 987, rating: 4.6, tags: ['api', 'testing', 'owasp'], summary: 'Structured methodology for identifying and testing API security vulnerabilities', readTime: '10 min'},
    {id: 'kb-severity-calc-005', title: 'Supply Chain Security Risk Management', category: 'Third Party', author: 'GRC Team', publishDate: '2026-01-20', views: 1234, rating: 4.3, tags: ['supply-chain', 'vendor', 'risk'], summary: 'Framework for managing security risks in the software supply chain', readTime: '14 min'},
    {id: 'kb-severity-calc-006', title: 'Kubernetes Security Hardening Checklist', category: 'Container', author: 'Platform Security', publishDate: '2026-04-10', views: 2103, rating: 4.7, tags: ['kubernetes', 'container', 'hardening'], summary: 'Step-by-step hardening checklist for production Kubernetes clusters', readTime: '11 min'},
    {id: 'kb-severity-calc-007', title: 'Security Metrics That Matter for Executives', category: 'Metrics', author: 'CISO Office', publishDate: '2026-03-05', views: 876, rating: 4.4, tags: ['metrics', 'kpi', 'reporting'], summary: 'Key security metrics and KPIs that resonate with board-level stakeholders', readTime: '8 min'},
    {id: 'kb-severity-calc-008', title: 'Phishing Simulation Campaign Design', category: 'Awareness', author: 'Security Awareness Team', publishDate: '2026-02-15', views: 1456, rating: 4.5, tags: ['phishing', 'simulation', 'training'], summary: 'Designing effective phishing simulations that drive real behavioral change', readTime: '9 min'},
    {id: 'kb-severity-calc-009', title: 'Data Loss Prevention Strategy and Implementation', category: 'Data Protection', author: 'Data Security Team', publishDate: '2026-03-28', views: 1678, rating: 4.6, tags: ['dlp', 'data-protection', 'compliance'], summary: 'Strategic approach to DLP covering people, process, and technology layers', readTime: '16 min'},
    {id: 'kb-severity-calc-010', title: 'Security Automation with SOAR Platforms', category: 'Automation', author: 'SOC Engineering', publishDate: '2026-04-05', views: 1098, rating: 4.7, tags: ['soar', 'automation', 'soc'], summary: 'Building effective security automation playbooks using SOAR technology', readTime: '13 min'},
    {id: 'kb-severity-calc-011', title: 'Third-Party Risk Assessment Framework', category: 'GRC', author: 'Vendor Management', publishDate: '2026-01-30', views: 1345, rating: 4.4, tags: ['vendor', 'risk', 'assessment'], summary: 'Structured framework for evaluating and monitoring third-party security risks', readTime: '12 min'},
    {id: 'kb-severity-calc-012', title: 'Network Detection and Response Best Practices', category: 'Network', author: 'Network Security Team', publishDate: '2026-03-18', views: 1567, rating: 4.5, tags: ['ndr', 'network', 'detection'], summary: 'Implementing effective network detection and response capabilities', readTime: '14 min'},
    {id: 'kb-severity-calc-013', title: 'Security Chaos Engineering Guide', category: 'Resilience', author: 'SRE Security', publishDate: '2026-04-12', views: 789, rating: 4.3, tags: ['chaos-engineering', 'resilience', 'testing'], summary: 'Applying chaos engineering principles to validate security controls', readTime: '10 min'},
    {id: 'kb-severity-calc-014', title: 'Incident Communication Templates', category: 'Incident Response', author: 'IR Team', publishDate: '2026-02-22', views: 2234, rating: 4.8, tags: ['incident', 'communication', 'templates'], summary: 'Ready-to-use communication templates for security incident scenarios', readTime: '7 min'},
    {id: 'kb-severity-calc-015', title: 'Endpoint Detection and Response Configuration', category: 'Endpoint', author: 'Endpoint Security', publishDate: '2026-03-08', views: 1345, rating: 4.5, tags: ['edr', 'endpoint', 'configuration'], summary: 'Optimal EDR configuration for maximum detection with minimal false positives', readTime: '11 min'},
    {id: 'kb-severity-calc-016', title: 'Security Code Review Standards', category: 'Development', author: 'AppSec Team', publishDate: '2026-02-10', views: 1890, rating: 4.6, tags: ['code-review', 'secure-sdlc', 'standards'], summary: 'Standards and checklists for security-focused code reviews', readTime: '13 min'},
    {id: 'kb-severity-calc-017', title: 'IoT Security Assessment Methodology', category: 'IoT', author: 'IoT Security Lab', publishDate: '2026-04-08', views: 654, rating: 4.2, tags: ['iot', 'embedded', 'assessment'], summary: 'Methodology for assessing security posture of IoT devices and ecosystems', readTime: '15 min'},
    {id: 'kb-severity-calc-018', title: 'Passwordless Authentication Migration Guide', category: 'Identity', author: 'IAM Team', publishDate: '2026-03-25', views: 1678, rating: 4.7, tags: ['passwordless', 'authentication', 'mfa'], summary: 'Step-by-step migration plan from password-based to passwordless authentication', readTime: '12 min'},
    {id: 'kb-severity-calc-019', title: 'Security Awareness Program Metrics', category: 'Awareness', author: 'Security Culture Team', publishDate: '2026-01-25', views: 1123, rating: 4.4, tags: ['awareness', 'metrics', 'culture'], summary: 'Measuring the effectiveness of your security awareness and training programs', readTime: '9 min'},
    {id: 'kb-severity-calc-020', title: 'Deception Technology Deployment Guide', category: 'Detection', author: 'Blue Team', publishDate: '2026-04-15', views: 876, rating: 4.5, tags: ['deception', 'honeypot', 'detection'], summary: 'Planning and deploying deception technology for advanced threat detection', readTime: '11 min'},
  ];
  ...
  // --- Security Scenario Planning Methods (SECA) ---
  private _initSecaScenarios(): void {
    const worstCaseScenarios = [
      { id: "seca-sc-01", name: "Ransomware Attack on Critical Infrastructure", category: "Malware", impact: 9500000, probability: 0.12, recoveryDays: 21, affectedSystems: 47, dataLossGb: 320, businessImpact: "critical" },
      { id: "seca-sc-02", name: "Insider Data Exfiltration via Authorized Channels", category: "Insider Threat", impact: 7200000, probability: 0.18, recoveryDays: 14, affectedSystems: 12, dataLossGb: 85, businessImpact: "high" },
      { id: "seca-sc-03", name: "Supply Chain Compromise via Third-Party Library", category: "Supply Chain", impact: 6800000, probability: 0.09, recoveryDays: 30, affectedSystems: 63, dataLossGb: 0, businessImpact: "critical" },
      { id: "seca-sc-04", name: "Cloud Misconfiguration Leading to Public Exposure", category: "Cloud", impact: 4100000, probability: 0.22, recoveryDays: 7, affectedSystems: 8, dataLossGb: 450, businessImpact: "high" },
      { id: "seca-sc-05", name: "Zero-Day Exploit in Core Application Framework", category: "Vulnerability", impact: 8900000, probability: 0.05, recoveryDays: 45, affectedSystems: 120, dataLossGb: 0, businessImpact: "critical" },
      { id: "seca-sc-06", name: "Distributed Denial of Service on Customer-Facing Services", category: "Availability", impact: 3400000, probability: 0.25, recoveryDays: 3, affectedSystems: 15, dataLossGb: 0, businessImpact: "medium" },
      { id: "seca-sc-07", name: "Credential Stuffing and Account Takeover Wave", category: "Identity", impact: 2800000, probability: 0.30, recoveryDays: 5, affectedSystems: 6, dataLossGb: 12, businessImpact: "medium" },
      { id: "seca-sc-08", name: "Physical Security Breach at Data Center Facility", category: "Physical", impact: 12000000, probability: 0.02, recoveryDays: 60, affectedSystems: 200, dataLossGb: 5000, businessImpact: "critical" },
    ];
    this._secaScenarios = worstCaseScenarios.map(s => ({ ...s, mitigationCost: Math.round(s.impact * 0.08), drillScheduled: false, lastDrillDate: null, drillScore: null, controlGaps: Math.floor(Math.random() * 5) + 1 }));
  }

  private _calculateSecaScenarioRisk(): void {
    this._secaScenarios.forEach(s => {
  ...
  private _scheduleSecaDrills(): void {
  ...
  private _secaScenarioComparison(): Record<string, unknown>[] {
    return this._secaScenarios.map(s => ({
  ...
  // --- Security Control Effectiveness Analytics (SECA) ---
  private _initSecaControls(): void {
    const controls = [
      { id: "seca-ct-01", name: "Network Segmentation", type: "preventive", maturity: "defined", score: 82, target: 90, failures: 3, lastTest: "2026-03-15" },
      { id: "seca-ct-02", name: "Endpoint Detection and Response", type: "detective", maturity: "managed", score: 88, target: 92, failures: 1, lastTest: "2026-04-01" },
      { id: "seca-ct-03", name: "Data Loss Prevention", type: "preventive", maturity: "repeatable", score: 71, target: 85, failures: 7, lastTest: "2026-03-22" },
      { id: "seca-ct-04", name: "Security Information and Event Management", type: "detective", maturity: "managed", score: 85, target: 90, failures: 2, lastTest: "2026-04-05" },
      { id: "seca-ct-05", name: "Identity and Access Management", type: "preventive", maturity: "defined", score: 76, target: 88, failures: 5, lastTest: "2026-03-28" },
      { id: "seca-ct-06", name: "Vulnerability Management", type: "corrective", maturity: "managed", score: 80, target: 90, failures: 4, lastTest: "2026-04-02" },
      { id: "seca-ct-07", name: "Email Security Gateway", type: "preventive", maturity: "managed", score: 90, target: 95, failures: 1, lastTest: "2026-04-08" },
      { id: "seca-ct-08", name: "Web Application Firewall", type: "preventive", maturity: "managed", score: 87, target: 92, failures: 2, lastTest: "2026-03-30" },
      { id: "seca-ct-09", name: "Patch Management", type: "corrective", maturity: "defined", score: 68, target: 85, failures: 9, lastTest: "2026-03-18" },
      { id: "seca-ct-10", name: "Encryption at Rest", type: "preventive", maturity: "managed", score: 93, target: 95, failures: 0, lastTest: "2026-04-10" },
      { id: "seca-ct-11", name: "Encryption in Transit", type: "preventive", maturity: "optimized", score: 96, target: 98, failures: 0, lastTest: "2026-04-12" },
      { id: "seca-ct-12", name: "Privileged Access Management", type: "preventive", maturity: "defined", score: 74, target: 88, failures: 6, lastTest: "2026-03-25" },
      { id: "seca-ct-13", name: "Security Awareness Training", type: "preventive", maturity: "repeatable", score: 65, target: 80, failures: 11, lastTest: "2026-03-20" },
      { id: "seca-ct-14", name: "Incident Response Plan", type: "corrective", maturity: "managed", score: 78, target: 88, failures: 4, lastTest: "2026-04-03" },
      { id: "seca-ct-15", name: "Backup and Recovery", type: "corrective", maturity: "managed", score: 84, target: 92, failures: 3, lastTest: "2026-04-06" },
      { id: "seca-ct-16", name: "Multi-Factor Authentication", type: "preventive", maturity: "managed", score: 91, target: 95, failures: 1, lastTest: "2026-04-09" },
      { id: "seca-ct-17", name: "Network Traffic Analysis", type: "detective", maturity: "defined", score: 72, target: 85, failures: 5, lastTest: "2026-03-27" },
      { id: "seca-ct-18", name: "Cloud Security Posture Management", type: "detective", maturity: "repeatable", score: 69, target: 82, failures: 8, lastTest: "2026-03-24" },
      { id: "seca-ct-19", name: "Container Security Scanning", type: "preventive", maturity: "defined", score: 75, target: 85, failures: 5, lastTest: "2026-04-04" },
      { id: "seca-ct-20", name: "API Security Gateway", type: "preventive", maturity: "repeatable", score: 70, target: 82, failures: 7, lastTest: "2026-03-29" },
      { id: "seca-ct-21", name: "Threat Intelligence Platform", type: "detective", maturity: "managed", score: 83, target: 90, failures: 2, lastTest: "2026-04-07" },
      { id: "seca-ct-22", name: "Database Activity Monitoring", type: "detective", maturity: "defined", score: 67, target: 80, failures: 8, lastTest: "2026-03-21" },
      { id: "seca-ct-23", name: "Deception Technology", type: "detective", maturity: "initial", score: 55, target: 75, failures: 12, lastTest: "2026-03-16" },
      { id: "seca-ct-24", name: "Security Orchestration Automation", type: "corrective", maturity: "repeatable", score: 73, target: 85, failures: 6, lastTest: "2026-04-01" },
      { id: "seca-ct-25", name: "Third-Party Risk Assessment", type: "preventive", maturity: "defined", score: 64, target: 78, failures: 9, lastTest: "2026-03-19" },
    ];
    this._secaControls = controls;
  }

  private _analyzeSecaControlEffectiveness(): void {
    const typeBreakdown: Record<string, { total: number; avgScore: number; avgFailures: number }> = {};
    this._secaControls.forEach(c => {
    ...
    this._secaControlTypeBreakdown = typeBreakdown;
  }

  private _secaControlOptimization(): Record<string, unknown>[] {
    return this._secaControls.filter(c => c.score < c.target).map(c => ({
  ...
  // --- Security Data Pipeline Health (SECA) ---
  private _initSecaPipelines(): void {
    const pipelines = [
      { id: "seca-pl-01", name: "SIEM Log Ingestion", status: "healthy", healthScore: 94, latencyMs: 120, freshnessMin: 2, errorRate: 0.02, throughputMbps: 450, recordsPerSec: 12000, backPressure: 0.05, uptime: 99.97 },
      { id: "seca-pl-02", name: "Threat Feed Aggregation", status: "healthy", healthScore: 91, latencyMs: 300, freshnessMin: 15, errorRate: 0.05, throughputMbps: 85, recordsPerSec: 3200, backPressure: 0.08, uptime: 99.92 },
      { id: "seca-pl-03", name: "Vulnerability Scan Results", status: "degraded", healthScore: 72, latencyMs: 2500, freshnessMin: 60, errorRate: 0.15, throughputMbps: 25, recordsPerSec: 800, backPressure: 0.35, uptime: 98.50 },
      { id: "seca-pl-04", name: "Endpoint Telemetry Stream", status: "healthy", healthScore: 88, latencyMs: 450, freshnessMin: 5, errorRate: 0.08, throughputMbps: 280, recordsPerSec: 8500, backPressure: 0.12, uptime: 99.85 },
      { id: "seca-pl-05", name: "Cloud Audit Log Pipeline", status: "healthy", healthScore: 86, latencyMs: 600, freshnessMin: 10, errorRate: 0.06, throughputMbps: 150, recordsPerSec: 4500, backPressure: 0.10, uptime: 99.80 },
      { id: "seca-pl-06", name: "Identity Event Stream", status: "warning", healthScore: 78, latencyMs: 1200, freshnessMin: 20, errorRate: 0.12, throughputMbps: 45, recordsPerSec: 1500, backPressure: 0.22, uptime: 99.40 },
      { id: "seca-pl-07", name: "Network Flow Collection", status: "healthy", healthScore: 82, latencyMs: 800, freshnessMin: 8, errorRate: 0.09, throughputMbps: 650, recordsPerSec: 18000, backPressure: 0.15, uptime: 99.70 },
      { id: "seca-pl-08", name: "DLP Incident Pipeline", status: "degraded", healthScore: 68, latencyMs: 3500, freshnessMin: 45, errorRate: 0.20, throughputMbps: 18, recordsPerSec: 500, backPressure: 0.42, uptime: 97.80 },
    ];
    this._secaPipelines = pipelines.map(p => ({ ...p, dependencies: [], slaBreached: p.healthScore < 75, alertThreshold: p.errorRate > 0.15 }));
    this._secaPipelines[0].dependencies = ["seca-pl-04", "seca-pl-07"];
    this._secaPipelines[4].dependencies = ["seca-pl-01"];
    this._secaPipelines[5].dependencies = ["seca-pl-02"];
    this._secaPipelines[7].dependencies = ["seca-pl-04", "seca-pl-05"];
  }

  private _secaPipelineTrend(): Record<string, unknown>[] {
    const hours = Array.from({ length: 24 }, (_, i) => `H${String(i).padStart(2, "0")}`);
    return this._secaPipelines.map(p => ({
  ...
  // --- Security Stakeholder Report Generator (SECA) ---
  private _initSecaReportTemplates(): void {
    this._secaReportTemplates = [
      { id: "seca-rp-01", name: "Board Security Report", audience: "board", frequency: "quarterly", sections: 6, autoGenerate: true, lastGenerated: "2026-03-31", pages: 12 },
      { id: "seca-rp-02", name: "Executive Risk Summary", audience: "executive", frequency: "monthly", sections: 8, autoGenerate: true, lastGenerated: "2026-04-01", pages: 8 },
      { id: "seca-rp-03", name: "Technical Security Review", audience: "technical", frequency: "weekly", sections: 12, autoGenerate: true, lastGenerated: "2026-04-21", pages: 25 },
      { id: "seca-rp-04", name: "Audit Compliance Report", audience: "audit", frequency: "quarterly", sections: 10, autoGenerate: false, lastGenerated: "2026-03-15", pages: 35 },
      { id: "seca-rp-05", name: "Regulatory Filing Package", audience: "regulatory", frequency: "annual", sections: 15, autoGenerate: false, lastGenerated: "2025-12-31", pages: 48 },
    ];
  }

  private _generateSecaExecSummary(): string {
    const totalRisk = this._secaScenarios.reduce((sum: number, s: any) => sum + s.riskScore, 0);
    const avgControl = Math.round(this._secaControls.reduce((sum: number, c: any) => sum + c.score, 0) / this._secaControls.length);
    const degradedPipes = this._secaPipelines.filter((p: any) => p.status !== "healthy").length;
    return `Overall risk exposure: ${(totalRisk / 1000000).toFixed(1)}M. Average control effectiveness: {avgControl}%. {degradedPipes} pipeline(s) need attention. {this._secaScenarios.filter((s: any) => s.mitigationPriority === "immediate").length} scenarios require immediate action.`;
  }

  // --- Security Technology Radar (SECA) ---
  private _initSecaTechRadar(): void {
    this._secaTechRadar = [
      { id: "seca-tr-01", name: "AI-Powered Threat Detection", ring: "adopt", category: "Detection", maturity: 4, trend: "up", investment: "high", roi: 3.2, vendorCount: 8 },
      { id: "seca-tr-02", name: "Zero Trust Architecture", ring: "adopt", category: "Architecture", maturity: 4, trend: "up", investment: "high", roi: 2.8, vendorCount: 12 },
      { id: "seca-tr-03", name: "SASE/SSE Platform", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.4, vendorCount: 10 },
      { id: "seca-tr-04", name: "CNAPP Cloud Security", ring: "trial", category: "Cloud", maturity: 3, trend: "up", investment: "medium", roi: 2.1, vendorCount: 7 },
      { id: "seca-tr-05", name: "Extended Detection and Response", ring: "adopt", category: "Detection", maturity: 4, trend: "stable", investment: "high", roi: 2.9, vendorCount: 9 },
      { id: "seca-tr-06", name: "Quantum-Safe Cryptography", ring: "assess", category: "Crypto", maturity: 2, trend: "up", investment: "low", roi: 0.8, vendorCount: 4 },
      { id: "seca-tr-07", name: "Security Service Edge", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.0, vendorCount: 6 },
      { id: "seca-tr-08", name: "SOAR 2.0 Autonomous SOC", ring: "assess", category: "Operations", maturity: 2, trend: "up", investment: "low", roi: 1.2, vendorCount: 5 },
      { id: "seca-tr-09", name: "Software Supply Chain Security", ring: "trial", category: "DevSecOps", maturity: 3, trend: "up", investment: "medium", roi: 2.3, vendorCount: 8 },
      { id: "seca-tr-10", name: "Identity Threat Detection", ring: "adopt", category: "Identity", maturity: 3, trend: "up", investment: "high", roi: 2.7, vendorCount: 7 },
      { id: "seca-tr-11", name: "Data Security Posture Management", ring: "trial", category: "Data", maturity: 3, trend: "up", investment: "medium", roi: 2.2, vendorCount: 9 },
      { id: "seca-tr-12", name: "Decentralized Identity Standards", ring: "hold", category: "Identity", maturity: 1, trend: "stable", investment: "low", roi: 0.5, vendorCount: 3 },
    ];
  }

  private _secaRadarSummary(): Record<string, unknown> {
    const rings: Record<string, number> = { adopt: 0, trial: 0, assess: 0, hold: 0 };
    this._secaTechRadar.forEach(t => { if (rings[t.ring as keyof typeof rings] !== undefined) rings[t.ring as keyof typeof rings]++; });
    return { adopt: rings.adopt, trial: rings.trial, assess: rings.assess, hold: rings.hold, total: this._secaTechRadar.length, avgMaturity: 2.8, topInvestment: this._secaTechRadar.filter(t => t.investment === "high").map(t => t.name) };
  }

  // --- SECA Security Compliance Mapping Matrix ---
  private _initSecaComplianceMatrix(): void {
    ...
    this._secaComplianceMatrix = frameworks.map(fw => ({
  ...
  private _secaComplianceTrend(): Record<string, unknown>[] {
    const quarters = ["Q1-2025", "Q2-2025", "Q3-2025", "Q4-2025", "Q1-2026", "Q2-2026"];
    return this._secaComplianceMatrix.map(fw => ({
  ...
  // --- SECA Threat Intelligence Correlation Engine ---
  private _initSecaThreatIntel(): void {
    ...
    this._secaThreatIntel = threatActors.map((actor, i) => ({
  ...
  private _secaThreatCorrelation(): Record<string, unknown>[] {
    return this._secaThreatIntel.filter(t => t.riskScore > 70).map(t => ({
  ...
  // --- SECA Security Metrics Deep Dive ---
  private _initSecaMetricsDeep(): void {
    ...
    this._secaMetricsDeep = metricCategories.map(cat => ({
  ...
  private _secaMetricsHeatmap(): number[][] {
    return this._secaMetricsDeep.map(cat =>
      cat.metrics.map(m => Math.round((m.value / m.target) * 100))
    );
  }

  // --- SECA Incident Cost Modeling ---
  private _initSecaCostModel(): void {
    const incidentTypes = ["Data Breach", "Ransomware", "DDoS", "Insider Threat", "Phishing", "Supply Chain", "Cloud Misconfig", "Zero-Day"];
    this._secaCostModel = incidentTypes.map(inc => ({
    ...
    this._secaCostModel.forEach(m => { m.totalAnnualExposure = m.avgCost * m.frequency; });
  }

  private _secaCostProjection(): Record<string, unknown>[] {
    return this._secaCostModel.map(m => ({
  ...
  // --- SECA Security Architecture Pattern Library ---
  private _initSecaArchPatterns(): void {
    ...
    this._secaArchPatterns = patterns.map((p, i) => ({
  ...
  private _secaArchPatternAnalysis(): Record<string, unknown> {
    const implemented = this._secaArchPatterns.filter(p => p.implementation > 75).length;
    const avgCoverage = Math.round(this._secaArchPatterns.reduce((s: number, p: any) => s + p.coverage, 0) / this._secaArchPatterns.length);
    return {
      totalPatterns: this._secaArchPatterns.length, fullyImplemented: implemented, avgCoverage,
      avgRiskReduction: Math.round(this._secaArchPatterns.reduce((s: number, p: any) => s + p.riskReduction, 0) / this._secaArchPatterns.length),
      gaps: this._secaArchPatterns.filter(p => p.implementation < 60).map(p => p.pattern),
      recommendations: this._secaArchPatterns.filter(p => p.implementation < 70).map(p => ({ pattern: p.pattern, current: p.implementation, target: 85, effort: p.costComplexity }))
    };
  }
  ...
  private _c24RiskAssessmentWorkflow = [
  ...
  private _c24TreatmentOptions = [
    {id: 'c24-T-001', riskId: 'RSK-042', risk: 'Ransomware Attack on Critical Infrastructure', before: 20, after: 6, strategy: 'Mitigate', controls: ['Next-gen EDR with behavioral detection and automated response', 'Zero-trust network micro-segmentation isolating critical assets', 'Immutable offline backup rotation with quarterly recovery testing', 'Incident response retainer with 1-hour response SLA'], cost: '$450K/yr', roi: '78% reduction vs $2.8M potential loss', timeline: 'Q3 2026', status: 'approved' as const},
  ...
  private _c24TalentPositions = [
    ...
    {id: 'c24-H-002', title: 'Cloud Security Engineer', dept: 'Cloud Security', level: 'Mid-Senior', loc: 'SF/Remote', salary: '$155K-$180K', posted: '2026-03-20', applicants: 45, pipeline: 12, interviews: 6, offers: 2, urgency: 'high' as const, manager: 'Mike Johnson', skills: ['AWS/Azure/GCP', 'Terraform', 'Kubernetes', 'IAM', 'CSPM'], nice: ['CCSP', 'CKS', 'Serverless']},
    {id: 'c24-H-003', title: 'SOC Analyst L2', dept: 'Security Operations', level: 'Mid', loc: 'Austin TX', salary: '$105K-$130K', posted: '2026-04-01', applicants: 67, pipeline: 18, interviews: 8, offers: 0, urgency: 'medium' as const, manager: 'Lisa Park', skills: ['SIEM', 'Incident triage', 'Malware analysis', 'TCP/IP'], nice: ['GCIA', 'SOAR', 'Kill chain']},
    {id: 'c24-H-004', title: 'Staff Security Architect', dept: 'Architecture', level: 'Staff', loc: 'Remote US', salary: '$200K-$240K', posted: '2026-02-28', applicants: 12, pipeline: 3, interviews: 2, offers: 0, urgency: 'critical' as const, manager: 'David Lee', skills: ['Enterprise arch', 'Zero trust', 'Risk frameworks', 'Board comms'], nice: ['CISSP+SABSA', 'Team building', 'Speaker']},
    {id: 'c24-H-005', title: 'GRC Analyst', dept: 'GRC', level: 'Mid', loc: 'NY/Remote', salary: '$110K-$135K', posted: '2026-04-05', applicants: 34, pipeline: 9, interviews: 3, offers: 1, urgency: 'medium' as const, manager: 'Rachel Green', skills: ['ISO 27001', 'SOC 2', 'Risk assessment', 'Audit'], nice: ['CRISC', 'CISA', 'GDPR']},
    {id: 'c24-H-006', title: 'Threat Intel Analyst', dept: 'Threat Intel', level: 'Mid-Senior', loc: 'Remote US', salary: '$140K-$170K', posted: '2026-03-25', applicants: 19, pipeline: 5, interviews: 2, offers: 0, urgency: 'low' as const, manager: 'James Wilson', skills: ['MITRE ATT&CK', 'OSINT', 'Threat modeling', 'Reverse eng'], nice: ['CTIA', 'Nation-state', 'Dark web']},
    {id: 'c24-H-007', title: 'AppSec Engineer', dept: 'AppSec', level: 'Senior', loc: 'Seattle/Remote', salary: '$160K-$190K', posted: '2026-04-10', applicants: 28, pipeline: 7, interviews: 3, offers: 0, urgency: 'medium' as const, manager: 'Emily Zhang', skills: ['SAST/DAST/SCA', 'Secure SDLC', 'Code review', 'CI/CD'], nice: ['CSSLP', 'Mobile security']},
    {id: 'c24-H-008', title: 'IAM Engineer', dept: 'IAM', level: 'Mid', loc: 'Remote US', salary: '$135K-$160K', posted: '2026-04-08', applicants: 31, pipeline: 10, interviews: 5, offers: 1, urgency: 'medium' as const, manager: 'Chris Martinez', skills: ['Okta/Azure AD', 'SAML/OIDC', 'PAM', 'RBAC'], nice: ['CISSP', 'Zero trust IAM']}
  ];

  private _c24InterviewScorecard = [
  ...
  private _c24OnboardingSteps = [
    {id: 'c24-OB-01', item: 'Provision security-hardened laptop, monitors, peripherals, and configured workstation image with all required security tools', cat: 'IT Setup', day: 'Day 1', owner: 'IT Operations'},
    {id: 'c24-OB-02', item: 'Create AD account, email, Slack, VPN, SSO enrollment, and all required application access accounts', cat: 'Access', day: 'Day 1', owner: 'IAM Team'},
    {id: 'c24-OB-03', item: 'Grant security tooling access: SIEM console, EDR dashboard, vuln scanner, pentest platforms, SOAR', cat: 'Tools', day: 'Day 1-2', owner: 'SOC Manager'},
    {id: 'c24-OB-04', item: 'Complete MFA enrollment, security awareness onboarding course, and acceptable use policy acknowledgment', cat: 'Security', day: 'Day 1', owner: 'Awareness Team'},
    {id: 'c24-OB-05', item: 'Sign NDA, employment agreement, IP assignment, and code of conduct documents with HR', cat: 'HR', day: 'Day 1', owner: 'Human Resources'},
    {id: 'c24-OB-06', item: 'Introduction meetings with direct team members and key cross-functional stakeholders', cat: 'Integration', day: 'Week 1', owner: 'Hiring Manager'},
    {id: 'c24-OB-07', item: 'Security architecture overview, tooling walkthrough, and operational runbook review sessions', cat: 'Training', day: 'Week 1-2', owner: 'Security Architect'},
    {id: 'c24-OB-08', item: 'Shadow experienced team member on active incident response and threat hunting investigations', cat: 'Mentoring', day: 'Week 2-3', owner: 'SOC Lead'},
    {id: 'c24-OB-09', item: 'Set up individual certification study plan aligned with role requirements and career goals', cat: 'Development', day: 'Week 2', owner: 'Hiring Manager'},
    {id: 'c24-OB-10', item: 'Establish 30-60-90 day goals, define success metrics, and schedule first performance checkpoint', cat: 'Goals', day: 'Week 1', owner: 'Hiring Manager'}
  ];

  private _c24MetricsDashboard = [
    {id: 'c24-MTD-001', name: 'Mean Time to Detect (MTTD)', value: '4.2 hours', trend: 'improving' as const, target: '< 4 hours', source: 'SIEM - Splunk', method: 'Median time from first alert to analyst acknowledgment', owner: 'SOC Manager', sla: '4 hours', prev: '5.1 hours', spark: [5.8, 5.4, 5.1, 4.9, 4.7, 4.5, 4.3, 4.2], breakdown: [{src: 'Network IDS', pct: '35%', avg: '3.8h'}, {src: 'Endpoint EDR', pct: '28%', avg: '4.1h'}, {src: 'Email Gateway', pct: '22%', avg: '3.5h'}, {src: 'Cloud Security', pct: '10%', avg: '5.2h'}, {src: 'User Reports', pct: '5%', avg: '6.8h'}]},
    {id: 'c24-MTD-002', name: 'Mean Time to Respond (MTTR)', value: '2.8 hours', trend: 'stable' as const, target: '< 3 hours', source: 'SOAR - XSOAR', method: 'Median time from acknowledgment to first containment action', owner: 'IR Lead', sla: '3 hours', prev: '2.7 hours', spark: [3.2, 3.0, 2.9, 2.8, 2.8, 2.9, 2.8, 2.8], breakdown: [{src: 'Auto Playbooks', pct: '42%', avg: '1.2h'}, {src: 'Semi-Auto', pct: '33%', avg: '2.5h'}, {src: 'Manual', pct: '20%', avg: '5.1h'}, {src: 'L3 Escalation', pct: '5%', avg: '8.3h'}]},
    {id: 'c24-MTD-003', name: 'Patch Compliance Rate', value: '97.6%', trend: 'improving' as const, target: '> 95%', source: 'Tenable.io', method: 'Pct of critical/high vulns patched within SLA windows', owner: 'Vuln Manager', sla: '95%', prev: '96.1%', spark: [93.2, 94.1, 94.8, 95.3, 95.9, 96.4, 97.0, 97.6], breakdown: [{src: 'Critical (7d)', pct: '40%', rate: '99.2%'}, {src: 'High (30d)', pct: '35%', rate: '97.8%'}, {src: 'Medium (90d)', pct: '20%', rate: '95.1%'}, {src: 'Low (180d)', pct: '5%', rate: '89.4%'}]},
    {id: 'c24-MTD-004', name: 'Phishing Click Rate', value: '3.2%', trend: 'improving' as const, target: '< 5%', source: 'KnowBe4 + Proofpoint', method: 'Pct of users clicking simulated phishing links monthly', owner: 'Awareness Lead', sla: '5%', prev: '4.1%', spark: [6.8, 6.2, 5.5, 5.1, 4.6, 4.1, 3.6, 3.2], breakdown: [{src: 'Spear Phishing', pct: '35%', rate: '5.1%'}, {src: 'Generic Phishing', pct: '30%', rate: '2.8%'}, {src: 'BEC Sim', pct: '20%', rate: '3.5%'}, {src: 'SMS Phishing', pct: '15%', rate: '1.9%'}]},
    {id: 'c24-MTD-005', name: 'Security Posture Score', value: '784', trend: 'improving' as const, target: '> 750', source: 'SecurityScorecard', method: 'Composite: external 30%, compliance 25%, vuln 25%, config 20%', owner: 'CISO', sla: '> 750', prev: '771', spark: [748, 752, 758, 762, 768, 773, 778, 784], breakdown: [{src: 'External Rating', pct: '30%', score: '791'}, {src: 'Compliance', pct: '25%', score: '812'}, {src: 'Vuln Mgmt', pct: '25%', score: '745'}, {src: 'Config', pct: '20%', score: '769'}]},
    {id: 'c24-MTD-006', name: 'Vulnerability Backlog', value: '142', trend: 'improving' as const, target: '< 150', source: 'Tenable + Qualys', method: 'Total open vulns across all scanned assets tracked weekly', owner: 'Vuln Manager', sla: '< 150', prev: '168', spark: [210, 198, 185, 175, 165, 158, 150, 142], breakdown: [{src: 'Critical', pct: '8%', count: '11'}, {src: 'High', pct: '22%', count: '31'}, {src: 'Medium', pct: '45%', count: '64'}, {src: 'Low', pct: '25%', count: '36'}]}
  ];

  private _c24ArchReview = [
    {id: 'c24-AR-001', cat: 'Network', item: 'Network segmentation between trust zones with dedicated firewalls and granular ACLs', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Micro-segmentation via NSX across all production zones'},
    {id: 'c24-AR-002', cat: 'Network', item: 'DMZ architecture isolating public services from internal network', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Dual-firewall DMZ with WAF and DDoS mitigation'},
    {id: 'c24-AR-003', cat: 'Network', item: 'Least-privilege traffic flows with deny-by-default and quarterly review', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Explicit allow rules with automated drift detection'},
    {id: 'c24-AR-004', cat: 'Network', item: 'Encrypted external communications with IPSec/TLS 1.3 and PFS', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'IKEv2 with PFS, TLS 1.3 for all external APIs'},
    {id: 'c24-AR-005', cat: 'Identity', item: 'Centralized IdP with SSO and automated SCIM provisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Okta SSO SAML 2.0 for 247 apps with SCIM'},
    {id: 'c24-AR-006', cat: 'Identity', item: 'RBAC with quarterly reviews and automated deprovisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Quarterly recertification with JIT admin access'},
    {id: 'c24-AR-007', cat: 'Identity', item: 'PAM controlling all admin and service accounts with session recording', sev: 'high' as const, status: 'fail' as const, reviewer: 'IAM Lead', notes: '3 legacy accounts pending CyberArk enrollment'},
    {id: 'c24-AR-008', cat: 'Identity', item: 'MFA enforced for all users, hardware tokens for privileged access', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'FIDO2 for admins, push MFA for users, 99.8% enrollment'},
    {id: 'c24-AR-009', cat: 'Data', item: 'Data classification enforced with automated labeling on all repos', sev: 'high' as const, status: 'partial' as const, reviewer: 'Data Protection Lead', notes: 'Framework deployed, gaps on 4 legacy file shares'},
    {id: 'c24-AR-010', cat: 'Data', item: 'AES-256 encryption at rest for all sensitive data stores', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'Full-disk, TDE on DBs, SSE-KMS on cloud storage'},
    {id: 'c24-AR-011', cat: 'Data', item: 'DLP monitoring and preventing unauthorized data exfiltration', sev: 'high' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'DLP active on email, endpoint, cloud, and network'},
    {id: 'c24-AR-012', cat: 'Data', item: 'Encrypted off-site backups with quarterly restore testing', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Infrastructure Lead', notes: '3-2-1 strategy with immutable backups'},
    {id: 'c24-AR-013', cat: 'AppSec', item: 'Secure SDLC with SAST, DAST, SCA in all CI/CD pipelines', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: '34 pipelines with security gates and PR blocking'},
    {id: 'c24-AR-014', cat: 'AppSec', item: 'API gateway with auth, rate limiting, schema validation', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: 'Kong with OAuth2, rate limits, OpenAPI validation'},
    {id: 'c24-AR-015', cat: 'AppSec', item: 'Container security with image scanning and runtime protection', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Trivy scanning, Falco runtime in 80% of clusters'},
    {id: 'c24-AR-016', cat: 'Monitoring', item: 'SIEM collecting from all critical systems with correlation rules', sev: 'critical' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'Splunk 98% coverage with 1200+ correlation rules'},
    {id: 'c24-AR-017', cat: 'Monitoring', item: 'EDR with behavioral detection on all managed endpoints', sev: 'high' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'CrowdStrike on 99.2% endpoints with 24/7 MDR'},
    {id: 'c24-AR-018', cat: 'Monitoring', item: 'Executive dashboards with real-time posture visibility', sev: 'medium' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'PowerBI dashboards with weekly exec summaries'},
    {id: 'c24-AR-019', cat: 'Cloud', item: 'CSPM monitoring and remediating cloud misconfigurations', sev: 'high' as const, status: 'pass' as const, reviewer: 'Cloud Security Lead', notes: 'Prisma Cloud 450+ checks with auto-remediation'},
    {id: 'c24-AR-020', cat: 'Cloud', item: 'IaC templates scanned before deployment to production', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Checkov for Terraform, cfn-nag for CloudFormation pending'}
  ];

  private _getc24RiskProgress(): number {
    return Math.round(this._c24RiskAssessmentWorkflow.filter(s => s.status === 'completed').length / this._c24RiskAssessmentWorkflow.length * 100);
  }

  private _getc24TreatmentSummary(): {total: number; approved: number; avgReduction: number; investment: string} {
    const a = this._c24TreatmentOptions.filter(r => r.status === 'approved').length;
    const avg = this._c24TreatmentOptions.reduce((s, r) => s + ((r.before - r.after) / r.before * 100), 0) / this._c24TreatmentOptions.length;
    return {total: this._c24TreatmentOptions.length, approved: a, avgReduction: Math.round(avg), investment: '$1.125M annually'};
  }

  private _getc24RuleSummary(): {total: number; passing: number; failing: number; avgRate: number; violations: number} {
    const p = this._c24ComplianceRules.filter(r => r.result === 'pass').length;
    const f = this._c24ComplianceRules.filter(r => r.result === 'fail').length;
    const avg = this._c24ComplianceRules.reduce((s, r) => s + r.passRate, 0) / this._c24ComplianceRules.length;
    const v = this._c24ComplianceRules.reduce((s, r) => s + r.violations, 0);
    return {total: this._c24ComplianceRules.length, passing: p, failing: f, avgRate: Math.round(avg * 10) / 10, violations: v};
  }

  private _getc24PipelineStats(): {positions: number; applicants: number; interviewing: number; offers: number; critical: number} {
    const t = this._c24TalentPositions.length;
    const a = this._c24TalentPositions.reduce((s, p) => s + p.applicants, 0);
    const i = this._c24TalentPositions.reduce((s, p) => s + p.interviews, 0);
    const o = this._c24TalentPositions.reduce((s, p) => s + p.offers, 0);
    const c = this._c24TalentPositions.filter(x => x.urgency === 'critical').length;
    return {positions: t, applicants: a, interviewing: i, offers: o, critical: c};
  }

  private _getc24MetricTrends(): {improving: number; stable: number; total: number} {
    const imp = this._c24MetricsDashboard.filter(m => m.trend === 'improving').length;
    const stb = this._c24MetricsDashboard.filter(m => m.trend === 'stable').length;
    return {improving: imp, stable: stb, total: this._c24MetricsDashboard.length};
  }

  private _getc24ArchScore(): {total: number; passed: number; failed: number; partial: number; score: number} {
    const p = this._c24ArchReview.filter(c => c.status === 'pass').length;
    const f = this._c24ArchReview.filter(c => c.status === 'fail').length;
    const pa = this._c24ArchReview.filter(c => c.status === 'partial').length;
    return {total: this._c24ArchReview.length, passed: p, failed: f, partial: pa, score: Math.round((p / this._c24ArchReview.length) * 100)};
  }


  private _zc24IncidentScenarios = [
    {id: 'zc24-IS-001', name: 'Ransomware Outbreak in Production Environment', category: 'Malware', likelihood: 'Medium', impact: 'Critical', riskScore: 15, playbooks: ['IR-Ransomware-Isolate', 'IR-Ransomware-Eradicate', 'IR-Ransomway-Recover'], estimatedMTTD: '15min', estimatedMTTR: '4h', involvedTeams: ['SOC', 'IR', 'Infrastructure', 'Legal', 'Comms'], keyIndicators: ['Mass file encryption events', 'Ransom note deployment', 'Lateral movement via SMB/RDP', 'Backup deletion attempts'], mitigationControls: ['EDR behavioral detection', 'Network segmentation', 'Immutable backups', 'Email filtering'], lessonsLearned: 'Ensure offline backups are tested quarterly and Ransomware-specific playbooks are updated with latest threat intelligence', lastDrillDate: '2026-03-15', nextDrillDate: '2026-06-15'},
    {id: 'zc24-IS-002', name: 'Cloud Misconfiguration Leading to Data Exposure', category: 'Cloud', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Cloud-Misconfig-Assess', 'IR-Cloud-Misconfig-Remediate', 'IR-Cloud-Misconfig-Notify'], estimatedMTTD: '2h', estimatedMTTR: '6h', involvedTeams: ['Cloud Security', 'SOC', 'Data Protection', 'Legal'], keyIndicators: ['Public S3 bucket exposure', 'Overly permissive IAM policies', 'Unencrypted database snapshots', 'Public API endpoint discovery'], mitigationControls: ['CSPM monitoring', 'IAM policy analyzer', 'Automated remediation', 'Cloud audit logging'], lessonsLearned: 'Implement CSPM with auto-remediation for common misconfigurations and conduct monthly cloud configuration reviews', lastDrillDate: '2026-02-20', nextDrillDate: '2026-05-20'},
    {id: 'zc24-IS-003', name: 'Insider Threat Data Exfiltration', category: 'Insider', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-Insider-Investigate', 'IR-Insider-Contain', 'IR-Insider-Escalate'], estimatedMTTD: '8h', estimatedMTTR: '24h', involvedTeams: ['SOC', 'IR', 'HR', 'Legal', 'IAM'], keyIndicators: ['Anomalous data access patterns', 'Large file downloads to USB or cloud', 'Off-hours access to sensitive systems', 'Accessing data outside normal job scope'], mitigationControls: ['UEBA analytics', 'DLP policies', 'PAM controls', 'Access reviews'], lessonsLearned: 'Strengthen UEBA baselines and implement automated alerts for data access pattern anomalies', lastDrillDate: '2026-01-10', nextDrillDate: '2026-04-10'},
    {id: 'zc24-IS-004', name: 'Supply Chain Attack via Compromised Vendor Software', category: 'Supply Chain', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-SupplyChain-Detect', 'IR-SupplyChain-Isolate', 'IR-SupplyChain-Remediate'], estimatedMTTD: '24h', estimatedMTTR: '72h', involvedTeams: ['SOC', 'IR', 'Procurement', 'Vendor Mgmt', 'Legal'], keyIndicators: ['Unexpected network connections from vendor software', 'Modified binaries or dependencies', 'Anomalous behavior from trusted applications', 'SBOM discrepancy alerts'], mitigationControls: ['SCA scanning', 'SBOM monitoring', 'Vendor security assessments', 'Network segmentation'], lessonsLearned: 'Require SBOM from all vendors and implement continuous monitoring of third-party component behavior', lastDrillDate: '2026-03-01', nextDrillDate: '2026-06-01'},
    {id: 'zc24-IS-005', name: 'Credential Stuffing Attack on Customer Portal', category: 'External Attack', likelihood: 'High', impact: 'Medium', riskScore: 12, playbooks: ['IR-CredentialStuffing-Detect', 'IR-CredentialStuffing-Block', 'IR-CredentialStuffing-Notify'], estimatedMTTD: '30min', estimatedMTTR: '2h', involvedTeams: ['SOC', 'AppSec', 'Customer Support', 'Comms'], keyIndicators: ['High volume of failed login attempts', 'Login attempts from known compromised credential lists', 'Unusual geographic distribution of login attempts', 'Account takeover indicators'], mitigationControls: ['WAF rate limiting', 'CAPTCHA on login', 'MFA enforcement', 'Credential monitoring service'], lessonsLearned: 'Deploy advanced bot detection on login endpoints and implement proactive credential monitoring for customer accounts', lastDrillDate: '2026-04-01', nextDrillDate: '2026-07-01'},
    {id: 'zc24-IS-006', name: 'Phishing Campaign Targeting Executive Team', category: 'Social Engineering', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Phishing-Contain', 'IR-Phishing-Investigate', 'IR-Phishing-Notify'], estimatedMTTD: '1h', estimatedMTTR: '4h', involvedTeams: ['SOC', 'Awareness', 'Executive Office', 'IT'], keyIndicators: ['Sophisticated spear-phishing emails to C-suite', 'Business email compromise indicators', 'Redirected MFA approval requests', 'Unauthorized wire transfer attempts'], mitigationControls: ['AI email filtering', 'Executive awareness training', 'Out-of-band verification policy', 'DMARC/DKIM/SPF'], lessonsLearned: 'Implement additional email security controls for executive mailboxes and conduct monthly spear-phishing simulations', lastDrillDate: '2026-02-15', nextDrillDate: '2026-05-15'}
  ];

  private _zc24SecurityFrameworks = [
  ...
  private _getzc24ScenarioSummary(): {total: number; critical: number; high: number; drilled: number; avgRiskScore: number} {
    const crit = this._zc24IncidentScenarios.filter(s => s.impact === 'Critical').length;
    const high = this._zc24IncidentScenarios.filter(s => s.impact === 'High').length;
    const avg = this._zc24IncidentScenarios.reduce((s, x) => s + x.riskScore, 0) / this._zc24IncidentScenarios.length;
    return {total: this._zc24IncidentScenarios.length, critical: crit, high: high, drilled: this._zc24IncidentScenarios.length, avgRiskScore: Math.round(avg)};
  }

  private _getzc24VendorRiskSummary(): {total: number; approved: number; conditional: number; remediation: number; avgScore: number} {
    const approved = this._zc24ThirdPartyRiskData.filter(v => v.status === 'approved').length;
    const cond = this._zc24ThirdPartyRiskData.filter(v => v.status === 'conditional').length;
    const rem = this._zc24ThirdPartyRiskData.filter(v => v.status === 'remediation_required').length;
    const avg = this._zc24ThirdPartyRiskData.reduce((s, v) => s + v.riskScore, 0) / this._zc24ThirdPartyRiskData.length;
    return {total: this._zc24ThirdPartyRiskData.length, approved, conditional: cond, remediation: rem, avgScore: Math.round(avg)};
  }

  private _getzc24TrainingSummary(): {total: number; avgCompletion: number; avgScore: number; overdueUpdates: number} {
    const avgComp = this._zc24SecurityTrainingCatalog.reduce((s, t) => s + t.completionRate, 0) / this._zc24SecurityTrainingCatalog.length;
    const avgScore = this._zc24SecurityTrainingCatalog.filter(t => t.avgScore > 0).reduce((s, t) => s + t.avgScore, 0) / this._zc24SecurityTrainingCatalog.filter(t => t.avgScore > 0).length;
    return {total: this._zc24SecurityTrainingCatalog.length, avgCompletion: Math.round(avgComp * 10) / 10, avgScore: Math.round(avgScore), overdueUpdates: 0};
  }

  private _getzc24ToolHealthSummary(): {total: number; healthy: number; degraded: number; annualCost: string} {
    const healthy = this._zc24SecurityToolInventory.filter(t => t.health === 'healthy').length;
    const degraded = this._zc24SecurityToolInventory.filter(t => t.health === 'degraded').length;
    const cost = this._zc24SecurityToolInventory.reduce((s, t) => s + parseInt(t.annualCost.replace(/[^0-9]/g, '')), 0);
    return {total: this._zc24SecurityToolInventory.length, healthy, degraded, annualCost: '$' + (cost / 1000).toFixed(0) + 'K'};
  }


  private _xc24SecurityZones = [
    {zoneId: 'xc24-Z-001', name: 'Internet DMZ', trustLevel: 'Untrusted', subnet: '10.0.0.0/24', segmentation: 'Dual-firewall', firewallRules: 45, monitoring: 'Full packet capture', assets: ['Web servers', 'Load balancers', 'WAF appliances', 'Reverse proxy'], riskLevel: 'high' as const, lastAssessment: '2026-04-15', nextAssessment: '2026-07-15'},
    {zoneId: 'xc24-Z-002', name: 'Corporate Network', trustLevel: 'Trusted', subnet: '10.1.0.0/16', segmentation: 'VLAN + micro-seg', firewallRules: 128, monitoring: 'NetFlow + IDS', assets: ['Workstations', 'Printers', 'Meeting room systems', 'VoIP phones'], riskLevel: 'medium' as const, lastAssessment: '2026-04-10', nextAssessment: '2026-07-10'},
    {zoneId: 'xc24-Z-003', name: 'Production Data Center', trustLevel: 'Restricted', subnet: '10.2.0.0/16', segmentation: 'Full micro-segmentation', firewallRules: 256, monitoring: 'Full packet capture + EDR', assets: ['Application servers', 'Database clusters', 'Message queues', 'API gateways'], riskLevel: 'low' as const, lastAssessment: '2026-04-12', nextAssessment: '2026-07-12'},
    {zoneId: 'xc24-Z-004', name: 'Management Network', trustLevel: 'Restricted', subnet: '10.3.0.0/24', segmentation: 'Isolated VLAN', firewallRules: 32, monitoring: 'Syslog + SNMP', assets: ['Hypervisors', 'Switch management', 'Storage controllers', 'Backup appliances'], riskLevel: 'low' as const, lastAssessment: '2026-04-08', nextAssessment: '2026-07-08'},
    {zoneId: 'xc24-Z-005', name: 'Cloud Production', trustLevel: 'Restricted', subnet: 'VPC: 172.16.0.0/12', segmentation: 'Security groups + NACLs', firewallRules: 189, monitoring: 'VPC Flow Logs + GuardDuty', assets: ['Kubernetes clusters', 'Managed databases', 'Object storage', 'Serverless functions'], riskLevel: 'medium' as const, lastAssessment: '2026-04-18', nextAssessment: '2026-07-18'}
  ];

  private _xc24SecurityAlerts = [
    {id: 'xc24-AL-001', severity: 'critical', title: 'Active ransomware encryption detected on PROD-DB-01', source: 'CrowdStrike', timestamp: '2026-04-22T13:45:00Z', status: 'investigating', assignee: 'SOC Analyst J. Smith', asset: 'PROD-DB-01 (10.2.1.15)', mitreTechnique: 'T1486 - Data Encrypted for Impact', playbook: 'IR-Ransomware-Isolate', sla: '15 minutes', elapsed: '12 minutes', notes: 'EDR detected mass file encryption event pattern. Automated isolation triggered. IR team paged.'},
    {id: 'xc24-AL-002', severity: 'high', title: 'Impossible travel detected for user admin@company.com', source: 'Okta', timestamp: '2026-04-22T12:30:00Z', status: 'triaged', assignee: 'SOC Analyst M. Johnson', asset: 'Okta - admin@company.com', mitreTechnique: 'T1078 - Valid Accounts', playbook: 'IR-ImpossibleTravel', sla: '30 minutes', elapsed: '87 minutes', notes: 'Login from Tokyo followed by login from New York within 30 minutes. Account temporarily locked. User confirmed via out-of-band channel they are in New York.'},
    {id: 'xc24-AL-003', severity: 'high', title: 'Public S3 bucket discovered with customer PII exposure', source: 'Prisma Cloud', timestamp: '2026-04-22T11:15:00Z', status: 'remediating', assignee: 'Cloud Security Lead A. Chen', asset: 'AWS S3: customer-data-archive-2024', mitreTechnique: 'N/A - Misconfiguration', playbook: 'IR-Cloud-Misconfig-Remediate', sla: '1 hour', elapsed: '2.5 hours', notes: 'Bucket containing archived customer records was publicly accessible. Bucket blocked and encrypted. Data exposure assessment in progress.'},
    {id: 'xc24-AL-004', severity: 'medium', title: 'Anomalous data download by user jane.developer@company.com', source: 'Microsoft DLP', timestamp: '2026-04-22T10:00:00Z', status: 'investigating', assignee: 'SOC Analyst R. Davis', asset: 'Endpoint: WS-DEV-042', mitreTechnique: 'T1567 - Exfil Over Web Service', playbook: 'IR-DLP-Investigation', sla: '4 hours', elapsed: '3.5 hours', notes: 'User downloaded 4.2GB of source code files to personal Google Drive. Interview scheduled with user and manager.'},
    {id: 'xc24-AL-005', severity: 'medium', title: 'Credential stuffing attack detected on customer portal', source: 'Cloudflare WAF', timestamp: '2026-04-22T09:30:00Z', status: 'contained', assignee: 'SOC Analyst K. Wilson', asset: 'Customer Portal (portal.company.com)', mitreTechnique: 'T1110 - Brute Force', playbook: 'IR-CredentialStuffing-Block', sla: '2 hours', elapsed: '4 hours', notes: 'Rate limiting and CAPTCHA automatically deployed. 12,000 failed login attempts from 800 unique IPs blocked. No successful account takeovers confirmed.'},
    {id: 'xc24-AL-006', severity: 'low', title: 'SSL certificate expiring in 14 days for api.company.com', source: 'Certificate monitoring', timestamp: '2026-04-22T08:00:00Z', status: 'remediated', assignee: 'Infrastructure Lead T. Brown', asset: 'api.company.com', mitreTechnique: 'N/A', playbook: 'Certificate Renewal', sla: '30 days', elapsed: '16 days', notes: 'Automated renewal triggered via ACME. New certificate deployed successfully. No service interruption.'}
  ];

  private _xc24ComplianceFrameworks = [
  ...
  private _xc24DataClassificationPolicy = [
  ...
  private _getxc24ZoneSummary(): {total: number; highRisk: number; mediumRisk: number; lowRisk: number; totalRules: number} {
    const high = this._xc24SecurityZones.filter(z => z.riskLevel === 'high').length;
    const med = this._xc24SecurityZones.filter(z => z.riskLevel === 'medium').length;
    const low = this._xc24SecurityZones.filter(z => z.riskLevel === 'low').length;
    const rules = this._xc24SecurityZones.reduce((s, z) => s + z.firewallRules, 0);
    return {total: this._xc24SecurityZones.length, highRisk: high, mediumRisk: med, lowRisk: low, totalRules: rules};
  }

  private _getxc24AlertSummary(): {total: number; critical: number; high: number; medium: number; low: number; open: number; contained: number} {
    const crit = this._xc24SecurityAlerts.filter(a => a.severity === 'critical').length;
    const high = this._xc24SecurityAlerts.filter(a => a.severity === 'high').length;
    const med = this._xc24SecurityAlerts.filter(a => a.severity === 'medium').length;
    const low = this._xc24SecurityAlerts.filter(a => a.severity === 'low').length;
    const open = this._xc24SecurityAlerts.filter(a => a.status === 'investigating' || a.status === 'triaged').length;
    const contained = this._xc24SecurityAlerts.filter(a => a.status === 'contained' || a.status === 'remediated').length;
    return {total: this._xc24SecurityAlerts.length, critical: crit, high, medium: med, low, open, contained};
  }

  private _getxc24ComplianceSummary(): {total: number; fullyCompliant: number; mostlyCompliant: number; avgComplianceRate: number} {
    const full = this._xc24ComplianceFrameworks.filter(f => f.status === 'compliant').length;
    const mostly = this._xc24ComplianceFrameworks.filter(f => f.status === 'mostly_compliant').length;
    const avgRate = this._xc24ComplianceFrameworks.reduce((s, f) => s + (f.compliant / f.requirements * 100), 0) / this._xc24ComplianceFrameworks.length;
    return {total: this._xc24ComplianceFrameworks.length, fullyCompliant: full, mostlyCompliant: mostly, avgComplianceRate: Math.round(avgRate * 10) / 10};
  }
  ...
  // === Security Risk Register (severity_cal) ===
  private _severity_calRiskRegister = [
    { id: "RSK-SEV-0001", title: "Data breach from unpatched systems", owner: "CTO", category: "Financial", status: "mitigating", trend: "stable", severity: "low", likelihood: 1, impact: 9, treatment: "Monitor and review quarterly", lastReview: "2026-04-05", nextReview: "2026-07-15" },
    { id: "RSK-SEV-0002", title: "Ransomware attack on critical infrastructure", owner: "CRO", category: "Strategic", status: "closed", trend: "decreasing", severity: "high", likelihood: 9, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-22", nextReview: "2026-07-25" },
    { id: "RSK-SEV-0003", title: "Insider threat data exfiltration", owner: "VP Eng", category: "Compliance", status: "accepted", trend: "increasing", severity: "medium", likelihood: 10, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-21", nextReview: "2026-07-22" },
    { id: "RSK-SEV-0004", title: "Supply chain compromise", owner: "Security Lead", category: "Technical", status: "open", trend: "stable", severity: "medium", likelihood: 9, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-18", nextReview: "2026-07-13" },
    { id: "RSK-SEV-0005", title: "Cloud misconfiguration exposure", owner: "Risk Mgr", category: "Reputational", status: "mitigating", trend: "decreasing", severity: "low", likelihood: 10, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-06", nextReview: "2026-07-18" },
    { id: "RSK-SEV-0006", title: "Phishing campaign success rate", owner: "Compliance Dir", category: "Legal", status: "closed", trend: "increasing", severity: "low", likelihood: 4, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-21", nextReview: "2026-07-09" },
    { id: "RSK-SEV-0007", title: "Third-party vendor data breach", owner: "IT Dir", category: "Regulatory", status: "accepted", trend: "stable", severity: "high", likelihood: 10, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-09", nextReview: "2026-07-12" },
    { id: "RSK-SEV-0008", title: "Regulatory non-compliance penalty", owner: "DevOps Lead", category: "Third-Party", status: "open", trend: "decreasing", severity: "critical", likelihood: 9, impact: 4, treatment: "Monitor and review quarterly", lastReview: "2026-04-19", nextReview: "2026-07-17" },
    { id: "RSK-SEV-0009", title: "Zero-day exploit in production", owner: "Architect", category: "Human Capital", status: "mitigating", trend: "increasing", severity: "critical", likelihood: 7, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-10", nextReview: "2026-07-17" },
    { id: "RSK-SEV-0010", title: "Insufficient access controls", owner: "CISO", category: "Operational", status: "closed", trend: "stable", severity: "high", likelihood: 9, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-07", nextReview: "2026-07-18" },
    { id: "RSK-SEV-0011", title: "DDoS attack on services", owner: "CTO", category: "Financial", status: "accepted", trend: "decreasing", severity: "high", likelihood: 10, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-22", nextReview: "2026-07-18" },
    { id: "RSK-SEV-0012", title: "Social engineering attack", owner: "CRO", category: "Strategic", status: "open", trend: "increasing", severity: "low", likelihood: 10, impact: 5, treatment: "Monitor and review quarterly", lastReview: "2026-04-18", nextReview: "2026-07-28" },
    { id: "RSK-SEV-0013", title: "API security vulnerability", owner: "VP Eng", category: "Compliance", status: "mitigating", trend: "stable", severity: "critical", likelihood: 3, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-22", nextReview: "2026-07-20" },
    { id: "RSK-SEV-0014", title: "Mobile device compromise", owner: "Security Lead", category: "Technical", status: "closed", trend: "decreasing", severity: "high", likelihood: 8, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-02", nextReview: "2026-07-03" },
    { id: "RSK-SEV-0015", title: "Physical security breach", owner: "Risk Mgr", category: "Reputational", status: "accepted", trend: "increasing", severity: "low", likelihood: 4, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-16", nextReview: "2026-07-10" },
    { id: "RSK-SEV-0016", title: "Password policy weakness", owner: "Compliance Dir", category: "Legal", status: "open", trend: "stable", severity: "low", likelihood: 7, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-02", nextReview: "2026-07-15" },
    { id: "RSK-SEV-0017", title: "Network segmentation gap", owner: "IT Dir", category: "Regulatory", status: "mitigating", trend: "decreasing", severity: "high", likelihood: 10, impact: 9, treatment: "Monitor and review quarterly", lastReview: "2026-04-21", nextReview: "2026-07-25" },
    { id: "RSK-SEV-0018", title: "Encryption key management failure", owner: "DevOps Lead", category: "Third-Party", status: "closed", trend: "increasing", severity: "low", likelihood: 8, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-10", nextReview: "2026-07-01" },
    { id: "RSK-SEV-0019", title: "Audit trail tampering", owner: "Architect", category: "Human Capital", status: "accepted", trend: "stable", severity: "high", likelihood: 1, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-06", nextReview: "2026-07-02" },
    { id: "RSK-SEV-0020", title: "Business email compromise", owner: "CISO", category: "Operational", status: "open", trend: "decreasing", severity: "low", likelihood: 1, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-06", nextReview: "2026-07-25" },
  ];
  private _severity_calRiskFilter: string = 'all';
  private _severity_calRiskSeverity: string = 'all';
  private _severity_calRiskStatus: string = 'all';
  private _severity_calExpandedRisk: string = '';
  private _severity_calFilterRisks() {
    const reg = this._severity_calRiskRegister;
    return reg.filter(r => {
      if (this._severity_calRiskFilter !== "all" && r.category !== this._severity_calRiskFilter) return false;
      if (this._severity_calRiskSeverity !== "all" && r.severity !== this._severity_calRiskSeverity) return false;
      if (this._severity_calRiskStatus !== "all" && r.status !== this._severity_calRiskStatus) return false;
      return true;
    });
  }
  private _severity_calGetRiskScore(r: any) { return Math.round(r.likelihood * r.impact * 1.5); }
  private _severity_calGetRiskTrendIcon(trend: string) {
    if (trend === "increasing") return "\u2191";
    if (trend === "decreasing") return "\u2193";
    return "\u2192";
  }
  private _severity_calGetRiskColor(sev: string) {
    if (sev === "critical") return "#dc2626";
    if (sev === "high") return "#ea580c";
    if (sev === "medium") return "#d97706";
    return "#16a34a";
  }
  private _severity_calGetRiskCounts() {
    const reg = this._severity_calRiskRegister;
  ...
  private _severity_calGetTreatmentProgress() {
    const reg = this._severity_calRiskRegister;
    const treated = reg.filter(r => r.status === "mitigating" || r.status === "closed").length;
    return Math.round((treated / Math.max(reg.length, 1)) * 100);
  }

  // === Security Metrics API Gateway (severity_cal) ===
  private _severity_calApiEndpoints = [
    { method: "GET", path: "/api/v1/threats", name: "Threat Intelligence Feed", status: "active", avgLatency: 338.3, reqPerMin: 2458.0, errorRate: 1.41, uptime: round(95.72701247467158,2), version: "v2.8.17" },
    { method: "POST", path: "/api/v1/scans/start", name: "Vulnerability Scanner", status: "active", avgLatency: 259.6, reqPerMin: 444.0, errorRate: 1.9, uptime: round(95.44904927772558,2), version: "v2.4.8" },
    { method: "GET", path: "/api/v1/assets", name: "Asset Inventory", status: "active", avgLatency: 232.8, reqPerMin: 2195.0, errorRate: 1.61, uptime: round(97.97650896103585,2), version: "v2.2.11" },
    { method: "POST", path: "/api/v1/alerts", name: "Alert Management", status: "active", avgLatency: 389.5, reqPerMin: 2038.0, errorRate: 0.33, uptime: round(96.45242721965963,2), version: "v3.6.17" },
    { method: "GET", path: "/api/v1/compliance", name: "Compliance Status", status: "active", avgLatency: 276.3, reqPerMin: 1354.0, errorRate: 1.84, uptime: round(96.47687803562398,2), version: "v1.9.12" },
    { method: "PUT", path: "/api/v1/policies", name: "Policy Engine", status: "active", avgLatency: 339.6, reqPerMin: 588.0, errorRate: 2.21, uptime: round(96.4711530964491,2), version: "v2.4.13" },
    { method: "GET", path: "/api/v1/incidents", name: "Incident Tracker", status: "active", avgLatency: 352.4, reqPerMin: 1878.0, errorRate: 2.28, uptime: round(95.93852732087234,2), version: "v3.7.4" },
    { method: "POST", path: "/api/v1/forensics", name: "Forensics Collector", status: "degraded", avgLatency: 399.0, reqPerMin: 876.0, errorRate: 2.4, uptime: round(98.22115290894624,2), version: "v2.5.2" },
    { method: "GET", path: "/api/v1/risk/assess", name: "Risk Assessment", status: "active", avgLatency: 167.0, reqPerMin: 931.0, errorRate: 2.38, uptime: round(97.40131091073428,2), version: "v2.8.6" },
    { method: "POST", path: "/api/v1/auth/verify", name: "Authentication", status: "active", avgLatency: 175.4, reqPerMin: 2149.0, errorRate: 0.09, uptime: round(99.18495422609267,2), version: "v3.9.11" },
    { method: "GET", path: "/api/v1/logs/audit", name: "Audit Log Query", status: "active", avgLatency: 211.8, reqPerMin: 189.0, errorRate: 1.47, uptime: round(95.01986355014337,2), version: "v2.4.7" },
    { method: "PUT", path: "/api/v1/users/roles", name: "Role Management", status: "active", avgLatency: 112.9, reqPerMin: 859.0, errorRate: 1.06, uptime: round(99.18892787477886,2), version: "v1.8.13" },
    { method: "POST", path: "/api/v1/encrypt", name: "Encryption Service", status: "active", avgLatency: 216.4, reqPerMin: 1156.0, errorRate: 2.25, uptime: round(98.97080009451801,2), version: "v1.1.7" },
    { method: "GET", path: "/api/v1/network/topo", name: "Network Topology", status: "maintenance", avgLatency: 265.5, reqPerMin: 2073.0, errorRate: 1.23, uptime: round(97.31936423883901,2), version: "v3.7.14" },
    { method: "DELETE", path: "/api/v1/sessions", name: "Session Manager", status: "active", avgLatency: 332.6, reqPerMin: 238.0, errorRate: 1.51, uptime: round(95.15805942068195,2), version: "v2.6.0" },
  ];
  private _severity_calApiKeys = [
    { id: "ak-000001", name: "key-seve-001", created: "2026-02-09", lastUsed: "2026-04-15", status: "active", calls: 35451, rateLimit: 100 },
    { id: "ak-000002", name: "key-seve-002", created: "2026-06-17", lastUsed: "2026-04-18", status: "revoked", calls: 4006, rateLimit: 100 },
    { id: "ak-000003", name: "key-seve-003", created: "2026-07-28", lastUsed: "2026-04-16", status: "active", calls: 30532, rateLimit: 5000 },
    { id: "ak-000004", name: "key-seve-004", created: "2026-01-22", lastUsed: "2026-04-17", status: "active", calls: 35595, rateLimit: 5000 },
    { id: "ak-000005", name: "key-seve-005", created: "2026-06-14", lastUsed: "2026-04-23", status: "active", calls: 3193, rateLimit: 100 },
    { id: "ak-000006", name: "key-seve-006", created: "2026-02-11", lastUsed: "2026-04-05", status: "revoked", calls: 34478, rateLimit: 1000 },
    { id: "ak-000007", name: "key-seve-007", created: "2026-04-10", lastUsed: "2026-04-18", status: "revoked", calls: 24534, rateLimit: 1000 },
    { id: "ak-000008", name: "key-seve-008", created: "2026-03-20", lastUsed: "2026-04-21", status: "active", calls: 31733, rateLimit: 100 },
  ];
  private _severity_calApiHealthSummary() {
    const eps = this._severity_calApiEndpoints;
  ...
  private _severity_calGetApiByMethod(method: string) { return this._severity_calApiEndpoints.filter(e=>e.method===method); }
  private _severity_calGetSlowEndpoints() { return this._severity_calApiEndpoints.filter(e=>e.avgLatency>200).sort((a,b)=>b.avgLatency-a.avgLatency); }
  private _severity_calGetHighErrorEndpoints() { return this._severity_calApiEndpoints.filter(e=>e.errorRate>1.0).sort((a,b)=>b.errorRate-a.errorRate); }

  // === Security Training Effectiveness (severity_cal) ===
  private _severity_calTrainingModules = [
    { id: "TRN-001", name: "Security Fundamentals", completionRate: 53.0, avgScore: 65.3, behaviorChange: 87.8, enrolled: 70, completed: 75, duration: "66min", category: "mandatory" },
    { id: "TRN-002", name: "Phishing Awareness", completionRate: 97.0, avgScore: 68.4, behaviorChange: 89.1, enrolled: 402, completed: 396, duration: "32min", category: "mandatory" },
    { id: "TRN-003", name: "Social Engineering Defense", completionRate: 81.0, avgScore: 83.3, behaviorChange: 72.7, enrolled: 438, completed: 178, duration: "105min", category: "optional" },
    { id: "TRN-004", name: "Password Hygiene", completionRate: 91.8, avgScore: 85.7, behaviorChange: 73.2, enrolled: 366, completed: 207, duration: "42min", category: "mandatory" },
    { id: "TRN-005", name: "Data Classification", completionRate: 76.4, avgScore: 73.2, behaviorChange: 36.7, enrolled: 498, completed: 324, duration: "53min", category: "mandatory" },
    { id: "TRN-006", name: "Incident Response Basics", completionRate: 94.3, avgScore: 57.5, behaviorChange: 75.3, enrolled: 493, completed: 393, duration: "49min", category: "mandatory" },
    { id: "TRN-007", name: "Secure Coding", completionRate: 96.6, avgScore: 78.9, behaviorChange: 67.2, enrolled: 251, completed: 93, duration: "21min", category: "optional" },
    { id: "TRN-008", name: "Cloud Security", completionRate: 90.2, avgScore: 94.9, behaviorChange: 47.6, enrolled: 291, completed: 313, duration: "101min", category: "mandatory" },
    { id: "TRN-009", name: "Mobile Device Security", completionRate: 63.1, avgScore: 81.7, behaviorChange: 81.5, enrolled: 303, completed: 431, duration: "63min", category: "mandatory" },
    { id: "TRN-010", name: "Network Security", completionRate: 60.1, avgScore: 91.4, behaviorChange: 46.9, enrolled: 50, completed: 84, duration: "105min", category: "mandatory" },
    { id: "TRN-011", name: "Physical Security", completionRate: 87.6, avgScore: 57.5, behaviorChange: 65.4, enrolled: 487, completed: 295, duration: "57min", category: "optional" },
    { id: "TRN-012", name: "Regulatory Compliance", completionRate: 90.5, avgScore: 56.0, behaviorChange: 82.8, enrolled: 103, completed: 89, duration: "34min", category: "mandatory" },
    { id: "TRN-013", name: "Risk Management", completionRate: 87.3, avgScore: 55.6, behaviorChange: 49.8, enrolled: 469, completed: 21, duration: "62min", category: "mandatory" },
    { id: "TRN-014", name: "Cryptography Basics", completionRate: 83.7, avgScore: 79.8, behaviorChange: 79.9, enrolled: 135, completed: 403, duration: "115min", category: "mandatory" },
    { id: "TRN-015", name: "Access Control", completionRate: 90.2, avgScore: 64.7, behaviorChange: 39.0, enrolled: 81, completed: 25, duration: "31min", category: "optional" },
    { id: "TRN-016", name: "Vendor Management", completionRate: 60.6, avgScore: 62.4, behaviorChange: 75.1, enrolled: 342, completed: 226, duration: "71min", category: "mandatory" },
  ];
  private _severity_calPhishingResults = [
    { month: "2026-01", sent: 243, clicked: 49, reported: 55, clickRate: round(20.16460905349794,1), reportRate: round(22.633744855967077,1) },
    { month: "2026-02", sent: 617, clicked: 92, reported: 159, clickRate: round(14.910858995137763,1), reportRate: round(25.76985413290113,1) },
    { month: "2026-03", sent: 827, clicked: 146, reported: 186, clickRate: round(17.654171704957676,1), reportRate: round(22.49093107617896,1) },
    { month: "2026-04", sent: 499, clicked: 37, reported: 154, clickRate: round(7.414829659318638,1), reportRate: round(30.861723446893784,1) },
    { month: "2026-05", sent: 839, clicked: 210, reported: 294, clickRate: round(25.029797377830754,1), reportRate: round(35.041716328963055,1) },
    { month: "2026-06", sent: 528, clicked: 104, reported: 153, clickRate: round(19.696969696969695,1), reportRate: round(28.97727272727273,1) },
  ];
  private _severity_calTrainingROI = { totalInvestment: 198192, avgCostPerEmployee: 315, riskReductionPct: round(random.uniform(15,45),1), incidentReductionPct: round(random.uniform(10,35),1), complianceScoreGain: round(random.uniform(5,25),1) };
  private _severity_calLearningPaths = [
    { name: "Beginner Security Analyst", totalModules: 8, completedModules: 6, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 52, enrolled: 30 },
    { name: "Advanced Threat Hunter", totalModules: 20, completedModules: 13, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 48, enrolled: 24 },
    { name: "Security Architect", totalModules: 18, completedModules: 5, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 38, enrolled: 49 },
    { name: "Incident Responder", totalModules: 12, completedModules: 4, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 73, enrolled: 123 },
    { name: "Compliance Specialist", totalModules: 9, completedModules: 2, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 63, enrolled: 84 },
    { name: "DevSecOps Engineer", totalModules: 20, completedModules: 17, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 73, enrolled: 127 },
  ];
  private _severity_calGetOverallCompletion() {
    const mods = this._severity_calTrainingModules;
    return round(mods.reduce((s,m)=>s+m.completionRate,0)/mods.length,1);
  }
  private _severity_calGetTopPerformers() { return [...this._severity_calTrainingModules].sort((a,b)=>b.avgScore-a.avgScore).slice(0,5); }
  private _severity_calGetModulesNeedingAttention() { return this._severity_calTrainingModules.filter(m=>m.completionRate<70||m.avgScore<65); }

  // === Security Governance Framework (severity_cal) ===
  private _severity_calGovBodies = [
    { name: "Security Steering Committee", members: 13, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-14", nextMeeting: "2026-05-23", quorum: 7 },
    { name: "Risk Management Board", members: 7, chair: "CTO", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-19", nextMeeting: "2026-05-27", quorum: 4 },
    { name: "Data Governance Council", members: 6, chair: "CRO", meetingFreq: "Monthly", lastMeeting: "2026-04-19", nextMeeting: "2026-05-12", quorum: 4 },
    { name: "Compliance Oversight Board", members: 14, chair: "CDO", meetingFreq: "Monthly", lastMeeting: "2026-04-14", nextMeeting: "2026-05-25", quorum: 7 },
    { name: "Architecture Review Board", members: 9, chair: "VP Eng", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-06", nextMeeting: "2026-05-21", quorum: 6 },
    { name: "Change Advisory Board", members: 10, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-04", nextMeeting: "2026-05-03", quorum: 3 },
    { name: "Incident Review Board", members: 11, chair: "CIO", meetingFreq: "As needed", lastMeeting: "2026-04-12", nextMeeting: "2026-05-14", quorum: 6 },
    { name: "Vendor Risk Committee", members: 6, chair: "CFO", meetingFreq: "Quarterly", lastMeeting: "2026-04-07", nextMeeting: "2026-05-18", quorum: 7 },
  ];
  private _severity_calDecisions = [
    { id: "DEC-0001", title: "Adopt zero-trust architecture framework", date: "2026-03-22", status: "approved", owner: "CISO" },
    { id: "DEC-0002", title: "Migrate to SIEM 2.0 platform", date: "2026-03-08", status: "implemented", owner: "CTO" },
    { id: "DEC-0003", title: "Implement DLP across all endpoints", date: "2026-03-03", status: "in-progress", owner: "Security Lead" },
    { id: "DEC-0004", title: "Mandate MFA for all external access", date: "2026-03-02", status: "pending", owner: "CRO" },
    { id: "DEC-0005", title: "Establish bug bounty program", date: "2026-02-04", status: "approved", owner: "VP Eng" },
    { id: "DEC-0006", title: "Deploy EDR solution enterprise-wide", date: "2026-01-18", status: "approved", owner: "CISO" },
    ...
    { id: "DEC-0009", title: "Establish security champion program", date: "2026-01-08", status: "pending", owner: "CRO" },
    { id: "DEC-0010", title: "Migrate to passwordless authentication", date: "2026-02-25", status: "approved", owner: "VP Eng" },
  ];
  private _severity_calPolicyLifecycle = [
    { name: "Information Security Policy", version: "v4.1", status: "active", lastUpdated: "2026-03-11", nextReview: "2026-11-13", owner: "CISO" },
    { name: "Acceptable Use Policy", version: "v4.4", status: "active", lastUpdated: "2026-04-26", nextReview: "2026-10-17", owner: "Legal" },
    { name: "Data Retention Policy", version: "v1.5", status: "under-review", lastUpdated: "2026-02-23", nextReview: "2026-8-11", owner: "DPO" },
    { name: "Access Control Policy", version: "v1.1", status: "active", lastUpdated: "2026-02-25", nextReview: "2026-7-05", owner: "IAM Lead" },
    { name: "Incident Response Policy", version: "v4.1", status: "draft", lastUpdated: "2026-02-15", nextReview: "2026-8-03", owner: "IR Lead" },
    { name: "Business Continuity Plan", version: "v1.0", status: "active", lastUpdated: "2026-03-06", nextReview: "2026-10-26", owner: "BCP Mgr" },
    { name: "Vendor Management Policy", version: "v3.2", status: "active", lastUpdated: "2026-02-03", nextReview: "2026-12-21", owner: "Procurement" },
    { name: "Encryption Standard", version: "v2.4", status: "under-review", lastUpdated: "2026-01-08", nextReview: "2026-8-07", owner: "Security Arch" },
  ];
  private _severity_calGovMaturityScore = { overall: 2.0, riskManagement: 5.2, compliance: 3.6, incidentResponse: 4.1, awareness: 3.2, technology: 3.0 };
  private _severity_calGetPendingDecisions() { return this._severity_calDecisions.filter(d=>d.status==='pending'||d.status==='in-progress'); }
  private _severity_calGetActivePolicies() { return this._severity_calPolicyLifecycle.filter(p=>p.status==='active'); }
  private _severity_calGetEscalationPath() { return ["L1 Analyst","L2 Senior","Security Lead","CISO","Board"]; }

  // === Security Innovation Lab (severity_cal) ===
  private _severity_calInnoProjects = [
    { id: "INN-001", name: "AI-Powered Threat Detection", description: "Machine learning models for real-time threat identification", status: "active", progress: 72, startDate: "2026-03-17", teamSize: 6, budget: 112376, milestones: 10, completedMilestones: 4 },
    { id: "INN-002", name: "Quantum-Resistant Cryptography", description: "Post-quantum encryption algorithm prototyping", status: "research", progress: 35, startDate: "2026-01-26", teamSize: 2, budget: 63254, milestones: 8, completedMilestones: 4 },
    { id: "INN-003", name: "Automated Red Teaming", description: "Autonomous penetration testing framework", status: "active", progress: 58, startDate: "2026-01-17", teamSize: 7, budget: 175146, milestones: 9, completedMilestones: 9 },
    { id: "INN-004", name: "Zero-Knowledge Authentication", description: "Privacy-preserving identity verification", status: "poc", progress: 88, startDate: "2026-02-05", teamSize: 2, budget: 127621, milestones: 5, completedMilestones: 5 },
    { id: "INN-005", name: "Blockchain Audit Trail", description: "Immutable security event logging", status: "active", progress: 45, startDate: "2026-02-01", teamSize: 4, budget: 162838, milestones: 6, completedMilestones: 2 },
    { id: "INN-006", name: "Behavioral Biometrics", description: "Continuous authentication via user behavior patterns", status: "research", progress: 22, startDate: "2026-03-19", teamSize: 3, budget: 140862, milestones: 9, completedMilestones: 8 },
    { id: "INN-007", name: "Deception Grid 2.0", description: "Advanced honeypot network with adaptive responses", status: "active", progress: 65, startDate: "2026-02-20", teamSize: 7, budget: 114163, milestones: 10, completedMilestones: 9 },
    { id: "INN-008", name: "Secure Enclave Integration", description: "Hardware-backed security for critical workloads", status: "poc", progress: 40, startDate: "2026-01-22", teamSize: 4, budget: 75481, milestones: 10, completedMilestones: 4 },
  ];
  private _severity_calTechEvaluations = [
    { name: "Rust for Security Tools", status: "evaluating", score: 6.6, recommendation: "Adopt", vendor: "Open Source" },
    { name: "eBPF for Runtime Detection", status: "completed", score: 6.2, recommendation: "Adopt", vendor: "AWS" },
  ...
  private _severity_calCollaborationPartners = [
  ...
  private _severity_calInnoMetrics = { totalProjects: 8, activeProjects: 4, avgTimeToValue: "153 days", pocSuccessRate: round(random.uniform(55,85),1), researchToProduction: round(random.uniform(20,50),1), innovationIndex: round(random.uniform(6.0,9.5),1) };
  private _severity_calGetProjectByStatus(status: string) { return this._severity_calInnoProjects.filter(p=>p.status===status); }
  private _severity_calGetTopEvaluations() { return [...this._severity_calTechEvaluations].sort((a,b)=>b.score-a.score).slice(0,3); }



  // === Compliance Dashboard Extension (severity_cal) ===
  private _severity_calComplianceFrameworks = [
    { name: "SOC 2 Type II", description: "Trust Services Criteria", totalControls: 5, implementedControls: 5, status: "compliant", lastAudit: "2026-03-14", nextAudit: "2026-10-10", evidenceCount: 327 },
    { name: "ISO 27001", description: "Information Security Management", totalControls: 114, implementedControls: 61, status: "in-review", lastAudit: "2026-04-12", nextAudit: "2026-10-19", evidenceCount: 476 },
    { name: "PCI DSS 4.0", description: "Payment Card Industry", totalControls: 12, implementedControls: 9, status: "partially-compliant", lastAudit: "2026-02-05", nextAudit: "2026-8-26", evidenceCount: 402 },
    { name: "HIPAA", description: "Health Insurance Portability", totalControls: 18, implementedControls: 13, status: "in-review", lastAudit: "2026-02-14", nextAudit: "2026-10-20", evidenceCount: 200 },
    { name: "GDPR", description: "Data Protection Regulation", totalControls: 99, implementedControls: 66, status: "partially-compliant", lastAudit: "2026-04-05", nextAudit: "2026-12-06", evidenceCount: 125 },
    { name: "NIST CSF 2.0", description: "Cybersecurity Framework", totalControls: 6, implementedControls: 6, status: "in-review", lastAudit: "2026-03-20", nextAudit: "2026-9-28", evidenceCount: 396 },
    { name: "FedRAMP", description: "Federal Risk Authorization", totalControls: 15, implementedControls: 12, status: "in-review", lastAudit: "2026-04-06", nextAudit: "2026-12-17", evidenceCount: 325 },
    { name: "SOX", description: "Sarbanes-Oxley Compliance", totalControls: 8, implementedControls: 6, status: "partially-compliant", lastAudit: "2026-03-23", nextAudit: "2026-12-06", evidenceCount: 104 },
    { name: "CIS Controls v8", description: "Center for Internet Security", totalControls: 18, implementedControls: 14, status: "in-review", lastAudit: "2026-01-19", nextAudit: "2026-9-03", evidenceCount: 394 },
    { name: "COBIT 2019", description: "IT Governance Framework", totalControls: 40, implementedControls: 23, status: "in-review", lastAudit: "2026-01-10", nextAudit: "2026-11-22", evidenceCount: 299 },
  ];
  private _severity_calGetComplianceScore() {
    const fw = this._severity_calComplianceFrameworks;
    return round(fw.reduce((s,f)=>s + (f.implementedControls/Math.max(f.totalControls,1))*100, 0) / fw.length, 1);
  }
  private _severity_calGetGaps() {
    return this._severity_calComplianceFrameworks.filter(f => f.status !== "compliant");
  }
  private _severity_calAuditTrail = [
    ...
    { id: "AUD-0004", action: "Remediation completed", auditor: "Compliance Officer", date: "2026-04-13", result: "pass", findings: 5 },
    ...
    { id: "AUD-0008", action: "Exception approved", auditor: "CISO Office", date: "2026-04-17", result: "conditional", findings: 0 },
    { id: "AUD-0009", action: "Risk accepted", auditor: "Board Audit", date: "2026-04-14", result: "pass", findings: 1 },
    { id: "AUD-0010", action: "Control enhanced", auditor: "Third Party", date: "2026-04-09", result: "pass", findings: 1 },
  ];

  // === Threat Intelligence Feed Extension (severity_cal) ===
  private _severity_calThreatActors = [
    { name: "APT-29", alias: "Cozy Bear", origin: "Russia", type: "Nation-State", severity: "high", lastActivity: "2026-04-06", targets: "Finance", indicators: 60, ttps: 36 },
    { name: "APT-41", alias: "Double Dragon", origin: "China", type: "Nation-State", severity: "critical", lastActivity: "2026-04-05", targets: "Technology", indicators: 392, ttps: 26 },
    { name: "Lazarus Group", alias: "Hidden Cobra", origin: "North Korea", type: "Nation-State", severity: "critical", lastActivity: "2026-04-08", targets: "Government", indicators: 492, ttps: 14 },
    { name: "FIN7", alias: "Carbanak", origin: "Eastern Europe", type: "Financial", severity: "high", lastActivity: "2026-04-19", targets: "Energy", indicators: 394, ttps: 39 },
    { name: "Conti", alias: "Wizard Spider", origin: "Russia", type: "Ransomware", severity: "critical", lastActivity: "2026-04-04", targets: "Healthcare", indicators: 477, ttps: 38 },
    { name: "LockBit", alias: "LockBit Gang", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-14", targets: "Finance", indicators: 442, ttps: 44 },
    { name: "Cl0p", alias: "Cl0p Team", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-13", targets: "Energy", indicators: 272, ttps: 49 },
    { name: "Sandworm", alias: "Unit 74455", origin: "Russia", type: "Nation-State", severity: "critical", lastActivity: "2026-04-05", targets: "Energy", indicators: 414, ttps: 33 },
  ];
  private _severity_calIoCFeed = [
    { id: "ioc-000001", type: "ip", value: "60.158.235.27", confidence: 47, source: "MISP", firstSeen: "2026-04-20", lastSeen: "2026-04-19" },
    { id: "ioc-000002", type: "ip", value: "35.186.138.33", confidence: 56, source: "VirusTotal", firstSeen: "2026-04-23", lastSeen: "2026-04-11" },
    { id: "ioc-000003", type: "ip", value: "73.83.249.1", confidence: 84, source: "STIX", firstSeen: "2026-04-02", lastSeen: "2026-04-04" },
    { id: "ioc-000004", type: "ip", value: "173.32.73.117", confidence: 50, source: "VirusTotal", firstSeen: "2026-04-05", lastSeen: "2026-04-11" },
    { id: "ioc-000005", type: "ip", value: "10.138.116.113", confidence: 93, source: "MISP", firstSeen: "2026-04-16", lastSeen: "2026-04-08" },
    { id: "ioc-000006", type: "ip", value: "142.231.147.179", confidence: 72, source: "STIX", firstSeen: "2026-04-16", lastSeen: "2026-04-18" },
    { id: "ioc-000007", type: "ip", value: "215.246.237.204", confidence: 72, source: "VirusTotal", firstSeen: "2026-04-22", lastSeen: "2026-04-05" },
    { id: "ioc-000008", type: "ip", value: "50.238.47.12", confidence: 97, source: "MISP", firstSeen: "2026-04-05", lastSeen: "2026-04-04" },
    { id: "ioc-000009", type: "ip", value: "192.16.223.199", confidence: 42, source: "VirusTotal", firstSeen: "2026-04-12", lastSeen: "2026-04-02" },
    { id: "ioc-000010", type: "ip", value: "29.17.2.64", confidence: 50, source: "STIX", firstSeen: "2026-04-17", lastSeen: "2026-04-07" },
    { id: "ioc-000011", type: "ip", value: "118.102.246.100", confidence: 45, source: "Mandiant", firstSeen: "2026-04-13", lastSeen: "2026-04-22" },
    { id: "ioc-000012", type: "ip", value: "239.67.252.83", confidence: 63, source: "VirusTotal", firstSeen: "2026-04-02", lastSeen: "2026-04-22" },
    { id: "ioc-000013", type: "ip", value: "81.108.21.159", confidence: 95, source: "VirusTotal", firstSeen: "2026-04-07", lastSeen: "2026-04-09" },
    { id: "ioc-000014", type: "ip", value: "242.55.205.169", confidence: 48, source: "CrowdStrike", firstSeen: "2026-04-02", lastSeen: "2026-04-04" },
    { id: "ioc-000015", type: "ip", value: "196.150.185.164", confidence: 67, source: "MISP", firstSeen: "2026-04-13", lastSeen: "2026-04-02" },
  ];
  private _severity_calGetActiveThreats() { return this._severity_calThreatActors.filter(a => a.severity === 'critical'); }
  private _severity_calGetThreatSummary() {
    const actors = this._severity_calThreatActors;
  ...
  // === Incident Management Extension (severity_cal) ===
  private _severity_calIncidents = [
    { id: "INC-20260001", title: "Unauthorized access detected", severity: "critical", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-01T12:21", affectedAssets: 8, rootCause: "Misconfiguration" },
    { id: "INC-20260002", title: "Malware outbreak on workstation", severity: "high", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-14T22:06", affectedAssets: 50, rootCause: "Credential compromise" },
    { id: "INC-20260003", title: "Data leak from S3 bucket", severity: "medium", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-16T17:25", affectedAssets: 26, rootCause: "Zero-day" },
    { id: "INC-20260004", title: "Phishing campaign targeting finance", severity: "low", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-10T21:54", affectedAssets: 38, rootCause: "Human error" },
    { id: "INC-20260005", title: "DDoS attack on web services", severity: "critical", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-07T23:19", affectedAssets: 39, rootCause: "Policy violation" },
    { id: "INC-20260006", title: "Ransomware encryption attempt", severity: "high", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-23T00:55", affectedAssets: 49, rootCause: "Unknown" },
    { id: "INC-20260007", title: "Insider data exfiltration", severity: "medium", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-17T03:43", affectedAssets: 12, rootCause: "Misconfiguration" },
    { id: "INC-20260008", title: "API key exposure in repo", severity: "low", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-12T12:24", affectedAssets: 29, rootCause: "Credential compromise" },
    { id: "INC-20260009", title: "SQL injection on portal", severity: "critical", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-17T20:59", affectedAssets: 16, rootCause: "Zero-day" },
    { id: "INC-20260010", title: "Brute force on VPN", severity: "high", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-02T14:04", affectedAssets: 18, rootCause: "Human error" },
    { id: "INC-20260011", title: "Supply chain alert from vendor", severity: "medium", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-13T20:36", affectedAssets: 38, rootCause: "Policy violation" },
    { id: "INC-20260012", title: "Suspicious lateral movement", severity: "low", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-22T10:29", affectedAssets: 11, rootCause: "Unknown" },
  ];
  private _severity_calGetIncidentStats() {
    const inc = this._severity_calIncidents;
    return { total: inc.length, open: inc.filter(i=>i.status==="open").length, investigating: inc.filter(i=>i.status==="investigating").length, mttd: 42, mttr: 159 };
  }
  private _severity_calGetSeverityDistribution() {
    const inc = this._severity_calIncidents;
  ...
  render() {    if (this._svcRules.length === 0) { this._initSvcRules(); this._initSvcCvss(); this._runSvcAnomalyDetection(); this._generateSvcPredictions(); this._initSvcApprovals(); this._initSvcActivity(); this._initSvcNotifications(); }
...
<div class="tabs">
        ...
        ${this._activeTab === 'rules' ? html`
          ${this._renderSvcRules()}
          <div style="margin-top:12px;font-size:11px;font-weight:600;margin-bottom:6px">Category Treemap</div>
          <div style="background:#1a1d27;border-radius:8px;padding:12px">${this._renderSvcTreemapSVG()}</div>
          <div style="margin-top:12px;font-size:11px;font-weight:600;margin-bottom:6px">Data Flow (Sankey)</div>
          <div style="background:#1a1d27;border-radius:8px;padding:12px">${this._renderSvcSankeySVG()}</div>
          ${this._renderSvcActivity()}
        ` : nothing}
        ${this._activeTab === 'anomalies' ? html`
          ${this._renderSvcAnomalies()}
          ${this._renderSvcNotifications()}
        ` : nothing}
        ${this._activeTab === 'predictions' ? html`
          ${this._renderSvcPredictions()}
          ...
          <div style="background:#1a1d27;border-radius:8px;padding:12px">
            <table class="history-table"><thead><tr><th>ID</th><th>Base</th><th>Temporal</th><th>Environmental</th><th>Overall</th></tr></thead><tbody>
              ${this._svccvssData.map(c => html`<tr><td>${c.itemId}</td><td style="color:${c.base >= 7 ? '#ef4444' : c.base >= 4 ? '#eab308' : '#22c55e'}">${c.base}</td><td>${c.temporal}</td><td>${c.environmental}</td><td style="font-weight:700;color:${c.overall >= 7 ? '#ef4444' : c.overall >= 4 ? '#eab308' : '#22c55e'}">${c.overall}</td></tr>`)}
            </tbody></table>
          </div>
        ` : nothing}
        ${this._activeTab === 'approvals' ? html`
          ${this._renderSvcApprovals()}
        ` : nothing}
        ${this._activeTab === 'config' ? html`
          ${this._renderSvcConfig()}
        ` : nothing}
        ${this._activeTab === 'new' ? html`
          <div class="form-section">
            <div class="form-title">Create New Severity Calc Entry</div>
            <div class="form-grid">
  ...
  @state() private _soarPlaybooks: Array<{
    ...
    {
      id: 'pb-soar-001', name: 'Severity Calc Auto Response', status: 'active',
      steps: [
    ...
    {
      id: 'pb-soar-002', name: 'Severity Calc Investigation Workflow', status: 'active',
      steps: [
    ...
    {
      id: 'pb-soar-003', name: 'Severity Calc Compliance Scan', status: 'draft',
      steps: [
  ...
  @state() private _severir15Timeline: Array<{id:string;time:string;event:string;severity:string;actor:string}> = [
  ...
  @state() private _severir15RootCauses: Array<{why:string;answer:string}> = [
  ...
  @state() private _severir15ImpactMatrix: Array<{system:string;users:number;data:string;revenue:string;status:string}> = [
  ...
  @state() private _severir15Actions: Array<{id:string;item:string;owner:string;deadline:string;priority:string;status:string}> = [
  ...
  @state() private _severir15Lessons: Array<{id:string;lesson:string;category:string;severity:string;applies_to:string}> = [
  ...
  @state() private _severir15ActiveTab: string = 'timeline';
  @state() private _severir15Benchmarks: Array<{metric:string;current:number;industry:number;target:number;unit:string;source:string}> = [
  ...
  @state() private _severir15MaturityLevels: Array<{domain:string;current:number;target:number;description:string}> = [
  ...
  @state() private _severir15QuarterlyData: Array<{quarter:string;score:number;improvement:number}> = [
  ...
  @state() private _severir15SelectedDomain: string = 'all';
  @state() private _severir15Alerts: Array<{id:string;name:string;severity:number;confidence:number;assetCrit:number;score:number;enriched:boolean;group:string;status:string;enrichData:Array<{key:string;value:string}>}> = [
  ...
  @state() private _severir15QualityMetrics: {fpRate:number;enrichSuccess:number;avgTriageTime:number;enrichedCount:number;totalCount:number} = {fpRate:0.12, enrichSuccess:0.78, avgTriageTime:4.5, enrichedCount:3, totalCount:5};
  @state() private _severir15RoutingRules: Array<{name:string;condition:string;channel:string;active:boolean}> = [
  ...
  @state() private _severir15Shapes: Array<{id:string;type:string;label:string;controls:string[]}> = [
  ...
  @state() private _severir15TrustBoundaries: Array<{from:string;to:string;label:string;strength:string}> = [
  ...
  @state() private _severir15ADRs: Array<{id:string;title:string;status:string;date:string;decision:string}> = [
  ...
  @state() private _severir15SelectedShape: string = '';
  @state() private _severir15Gauges: Array<{name:string;value:number;max:number;unit:string;status:string;color:string}> = [
  ...
  @state() private _severir15Anomalies: Array<{id:string;time:string;description:string;severity:string;acknowledged:boolean}> = [
  ...
  @state() private _severir15Integrations: Array<{name:string;status:string;uptime:number;lastCheck:string;latency:number}> = [
  ...
  @state() private _severir15AlertFatigue: Array<{analyst:string;alertsPerDay:number;escalated:number;dismissed:number;avgResponseMin:number}> = [
  ...
  @state() private _severir15SlaTarget: number = 99.9;
  ...
  private _scRenderInsiderThreat() {
    ...
    return html`<div class="card" style="padding:14px;margin-bottom:8px">
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:4px;margin-bottom:10px;flex-wrap:wrap">
            ${['timeline','rootcause','impact','actions','lessons'].map(t => html`
              <button class="btn ${this._severir15ActiveTab === t ? 'btn-primary' : ''}" style="font-size:10px;padding:3px 8px" @click=${() => { this._severir15ActiveTab = t; }}>${t.charAt(0).toUpperCase() + t.slice(1)}</button>
            `)}
          </div>
          ${this._severir15ActiveTab === 'timeline' ? html`
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._severir15Timeline.map(e => html`
          ...
          ` : this._severir15ActiveTab === 'rootcause' ? html`
            <div style="margin-bottom:8px">${{__html: this._severir15RenderFishbone()}}</div>
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._severir15RootCauses.map((rc, i) => html`
          ...
          ` : this._severir15ActiveTab === 'impact' ? html`
            <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:10px">
              ...
              <tbody>${this._severir15ImpactMatrix.map(imp => html`
          ...
          ` : this._severir15ActiveTab === 'actions' ? html`
            <div style="display:flex;gap:12px;margin-bottom:8px;font-size:10px;color:#888">
              ${Object.entries(this._severir15GetActionStats()).map(([k,v]) => html`<span>${k}: <b style="color:#ddd">${v}</b></span>`)}
            </div>
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._severir15Actions.map(a => html`
                <div style="display:flex;gap:8px;align-items:center;padding:6px;background:#1a1d27;border-radius:4px;cursor:pointer" @click=${() => this._severir15ToggleAction(a.id)}>
          ...
          ` : html`
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._severir15Lessons.map(l => html`
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:4px;margin-bottom:10px;flex-wrap:wrap">
            ${['all', ...this._severir15MaturityLevels.map(m => m.domain)].map(d => html`
              <button class="btn ${this._severir15SelectedDomain === d ? 'btn-primary' : ''}" style="font-size:9px;padding:2px 6px" @click=${() => { this._severir15SelectedDomain = d; }}>${d}</button>
            `)}
          </div>
          <div style="display:flex;gap:16px;margin-bottom:10px">
            <div style="text-align:center"><div style="color:#e0e0e0;font-size:20px;font-weight:bold">${this._severir15GetOverallMaturity()}/5</div><div style="color:#888;font-size:10px">Maturity Level</div></div>
            <div style="text-align:center"><div style="color:#4f4;font-size:20px;font-weight:bold">${this._severir15GetGapAnalysis().filter(g => g.isAbove).length}/${this._severir15Benchmarks.length}</div><div style="color:#888;font-size:10px">Above Industry</div></div>
          </div>
          <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:10px">
            ...
            <tbody>${this._severir15GetGapAnalysis().map(b => html`
          ...
          <div style="margin-top:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Maturity by Domain</div>
            ${this._severir15MaturityLevels.filter(m => this._severir15SelectedDomain === 'all' || m.domain === this._severir15SelectedDomain).map(m => html`
          ...
          <div style="margin-top:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Quarterly Trend</div>
            <div style="display:flex;height:40px;align-items:flex-end;gap:4px">
              ${this._severir15QuarterlyData.map(q => html`<div style="flex:1;text-align:center"><div style="background:#4a9eff;height:${q.score * 0.5}px;border-radius:2px 2px 0 0" title="${q.score}"></div><div style="color:#666;font-size:8px;margin-top:2px">${q.quarter}</div></div>`)}
            </div>
          </div>
        </div>
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:12px;margin-bottom:8px;font-size:10px">
            <div style="display:flex;gap:4px;flex-wrap:wrap">${this._severir15GroupAlerts().map(g => html`<span style="color:#888;padding:2px 6px;background:#1a1d27;border-radius:3px">${g.group}: ${g.count}</span>`)}</div>
          </div>
          <div style="display:flex;gap:8px;margin-bottom:8px;font-size:10px;color:#888">
            <span>FP Rate: <b style="color:${this._severir15QualityMetrics.fpRate > 0.15 ? '#f44' : '#4f4'}">${(this._severir15QualityMetrics.fpRate * 100).toFixed(1)}%</b></span>
            <span>Enrich: <b style="color:#4a9eff">${(this._severir15QualityMetrics.enrichSuccess * 100).toFixed(0)}%</b></span>
            <span>Avg Triage: <b style="color:#ddd">${this._severir15QualityMetrics.avgTriageTime}m</b></span>
          </div>
          <div style="display:flex;flex-direction:column;gap:4px;max-height:200px;overflow-y:auto">
            ${this._severir15Alerts.sort((a, b) => this._severir15CalcScore(b) - this._severir15CalcScore(a)).map(a => {
              const score = this._severir15CalcScore(a);
              return html`
                <div style="padding:6px;background:#1a1d27;border-radius:4px;border-left:3px solid ${a.severity >= 4 ? '#f44' : a.severity >= 3 ? '#fa0' : '#4a9eff'}">
                  <div style="display:flex;justify-content:space-between;align-items:center">
                    ...
                    <button class="btn" style="font-size:9px;padding:1px 6px;margin-left:4px" @click=${() => this._severir15EnrichAlert(a.id)}>${a.enriched ? 'Re-enrich' : 'Enrich'}</button>
                  </div>
          ...
          <div style="margin-top:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Routing Rules</div>
            ${this._severir15RoutingRules.map(r => html`
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap">
            ${this._severir15Shapes.map(s => {
              const v = this._severir15ValidateControl(s.id);
              return html`<div style="padding:4px 8px;background:#1a1d27;border-radius:3px;cursor:pointer;border:1px solid ${this._severir15SelectedShape === s.id ? '#4a9eff' : '#2a2d37'}" @click=${() => { this._severir15SelectedShape = this._severir15SelectedShape === s.id ? '' : s.id; }}>
          ...
          ${this._severir15SelectedShape ? html`
            ${(() => {
              const shape = this._severir15Shapes.find(s => s.id === this._severir15SelectedShape);
              const v = this._severir15ValidateControl(this._severir15SelectedShape);
              return shape ? html`
          ...
          <div style="margin-bottom:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Trust Boundaries</div>
            ${this._severir15TrustBoundaries.map(tb => html`
              <div style="display:flex;gap:6px;align-items:center;padding:3px;background:#1a1d27;border-radius:3px;margin-bottom:2px;font-size:9px">
                <span style="color:#ddd">${this._severir15Shapes.find(s => s.id === tb.from)?.label || tb.from}</span>
                <span style="color:#4a9eff">\u2194</span>
                <span style="color:#ddd">${this._severir15Shapes.find(s => s.id === tb.to)?.label || tb.to}</span>
          ...
          <div><div style="color:#888;font-size:10px;margin-bottom:4px">Architecture Decision Records</div>
            ${this._severir15ADRs.map(adr => html`
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap">
            ${this._severir15Gauges.map(g => html`
          ...
          <div style="display:flex;gap:8px;margin-bottom:8px;align-items:center">
            ...
            <span style="color:${this._severir15GetOverallHealth().score >= 99 ? '#4f4' : '#fa0'};font-size:12px;font-weight:bold">${this._severir15GetOverallHealth().score}%</span>
            <span style="color:#888;font-size:10px">(Target: ${this._severir15SlaTarget}%)</span>
            <span style="color:#4f4;font-size:10px">${this._severir15GetOverallHealth().healthy} healthy</span>
            <span style="color:#fa0;font-size:10px">${this._severir15GetOverallHealth().degraded} degraded</span>
          </div>
          <div style="margin-bottom:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Anomaly Stream</div>
            ${this._severir15Anomalies.slice(0, 3).map(a => html`
              <div style="display:flex;gap:6px;align-items:center;padding:4px;background:#1a1d27;border-radius:3px;margin-bottom:2px;opacity:${a.acknowledged ? 0.5 : 1}">
                ...
                ${!a.acknowledged ? html`<button class="btn" style="font-size:8px;padding:1px 4px" @click=${() => this._severir15AckAnomaly(a.id)}>ACK</button>` : html`<span style="color:#4f4;font-size:9px">ACK'd</span>`}
              </div>
            `)}
          </div>
          <div style="margin-bottom:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Integration Health</div>
            ${this._severir15Integrations.map(i => html`
              <div style="display:flex;gap:6px;align-items:center;padding:3px;background:#1a1d27;border-radius:3px;margin-bottom:2px;font-size:9px">
          ...
          <div><div style="color:#888;font-size:10px;margin-bottom:4px">Alert Fatigue Analysis</div>
            <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:9px">
              ...
              <tbody>${this._severir15AlertFatigue.map(af => html`
  ...
  private _severir15RenderFishbone(): string {
  ...
  private _severir15ToggleAction(id: string) {
    this._severir15Actions = this._severir15Actions.map(a => a.id === id ? {...a, status: a.status === 'pending' ? 'in_progress' : a.status === 'in_progress' ? 'completed' : 'pending'} : a);
  }

  private _severir15GetActionStats() {
    const total = this._severir15Actions.length;
    const done = this._severir15Actions.filter(a => a.status === 'completed').length;
    const inProg = this._severir15Actions.filter(a => a.status === 'in_progress').length;
    return { total, done, inProg, pending: total - done - inProg };
  }

  private _severir15GetOverallMaturity(): number {
    if (this._severir15SelectedDomain === 'all') {
      return Math.round(this._severir15MaturityLevels.reduce((s, m) => s + m.current, 0) / this._severir15MaturityLevels.length * 10) / 10;
    }
    const d = this._severir15MaturityLevels.find(m => m.domain === this._severir15SelectedDomain);
    return d ? d.current : 0;
  }

  private _severir15GetGapAnalysis() {
    return this._severir15Benchmarks.map(b => {
  ...
  private _severir15CalcScore(alert: any): number {
    return Math.round(alert.severity * alert.confidence * alert.assetCrit * 100) / 100;
  }

  private _severir15EnrichAlert(id: string) {
    this._severir15Alerts = this._severir15Alerts.map(a => {
      if (a.id !== id) return a;
      const score = this._severir15CalcScore(a);
  ...
  private _severir15GroupAlerts() {
    const groups: Record<string, number> = {};
    this._severir15Alerts.forEach(a => { groups[a.group] = (groups[a.group] || 0) + 1; });
    return Object.entries(groups).map(([g, c]) => ({group: g, count: c}));
  }

  private _severir15ValidateControl(shapeId: string): {valid:boolean;missing:string[]} {
    const shape = this._severir15Shapes.find(s => s.id === shapeId);
  ...
  private _severir15GetOverallHealth(): {healthy:number;degraded:number;down:number;score:number} {
    const online = this._severir15Integrations.filter(i => i.status === 'online').length;
    const degraded = this._severir15Integrations.filter(i => i.status === 'degraded').length;
    const total = this._severir15Integrations.length;
    return { healthy: online, degraded, down: total - online - degraded, score: Math.round(online / total * 100) };
  }

  private _severir15AckAnomaly(id: string) {
    this._severir15Anomalies = this._severir15Anomalies.map(a => a.id === id ? {...a, acknowledged: true} : a);
  }
...
// ===== SECURITY CONTROL TESTING (R22) =====
  ...
  private _renderBudgetPlanning() {
    const budgetData = [
      { category: "Personnel & Training", planned: 2949000, actual: 4285000, utilization: 145.3, q1: "21%", q2: "26%", q3: "24%", q4: "16%" },
      { category: "Tooling & Licensing", planned: 2956000, actual: 4298000, utilization: 145.4, q1: "24%", q2: "31%", q3: "19%", q4: "17%" },
      { category: "Infrastructure Security", planned: 2963000, actual: 4311000, utilization: 145.5, q1: "27%", q2: "20%", q3: "30%", q4: "18%" },
      { category: "Compliance & Audit", planned: 2970000, actual: 4324000, utilization: 145.6, q1: "30%", q2: "25%", q3: "25%", q4: "19%" },
      { category: "Incident Response", planned: 2977000, actual: 4337000, utilization: 145.7, q1: "17%", q2: "30%", q3: "20%", q4: "20%" },
      { category: "Third-Party Assessments", planned: 2984000, actual: 4350000, utilization: 145.8, q1: "20%", q2: "35%", q3: "31%", q4: "21%" },
      { category: "Security Awareness", planned: 2991000, actual: 4363000, utilization: 145.9, q1: "23%", q2: "24%", q3: "26%", q4: "22%" },
      { category: "Research & Innovation", planned: 2998000, actual: 4376000, utilization: 146.0, q1: "26%", q2: "29%", q3: "21%", q4: "23%" },
    ];
    ...
    const headcount = [
      { team: "SOC Tier 1", current: 18, target: 21, gap: 0, avgSalary: "142k" },
      { team: "SOC Tier 2", current: 10, target: 20, gap: 5, avgSalary: "171k" },
      { team: "Threat Intel", current: 2, target: 19, gap: 4, avgSalary: "89k" },
      { team: "Red Team", current: 11, target: 18, gap: 3, avgSalary: "118k" },
      { team: "GRC", current: 3, target: 17, gap: 2, avgSalary: "147k" },
      { team: "AppSec", current: 12, target: 16, gap: 1, avgSalary: "176k" },
      { team: "Cloud Sec", current: 4, target: 15, gap: 0, avgSalary: "94k" },
      { team: "Identity & Access", current: 13, target: 14, gap: 5, avgSalary: "123k" },
    ];
    const vendorSpend = [
      { vendor: "CrowdStrike", annual: "791k", contractEnd: "2026-07", renewalRisk: "Low", satisfaction: 3 },
      { vendor: "Palo Alto", annual: "71k", contractEnd: "2026-08", renewalRisk: "Medium", satisfaction: 5 },
      { vendor: "Splunk", annual: "102k", contractEnd: "2026-09", renewalRisk: "High", satisfaction: 4 },
      { vendor: "Qualys", annual: "133k", contractEnd: "2026-10", renewalRisk: "Low", satisfaction: 3 },
      { vendor: "Rapid7", annual: "164k", contractEnd: "2026-11", renewalRisk: "Medium", satisfaction: 5 },
      { vendor: "Mandiant", annual: "195k", contractEnd: "2026-12", renewalRisk: "High", satisfaction: 4 },
      { vendor: "Zscaler", annual: "226k", contractEnd: "2026-01", renewalRisk: "Low", satisfaction: 3 },
      { vendor: "Duo Security", annual: "257k", contractEnd: "2026-02", renewalRisk: "Medium", satisfaction: 5 },
    ];
    const roiProjections = [
      { area: "Threat Detection", investment: "127k", projectedReturn: "904k", roiMultiple: "1.7x", confidence: 78 },
      { area: "Incident Reduction", investment: "170k", projectedReturn: "951k", roiMultiple: "1.6x", confidence: 95 },
      { area: "Compliance Savings", investment: "213k", projectedReturn: "998k", roiMultiple: "1.5x", confidence: 76 },
      { area: "Automation Gains", investment: "256k", projectedReturn: "1045k", roiMultiple: "4.0x", confidence: 93 },
      { area: "Risk Avoidance", investment: "299k", projectedReturn: "1092k", roiMultiple: "3.9x", confidence: 74 },
    ];
    return html`
  ...
  private _renderMetricsNormalization() {
    const kpiCatalog = [
      { id: "kpi-1", name: "MTTD", owner: "SOC", unit: "minutes", target: 97, current: 86, benchmark: 94, collection: "auto", frequency: "realtime" },
      { id: "kpi-2", name: "MTTR", owner: "GRC", unit: "%", target: 70, current: 93, benchmark: 71, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-3", name: "MTTC", owner: "AppSec", unit: "score", target: 73, current: 100, benchmark: 82, collection: "manual", frequency: "weekly" },
      { id: "kpi-4", name: "Vuln SLA Compliance", owner: "Cloud Sec", unit: "count", target: 76, current: 61, benchmark: 93, collection: "auto", frequency: "monthly" },
      { id: "kpi-5", name: "Patch Coverage", owner: "Identity", unit: "days", target: 79, current: 68, benchmark: 70, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-6", name: "Phishing Click Rate", owner: "Threat Intel", unit: "minutes", target: 82, current: 75, benchmark: 81, collection: "manual", frequency: "daily" },
      { id: "kpi-7", name: "Training Completion", owner: "Security Ops", unit: "%", target: 85, current: 82, benchmark: 92, collection: "auto", frequency: "weekly" },
      { id: "kpi-8", name: "Escalation Rate", owner: "Risk Mgmt", unit: "score", target: 88, current: 89, benchmark: 69, collection: "semi-auto", frequency: "monthly" },
      { id: "kpi-9", name: "False Positive Rate", owner: "SOC", unit: "count", target: 91, current: 96, benchmark: 80, collection: "manual", frequency: "realtime" },
      { id: "kpi-10", name: "Threat Intel Actionability", owner: "GRC", unit: "days", target: 94, current: 57, benchmark: 91, collection: "auto", frequency: "daily" },
      { id: "kpi-11", name: "Endpoint Compliance", owner: "AppSec", unit: "minutes", target: 97, current: 64, benchmark: 68, collection: "semi-auto", frequency: "weekly" },
      { id: "kpi-12", name: "Cloud Misconfig Score", owner: "Cloud Sec", unit: "%", target: 70, current: 71, benchmark: 79, collection: "manual", frequency: "monthly" },
      { id: "kpi-13", name: "Identity Anomaly Rate", owner: "Identity", unit: "score", target: 73, current: 78, benchmark: 90, collection: "auto", frequency: "realtime" },
      { id: "kpi-14", name: "DLP Events", owner: "Threat Intel", unit: "count", target: 76, current: 85, benchmark: 67, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-15", name: "Vendor Risk Avg", owner: "Security Ops", unit: "days", target: 79, current: 92, benchmark: 78, collection: "manual", frequency: "weekly" },
      { id: "kpi-16", name: "Compliance Audit Pass Rate", owner: "Risk Mgmt", unit: "minutes", target: 82, current: 99, benchmark: 89, collection: "auto", frequency: "monthly" },
      { id: "kpi-17", name: "Awareness Score", owner: "SOC", unit: "%", target: 85, current: 60, benchmark: 66, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-18", name: "SOC Utilization", owner: "GRC", unit: "score", target: 88, current: 67, benchmark: 77, collection: "manual", frequency: "daily" },
      { id: "kpi-19", name: "Automation Coverage", owner: "AppSec", unit: "count", target: 91, current: 74, benchmark: 88, collection: "auto", frequency: "weekly" },
      { id: "kpi-20", name: "Risk Register Currency", owner: "Cloud Sec", unit: "days", target: 94, current: 81, benchmark: 65, collection: "semi-auto", frequency: "monthly" },
    ];
    const benchmarkSources = [
      { source: "NIST CSF", mappedKPIs: 6, alignment: 60, lastReview: "2026-04-10", status: "aligned" },
      { source: "CIS Controls v8", mappedKPIs: 7, alignment: 77, lastReview: "2026-05-05", status: "partial" },
      { source: "ISO 27001:2022", mappedKPIs: 8, alignment: 94, lastReview: "2026-06-28", status: "reviewing" },
      { source: "PCI DSS 4.0", mappedKPIs: 3, alignment: 72, lastReview: "2026-01-23", status: "aligned" },
      { source: "SOC 2 Type II", mappedKPIs: 4, alignment: 89, lastReview: "2026-02-18", status: "partial" },
      { source: "MITRE ATT&CK", mappedKPIs: 5, alignment: 67, lastReview: "2026-03-13", status: "reviewing" },
      { source: "SANS Top 20", mappedKPIs: 6, alignment: 84, lastReview: "2026-04-08", status: "aligned" },
      { source: "OWASP Top 10", mappedKPIs: 7, alignment: 62, lastReview: "2026-05-03", status: "partial" },
    ];
  ...
  private _renderThreatHuntingCampaigns() {
    const campaigns = [
      { id: "HC-1001", name: "Lateral Movement Sweep", status: "active", hypothesis: "H1: Actors using pass-the-hash for lateral movement", leadHunter: "Alice Chen", findings: 37, startDate: "2026-02-02", endDate: null, effectiveness: 53 },
      { id: "HC-1002", name: "Credential Harvesting Hunt", status: "completed", hypothesis: "H2: Actors using web shells for persistence", leadHunter: "Bob Martinez", findings: 40, startDate: "2026-01-13", endDate: "2026-06-19", effectiveness: 72 },
      { id: "HC-1003", name: "Persistence Mechanism Audit", status: "planned", hypothesis: "H3: Actors using scheduled tasks for data theft", leadHunter: "Carol Wu", findings: 43, startDate: "2026-04-24", endDate: null, effectiveness: 91 },
      { id: "HC-1004", name: "C2 Beacon Detection", status: "in-review", hypothesis: "H4: Actors using DNS tunneling for C2 communication", leadHunter: "Dave Kim", findings: 46, startDate: "2026-03-07", endDate: null, effectiveness: 51 },
      { id: "HC-1005", name: "Data Exfiltration Patterns", status: "active", hypothesis: "H5: Actors using encrypted channels for privilege escalation", leadHunter: "Eve Johnson", findings: 1, startDate: "2026-02-18", endDate: null, effectiveness: 70 },
      { id: "HC-1006", name: "Privilege Escalation Scan", status: "completed", hypothesis: "H6: Actors using token impersonation for defense evasion", leadHunter: "Frank Liu", findings: 4, startDate: "2026-01-01", endDate: "2026-04-03", effectiveness: 89 },
      { id: "HC-1007", name: "Supply Chain Implant Hunt", status: "planned", hypothesis: "H7: Actors using poisoned images for initial access", leadHunter: "Grace Park", findings: 7, startDate: "2026-04-12", endDate: null, effectiveness: 49 },
      { id: "HC-1008", name: "Insider Threat Indicators", status: "in-review", hypothesis: "H8: Actors using legitimate tools for credential access", leadHunter: "Hector Silva", findings: 10, startDate: "2026-03-23", endDate: null, effectiveness: 68 },
      { id: "HC-1009", name: "Cloud Metadata Analysis", status: "active", hypothesis: "H9: Actors using API keys for command execution", leadHunter: "Alice Chen", findings: 13, startDate: "2026-02-06", endDate: null, effectiveness: 87 },
      { id: "HC-1010", name: "DNS Tunnel Detection", status: "completed", hypothesis: "H10: Actors using encoded subdomains for exfiltration", leadHunter: "Bob Martinez", findings: 16, startDate: "2026-01-17", endDate: "2026-05-15", effectiveness: 47 },
      { id: "HC-1011", name: "Fileless Malware Search", status: "planned", hypothesis: "H11: Actors using WMI providers for discovery", leadHunter: "Carol Wu", findings: 19, startDate: "2026-04-28", endDate: null, effectiveness: 66 },
      { id: "HC-1012", name: "Zero-Day Exploit Traces", status: "in-review", hypothesis: "H12: Actors using exploit kits for collection", leadHunter: "Dave Kim", findings: 22, startDate: "2026-03-11", endDate: null, effectiveness: 85 },
    ];
    const hunterLeaderboard = [
      { hunter: "Alice Chen", campaigns: 15, findings: 102, highSeverity: 25, avgScore: 76, streak: 6 },
      { hunter: "Bob Martinez", campaigns: 12, findings: 15, highSeverity: 4, avgScore: 69, streak: 7 },
      { hunter: "Carol Wu", campaigns: 9, findings: 44, highSeverity: 9, avgScore: 62, streak: 8 },
      { hunter: "Dave Kim", campaigns: 6, findings: 73, highSeverity: 14, avgScore: 55, streak: 1 },
      { hunter: "Eve Johnson", campaigns: 3, findings: 102, highSeverity: 19, avgScore: 92, streak: 2 },
      { hunter: "Frank Liu", campaigns: 13, findings: 15, highSeverity: 24, avgScore: 85, streak: 3 },
      { hunter: "Grace Park", campaigns: 10, findings: 44, highSeverity: 3, avgScore: 78, streak: 4 },
      { hunter: "Hector Silva", campaigns: 7, findings: 73, highSeverity: 8, avgScore: 71, streak: 5 },
    ];
    const mitreMapping = [
      { tactic: "Initial Access", techniques: 12, campaigns: 2, coverage: 56 },
      { tactic: "Execution", techniques: 11, campaigns: 1, coverage: 36 },
      { tactic: "Persistence", techniques: 10, campaigns: 6, coverage: 87 },
      { tactic: "Privilege Escalation", techniques: 9, campaigns: 5, coverage: 67 },
      { tactic: "Defense Evasion", techniques: 8, campaigns: 4, coverage: 47 },
      { tactic: "Credential Access", techniques: 7, campaigns: 3, coverage: 98 },
      { tactic: "Discovery", techniques: 6, campaigns: 2, coverage: 78 },
      { tactic: "Lateral Movement", techniques: 5, campaigns: 1, coverage: 58 },
      { tactic: "Collection", techniques: 4, campaigns: 6, coverage: 38 },
      { tactic: "Exfiltration", techniques: 3, campaigns: 5, coverage: 89 },
      { tactic: "Command & Control", techniques: 2, campaigns: 4, coverage: 69 },
      { tactic: "Impact", techniques: 12, campaigns: 3, coverage: 49 },
    ];
    return html`
  ...
  private _renderControlInventory() {
    const controls = [
      { id: "CTL-2001", name: "MFA Enforcement", domain: "Access Control", status: "implemented", effectiveness: 50, lastTest: "2026-02-22", nextReview: "2026-10-22", owner: "SOC", risk: "Low" },
      { id: "CTL-2002", name: "Network Segmentation", domain: "Network Security", status: "partial", effectiveness: 30, lastTest: "2026-01-07", nextReview: "2026-11-13", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2003", name: "EDR Deployment", domain: "Endpoint Protection", status: "planned", effectiveness: 37, lastTest: "2026-04-20", nextReview: "2026-12-04", owner: "IT Ops", risk: "High" },
      { id: "CTL-2004", name: "DLP Policy", domain: "Data Protection", status: "gap", effectiveness: 44, lastTest: "2026-03-05", nextReview: "2026-05-23", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2005", name: "SSO Integration", domain: "Identity Management", status: "implemented", effectiveness: 62, lastTest: "2026-02-18", nextReview: "2026-06-14", owner: "IAM", risk: "Low" },
      { id: "CTL-2006", name: "SAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 7, lastTest: "2026-01-03", nextReview: "2026-07-05", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2007", name: "CSPM Scanning", domain: "Cloud Security", status: "planned", effectiveness: 14, lastTest: "2026-04-16", nextReview: "2026-08-24", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2008", name: "Badge Access", domain: "Physical Security", status: "gap", effectiveness: 21, lastTest: "2026-03-01", nextReview: "2026-09-15", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2009", name: "Least Privilege", domain: "Access Control", status: "implemented", effectiveness: 74, lastTest: "2026-02-14", nextReview: "2026-10-06", owner: "SOC", risk: "Low" },
      { id: "CTL-2010", name: "Firewall Rules", domain: "Network Security", status: "partial", effectiveness: 35, lastTest: "2026-01-27", nextReview: "2026-11-25", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2011", name: "Disk Encryption", domain: "Endpoint Protection", status: "planned", effectiveness: 42, lastTest: "2026-04-12", nextReview: "2026-12-16", owner: "IT Ops", risk: "High" },
      { id: "CTL-2012", name: "Data Classification", domain: "Data Protection", status: "gap", effectiveness: 49, lastTest: "2026-03-25", nextReview: "2026-05-07", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2013", name: "PAM Implementation", domain: "Identity Management", status: "implemented", effectiveness: 86, lastTest: "2026-02-10", nextReview: "2026-06-26", owner: "IAM", risk: "Low" },
      { id: "CTL-2014", name: "DAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 12, lastTest: "2026-01-23", nextReview: "2026-07-17", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2015", name: "IAM Policy Review", domain: "Cloud Security", status: "planned", effectiveness: 19, lastTest: "2026-04-08", nextReview: "2026-08-08", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2016", name: "Visitor Management", domain: "Physical Security", status: "gap", effectiveness: 26, lastTest: "2026-03-21", nextReview: "2026-09-27", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2017", name: "Access Reviews", domain: "Access Control", status: "implemented", effectiveness: 98, lastTest: "2026-02-06", nextReview: "2026-10-18", owner: "SOC", risk: "Low" },
      { id: "CTL-2018", name: "IDS/IPS Tuning", domain: "Network Security", status: "partial", effectiveness: 40, lastTest: "2026-01-19", nextReview: "2026-11-09", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2019", name: "Patch Management", domain: "Endpoint Protection", status: "planned", effectiveness: 47, lastTest: "2026-04-04", nextReview: "2026-12-28", owner: "IT Ops", risk: "High" },
      { id: "CTL-2020", name: "Backup Encryption", domain: "Data Protection", status: "gap", effectiveness: 3, lastTest: "2026-03-17", nextReview: "2026-05-19", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2021", name: "Password Policy", domain: "Identity Management", status: "implemented", effectiveness: 51, lastTest: "2026-02-02", nextReview: "2026-06-10", owner: "IAM", risk: "Low" },
      { id: "CTL-2022", name: "Container Scanning", domain: "Application Security", status: "partial", effectiveness: 17, lastTest: "2026-01-15", nextReview: "2026-07-01", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2023", name: "WAF Configuration", domain: "Cloud Security", status: "planned", effectiveness: 24, lastTest: "2026-04-28", nextReview: "2026-08-20", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2024", name: "CCTV Coverage", domain: "Physical Security", status: "gap", effectiveness: 31, lastTest: "2026-03-13", nextReview: "2026-09-11", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2025", name: "RBAC Enforcement", domain: "Access Control", status: "implemented", effectiveness: 63, lastTest: "2026-02-26", nextReview: "2026-10-02", owner: "SOC", risk: "Low" },
      { id: "CTL-2026", name: "VPN Management", domain: "Network Security", status: "partial", effectiveness: 45, lastTest: "2026-01-11", nextReview: "2026-11-21", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2027", name: "App Whitelisting", domain: "Endpoint Protection", status: "planned", effectiveness: 1, lastTest: "2026-04-24", nextReview: "2026-12-12", owner: "IT Ops", risk: "High" },
      { id: "CTL-2028", name: "Key Management", domain: "Data Protection", status: "gap", effectiveness: 8, lastTest: "2026-03-09", nextReview: "2026-05-03", owner: "Data Gov", risk: "Critical" },
    ];
    const gapAnalysis = [
      { gap: "Insufficient MFA coverage for legacy apps", severity: "High", remediationPlan: "Plan R3001", eta: "2026-Q4", estimatedCost: "54k" },
      { gap: "Missing network micro-segmentation", severity: "Medium", remediationPlan: "Plan R3002", eta: "2026-Q3", estimatedCost: "83k" },
      { gap: "Inconsistent EDR deployment", severity: "Medium", remediationPlan: "Plan R3003", eta: "2026-Q2", estimatedCost: "112k" },
      { gap: "DLP not covering cloud storage", severity: "Low", remediationPlan: "Plan R3004", eta: "2026-Q4", estimatedCost: "141k" },
      { gap: "SSO not integrated with all SaaS", severity: "High", remediationPlan: "Plan R3005", eta: "2026-Q3", estimatedCost: "170k" },
    ];
    return html`
  ...
  private _renderIncidentCostTracker() {
    const incidents = [
      { id: "INC-7001", name: "Security Incident 1", severity: "Critical", totalCost: 212000, responseCost: 44520, recoveryCost: 55120, legalCost: 21200, regulatoryCost: 27560, insuranceClaim: 69960, avoidedCost: 218000, date: "2026-02-14" },
      { id: "INC-7002", name: "Security Incident 2", severity: "High", totalCost: 215000, responseCost: 60200, recoveryCost: 79550, legalCost: 15050, regulatoryCost: 17200, insuranceClaim: 0, avoidedCost: 247000, date: "2026-01-23" },
      { id: "INC-7003", name: "Security Incident 3", severity: "Medium", totalCost: 218000, responseCost: 76300, recoveryCost: 58860, legalCost: 43600, regulatoryCost: 30520, insuranceClaim: 172220, avoidedCost: 276000, date: "2026-04-04" },
      { id: "INC-7004", name: "Security Incident 4", severity: "Low", totalCost: 221000, responseCost: 46410, recoveryCost: 83980, legalCost: 37570, regulatoryCost: 19890, insuranceClaim: 0, avoidedCost: 305000, date: "2026-03-13" },
      { id: "INC-7005", name: "Security Incident 5", severity: "Critical", totalCost: 224000, responseCost: 62720, recoveryCost: 62720, legalCost: 31360, regulatoryCost: 33600, insuranceClaim: 165760, avoidedCost: 334000, date: "2026-02-22" },
      { id: "INC-7006", name: "Security Incident 6", severity: "High", totalCost: 227000, responseCost: 79450, recoveryCost: 88530, legalCost: 24970, regulatoryCost: 22700, insuranceClaim: 0, avoidedCost: 363000, date: "2026-01-03" },
      { id: "INC-7007", name: "Security Incident 7", severity: "Medium", totalCost: 230000, responseCost: 48300, recoveryCost: 66700, legalCost: 18400, regulatoryCost: 11500, insuranceClaim: 158700, avoidedCost: 392000, date: "2026-04-12" },
      { id: "INC-7008", name: "Security Incident 8", severity: "Low", totalCost: 233000, responseCost: 65240, recoveryCost: 93200, legalCost: 11650, regulatoryCost: 25630, insuranceClaim: 0, avoidedCost: 421000, date: "2026-03-21" },
      { id: "INC-7009", name: "Security Incident 9", severity: "Critical", totalCost: 236000, responseCost: 82600, recoveryCost: 70800, legalCost: 42480, regulatoryCost: 14160, insuranceClaim: 151040, avoidedCost: 450000, date: "2026-02-02" },
      { id: "INC-7010", name: "Security Incident 10", severity: "High", totalCost: 239000, responseCost: 50190, recoveryCost: 47800, legalCost: 35850, regulatoryCost: 28680, insuranceClaim: 0, avoidedCost: 479000, date: "2026-01-11" },
      { id: "INC-7011", name: "Security Incident 11", severity: "Medium", totalCost: 242000, responseCost: 67760, recoveryCost: 75020, legalCost: 29040, regulatoryCost: 16940, insuranceClaim: 142780, avoidedCost: 12000, date: "2026-04-20" },
      { id: "INC-7012", name: "Security Incident 12", severity: "Low", totalCost: 245000, responseCost: 85750, recoveryCost: 51450, legalCost: 22050, regulatoryCost: 31850, insuranceClaim: 0, avoidedCost: 41000, date: "2026-03-01" },
    ];
    const yearlyTrend = [
      { month: "Jan", incidents: 10, totalCost: "284k", avgCost: "149k", insured: 23 },
      { month: "Feb", incidents: 7, totalCost: "327k", avgCost: "196k", insured: 23 },
      { month: "Mar", incidents: 4, totalCost: "370k", avgCost: "62k", insured: 23 },
      { month: "Apr", incidents: 12, totalCost: "413k", avgCost: "109k", insured: 23 },
      { month: "May", incidents: 9, totalCost: "456k", avgCost: "156k", insured: 23 },
      { month: "Jun", incidents: 6, totalCost: "499k", avgCost: "22k", insured: 23 },
    ];
...
declare global { interface HTMLElementTagNameMap { 'sc-severity-calc': ScSeverityCalc; } }
