...
export class ScSocMetrics extends LitElement {
  ...
  // --- SOCMetrics Risk Scoring Engine ---
  @state() private _socmriskScore = 0;
  @state() private _socmriskLevel = 'low';
  private _socmriskFactors: Record<string, { weight: number; label: string }> = {
  ...
  private _computeSOCMetricsRisk(): { score: number; level: string; factors: { name: string; score: number; label: string }[] } {
    const factors: { name: string; score: number; label: string }[] = Object.entries(this._socmriskFactors).map(([name, f]) => ({
      name, score: Math.floor(Math.random() * 60) + 20, label: f.label,
    }));
    const score = Math.round(factors.reduce((s, f) => s + f.score * (this._socmriskFactors[f.name]?.weight || 0.15), 0));
    const level = score > 75 ? 'critical' : score > 50 ? 'high' : score > 25 ? 'medium' : 'low';
    this._socmriskScore = score;
    this._socmriskLevel = level;
    return { score, level, factors };
  }

  // --- MITRE ATT&CK Correlation ---
  private _socmmitreMap: Record<string, { techniqueId: string; techniqueName: string; tactic: string }> = {
  ...
  private _correlateSOCMetricsMitre(): { tactic: string; techniques: { id: string; name: string; count: number }[] }[] {
    const tacticMap: Record<string, { id: string; name: string; count: number }[]> = {};
    for (const [key, mitre] of Object.entries(this._socmmitreMap)) {
  ...
  // --- Radar Chart SVG ---
  private _socmradarSVG(): string {
    const dims = Object.values(this._socmriskFactors).map(f => f.label);
  ...
  // --- Heatmap SVG ---
  private _socmheatmapSVG(): string {
    const w = 500, h = 180;
  ...
  // --- Collaboration ---
  @state() private _socmteam: { id: string; name: string; role: string; status: string }[] = [
    { id: 'socm1', name: 'Team Lead', role: 'Management', status: 'online' },
    { id: 'socm2', name: 'Analyst', role: 'Operations', status: 'online' },
    { id: 'socm3', name: 'Engineer', role: 'Technical', status: 'busy' },
  ];
  @state() private _socmcomments: { id: string; userId: string; text: string; timestamp: string }[] = [];
  @state() private _socmcommentText = '';

  private _addSOCMetricsComment() {
    if (!this._socmcommentText.trim()) return;
    this._socmcomments = [{ id: 'socmc' + Date.now(), userId: 'You', text: this._socmcommentText.trim(), timestamp: new Date().toISOString() }, ...this._socmcomments].slice(0, 30);
    this._socmcommentText = '';
  }

  private _renderSOCMetricsCollab(): any {
    return html`
      <div style="margin-top:16px;background:#1a1d27;border-radius:8px;padding:12px">
        ...
        ${
          this._socmcomments.length > 0 ? html`
            <div style="max-height:60px;overflow-y:auto;margin-bottom:6px">
              ${
                this._socmcomments.slice(0, 5).map(c => html`<div style="font-size:10px;padding:3px 0;border-bottom:1px solid #1f2937"><span style="font-weight:600;color:#e2e8f0">${c.userId}</span><span style="color:#9ca3af">: ${c.text}</span></div>`)
              }
            </div>
          ` : nothing
        }
        <div style="display:flex;gap:6px">
          <input style="flex:1;background:#0f172a;border:1px solid #374151;border-radius:4px;padding:4px 8px;color:#e2e8f0;font-size:10px" placeholder="Comment..." .value=${this._socmcommentText} @input=${(e: any) => this._socmcommentText = e.target.value}>
          <button style="background:#f59e0b;color:#111827;border:none;border-radius:4px;padding:4px 10px;font-size:10px;font-weight:600;cursor:pointer" @click=${this._addSOCMetricsComment}>Post</button>
        </div>
      </div>
    `;
  }
  ...
  // --- Auto-Insights ---
  private _generateSOCMetricsInsights(): { icon: string; text: string; severity: string }[] {
    const insights: { icon: string; text: string; severity: string }[] = [];
    const risk = this._computeSOCMetricsRisk();
    if (risk.score > 75) insights.push({ icon: '\u26A0\uFE0F', text: 'SOCMetrics risk score is ' + risk.score + '/100. Immediate attention required.', severity: 'critical' });
    else if (risk.score > 50) insights.push({ icon: '\uD83D\uDD04', text: 'SOCMetrics risk is elevated at ' + risk.score + '/100.', severity: 'high' });
    return insights.length > 0 ? insights : [{ icon: '\u2705', text: 'SOCMetrics status is normal.', severity: 'low' }];
  }

  private _renderSOCMetricsInsights(): any {
    const insights = this._generateSOCMetricsInsights();
    return html`
  ...
  // --- Panel Config ---
  @state() private _socmconfig: { showRadar: boolean; showHeatmap: boolean; showCollab: boolean; autoRefresh: boolean } = { showRadar: true, showHeatmap: true, showCollab: true, autoRefresh: false };

  private _renderSOCMetricsConfig(): any {
    return html`
      <div style="margin-top:12px;background:#1a1d27;border-radius:8px;padding:12px">
        <div style="font-weight:700;font-size:12px;margin-bottom:8px">Configuration</div>
        ${
          Object.entries(this._socmconfig).map(([key, val]) => html`
            <div style="display:flex;justify-content:space-between;align-items:center;padding:4px 0;border-bottom:1px solid #1f2937">
              ...
              <div style="width:32px;height:18px;border-radius:9px;background:${val ? '#22c55e' : '#374151'};cursor:pointer;position:relative" @click=${() => { this._socmconfig = { ...this._socmconfig, [key]: !val }; }}>
  ...
  // --- Anomaly Detection ---
  private _detectSOCMetricsAnomalies(): { type: string; description: string; severity: string }[] {
    const anomalies: { type: string; description: string; severity: string }[] = [];
    const risk = this._computeSOCMetricsRisk();
    if (risk.score > 80) anomalies.push({ type: 'Critical Risk', description: 'SOCMetrics risk exceeds 80 threshold.', severity: 'critical' });
    return anomalies;
  }

  private _renderSOCMetricsAnomalies(): any {
    const anomalies = this._detectSOCMetricsAnomalies();
  ...
  // --- Trend Prediction ---
  private _predictSOCMetricsTrend(): { direction: string; confidence: number; reason: string } {
    const risk = this._computeSOCMetricsRisk();
  ...
  private _renderSOCMetricsTrend(): any {
    const trend = this._predictSOCMetricsTrend();
    return html`
      <div style="margin-top:12px;background:#1a1d27;border-radius:8px;padding:12px">
  ...
  // --- Compliance ---
  private _socmcomplianceRules: { rule: string; standard: string; status: 'pass' | 'fail' | 'warning' }[] = [
  ...
  private _renderSOCMetricsCompliance(): any {
    return html`
      <div style="margin-top:12px;background:#1a1d27;border-radius:8px;padding:12px">
        <div style="font-weight:700;font-size:12px;margin-bottom:8px">Compliance</div>
        ${
          this._socmcomplianceRules.map(r => html`
...
`;
  ...
  private _items: PanelItem[] = [
      {
            "id": "soc-metrics-1",
            "title": "Soc Metrics Finding #1",
            "severity": "low",
            "status": "open",
            "category": "Security",
            "source": "SIEM",
            "description": "Automated soc metrics assessment finding detected during routine scan of infrastructure component. Review and remediation recommended.",
  ...
  private _history: HistoryEntry[] = [
    { timestamp: '2026-04-23 04:15', action: 'Auto-correlated', user: 'system', details: 'Grouped related findings from Soc Metrics analysis' },
    { timestamp: '2026-04-23 03:00', action: 'Created', user: 'scanner', details: 'New Soc Metrics finding detected by automated assessment' },
    ...
    { timestamp: '2026-04-22 22:00', action: 'Resolved', user: 'security-eng', details: 'Remediation applied and verified for Soc Metrics finding' },
    { timestamp: '2026-04-22 18:00', action: 'Created', user: 'audit', details: 'Compliance audit identified Soc Metrics gap requiring remediation' },
    ...
    { timestamp: '2026-04-21 16:00', action: 'Resolved', user: 'soc-tier1', details: 'False positive confirmed - benign activity flagged by Soc Metrics scan' },
    { timestamp: '2026-04-21 12:00', action: 'Exported', user: 'compliance', details: 'Exported Soc Metrics findings for Q1 compliance report' },
  ];
  ...
  private _exportJSON() {
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'sc-soc-metrics-data.json'; a.click();
  ...
  private _exportCSV() {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'sc-soc-metrics-data.csv'; a.click();
    URL.revokeObjectURL(url);
  }


  // --- Domain Rules Engine ---
  @state() private _socRules: { id: string; name: string; category: string; severity: Severity; enabled: boolean; lastEval: string; passRate: number }[] = [];
  private _initSocRules() {
    const rules = [
    ...
    this._socRules = rules;
  }
  private _evaluateSocRules(): { passed: number; failed: number; skipped: number; total: number } {
    let passed = 0, failed = 0, skipped = 0;
    this._socRules.forEach(r => { if (!r.enabled) { skipped++; } else if (r.passRate >= 80) { passed++; } else { failed++; } });
    return { passed, failed, skipped, total: this._socRules.length };
  }

  // --- CVSS Scoring ---
  @state() private _soccvssData: { itemId: string; vector: string; base: number; temporal: number; environmental: number; overall: number }[] = [];
  private _initSocCvss() {
    ...
    this._soccvssData = vectors.map((v, i) => {
  ...
  // --- Anomaly Detection ---
  @state() private _socanomalies: { id: string; type: string; severity: Severity; description: string; detected: string; confidence: number; affected: string[] }[] = [];
  private _runSocAnomalyDetection() {
    const types = [
    ...
    this._socanomalies = types.map((a, i) => ({
  ...
  // --- Trend Prediction ---
  @state() private _socpredictions: { horizon: string; metric: string; current: number; predicted: number; direction: 'up' | 'down' | 'stable'; confidence: number }[] = [];
  private _generateSocPredictions() {
    this._socpredictions = [
  ...
  // --- Approval Workflow ---
  @state() private _socApprovals: { id: string; title: string; requester: string; status: 'pending' | 'approved' | 'rejected' | 'expired'; createdAt: string; priority: Priority; type: string }[] = [];
  private _initSocApprovals() {
    this._socApprovals = [
  ...
  private _approveSocItem(id: string) { const item = this._socApprovals.find(a => a.id === id); if (item) item.status = 'approved'; this.requestUpdate(); }
  private _rejectSocItem(id: string) { const item = this._socApprovals.find(a => a.id === id); if (item) item.status = 'rejected'; this.requestUpdate(); }

  // --- Activity Feed ---
  @state() private _socActivity: { id: string; action: string; user: string; target: string; timestamp: string }[] = [];
  private _initSocActivity() {
    const actions = [
    ...
    this._socActivity = actions.map((a, i) => ({ id: 'ACT-' + String(i + 1).padStart(3, '0'), ...a, timestamp: new Date(Date.now() - i * 3600000).toISOString() }));
  }

  // --- Notification System ---
  @state() private _socNotifications: { id: string; message: string; type: 'info' | 'warning' | 'error' | 'success'; read: boolean; timestamp: string }[] = [];
  private _initSocNotifications() {
    this._socNotifications = [
  ...
  private _markSocNotifRead(id: string) { const n = this._socNotifications.find(x => x.id === id); if (n) n.read = true; this.requestUpdate(); }

  // --- Panel Configuration ---
  @state() private _socConfig: { layout: 'compact' | 'default' | 'expanded'; theme: 'dark' | 'midnight' | 'slate'; showAnomalies: boolean; showPredictions: boolean; autoRefresh: boolean; refreshInterval: number } = {
    layout: 'default', theme: 'dark', showAnomalies: true, showPredictions: true, autoRefresh: true, refreshInterval: 60,
  };
  private _socPresets: { name: string; config: typeof this._socConfig }[] = [
  ...
  private _applySocPreset(preset: typeof this._socPresets[0]) { this._socConfig = { ...preset.config }; this.requestUpdate(); }

  private _renderSocTreemapSVG(): string {
    const categories = [
  ...
  private _renderSocSankeySVG(): string {
  ...
  // --- Render: Rules Engine ---
  private _renderSocRules(): any {
    const ev = this._evaluateSocRules();
    return html`
  ...
  private _renderSocAnomalies(): any {
  ...
  // --- Render: Approvals ---
  private _renderSocApprovals(): any {
            ...
            <div style="padding:6px 8px;background:#1f2937;border-radius:4px">
              ...
              $${a.status === 'pending' ? html`
                <div style="display:flex;gap:4px;margin-top:4px">
                  <button class="btn success" style="padding:2px 8px;font-size:9px" @click=$${() => this._approveSocItem(a.id)}>Approve</button>
                  <button class="btn error" style="padding:2px 8px;font-size:9px" @click=$${() => this._rejectSocItem(a.id)}>Reject</button>
                </div>` : nothing}
            </div>`)}
        </div>
      </div>`;
  ...
  private _renderSocNotifications(): any {
    const tc = (t: string) => t === 'error' ? '#ef4444' : t === 'warning' ? '#eab308' : t === 'success' ? '#22c55e' : '#3b82f6';
    const unread = this._socNotifications.filter(n => !n.read).length;
    return html`
      <div style="background:#1a1d27;border-radius:8px;padding:12px;margin-top:12px">
        ...
        <div style="display:flex;flex-direction:column;gap:4px;max-height:120px;overflow-y:auto">
          $${this._socNotifications.map(n => html`
            <div style="display:flex;align-items:center;gap:8px;padding:4px 8px;background:$${n.read ? '#1f2937' : '#252a36'};border-radius:4px;font-size:10px;opacity:$${n.read ? '0.6' : '1'};cursor:pointer" @click=$${() => this._markSocNotifRead(n.id)}>
      ...
      <div style="background:#1a1d27;border-radius:8px;padding:12px;margin-top:12px">
        ...
        <div style="display:flex;flex-direction:column;gap:6px;font-size:10px">
          <div style="display:flex;align-items:center;gap:8px">
            ...
            <select class="form-input" style="flex:1" @change=$${(e: Event) => { this._socConfig.layout = (e.target as HTMLSelectElement).value as any; this.requestUpdate(); }}>
              <option value="compact" ?selected=$${this._socConfig.layout === 'compact'}>Compact</option>
              <option value="default" ?selected=$${this._socConfig.layout === 'default'}>Default</option>
              <option value="expanded" ?selected=$${this._socConfig.layout === 'expanded'}>Expanded</option>
            </select>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:100px">Theme</span>
            <select class="form-input" style="flex:1" @change=$${(e: Event) => { this._socConfig.theme = (e.target as HTMLSelectElement).value as any; this.requestUpdate(); }}>
              <option value="dark" ?selected=$${this._socConfig.theme === 'dark'}>Dark</option>
              <option value="midnight" ?selected=$${this._socConfig.theme === 'midnight'}>Midnight</option>
              <option value="slate" ?selected=$${this._socConfig.theme === 'slate'}>Slate</option>
            </select>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:100px">Auto Refresh</span>
            <input type="checkbox" ?checked=$${this._socConfig.autoRefresh} @change=$${() => { this._socConfig.autoRefresh = !this._socConfig.autoRefresh; this.requestUpdate(); }}/>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:100px">Show Anomalies</span>
            <input type="checkbox" ?checked=$${this._socConfig.showAnomalies} @change=$${() => { this._socConfig.showAnomalies = !this._socConfig.showAnomalies; this.requestUpdate(); }}/>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:100px">Show Predictions</span>
            <input type="checkbox" ?checked=$${this._socConfig.showPredictions} @change=$${() => { this._socConfig.showPredictions = !this._socConfig.showPredictions; this.requestUpdate(); }}/>
          </div>
          <div style="margin-top:6px;font-weight:600">Presets</div>
          <div style="display:flex;gap:4px">
            $${this._socPresets.map(ps => html`<button class="btn" style="padding:2px 8px;font-size:9px" @click=$${() => this._applySocPreset(ps)}>$${ps.name}</button>`)}
          </div>
        </div>
      </div>`;
  }
  ...
  @state() private _socMetricsCorrRules: Array<{id:string;name:string;sources:string[];logic:string;severity:string;active:boolean;lastTriggered:string}> = [
  ...
  @state() private _socMetricsEventTimeline: Array<{timestamp:string;source:string;eventType:string;details:string;correlated:boolean}> = [
  ...
  @state() private _socMetricsFalsePosMetrics: {totalEvents:number;correlatedEvents:number;falsePositives:number;fpRate:number;topFpRules:string[]} = {
  ...
  @state() private _socMetricsEventPatterns: Array<{id:string;pattern:string;frequency:number;firstSeen:string;lastSeen:string;status:string}> = [
  ...
  private _renderSocmetricsEventCorr(): TemplateResult {
    const rules = this._socMetricsCorrRules;
    const timeline = this._socMetricsEventTimeline;
    const fpMetrics = this._socMetricsFalsePosMetrics;
    return html`
  ...
  @state() private _socMetricsCourses: Array<{id:string;title:string;category:string;duration:number;enrolled:number;completed:number;difficulty:string;rating:number}> = [
  ...
  @state() private _socMetricsLearningPaths: Array<{id:string;name:string;courseIds:string[];progress:number;enrolled:number}> = [
  ...
  @state() private _socMetricsDeptCompliance: Array<{dept:string;trainedPct:number;targetPct:number;avgScore:number;certCount:number}> = [
  ...
  @state() private _socMetricsSkillsGaps: Array<{skill:string;current:number;required:number;gap:number;priority:string}> = [
  ...
  private _renderSocmetricsTraining(): TemplateResult {
    const courses = this._socMetricsCourses;
    const deptComp = this._socMetricsDeptCompliance;
    return html`
  ...
  @state() private _socMetricsZones: Array<{id:string;name:string;trustLevel:number;subnet:string;devices:number;policy:string;lastAudit:string}> = [
  ...
  @state() private _socMetricsSegRules: Array<{id:string;source:string;dest:string;action:string;protocol:string;port:string;status:string;hits:number}> = [
  ...
  @state() private _socMetricsCrossZoneTraffic: Array<{source:string;dest:string;bytes:number;sessions:number;violations:number}> = [
  ...
  @state() private _socMetricsMicroSegGaps: Array<{id:string;zone:string;gapType:string;severity:string;recommendation:string}> = [
  ...
  private _renderSocmetricsNetworkSeg(): TemplateResult {
    const zones = this._socMetricsZones;
    const gaps = this._socMetricsMicroSegGaps;
    return html`
  ...
  @state() private _socMetricsContainerScans: Array<{image:string;registry:string;critical:number;high:number;medium:number;scanDate:string;status:string}> = [
  ...
  @state() private _socMetricsK8sPods: Array<{namespace:string;pod:string;securityContext:string;hostIPC:boolean;hostPID:boolean;privileged:boolean;riskLevel:string}> = [
  ...
  @state() private _socMetricsServerlessRisk: Array<{function:string;runtime:string;timeout:number;iamPerms:string;externalCalls:number;riskScore:number}> = [
  ...
  @state() private _socMetricsRuntimeAlerts: Array<{id:string;workload:string;alertType:string;severity:string;description:string;timestamp:string}> = [
  ...
  private _renderSocmetricsCloudWl(): TemplateResult {
    const containers = this._socMetricsContainerScans;
    const pods = this._socMetricsK8sPods;
    const alerts = this._socMetricsRuntimeAlerts;
    return html`
  ...
  @state() private _complianceCalendarEvents: Array<{id: string; title: string; deadline: string; category: string; status: string; assignee: string; notes: string}> = [
    {id: 'cc-soc-metrics-001', title: 'SOX Section 404 Compliance Review', deadline: '2026-06-30', category: 'Regulatory', status: 'In Progress', assignee: 'Compliance Team', notes: 'Annual internal control assessment for financial reporting'},
    {id: 'cc-soc-metrics-002', title: 'GDPR Data Protection Impact Assessment', deadline: '2026-05-15', category: 'Privacy', status: 'Pending', assignee: 'DPO Office', notes: 'Quarterly DPIA for high-risk processing activities'},
    {id: 'cc-soc-metrics-003', title: 'ISO 27001 Surveillance Audit', deadline: '2026-07-20', category: 'Certification', status: 'Scheduled', assignee: 'QA Lead', notes: 'Stage 2 surveillance audit for ISMS certification renewal'},
    {id: 'cc-soc-metrics-004', title: 'PCI DSS v4.0 Gap Analysis', deadline: '2026-05-30', category: 'Payment', status: 'Not Started', assignee: 'Security Architect', notes: 'Assess current state against PCI DSS v4.0 requirements'},
    {id: 'cc-soc-metrics-005', title: 'NIST CSF Self-Assessment', deadline: '2026-08-15', category: 'Framework', status: 'Pending', assignee: 'CISO', notes: 'Biannual self-assessment against NIST Cybersecurity Framework'},
    {id: 'cc-soc-metrics-006', title: 'HIPAA Security Rule Audit', deadline: '2026-06-15', category: 'Healthcare', status: 'In Progress', assignee: 'Compliance Analyst', notes: 'Annual audit of administrative, physical, and technical safeguards'},
    {id: 'cc-soc-metrics-007', title: 'SOC 2 Type II Report Renewal', deadline: '2026-09-30', category: 'Audit', status: 'Scheduled', assignee: 'External Auditor', notes: 'Engage auditor for SOC 2 Type II examination period'},
    {id: 'cc-soc-metrics-008', title: 'FedRAMP Authorization Review', deadline: '2026-07-01', category: 'Government', status: 'Pending', assignee: 'FedRAMP PMO', notes: 'Review and update security authorization package'},
    {id: 'cc-soc-metrics-009', title: 'Annual Penetration Test', deadline: '2026-10-15', category: 'Testing', status: 'Not Started', assignee: 'Red Team Lead', notes: 'Comprehensive external and internal penetration testing engagement'},
    {id: 'cc-soc-metrics-010', title: 'Security Awareness Training Rollout', deadline: '2026-05-01', category: 'Training', status: 'Completed', assignee: 'Security Awareness Manager', notes: 'Q2 organization-wide security awareness training program'},
    {id: 'cc-soc-metrics-011', title: 'Vendor Security Review Cycle', deadline: '2026-06-01', category: 'Third Party', status: 'In Progress', assignee: 'Vendor Manager', notes: 'Quarterly review of critical vendor security posture and SLA compliance'},
    {id: 'cc-soc-metrics-012', title: 'Incident Response Plan Update', deadline: '2026-05-20', category: 'Operations', status: 'Pending', assignee: 'IR Team Lead', notes: 'Update IR procedures based on lessons learned from recent incidents'},
  ];
  ...
  @state() private _dataQualityMetrics: Array<{id: string; dataSource: string; completenessScore: number; accuracyScore: number; timelinessScore: number; consistencyScore: number; reliabilityRank: number; lastUpdated: string; issues: string[]; trend: string}> = [
    {id: 'dq-soc-metrics-001', dataSource: 'Vulnerability Scanner Feed', completenessScore: 94, accuracyScore: 91, timelinessScore: 88, consistencyScore: 95, reliabilityRank: 1, lastUpdated: '2026-04-23T08:00:00Z', issues: ['Minor delay in Nessus plugin updates'], trend: 'improving'},
    {id: 'dq-soc-metrics-002', dataSource: 'SIEM Event Logs', completenessScore: 87, accuracyScore: 93, timelinessScore: 96, consistencyScore: 89, reliabilityRank: 2, lastUpdated: '2026-04-23T07:30:00Z', issues: ['Some log sources showing gaps in off-hours'], trend: 'stable'},
    {id: 'dq-soc-metrics-003', dataSource: 'Asset Inventory Database', completenessScore: 82, accuracyScore: 78, timelinessScore: 75, consistencyScore: 84, reliabilityRank: 4, lastUpdated: '2026-04-22T18:00:00Z', issues: ['Shadow IT devices not fully cataloged', 'Decommissioned assets still listed'], trend: 'declining'},
    {id: 'dq-soc-metrics-004', dataSource: 'Threat Intelligence Platform', completenessScore: 91, accuracyScore: 89, timelinessScore: 92, consistencyScore: 87, reliabilityRank: 3, lastUpdated: '2026-04-23T06:00:00Z', issues: ['Some IOC sources lack confidence scoring'], trend: 'improving'},
    {id: 'dq-soc-metrics-005', dataSource: 'Identity Provider Logs', completenessScore: 96, accuracyScore: 97, timelinessScore: 98, consistencyScore: 96, reliabilityRank: 1, lastUpdated: '2026-04-23T09:00:00Z', issues: [], trend: 'stable'},
    {id: 'dq-soc-metrics-006', dataSource: 'Cloud Security Posture Data', completenessScore: 79, accuracyScore: 85, timelinessScore: 82, consistencyScore: 81, reliabilityRank: 5, lastUpdated: '2026-04-23T05:00:00Z', issues: ['Multi-cloud coverage gaps', 'API rate limiting affects scan frequency'], trend: 'declining'},
    {id: 'dq-soc-metrics-007', dataSource: 'Compliance Evidence Store', completenessScore: 88, accuracyScore: 86, timelinessScore: 80, consistencyScore: 83, reliabilityRank: 4, lastUpdated: '2026-04-21T14:00:00Z', issues: ['Manual evidence collection delays', 'Inconsistent formatting across frameworks'], trend: 'stable'},
    {id: 'dq-soc-metrics-008', dataSource: 'Network Flow Data', completenessScore: 92, accuracyScore: 90, timelinessScore: 94, consistencyScore: 91, reliabilityRank: 2, lastUpdated: '2026-04-23T08:30:00Z', issues: ['Encrypted traffic classification accuracy needs improvement'], trend: 'improving'},
  ];
  ...
  @state() private _workflowTemplates: Array<{id: string; name: string; description: string; category: string; steps: number; avgDuration: string; successRate: number; lastRun: string; manualInterventions: number; optimizationScore: number}> = [
    {id: 'wf-soc-metrics-001', name: 'Vulnerability Triage Pipeline', description: 'Automated vulnerability assessment, prioritization, and assignment workflow', category: 'Vulnerability Management', steps: 8, avgDuration: '12 min', successRate: 94.5, lastRun: '2026-04-23T06:00:00Z', manualInterventions: 2, optimizationScore: 87},
    {id: 'wf-soc-metrics-002', name: 'Incident Response Orchestration', description: 'End-to-end IR workflow from detection to containment and recovery', category: 'Incident Response', steps: 15, avgDuration: '45 min', successRate: 89.2, lastRun: '2026-04-22T14:30:00Z', manualInterventions: 5, optimizationScore: 72},
    {id: 'wf-soc-metrics-003', name: 'Access Request Approval Chain', description: 'Multi-level approval workflow for privileged access requests', category: 'Identity & Access', steps: 6, avgDuration: '4 hours', successRate: 97.1, lastRun: '2026-04-23T09:15:00Z', manualInterventions: 1, optimizationScore: 92},
    {id: 'wf-soc-metrics-004', name: 'Compliance Evidence Collection', description: 'Automated gathering and packaging of audit evidence artifacts', category: 'Compliance', steps: 10, avgDuration: '30 min', successRate: 91.8, lastRun: '2026-04-21T10:00:00Z', manualInterventions: 3, optimizationScore: 78},
    {id: 'wf-soc-metrics-005', name: 'Security Patch Deployment', description: 'Staged patch rollout with validation and rollback capability', category: 'Patch Management', steps: 12, avgDuration: '2 hours', successRate: 96.3, lastRun: '2026-04-20T22:00:00Z', manualInterventions: 1, optimizationScore: 85},
    {id: 'wf-soc-metrics-006', name: 'Threat Intelligence Enrichment', description: 'IOC enrichment and correlation with internal threat data', category: 'Threat Intelligence', steps: 7, avgDuration: '8 min', successRate: 93.7, lastRun: '2026-04-23T07:00:00Z', manualInterventions: 0, optimizationScore: 95},
    {id: 'wf-soc-metrics-007', name: 'Security Reporting Pipeline', description: 'Automated generation and distribution of security metrics reports', category: 'Reporting', steps: 5, avgDuration: '15 min', successRate: 98.9, lastRun: '2026-04-23T08:00:00Z', manualInterventions: 0, optimizationScore: 97},
    {id: 'wf-soc-metrics-008', name: 'Vendor Risk Assessment', description: 'Automated vendor security questionnaire distribution and scoring', category: 'Third Party', steps: 9, avgDuration: '3 days', successRate: 85.4, lastRun: '2026-04-19T11:00:00Z', manualInterventions: 4, optimizationScore: 65},
  ];
  ...
  @state() private _workflowExecutionHistory: Array<{workflowId: string; runDate: string; duration: string; status: string; trigger: string}> = [
    {workflowId: 'wf-soc-metrics-001', runDate: '2026-04-23T06:00:00Z', duration: '11 min 23 sec', status: 'Success', trigger: 'Scheduled'},
    {workflowId: 'wf-soc-metrics-002', runDate: '2026-04-22T14:30:00Z', duration: '42 min 15 sec', status: 'Success', trigger: 'Alert'},
    {workflowId: 'wf-soc-metrics-003', runDate: '2026-04-23T09:15:00Z', duration: '3h 45 min', status: 'Success', trigger: 'User Request'},
    {workflowId: 'wf-soc-metrics-001', runDate: '2026-04-22T06:00:00Z', duration: '13 min 08 sec', status: 'Partial', trigger: 'Scheduled'},
    {workflowId: 'wf-soc-metrics-004', runDate: '2026-04-21T10:00:00Z', duration: '28 min 42 sec', status: 'Success', trigger: 'Scheduled'},
  ];
  ...
  @state() private _knowledgeArticles: Array<{id: string; title: string; category: string; author: string; publishDate: string; views: number; rating: number; tags: string[]; summary: string; readTime: string}> = [
    {id: 'kb-soc-metrics-001', title: 'Zero Trust Architecture Implementation Guide', category: 'Architecture', author: 'Security Architecture Team', publishDate: '2026-03-15', views: 1842, rating: 4.8, tags: ['zero-trust', 'network', 'identity'], summary: 'Comprehensive guide for implementing zero trust principles across enterprise infrastructure', readTime: '15 min'},
    {id: 'kb-soc-metrics-002', title: 'Ransomware Defense Playbook 2026', category: 'Threats', author: 'Threat Intelligence Team', publishDate: '2026-04-01', views: 2341, rating: 4.9, tags: ['ransomware', 'incident-response', 'defense'], summary: 'Updated playbook with latest ransomware tactics, techniques, and defensive measures', readTime: '12 min'},
    {id: 'kb-soc-metrics-003', title: 'Cloud Security Best Practices for Multi-Cloud', category: 'Cloud', author: 'Cloud Security Team', publishDate: '2026-02-28', views: 1567, rating: 4.5, tags: ['cloud', 'aws', 'azure', 'gcp'], summary: 'Best practices for securing workloads across multiple cloud service providers', readTime: '18 min'},
    {id: 'kb-soc-metrics-004', title: 'API Security Testing Methodology', category: 'Testing', author: 'Red Team', publishDate: '2026-03-22', views: 987, rating: 4.6, tags: ['api', 'testing', 'owasp'], summary: 'Structured methodology for identifying and testing API security vulnerabilities', readTime: '10 min'},
    {id: 'kb-soc-metrics-005', title: 'Supply Chain Security Risk Management', category: 'Third Party', author: 'GRC Team', publishDate: '2026-01-20', views: 1234, rating: 4.3, tags: ['supply-chain', 'vendor', 'risk'], summary: 'Framework for managing security risks in the software supply chain', readTime: '14 min'},
    {id: 'kb-soc-metrics-006', title: 'Kubernetes Security Hardening Checklist', category: 'Container', author: 'Platform Security', publishDate: '2026-04-10', views: 2103, rating: 4.7, tags: ['kubernetes', 'container', 'hardening'], summary: 'Step-by-step hardening checklist for production Kubernetes clusters', readTime: '11 min'},
    {id: 'kb-soc-metrics-007', title: 'Security Metrics That Matter for Executives', category: 'Metrics', author: 'CISO Office', publishDate: '2026-03-05', views: 876, rating: 4.4, tags: ['metrics', 'kpi', 'reporting'], summary: 'Key security metrics and KPIs that resonate with board-level stakeholders', readTime: '8 min'},
    {id: 'kb-soc-metrics-008', title: 'Phishing Simulation Campaign Design', category: 'Awareness', author: 'Security Awareness Team', publishDate: '2026-02-15', views: 1456, rating: 4.5, tags: ['phishing', 'simulation', 'training'], summary: 'Designing effective phishing simulations that drive real behavioral change', readTime: '9 min'},
    {id: 'kb-soc-metrics-009', title: 'Data Loss Prevention Strategy and Implementation', category: 'Data Protection', author: 'Data Security Team', publishDate: '2026-03-28', views: 1678, rating: 4.6, tags: ['dlp', 'data-protection', 'compliance'], summary: 'Strategic approach to DLP covering people, process, and technology layers', readTime: '16 min'},
    {id: 'kb-soc-metrics-010', title: 'Security Automation with SOAR Platforms', category: 'Automation', author: 'SOC Engineering', publishDate: '2026-04-05', views: 1098, rating: 4.7, tags: ['soar', 'automation', 'soc'], summary: 'Building effective security automation playbooks using SOAR technology', readTime: '13 min'},
    {id: 'kb-soc-metrics-011', title: 'Third-Party Risk Assessment Framework', category: 'GRC', author: 'Vendor Management', publishDate: '2026-01-30', views: 1345, rating: 4.4, tags: ['vendor', 'risk', 'assessment'], summary: 'Structured framework for evaluating and monitoring third-party security risks', readTime: '12 min'},
    {id: 'kb-soc-metrics-012', title: 'Network Detection and Response Best Practices', category: 'Network', author: 'Network Security Team', publishDate: '2026-03-18', views: 1567, rating: 4.5, tags: ['ndr', 'network', 'detection'], summary: 'Implementing effective network detection and response capabilities', readTime: '14 min'},
    {id: 'kb-soc-metrics-013', title: 'Security Chaos Engineering Guide', category: 'Resilience', author: 'SRE Security', publishDate: '2026-04-12', views: 789, rating: 4.3, tags: ['chaos-engineering', 'resilience', 'testing'], summary: 'Applying chaos engineering principles to validate security controls', readTime: '10 min'},
    {id: 'kb-soc-metrics-014', title: 'Incident Communication Templates', category: 'Incident Response', author: 'IR Team', publishDate: '2026-02-22', views: 2234, rating: 4.8, tags: ['incident', 'communication', 'templates'], summary: 'Ready-to-use communication templates for security incident scenarios', readTime: '7 min'},
    {id: 'kb-soc-metrics-015', title: 'Endpoint Detection and Response Configuration', category: 'Endpoint', author: 'Endpoint Security', publishDate: '2026-03-08', views: 1345, rating: 4.5, tags: ['edr', 'endpoint', 'configuration'], summary: 'Optimal EDR configuration for maximum detection with minimal false positives', readTime: '11 min'},
    {id: 'kb-soc-metrics-016', title: 'Security Code Review Standards', category: 'Development', author: 'AppSec Team', publishDate: '2026-02-10', views: 1890, rating: 4.6, tags: ['code-review', 'secure-sdlc', 'standards'], summary: 'Standards and checklists for security-focused code reviews', readTime: '13 min'},
    {id: 'kb-soc-metrics-017', title: 'IoT Security Assessment Methodology', category: 'IoT', author: 'IoT Security Lab', publishDate: '2026-04-08', views: 654, rating: 4.2, tags: ['iot', 'embedded', 'assessment'], summary: 'Methodology for assessing security posture of IoT devices and ecosystems', readTime: '15 min'},
    {id: 'kb-soc-metrics-018', title: 'Passwordless Authentication Migration Guide', category: 'Identity', author: 'IAM Team', publishDate: '2026-03-25', views: 1678, rating: 4.7, tags: ['passwordless', 'authentication', 'mfa'], summary: 'Step-by-step migration plan from password-based to passwordless authentication', readTime: '12 min'},
    {id: 'kb-soc-metrics-019', title: 'Security Awareness Program Metrics', category: 'Awareness', author: 'Security Culture Team', publishDate: '2026-01-25', views: 1123, rating: 4.4, tags: ['awareness', 'metrics', 'culture'], summary: 'Measuring the effectiveness of your security awareness and training programs', readTime: '9 min'},
    {id: 'kb-soc-metrics-020', title: 'Deception Technology Deployment Guide', category: 'Detection', author: 'Blue Team', publishDate: '2026-04-15', views: 876, rating: 4.5, tags: ['deception', 'honeypot', 'detection'], summary: 'Planning and deploying deception technology for advanced threat detection', readTime: '11 min'},
  ];
  ...
  // --- Security Scenario Planning Methods (SOME) ---
  private _initSomeScenarios(): void {
    const worstCaseScenarios = [
      { id: "some-sc-01", name: "Ransomware Attack on Critical Infrastructure", category: "Malware", impact: 9500000, probability: 0.12, recoveryDays: 21, affectedSystems: 47, dataLossGb: 320, businessImpact: "critical" },
      { id: "some-sc-02", name: "Insider Data Exfiltration via Authorized Channels", category: "Insider Threat", impact: 7200000, probability: 0.18, recoveryDays: 14, affectedSystems: 12, dataLossGb: 85, businessImpact: "high" },
      { id: "some-sc-03", name: "Supply Chain Compromise via Third-Party Library", category: "Supply Chain", impact: 6800000, probability: 0.09, recoveryDays: 30, affectedSystems: 63, dataLossGb: 0, businessImpact: "critical" },
      { id: "some-sc-04", name: "Cloud Misconfiguration Leading to Public Exposure", category: "Cloud", impact: 4100000, probability: 0.22, recoveryDays: 7, affectedSystems: 8, dataLossGb: 450, businessImpact: "high" },
      { id: "some-sc-05", name: "Zero-Day Exploit in Core Application Framework", category: "Vulnerability", impact: 8900000, probability: 0.05, recoveryDays: 45, affectedSystems: 120, dataLossGb: 0, businessImpact: "critical" },
      { id: "some-sc-06", name: "Distributed Denial of Service on Customer-Facing Services", category: "Availability", impact: 3400000, probability: 0.25, recoveryDays: 3, affectedSystems: 15, dataLossGb: 0, businessImpact: "medium" },
      { id: "some-sc-07", name: "Credential Stuffing and Account Takeover Wave", category: "Identity", impact: 2800000, probability: 0.30, recoveryDays: 5, affectedSystems: 6, dataLossGb: 12, businessImpact: "medium" },
      { id: "some-sc-08", name: "Physical Security Breach at Data Center Facility", category: "Physical", impact: 12000000, probability: 0.02, recoveryDays: 60, affectedSystems: 200, dataLossGb: 5000, businessImpact: "critical" },
    ];
    this._someScenarios = worstCaseScenarios.map(s => ({ ...s, mitigationCost: Math.round(s.impact * 0.08), drillScheduled: false, lastDrillDate: null, drillScore: null, controlGaps: Math.floor(Math.random() * 5) + 1 }));
  }

  private _calculateSomeScenarioRisk(): void {
    this._someScenarios.forEach(s => {
  ...
  private _scheduleSomeDrills(): void {
  ...
  private _someScenarioComparison(): Record<string, unknown>[] {
    return this._someScenarios.map(s => ({
  ...
  // --- Security Control Effectiveness Analytics (SOME) ---
  private _initSomeControls(): void {
    const controls = [
      { id: "some-ct-01", name: "Network Segmentation", type: "preventive", maturity: "defined", score: 82, target: 90, failures: 3, lastTest: "2026-03-15" },
      { id: "some-ct-02", name: "Endpoint Detection and Response", type: "detective", maturity: "managed", score: 88, target: 92, failures: 1, lastTest: "2026-04-01" },
      { id: "some-ct-03", name: "Data Loss Prevention", type: "preventive", maturity: "repeatable", score: 71, target: 85, failures: 7, lastTest: "2026-03-22" },
      { id: "some-ct-04", name: "Security Information and Event Management", type: "detective", maturity: "managed", score: 85, target: 90, failures: 2, lastTest: "2026-04-05" },
      { id: "some-ct-05", name: "Identity and Access Management", type: "preventive", maturity: "defined", score: 76, target: 88, failures: 5, lastTest: "2026-03-28" },
      { id: "some-ct-06", name: "Vulnerability Management", type: "corrective", maturity: "managed", score: 80, target: 90, failures: 4, lastTest: "2026-04-02" },
      { id: "some-ct-07", name: "Email Security Gateway", type: "preventive", maturity: "managed", score: 90, target: 95, failures: 1, lastTest: "2026-04-08" },
      { id: "some-ct-08", name: "Web Application Firewall", type: "preventive", maturity: "managed", score: 87, target: 92, failures: 2, lastTest: "2026-03-30" },
      { id: "some-ct-09", name: "Patch Management", type: "corrective", maturity: "defined", score: 68, target: 85, failures: 9, lastTest: "2026-03-18" },
      { id: "some-ct-10", name: "Encryption at Rest", type: "preventive", maturity: "managed", score: 93, target: 95, failures: 0, lastTest: "2026-04-10" },
      { id: "some-ct-11", name: "Encryption in Transit", type: "preventive", maturity: "optimized", score: 96, target: 98, failures: 0, lastTest: "2026-04-12" },
      { id: "some-ct-12", name: "Privileged Access Management", type: "preventive", maturity: "defined", score: 74, target: 88, failures: 6, lastTest: "2026-03-25" },
      { id: "some-ct-13", name: "Security Awareness Training", type: "preventive", maturity: "repeatable", score: 65, target: 80, failures: 11, lastTest: "2026-03-20" },
      { id: "some-ct-14", name: "Incident Response Plan", type: "corrective", maturity: "managed", score: 78, target: 88, failures: 4, lastTest: "2026-04-03" },
      { id: "some-ct-15", name: "Backup and Recovery", type: "corrective", maturity: "managed", score: 84, target: 92, failures: 3, lastTest: "2026-04-06" },
      { id: "some-ct-16", name: "Multi-Factor Authentication", type: "preventive", maturity: "managed", score: 91, target: 95, failures: 1, lastTest: "2026-04-09" },
      { id: "some-ct-17", name: "Network Traffic Analysis", type: "detective", maturity: "defined", score: 72, target: 85, failures: 5, lastTest: "2026-03-27" },
      { id: "some-ct-18", name: "Cloud Security Posture Management", type: "detective", maturity: "repeatable", score: 69, target: 82, failures: 8, lastTest: "2026-03-24" },
      { id: "some-ct-19", name: "Container Security Scanning", type: "preventive", maturity: "defined", score: 75, target: 85, failures: 5, lastTest: "2026-04-04" },
      { id: "some-ct-20", name: "API Security Gateway", type: "preventive", maturity: "repeatable", score: 70, target: 82, failures: 7, lastTest: "2026-03-29" },
      { id: "some-ct-21", name: "Threat Intelligence Platform", type: "detective", maturity: "managed", score: 83, target: 90, failures: 2, lastTest: "2026-04-07" },
      { id: "some-ct-22", name: "Database Activity Monitoring", type: "detective", maturity: "defined", score: 67, target: 80, failures: 8, lastTest: "2026-03-21" },
      { id: "some-ct-23", name: "Deception Technology", type: "detective", maturity: "initial", score: 55, target: 75, failures: 12, lastTest: "2026-03-16" },
      { id: "some-ct-24", name: "Security Orchestration Automation", type: "corrective", maturity: "repeatable", score: 73, target: 85, failures: 6, lastTest: "2026-04-01" },
      { id: "some-ct-25", name: "Third-Party Risk Assessment", type: "preventive", maturity: "defined", score: 64, target: 78, failures: 9, lastTest: "2026-03-19" },
    ];
    this._someControls = controls;
  }

  private _analyzeSomeControlEffectiveness(): void {
    const typeBreakdown: Record<string, { total: number; avgScore: number; avgFailures: number }> = {};
    this._someControls.forEach(c => {
    ...
    this._someControlTypeBreakdown = typeBreakdown;
  }

  private _someControlOptimization(): Record<string, unknown>[] {
    return this._someControls.filter(c => c.score < c.target).map(c => ({
  ...
  // --- Security Data Pipeline Health (SOME) ---
  private _initSomePipelines(): void {
    const pipelines = [
      { id: "some-pl-01", name: "SIEM Log Ingestion", status: "healthy", healthScore: 94, latencyMs: 120, freshnessMin: 2, errorRate: 0.02, throughputMbps: 450, recordsPerSec: 12000, backPressure: 0.05, uptime: 99.97 },
      { id: "some-pl-02", name: "Threat Feed Aggregation", status: "healthy", healthScore: 91, latencyMs: 300, freshnessMin: 15, errorRate: 0.05, throughputMbps: 85, recordsPerSec: 3200, backPressure: 0.08, uptime: 99.92 },
      { id: "some-pl-03", name: "Vulnerability Scan Results", status: "degraded", healthScore: 72, latencyMs: 2500, freshnessMin: 60, errorRate: 0.15, throughputMbps: 25, recordsPerSec: 800, backPressure: 0.35, uptime: 98.50 },
      { id: "some-pl-04", name: "Endpoint Telemetry Stream", status: "healthy", healthScore: 88, latencyMs: 450, freshnessMin: 5, errorRate: 0.08, throughputMbps: 280, recordsPerSec: 8500, backPressure: 0.12, uptime: 99.85 },
      { id: "some-pl-05", name: "Cloud Audit Log Pipeline", status: "healthy", healthScore: 86, latencyMs: 600, freshnessMin: 10, errorRate: 0.06, throughputMbps: 150, recordsPerSec: 4500, backPressure: 0.10, uptime: 99.80 },
      { id: "some-pl-06", name: "Identity Event Stream", status: "warning", healthScore: 78, latencyMs: 1200, freshnessMin: 20, errorRate: 0.12, throughputMbps: 45, recordsPerSec: 1500, backPressure: 0.22, uptime: 99.40 },
      { id: "some-pl-07", name: "Network Flow Collection", status: "healthy", healthScore: 82, latencyMs: 800, freshnessMin: 8, errorRate: 0.09, throughputMbps: 650, recordsPerSec: 18000, backPressure: 0.15, uptime: 99.70 },
      { id: "some-pl-08", name: "DLP Incident Pipeline", status: "degraded", healthScore: 68, latencyMs: 3500, freshnessMin: 45, errorRate: 0.20, throughputMbps: 18, recordsPerSec: 500, backPressure: 0.42, uptime: 97.80 },
    ];
    this._somePipelines = pipelines.map(p => ({ ...p, dependencies: [], slaBreached: p.healthScore < 75, alertThreshold: p.errorRate > 0.15 }));
    this._somePipelines[0].dependencies = ["some-pl-04", "some-pl-07"];
    this._somePipelines[4].dependencies = ["some-pl-01"];
    this._somePipelines[5].dependencies = ["some-pl-02"];
    this._somePipelines[7].dependencies = ["some-pl-04", "some-pl-05"];
  }

  private _somePipelineTrend(): Record<string, unknown>[] {
    const hours = Array.from({ length: 24 }, (_, i) => `H${String(i).padStart(2, "0")}`);
    return this._somePipelines.map(p => ({
  ...
  // --- Security Stakeholder Report Generator (SOME) ---
  private _initSomeReportTemplates(): void {
    this._someReportTemplates = [
      { id: "some-rp-01", name: "Board Security Report", audience: "board", frequency: "quarterly", sections: 6, autoGenerate: true, lastGenerated: "2026-03-31", pages: 12 },
      { id: "some-rp-02", name: "Executive Risk Summary", audience: "executive", frequency: "monthly", sections: 8, autoGenerate: true, lastGenerated: "2026-04-01", pages: 8 },
      { id: "some-rp-03", name: "Technical Security Review", audience: "technical", frequency: "weekly", sections: 12, autoGenerate: true, lastGenerated: "2026-04-21", pages: 25 },
      { id: "some-rp-04", name: "Audit Compliance Report", audience: "audit", frequency: "quarterly", sections: 10, autoGenerate: false, lastGenerated: "2026-03-15", pages: 35 },
      { id: "some-rp-05", name: "Regulatory Filing Package", audience: "regulatory", frequency: "annual", sections: 15, autoGenerate: false, lastGenerated: "2025-12-31", pages: 48 },
    ];
  }

  private _generateSomeExecSummary(): string {
    const totalRisk = this._someScenarios.reduce((sum: number, s: any) => sum + s.riskScore, 0);
    const avgControl = Math.round(this._someControls.reduce((sum: number, c: any) => sum + c.score, 0) / this._someControls.length);
    const degradedPipes = this._somePipelines.filter((p: any) => p.status !== "healthy").length;
    return `Overall risk exposure: ${(totalRisk / 1000000).toFixed(1)}M. Average control effectiveness: {avgControl}%. {degradedPipes} pipeline(s) need attention. {this._someScenarios.filter((s: any) => s.mitigationPriority === "immediate").length} scenarios require immediate action.`;
  }

  // --- Security Technology Radar (SOME) ---
  private _initSomeTechRadar(): void {
    this._someTechRadar = [
      { id: "some-tr-01", name: "AI-Powered Threat Detection", ring: "adopt", category: "Detection", maturity: 4, trend: "up", investment: "high", roi: 3.2, vendorCount: 8 },
      { id: "some-tr-02", name: "Zero Trust Architecture", ring: "adopt", category: "Architecture", maturity: 4, trend: "up", investment: "high", roi: 2.8, vendorCount: 12 },
      { id: "some-tr-03", name: "SASE/SSE Platform", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.4, vendorCount: 10 },
      { id: "some-tr-04", name: "CNAPP Cloud Security", ring: "trial", category: "Cloud", maturity: 3, trend: "up", investment: "medium", roi: 2.1, vendorCount: 7 },
      { id: "some-tr-05", name: "Extended Detection and Response", ring: "adopt", category: "Detection", maturity: 4, trend: "stable", investment: "high", roi: 2.9, vendorCount: 9 },
      { id: "some-tr-06", name: "Quantum-Safe Cryptography", ring: "assess", category: "Crypto", maturity: 2, trend: "up", investment: "low", roi: 0.8, vendorCount: 4 },
      { id: "some-tr-07", name: "Security Service Edge", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.0, vendorCount: 6 },
      { id: "some-tr-08", name: "SOAR 2.0 Autonomous SOC", ring: "assess", category: "Operations", maturity: 2, trend: "up", investment: "low", roi: 1.2, vendorCount: 5 },
      { id: "some-tr-09", name: "Software Supply Chain Security", ring: "trial", category: "DevSecOps", maturity: 3, trend: "up", investment: "medium", roi: 2.3, vendorCount: 8 },
      { id: "some-tr-10", name: "Identity Threat Detection", ring: "adopt", category: "Identity", maturity: 3, trend: "up", investment: "high", roi: 2.7, vendorCount: 7 },
      { id: "some-tr-11", name: "Data Security Posture Management", ring: "trial", category: "Data", maturity: 3, trend: "up", investment: "medium", roi: 2.2, vendorCount: 9 },
      { id: "some-tr-12", name: "Decentralized Identity Standards", ring: "hold", category: "Identity", maturity: 1, trend: "stable", investment: "low", roi: 0.5, vendorCount: 3 },
    ];
  }

  private _someRadarSummary(): Record<string, unknown> {
    const rings: Record<string, number> = { adopt: 0, trial: 0, assess: 0, hold: 0 };
    this._someTechRadar.forEach(t => { if (rings[t.ring as keyof typeof rings] !== undefined) rings[t.ring as keyof typeof rings]++; });
    return { adopt: rings.adopt, trial: rings.trial, assess: rings.assess, hold: rings.hold, total: this._someTechRadar.length, avgMaturity: 2.8, topInvestment: this._someTechRadar.filter(t => t.investment === "high").map(t => t.name) };
  }

  // --- SOME Security Compliance Mapping Matrix ---
  private _initSomeComplianceMatrix(): void {
    ...
    this._someComplianceMatrix = frameworks.map(fw => ({
  ...
  private _someComplianceTrend(): Record<string, unknown>[] {
    const quarters = ["Q1-2025", "Q2-2025", "Q3-2025", "Q4-2025", "Q1-2026", "Q2-2026"];
    return this._someComplianceMatrix.map(fw => ({
  ...
  // --- SOME Threat Intelligence Correlation Engine ---
  private _initSomeThreatIntel(): void {
    ...
    this._someThreatIntel = threatActors.map((actor, i) => ({
  ...
  private _someThreatCorrelation(): Record<string, unknown>[] {
    return this._someThreatIntel.filter(t => t.riskScore > 70).map(t => ({
  ...
  // --- SOME Security Metrics Deep Dive ---
  private _initSomeMetricsDeep(): void {
    ...
    this._someMetricsDeep = metricCategories.map(cat => ({
  ...
  private _someMetricsHeatmap(): number[][] {
    return this._someMetricsDeep.map(cat =>
      cat.metrics.map(m => Math.round((m.value / m.target) * 100))
    );
  }

  // --- SOME Incident Cost Modeling ---
  private _initSomeCostModel(): void {
    const incidentTypes = ["Data Breach", "Ransomware", "DDoS", "Insider Threat", "Phishing", "Supply Chain", "Cloud Misconfig", "Zero-Day"];
    this._someCostModel = incidentTypes.map(inc => ({
    ...
    this._someCostModel.forEach(m => { m.totalAnnualExposure = m.avgCost * m.frequency; });
  }

  private _someCostProjection(): Record<string, unknown>[] {
    return this._someCostModel.map(m => ({
  ...
  // --- SOME Security Architecture Pattern Library ---
  private _initSomeArchPatterns(): void {
    ...
    this._someArchPatterns = patterns.map((p, i) => ({
  ...
  private _someArchPatternAnalysis(): Record<string, unknown> {
    const implemented = this._someArchPatterns.filter(p => p.implementation > 75).length;
    const avgCoverage = Math.round(this._someArchPatterns.reduce((s: number, p: any) => s + p.coverage, 0) / this._someArchPatterns.length);
    return {
      totalPatterns: this._someArchPatterns.length, fullyImplemented: implemented, avgCoverage,
      avgRiskReduction: Math.round(this._someArchPatterns.reduce((s: number, p: any) => s + p.riskReduction, 0) / this._someArchPatterns.length),
      gaps: this._someArchPatterns.filter(p => p.implementation < 60).map(p => p.pattern),
      recommendations: this._someArchPatterns.filter(p => p.implementation < 70).map(p => ({ pattern: p.pattern, current: p.implementation, target: 85, effort: p.costComplexity }))
    };
  }
  ...
  private _8bcRiskAssessmentWorkflow = [
  ...
  private _8bcTreatmentOptions = [
    {id: '8bc-T-001', riskId: 'RSK-042', risk: 'Ransomware Attack on Critical Infrastructure', before: 20, after: 6, strategy: 'Mitigate', controls: ['Next-gen EDR with behavioral detection and automated response', 'Zero-trust network micro-segmentation isolating critical assets', 'Immutable offline backup rotation with quarterly recovery testing', 'Incident response retainer with 1-hour response SLA'], cost: '$450K/yr', roi: '78% reduction vs $2.8M potential loss', timeline: 'Q3 2026', status: 'approved' as const},
  ...
  private _8bcTalentPositions = [
    ...
    {id: '8bc-H-002', title: 'Cloud Security Engineer', dept: 'Cloud Security', level: 'Mid-Senior', loc: 'SF/Remote', salary: '$155K-$180K', posted: '2026-03-20', applicants: 45, pipeline: 12, interviews: 6, offers: 2, urgency: 'high' as const, manager: 'Mike Johnson', skills: ['AWS/Azure/GCP', 'Terraform', 'Kubernetes', 'IAM', 'CSPM'], nice: ['CCSP', 'CKS', 'Serverless']},
    {id: '8bc-H-003', title: 'SOC Analyst L2', dept: 'Security Operations', level: 'Mid', loc: 'Austin TX', salary: '$105K-$130K', posted: '2026-04-01', applicants: 67, pipeline: 18, interviews: 8, offers: 0, urgency: 'medium' as const, manager: 'Lisa Park', skills: ['SIEM', 'Incident triage', 'Malware analysis', 'TCP/IP'], nice: ['GCIA', 'SOAR', 'Kill chain']},
    {id: '8bc-H-004', title: 'Staff Security Architect', dept: 'Architecture', level: 'Staff', loc: 'Remote US', salary: '$200K-$240K', posted: '2026-02-28', applicants: 12, pipeline: 3, interviews: 2, offers: 0, urgency: 'critical' as const, manager: 'David Lee', skills: ['Enterprise arch', 'Zero trust', 'Risk frameworks', 'Board comms'], nice: ['CISSP+SABSA', 'Team building', 'Speaker']},
    {id: '8bc-H-005', title: 'GRC Analyst', dept: 'GRC', level: 'Mid', loc: 'NY/Remote', salary: '$110K-$135K', posted: '2026-04-05', applicants: 34, pipeline: 9, interviews: 3, offers: 1, urgency: 'medium' as const, manager: 'Rachel Green', skills: ['ISO 27001', 'SOC 2', 'Risk assessment', 'Audit'], nice: ['CRISC', 'CISA', 'GDPR']},
    {id: '8bc-H-006', title: 'Threat Intel Analyst', dept: 'Threat Intel', level: 'Mid-Senior', loc: 'Remote US', salary: '$140K-$170K', posted: '2026-03-25', applicants: 19, pipeline: 5, interviews: 2, offers: 0, urgency: 'low' as const, manager: 'James Wilson', skills: ['MITRE ATT&CK', 'OSINT', 'Threat modeling', 'Reverse eng'], nice: ['CTIA', 'Nation-state', 'Dark web']},
    {id: '8bc-H-007', title: 'AppSec Engineer', dept: 'AppSec', level: 'Senior', loc: 'Seattle/Remote', salary: '$160K-$190K', posted: '2026-04-10', applicants: 28, pipeline: 7, interviews: 3, offers: 0, urgency: 'medium' as const, manager: 'Emily Zhang', skills: ['SAST/DAST/SCA', 'Secure SDLC', 'Code review', 'CI/CD'], nice: ['CSSLP', 'Mobile security']},
    {id: '8bc-H-008', title: 'IAM Engineer', dept: 'IAM', level: 'Mid', loc: 'Remote US', salary: '$135K-$160K', posted: '2026-04-08', applicants: 31, pipeline: 10, interviews: 5, offers: 1, urgency: 'medium' as const, manager: 'Chris Martinez', skills: ['Okta/Azure AD', 'SAML/OIDC', 'PAM', 'RBAC'], nice: ['CISSP', 'Zero trust IAM']}
  ];

  private _8bcInterviewScorecard = [
  ...
  private _8bcOnboardingSteps = [
    {id: '8bc-OB-01', item: 'Provision security-hardened laptop, monitors, peripherals, and configured workstation image with all required security tools', cat: 'IT Setup', day: 'Day 1', owner: 'IT Operations'},
    {id: '8bc-OB-02', item: 'Create AD account, email, Slack, VPN, SSO enrollment, and all required application access accounts', cat: 'Access', day: 'Day 1', owner: 'IAM Team'},
    {id: '8bc-OB-03', item: 'Grant security tooling access: SIEM console, EDR dashboard, vuln scanner, pentest platforms, SOAR', cat: 'Tools', day: 'Day 1-2', owner: 'SOC Manager'},
    {id: '8bc-OB-04', item: 'Complete MFA enrollment, security awareness onboarding course, and acceptable use policy acknowledgment', cat: 'Security', day: 'Day 1', owner: 'Awareness Team'},
    {id: '8bc-OB-05', item: 'Sign NDA, employment agreement, IP assignment, and code of conduct documents with HR', cat: 'HR', day: 'Day 1', owner: 'Human Resources'},
    {id: '8bc-OB-06', item: 'Introduction meetings with direct team members and key cross-functional stakeholders', cat: 'Integration', day: 'Week 1', owner: 'Hiring Manager'},
    {id: '8bc-OB-07', item: 'Security architecture overview, tooling walkthrough, and operational runbook review sessions', cat: 'Training', day: 'Week 1-2', owner: 'Security Architect'},
    {id: '8bc-OB-08', item: 'Shadow experienced team member on active incident response and threat hunting investigations', cat: 'Mentoring', day: 'Week 2-3', owner: 'SOC Lead'},
    {id: '8bc-OB-09', item: 'Set up individual certification study plan aligned with role requirements and career goals', cat: 'Development', day: 'Week 2', owner: 'Hiring Manager'},
    {id: '8bc-OB-10', item: 'Establish 30-60-90 day goals, define success metrics, and schedule first performance checkpoint', cat: 'Goals', day: 'Week 1', owner: 'Hiring Manager'}
  ];

  private _8bcMetricsDashboard = [
    {id: '8bc-MTD-001', name: 'Mean Time to Detect (MTTD)', value: '4.2 hours', trend: 'improving' as const, target: '< 4 hours', source: 'SIEM - Splunk', method: 'Median time from first alert to analyst acknowledgment', owner: 'SOC Manager', sla: '4 hours', prev: '5.1 hours', spark: [5.8, 5.4, 5.1, 4.9, 4.7, 4.5, 4.3, 4.2], breakdown: [{src: 'Network IDS', pct: '35%', avg: '3.8h'}, {src: 'Endpoint EDR', pct: '28%', avg: '4.1h'}, {src: 'Email Gateway', pct: '22%', avg: '3.5h'}, {src: 'Cloud Security', pct: '10%', avg: '5.2h'}, {src: 'User Reports', pct: '5%', avg: '6.8h'}]},
    {id: '8bc-MTD-002', name: 'Mean Time to Respond (MTTR)', value: '2.8 hours', trend: 'stable' as const, target: '< 3 hours', source: 'SOAR - XSOAR', method: 'Median time from acknowledgment to first containment action', owner: 'IR Lead', sla: '3 hours', prev: '2.7 hours', spark: [3.2, 3.0, 2.9, 2.8, 2.8, 2.9, 2.8, 2.8], breakdown: [{src: 'Auto Playbooks', pct: '42%', avg: '1.2h'}, {src: 'Semi-Auto', pct: '33%', avg: '2.5h'}, {src: 'Manual', pct: '20%', avg: '5.1h'}, {src: 'L3 Escalation', pct: '5%', avg: '8.3h'}]},
    {id: '8bc-MTD-003', name: 'Patch Compliance Rate', value: '97.6%', trend: 'improving' as const, target: '> 95%', source: 'Tenable.io', method: 'Pct of critical/high vulns patched within SLA windows', owner: 'Vuln Manager', sla: '95%', prev: '96.1%', spark: [93.2, 94.1, 94.8, 95.3, 95.9, 96.4, 97.0, 97.6], breakdown: [{src: 'Critical (7d)', pct: '40%', rate: '99.2%'}, {src: 'High (30d)', pct: '35%', rate: '97.8%'}, {src: 'Medium (90d)', pct: '20%', rate: '95.1%'}, {src: 'Low (180d)', pct: '5%', rate: '89.4%'}]},
    {id: '8bc-MTD-004', name: 'Phishing Click Rate', value: '3.2%', trend: 'improving' as const, target: '< 5%', source: 'KnowBe4 + Proofpoint', method: 'Pct of users clicking simulated phishing links monthly', owner: 'Awareness Lead', sla: '5%', prev: '4.1%', spark: [6.8, 6.2, 5.5, 5.1, 4.6, 4.1, 3.6, 3.2], breakdown: [{src: 'Spear Phishing', pct: '35%', rate: '5.1%'}, {src: 'Generic Phishing', pct: '30%', rate: '2.8%'}, {src: 'BEC Sim', pct: '20%', rate: '3.5%'}, {src: 'SMS Phishing', pct: '15%', rate: '1.9%'}]},
    {id: '8bc-MTD-005', name: 'Security Posture Score', value: '784', trend: 'improving' as const, target: '> 750', source: 'SecurityScorecard', method: 'Composite: external 30%, compliance 25%, vuln 25%, config 20%', owner: 'CISO', sla: '> 750', prev: '771', spark: [748, 752, 758, 762, 768, 773, 778, 784], breakdown: [{src: 'External Rating', pct: '30%', score: '791'}, {src: 'Compliance', pct: '25%', score: '812'}, {src: 'Vuln Mgmt', pct: '25%', score: '745'}, {src: 'Config', pct: '20%', score: '769'}]},
    {id: '8bc-MTD-006', name: 'Vulnerability Backlog', value: '142', trend: 'improving' as const, target: '< 150', source: 'Tenable + Qualys', method: 'Total open vulns across all scanned assets tracked weekly', owner: 'Vuln Manager', sla: '< 150', prev: '168', spark: [210, 198, 185, 175, 165, 158, 150, 142], breakdown: [{src: 'Critical', pct: '8%', count: '11'}, {src: 'High', pct: '22%', count: '31'}, {src: 'Medium', pct: '45%', count: '64'}, {src: 'Low', pct: '25%', count: '36'}]}
  ];

  private _8bcArchReview = [
    {id: '8bc-AR-001', cat: 'Network', item: 'Network segmentation between trust zones with dedicated firewalls and granular ACLs', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Micro-segmentation via NSX across all production zones'},
    {id: '8bc-AR-002', cat: 'Network', item: 'DMZ architecture isolating public services from internal network', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Dual-firewall DMZ with WAF and DDoS mitigation'},
    {id: '8bc-AR-003', cat: 'Network', item: 'Least-privilege traffic flows with deny-by-default and quarterly review', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Explicit allow rules with automated drift detection'},
    {id: '8bc-AR-004', cat: 'Network', item: 'Encrypted external communications with IPSec/TLS 1.3 and PFS', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'IKEv2 with PFS, TLS 1.3 for all external APIs'},
    {id: '8bc-AR-005', cat: 'Identity', item: 'Centralized IdP with SSO and automated SCIM provisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Okta SSO SAML 2.0 for 247 apps with SCIM'},
    {id: '8bc-AR-006', cat: 'Identity', item: 'RBAC with quarterly reviews and automated deprovisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Quarterly recertification with JIT admin access'},
    {id: '8bc-AR-007', cat: 'Identity', item: 'PAM controlling all admin and service accounts with session recording', sev: 'high' as const, status: 'fail' as const, reviewer: 'IAM Lead', notes: '3 legacy accounts pending CyberArk enrollment'},
    {id: '8bc-AR-008', cat: 'Identity', item: 'MFA enforced for all users, hardware tokens for privileged access', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'FIDO2 for admins, push MFA for users, 99.8% enrollment'},
    {id: '8bc-AR-009', cat: 'Data', item: 'Data classification enforced with automated labeling on all repos', sev: 'high' as const, status: 'partial' as const, reviewer: 'Data Protection Lead', notes: 'Framework deployed, gaps on 4 legacy file shares'},
    {id: '8bc-AR-010', cat: 'Data', item: 'AES-256 encryption at rest for all sensitive data stores', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'Full-disk, TDE on DBs, SSE-KMS on cloud storage'},
    {id: '8bc-AR-011', cat: 'Data', item: 'DLP monitoring and preventing unauthorized data exfiltration', sev: 'high' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'DLP active on email, endpoint, cloud, and network'},
    {id: '8bc-AR-012', cat: 'Data', item: 'Encrypted off-site backups with quarterly restore testing', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Infrastructure Lead', notes: '3-2-1 strategy with immutable backups'},
    {id: '8bc-AR-013', cat: 'AppSec', item: 'Secure SDLC with SAST, DAST, SCA in all CI/CD pipelines', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: '34 pipelines with security gates and PR blocking'},
    {id: '8bc-AR-014', cat: 'AppSec', item: 'API gateway with auth, rate limiting, schema validation', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: 'Kong with OAuth2, rate limits, OpenAPI validation'},
    {id: '8bc-AR-015', cat: 'AppSec', item: 'Container security with image scanning and runtime protection', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Trivy scanning, Falco runtime in 80% of clusters'},
    {id: '8bc-AR-016', cat: 'Monitoring', item: 'SIEM collecting from all critical systems with correlation rules', sev: 'critical' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'Splunk 98% coverage with 1200+ correlation rules'},
    {id: '8bc-AR-017', cat: 'Monitoring', item: 'EDR with behavioral detection on all managed endpoints', sev: 'high' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'CrowdStrike on 99.2% endpoints with 24/7 MDR'},
    {id: '8bc-AR-018', cat: 'Monitoring', item: 'Executive dashboards with real-time posture visibility', sev: 'medium' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'PowerBI dashboards with weekly exec summaries'},
    {id: '8bc-AR-019', cat: 'Cloud', item: 'CSPM monitoring and remediating cloud misconfigurations', sev: 'high' as const, status: 'pass' as const, reviewer: 'Cloud Security Lead', notes: 'Prisma Cloud 450+ checks with auto-remediation'},
    {id: '8bc-AR-020', cat: 'Cloud', item: 'IaC templates scanned before deployment to production', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Checkov for Terraform, cfn-nag for CloudFormation pending'}
  ];

  private _get8bcRiskProgress(): number {
    return Math.round(this._8bcRiskAssessmentWorkflow.filter(s => s.status === 'completed').length / this._8bcRiskAssessmentWorkflow.length * 100);
  }

  private _get8bcTreatmentSummary(): {total: number; approved: number; avgReduction: number; investment: string} {
    const a = this._8bcTreatmentOptions.filter(r => r.status === 'approved').length;
    const avg = this._8bcTreatmentOptions.reduce((s, r) => s + ((r.before - r.after) / r.before * 100), 0) / this._8bcTreatmentOptions.length;
    return {total: this._8bcTreatmentOptions.length, approved: a, avgReduction: Math.round(avg), investment: '$1.125M annually'};
  }

  private _get8bcRuleSummary(): {total: number; passing: number; failing: number; avgRate: number; violations: number} {
    const p = this._8bcComplianceRules.filter(r => r.result === 'pass').length;
    const f = this._8bcComplianceRules.filter(r => r.result === 'fail').length;
    const avg = this._8bcComplianceRules.reduce((s, r) => s + r.passRate, 0) / this._8bcComplianceRules.length;
    const v = this._8bcComplianceRules.reduce((s, r) => s + r.violations, 0);
    return {total: this._8bcComplianceRules.length, passing: p, failing: f, avgRate: Math.round(avg * 10) / 10, violations: v};
  }

  private _get8bcPipelineStats(): {positions: number; applicants: number; interviewing: number; offers: number; critical: number} {
    const t = this._8bcTalentPositions.length;
    const a = this._8bcTalentPositions.reduce((s, p) => s + p.applicants, 0);
    const i = this._8bcTalentPositions.reduce((s, p) => s + p.interviews, 0);
    const o = this._8bcTalentPositions.reduce((s, p) => s + p.offers, 0);
    const c = this._8bcTalentPositions.filter(x => x.urgency === 'critical').length;
    return {positions: t, applicants: a, interviewing: i, offers: o, critical: c};
  }

  private _get8bcMetricTrends(): {improving: number; stable: number; total: number} {
    const imp = this._8bcMetricsDashboard.filter(m => m.trend === 'improving').length;
    const stb = this._8bcMetricsDashboard.filter(m => m.trend === 'stable').length;
    return {improving: imp, stable: stb, total: this._8bcMetricsDashboard.length};
  }

  private _get8bcArchScore(): {total: number; passed: number; failed: number; partial: number; score: number} {
    const p = this._8bcArchReview.filter(c => c.status === 'pass').length;
    const f = this._8bcArchReview.filter(c => c.status === 'fail').length;
    const pa = this._8bcArchReview.filter(c => c.status === 'partial').length;
    return {total: this._8bcArchReview.length, passed: p, failed: f, partial: pa, score: Math.round((p / this._8bcArchReview.length) * 100)};
  }


  private _z8bcIncidentScenarios = [
    {id: 'z8bc-IS-001', name: 'Ransomware Outbreak in Production Environment', category: 'Malware', likelihood: 'Medium', impact: 'Critical', riskScore: 15, playbooks: ['IR-Ransomware-Isolate', 'IR-Ransomware-Eradicate', 'IR-Ransomway-Recover'], estimatedMTTD: '15min', estimatedMTTR: '4h', involvedTeams: ['SOC', 'IR', 'Infrastructure', 'Legal', 'Comms'], keyIndicators: ['Mass file encryption events', 'Ransom note deployment', 'Lateral movement via SMB/RDP', 'Backup deletion attempts'], mitigationControls: ['EDR behavioral detection', 'Network segmentation', 'Immutable backups', 'Email filtering'], lessonsLearned: 'Ensure offline backups are tested quarterly and Ransomware-specific playbooks are updated with latest threat intelligence', lastDrillDate: '2026-03-15', nextDrillDate: '2026-06-15'},
    {id: 'z8bc-IS-002', name: 'Cloud Misconfiguration Leading to Data Exposure', category: 'Cloud', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Cloud-Misconfig-Assess', 'IR-Cloud-Misconfig-Remediate', 'IR-Cloud-Misconfig-Notify'], estimatedMTTD: '2h', estimatedMTTR: '6h', involvedTeams: ['Cloud Security', 'SOC', 'Data Protection', 'Legal'], keyIndicators: ['Public S3 bucket exposure', 'Overly permissive IAM policies', 'Unencrypted database snapshots', 'Public API endpoint discovery'], mitigationControls: ['CSPM monitoring', 'IAM policy analyzer', 'Automated remediation', 'Cloud audit logging'], lessonsLearned: 'Implement CSPM with auto-remediation for common misconfigurations and conduct monthly cloud configuration reviews', lastDrillDate: '2026-02-20', nextDrillDate: '2026-05-20'},
    {id: 'z8bc-IS-003', name: 'Insider Threat Data Exfiltration', category: 'Insider', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-Insider-Investigate', 'IR-Insider-Contain', 'IR-Insider-Escalate'], estimatedMTTD: '8h', estimatedMTTR: '24h', involvedTeams: ['SOC', 'IR', 'HR', 'Legal', 'IAM'], keyIndicators: ['Anomalous data access patterns', 'Large file downloads to USB or cloud', 'Off-hours access to sensitive systems', 'Accessing data outside normal job scope'], mitigationControls: ['UEBA analytics', 'DLP policies', 'PAM controls', 'Access reviews'], lessonsLearned: 'Strengthen UEBA baselines and implement automated alerts for data access pattern anomalies', lastDrillDate: '2026-01-10', nextDrillDate: '2026-04-10'},
    {id: 'z8bc-IS-004', name: 'Supply Chain Attack via Compromised Vendor Software', category: 'Supply Chain', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-SupplyChain-Detect', 'IR-SupplyChain-Isolate', 'IR-SupplyChain-Remediate'], estimatedMTTD: '24h', estimatedMTTR: '72h', involvedTeams: ['SOC', 'IR', 'Procurement', 'Vendor Mgmt', 'Legal'], keyIndicators: ['Unexpected network connections from vendor software', 'Modified binaries or dependencies', 'Anomalous behavior from trusted applications', 'SBOM discrepancy alerts'], mitigationControls: ['SCA scanning', 'SBOM monitoring', 'Vendor security assessments', 'Network segmentation'], lessonsLearned: 'Require SBOM from all vendors and implement continuous monitoring of third-party component behavior', lastDrillDate: '2026-03-01', nextDrillDate: '2026-06-01'},
    {id: 'z8bc-IS-005', name: 'Credential Stuffing Attack on Customer Portal', category: 'External Attack', likelihood: 'High', impact: 'Medium', riskScore: 12, playbooks: ['IR-CredentialStuffing-Detect', 'IR-CredentialStuffing-Block', 'IR-CredentialStuffing-Notify'], estimatedMTTD: '30min', estimatedMTTR: '2h', involvedTeams: ['SOC', 'AppSec', 'Customer Support', 'Comms'], keyIndicators: ['High volume of failed login attempts', 'Login attempts from known compromised credential lists', 'Unusual geographic distribution of login attempts', 'Account takeover indicators'], mitigationControls: ['WAF rate limiting', 'CAPTCHA on login', 'MFA enforcement', 'Credential monitoring service'], lessonsLearned: 'Deploy advanced bot detection on login endpoints and implement proactive credential monitoring for customer accounts', lastDrillDate: '2026-04-01', nextDrillDate: '2026-07-01'},
    {id: 'z8bc-IS-006', name: 'Phishing Campaign Targeting Executive Team', category: 'Social Engineering', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Phishing-Contain', 'IR-Phishing-Investigate', 'IR-Phishing-Notify'], estimatedMTTD: '1h', estimatedMTTR: '4h', involvedTeams: ['SOC', 'Awareness', 'Executive Office', 'IT'], keyIndicators: ['Sophisticated spear-phishing emails to C-suite', 'Business email compromise indicators', 'Redirected MFA approval requests', 'Unauthorized wire transfer attempts'], mitigationControls: ['AI email filtering', 'Executive awareness training', 'Out-of-band verification policy', 'DMARC/DKIM/SPF'], lessonsLearned: 'Implement additional email security controls for executive mailboxes and conduct monthly spear-phishing simulations', lastDrillDate: '2026-02-15', nextDrillDate: '2026-05-15'}
  ];

  private _z8bcSecurityFrameworks = [
  ...
  private _getz8bcScenarioSummary(): {total: number; critical: number; high: number; drilled: number; avgRiskScore: number} {
    const crit = this._z8bcIncidentScenarios.filter(s => s.impact === 'Critical').length;
    const high = this._z8bcIncidentScenarios.filter(s => s.impact === 'High').length;
    const avg = this._z8bcIncidentScenarios.reduce((s, x) => s + x.riskScore, 0) / this._z8bcIncidentScenarios.length;
    return {total: this._z8bcIncidentScenarios.length, critical: crit, high: high, drilled: this._z8bcIncidentScenarios.length, avgRiskScore: Math.round(avg)};
  }

  private _getz8bcVendorRiskSummary(): {total: number; approved: number; conditional: number; remediation: number; avgScore: number} {
    const approved = this._z8bcThirdPartyRiskData.filter(v => v.status === 'approved').length;
    const cond = this._z8bcThirdPartyRiskData.filter(v => v.status === 'conditional').length;
    const rem = this._z8bcThirdPartyRiskData.filter(v => v.status === 'remediation_required').length;
    const avg = this._z8bcThirdPartyRiskData.reduce((s, v) => s + v.riskScore, 0) / this._z8bcThirdPartyRiskData.length;
    return {total: this._z8bcThirdPartyRiskData.length, approved, conditional: cond, remediation: rem, avgScore: Math.round(avg)};
  }

  private _getz8bcTrainingSummary(): {total: number; avgCompletion: number; avgScore: number; overdueUpdates: number} {
    const avgComp = this._z8bcSecurityTrainingCatalog.reduce((s, t) => s + t.completionRate, 0) / this._z8bcSecurityTrainingCatalog.length;
    const avgScore = this._z8bcSecurityTrainingCatalog.filter(t => t.avgScore > 0).reduce((s, t) => s + t.avgScore, 0) / this._z8bcSecurityTrainingCatalog.filter(t => t.avgScore > 0).length;
    return {total: this._z8bcSecurityTrainingCatalog.length, avgCompletion: Math.round(avgComp * 10) / 10, avgScore: Math.round(avgScore), overdueUpdates: 0};
  }

  private _getz8bcToolHealthSummary(): {total: number; healthy: number; degraded: number; annualCost: string} {
    const healthy = this._z8bcSecurityToolInventory.filter(t => t.health === 'healthy').length;
    const degraded = this._z8bcSecurityToolInventory.filter(t => t.health === 'degraded').length;
    const cost = this._z8bcSecurityToolInventory.reduce((s, t) => s + parseInt(t.annualCost.replace(/[^0-9]/g, '')), 0);
    return {total: this._z8bcSecurityToolInventory.length, healthy, degraded, annualCost: '$' + (cost / 1000).toFixed(0) + 'K'};
  }


  private _x8bcSecurityZones = [
    {zoneId: 'x8bc-Z-001', name: 'Internet DMZ', trustLevel: 'Untrusted', subnet: '10.0.0.0/24', segmentation: 'Dual-firewall', firewallRules: 45, monitoring: 'Full packet capture', assets: ['Web servers', 'Load balancers', 'WAF appliances', 'Reverse proxy'], riskLevel: 'high' as const, lastAssessment: '2026-04-15', nextAssessment: '2026-07-15'},
    {zoneId: 'x8bc-Z-002', name: 'Corporate Network', trustLevel: 'Trusted', subnet: '10.1.0.0/16', segmentation: 'VLAN + micro-seg', firewallRules: 128, monitoring: 'NetFlow + IDS', assets: ['Workstations', 'Printers', 'Meeting room systems', 'VoIP phones'], riskLevel: 'medium' as const, lastAssessment: '2026-04-10', nextAssessment: '2026-07-10'},
    {zoneId: 'x8bc-Z-003', name: 'Production Data Center', trustLevel: 'Restricted', subnet: '10.2.0.0/16', segmentation: 'Full micro-segmentation', firewallRules: 256, monitoring: 'Full packet capture + EDR', assets: ['Application servers', 'Database clusters', 'Message queues', 'API gateways'], riskLevel: 'low' as const, lastAssessment: '2026-04-12', nextAssessment: '2026-07-12'},
    {zoneId: 'x8bc-Z-004', name: 'Management Network', trustLevel: 'Restricted', subnet: '10.3.0.0/24', segmentation: 'Isolated VLAN', firewallRules: 32, monitoring: 'Syslog + SNMP', assets: ['Hypervisors', 'Switch management', 'Storage controllers', 'Backup appliances'], riskLevel: 'low' as const, lastAssessment: '2026-04-08', nextAssessment: '2026-07-08'},
    {zoneId: 'x8bc-Z-005', name: 'Cloud Production', trustLevel: 'Restricted', subnet: 'VPC: 172.16.0.0/12', segmentation: 'Security groups + NACLs', firewallRules: 189, monitoring: 'VPC Flow Logs + GuardDuty', assets: ['Kubernetes clusters', 'Managed databases', 'Object storage', 'Serverless functions'], riskLevel: 'medium' as const, lastAssessment: '2026-04-18', nextAssessment: '2026-07-18'}
  ];

  private _x8bcSecurityAlerts = [
    {id: 'x8bc-AL-001', severity: 'critical', title: 'Active ransomware encryption detected on PROD-DB-01', source: 'CrowdStrike', timestamp: '2026-04-22T13:45:00Z', status: 'investigating', assignee: 'SOC Analyst J. Smith', asset: 'PROD-DB-01 (10.2.1.15)', mitreTechnique: 'T1486 - Data Encrypted for Impact', playbook: 'IR-Ransomware-Isolate', sla: '15 minutes', elapsed: '12 minutes', notes: 'EDR detected mass file encryption event pattern. Automated isolation triggered. IR team paged.'},
    {id: 'x8bc-AL-002', severity: 'high', title: 'Impossible travel detected for user admin@company.com', source: 'Okta', timestamp: '2026-04-22T12:30:00Z', status: 'triaged', assignee: 'SOC Analyst M. Johnson', asset: 'Okta - admin@company.com', mitreTechnique: 'T1078 - Valid Accounts', playbook: 'IR-ImpossibleTravel', sla: '30 minutes', elapsed: '87 minutes', notes: 'Login from Tokyo followed by login from New York within 30 minutes. Account temporarily locked. User confirmed via out-of-band channel they are in New York.'},
    {id: 'x8bc-AL-003', severity: 'high', title: 'Public S3 bucket discovered with customer PII exposure', source: 'Prisma Cloud', timestamp: '2026-04-22T11:15:00Z', status: 'remediating', assignee: 'Cloud Security Lead A. Chen', asset: 'AWS S3: customer-data-archive-2024', mitreTechnique: 'N/A - Misconfiguration', playbook: 'IR-Cloud-Misconfig-Remediate', sla: '1 hour', elapsed: '2.5 hours', notes: 'Bucket containing archived customer records was publicly accessible. Bucket blocked and encrypted. Data exposure assessment in progress.'},
    {id: 'x8bc-AL-004', severity: 'medium', title: 'Anomalous data download by user jane.developer@company.com', source: 'Microsoft DLP', timestamp: '2026-04-22T10:00:00Z', status: 'investigating', assignee: 'SOC Analyst R. Davis', asset: 'Endpoint: WS-DEV-042', mitreTechnique: 'T1567 - Exfil Over Web Service', playbook: 'IR-DLP-Investigation', sla: '4 hours', elapsed: '3.5 hours', notes: 'User downloaded 4.2GB of source code files to personal Google Drive. Interview scheduled with user and manager.'},
    {id: 'x8bc-AL-005', severity: 'medium', title: 'Credential stuffing attack detected on customer portal', source: 'Cloudflare WAF', timestamp: '2026-04-22T09:30:00Z', status: 'contained', assignee: 'SOC Analyst K. Wilson', asset: 'Customer Portal (portal.company.com)', mitreTechnique: 'T1110 - Brute Force', playbook: 'IR-CredentialStuffing-Block', sla: '2 hours', elapsed: '4 hours', notes: 'Rate limiting and CAPTCHA automatically deployed. 12,000 failed login attempts from 800 unique IPs blocked. No successful account takeovers confirmed.'},
    {id: 'x8bc-AL-006', severity: 'low', title: 'SSL certificate expiring in 14 days for api.company.com', source: 'Certificate monitoring', timestamp: '2026-04-22T08:00:00Z', status: 'remediated', assignee: 'Infrastructure Lead T. Brown', asset: 'api.company.com', mitreTechnique: 'N/A', playbook: 'Certificate Renewal', sla: '30 days', elapsed: '16 days', notes: 'Automated renewal triggered via ACME. New certificate deployed successfully. No service interruption.'}
  ];

  private _x8bcComplianceFrameworks = [
  ...
  private _x8bcDataClassificationPolicy = [
  ...
  private _getx8bcZoneSummary(): {total: number; highRisk: number; mediumRisk: number; lowRisk: number; totalRules: number} {
    const high = this._x8bcSecurityZones.filter(z => z.riskLevel === 'high').length;
    const med = this._x8bcSecurityZones.filter(z => z.riskLevel === 'medium').length;
    const low = this._x8bcSecurityZones.filter(z => z.riskLevel === 'low').length;
    const rules = this._x8bcSecurityZones.reduce((s, z) => s + z.firewallRules, 0);
    return {total: this._x8bcSecurityZones.length, highRisk: high, mediumRisk: med, lowRisk: low, totalRules: rules};
  }

  private _getx8bcAlertSummary(): {total: number; critical: number; high: number; medium: number; low: number; open: number; contained: number} {
    const crit = this._x8bcSecurityAlerts.filter(a => a.severity === 'critical').length;
    const high = this._x8bcSecurityAlerts.filter(a => a.severity === 'high').length;
    const med = this._x8bcSecurityAlerts.filter(a => a.severity === 'medium').length;
    const low = this._x8bcSecurityAlerts.filter(a => a.severity === 'low').length;
    const open = this._x8bcSecurityAlerts.filter(a => a.status === 'investigating' || a.status === 'triaged').length;
    const contained = this._x8bcSecurityAlerts.filter(a => a.status === 'contained' || a.status === 'remediated').length;
    return {total: this._x8bcSecurityAlerts.length, critical: crit, high, medium: med, low, open, contained};
  }

  private _getx8bcComplianceSummary(): {total: number; fullyCompliant: number; mostlyCompliant: number; avgComplianceRate: number} {
    const full = this._x8bcComplianceFrameworks.filter(f => f.status === 'compliant').length;
    const mostly = this._x8bcComplianceFrameworks.filter(f => f.status === 'mostly_compliant').length;
    const avgRate = this._x8bcComplianceFrameworks.reduce((s, f) => s + (f.compliant / f.requirements * 100), 0) / this._x8bcComplianceFrameworks.length;
    return {total: this._x8bcComplianceFrameworks.length, fullyCompliant: full, mostlyCompliant: mostly, avgComplianceRate: Math.round(avgRate * 10) / 10};
  }
  ...
  // === Security Risk Register (soc_metrics) ===
  private _soc_metricsRiskRegister = [
    { id: "RSK-SOC-0001", title: "Data breach from unpatched systems", owner: "CTO", category: "Financial", status: "mitigating", trend: "stable", severity: "high", likelihood: 2, impact: 5, treatment: "Monitor and review quarterly", lastReview: "2026-04-02", nextReview: "2026-07-28" },
    { id: "RSK-SOC-0002", title: "Ransomware attack on critical infrastructure", owner: "CRO", category: "Strategic", status: "closed", trend: "decreasing", severity: "critical", likelihood: 6, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-04", nextReview: "2026-07-04" },
    { id: "RSK-SOC-0003", title: "Insider threat data exfiltration", owner: "VP Eng", category: "Compliance", status: "accepted", trend: "increasing", severity: "critical", likelihood: 1, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-10", nextReview: "2026-07-04" },
    { id: "RSK-SOC-0004", title: "Supply chain compromise", owner: "Security Lead", category: "Technical", status: "open", trend: "stable", severity: "low", likelihood: 7, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-12", nextReview: "2026-07-14" },
    { id: "RSK-SOC-0005", title: "Cloud misconfiguration exposure", owner: "Risk Mgr", category: "Reputational", status: "mitigating", trend: "decreasing", severity: "critical", likelihood: 8, impact: 9, treatment: "Monitor and review quarterly", lastReview: "2026-04-23", nextReview: "2026-07-06" },
    { id: "RSK-SOC-0006", title: "Phishing campaign success rate", owner: "Compliance Dir", category: "Legal", status: "closed", trend: "increasing", severity: "low", likelihood: 8, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-15", nextReview: "2026-07-17" },
    { id: "RSK-SOC-0007", title: "Third-party vendor data breach", owner: "IT Dir", category: "Regulatory", status: "accepted", trend: "stable", severity: "low", likelihood: 2, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-21", nextReview: "2026-07-14" },
    { id: "RSK-SOC-0008", title: "Regulatory non-compliance penalty", owner: "DevOps Lead", category: "Third-Party", status: "open", trend: "decreasing", severity: "medium", likelihood: 7, impact: 4, treatment: "Monitor and review quarterly", lastReview: "2026-04-18", nextReview: "2026-07-01" },
    { id: "RSK-SOC-0009", title: "Zero-day exploit in production", owner: "Architect", category: "Human Capital", status: "mitigating", trend: "increasing", severity: "low", likelihood: 7, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-18", nextReview: "2026-07-05" },
    { id: "RSK-SOC-0010", title: "Insufficient access controls", owner: "CISO", category: "Operational", status: "closed", trend: "stable", severity: "medium", likelihood: 10, impact: 4, treatment: "Monitor and review quarterly", lastReview: "2026-04-03", nextReview: "2026-07-16" },
    { id: "RSK-SOC-0011", title: "DDoS attack on services", owner: "CTO", category: "Financial", status: "accepted", trend: "decreasing", severity: "low", likelihood: 10, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-06", nextReview: "2026-07-27" },
    { id: "RSK-SOC-0012", title: "Social engineering attack", owner: "CRO", category: "Strategic", status: "open", trend: "increasing", severity: "medium", likelihood: 10, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-11", nextReview: "2026-07-25" },
    { id: "RSK-SOC-0013", title: "API security vulnerability", owner: "VP Eng", category: "Compliance", status: "mitigating", trend: "stable", severity: "critical", likelihood: 10, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-18", nextReview: "2026-07-17" },
    { id: "RSK-SOC-0014", title: "Mobile device compromise", owner: "Security Lead", category: "Technical", status: "closed", trend: "decreasing", severity: "medium", likelihood: 5, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-20", nextReview: "2026-07-08" },
    { id: "RSK-SOC-0015", title: "Physical security breach", owner: "Risk Mgr", category: "Reputational", status: "accepted", trend: "increasing", severity: "low", likelihood: 6, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-14", nextReview: "2026-07-02" },
    { id: "RSK-SOC-0016", title: "Password policy weakness", owner: "Compliance Dir", category: "Legal", status: "open", trend: "stable", severity: "high", likelihood: 8, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-05", nextReview: "2026-07-13" },
    { id: "RSK-SOC-0017", title: "Network segmentation gap", owner: "IT Dir", category: "Regulatory", status: "mitigating", trend: "decreasing", severity: "high", likelihood: 4, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-18", nextReview: "2026-07-12" },
    { id: "RSK-SOC-0018", title: "Encryption key management failure", owner: "DevOps Lead", category: "Third-Party", status: "closed", trend: "increasing", severity: "low", likelihood: 5, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-17", nextReview: "2026-07-01" },
    { id: "RSK-SOC-0019", title: "Audit trail tampering", owner: "Architect", category: "Human Capital", status: "accepted", trend: "stable", severity: "critical", likelihood: 9, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-10", nextReview: "2026-07-22" },
    { id: "RSK-SOC-0020", title: "Business email compromise", owner: "CISO", category: "Operational", status: "open", trend: "decreasing", severity: "low", likelihood: 3, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-23", nextReview: "2026-07-27" },
  ];
  private _soc_metricsRiskFilter: string = 'all';
  private _soc_metricsRiskSeverity: string = 'all';
  private _soc_metricsRiskStatus: string = 'all';
  private _soc_metricsExpandedRisk: string = '';
  private _soc_metricsFilterRisks() {
    const reg = this._soc_metricsRiskRegister;
    return reg.filter(r => {
      if (this._soc_metricsRiskFilter !== "all" && r.category !== this._soc_metricsRiskFilter) return false;
      if (this._soc_metricsRiskSeverity !== "all" && r.severity !== this._soc_metricsRiskSeverity) return false;
      if (this._soc_metricsRiskStatus !== "all" && r.status !== this._soc_metricsRiskStatus) return false;
      return true;
    });
  }
  private _soc_metricsGetRiskScore(r: any) { return Math.round(r.likelihood * r.impact * 1.5); }
  private _soc_metricsGetRiskTrendIcon(trend: string) {
    if (trend === "increasing") return "\u2191";
    if (trend === "decreasing") return "\u2193";
    return "\u2192";
  }
  private _soc_metricsGetRiskColor(sev: string) {
    if (sev === "critical") return "#dc2626";
    if (sev === "high") return "#ea580c";
    if (sev === "medium") return "#d97706";
    return "#16a34a";
  }
  private _soc_metricsGetRiskCounts() {
    const reg = this._soc_metricsRiskRegister;
  ...
  private _soc_metricsGetTreatmentProgress() {
    const reg = this._soc_metricsRiskRegister;
    const treated = reg.filter(r => r.status === "mitigating" || r.status === "closed").length;
    return Math.round((treated / Math.max(reg.length, 1)) * 100);
  }

  // === Security Metrics API Gateway (soc_metrics) ===
  private _soc_metricsApiEndpoints = [
    { method: "GET", path: "/api/v1/threats", name: "Threat Intelligence Feed", status: "active", avgLatency: 342.2, reqPerMin: 1100.0, errorRate: 1.61, uptime: round(99.13508572569461,2), version: "v1.2.15" },
    { method: "POST", path: "/api/v1/scans/start", name: "Vulnerability Scanner", status: "active", avgLatency: 353.1, reqPerMin: 1913.0, errorRate: 2.47, uptime: round(98.73059595586534,2), version: "v3.7.19" },
    { method: "GET", path: "/api/v1/assets", name: "Asset Inventory", status: "active", avgLatency: 224.4, reqPerMin: 650.0, errorRate: 2.07, uptime: round(95.15237174639996,2), version: "v1.6.11" },
    { method: "POST", path: "/api/v1/alerts", name: "Alert Management", status: "active", avgLatency: 109.8, reqPerMin: 815.0, errorRate: 2.4, uptime: round(95.56219340425939,2), version: "v2.7.7" },
    { method: "GET", path: "/api/v1/compliance", name: "Compliance Status", status: "active", avgLatency: 421.5, reqPerMin: 1436.0, errorRate: 1.33, uptime: round(99.83746674030982,2), version: "v2.9.7" },
    { method: "PUT", path: "/api/v1/policies", name: "Policy Engine", status: "active", avgLatency: 352.4, reqPerMin: 2157.0, errorRate: 1.35, uptime: round(99.33657202135093,2), version: "v3.3.11" },
    { method: "GET", path: "/api/v1/incidents", name: "Incident Tracker", status: "active", avgLatency: 71.8, reqPerMin: 2160.0, errorRate: 0.62, uptime: round(97.54809887398089,2), version: "v2.7.2" },
    { method: "POST", path: "/api/v1/forensics", name: "Forensics Collector", status: "degraded", avgLatency: 258.0, reqPerMin: 988.0, errorRate: 1.33, uptime: round(98.27419305309263,2), version: "v3.0.10" },
    { method: "GET", path: "/api/v1/risk/assess", name: "Risk Assessment", status: "active", avgLatency: 246.6, reqPerMin: 2291.0, errorRate: 1.02, uptime: round(99.73033212548692,2), version: "v3.7.15" },
    { method: "POST", path: "/api/v1/auth/verify", name: "Authentication", status: "active", avgLatency: 257.0, reqPerMin: 1386.0, errorRate: 0.31, uptime: round(97.51777510811038,2), version: "v2.1.15" },
    { method: "GET", path: "/api/v1/logs/audit", name: "Audit Log Query", status: "active", avgLatency: 56.5, reqPerMin: 2315.0, errorRate: 0.2, uptime: round(96.90922385793333,2), version: "v3.5.5" },
    { method: "PUT", path: "/api/v1/users/roles", name: "Role Management", status: "active", avgLatency: 408.9, reqPerMin: 670.0, errorRate: 0.12, uptime: round(98.87986009612264,2), version: "v2.7.2" },
    { method: "POST", path: "/api/v1/encrypt", name: "Encryption Service", status: "active", avgLatency: 355.3, reqPerMin: 1995.0, errorRate: 0.68, uptime: round(98.26146296099756,2), version: "v1.0.7" },
    { method: "GET", path: "/api/v1/network/topo", name: "Network Topology", status: "maintenance", avgLatency: 156.0, reqPerMin: 2337.0, errorRate: 1.19, uptime: round(95.50655105599375,2), version: "v1.3.0" },
    { method: "DELETE", path: "/api/v1/sessions", name: "Session Manager", status: "active", avgLatency: 398.8, reqPerMin: 2203.0, errorRate: 0.65, uptime: round(97.65256502469282,2), version: "v3.9.8" },
  ];
  private _soc_metricsApiKeys = [
    { id: "ak-000001", name: "key-soc_-001", created: "2026-09-11", lastUsed: "2026-04-04", status: "revoked", calls: 9368, rateLimit: 500 },
    { id: "ak-000002", name: "key-soc_-002", created: "2026-01-02", lastUsed: "2026-04-13", status: "active", calls: 13804, rateLimit: 5000 },
    { id: "ak-000003", name: "key-soc_-003", created: "2026-02-15", lastUsed: "2026-04-07", status: "active", calls: 33418, rateLimit: 500 },
    { id: "ak-000004", name: "key-soc_-004", created: "2026-02-05", lastUsed: "2026-04-22", status: "active", calls: 13467, rateLimit: 500 },
    { id: "ak-000005", name: "key-soc_-005", created: "2026-05-16", lastUsed: "2026-04-04", status: "active", calls: 43714, rateLimit: 5000 },
    { id: "ak-000006", name: "key-soc_-006", created: "2026-04-27", lastUsed: "2026-04-05", status: "active", calls: 47064, rateLimit: 500 },
    { id: "ak-000007", name: "key-soc_-007", created: "2026-08-06", lastUsed: "2026-04-05", status: "active", calls: 33211, rateLimit: 500 },
    { id: "ak-000008", name: "key-soc_-008", created: "2026-04-09", lastUsed: "2026-04-15", status: "active", calls: 33971, rateLimit: 100 },
  ];
  private _soc_metricsApiHealthSummary() {
    const eps = this._soc_metricsApiEndpoints;
  ...
  private _soc_metricsGetApiByMethod(method: string) { return this._soc_metricsApiEndpoints.filter(e=>e.method===method); }
  private _soc_metricsGetSlowEndpoints() { return this._soc_metricsApiEndpoints.filter(e=>e.avgLatency>200).sort((a,b)=>b.avgLatency-a.avgLatency); }
  private _soc_metricsGetHighErrorEndpoints() { return this._soc_metricsApiEndpoints.filter(e=>e.errorRate>1.0).sort((a,b)=>b.errorRate-a.errorRate); }

  // === Security Training Effectiveness (soc_metrics) ===
  private _soc_metricsTrainingModules = [
    { id: "TRN-001", name: "Security Fundamentals", completionRate: 91.7, avgScore: 66.0, behaviorChange: 78.3, enrolled: 242, completed: 104, duration: "94min", category: "mandatory" },
    { id: "TRN-002", name: "Phishing Awareness", completionRate: 85.7, avgScore: 86.7, behaviorChange: 72.2, enrolled: 206, completed: 307, duration: "104min", category: "mandatory" },
    { id: "TRN-003", name: "Social Engineering Defense", completionRate: 63.7, avgScore: 77.5, behaviorChange: 75.7, enrolled: 339, completed: 278, duration: "86min", category: "optional" },
    { id: "TRN-004", name: "Password Hygiene", completionRate: 74.3, avgScore: 61.9, behaviorChange: 77.0, enrolled: 401, completed: 191, duration: "96min", category: "mandatory" },
    { id: "TRN-005", name: "Data Classification", completionRate: 75.8, avgScore: 79.8, behaviorChange: 40.2, enrolled: 181, completed: 318, duration: "54min", category: "mandatory" },
    { id: "TRN-006", name: "Incident Response Basics", completionRate: 46.9, avgScore: 94.0, behaviorChange: 42.9, enrolled: 416, completed: 378, duration: "87min", category: "mandatory" },
    { id: "TRN-007", name: "Secure Coding", completionRate: 94.1, avgScore: 89.3, behaviorChange: 49.3, enrolled: 70, completed: 205, duration: "84min", category: "optional" },
    { id: "TRN-008", name: "Cloud Security", completionRate: 91.6, avgScore: 78.1, behaviorChange: 34.1, enrolled: 223, completed: 159, duration: "102min", category: "mandatory" },
    { id: "TRN-009", name: "Mobile Device Security", completionRate: 81.5, avgScore: 64.4, behaviorChange: 88.7, enrolled: 302, completed: 449, duration: "18min", category: "mandatory" },
    { id: "TRN-010", name: "Network Security", completionRate: 47.0, avgScore: 58.7, behaviorChange: 59.6, enrolled: 450, completed: 148, duration: "108min", category: "mandatory" },
    { id: "TRN-011", name: "Physical Security", completionRate: 83.4, avgScore: 66.2, behaviorChange: 71.0, enrolled: 331, completed: 373, duration: "89min", category: "optional" },
    { id: "TRN-012", name: "Regulatory Compliance", completionRate: 69.9, avgScore: 59.4, behaviorChange: 33.2, enrolled: 362, completed: 112, duration: "117min", category: "mandatory" },
    { id: "TRN-013", name: "Risk Management", completionRate: 91.9, avgScore: 81.8, behaviorChange: 73.0, enrolled: 465, completed: 52, duration: "94min", category: "mandatory" },
    { id: "TRN-014", name: "Cryptography Basics", completionRate: 74.8, avgScore: 82.4, behaviorChange: 50.6, enrolled: 109, completed: 102, duration: "77min", category: "mandatory" },
    { id: "TRN-015", name: "Access Control", completionRate: 60.9, avgScore: 55.3, behaviorChange: 85.5, enrolled: 422, completed: 307, duration: "54min", category: "optional" },
    { id: "TRN-016", name: "Vendor Management", completionRate: 79.7, avgScore: 70.7, behaviorChange: 74.7, enrolled: 229, completed: 315, duration: "55min", category: "mandatory" },
  ];
  private _soc_metricsPhishingResults = [
    { month: "2026-01", sent: 767, clicked: 63, reported: 438, clickRate: round(8.213820078226858,1), reportRate: round(57.10560625814863,1) },
    { month: "2026-02", sent: 396, clicked: 44, reported: 18, clickRate: round(11.11111111111111,1), reportRate: round(4.545454545454546,1) },
    { month: "2026-03", sent: 875, clicked: 67, reported: 309, clickRate: round(7.657142857142857,1), reportRate: round(35.31428571428572,1) },
    { month: "2026-04", sent: 376, clicked: 104, reported: 123, clickRate: round(27.659574468085108,1), reportRate: round(32.71276595744681,1) },
    { month: "2026-05", sent: 858, clicked: 109, reported: 214, clickRate: round(12.703962703962704,1), reportRate: round(24.941724941724942,1) },
    { month: "2026-06", sent: 851, clicked: 165, reported: 381, clickRate: round(19.388954171562865,1), reportRate: round(44.770857814336075,1) },
  ];
  private _soc_metricsTrainingROI = { totalInvestment: 132262, avgCostPerEmployee: 779, riskReductionPct: round(random.uniform(15,45),1), incidentReductionPct: round(random.uniform(10,35),1), complianceScoreGain: round(random.uniform(5,25),1) };
  private _soc_metricsLearningPaths = [
    { name: "Beginner Security Analyst", totalModules: 17, completedModules: 5, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 60, enrolled: 108 },
    { name: "Advanced Threat Hunter", totalModules: 18, completedModules: 11, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 66, enrolled: 101 },
    { name: "Security Architect", totalModules: 9, completedModules: 4, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 21, enrolled: 177 },
    { name: "Incident Responder", totalModules: 9, completedModules: 11, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 54, enrolled: 119 },
    { name: "Compliance Specialist", totalModules: 18, completedModules: 2, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 70, enrolled: 118 },
    { name: "DevSecOps Engineer", totalModules: 8, completedModules: 15, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 71, enrolled: 168 },
  ];
  private _soc_metricsGetOverallCompletion() {
    const mods = this._soc_metricsTrainingModules;
    return round(mods.reduce((s,m)=>s+m.completionRate,0)/mods.length,1);
  }
  private _soc_metricsGetTopPerformers() { return [...this._soc_metricsTrainingModules].sort((a,b)=>b.avgScore-a.avgScore).slice(0,5); }
  private _soc_metricsGetModulesNeedingAttention() { return this._soc_metricsTrainingModules.filter(m=>m.completionRate<70||m.avgScore<65); }

  // === Security Governance Framework (soc_metrics) ===
  private _soc_metricsGovBodies = [
    { name: "Security Steering Committee", members: 12, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-22", nextMeeting: "2026-05-07", quorum: 4 },
    { name: "Risk Management Board", members: 13, chair: "CTO", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-05", nextMeeting: "2026-05-20", quorum: 8 },
    { name: "Data Governance Council", members: 15, chair: "CRO", meetingFreq: "Monthly", lastMeeting: "2026-04-15", nextMeeting: "2026-05-03", quorum: 3 },
    { name: "Compliance Oversight Board", members: 9, chair: "CDO", meetingFreq: "Monthly", lastMeeting: "2026-04-02", nextMeeting: "2026-05-13", quorum: 5 },
    { name: "Architecture Review Board", members: 8, chair: "VP Eng", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-20", nextMeeting: "2026-05-28", quorum: 6 },
    { name: "Change Advisory Board", members: 10, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-07", nextMeeting: "2026-05-15", quorum: 5 },
    { name: "Incident Review Board", members: 9, chair: "CIO", meetingFreq: "As needed", lastMeeting: "2026-04-11", nextMeeting: "2026-05-24", quorum: 8 },
    { name: "Vendor Risk Committee", members: 15, chair: "CFO", meetingFreq: "Quarterly", lastMeeting: "2026-04-08", nextMeeting: "2026-05-01", quorum: 7 },
  ];
  private _soc_metricsDecisions = [
    { id: "DEC-0001", title: "Adopt zero-trust architecture framework", date: "2026-01-11", status: "approved", owner: "CISO" },
    { id: "DEC-0002", title: "Migrate to SIEM 2.0 platform", date: "2026-01-28", status: "implemented", owner: "CTO" },
    ...
    { id: "DEC-0006", title: "Deploy EDR solution enterprise-wide", date: "2026-01-03", status: "approved", owner: "CISO" },
    { id: "DEC-0007", title: "Conduct annual penetration testing", date: "2026-02-25", status: "implemented", owner: "CTO" },
    { id: "DEC-0008", title: "Implement network micro-segmentation", date: "2026-02-22", status: "in-progress", owner: "Security Lead" },
    { id: "DEC-0009", title: "Establish security champion program", date: "2026-02-10", status: "pending", owner: "CRO" },
    { id: "DEC-0010", title: "Migrate to passwordless authentication", date: "2026-02-21", status: "approved", owner: "VP Eng" },
  ];
  private _soc_metricsPolicyLifecycle = [
    { name: "Information Security Policy", version: "v4.5", status: "active", lastUpdated: "2026-03-15", nextReview: "2026-7-06", owner: "CISO" },
    { name: "Acceptable Use Policy", version: "v2.2", status: "active", lastUpdated: "2026-03-02", nextReview: "2026-7-27", owner: "Legal" },
    { name: "Data Retention Policy", version: "v3.5", status: "under-review", lastUpdated: "2026-02-10", nextReview: "2026-7-06", owner: "DPO" },
    { name: "Access Control Policy", version: "v1.3", status: "active", lastUpdated: "2026-04-26", nextReview: "2026-7-13", owner: "IAM Lead" },
    { name: "Incident Response Policy", version: "v3.1", status: "draft", lastUpdated: "2026-01-15", nextReview: "2026-9-01", owner: "IR Lead" },
    { name: "Business Continuity Plan", version: "v3.3", status: "active", lastUpdated: "2026-02-13", nextReview: "2026-9-05", owner: "BCP Mgr" },
    { name: "Vendor Management Policy", version: "v4.5", status: "active", lastUpdated: "2026-04-11", nextReview: "2026-8-28", owner: "Procurement" },
    { name: "Encryption Standard", version: "v4.0", status: "under-review", lastUpdated: "2026-02-02", nextReview: "2026-8-24", owner: "Security Arch" },
  ];
  private _soc_metricsGovMaturityScore = { overall: 3.3, riskManagement: 5.6, compliance: 3.5, incidentResponse: 5.0, awareness: 3.7, technology: 3.3 };
  private _soc_metricsGetPendingDecisions() { return this._soc_metricsDecisions.filter(d=>d.status==='pending'||d.status==='in-progress'); }
  private _soc_metricsGetActivePolicies() { return this._soc_metricsPolicyLifecycle.filter(p=>p.status==='active'); }
  private _soc_metricsGetEscalationPath() { return ["L1 Analyst","L2 Senior","Security Lead","CISO","Board"]; }

  // === Security Innovation Lab (soc_metrics) ===
  private _soc_metricsInnoProjects = [
    { id: "INN-001", name: "AI-Powered Threat Detection", description: "Machine learning models for real-time threat identification", status: "active", progress: 72, startDate: "2026-03-16", teamSize: 4, budget: 145048, milestones: 5, completedMilestones: 3 },
    { id: "INN-002", name: "Quantum-Resistant Cryptography", description: "Post-quantum encryption algorithm prototyping", status: "research", progress: 35, startDate: "2026-03-05", teamSize: 2, budget: 105233, milestones: 4, completedMilestones: 7 },
    { id: "INN-003", name: "Automated Red Teaming", description: "Autonomous penetration testing framework", status: "active", progress: 58, startDate: "2026-01-04", teamSize: 2, budget: 181593, milestones: 5, completedMilestones: 6 },
    { id: "INN-004", name: "Zero-Knowledge Authentication", description: "Privacy-preserving identity verification", status: "poc", progress: 88, startDate: "2026-02-26", teamSize: 2, budget: 146216, milestones: 7, completedMilestones: 6 },
    { id: "INN-005", name: "Blockchain Audit Trail", description: "Immutable security event logging", status: "active", progress: 45, startDate: "2026-01-08", teamSize: 2, budget: 27625, milestones: 6, completedMilestones: 4 },
    { id: "INN-006", name: "Behavioral Biometrics", description: "Continuous authentication via user behavior patterns", status: "research", progress: 22, startDate: "2026-01-11", teamSize: 6, budget: 102655, milestones: 5, completedMilestones: 1 },
    { id: "INN-007", name: "Deception Grid 2.0", description: "Advanced honeypot network with adaptive responses", status: "active", progress: 65, startDate: "2026-01-17", teamSize: 7, budget: 166720, milestones: 9, completedMilestones: 8 },
    { id: "INN-008", name: "Secure Enclave Integration", description: "Hardware-backed security for critical workloads", status: "poc", progress: 40, startDate: "2026-03-13", teamSize: 6, budget: 52250, milestones: 6, completedMilestones: 4 },
  ];
  private _soc_metricsTechEvaluations = [
    ...
    { name: "Homomorphic Encryption", status: "evaluating", score: 10.3, recommendation: "Pilot", vendor: "GCP" },
  ...
  private _soc_metricsCollaborationPartners = [
  ...
  private _soc_metricsInnoMetrics = { totalProjects: 8, activeProjects: 4, avgTimeToValue: "123 days", pocSuccessRate: round(random.uniform(55,85),1), researchToProduction: round(random.uniform(20,50),1), innovationIndex: round(random.uniform(6.0,9.5),1) };
  private _soc_metricsGetProjectByStatus(status: string) { return this._soc_metricsInnoProjects.filter(p=>p.status===status); }
  private _soc_metricsGetTopEvaluations() { return [...this._soc_metricsTechEvaluations].sort((a,b)=>b.score-a.score).slice(0,3); }



  // === Compliance Dashboard Extension (soc_metrics) ===
  private _soc_metricsComplianceFrameworks = [
    { name: "SOC 2 Type II", description: "Trust Services Criteria", totalControls: 5, implementedControls: 3, status: "in-review", lastAudit: "2026-04-17", nextAudit: "2026-10-16", evidenceCount: 134 },
    { name: "ISO 27001", description: "Information Security Management", totalControls: 114, implementedControls: 63, status: "partially-compliant", lastAudit: "2026-04-26", nextAudit: "2026-10-24", evidenceCount: 240 },
    { name: "PCI DSS 4.0", description: "Payment Card Industry", totalControls: 12, implementedControls: 7, status: "partially-compliant", lastAudit: "2026-04-17", nextAudit: "2026-7-12", evidenceCount: 456 },
    { name: "HIPAA", description: "Health Insurance Portability", totalControls: 18, implementedControls: 13, status: "in-review", lastAudit: "2026-03-26", nextAudit: "2026-11-12", evidenceCount: 271 },
    { name: "GDPR", description: "Data Protection Regulation", totalControls: 99, implementedControls: 90, status: "in-review", lastAudit: "2026-02-18", nextAudit: "2026-10-16", evidenceCount: 340 },
    { name: "NIST CSF 2.0", description: "Cybersecurity Framework", totalControls: 6, implementedControls: 5, status: "partially-compliant", lastAudit: "2026-02-05", nextAudit: "2026-7-18", evidenceCount: 292 },
    { name: "FedRAMP", description: "Federal Risk Authorization", totalControls: 15, implementedControls: 15, status: "in-review", lastAudit: "2026-02-26", nextAudit: "2026-7-13", evidenceCount: 420 },
    { name: "SOX", description: "Sarbanes-Oxley Compliance", totalControls: 8, implementedControls: 4, status: "partially-compliant", lastAudit: "2026-04-25", nextAudit: "2026-11-13", evidenceCount: 153 },
    { name: "CIS Controls v8", description: "Center for Internet Security", totalControls: 18, implementedControls: 16, status: "in-review", lastAudit: "2026-01-22", nextAudit: "2026-8-18", evidenceCount: 387 },
    { name: "COBIT 2019", description: "IT Governance Framework", totalControls: 40, implementedControls: 28, status: "compliant", lastAudit: "2026-02-12", nextAudit: "2026-11-18", evidenceCount: 476 },
  ];
  private _soc_metricsGetComplianceScore() {
    const fw = this._soc_metricsComplianceFrameworks;
    return round(fw.reduce((s,f)=>s + (f.implementedControls/Math.max(f.totalControls,1))*100, 0) / fw.length, 1);
  }
  private _soc_metricsGetGaps() {
    return this._soc_metricsComplianceFrameworks.filter(f => f.status !== "compliant");
  }
  private _soc_metricsAuditTrail = [
    { id: "AUD-0001", action: "Control tested", auditor: "Internal Audit", date: "2026-04-05", result: "pass", findings: 1 },
    { id: "AUD-0002", action: "Evidence collected", auditor: "External Auditor", date: "2026-04-13", result: "pass", findings: 3 },
    { id: "AUD-0003", action: "Gap identified", auditor: "Security Team", date: "2026-04-19", result: "fail", findings: 0 },
    { id: "AUD-0004", action: "Remediation completed", auditor: "Compliance Officer", date: "2026-04-08", result: "pass", findings: 2 },
    { id: "AUD-0005", action: "Policy updated", auditor: "IT Audit", date: "2026-04-15", result: "pass", findings: 0 },
    { id: "AUD-0006", action: "Training verified", auditor: "Risk Team", date: "2026-04-22", result: "pass", findings: 1 },
    { id: "AUD-0007", action: "Access reviewed", auditor: "QA Team", date: "2026-04-22", result: "pass", findings: 3 },
    { id: "AUD-0008", action: "Exception approved", auditor: "CISO Office", date: "2026-04-04", result: "conditional", findings: 5 },
    { id: "AUD-0009", action: "Risk accepted", auditor: "Board Audit", date: "2026-04-06", result: "pass", findings: 3 },
    { id: "AUD-0010", action: "Control enhanced", auditor: "Third Party", date: "2026-04-17", result: "pass", findings: 5 },
  ];

  // === Threat Intelligence Feed Extension (soc_metrics) ===
  private _soc_metricsThreatActors = [
    { name: "APT-29", alias: "Cozy Bear", origin: "Russia", type: "Nation-State", severity: "high", lastActivity: "2026-04-23", targets: "Government", indicators: 134, ttps: 42 },
    { name: "APT-41", alias: "Double Dragon", origin: "China", type: "Nation-State", severity: "critical", lastActivity: "2026-04-03", targets: "Manufacturing", indicators: 83, ttps: 38 },
    { name: "Lazarus Group", alias: "Hidden Cobra", origin: "North Korea", type: "Nation-State", severity: "critical", lastActivity: "2026-04-03", targets: "Energy", indicators: 470, ttps: 13 },
    { name: "FIN7", alias: "Carbanak", origin: "Eastern Europe", type: "Financial", severity: "high", lastActivity: "2026-04-11", targets: "Technology", indicators: 378, ttps: 14 },
    { name: "Conti", alias: "Wizard Spider", origin: "Russia", type: "Ransomware", severity: "critical", lastActivity: "2026-04-09", targets: "Healthcare", indicators: 24, ttps: 35 },
    { name: "LockBit", alias: "LockBit Gang", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-07", targets: "Energy", indicators: 385, ttps: 15 },
    { name: "Cl0p", alias: "Cl0p Team", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-19", targets: "Finance", indicators: 109, ttps: 6 },
    { name: "Sandworm", alias: "Unit 74455", origin: "Russia", type: "Nation-State", severity: "critical", lastActivity: "2026-04-13", targets: "Healthcare", indicators: 402, ttps: 23 },
  ];
  private _soc_metricsIoCFeed = [
    { id: "ioc-000001", type: "ip", value: "109.147.26.124", confidence: 46, source: "CrowdStrike", firstSeen: "2026-04-03", lastSeen: "2026-04-06" },
    { id: "ioc-000002", type: "ip", value: "155.218.220.6", confidence: 44, source: "AlienVault", firstSeen: "2026-04-14", lastSeen: "2026-04-01" },
    { id: "ioc-000003", type: "ip", value: "183.155.24.151", confidence: 90, source: "STIX", firstSeen: "2026-04-13", lastSeen: "2026-04-14" },
    { id: "ioc-000004", type: "ip", value: "134.228.33.22", confidence: 57, source: "AlienVault", firstSeen: "2026-04-06", lastSeen: "2026-04-03" },
    { id: "ioc-000005", type: "ip", value: "205.247.55.137", confidence: 87, source: "CrowdStrike", firstSeen: "2026-04-17", lastSeen: "2026-04-16" },
    { id: "ioc-000006", type: "ip", value: "211.11.81.185", confidence: 46, source: "AlienVault", firstSeen: "2026-04-21", lastSeen: "2026-04-19" },
    { id: "ioc-000007", type: "ip", value: "150.74.147.98", confidence: 85, source: "STIX", firstSeen: "2026-04-09", lastSeen: "2026-04-05" },
    { id: "ioc-000008", type: "ip", value: "98.20.176.48", confidence: 51, source: "MISP", firstSeen: "2026-04-04", lastSeen: "2026-04-19" },
    { id: "ioc-000009", type: "ip", value: "64.93.24.182", confidence: 41, source: "Mandiant", firstSeen: "2026-04-12", lastSeen: "2026-04-01" },
    { id: "ioc-000010", type: "ip", value: "61.209.23.195", confidence: 82, source: "AlienVault", firstSeen: "2026-04-08", lastSeen: "2026-04-23" },
    { id: "ioc-000011", type: "ip", value: "162.69.250.94", confidence: 42, source: "CrowdStrike", firstSeen: "2026-04-10", lastSeen: "2026-04-21" },
    { id: "ioc-000012", type: "ip", value: "59.15.22.75", confidence: 97, source: "CrowdStrike", firstSeen: "2026-04-08", lastSeen: "2026-04-13" },
    { id: "ioc-000013", type: "ip", value: "61.108.119.160", confidence: 78, source: "Mandiant", firstSeen: "2026-04-05", lastSeen: "2026-04-08" },
    { id: "ioc-000014", type: "ip", value: "124.164.69.71", confidence: 64, source: "Mandiant", firstSeen: "2026-04-17", lastSeen: "2026-04-17" },
    { id: "ioc-000015", type: "ip", value: "8.213.124.164", confidence: 40, source: "CrowdStrike", firstSeen: "2026-04-02", lastSeen: "2026-04-13" },
  ];
  private _soc_metricsGetActiveThreats() { return this._soc_metricsThreatActors.filter(a => a.severity === 'critical'); }
  private _soc_metricsGetThreatSummary() {
    const actors = this._soc_metricsThreatActors;
  ...
  // === Incident Management Extension (soc_metrics) ===
  private _soc_metricsIncidents = [
    { id: "INC-20260001", title: "Unauthorized access detected", severity: "critical", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-16T23:42", affectedAssets: 39, rootCause: "Misconfiguration" },
    { id: "INC-20260002", title: "Malware outbreak on workstation", severity: "high", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-05T12:33", affectedAssets: 38, rootCause: "Credential compromise" },
    { id: "INC-20260003", title: "Data leak from S3 bucket", severity: "medium", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-07T22:31", affectedAssets: 43, rootCause: "Zero-day" },
    { id: "INC-20260004", title: "Phishing campaign targeting finance", severity: "low", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-23T11:15", affectedAssets: 37, rootCause: "Human error" },
    { id: "INC-20260005", title: "DDoS attack on web services", severity: "critical", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-19T20:35", affectedAssets: 13, rootCause: "Policy violation" },
    { id: "INC-20260006", title: "Ransomware encryption attempt", severity: "high", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-03T14:03", affectedAssets: 8, rootCause: "Unknown" },
    { id: "INC-20260007", title: "Insider data exfiltration", severity: "medium", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-02T17:07", affectedAssets: 10, rootCause: "Misconfiguration" },
    { id: "INC-20260008", title: "API key exposure in repo", severity: "low", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-03T06:08", affectedAssets: 22, rootCause: "Credential compromise" },
    { id: "INC-20260009", title: "SQL injection on portal", severity: "critical", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-21T19:32", affectedAssets: 4, rootCause: "Zero-day" },
    { id: "INC-20260010", title: "Brute force on VPN", severity: "high", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-21T01:24", affectedAssets: 18, rootCause: "Human error" },
    { id: "INC-20260011", title: "Supply chain alert from vendor", severity: "medium", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-06T22:42", affectedAssets: 48, rootCause: "Policy violation" },
    { id: "INC-20260012", title: "Suspicious lateral movement", severity: "low", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-01T13:07", affectedAssets: 32, rootCause: "Unknown" },
  ];
  private _soc_metricsGetIncidentStats() {
    const inc = this._soc_metricsIncidents;
    return { total: inc.length, open: inc.filter(i=>i.status==="open").length, investigating: inc.filter(i=>i.status==="investigating").length, mttd: 8, mttr: 118 };
  }
  private _soc_metricsGetSeverityDistribution() {
    const inc = this._soc_metricsIncidents;
  ...
  private _initSocMetricsInvestmentHeatmap() {
    this._soc_metricsInvestmentAreas = [
    ...
    this._soc_metricsInvestmentRebalance = this._recommendSocMetricsRebalance();
    this._soc_metricsInvestmentROI = this._calcSocMetricsInvestmentROI();
  }

  private _recommendSocMetricsRebalance(): Array<Record<string, unknown>> {
    const totalBudget = this._soc_metricsInvestmentAreas.reduce((sum: number, a: Record<string, unknown>) => sum + Number(a.budget || 0), 0);
    return this._soc_metricsInvestmentAreas
  ...
  private _calcSocMetricsInvestmentROI(): Array<Record<string, unknown>> {
    return this._soc_metricsInvestmentAreas.map((a: Record<string, unknown>) => ({
  ...
  private _initSocMetricsSocIntelHub() {
    this._soc_metricsThreatActors = [
    ...
    this._soc_metricsTtpMapping = this._buildSocMetricsTtpMatrix();
    this._soc_metricsThreatCampaigns = this._trackSocMetricsActiveCampaigns();
    this._soc_metricsIntelFeeds = this._configureSocMetricsIntelFeeds();
    this._soc_metricsThreatScores = this._calcSocMetricsThreatScores();
    this._soc_metricsPredictiveAnalysis = this._runSocMetricsPredictiveThreatModel();
  }

  private _buildSocMetricsTtpMatrix(): Array<Record<string, unknown>> {
    const techniques = [
  ...
  private _trackSocMetricsActiveCampaigns(): Array<Record<string, unknown>> {
    return [
  ...
  private _configureSocMetricsIntelFeeds(): Array<Record<string, unknown>> {
    return [
  ...
  private _calcSocMetricsThreatScores(): Array<Record<string, unknown>> {
    return this._soc_metricsThreatActors.map((actor: Record<string, unknown>) => ({
  ...
  private _runSocMetricsPredictiveThreatModel(): Array<Record<string, unknown>> {
    return [
  ...
  private _initSocMetricsWorkflowEngine() {
    this._soc_metricsWorkflows = [
    ...
    this._soc_metricsWorkflowMetrics = this._aggregateSocMetricsWorkflowMetrics();
    this._soc_metricsWorkflowBottlenecks = this._identifySocMetricsBottlenecks();
    this._soc_metricsAutomationROI = this._calcSocMetricsAutomationROI();
  }

  private _aggregateSocMetricsWorkflowMetrics(): Record<string, unknown> {
    const totalRuns = this._soc_metricsWorkflows.reduce((s: number, w: Record<string, unknown>) => s + Number(w.totalRuns), 0);
    const avgSuccess = this._soc_metricsWorkflows.reduce((s: number, w: Record<string, unknown>) => s + Number(w.successRate), 0) / this._soc_metricsWorkflows.length;
    const totalAutoSteps = this._soc_metricsWorkflows.reduce((s: number, w: Record<string, unknown>) => s + Number(w.autoSteps), 0);
    const totalManualSteps = this._soc_metricsWorkflows.reduce((s: number, w: Record<string, unknown>) => s + Number(w.manualSteps), 0);
    return {
      ...
      activeWorkflows: this._soc_metricsWorkflows.filter((w: Record<string, unknown>) => w.active).length,
      avgStepsPerWorkflow: Math.round((totalAutoSteps + totalManualSteps) / this._soc_metricsWorkflows.length)
    };
  }

  private _identifySocMetricsBottlenecks(): Array<Record<string, unknown>> {
    return [
  ...
  private _calcSocMetricsAutomationROI(): Array<Record<string, unknown>> {
    return this._soc_metricsWorkflows.map((w: Record<string, unknown>) => ({
  ...
  private _runSocMetricsPostureAssessment() {
    this._soc_metricsPostureDimensions = [
    ...
    this._soc_metricsOverallPostureScore = this._soc_metricsPostureDimensions.reduce(
      (s: number, d: Record<string, unknown>) => s + Number(d.score) * Number(d.weight), 0
    );
    this._soc_metricsPostureHistory = Array.from({ length: 30 }, (_, i) => ({
    ...
    this._soc_metricsPostureRecommendations = this._generateSocMetricsPostureRecommendations();
  }

  private _generateSocMetricsPostureRecommendations(): Array<Record<string, unknown>> {
    return [
  ...
  private _initSocMetricsRiskQuantification() {
    this._soc_metricsRiskScenarios = [
    ...
    this._soc_metricsRiskQuantification = this._calcSocMetricsFinancialRisk();
    this._soc_metricsScenarioHeatmap = this._buildSocMetricsScenarioHeatmap();
    this._soc_metricsRiskMitigationPlans = this._generateSocMetricsMitigationPlans();
    this._soc_metricsMonteCarloResults = this._runSocMetricsMonteCarloSimulation();
  }

  private _calcSocMetricsFinancialRisk(): Array<Record<string, unknown>> {
    return this._soc_metricsRiskScenarios.map((s: Record<string, unknown>) => ({
  ...
  private _buildSocMetricsScenarioHeatmap(): Array<Record<string, unknown>> {
    return this._soc_metricsRiskScenarios.map((s: Record<string, unknown>) => ({
      scenarioId: s.scenarioId, scenarioName: s.name,
  ...
  private _generateSocMetricsMitigationPlans(): Array<Record<string, unknown>> {
    return this._soc_metricsRiskScenarios.filter((s: Record<string, unknown>) => String(s.residualRisk) !== 'low').map((s: Record<string, unknown>) => ({
      scenarioId: s.scenarioId, scenarioName: s.name,
  ...
  private _runSocMetricsMonteCarloSimulation(): Record<string, unknown> {
    ...
    for (let i = 0; i < iterations; i++) {
      let annualLoss = 0;
      for (const s of this._soc_metricsRiskScenarios) {
    ...
    return {
      ...
      probabilityOfBreach: Math.round(this._soc_metricsRiskScenarios.reduce((s: number, r: Record<string, unknown>) => s + Number(r.likelihood), 0) * 100) / this._soc_metricsRiskScenarios.length
    };
  }

  // === Security Incident Cost Modeling ===
  private _initSocMetricsIncidentCostModel() {
    this._soc_metricsIncidentCostCategories = [
    ...
    this._soc_metricsTotalIncidentCost = this._soc_metricsIncidentCostCategories.reduce((s: number, c: Record<string, unknown>) => s + Number(c.avgCost), 0);
    this._soc_metricsCostBySeverity = this._modelSocMetricsCostBySeverity();
  }

  private _modelSocMetricsCostBySeverity(): Array<Record<string, unknown>> {
    return [
    ...
    return html`
      <div class="panel">
        ...
        ${this._activeTab === 'rules' ? html`
          ${this._renderSocRules()}
          <div style="margin-top:12px;font-size:11px;font-weight:600;margin-bottom:6px">Category Treemap</div>
          <div style="background:#1a1d27;border-radius:8px;padding:12px">${this._renderSocTreemapSVG()}</div>
          <div style="margin-top:12px;font-size:11px;font-weight:600;margin-bottom:6px">Data Flow (Sankey)</div>
          <div style="background:#1a1d27;border-radius:8px;padding:12px">${this._renderSocSankeySVG()}</div>
          ${this._renderSocActivity()}
        ` : nothing}
        ${this._activeTab === 'anomalies' ? html`
          ${this._renderSocAnomalies()}
          ${this._renderSocNotifications()}
        ` : nothing}
        ${this._activeTab === 'predictions' ? html`
          ${this._renderSocPredictions()}
          ...
          <div style="background:#1a1d27;border-radius:8px;padding:12px">
            <table class="history-table"><thead><tr><th>ID</th><th>Base</th><th>Temporal</th><th>Environmental</th><th>Overall</th></tr></thead><tbody>
              ${this._soccvssData.map(c => html`<tr><td>${c.itemId}</td><td style="color:${c.base >= 7 ? '#ef4444' : c.base >= 4 ? '#eab308' : '#22c55e'}">${c.base}</td><td>${c.temporal}</td><td>${c.environmental}</td><td style="font-weight:700;color:${c.overall >= 7 ? '#ef4444' : c.overall >= 4 ? '#eab308' : '#22c55e'}">${c.overall}</td></tr>`)}
            </tbody></table>
          </div>
        ` : nothing}
        ${this._activeTab === 'approvals' ? html`
          ${this._renderSocApprovals()}
        ` : nothing}
        ${this._activeTab === 'config' ? html`
          ${this._renderSocConfig()}
        ` : nothing}
        ${this._activeTab === 'new' ? html`
          <div class="form-section">
            <div class="form-title">Create New Soc Metrics Entry</div>
            <div class="form-grid">
  ...
  private _renderAdvancedGrid(): any {
    ...
    return html`
      <div style="background:#1f2937;border-radius:8px;padding:14px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
          <span style="font-weight:600;font-size:12px;color:#9ca3af;text-transform:uppercase">Soc Metrics Findings Grid</span>
          <div style="display:flex;gap:4px">
  ...
  private _initThreatModel() {
    const categories = [
      ...
      { id: 'tc2', category: 'Tampering', threats: [
        { id: 't4', name: 'Data injection in SOC operations', stride: 'T', likelihood: 7, impact: 8, dreadScore: 7.0, status: 'open', mitigations: ['Input validation', 'WAF rules'], assignedTo: 'Dev Team', discoveredDate: '2024-01-08', lastReviewed: '2024-02-10' },
...
// ===== SECURITY CONTROL TESTING (R22) =====
  ...
  private _renderBudgetPlanning() {
    const budgetData = [
      { category: "Personnel & Training", planned: 2566000, actual: 4010000, utilization: 156.3, q1: "28%", q2: "33%", q3: "31%", q4: "23%" },
      { category: "Tooling & Licensing", planned: 2573000, actual: 4023000, utilization: 156.4, q1: "15%", q2: "22%", q3: "26%", q4: "24%" },
      { category: "Infrastructure Security", planned: 2580000, actual: 4036000, utilization: 156.4, q1: "18%", q2: "27%", q3: "21%", q4: "25%" },
      { category: "Compliance & Audit", planned: 2587000, actual: 4049000, utilization: 156.5, q1: "21%", q2: "32%", q3: "32%", q4: "10%" },
      { category: "Incident Response", planned: 2594000, actual: 4062000, utilization: 156.6, q1: "24%", q2: "21%", q3: "27%", q4: "11%" },
      { category: "Third-Party Assessments", planned: 2601000, actual: 4075000, utilization: 156.7, q1: "27%", q2: "26%", q3: "22%", q4: "12%" },
      { category: "Security Awareness", planned: 2608000, actual: 4088000, utilization: 156.7, q1: "30%", q2: "31%", q3: "33%", q4: "13%" },
      { category: "Research & Innovation", planned: 2615000, actual: 4101000, utilization: 156.8, q1: "17%", q2: "20%", q3: "28%", q4: "14%" },
    ];
    ...
    const headcount = [
      { team: "SOC Tier 1", current: 5, target: 20, gap: 1, avgSalary: "110k" },
      { team: "SOC Tier 2", current: 14, target: 19, gap: 0, avgSalary: "139k" },
      { team: "Threat Intel", current: 6, target: 18, gap: 5, avgSalary: "168k" },
      { team: "Red Team", current: 15, target: 17, gap: 4, avgSalary: "86k" },
      { team: "GRC", current: 7, target: 16, gap: 3, avgSalary: "115k" },
      { team: "AppSec", current: 16, target: 15, gap: 2, avgSalary: "144k" },
      { team: "Cloud Sec", current: 8, target: 14, gap: 1, avgSalary: "173k" },
      { team: "Identity & Access", current: 17, target: 13, gap: 0, avgSalary: "91k" },
    ];
    const vendorSpend = [
      { vendor: "CrowdStrike", annual: "573k", contractEnd: "2026-02", renewalRisk: "Low", satisfaction: 4 },
      { vendor: "Palo Alto", annual: "604k", contractEnd: "2026-03", renewalRisk: "Medium", satisfaction: 3 },
      { vendor: "Splunk", annual: "635k", contractEnd: "2026-04", renewalRisk: "High", satisfaction: 5 },
      { vendor: "Qualys", annual: "666k", contractEnd: "2026-05", renewalRisk: "Low", satisfaction: 4 },
      { vendor: "Rapid7", annual: "697k", contractEnd: "2026-06", renewalRisk: "Medium", satisfaction: 3 },
      { vendor: "Mandiant", annual: "728k", contractEnd: "2026-07", renewalRisk: "High", satisfaction: 5 },
      { vendor: "Zscaler", annual: "759k", contractEnd: "2026-08", renewalRisk: "Low", satisfaction: 4 },
      { vendor: "Duo Security", annual: "790k", contractEnd: "2026-09", renewalRisk: "Medium", satisfaction: 3 },
    ];
    const roiProjections = [
      { area: "Threat Detection", investment: "824k", projectedReturn: "612k", roiMultiple: "2.0x", confidence: 73 },
      { area: "Incident Reduction", investment: "867k", projectedReturn: "659k", roiMultiple: "1.9x", confidence: 90 },
      { area: "Compliance Savings", investment: "109k", projectedReturn: "706k", roiMultiple: "1.8x", confidence: 71 },
      { area: "Automation Gains", investment: "152k", projectedReturn: "753k", roiMultiple: "1.7x", confidence: 88 },
      { area: "Risk Avoidance", investment: "195k", projectedReturn: "800k", roiMultiple: "1.6x", confidence: 69 },
    ];
    return html`
  ...
  private _renderMetricsNormalization() {
    const kpiCatalog = [
      { id: "kpi-1", name: "MTTD", owner: "SOC", unit: "minutes", target: 86, current: 85, benchmark: 81, collection: "auto", frequency: "realtime" },
      { id: "kpi-2", name: "MTTR", owner: "GRC", unit: "%", target: 89, current: 92, benchmark: 92, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-3", name: "MTTC", owner: "AppSec", unit: "score", target: 92, current: 99, benchmark: 69, collection: "manual", frequency: "weekly" },
      { id: "kpi-4", name: "Vuln SLA Compliance", owner: "Cloud Sec", unit: "count", target: 95, current: 60, benchmark: 80, collection: "auto", frequency: "monthly" },
      { id: "kpi-5", name: "Patch Coverage", owner: "Identity", unit: "days", target: 98, current: 67, benchmark: 91, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-6", name: "Phishing Click Rate", owner: "Threat Intel", unit: "minutes", target: 71, current: 74, benchmark: 68, collection: "manual", frequency: "daily" },
      { id: "kpi-7", name: "Training Completion", owner: "Security Ops", unit: "%", target: 74, current: 81, benchmark: 79, collection: "auto", frequency: "weekly" },
      { id: "kpi-8", name: "Escalation Rate", owner: "Risk Mgmt", unit: "score", target: 77, current: 88, benchmark: 90, collection: "semi-auto", frequency: "monthly" },
      { id: "kpi-9", name: "False Positive Rate", owner: "SOC", unit: "count", target: 80, current: 95, benchmark: 67, collection: "manual", frequency: "realtime" },
      { id: "kpi-10", name: "Threat Intel Actionability", owner: "GRC", unit: "days", target: 83, current: 56, benchmark: 78, collection: "auto", frequency: "daily" },
      { id: "kpi-11", name: "Endpoint Compliance", owner: "AppSec", unit: "minutes", target: 86, current: 63, benchmark: 89, collection: "semi-auto", frequency: "weekly" },
      { id: "kpi-12", name: "Cloud Misconfig Score", owner: "Cloud Sec", unit: "%", target: 89, current: 70, benchmark: 66, collection: "manual", frequency: "monthly" },
      { id: "kpi-13", name: "Identity Anomaly Rate", owner: "Identity", unit: "score", target: 92, current: 77, benchmark: 77, collection: "auto", frequency: "realtime" },
      { id: "kpi-14", name: "DLP Events", owner: "Threat Intel", unit: "count", target: 95, current: 84, benchmark: 88, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-15", name: "Vendor Risk Avg", owner: "Security Ops", unit: "days", target: 98, current: 91, benchmark: 65, collection: "manual", frequency: "weekly" },
      { id: "kpi-16", name: "Compliance Audit Pass Rate", owner: "Risk Mgmt", unit: "minutes", target: 71, current: 98, benchmark: 76, collection: "auto", frequency: "monthly" },
      { id: "kpi-17", name: "Awareness Score", owner: "SOC", unit: "%", target: 74, current: 59, benchmark: 87, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-18", name: "SOC Utilization", owner: "GRC", unit: "score", target: 77, current: 66, benchmark: 98, collection: "manual", frequency: "daily" },
      { id: "kpi-19", name: "Automation Coverage", owner: "AppSec", unit: "count", target: 80, current: 73, benchmark: 75, collection: "auto", frequency: "weekly" },
      { id: "kpi-20", name: "Risk Register Currency", owner: "Cloud Sec", unit: "days", target: 83, current: 80, benchmark: 86, collection: "semi-auto", frequency: "monthly" },
    ];
    const benchmarkSources = [
      { source: "NIST CSF", mappedKPIs: 7, alignment: 76, lastReview: "2026-05-05", status: "aligned" },
      { source: "CIS Controls v8", mappedKPIs: 8, alignment: 93, lastReview: "2026-06-28", status: "partial" },
      { source: "ISO 27001:2022", mappedKPIs: 3, alignment: 71, lastReview: "2026-01-23", status: "reviewing" },
      { source: "PCI DSS 4.0", mappedKPIs: 4, alignment: 88, lastReview: "2026-02-18", status: "aligned" },
      { source: "SOC 2 Type II", mappedKPIs: 5, alignment: 66, lastReview: "2026-03-13", status: "partial" },
      { source: "MITRE ATT&CK", mappedKPIs: 6, alignment: 83, lastReview: "2026-04-08", status: "reviewing" },
      { source: "SANS Top 20", mappedKPIs: 7, alignment: 61, lastReview: "2026-05-03", status: "aligned" },
      { source: "OWASP Top 10", mappedKPIs: 8, alignment: 78, lastReview: "2026-06-26", status: "partial" },
    ];
  ...
  private _renderThreatHuntingCampaigns() {
    const campaigns = [
      { id: "HC-1001", name: "Lateral Movement Sweep", status: "active", hypothesis: "H1: Actors using pass-the-hash for lateral movement", leadHunter: "Alice Chen", findings: 44, startDate: "2026-01-25", endDate: null, effectiveness: 69 },
      { id: "HC-1002", name: "Credential Harvesting Hunt", status: "completed", hypothesis: "H2: Actors using web shells for persistence", leadHunter: "Bob Martinez", findings: 47, startDate: "2026-04-08", endDate: "2026-04-14", effectiveness: 88 },
      { id: "HC-1003", name: "Persistence Mechanism Audit", status: "planned", hypothesis: "H3: Actors using scheduled tasks for data theft", leadHunter: "Carol Wu", findings: 2, startDate: "2026-03-19", endDate: null, effectiveness: 48 },
      { id: "HC-1004", name: "C2 Beacon Detection", status: "in-review", hypothesis: "H4: Actors using DNS tunneling for C2 communication", leadHunter: "Dave Kim", findings: 5, startDate: "2026-02-02", endDate: null, effectiveness: 67 },
      { id: "HC-1005", name: "Data Exfiltration Patterns", status: "active", hypothesis: "H5: Actors using encrypted channels for privilege escalation", leadHunter: "Eve Johnson", findings: 8, startDate: "2026-01-13", endDate: null, effectiveness: 86 },
      { id: "HC-1006", name: "Privilege Escalation Scan", status: "completed", hypothesis: "H6: Actors using token impersonation for defense evasion", leadHunter: "Frank Liu", findings: 11, startDate: "2026-04-24", endDate: "2026-05-26", effectiveness: 46 },
      { id: "HC-1007", name: "Supply Chain Implant Hunt", status: "planned", hypothesis: "H7: Actors using poisoned images for initial access", leadHunter: "Grace Park", findings: 14, startDate: "2026-03-07", endDate: null, effectiveness: 65 },
      { id: "HC-1008", name: "Insider Threat Indicators", status: "in-review", hypothesis: "H8: Actors using legitimate tools for credential access", leadHunter: "Hector Silva", findings: 17, startDate: "2026-02-18", endDate: null, effectiveness: 84 },
      { id: "HC-1009", name: "Cloud Metadata Analysis", status: "active", hypothesis: "H9: Actors using API keys for command execution", leadHunter: "Alice Chen", findings: 20, startDate: "2026-01-01", endDate: null, effectiveness: 44 },
      { id: "HC-1010", name: "DNS Tunnel Detection", status: "completed", hypothesis: "H10: Actors using encoded subdomains for exfiltration", leadHunter: "Bob Martinez", findings: 23, startDate: "2026-04-12", endDate: "2026-06-10", effectiveness: 63 },
      { id: "HC-1011", name: "Fileless Malware Search", status: "planned", hypothesis: "H11: Actors using WMI providers for discovery", leadHunter: "Carol Wu", findings: 26, startDate: "2026-03-23", endDate: null, effectiveness: 82 },
      { id: "HC-1012", name: "Zero-Day Exploit Traces", status: "in-review", hypothesis: "H12: Actors using exploit kits for collection", leadHunter: "Dave Kim", findings: 29, startDate: "2026-02-06", endDate: null, effectiveness: 42 },
    ];
    const hunterLeaderboard = [
      { hunter: "Alice Chen", campaigns: 5, findings: 33, highSeverity: 2, avgScore: 95, streak: 5 },
      { hunter: "Bob Martinez", campaigns: 15, findings: 62, highSeverity: 7, avgScore: 88, streak: 6 },
      { hunter: "Carol Wu", campaigns: 12, findings: 91, highSeverity: 12, avgScore: 81, streak: 7 },
      { hunter: "Dave Kim", campaigns: 9, findings: 120, highSeverity: 17, avgScore: 74, streak: 8 },
      { hunter: "Eve Johnson", campaigns: 6, findings: 33, highSeverity: 22, avgScore: 67, streak: 1 },
      { hunter: "Frank Liu", campaigns: 3, findings: 62, highSeverity: 1, avgScore: 60, streak: 2 },
      { hunter: "Grace Park", campaigns: 13, findings: 91, highSeverity: 6, avgScore: 97, streak: 3 },
      { hunter: "Hector Silva", campaigns: 10, findings: 120, highSeverity: 11, avgScore: 90, streak: 4 },
    ];
    const mitreMapping = [
      { tactic: "Initial Access", techniques: 9, campaigns: 3, coverage: 99 },
      { tactic: "Execution", techniques: 8, campaigns: 2, coverage: 79 },
      { tactic: "Persistence", techniques: 7, campaigns: 1, coverage: 59 },
      { tactic: "Privilege Escalation", techniques: 6, campaigns: 6, coverage: 39 },
      { tactic: "Defense Evasion", techniques: 5, campaigns: 5, coverage: 90 },
      { tactic: "Credential Access", techniques: 4, campaigns: 4, coverage: 70 },
      { tactic: "Discovery", techniques: 3, campaigns: 3, coverage: 50 },
      { tactic: "Lateral Movement", techniques: 2, campaigns: 2, coverage: 30 },
      { tactic: "Collection", techniques: 12, campaigns: 1, coverage: 81 },
      { tactic: "Exfiltration", techniques: 11, campaigns: 6, coverage: 61 },
      { tactic: "Command & Control", techniques: 10, campaigns: 5, coverage: 41 },
      { tactic: "Impact", techniques: 9, campaigns: 4, coverage: 92 },
    ];
    return html`
  ...
  private _renderControlInventory() {
    const controls = [
      { id: "CTL-2001", name: "MFA Enforcement", domain: "Access Control", status: "implemented", effectiveness: 66, lastTest: "2026-01-17", nextReview: "2026-09-17", owner: "SOC", risk: "Low" },
      { id: "CTL-2002", name: "Network Segmentation", domain: "Network Security", status: "partial", effectiveness: 34, lastTest: "2026-04-02", nextReview: "2026-10-08", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2003", name: "EDR Deployment", domain: "Endpoint Protection", status: "planned", effectiveness: 41, lastTest: "2026-03-15", nextReview: "2026-11-27", owner: "IT Ops", risk: "High" },
      { id: "CTL-2004", name: "DLP Policy", domain: "Data Protection", status: "gap", effectiveness: 48, lastTest: "2026-02-28", nextReview: "2026-12-18", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2005", name: "SSO Integration", domain: "Identity Management", status: "implemented", effectiveness: 78, lastTest: "2026-01-13", nextReview: "2026-05-09", owner: "IAM", risk: "Low" },
      { id: "CTL-2006", name: "SAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 11, lastTest: "2026-04-26", nextReview: "2026-06-28", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2007", name: "CSPM Scanning", domain: "Cloud Security", status: "planned", effectiveness: 18, lastTest: "2026-03-11", nextReview: "2026-07-19", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2008", name: "Badge Access", domain: "Physical Security", status: "gap", effectiveness: 25, lastTest: "2026-02-24", nextReview: "2026-08-10", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2009", name: "Least Privilege", domain: "Access Control", status: "implemented", effectiveness: 90, lastTest: "2026-01-09", nextReview: "2026-09-01", owner: "SOC", risk: "Low" },
      { id: "CTL-2010", name: "Firewall Rules", domain: "Network Security", status: "partial", effectiveness: 39, lastTest: "2026-04-22", nextReview: "2026-10-20", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2011", name: "Disk Encryption", domain: "Endpoint Protection", status: "planned", effectiveness: 46, lastTest: "2026-03-07", nextReview: "2026-11-11", owner: "IT Ops", risk: "High" },
      { id: "CTL-2012", name: "Data Classification", domain: "Data Protection", status: "gap", effectiveness: 2, lastTest: "2026-02-20", nextReview: "2026-12-02", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2013", name: "PAM Implementation", domain: "Identity Management", status: "implemented", effectiveness: 43, lastTest: "2026-01-05", nextReview: "2026-05-21", owner: "IAM", risk: "Low" },
      { id: "CTL-2014", name: "DAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 16, lastTest: "2026-04-18", nextReview: "2026-06-12", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2015", name: "IAM Policy Review", domain: "Cloud Security", status: "planned", effectiveness: 23, lastTest: "2026-03-03", nextReview: "2026-07-03", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2016", name: "Visitor Management", domain: "Physical Security", status: "gap", effectiveness: 30, lastTest: "2026-02-16", nextReview: "2026-08-22", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2017", name: "Access Reviews", domain: "Access Control", status: "implemented", effectiveness: 55, lastTest: "2026-01-01", nextReview: "2026-09-13", owner: "SOC", risk: "Low" },
      { id: "CTL-2018", name: "IDS/IPS Tuning", domain: "Network Security", status: "partial", effectiveness: 44, lastTest: "2026-04-14", nextReview: "2026-10-04", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2019", name: "Patch Management", domain: "Endpoint Protection", status: "planned", effectiveness: 0, lastTest: "2026-03-27", nextReview: "2026-11-23", owner: "IT Ops", risk: "High" },
      { id: "CTL-2020", name: "Backup Encryption", domain: "Data Protection", status: "gap", effectiveness: 7, lastTest: "2026-02-12", nextReview: "2026-12-14", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2021", name: "Password Policy", domain: "Identity Management", status: "implemented", effectiveness: 67, lastTest: "2026-01-25", nextReview: "2026-05-05", owner: "IAM", risk: "Low" },
      { id: "CTL-2022", name: "Container Scanning", domain: "Application Security", status: "partial", effectiveness: 21, lastTest: "2026-04-10", nextReview: "2026-06-24", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2023", name: "WAF Configuration", domain: "Cloud Security", status: "planned", effectiveness: 28, lastTest: "2026-03-23", nextReview: "2026-07-15", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2024", name: "CCTV Coverage", domain: "Physical Security", status: "gap", effectiveness: 35, lastTest: "2026-02-08", nextReview: "2026-08-06", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2025", name: "RBAC Enforcement", domain: "Access Control", status: "implemented", effectiveness: 79, lastTest: "2026-01-21", nextReview: "2026-09-25", owner: "SOC", risk: "Low" },
      { id: "CTL-2026", name: "VPN Management", domain: "Network Security", status: "partial", effectiveness: 49, lastTest: "2026-04-06", nextReview: "2026-10-16", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2027", name: "App Whitelisting", domain: "Endpoint Protection", status: "planned", effectiveness: 5, lastTest: "2026-03-19", nextReview: "2026-11-07", owner: "IT Ops", risk: "High" },
      { id: "CTL-2028", name: "Key Management", domain: "Data Protection", status: "gap", effectiveness: 12, lastTest: "2026-02-04", nextReview: "2026-12-26", owner: "Data Gov", risk: "Critical" },
    ];
    const gapAnalysis = [
      { gap: "Insufficient MFA coverage for legacy apps", severity: "High", remediationPlan: "Plan R3001", eta: "2026-Q2", estimatedCost: "71k" },
      { gap: "Missing network micro-segmentation", severity: "Medium", remediationPlan: "Plan R3002", eta: "2026-Q4", estimatedCost: "100k" },
      { gap: "Inconsistent EDR deployment", severity: "Medium", remediationPlan: "Plan R3003", eta: "2026-Q3", estimatedCost: "129k" },
      { gap: "DLP not covering cloud storage", severity: "Low", remediationPlan: "Plan R3004", eta: "2026-Q2", estimatedCost: "158k" },
      { gap: "SSO not integrated with all SaaS", severity: "High", remediationPlan: "Plan R3005", eta: "2026-Q4", estimatedCost: "187k" },
    ];
    return html`
  ...
  private _renderIncidentCostTracker() {
    const incidents = [
      { id: "INC-7001", name: "Security Incident 1", severity: "Critical", totalCost: 1211000, responseCost: 193760, recoveryCost: 254310, legalCost: 205870, regulatoryCost: 121100, insuranceClaim: 0, avoidedCost: 417000, date: "2026-01-09" },
      { id: "INC-7002", name: "Security Incident 2", severity: "High", totalCost: 1214000, responseCost: 279220, recoveryCost: 388480, legalCost: 169960, regulatoryCost: 60700, insuranceClaim: 728400, avoidedCost: 446000, date: "2026-04-18" },
      { id: "INC-7003", name: "Security Incident 3", severity: "Medium", totalCost: 1217000, responseCost: 365100, recoveryCost: 267740, legalCost: 133870, regulatoryCost: 133870, insuranceClaim: 0, avoidedCost: 475000, date: "2026-03-27" },
      { id: "INC-7004", name: "Security Incident 4", severity: "Low", totalCost: 1220000, responseCost: 195200, recoveryCost: 402600, legalCost: 97600, regulatoryCost: 73200, insuranceClaim: 671000, avoidedCost: 8000, date: "2026-02-08" },
      { id: "INC-7005", name: "Security Incident 5", severity: "Critical", totalCost: 1223000, responseCost: 281290, recoveryCost: 281290, legalCost: 61150, regulatoryCost: 146760, insuranceClaim: 0, avoidedCost: 37000, date: "2026-01-17" },
      { id: "INC-7006", name: "Security Incident 6", severity: "High", totalCost: 1226000, responseCost: 367800, recoveryCost: 416840, legalCost: 220680, regulatoryCost: 85820, insuranceClaim: 613000, avoidedCost: 66000, date: "2026-04-26" },
      { id: "INC-7007", name: "Security Incident 7", severity: "Medium", totalCost: 1229000, responseCost: 196640, recoveryCost: 294960, legalCost: 184350, regulatoryCost: 159770, insuranceClaim: 0, avoidedCost: 95000, date: "2026-03-07" },
      { id: "INC-7008", name: "Security Incident 8", severity: "Low", totalCost: 1232000, responseCost: 283360, recoveryCost: 431200, legalCost: 147840, regulatoryCost: 98560, insuranceClaim: 554400, avoidedCost: 124000, date: "2026-02-16" },
      { id: "INC-7009", name: "Security Incident 9", severity: "Critical", totalCost: 1235000, responseCost: 370500, recoveryCost: 308750, legalCost: 111150, regulatoryCost: 172900, insuranceClaim: 0, avoidedCost: 153000, date: "2026-01-25" },
      { id: "INC-7010", name: "Security Incident 10", severity: "High", totalCost: 1238000, responseCost: 198080, recoveryCost: 445680, legalCost: 74280, regulatoryCost: 111420, insuranceClaim: 495200, avoidedCost: 182000, date: "2026-04-06" },
      { id: "INC-7011", name: "Security Incident 11", severity: "Medium", totalCost: 1241000, responseCost: 285430, recoveryCost: 322660, legalCost: 235790, regulatoryCost: 186150, insuranceClaim: 0, avoidedCost: 211000, date: "2026-03-15" },
      { id: "INC-7012", name: "Security Incident 12", severity: "Low", totalCost: 1244000, responseCost: 373200, recoveryCost: 460280, legalCost: 199040, regulatoryCost: 124400, insuranceClaim: 435400, avoidedCost: 240000, date: "2026-02-24" },
    ];
    const yearlyTrend = [
      { month: "Jan", incidents: 7, totalCost: "66k", avgCost: "166k", insured: 27 },
      { month: "Feb", incidents: 4, totalCost: "109k", avgCost: "32k", insured: 27 },
      { month: "Mar", incidents: 12, totalCost: "152k", avgCost: "79k", insured: 27 },
      { month: "Apr", incidents: 9, totalCost: "195k", avgCost: "126k", insured: 27 },
      { month: "May", incidents: 6, totalCost: "238k", avgCost: "173k", insured: 27 },
      { month: "Jun", incidents: 3, totalCost: "281k", avgCost: "39k", insured: 27 },
    ];
