...
@state() private _spmRiskRegister3 = { id: "RSK-003", title: "Risk Item 3", category: "operational", likelihood: 4, impact: "high", owner: "Security Team", status: "open", mitigation: "Implement compensating controls", targetDate: "2026-02-13" } as any;
  ...
  @state() private _darkSimEnginCorrRules: Array<{id:string;name:string;sources:string[];logic:string;severity:string;active:boolean;lastTriggered:string}> = [
  ...
  @state() private _darkSimEnginEventTimeline: Array<{timestamp:string;source:string;eventType:string;details:string;correlated:boolean}> = [
  ...
  @state() private _darkSimEnginFalsePosMetrics: {totalEvents:number;correlatedEvents:number;falsePositives:number;fpRate:number;topFpRules:string[]} = {
  ...
  @state() private _darkSimEnginEventPatterns: Array<{id:string;pattern:string;frequency:number;firstSeen:string;lastSeen:string;status:string}> = [
  ...
  private _renderDarksimenginEventCorr(): TemplateResult {
    const rules = this._darkSimEnginCorrRules;
    const timeline = this._darkSimEnginEventTimeline;
    const fpMetrics = this._darkSimEnginFalsePosMetrics;
    return html`
  ...
  @state() private _darkSimEnginZones: Array<{id:string;name:string;trustLevel:number;subnet:string;devices:number;policy:string;lastAudit:string}> = [
  ...
  @state() private _darkSimEnginSegRules: Array<{id:string;source:string;dest:string;action:string;protocol:string;port:string;status:string;hits:number}> = [
  ...
  @state() private _darkSimEnginCrossZoneTraffic: Array<{source:string;dest:string;bytes:number;sessions:number;violations:number}> = [
  ...
  @state() private _darkSimEnginMicroSegGaps: Array<{id:string;zone:string;gapType:string;severity:string;recommendation:string}> = [
  ...
  private _renderDarksimenginNetworkSeg(): TemplateResult {
    const zones = this._darkSimEnginZones;
    const gaps = this._darkSimEnginMicroSegGaps;
    return html`
  ...
  @state() private _darkSimEnginContainerScans: Array<{image:string;registry:string;critical:number;high:number;medium:number;scanDate:string;status:string}> = [
  ...
  @state() private _darkSimEnginK8sPods: Array<{namespace:string;pod:string;securityContext:string;hostIPC:boolean;hostPID:boolean;privileged:boolean;riskLevel:string}> = [
  ...
  @state() private _darkSimEnginServerlessRisk: Array<{function:string;runtime:string;timeout:number;iamPerms:string;externalCalls:number;riskScore:number}> = [
  ...
  @state() private _darkSimEnginRuntimeAlerts: Array<{id:string;workload:string;alertType:string;severity:string;description:string;timestamp:string}> = [
  ...
  private _renderDarksimenginCloudWl(): TemplateResult {
    const containers = this._darkSimEnginContainerScans;
    const pods = this._darkSimEnginK8sPods;
    const alerts = this._darkSimEnginRuntimeAlerts;
    return html`
  ...
  @state() private _darkSimEnginCourses: Array<{id:string;title:string;category:string;duration:number;enrolled:number;completed:number;difficulty:string;rating:number}> = [
  ...
  @state() private _darkSimEnginLearningPaths: Array<{id:string;name:string;courseIds:string[];progress:number;enrolled:number}> = [
  ...
  @state() private _darkSimEnginDeptCompliance: Array<{dept:string;trainedPct:number;targetPct:number;avgScore:number;certCount:number}> = [
  ...
  @state() private _darkSimEnginSkillsGaps: Array<{skill:string;current:number;required:number;gap:number;priority:string}> = [
  ...
  private _renderDarksimenginTraining(): TemplateResult {
    const courses = this._darkSimEnginCourses;
    const deptComp = this._darkSimEnginDeptCompliance;
    return html`
  ...
  @state() private _complianceCalendarEvents: Array<{id: string; title: string; deadline: string; category: string; status: string; assignee: string; notes: string}> = [
    {id: 'cc-dark-sim-engine-001', title: 'SOX Section 404 Compliance Review', deadline: '2026-06-30', category: 'Regulatory', status: 'In Progress', assignee: 'Compliance Team', notes: 'Annual internal control assessment for financial reporting'},
    {id: 'cc-dark-sim-engine-002', title: 'GDPR Data Protection Impact Assessment', deadline: '2026-05-15', category: 'Privacy', status: 'Pending', assignee: 'DPO Office', notes: 'Quarterly DPIA for high-risk processing activities'},
    {id: 'cc-dark-sim-engine-003', title: 'ISO 27001 Surveillance Audit', deadline: '2026-07-20', category: 'Certification', status: 'Scheduled', assignee: 'QA Lead', notes: 'Stage 2 surveillance audit for ISMS certification renewal'},
    {id: 'cc-dark-sim-engine-004', title: 'PCI DSS v4.0 Gap Analysis', deadline: '2026-05-30', category: 'Payment', status: 'Not Started', assignee: 'Security Architect', notes: 'Assess current state against PCI DSS v4.0 requirements'},
    {id: 'cc-dark-sim-engine-005', title: 'NIST CSF Self-Assessment', deadline: '2026-08-15', category: 'Framework', status: 'Pending', assignee: 'CISO', notes: 'Biannual self-assessment against NIST Cybersecurity Framework'},
    {id: 'cc-dark-sim-engine-006', title: 'HIPAA Security Rule Audit', deadline: '2026-06-15', category: 'Healthcare', status: 'In Progress', assignee: 'Compliance Analyst', notes: 'Annual audit of administrative, physical, and technical safeguards'},
    {id: 'cc-dark-sim-engine-007', title: 'SOC 2 Type II Report Renewal', deadline: '2026-09-30', category: 'Audit', status: 'Scheduled', assignee: 'External Auditor', notes: 'Engage auditor for SOC 2 Type II examination period'},
    {id: 'cc-dark-sim-engine-008', title: 'FedRAMP Authorization Review', deadline: '2026-07-01', category: 'Government', status: 'Pending', assignee: 'FedRAMP PMO', notes: 'Review and update security authorization package'},
    {id: 'cc-dark-sim-engine-009', title: 'Annual Penetration Test', deadline: '2026-10-15', category: 'Testing', status: 'Not Started', assignee: 'Red Team Lead', notes: 'Comprehensive external and internal penetration testing engagement'},
    {id: 'cc-dark-sim-engine-010', title: 'Security Awareness Training Rollout', deadline: '2026-05-01', category: 'Training', status: 'Completed', assignee: 'Security Awareness Manager', notes: 'Q2 organization-wide security awareness training program'},
    {id: 'cc-dark-sim-engine-011', title: 'Vendor Security Review Cycle', deadline: '2026-06-01', category: 'Third Party', status: 'In Progress', assignee: 'Vendor Manager', notes: 'Quarterly review of critical vendor security posture and SLA compliance'},
    {id: 'cc-dark-sim-engine-012', title: 'Incident Response Plan Update', deadline: '2026-05-20', category: 'Operations', status: 'Pending', assignee: 'IR Team Lead', notes: 'Update IR procedures based on lessons learned from recent incidents'},
  ];
  ...
  @state() private _dataQualityMetrics: Array<{id: string; dataSource: string; completenessScore: number; accuracyScore: number; timelinessScore: number; consistencyScore: number; reliabilityRank: number; lastUpdated: string; issues: string[]; trend: string}> = [
    {id: 'dq-dark-sim-engine-001', dataSource: 'Vulnerability Scanner Feed', completenessScore: 94, accuracyScore: 91, timelinessScore: 88, consistencyScore: 95, reliabilityRank: 1, lastUpdated: '2026-04-23T08:00:00Z', issues: ['Minor delay in Nessus plugin updates'], trend: 'improving'},
    {id: 'dq-dark-sim-engine-002', dataSource: 'SIEM Event Logs', completenessScore: 87, accuracyScore: 93, timelinessScore: 96, consistencyScore: 89, reliabilityRank: 2, lastUpdated: '2026-04-23T07:30:00Z', issues: ['Some log sources showing gaps in off-hours'], trend: 'stable'},
    {id: 'dq-dark-sim-engine-003', dataSource: 'Asset Inventory Database', completenessScore: 82, accuracyScore: 78, timelinessScore: 75, consistencyScore: 84, reliabilityRank: 4, lastUpdated: '2026-04-22T18:00:00Z', issues: ['Shadow IT devices not fully cataloged', 'Decommissioned assets still listed'], trend: 'declining'},
    {id: 'dq-dark-sim-engine-004', dataSource: 'Threat Intelligence Platform', completenessScore: 91, accuracyScore: 89, timelinessScore: 92, consistencyScore: 87, reliabilityRank: 3, lastUpdated: '2026-04-23T06:00:00Z', issues: ['Some IOC sources lack confidence scoring'], trend: 'improving'},
    {id: 'dq-dark-sim-engine-005', dataSource: 'Identity Provider Logs', completenessScore: 96, accuracyScore: 97, timelinessScore: 98, consistencyScore: 96, reliabilityRank: 1, lastUpdated: '2026-04-23T09:00:00Z', issues: [], trend: 'stable'},
    {id: 'dq-dark-sim-engine-006', dataSource: 'Cloud Security Posture Data', completenessScore: 79, accuracyScore: 85, timelinessScore: 82, consistencyScore: 81, reliabilityRank: 5, lastUpdated: '2026-04-23T05:00:00Z', issues: ['Multi-cloud coverage gaps', 'API rate limiting affects scan frequency'], trend: 'declining'},
    {id: 'dq-dark-sim-engine-007', dataSource: 'Compliance Evidence Store', completenessScore: 88, accuracyScore: 86, timelinessScore: 80, consistencyScore: 83, reliabilityRank: 4, lastUpdated: '2026-04-21T14:00:00Z', issues: ['Manual evidence collection delays', 'Inconsistent formatting across frameworks'], trend: 'stable'},
    {id: 'dq-dark-sim-engine-008', dataSource: 'Network Flow Data', completenessScore: 92, accuracyScore: 90, timelinessScore: 94, consistencyScore: 91, reliabilityRank: 2, lastUpdated: '2026-04-23T08:30:00Z', issues: ['Encrypted traffic classification accuracy needs improvement'], trend: 'improving'},
  ];
  ...
  @state() private _workflowTemplates: Array<{id: string; name: string; description: string; category: string; steps: number; avgDuration: string; successRate: number; lastRun: string; manualInterventions: number; optimizationScore: number}> = [
    {id: 'wf-dark-sim-engine-001', name: 'Vulnerability Triage Pipeline', description: 'Automated vulnerability assessment, prioritization, and assignment workflow', category: 'Vulnerability Management', steps: 8, avgDuration: '12 min', successRate: 94.5, lastRun: '2026-04-23T06:00:00Z', manualInterventions: 2, optimizationScore: 87},
    {id: 'wf-dark-sim-engine-002', name: 'Incident Response Orchestration', description: 'End-to-end IR workflow from detection to containment and recovery', category: 'Incident Response', steps: 15, avgDuration: '45 min', successRate: 89.2, lastRun: '2026-04-22T14:30:00Z', manualInterventions: 5, optimizationScore: 72},
    {id: 'wf-dark-sim-engine-003', name: 'Access Request Approval Chain', description: 'Multi-level approval workflow for privileged access requests', category: 'Identity & Access', steps: 6, avgDuration: '4 hours', successRate: 97.1, lastRun: '2026-04-23T09:15:00Z', manualInterventions: 1, optimizationScore: 92},
    {id: 'wf-dark-sim-engine-004', name: 'Compliance Evidence Collection', description: 'Automated gathering and packaging of audit evidence artifacts', category: 'Compliance', steps: 10, avgDuration: '30 min', successRate: 91.8, lastRun: '2026-04-21T10:00:00Z', manualInterventions: 3, optimizationScore: 78},
    {id: 'wf-dark-sim-engine-005', name: 'Security Patch Deployment', description: 'Staged patch rollout with validation and rollback capability', category: 'Patch Management', steps: 12, avgDuration: '2 hours', successRate: 96.3, lastRun: '2026-04-20T22:00:00Z', manualInterventions: 1, optimizationScore: 85},
    {id: 'wf-dark-sim-engine-006', name: 'Threat Intelligence Enrichment', description: 'IOC enrichment and correlation with internal threat data', category: 'Threat Intelligence', steps: 7, avgDuration: '8 min', successRate: 93.7, lastRun: '2026-04-23T07:00:00Z', manualInterventions: 0, optimizationScore: 95},
    {id: 'wf-dark-sim-engine-007', name: 'Security Reporting Pipeline', description: 'Automated generation and distribution of security metrics reports', category: 'Reporting', steps: 5, avgDuration: '15 min', successRate: 98.9, lastRun: '2026-04-23T08:00:00Z', manualInterventions: 0, optimizationScore: 97},
    {id: 'wf-dark-sim-engine-008', name: 'Vendor Risk Assessment', description: 'Automated vendor security questionnaire distribution and scoring', category: 'Third Party', steps: 9, avgDuration: '3 days', successRate: 85.4, lastRun: '2026-04-19T11:00:00Z', manualInterventions: 4, optimizationScore: 65},
  ];
  ...
  @state() private _workflowExecutionHistory: Array<{workflowId: string; runDate: string; duration: string; status: string; trigger: string}> = [
    {workflowId: 'wf-dark-sim-engine-001', runDate: '2026-04-23T06:00:00Z', duration: '11 min 23 sec', status: 'Success', trigger: 'Scheduled'},
    {workflowId: 'wf-dark-sim-engine-002', runDate: '2026-04-22T14:30:00Z', duration: '42 min 15 sec', status: 'Success', trigger: 'Alert'},
    {workflowId: 'wf-dark-sim-engine-003', runDate: '2026-04-23T09:15:00Z', duration: '3h 45 min', status: 'Success', trigger: 'User Request'},
    {workflowId: 'wf-dark-sim-engine-001', runDate: '2026-04-22T06:00:00Z', duration: '13 min 08 sec', status: 'Partial', trigger: 'Scheduled'},
    {workflowId: 'wf-dark-sim-engine-004', runDate: '2026-04-21T10:00:00Z', duration: '28 min 42 sec', status: 'Success', trigger: 'Scheduled'},
  ];
  ...
  @state() private _knowledgeArticles: Array<{id: string; title: string; category: string; author: string; publishDate: string; views: number; rating: number; tags: string[]; summary: string; readTime: string}> = [
    {id: 'kb-dark-sim-engine-001', title: 'Zero Trust Architecture Implementation Guide', category: 'Architecture', author: 'Security Architecture Team', publishDate: '2026-03-15', views: 1842, rating: 4.8, tags: ['zero-trust', 'network', 'identity'], summary: 'Comprehensive guide for implementing zero trust principles across enterprise infrastructure', readTime: '15 min'},
    {id: 'kb-dark-sim-engine-002', title: 'Ransomware Defense Playbook 2026', category: 'Threats', author: 'Threat Intelligence Team', publishDate: '2026-04-01', views: 2341, rating: 4.9, tags: ['ransomware', 'incident-response', 'defense'], summary: 'Updated playbook with latest ransomware tactics, techniques, and defensive measures', readTime: '12 min'},
    {id: 'kb-dark-sim-engine-003', title: 'Cloud Security Best Practices for Multi-Cloud', category: 'Cloud', author: 'Cloud Security Team', publishDate: '2026-02-28', views: 1567, rating: 4.5, tags: ['cloud', 'aws', 'azure', 'gcp'], summary: 'Best practices for securing workloads across multiple cloud service providers', readTime: '18 min'},
    {id: 'kb-dark-sim-engine-004', title: 'API Security Testing Methodology', category: 'Testing', author: 'Red Team', publishDate: '2026-03-22', views: 987, rating: 4.6, tags: ['api', 'testing', 'owasp'], summary: 'Structured methodology for identifying and testing API security vulnerabilities', readTime: '10 min'},
    {id: 'kb-dark-sim-engine-005', title: 'Supply Chain Security Risk Management', category: 'Third Party', author: 'GRC Team', publishDate: '2026-01-20', views: 1234, rating: 4.3, tags: ['supply-chain', 'vendor', 'risk'], summary: 'Framework for managing security risks in the software supply chain', readTime: '14 min'},
    {id: 'kb-dark-sim-engine-006', title: 'Kubernetes Security Hardening Checklist', category: 'Container', author: 'Platform Security', publishDate: '2026-04-10', views: 2103, rating: 4.7, tags: ['kubernetes', 'container', 'hardening'], summary: 'Step-by-step hardening checklist for production Kubernetes clusters', readTime: '11 min'},
    {id: 'kb-dark-sim-engine-007', title: 'Security Metrics That Matter for Executives', category: 'Metrics', author: 'CISO Office', publishDate: '2026-03-05', views: 876, rating: 4.4, tags: ['metrics', 'kpi', 'reporting'], summary: 'Key security metrics and KPIs that resonate with board-level stakeholders', readTime: '8 min'},
    {id: 'kb-dark-sim-engine-008', title: 'Phishing Simulation Campaign Design', category: 'Awareness', author: 'Security Awareness Team', publishDate: '2026-02-15', views: 1456, rating: 4.5, tags: ['phishing', 'simulation', 'training'], summary: 'Designing effective phishing simulations that drive real behavioral change', readTime: '9 min'},
    {id: 'kb-dark-sim-engine-009', title: 'Data Loss Prevention Strategy and Implementation', category: 'Data Protection', author: 'Data Security Team', publishDate: '2026-03-28', views: 1678, rating: 4.6, tags: ['dlp', 'data-protection', 'compliance'], summary: 'Strategic approach to DLP covering people, process, and technology layers', readTime: '16 min'},
    {id: 'kb-dark-sim-engine-010', title: 'Security Automation with SOAR Platforms', category: 'Automation', author: 'SOC Engineering', publishDate: '2026-04-05', views: 1098, rating: 4.7, tags: ['soar', 'automation', 'soc'], summary: 'Building effective security automation playbooks using SOAR technology', readTime: '13 min'},
    {id: 'kb-dark-sim-engine-011', title: 'Third-Party Risk Assessment Framework', category: 'GRC', author: 'Vendor Management', publishDate: '2026-01-30', views: 1345, rating: 4.4, tags: ['vendor', 'risk', 'assessment'], summary: 'Structured framework for evaluating and monitoring third-party security risks', readTime: '12 min'},
    {id: 'kb-dark-sim-engine-012', title: 'Network Detection and Response Best Practices', category: 'Network', author: 'Network Security Team', publishDate: '2026-03-18', views: 1567, rating: 4.5, tags: ['ndr', 'network', 'detection'], summary: 'Implementing effective network detection and response capabilities', readTime: '14 min'},
    {id: 'kb-dark-sim-engine-013', title: 'Security Chaos Engineering Guide', category: 'Resilience', author: 'SRE Security', publishDate: '2026-04-12', views: 789, rating: 4.3, tags: ['chaos-engineering', 'resilience', 'testing'], summary: 'Applying chaos engineering principles to validate security controls', readTime: '10 min'},
    {id: 'kb-dark-sim-engine-014', title: 'Incident Communication Templates', category: 'Incident Response', author: 'IR Team', publishDate: '2026-02-22', views: 2234, rating: 4.8, tags: ['incident', 'communication', 'templates'], summary: 'Ready-to-use communication templates for security incident scenarios', readTime: '7 min'},
    {id: 'kb-dark-sim-engine-015', title: 'Endpoint Detection and Response Configuration', category: 'Endpoint', author: 'Endpoint Security', publishDate: '2026-03-08', views: 1345, rating: 4.5, tags: ['edr', 'endpoint', 'configuration'], summary: 'Optimal EDR configuration for maximum detection with minimal false positives', readTime: '11 min'},
    {id: 'kb-dark-sim-engine-016', title: 'Security Code Review Standards', category: 'Development', author: 'AppSec Team', publishDate: '2026-02-10', views: 1890, rating: 4.6, tags: ['code-review', 'secure-sdlc', 'standards'], summary: 'Standards and checklists for security-focused code reviews', readTime: '13 min'},
    {id: 'kb-dark-sim-engine-017', title: 'IoT Security Assessment Methodology', category: 'IoT', author: 'IoT Security Lab', publishDate: '2026-04-08', views: 654, rating: 4.2, tags: ['iot', 'embedded', 'assessment'], summary: 'Methodology for assessing security posture of IoT devices and ecosystems', readTime: '15 min'},
    {id: 'kb-dark-sim-engine-018', title: 'Passwordless Authentication Migration Guide', category: 'Identity', author: 'IAM Team', publishDate: '2026-03-25', views: 1678, rating: 4.7, tags: ['passwordless', 'authentication', 'mfa'], summary: 'Step-by-step migration plan from password-based to passwordless authentication', readTime: '12 min'},
    {id: 'kb-dark-sim-engine-019', title: 'Security Awareness Program Metrics', category: 'Awareness', author: 'Security Culture Team', publishDate: '2026-01-25', views: 1123, rating: 4.4, tags: ['awareness', 'metrics', 'culture'], summary: 'Measuring the effectiveness of your security awareness and training programs', readTime: '9 min'},
    {id: 'kb-dark-sim-engine-020', title: 'Deception Technology Deployment Guide', category: 'Detection', author: 'Blue Team', publishDate: '2026-04-15', views: 876, rating: 4.5, tags: ['deception', 'honeypot', 'detection'], summary: 'Planning and deploying deception technology for advanced threat detection', readTime: '11 min'},
  ];
  ...
  // --- Security Scenario Planning Methods (DASIEN) ---
  private _initDasienScenarios(): void {
    const worstCaseScenarios = [
      { id: "dasien-sc-01", name: "Ransomware Attack on Critical Infrastructure", category: "Malware", impact: 9500000, probability: 0.12, recoveryDays: 21, affectedSystems: 47, dataLossGb: 320, businessImpact: "critical" },
      { id: "dasien-sc-02", name: "Insider Data Exfiltration via Authorized Channels", category: "Insider Threat", impact: 7200000, probability: 0.18, recoveryDays: 14, affectedSystems: 12, dataLossGb: 85, businessImpact: "high" },
      { id: "dasien-sc-03", name: "Supply Chain Compromise via Third-Party Library", category: "Supply Chain", impact: 6800000, probability: 0.09, recoveryDays: 30, affectedSystems: 63, dataLossGb: 0, businessImpact: "critical" },
      { id: "dasien-sc-04", name: "Cloud Misconfiguration Leading to Public Exposure", category: "Cloud", impact: 4100000, probability: 0.22, recoveryDays: 7, affectedSystems: 8, dataLossGb: 450, businessImpact: "high" },
      { id: "dasien-sc-05", name: "Zero-Day Exploit in Core Application Framework", category: "Vulnerability", impact: 8900000, probability: 0.05, recoveryDays: 45, affectedSystems: 120, dataLossGb: 0, businessImpact: "critical" },
      { id: "dasien-sc-06", name: "Distributed Denial of Service on Customer-Facing Services", category: "Availability", impact: 3400000, probability: 0.25, recoveryDays: 3, affectedSystems: 15, dataLossGb: 0, businessImpact: "medium" },
      { id: "dasien-sc-07", name: "Credential Stuffing and Account Takeover Wave", category: "Identity", impact: 2800000, probability: 0.30, recoveryDays: 5, affectedSystems: 6, dataLossGb: 12, businessImpact: "medium" },
      { id: "dasien-sc-08", name: "Physical Security Breach at Data Center Facility", category: "Physical", impact: 12000000, probability: 0.02, recoveryDays: 60, affectedSystems: 200, dataLossGb: 5000, businessImpact: "critical" },
    ];
    this._dasienScenarios = worstCaseScenarios.map(s => ({ ...s, mitigationCost: Math.round(s.impact * 0.08), drillScheduled: false, lastDrillDate: null, drillScore: null, controlGaps: Math.floor(Math.random() * 5) + 1 }));
  }

  private _calculateDasienScenarioRisk(): void {
    this._dasienScenarios.forEach(s => {
  ...
  private _scheduleDasienDrills(): void {
  ...
  private _dasienScenarioComparison(): Record<string, unknown>[] {
    return this._dasienScenarios.map(s => ({
  ...
  // --- Security Control Effectiveness Analytics (DASIEN) ---
  private _initDasienControls(): void {
    const controls = [
      { id: "dasien-ct-01", name: "Network Segmentation", type: "preventive", maturity: "defined", score: 82, target: 90, failures: 3, lastTest: "2026-03-15" },
      { id: "dasien-ct-02", name: "Endpoint Detection and Response", type: "detective", maturity: "managed", score: 88, target: 92, failures: 1, lastTest: "2026-04-01" },
      { id: "dasien-ct-03", name: "Data Loss Prevention", type: "preventive", maturity: "repeatable", score: 71, target: 85, failures: 7, lastTest: "2026-03-22" },
      { id: "dasien-ct-04", name: "Security Information and Event Management", type: "detective", maturity: "managed", score: 85, target: 90, failures: 2, lastTest: "2026-04-05" },
      { id: "dasien-ct-05", name: "Identity and Access Management", type: "preventive", maturity: "defined", score: 76, target: 88, failures: 5, lastTest: "2026-03-28" },
      { id: "dasien-ct-06", name: "Vulnerability Management", type: "corrective", maturity: "managed", score: 80, target: 90, failures: 4, lastTest: "2026-04-02" },
      { id: "dasien-ct-07", name: "Email Security Gateway", type: "preventive", maturity: "managed", score: 90, target: 95, failures: 1, lastTest: "2026-04-08" },
      { id: "dasien-ct-08", name: "Web Application Firewall", type: "preventive", maturity: "managed", score: 87, target: 92, failures: 2, lastTest: "2026-03-30" },
      { id: "dasien-ct-09", name: "Patch Management", type: "corrective", maturity: "defined", score: 68, target: 85, failures: 9, lastTest: "2026-03-18" },
      { id: "dasien-ct-10", name: "Encryption at Rest", type: "preventive", maturity: "managed", score: 93, target: 95, failures: 0, lastTest: "2026-04-10" },
      { id: "dasien-ct-11", name: "Encryption in Transit", type: "preventive", maturity: "optimized", score: 96, target: 98, failures: 0, lastTest: "2026-04-12" },
      { id: "dasien-ct-12", name: "Privileged Access Management", type: "preventive", maturity: "defined", score: 74, target: 88, failures: 6, lastTest: "2026-03-25" },
      { id: "dasien-ct-13", name: "Security Awareness Training", type: "preventive", maturity: "repeatable", score: 65, target: 80, failures: 11, lastTest: "2026-03-20" },
      { id: "dasien-ct-14", name: "Incident Response Plan", type: "corrective", maturity: "managed", score: 78, target: 88, failures: 4, lastTest: "2026-04-03" },
      { id: "dasien-ct-15", name: "Backup and Recovery", type: "corrective", maturity: "managed", score: 84, target: 92, failures: 3, lastTest: "2026-04-06" },
      { id: "dasien-ct-16", name: "Multi-Factor Authentication", type: "preventive", maturity: "managed", score: 91, target: 95, failures: 1, lastTest: "2026-04-09" },
      { id: "dasien-ct-17", name: "Network Traffic Analysis", type: "detective", maturity: "defined", score: 72, target: 85, failures: 5, lastTest: "2026-03-27" },
      { id: "dasien-ct-18", name: "Cloud Security Posture Management", type: "detective", maturity: "repeatable", score: 69, target: 82, failures: 8, lastTest: "2026-03-24" },
      { id: "dasien-ct-19", name: "Container Security Scanning", type: "preventive", maturity: "defined", score: 75, target: 85, failures: 5, lastTest: "2026-04-04" },
      { id: "dasien-ct-20", name: "API Security Gateway", type: "preventive", maturity: "repeatable", score: 70, target: 82, failures: 7, lastTest: "2026-03-29" },
      { id: "dasien-ct-21", name: "Threat Intelligence Platform", type: "detective", maturity: "managed", score: 83, target: 90, failures: 2, lastTest: "2026-04-07" },
      { id: "dasien-ct-22", name: "Database Activity Monitoring", type: "detective", maturity: "defined", score: 67, target: 80, failures: 8, lastTest: "2026-03-21" },
      { id: "dasien-ct-23", name: "Deception Technology", type: "detective", maturity: "initial", score: 55, target: 75, failures: 12, lastTest: "2026-03-16" },
      { id: "dasien-ct-24", name: "Security Orchestration Automation", type: "corrective", maturity: "repeatable", score: 73, target: 85, failures: 6, lastTest: "2026-04-01" },
      { id: "dasien-ct-25", name: "Third-Party Risk Assessment", type: "preventive", maturity: "defined", score: 64, target: 78, failures: 9, lastTest: "2026-03-19" },
    ];
    this._dasienControls = controls;
  }

  private _analyzeDasienControlEffectiveness(): void {
    const typeBreakdown: Record<string, { total: number; avgScore: number; avgFailures: number }> = {};
    this._dasienControls.forEach(c => {
    ...
    this._dasienControlTypeBreakdown = typeBreakdown;
  }

  private _dasienControlOptimization(): Record<string, unknown>[] {
    return this._dasienControls.filter(c => c.score < c.target).map(c => ({
  ...
  // --- Security Data Pipeline Health (DASIEN) ---
  private _initDasienPipelines(): void {
    const pipelines = [
      { id: "dasien-pl-01", name: "SIEM Log Ingestion", status: "healthy", healthScore: 94, latencyMs: 120, freshnessMin: 2, errorRate: 0.02, throughputMbps: 450, recordsPerSec: 12000, backPressure: 0.05, uptime: 99.97 },
      { id: "dasien-pl-02", name: "Threat Feed Aggregation", status: "healthy", healthScore: 91, latencyMs: 300, freshnessMin: 15, errorRate: 0.05, throughputMbps: 85, recordsPerSec: 3200, backPressure: 0.08, uptime: 99.92 },
      { id: "dasien-pl-03", name: "Vulnerability Scan Results", status: "degraded", healthScore: 72, latencyMs: 2500, freshnessMin: 60, errorRate: 0.15, throughputMbps: 25, recordsPerSec: 800, backPressure: 0.35, uptime: 98.50 },
      { id: "dasien-pl-04", name: "Endpoint Telemetry Stream", status: "healthy", healthScore: 88, latencyMs: 450, freshnessMin: 5, errorRate: 0.08, throughputMbps: 280, recordsPerSec: 8500, backPressure: 0.12, uptime: 99.85 },
      { id: "dasien-pl-05", name: "Cloud Audit Log Pipeline", status: "healthy", healthScore: 86, latencyMs: 600, freshnessMin: 10, errorRate: 0.06, throughputMbps: 150, recordsPerSec: 4500, backPressure: 0.10, uptime: 99.80 },
      { id: "dasien-pl-06", name: "Identity Event Stream", status: "warning", healthScore: 78, latencyMs: 1200, freshnessMin: 20, errorRate: 0.12, throughputMbps: 45, recordsPerSec: 1500, backPressure: 0.22, uptime: 99.40 },
      { id: "dasien-pl-07", name: "Network Flow Collection", status: "healthy", healthScore: 82, latencyMs: 800, freshnessMin: 8, errorRate: 0.09, throughputMbps: 650, recordsPerSec: 18000, backPressure: 0.15, uptime: 99.70 },
      { id: "dasien-pl-08", name: "DLP Incident Pipeline", status: "degraded", healthScore: 68, latencyMs: 3500, freshnessMin: 45, errorRate: 0.20, throughputMbps: 18, recordsPerSec: 500, backPressure: 0.42, uptime: 97.80 },
    ];
    this._dasienPipelines = pipelines.map(p => ({ ...p, dependencies: [], slaBreached: p.healthScore < 75, alertThreshold: p.errorRate > 0.15 }));
    this._dasienPipelines[0].dependencies = ["dasien-pl-04", "dasien-pl-07"];
    this._dasienPipelines[4].dependencies = ["dasien-pl-01"];
    this._dasienPipelines[5].dependencies = ["dasien-pl-02"];
    this._dasienPipelines[7].dependencies = ["dasien-pl-04", "dasien-pl-05"];
  }

  private _dasienPipelineTrend(): Record<string, unknown>[] {
    const hours = Array.from({ length: 24 }, (_, i) => `H${String(i).padStart(2, "0")}`);
    return this._dasienPipelines.map(p => ({
  ...
  // --- Security Stakeholder Report Generator (DASIEN) ---
  private _initDasienReportTemplates(): void {
    this._dasienReportTemplates = [
      { id: "dasien-rp-01", name: "Board Security Report", audience: "board", frequency: "quarterly", sections: 6, autoGenerate: true, lastGenerated: "2026-03-31", pages: 12 },
      { id: "dasien-rp-02", name: "Executive Risk Summary", audience: "executive", frequency: "monthly", sections: 8, autoGenerate: true, lastGenerated: "2026-04-01", pages: 8 },
      { id: "dasien-rp-03", name: "Technical Security Review", audience: "technical", frequency: "weekly", sections: 12, autoGenerate: true, lastGenerated: "2026-04-21", pages: 25 },
      { id: "dasien-rp-04", name: "Audit Compliance Report", audience: "audit", frequency: "quarterly", sections: 10, autoGenerate: false, lastGenerated: "2026-03-15", pages: 35 },
      { id: "dasien-rp-05", name: "Regulatory Filing Package", audience: "regulatory", frequency: "annual", sections: 15, autoGenerate: false, lastGenerated: "2025-12-31", pages: 48 },
    ];
  }

  private _generateDasienExecSummary(): string {
    const totalRisk = this._dasienScenarios.reduce((sum: number, s: any) => sum + s.riskScore, 0);
    const avgControl = Math.round(this._dasienControls.reduce((sum: number, c: any) => sum + c.score, 0) / this._dasienControls.length);
    const degradedPipes = this._dasienPipelines.filter((p: any) => p.status !== "healthy").length;
    return `Overall risk exposure: ${(totalRisk / 1000000).toFixed(1)}M. Average control effectiveness: {avgControl}%. {degradedPipes} pipeline(s) need attention. {this._dasienScenarios.filter((s: any) => s.mitigationPriority === "immediate").length} scenarios require immediate action.`;
  }

  // --- Security Technology Radar (DASIEN) ---
  private _initDasienTechRadar(): void {
    this._dasienTechRadar = [
      { id: "dasien-tr-01", name: "AI-Powered Threat Detection", ring: "adopt", category: "Detection", maturity: 4, trend: "up", investment: "high", roi: 3.2, vendorCount: 8 },
      { id: "dasien-tr-02", name: "Zero Trust Architecture", ring: "adopt", category: "Architecture", maturity: 4, trend: "up", investment: "high", roi: 2.8, vendorCount: 12 },
      { id: "dasien-tr-03", name: "SASE/SSE Platform", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.4, vendorCount: 10 },
      { id: "dasien-tr-04", name: "CNAPP Cloud Security", ring: "trial", category: "Cloud", maturity: 3, trend: "up", investment: "medium", roi: 2.1, vendorCount: 7 },
      { id: "dasien-tr-05", name: "Extended Detection and Response", ring: "adopt", category: "Detection", maturity: 4, trend: "stable", investment: "high", roi: 2.9, vendorCount: 9 },
      { id: "dasien-tr-06", name: "Quantum-Safe Cryptography", ring: "assess", category: "Crypto", maturity: 2, trend: "up", investment: "low", roi: 0.8, vendorCount: 4 },
      { id: "dasien-tr-07", name: "Security Service Edge", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.0, vendorCount: 6 },
      { id: "dasien-tr-08", name: "SOAR 2.0 Autonomous SOC", ring: "assess", category: "Operations", maturity: 2, trend: "up", investment: "low", roi: 1.2, vendorCount: 5 },
      { id: "dasien-tr-09", name: "Software Supply Chain Security", ring: "trial", category: "DevSecOps", maturity: 3, trend: "up", investment: "medium", roi: 2.3, vendorCount: 8 },
      { id: "dasien-tr-10", name: "Identity Threat Detection", ring: "adopt", category: "Identity", maturity: 3, trend: "up", investment: "high", roi: 2.7, vendorCount: 7 },
      { id: "dasien-tr-11", name: "Data Security Posture Management", ring: "trial", category: "Data", maturity: 3, trend: "up", investment: "medium", roi: 2.2, vendorCount: 9 },
      { id: "dasien-tr-12", name: "Decentralized Identity Standards", ring: "hold", category: "Identity", maturity: 1, trend: "stable", investment: "low", roi: 0.5, vendorCount: 3 },
    ];
  }

  private _dasienRadarSummary(): Record<string, unknown> {
    const rings: Record<string, number> = { adopt: 0, trial: 0, assess: 0, hold: 0 };
    this._dasienTechRadar.forEach(t => { if (rings[t.ring as keyof typeof rings] !== undefined) rings[t.ring as keyof typeof rings]++; });
    return { adopt: rings.adopt, trial: rings.trial, assess: rings.assess, hold: rings.hold, total: this._dasienTechRadar.length, avgMaturity: 2.8, topInvestment: this._dasienTechRadar.filter(t => t.investment === "high").map(t => t.name) };
  }

  // --- DASIEN Security Compliance Mapping Matrix ---
  private _initDasienComplianceMatrix(): void {
    ...
    this._dasienComplianceMatrix = frameworks.map(fw => ({
  ...
  private _dasienComplianceTrend(): Record<string, unknown>[] {
    const quarters = ["Q1-2025", "Q2-2025", "Q3-2025", "Q4-2025", "Q1-2026", "Q2-2026"];
    return this._dasienComplianceMatrix.map(fw => ({
  ...
  // --- DASIEN Threat Intelligence Correlation Engine ---
  private _initDasienThreatIntel(): void {
    ...
    this._dasienThreatIntel = threatActors.map((actor, i) => ({
  ...
  private _dasienThreatCorrelation(): Record<string, unknown>[] {
    return this._dasienThreatIntel.filter(t => t.riskScore > 70).map(t => ({
  ...
  // --- DASIEN Security Metrics Deep Dive ---
  private _initDasienMetricsDeep(): void {
    ...
    this._dasienMetricsDeep = metricCategories.map(cat => ({
  ...
  private _dasienMetricsHeatmap(): number[][] {
    return this._dasienMetricsDeep.map(cat =>
      cat.metrics.map(m => Math.round((m.value / m.target) * 100))
    );
  }

  // --- DASIEN Incident Cost Modeling ---
  private _initDasienCostModel(): void {
    const incidentTypes = ["Data Breach", "Ransomware", "DDoS", "Insider Threat", "Phishing", "Supply Chain", "Cloud Misconfig", "Zero-Day"];
    this._dasienCostModel = incidentTypes.map(inc => ({
    ...
    this._dasienCostModel.forEach(m => { m.totalAnnualExposure = m.avgCost * m.frequency; });
  }

  private _dasienCostProjection(): Record<string, unknown>[] {
    return this._dasienCostModel.map(m => ({
  ...
  // --- DASIEN Security Architecture Pattern Library ---
  private _initDasienArchPatterns(): void {
    ...
    this._dasienArchPatterns = patterns.map((p, i) => ({
  ...
  private _dasienArchPatternAnalysis(): Record<string, unknown> {
    const implemented = this._dasienArchPatterns.filter(p => p.implementation > 75).length;
    const avgCoverage = Math.round(this._dasienArchPatterns.reduce((s: number, p: any) => s + p.coverage, 0) / this._dasienArchPatterns.length);
    return {
      totalPatterns: this._dasienArchPatterns.length, fullyImplemented: implemented, avgCoverage,
      avgRiskReduction: Math.round(this._dasienArchPatterns.reduce((s: number, p: any) => s + p.riskReduction, 0) / this._dasienArchPatterns.length),
      gaps: this._dasienArchPatterns.filter(p => p.implementation < 60).map(p => p.pattern),
      recommendations: this._dasienArchPatterns.filter(p => p.implementation < 70).map(p => ({ pattern: p.pattern, current: p.implementation, target: 85, effort: p.costComplexity }))
    };
  }
  ...
  private _625RiskAssessmentWorkflow = [
  ...
  private _625TreatmentOptions = [
    {id: '625-T-001', riskId: 'RSK-042', risk: 'Ransomware Attack on Critical Infrastructure', before: 20, after: 6, strategy: 'Mitigate', controls: ['Next-gen EDR with behavioral detection and automated response', 'Zero-trust network micro-segmentation isolating critical assets', 'Immutable offline backup rotation with quarterly recovery testing', 'Incident response retainer with 1-hour response SLA'], cost: '$450K/yr', roi: '78% reduction vs $2.8M potential loss', timeline: 'Q3 2026', status: 'approved' as const},
  ...
  private _625TalentPositions = [
    ...
    {id: '625-H-002', title: 'Cloud Security Engineer', dept: 'Cloud Security', level: 'Mid-Senior', loc: 'SF/Remote', salary: '$155K-$180K', posted: '2026-03-20', applicants: 45, pipeline: 12, interviews: 6, offers: 2, urgency: 'high' as const, manager: 'Mike Johnson', skills: ['AWS/Azure/GCP', 'Terraform', 'Kubernetes', 'IAM', 'CSPM'], nice: ['CCSP', 'CKS', 'Serverless']},
    {id: '625-H-003', title: 'SOC Analyst L2', dept: 'Security Operations', level: 'Mid', loc: 'Austin TX', salary: '$105K-$130K', posted: '2026-04-01', applicants: 67, pipeline: 18, interviews: 8, offers: 0, urgency: 'medium' as const, manager: 'Lisa Park', skills: ['SIEM', 'Incident triage', 'Malware analysis', 'TCP/IP'], nice: ['GCIA', 'SOAR', 'Kill chain']},
    {id: '625-H-004', title: 'Staff Security Architect', dept: 'Architecture', level: 'Staff', loc: 'Remote US', salary: '$200K-$240K', posted: '2026-02-28', applicants: 12, pipeline: 3, interviews: 2, offers: 0, urgency: 'critical' as const, manager: 'David Lee', skills: ['Enterprise arch', 'Zero trust', 'Risk frameworks', 'Board comms'], nice: ['CISSP+SABSA', 'Team building', 'Speaker']},
    {id: '625-H-005', title: 'GRC Analyst', dept: 'GRC', level: 'Mid', loc: 'NY/Remote', salary: '$110K-$135K', posted: '2026-04-05', applicants: 34, pipeline: 9, interviews: 3, offers: 1, urgency: 'medium' as const, manager: 'Rachel Green', skills: ['ISO 27001', 'SOC 2', 'Risk assessment', 'Audit'], nice: ['CRISC', 'CISA', 'GDPR']},
    {id: '625-H-006', title: 'Threat Intel Analyst', dept: 'Threat Intel', level: 'Mid-Senior', loc: 'Remote US', salary: '$140K-$170K', posted: '2026-03-25', applicants: 19, pipeline: 5, interviews: 2, offers: 0, urgency: 'low' as const, manager: 'James Wilson', skills: ['MITRE ATT&CK', 'OSINT', 'Threat modeling', 'Reverse eng'], nice: ['CTIA', 'Nation-state', 'Dark web']},
    {id: '625-H-007', title: 'AppSec Engineer', dept: 'AppSec', level: 'Senior', loc: 'Seattle/Remote', salary: '$160K-$190K', posted: '2026-04-10', applicants: 28, pipeline: 7, interviews: 3, offers: 0, urgency: 'medium' as const, manager: 'Emily Zhang', skills: ['SAST/DAST/SCA', 'Secure SDLC', 'Code review', 'CI/CD'], nice: ['CSSLP', 'Mobile security']},
    {id: '625-H-008', title: 'IAM Engineer', dept: 'IAM', level: 'Mid', loc: 'Remote US', salary: '$135K-$160K', posted: '2026-04-08', applicants: 31, pipeline: 10, interviews: 5, offers: 1, urgency: 'medium' as const, manager: 'Chris Martinez', skills: ['Okta/Azure AD', 'SAML/OIDC', 'PAM', 'RBAC'], nice: ['CISSP', 'Zero trust IAM']}
  ];

  private _625InterviewScorecard = [
  ...
  private _625OnboardingSteps = [
    {id: '625-OB-01', item: 'Provision security-hardened laptop, monitors, peripherals, and configured workstation image with all required security tools', cat: 'IT Setup', day: 'Day 1', owner: 'IT Operations'},
    {id: '625-OB-02', item: 'Create AD account, email, Slack, VPN, SSO enrollment, and all required application access accounts', cat: 'Access', day: 'Day 1', owner: 'IAM Team'},
    {id: '625-OB-03', item: 'Grant security tooling access: SIEM console, EDR dashboard, vuln scanner, pentest platforms, SOAR', cat: 'Tools', day: 'Day 1-2', owner: 'SOC Manager'},
    {id: '625-OB-04', item: 'Complete MFA enrollment, security awareness onboarding course, and acceptable use policy acknowledgment', cat: 'Security', day: 'Day 1', owner: 'Awareness Team'},
    {id: '625-OB-05', item: 'Sign NDA, employment agreement, IP assignment, and code of conduct documents with HR', cat: 'HR', day: 'Day 1', owner: 'Human Resources'},
    {id: '625-OB-06', item: 'Introduction meetings with direct team members and key cross-functional stakeholders', cat: 'Integration', day: 'Week 1', owner: 'Hiring Manager'},
    {id: '625-OB-07', item: 'Security architecture overview, tooling walkthrough, and operational runbook review sessions', cat: 'Training', day: 'Week 1-2', owner: 'Security Architect'},
    {id: '625-OB-08', item: 'Shadow experienced team member on active incident response and threat hunting investigations', cat: 'Mentoring', day: 'Week 2-3', owner: 'SOC Lead'},
    {id: '625-OB-09', item: 'Set up individual certification study plan aligned with role requirements and career goals', cat: 'Development', day: 'Week 2', owner: 'Hiring Manager'},
    {id: '625-OB-10', item: 'Establish 30-60-90 day goals, define success metrics, and schedule first performance checkpoint', cat: 'Goals', day: 'Week 1', owner: 'Hiring Manager'}
  ];

  private _625MetricsDashboard = [
    {id: '625-MTD-001', name: 'Mean Time to Detect (MTTD)', value: '4.2 hours', trend: 'improving' as const, target: '< 4 hours', source: 'SIEM - Splunk', method: 'Median time from first alert to analyst acknowledgment', owner: 'SOC Manager', sla: '4 hours', prev: '5.1 hours', spark: [5.8, 5.4, 5.1, 4.9, 4.7, 4.5, 4.3, 4.2], breakdown: [{src: 'Network IDS', pct: '35%', avg: '3.8h'}, {src: 'Endpoint EDR', pct: '28%', avg: '4.1h'}, {src: 'Email Gateway', pct: '22%', avg: '3.5h'}, {src: 'Cloud Security', pct: '10%', avg: '5.2h'}, {src: 'User Reports', pct: '5%', avg: '6.8h'}]},
    {id: '625-MTD-002', name: 'Mean Time to Respond (MTTR)', value: '2.8 hours', trend: 'stable' as const, target: '< 3 hours', source: 'SOAR - XSOAR', method: 'Median time from acknowledgment to first containment action', owner: 'IR Lead', sla: '3 hours', prev: '2.7 hours', spark: [3.2, 3.0, 2.9, 2.8, 2.8, 2.9, 2.8, 2.8], breakdown: [{src: 'Auto Playbooks', pct: '42%', avg: '1.2h'}, {src: 'Semi-Auto', pct: '33%', avg: '2.5h'}, {src: 'Manual', pct: '20%', avg: '5.1h'}, {src: 'L3 Escalation', pct: '5%', avg: '8.3h'}]},
    {id: '625-MTD-003', name: 'Patch Compliance Rate', value: '97.6%', trend: 'improving' as const, target: '> 95%', source: 'Tenable.io', method: 'Pct of critical/high vulns patched within SLA windows', owner: 'Vuln Manager', sla: '95%', prev: '96.1%', spark: [93.2, 94.1, 94.8, 95.3, 95.9, 96.4, 97.0, 97.6], breakdown: [{src: 'Critical (7d)', pct: '40%', rate: '99.2%'}, {src: 'High (30d)', pct: '35%', rate: '97.8%'}, {src: 'Medium (90d)', pct: '20%', rate: '95.1%'}, {src: 'Low (180d)', pct: '5%', rate: '89.4%'}]},
    {id: '625-MTD-004', name: 'Phishing Click Rate', value: '3.2%', trend: 'improving' as const, target: '< 5%', source: 'KnowBe4 + Proofpoint', method: 'Pct of users clicking simulated phishing links monthly', owner: 'Awareness Lead', sla: '5%', prev: '4.1%', spark: [6.8, 6.2, 5.5, 5.1, 4.6, 4.1, 3.6, 3.2], breakdown: [{src: 'Spear Phishing', pct: '35%', rate: '5.1%'}, {src: 'Generic Phishing', pct: '30%', rate: '2.8%'}, {src: 'BEC Sim', pct: '20%', rate: '3.5%'}, {src: 'SMS Phishing', pct: '15%', rate: '1.9%'}]},
    {id: '625-MTD-005', name: 'Security Posture Score', value: '784', trend: 'improving' as const, target: '> 750', source: 'SecurityScorecard', method: 'Composite: external 30%, compliance 25%, vuln 25%, config 20%', owner: 'CISO', sla: '> 750', prev: '771', spark: [748, 752, 758, 762, 768, 773, 778, 784], breakdown: [{src: 'External Rating', pct: '30%', score: '791'}, {src: 'Compliance', pct: '25%', score: '812'}, {src: 'Vuln Mgmt', pct: '25%', score: '745'}, {src: 'Config', pct: '20%', score: '769'}]},
    {id: '625-MTD-006', name: 'Vulnerability Backlog', value: '142', trend: 'improving' as const, target: '< 150', source: 'Tenable + Qualys', method: 'Total open vulns across all scanned assets tracked weekly', owner: 'Vuln Manager', sla: '< 150', prev: '168', spark: [210, 198, 185, 175, 165, 158, 150, 142], breakdown: [{src: 'Critical', pct: '8%', count: '11'}, {src: 'High', pct: '22%', count: '31'}, {src: 'Medium', pct: '45%', count: '64'}, {src: 'Low', pct: '25%', count: '36'}]}
  ];

  private _625ArchReview = [
    {id: '625-AR-001', cat: 'Network', item: 'Network segmentation between trust zones with dedicated firewalls and granular ACLs', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Micro-segmentation via NSX across all production zones'},
    {id: '625-AR-002', cat: 'Network', item: 'DMZ architecture isolating public services from internal network', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Dual-firewall DMZ with WAF and DDoS mitigation'},
    {id: '625-AR-003', cat: 'Network', item: 'Least-privilege traffic flows with deny-by-default and quarterly review', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Explicit allow rules with automated drift detection'},
    {id: '625-AR-004', cat: 'Network', item: 'Encrypted external communications with IPSec/TLS 1.3 and PFS', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'IKEv2 with PFS, TLS 1.3 for all external APIs'},
    {id: '625-AR-005', cat: 'Identity', item: 'Centralized IdP with SSO and automated SCIM provisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Okta SSO SAML 2.0 for 247 apps with SCIM'},
    {id: '625-AR-006', cat: 'Identity', item: 'RBAC with quarterly reviews and automated deprovisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Quarterly recertification with JIT admin access'},
    {id: '625-AR-007', cat: 'Identity', item: 'PAM controlling all admin and service accounts with session recording', sev: 'high' as const, status: 'fail' as const, reviewer: 'IAM Lead', notes: '3 legacy accounts pending CyberArk enrollment'},
    {id: '625-AR-008', cat: 'Identity', item: 'MFA enforced for all users, hardware tokens for privileged access', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'FIDO2 for admins, push MFA for users, 99.8% enrollment'},
    {id: '625-AR-009', cat: 'Data', item: 'Data classification enforced with automated labeling on all repos', sev: 'high' as const, status: 'partial' as const, reviewer: 'Data Protection Lead', notes: 'Framework deployed, gaps on 4 legacy file shares'},
    {id: '625-AR-010', cat: 'Data', item: 'AES-256 encryption at rest for all sensitive data stores', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'Full-disk, TDE on DBs, SSE-KMS on cloud storage'},
    {id: '625-AR-011', cat: 'Data', item: 'DLP monitoring and preventing unauthorized data exfiltration', sev: 'high' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'DLP active on email, endpoint, cloud, and network'},
    {id: '625-AR-012', cat: 'Data', item: 'Encrypted off-site backups with quarterly restore testing', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Infrastructure Lead', notes: '3-2-1 strategy with immutable backups'},
    {id: '625-AR-013', cat: 'AppSec', item: 'Secure SDLC with SAST, DAST, SCA in all CI/CD pipelines', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: '34 pipelines with security gates and PR blocking'},
    {id: '625-AR-014', cat: 'AppSec', item: 'API gateway with auth, rate limiting, schema validation', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: 'Kong with OAuth2, rate limits, OpenAPI validation'},
    {id: '625-AR-015', cat: 'AppSec', item: 'Container security with image scanning and runtime protection', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Trivy scanning, Falco runtime in 80% of clusters'},
    {id: '625-AR-016', cat: 'Monitoring', item: 'SIEM collecting from all critical systems with correlation rules', sev: 'critical' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'Splunk 98% coverage with 1200+ correlation rules'},
    {id: '625-AR-017', cat: 'Monitoring', item: 'EDR with behavioral detection on all managed endpoints', sev: 'high' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'CrowdStrike on 99.2% endpoints with 24/7 MDR'},
    {id: '625-AR-018', cat: 'Monitoring', item: 'Executive dashboards with real-time posture visibility', sev: 'medium' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'PowerBI dashboards with weekly exec summaries'},
    {id: '625-AR-019', cat: 'Cloud', item: 'CSPM monitoring and remediating cloud misconfigurations', sev: 'high' as const, status: 'pass' as const, reviewer: 'Cloud Security Lead', notes: 'Prisma Cloud 450+ checks with auto-remediation'},
    {id: '625-AR-020', cat: 'Cloud', item: 'IaC templates scanned before deployment to production', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Checkov for Terraform, cfn-nag for CloudFormation pending'}
  ];

  private _get625RiskProgress(): number {
    return Math.round(this._625RiskAssessmentWorkflow.filter(s => s.status === 'completed').length / this._625RiskAssessmentWorkflow.length * 100);
  }

  private _get625TreatmentSummary(): {total: number; approved: number; avgReduction: number; investment: string} {
    const a = this._625TreatmentOptions.filter(r => r.status === 'approved').length;
    const avg = this._625TreatmentOptions.reduce((s, r) => s + ((r.before - r.after) / r.before * 100), 0) / this._625TreatmentOptions.length;
    return {total: this._625TreatmentOptions.length, approved: a, avgReduction: Math.round(avg), investment: '$1.125M annually'};
  }

  private _get625RuleSummary(): {total: number; passing: number; failing: number; avgRate: number; violations: number} {
    const p = this._625ComplianceRules.filter(r => r.result === 'pass').length;
    const f = this._625ComplianceRules.filter(r => r.result === 'fail').length;
    const avg = this._625ComplianceRules.reduce((s, r) => s + r.passRate, 0) / this._625ComplianceRules.length;
    const v = this._625ComplianceRules.reduce((s, r) => s + r.violations, 0);
    return {total: this._625ComplianceRules.length, passing: p, failing: f, avgRate: Math.round(avg * 10) / 10, violations: v};
  }

  private _get625PipelineStats(): {positions: number; applicants: number; interviewing: number; offers: number; critical: number} {
    const t = this._625TalentPositions.length;
    const a = this._625TalentPositions.reduce((s, p) => s + p.applicants, 0);
    const i = this._625TalentPositions.reduce((s, p) => s + p.interviews, 0);
    const o = this._625TalentPositions.reduce((s, p) => s + p.offers, 0);
    const c = this._625TalentPositions.filter(x => x.urgency === 'critical').length;
    return {positions: t, applicants: a, interviewing: i, offers: o, critical: c};
  }

  private _get625MetricTrends(): {improving: number; stable: number; total: number} {
    const imp = this._625MetricsDashboard.filter(m => m.trend === 'improving').length;
    const stb = this._625MetricsDashboard.filter(m => m.trend === 'stable').length;
    return {improving: imp, stable: stb, total: this._625MetricsDashboard.length};
  }

  private _get625ArchScore(): {total: number; passed: number; failed: number; partial: number; score: number} {
    const p = this._625ArchReview.filter(c => c.status === 'pass').length;
    const f = this._625ArchReview.filter(c => c.status === 'fail').length;
    const pa = this._625ArchReview.filter(c => c.status === 'partial').length;
    return {total: this._625ArchReview.length, passed: p, failed: f, partial: pa, score: Math.round((p / this._625ArchReview.length) * 100)};
  }


  private _z625IncidentScenarios = [
    {id: 'z625-IS-001', name: 'Ransomware Outbreak in Production Environment', category: 'Malware', likelihood: 'Medium', impact: 'Critical', riskScore: 15, playbooks: ['IR-Ransomware-Isolate', 'IR-Ransomware-Eradicate', 'IR-Ransomway-Recover'], estimatedMTTD: '15min', estimatedMTTR: '4h', involvedTeams: ['SOC', 'IR', 'Infrastructure', 'Legal', 'Comms'], keyIndicators: ['Mass file encryption events', 'Ransom note deployment', 'Lateral movement via SMB/RDP', 'Backup deletion attempts'], mitigationControls: ['EDR behavioral detection', 'Network segmentation', 'Immutable backups', 'Email filtering'], lessonsLearned: 'Ensure offline backups are tested quarterly and Ransomware-specific playbooks are updated with latest threat intelligence', lastDrillDate: '2026-03-15', nextDrillDate: '2026-06-15'},
    {id: 'z625-IS-002', name: 'Cloud Misconfiguration Leading to Data Exposure', category: 'Cloud', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Cloud-Misconfig-Assess', 'IR-Cloud-Misconfig-Remediate', 'IR-Cloud-Misconfig-Notify'], estimatedMTTD: '2h', estimatedMTTR: '6h', involvedTeams: ['Cloud Security', 'SOC', 'Data Protection', 'Legal'], keyIndicators: ['Public S3 bucket exposure', 'Overly permissive IAM policies', 'Unencrypted database snapshots', 'Public API endpoint discovery'], mitigationControls: ['CSPM monitoring', 'IAM policy analyzer', 'Automated remediation', 'Cloud audit logging'], lessonsLearned: 'Implement CSPM with auto-remediation for common misconfigurations and conduct monthly cloud configuration reviews', lastDrillDate: '2026-02-20', nextDrillDate: '2026-05-20'},
    {id: 'z625-IS-003', name: 'Insider Threat Data Exfiltration', category: 'Insider', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-Insider-Investigate', 'IR-Insider-Contain', 'IR-Insider-Escalate'], estimatedMTTD: '8h', estimatedMTTR: '24h', involvedTeams: ['SOC', 'IR', 'HR', 'Legal', 'IAM'], keyIndicators: ['Anomalous data access patterns', 'Large file downloads to USB or cloud', 'Off-hours access to sensitive systems', 'Accessing data outside normal job scope'], mitigationControls: ['UEBA analytics', 'DLP policies', 'PAM controls', 'Access reviews'], lessonsLearned: 'Strengthen UEBA baselines and implement automated alerts for data access pattern anomalies', lastDrillDate: '2026-01-10', nextDrillDate: '2026-04-10'},
    {id: 'z625-IS-004', name: 'Supply Chain Attack via Compromised Vendor Software', category: 'Supply Chain', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-SupplyChain-Detect', 'IR-SupplyChain-Isolate', 'IR-SupplyChain-Remediate'], estimatedMTTD: '24h', estimatedMTTR: '72h', involvedTeams: ['SOC', 'IR', 'Procurement', 'Vendor Mgmt', 'Legal'], keyIndicators: ['Unexpected network connections from vendor software', 'Modified binaries or dependencies', 'Anomalous behavior from trusted applications', 'SBOM discrepancy alerts'], mitigationControls: ['SCA scanning', 'SBOM monitoring', 'Vendor security assessments', 'Network segmentation'], lessonsLearned: 'Require SBOM from all vendors and implement continuous monitoring of third-party component behavior', lastDrillDate: '2026-03-01', nextDrillDate: '2026-06-01'},
    {id: 'z625-IS-005', name: 'Credential Stuffing Attack on Customer Portal', category: 'External Attack', likelihood: 'High', impact: 'Medium', riskScore: 12, playbooks: ['IR-CredentialStuffing-Detect', 'IR-CredentialStuffing-Block', 'IR-CredentialStuffing-Notify'], estimatedMTTD: '30min', estimatedMTTR: '2h', involvedTeams: ['SOC', 'AppSec', 'Customer Support', 'Comms'], keyIndicators: ['High volume of failed login attempts', 'Login attempts from known compromised credential lists', 'Unusual geographic distribution of login attempts', 'Account takeover indicators'], mitigationControls: ['WAF rate limiting', 'CAPTCHA on login', 'MFA enforcement', 'Credential monitoring service'], lessonsLearned: 'Deploy advanced bot detection on login endpoints and implement proactive credential monitoring for customer accounts', lastDrillDate: '2026-04-01', nextDrillDate: '2026-07-01'},
    {id: 'z625-IS-006', name: 'Phishing Campaign Targeting Executive Team', category: 'Social Engineering', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Phishing-Contain', 'IR-Phishing-Investigate', 'IR-Phishing-Notify'], estimatedMTTD: '1h', estimatedMTTR: '4h', involvedTeams: ['SOC', 'Awareness', 'Executive Office', 'IT'], keyIndicators: ['Sophisticated spear-phishing emails to C-suite', 'Business email compromise indicators', 'Redirected MFA approval requests', 'Unauthorized wire transfer attempts'], mitigationControls: ['AI email filtering', 'Executive awareness training', 'Out-of-band verification policy', 'DMARC/DKIM/SPF'], lessonsLearned: 'Implement additional email security controls for executive mailboxes and conduct monthly spear-phishing simulations', lastDrillDate: '2026-02-15', nextDrillDate: '2026-05-15'}
  ];

  private _z625SecurityFrameworks = [
  ...
  private _getz625ScenarioSummary(): {total: number; critical: number; high: number; drilled: number; avgRiskScore: number} {
    const crit = this._z625IncidentScenarios.filter(s => s.impact === 'Critical').length;
    const high = this._z625IncidentScenarios.filter(s => s.impact === 'High').length;
    const avg = this._z625IncidentScenarios.reduce((s, x) => s + x.riskScore, 0) / this._z625IncidentScenarios.length;
    return {total: this._z625IncidentScenarios.length, critical: crit, high: high, drilled: this._z625IncidentScenarios.length, avgRiskScore: Math.round(avg)};
  }

  private _getz625VendorRiskSummary(): {total: number; approved: number; conditional: number; remediation: number; avgScore: number} {
    const approved = this._z625ThirdPartyRiskData.filter(v => v.status === 'approved').length;
    const cond = this._z625ThirdPartyRiskData.filter(v => v.status === 'conditional').length;
    const rem = this._z625ThirdPartyRiskData.filter(v => v.status === 'remediation_required').length;
    const avg = this._z625ThirdPartyRiskData.reduce((s, v) => s + v.riskScore, 0) / this._z625ThirdPartyRiskData.length;
    return {total: this._z625ThirdPartyRiskData.length, approved, conditional: cond, remediation: rem, avgScore: Math.round(avg)};
  }

  private _getz625TrainingSummary(): {total: number; avgCompletion: number; avgScore: number; overdueUpdates: number} {
    const avgComp = this._z625SecurityTrainingCatalog.reduce((s, t) => s + t.completionRate, 0) / this._z625SecurityTrainingCatalog.length;
    const avgScore = this._z625SecurityTrainingCatalog.filter(t => t.avgScore > 0).reduce((s, t) => s + t.avgScore, 0) / this._z625SecurityTrainingCatalog.filter(t => t.avgScore > 0).length;
    return {total: this._z625SecurityTrainingCatalog.length, avgCompletion: Math.round(avgComp * 10) / 10, avgScore: Math.round(avgScore), overdueUpdates: 0};
  }

  private _getz625ToolHealthSummary(): {total: number; healthy: number; degraded: number; annualCost: string} {
    const healthy = this._z625SecurityToolInventory.filter(t => t.health === 'healthy').length;
    const degraded = this._z625SecurityToolInventory.filter(t => t.health === 'degraded').length;
    const cost = this._z625SecurityToolInventory.reduce((s, t) => s + parseInt(t.annualCost.replace(/[^0-9]/g, '')), 0);
    return {total: this._z625SecurityToolInventory.length, healthy, degraded, annualCost: '$' + (cost / 1000).toFixed(0) + 'K'};
  }


  private _x625SecurityZones = [
    {zoneId: 'x625-Z-001', name: 'Internet DMZ', trustLevel: 'Untrusted', subnet: '10.0.0.0/24', segmentation: 'Dual-firewall', firewallRules: 45, monitoring: 'Full packet capture', assets: ['Web servers', 'Load balancers', 'WAF appliances', 'Reverse proxy'], riskLevel: 'high' as const, lastAssessment: '2026-04-15', nextAssessment: '2026-07-15'},
    {zoneId: 'x625-Z-002', name: 'Corporate Network', trustLevel: 'Trusted', subnet: '10.1.0.0/16', segmentation: 'VLAN + micro-seg', firewallRules: 128, monitoring: 'NetFlow + IDS', assets: ['Workstations', 'Printers', 'Meeting room systems', 'VoIP phones'], riskLevel: 'medium' as const, lastAssessment: '2026-04-10', nextAssessment: '2026-07-10'},
    ...
    {zoneId: 'x625-Z-004', name: 'Management Network', trustLevel: 'Restricted', subnet: '10.3.0.0/24', segmentation: 'Isolated VLAN', firewallRules: 32, monitoring: 'Syslog + SNMP', assets: ['Hypervisors', 'Switch management', 'Storage controllers', 'Backup appliances'], riskLevel: 'low' as const, lastAssessment: '2026-04-08', nextAssessment: '2026-07-08'},
  ...
  private _x625DataClassificationPolicy = [
  ...
  private _getx625ZoneSummary(): {total: number; highRisk: number; mediumRisk: number; lowRisk: number; totalRules: number} {
    const high = this._x625SecurityZones.filter(z => z.riskLevel === 'high').length;
    const med = this._x625SecurityZones.filter(z => z.riskLevel === 'medium').length;
    const low = this._x625SecurityZones.filter(z => z.riskLevel === 'low').length;
    const rules = this._x625SecurityZones.reduce((s, z) => s + z.firewallRules, 0);
    return {total: this._x625SecurityZones.length, highRisk: high, mediumRisk: med, lowRisk: low, totalRules: rules};
  }

  private _getx625AlertSummary(): {total: number; critical: number; high: number; medium: number; low: number; open: number; contained: number} {
    const crit = this._x625SecurityAlerts.filter(a => a.severity === 'critical').length;
    const high = this._x625SecurityAlerts.filter(a => a.severity === 'high').length;
    const med = this._x625SecurityAlerts.filter(a => a.severity === 'medium').length;
    const low = this._x625SecurityAlerts.filter(a => a.severity === 'low').length;
    const open = this._x625SecurityAlerts.filter(a => a.status === 'investigating' || a.status === 'triaged').length;
    const contained = this._x625SecurityAlerts.filter(a => a.status === 'contained' || a.status === 'remediated').length;
    return {total: this._x625SecurityAlerts.length, critical: crit, high, medium: med, low, open, contained};
  }

  private _getx625ComplianceSummary(): {total: number; fullyCompliant: number; mostlyCompliant: number; avgComplianceRate: number} {
    const full = this._x625ComplianceFrameworks.filter(f => f.status === 'compliant').length;
    const mostly = this._x625ComplianceFrameworks.filter(f => f.status === 'mostly_compliant').length;
    const avgRate = this._x625ComplianceFrameworks.reduce((s, f) => s + (f.compliant / f.requirements * 100), 0) / this._x625ComplianceFrameworks.length;
  ...
  // === Security Risk Register (dark_sim_eng) ===
  private _dark_sim_engRiskRegister = [
    { id: "RSK-DAR-0001", title: "Data breach from unpatched systems", owner: "CTO", category: "Financial", status: "mitigating", trend: "stable", severity: "high", likelihood: 9, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-09", nextReview: "2026-07-10" },
    { id: "RSK-DAR-0002", title: "Ransomware attack on critical infrastructure", owner: "CRO", category: "Strategic", status: "closed", trend: "decreasing", severity: "critical", likelihood: 1, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-08", nextReview: "2026-07-14" },
    { id: "RSK-DAR-0003", title: "Insider threat data exfiltration", owner: "VP Eng", category: "Compliance", status: "accepted", trend: "increasing", severity: "medium", likelihood: 5, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-09", nextReview: "2026-07-03" },
    { id: "RSK-DAR-0004", title: "Supply chain compromise", owner: "Security Lead", category: "Technical", status: "open", trend: "stable", severity: "medium", likelihood: 7, impact: 9, treatment: "Monitor and review quarterly", lastReview: "2026-04-10", nextReview: "2026-07-01" },
    { id: "RSK-DAR-0005", title: "Cloud misconfiguration exposure", owner: "Risk Mgr", category: "Reputational", status: "mitigating", trend: "decreasing", severity: "low", likelihood: 2, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-18", nextReview: "2026-07-19" },
    { id: "RSK-DAR-0006", title: "Phishing campaign success rate", owner: "Compliance Dir", category: "Legal", status: "closed", trend: "increasing", severity: "critical", likelihood: 2, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-03", nextReview: "2026-07-01" },
    { id: "RSK-DAR-0007", title: "Third-party vendor data breach", owner: "IT Dir", category: "Regulatory", status: "accepted", trend: "stable", severity: "medium", likelihood: 2, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-13", nextReview: "2026-07-05" },
    { id: "RSK-DAR-0008", title: "Regulatory non-compliance penalty", owner: "DevOps Lead", category: "Third-Party", status: "open", trend: "decreasing", severity: "high", likelihood: 8, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-04", nextReview: "2026-07-25" },
    { id: "RSK-DAR-0009", title: "Zero-day exploit in production", owner: "Architect", category: "Human Capital", status: "mitigating", trend: "increasing", severity: "high", likelihood: 1, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-14", nextReview: "2026-07-01" },
    { id: "RSK-DAR-0010", title: "Insufficient access controls", owner: "CISO", category: "Operational", status: "closed", trend: "stable", severity: "high", likelihood: 1, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-05", nextReview: "2026-07-11" },
    { id: "RSK-DAR-0011", title: "DDoS attack on services", owner: "CTO", category: "Financial", status: "accepted", trend: "decreasing", severity: "critical", likelihood: 4, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-03", nextReview: "2026-07-10" },
    { id: "RSK-DAR-0012", title: "Social engineering attack", owner: "CRO", category: "Strategic", status: "open", trend: "increasing", severity: "high", likelihood: 6, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-10", nextReview: "2026-07-02" },
    { id: "RSK-DAR-0013", title: "API security vulnerability", owner: "VP Eng", category: "Compliance", status: "mitigating", trend: "stable", severity: "low", likelihood: 1, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-06", nextReview: "2026-07-22" },
    { id: "RSK-DAR-0014", title: "Mobile device compromise", owner: "Security Lead", category: "Technical", status: "closed", trend: "decreasing", severity: "high", likelihood: 6, impact: 5, treatment: "Monitor and review quarterly", lastReview: "2026-04-06", nextReview: "2026-07-18" },
    { id: "RSK-DAR-0015", title: "Physical security breach", owner: "Risk Mgr", category: "Reputational", status: "accepted", trend: "increasing", severity: "high", likelihood: 9, impact: 5, treatment: "Monitor and review quarterly", lastReview: "2026-04-01", nextReview: "2026-07-03" },
    { id: "RSK-DAR-0016", title: "Password policy weakness", owner: "Compliance Dir", category: "Legal", status: "open", trend: "stable", severity: "high", likelihood: 2, impact: 9, treatment: "Monitor and review quarterly", lastReview: "2026-04-15", nextReview: "2026-07-16" },
    { id: "RSK-DAR-0017", title: "Network segmentation gap", owner: "IT Dir", category: "Regulatory", status: "mitigating", trend: "decreasing", severity: "low", likelihood: 6, impact: 5, treatment: "Monitor and review quarterly", lastReview: "2026-04-06", nextReview: "2026-07-10" },
    { id: "RSK-DAR-0018", title: "Encryption key management failure", owner: "DevOps Lead", category: "Third-Party", status: "closed", trend: "increasing", severity: "low", likelihood: 2, impact: 5, treatment: "Monitor and review quarterly", lastReview: "2026-04-06", nextReview: "2026-07-27" },
    { id: "RSK-DAR-0019", title: "Audit trail tampering", owner: "Architect", category: "Human Capital", status: "accepted", trend: "stable", severity: "high", likelihood: 1, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-12", nextReview: "2026-07-26" },
    { id: "RSK-DAR-0020", title: "Business email compromise", owner: "CISO", category: "Operational", status: "open", trend: "decreasing", severity: "medium", likelihood: 8, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-04", nextReview: "2026-07-27" },
  ];
  private _dark_sim_engRiskFilter: string = 'all';
  private _dark_sim_engRiskSeverity: string = 'all';
  private _dark_sim_engRiskStatus: string = 'all';
  private _dark_sim_engExpandedRisk: string = '';
  private _dark_sim_engFilterRisks() {
    const reg = this._dark_sim_engRiskRegister;
    return reg.filter(r => {
      if (this._dark_sim_engRiskFilter !== "all" && r.category !== this._dark_sim_engRiskFilter) return false;
      if (this._dark_sim_engRiskSeverity !== "all" && r.severity !== this._dark_sim_engRiskSeverity) return false;
      if (this._dark_sim_engRiskStatus !== "all" && r.status !== this._dark_sim_engRiskStatus) return false;
      return true;
    });
  }
  private _dark_sim_engGetRiskScore(r: any) { return Math.round(r.likelihood * r.impact * 1.5); }
  private _dark_sim_engGetRiskTrendIcon(trend: string) {
    if (trend === "increasing") return "\u2191";
    if (trend === "decreasing") return "\u2193";
    return "\u2192";
  }
  private _dark_sim_engGetRiskColor(sev: string) {
    if (sev === "critical") return "#dc2626";
    if (sev === "high") return "#ea580c";
    if (sev === "medium") return "#d97706";
    return "#16a34a";
  }
  private _dark_sim_engGetRiskCounts() {
    const reg = this._dark_sim_engRiskRegister;
  ...
  private _dark_sim_engGetTreatmentProgress() {
    const reg = this._dark_sim_engRiskRegister;
    const treated = reg.filter(r => r.status === "mitigating" || r.status === "closed").length;
    return Math.round((treated / Math.max(reg.length, 1)) * 100);
  }

  // === Security Metrics API Gateway (dark_sim_eng) ===
  private _dark_sim_engApiEndpoints = [
    { method: "GET", path: "/api/v1/threats", name: "Threat Intelligence Feed", status: "active", avgLatency: 114.6, reqPerMin: 2497.0, errorRate: 0.54, uptime: round(96.80989738876457,2), version: "v3.4.19" },
    { method: "POST", path: "/api/v1/scans/start", name: "Vulnerability Scanner", status: "active", avgLatency: 118.4, reqPerMin: 1898.0, errorRate: 1.22, uptime: round(96.2034752365035,2), version: "v1.6.12" },
    { method: "GET", path: "/api/v1/assets", name: "Asset Inventory", status: "active", avgLatency: 78.7, reqPerMin: 1999.0, errorRate: 1.61, uptime: round(99.33179604407718,2), version: "v2.4.8" },
    { method: "POST", path: "/api/v1/alerts", name: "Alert Management", status: "active", avgLatency: 433.3, reqPerMin: 501.0, errorRate: 1.65, uptime: round(98.22599618899636,2), version: "v2.5.0" },
    { method: "GET", path: "/api/v1/compliance", name: "Compliance Status", status: "active", avgLatency: 359.3, reqPerMin: 361.0, errorRate: 0.84, uptime: round(99.5688071599817,2), version: "v1.6.9" },
    { method: "PUT", path: "/api/v1/policies", name: "Policy Engine", status: "active", avgLatency: 168.5, reqPerMin: 467.0, errorRate: 1.14, uptime: round(98.28963764975907,2), version: "v3.6.19" },
    { method: "GET", path: "/api/v1/incidents", name: "Incident Tracker", status: "active", avgLatency: 159.7, reqPerMin: 1029.0, errorRate: 0.49, uptime: round(97.25685434133297,2), version: "v2.3.10" },
    { method: "POST", path: "/api/v1/forensics", name: "Forensics Collector", status: "degraded", avgLatency: 105.7, reqPerMin: 1257.0, errorRate: 2.13, uptime: round(98.55512038018773,2), version: "v3.9.13" },
    { method: "GET", path: "/api/v1/risk/assess", name: "Risk Assessment", status: "active", avgLatency: 305.4, reqPerMin: 976.0, errorRate: 2.21, uptime: round(96.15104298942838,2), version: "v1.3.10" },
    { method: "POST", path: "/api/v1/auth/verify", name: "Authentication", status: "active", avgLatency: 237.1, reqPerMin: 655.0, errorRate: 0.61, uptime: round(96.91382046403055,2), version: "v2.0.9" },
    { method: "GET", path: "/api/v1/logs/audit", name: "Audit Log Query", status: "active", avgLatency: 130.7, reqPerMin: 499.0, errorRate: 0.32, uptime: round(96.89183497987862,2), version: "v3.0.11" },
    { method: "PUT", path: "/api/v1/users/roles", name: "Role Management", status: "active", avgLatency: 127.4, reqPerMin: 907.0, errorRate: 0.46, uptime: round(98.40369202738223,2), version: "v2.6.4" },
    { method: "POST", path: "/api/v1/encrypt", name: "Encryption Service", status: "active", avgLatency: 387.4, reqPerMin: 1812.0, errorRate: 0.29, uptime: round(98.5489860744257,2), version: "v3.2.18" },
    { method: "GET", path: "/api/v1/network/topo", name: "Network Topology", status: "maintenance", avgLatency: 407.8, reqPerMin: 2304.0, errorRate: 0.68, uptime: round(97.07580663008189,2), version: "v2.0.9" },
    { method: "DELETE", path: "/api/v1/sessions", name: "Session Manager", status: "active", avgLatency: 408.6, reqPerMin: 435.0, errorRate: 2.4, uptime: round(98.76902228380258,2), version: "v2.8.14" },
  ];
  private _dark_sim_engApiKeys = [
    { id: "ak-000001", name: "key-dark-001", created: "2026-06-14", lastUsed: "2026-04-16", status: "active", calls: 39290, rateLimit: 1000 },
    { id: "ak-000002", name: "key-dark-002", created: "2026-06-20", lastUsed: "2026-04-11", status: "active", calls: 40440, rateLimit: 5000 },
    { id: "ak-000003", name: "key-dark-003", created: "2026-02-15", lastUsed: "2026-04-01", status: "active", calls: 38315, rateLimit: 100 },
    { id: "ak-000004", name: "key-dark-004", created: "2026-01-18", lastUsed: "2026-04-19", status: "active", calls: 19598, rateLimit: 100 },
    { id: "ak-000005", name: "key-dark-005", created: "2026-07-05", lastUsed: "2026-04-08", status: "active", calls: 13658, rateLimit: 5000 },
    { id: "ak-000006", name: "key-dark-006", created: "2026-04-15", lastUsed: "2026-04-19", status: "active", calls: 1571, rateLimit: 100 },
    { id: "ak-000007", name: "key-dark-007", created: "2026-03-05", lastUsed: "2026-04-18", status: "active", calls: 27845, rateLimit: 100 },
    { id: "ak-000008", name: "key-dark-008", created: "2026-01-05", lastUsed: "2026-04-20", status: "active", calls: 26234, rateLimit: 5000 },
  ];
  private _dark_sim_engApiHealthSummary() {
    const eps = this._dark_sim_engApiEndpoints;
  ...
  private _dark_sim_engGetApiByMethod(method: string) { return this._dark_sim_engApiEndpoints.filter(e=>e.method===method); }
  private _dark_sim_engGetSlowEndpoints() { return this._dark_sim_engApiEndpoints.filter(e=>e.avgLatency>200).sort((a,b)=>b.avgLatency-a.avgLatency); }
  private _dark_sim_engGetHighErrorEndpoints() { return this._dark_sim_engApiEndpoints.filter(e=>e.errorRate>1.0).sort((a,b)=>b.errorRate-a.errorRate); }

  // === Security Training Effectiveness (dark_sim_eng) ===
  private _dark_sim_engTrainingModules = [
    { id: "TRN-001", name: "Security Fundamentals", completionRate: 61.8, avgScore: 72.5, behaviorChange: 50.8, enrolled: 243, completed: 289, duration: "95min", category: "mandatory" },
    { id: "TRN-002", name: "Phishing Awareness", completionRate: 68.3, avgScore: 62.5, behaviorChange: 46.2, enrolled: 263, completed: 78, duration: "38min", category: "mandatory" },
    { id: "TRN-003", name: "Social Engineering Defense", completionRate: 78.2, avgScore: 81.5, behaviorChange: 54.1, enrolled: 425, completed: 241, duration: "68min", category: "optional" },
    { id: "TRN-004", name: "Password Hygiene", completionRate: 97.5, avgScore: 93.0, behaviorChange: 48.3, enrolled: 419, completed: 208, duration: "98min", category: "mandatory" },
    { id: "TRN-005", name: "Data Classification", completionRate: 84.1, avgScore: 72.8, behaviorChange: 72.9, enrolled: 141, completed: 54, duration: "82min", category: "mandatory" },
    { id: "TRN-006", name: "Incident Response Basics", completionRate: 81.4, avgScore: 71.4, behaviorChange: 42.4, enrolled: 137, completed: 106, duration: "113min", category: "mandatory" },
    { id: "TRN-007", name: "Secure Coding", completionRate: 63.7, avgScore: 77.4, behaviorChange: 59.4, enrolled: 421, completed: 221, duration: "46min", category: "optional" },
    { id: "TRN-008", name: "Cloud Security", completionRate: 76.8, avgScore: 92.6, behaviorChange: 60.1, enrolled: 65, completed: 183, duration: "109min", category: "mandatory" },
    { id: "TRN-009", name: "Mobile Device Security", completionRate: 68.7, avgScore: 60.7, behaviorChange: 70.4, enrolled: 325, completed: 94, duration: "59min", category: "mandatory" },
    { id: "TRN-010", name: "Network Security", completionRate: 72.2, avgScore: 55.0, behaviorChange: 88.7, enrolled: 188, completed: 224, duration: "16min", category: "mandatory" },
    { id: "TRN-011", name: "Physical Security", completionRate: 63.0, avgScore: 73.1, behaviorChange: 76.9, enrolled: 382, completed: 405, duration: "114min", category: "optional" },
    { id: "TRN-012", name: "Regulatory Compliance", completionRate: 49.0, avgScore: 81.5, behaviorChange: 47.8, enrolled: 194, completed: 98, duration: "87min", category: "mandatory" },
    { id: "TRN-013", name: "Risk Management", completionRate: 54.4, avgScore: 65.7, behaviorChange: 90.0, enrolled: 194, completed: 333, duration: "24min", category: "mandatory" },
    { id: "TRN-014", name: "Cryptography Basics", completionRate: 52.8, avgScore: 81.0, behaviorChange: 88.7, enrolled: 223, completed: 264, duration: "19min", category: "mandatory" },
    { id: "TRN-015", name: "Access Control", completionRate: 78.7, avgScore: 89.6, behaviorChange: 87.9, enrolled: 170, completed: 277, duration: "76min", category: "optional" },
    { id: "TRN-016", name: "Vendor Management", completionRate: 74.9, avgScore: 86.2, behaviorChange: 79.2, enrolled: 271, completed: 326, duration: "74min", category: "mandatory" },
  ];
  private _dark_sim_engPhishingResults = [
    { month: "2026-01", sent: 395, clicked: 28, reported: 216, clickRate: round(7.088607594936709,1), reportRate: round(54.68354430379747,1) },
    { month: "2026-02", sent: 345, clicked: 8, reported: 200, clickRate: round(2.318840579710145,1), reportRate: round(57.971014492753625,1) },
    { month: "2026-03", sent: 641, clicked: 22, reported: 254, clickRate: round(3.43213728549142,1), reportRate: round(39.62558502340094,1) },
    { month: "2026-04", sent: 408, clicked: 15, reported: 227, clickRate: round(3.6764705882352944,1), reportRate: round(55.63725490196079,1) },
    { month: "2026-05", sent: 834, clicked: 192, reported: 187, clickRate: round(23.021582733812952,1), reportRate: round(22.422062350119905,1) },
    { month: "2026-06", sent: 541, clicked: 132, reported: 104, clickRate: round(24.399260628465804,1), reportRate: round(19.223659889094268,1) },
  ];
  private _dark_sim_engTrainingROI = { totalInvestment: 88163, avgCostPerEmployee: 565, riskReductionPct: round(random.uniform(15,45),1), incidentReductionPct: round(random.uniform(10,35),1), complianceScoreGain: round(random.uniform(5,25),1) };
  private _dark_sim_engLearningPaths = [
    { name: "Beginner Security Analyst", totalModules: 8, completedModules: 5, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 72, enrolled: 98 },
    { name: "Advanced Threat Hunter", totalModules: 8, completedModules: 17, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 22, enrolled: 171 },
    { name: "Security Architect", totalModules: 11, completedModules: 16, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 31, enrolled: 177 },
    { name: "Incident Responder", totalModules: 18, completedModules: 7, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 42, enrolled: 129 },
    { name: "Compliance Specialist", totalModules: 16, completedModules: 7, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 40, enrolled: 133 },
    { name: "DevSecOps Engineer", totalModules: 8, completedModules: 7, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 60, enrolled: 117 },
  ];
  private _dark_sim_engGetOverallCompletion() {
    const mods = this._dark_sim_engTrainingModules;
    return round(mods.reduce((s,m)=>s+m.completionRate,0)/mods.length,1);
  }
  private _dark_sim_engGetTopPerformers() { return [...this._dark_sim_engTrainingModules].sort((a,b)=>b.avgScore-a.avgScore).slice(0,5); }
  private _dark_sim_engGetModulesNeedingAttention() { return this._dark_sim_engTrainingModules.filter(m=>m.completionRate<70||m.avgScore<65); }

  // === Security Governance Framework (dark_sim_eng) ===
  private _dark_sim_engGovBodies = [
    { name: "Security Steering Committee", members: 8, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-17", nextMeeting: "2026-05-17", quorum: 3 },
    { name: "Risk Management Board", members: 13, chair: "CTO", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-06", nextMeeting: "2026-05-02", quorum: 4 },
    { name: "Data Governance Council", members: 7, chair: "CRO", meetingFreq: "Monthly", lastMeeting: "2026-04-18", nextMeeting: "2026-05-04", quorum: 5 },
    { name: "Compliance Oversight Board", members: 9, chair: "CDO", meetingFreq: "Monthly", lastMeeting: "2026-04-04", nextMeeting: "2026-05-17", quorum: 8 },
    { name: "Architecture Review Board", members: 7, chair: "VP Eng", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-03", nextMeeting: "2026-05-17", quorum: 8 },
    { name: "Change Advisory Board", members: 10, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-04", nextMeeting: "2026-05-20", quorum: 7 },
    { name: "Incident Review Board", members: 8, chair: "CIO", meetingFreq: "As needed", lastMeeting: "2026-04-18", nextMeeting: "2026-05-01", quorum: 4 },
    { name: "Vendor Risk Committee", members: 8, chair: "CFO", meetingFreq: "Quarterly", lastMeeting: "2026-04-12", nextMeeting: "2026-05-26", quorum: 6 },
  ];
  private _dark_sim_engDecisions = [
    { id: "DEC-0001", title: "Adopt zero-trust architecture framework", date: "2026-02-17", status: "approved", owner: "CISO" },
    ...
    { id: "DEC-0004", title: "Mandate MFA for all external access", date: "2026-03-08", status: "pending", owner: "CRO" },
    { id: "DEC-0005", title: "Establish bug bounty program", date: "2026-01-02", status: "approved", owner: "VP Eng" },
    { id: "DEC-0006", title: "Deploy EDR solution enterprise-wide", date: "2026-02-16", status: "approved", owner: "CISO" },
    { id: "DEC-0007", title: "Conduct annual penetration testing", date: "2026-02-12", status: "implemented", owner: "CTO" },
    { id: "DEC-0008", title: "Implement network micro-segmentation", date: "2026-04-13", status: "in-progress", owner: "Security Lead" },
  ...
  private _dark_sim_engPolicyLifecycle = [
    { name: "Information Security Policy", version: "v4.3", status: "active", lastUpdated: "2026-02-07", nextReview: "2026-12-23", owner: "CISO" },
    { name: "Acceptable Use Policy", version: "v2.0", status: "active", lastUpdated: "2026-02-19", nextReview: "2026-8-10", owner: "Legal" },
    { name: "Data Retention Policy", version: "v2.4", status: "under-review", lastUpdated: "2026-01-09", nextReview: "2026-9-09", owner: "DPO" },
    { name: "Access Control Policy", version: "v1.4", status: "active", lastUpdated: "2026-02-07", nextReview: "2026-9-15", owner: "IAM Lead" },
    { name: "Incident Response Policy", version: "v3.2", status: "draft", lastUpdated: "2026-02-28", nextReview: "2026-10-28", owner: "IR Lead" },
    { name: "Business Continuity Plan", version: "v1.2", status: "active", lastUpdated: "2026-02-05", nextReview: "2026-12-21", owner: "BCP Mgr" },
    { name: "Vendor Management Policy", version: "v2.5", status: "active", lastUpdated: "2026-04-24", nextReview: "2026-7-20", owner: "Procurement" },
    { name: "Encryption Standard", version: "v2.1", status: "under-review", lastUpdated: "2026-03-18", nextReview: "2026-8-10", owner: "Security Arch" },
  ];
  private _dark_sim_engGovMaturityScore = { overall: 2.0, riskManagement: 3.6, compliance: 4.5, incidentResponse: 5.8, awareness: 5.7, technology: 2.0 };
  private _dark_sim_engGetPendingDecisions() { return this._dark_sim_engDecisions.filter(d=>d.status==='pending'||d.status==='in-progress'); }
  private _dark_sim_engGetActivePolicies() { return this._dark_sim_engPolicyLifecycle.filter(p=>p.status==='active'); }
  private _dark_sim_engGetEscalationPath() { return ["L1 Analyst","L2 Senior","Security Lead","CISO","Board"]; }

  // === Security Innovation Lab (dark_sim_eng) ===
  private _dark_sim_engInnoProjects = [
    { id: "INN-001", name: "AI-Powered Threat Detection", description: "Machine learning models for real-time threat identification", status: "active", progress: 72, startDate: "2026-01-25", teamSize: 6, budget: 53384, milestones: 6, completedMilestones: 2 },
    { id: "INN-002", name: "Quantum-Resistant Cryptography", description: "Post-quantum encryption algorithm prototyping", status: "research", progress: 35, startDate: "2026-01-04", teamSize: 4, budget: 115869, milestones: 5, completedMilestones: 8 },
    { id: "INN-003", name: "Automated Red Teaming", description: "Autonomous penetration testing framework", status: "active", progress: 58, startDate: "2026-01-25", teamSize: 3, budget: 47152, milestones: 3, completedMilestones: 5 },
    { id: "INN-004", name: "Zero-Knowledge Authentication", description: "Privacy-preserving identity verification", status: "poc", progress: 88, startDate: "2026-02-19", teamSize: 6, budget: 65508, milestones: 4, completedMilestones: 8 },
    { id: "INN-005", name: "Blockchain Audit Trail", description: "Immutable security event logging", status: "active", progress: 45, startDate: "2026-02-17", teamSize: 2, budget: 49139, milestones: 4, completedMilestones: 6 },
    { id: "INN-006", name: "Behavioral Biometrics", description: "Continuous authentication via user behavior patterns", status: "research", progress: 22, startDate: "2026-03-16", teamSize: 5, budget: 111355, milestones: 10, completedMilestones: 1 },
    { id: "INN-007", name: "Deception Grid 2.0", description: "Advanced honeypot network with adaptive responses", status: "active", progress: 65, startDate: "2026-02-02", teamSize: 7, budget: 83899, milestones: 5, completedMilestones: 3 },
    { id: "INN-008", name: "Secure Enclave Integration", description: "Hardware-backed security for critical workloads", status: "poc", progress: 40, startDate: "2026-01-08", teamSize: 8, budget: 18773, milestones: 3, completedMilestones: 6 },
  ];
  private _dark_sim_engTechEvaluations = [
    ...
    { name: "Confidential Computing", status: "planned", score: 7.9, recommendation: "Investigate", vendor: "Azure" },
    { name: "Homomorphic Encryption", status: "evaluating", score: 4.6, recommendation: "Pilot", vendor: "GCP" },
  ...
  private _dark_sim_engCollaborationPartners = [
  ...
  private _dark_sim_engInnoMetrics = { totalProjects: 8, activeProjects: 4, avgTimeToValue: "102 days", pocSuccessRate: round(random.uniform(55,85),1), researchToProduction: round(random.uniform(20,50),1), innovationIndex: round(random.uniform(6.0,9.5),1) };
  private _dark_sim_engGetProjectByStatus(status: string) { return this._dark_sim_engInnoProjects.filter(p=>p.status===status); }
  private _dark_sim_engGetTopEvaluations() { return [...this._dark_sim_engTechEvaluations].sort((a,b)=>b.score-a.score).slice(0,3); }



  // === Compliance Dashboard Extension (dark_sim_eng) ===
  private _dark_sim_engComplianceFrameworks = [
    { name: "SOC 2 Type II", description: "Trust Services Criteria", totalControls: 5, implementedControls: 5, status: "compliant", lastAudit: "2026-02-26", nextAudit: "2026-9-02", evidenceCount: 405 },
    { name: "ISO 27001", description: "Information Security Management", totalControls: 114, implementedControls: 74, status: "partially-compliant", lastAudit: "2026-02-22", nextAudit: "2026-11-22", evidenceCount: 224 },
    { name: "PCI DSS 4.0", description: "Payment Card Industry", totalControls: 12, implementedControls: 8, status: "non-compliant", lastAudit: "2026-01-06", nextAudit: "2026-11-17", evidenceCount: 456 },
    { name: "HIPAA", description: "Health Insurance Portability", totalControls: 18, implementedControls: 16, status: "partially-compliant", lastAudit: "2026-03-26", nextAudit: "2026-9-08", evidenceCount: 291 },
    { name: "GDPR", description: "Data Protection Regulation", totalControls: 99, implementedControls: 70, status: "non-compliant", lastAudit: "2026-02-16", nextAudit: "2026-8-21", evidenceCount: 117 },
    { name: "NIST CSF 2.0", description: "Cybersecurity Framework", totalControls: 6, implementedControls: 3, status: "partially-compliant", lastAudit: "2026-03-21", nextAudit: "2026-9-28", evidenceCount: 344 },
    { name: "FedRAMP", description: "Federal Risk Authorization", totalControls: 15, implementedControls: 10, status: "partially-compliant", lastAudit: "2026-03-01", nextAudit: "2026-11-02", evidenceCount: 382 },
    { name: "SOX", description: "Sarbanes-Oxley Compliance", totalControls: 8, implementedControls: 7, status: "in-review", lastAudit: "2026-01-15", nextAudit: "2026-9-17", evidenceCount: 353 },
    { name: "CIS Controls v8", description: "Center for Internet Security", totalControls: 18, implementedControls: 9, status: "compliant", lastAudit: "2026-01-24", nextAudit: "2026-10-05", evidenceCount: 307 },
    { name: "COBIT 2019", description: "IT Governance Framework", totalControls: 40, implementedControls: 32, status: "in-review", lastAudit: "2026-02-18", nextAudit: "2026-7-03", evidenceCount: 82 },
  ];
  private _dark_sim_engGetComplianceScore() {
    const fw = this._dark_sim_engComplianceFrameworks;
    return round(fw.reduce((s,f)=>s + (f.implementedControls/Math.max(f.totalControls,1))*100, 0) / fw.length, 1);
  }
  private _dark_sim_engGetGaps() {
    return this._dark_sim_engComplianceFrameworks.filter(f => f.status !== "compliant");
  }
  private _dark_sim_engAuditTrail = [
    { id: "AUD-0001", action: "Control tested", auditor: "Internal Audit", date: "2026-04-05", result: "pass", findings: 4 },
    { id: "AUD-0002", action: "Evidence collected", auditor: "External Auditor", date: "2026-04-10", result: "pass", findings: 5 },
    { id: "AUD-0003", action: "Gap identified", auditor: "Security Team", date: "2026-04-21", result: "fail", findings: 1 },
    { id: "AUD-0004", action: "Remediation completed", auditor: "Compliance Officer", date: "2026-04-14", result: "pass", findings: 0 },
    ...
    { id: "AUD-0009", action: "Risk accepted", auditor: "Board Audit", date: "2026-04-07", result: "pass", findings: 3 },
    { id: "AUD-0010", action: "Control enhanced", auditor: "Third Party", date: "2026-04-12", result: "pass", findings: 5 },
  ];

  // === Threat Intelligence Feed Extension (dark_sim_eng) ===
  private _dark_sim_engThreatActors = [
    { name: "APT-29", alias: "Cozy Bear", origin: "Russia", type: "Nation-State", severity: "high", lastActivity: "2026-04-03", targets: "Healthcare", indicators: 246, ttps: 16 },
    { name: "APT-41", alias: "Double Dragon", origin: "China", type: "Nation-State", severity: "critical", lastActivity: "2026-04-18", targets: "Finance", indicators: 377, ttps: 26 },
    { name: "Lazarus Group", alias: "Hidden Cobra", origin: "North Korea", type: "Nation-State", severity: "critical", lastActivity: "2026-04-21", targets: "Manufacturing", indicators: 217, ttps: 21 },
    { name: "FIN7", alias: "Carbanak", origin: "Eastern Europe", type: "Financial", severity: "high", lastActivity: "2026-04-14", targets: "Energy", indicators: 57, ttps: 50 },
    { name: "Conti", alias: "Wizard Spider", origin: "Russia", type: "Ransomware", severity: "critical", lastActivity: "2026-04-04", targets: "Finance", indicators: 325, ttps: 31 },
    { name: "LockBit", alias: "LockBit Gang", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-10", targets: "Energy", indicators: 46, ttps: 11 },
    { name: "Cl0p", alias: "Cl0p Team", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-23", targets: "Government", indicators: 422, ttps: 29 },
    { name: "Sandworm", alias: "Unit 74455", origin: "Russia", type: "Nation-State", severity: "critical", lastActivity: "2026-04-02", targets: "Government", indicators: 330, ttps: 30 },
  ];
  private _dark_sim_engIoCFeed = [
    { id: "ioc-000001", type: "ip", value: "85.147.35.237", confidence: 72, source: "VirusTotal", firstSeen: "2026-04-03", lastSeen: "2026-04-01" },
    { id: "ioc-000002", type: "ip", value: "154.125.205.120", confidence: 50, source: "VirusTotal", firstSeen: "2026-04-22", lastSeen: "2026-04-18" },
    { id: "ioc-000003", type: "ip", value: "98.212.194.82", confidence: 70, source: "VirusTotal", firstSeen: "2026-04-11", lastSeen: "2026-04-19" },
    { id: "ioc-000004", type: "ip", value: "96.143.141.202", confidence: 91, source: "VirusTotal", firstSeen: "2026-04-08", lastSeen: "2026-04-11" },
    { id: "ioc-000005", type: "ip", value: "209.41.63.2", confidence: 48, source: "STIX", firstSeen: "2026-04-07", lastSeen: "2026-04-22" },
    { id: "ioc-000006", type: "ip", value: "175.109.84.202", confidence: 41, source: "MISP", firstSeen: "2026-04-02", lastSeen: "2026-04-03" },
    { id: "ioc-000007", type: "ip", value: "61.2.94.120", confidence: 64, source: "Mandiant", firstSeen: "2026-04-23", lastSeen: "2026-04-11" },
    { id: "ioc-000008", type: "ip", value: "36.189.57.171", confidence: 46, source: "Mandiant", firstSeen: "2026-04-11", lastSeen: "2026-04-05" },
    { id: "ioc-000009", type: "ip", value: "200.179.246.244", confidence: 65, source: "MISP", firstSeen: "2026-04-23", lastSeen: "2026-04-07" },
    { id: "ioc-000010", type: "ip", value: "249.26.23.212", confidence: 69, source: "CrowdStrike", firstSeen: "2026-04-05", lastSeen: "2026-04-03" },
    { id: "ioc-000011", type: "ip", value: "120.219.78.105", confidence: 62, source: "MISP", firstSeen: "2026-04-17", lastSeen: "2026-04-05" },
    { id: "ioc-000012", type: "ip", value: "242.171.146.96", confidence: 94, source: "MISP", firstSeen: "2026-04-06", lastSeen: "2026-04-14" },
    { id: "ioc-000013", type: "ip", value: "117.20.87.52", confidence: 71, source: "VirusTotal", firstSeen: "2026-04-04", lastSeen: "2026-04-08" },
    { id: "ioc-000014", type: "ip", value: "7.14.203.74", confidence: 82, source: "CrowdStrike", firstSeen: "2026-04-22", lastSeen: "2026-04-21" },
    { id: "ioc-000015", type: "ip", value: "131.13.218.225", confidence: 75, source: "MISP", firstSeen: "2026-04-16", lastSeen: "2026-04-16" },
  ];
  private _dark_sim_engGetActiveThreats() { return this._dark_sim_engThreatActors.filter(a => a.severity === 'critical'); }
  private _dark_sim_engGetThreatSummary() {
    const actors = this._dark_sim_engThreatActors;
  ...
  // === Incident Management Extension (dark_sim_eng) ===
  private _dark_sim_engIncidents = [
    { id: "INC-20260001", title: "Unauthorized access detected", severity: "critical", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-22T13:59", affectedAssets: 43, rootCause: "Misconfiguration" },
    { id: "INC-20260002", title: "Malware outbreak on workstation", severity: "high", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-04T05:40", affectedAssets: 41, rootCause: "Credential compromise" },
    { id: "INC-20260003", title: "Data leak from S3 bucket", severity: "medium", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-04T17:04", affectedAssets: 30, rootCause: "Zero-day" },
    { id: "INC-20260004", title: "Phishing campaign targeting finance", severity: "low", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-11T18:47", affectedAssets: 35, rootCause: "Human error" },
    { id: "INC-20260005", title: "DDoS attack on web services", severity: "critical", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-02T12:52", affectedAssets: 34, rootCause: "Policy violation" },
    { id: "INC-20260006", title: "Ransomware encryption attempt", severity: "high", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-22T00:22", affectedAssets: 13, rootCause: "Unknown" },
    { id: "INC-20260007", title: "Insider data exfiltration", severity: "medium", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-04T19:58", affectedAssets: 8, rootCause: "Misconfiguration" },
    { id: "INC-20260008", title: "API key exposure in repo", severity: "low", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-08T00:38", affectedAssets: 21, rootCause: "Credential compromise" },
    { id: "INC-20260009", title: "SQL injection on portal", severity: "critical", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-19T19:25", affectedAssets: 28, rootCause: "Zero-day" },
    { id: "INC-20260010", title: "Brute force on VPN", severity: "high", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-15T09:14", affectedAssets: 5, rootCause: "Human error" },
    { id: "INC-20260011", title: "Supply chain alert from vendor", severity: "medium", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-21T08:27", affectedAssets: 8, rootCause: "Policy violation" },
    { id: "INC-20260012", title: "Suspicious lateral movement", severity: "low", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-09T22:17", affectedAssets: 27, rootCause: "Unknown" },
  ];
  private _dark_sim_engGetIncidentStats() {
    const inc = this._dark_sim_engIncidents;
    return { total: inc.length, open: inc.filter(i=>i.status==="open").length, investigating: inc.filter(i=>i.status==="investigating").length, mttd: 16, mttr: 71 };
  }
  private _dark_sim_engGetSeverityDistribution() {
    const inc = this._dark_sim_engIncidents;
  ...
  // === Security Threat Landscape Dashboard (DaSiEn) ===
  private _DaSiEnLThreatData = {} as Record<string, unknown[]>;
  private _DaSiEnLThreatReady = false;
  private _DaSiEnLInitThreatLandscape() {
    if (this._DaSiEnLThreatReady) return;
    this._DaSiEnLThreatReady = true;
    ...
    this._DaSiEnLThreatData["severityTrend"] = Object.entries(severityTrend).map(([cat, vals]) => ({ category: cat, monthlyScores: vals }));
    ...
    this._DaSiEnLThreatData["emergingRadar"] = emergingThreats.map(([name, severity, vector, score]) => ({
    ...
    this._DaSiEnLThreatData["volumeByVector"] = threatVectors.map(v => ({
      vector: v,
    ...
    this._DaSiEnLThreatData["sophisticationProgression"] = [
    ...
    this._DaSiEnLThreatData["geographicDistribution"] = geoRegions.map(region => ({
    ...
    this._DaSiEnLThreatData["predictionModel"] = {
  ...
  // === Security Controls Gap Analysis (DaSiEn) ===
  private _DaSiEnGControls: Array<Record<string, unknown>> = [];
  private _DaSiEnGGapReady = false;
  private _DaSiEnGInitGapAnalysis() {
    if (this._DaSiEnGGapReady) return;
    this._DaSiEnGGapReady = true;
    ...
    this._DaSiEnGControls = controlNames.map((name, idx) => {
    ...
    this._DaSiEnGControls.sort((a, b) => (b.remediationPriority as number) - (a.remediationPriority as number));
    this.requestUpdate();
  }
  private _DaSiEnGGetGapStats() {
    const total = this._DaSiEnGControls.length;
    const critical = this._DaSiEnGControls.filter(c => c.gapSeverity === "critical").length;
    const high = this._DaSiEnGControls.filter(c => c.gapSeverity === "high").length;
    const medium = this._DaSiEnGControls.filter(c => c.gapSeverity === "medium").length;
    const low = this._DaSiEnGControls.filter(c => c.gapSeverity === "low").length;
    const totalEffort = this._DaSiEnGControls.reduce((s, c) => s + (c.estimatedEffortDays as number), 0);
    const avgImpl = this._DaSiEnGControls.reduce((s, c) => s + (c.implementationPct as number), 0) / total;
    return { total, critical, high, medium, low, totalEffort, avgImplementation: Math.round(avgImpl) };
  }

  // === Security Program ROI Calculator (DaSiEn) ===
  private _DaSiEnRRoiData: Record<string, unknown> = {};
  private _DaSiEnRRoiReady = false;
  private _DaSiEnRInitRoiCalc() {
    if (this._DaSiEnRRoiReady) return;
    this._DaSiEnRRoiReady = true;
    ...
    this._DaSiEnRRoiData["categories"] = roiDetails;
    ...
    this._DaSiEnRRoiData["executiveSummary"] = {
  ...
  private _DaSiEnRGetRoiGauge(value: number): string {
  ...
  // === Security Team Workload Balancer (DaSiEn) ===
  private _DaSiEnWTMembers: Array<Record<string, unknown>> = [];
  private _DaSiEnWTReady = false;
  private _DaSiEnWTInitWorkload() {
    if (this._DaSiEnWTReady) return;
    this._DaSiEnWTReady = true;
    ...
    this._DaSiEnWTMembers = members.map((name, idx) => {
  ...
  private _DaSiEnWTGetTeamStats() {
    const members = this._DaSiEnWTMembers;
  ...
  // === Security Regulatory Tracker (DaSiEn) ===
  private _DaSiEnRTRegs: Array<Record<string, unknown>> = [];
  private _DaSiEnRTChanges: Array<Record<string, unknown>> = [];
  private _DaSiEnRTReady = false;
  private _DaSiEnRTInitRegTracker() {
    if (this._DaSiEnRTReady) return;
    this._DaSiEnRTReady = true;
    ...
    this._DaSiEnRTRegs = regulations.map((reg, idx) => {
    ...
    this._DaSiEnRTChanges = changeDescriptions.map((desc, idx) => ({
  ...
  private _DaSiEnRTGetComplianceSummary() {
    const regs = this._DaSiEnRTRegs;
  ...
  @state() private _dseAle: number = 0;
  @state() private _dseSroi: number = 0;
  @state() private _dseCpi: number = 0;
  @state() private _dseBudgetAlloc: number = 0;
  @state() private _dseCostBenefit: number = 0;

  // Security Economics Calculator
  private dseInitEconomics() {
    this._dseAle = Math.round(2850000 + Math.random() * 4500000);
    this._dseSroi = Math.round(180 + Math.random() * 320);
    this._dseCpi = Math.round(45000 + Math.random() * 120000);
    this._dseBudgetAlloc = Math.round(2500000 + Math.random() * 3000000);
    this._dseCostBenefit = Math.round(220 + Math.random() * 180);
  }

  private _dseCalcAle(): { annual: number; perIncident: number; byCategory: Array<{name: string; value: number}> } {
    const base = this._dseAle;
    const categories = [
  ...
  private _dseCalcSroi(): Array<{year: number; investment: number; savings: number; roi: number}> {
    const baseInv = this._dseSroi * 10000;
  ...
  private _dseGetBudgetAlloc(): Array<{category: string; amount: number; pct: number; trend: string}> {
    const total = this._dseBudgetAlloc;
    const items = [
  ...
  private _dseGetCostBenefit(): Array<{control: string; cost: number; benefit: number; ratio: number; priority: string}> {
    const base = this._dseCostBenefit;
    const controls = [
  ...
  private _dseRenderEconomics() {
    const ale = this._dseCalcAle();
    const roi = this._dseCalcSroi();
    const budget = this._dseGetBudgetAlloc();
    const cb = this._dseGetCostBenefit();
    const cpi = this._dseCpi;
  ...
  @state() private _dseThreatLevel: any = null;
  @state() private _dseEmergingThreats: any = null;
  @state() private _dseThreatTrends: any = null;
  @state() private _dseSectorRadar: any = null;
  @state() private _dseActorActivity: any = null;

  // Threat Landscape Intelligence
  private dseInitThreatIntel() {
    this._dseThreatLevel = {
  ...
  private _dseRenderThreatIntel() {
    const tl = this._dseThreatLevel;
    const et = this._dseEmergingThreats;
    const sr = this._dseSectorRadar;
    const aa = this._dseActorActivity;
  ...
  @state() private _dsePolicies: any = null;
  @state() private _dseExceptions: any = null;
  @state() private _dseRiskRegister: any = null;
  @state() private _dseMeetings: any = null;
  @state() private _dseDeadlines: any = null;

  // Security Governance Dashboard
  private dseInitGovernance() {
    this._dsePolicies = [
  ...
  private _dseRenderGovernance() {
    const policies = this._dsePolicies;
    const risks = this._dseRiskRegister;
    const deadlines = this._dseDeadlines;
  ...
  @state() private _dseCriticalAssets: any = null;
  @state() private _dseAssetDeps: any = null;
  @state() private _dseEolAssets: any = null;
  @state() private _dseAssetRisk: any = null;

  // Asset Intelligence
  private dseInitAssetIntel() {
    this._dseCriticalAssets = [
    ...
    this._dseAssetRisk = { critical: 7, high: 23, medium: 45, low: 128, total: 203 };
  }

  private _dseRenderAssetIntel() {
    const assets = this._dseCriticalAssets;
    const eol = this._dseEolAssets;
    const ar = this._dseAssetRisk;
  ...
  @state() private _dseUserBaseline: any = null;
  @state() private _dseAnomalyRules: any = null;
  @state() private _dseDataAccess: any = null;
  @state() private _dseInsiderRisk: any = null;
  @state() private _darksir15Timeline: Array<{id:string;time:string;event:string;severity:string;actor:string}> = [
  ...
  @state() private _darksir15RootCauses: Array<{why:string;answer:string}> = [
  ...
  @state() private _darksir15ImpactMatrix: Array<{system:string;users:number;data:string;revenue:string;status:string}> = [
  ...
  @state() private _darksir15Actions: Array<{id:string;item:string;owner:string;deadline:string;priority:string;status:string}> = [
  ...
  @state() private _darksir15Lessons: Array<{id:string;lesson:string;category:string;severity:string;applies_to:string}> = [
  ...
  @state() private _darksir15ActiveTab: string = 'timeline';
  @state() private _darksir15Benchmarks: Array<{metric:string;current:number;industry:number;target:number;unit:string;source:string}> = [
  ...
  @state() private _darksir15MaturityLevels: Array<{domain:string;current:number;target:number;description:string}> = [
  ...
  @state() private _darksir15QuarterlyData: Array<{quarter:string;score:number;improvement:number}> = [
  ...
  @state() private _darksir15SelectedDomain: string = 'all';
  @state() private _darksir15Alerts: Array<{id:string;name:string;severity:number;confidence:number;assetCrit:number;score:number;enriched:boolean;group:string;status:string;enrichData:Array<{key:string;value:string}>}> = [
  ...
  @state() private _darksir15QualityMetrics: {fpRate:number;enrichSuccess:number;avgTriageTime:number;enrichedCount:number;totalCount:number} = {fpRate:0.12, enrichSuccess:0.78, avgTriageTime:4.5, enrichedCount:3, totalCount:5};
  @state() private _darksir15RoutingRules: Array<{name:string;condition:string;channel:string;active:boolean}> = [
  ...
  @state() private _darksir15Shapes: Array<{id:string;type:string;label:string;controls:string[]}> = [
  ...
  @state() private _darksir15TrustBoundaries: Array<{from:string;to:string;label:string;strength:string}> = [
  ...
  @state() private _darksir15ADRs: Array<{id:string;title:string;status:string;date:string;decision:string}> = [
  ...
  @state() private _darksir15SelectedShape: string = '';
  @state() private _darksir15Gauges: Array<{name:string;value:number;max:number;unit:string;status:string;color:string}> = [
  ...
  @state() private _darksir15Anomalies: Array<{id:string;time:string;description:string;severity:string;acknowledged:boolean}> = [
  ...
  @state() private _darksir15Integrations: Array<{name:string;status:string;uptime:number;lastCheck:string;latency:number}> = [
  ...
  @state() private _darksir15AlertFatigue: Array<{analyst:string;alertsPerDay:number;escalated:number;dismissed:number;avgResponseMin:number}> = [
  ...
  @state() private _darksir15SlaTarget: number = 99.9;

  // Insider Threat Detection
  private dseInitInsiderThreat() {
    this._dseUserBaseline = [
    ...
    this._dseInsiderRisk = { totalUsers: 2847, monitored: 342, flagged: 18, investigated: 5, confirmed: 1 };
  }

  private _dseRenderInsiderThreat() {
    const baseline = this._dseUserBaseline;
    const rules = this._dseAnomalyRules;
    const ir = this._dseInsiderRisk;
    ...
    return html`<div class="card" style="padding:14px;margin-bottom:8px">
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:4px;margin-bottom:10px;flex-wrap:wrap">
            ${['timeline','rootcause','impact','actions','lessons'].map(t => html`
              <button class="btn ${this._darksir15ActiveTab === t ? 'btn-primary' : ''}" style="font-size:10px;padding:3px 8px" @click=${() => { this._darksir15ActiveTab = t; }}>${t.charAt(0).toUpperCase() + t.slice(1)}</button>
            `)}
          </div>
          ${this._darksir15ActiveTab === 'timeline' ? html`
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._darksir15Timeline.map(e => html`
          ...
          ` : this._darksir15ActiveTab === 'rootcause' ? html`
            <div style="margin-bottom:8px">${{__html: this._darksir15RenderFishbone()}}</div>
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._darksir15RootCauses.map((rc, i) => html`
          ...
          ` : this._darksir15ActiveTab === 'impact' ? html`
            <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:10px">
              ...
              <tbody>${this._darksir15ImpactMatrix.map(imp => html`
          ...
          ` : this._darksir15ActiveTab === 'actions' ? html`
            <div style="display:flex;gap:12px;margin-bottom:8px;font-size:10px;color:#888">
              ${Object.entries(this._darksir15GetActionStats()).map(([k,v]) => html`<span>${k}: <b style="color:#ddd">${v}</b></span>`)}
            </div>
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._darksir15Actions.map(a => html`
                <div style="display:flex;gap:8px;align-items:center;padding:6px;background:#1a1d27;border-radius:4px;cursor:pointer" @click=${() => this._darksir15ToggleAction(a.id)}>
          ...
          ` : html`
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._darksir15Lessons.map(l => html`
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:4px;margin-bottom:10px;flex-wrap:wrap">
            ${['all', ...this._darksir15MaturityLevels.map(m => m.domain)].map(d => html`
              <button class="btn ${this._darksir15SelectedDomain === d ? 'btn-primary' : ''}" style="font-size:9px;padding:2px 6px" @click=${() => { this._darksir15SelectedDomain = d; }}>${d}</button>
            `)}
          </div>
          <div style="display:flex;gap:16px;margin-bottom:10px">
            <div style="text-align:center"><div style="color:#e0e0e0;font-size:20px;font-weight:bold">${this._darksir15GetOverallMaturity()}/5</div><div style="color:#888;font-size:10px">Maturity Level</div></div>
            <div style="text-align:center"><div style="color:#4f4;font-size:20px;font-weight:bold">${this._darksir15GetGapAnalysis().filter(g => g.isAbove).length}/${this._darksir15Benchmarks.length}</div><div style="color:#888;font-size:10px">Above Industry</div></div>
          </div>
          <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:10px">
            ...
            <tbody>${this._darksir15GetGapAnalysis().map(b => html`
          ...
          <div style="margin-top:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Maturity by Domain</div>
            ${this._darksir15MaturityLevels.filter(m => this._darksir15SelectedDomain === 'all' || m.domain === this._darksir15SelectedDomain).map(m => html`
          ...
          <div style="margin-top:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Quarterly Trend</div>
            <div style="display:flex;height:40px;align-items:flex-end;gap:4px">
              ${this._darksir15QuarterlyData.map(q => html`<div style="flex:1;text-align:center"><div style="background:#4a9eff;height:${q.score * 0.5}px;border-radius:2px 2px 0 0" title="${q.score}"></div><div style="color:#666;font-size:8px;margin-top:2px">${q.quarter}</div></div>`)}
            </div>
          </div>
        </div>
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:12px;margin-bottom:8px;font-size:10px">
            <div style="display:flex;gap:4px;flex-wrap:wrap">${this._darksir15GroupAlerts().map(g => html`<span style="color:#888;padding:2px 6px;background:#1a1d27;border-radius:3px">${g.group}: ${g.count}</span>`)}</div>
          </div>
          <div style="display:flex;gap:8px;margin-bottom:8px;font-size:10px;color:#888">
            <span>FP Rate: <b style="color:${this._darksir15QualityMetrics.fpRate > 0.15 ? '#f44' : '#4f4'}">${(this._darksir15QualityMetrics.fpRate * 100).toFixed(1)}%</b></span>
            <span>Enrich: <b style="color:#4a9eff">${(this._darksir15QualityMetrics.enrichSuccess * 100).toFixed(0)}%</b></span>
            <span>Avg Triage: <b style="color:#ddd">${this._darksir15QualityMetrics.avgTriageTime}m</b></span>
          </div>
          <div style="display:flex;flex-direction:column;gap:4px;max-height:200px;overflow-y:auto">
            ${this._darksir15Alerts.sort((a, b) => this._darksir15CalcScore(b) - this._darksir15CalcScore(a)).map(a => {
              const score = this._darksir15CalcScore(a);
              return html`
                <div style="padding:6px;background:#1a1d27;border-radius:4px;border-left:3px solid ${a.severity >= 4 ? '#f44' : a.severity >= 3 ? '#fa0' : '#4a9eff'}">
                  <div style="display:flex;justify-content:space-between;align-items:center">
                    ...
                    <button class="btn" style="font-size:9px;padding:1px 6px;margin-left:4px" @click=${() => this._darksir15EnrichAlert(a.id)}>${a.enriched ? 'Re-enrich' : 'Enrich'}</button>
                  </div>
          ...
          <div style="margin-top:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Routing Rules</div>
            ${this._darksir15RoutingRules.map(r => html`
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap">
            ${this._darksir15Shapes.map(s => {
              const v = this._darksir15ValidateControl(s.id);
              return html`<div style="padding:4px 8px;background:#1a1d27;border-radius:3px;cursor:pointer;border:1px solid ${this._darksir15SelectedShape === s.id ? '#4a9eff' : '#2a2d37'}" @click=${() => { this._darksir15SelectedShape = this._darksir15SelectedShape === s.id ? '' : s.id; }}>
          ...
          ${this._darksir15SelectedShape ? html`
            ${(() => {
              const shape = this._darksir15Shapes.find(s => s.id === this._darksir15SelectedShape);
              const v = this._darksir15ValidateControl(this._darksir15SelectedShape);
              return shape ? html`
          ...
          <div style="margin-bottom:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Trust Boundaries</div>
            ${this._darksir15TrustBoundaries.map(tb => html`
              <div style="display:flex;gap:6px;align-items:center;padding:3px;background:#1a1d27;border-radius:3px;margin-bottom:2px;font-size:9px">
                <span style="color:#ddd">${this._darksir15Shapes.find(s => s.id === tb.from)?.label || tb.from}</span>
                <span style="color:#4a9eff">\u2194</span>
                <span style="color:#ddd">${this._darksir15Shapes.find(s => s.id === tb.to)?.label || tb.to}</span>
          ...
          <div><div style="color:#888;font-size:10px;margin-bottom:4px">Architecture Decision Records</div>
            ${this._darksir15ADRs.map(adr => html`
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap">
            ${this._darksir15Gauges.map(g => html`
          ...
          <div style="display:flex;gap:8px;margin-bottom:8px;align-items:center">
            ...
            <span style="color:${this._darksir15GetOverallHealth().score >= 99 ? '#4f4' : '#fa0'};font-size:12px;font-weight:bold">${this._darksir15GetOverallHealth().score}%</span>
            <span style="color:#888;font-size:10px">(Target: ${this._darksir15SlaTarget}%)</span>
            <span style="color:#4f4;font-size:10px">${this._darksir15GetOverallHealth().healthy} healthy</span>
            <span style="color:#fa0;font-size:10px">${this._darksir15GetOverallHealth().degraded} degraded</span>
          </div>
          <div style="margin-bottom:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Anomaly Stream</div>
            ${this._darksir15Anomalies.slice(0, 3).map(a => html`
              <div style="display:flex;gap:6px;align-items:center;padding:4px;background:#1a1d27;border-radius:3px;margin-bottom:2px;opacity:${a.acknowledged ? 0.5 : 1}">
                ...
                ${!a.acknowledged ? html`<button class="btn" style="font-size:8px;padding:1px 4px" @click=${() => this._darksir15AckAnomaly(a.id)}>ACK</button>` : html`<span style="color:#4f4;font-size:9px">ACK'd</span>`}
              </div>
            `)}
          </div>
          <div style="margin-bottom:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Integration Health</div>
            ${this._darksir15Integrations.map(i => html`
              <div style="display:flex;gap:6px;align-items:center;padding:3px;background:#1a1d27;border-radius:3px;margin-bottom:2px;font-size:9px">
          ...
          <div><div style="color:#888;font-size:10px;margin-bottom:4px">Alert Fatigue Analysis</div>
            <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:9px">
              ...
              <tbody>${this._darksir15AlertFatigue.map(af => html`
  ...
  private _darksir15RenderFishbone(): string {
  ...
  private _darksir15ToggleAction(id: string) {
    this._darksir15Actions = this._darksir15Actions.map(a => a.id === id ? {...a, status: a.status === 'pending' ? 'in_progress' : a.status === 'in_progress' ? 'completed' : 'pending'} : a);
  }

  private _darksir15GetActionStats() {
    const total = this._darksir15Actions.length;
    const done = this._darksir15Actions.filter(a => a.status === 'completed').length;
    const inProg = this._darksir15Actions.filter(a => a.status === 'in_progress').length;
    return { total, done, inProg, pending: total - done - inProg };
  }

  private _darksir15GetOverallMaturity(): number {
    if (this._darksir15SelectedDomain === 'all') {
      return Math.round(this._darksir15MaturityLevels.reduce((s, m) => s + m.current, 0) / this._darksir15MaturityLevels.length * 10) / 10;
    }
    const d = this._darksir15MaturityLevels.find(m => m.domain === this._darksir15SelectedDomain);
    return d ? d.current : 0;
  }

  private _darksir15GetGapAnalysis() {
    return this._darksir15Benchmarks.map(b => {
  ...
  private _darksir15CalcScore(alert: any): number {
    return Math.round(alert.severity * alert.confidence * alert.assetCrit * 100) / 100;
  }

  private _darksir15EnrichAlert(id: string) {
    this._darksir15Alerts = this._darksir15Alerts.map(a => {
      if (a.id !== id) return a;
      const score = this._darksir15CalcScore(a);
  ...
  private _darksir15GroupAlerts() {
    const groups: Record<string, number> = {};
    this._darksir15Alerts.forEach(a => { groups[a.group] = (groups[a.group] || 0) + 1; });
    return Object.entries(groups).map(([g, c]) => ({group: g, count: c}));
  }

  private _darksir15ValidateControl(shapeId: string): {valid:boolean;missing:string[]} {
    const shape = this._darksir15Shapes.find(s => s.id === shapeId);
  ...
  private _darksir15GetOverallHealth(): {healthy:number;degraded:number;down:number;score:number} {
    const online = this._darksir15Integrations.filter(i => i.status === 'online').length;
    const degraded = this._darksir15Integrations.filter(i => i.status === 'degraded').length;
    const total = this._darksir15Integrations.length;
    return { healthy: online, degraded, down: total - online - degraded, score: Math.round(online / total * 100) };
  }

  private _darksir15AckAnomaly(id: string) {
    this._darksir15Anomalies = this._darksir15Anomalies.map(a => a.id === id ? {...a, acknowledged: true} : a);
  }
...
// ===== SECURITY CONTROL TESTING (R22) =====
  ...
  private _renderBudgetPlanning() {
    const budgetData = [
      { category: "Personnel & Training", planned: 2665000, actual: 3252000, utilization: 122.0, q1: "27%", q2: "32%", q3: "30%", q4: "22%" },
      { category: "Tooling & Licensing", planned: 2672000, actual: 3265000, utilization: 122.2, q1: "30%", q2: "21%", q3: "25%", q4: "23%" },
      { category: "Infrastructure Security", planned: 2679000, actual: 3278000, utilization: 122.4, q1: "17%", q2: "26%", q3: "20%", q4: "24%" },
      { category: "Compliance & Audit", planned: 2686000, actual: 3291000, utilization: 122.5, q1: "20%", q2: "31%", q3: "31%", q4: "25%" },
      { category: "Incident Response", planned: 2693000, actual: 3304000, utilization: 122.7, q1: "23%", q2: "20%", q3: "26%", q4: "10%" },
      { category: "Third-Party Assessments", planned: 2700000, actual: 3317000, utilization: 122.9, q1: "26%", q2: "25%", q3: "21%", q4: "11%" },
      { category: "Security Awareness", planned: 2707000, actual: 3330000, utilization: 123.0, q1: "29%", q2: "30%", q3: "32%", q4: "12%" },
      { category: "Research & Innovation", planned: 2714000, actual: 3343000, utilization: 123.2, q1: "16%", q2: "35%", q3: "27%", q4: "13%" },
    ];
    ...
    const headcount = [
      { team: "SOC Tier 1", current: 12, target: 11, gap: 2, avgSalary: "108k" },
      { team: "SOC Tier 2", current: 4, target: 10, gap: 1, avgSalary: "137k" },
      { team: "Threat Intel", current: 13, target: 9, gap: 0, avgSalary: "166k" },
      { team: "Red Team", current: 5, target: 8, gap: 5, avgSalary: "195k" },
      { team: "GRC", current: 14, target: 7, gap: 4, avgSalary: "113k" },
      { team: "AppSec", current: 6, target: 6, gap: 3, avgSalary: "142k" },
      { team: "Cloud Sec", current: 15, target: 5, gap: 2, avgSalary: "171k" },
      { team: "Identity & Access", current: 7, target: 4, gap: 1, avgSalary: "89k" },
    ];
    const vendorSpend = [
      { vendor: "CrowdStrike", annual: "417k", contractEnd: "2026-09", renewalRisk: "Low", satisfaction: 5 },
      { vendor: "Palo Alto", annual: "448k", contractEnd: "2026-10", renewalRisk: "Medium", satisfaction: 4 },
      { vendor: "Splunk", annual: "479k", contractEnd: "2026-11", renewalRisk: "High", satisfaction: 3 },
      { vendor: "Qualys", annual: "510k", contractEnd: "2026-12", renewalRisk: "Low", satisfaction: 5 },
      { vendor: "Rapid7", annual: "541k", contractEnd: "2026-01", renewalRisk: "Medium", satisfaction: 4 },
      { vendor: "Mandiant", annual: "572k", contractEnd: "2026-02", renewalRisk: "High", satisfaction: 3 },
      { vendor: "Zscaler", annual: "603k", contractEnd: "2026-03", renewalRisk: "Low", satisfaction: 5 },
      { vendor: "Duo Security", annual: "634k", contractEnd: "2026-04", renewalRisk: "Medium", satisfaction: 4 },
    ];
    const roiProjections = [
      { area: "Threat Detection", investment: "339k", projectedReturn: "1265k", roiMultiple: "1.7x", confidence: 92 },
      { area: "Incident Reduction", investment: "382k", projectedReturn: "1312k", roiMultiple: "1.6x", confidence: 73 },
      { area: "Compliance Savings", investment: "425k", projectedReturn: "1359k", roiMultiple: "1.5x", confidence: 90 },
      { area: "Automation Gains", investment: "468k", projectedReturn: "1406k", roiMultiple: "4.0x", confidence: 71 },
      { area: "Risk Avoidance", investment: "511k", projectedReturn: "1453k", roiMultiple: "3.9x", confidence: 88 },
    ];
    return html`
  ...
  private _renderMetricsNormalization() {
    const kpiCatalog = [
      { id: "kpi-1", name: "MTTD", owner: "SOC", unit: "minutes", target: 87, current: 88, benchmark: 88, collection: "auto", frequency: "realtime" },
      { id: "kpi-2", name: "MTTR", owner: "GRC", unit: "%", target: 90, current: 95, benchmark: 65, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-3", name: "MTTC", owner: "AppSec", unit: "score", target: 93, current: 56, benchmark: 76, collection: "manual", frequency: "weekly" },
      { id: "kpi-4", name: "Vuln SLA Compliance", owner: "Cloud Sec", unit: "count", target: 96, current: 63, benchmark: 87, collection: "auto", frequency: "monthly" },
      { id: "kpi-5", name: "Patch Coverage", owner: "Identity", unit: "days", target: 99, current: 70, benchmark: 98, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-6", name: "Phishing Click Rate", owner: "Threat Intel", unit: "minutes", target: 72, current: 77, benchmark: 75, collection: "manual", frequency: "daily" },
      { id: "kpi-7", name: "Training Completion", owner: "Security Ops", unit: "%", target: 75, current: 84, benchmark: 86, collection: "auto", frequency: "weekly" },
      { id: "kpi-8", name: "Escalation Rate", owner: "Risk Mgmt", unit: "score", target: 78, current: 91, benchmark: 97, collection: "semi-auto", frequency: "monthly" },
      { id: "kpi-9", name: "False Positive Rate", owner: "SOC", unit: "count", target: 81, current: 98, benchmark: 74, collection: "manual", frequency: "realtime" },
      { id: "kpi-10", name: "Threat Intel Actionability", owner: "GRC", unit: "days", target: 84, current: 59, benchmark: 85, collection: "auto", frequency: "daily" },
      { id: "kpi-11", name: "Endpoint Compliance", owner: "AppSec", unit: "minutes", target: 87, current: 66, benchmark: 96, collection: "semi-auto", frequency: "weekly" },
      { id: "kpi-12", name: "Cloud Misconfig Score", owner: "Cloud Sec", unit: "%", target: 90, current: 73, benchmark: 73, collection: "manual", frequency: "monthly" },
      { id: "kpi-13", name: "Identity Anomaly Rate", owner: "Identity", unit: "score", target: 93, current: 80, benchmark: 84, collection: "auto", frequency: "realtime" },
      { id: "kpi-14", name: "DLP Events", owner: "Threat Intel", unit: "count", target: 96, current: 87, benchmark: 95, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-15", name: "Vendor Risk Avg", owner: "Security Ops", unit: "days", target: 99, current: 94, benchmark: 72, collection: "manual", frequency: "weekly" },
      { id: "kpi-16", name: "Compliance Audit Pass Rate", owner: "Risk Mgmt", unit: "minutes", target: 72, current: 55, benchmark: 83, collection: "auto", frequency: "monthly" },
      { id: "kpi-17", name: "Awareness Score", owner: "SOC", unit: "%", target: 75, current: 62, benchmark: 94, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-18", name: "SOC Utilization", owner: "GRC", unit: "score", target: 78, current: 69, benchmark: 71, collection: "manual", frequency: "daily" },
      { id: "kpi-19", name: "Automation Coverage", owner: "AppSec", unit: "count", target: 81, current: 76, benchmark: 82, collection: "auto", frequency: "weekly" },
      { id: "kpi-20", name: "Risk Register Currency", owner: "Cloud Sec", unit: "days", target: 84, current: 83, benchmark: 93, collection: "semi-auto", frequency: "monthly" },
    ];
    const benchmarkSources = [
      { source: "NIST CSF", mappedKPIs: 8, alignment: 86, lastReview: "2026-06-04", status: "aligned" },
      { source: "CIS Controls v8", mappedKPIs: 3, alignment: 64, lastReview: "2026-01-27", status: "partial" },
      { source: "ISO 27001:2022", mappedKPIs: 4, alignment: 81, lastReview: "2026-02-22", status: "reviewing" },
      { source: "PCI DSS 4.0", mappedKPIs: 5, alignment: 98, lastReview: "2026-03-17", status: "aligned" },
      { source: "SOC 2 Type II", mappedKPIs: 6, alignment: 76, lastReview: "2026-04-12", status: "partial" },
      { source: "MITRE ATT&CK", mappedKPIs: 7, alignment: 93, lastReview: "2026-05-07", status: "reviewing" },
      { source: "SANS Top 20", mappedKPIs: 8, alignment: 71, lastReview: "2026-06-02", status: "aligned" },
      { source: "OWASP Top 10", mappedKPIs: 3, alignment: 88, lastReview: "2026-01-25", status: "partial" },
    ];
  ...
  private _renderThreatHuntingCampaigns() {
    const campaigns = [
      { id: "HC-1001", name: "Lateral Movement Sweep", status: "active", hypothesis: "H1: Actors using pass-the-hash for lateral movement", leadHunter: "Alice Chen", findings: 27, startDate: "2026-04-24", endDate: null, effectiveness: 46 },
      { id: "HC-1002", name: "Credential Harvesting Hunt", status: "completed", hypothesis: "H2: Actors using web shells for persistence", leadHunter: "Bob Martinez", findings: 30, startDate: "2026-03-07", endDate: "2026-05-13", effectiveness: 65 },
      { id: "HC-1003", name: "Persistence Mechanism Audit", status: "planned", hypothesis: "H3: Actors using scheduled tasks for data theft", leadHunter: "Carol Wu", findings: 33, startDate: "2026-02-18", endDate: null, effectiveness: 84 },
      { id: "HC-1004", name: "C2 Beacon Detection", status: "in-review", hypothesis: "H4: Actors using DNS tunneling for C2 communication", leadHunter: "Dave Kim", findings: 36, startDate: "2026-01-01", endDate: null, effectiveness: 44 },
      { id: "HC-1005", name: "Data Exfiltration Patterns", status: "active", hypothesis: "H5: Actors using encrypted channels for privilege escalation", leadHunter: "Eve Johnson", findings: 39, startDate: "2026-04-12", endDate: null, effectiveness: 63 },
      { id: "HC-1006", name: "Privilege Escalation Scan", status: "completed", hypothesis: "H6: Actors using token impersonation for defense evasion", leadHunter: "Frank Liu", findings: 42, startDate: "2026-03-23", endDate: "2026-06-25", effectiveness: 82 },
      { id: "HC-1007", name: "Supply Chain Implant Hunt", status: "planned", hypothesis: "H7: Actors using poisoned images for initial access", leadHunter: "Grace Park", findings: 45, startDate: "2026-02-06", endDate: null, effectiveness: 42 },
      { id: "HC-1008", name: "Insider Threat Indicators", status: "in-review", hypothesis: "H8: Actors using legitimate tools for credential access", leadHunter: "Hector Silva", findings: 0, startDate: "2026-01-17", endDate: null, effectiveness: 61 },
      { id: "HC-1009", name: "Cloud Metadata Analysis", status: "active", hypothesis: "H9: Actors using API keys for command execution", leadHunter: "Alice Chen", findings: 3, startDate: "2026-04-28", endDate: null, effectiveness: 80 },
      { id: "HC-1010", name: "DNS Tunnel Detection", status: "completed", hypothesis: "H10: Actors using encoded subdomains for exfiltration", leadHunter: "Bob Martinez", findings: 6, startDate: "2026-03-11", endDate: "2026-04-09", effectiveness: 40 },
      { id: "HC-1011", name: "Fileless Malware Search", status: "planned", hypothesis: "H11: Actors using WMI providers for discovery", leadHunter: "Carol Wu", findings: 9, startDate: "2026-02-22", endDate: null, effectiveness: 59 },
      { id: "HC-1012", name: "Zero-Day Exploit Traces", status: "in-review", hypothesis: "H12: Actors using exploit kits for collection", leadHunter: "Dave Kim", findings: 12, startDate: "2026-01-05", endDate: null, effectiveness: 78 },
    ];
    const hunterLeaderboard = [
      { hunter: "Alice Chen", campaigns: 15, findings: 44, highSeverity: 25, avgScore: 62, streak: 4 },
      { hunter: "Bob Martinez", campaigns: 12, findings: 73, highSeverity: 4, avgScore: 55, streak: 5 },
      { hunter: "Carol Wu", campaigns: 9, findings: 102, highSeverity: 9, avgScore: 92, streak: 6 },
      { hunter: "Dave Kim", campaigns: 6, findings: 15, highSeverity: 14, avgScore: 85, streak: 7 },
      { hunter: "Eve Johnson", campaigns: 3, findings: 44, highSeverity: 19, avgScore: 78, streak: 8 },
      { hunter: "Frank Liu", campaigns: 13, findings: 73, highSeverity: 24, avgScore: 71, streak: 1 },
      { hunter: "Grace Park", campaigns: 10, findings: 102, highSeverity: 3, avgScore: 64, streak: 2 },
      { hunter: "Hector Silva", campaigns: 7, findings: 15, highSeverity: 8, avgScore: 57, streak: 3 },
    ];
    const mitreMapping = [
      { tactic: "Initial Access", techniques: 9, campaigns: 4, coverage: 32 },
      { tactic: "Execution", techniques: 8, campaigns: 3, coverage: 83 },
      { tactic: "Persistence", techniques: 7, campaigns: 2, coverage: 63 },
      { tactic: "Privilege Escalation", techniques: 6, campaigns: 1, coverage: 43 },
      { tactic: "Defense Evasion", techniques: 5, campaigns: 6, coverage: 94 },
      { tactic: "Credential Access", techniques: 4, campaigns: 5, coverage: 74 },
      { tactic: "Discovery", techniques: 3, campaigns: 4, coverage: 54 },
      { tactic: "Lateral Movement", techniques: 2, campaigns: 3, coverage: 34 },
      { tactic: "Collection", techniques: 12, campaigns: 2, coverage: 85 },
      { tactic: "Exfiltration", techniques: 11, campaigns: 1, coverage: 65 },
      { tactic: "Command & Control", techniques: 10, campaigns: 6, coverage: 45 },
      { tactic: "Impact", techniques: 9, campaigns: 5, coverage: 96 },
    ];
    return html`
  ...
  private _renderControlInventory() {
    const controls = [
      { id: "CTL-2001", name: "MFA Enforcement", domain: "Access Control", status: "implemented", effectiveness: 43, lastTest: "2026-04-16", nextReview: "2026-08-16", owner: "SOC", risk: "Low" },
      { id: "CTL-2002", name: "Network Segmentation", domain: "Network Security", status: "partial", effectiveness: 41, lastTest: "2026-03-01", nextReview: "2026-09-07", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2003", name: "EDR Deployment", domain: "Endpoint Protection", status: "planned", effectiveness: 48, lastTest: "2026-02-14", nextReview: "2026-10-26", owner: "IT Ops", risk: "High" },
      { id: "CTL-2004", name: "DLP Policy", domain: "Data Protection", status: "gap", effectiveness: 4, lastTest: "2026-01-27", nextReview: "2026-11-17", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2005", name: "SSO Integration", domain: "Identity Management", status: "implemented", effectiveness: 55, lastTest: "2026-04-12", nextReview: "2026-12-08", owner: "IAM", risk: "Low" },
      { id: "CTL-2006", name: "SAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 18, lastTest: "2026-03-25", nextReview: "2026-05-27", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2007", name: "CSPM Scanning", domain: "Cloud Security", status: "planned", effectiveness: 25, lastTest: "2026-02-10", nextReview: "2026-06-18", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2008", name: "Badge Access", domain: "Physical Security", status: "gap", effectiveness: 32, lastTest: "2026-01-23", nextReview: "2026-07-09", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2009", name: "Least Privilege", domain: "Access Control", status: "implemented", effectiveness: 67, lastTest: "2026-04-08", nextReview: "2026-08-28", owner: "SOC", risk: "Low" },
      { id: "CTL-2010", name: "Firewall Rules", domain: "Network Security", status: "partial", effectiveness: 46, lastTest: "2026-03-21", nextReview: "2026-09-19", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2011", name: "Disk Encryption", domain: "Endpoint Protection", status: "planned", effectiveness: 2, lastTest: "2026-02-06", nextReview: "2026-10-10", owner: "IT Ops", risk: "High" },
      { id: "CTL-2012", name: "Data Classification", domain: "Data Protection", status: "gap", effectiveness: 9, lastTest: "2026-01-19", nextReview: "2026-11-01", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2013", name: "PAM Implementation", domain: "Identity Management", status: "implemented", effectiveness: 79, lastTest: "2026-04-04", nextReview: "2026-12-20", owner: "IAM", risk: "Low" },
      { id: "CTL-2014", name: "DAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 23, lastTest: "2026-03-17", nextReview: "2026-05-11", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2015", name: "IAM Policy Review", domain: "Cloud Security", status: "planned", effectiveness: 30, lastTest: "2026-02-02", nextReview: "2026-06-02", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2016", name: "Visitor Management", domain: "Physical Security", status: "gap", effectiveness: 37, lastTest: "2026-01-15", nextReview: "2026-07-21", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2017", name: "Access Reviews", domain: "Access Control", status: "implemented", effectiveness: 91, lastTest: "2026-04-28", nextReview: "2026-08-12", owner: "SOC", risk: "Low" },
      { id: "CTL-2018", name: "IDS/IPS Tuning", domain: "Network Security", status: "partial", effectiveness: 0, lastTest: "2026-03-13", nextReview: "2026-09-03", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2019", name: "Patch Management", domain: "Endpoint Protection", status: "planned", effectiveness: 7, lastTest: "2026-02-26", nextReview: "2026-10-22", owner: "IT Ops", risk: "High" },
      { id: "CTL-2020", name: "Backup Encryption", domain: "Data Protection", status: "gap", effectiveness: 14, lastTest: "2026-01-11", nextReview: "2026-11-13", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2021", name: "Password Policy", domain: "Identity Management", status: "implemented", effectiveness: 44, lastTest: "2026-04-24", nextReview: "2026-12-04", owner: "IAM", risk: "Low" },
      { id: "CTL-2022", name: "Container Scanning", domain: "Application Security", status: "partial", effectiveness: 28, lastTest: "2026-03-09", nextReview: "2026-05-23", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2023", name: "WAF Configuration", domain: "Cloud Security", status: "planned", effectiveness: 35, lastTest: "2026-02-22", nextReview: "2026-06-14", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2024", name: "CCTV Coverage", domain: "Physical Security", status: "gap", effectiveness: 42, lastTest: "2026-01-07", nextReview: "2026-07-05", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2025", name: "RBAC Enforcement", domain: "Access Control", status: "implemented", effectiveness: 56, lastTest: "2026-04-20", nextReview: "2026-08-24", owner: "SOC", risk: "Low" },
      { id: "CTL-2026", name: "VPN Management", domain: "Network Security", status: "partial", effectiveness: 5, lastTest: "2026-03-05", nextReview: "2026-09-15", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2027", name: "App Whitelisting", domain: "Endpoint Protection", status: "planned", effectiveness: 12, lastTest: "2026-02-18", nextReview: "2026-10-06", owner: "IT Ops", risk: "High" },
      { id: "CTL-2028", name: "Key Management", domain: "Data Protection", status: "gap", effectiveness: 19, lastTest: "2026-01-03", nextReview: "2026-11-25", owner: "Data Gov", risk: "Critical" },
    ];
    const gapAnalysis = [
      { gap: "Insufficient MFA coverage for legacy apps", severity: "High", remediationPlan: "Plan R3001", eta: "2026-Q3", estimatedCost: "101k" },
      { gap: "Missing network micro-segmentation", severity: "Medium", remediationPlan: "Plan R3002", eta: "2026-Q2", estimatedCost: "130k" },
      { gap: "Inconsistent EDR deployment", severity: "Medium", remediationPlan: "Plan R3003", eta: "2026-Q4", estimatedCost: "159k" },
      { gap: "DLP not covering cloud storage", severity: "Low", remediationPlan: "Plan R3004", eta: "2026-Q3", estimatedCost: "188k" },
      { gap: "SSO not integrated with all SaaS", severity: "High", remediationPlan: "Plan R3005", eta: "2026-Q2", estimatedCost: "36k" },
    ];
    return html`
  ...
  private _renderIncidentCostTracker() {
    const incidents = [
      { id: "INC-7001", name: "Security Incident 1", severity: "Critical", totalCost: 795000, responseCost: 230550, recoveryCost: 270300, legalCost: 127200, regulatoryCost: 79500, insuranceClaim: 349800, avoidedCost: 160000, date: "2026-04-08" },
      { id: "INC-7002", name: "Security Incident 2", severity: "High", totalCost: 798000, responseCost: 119700, recoveryCost: 191520, legalCost: 103740, regulatoryCost: 39900, insuranceClaim: 0, avoidedCost: 189000, date: "2026-03-17" },
      { id: "INC-7003", name: "Security Incident 3", severity: "Medium", totalCost: 801000, responseCost: 176220, recoveryCost: 280350, legalCost: 80100, regulatoryCost: 88110, insuranceClaim: 312390, avoidedCost: 218000, date: "2026-02-26" },
      { id: "INC-7004", name: "Security Incident 4", severity: "Low", totalCost: 804000, responseCost: 233160, recoveryCost: 201000, legalCost: 56280, regulatoryCost: 48240, insuranceClaim: 0, avoidedCost: 247000, date: "2026-01-07" },
      { id: "INC-7005", name: "Security Incident 5", severity: "Critical", totalCost: 807000, responseCost: 121050, recoveryCost: 290520, legalCost: 161400, regulatoryCost: 96840, insuranceClaim: 274380, avoidedCost: 276000, date: "2026-04-16" },
      { id: "INC-7006", name: "Security Incident 6", severity: "High", totalCost: 810000, responseCost: 178200, recoveryCost: 210600, legalCost: 137700, regulatoryCost: 56700, insuranceClaim: 0, avoidedCost: 305000, date: "2026-03-25" },
      { id: "INC-7007", name: "Security Incident 7", severity: "Medium", totalCost: 813000, responseCost: 235770, recoveryCost: 300810, legalCost: 113820, regulatoryCost: 105690, insuranceClaim: 650400, avoidedCost: 334000, date: "2026-02-06" },
      { id: "INC-7008", name: "Security Incident 8", severity: "Low", totalCost: 816000, responseCost: 122400, recoveryCost: 220320, legalCost: 89760, regulatoryCost: 65280, insuranceClaim: 0, avoidedCost: 363000, date: "2026-01-15" },
      { id: "INC-7009", name: "Security Incident 9", severity: "Critical", totalCost: 819000, responseCost: 180180, recoveryCost: 311220, legalCost: 65520, regulatoryCost: 114660, insuranceClaim: 614250, avoidedCost: 392000, date: "2026-04-24" },
      { id: "INC-7010", name: "Security Incident 10", severity: "High", totalCost: 822000, responseCost: 238380, recoveryCost: 230160, legalCost: 41100, regulatoryCost: 73980, insuranceClaim: 0, avoidedCost: 421000, date: "2026-03-05" },
      { id: "INC-7011", name: "Security Incident 11", severity: "Medium", totalCost: 825000, responseCost: 123750, recoveryCost: 321750, legalCost: 148500, regulatoryCost: 123750, insuranceClaim: 577500, avoidedCost: 450000, date: "2026-02-14" },
      { id: "INC-7012", name: "Security Incident 12", severity: "Low", totalCost: 828000, responseCost: 182160, recoveryCost: 240120, legalCost: 124200, regulatoryCost: 82800, insuranceClaim: 0, avoidedCost: 479000, date: "2026-01-23" },
    ];
    const yearlyTrend = [
      { month: "Jan", incidents: 7, totalCost: "661k", avgCost: "196k", insured: 34 },
      { month: "Feb", incidents: 4, totalCost: "704k", avgCost: "62k", insured: 34 },
      { month: "Mar", incidents: 12, totalCost: "747k", avgCost: "109k", insured: 34 },
      { month: "Apr", incidents: 9, totalCost: "790k", avgCost: "156k", insured: 34 },
      { month: "May", incidents: 6, totalCost: "82k", avgCost: "22k", insured: 34 },
      { month: "Jun", incidents: 3, totalCost: "125k", avgCost: "69k", insured: 34 },
    ];
