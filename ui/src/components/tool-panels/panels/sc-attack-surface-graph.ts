...
export class ScAttackSurfaceGraph extends LitElement {
  ...
  private _data: MockItem[] = [
    {name:"Internet-facing Web App",status:"exposed",risk:"critical",detail:"Port 443, 8080 open. WAF bypass via header splitting. API /graphql introspection enabled."},
    {name:"Internal Database Cluster",status:"segmented",risk:"high",detail:"MySQL 3306 accessible from app tier. No TLS between microservices. Default credentials on replica."},
    {name:"Employee VPN Gateway",status:"exposed",risk:"high",detail:"WireGuard + MFA. Split tunnel enabled. DNS leak detected. 12 expired certificates."},
    {name:"Cloud KMS & Secret Store",status:"protected",risk:"critical",detail:"AWS KMS with CMK rotation. Vault sealed. 3 stale service accounts with read access."},
    {name:"SCADA/OT Network",status:"air-gapped",risk:"critical",detail:"Modbus TCP on 502. No authentication. HMI exposed via jump box. Firmware v2.1 (CVE-2025-xxxx)."},
    {name:"Partner API Integration",status:"monitored",risk:"medium",detail:"SAML SSO with 3 partners. IP whitelist configured. Rate limiting: 1000/min. Last pen test: 90 days ago."}
  ];
  ...
  render() {
    ...
    return html`<div class="panel"><div class="pt">Attack Surface Graph</div>
