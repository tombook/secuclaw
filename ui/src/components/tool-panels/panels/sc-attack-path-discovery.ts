...
`;
  ...
  @state() private _attackPathDiZones: Array<{id:string;name:string;trustLevel:number;subnet:string;devices:number;policy:string;lastAudit:string}> = [
  ...
  @state() private _attackPathDiSegRules: Array<{id:string;source:string;dest:string;action:string;protocol:string;port:string;status:string;hits:number}> = [
  ...
  @state() private _attackPathDiCrossZoneTraffic: Array<{source:string;dest:string;bytes:number;sessions:number;violations:number}> = [
  ...
  @state() private _attackPathDiMicroSegGaps: Array<{id:string;zone:string;gapType:string;severity:string;recommendation:string}> = [
  ...
  private _renderAttackpathdiNetworkSeg(): TemplateResult {
    const zones = this._attackPathDiZones;
    const gaps = this._attackPathDiMicroSegGaps;
    return html`
  ...
  @state() private _attackPathDiCorrRules: Array<{id:string;name:string;sources:string[];logic:string;severity:string;active:boolean;lastTriggered:string}> = [
  ...
  @state() private _attackPathDiEventTimeline: Array<{timestamp:string;source:string;eventType:string;details:string;correlated:boolean}> = [
  ...
  @state() private _attackPathDiFalsePosMetrics: {totalEvents:number;correlatedEvents:number;falsePositives:number;fpRate:number;topFpRules:string[]} = {
  ...
  @state() private _attackPathDiEventPatterns: Array<{id:string;pattern:string;frequency:number;firstSeen:string;lastSeen:string;status:string}> = [
  ...
  private _renderAttackpathdiEventCorr(): TemplateResult {
    const rules = this._attackPathDiCorrRules;
    const timeline = this._attackPathDiEventTimeline;
    const fpMetrics = this._attackPathDiFalsePosMetrics;
    return html`
  ...
  @state() private _attackPathDiContainerScans: Array<{image:string;registry:string;critical:number;high:number;medium:number;scanDate:string;status:string}> = [
  ...
  @state() private _attackPathDiK8sPods: Array<{namespace:string;pod:string;securityContext:string;hostIPC:boolean;hostPID:boolean;privileged:boolean;riskLevel:string}> = [
  ...
  @state() private _attackPathDiServerlessRisk: Array<{function:string;runtime:string;timeout:number;iamPerms:string;externalCalls:number;riskScore:number}> = [
  ...
  @state() private _attackPathDiRuntimeAlerts: Array<{id:string;workload:string;alertType:string;severity:string;description:string;timestamp:string}> = [
  ...
  private _renderAttackpathdiCloudWl(): TemplateResult {
    const containers = this._attackPathDiContainerScans;
    const pods = this._attackPathDiK8sPods;
    const alerts = this._attackPathDiRuntimeAlerts;
    return html`
  ...
  @state() private _attackPathDiRiskTopTen: Array<{id:string;name:string;score:number;trend:string;owner:string;category:string;lastAssessed:string}> = [
  ...
  @state() private _attackPathDiRiskCategories: Array<{category:string;count:number;avgScore:number;color:string}> = [
  ...
  @state() private _attackPathDiRiskVelocity: Array<{week:string;newRisks:number;closedRisks:number;netChange:number}> = [
  ...
  @state() private _attackPathDiRiskAppetite: {current:number;threshold:number;maxTolerated:number;status:string} = {
    current: 68, threshold: 55, maxTolerated: 80, status: 'Warning'
  };
  @state() private _attackPathDiRiskOwnerMatrix: Array<{owner:string;criticalCount:number;highCount:number;mediumCount:number;complianceRate:number}> = [
  ...
  private _renderAttackpathdiRiskDash(): TemplateResult {
    const riskItems = this._attackPathDiRiskTopTen;
    const velocity = this._attackPathDiRiskVelocity;
    const appetite = this._attackPathDiRiskAppetite;
    return html`
  ...
  @state() private _complianceCalendarEvents: Array<{id: string; title: string; deadline: string; category: string; status: string; assignee: string; notes: string}> = [
    {id: 'cc-attack-path-discovery-001', title: 'SOX Section 404 Compliance Review', deadline: '2026-06-30', category: 'Regulatory', status: 'In Progress', assignee: 'Compliance Team', notes: 'Annual internal control assessment for financial reporting'},
    {id: 'cc-attack-path-discovery-002', title: 'GDPR Data Protection Impact Assessment', deadline: '2026-05-15', category: 'Privacy', status: 'Pending', assignee: 'DPO Office', notes: 'Quarterly DPIA for high-risk processing activities'},
    {id: 'cc-attack-path-discovery-003', title: 'ISO 27001 Surveillance Audit', deadline: '2026-07-20', category: 'Certification', status: 'Scheduled', assignee: 'QA Lead', notes: 'Stage 2 surveillance audit for ISMS certification renewal'},
    {id: 'cc-attack-path-discovery-004', title: 'PCI DSS v4.0 Gap Analysis', deadline: '2026-05-30', category: 'Payment', status: 'Not Started', assignee: 'Security Architect', notes: 'Assess current state against PCI DSS v4.0 requirements'},
    {id: 'cc-attack-path-discovery-005', title: 'NIST CSF Self-Assessment', deadline: '2026-08-15', category: 'Framework', status: 'Pending', assignee: 'CISO', notes: 'Biannual self-assessment against NIST Cybersecurity Framework'},
    {id: 'cc-attack-path-discovery-006', title: 'HIPAA Security Rule Audit', deadline: '2026-06-15', category: 'Healthcare', status: 'In Progress', assignee: 'Compliance Analyst', notes: 'Annual audit of administrative, physical, and technical safeguards'},
    {id: 'cc-attack-path-discovery-007', title: 'SOC 2 Type II Report Renewal', deadline: '2026-09-30', category: 'Audit', status: 'Scheduled', assignee: 'External Auditor', notes: 'Engage auditor for SOC 2 Type II examination period'},
    {id: 'cc-attack-path-discovery-008', title: 'FedRAMP Authorization Review', deadline: '2026-07-01', category: 'Government', status: 'Pending', assignee: 'FedRAMP PMO', notes: 'Review and update security authorization package'},
    {id: 'cc-attack-path-discovery-009', title: 'Annual Penetration Test', deadline: '2026-10-15', category: 'Testing', status: 'Not Started', assignee: 'Red Team Lead', notes: 'Comprehensive external and internal penetration testing engagement'},
    {id: 'cc-attack-path-discovery-010', title: 'Security Awareness Training Rollout', deadline: '2026-05-01', category: 'Training', status: 'Completed', assignee: 'Security Awareness Manager', notes: 'Q2 organization-wide security awareness training program'},
    {id: 'cc-attack-path-discovery-011', title: 'Vendor Security Review Cycle', deadline: '2026-06-01', category: 'Third Party', status: 'In Progress', assignee: 'Vendor Manager', notes: 'Quarterly review of critical vendor security posture and SLA compliance'},
    {id: 'cc-attack-path-discovery-012', title: 'Incident Response Plan Update', deadline: '2026-05-20', category: 'Operations', status: 'Pending', assignee: 'IR Team Lead', notes: 'Update IR procedures based on lessons learned from recent incidents'},
  ];
  ...
  @state() private _dataQualityMetrics: Array<{id: string; dataSource: string; completenessScore: number; accuracyScore: number; timelinessScore: number; consistencyScore: number; reliabilityRank: number; lastUpdated: string; issues: string[]; trend: string}> = [
    {id: 'dq-attack-path-discovery-001', dataSource: 'Vulnerability Scanner Feed', completenessScore: 94, accuracyScore: 91, timelinessScore: 88, consistencyScore: 95, reliabilityRank: 1, lastUpdated: '2026-04-23T08:00:00Z', issues: ['Minor delay in Nessus plugin updates'], trend: 'improving'},
    {id: 'dq-attack-path-discovery-002', dataSource: 'SIEM Event Logs', completenessScore: 87, accuracyScore: 93, timelinessScore: 96, consistencyScore: 89, reliabilityRank: 2, lastUpdated: '2026-04-23T07:30:00Z', issues: ['Some log sources showing gaps in off-hours'], trend: 'stable'},
    {id: 'dq-attack-path-discovery-003', dataSource: 'Asset Inventory Database', completenessScore: 82, accuracyScore: 78, timelinessScore: 75, consistencyScore: 84, reliabilityRank: 4, lastUpdated: '2026-04-22T18:00:00Z', issues: ['Shadow IT devices not fully cataloged', 'Decommissioned assets still listed'], trend: 'declining'},
    {id: 'dq-attack-path-discovery-004', dataSource: 'Threat Intelligence Platform', completenessScore: 91, accuracyScore: 89, timelinessScore: 92, consistencyScore: 87, reliabilityRank: 3, lastUpdated: '2026-04-23T06:00:00Z', issues: ['Some IOC sources lack confidence scoring'], trend: 'improving'},
    {id: 'dq-attack-path-discovery-005', dataSource: 'Identity Provider Logs', completenessScore: 96, accuracyScore: 97, timelinessScore: 98, consistencyScore: 96, reliabilityRank: 1, lastUpdated: '2026-04-23T09:00:00Z', issues: [], trend: 'stable'},
    {id: 'dq-attack-path-discovery-006', dataSource: 'Cloud Security Posture Data', completenessScore: 79, accuracyScore: 85, timelinessScore: 82, consistencyScore: 81, reliabilityRank: 5, lastUpdated: '2026-04-23T05:00:00Z', issues: ['Multi-cloud coverage gaps', 'API rate limiting affects scan frequency'], trend: 'declining'},
    {id: 'dq-attack-path-discovery-007', dataSource: 'Compliance Evidence Store', completenessScore: 88, accuracyScore: 86, timelinessScore: 80, consistencyScore: 83, reliabilityRank: 4, lastUpdated: '2026-04-21T14:00:00Z', issues: ['Manual evidence collection delays', 'Inconsistent formatting across frameworks'], trend: 'stable'},
    {id: 'dq-attack-path-discovery-008', dataSource: 'Network Flow Data', completenessScore: 92, accuracyScore: 90, timelinessScore: 94, consistencyScore: 91, reliabilityRank: 2, lastUpdated: '2026-04-23T08:30:00Z', issues: ['Encrypted traffic classification accuracy needs improvement'], trend: 'improving'},
  ];
  ...
  @state() private _workflowTemplates: Array<{id: string; name: string; description: string; category: string; steps: number; avgDuration: string; successRate: number; lastRun: string; manualInterventions: number; optimizationScore: number}> = [
    {id: 'wf-attack-path-discovery-001', name: 'Vulnerability Triage Pipeline', description: 'Automated vulnerability assessment, prioritization, and assignment workflow', category: 'Vulnerability Management', steps: 8, avgDuration: '12 min', successRate: 94.5, lastRun: '2026-04-23T06:00:00Z', manualInterventions: 2, optimizationScore: 87},
    {id: 'wf-attack-path-discovery-002', name: 'Incident Response Orchestration', description: 'End-to-end IR workflow from detection to containment and recovery', category: 'Incident Response', steps: 15, avgDuration: '45 min', successRate: 89.2, lastRun: '2026-04-22T14:30:00Z', manualInterventions: 5, optimizationScore: 72},
    {id: 'wf-attack-path-discovery-003', name: 'Access Request Approval Chain', description: 'Multi-level approval workflow for privileged access requests', category: 'Identity & Access', steps: 6, avgDuration: '4 hours', successRate: 97.1, lastRun: '2026-04-23T09:15:00Z', manualInterventions: 1, optimizationScore: 92},
    {id: 'wf-attack-path-discovery-004', name: 'Compliance Evidence Collection', description: 'Automated gathering and packaging of audit evidence artifacts', category: 'Compliance', steps: 10, avgDuration: '30 min', successRate: 91.8, lastRun: '2026-04-21T10:00:00Z', manualInterventions: 3, optimizationScore: 78},
    {id: 'wf-attack-path-discovery-005', name: 'Security Patch Deployment', description: 'Staged patch rollout with validation and rollback capability', category: 'Patch Management', steps: 12, avgDuration: '2 hours', successRate: 96.3, lastRun: '2026-04-20T22:00:00Z', manualInterventions: 1, optimizationScore: 85},
    {id: 'wf-attack-path-discovery-006', name: 'Threat Intelligence Enrichment', description: 'IOC enrichment and correlation with internal threat data', category: 'Threat Intelligence', steps: 7, avgDuration: '8 min', successRate: 93.7, lastRun: '2026-04-23T07:00:00Z', manualInterventions: 0, optimizationScore: 95},
    {id: 'wf-attack-path-discovery-007', name: 'Security Reporting Pipeline', description: 'Automated generation and distribution of security metrics reports', category: 'Reporting', steps: 5, avgDuration: '15 min', successRate: 98.9, lastRun: '2026-04-23T08:00:00Z', manualInterventions: 0, optimizationScore: 97},
    {id: 'wf-attack-path-discovery-008', name: 'Vendor Risk Assessment', description: 'Automated vendor security questionnaire distribution and scoring', category: 'Third Party', steps: 9, avgDuration: '3 days', successRate: 85.4, lastRun: '2026-04-19T11:00:00Z', manualInterventions: 4, optimizationScore: 65},
  ];
  ...
  @state() private _workflowExecutionHistory: Array<{workflowId: string; runDate: string; duration: string; status: string; trigger: string}> = [
    {workflowId: 'wf-attack-path-discovery-001', runDate: '2026-04-23T06:00:00Z', duration: '11 min 23 sec', status: 'Success', trigger: 'Scheduled'},
    {workflowId: 'wf-attack-path-discovery-002', runDate: '2026-04-22T14:30:00Z', duration: '42 min 15 sec', status: 'Success', trigger: 'Alert'},
    {workflowId: 'wf-attack-path-discovery-003', runDate: '2026-04-23T09:15:00Z', duration: '3h 45 min', status: 'Success', trigger: 'User Request'},
    {workflowId: 'wf-attack-path-discovery-001', runDate: '2026-04-22T06:00:00Z', duration: '13 min 08 sec', status: 'Partial', trigger: 'Scheduled'},
    {workflowId: 'wf-attack-path-discovery-004', runDate: '2026-04-21T10:00:00Z', duration: '28 min 42 sec', status: 'Success', trigger: 'Scheduled'},
  ];
  ...
  @state() private _knowledgeArticles: Array<{id: string; title: string; category: string; author: string; publishDate: string; views: number; rating: number; tags: string[]; summary: string; readTime: string}> = [
    {id: 'kb-attack-path-discovery-001', title: 'Zero Trust Architecture Implementation Guide', category: 'Architecture', author: 'Security Architecture Team', publishDate: '2026-03-15', views: 1842, rating: 4.8, tags: ['zero-trust', 'network', 'identity'], summary: 'Comprehensive guide for implementing zero trust principles across enterprise infrastructure', readTime: '15 min'},
    {id: 'kb-attack-path-discovery-002', title: 'Ransomware Defense Playbook 2026', category: 'Threats', author: 'Threat Intelligence Team', publishDate: '2026-04-01', views: 2341, rating: 4.9, tags: ['ransomware', 'incident-response', 'defense'], summary: 'Updated playbook with latest ransomware tactics, techniques, and defensive measures', readTime: '12 min'},
    {id: 'kb-attack-path-discovery-003', title: 'Cloud Security Best Practices for Multi-Cloud', category: 'Cloud', author: 'Cloud Security Team', publishDate: '2026-02-28', views: 1567, rating: 4.5, tags: ['cloud', 'aws', 'azure', 'gcp'], summary: 'Best practices for securing workloads across multiple cloud service providers', readTime: '18 min'},
    {id: 'kb-attack-path-discovery-004', title: 'API Security Testing Methodology', category: 'Testing', author: 'Red Team', publishDate: '2026-03-22', views: 987, rating: 4.6, tags: ['api', 'testing', 'owasp'], summary: 'Structured methodology for identifying and testing API security vulnerabilities', readTime: '10 min'},
    {id: 'kb-attack-path-discovery-005', title: 'Supply Chain Security Risk Management', category: 'Third Party', author: 'GRC Team', publishDate: '2026-01-20', views: 1234, rating: 4.3, tags: ['supply-chain', 'vendor', 'risk'], summary: 'Framework for managing security risks in the software supply chain', readTime: '14 min'},
    {id: 'kb-attack-path-discovery-006', title: 'Kubernetes Security Hardening Checklist', category: 'Container', author: 'Platform Security', publishDate: '2026-04-10', views: 2103, rating: 4.7, tags: ['kubernetes', 'container', 'hardening'], summary: 'Step-by-step hardening checklist for production Kubernetes clusters', readTime: '11 min'},
    {id: 'kb-attack-path-discovery-007', title: 'Security Metrics That Matter for Executives', category: 'Metrics', author: 'CISO Office', publishDate: '2026-03-05', views: 876, rating: 4.4, tags: ['metrics', 'kpi', 'reporting'], summary: 'Key security metrics and KPIs that resonate with board-level stakeholders', readTime: '8 min'},
    {id: 'kb-attack-path-discovery-008', title: 'Phishing Simulation Campaign Design', category: 'Awareness', author: 'Security Awareness Team', publishDate: '2026-02-15', views: 1456, rating: 4.5, tags: ['phishing', 'simulation', 'training'], summary: 'Designing effective phishing simulations that drive real behavioral change', readTime: '9 min'},
    {id: 'kb-attack-path-discovery-009', title: 'Data Loss Prevention Strategy and Implementation', category: 'Data Protection', author: 'Data Security Team', publishDate: '2026-03-28', views: 1678, rating: 4.6, tags: ['dlp', 'data-protection', 'compliance'], summary: 'Strategic approach to DLP covering people, process, and technology layers', readTime: '16 min'},
    {id: 'kb-attack-path-discovery-010', title: 'Security Automation with SOAR Platforms', category: 'Automation', author: 'SOC Engineering', publishDate: '2026-04-05', views: 1098, rating: 4.7, tags: ['soar', 'automation', 'soc'], summary: 'Building effective security automation playbooks using SOAR technology', readTime: '13 min'},
    {id: 'kb-attack-path-discovery-011', title: 'Third-Party Risk Assessment Framework', category: 'GRC', author: 'Vendor Management', publishDate: '2026-01-30', views: 1345, rating: 4.4, tags: ['vendor', 'risk', 'assessment'], summary: 'Structured framework for evaluating and monitoring third-party security risks', readTime: '12 min'},
    {id: 'kb-attack-path-discovery-012', title: 'Network Detection and Response Best Practices', category: 'Network', author: 'Network Security Team', publishDate: '2026-03-18', views: 1567, rating: 4.5, tags: ['ndr', 'network', 'detection'], summary: 'Implementing effective network detection and response capabilities', readTime: '14 min'},
    {id: 'kb-attack-path-discovery-013', title: 'Security Chaos Engineering Guide', category: 'Resilience', author: 'SRE Security', publishDate: '2026-04-12', views: 789, rating: 4.3, tags: ['chaos-engineering', 'resilience', 'testing'], summary: 'Applying chaos engineering principles to validate security controls', readTime: '10 min'},
    {id: 'kb-attack-path-discovery-014', title: 'Incident Communication Templates', category: 'Incident Response', author: 'IR Team', publishDate: '2026-02-22', views: 2234, rating: 4.8, tags: ['incident', 'communication', 'templates'], summary: 'Ready-to-use communication templates for security incident scenarios', readTime: '7 min'},
    {id: 'kb-attack-path-discovery-015', title: 'Endpoint Detection and Response Configuration', category: 'Endpoint', author: 'Endpoint Security', publishDate: '2026-03-08', views: 1345, rating: 4.5, tags: ['edr', 'endpoint', 'configuration'], summary: 'Optimal EDR configuration for maximum detection with minimal false positives', readTime: '11 min'},
    {id: 'kb-attack-path-discovery-016', title: 'Security Code Review Standards', category: 'Development', author: 'AppSec Team', publishDate: '2026-02-10', views: 1890, rating: 4.6, tags: ['code-review', 'secure-sdlc', 'standards'], summary: 'Standards and checklists for security-focused code reviews', readTime: '13 min'},
    {id: 'kb-attack-path-discovery-017', title: 'IoT Security Assessment Methodology', category: 'IoT', author: 'IoT Security Lab', publishDate: '2026-04-08', views: 654, rating: 4.2, tags: ['iot', 'embedded', 'assessment'], summary: 'Methodology for assessing security posture of IoT devices and ecosystems', readTime: '15 min'},
    {id: 'kb-attack-path-discovery-018', title: 'Passwordless Authentication Migration Guide', category: 'Identity', author: 'IAM Team', publishDate: '2026-03-25', views: 1678, rating: 4.7, tags: ['passwordless', 'authentication', 'mfa'], summary: 'Step-by-step migration plan from password-based to passwordless authentication', readTime: '12 min'},
    {id: 'kb-attack-path-discovery-019', title: 'Security Awareness Program Metrics', category: 'Awareness', author: 'Security Culture Team', publishDate: '2026-01-25', views: 1123, rating: 4.4, tags: ['awareness', 'metrics', 'culture'], summary: 'Measuring the effectiveness of your security awareness and training programs', readTime: '9 min'},
    {id: 'kb-attack-path-discovery-020', title: 'Deception Technology Deployment Guide', category: 'Detection', author: 'Blue Team', publishDate: '2026-04-15', views: 876, rating: 4.5, tags: ['deception', 'honeypot', 'detection'], summary: 'Planning and deploying deception technology for advanced threat detection', readTime: '11 min'},
  ];
  ...
  // --- Security Scenario Planning Methods (ATPADI) ---
  private _initAtpadiScenarios(): void {
    const worstCaseScenarios = [
      { id: "atpadi-sc-01", name: "Ransomware Attack on Critical Infrastructure", category: "Malware", impact: 9500000, probability: 0.12, recoveryDays: 21, affectedSystems: 47, dataLossGb: 320, businessImpact: "critical" },
      { id: "atpadi-sc-02", name: "Insider Data Exfiltration via Authorized Channels", category: "Insider Threat", impact: 7200000, probability: 0.18, recoveryDays: 14, affectedSystems: 12, dataLossGb: 85, businessImpact: "high" },
      { id: "atpadi-sc-03", name: "Supply Chain Compromise via Third-Party Library", category: "Supply Chain", impact: 6800000, probability: 0.09, recoveryDays: 30, affectedSystems: 63, dataLossGb: 0, businessImpact: "critical" },
      { id: "atpadi-sc-04", name: "Cloud Misconfiguration Leading to Public Exposure", category: "Cloud", impact: 4100000, probability: 0.22, recoveryDays: 7, affectedSystems: 8, dataLossGb: 450, businessImpact: "high" },
      { id: "atpadi-sc-05", name: "Zero-Day Exploit in Core Application Framework", category: "Vulnerability", impact: 8900000, probability: 0.05, recoveryDays: 45, affectedSystems: 120, dataLossGb: 0, businessImpact: "critical" },
      { id: "atpadi-sc-06", name: "Distributed Denial of Service on Customer-Facing Services", category: "Availability", impact: 3400000, probability: 0.25, recoveryDays: 3, affectedSystems: 15, dataLossGb: 0, businessImpact: "medium" },
      { id: "atpadi-sc-07", name: "Credential Stuffing and Account Takeover Wave", category: "Identity", impact: 2800000, probability: 0.30, recoveryDays: 5, affectedSystems: 6, dataLossGb: 12, businessImpact: "medium" },
      { id: "atpadi-sc-08", name: "Physical Security Breach at Data Center Facility", category: "Physical", impact: 12000000, probability: 0.02, recoveryDays: 60, affectedSystems: 200, dataLossGb: 5000, businessImpact: "critical" },
    ];
    this._atpadiScenarios = worstCaseScenarios.map(s => ({ ...s, mitigationCost: Math.round(s.impact * 0.08), drillScheduled: false, lastDrillDate: null, drillScore: null, controlGaps: Math.floor(Math.random() * 5) + 1 }));
  }

  private _calculateAtpadiScenarioRisk(): void {
    this._atpadiScenarios.forEach(s => {
  ...
  private _scheduleAtpadiDrills(): void {
  ...
  private _atpadiScenarioComparison(): Record<string, unknown>[] {
    return this._atpadiScenarios.map(s => ({
  ...
  // --- Security Control Effectiveness Analytics (ATPADI) ---
  private _initAtpadiControls(): void {
    const controls = [
      { id: "atpadi-ct-01", name: "Network Segmentation", type: "preventive", maturity: "defined", score: 82, target: 90, failures: 3, lastTest: "2026-03-15" },
      { id: "atpadi-ct-02", name: "Endpoint Detection and Response", type: "detective", maturity: "managed", score: 88, target: 92, failures: 1, lastTest: "2026-04-01" },
      { id: "atpadi-ct-03", name: "Data Loss Prevention", type: "preventive", maturity: "repeatable", score: 71, target: 85, failures: 7, lastTest: "2026-03-22" },
      { id: "atpadi-ct-04", name: "Security Information and Event Management", type: "detective", maturity: "managed", score: 85, target: 90, failures: 2, lastTest: "2026-04-05" },
      { id: "atpadi-ct-05", name: "Identity and Access Management", type: "preventive", maturity: "defined", score: 76, target: 88, failures: 5, lastTest: "2026-03-28" },
      { id: "atpadi-ct-06", name: "Vulnerability Management", type: "corrective", maturity: "managed", score: 80, target: 90, failures: 4, lastTest: "2026-04-02" },
      { id: "atpadi-ct-07", name: "Email Security Gateway", type: "preventive", maturity: "managed", score: 90, target: 95, failures: 1, lastTest: "2026-04-08" },
      { id: "atpadi-ct-08", name: "Web Application Firewall", type: "preventive", maturity: "managed", score: 87, target: 92, failures: 2, lastTest: "2026-03-30" },
      { id: "atpadi-ct-09", name: "Patch Management", type: "corrective", maturity: "defined", score: 68, target: 85, failures: 9, lastTest: "2026-03-18" },
      { id: "atpadi-ct-10", name: "Encryption at Rest", type: "preventive", maturity: "managed", score: 93, target: 95, failures: 0, lastTest: "2026-04-10" },
      { id: "atpadi-ct-11", name: "Encryption in Transit", type: "preventive", maturity: "optimized", score: 96, target: 98, failures: 0, lastTest: "2026-04-12" },
      { id: "atpadi-ct-12", name: "Privileged Access Management", type: "preventive", maturity: "defined", score: 74, target: 88, failures: 6, lastTest: "2026-03-25" },
      { id: "atpadi-ct-13", name: "Security Awareness Training", type: "preventive", maturity: "repeatable", score: 65, target: 80, failures: 11, lastTest: "2026-03-20" },
      { id: "atpadi-ct-14", name: "Incident Response Plan", type: "corrective", maturity: "managed", score: 78, target: 88, failures: 4, lastTest: "2026-04-03" },
      { id: "atpadi-ct-15", name: "Backup and Recovery", type: "corrective", maturity: "managed", score: 84, target: 92, failures: 3, lastTest: "2026-04-06" },
      { id: "atpadi-ct-16", name: "Multi-Factor Authentication", type: "preventive", maturity: "managed", score: 91, target: 95, failures: 1, lastTest: "2026-04-09" },
      { id: "atpadi-ct-17", name: "Network Traffic Analysis", type: "detective", maturity: "defined", score: 72, target: 85, failures: 5, lastTest: "2026-03-27" },
      { id: "atpadi-ct-18", name: "Cloud Security Posture Management", type: "detective", maturity: "repeatable", score: 69, target: 82, failures: 8, lastTest: "2026-03-24" },
      { id: "atpadi-ct-19", name: "Container Security Scanning", type: "preventive", maturity: "defined", score: 75, target: 85, failures: 5, lastTest: "2026-04-04" },
      { id: "atpadi-ct-20", name: "API Security Gateway", type: "preventive", maturity: "repeatable", score: 70, target: 82, failures: 7, lastTest: "2026-03-29" },
      { id: "atpadi-ct-21", name: "Threat Intelligence Platform", type: "detective", maturity: "managed", score: 83, target: 90, failures: 2, lastTest: "2026-04-07" },
      { id: "atpadi-ct-22", name: "Database Activity Monitoring", type: "detective", maturity: "defined", score: 67, target: 80, failures: 8, lastTest: "2026-03-21" },
      { id: "atpadi-ct-23", name: "Deception Technology", type: "detective", maturity: "initial", score: 55, target: 75, failures: 12, lastTest: "2026-03-16" },
      { id: "atpadi-ct-24", name: "Security Orchestration Automation", type: "corrective", maturity: "repeatable", score: 73, target: 85, failures: 6, lastTest: "2026-04-01" },
      { id: "atpadi-ct-25", name: "Third-Party Risk Assessment", type: "preventive", maturity: "defined", score: 64, target: 78, failures: 9, lastTest: "2026-03-19" },
    ];
    this._atpadiControls = controls;
  }

  private _analyzeAtpadiControlEffectiveness(): void {
    const typeBreakdown: Record<string, { total: number; avgScore: number; avgFailures: number }> = {};
    this._atpadiControls.forEach(c => {
    ...
    this._atpadiControlTypeBreakdown = typeBreakdown;
  }

  private _atpadiControlOptimization(): Record<string, unknown>[] {
    return this._atpadiControls.filter(c => c.score < c.target).map(c => ({
  ...
  // --- Security Data Pipeline Health (ATPADI) ---
  private _initAtpadiPipelines(): void {
    const pipelines = [
      { id: "atpadi-pl-01", name: "SIEM Log Ingestion", status: "healthy", healthScore: 94, latencyMs: 120, freshnessMin: 2, errorRate: 0.02, throughputMbps: 450, recordsPerSec: 12000, backPressure: 0.05, uptime: 99.97 },
      { id: "atpadi-pl-02", name: "Threat Feed Aggregation", status: "healthy", healthScore: 91, latencyMs: 300, freshnessMin: 15, errorRate: 0.05, throughputMbps: 85, recordsPerSec: 3200, backPressure: 0.08, uptime: 99.92 },
      { id: "atpadi-pl-03", name: "Vulnerability Scan Results", status: "degraded", healthScore: 72, latencyMs: 2500, freshnessMin: 60, errorRate: 0.15, throughputMbps: 25, recordsPerSec: 800, backPressure: 0.35, uptime: 98.50 },
      { id: "atpadi-pl-04", name: "Endpoint Telemetry Stream", status: "healthy", healthScore: 88, latencyMs: 450, freshnessMin: 5, errorRate: 0.08, throughputMbps: 280, recordsPerSec: 8500, backPressure: 0.12, uptime: 99.85 },
      { id: "atpadi-pl-05", name: "Cloud Audit Log Pipeline", status: "healthy", healthScore: 86, latencyMs: 600, freshnessMin: 10, errorRate: 0.06, throughputMbps: 150, recordsPerSec: 4500, backPressure: 0.10, uptime: 99.80 },
      { id: "atpadi-pl-06", name: "Identity Event Stream", status: "warning", healthScore: 78, latencyMs: 1200, freshnessMin: 20, errorRate: 0.12, throughputMbps: 45, recordsPerSec: 1500, backPressure: 0.22, uptime: 99.40 },
      { id: "atpadi-pl-07", name: "Network Flow Collection", status: "healthy", healthScore: 82, latencyMs: 800, freshnessMin: 8, errorRate: 0.09, throughputMbps: 650, recordsPerSec: 18000, backPressure: 0.15, uptime: 99.70 },
      { id: "atpadi-pl-08", name: "DLP Incident Pipeline", status: "degraded", healthScore: 68, latencyMs: 3500, freshnessMin: 45, errorRate: 0.20, throughputMbps: 18, recordsPerSec: 500, backPressure: 0.42, uptime: 97.80 },
    ];
    this._atpadiPipelines = pipelines.map(p => ({ ...p, dependencies: [], slaBreached: p.healthScore < 75, alertThreshold: p.errorRate > 0.15 }));
    this._atpadiPipelines[0].dependencies = ["atpadi-pl-04", "atpadi-pl-07"];
    this._atpadiPipelines[4].dependencies = ["atpadi-pl-01"];
    this._atpadiPipelines[5].dependencies = ["atpadi-pl-02"];
    this._atpadiPipelines[7].dependencies = ["atpadi-pl-04", "atpadi-pl-05"];
  }

  private _atpadiPipelineTrend(): Record<string, unknown>[] {
    const hours = Array.from({ length: 24 }, (_, i) => `H${String(i).padStart(2, "0")}`);
    return this._atpadiPipelines.map(p => ({
  ...
  // --- Security Stakeholder Report Generator (ATPADI) ---
  private _initAtpadiReportTemplates(): void {
    this._atpadiReportTemplates = [
      { id: "atpadi-rp-01", name: "Board Security Report", audience: "board", frequency: "quarterly", sections: 6, autoGenerate: true, lastGenerated: "2026-03-31", pages: 12 },
      { id: "atpadi-rp-02", name: "Executive Risk Summary", audience: "executive", frequency: "monthly", sections: 8, autoGenerate: true, lastGenerated: "2026-04-01", pages: 8 },
      { id: "atpadi-rp-03", name: "Technical Security Review", audience: "technical", frequency: "weekly", sections: 12, autoGenerate: true, lastGenerated: "2026-04-21", pages: 25 },
      { id: "atpadi-rp-04", name: "Audit Compliance Report", audience: "audit", frequency: "quarterly", sections: 10, autoGenerate: false, lastGenerated: "2026-03-15", pages: 35 },
      { id: "atpadi-rp-05", name: "Regulatory Filing Package", audience: "regulatory", frequency: "annual", sections: 15, autoGenerate: false, lastGenerated: "2025-12-31", pages: 48 },
    ];
  }

  private _generateAtpadiExecSummary(): string {
    const totalRisk = this._atpadiScenarios.reduce((sum: number, s: any) => sum + s.riskScore, 0);
    const avgControl = Math.round(this._atpadiControls.reduce((sum: number, c: any) => sum + c.score, 0) / this._atpadiControls.length);
    const degradedPipes = this._atpadiPipelines.filter((p: any) => p.status !== "healthy").length;
    return `Overall risk exposure: ${(totalRisk / 1000000).toFixed(1)}M. Average control effectiveness: {avgControl}%. {degradedPipes} pipeline(s) need attention. {this._atpadiScenarios.filter((s: any) => s.mitigationPriority === "immediate").length} scenarios require immediate action.`;
  }

  // --- Security Technology Radar (ATPADI) ---
  private _initAtpadiTechRadar(): void {
    this._atpadiTechRadar = [
      { id: "atpadi-tr-01", name: "AI-Powered Threat Detection", ring: "adopt", category: "Detection", maturity: 4, trend: "up", investment: "high", roi: 3.2, vendorCount: 8 },
      { id: "atpadi-tr-02", name: "Zero Trust Architecture", ring: "adopt", category: "Architecture", maturity: 4, trend: "up", investment: "high", roi: 2.8, vendorCount: 12 },
      { id: "atpadi-tr-03", name: "SASE/SSE Platform", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.4, vendorCount: 10 },
      { id: "atpadi-tr-04", name: "CNAPP Cloud Security", ring: "trial", category: "Cloud", maturity: 3, trend: "up", investment: "medium", roi: 2.1, vendorCount: 7 },
      { id: "atpadi-tr-05", name: "Extended Detection and Response", ring: "adopt", category: "Detection", maturity: 4, trend: "stable", investment: "high", roi: 2.9, vendorCount: 9 },
      { id: "atpadi-tr-06", name: "Quantum-Safe Cryptography", ring: "assess", category: "Crypto", maturity: 2, trend: "up", investment: "low", roi: 0.8, vendorCount: 4 },
      { id: "atpadi-tr-07", name: "Security Service Edge", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.0, vendorCount: 6 },
      { id: "atpadi-tr-08", name: "SOAR 2.0 Autonomous SOC", ring: "assess", category: "Operations", maturity: 2, trend: "up", investment: "low", roi: 1.2, vendorCount: 5 },
      { id: "atpadi-tr-09", name: "Software Supply Chain Security", ring: "trial", category: "DevSecOps", maturity: 3, trend: "up", investment: "medium", roi: 2.3, vendorCount: 8 },
      { id: "atpadi-tr-10", name: "Identity Threat Detection", ring: "adopt", category: "Identity", maturity: 3, trend: "up", investment: "high", roi: 2.7, vendorCount: 7 },
      { id: "atpadi-tr-11", name: "Data Security Posture Management", ring: "trial", category: "Data", maturity: 3, trend: "up", investment: "medium", roi: 2.2, vendorCount: 9 },
      { id: "atpadi-tr-12", name: "Decentralized Identity Standards", ring: "hold", category: "Identity", maturity: 1, trend: "stable", investment: "low", roi: 0.5, vendorCount: 3 },
    ];
  }

  private _atpadiRadarSummary(): Record<string, unknown> {
    const rings: Record<string, number> = { adopt: 0, trial: 0, assess: 0, hold: 0 };
    this._atpadiTechRadar.forEach(t => { if (rings[t.ring as keyof typeof rings] !== undefined) rings[t.ring as keyof typeof rings]++; });
    return { adopt: rings.adopt, trial: rings.trial, assess: rings.assess, hold: rings.hold, total: this._atpadiTechRadar.length, avgMaturity: 2.8, topInvestment: this._atpadiTechRadar.filter(t => t.investment === "high").map(t => t.name) };
  }

  // --- ATPADI Security Compliance Mapping Matrix ---
  private _initAtpadiComplianceMatrix(): void {
    ...
    this._atpadiComplianceMatrix = frameworks.map(fw => ({
  ...
  private _atpadiComplianceTrend(): Record<string, unknown>[] {
    const quarters = ["Q1-2025", "Q2-2025", "Q3-2025", "Q4-2025", "Q1-2026", "Q2-2026"];
    return this._atpadiComplianceMatrix.map(fw => ({
  ...
  // --- ATPADI Threat Intelligence Correlation Engine ---
  private _initAtpadiThreatIntel(): void {
    ...
    this._atpadiThreatIntel = threatActors.map((actor, i) => ({
  ...
  private _atpadiThreatCorrelation(): Record<string, unknown>[] {
    return this._atpadiThreatIntel.filter(t => t.riskScore > 70).map(t => ({
  ...
  // --- ATPADI Security Metrics Deep Dive ---
  private _initAtpadiMetricsDeep(): void {
    ...
    this._atpadiMetricsDeep = metricCategories.map(cat => ({
  ...
  private _atpadiMetricsHeatmap(): number[][] {
    return this._atpadiMetricsDeep.map(cat =>
      cat.metrics.map(m => Math.round((m.value / m.target) * 100))
    );
  }

  // --- ATPADI Incident Cost Modeling ---
  private _initAtpadiCostModel(): void {
    const incidentTypes = ["Data Breach", "Ransomware", "DDoS", "Insider Threat", "Phishing", "Supply Chain", "Cloud Misconfig", "Zero-Day"];
    this._atpadiCostModel = incidentTypes.map(inc => ({
    ...
    this._atpadiCostModel.forEach(m => { m.totalAnnualExposure = m.avgCost * m.frequency; });
  }

  private _atpadiCostProjection(): Record<string, unknown>[] {
    return this._atpadiCostModel.map(m => ({
  ...
  // --- ATPADI Security Architecture Pattern Library ---
  private _initAtpadiArchPatterns(): void {
    ...
    this._atpadiArchPatterns = patterns.map((p, i) => ({
  ...
  private _atpadiArchPatternAnalysis(): Record<string, unknown> {
    const implemented = this._atpadiArchPatterns.filter(p => p.implementation > 75).length;
    const avgCoverage = Math.round(this._atpadiArchPatterns.reduce((s: number, p: any) => s + p.coverage, 0) / this._atpadiArchPatterns.length);
    return {
      totalPatterns: this._atpadiArchPatterns.length, fullyImplemented: implemented, avgCoverage,
      avgRiskReduction: Math.round(this._atpadiArchPatterns.reduce((s: number, p: any) => s + p.riskReduction, 0) / this._atpadiArchPatterns.length),
      gaps: this._atpadiArchPatterns.filter(p => p.implementation < 60).map(p => p.pattern),
      recommendations: this._atpadiArchPatterns.filter(p => p.implementation < 70).map(p => ({ pattern: p.pattern, current: p.implementation, target: 85, effort: p.costComplexity }))
    };
  }
  ...
  private _33cRiskAssessmentWorkflow = [
  ...
  private _33cTreatmentOptions = [
    {id: '33c-T-001', riskId: 'RSK-042', risk: 'Ransomware Attack on Critical Infrastructure', before: 20, after: 6, strategy: 'Mitigate', controls: ['Next-gen EDR with behavioral detection and automated response', 'Zero-trust network micro-segmentation isolating critical assets', 'Immutable offline backup rotation with quarterly recovery testing', 'Incident response retainer with 1-hour response SLA'], cost: '$450K/yr', roi: '78% reduction vs $2.8M potential loss', timeline: 'Q3 2026', status: 'approved' as const},
  ...
  private _33cTalentPositions = [
    ...
    {id: '33c-H-002', title: 'Cloud Security Engineer', dept: 'Cloud Security', level: 'Mid-Senior', loc: 'SF/Remote', salary: '$155K-$180K', posted: '2026-03-20', applicants: 45, pipeline: 12, interviews: 6, offers: 2, urgency: 'high' as const, manager: 'Mike Johnson', skills: ['AWS/Azure/GCP', 'Terraform', 'Kubernetes', 'IAM', 'CSPM'], nice: ['CCSP', 'CKS', 'Serverless']},
    {id: '33c-H-003', title: 'SOC Analyst L2', dept: 'Security Operations', level: 'Mid', loc: 'Austin TX', salary: '$105K-$130K', posted: '2026-04-01', applicants: 67, pipeline: 18, interviews: 8, offers: 0, urgency: 'medium' as const, manager: 'Lisa Park', skills: ['SIEM', 'Incident triage', 'Malware analysis', 'TCP/IP'], nice: ['GCIA', 'SOAR', 'Kill chain']},
    {id: '33c-H-004', title: 'Staff Security Architect', dept: 'Architecture', level: 'Staff', loc: 'Remote US', salary: '$200K-$240K', posted: '2026-02-28', applicants: 12, pipeline: 3, interviews: 2, offers: 0, urgency: 'critical' as const, manager: 'David Lee', skills: ['Enterprise arch', 'Zero trust', 'Risk frameworks', 'Board comms'], nice: ['CISSP+SABSA', 'Team building', 'Speaker']},
    {id: '33c-H-005', title: 'GRC Analyst', dept: 'GRC', level: 'Mid', loc: 'NY/Remote', salary: '$110K-$135K', posted: '2026-04-05', applicants: 34, pipeline: 9, interviews: 3, offers: 1, urgency: 'medium' as const, manager: 'Rachel Green', skills: ['ISO 27001', 'SOC 2', 'Risk assessment', 'Audit'], nice: ['CRISC', 'CISA', 'GDPR']},
    {id: '33c-H-006', title: 'Threat Intel Analyst', dept: 'Threat Intel', level: 'Mid-Senior', loc: 'Remote US', salary: '$140K-$170K', posted: '2026-03-25', applicants: 19, pipeline: 5, interviews: 2, offers: 0, urgency: 'low' as const, manager: 'James Wilson', skills: ['MITRE ATT&CK', 'OSINT', 'Threat modeling', 'Reverse eng'], nice: ['CTIA', 'Nation-state', 'Dark web']},
    {id: '33c-H-007', title: 'AppSec Engineer', dept: 'AppSec', level: 'Senior', loc: 'Seattle/Remote', salary: '$160K-$190K', posted: '2026-04-10', applicants: 28, pipeline: 7, interviews: 3, offers: 0, urgency: 'medium' as const, manager: 'Emily Zhang', skills: ['SAST/DAST/SCA', 'Secure SDLC', 'Code review', 'CI/CD'], nice: ['CSSLP', 'Mobile security']},
    {id: '33c-H-008', title: 'IAM Engineer', dept: 'IAM', level: 'Mid', loc: 'Remote US', salary: '$135K-$160K', posted: '2026-04-08', applicants: 31, pipeline: 10, interviews: 5, offers: 1, urgency: 'medium' as const, manager: 'Chris Martinez', skills: ['Okta/Azure AD', 'SAML/OIDC', 'PAM', 'RBAC'], nice: ['CISSP', 'Zero trust IAM']}
  ];

  private _33cInterviewScorecard = [
  ...
  private _33cOnboardingSteps = [
    {id: '33c-OB-01', item: 'Provision security-hardened laptop, monitors, peripherals, and configured workstation image with all required security tools', cat: 'IT Setup', day: 'Day 1', owner: 'IT Operations'},
    {id: '33c-OB-02', item: 'Create AD account, email, Slack, VPN, SSO enrollment, and all required application access accounts', cat: 'Access', day: 'Day 1', owner: 'IAM Team'},
    {id: '33c-OB-03', item: 'Grant security tooling access: SIEM console, EDR dashboard, vuln scanner, pentest platforms, SOAR', cat: 'Tools', day: 'Day 1-2', owner: 'SOC Manager'},
    {id: '33c-OB-04', item: 'Complete MFA enrollment, security awareness onboarding course, and acceptable use policy acknowledgment', cat: 'Security', day: 'Day 1', owner: 'Awareness Team'},
    {id: '33c-OB-05', item: 'Sign NDA, employment agreement, IP assignment, and code of conduct documents with HR', cat: 'HR', day: 'Day 1', owner: 'Human Resources'},
    {id: '33c-OB-06', item: 'Introduction meetings with direct team members and key cross-functional stakeholders', cat: 'Integration', day: 'Week 1', owner: 'Hiring Manager'},
    {id: '33c-OB-07', item: 'Security architecture overview, tooling walkthrough, and operational runbook review sessions', cat: 'Training', day: 'Week 1-2', owner: 'Security Architect'},
    {id: '33c-OB-08', item: 'Shadow experienced team member on active incident response and threat hunting investigations', cat: 'Mentoring', day: 'Week 2-3', owner: 'SOC Lead'},
    {id: '33c-OB-09', item: 'Set up individual certification study plan aligned with role requirements and career goals', cat: 'Development', day: 'Week 2', owner: 'Hiring Manager'},
    {id: '33c-OB-10', item: 'Establish 30-60-90 day goals, define success metrics, and schedule first performance checkpoint', cat: 'Goals', day: 'Week 1', owner: 'Hiring Manager'}
  ];

  private _33cMetricsDashboard = [
    {id: '33c-MTD-001', name: 'Mean Time to Detect (MTTD)', value: '4.2 hours', trend: 'improving' as const, target: '< 4 hours', source: 'SIEM - Splunk', method: 'Median time from first alert to analyst acknowledgment', owner: 'SOC Manager', sla: '4 hours', prev: '5.1 hours', spark: [5.8, 5.4, 5.1, 4.9, 4.7, 4.5, 4.3, 4.2], breakdown: [{src: 'Network IDS', pct: '35%', avg: '3.8h'}, {src: 'Endpoint EDR', pct: '28%', avg: '4.1h'}, {src: 'Email Gateway', pct: '22%', avg: '3.5h'}, {src: 'Cloud Security', pct: '10%', avg: '5.2h'}, {src: 'User Reports', pct: '5%', avg: '6.8h'}]},
    {id: '33c-MTD-002', name: 'Mean Time to Respond (MTTR)', value: '2.8 hours', trend: 'stable' as const, target: '< 3 hours', source: 'SOAR - XSOAR', method: 'Median time from acknowledgment to first containment action', owner: 'IR Lead', sla: '3 hours', prev: '2.7 hours', spark: [3.2, 3.0, 2.9, 2.8, 2.8, 2.9, 2.8, 2.8], breakdown: [{src: 'Auto Playbooks', pct: '42%', avg: '1.2h'}, {src: 'Semi-Auto', pct: '33%', avg: '2.5h'}, {src: 'Manual', pct: '20%', avg: '5.1h'}, {src: 'L3 Escalation', pct: '5%', avg: '8.3h'}]},
    {id: '33c-MTD-003', name: 'Patch Compliance Rate', value: '97.6%', trend: 'improving' as const, target: '> 95%', source: 'Tenable.io', method: 'Pct of critical/high vulns patched within SLA windows', owner: 'Vuln Manager', sla: '95%', prev: '96.1%', spark: [93.2, 94.1, 94.8, 95.3, 95.9, 96.4, 97.0, 97.6], breakdown: [{src: 'Critical (7d)', pct: '40%', rate: '99.2%'}, {src: 'High (30d)', pct: '35%', rate: '97.8%'}, {src: 'Medium (90d)', pct: '20%', rate: '95.1%'}, {src: 'Low (180d)', pct: '5%', rate: '89.4%'}]},
    {id: '33c-MTD-004', name: 'Phishing Click Rate', value: '3.2%', trend: 'improving' as const, target: '< 5%', source: 'KnowBe4 + Proofpoint', method: 'Pct of users clicking simulated phishing links monthly', owner: 'Awareness Lead', sla: '5%', prev: '4.1%', spark: [6.8, 6.2, 5.5, 5.1, 4.6, 4.1, 3.6, 3.2], breakdown: [{src: 'Spear Phishing', pct: '35%', rate: '5.1%'}, {src: 'Generic Phishing', pct: '30%', rate: '2.8%'}, {src: 'BEC Sim', pct: '20%', rate: '3.5%'}, {src: 'SMS Phishing', pct: '15%', rate: '1.9%'}]},
    {id: '33c-MTD-005', name: 'Security Posture Score', value: '784', trend: 'improving' as const, target: '> 750', source: 'SecurityScorecard', method: 'Composite: external 30%, compliance 25%, vuln 25%, config 20%', owner: 'CISO', sla: '> 750', prev: '771', spark: [748, 752, 758, 762, 768, 773, 778, 784], breakdown: [{src: 'External Rating', pct: '30%', score: '791'}, {src: 'Compliance', pct: '25%', score: '812'}, {src: 'Vuln Mgmt', pct: '25%', score: '745'}, {src: 'Config', pct: '20%', score: '769'}]},
    {id: '33c-MTD-006', name: 'Vulnerability Backlog', value: '142', trend: 'improving' as const, target: '< 150', source: 'Tenable + Qualys', method: 'Total open vulns across all scanned assets tracked weekly', owner: 'Vuln Manager', sla: '< 150', prev: '168', spark: [210, 198, 185, 175, 165, 158, 150, 142], breakdown: [{src: 'Critical', pct: '8%', count: '11'}, {src: 'High', pct: '22%', count: '31'}, {src: 'Medium', pct: '45%', count: '64'}, {src: 'Low', pct: '25%', count: '36'}]}
  ];

  private _33cArchReview = [
    {id: '33c-AR-001', cat: 'Network', item: 'Network segmentation between trust zones with dedicated firewalls and granular ACLs', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Micro-segmentation via NSX across all production zones'},
    {id: '33c-AR-002', cat: 'Network', item: 'DMZ architecture isolating public services from internal network', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Dual-firewall DMZ with WAF and DDoS mitigation'},
    {id: '33c-AR-003', cat: 'Network', item: 'Least-privilege traffic flows with deny-by-default and quarterly review', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Explicit allow rules with automated drift detection'},
    {id: '33c-AR-004', cat: 'Network', item: 'Encrypted external communications with IPSec/TLS 1.3 and PFS', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'IKEv2 with PFS, TLS 1.3 for all external APIs'},
    {id: '33c-AR-005', cat: 'Identity', item: 'Centralized IdP with SSO and automated SCIM provisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Okta SSO SAML 2.0 for 247 apps with SCIM'},
    {id: '33c-AR-006', cat: 'Identity', item: 'RBAC with quarterly reviews and automated deprovisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Quarterly recertification with JIT admin access'},
    {id: '33c-AR-007', cat: 'Identity', item: 'PAM controlling all admin and service accounts with session recording', sev: 'high' as const, status: 'fail' as const, reviewer: 'IAM Lead', notes: '3 legacy accounts pending CyberArk enrollment'},
    {id: '33c-AR-008', cat: 'Identity', item: 'MFA enforced for all users, hardware tokens for privileged access', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'FIDO2 for admins, push MFA for users, 99.8% enrollment'},
    {id: '33c-AR-009', cat: 'Data', item: 'Data classification enforced with automated labeling on all repos', sev: 'high' as const, status: 'partial' as const, reviewer: 'Data Protection Lead', notes: 'Framework deployed, gaps on 4 legacy file shares'},
    {id: '33c-AR-010', cat: 'Data', item: 'AES-256 encryption at rest for all sensitive data stores', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'Full-disk, TDE on DBs, SSE-KMS on cloud storage'},
    {id: '33c-AR-011', cat: 'Data', item: 'DLP monitoring and preventing unauthorized data exfiltration', sev: 'high' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'DLP active on email, endpoint, cloud, and network'},
    {id: '33c-AR-012', cat: 'Data', item: 'Encrypted off-site backups with quarterly restore testing', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Infrastructure Lead', notes: '3-2-1 strategy with immutable backups'},
    {id: '33c-AR-013', cat: 'AppSec', item: 'Secure SDLC with SAST, DAST, SCA in all CI/CD pipelines', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: '34 pipelines with security gates and PR blocking'},
    {id: '33c-AR-014', cat: 'AppSec', item: 'API gateway with auth, rate limiting, schema validation', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: 'Kong with OAuth2, rate limits, OpenAPI validation'},
    {id: '33c-AR-015', cat: 'AppSec', item: 'Container security with image scanning and runtime protection', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Trivy scanning, Falco runtime in 80% of clusters'},
    {id: '33c-AR-016', cat: 'Monitoring', item: 'SIEM collecting from all critical systems with correlation rules', sev: 'critical' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'Splunk 98% coverage with 1200+ correlation rules'},
    {id: '33c-AR-017', cat: 'Monitoring', item: 'EDR with behavioral detection on all managed endpoints', sev: 'high' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'CrowdStrike on 99.2% endpoints with 24/7 MDR'},
    {id: '33c-AR-018', cat: 'Monitoring', item: 'Executive dashboards with real-time posture visibility', sev: 'medium' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'PowerBI dashboards with weekly exec summaries'},
    {id: '33c-AR-019', cat: 'Cloud', item: 'CSPM monitoring and remediating cloud misconfigurations', sev: 'high' as const, status: 'pass' as const, reviewer: 'Cloud Security Lead', notes: 'Prisma Cloud 450+ checks with auto-remediation'},
    {id: '33c-AR-020', cat: 'Cloud', item: 'IaC templates scanned before deployment to production', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Checkov for Terraform, cfn-nag for CloudFormation pending'}
  ];

  private _get33cRiskProgress(): number {
    return Math.round(this._33cRiskAssessmentWorkflow.filter(s => s.status === 'completed').length / this._33cRiskAssessmentWorkflow.length * 100);
  }

  private _get33cTreatmentSummary(): {total: number; approved: number; avgReduction: number; investment: string} {
    const a = this._33cTreatmentOptions.filter(r => r.status === 'approved').length;
    const avg = this._33cTreatmentOptions.reduce((s, r) => s + ((r.before - r.after) / r.before * 100), 0) / this._33cTreatmentOptions.length;
    return {total: this._33cTreatmentOptions.length, approved: a, avgReduction: Math.round(avg), investment: '$1.125M annually'};
  }

  private _get33cRuleSummary(): {total: number; passing: number; failing: number; avgRate: number; violations: number} {
    const p = this._33cComplianceRules.filter(r => r.result === 'pass').length;
    const f = this._33cComplianceRules.filter(r => r.result === 'fail').length;
    const avg = this._33cComplianceRules.reduce((s, r) => s + r.passRate, 0) / this._33cComplianceRules.length;
    const v = this._33cComplianceRules.reduce((s, r) => s + r.violations, 0);
    return {total: this._33cComplianceRules.length, passing: p, failing: f, avgRate: Math.round(avg * 10) / 10, violations: v};
  }

  private _get33cPipelineStats(): {positions: number; applicants: number; interviewing: number; offers: number; critical: number} {
    const t = this._33cTalentPositions.length;
    const a = this._33cTalentPositions.reduce((s, p) => s + p.applicants, 0);
    const i = this._33cTalentPositions.reduce((s, p) => s + p.interviews, 0);
    const o = this._33cTalentPositions.reduce((s, p) => s + p.offers, 0);
    const c = this._33cTalentPositions.filter(x => x.urgency === 'critical').length;
    return {positions: t, applicants: a, interviewing: i, offers: o, critical: c};
  }

  private _get33cMetricTrends(): {improving: number; stable: number; total: number} {
    const imp = this._33cMetricsDashboard.filter(m => m.trend === 'improving').length;
    const stb = this._33cMetricsDashboard.filter(m => m.trend === 'stable').length;
    return {improving: imp, stable: stb, total: this._33cMetricsDashboard.length};
  }

  private _get33cArchScore(): {total: number; passed: number; failed: number; partial: number; score: number} {
    const p = this._33cArchReview.filter(c => c.status === 'pass').length;
    const f = this._33cArchReview.filter(c => c.status === 'fail').length;
    const pa = this._33cArchReview.filter(c => c.status === 'partial').length;
    return {total: this._33cArchReview.length, passed: p, failed: f, partial: pa, score: Math.round((p / this._33cArchReview.length) * 100)};
  }


  private _z33cIncidentScenarios = [
    {id: 'z33c-IS-001', name: 'Ransomware Outbreak in Production Environment', category: 'Malware', likelihood: 'Medium', impact: 'Critical', riskScore: 15, playbooks: ['IR-Ransomware-Isolate', 'IR-Ransomware-Eradicate', 'IR-Ransomway-Recover'], estimatedMTTD: '15min', estimatedMTTR: '4h', involvedTeams: ['SOC', 'IR', 'Infrastructure', 'Legal', 'Comms'], keyIndicators: ['Mass file encryption events', 'Ransom note deployment', 'Lateral movement via SMB/RDP', 'Backup deletion attempts'], mitigationControls: ['EDR behavioral detection', 'Network segmentation', 'Immutable backups', 'Email filtering'], lessonsLearned: 'Ensure offline backups are tested quarterly and Ransomware-specific playbooks are updated with latest threat intelligence', lastDrillDate: '2026-03-15', nextDrillDate: '2026-06-15'},
    {id: 'z33c-IS-002', name: 'Cloud Misconfiguration Leading to Data Exposure', category: 'Cloud', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Cloud-Misconfig-Assess', 'IR-Cloud-Misconfig-Remediate', 'IR-Cloud-Misconfig-Notify'], estimatedMTTD: '2h', estimatedMTTR: '6h', involvedTeams: ['Cloud Security', 'SOC', 'Data Protection', 'Legal'], keyIndicators: ['Public S3 bucket exposure', 'Overly permissive IAM policies', 'Unencrypted database snapshots', 'Public API endpoint discovery'], mitigationControls: ['CSPM monitoring', 'IAM policy analyzer', 'Automated remediation', 'Cloud audit logging'], lessonsLearned: 'Implement CSPM with auto-remediation for common misconfigurations and conduct monthly cloud configuration reviews', lastDrillDate: '2026-02-20', nextDrillDate: '2026-05-20'},
    {id: 'z33c-IS-003', name: 'Insider Threat Data Exfiltration', category: 'Insider', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-Insider-Investigate', 'IR-Insider-Contain', 'IR-Insider-Escalate'], estimatedMTTD: '8h', estimatedMTTR: '24h', involvedTeams: ['SOC', 'IR', 'HR', 'Legal', 'IAM'], keyIndicators: ['Anomalous data access patterns', 'Large file downloads to USB or cloud', 'Off-hours access to sensitive systems', 'Accessing data outside normal job scope'], mitigationControls: ['UEBA analytics', 'DLP policies', 'PAM controls', 'Access reviews'], lessonsLearned: 'Strengthen UEBA baselines and implement automated alerts for data access pattern anomalies', lastDrillDate: '2026-01-10', nextDrillDate: '2026-04-10'},
    {id: 'z33c-IS-004', name: 'Supply Chain Attack via Compromised Vendor Software', category: 'Supply Chain', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-SupplyChain-Detect', 'IR-SupplyChain-Isolate', 'IR-SupplyChain-Remediate'], estimatedMTTD: '24h', estimatedMTTR: '72h', involvedTeams: ['SOC', 'IR', 'Procurement', 'Vendor Mgmt', 'Legal'], keyIndicators: ['Unexpected network connections from vendor software', 'Modified binaries or dependencies', 'Anomalous behavior from trusted applications', 'SBOM discrepancy alerts'], mitigationControls: ['SCA scanning', 'SBOM monitoring', 'Vendor security assessments', 'Network segmentation'], lessonsLearned: 'Require SBOM from all vendors and implement continuous monitoring of third-party component behavior', lastDrillDate: '2026-03-01', nextDrillDate: '2026-06-01'},
    {id: 'z33c-IS-005', name: 'Credential Stuffing Attack on Customer Portal', category: 'External Attack', likelihood: 'High', impact: 'Medium', riskScore: 12, playbooks: ['IR-CredentialStuffing-Detect', 'IR-CredentialStuffing-Block', 'IR-CredentialStuffing-Notify'], estimatedMTTD: '30min', estimatedMTTR: '2h', involvedTeams: ['SOC', 'AppSec', 'Customer Support', 'Comms'], keyIndicators: ['High volume of failed login attempts', 'Login attempts from known compromised credential lists', 'Unusual geographic distribution of login attempts', 'Account takeover indicators'], mitigationControls: ['WAF rate limiting', 'CAPTCHA on login', 'MFA enforcement', 'Credential monitoring service'], lessonsLearned: 'Deploy advanced bot detection on login endpoints and implement proactive credential monitoring for customer accounts', lastDrillDate: '2026-04-01', nextDrillDate: '2026-07-01'},
    {id: 'z33c-IS-006', name: 'Phishing Campaign Targeting Executive Team', category: 'Social Engineering', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Phishing-Contain', 'IR-Phishing-Investigate', 'IR-Phishing-Notify'], estimatedMTTD: '1h', estimatedMTTR: '4h', involvedTeams: ['SOC', 'Awareness', 'Executive Office', 'IT'], keyIndicators: ['Sophisticated spear-phishing emails to C-suite', 'Business email compromise indicators', 'Redirected MFA approval requests', 'Unauthorized wire transfer attempts'], mitigationControls: ['AI email filtering', 'Executive awareness training', 'Out-of-band verification policy', 'DMARC/DKIM/SPF'], lessonsLearned: 'Implement additional email security controls for executive mailboxes and conduct monthly spear-phishing simulations', lastDrillDate: '2026-02-15', nextDrillDate: '2026-05-15'}
  ];

  private _z33cSecurityFrameworks = [
  ...
  private _getz33cScenarioSummary(): {total: number; critical: number; high: number; drilled: number; avgRiskScore: number} {
    const crit = this._z33cIncidentScenarios.filter(s => s.impact === 'Critical').length;
    const high = this._z33cIncidentScenarios.filter(s => s.impact === 'High').length;
    const avg = this._z33cIncidentScenarios.reduce((s, x) => s + x.riskScore, 0) / this._z33cIncidentScenarios.length;
    return {total: this._z33cIncidentScenarios.length, critical: crit, high: high, drilled: this._z33cIncidentScenarios.length, avgRiskScore: Math.round(avg)};
  }

  private _getz33cVendorRiskSummary(): {total: number; approved: number; conditional: number; remediation: number; avgScore: number} {
    const approved = this._z33cThirdPartyRiskData.filter(v => v.status === 'approved').length;
    const cond = this._z33cThirdPartyRiskData.filter(v => v.status === 'conditional').length;
    const rem = this._z33cThirdPartyRiskData.filter(v => v.status === 'remediation_required').length;
    const avg = this._z33cThirdPartyRiskData.reduce((s, v) => s + v.riskScore, 0) / this._z33cThirdPartyRiskData.length;
    return {total: this._z33cThirdPartyRiskData.length, approved, conditional: cond, remediation: rem, avgScore: Math.round(avg)};
  }

  private _getz33cTrainingSummary(): {total: number; avgCompletion: number; avgScore: number; overdueUpdates: number} {
    const avgComp = this._z33cSecurityTrainingCatalog.reduce((s, t) => s + t.completionRate, 0) / this._z33cSecurityTrainingCatalog.length;
    const avgScore = this._z33cSecurityTrainingCatalog.filter(t => t.avgScore > 0).reduce((s, t) => s + t.avgScore, 0) / this._z33cSecurityTrainingCatalog.filter(t => t.avgScore > 0).length;
    return {total: this._z33cSecurityTrainingCatalog.length, avgCompletion: Math.round(avgComp * 10) / 10, avgScore: Math.round(avgScore), overdueUpdates: 0};
  }

  private _getz33cToolHealthSummary(): {total: number; healthy: number; degraded: number; annualCost: string} {
    const healthy = this._z33cSecurityToolInventory.filter(t => t.health === 'healthy').length;
    const degraded = this._z33cSecurityToolInventory.filter(t => t.health === 'degraded').length;
    const cost = this._z33cSecurityToolInventory.reduce((s, t) => s + parseInt(t.annualCost.replace(/[^0-9]/g, '')), 0);
    return {total: this._z33cSecurityToolInventory.length, healthy, degraded, annualCost: '$' + (cost / 1000).toFixed(0) + 'K'};
  }


  private _x33cSecurityZones = [
    {zoneId: 'x33c-Z-001', name: 'Internet DMZ', trustLevel: 'Untrusted', subnet: '10.0.0.0/24', segmentation: 'Dual-firewall', firewallRules: 45, monitoring: 'Full packet capture', assets: ['Web servers', 'Load balancers', 'WAF appliances', 'Reverse proxy'], riskLevel: 'high' as const, lastAssessment: '2026-04-15', nextAssessment: '2026-07-15'},
    {zoneId: 'x33c-Z-002', name: 'Corporate Network', trustLevel: 'Trusted', subnet: '10.1.0.0/16', segmentation: 'VLAN + micro-seg', firewallRules: 128, monitoring: 'NetFlow + IDS', assets: ['Workstations', 'Printers', 'Meeting room systems', 'VoIP phones'], riskLevel: 'medium' as const, lastAssessment: '2026-04-10', nextAssessment: '2026-07-10'},
    {zoneId: 'x33c-Z-003', name: 'Production Data Center', trustLevel: 'Restricted', subnet: '10.2.0.0/16', segmentation: 'Full micro-segmentation', firewallRules: 256, monitoring: 'Full packet capture + EDR', assets: ['Application servers', 'Database clusters', 'Message queues', 'API gateways'], riskLevel: 'low' as const, lastAssessment: '2026-04-12', nextAssessment: '2026-07-12'},
    {zoneId: 'x33c-Z-004', name: 'Management Network', trustLevel: 'Restricted', subnet: '10.3.0.0/24', segmentation: 'Isolated VLAN', firewallRules: 32, monitoring: 'Syslog + SNMP', assets: ['Hypervisors', 'Switch management', 'Storage controllers', 'Backup appliances'], riskLevel: 'low' as const, lastAssessment: '2026-04-08', nextAssessment: '2026-07-08'},
    {zoneId: 'x33c-Z-005', name: 'Cloud Production', trustLevel: 'Restricted', subnet: 'VPC: 172.16.0.0/12', segmentation: 'Security groups + NACLs', firewallRules: 189, monitoring: 'VPC Flow Logs + GuardDuty', assets: ['Kubernetes clusters', 'Managed databases', 'Object storage', 'Serverless functions'], riskLevel: 'medium' as const, lastAssessment: '2026-04-18', nextAssessment: '2026-07-18'}
  ];

  private _x33cSecurityAlerts = [
    {id: 'x33c-AL-001', severity: 'critical', title: 'Active ransomware encryption detected on PROD-DB-01', source: 'CrowdStrike', timestamp: '2026-04-22T13:45:00Z', status: 'investigating', assignee: 'SOC Analyst J. Smith', asset: 'PROD-DB-01 (10.2.1.15)', mitreTechnique: 'T1486 - Data Encrypted for Impact', playbook: 'IR-Ransomware-Isolate', sla: '15 minutes', elapsed: '12 minutes', notes: 'EDR detected mass file encryption event pattern. Automated isolation triggered. IR team paged.'},
    {id: 'x33c-AL-002', severity: 'high', title: 'Impossible travel detected for user admin@company.com', source: 'Okta', timestamp: '2026-04-22T12:30:00Z', status: 'triaged', assignee: 'SOC Analyst M. Johnson', asset: 'Okta - admin@company.com', mitreTechnique: 'T1078 - Valid Accounts', playbook: 'IR-ImpossibleTravel', sla: '30 minutes', elapsed: '87 minutes', notes: 'Login from Tokyo followed by login from New York within 30 minutes. Account temporarily locked. User confirmed via out-of-band channel they are in New York.'},
    {id: 'x33c-AL-003', severity: 'high', title: 'Public S3 bucket discovered with customer PII exposure', source: 'Prisma Cloud', timestamp: '2026-04-22T11:15:00Z', status: 'remediating', assignee: 'Cloud Security Lead A. Chen', asset: 'AWS S3: customer-data-archive-2024', mitreTechnique: 'N/A - Misconfiguration', playbook: 'IR-Cloud-Misconfig-Remediate', sla: '1 hour', elapsed: '2.5 hours', notes: 'Bucket containing archived customer records was publicly accessible. Bucket blocked and encrypted. Data exposure assessment in progress.'},
    {id: 'x33c-AL-004', severity: 'medium', title: 'Anomalous data download by user jane.developer@company.com', source: 'Microsoft DLP', timestamp: '2026-04-22T10:00:00Z', status: 'investigating', assignee: 'SOC Analyst R. Davis', asset: 'Endpoint: WS-DEV-042', mitreTechnique: 'T1567 - Exfil Over Web Service', playbook: 'IR-DLP-Investigation', sla: '4 hours', elapsed: '3.5 hours', notes: 'User downloaded 4.2GB of source code files to personal Google Drive. Interview scheduled with user and manager.'},
    {id: 'x33c-AL-005', severity: 'medium', title: 'Credential stuffing attack detected on customer portal', source: 'Cloudflare WAF', timestamp: '2026-04-22T09:30:00Z', status: 'contained', assignee: 'SOC Analyst K. Wilson', asset: 'Customer Portal (portal.company.com)', mitreTechnique: 'T1110 - Brute Force', playbook: 'IR-CredentialStuffing-Block', sla: '2 hours', elapsed: '4 hours', notes: 'Rate limiting and CAPTCHA automatically deployed. 12,000 failed login attempts from 800 unique IPs blocked. No successful account takeovers confirmed.'},
    {id: 'x33c-AL-006', severity: 'low', title: 'SSL certificate expiring in 14 days for api.company.com', source: 'Certificate monitoring', timestamp: '2026-04-22T08:00:00Z', status: 'remediated', assignee: 'Infrastructure Lead T. Brown', asset: 'api.company.com', mitreTechnique: 'N/A', playbook: 'Certificate Renewal', sla: '30 days', elapsed: '16 days', notes: 'Automated renewal triggered via ACME. New certificate deployed successfully. No service interruption.'}
  ];

  private _x33cComplianceFrameworks = [
  ...
  private _x33cDataClassificationPolicy = [
  ...
  private _getx33cZoneSummary(): {total: number; highRisk: number; mediumRisk: number; lowRisk: number; totalRules: number} {
    const high = this._x33cSecurityZones.filter(z => z.riskLevel === 'high').length;
    const med = this._x33cSecurityZones.filter(z => z.riskLevel === 'medium').length;
    const low = this._x33cSecurityZones.filter(z => z.riskLevel === 'low').length;
    const rules = this._x33cSecurityZones.reduce((s, z) => s + z.firewallRules, 0);
    return {total: this._x33cSecurityZones.length, highRisk: high, mediumRisk: med, lowRisk: low, totalRules: rules};
  }

  private _getx33cAlertSummary(): {total: number; critical: number; high: number; medium: number; low: number; open: number; contained: number} {
    const crit = this._x33cSecurityAlerts.filter(a => a.severity === 'critical').length;
    const high = this._x33cSecurityAlerts.filter(a => a.severity === 'high').length;
    const med = this._x33cSecurityAlerts.filter(a => a.severity === 'medium').length;
    const low = this._x33cSecurityAlerts.filter(a => a.severity === 'low').length;
    const open = this._x33cSecurityAlerts.filter(a => a.status === 'investigating' || a.status === 'triaged').length;
    const contained = this._x33cSecurityAlerts.filter(a => a.status === 'contained' || a.status === 'remediated').length;
    return {total: this._x33cSecurityAlerts.length, critical: crit, high, medium: med, low, open, contained};
  }

  private _getx33cComplianceSummary(): {total: number; fullyCompliant: number; mostlyCompliant: number; avgComplianceRate: number} {
    const full = this._x33cComplianceFrameworks.filter(f => f.status === 'compliant').length;
    const mostly = this._x33cComplianceFrameworks.filter(f => f.status === 'mostly_compliant').length;
    const avgRate = this._x33cComplianceFrameworks.reduce((s, f) => s + (f.compliant / f.requirements * 100), 0) / this._x33cComplianceFrameworks.length;
    return {total: this._x33cComplianceFrameworks.length, fullyCompliant: full, mostlyCompliant: mostly, avgComplianceRate: Math.round(avgRate * 10) / 10};
  }
  ...
  private _attack_path_RiskRegister = [
    { id: "RSK-ATT-0001", title: "Data breach from unpatched systems", owner: "CTO", category: "Financial", status: "mitigating", trend: "stable", severity: "critical", likelihood: 5, impact: 4, treatment: "Monitor and review quarterly", lastReview: "2026-04-02", nextReview: "2026-07-09" },
    { id: "RSK-ATT-0002", title: "Ransomware attack on critical infrastructure", owner: "CRO", category: "Strategic", status: "closed", trend: "decreasing", severity: "high", likelihood: 9, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-15", nextReview: "2026-07-20" },
    { id: "RSK-ATT-0003", title: "Insider threat data exfiltration", owner: "VP Eng", category: "Compliance", status: "accepted", trend: "increasing", severity: "low", likelihood: 3, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-04", nextReview: "2026-07-04" },
    { id: "RSK-ATT-0004", title: "Supply chain compromise", owner: "Security Lead", category: "Technical", status: "open", trend: "stable", severity: "low", likelihood: 2, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-16", nextReview: "2026-07-06" },
    { id: "RSK-ATT-0005", title: "Cloud misconfiguration exposure", owner: "Risk Mgr", category: "Reputational", status: "mitigating", trend: "decreasing", severity: "critical", likelihood: 6, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-04", nextReview: "2026-07-27" },
    { id: "RSK-ATT-0006", title: "Phishing campaign success rate", owner: "Compliance Dir", category: "Legal", status: "closed", trend: "increasing", severity: "critical", likelihood: 10, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-13", nextReview: "2026-07-27" },
    { id: "RSK-ATT-0007", title: "Third-party vendor data breach", owner: "IT Dir", category: "Regulatory", status: "accepted", trend: "stable", severity: "critical", likelihood: 3, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-17", nextReview: "2026-07-04" },
    { id: "RSK-ATT-0008", title: "Regulatory non-compliance penalty", owner: "DevOps Lead", category: "Third-Party", status: "open", trend: "decreasing", severity: "low", likelihood: 6, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-02", nextReview: "2026-07-23" },
    { id: "RSK-ATT-0009", title: "Zero-day exploit in production", owner: "Architect", category: "Human Capital", status: "mitigating", trend: "increasing", severity: "critical", likelihood: 4, impact: 4, treatment: "Monitor and review quarterly", lastReview: "2026-04-11", nextReview: "2026-07-04" },
    { id: "RSK-ATT-0010", title: "Insufficient access controls", owner: "CISO", category: "Operational", status: "closed", trend: "stable", severity: "high", likelihood: 8, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-03", nextReview: "2026-07-04" },
    { id: "RSK-ATT-0011", title: "DDoS attack on services", owner: "CTO", category: "Financial", status: "accepted", trend: "decreasing", severity: "low", likelihood: 8, impact: 4, treatment: "Monitor and review quarterly", lastReview: "2026-04-09", nextReview: "2026-07-09" },
    { id: "RSK-ATT-0012", title: "Social engineering attack", owner: "CRO", category: "Strategic", status: "open", trend: "increasing", severity: "critical", likelihood: 6, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-15", nextReview: "2026-07-14" },
    { id: "RSK-ATT-0013", title: "API security vulnerability", owner: "VP Eng", category: "Compliance", status: "mitigating", trend: "stable", severity: "critical", likelihood: 5, impact: 5, treatment: "Monitor and review quarterly", lastReview: "2026-04-06", nextReview: "2026-07-21" },
    { id: "RSK-ATT-0014", title: "Mobile device compromise", owner: "Security Lead", category: "Technical", status: "closed", trend: "decreasing", severity: "low", likelihood: 8, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-02", nextReview: "2026-07-14" },
    { id: "RSK-ATT-0015", title: "Physical security breach", owner: "Risk Mgr", category: "Reputational", status: "accepted", trend: "increasing", severity: "high", likelihood: 4, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-05", nextReview: "2026-07-28" },
    { id: "RSK-ATT-0016", title: "Password policy weakness", owner: "Compliance Dir", category: "Legal", status: "open", trend: "stable", severity: "high", likelihood: 4, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-13", nextReview: "2026-07-26" },
    { id: "RSK-ATT-0017", title: "Network segmentation gap", owner: "IT Dir", category: "Regulatory", status: "mitigating", trend: "decreasing", severity: "low", likelihood: 1, impact: 4, treatment: "Monitor and review quarterly", lastReview: "2026-04-08", nextReview: "2026-07-17" },
    { id: "RSK-ATT-0018", title: "Encryption key management failure", owner: "DevOps Lead", category: "Third-Party", status: "closed", trend: "increasing", severity: "high", likelihood: 1, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-18", nextReview: "2026-07-11" },
    { id: "RSK-ATT-0019", title: "Audit trail tampering", owner: "Architect", category: "Human Capital", status: "accepted", trend: "stable", severity: "critical", likelihood: 1, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-22", nextReview: "2026-07-02" },
    { id: "RSK-ATT-0020", title: "Business email compromise", owner: "CISO", category: "Operational", status: "open", trend: "decreasing", severity: "low", likelihood: 9, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-23", nextReview: "2026-07-20" },
  ];
  ...
  private _attack_path_ApiEndpoints = [
    { method: "GET", path: "/api/v1/threats", name: "Threat Intelligence Feed", status: "active", avgLatency: 434.4, reqPerMin: 2295.0, errorRate: 1.08, uptime: round(97.10157667922677,2), version: "v1.2.3" },
    { method: "POST", path: "/api/v1/scans/start", name: "Vulnerability Scanner", status: "active", avgLatency: 132.2, reqPerMin: 2299.0, errorRate: 0.53, uptime: round(98.06383158772945,2), version: "v2.5.14" },
    { method: "GET", path: "/api/v1/assets", name: "Asset Inventory", status: "active", avgLatency: 210.2, reqPerMin: 451.0, errorRate: 2.46, uptime: round(96.33102662611486,2), version: "v2.9.15" },
    { method: "POST", path: "/api/v1/alerts", name: "Alert Management", status: "active", avgLatency: 222.4, reqPerMin: 702.0, errorRate: 0.94, uptime: round(96.55064472223744,2), version: "v1.7.4" },
    { method: "GET", path: "/api/v1/compliance", name: "Compliance Status", status: "active", avgLatency: 263.8, reqPerMin: 787.0, errorRate: 0.74, uptime: round(95.19448108800884,2), version: "v1.3.13" },
    { method: "PUT", path: "/api/v1/policies", name: "Policy Engine", status: "active", avgLatency: 258.1, reqPerMin: 189.0, errorRate: 0.36, uptime: round(96.34857994775594,2), version: "v1.9.4" },
    { method: "GET", path: "/api/v1/incidents", name: "Incident Tracker", status: "active", avgLatency: 417.8, reqPerMin: 492.0, errorRate: 0.34, uptime: round(95.60404341083813,2), version: "v3.3.20" },
    { method: "POST", path: "/api/v1/forensics", name: "Forensics Collector", status: "degraded", avgLatency: 110.7, reqPerMin: 2070.0, errorRate: 0.16, uptime: round(96.39563128181797,2), version: "v1.5.14" },
    { method: "GET", path: "/api/v1/risk/assess", name: "Risk Assessment", status: "active", avgLatency: 313.6, reqPerMin: 2487.0, errorRate: 0.08, uptime: round(95.27198545075315,2), version: "v3.7.6" },
    { method: "POST", path: "/api/v1/auth/verify", name: "Authentication", status: "active", avgLatency: 47.7, reqPerMin: 2016.0, errorRate: 1.73, uptime: round(99.54890172141785,2), version: "v2.1.1" },
    { method: "GET", path: "/api/v1/logs/audit", name: "Audit Log Query", status: "active", avgLatency: 423.4, reqPerMin: 2422.0, errorRate: 0.74, uptime: round(99.29920304046391,2), version: "v1.7.10" },
    { method: "PUT", path: "/api/v1/users/roles", name: "Role Management", status: "active", avgLatency: 125.3, reqPerMin: 2426.0, errorRate: 0.05, uptime: round(96.66522536223148,2), version: "v3.1.17" },
    { method: "POST", path: "/api/v1/encrypt", name: "Encryption Service", status: "active", avgLatency: 124.4, reqPerMin: 1181.0, errorRate: 0.18, uptime: round(97.59964618304095,2), version: "v1.0.10" },
    { method: "GET", path: "/api/v1/network/topo", name: "Network Topology", status: "maintenance", avgLatency: 252.6, reqPerMin: 1883.0, errorRate: 0.92, uptime: round(99.28547972451612,2), version: "v2.5.15" },
    { method: "DELETE", path: "/api/v1/sessions", name: "Session Manager", status: "active", avgLatency: 266.8, reqPerMin: 1831.0, errorRate: 2.31, uptime: round(99.4247630342762,2), version: "v3.3.10" },
  ];
  private _attack_path_ApiKeys = [
    { id: "ak-000001", name: "key-atta-001", created: "2026-03-24", lastUsed: "2026-04-15", status: "active", calls: 47141, rateLimit: 1000 },
    { id: "ak-000002", name: "key-atta-002", created: "2026-06-03", lastUsed: "2026-04-05", status: "active", calls: 30018, rateLimit: 100 },
    { id: "ak-000003", name: "key-atta-003", created: "2026-07-06", lastUsed: "2026-04-11", status: "active", calls: 10709, rateLimit: 5000 },
    { id: "ak-000004", name: "key-atta-004", created: "2026-05-25", lastUsed: "2026-04-06", status: "active", calls: 41814, rateLimit: 100 },
    { id: "ak-000005", name: "key-atta-005", created: "2026-09-19", lastUsed: "2026-04-10", status: "active", calls: 39558, rateLimit: 5000 },
    { id: "ak-000006", name: "key-atta-006", created: "2026-08-11", lastUsed: "2026-04-01", status: "active", calls: 26129, rateLimit: 1000 },
    { id: "ak-000007", name: "key-atta-007", created: "2026-06-25", lastUsed: "2026-04-17", status: "revoked", calls: 16535, rateLimit: 1000 },
    { id: "ak-000008", name: "key-atta-008", created: "2026-09-27", lastUsed: "2026-04-23", status: "active", calls: 39159, rateLimit: 5000 },
  ];
  ...
  private _attack_path_TrainingModules = [
    { id: "TRN-001", name: "Security Fundamentals", completionRate: 49.0, avgScore: 88.8, behaviorChange: 77.4, enrolled: 496, completed: 368, duration: "62min", category: "mandatory" },
    { id: "TRN-002", name: "Phishing Awareness", completionRate: 96.7, avgScore: 59.8, behaviorChange: 43.7, enrolled: 248, completed: 348, duration: "119min", category: "mandatory" },
    { id: "TRN-003", name: "Social Engineering Defense", completionRate: 67.7, avgScore: 60.8, behaviorChange: 31.4, enrolled: 470, completed: 43, duration: "43min", category: "optional" },
    { id: "TRN-004", name: "Password Hygiene", completionRate: 66.7, avgScore: 75.5, behaviorChange: 41.0, enrolled: 341, completed: 311, duration: "81min", category: "mandatory" },
    { id: "TRN-005", name: "Data Classification", completionRate: 83.0, avgScore: 94.6, behaviorChange: 64.6, enrolled: 357, completed: 91, duration: "86min", category: "mandatory" },
    { id: "TRN-006", name: "Incident Response Basics", completionRate: 59.5, avgScore: 75.6, behaviorChange: 70.9, enrolled: 466, completed: 20, duration: "27min", category: "mandatory" },
    { id: "TRN-007", name: "Secure Coding", completionRate: 83.4, avgScore: 64.2, behaviorChange: 37.7, enrolled: 134, completed: 89, duration: "25min", category: "optional" },
    { id: "TRN-008", name: "Cloud Security", completionRate: 60.3, avgScore: 70.6, behaviorChange: 44.2, enrolled: 487, completed: 158, duration: "77min", category: "mandatory" },
    { id: "TRN-009", name: "Mobile Device Security", completionRate: 69.9, avgScore: 60.9, behaviorChange: 68.6, enrolled: 190, completed: 259, duration: "101min", category: "mandatory" },
    { id: "TRN-010", name: "Network Security", completionRate: 86.2, avgScore: 67.2, behaviorChange: 81.6, enrolled: 486, completed: 233, duration: "84min", category: "mandatory" },
    { id: "TRN-011", name: "Physical Security", completionRate: 81.1, avgScore: 59.2, behaviorChange: 37.2, enrolled: 164, completed: 270, duration: "101min", category: "optional" },
    { id: "TRN-012", name: "Regulatory Compliance", completionRate: 62.9, avgScore: 61.7, behaviorChange: 33.8, enrolled: 489, completed: 113, duration: "53min", category: "mandatory" },
    { id: "TRN-013", name: "Risk Management", completionRate: 57.6, avgScore: 91.7, behaviorChange: 44.3, enrolled: 99, completed: 167, duration: "98min", category: "mandatory" },
    { id: "TRN-014", name: "Cryptography Basics", completionRate: 49.1, avgScore: 77.7, behaviorChange: 56.4, enrolled: 452, completed: 438, duration: "119min", category: "mandatory" },
    { id: "TRN-015", name: "Access Control", completionRate: 54.5, avgScore: 66.0, behaviorChange: 62.7, enrolled: 55, completed: 168, duration: "62min", category: "optional" },
    { id: "TRN-016", name: "Vendor Management", completionRate: 84.4, avgScore: 67.8, behaviorChange: 57.5, enrolled: 404, completed: 255, duration: "92min", category: "mandatory" },
  ];
  private _attack_path_PhishingResults = [
    { month: "2026-01", sent: 814, clicked: 163, reported: 71, clickRate: round(20.024570024570025,1), reportRate: round(8.722358722358722,1) },
    { month: "2026-02", sent: 478, clicked: 51, reported: 127, clickRate: round(10.669456066945607,1), reportRate: round(26.569037656903767,1) },
    { month: "2026-03", sent: 526, clicked: 96, reported: 114, clickRate: round(18.250950570342205,1), reportRate: round(21.673003802281368,1) },
    { month: "2026-04", sent: 851, clicked: 222, reported: 410, clickRate: round(26.08695652173913,1), reportRate: round(48.1786133960047,1) },
    { month: "2026-05", sent: 832, clicked: 245, reported: 466, clickRate: round(29.447115384615387,1), reportRate: round(56.00961538461539,1) },
    { month: "2026-06", sent: 714, clicked: 31, reported: 293, clickRate: round(4.341736694677871,1), reportRate: round(41.03641456582633,1) },
  ];
  private _attack_path_TrainingROI = { totalInvestment: 103324, avgCostPerEmployee: 703, riskReductionPct: round(random.uniform(15,45),1), incidentReductionPct: round(random.uniform(10,35),1), complianceScoreGain: round(random.uniform(5,25),1) };
  private _attack_path_LearningPaths = [
    { name: "Beginner Security Analyst", totalModules: 13, completedModules: 14, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 51, enrolled: 170 },
    { name: "Advanced Threat Hunter", totalModules: 19, completedModules: 11, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 47, enrolled: 99 },
    { name: "Security Architect", totalModules: 8, completedModules: 2, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 53, enrolled: 102 },
    { name: "Incident Responder", totalModules: 19, completedModules: 11, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 18, enrolled: 160 },
    { name: "Compliance Specialist", totalModules: 12, completedModules: 8, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 43, enrolled: 27 },
    { name: "DevSecOps Engineer", totalModules: 12, completedModules: 2, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 69, enrolled: 139 },
  ];
  ...
  private _attack_path_GovBodies = [
    { name: "Security Steering Committee", members: 14, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-17", nextMeeting: "2026-05-07", quorum: 8 },
    { name: "Risk Management Board", members: 8, chair: "CTO", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-17", nextMeeting: "2026-05-04", quorum: 8 },
    { name: "Data Governance Council", members: 5, chair: "CRO", meetingFreq: "Monthly", lastMeeting: "2026-04-18", nextMeeting: "2026-05-26", quorum: 7 },
    { name: "Compliance Oversight Board", members: 10, chair: "CDO", meetingFreq: "Monthly", lastMeeting: "2026-04-16", nextMeeting: "2026-05-02", quorum: 3 },
    { name: "Architecture Review Board", members: 13, chair: "VP Eng", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-07", nextMeeting: "2026-05-23", quorum: 3 },
    { name: "Change Advisory Board", members: 10, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-17", nextMeeting: "2026-05-11", quorum: 4 },
    { name: "Incident Review Board", members: 9, chair: "CIO", meetingFreq: "As needed", lastMeeting: "2026-04-14", nextMeeting: "2026-05-08", quorum: 5 },
    { name: "Vendor Risk Committee", members: 7, chair: "CFO", meetingFreq: "Quarterly", lastMeeting: "2026-04-15", nextMeeting: "2026-05-25", quorum: 8 },
  ];
  private _attack_path_Decisions = [
    ...
    { id: "DEC-0003", title: "Implement DLP across all endpoints", date: "2026-01-03", status: "in-progress", owner: "Security Lead" },
    { id: "DEC-0004", title: "Mandate MFA for all external access", date: "2026-02-03", status: "pending", owner: "CRO" },
    ...
    { id: "DEC-0007", title: "Conduct annual penetration testing", date: "2026-04-14", status: "implemented", owner: "CTO" },
    ...
    { id: "DEC-0010", title: "Migrate to passwordless authentication", date: "2026-02-26", status: "approved", owner: "VP Eng" },
  ];
  private _attack_path_PolicyLifecycle = [
    { name: "Information Security Policy", version: "v4.5", status: "active", lastUpdated: "2026-04-01", nextReview: "2026-10-24", owner: "CISO" },
    { name: "Acceptable Use Policy", version: "v3.3", status: "active", lastUpdated: "2026-03-28", nextReview: "2026-8-14", owner: "Legal" },
    { name: "Data Retention Policy", version: "v4.4", status: "under-review", lastUpdated: "2026-04-07", nextReview: "2026-8-18", owner: "DPO" },
    { name: "Access Control Policy", version: "v3.5", status: "active", lastUpdated: "2026-02-12", nextReview: "2026-11-01", owner: "IAM Lead" },
    { name: "Incident Response Policy", version: "v2.5", status: "draft", lastUpdated: "2026-03-24", nextReview: "2026-12-04", owner: "IR Lead" },
    { name: "Business Continuity Plan", version: "v2.3", status: "active", lastUpdated: "2026-03-03", nextReview: "2026-11-02", owner: "BCP Mgr" },
    { name: "Vendor Management Policy", version: "v1.0", status: "active", lastUpdated: "2026-02-05", nextReview: "2026-7-21", owner: "Procurement" },
    { name: "Encryption Standard", version: "v2.4", status: "under-review", lastUpdated: "2026-02-11", nextReview: "2026-10-15", owner: "Security Arch" },
  ];
  private _attack_path_GovMaturityScore = { overall: 4.9, riskManagement: 5.0, compliance: 4.4, incidentResponse: 3.0, awareness: 2.5, technology: 5.4 };
  ...
  private _attack_path_InnoProjects = [
    { id: "INN-001", name: "AI-Powered Threat Detection", description: "Machine learning models for real-time threat identification", status: "active", progress: 72, startDate: "2026-03-02", teamSize: 2, budget: 168085, milestones: 10, completedMilestones: 4 },
    { id: "INN-002", name: "Quantum-Resistant Cryptography", description: "Post-quantum encryption algorithm prototyping", status: "research", progress: 35, startDate: "2026-02-18", teamSize: 4, budget: 51739, milestones: 3, completedMilestones: 9 },
    { id: "INN-003", name: "Automated Red Teaming", description: "Autonomous penetration testing framework", status: "active", progress: 58, startDate: "2026-03-09", teamSize: 8, budget: 144795, milestones: 4, completedMilestones: 2 },
    { id: "INN-004", name: "Zero-Knowledge Authentication", description: "Privacy-preserving identity verification", status: "poc", progress: 88, startDate: "2026-01-16", teamSize: 8, budget: 99491, milestones: 10, completedMilestones: 8 },
    { id: "INN-005", name: "Blockchain Audit Trail", description: "Immutable security event logging", status: "active", progress: 45, startDate: "2026-03-28", teamSize: 8, budget: 31704, milestones: 4, completedMilestones: 5 },
    { id: "INN-006", name: "Behavioral Biometrics", description: "Continuous authentication via user behavior patterns", status: "research", progress: 22, startDate: "2026-03-22", teamSize: 7, budget: 147449, milestones: 5, completedMilestones: 6 },
    { id: "INN-007", name: "Deception Grid 2.0", description: "Advanced honeypot network with adaptive responses", status: "active", progress: 65, startDate: "2026-03-13", teamSize: 6, budget: 48010, milestones: 5, completedMilestones: 9 },
    { id: "INN-008", name: "Secure Enclave Integration", description: "Hardware-backed security for critical workloads", status: "poc", progress: 40, startDate: "2026-03-24", teamSize: 5, budget: 80068, milestones: 7, completedMilestones: 8 },
  ];
  private _attack_path_TechEvaluations = [
    ...
    { name: "eBPF for Runtime Detection", status: "completed", score: 8.7, recommendation: "Adopt", vendor: "AWS" },
    { name: "Confidential Computing", status: "planned", score: 5.8, recommendation: "Investigate", vendor: "Azure" },
    { name: "Homomorphic Encryption", status: "evaluating", score: 10.9, recommendation: "Pilot", vendor: "GCP" },
  ...
  private _attack_path_InnoMetrics = { totalProjects: 8, activeProjects: 4, avgTimeToValue: "112 days", pocSuccessRate: round(random.uniform(55,85),1), researchToProduction: round(random.uniform(20,50),1), innovationIndex: round(random.uniform(6.0,9.5),1) };
  ...
  private _attack_path_ComplianceFrameworks = [
    { name: "SOC 2 Type II", description: "Trust Services Criteria", totalControls: 5, implementedControls: 5, status: "in-review", lastAudit: "2026-01-28", nextAudit: "2026-9-01", evidenceCount: 55 },
    { name: "ISO 27001", description: "Information Security Management", totalControls: 114, implementedControls: 109, status: "in-review", lastAudit: "2026-04-26", nextAudit: "2026-8-24", evidenceCount: 137 },
    { name: "PCI DSS 4.0", description: "Payment Card Industry", totalControls: 12, implementedControls: 10, status: "in-review", lastAudit: "2026-03-26", nextAudit: "2026-7-24", evidenceCount: 452 },
    { name: "HIPAA", description: "Health Insurance Portability", totalControls: 18, implementedControls: 12, status: "partially-compliant", lastAudit: "2026-04-14", nextAudit: "2026-8-05", evidenceCount: 170 },
    { name: "GDPR", description: "Data Protection Regulation", totalControls: 99, implementedControls: 79, status: "compliant", lastAudit: "2026-01-05", nextAudit: "2026-7-25", evidenceCount: 196 },
    { name: "NIST CSF 2.0", description: "Cybersecurity Framework", totalControls: 6, implementedControls: 6, status: "compliant", lastAudit: "2026-03-14", nextAudit: "2026-8-24", evidenceCount: 409 },
    { name: "FedRAMP", description: "Federal Risk Authorization", totalControls: 15, implementedControls: 13, status: "compliant", lastAudit: "2026-04-26", nextAudit: "2026-8-23", evidenceCount: 331 },
    { name: "SOX", description: "Sarbanes-Oxley Compliance", totalControls: 8, implementedControls: 7, status: "non-compliant", lastAudit: "2026-04-23", nextAudit: "2026-10-21", evidenceCount: 142 },
    { name: "CIS Controls v8", description: "Center for Internet Security", totalControls: 18, implementedControls: 17, status: "compliant", lastAudit: "2026-02-20", nextAudit: "2026-11-15", evidenceCount: 253 },
    { name: "COBIT 2019", description: "IT Governance Framework", totalControls: 40, implementedControls: 21, status: "partially-compliant", lastAudit: "2026-01-10", nextAudit: "2026-10-14", evidenceCount: 463 },
  ];
  ...
  private _attack_path_AuditTrail = [
    ...
    { id: "AUD-0004", action: "Remediation completed", auditor: "Compliance Officer", date: "2026-04-15", result: "pass", findings: 5 },
    { id: "AUD-0005", action: "Policy updated", auditor: "IT Audit", date: "2026-04-19", result: "pass", findings: 0 },
    { id: "AUD-0006", action: "Training verified", auditor: "Risk Team", date: "2026-04-21", result: "pass", findings: 0 },
    { id: "AUD-0007", action: "Access reviewed", auditor: "QA Team", date: "2026-04-15", result: "pass", findings: 5 },
    { id: "AUD-0008", action: "Exception approved", auditor: "CISO Office", date: "2026-04-16", result: "conditional", findings: 2 },
    { id: "AUD-0009", action: "Risk accepted", auditor: "Board Audit", date: "2026-04-05", result: "pass", findings: 3 },
    { id: "AUD-0010", action: "Control enhanced", auditor: "Third Party", date: "2026-04-09", result: "pass", findings: 4 },
  ];

  // === Threat Intelligence Feed Extension (attack_path_) ===
  private _attack_path_ThreatActors = [
    { name: "APT-29", alias: "Cozy Bear", origin: "Russia", type: "Nation-State", severity: "high", lastActivity: "2026-04-10", targets: "Energy", indicators: 68, ttps: 30 },
    { name: "APT-41", alias: "Double Dragon", origin: "China", type: "Nation-State", severity: "critical", lastActivity: "2026-04-12", targets: "Healthcare", indicators: 360, ttps: 23 },
    { name: "Lazarus Group", alias: "Hidden Cobra", origin: "North Korea", type: "Nation-State", severity: "critical", lastActivity: "2026-04-23", targets: "Energy", indicators: 453, ttps: 41 },
    { name: "FIN7", alias: "Carbanak", origin: "Eastern Europe", type: "Financial", severity: "high", lastActivity: "2026-04-09", targets: "Finance", indicators: 75, ttps: 25 },
    { name: "Conti", alias: "Wizard Spider", origin: "Russia", type: "Ransomware", severity: "critical", lastActivity: "2026-04-22", targets: "Finance", indicators: 352, ttps: 29 },
    { name: "LockBit", alias: "LockBit Gang", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-18", targets: "Manufacturing", indicators: 35, ttps: 28 },
    { name: "Cl0p", alias: "Cl0p Team", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-08", targets: "Technology", indicators: 334, ttps: 28 },
    { name: "Sandworm", alias: "Unit 74455", origin: "Russia", type: "Nation-State", severity: "critical", lastActivity: "2026-04-11", targets: "Technology", indicators: 170, ttps: 18 },
  ];
  private _attack_path_IoCFeed = [
    { id: "ioc-000001", type: "ip", value: "222.160.93.220", confidence: 88, source: "STIX", firstSeen: "2026-04-23", lastSeen: "2026-04-07" },
    { id: "ioc-000002", type: "ip", value: "91.95.123.242", confidence: 88, source: "VirusTotal", firstSeen: "2026-04-22", lastSeen: "2026-04-19" },
    { id: "ioc-000003", type: "ip", value: "159.206.221.66", confidence: 85, source: "VirusTotal", firstSeen: "2026-04-19", lastSeen: "2026-04-08" },
    { id: "ioc-000004", type: "ip", value: "155.150.104.205", confidence: 64, source: "MISP", firstSeen: "2026-04-14", lastSeen: "2026-04-02" },
    { id: "ioc-000005", type: "ip", value: "55.62.126.208", confidence: 46, source: "AlienVault", firstSeen: "2026-04-21", lastSeen: "2026-04-17" },
    { id: "ioc-000006", type: "ip", value: "113.41.176.60", confidence: 81, source: "STIX", firstSeen: "2026-04-18", lastSeen: "2026-04-12" },
    { id: "ioc-000007", type: "ip", value: "12.75.120.114", confidence: 59, source: "CrowdStrike", firstSeen: "2026-04-06", lastSeen: "2026-04-21" },
    { id: "ioc-000008", type: "ip", value: "157.171.224.47", confidence: 84, source: "CrowdStrike", firstSeen: "2026-04-02", lastSeen: "2026-04-02" },
    { id: "ioc-000009", type: "ip", value: "13.86.153.216", confidence: 93, source: "STIX", firstSeen: "2026-04-19", lastSeen: "2026-04-01" },
    { id: "ioc-000010", type: "ip", value: "228.228.250.86", confidence: 75, source: "Mandiant", firstSeen: "2026-04-13", lastSeen: "2026-04-23" },
    { id: "ioc-000011", type: "ip", value: "225.52.250.1", confidence: 54, source: "VirusTotal", firstSeen: "2026-04-10", lastSeen: "2026-04-19" },
    { id: "ioc-000012", type: "ip", value: "170.113.103.152", confidence: 43, source: "MISP", firstSeen: "2026-04-23", lastSeen: "2026-04-18" },
    { id: "ioc-000013", type: "ip", value: "68.104.223.32", confidence: 65, source: "VirusTotal", firstSeen: "2026-04-04", lastSeen: "2026-04-05" },
    { id: "ioc-000014", type: "ip", value: "216.95.189.232", confidence: 41, source: "STIX", firstSeen: "2026-04-18", lastSeen: "2026-04-20" },
    { id: "ioc-000015", type: "ip", value: "90.224.178.83", confidence: 97, source: "CrowdStrike", firstSeen: "2026-04-09", lastSeen: "2026-04-01" },
  ];
  ...
  private _attack_path_Incidents = [
    { id: "INC-20260001", title: "Unauthorized access detected", severity: "critical", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-21T07:05", affectedAssets: 37, rootCause: "Misconfiguration" },
    { id: "INC-20260002", title: "Malware outbreak on workstation", severity: "high", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-08T12:17", affectedAssets: 48, rootCause: "Credential compromise" },
    { id: "INC-20260003", title: "Data leak from S3 bucket", severity: "medium", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-07T11:16", affectedAssets: 25, rootCause: "Zero-day" },
    { id: "INC-20260004", title: "Phishing campaign targeting finance", severity: "low", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-17T11:30", affectedAssets: 45, rootCause: "Human error" },
    { id: "INC-20260005", title: "DDoS attack on web services", severity: "critical", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-04T16:38", affectedAssets: 13, rootCause: "Policy violation" },
    { id: "INC-20260006", title: "Ransomware encryption attempt", severity: "high", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-02T17:47", affectedAssets: 10, rootCause: "Unknown" },
    { id: "INC-20260007", title: "Insider data exfiltration", severity: "medium", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-01T00:48", affectedAssets: 11, rootCause: "Misconfiguration" },
    { id: "INC-20260008", title: "API key exposure in repo", severity: "low", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-04T11:39", affectedAssets: 37, rootCause: "Credential compromise" },
    { id: "INC-20260009", title: "SQL injection on portal", severity: "critical", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-20T18:46", affectedAssets: 46, rootCause: "Zero-day" },
    { id: "INC-20260010", title: "Brute force on VPN", severity: "high", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-17T03:19", affectedAssets: 7, rootCause: "Human error" },
    { id: "INC-20260011", title: "Supply chain alert from vendor", severity: "medium", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-02T18:45", affectedAssets: 15, rootCause: "Policy violation" },
    { id: "INC-20260012", title: "Suspicious lateral movement", severity: "low", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-02T09:07", affectedAssets: 33, rootCause: "Unknown" },
  ];
  private _attack_path_GetIncidentStats() {
    const inc = this._attack_path_Incidents;
    return { total: inc.length, open: inc.filter(i=>i.status==="open").length, investigating: inc.filter(i=>i.status==="investigating").length, mttd: 43, mttr: 121 };
  }
  ...
  @state() private _soarPlaybooks: Array<{
    ...
    {
      id: 'pb-soar-001', name: 'Attack Path Discovery Auto Response', status: 'active',
      steps: [
    ...
    {
      id: 'pb-soar-002', name: 'Attack Path Discovery Investigation Workflow', status: 'active',
      steps: [
    ...
    {
      id: 'pb-soar-003', name: 'Attack Path Discovery Compliance Scan', status: 'draft',
      steps: [
  ...
  @state() private _apdAle: number = 0;
  @state() private _apdSroi: number = 0;
  @state() private _apdCpi: number = 0;
  @state() private _apdBudgetAlloc: number = 0;
  @state() private _apdCostBenefit: number = 0;

  // Security Economics Calculator
  private apdInitEconomics() {
    this._apdAle = Math.round(2850000 + Math.random() * 4500000);
    this._apdSroi = Math.round(180 + Math.random() * 320);
    this._apdCpi = Math.round(45000 + Math.random() * 120000);
    this._apdBudgetAlloc = Math.round(2500000 + Math.random() * 3000000);
    this._apdCostBenefit = Math.round(220 + Math.random() * 180);
  }

  private _apdCalcAle(): { annual: number; perIncident: number; byCategory: Array<{name: string; value: number}> } {
    const base = this._apdAle;
    const categories = [
  ...
  private _apdCalcSroi(): Array<{year: number; investment: number; savings: number; roi: number}> {
    const baseInv = this._apdSroi * 10000;
  ...
  private _apdGetBudgetAlloc(): Array<{category: string; amount: number; pct: number; trend: string}> {
    const total = this._apdBudgetAlloc;
    const items = [
  ...
  private _apdGetCostBenefit(): Array<{control: string; cost: number; benefit: number; ratio: number; priority: string}> {
    const base = this._apdCostBenefit;
    const controls = [
  ...
  private _apdRenderEconomics() {
    const ale = this._apdCalcAle();
    const roi = this._apdCalcSroi();
    const budget = this._apdGetBudgetAlloc();
    const cb = this._apdGetCostBenefit();
    const cpi = this._apdCpi;
  ...
  @state() private _apdThreatLevel: any = null;
  @state() private _apdEmergingThreats: any = null;
  @state() private _apdThreatTrends: any = null;
  @state() private _apdSectorRadar: any = null;
  @state() private _apdActorActivity: any = null;

  // Threat Landscape Intelligence
  private apdInitThreatIntel() {
    this._apdThreatLevel = {
  ...
  private _apdRenderThreatIntel() {
    const tl = this._apdThreatLevel;
    const et = this._apdEmergingThreats;
    const sr = this._apdSectorRadar;
    const aa = this._apdActorActivity;
  ...
  @state() private _apdPolicies: any = null;
  @state() private _apdExceptions: any = null;
  @state() private _apdRiskRegister: any = null;
  @state() private _apdMeetings: any = null;
  @state() private _apdDeadlines: any = null;

  // Security Governance Dashboard
  private apdInitGovernance() {
    this._apdPolicies = [
  ...
  private _apdRenderGovernance() {
    const policies = this._apdPolicies;
    const risks = this._apdRiskRegister;
    const deadlines = this._apdDeadlines;
  ...
  @state() private _apdCriticalAssets: any = null;
  @state() private _apdAssetDeps: any = null;
  @state() private _apdEolAssets: any = null;
  @state() private _apdAssetRisk: any = null;

  // Asset Intelligence
  private apdInitAssetIntel() {
    this._apdCriticalAssets = [
    ...
    this._apdAssetRisk = { critical: 7, high: 23, medium: 45, low: 128, total: 203 };
  }

  private _apdRenderAssetIntel() {
    const assets = this._apdCriticalAssets;
    const eol = this._apdEolAssets;
    const ar = this._apdAssetRisk;
  ...
  @state() private _apdUserBaseline: any = null;
  @state() private _apdAnomalyRules: any = null;
  @state() private _apdDataAccess: any = null;
  @state() private _apdInsiderRisk: any = null;
  ...
  private apdInitInsiderThreat() {
    this._apdUserBaseline = [
    ...
    this._apdInsiderRisk = { totalUsers: 2847, monitored: 342, flagged: 18, investigated: 5, confirmed: 1 };
  }

  private _apdRenderInsiderThreat() {
    const baseline = this._apdUserBaseline;
    const rules = this._apdAnomalyRules;
    const ir = this._apdInsiderRisk;
...
// ===== SECURITY CONTROL TESTING (R22) =====
  ...
  private _renderBudgetPlanning() {
    const budgetData = [
      { category: "Personnel & Training", planned: 3524000, actual: 2644000, utilization: 75.0, q1: "24%", q2: "29%", q3: "27%", q4: "19%" },
      { category: "Tooling & Licensing", planned: 3531000, actual: 2657000, utilization: 75.2, q1: "27%", q2: "34%", q3: "22%", q4: "20%" },
      { category: "Infrastructure Security", planned: 3538000, actual: 2670000, utilization: 75.5, q1: "30%", q2: "23%", q3: "33%", q4: "21%" },
      { category: "Compliance & Audit", planned: 3545000, actual: 2683000, utilization: 75.7, q1: "17%", q2: "28%", q3: "28%", q4: "22%" },
      { category: "Incident Response", planned: 3552000, actual: 2696000, utilization: 75.9, q1: "20%", q2: "33%", q3: "23%", q4: "23%" },
      { category: "Third-Party Assessments", planned: 3559000, actual: 2709000, utilization: 76.1, q1: "23%", q2: "22%", q3: "18%", q4: "24%" },
      { category: "Security Awareness", planned: 3566000, actual: 2722000, utilization: 76.3, q1: "26%", q2: "27%", q3: "29%", q4: "25%" },
      { category: "Research & Innovation", planned: 3573000, actual: 2735000, utilization: 76.5, q1: "29%", q2: "32%", q3: "24%", q4: "10%" },
    ];
    ...
    const headcount = [
      { team: "SOC Tier 1", current: 13, target: 20, gap: 3, avgSalary: "160k" },
      { team: "SOC Tier 2", current: 5, target: 19, gap: 2, avgSalary: "189k" },
      { team: "Threat Intel", current: 14, target: 18, gap: 1, avgSalary: "107k" },
      { team: "Red Team", current: 6, target: 17, gap: 0, avgSalary: "136k" },
      { team: "GRC", current: 15, target: 16, gap: 5, avgSalary: "165k" },
      { team: "AppSec", current: 7, target: 15, gap: 4, avgSalary: "194k" },
      { team: "Cloud Sec", current: 16, target: 14, gap: 3, avgSalary: "112k" },
      { team: "Identity & Access", current: 8, target: 13, gap: 2, avgSalary: "141k" },
    ];
    const vendorSpend = [
      { vendor: "CrowdStrike", annual: "491k", contractEnd: "2026-10", renewalRisk: "Low", satisfaction: 3 },
      { vendor: "Palo Alto", annual: "522k", contractEnd: "2026-11", renewalRisk: "Medium", satisfaction: 5 },
      { vendor: "Splunk", annual: "553k", contractEnd: "2026-12", renewalRisk: "High", satisfaction: 4 },
      { vendor: "Qualys", annual: "584k", contractEnd: "2026-01", renewalRisk: "Low", satisfaction: 3 },
      { vendor: "Rapid7", annual: "615k", contractEnd: "2026-02", renewalRisk: "Medium", satisfaction: 5 },
      { vendor: "Mandiant", annual: "646k", contractEnd: "2026-03", renewalRisk: "High", satisfaction: 4 },
      { vendor: "Zscaler", annual: "677k", contractEnd: "2026-04", renewalRisk: "Low", satisfaction: 3 },
      { vendor: "Duo Security", annual: "708k", contractEnd: "2026-05", renewalRisk: "Medium", satisfaction: 5 },
    ];
    const roiProjections = [
      { area: "Threat Detection", investment: "730k", projectedReturn: "1386k", roiMultiple: "3.0x", confidence: 69 },
      { area: "Incident Reduction", investment: "773k", projectedReturn: "1433k", roiMultiple: "2.9x", confidence: 86 },
      { area: "Compliance Savings", investment: "816k", projectedReturn: "1480k", roiMultiple: "2.8x", confidence: 67 },
      { area: "Automation Gains", investment: "859k", projectedReturn: "1527k", roiMultiple: "2.7x", confidence: 84 },
      { area: "Risk Avoidance", investment: "101k", projectedReturn: "1574k", roiMultiple: "2.6x", confidence: 65 },
    ];
    return html`
  ...
  private _renderMetricsNormalization() {
    const kpiCatalog = [
      { id: "kpi-1", name: "MTTD", owner: "SOC", unit: "minutes", target: 76, current: 63, benchmark: 89, collection: "auto", frequency: "realtime" },
      { id: "kpi-2", name: "MTTR", owner: "GRC", unit: "%", target: 79, current: 70, benchmark: 66, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-3", name: "MTTC", owner: "AppSec", unit: "score", target: 82, current: 77, benchmark: 77, collection: "manual", frequency: "weekly" },
      { id: "kpi-4", name: "Vuln SLA Compliance", owner: "Cloud Sec", unit: "count", target: 85, current: 84, benchmark: 88, collection: "auto", frequency: "monthly" },
      { id: "kpi-5", name: "Patch Coverage", owner: "Identity", unit: "days", target: 88, current: 91, benchmark: 65, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-6", name: "Phishing Click Rate", owner: "Threat Intel", unit: "minutes", target: 91, current: 98, benchmark: 76, collection: "manual", frequency: "daily" },
      { id: "kpi-7", name: "Training Completion", owner: "Security Ops", unit: "%", target: 94, current: 59, benchmark: 87, collection: "auto", frequency: "weekly" },
      { id: "kpi-8", name: "Escalation Rate", owner: "Risk Mgmt", unit: "score", target: 97, current: 66, benchmark: 98, collection: "semi-auto", frequency: "monthly" },
      { id: "kpi-9", name: "False Positive Rate", owner: "SOC", unit: "count", target: 70, current: 73, benchmark: 75, collection: "manual", frequency: "realtime" },
      { id: "kpi-10", name: "Threat Intel Actionability", owner: "GRC", unit: "days", target: 73, current: 80, benchmark: 86, collection: "auto", frequency: "daily" },
      { id: "kpi-11", name: "Endpoint Compliance", owner: "AppSec", unit: "minutes", target: 76, current: 87, benchmark: 97, collection: "semi-auto", frequency: "weekly" },
      { id: "kpi-12", name: "Cloud Misconfig Score", owner: "Cloud Sec", unit: "%", target: 79, current: 94, benchmark: 74, collection: "manual", frequency: "monthly" },
      { id: "kpi-13", name: "Identity Anomaly Rate", owner: "Identity", unit: "score", target: 82, current: 55, benchmark: 85, collection: "auto", frequency: "realtime" },
      { id: "kpi-14", name: "DLP Events", owner: "Threat Intel", unit: "count", target: 85, current: 62, benchmark: 96, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-15", name: "Vendor Risk Avg", owner: "Security Ops", unit: "days", target: 88, current: 69, benchmark: 73, collection: "manual", frequency: "weekly" },
      { id: "kpi-16", name: "Compliance Audit Pass Rate", owner: "Risk Mgmt", unit: "minutes", target: 91, current: 76, benchmark: 84, collection: "auto", frequency: "monthly" },
      { id: "kpi-17", name: "Awareness Score", owner: "SOC", unit: "%", target: 94, current: 83, benchmark: 95, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-18", name: "SOC Utilization", owner: "GRC", unit: "score", target: 97, current: 90, benchmark: 72, collection: "manual", frequency: "daily" },
      { id: "kpi-19", name: "Automation Coverage", owner: "AppSec", unit: "count", target: 70, current: 97, benchmark: 83, collection: "auto", frequency: "weekly" },
      { id: "kpi-20", name: "Risk Register Currency", owner: "Cloud Sec", unit: "days", target: 73, current: 58, benchmark: 94, collection: "semi-auto", frequency: "monthly" },
    ];
    const benchmarkSources = [
      { source: "NIST CSF", mappedKPIs: 3, alignment: 60, lastReview: "2026-01-01", status: "aligned" },
      { source: "CIS Controls v8", mappedKPIs: 4, alignment: 77, lastReview: "2026-02-24", status: "partial" },
      { source: "ISO 27001:2022", mappedKPIs: 5, alignment: 94, lastReview: "2026-03-19", status: "reviewing" },
      { source: "PCI DSS 4.0", mappedKPIs: 6, alignment: 72, lastReview: "2026-04-14", status: "aligned" },
      { source: "SOC 2 Type II", mappedKPIs: 7, alignment: 89, lastReview: "2026-05-09", status: "partial" },
      { source: "MITRE ATT&CK", mappedKPIs: 8, alignment: 67, lastReview: "2026-06-04", status: "reviewing" },
      { source: "SANS Top 20", mappedKPIs: 3, alignment: 84, lastReview: "2026-01-27", status: "aligned" },
      { source: "OWASP Top 10", mappedKPIs: 4, alignment: 62, lastReview: "2026-02-22", status: "partial" },
    ];
  ...
  private _renderThreatHuntingCampaigns() {
    const campaigns = [
      { id: "HC-1001", name: "Lateral Movement Sweep", status: "active", hypothesis: "H1: Actors using pass-the-hash for lateral movement", leadHunter: "Alice Chen", findings: 40, startDate: "2026-01-21", endDate: null, effectiveness: 72 },
      { id: "HC-1002", name: "Credential Harvesting Hunt", status: "completed", hypothesis: "H2: Actors using web shells for persistence", leadHunter: "Bob Martinez", findings: 43, startDate: "2026-04-04", endDate: "2026-06-10", effectiveness: 91 },
      { id: "HC-1003", name: "Persistence Mechanism Audit", status: "planned", hypothesis: "H3: Actors using scheduled tasks for data theft", leadHunter: "Carol Wu", findings: 46, startDate: "2026-03-15", endDate: null, effectiveness: 51 },
      { id: "HC-1004", name: "C2 Beacon Detection", status: "in-review", hypothesis: "H4: Actors using DNS tunneling for C2 communication", leadHunter: "Dave Kim", findings: 1, startDate: "2026-02-26", endDate: null, effectiveness: 70 },
      { id: "HC-1005", name: "Data Exfiltration Patterns", status: "active", hypothesis: "H5: Actors using encrypted channels for privilege escalation", leadHunter: "Eve Johnson", findings: 4, startDate: "2026-01-09", endDate: null, effectiveness: 89 },
      { id: "HC-1006", name: "Privilege Escalation Scan", status: "completed", hypothesis: "H6: Actors using token impersonation for defense evasion", leadHunter: "Frank Liu", findings: 7, startDate: "2026-04-20", endDate: "2026-04-22", effectiveness: 49 },
      { id: "HC-1007", name: "Supply Chain Implant Hunt", status: "planned", hypothesis: "H7: Actors using poisoned images for initial access", leadHunter: "Grace Park", findings: 10, startDate: "2026-03-03", endDate: null, effectiveness: 68 },
      { id: "HC-1008", name: "Insider Threat Indicators", status: "in-review", hypothesis: "H8: Actors using legitimate tools for credential access", leadHunter: "Hector Silva", findings: 13, startDate: "2026-02-14", endDate: null, effectiveness: 87 },
      { id: "HC-1009", name: "Cloud Metadata Analysis", status: "active", hypothesis: "H9: Actors using API keys for command execution", leadHunter: "Alice Chen", findings: 16, startDate: "2026-01-25", endDate: null, effectiveness: 47 },
      { id: "HC-1010", name: "DNS Tunnel Detection", status: "completed", hypothesis: "H10: Actors using encoded subdomains for exfiltration", leadHunter: "Bob Martinez", findings: 19, startDate: "2026-04-08", endDate: "2026-05-06", effectiveness: 66 },
      { id: "HC-1011", name: "Fileless Malware Search", status: "planned", hypothesis: "H11: Actors using WMI providers for discovery", leadHunter: "Carol Wu", findings: 22, startDate: "2026-03-19", endDate: null, effectiveness: 85 },
      { id: "HC-1012", name: "Zero-Day Exploit Traces", status: "in-review", hypothesis: "H12: Actors using exploit kits for collection", leadHunter: "Dave Kim", findings: 25, startDate: "2026-02-02", endDate: null, effectiveness: 45 },
    ];
    const hunterLeaderboard = [
      { hunter: "Alice Chen", campaigns: 15, findings: 117, highSeverity: 12, avgScore: 67, streak: 1 },
      { hunter: "Bob Martinez", campaigns: 12, findings: 30, highSeverity: 17, avgScore: 60, streak: 2 },
      { hunter: "Carol Wu", campaigns: 9, findings: 59, highSeverity: 22, avgScore: 97, streak: 3 },
      { hunter: "Dave Kim", campaigns: 6, findings: 88, highSeverity: 1, avgScore: 90, streak: 4 },
      { hunter: "Eve Johnson", campaigns: 3, findings: 117, highSeverity: 6, avgScore: 83, streak: 5 },
      { hunter: "Frank Liu", campaigns: 13, findings: 30, highSeverity: 11, avgScore: 76, streak: 6 },
      { hunter: "Grace Park", campaigns: 10, findings: 59, highSeverity: 16, avgScore: 69, streak: 7 },
      { hunter: "Hector Silva", campaigns: 7, findings: 88, highSeverity: 21, avgScore: 62, streak: 8 },
    ];
    const mitreMapping = [
      { tactic: "Initial Access", techniques: 3, campaigns: 5, coverage: 54 },
      { tactic: "Execution", techniques: 2, campaigns: 4, coverage: 34 },
      { tactic: "Persistence", techniques: 12, campaigns: 3, coverage: 85 },
      { tactic: "Privilege Escalation", techniques: 11, campaigns: 2, coverage: 65 },
      { tactic: "Defense Evasion", techniques: 10, campaigns: 1, coverage: 45 },
      { tactic: "Credential Access", techniques: 9, campaigns: 6, coverage: 96 },
      { tactic: "Discovery", techniques: 8, campaigns: 5, coverage: 76 },
      { tactic: "Lateral Movement", techniques: 7, campaigns: 4, coverage: 56 },
      { tactic: "Collection", techniques: 6, campaigns: 3, coverage: 36 },
      { tactic: "Exfiltration", techniques: 5, campaigns: 2, coverage: 87 },
      { tactic: "Command & Control", techniques: 4, campaigns: 1, coverage: 67 },
      { tactic: "Impact", techniques: 3, campaigns: 6, coverage: 47 },
    ];
    return html`
  ...
  private _renderControlInventory() {
    const controls = [
      { id: "CTL-2001", name: "MFA Enforcement", domain: "Access Control", status: "implemented", effectiveness: 69, lastTest: "2026-01-13", nextReview: "2026-05-13", owner: "SOC", risk: "Low" },
      { id: "CTL-2002", name: "Network Segmentation", domain: "Network Security", status: "partial", effectiveness: 42, lastTest: "2026-04-26", nextReview: "2026-06-04", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2003", name: "EDR Deployment", domain: "Endpoint Protection", status: "planned", effectiveness: 49, lastTest: "2026-03-11", nextReview: "2026-07-23", owner: "IT Ops", risk: "High" },
      { id: "CTL-2004", name: "DLP Policy", domain: "Data Protection", status: "gap", effectiveness: 5, lastTest: "2026-02-24", nextReview: "2026-08-14", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2005", name: "SSO Integration", domain: "Identity Management", status: "implemented", effectiveness: 81, lastTest: "2026-01-09", nextReview: "2026-09-05", owner: "IAM", risk: "Low" },
      { id: "CTL-2006", name: "SAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 19, lastTest: "2026-04-22", nextReview: "2026-10-24", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2007", name: "CSPM Scanning", domain: "Cloud Security", status: "planned", effectiveness: 26, lastTest: "2026-03-07", nextReview: "2026-11-15", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2008", name: "Badge Access", domain: "Physical Security", status: "gap", effectiveness: 33, lastTest: "2026-02-20", nextReview: "2026-12-06", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2009", name: "Least Privilege", domain: "Access Control", status: "implemented", effectiveness: 93, lastTest: "2026-01-05", nextReview: "2026-05-25", owner: "SOC", risk: "Low" },
      { id: "CTL-2010", name: "Firewall Rules", domain: "Network Security", status: "partial", effectiveness: 47, lastTest: "2026-04-18", nextReview: "2026-06-16", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2011", name: "Disk Encryption", domain: "Endpoint Protection", status: "planned", effectiveness: 3, lastTest: "2026-03-03", nextReview: "2026-07-07", owner: "IT Ops", risk: "High" },
      { id: "CTL-2012", name: "Data Classification", domain: "Data Protection", status: "gap", effectiveness: 10, lastTest: "2026-02-16", nextReview: "2026-08-26", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2013", name: "PAM Implementation", domain: "Identity Management", status: "implemented", effectiveness: 46, lastTest: "2026-01-01", nextReview: "2026-09-17", owner: "IAM", risk: "Low" },
      { id: "CTL-2014", name: "DAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 24, lastTest: "2026-04-14", nextReview: "2026-10-08", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2015", name: "IAM Policy Review", domain: "Cloud Security", status: "planned", effectiveness: 31, lastTest: "2026-03-27", nextReview: "2026-11-27", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2016", name: "Visitor Management", domain: "Physical Security", status: "gap", effectiveness: 38, lastTest: "2026-02-12", nextReview: "2026-12-18", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2017", name: "Access Reviews", domain: "Access Control", status: "implemented", effectiveness: 58, lastTest: "2026-01-25", nextReview: "2026-05-09", owner: "SOC", risk: "Low" },
      { id: "CTL-2018", name: "IDS/IPS Tuning", domain: "Network Security", status: "partial", effectiveness: 1, lastTest: "2026-04-10", nextReview: "2026-06-28", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2019", name: "Patch Management", domain: "Endpoint Protection", status: "planned", effectiveness: 8, lastTest: "2026-03-23", nextReview: "2026-07-19", owner: "IT Ops", risk: "High" },
      { id: "CTL-2020", name: "Backup Encryption", domain: "Data Protection", status: "gap", effectiveness: 15, lastTest: "2026-02-08", nextReview: "2026-08-10", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2021", name: "Password Policy", domain: "Identity Management", status: "implemented", effectiveness: 70, lastTest: "2026-01-21", nextReview: "2026-09-01", owner: "IAM", risk: "Low" },
      { id: "CTL-2022", name: "Container Scanning", domain: "Application Security", status: "partial", effectiveness: 29, lastTest: "2026-04-06", nextReview: "2026-10-20", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2023", name: "WAF Configuration", domain: "Cloud Security", status: "planned", effectiveness: 36, lastTest: "2026-03-19", nextReview: "2026-11-11", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2024", name: "CCTV Coverage", domain: "Physical Security", status: "gap", effectiveness: 43, lastTest: "2026-02-04", nextReview: "2026-12-02", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2025", name: "RBAC Enforcement", domain: "Access Control", status: "implemented", effectiveness: 82, lastTest: "2026-01-17", nextReview: "2026-05-21", owner: "SOC", risk: "Low" },
      { id: "CTL-2026", name: "VPN Management", domain: "Network Security", status: "partial", effectiveness: 6, lastTest: "2026-04-02", nextReview: "2026-06-12", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2027", name: "App Whitelisting", domain: "Endpoint Protection", status: "planned", effectiveness: 13, lastTest: "2026-03-15", nextReview: "2026-07-03", owner: "IT Ops", risk: "High" },
      { id: "CTL-2028", name: "Key Management", domain: "Data Protection", status: "gap", effectiveness: 20, lastTest: "2026-02-28", nextReview: "2026-08-22", owner: "Data Gov", risk: "Critical" },
    ];
    const gapAnalysis = [
  ...
  private _renderIncidentCostTracker() {
    const incidents = [
      { id: "INC-7001", name: "Security Incident 1", severity: "Critical", totalCost: 312000, responseCost: 102960, recoveryCost: 118560, legalCost: 40560, regulatoryCost: 46800, insuranceClaim: 0, avoidedCost: 45000, date: "2026-01-05" },
      { id: "INC-7002", name: "Security Incident 2", severity: "High", totalCost: 315000, responseCost: 59850, recoveryCost: 88200, legalCost: 31500, regulatoryCost: 31500, insuranceClaim: 214200, avoidedCost: 74000, date: "2026-04-14" },
      { id: "INC-7003", name: "Security Incident 3", severity: "Medium", totalCost: 318000, responseCost: 82680, recoveryCost: 124020, legalCost: 22260, regulatoryCost: 15900, insuranceClaim: 0, avoidedCost: 103000, date: "2026-03-23" },
      { id: "INC-7004", name: "Security Incident 4", severity: "Low", totalCost: 321000, responseCost: 105930, recoveryCost: 93090, legalCost: 64200, regulatoryCost: 35310, insuranceClaim: 202230, avoidedCost: 132000, date: "2026-02-04" },
      { id: "INC-7005", name: "Security Incident 5", severity: "Critical", totalCost: 324000, responseCost: 61560, recoveryCost: 129600, legalCost: 55080, regulatoryCost: 19440, insuranceClaim: 0, avoidedCost: 161000, date: "2026-01-13" },
      { id: "INC-7006", name: "Security Incident 6", severity: "High", totalCost: 327000, responseCost: 85020, recoveryCost: 98100, legalCost: 45780, regulatoryCost: 39240, insuranceClaim: 189660, avoidedCost: 190000, date: "2026-04-22" },
      { id: "INC-7007", name: "Security Incident 7", severity: "Medium", totalCost: 330000, responseCost: 108900, recoveryCost: 66000, legalCost: 36300, regulatoryCost: 23100, insuranceClaim: 0, avoidedCost: 219000, date: "2026-03-03" },
      { id: "INC-7008", name: "Security Incident 8", severity: "Low", totalCost: 333000, responseCost: 63270, recoveryCost: 103230, legalCost: 26640, regulatoryCost: 43290, insuranceClaim: 176490, avoidedCost: 248000, date: "2026-02-12" },
      { id: "INC-7009", name: "Security Incident 9", severity: "Critical", totalCost: 336000, responseCost: 87360, recoveryCost: 70560, legalCost: 16800, regulatoryCost: 26880, insuranceClaim: 0, avoidedCost: 277000, date: "2026-01-21" },
      { id: "INC-7010", name: "Security Incident 10", severity: "High", totalCost: 339000, responseCost: 111870, recoveryCost: 108480, legalCost: 61020, regulatoryCost: 47460, insuranceClaim: 162720, avoidedCost: 306000, date: "2026-04-02" },
      { id: "INC-7011", name: "Security Incident 11", severity: "Medium", totalCost: 342000, responseCost: 64980, recoveryCost: 75240, legalCost: 51300, regulatoryCost: 30780, insuranceClaim: 0, avoidedCost: 335000, date: "2026-03-11" },
      { id: "INC-7012", name: "Security Incident 12", severity: "Low", totalCost: 345000, responseCost: 89700, recoveryCost: 113850, legalCost: 41400, regulatoryCost: 51750, insuranceClaim: 148350, avoidedCost: 364000, date: "2026-02-20" },
    ];
    const yearlyTrend = [
      { month: "Jan", incidents: 12, totalCost: "735k", avgCost: "150k", insured: 35 },
      { month: "Feb", incidents: 9, totalCost: "778k", avgCost: "197k", insured: 35 },
      { month: "Mar", incidents: 6, totalCost: "70k", avgCost: "63k", insured: 35 },
      { month: "Apr", incidents: 3, totalCost: "113k", avgCost: "110k", insured: 35 },
      { month: "May", incidents: 11, totalCost: "156k", avgCost: "157k", insured: 35 },
      { month: "Jun", incidents: 8, totalCost: "199k", avgCost: "23k", insured: 35 },
    ];
