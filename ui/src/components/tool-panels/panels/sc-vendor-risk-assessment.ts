...
`;
  ...
  @state() private _vendorRiskAsRiskTopTen: Array<{id:string;name:string;score:number;trend:string;owner:string;category:string;lastAssessed:string}> = [
  ...
  @state() private _vendorRiskAsRiskCategories: Array<{category:string;count:number;avgScore:number;color:string}> = [
  ...
  @state() private _vendorRiskAsRiskVelocity: Array<{week:string;newRisks:number;closedRisks:number;netChange:number}> = [
  ...
  @state() private _vendorRiskAsRiskAppetite: {current:number;threshold:number;maxTolerated:number;status:string} = {
    current: 68, threshold: 55, maxTolerated: 80, status: 'Warning'
  };
  @state() private _vendorRiskAsRiskOwnerMatrix: Array<{owner:string;criticalCount:number;highCount:number;mediumCount:number;complianceRate:number}> = [
  ...
  private _renderVendorriskasRiskDash(): TemplateResult {
    const riskItems = this._vendorRiskAsRiskTopTen;
    const velocity = this._vendorRiskAsRiskVelocity;
    const appetite = this._vendorRiskAsRiskAppetite;
    return html`
  ...
  @state() private _vendorRiskAsContainerScans: Array<{image:string;registry:string;critical:number;high:number;medium:number;scanDate:string;status:string}> = [
  ...
  @state() private _vendorRiskAsK8sPods: Array<{namespace:string;pod:string;securityContext:string;hostIPC:boolean;hostPID:boolean;privileged:boolean;riskLevel:string}> = [
  ...
  @state() private _vendorRiskAsServerlessRisk: Array<{function:string;runtime:string;timeout:number;iamPerms:string;externalCalls:number;riskScore:number}> = [
  ...
  @state() private _vendorRiskAsRuntimeAlerts: Array<{id:string;workload:string;alertType:string;severity:string;description:string;timestamp:string}> = [
  ...
  private _renderVendorriskasCloudWl(): TemplateResult {
    const containers = this._vendorRiskAsContainerScans;
    const pods = this._vendorRiskAsK8sPods;
    const alerts = this._vendorRiskAsRuntimeAlerts;
    return html`
  ...
  @state() private _vendorRiskAsCorrRules: Array<{id:string;name:string;sources:string[];logic:string;severity:string;active:boolean;lastTriggered:string}> = [
  ...
  @state() private _vendorRiskAsEventTimeline: Array<{timestamp:string;source:string;eventType:string;details:string;correlated:boolean}> = [
  ...
  @state() private _vendorRiskAsFalsePosMetrics: {totalEvents:number;correlatedEvents:number;falsePositives:number;fpRate:number;topFpRules:string[]} = {
  ...
  @state() private _vendorRiskAsEventPatterns: Array<{id:string;pattern:string;frequency:number;firstSeen:string;lastSeen:string;status:string}> = [
  ...
  private _renderVendorriskasEventCorr(): TemplateResult {
    const rules = this._vendorRiskAsCorrRules;
    const timeline = this._vendorRiskAsEventTimeline;
    const fpMetrics = this._vendorRiskAsFalsePosMetrics;
    return html`
  ...
  @state() private _vendorRiskAsCourses: Array<{id:string;title:string;category:string;duration:number;enrolled:number;completed:number;difficulty:string;rating:number}> = [
  ...
  @state() private _vendorRiskAsLearningPaths: Array<{id:string;name:string;courseIds:string[];progress:number;enrolled:number}> = [
  ...
  @state() private _vendorRiskAsDeptCompliance: Array<{dept:string;trainedPct:number;targetPct:number;avgScore:number;certCount:number}> = [
  ...
  @state() private _vendorRiskAsSkillsGaps: Array<{skill:string;current:number;required:number;gap:number;priority:string}> = [
  ...
  private _renderVendorriskasTraining(): TemplateResult {
    const courses = this._vendorRiskAsCourses;
    const deptComp = this._vendorRiskAsDeptCompliance;
    return html`
  ...
  @state() private _complianceCalendarEvents: Array<{id: string; title: string; deadline: string; category: string; status: string; assignee: string; notes: string}> = [
    {id: 'cc-vendor-risk-assessment-001', title: 'SOX Section 404 Compliance Review', deadline: '2026-06-30', category: 'Regulatory', status: 'In Progress', assignee: 'Compliance Team', notes: 'Annual internal control assessment for financial reporting'},
    {id: 'cc-vendor-risk-assessment-002', title: 'GDPR Data Protection Impact Assessment', deadline: '2026-05-15', category: 'Privacy', status: 'Pending', assignee: 'DPO Office', notes: 'Quarterly DPIA for high-risk processing activities'},
    {id: 'cc-vendor-risk-assessment-003', title: 'ISO 27001 Surveillance Audit', deadline: '2026-07-20', category: 'Certification', status: 'Scheduled', assignee: 'QA Lead', notes: 'Stage 2 surveillance audit for ISMS certification renewal'},
    {id: 'cc-vendor-risk-assessment-004', title: 'PCI DSS v4.0 Gap Analysis', deadline: '2026-05-30', category: 'Payment', status: 'Not Started', assignee: 'Security Architect', notes: 'Assess current state against PCI DSS v4.0 requirements'},
    {id: 'cc-vendor-risk-assessment-005', title: 'NIST CSF Self-Assessment', deadline: '2026-08-15', category: 'Framework', status: 'Pending', assignee: 'CISO', notes: 'Biannual self-assessment against NIST Cybersecurity Framework'},
    {id: 'cc-vendor-risk-assessment-006', title: 'HIPAA Security Rule Audit', deadline: '2026-06-15', category: 'Healthcare', status: 'In Progress', assignee: 'Compliance Analyst', notes: 'Annual audit of administrative, physical, and technical safeguards'},
    {id: 'cc-vendor-risk-assessment-007', title: 'SOC 2 Type II Report Renewal', deadline: '2026-09-30', category: 'Audit', status: 'Scheduled', assignee: 'External Auditor', notes: 'Engage auditor for SOC 2 Type II examination period'},
    {id: 'cc-vendor-risk-assessment-008', title: 'FedRAMP Authorization Review', deadline: '2026-07-01', category: 'Government', status: 'Pending', assignee: 'FedRAMP PMO', notes: 'Review and update security authorization package'},
    {id: 'cc-vendor-risk-assessment-009', title: 'Annual Penetration Test', deadline: '2026-10-15', category: 'Testing', status: 'Not Started', assignee: 'Red Team Lead', notes: 'Comprehensive external and internal penetration testing engagement'},
    {id: 'cc-vendor-risk-assessment-010', title: 'Security Awareness Training Rollout', deadline: '2026-05-01', category: 'Training', status: 'Completed', assignee: 'Security Awareness Manager', notes: 'Q2 organization-wide security awareness training program'},
    {id: 'cc-vendor-risk-assessment-011', title: 'Vendor Security Review Cycle', deadline: '2026-06-01', category: 'Third Party', status: 'In Progress', assignee: 'Vendor Manager', notes: 'Quarterly review of critical vendor security posture and SLA compliance'},
    {id: 'cc-vendor-risk-assessment-012', title: 'Incident Response Plan Update', deadline: '2026-05-20', category: 'Operations', status: 'Pending', assignee: 'IR Team Lead', notes: 'Update IR procedures based on lessons learned from recent incidents'},
  ];
  ...
  @state() private _dataQualityMetrics: Array<{id: string; dataSource: string; completenessScore: number; accuracyScore: number; timelinessScore: number; consistencyScore: number; reliabilityRank: number; lastUpdated: string; issues: string[]; trend: string}> = [
    {id: 'dq-vendor-risk-assessment-001', dataSource: 'Vulnerability Scanner Feed', completenessScore: 94, accuracyScore: 91, timelinessScore: 88, consistencyScore: 95, reliabilityRank: 1, lastUpdated: '2026-04-23T08:00:00Z', issues: ['Minor delay in Nessus plugin updates'], trend: 'improving'},
    {id: 'dq-vendor-risk-assessment-002', dataSource: 'SIEM Event Logs', completenessScore: 87, accuracyScore: 93, timelinessScore: 96, consistencyScore: 89, reliabilityRank: 2, lastUpdated: '2026-04-23T07:30:00Z', issues: ['Some log sources showing gaps in off-hours'], trend: 'stable'},
    {id: 'dq-vendor-risk-assessment-003', dataSource: 'Asset Inventory Database', completenessScore: 82, accuracyScore: 78, timelinessScore: 75, consistencyScore: 84, reliabilityRank: 4, lastUpdated: '2026-04-22T18:00:00Z', issues: ['Shadow IT devices not fully cataloged', 'Decommissioned assets still listed'], trend: 'declining'},
    {id: 'dq-vendor-risk-assessment-004', dataSource: 'Threat Intelligence Platform', completenessScore: 91, accuracyScore: 89, timelinessScore: 92, consistencyScore: 87, reliabilityRank: 3, lastUpdated: '2026-04-23T06:00:00Z', issues: ['Some IOC sources lack confidence scoring'], trend: 'improving'},
    {id: 'dq-vendor-risk-assessment-005', dataSource: 'Identity Provider Logs', completenessScore: 96, accuracyScore: 97, timelinessScore: 98, consistencyScore: 96, reliabilityRank: 1, lastUpdated: '2026-04-23T09:00:00Z', issues: [], trend: 'stable'},
    {id: 'dq-vendor-risk-assessment-006', dataSource: 'Cloud Security Posture Data', completenessScore: 79, accuracyScore: 85, timelinessScore: 82, consistencyScore: 81, reliabilityRank: 5, lastUpdated: '2026-04-23T05:00:00Z', issues: ['Multi-cloud coverage gaps', 'API rate limiting affects scan frequency'], trend: 'declining'},
    {id: 'dq-vendor-risk-assessment-007', dataSource: 'Compliance Evidence Store', completenessScore: 88, accuracyScore: 86, timelinessScore: 80, consistencyScore: 83, reliabilityRank: 4, lastUpdated: '2026-04-21T14:00:00Z', issues: ['Manual evidence collection delays', 'Inconsistent formatting across frameworks'], trend: 'stable'},
    {id: 'dq-vendor-risk-assessment-008', dataSource: 'Network Flow Data', completenessScore: 92, accuracyScore: 90, timelinessScore: 94, consistencyScore: 91, reliabilityRank: 2, lastUpdated: '2026-04-23T08:30:00Z', issues: ['Encrypted traffic classification accuracy needs improvement'], trend: 'improving'},
  ];
  ...
  @state() private _workflowTemplates: Array<{id: string; name: string; description: string; category: string; steps: number; avgDuration: string; successRate: number; lastRun: string; manualInterventions: number; optimizationScore: number}> = [
    {id: 'wf-vendor-risk-assessment-001', name: 'Vulnerability Triage Pipeline', description: 'Automated vulnerability assessment, prioritization, and assignment workflow', category: 'Vulnerability Management', steps: 8, avgDuration: '12 min', successRate: 94.5, lastRun: '2026-04-23T06:00:00Z', manualInterventions: 2, optimizationScore: 87},
    {id: 'wf-vendor-risk-assessment-002', name: 'Incident Response Orchestration', description: 'End-to-end IR workflow from detection to containment and recovery', category: 'Incident Response', steps: 15, avgDuration: '45 min', successRate: 89.2, lastRun: '2026-04-22T14:30:00Z', manualInterventions: 5, optimizationScore: 72},
    {id: 'wf-vendor-risk-assessment-003', name: 'Access Request Approval Chain', description: 'Multi-level approval workflow for privileged access requests', category: 'Identity & Access', steps: 6, avgDuration: '4 hours', successRate: 97.1, lastRun: '2026-04-23T09:15:00Z', manualInterventions: 1, optimizationScore: 92},
    {id: 'wf-vendor-risk-assessment-004', name: 'Compliance Evidence Collection', description: 'Automated gathering and packaging of audit evidence artifacts', category: 'Compliance', steps: 10, avgDuration: '30 min', successRate: 91.8, lastRun: '2026-04-21T10:00:00Z', manualInterventions: 3, optimizationScore: 78},
    {id: 'wf-vendor-risk-assessment-005', name: 'Security Patch Deployment', description: 'Staged patch rollout with validation and rollback capability', category: 'Patch Management', steps: 12, avgDuration: '2 hours', successRate: 96.3, lastRun: '2026-04-20T22:00:00Z', manualInterventions: 1, optimizationScore: 85},
    {id: 'wf-vendor-risk-assessment-006', name: 'Threat Intelligence Enrichment', description: 'IOC enrichment and correlation with internal threat data', category: 'Threat Intelligence', steps: 7, avgDuration: '8 min', successRate: 93.7, lastRun: '2026-04-23T07:00:00Z', manualInterventions: 0, optimizationScore: 95},
    {id: 'wf-vendor-risk-assessment-007', name: 'Security Reporting Pipeline', description: 'Automated generation and distribution of security metrics reports', category: 'Reporting', steps: 5, avgDuration: '15 min', successRate: 98.9, lastRun: '2026-04-23T08:00:00Z', manualInterventions: 0, optimizationScore: 97},
    {id: 'wf-vendor-risk-assessment-008', name: 'Vendor Risk Assessment', description: 'Automated vendor security questionnaire distribution and scoring', category: 'Third Party', steps: 9, avgDuration: '3 days', successRate: 85.4, lastRun: '2026-04-19T11:00:00Z', manualInterventions: 4, optimizationScore: 65},
  ];
  ...
  @state() private _workflowExecutionHistory: Array<{workflowId: string; runDate: string; duration: string; status: string; trigger: string}> = [
    {workflowId: 'wf-vendor-risk-assessment-001', runDate: '2026-04-23T06:00:00Z', duration: '11 min 23 sec', status: 'Success', trigger: 'Scheduled'},
    {workflowId: 'wf-vendor-risk-assessment-002', runDate: '2026-04-22T14:30:00Z', duration: '42 min 15 sec', status: 'Success', trigger: 'Alert'},
    {workflowId: 'wf-vendor-risk-assessment-003', runDate: '2026-04-23T09:15:00Z', duration: '3h 45 min', status: 'Success', trigger: 'User Request'},
    {workflowId: 'wf-vendor-risk-assessment-001', runDate: '2026-04-22T06:00:00Z', duration: '13 min 08 sec', status: 'Partial', trigger: 'Scheduled'},
    {workflowId: 'wf-vendor-risk-assessment-004', runDate: '2026-04-21T10:00:00Z', duration: '28 min 42 sec', status: 'Success', trigger: 'Scheduled'},
  ];
  ...
  @state() private _knowledgeArticles: Array<{id: string; title: string; category: string; author: string; publishDate: string; views: number; rating: number; tags: string[]; summary: string; readTime: string}> = [
    {id: 'kb-vendor-risk-assessment-001', title: 'Zero Trust Architecture Implementation Guide', category: 'Architecture', author: 'Security Architecture Team', publishDate: '2026-03-15', views: 1842, rating: 4.8, tags: ['zero-trust', 'network', 'identity'], summary: 'Comprehensive guide for implementing zero trust principles across enterprise infrastructure', readTime: '15 min'},
    {id: 'kb-vendor-risk-assessment-002', title: 'Ransomware Defense Playbook 2026', category: 'Threats', author: 'Threat Intelligence Team', publishDate: '2026-04-01', views: 2341, rating: 4.9, tags: ['ransomware', 'incident-response', 'defense'], summary: 'Updated playbook with latest ransomware tactics, techniques, and defensive measures', readTime: '12 min'},
    {id: 'kb-vendor-risk-assessment-003', title: 'Cloud Security Best Practices for Multi-Cloud', category: 'Cloud', author: 'Cloud Security Team', publishDate: '2026-02-28', views: 1567, rating: 4.5, tags: ['cloud', 'aws', 'azure', 'gcp'], summary: 'Best practices for securing workloads across multiple cloud service providers', readTime: '18 min'},
    {id: 'kb-vendor-risk-assessment-004', title: 'API Security Testing Methodology', category: 'Testing', author: 'Red Team', publishDate: '2026-03-22', views: 987, rating: 4.6, tags: ['api', 'testing', 'owasp'], summary: 'Structured methodology for identifying and testing API security vulnerabilities', readTime: '10 min'},
    {id: 'kb-vendor-risk-assessment-005', title: 'Supply Chain Security Risk Management', category: 'Third Party', author: 'GRC Team', publishDate: '2026-01-20', views: 1234, rating: 4.3, tags: ['supply-chain', 'vendor', 'risk'], summary: 'Framework for managing security risks in the software supply chain', readTime: '14 min'},
    {id: 'kb-vendor-risk-assessment-006', title: 'Kubernetes Security Hardening Checklist', category: 'Container', author: 'Platform Security', publishDate: '2026-04-10', views: 2103, rating: 4.7, tags: ['kubernetes', 'container', 'hardening'], summary: 'Step-by-step hardening checklist for production Kubernetes clusters', readTime: '11 min'},
    {id: 'kb-vendor-risk-assessment-007', title: 'Security Metrics That Matter for Executives', category: 'Metrics', author: 'CISO Office', publishDate: '2026-03-05', views: 876, rating: 4.4, tags: ['metrics', 'kpi', 'reporting'], summary: 'Key security metrics and KPIs that resonate with board-level stakeholders', readTime: '8 min'},
    {id: 'kb-vendor-risk-assessment-008', title: 'Phishing Simulation Campaign Design', category: 'Awareness', author: 'Security Awareness Team', publishDate: '2026-02-15', views: 1456, rating: 4.5, tags: ['phishing', 'simulation', 'training'], summary: 'Designing effective phishing simulations that drive real behavioral change', readTime: '9 min'},
    {id: 'kb-vendor-risk-assessment-009', title: 'Data Loss Prevention Strategy and Implementation', category: 'Data Protection', author: 'Data Security Team', publishDate: '2026-03-28', views: 1678, rating: 4.6, tags: ['dlp', 'data-protection', 'compliance'], summary: 'Strategic approach to DLP covering people, process, and technology layers', readTime: '16 min'},
    {id: 'kb-vendor-risk-assessment-010', title: 'Security Automation with SOAR Platforms', category: 'Automation', author: 'SOC Engineering', publishDate: '2026-04-05', views: 1098, rating: 4.7, tags: ['soar', 'automation', 'soc'], summary: 'Building effective security automation playbooks using SOAR technology', readTime: '13 min'},
    {id: 'kb-vendor-risk-assessment-011', title: 'Third-Party Risk Assessment Framework', category: 'GRC', author: 'Vendor Management', publishDate: '2026-01-30', views: 1345, rating: 4.4, tags: ['vendor', 'risk', 'assessment'], summary: 'Structured framework for evaluating and monitoring third-party security risks', readTime: '12 min'},
    {id: 'kb-vendor-risk-assessment-012', title: 'Network Detection and Response Best Practices', category: 'Network', author: 'Network Security Team', publishDate: '2026-03-18', views: 1567, rating: 4.5, tags: ['ndr', 'network', 'detection'], summary: 'Implementing effective network detection and response capabilities', readTime: '14 min'},
    {id: 'kb-vendor-risk-assessment-013', title: 'Security Chaos Engineering Guide', category: 'Resilience', author: 'SRE Security', publishDate: '2026-04-12', views: 789, rating: 4.3, tags: ['chaos-engineering', 'resilience', 'testing'], summary: 'Applying chaos engineering principles to validate security controls', readTime: '10 min'},
    {id: 'kb-vendor-risk-assessment-014', title: 'Incident Communication Templates', category: 'Incident Response', author: 'IR Team', publishDate: '2026-02-22', views: 2234, rating: 4.8, tags: ['incident', 'communication', 'templates'], summary: 'Ready-to-use communication templates for security incident scenarios', readTime: '7 min'},
    {id: 'kb-vendor-risk-assessment-015', title: 'Endpoint Detection and Response Configuration', category: 'Endpoint', author: 'Endpoint Security', publishDate: '2026-03-08', views: 1345, rating: 4.5, tags: ['edr', 'endpoint', 'configuration'], summary: 'Optimal EDR configuration for maximum detection with minimal false positives', readTime: '11 min'},
    {id: 'kb-vendor-risk-assessment-016', title: 'Security Code Review Standards', category: 'Development', author: 'AppSec Team', publishDate: '2026-02-10', views: 1890, rating: 4.6, tags: ['code-review', 'secure-sdlc', 'standards'], summary: 'Standards and checklists for security-focused code reviews', readTime: '13 min'},
    {id: 'kb-vendor-risk-assessment-017', title: 'IoT Security Assessment Methodology', category: 'IoT', author: 'IoT Security Lab', publishDate: '2026-04-08', views: 654, rating: 4.2, tags: ['iot', 'embedded', 'assessment'], summary: 'Methodology for assessing security posture of IoT devices and ecosystems', readTime: '15 min'},
    {id: 'kb-vendor-risk-assessment-018', title: 'Passwordless Authentication Migration Guide', category: 'Identity', author: 'IAM Team', publishDate: '2026-03-25', views: 1678, rating: 4.7, tags: ['passwordless', 'authentication', 'mfa'], summary: 'Step-by-step migration plan from password-based to passwordless authentication', readTime: '12 min'},
    {id: 'kb-vendor-risk-assessment-019', title: 'Security Awareness Program Metrics', category: 'Awareness', author: 'Security Culture Team', publishDate: '2026-01-25', views: 1123, rating: 4.4, tags: ['awareness', 'metrics', 'culture'], summary: 'Measuring the effectiveness of your security awareness and training programs', readTime: '9 min'},
    {id: 'kb-vendor-risk-assessment-020', title: 'Deception Technology Deployment Guide', category: 'Detection', author: 'Blue Team', publishDate: '2026-04-15', views: 876, rating: 4.5, tags: ['deception', 'honeypot', 'detection'], summary: 'Planning and deploying deception technology for advanced threat detection', readTime: '11 min'},
  ];
  ...
  // --- Security Scenario Planning Methods (VERIAS) ---
  private _initVeriasScenarios(): void {
    const worstCaseScenarios = [
      { id: "verias-sc-01", name: "Ransomware Attack on Critical Infrastructure", category: "Malware", impact: 9500000, probability: 0.12, recoveryDays: 21, affectedSystems: 47, dataLossGb: 320, businessImpact: "critical" },
      { id: "verias-sc-02", name: "Insider Data Exfiltration via Authorized Channels", category: "Insider Threat", impact: 7200000, probability: 0.18, recoveryDays: 14, affectedSystems: 12, dataLossGb: 85, businessImpact: "high" },
      { id: "verias-sc-03", name: "Supply Chain Compromise via Third-Party Library", category: "Supply Chain", impact: 6800000, probability: 0.09, recoveryDays: 30, affectedSystems: 63, dataLossGb: 0, businessImpact: "critical" },
      { id: "verias-sc-04", name: "Cloud Misconfiguration Leading to Public Exposure", category: "Cloud", impact: 4100000, probability: 0.22, recoveryDays: 7, affectedSystems: 8, dataLossGb: 450, businessImpact: "high" },
      { id: "verias-sc-05", name: "Zero-Day Exploit in Core Application Framework", category: "Vulnerability", impact: 8900000, probability: 0.05, recoveryDays: 45, affectedSystems: 120, dataLossGb: 0, businessImpact: "critical" },
      { id: "verias-sc-06", name: "Distributed Denial of Service on Customer-Facing Services", category: "Availability", impact: 3400000, probability: 0.25, recoveryDays: 3, affectedSystems: 15, dataLossGb: 0, businessImpact: "medium" },
      { id: "verias-sc-07", name: "Credential Stuffing and Account Takeover Wave", category: "Identity", impact: 2800000, probability: 0.30, recoveryDays: 5, affectedSystems: 6, dataLossGb: 12, businessImpact: "medium" },
      { id: "verias-sc-08", name: "Physical Security Breach at Data Center Facility", category: "Physical", impact: 12000000, probability: 0.02, recoveryDays: 60, affectedSystems: 200, dataLossGb: 5000, businessImpact: "critical" },
    ];
    this._veriasScenarios = worstCaseScenarios.map(s => ({ ...s, mitigationCost: Math.round(s.impact * 0.08), drillScheduled: false, lastDrillDate: null, drillScore: null, controlGaps: Math.floor(Math.random() * 5) + 1 }));
  }

  private _calculateVeriasScenarioRisk(): void {
    this._veriasScenarios.forEach(s => {
  ...
  private _scheduleVeriasDrills(): void {
  ...
  private _veriasScenarioComparison(): Record<string, unknown>[] {
    return this._veriasScenarios.map(s => ({
  ...
  // --- Security Control Effectiveness Analytics (VERIAS) ---
  private _initVeriasControls(): void {
    const controls = [
      { id: "verias-ct-01", name: "Network Segmentation", type: "preventive", maturity: "defined", score: 82, target: 90, failures: 3, lastTest: "2026-03-15" },
      { id: "verias-ct-02", name: "Endpoint Detection and Response", type: "detective", maturity: "managed", score: 88, target: 92, failures: 1, lastTest: "2026-04-01" },
      { id: "verias-ct-03", name: "Data Loss Prevention", type: "preventive", maturity: "repeatable", score: 71, target: 85, failures: 7, lastTest: "2026-03-22" },
      { id: "verias-ct-04", name: "Security Information and Event Management", type: "detective", maturity: "managed", score: 85, target: 90, failures: 2, lastTest: "2026-04-05" },
      { id: "verias-ct-05", name: "Identity and Access Management", type: "preventive", maturity: "defined", score: 76, target: 88, failures: 5, lastTest: "2026-03-28" },
      { id: "verias-ct-06", name: "Vulnerability Management", type: "corrective", maturity: "managed", score: 80, target: 90, failures: 4, lastTest: "2026-04-02" },
      { id: "verias-ct-07", name: "Email Security Gateway", type: "preventive", maturity: "managed", score: 90, target: 95, failures: 1, lastTest: "2026-04-08" },
      { id: "verias-ct-08", name: "Web Application Firewall", type: "preventive", maturity: "managed", score: 87, target: 92, failures: 2, lastTest: "2026-03-30" },
      { id: "verias-ct-09", name: "Patch Management", type: "corrective", maturity: "defined", score: 68, target: 85, failures: 9, lastTest: "2026-03-18" },
      { id: "verias-ct-10", name: "Encryption at Rest", type: "preventive", maturity: "managed", score: 93, target: 95, failures: 0, lastTest: "2026-04-10" },
      { id: "verias-ct-11", name: "Encryption in Transit", type: "preventive", maturity: "optimized", score: 96, target: 98, failures: 0, lastTest: "2026-04-12" },
      { id: "verias-ct-12", name: "Privileged Access Management", type: "preventive", maturity: "defined", score: 74, target: 88, failures: 6, lastTest: "2026-03-25" },
      { id: "verias-ct-13", name: "Security Awareness Training", type: "preventive", maturity: "repeatable", score: 65, target: 80, failures: 11, lastTest: "2026-03-20" },
      { id: "verias-ct-14", name: "Incident Response Plan", type: "corrective", maturity: "managed", score: 78, target: 88, failures: 4, lastTest: "2026-04-03" },
      { id: "verias-ct-15", name: "Backup and Recovery", type: "corrective", maturity: "managed", score: 84, target: 92, failures: 3, lastTest: "2026-04-06" },
      { id: "verias-ct-16", name: "Multi-Factor Authentication", type: "preventive", maturity: "managed", score: 91, target: 95, failures: 1, lastTest: "2026-04-09" },
      { id: "verias-ct-17", name: "Network Traffic Analysis", type: "detective", maturity: "defined", score: 72, target: 85, failures: 5, lastTest: "2026-03-27" },
      { id: "verias-ct-18", name: "Cloud Security Posture Management", type: "detective", maturity: "repeatable", score: 69, target: 82, failures: 8, lastTest: "2026-03-24" },
      { id: "verias-ct-19", name: "Container Security Scanning", type: "preventive", maturity: "defined", score: 75, target: 85, failures: 5, lastTest: "2026-04-04" },
      { id: "verias-ct-20", name: "API Security Gateway", type: "preventive", maturity: "repeatable", score: 70, target: 82, failures: 7, lastTest: "2026-03-29" },
      { id: "verias-ct-21", name: "Threat Intelligence Platform", type: "detective", maturity: "managed", score: 83, target: 90, failures: 2, lastTest: "2026-04-07" },
      { id: "verias-ct-22", name: "Database Activity Monitoring", type: "detective", maturity: "defined", score: 67, target: 80, failures: 8, lastTest: "2026-03-21" },
      { id: "verias-ct-23", name: "Deception Technology", type: "detective", maturity: "initial", score: 55, target: 75, failures: 12, lastTest: "2026-03-16" },
      { id: "verias-ct-24", name: "Security Orchestration Automation", type: "corrective", maturity: "repeatable", score: 73, target: 85, failures: 6, lastTest: "2026-04-01" },
      { id: "verias-ct-25", name: "Third-Party Risk Assessment", type: "preventive", maturity: "defined", score: 64, target: 78, failures: 9, lastTest: "2026-03-19" },
    ];
    this._veriasControls = controls;
  }

  private _analyzeVeriasControlEffectiveness(): void {
    const typeBreakdown: Record<string, { total: number; avgScore: number; avgFailures: number }> = {};
    this._veriasControls.forEach(c => {
    ...
    this._veriasControlTypeBreakdown = typeBreakdown;
  }

  private _veriasControlOptimization(): Record<string, unknown>[] {
    return this._veriasControls.filter(c => c.score < c.target).map(c => ({
  ...
  // --- Security Data Pipeline Health (VERIAS) ---
  private _initVeriasPipelines(): void {
    const pipelines = [
      { id: "verias-pl-01", name: "SIEM Log Ingestion", status: "healthy", healthScore: 94, latencyMs: 120, freshnessMin: 2, errorRate: 0.02, throughputMbps: 450, recordsPerSec: 12000, backPressure: 0.05, uptime: 99.97 },
      { id: "verias-pl-02", name: "Threat Feed Aggregation", status: "healthy", healthScore: 91, latencyMs: 300, freshnessMin: 15, errorRate: 0.05, throughputMbps: 85, recordsPerSec: 3200, backPressure: 0.08, uptime: 99.92 },
      { id: "verias-pl-03", name: "Vulnerability Scan Results", status: "degraded", healthScore: 72, latencyMs: 2500, freshnessMin: 60, errorRate: 0.15, throughputMbps: 25, recordsPerSec: 800, backPressure: 0.35, uptime: 98.50 },
      { id: "verias-pl-04", name: "Endpoint Telemetry Stream", status: "healthy", healthScore: 88, latencyMs: 450, freshnessMin: 5, errorRate: 0.08, throughputMbps: 280, recordsPerSec: 8500, backPressure: 0.12, uptime: 99.85 },
      { id: "verias-pl-05", name: "Cloud Audit Log Pipeline", status: "healthy", healthScore: 86, latencyMs: 600, freshnessMin: 10, errorRate: 0.06, throughputMbps: 150, recordsPerSec: 4500, backPressure: 0.10, uptime: 99.80 },
      { id: "verias-pl-06", name: "Identity Event Stream", status: "warning", healthScore: 78, latencyMs: 1200, freshnessMin: 20, errorRate: 0.12, throughputMbps: 45, recordsPerSec: 1500, backPressure: 0.22, uptime: 99.40 },
      { id: "verias-pl-07", name: "Network Flow Collection", status: "healthy", healthScore: 82, latencyMs: 800, freshnessMin: 8, errorRate: 0.09, throughputMbps: 650, recordsPerSec: 18000, backPressure: 0.15, uptime: 99.70 },
      { id: "verias-pl-08", name: "DLP Incident Pipeline", status: "degraded", healthScore: 68, latencyMs: 3500, freshnessMin: 45, errorRate: 0.20, throughputMbps: 18, recordsPerSec: 500, backPressure: 0.42, uptime: 97.80 },
    ];
    this._veriasPipelines = pipelines.map(p => ({ ...p, dependencies: [], slaBreached: p.healthScore < 75, alertThreshold: p.errorRate > 0.15 }));
    this._veriasPipelines[0].dependencies = ["verias-pl-04", "verias-pl-07"];
    this._veriasPipelines[4].dependencies = ["verias-pl-01"];
    this._veriasPipelines[5].dependencies = ["verias-pl-02"];
    this._veriasPipelines[7].dependencies = ["verias-pl-04", "verias-pl-05"];
  }

  private _veriasPipelineTrend(): Record<string, unknown>[] {
    const hours = Array.from({ length: 24 }, (_, i) => `H${String(i).padStart(2, "0")}`);
    return this._veriasPipelines.map(p => ({
  ...
  // --- Security Stakeholder Report Generator (VERIAS) ---
  private _initVeriasReportTemplates(): void {
    this._veriasReportTemplates = [
      { id: "verias-rp-01", name: "Board Security Report", audience: "board", frequency: "quarterly", sections: 6, autoGenerate: true, lastGenerated: "2026-03-31", pages: 12 },
      { id: "verias-rp-02", name: "Executive Risk Summary", audience: "executive", frequency: "monthly", sections: 8, autoGenerate: true, lastGenerated: "2026-04-01", pages: 8 },
      { id: "verias-rp-03", name: "Technical Security Review", audience: "technical", frequency: "weekly", sections: 12, autoGenerate: true, lastGenerated: "2026-04-21", pages: 25 },
      { id: "verias-rp-04", name: "Audit Compliance Report", audience: "audit", frequency: "quarterly", sections: 10, autoGenerate: false, lastGenerated: "2026-03-15", pages: 35 },
      { id: "verias-rp-05", name: "Regulatory Filing Package", audience: "regulatory", frequency: "annual", sections: 15, autoGenerate: false, lastGenerated: "2025-12-31", pages: 48 },
    ];
  }

  private _generateVeriasExecSummary(): string {
    const totalRisk = this._veriasScenarios.reduce((sum: number, s: any) => sum + s.riskScore, 0);
    const avgControl = Math.round(this._veriasControls.reduce((sum: number, c: any) => sum + c.score, 0) / this._veriasControls.length);
    const degradedPipes = this._veriasPipelines.filter((p: any) => p.status !== "healthy").length;
    return `Overall risk exposure: ${(totalRisk / 1000000).toFixed(1)}M. Average control effectiveness: {avgControl}%. {degradedPipes} pipeline(s) need attention. {this._veriasScenarios.filter((s: any) => s.mitigationPriority === "immediate").length} scenarios require immediate action.`;
  }

  // --- Security Technology Radar (VERIAS) ---
  private _initVeriasTechRadar(): void {
    this._veriasTechRadar = [
      { id: "verias-tr-01", name: "AI-Powered Threat Detection", ring: "adopt", category: "Detection", maturity: 4, trend: "up", investment: "high", roi: 3.2, vendorCount: 8 },
      { id: "verias-tr-02", name: "Zero Trust Architecture", ring: "adopt", category: "Architecture", maturity: 4, trend: "up", investment: "high", roi: 2.8, vendorCount: 12 },
      { id: "verias-tr-03", name: "SASE/SSE Platform", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.4, vendorCount: 10 },
      { id: "verias-tr-04", name: "CNAPP Cloud Security", ring: "trial", category: "Cloud", maturity: 3, trend: "up", investment: "medium", roi: 2.1, vendorCount: 7 },
      { id: "verias-tr-05", name: "Extended Detection and Response", ring: "adopt", category: "Detection", maturity: 4, trend: "stable", investment: "high", roi: 2.9, vendorCount: 9 },
      { id: "verias-tr-06", name: "Quantum-Safe Cryptography", ring: "assess", category: "Crypto", maturity: 2, trend: "up", investment: "low", roi: 0.8, vendorCount: 4 },
      { id: "verias-tr-07", name: "Security Service Edge", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.0, vendorCount: 6 },
      { id: "verias-tr-08", name: "SOAR 2.0 Autonomous SOC", ring: "assess", category: "Operations", maturity: 2, trend: "up", investment: "low", roi: 1.2, vendorCount: 5 },
      { id: "verias-tr-09", name: "Software Supply Chain Security", ring: "trial", category: "DevSecOps", maturity: 3, trend: "up", investment: "medium", roi: 2.3, vendorCount: 8 },
      { id: "verias-tr-10", name: "Identity Threat Detection", ring: "adopt", category: "Identity", maturity: 3, trend: "up", investment: "high", roi: 2.7, vendorCount: 7 },
      { id: "verias-tr-11", name: "Data Security Posture Management", ring: "trial", category: "Data", maturity: 3, trend: "up", investment: "medium", roi: 2.2, vendorCount: 9 },
      { id: "verias-tr-12", name: "Decentralized Identity Standards", ring: "hold", category: "Identity", maturity: 1, trend: "stable", investment: "low", roi: 0.5, vendorCount: 3 },
    ];
  }

  private _veriasRadarSummary(): Record<string, unknown> {
    const rings: Record<string, number> = { adopt: 0, trial: 0, assess: 0, hold: 0 };
    this._veriasTechRadar.forEach(t => { if (rings[t.ring as keyof typeof rings] !== undefined) rings[t.ring as keyof typeof rings]++; });
    return { adopt: rings.adopt, trial: rings.trial, assess: rings.assess, hold: rings.hold, total: this._veriasTechRadar.length, avgMaturity: 2.8, topInvestment: this._veriasTechRadar.filter(t => t.investment === "high").map(t => t.name) };
  }

  // --- VERIAS Security Compliance Mapping Matrix ---
  private _initVeriasComplianceMatrix(): void {
    ...
    this._veriasComplianceMatrix = frameworks.map(fw => ({
  ...
  private _veriasComplianceTrend(): Record<string, unknown>[] {
    const quarters = ["Q1-2025", "Q2-2025", "Q3-2025", "Q4-2025", "Q1-2026", "Q2-2026"];
    return this._veriasComplianceMatrix.map(fw => ({
  ...
  // --- VERIAS Threat Intelligence Correlation Engine ---
  private _initVeriasThreatIntel(): void {
    ...
    this._veriasThreatIntel = threatActors.map((actor, i) => ({
  ...
  private _veriasThreatCorrelation(): Record<string, unknown>[] {
    return this._veriasThreatIntel.filter(t => t.riskScore > 70).map(t => ({
  ...
  // --- VERIAS Security Metrics Deep Dive ---
  private _initVeriasMetricsDeep(): void {
    ...
    this._veriasMetricsDeep = metricCategories.map(cat => ({
  ...
  private _veriasMetricsHeatmap(): number[][] {
    return this._veriasMetricsDeep.map(cat =>
      cat.metrics.map(m => Math.round((m.value / m.target) * 100))
    );
  }

  // --- VERIAS Incident Cost Modeling ---
  private _initVeriasCostModel(): void {
    const incidentTypes = ["Data Breach", "Ransomware", "DDoS", "Insider Threat", "Phishing", "Supply Chain", "Cloud Misconfig", "Zero-Day"];
    this._veriasCostModel = incidentTypes.map(inc => ({
    ...
    this._veriasCostModel.forEach(m => { m.totalAnnualExposure = m.avgCost * m.frequency; });
  }

  private _veriasCostProjection(): Record<string, unknown>[] {
    return this._veriasCostModel.map(m => ({
  ...
  // --- VERIAS Security Architecture Pattern Library ---
  private _initVeriasArchPatterns(): void {
    ...
    this._veriasArchPatterns = patterns.map((p, i) => ({
  ...
  private _veriasArchPatternAnalysis(): Record<string, unknown> {
    const implemented = this._veriasArchPatterns.filter(p => p.implementation > 75).length;
    const avgCoverage = Math.round(this._veriasArchPatterns.reduce((s: number, p: any) => s + p.coverage, 0) / this._veriasArchPatterns.length);
    return {
      totalPatterns: this._veriasArchPatterns.length, fullyImplemented: implemented, avgCoverage,
      avgRiskReduction: Math.round(this._veriasArchPatterns.reduce((s: number, p: any) => s + p.riskReduction, 0) / this._veriasArchPatterns.length),
      gaps: this._veriasArchPatterns.filter(p => p.implementation < 60).map(p => p.pattern),
      recommendations: this._veriasArchPatterns.filter(p => p.implementation < 70).map(p => ({ pattern: p.pattern, current: p.implementation, target: 85, effort: p.costComplexity }))
    };
  }
  ...
  private _a12RiskAssessmentWorkflow = [
  ...
  private _a12TreatmentOptions = [
    {id: 'a12-T-001', riskId: 'RSK-042', risk: 'Ransomware Attack on Critical Infrastructure', before: 20, after: 6, strategy: 'Mitigate', controls: ['Next-gen EDR with behavioral detection and automated response', 'Zero-trust network micro-segmentation isolating critical assets', 'Immutable offline backup rotation with quarterly recovery testing', 'Incident response retainer with 1-hour response SLA'], cost: '$450K/yr', roi: '78% reduction vs $2.8M potential loss', timeline: 'Q3 2026', status: 'approved' as const},
  ...
  private _a12TalentPositions = [
    ...
    {id: 'a12-H-002', title: 'Cloud Security Engineer', dept: 'Cloud Security', level: 'Mid-Senior', loc: 'SF/Remote', salary: '$155K-$180K', posted: '2026-03-20', applicants: 45, pipeline: 12, interviews: 6, offers: 2, urgency: 'high' as const, manager: 'Mike Johnson', skills: ['AWS/Azure/GCP', 'Terraform', 'Kubernetes', 'IAM', 'CSPM'], nice: ['CCSP', 'CKS', 'Serverless']},
    {id: 'a12-H-003', title: 'SOC Analyst L2', dept: 'Security Operations', level: 'Mid', loc: 'Austin TX', salary: '$105K-$130K', posted: '2026-04-01', applicants: 67, pipeline: 18, interviews: 8, offers: 0, urgency: 'medium' as const, manager: 'Lisa Park', skills: ['SIEM', 'Incident triage', 'Malware analysis', 'TCP/IP'], nice: ['GCIA', 'SOAR', 'Kill chain']},
    ...
    {id: 'a12-H-005', title: 'GRC Analyst', dept: 'GRC', level: 'Mid', loc: 'NY/Remote', salary: '$110K-$135K', posted: '2026-04-05', applicants: 34, pipeline: 9, interviews: 3, offers: 1, urgency: 'medium' as const, manager: 'Rachel Green', skills: ['ISO 27001', 'SOC 2', 'Risk assessment', 'Audit'], nice: ['CRISC', 'CISA', 'GDPR']},
    {id: 'a12-H-006', title: 'Threat Intel Analyst', dept: 'Threat Intel', level: 'Mid-Senior', loc: 'Remote US', salary: '$140K-$170K', posted: '2026-03-25', applicants: 19, pipeline: 5, interviews: 2, offers: 0, urgency: 'low' as const, manager: 'James Wilson', skills: ['MITRE ATT&CK', 'OSINT', 'Threat modeling', 'Reverse eng'], nice: ['CTIA', 'Nation-state', 'Dark web']},
    {id: 'a12-H-007', title: 'AppSec Engineer', dept: 'AppSec', level: 'Senior', loc: 'Seattle/Remote', salary: '$160K-$190K', posted: '2026-04-10', applicants: 28, pipeline: 7, interviews: 3, offers: 0, urgency: 'medium' as const, manager: 'Emily Zhang', skills: ['SAST/DAST/SCA', 'Secure SDLC', 'Code review', 'CI/CD'], nice: ['CSSLP', 'Mobile security']},
    {id: 'a12-H-008', title: 'IAM Engineer', dept: 'IAM', level: 'Mid', loc: 'Remote US', salary: '$135K-$160K', posted: '2026-04-08', applicants: 31, pipeline: 10, interviews: 5, offers: 1, urgency: 'medium' as const, manager: 'Chris Martinez', skills: ['Okta/Azure AD', 'SAML/OIDC', 'PAM', 'RBAC'], nice: ['CISSP', 'Zero trust IAM']}
  ];

  private _a12InterviewScorecard = [
  ...
  private _a12OnboardingSteps = [
    {id: 'a12-OB-01', item: 'Provision security-hardened laptop, monitors, peripherals, and configured workstation image with all required security tools', cat: 'IT Setup', day: 'Day 1', owner: 'IT Operations'},
    {id: 'a12-OB-02', item: 'Create AD account, email, Slack, VPN, SSO enrollment, and all required application access accounts', cat: 'Access', day: 'Day 1', owner: 'IAM Team'},
    {id: 'a12-OB-03', item: 'Grant security tooling access: SIEM console, EDR dashboard, vuln scanner, pentest platforms, SOAR', cat: 'Tools', day: 'Day 1-2', owner: 'SOC Manager'},
    {id: 'a12-OB-04', item: 'Complete MFA enrollment, security awareness onboarding course, and acceptable use policy acknowledgment', cat: 'Security', day: 'Day 1', owner: 'Awareness Team'},
    {id: 'a12-OB-05', item: 'Sign NDA, employment agreement, IP assignment, and code of conduct documents with HR', cat: 'HR', day: 'Day 1', owner: 'Human Resources'},
    {id: 'a12-OB-06', item: 'Introduction meetings with direct team members and key cross-functional stakeholders', cat: 'Integration', day: 'Week 1', owner: 'Hiring Manager'},
    {id: 'a12-OB-07', item: 'Security architecture overview, tooling walkthrough, and operational runbook review sessions', cat: 'Training', day: 'Week 1-2', owner: 'Security Architect'},
    {id: 'a12-OB-08', item: 'Shadow experienced team member on active incident response and threat hunting investigations', cat: 'Mentoring', day: 'Week 2-3', owner: 'SOC Lead'},
    {id: 'a12-OB-09', item: 'Set up individual certification study plan aligned with role requirements and career goals', cat: 'Development', day: 'Week 2', owner: 'Hiring Manager'},
    {id: 'a12-OB-10', item: 'Establish 30-60-90 day goals, define success metrics, and schedule first performance checkpoint', cat: 'Goals', day: 'Week 1', owner: 'Hiring Manager'}
  ];

  private _a12MetricsDashboard = [
    {id: 'a12-MTD-001', name: 'Mean Time to Detect (MTTD)', value: '4.2 hours', trend: 'improving' as const, target: '< 4 hours', source: 'SIEM - Splunk', method: 'Median time from first alert to analyst acknowledgment', owner: 'SOC Manager', sla: '4 hours', prev: '5.1 hours', spark: [5.8, 5.4, 5.1, 4.9, 4.7, 4.5, 4.3, 4.2], breakdown: [{src: 'Network IDS', pct: '35%', avg: '3.8h'}, {src: 'Endpoint EDR', pct: '28%', avg: '4.1h'}, {src: 'Email Gateway', pct: '22%', avg: '3.5h'}, {src: 'Cloud Security', pct: '10%', avg: '5.2h'}, {src: 'User Reports', pct: '5%', avg: '6.8h'}]},
    {id: 'a12-MTD-002', name: 'Mean Time to Respond (MTTR)', value: '2.8 hours', trend: 'stable' as const, target: '< 3 hours', source: 'SOAR - XSOAR', method: 'Median time from acknowledgment to first containment action', owner: 'IR Lead', sla: '3 hours', prev: '2.7 hours', spark: [3.2, 3.0, 2.9, 2.8, 2.8, 2.9, 2.8, 2.8], breakdown: [{src: 'Auto Playbooks', pct: '42%', avg: '1.2h'}, {src: 'Semi-Auto', pct: '33%', avg: '2.5h'}, {src: 'Manual', pct: '20%', avg: '5.1h'}, {src: 'L3 Escalation', pct: '5%', avg: '8.3h'}]},
    {id: 'a12-MTD-003', name: 'Patch Compliance Rate', value: '97.6%', trend: 'improving' as const, target: '> 95%', source: 'Tenable.io', method: 'Pct of critical/high vulns patched within SLA windows', owner: 'Vuln Manager', sla: '95%', prev: '96.1%', spark: [93.2, 94.1, 94.8, 95.3, 95.9, 96.4, 97.0, 97.6], breakdown: [{src: 'Critical (7d)', pct: '40%', rate: '99.2%'}, {src: 'High (30d)', pct: '35%', rate: '97.8%'}, {src: 'Medium (90d)', pct: '20%', rate: '95.1%'}, {src: 'Low (180d)', pct: '5%', rate: '89.4%'}]},
    {id: 'a12-MTD-004', name: 'Phishing Click Rate', value: '3.2%', trend: 'improving' as const, target: '< 5%', source: 'KnowBe4 + Proofpoint', method: 'Pct of users clicking simulated phishing links monthly', owner: 'Awareness Lead', sla: '5%', prev: '4.1%', spark: [6.8, 6.2, 5.5, 5.1, 4.6, 4.1, 3.6, 3.2], breakdown: [{src: 'Spear Phishing', pct: '35%', rate: '5.1%'}, {src: 'Generic Phishing', pct: '30%', rate: '2.8%'}, {src: 'BEC Sim', pct: '20%', rate: '3.5%'}, {src: 'SMS Phishing', pct: '15%', rate: '1.9%'}]},
    {id: 'a12-MTD-005', name: 'Security Posture Score', value: '784', trend: 'improving' as const, target: '> 750', source: 'SecurityScorecard', method: 'Composite: external 30%, compliance 25%, vuln 25%, config 20%', owner: 'CISO', sla: '> 750', prev: '771', spark: [748, 752, 758, 762, 768, 773, 778, 784], breakdown: [{src: 'External Rating', pct: '30%', score: '791'}, {src: 'Compliance', pct: '25%', score: '812'}, {src: 'Vuln Mgmt', pct: '25%', score: '745'}, {src: 'Config', pct: '20%', score: '769'}]},
    {id: 'a12-MTD-006', name: 'Vulnerability Backlog', value: '142', trend: 'improving' as const, target: '< 150', source: 'Tenable + Qualys', method: 'Total open vulns across all scanned assets tracked weekly', owner: 'Vuln Manager', sla: '< 150', prev: '168', spark: [210, 198, 185, 175, 165, 158, 150, 142], breakdown: [{src: 'Critical', pct: '8%', count: '11'}, {src: 'High', pct: '22%', count: '31'}, {src: 'Medium', pct: '45%', count: '64'}, {src: 'Low', pct: '25%', count: '36'}]}
  ];

  private _a12ArchReview = [
    {id: 'a12-AR-001', cat: 'Network', item: 'Network segmentation between trust zones with dedicated firewalls and granular ACLs', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Micro-segmentation via NSX across all production zones'},
    {id: 'a12-AR-002', cat: 'Network', item: 'DMZ architecture isolating public services from internal network', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Dual-firewall DMZ with WAF and DDoS mitigation'},
    {id: 'a12-AR-003', cat: 'Network', item: 'Least-privilege traffic flows with deny-by-default and quarterly review', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Explicit allow rules with automated drift detection'},
    {id: 'a12-AR-004', cat: 'Network', item: 'Encrypted external communications with IPSec/TLS 1.3 and PFS', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'IKEv2 with PFS, TLS 1.3 for all external APIs'},
    {id: 'a12-AR-005', cat: 'Identity', item: 'Centralized IdP with SSO and automated SCIM provisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Okta SSO SAML 2.0 for 247 apps with SCIM'},
    {id: 'a12-AR-006', cat: 'Identity', item: 'RBAC with quarterly reviews and automated deprovisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Quarterly recertification with JIT admin access'},
    {id: 'a12-AR-007', cat: 'Identity', item: 'PAM controlling all admin and service accounts with session recording', sev: 'high' as const, status: 'fail' as const, reviewer: 'IAM Lead', notes: '3 legacy accounts pending CyberArk enrollment'},
    {id: 'a12-AR-008', cat: 'Identity', item: 'MFA enforced for all users, hardware tokens for privileged access', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'FIDO2 for admins, push MFA for users, 99.8% enrollment'},
    {id: 'a12-AR-009', cat: 'Data', item: 'Data classification enforced with automated labeling on all repos', sev: 'high' as const, status: 'partial' as const, reviewer: 'Data Protection Lead', notes: 'Framework deployed, gaps on 4 legacy file shares'},
    {id: 'a12-AR-010', cat: 'Data', item: 'AES-256 encryption at rest for all sensitive data stores', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'Full-disk, TDE on DBs, SSE-KMS on cloud storage'},
    {id: 'a12-AR-011', cat: 'Data', item: 'DLP monitoring and preventing unauthorized data exfiltration', sev: 'high' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'DLP active on email, endpoint, cloud, and network'},
    {id: 'a12-AR-012', cat: 'Data', item: 'Encrypted off-site backups with quarterly restore testing', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Infrastructure Lead', notes: '3-2-1 strategy with immutable backups'},
    {id: 'a12-AR-013', cat: 'AppSec', item: 'Secure SDLC with SAST, DAST, SCA in all CI/CD pipelines', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: '34 pipelines with security gates and PR blocking'},
    {id: 'a12-AR-014', cat: 'AppSec', item: 'API gateway with auth, rate limiting, schema validation', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: 'Kong with OAuth2, rate limits, OpenAPI validation'},
    {id: 'a12-AR-015', cat: 'AppSec', item: 'Container security with image scanning and runtime protection', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Trivy scanning, Falco runtime in 80% of clusters'},
    {id: 'a12-AR-016', cat: 'Monitoring', item: 'SIEM collecting from all critical systems with correlation rules', sev: 'critical' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'Splunk 98% coverage with 1200+ correlation rules'},
    {id: 'a12-AR-017', cat: 'Monitoring', item: 'EDR with behavioral detection on all managed endpoints', sev: 'high' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'CrowdStrike on 99.2% endpoints with 24/7 MDR'},
    {id: 'a12-AR-018', cat: 'Monitoring', item: 'Executive dashboards with real-time posture visibility', sev: 'medium' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'PowerBI dashboards with weekly exec summaries'},
    {id: 'a12-AR-019', cat: 'Cloud', item: 'CSPM monitoring and remediating cloud misconfigurations', sev: 'high' as const, status: 'pass' as const, reviewer: 'Cloud Security Lead', notes: 'Prisma Cloud 450+ checks with auto-remediation'},
    {id: 'a12-AR-020', cat: 'Cloud', item: 'IaC templates scanned before deployment to production', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Checkov for Terraform, cfn-nag for CloudFormation pending'}
  ];

  private _geta12RiskProgress(): number {
    return Math.round(this._a12RiskAssessmentWorkflow.filter(s => s.status === 'completed').length / this._a12RiskAssessmentWorkflow.length * 100);
  }

  private _geta12TreatmentSummary(): {total: number; approved: number; avgReduction: number; investment: string} {
    const a = this._a12TreatmentOptions.filter(r => r.status === 'approved').length;
    const avg = this._a12TreatmentOptions.reduce((s, r) => s + ((r.before - r.after) / r.before * 100), 0) / this._a12TreatmentOptions.length;
    return {total: this._a12TreatmentOptions.length, approved: a, avgReduction: Math.round(avg), investment: '$1.125M annually'};
  }

  private _geta12RuleSummary(): {total: number; passing: number; failing: number; avgRate: number; violations: number} {
    const p = this._a12ComplianceRules.filter(r => r.result === 'pass').length;
    const f = this._a12ComplianceRules.filter(r => r.result === 'fail').length;
    const avg = this._a12ComplianceRules.reduce((s, r) => s + r.passRate, 0) / this._a12ComplianceRules.length;
    const v = this._a12ComplianceRules.reduce((s, r) => s + r.violations, 0);
    return {total: this._a12ComplianceRules.length, passing: p, failing: f, avgRate: Math.round(avg * 10) / 10, violations: v};
  }

  private _geta12PipelineStats(): {positions: number; applicants: number; interviewing: number; offers: number; critical: number} {
    const t = this._a12TalentPositions.length;
    const a = this._a12TalentPositions.reduce((s, p) => s + p.applicants, 0);
    const i = this._a12TalentPositions.reduce((s, p) => s + p.interviews, 0);
    const o = this._a12TalentPositions.reduce((s, p) => s + p.offers, 0);
    const c = this._a12TalentPositions.filter(x => x.urgency === 'critical').length;
    return {positions: t, applicants: a, interviewing: i, offers: o, critical: c};
  }

  private _geta12MetricTrends(): {improving: number; stable: number; total: number} {
    const imp = this._a12MetricsDashboard.filter(m => m.trend === 'improving').length;
    const stb = this._a12MetricsDashboard.filter(m => m.trend === 'stable').length;
    return {improving: imp, stable: stb, total: this._a12MetricsDashboard.length};
  }

  private _geta12ArchScore(): {total: number; passed: number; failed: number; partial: number; score: number} {
    const p = this._a12ArchReview.filter(c => c.status === 'pass').length;
    const f = this._a12ArchReview.filter(c => c.status === 'fail').length;
    const pa = this._a12ArchReview.filter(c => c.status === 'partial').length;
    return {total: this._a12ArchReview.length, passed: p, failed: f, partial: pa, score: Math.round((p / this._a12ArchReview.length) * 100)};
  }


  private _za12IncidentScenarios = [
    {id: 'za12-IS-001', name: 'Ransomware Outbreak in Production Environment', category: 'Malware', likelihood: 'Medium', impact: 'Critical', riskScore: 15, playbooks: ['IR-Ransomware-Isolate', 'IR-Ransomware-Eradicate', 'IR-Ransomway-Recover'], estimatedMTTD: '15min', estimatedMTTR: '4h', involvedTeams: ['SOC', 'IR', 'Infrastructure', 'Legal', 'Comms'], keyIndicators: ['Mass file encryption events', 'Ransom note deployment', 'Lateral movement via SMB/RDP', 'Backup deletion attempts'], mitigationControls: ['EDR behavioral detection', 'Network segmentation', 'Immutable backups', 'Email filtering'], lessonsLearned: 'Ensure offline backups are tested quarterly and Ransomware-specific playbooks are updated with latest threat intelligence', lastDrillDate: '2026-03-15', nextDrillDate: '2026-06-15'},
    {id: 'za12-IS-002', name: 'Cloud Misconfiguration Leading to Data Exposure', category: 'Cloud', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Cloud-Misconfig-Assess', 'IR-Cloud-Misconfig-Remediate', 'IR-Cloud-Misconfig-Notify'], estimatedMTTD: '2h', estimatedMTTR: '6h', involvedTeams: ['Cloud Security', 'SOC', 'Data Protection', 'Legal'], keyIndicators: ['Public S3 bucket exposure', 'Overly permissive IAM policies', 'Unencrypted database snapshots', 'Public API endpoint discovery'], mitigationControls: ['CSPM monitoring', 'IAM policy analyzer', 'Automated remediation', 'Cloud audit logging'], lessonsLearned: 'Implement CSPM with auto-remediation for common misconfigurations and conduct monthly cloud configuration reviews', lastDrillDate: '2026-02-20', nextDrillDate: '2026-05-20'},
    {id: 'za12-IS-003', name: 'Insider Threat Data Exfiltration', category: 'Insider', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-Insider-Investigate', 'IR-Insider-Contain', 'IR-Insider-Escalate'], estimatedMTTD: '8h', estimatedMTTR: '24h', involvedTeams: ['SOC', 'IR', 'HR', 'Legal', 'IAM'], keyIndicators: ['Anomalous data access patterns', 'Large file downloads to USB or cloud', 'Off-hours access to sensitive systems', 'Accessing data outside normal job scope'], mitigationControls: ['UEBA analytics', 'DLP policies', 'PAM controls', 'Access reviews'], lessonsLearned: 'Strengthen UEBA baselines and implement automated alerts for data access pattern anomalies', lastDrillDate: '2026-01-10', nextDrillDate: '2026-04-10'},
    {id: 'za12-IS-004', name: 'Supply Chain Attack via Compromised Vendor Software', category: 'Supply Chain', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-SupplyChain-Detect', 'IR-SupplyChain-Isolate', 'IR-SupplyChain-Remediate'], estimatedMTTD: '24h', estimatedMTTR: '72h', involvedTeams: ['SOC', 'IR', 'Procurement', 'Vendor Mgmt', 'Legal'], keyIndicators: ['Unexpected network connections from vendor software', 'Modified binaries or dependencies', 'Anomalous behavior from trusted applications', 'SBOM discrepancy alerts'], mitigationControls: ['SCA scanning', 'SBOM monitoring', 'Vendor security assessments', 'Network segmentation'], lessonsLearned: 'Require SBOM from all vendors and implement continuous monitoring of third-party component behavior', lastDrillDate: '2026-03-01', nextDrillDate: '2026-06-01'},
    {id: 'za12-IS-005', name: 'Credential Stuffing Attack on Customer Portal', category: 'External Attack', likelihood: 'High', impact: 'Medium', riskScore: 12, playbooks: ['IR-CredentialStuffing-Detect', 'IR-CredentialStuffing-Block', 'IR-CredentialStuffing-Notify'], estimatedMTTD: '30min', estimatedMTTR: '2h', involvedTeams: ['SOC', 'AppSec', 'Customer Support', 'Comms'], keyIndicators: ['High volume of failed login attempts', 'Login attempts from known compromised credential lists', 'Unusual geographic distribution of login attempts', 'Account takeover indicators'], mitigationControls: ['WAF rate limiting', 'CAPTCHA on login', 'MFA enforcement', 'Credential monitoring service'], lessonsLearned: 'Deploy advanced bot detection on login endpoints and implement proactive credential monitoring for customer accounts', lastDrillDate: '2026-04-01', nextDrillDate: '2026-07-01'},
    {id: 'za12-IS-006', name: 'Phishing Campaign Targeting Executive Team', category: 'Social Engineering', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Phishing-Contain', 'IR-Phishing-Investigate', 'IR-Phishing-Notify'], estimatedMTTD: '1h', estimatedMTTR: '4h', involvedTeams: ['SOC', 'Awareness', 'Executive Office', 'IT'], keyIndicators: ['Sophisticated spear-phishing emails to C-suite', 'Business email compromise indicators', 'Redirected MFA approval requests', 'Unauthorized wire transfer attempts'], mitigationControls: ['AI email filtering', 'Executive awareness training', 'Out-of-band verification policy', 'DMARC/DKIM/SPF'], lessonsLearned: 'Implement additional email security controls for executive mailboxes and conduct monthly spear-phishing simulations', lastDrillDate: '2026-02-15', nextDrillDate: '2026-05-15'}
  ];

  private _za12SecurityFrameworks = [
  ...
  private _getza12ScenarioSummary(): {total: number; critical: number; high: number; drilled: number; avgRiskScore: number} {
    const crit = this._za12IncidentScenarios.filter(s => s.impact === 'Critical').length;
    const high = this._za12IncidentScenarios.filter(s => s.impact === 'High').length;
    const avg = this._za12IncidentScenarios.reduce((s, x) => s + x.riskScore, 0) / this._za12IncidentScenarios.length;
    return {total: this._za12IncidentScenarios.length, critical: crit, high: high, drilled: this._za12IncidentScenarios.length, avgRiskScore: Math.round(avg)};
  }

  private _getza12VendorRiskSummary(): {total: number; approved: number; conditional: number; remediation: number; avgScore: number} {
    const approved = this._za12ThirdPartyRiskData.filter(v => v.status === 'approved').length;
    const cond = this._za12ThirdPartyRiskData.filter(v => v.status === 'conditional').length;
    const rem = this._za12ThirdPartyRiskData.filter(v => v.status === 'remediation_required').length;
    const avg = this._za12ThirdPartyRiskData.reduce((s, v) => s + v.riskScore, 0) / this._za12ThirdPartyRiskData.length;
    return {total: this._za12ThirdPartyRiskData.length, approved, conditional: cond, remediation: rem, avgScore: Math.round(avg)};
  }

  private _getza12TrainingSummary(): {total: number; avgCompletion: number; avgScore: number; overdueUpdates: number} {
    const avgComp = this._za12SecurityTrainingCatalog.reduce((s, t) => s + t.completionRate, 0) / this._za12SecurityTrainingCatalog.length;
    const avgScore = this._za12SecurityTrainingCatalog.filter(t => t.avgScore > 0).reduce((s, t) => s + t.avgScore, 0) / this._za12SecurityTrainingCatalog.filter(t => t.avgScore > 0).length;
    return {total: this._za12SecurityTrainingCatalog.length, avgCompletion: Math.round(avgComp * 10) / 10, avgScore: Math.round(avgScore), overdueUpdates: 0};
  }

  private _getza12ToolHealthSummary(): {total: number; healthy: number; degraded: number; annualCost: string} {
    const healthy = this._za12SecurityToolInventory.filter(t => t.health === 'healthy').length;
    const degraded = this._za12SecurityToolInventory.filter(t => t.health === 'degraded').length;
    const cost = this._za12SecurityToolInventory.reduce((s, t) => s + parseInt(t.annualCost.replace(/[^0-9]/g, '')), 0);
    return {total: this._za12SecurityToolInventory.length, healthy, degraded, annualCost: '$' + (cost / 1000).toFixed(0) + 'K'};
  }


  private _xa12SecurityZones = [
    {zoneId: 'xa12-Z-001', name: 'Internet DMZ', trustLevel: 'Untrusted', subnet: '10.0.0.0/24', segmentation: 'Dual-firewall', firewallRules: 45, monitoring: 'Full packet capture', assets: ['Web servers', 'Load balancers', 'WAF appliances', 'Reverse proxy'], riskLevel: 'high' as const, lastAssessment: '2026-04-15', nextAssessment: '2026-07-15'},
    {zoneId: 'xa12-Z-002', name: 'Corporate Network', trustLevel: 'Trusted', subnet: '10.1.0.0/16', segmentation: 'VLAN + micro-seg', firewallRules: 128, monitoring: 'NetFlow + IDS', assets: ['Workstations', 'Printers', 'Meeting room systems', 'VoIP phones'], riskLevel: 'medium' as const, lastAssessment: '2026-04-10', nextAssessment: '2026-07-10'},
    {zoneId: 'xa12-Z-003', name: 'Production Data Center', trustLevel: 'Restricted', subnet: '10.2.0.0/16', segmentation: 'Full micro-segmentation', firewallRules: 256, monitoring: 'Full packet capture + EDR', assets: ['Application servers', 'Database clusters', 'Message queues', 'API gateways'], riskLevel: 'low' as const, lastAssessment: '2026-04-12', nextAssessment: '2026-07-12'},
    {zoneId: 'xa12-Z-004', name: 'Management Network', trustLevel: 'Restricted', subnet: '10.3.0.0/24', segmentation: 'Isolated VLAN', firewallRules: 32, monitoring: 'Syslog + SNMP', assets: ['Hypervisors', 'Switch management', 'Storage controllers', 'Backup appliances'], riskLevel: 'low' as const, lastAssessment: '2026-04-08', nextAssessment: '2026-07-08'},
    {zoneId: 'xa12-Z-005', name: 'Cloud Production', trustLevel: 'Restricted', subnet: 'VPC: 172.16.0.0/12', segmentation: 'Security groups + NACLs', firewallRules: 189, monitoring: 'VPC Flow Logs + GuardDuty', assets: ['Kubernetes clusters', 'Managed databases', 'Object storage', 'Serverless functions'], riskLevel: 'medium' as const, lastAssessment: '2026-04-18', nextAssessment: '2026-07-18'}
  ];

  private _xa12SecurityAlerts = [
    {id: 'xa12-AL-001', severity: 'critical', title: 'Active ransomware encryption detected on PROD-DB-01', source: 'CrowdStrike', timestamp: '2026-04-22T13:45:00Z', status: 'investigating', assignee: 'SOC Analyst J. Smith', asset: 'PROD-DB-01 (10.2.1.15)', mitreTechnique: 'T1486 - Data Encrypted for Impact', playbook: 'IR-Ransomware-Isolate', sla: '15 minutes', elapsed: '12 minutes', notes: 'EDR detected mass file encryption event pattern. Automated isolation triggered. IR team paged.'},
    {id: 'xa12-AL-002', severity: 'high', title: 'Impossible travel detected for user admin@company.com', source: 'Okta', timestamp: '2026-04-22T12:30:00Z', status: 'triaged', assignee: 'SOC Analyst M. Johnson', asset: 'Okta - admin@company.com', mitreTechnique: 'T1078 - Valid Accounts', playbook: 'IR-ImpossibleTravel', sla: '30 minutes', elapsed: '87 minutes', notes: 'Login from Tokyo followed by login from New York within 30 minutes. Account temporarily locked. User confirmed via out-of-band channel they are in New York.'},
    {id: 'xa12-AL-003', severity: 'high', title: 'Public S3 bucket discovered with customer PII exposure', source: 'Prisma Cloud', timestamp: '2026-04-22T11:15:00Z', status: 'remediating', assignee: 'Cloud Security Lead A. Chen', asset: 'AWS S3: customer-data-archive-2024', mitreTechnique: 'N/A - Misconfiguration', playbook: 'IR-Cloud-Misconfig-Remediate', sla: '1 hour', elapsed: '2.5 hours', notes: 'Bucket containing archived customer records was publicly accessible. Bucket blocked and encrypted. Data exposure assessment in progress.'},
    {id: 'xa12-AL-004', severity: 'medium', title: 'Anomalous data download by user jane.developer@company.com', source: 'Microsoft DLP', timestamp: '2026-04-22T10:00:00Z', status: 'investigating', assignee: 'SOC Analyst R. Davis', asset: 'Endpoint: WS-DEV-042', mitreTechnique: 'T1567 - Exfil Over Web Service', playbook: 'IR-DLP-Investigation', sla: '4 hours', elapsed: '3.5 hours', notes: 'User downloaded 4.2GB of source code files to personal Google Drive. Interview scheduled with user and manager.'},
    {id: 'xa12-AL-005', severity: 'medium', title: 'Credential stuffing attack detected on customer portal', source: 'Cloudflare WAF', timestamp: '2026-04-22T09:30:00Z', status: 'contained', assignee: 'SOC Analyst K. Wilson', asset: 'Customer Portal (portal.company.com)', mitreTechnique: 'T1110 - Brute Force', playbook: 'IR-CredentialStuffing-Block', sla: '2 hours', elapsed: '4 hours', notes: 'Rate limiting and CAPTCHA automatically deployed. 12,000 failed login attempts from 800 unique IPs blocked. No successful account takeovers confirmed.'},
    {id: 'xa12-AL-006', severity: 'low', title: 'SSL certificate expiring in 14 days for api.company.com', source: 'Certificate monitoring', timestamp: '2026-04-22T08:00:00Z', status: 'remediated', assignee: 'Infrastructure Lead T. Brown', asset: 'api.company.com', mitreTechnique: 'N/A', playbook: 'Certificate Renewal', sla: '30 days', elapsed: '16 days', notes: 'Automated renewal triggered via ACME. New certificate deployed successfully. No service interruption.'}
  ];

  private _xa12ComplianceFrameworks = [
  ...
  private _xa12DataClassificationPolicy = [
  ...
  private _getxa12ZoneSummary(): {total: number; highRisk: number; mediumRisk: number; lowRisk: number; totalRules: number} {
    const high = this._xa12SecurityZones.filter(z => z.riskLevel === 'high').length;
    const med = this._xa12SecurityZones.filter(z => z.riskLevel === 'medium').length;
    const low = this._xa12SecurityZones.filter(z => z.riskLevel === 'low').length;
    const rules = this._xa12SecurityZones.reduce((s, z) => s + z.firewallRules, 0);
    return {total: this._xa12SecurityZones.length, highRisk: high, mediumRisk: med, lowRisk: low, totalRules: rules};
  }

  private _getxa12AlertSummary(): {total: number; critical: number; high: number; medium: number; low: number; open: number; contained: number} {
    const crit = this._xa12SecurityAlerts.filter(a => a.severity === 'critical').length;
    const high = this._xa12SecurityAlerts.filter(a => a.severity === 'high').length;
    const med = this._xa12SecurityAlerts.filter(a => a.severity === 'medium').length;
    const low = this._xa12SecurityAlerts.filter(a => a.severity === 'low').length;
    const open = this._xa12SecurityAlerts.filter(a => a.status === 'investigating' || a.status === 'triaged').length;
    const contained = this._xa12SecurityAlerts.filter(a => a.status === 'contained' || a.status === 'remediated').length;
    return {total: this._xa12SecurityAlerts.length, critical: crit, high, medium: med, low, open, contained};
  }

  private _getxa12ComplianceSummary(): {total: number; fullyCompliant: number; mostlyCompliant: number; avgComplianceRate: number} {
    const full = this._xa12ComplianceFrameworks.filter(f => f.status === 'compliant').length;
    const mostly = this._xa12ComplianceFrameworks.filter(f => f.status === 'mostly_compliant').length;
    const avgRate = this._xa12ComplianceFrameworks.reduce((s, f) => s + (f.compliant / f.requirements * 100), 0) / this._xa12ComplianceFrameworks.length;
  ...
  // === Security Risk Register (vendor_risk_) ===
  private _vendor_risk_RiskRegister = [
    { id: "RSK-VEN-0001", title: "Data breach from unpatched systems", owner: "CTO", category: "Financial", status: "mitigating", trend: "stable", severity: "critical", likelihood: 7, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-13", nextReview: "2026-07-14" },
    { id: "RSK-VEN-0002", title: "Ransomware attack on critical infrastructure", owner: "CRO", category: "Strategic", status: "closed", trend: "decreasing", severity: "low", likelihood: 6, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-20", nextReview: "2026-07-05" },
    { id: "RSK-VEN-0003", title: "Insider threat data exfiltration", owner: "VP Eng", category: "Compliance", status: "accepted", trend: "increasing", severity: "medium", likelihood: 7, impact: 4, treatment: "Monitor and review quarterly", lastReview: "2026-04-02", nextReview: "2026-07-25" },
    { id: "RSK-VEN-0004", title: "Supply chain compromise", owner: "Security Lead", category: "Technical", status: "open", trend: "stable", severity: "critical", likelihood: 7, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-04", nextReview: "2026-07-18" },
    { id: "RSK-VEN-0005", title: "Cloud misconfiguration exposure", owner: "Risk Mgr", category: "Reputational", status: "mitigating", trend: "decreasing", severity: "high", likelihood: 6, impact: 5, treatment: "Monitor and review quarterly", lastReview: "2026-04-09", nextReview: "2026-07-12" },
    { id: "RSK-VEN-0006", title: "Phishing campaign success rate", owner: "Compliance Dir", category: "Legal", status: "closed", trend: "increasing", severity: "low", likelihood: 10, impact: 5, treatment: "Monitor and review quarterly", lastReview: "2026-04-01", nextReview: "2026-07-26" },
    { id: "RSK-VEN-0007", title: "Third-party vendor data breach", owner: "IT Dir", category: "Regulatory", status: "accepted", trend: "stable", severity: "high", likelihood: 3, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-12", nextReview: "2026-07-04" },
    { id: "RSK-VEN-0008", title: "Regulatory non-compliance penalty", owner: "DevOps Lead", category: "Third-Party", status: "open", trend: "decreasing", severity: "low", likelihood: 3, impact: 9, treatment: "Monitor and review quarterly", lastReview: "2026-04-17", nextReview: "2026-07-11" },
    { id: "RSK-VEN-0009", title: "Zero-day exploit in production", owner: "Architect", category: "Human Capital", status: "mitigating", trend: "increasing", severity: "high", likelihood: 5, impact: 9, treatment: "Monitor and review quarterly", lastReview: "2026-04-21", nextReview: "2026-07-28" },
    { id: "RSK-VEN-0010", title: "Insufficient access controls", owner: "CISO", category: "Operational", status: "closed", trend: "stable", severity: "critical", likelihood: 4, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-16", nextReview: "2026-07-17" },
    { id: "RSK-VEN-0011", title: "DDoS attack on services", owner: "CTO", category: "Financial", status: "accepted", trend: "decreasing", severity: "medium", likelihood: 5, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-22", nextReview: "2026-07-24" },
    { id: "RSK-VEN-0012", title: "Social engineering attack", owner: "CRO", category: "Strategic", status: "open", trend: "increasing", severity: "critical", likelihood: 5, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-03", nextReview: "2026-07-26" },
    { id: "RSK-VEN-0013", title: "API security vulnerability", owner: "VP Eng", category: "Compliance", status: "mitigating", trend: "stable", severity: "medium", likelihood: 5, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-04", nextReview: "2026-07-14" },
    { id: "RSK-VEN-0014", title: "Mobile device compromise", owner: "Security Lead", category: "Technical", status: "closed", trend: "decreasing", severity: "high", likelihood: 6, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-15", nextReview: "2026-07-16" },
    { id: "RSK-VEN-0015", title: "Physical security breach", owner: "Risk Mgr", category: "Reputational", status: "accepted", trend: "increasing", severity: "medium", likelihood: 8, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-07", nextReview: "2026-07-01" },
    { id: "RSK-VEN-0016", title: "Password policy weakness", owner: "Compliance Dir", category: "Legal", status: "open", trend: "stable", severity: "critical", likelihood: 7, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-05", nextReview: "2026-07-10" },
    { id: "RSK-VEN-0017", title: "Network segmentation gap", owner: "IT Dir", category: "Regulatory", status: "mitigating", trend: "decreasing", severity: "low", likelihood: 2, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-21", nextReview: "2026-07-11" },
    { id: "RSK-VEN-0018", title: "Encryption key management failure", owner: "DevOps Lead", category: "Third-Party", status: "closed", trend: "increasing", severity: "critical", likelihood: 7, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-15", nextReview: "2026-07-22" },
    { id: "RSK-VEN-0019", title: "Audit trail tampering", owner: "Architect", category: "Human Capital", status: "accepted", trend: "stable", severity: "medium", likelihood: 4, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-16", nextReview: "2026-07-07" },
    { id: "RSK-VEN-0020", title: "Business email compromise", owner: "CISO", category: "Operational", status: "open", trend: "decreasing", severity: "medium", likelihood: 7, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-01", nextReview: "2026-07-18" },
  ];
  private _vendor_risk_RiskFilter: string = 'all';
  private _vendor_risk_RiskSeverity: string = 'all';
  private _vendor_risk_RiskStatus: string = 'all';
  private _vendor_risk_ExpandedRisk: string = '';
  private _vendor_risk_FilterRisks() {
    const reg = this._vendor_risk_RiskRegister;
    return reg.filter(r => {
      if (this._vendor_risk_RiskFilter !== "all" && r.category !== this._vendor_risk_RiskFilter) return false;
      if (this._vendor_risk_RiskSeverity !== "all" && r.severity !== this._vendor_risk_RiskSeverity) return false;
      if (this._vendor_risk_RiskStatus !== "all" && r.status !== this._vendor_risk_RiskStatus) return false;
      return true;
    });
  }
  private _vendor_risk_GetRiskScore(r: any) { return Math.round(r.likelihood * r.impact * 1.5); }
  private _vendor_risk_GetRiskTrendIcon(trend: string) {
    if (trend === "increasing") return "\u2191";
    if (trend === "decreasing") return "\u2193";
    return "\u2192";
  }
  private _vendor_risk_GetRiskColor(sev: string) {
    if (sev === "critical") return "#dc2626";
    if (sev === "high") return "#ea580c";
    if (sev === "medium") return "#d97706";
    return "#16a34a";
  }
  private _vendor_risk_GetRiskCounts() {
    const reg = this._vendor_risk_RiskRegister;
  ...
  private _vendor_risk_GetTreatmentProgress() {
    const reg = this._vendor_risk_RiskRegister;
    const treated = reg.filter(r => r.status === "mitigating" || r.status === "closed").length;
    return Math.round((treated / Math.max(reg.length, 1)) * 100);
  }

  // === Security Metrics API Gateway (vendor_risk_) ===
  private _vendor_risk_ApiEndpoints = [
    { method: "GET", path: "/api/v1/threats", name: "Threat Intelligence Feed", status: "active", avgLatency: 350.7, reqPerMin: 716.0, errorRate: 0.97, uptime: round(98.00302698047986,2), version: "v1.0.4" },
    { method: "POST", path: "/api/v1/scans/start", name: "Vulnerability Scanner", status: "active", avgLatency: 207.4, reqPerMin: 2284.0, errorRate: 1.92, uptime: round(98.25054841878608,2), version: "v3.7.0" },
    { method: "GET", path: "/api/v1/assets", name: "Asset Inventory", status: "active", avgLatency: 447.3, reqPerMin: 1188.0, errorRate: 1.09, uptime: round(97.40613542914753,2), version: "v2.3.0" },
    { method: "POST", path: "/api/v1/alerts", name: "Alert Management", status: "active", avgLatency: 421.0, reqPerMin: 1427.0, errorRate: 0.39, uptime: round(95.6554134743215,2), version: "v2.5.6" },
    { method: "GET", path: "/api/v1/compliance", name: "Compliance Status", status: "active", avgLatency: 297.3, reqPerMin: 2023.0, errorRate: 0.12, uptime: round(96.9238537476504,2), version: "v1.0.7" },
    { method: "PUT", path: "/api/v1/policies", name: "Policy Engine", status: "active", avgLatency: 269.4, reqPerMin: 617.0, errorRate: 2.11, uptime: round(97.46597984968822,2), version: "v1.9.7" },
    { method: "GET", path: "/api/v1/incidents", name: "Incident Tracker", status: "active", avgLatency: 259.8, reqPerMin: 373.0, errorRate: 2.08, uptime: round(95.09468820273678,2), version: "v3.0.9" },
    { method: "POST", path: "/api/v1/forensics", name: "Forensics Collector", status: "degraded", avgLatency: 390.9, reqPerMin: 1447.0, errorRate: 0.03, uptime: round(95.49566799533225,2), version: "v3.4.15" },
    { method: "GET", path: "/api/v1/risk/assess", name: "Risk Assessment", status: "active", avgLatency: 163.7, reqPerMin: 1699.0, errorRate: 0.17, uptime: round(95.30004242853781,2), version: "v2.2.18" },
    { method: "POST", path: "/api/v1/auth/verify", name: "Authentication", status: "active", avgLatency: 64.9, reqPerMin: 1681.0, errorRate: 2.49, uptime: round(96.49916030633858,2), version: "v2.4.19" },
    { method: "GET", path: "/api/v1/logs/audit", name: "Audit Log Query", status: "active", avgLatency: 157.3, reqPerMin: 2223.0, errorRate: 2.3, uptime: round(97.33473310854039,2), version: "v2.6.0" },
    { method: "PUT", path: "/api/v1/users/roles", name: "Role Management", status: "active", avgLatency: 385.6, reqPerMin: 1609.0, errorRate: 2.14, uptime: round(97.60233332687002,2), version: "v2.8.9" },
    { method: "POST", path: "/api/v1/encrypt", name: "Encryption Service", status: "active", avgLatency: 428.3, reqPerMin: 724.0, errorRate: 2.16, uptime: round(96.21322910894042,2), version: "v2.1.20" },
    { method: "GET", path: "/api/v1/network/topo", name: "Network Topology", status: "maintenance", avgLatency: 27.4, reqPerMin: 556.0, errorRate: 1.38, uptime: round(96.02710278136831,2), version: "v2.8.6" },
    { method: "DELETE", path: "/api/v1/sessions", name: "Session Manager", status: "active", avgLatency: 375.2, reqPerMin: 2247.0, errorRate: 2.09, uptime: round(97.82872741741078,2), version: "v3.0.15" },
  ];
  private _vendor_risk_ApiKeys = [
    { id: "ak-000001", name: "key-vend-001", created: "2026-03-07", lastUsed: "2026-04-12", status: "active", calls: 15481, rateLimit: 5000 },
    { id: "ak-000002", name: "key-vend-002", created: "2026-09-11", lastUsed: "2026-04-10", status: "active", calls: 39728, rateLimit: 5000 },
    { id: "ak-000003", name: "key-vend-003", created: "2026-09-10", lastUsed: "2026-04-20", status: "revoked", calls: 31968, rateLimit: 100 },
    { id: "ak-000004", name: "key-vend-004", created: "2026-08-04", lastUsed: "2026-04-21", status: "revoked", calls: 9145, rateLimit: 100 },
    { id: "ak-000005", name: "key-vend-005", created: "2026-09-22", lastUsed: "2026-04-06", status: "active", calls: 40114, rateLimit: 500 },
    { id: "ak-000006", name: "key-vend-006", created: "2026-03-19", lastUsed: "2026-04-04", status: "active", calls: 42981, rateLimit: 5000 },
    { id: "ak-000007", name: "key-vend-007", created: "2026-02-03", lastUsed: "2026-04-08", status: "active", calls: 30617, rateLimit: 1000 },
    { id: "ak-000008", name: "key-vend-008", created: "2026-03-28", lastUsed: "2026-04-03", status: "active", calls: 32788, rateLimit: 100 },
  ];
  private _vendor_risk_ApiHealthSummary() {
    const eps = this._vendor_risk_ApiEndpoints;
  ...
  private _vendor_risk_GetApiByMethod(method: string) { return this._vendor_risk_ApiEndpoints.filter(e=>e.method===method); }
  private _vendor_risk_GetSlowEndpoints() { return this._vendor_risk_ApiEndpoints.filter(e=>e.avgLatency>200).sort((a,b)=>b.avgLatency-a.avgLatency); }
  private _vendor_risk_GetHighErrorEndpoints() { return this._vendor_risk_ApiEndpoints.filter(e=>e.errorRate>1.0).sort((a,b)=>b.errorRate-a.errorRate); }

  // === Security Training Effectiveness (vendor_risk_) ===
  private _vendor_risk_TrainingModules = [
    { id: "TRN-001", name: "Security Fundamentals", completionRate: 63.7, avgScore: 68.2, behaviorChange: 69.9, enrolled: 60, completed: 419, duration: "103min", category: "mandatory" },
    { id: "TRN-002", name: "Phishing Awareness", completionRate: 46.2, avgScore: 69.7, behaviorChange: 38.8, enrolled: 282, completed: 117, duration: "88min", category: "mandatory" },
    { id: "TRN-003", name: "Social Engineering Defense", completionRate: 85.6, avgScore: 89.0, behaviorChange: 36.6, enrolled: 139, completed: 82, duration: "81min", category: "optional" },
    { id: "TRN-004", name: "Password Hygiene", completionRate: 47.3, avgScore: 78.9, behaviorChange: 51.4, enrolled: 464, completed: 420, duration: "54min", category: "mandatory" },
    { id: "TRN-005", name: "Data Classification", completionRate: 60.3, avgScore: 69.0, behaviorChange: 85.2, enrolled: 94, completed: 97, duration: "66min", category: "mandatory" },
    { id: "TRN-006", name: "Incident Response Basics", completionRate: 90.4, avgScore: 78.0, behaviorChange: 39.5, enrolled: 144, completed: 253, duration: "105min", category: "mandatory" },
    { id: "TRN-007", name: "Secure Coding", completionRate: 82.0, avgScore: 57.3, behaviorChange: 54.8, enrolled: 484, completed: 119, duration: "68min", category: "optional" },
    { id: "TRN-008", name: "Cloud Security", completionRate: 64.1, avgScore: 61.7, behaviorChange: 54.7, enrolled: 57, completed: 35, duration: "54min", category: "mandatory" },
    { id: "TRN-009", name: "Mobile Device Security", completionRate: 84.0, avgScore: 76.6, behaviorChange: 51.0, enrolled: 252, completed: 291, duration: "120min", category: "mandatory" },
    { id: "TRN-010", name: "Network Security", completionRate: 49.4, avgScore: 84.9, behaviorChange: 70.3, enrolled: 139, completed: 262, duration: "88min", category: "mandatory" },
    { id: "TRN-011", name: "Physical Security", completionRate: 97.5, avgScore: 83.0, behaviorChange: 53.4, enrolled: 401, completed: 420, duration: "115min", category: "optional" },
    { id: "TRN-012", name: "Regulatory Compliance", completionRate: 91.7, avgScore: 85.9, behaviorChange: 84.2, enrolled: 273, completed: 401, duration: "77min", category: "mandatory" },
    { id: "TRN-013", name: "Risk Management", completionRate: 71.2, avgScore: 80.6, behaviorChange: 89.7, enrolled: 467, completed: 209, duration: "66min", category: "mandatory" },
    { id: "TRN-014", name: "Cryptography Basics", completionRate: 64.8, avgScore: 68.0, behaviorChange: 66.0, enrolled: 403, completed: 68, duration: "45min", category: "mandatory" },
    { id: "TRN-015", name: "Access Control", completionRate: 56.8, avgScore: 81.0, behaviorChange: 72.9, enrolled: 129, completed: 212, duration: "89min", category: "optional" },
    { id: "TRN-016", name: "Vendor Management", completionRate: 47.4, avgScore: 85.9, behaviorChange: 78.3, enrolled: 203, completed: 118, duration: "90min", category: "mandatory" },
  ];
  private _vendor_risk_PhishingResults = [
    { month: "2026-01", sent: 976, clicked: 6, reported: 566, clickRate: round(0.6147540983606558,1), reportRate: round(57.99180327868852,1) },
    { month: "2026-02", sent: 329, clicked: 44, reported: 113, clickRate: round(13.37386018237082,1), reportRate: round(34.34650455927052,1) },
    { month: "2026-03", sent: 401, clicked: 17, reported: 240, clickRate: round(4.239401496259352,1), reportRate: round(59.85037406483791,1) },
    { month: "2026-04", sent: 421, clicked: 102, reported: 89, clickRate: round(24.228028503562946,1), reportRate: round(21.140142517814727,1) },
    { month: "2026-05", sent: 680, clicked: 101, reported: 386, clickRate: round(14.852941176470589,1), reportRate: round(56.76470588235294,1) },
    { month: "2026-06", sent: 604, clicked: 75, reported: 81, clickRate: round(12.417218543046356,1), reportRate: round(13.410596026490065,1) },
  ];
  private _vendor_risk_TrainingROI = { totalInvestment: 65649, avgCostPerEmployee: 371, riskReductionPct: round(random.uniform(15,45),1), incidentReductionPct: round(random.uniform(10,35),1), complianceScoreGain: round(random.uniform(5,25),1) };
  private _vendor_risk_LearningPaths = [
    { name: "Beginner Security Analyst", totalModules: 13, completedModules: 14, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 54, enrolled: 48 },
    { name: "Advanced Threat Hunter", totalModules: 10, completedModules: 14, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 15, enrolled: 128 },
    { name: "Security Architect", totalModules: 8, completedModules: 15, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 30, enrolled: 144 },
    { name: "Incident Responder", totalModules: 12, completedModules: 15, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 17, enrolled: 163 },
    { name: "Compliance Specialist", totalModules: 17, completedModules: 11, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 61, enrolled: 178 },
    { name: "DevSecOps Engineer", totalModules: 15, completedModules: 4, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 68, enrolled: 46 },
  ];
  private _vendor_risk_GetOverallCompletion() {
    const mods = this._vendor_risk_TrainingModules;
    return round(mods.reduce((s,m)=>s+m.completionRate,0)/mods.length,1);
  }
  private _vendor_risk_GetTopPerformers() { return [...this._vendor_risk_TrainingModules].sort((a,b)=>b.avgScore-a.avgScore).slice(0,5); }
  private _vendor_risk_GetModulesNeedingAttention() { return this._vendor_risk_TrainingModules.filter(m=>m.completionRate<70||m.avgScore<65); }

  // === Security Governance Framework (vendor_risk_) ===
  private _vendor_risk_GovBodies = [
    { name: "Security Steering Committee", members: 13, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-22", nextMeeting: "2026-05-01", quorum: 3 },
    { name: "Risk Management Board", members: 5, chair: "CTO", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-19", nextMeeting: "2026-05-23", quorum: 7 },
    { name: "Data Governance Council", members: 14, chair: "CRO", meetingFreq: "Monthly", lastMeeting: "2026-04-22", nextMeeting: "2026-05-16", quorum: 3 },
    { name: "Compliance Oversight Board", members: 12, chair: "CDO", meetingFreq: "Monthly", lastMeeting: "2026-04-21", nextMeeting: "2026-05-07", quorum: 7 },
    { name: "Architecture Review Board", members: 13, chair: "VP Eng", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-21", nextMeeting: "2026-05-20", quorum: 3 },
    { name: "Change Advisory Board", members: 5, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-16", nextMeeting: "2026-05-02", quorum: 5 },
    { name: "Incident Review Board", members: 10, chair: "CIO", meetingFreq: "As needed", lastMeeting: "2026-04-15", nextMeeting: "2026-05-08", quorum: 8 },
    { name: "Vendor Risk Committee", members: 9, chair: "CFO", meetingFreq: "Quarterly", lastMeeting: "2026-04-20", nextMeeting: "2026-05-11", quorum: 8 },
  ];
  private _vendor_risk_Decisions = [
    { id: "DEC-0001", title: "Adopt zero-trust architecture framework", date: "2026-03-15", status: "approved", owner: "CISO" },
    { id: "DEC-0002", title: "Migrate to SIEM 2.0 platform", date: "2026-03-05", status: "implemented", owner: "CTO" },
    ...
    { id: "DEC-0008", title: "Implement network micro-segmentation", date: "2026-03-24", status: "in-progress", owner: "Security Lead" },
    { id: "DEC-0009", title: "Establish security champion program", date: "2026-02-05", status: "pending", owner: "CRO" },
    { id: "DEC-0010", title: "Migrate to passwordless authentication", date: "2026-02-04", status: "approved", owner: "VP Eng" },
  ];
  private _vendor_risk_PolicyLifecycle = [
    { name: "Information Security Policy", version: "v2.3", status: "active", lastUpdated: "2026-03-02", nextReview: "2026-7-28", owner: "CISO" },
    { name: "Acceptable Use Policy", version: "v1.1", status: "active", lastUpdated: "2026-03-13", nextReview: "2026-12-28", owner: "Legal" },
    { name: "Data Retention Policy", version: "v3.0", status: "under-review", lastUpdated: "2026-03-05", nextReview: "2026-7-26", owner: "DPO" },
    { name: "Access Control Policy", version: "v1.2", status: "active", lastUpdated: "2026-02-25", nextReview: "2026-11-24", owner: "IAM Lead" },
    { name: "Incident Response Policy", version: "v4.1", status: "draft", lastUpdated: "2026-04-03", nextReview: "2026-9-07", owner: "IR Lead" },
    { name: "Business Continuity Plan", version: "v3.3", status: "active", lastUpdated: "2026-01-18", nextReview: "2026-7-21", owner: "BCP Mgr" },
    { name: "Vendor Management Policy", version: "v3.1", status: "active", lastUpdated: "2026-01-06", nextReview: "2026-8-18", owner: "Procurement" },
    { name: "Encryption Standard", version: "v4.2", status: "under-review", lastUpdated: "2026-03-05", nextReview: "2026-7-09", owner: "Security Arch" },
  ];
  private _vendor_risk_GovMaturityScore = { overall: 2.6, riskManagement: 4.9, compliance: 2.8, incidentResponse: 3.9, awareness: 5.5, technology: 5.2 };
  private _vendor_risk_GetPendingDecisions() { return this._vendor_risk_Decisions.filter(d=>d.status==='pending'||d.status==='in-progress'); }
  private _vendor_risk_GetActivePolicies() { return this._vendor_risk_PolicyLifecycle.filter(p=>p.status==='active'); }
  private _vendor_risk_GetEscalationPath() { return ["L1 Analyst","L2 Senior","Security Lead","CISO","Board"]; }

  // === Security Innovation Lab (vendor_risk_) ===
  private _vendor_risk_InnoProjects = [
    { id: "INN-001", name: "AI-Powered Threat Detection", description: "Machine learning models for real-time threat identification", status: "active", progress: 72, startDate: "2026-03-14", teamSize: 8, budget: 99453, milestones: 7, completedMilestones: 8 },
    { id: "INN-002", name: "Quantum-Resistant Cryptography", description: "Post-quantum encryption algorithm prototyping", status: "research", progress: 35, startDate: "2026-03-19", teamSize: 2, budget: 106388, milestones: 5, completedMilestones: 6 },
    { id: "INN-003", name: "Automated Red Teaming", description: "Autonomous penetration testing framework", status: "active", progress: 58, startDate: "2026-01-15", teamSize: 3, budget: 167782, milestones: 10, completedMilestones: 7 },
    { id: "INN-004", name: "Zero-Knowledge Authentication", description: "Privacy-preserving identity verification", status: "poc", progress: 88, startDate: "2026-03-12", teamSize: 8, budget: 176852, milestones: 7, completedMilestones: 8 },
    { id: "INN-005", name: "Blockchain Audit Trail", description: "Immutable security event logging", status: "active", progress: 45, startDate: "2026-01-03", teamSize: 5, budget: 196931, milestones: 9, completedMilestones: 8 },
    { id: "INN-006", name: "Behavioral Biometrics", description: "Continuous authentication via user behavior patterns", status: "research", progress: 22, startDate: "2026-01-08", teamSize: 6, budget: 27805, milestones: 8, completedMilestones: 4 },
    { id: "INN-007", name: "Deception Grid 2.0", description: "Advanced honeypot network with adaptive responses", status: "active", progress: 65, startDate: "2026-03-17", teamSize: 3, budget: 28276, milestones: 4, completedMilestones: 4 },
    { id: "INN-008", name: "Secure Enclave Integration", description: "Hardware-backed security for critical workloads", status: "poc", progress: 40, startDate: "2026-02-21", teamSize: 2, budget: 150946, milestones: 10, completedMilestones: 3 },
  ];
  private _vendor_risk_TechEvaluations = [
    ...
    { name: "SASE Architecture", status: "completed", score: 10.1, recommendation: "Adopt", vendor: "Multiple" },
    { name: "SOAR Platform 3.0", status: "planned", score: 9.2, recommendation: "Monitor", vendor: "Splunk" },
  ];
  private _vendor_risk_CollaborationPartners = [
  ...
  private _vendor_risk_InnoMetrics = { totalProjects: 8, activeProjects: 4, avgTimeToValue: "91 days", pocSuccessRate: round(random.uniform(55,85),1), researchToProduction: round(random.uniform(20,50),1), innovationIndex: round(random.uniform(6.0,9.5),1) };
  private _vendor_risk_GetProjectByStatus(status: string) { return this._vendor_risk_InnoProjects.filter(p=>p.status===status); }
  private _vendor_risk_GetTopEvaluations() { return [...this._vendor_risk_TechEvaluations].sort((a,b)=>b.score-a.score).slice(0,3); }



  // === Compliance Dashboard Extension (vendor_risk_) ===
  private _vendor_risk_ComplianceFrameworks = [
    { name: "SOC 2 Type II", description: "Trust Services Criteria", totalControls: 5, implementedControls: 4, status: "non-compliant", lastAudit: "2026-02-11", nextAudit: "2026-8-15", evidenceCount: 220 },
    { name: "ISO 27001", description: "Information Security Management", totalControls: 114, implementedControls: 114, status: "compliant", lastAudit: "2026-03-02", nextAudit: "2026-7-24", evidenceCount: 370 },
    { name: "PCI DSS 4.0", description: "Payment Card Industry", totalControls: 12, implementedControls: 7, status: "non-compliant", lastAudit: "2026-02-18", nextAudit: "2026-12-17", evidenceCount: 71 },
    { name: "HIPAA", description: "Health Insurance Portability", totalControls: 18, implementedControls: 11, status: "in-review", lastAudit: "2026-04-27", nextAudit: "2026-11-08", evidenceCount: 358 },
    { name: "GDPR", description: "Data Protection Regulation", totalControls: 99, implementedControls: 81, status: "in-review", lastAudit: "2026-03-13", nextAudit: "2026-7-23", evidenceCount: 170 },
    { name: "NIST CSF 2.0", description: "Cybersecurity Framework", totalControls: 6, implementedControls: 3, status: "compliant", lastAudit: "2026-02-19", nextAudit: "2026-10-07", evidenceCount: 102 },
    { name: "FedRAMP", description: "Federal Risk Authorization", totalControls: 15, implementedControls: 7, status: "in-review", lastAudit: "2026-03-24", nextAudit: "2026-8-17", evidenceCount: 266 },
    { name: "SOX", description: "Sarbanes-Oxley Compliance", totalControls: 8, implementedControls: 8, status: "partially-compliant", lastAudit: "2026-03-05", nextAudit: "2026-11-05", evidenceCount: 375 },
    { name: "CIS Controls v8", description: "Center for Internet Security", totalControls: 18, implementedControls: 14, status: "partially-compliant", lastAudit: "2026-02-09", nextAudit: "2026-10-10", evidenceCount: 99 },
    { name: "COBIT 2019", description: "IT Governance Framework", totalControls: 40, implementedControls: 26, status: "compliant", lastAudit: "2026-01-04", nextAudit: "2026-12-12", evidenceCount: 55 },
  ];
  private _vendor_risk_GetComplianceScore() {
    const fw = this._vendor_risk_ComplianceFrameworks;
    return round(fw.reduce((s,f)=>s + (f.implementedControls/Math.max(f.totalControls,1))*100, 0) / fw.length, 1);
  }
  private _vendor_risk_GetGaps() {
    return this._vendor_risk_ComplianceFrameworks.filter(f => f.status !== "compliant");
  }
  private _vendor_risk_AuditTrail = [
    ...
    { id: "AUD-0004", action: "Remediation completed", auditor: "Compliance Officer", date: "2026-04-05", result: "pass", findings: 4 },
    { id: "AUD-0005", action: "Policy updated", auditor: "IT Audit", date: "2026-04-02", result: "pass", findings: 1 },
    { id: "AUD-0006", action: "Training verified", auditor: "Risk Team", date: "2026-04-12", result: "pass", findings: 5 },
    { id: "AUD-0007", action: "Access reviewed", auditor: "QA Team", date: "2026-04-16", result: "pass", findings: 4 },
  ...
  // === Threat Intelligence Feed Extension (vendor_risk_) ===
  private _vendor_risk_ThreatActors = [
    { name: "APT-29", alias: "Cozy Bear", origin: "Russia", type: "Nation-State", severity: "high", lastActivity: "2026-04-13", targets: "Government", indicators: 182, ttps: 38 },
    { name: "APT-41", alias: "Double Dragon", origin: "China", type: "Nation-State", severity: "critical", lastActivity: "2026-04-01", targets: "Technology", indicators: 457, ttps: 29 },
    { name: "Lazarus Group", alias: "Hidden Cobra", origin: "North Korea", type: "Nation-State", severity: "critical", lastActivity: "2026-04-03", targets: "Healthcare", indicators: 54, ttps: 16 },
    { name: "FIN7", alias: "Carbanak", origin: "Eastern Europe", type: "Financial", severity: "high", lastActivity: "2026-04-16", targets: "Finance", indicators: 321, ttps: 43 },
    { name: "Conti", alias: "Wizard Spider", origin: "Russia", type: "Ransomware", severity: "critical", lastActivity: "2026-04-21", targets: "Government", indicators: 61, ttps: 21 },
    { name: "LockBit", alias: "LockBit Gang", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-05", targets: "Manufacturing", indicators: 300, ttps: 10 },
    { name: "Cl0p", alias: "Cl0p Team", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-07", targets: "Energy", indicators: 66, ttps: 15 },
    { name: "Sandworm", alias: "Unit 74455", origin: "Russia", type: "Nation-State", severity: "critical", lastActivity: "2026-04-21", targets: "Energy", indicators: 154, ttps: 29 },
  ];
  private _vendor_risk_IoCFeed = [
    { id: "ioc-000001", type: "ip", value: "58.180.43.125", confidence: 65, source: "CrowdStrike", firstSeen: "2026-04-17", lastSeen: "2026-04-18" },
    { id: "ioc-000002", type: "ip", value: "35.27.100.12", confidence: 41, source: "STIX", firstSeen: "2026-04-20", lastSeen: "2026-04-08" },
    { id: "ioc-000003", type: "ip", value: "234.197.167.238", confidence: 85, source: "VirusTotal", firstSeen: "2026-04-04", lastSeen: "2026-04-10" },
    { id: "ioc-000004", type: "ip", value: "245.153.6.232", confidence: 77, source: "Mandiant", firstSeen: "2026-04-23", lastSeen: "2026-04-07" },
    { id: "ioc-000005", type: "ip", value: "19.137.226.199", confidence: 69, source: "STIX", firstSeen: "2026-04-17", lastSeen: "2026-04-03" },
    { id: "ioc-000006", type: "ip", value: "172.154.63.208", confidence: 66, source: "Mandiant", firstSeen: "2026-04-08", lastSeen: "2026-04-16" },
    { id: "ioc-000007", type: "ip", value: "41.228.150.214", confidence: 55, source: "CrowdStrike", firstSeen: "2026-04-03", lastSeen: "2026-04-02" },
    { id: "ioc-000008", type: "ip", value: "57.85.108.1", confidence: 75, source: "AlienVault", firstSeen: "2026-04-04", lastSeen: "2026-04-15" },
    { id: "ioc-000009", type: "ip", value: "203.212.76.180", confidence: 69, source: "AlienVault", firstSeen: "2026-04-21", lastSeen: "2026-04-14" },
    { id: "ioc-000010", type: "ip", value: "54.204.24.238", confidence: 88, source: "CrowdStrike", firstSeen: "2026-04-21", lastSeen: "2026-04-22" },
    { id: "ioc-000011", type: "ip", value: "96.99.187.47", confidence: 91, source: "AlienVault", firstSeen: "2026-04-12", lastSeen: "2026-04-05" },
    { id: "ioc-000012", type: "ip", value: "147.23.208.151", confidence: 84, source: "STIX", firstSeen: "2026-04-17", lastSeen: "2026-04-20" },
    { id: "ioc-000013", type: "ip", value: "32.73.213.105", confidence: 77, source: "AlienVault", firstSeen: "2026-04-01", lastSeen: "2026-04-11" },
    { id: "ioc-000014", type: "ip", value: "64.51.194.80", confidence: 84, source: "VirusTotal", firstSeen: "2026-04-10", lastSeen: "2026-04-15" },
    { id: "ioc-000015", type: "ip", value: "97.135.223.18", confidence: 87, source: "AlienVault", firstSeen: "2026-04-01", lastSeen: "2026-04-19" },
  ];
  private _vendor_risk_GetActiveThreats() { return this._vendor_risk_ThreatActors.filter(a => a.severity === 'critical'); }
  private _vendor_risk_GetThreatSummary() {
    const actors = this._vendor_risk_ThreatActors;
  ...
  // === Incident Management Extension (vendor_risk_) ===
  private _vendor_risk_Incidents = [
    { id: "INC-20260001", title: "Unauthorized access detected", severity: "critical", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-19T02:10", affectedAssets: 24, rootCause: "Misconfiguration" },
    { id: "INC-20260002", title: "Malware outbreak on workstation", severity: "high", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-17T05:51", affectedAssets: 28, rootCause: "Credential compromise" },
    { id: "INC-20260003", title: "Data leak from S3 bucket", severity: "medium", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-22T00:27", affectedAssets: 23, rootCause: "Zero-day" },
    { id: "INC-20260004", title: "Phishing campaign targeting finance", severity: "low", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-18T04:09", affectedAssets: 49, rootCause: "Human error" },
    { id: "INC-20260005", title: "DDoS attack on web services", severity: "critical", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-16T19:10", affectedAssets: 29, rootCause: "Policy violation" },
    { id: "INC-20260006", title: "Ransomware encryption attempt", severity: "high", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-15T17:29", affectedAssets: 33, rootCause: "Unknown" },
    { id: "INC-20260007", title: "Insider data exfiltration", severity: "medium", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-13T18:20", affectedAssets: 34, rootCause: "Misconfiguration" },
    { id: "INC-20260008", title: "API key exposure in repo", severity: "low", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-20T20:54", affectedAssets: 37, rootCause: "Credential compromise" },
    { id: "INC-20260009", title: "SQL injection on portal", severity: "critical", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-05T08:20", affectedAssets: 32, rootCause: "Zero-day" },
    { id: "INC-20260010", title: "Brute force on VPN", severity: "high", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-08T22:12", affectedAssets: 1, rootCause: "Human error" },
    { id: "INC-20260011", title: "Supply chain alert from vendor", severity: "medium", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-23T07:46", affectedAssets: 21, rootCause: "Policy violation" },
    { id: "INC-20260012", title: "Suspicious lateral movement", severity: "low", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-08T00:10", affectedAssets: 28, rootCause: "Unknown" },
  ];
  private _vendor_risk_GetIncidentStats() {
    const inc = this._vendor_risk_Incidents;
    return { total: inc.length, open: inc.filter(i=>i.status==="open").length, investigating: inc.filter(i=>i.status==="investigating").length, mttd: 11, mttr: 74 };
  }
  private _vendor_risk_GetSeverityDistribution() {
    const inc = this._vendor_risk_Incidents;
  ...
  @state() private _vraAle: number = 0;
  @state() private _vraSroi: number = 0;
  @state() private _vraCpi: number = 0;
  @state() private _vraBudgetAlloc: number = 0;
  @state() private _vraCostBenefit: number = 0;

  // Security Economics Calculator
  private vraInitEconomics() {
    this._vraAle = Math.round(2850000 + Math.random() * 4500000);
    this._vraSroi = Math.round(180 + Math.random() * 320);
    this._vraCpi = Math.round(45000 + Math.random() * 120000);
    this._vraBudgetAlloc = Math.round(2500000 + Math.random() * 3000000);
    this._vraCostBenefit = Math.round(220 + Math.random() * 180);
  }

  private _vraCalcAle(): { annual: number; perIncident: number; byCategory: Array<{name: string; value: number}> } {
    const base = this._vraAle;
    const categories = [
  ...
  private _vraCalcSroi(): Array<{year: number; investment: number; savings: number; roi: number}> {
    const baseInv = this._vraSroi * 10000;
  ...
  private _vraGetBudgetAlloc(): Array<{category: string; amount: number; pct: number; trend: string}> {
    const total = this._vraBudgetAlloc;
    const items = [
  ...
  private _vraGetCostBenefit(): Array<{control: string; cost: number; benefit: number; ratio: number; priority: string}> {
    const base = this._vraCostBenefit;
    const controls = [
  ...
  private _vraRenderEconomics() {
    const ale = this._vraCalcAle();
    const roi = this._vraCalcSroi();
    const budget = this._vraGetBudgetAlloc();
    const cb = this._vraGetCostBenefit();
    const cpi = this._vraCpi;
  ...
  @state() private _vraThreatLevel: any = null;
  @state() private _vraEmergingThreats: any = null;
  @state() private _vraThreatTrends: any = null;
  @state() private _vraSectorRadar: any = null;
  @state() private _vraActorActivity: any = null;

  // Threat Landscape Intelligence
  private vraInitThreatIntel() {
    this._vraThreatLevel = {
  ...
  private _vraRenderThreatIntel() {
    const tl = this._vraThreatLevel;
    const et = this._vraEmergingThreats;
    const sr = this._vraSectorRadar;
    const aa = this._vraActorActivity;
  ...
  @state() private _vraPolicies: any = null;
  @state() private _vraExceptions: any = null;
  @state() private _vraRiskRegister: any = null;
  @state() private _vraMeetings: any = null;
  @state() private _vraDeadlines: any = null;

  // Security Governance Dashboard
  private vraInitGovernance() {
    this._vraPolicies = [
  ...
  private _vraRenderGovernance() {
    const policies = this._vraPolicies;
    const risks = this._vraRiskRegister;
    const deadlines = this._vraDeadlines;
  ...
  @state() private _vraCriticalAssets: any = null;
  @state() private _vraAssetDeps: any = null;
  @state() private _vraEolAssets: any = null;
  @state() private _vraAssetRisk: any = null;

  // Asset Intelligence
  private vraInitAssetIntel() {
    this._vraCriticalAssets = [
    ...
    this._vraAssetRisk = { critical: 7, high: 23, medium: 45, low: 128, total: 203 };
  }

  private _vraRenderAssetIntel() {
    const assets = this._vraCriticalAssets;
    const eol = this._vraEolAssets;
    const ar = this._vraAssetRisk;
  ...
  @state() private _vraUserBaseline: any = null;
  @state() private _vraAnomalyRules: any = null;
  @state() private _vraDataAccess: any = null;
  @state() private _vraInsiderRisk: any = null;
  ...
  private vraInitInsiderThreat() {
    this._vraUserBaseline = [
    ...
    this._vraInsiderRisk = { totalUsers: 2847, monitored: 342, flagged: 18, investigated: 5, confirmed: 1 };
  }

  private _vraRenderInsiderThreat() {
    const baseline = this._vraUserBaseline;
    const rules = this._vraAnomalyRules;
    const ir = this._vraInsiderRisk;
...
// ===== THREAT ACTOR PROFILE DATABASE (R22) =====
  ...
  private _renderBudgetPlanning() {
    const budgetData = [
      { category: "Personnel & Training", planned: 3942000, actual: 3237000, utilization: 82.1, q1: "22%", q2: "27%", q3: "25%", q4: "17%" },
      { category: "Tooling & Licensing", planned: 3949000, actual: 3250000, utilization: 82.3, q1: "25%", q2: "32%", q3: "20%", q4: "18%" },
      { category: "Infrastructure Security", planned: 3956000, actual: 3263000, utilization: 82.5, q1: "28%", q2: "21%", q3: "31%", q4: "19%" },
      { category: "Compliance & Audit", planned: 3963000, actual: 3276000, utilization: 82.7, q1: "15%", q2: "26%", q3: "26%", q4: "20%" },
      { category: "Incident Response", planned: 3970000, actual: 3289000, utilization: 82.8, q1: "18%", q2: "31%", q3: "21%", q4: "21%" },
      { category: "Third-Party Assessments", planned: 3977000, actual: 3302000, utilization: 83.0, q1: "21%", q2: "20%", q3: "32%", q4: "22%" },
      { category: "Security Awareness", planned: 3984000, actual: 3315000, utilization: 83.2, q1: "24%", q2: "25%", q3: "27%", q4: "23%" },
      { category: "Research & Innovation", planned: 3991000, actual: 3328000, utilization: 83.4, q1: "27%", q2: "30%", q3: "22%", q4: "24%" },
    ];
    ...
    const headcount = [
      { team: "SOC Tier 1", current: 15, target: 10, gap: 5, avgSalary: "156k" },
      { team: "SOC Tier 2", current: 7, target: 9, gap: 4, avgSalary: "185k" },
      { team: "Threat Intel", current: 16, target: 8, gap: 3, avgSalary: "103k" },
      { team: "Red Team", current: 8, target: 7, gap: 2, avgSalary: "132k" },
      { team: "GRC", current: 17, target: 6, gap: 1, avgSalary: "161k" },
      { team: "AppSec", current: 9, target: 5, gap: 0, avgSalary: "190k" },
      { team: "Cloud Sec", current: 18, target: 4, gap: 5, avgSalary: "108k" },
      { team: "Identity & Access", current: 10, target: 3, gap: 4, avgSalary: "137k" },
    ];
    const vendorSpend = [
      { vendor: "CrowdStrike", annual: "702k", contractEnd: "2026-12", renewalRisk: "Low", satisfaction: 5 },
      { vendor: "Palo Alto", annual: "733k", contractEnd: "2026-01", renewalRisk: "Medium", satisfaction: 4 },
      { vendor: "Splunk", annual: "764k", contractEnd: "2026-02", renewalRisk: "High", satisfaction: 3 },
      { vendor: "Qualys", annual: "795k", contractEnd: "2026-03", renewalRisk: "Low", satisfaction: 5 },
      { vendor: "Rapid7", annual: "75k", contractEnd: "2026-04", renewalRisk: "Medium", satisfaction: 4 },
      { vendor: "Mandiant", annual: "106k", contractEnd: "2026-05", renewalRisk: "High", satisfaction: 3 },
      { vendor: "Zscaler", annual: "137k", contractEnd: "2026-06", renewalRisk: "Low", satisfaction: 5 },
      { vendor: "Duo Security", annual: "168k", contractEnd: "2026-07", renewalRisk: "Medium", satisfaction: 4 },
    ];
    const roiProjections = [
      { area: "Threat Detection", investment: "420k", projectedReturn: "1776k", roiMultiple: "1.6x", confidence: 83 },
      { area: "Incident Reduction", investment: "463k", projectedReturn: "1823k", roiMultiple: "1.5x", confidence: 64 },
      { area: "Compliance Savings", investment: "506k", projectedReturn: "1870k", roiMultiple: "4.0x", confidence: 81 },
      { area: "Automation Gains", investment: "549k", projectedReturn: "1917k", roiMultiple: "3.9x", confidence: 62 },
      { area: "Risk Avoidance", investment: "592k", projectedReturn: "1964k", roiMultiple: "3.8x", confidence: 79 },
    ];
    return html`
  ...
  private _renderMetricsNormalization() {
    const kpiCatalog = [
      { id: "kpi-1", name: "MTTD", owner: "SOC", unit: "minutes", target: 96, current: 81, benchmark: 91, collection: "auto", frequency: "realtime" },
      { id: "kpi-2", name: "MTTR", owner: "GRC", unit: "%", target: 99, current: 88, benchmark: 68, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-3", name: "MTTC", owner: "AppSec", unit: "score", target: 72, current: 95, benchmark: 79, collection: "manual", frequency: "weekly" },
      { id: "kpi-4", name: "Vuln SLA Compliance", owner: "Cloud Sec", unit: "count", target: 75, current: 56, benchmark: 90, collection: "auto", frequency: "monthly" },
      { id: "kpi-5", name: "Patch Coverage", owner: "Identity", unit: "days", target: 78, current: 63, benchmark: 67, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-6", name: "Phishing Click Rate", owner: "Threat Intel", unit: "minutes", target: 81, current: 70, benchmark: 78, collection: "manual", frequency: "daily" },
      { id: "kpi-7", name: "Training Completion", owner: "Security Ops", unit: "%", target: 84, current: 77, benchmark: 89, collection: "auto", frequency: "weekly" },
      { id: "kpi-8", name: "Escalation Rate", owner: "Risk Mgmt", unit: "score", target: 87, current: 84, benchmark: 66, collection: "semi-auto", frequency: "monthly" },
      { id: "kpi-9", name: "False Positive Rate", owner: "SOC", unit: "count", target: 90, current: 91, benchmark: 77, collection: "manual", frequency: "realtime" },
      { id: "kpi-10", name: "Threat Intel Actionability", owner: "GRC", unit: "days", target: 93, current: 98, benchmark: 88, collection: "auto", frequency: "daily" },
      { id: "kpi-11", name: "Endpoint Compliance", owner: "AppSec", unit: "minutes", target: 96, current: 59, benchmark: 65, collection: "semi-auto", frequency: "weekly" },
      { id: "kpi-12", name: "Cloud Misconfig Score", owner: "Cloud Sec", unit: "%", target: 99, current: 66, benchmark: 76, collection: "manual", frequency: "monthly" },
      { id: "kpi-13", name: "Identity Anomaly Rate", owner: "Identity", unit: "score", target: 72, current: 73, benchmark: 87, collection: "auto", frequency: "realtime" },
      { id: "kpi-14", name: "DLP Events", owner: "Threat Intel", unit: "count", target: 75, current: 80, benchmark: 98, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-15", name: "Vendor Risk Avg", owner: "Security Ops", unit: "days", target: 78, current: 87, benchmark: 75, collection: "manual", frequency: "weekly" },
      { id: "kpi-16", name: "Compliance Audit Pass Rate", owner: "Risk Mgmt", unit: "minutes", target: 81, current: 94, benchmark: 86, collection: "auto", frequency: "monthly" },
      { id: "kpi-17", name: "Awareness Score", owner: "SOC", unit: "%", target: 84, current: 55, benchmark: 97, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-18", name: "SOC Utilization", owner: "GRC", unit: "score", target: 87, current: 62, benchmark: 74, collection: "manual", frequency: "daily" },
      { id: "kpi-19", name: "Automation Coverage", owner: "AppSec", unit: "count", target: 90, current: 69, benchmark: 85, collection: "auto", frequency: "weekly" },
      { id: "kpi-20", name: "Risk Register Currency", owner: "Cloud Sec", unit: "days", target: 93, current: 76, benchmark: 96, collection: "semi-auto", frequency: "monthly" },
    ];
    const benchmarkSources = [
      { source: "NIST CSF", mappedKPIs: 5, alignment: 98, lastReview: "2026-03-11", status: "aligned" },
      { source: "CIS Controls v8", mappedKPIs: 6, alignment: 76, lastReview: "2026-04-06", status: "partial" },
      { source: "ISO 27001:2022", mappedKPIs: 7, alignment: 93, lastReview: "2026-05-01", status: "reviewing" },
      { source: "PCI DSS 4.0", mappedKPIs: 8, alignment: 71, lastReview: "2026-06-24", status: "aligned" },
      { source: "SOC 2 Type II", mappedKPIs: 3, alignment: 88, lastReview: "2026-01-19", status: "partial" },
      { source: "MITRE ATT&CK", mappedKPIs: 4, alignment: 66, lastReview: "2026-02-14", status: "reviewing" },
      { source: "SANS Top 20", mappedKPIs: 5, alignment: 83, lastReview: "2026-03-09", status: "aligned" },
      { source: "OWASP Top 10", mappedKPIs: 6, alignment: 61, lastReview: "2026-04-04", status: "partial" },
    ];
  ...
  private _renderThreatHuntingCampaigns() {
    const campaigns = [
      { id: "HC-1001", name: "Lateral Movement Sweep", status: "active", hypothesis: "H1: Actors using pass-the-hash for lateral movement", leadHunter: "Alice Chen", findings: 6, startDate: "2026-03-03", endDate: null, effectiveness: 97 },
      { id: "HC-1002", name: "Credential Harvesting Hunt", status: "completed", hypothesis: "H2: Actors using web shells for persistence", leadHunter: "Bob Martinez", findings: 9, startDate: "2026-02-14", endDate: "2026-05-20", effectiveness: 57 },
      { id: "HC-1003", name: "Persistence Mechanism Audit", status: "planned", hypothesis: "H3: Actors using scheduled tasks for data theft", leadHunter: "Carol Wu", findings: 12, startDate: "2026-01-25", endDate: null, effectiveness: 76 },
      { id: "HC-1004", name: "C2 Beacon Detection", status: "in-review", hypothesis: "H4: Actors using DNS tunneling for C2 communication", leadHunter: "Dave Kim", findings: 15, startDate: "2026-04-08", endDate: null, effectiveness: 95 },
      { id: "HC-1005", name: "Data Exfiltration Patterns", status: "active", hypothesis: "H5: Actors using encrypted channels for privilege escalation", leadHunter: "Eve Johnson", findings: 18, startDate: "2026-03-19", endDate: null, effectiveness: 55 },
      { id: "HC-1006", name: "Privilege Escalation Scan", status: "completed", hypothesis: "H6: Actors using token impersonation for defense evasion", leadHunter: "Frank Liu", findings: 21, startDate: "2026-02-02", endDate: "2026-06-04", effectiveness: 74 },
      { id: "HC-1007", name: "Supply Chain Implant Hunt", status: "planned", hypothesis: "H7: Actors using poisoned images for initial access", leadHunter: "Grace Park", findings: 24, startDate: "2026-01-13", endDate: null, effectiveness: 93 },
      { id: "HC-1008", name: "Insider Threat Indicators", status: "in-review", hypothesis: "H8: Actors using legitimate tools for credential access", leadHunter: "Hector Silva", findings: 27, startDate: "2026-04-24", endDate: null, effectiveness: 53 },
      { id: "HC-1009", name: "Cloud Metadata Analysis", status: "active", hypothesis: "H9: Actors using API keys for command execution", leadHunter: "Alice Chen", findings: 30, startDate: "2026-03-07", endDate: null, effectiveness: 72 },
      { id: "HC-1010", name: "DNS Tunnel Detection", status: "completed", hypothesis: "H10: Actors using encoded subdomains for exfiltration", leadHunter: "Bob Martinez", findings: 33, startDate: "2026-02-18", endDate: "2026-04-16", effectiveness: 91 },
      { id: "HC-1011", name: "Fileless Malware Search", status: "planned", hypothesis: "H11: Actors using WMI providers for discovery", leadHunter: "Carol Wu", findings: 36, startDate: "2026-01-01", endDate: null, effectiveness: 51 },
      { id: "HC-1012", name: "Zero-Day Exploit Traces", status: "in-review", hypothesis: "H12: Actors using exploit kits for collection", leadHunter: "Dave Kim", findings: 39, startDate: "2026-04-12", endDate: null, effectiveness: 70 },
    ];
    const hunterLeaderboard = [
      { hunter: "Alice Chen", campaigns: 14, findings: 27, highSeverity: 24, avgScore: 69, streak: 7 },
      { hunter: "Bob Martinez", campaigns: 11, findings: 56, highSeverity: 3, avgScore: 62, streak: 8 },
      { hunter: "Carol Wu", campaigns: 8, findings: 85, highSeverity: 8, avgScore: 55, streak: 1 },
      { hunter: "Dave Kim", campaigns: 5, findings: 114, highSeverity: 13, avgScore: 92, streak: 2 },
      { hunter: "Eve Johnson", campaigns: 15, findings: 27, highSeverity: 18, avgScore: 85, streak: 3 },
      { hunter: "Frank Liu", campaigns: 12, findings: 56, highSeverity: 23, avgScore: 78, streak: 4 },
      { hunter: "Grace Park", campaigns: 9, findings: 85, highSeverity: 2, avgScore: 71, streak: 5 },
      { hunter: "Hector Silva", campaigns: 6, findings: 114, highSeverity: 7, avgScore: 64, streak: 6 },
    ];
    const mitreMapping = [
      { tactic: "Initial Access", techniques: 5, campaigns: 1, coverage: 55 },
      { tactic: "Execution", techniques: 4, campaigns: 6, coverage: 35 },
      { tactic: "Persistence", techniques: 3, campaigns: 5, coverage: 86 },
      { tactic: "Privilege Escalation", techniques: 2, campaigns: 4, coverage: 66 },
      { tactic: "Defense Evasion", techniques: 12, campaigns: 3, coverage: 46 },
      { tactic: "Credential Access", techniques: 11, campaigns: 2, coverage: 97 },
      { tactic: "Discovery", techniques: 10, campaigns: 1, coverage: 77 },
      { tactic: "Lateral Movement", techniques: 9, campaigns: 6, coverage: 57 },
      { tactic: "Collection", techniques: 8, campaigns: 5, coverage: 37 },
      { tactic: "Exfiltration", techniques: 7, campaigns: 4, coverage: 88 },
      { tactic: "Command & Control", techniques: 6, campaigns: 3, coverage: 68 },
      { tactic: "Impact", techniques: 5, campaigns: 2, coverage: 48 },
    ];
    return html`
  ...
  private _renderControlInventory() {
    const controls = [
      { id: "CTL-2001", name: "MFA Enforcement", domain: "Access Control", status: "implemented", effectiveness: 94, lastTest: "2026-03-23", nextReview: "2026-11-23", owner: "SOC", risk: "Low" },
      { id: "CTL-2002", name: "Network Segmentation", domain: "Network Security", status: "partial", effectiveness: 44, lastTest: "2026-02-08", nextReview: "2026-12-14", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2003", name: "EDR Deployment", domain: "Endpoint Protection", status: "planned", effectiveness: 0, lastTest: "2026-01-21", nextReview: "2026-05-05", owner: "IT Ops", risk: "High" },
      { id: "CTL-2004", name: "DLP Policy", domain: "Data Protection", status: "gap", effectiveness: 7, lastTest: "2026-04-06", nextReview: "2026-06-24", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2005", name: "SSO Integration", domain: "Identity Management", status: "implemented", effectiveness: 47, lastTest: "2026-03-19", nextReview: "2026-07-15", owner: "IAM", risk: "Low" },
      { id: "CTL-2006", name: "SAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 21, lastTest: "2026-02-04", nextReview: "2026-08-06", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2007", name: "CSPM Scanning", domain: "Cloud Security", status: "planned", effectiveness: 28, lastTest: "2026-01-17", nextReview: "2026-09-25", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2008", name: "Badge Access", domain: "Physical Security", status: "gap", effectiveness: 35, lastTest: "2026-04-02", nextReview: "2026-10-16", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2009", name: "Least Privilege", domain: "Access Control", status: "implemented", effectiveness: 59, lastTest: "2026-03-15", nextReview: "2026-11-07", owner: "SOC", risk: "Low" },
      { id: "CTL-2010", name: "Firewall Rules", domain: "Network Security", status: "partial", effectiveness: 49, lastTest: "2026-02-28", nextReview: "2026-12-26", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2011", name: "Disk Encryption", domain: "Endpoint Protection", status: "planned", effectiveness: 5, lastTest: "2026-01-13", nextReview: "2026-05-17", owner: "IT Ops", risk: "High" },
      { id: "CTL-2012", name: "Data Classification", domain: "Data Protection", status: "gap", effectiveness: 12, lastTest: "2026-04-26", nextReview: "2026-06-08", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2013", name: "PAM Implementation", domain: "Identity Management", status: "implemented", effectiveness: 71, lastTest: "2026-03-11", nextReview: "2026-07-27", owner: "IAM", risk: "Low" },
      { id: "CTL-2014", name: "DAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 26, lastTest: "2026-02-24", nextReview: "2026-08-18", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2015", name: "IAM Policy Review", domain: "Cloud Security", status: "planned", effectiveness: 33, lastTest: "2026-01-09", nextReview: "2026-09-09", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2016", name: "Visitor Management", domain: "Physical Security", status: "gap", effectiveness: 40, lastTest: "2026-04-22", nextReview: "2026-10-28", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2017", name: "Access Reviews", domain: "Access Control", status: "implemented", effectiveness: 83, lastTest: "2026-03-07", nextReview: "2026-11-19", owner: "SOC", risk: "Low" },
      { id: "CTL-2018", name: "IDS/IPS Tuning", domain: "Network Security", status: "partial", effectiveness: 3, lastTest: "2026-02-20", nextReview: "2026-12-10", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2019", name: "Patch Management", domain: "Endpoint Protection", status: "planned", effectiveness: 10, lastTest: "2026-01-05", nextReview: "2026-05-01", owner: "IT Ops", risk: "High" },
      { id: "CTL-2020", name: "Backup Encryption", domain: "Data Protection", status: "gap", effectiveness: 17, lastTest: "2026-04-18", nextReview: "2026-06-20", owner: "Data Gov", risk: "Critical" },
      { id: "CTL-2021", name: "Password Policy", domain: "Identity Management", status: "implemented", effectiveness: 95, lastTest: "2026-03-03", nextReview: "2026-07-11", owner: "IAM", risk: "Low" },
      { id: "CTL-2022", name: "Container Scanning", domain: "Application Security", status: "partial", effectiveness: 31, lastTest: "2026-02-16", nextReview: "2026-08-02", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2023", name: "WAF Configuration", domain: "Cloud Security", status: "planned", effectiveness: 38, lastTest: "2026-01-01", nextReview: "2026-09-21", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2024", name: "CCTV Coverage", domain: "Physical Security", status: "gap", effectiveness: 45, lastTest: "2026-04-14", nextReview: "2026-10-12", owner: "Facilities", risk: "Critical" },
      { id: "CTL-2025", name: "RBAC Enforcement", domain: "Access Control", status: "implemented", effectiveness: 48, lastTest: "2026-03-27", nextReview: "2026-11-03", owner: "SOC", risk: "Low" },
      { id: "CTL-2026", name: "VPN Management", domain: "Network Security", status: "partial", effectiveness: 8, lastTest: "2026-02-12", nextReview: "2026-12-22", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2027", name: "App Whitelisting", domain: "Endpoint Protection", status: "planned", effectiveness: 15, lastTest: "2026-01-25", nextReview: "2026-05-13", owner: "IT Ops", risk: "High" },
      { id: "CTL-2028", name: "Key Management", domain: "Data Protection", status: "gap", effectiveness: 22, lastTest: "2026-04-10", nextReview: "2026-06-04", owner: "Data Gov", risk: "Critical" },
    ];
    const gapAnalysis = [
  ...
  private _renderIncidentCostTracker() {
    const incidents = [
      { id: "INC-7001", name: "Security Incident 1", severity: "Critical", totalCost: 1382000, responseCost: 400780, recoveryCost: 469880, legalCost: 152020, regulatoryCost: 82920, insuranceClaim: 0, avoidedCost: 459000, date: "2026-03-15" },
      { id: "INC-7002", name: "Security Incident 2", severity: "High", totalCost: 1385000, responseCost: 207750, recoveryCost: 332400, legalCost: 110800, regulatoryCost: 166200, insuranceClaim: 969500, avoidedCost: 488000, date: "2026-02-24" },
      { id: "INC-7003", name: "Security Incident 3", severity: "Medium", totalCost: 1388000, responseCost: 305360, recoveryCost: 485800, legalCost: 69400, regulatoryCost: 97160, insuranceClaim: 0, avoidedCost: 21000, date: "2026-01-05" },
      { id: "INC-7004", name: "Security Incident 4", severity: "Low", totalCost: 1391000, responseCost: 403390, recoveryCost: 347750, legalCost: 250380, regulatoryCost: 180830, insuranceClaim: 904150, avoidedCost: 50000, date: "2026-04-14" },
      { id: "INC-7005", name: "Security Incident 5", severity: "Critical", totalCost: 1394000, responseCost: 209100, recoveryCost: 501840, legalCost: 209100, regulatoryCost: 111520, insuranceClaim: 0, avoidedCost: 79000, date: "2026-03-23" },
      { id: "INC-7006", name: "Security Incident 6", severity: "High", totalCost: 1397000, responseCost: 307340, recoveryCost: 363220, legalCost: 167640, regulatoryCost: 195580, insuranceClaim: 838200, avoidedCost: 108000, date: "2026-02-04" },
      { id: "INC-7007", name: "Security Incident 7", severity: "Medium", totalCost: 1400000, responseCost: 406000, recoveryCost: 518000, legalCost: 126000, regulatoryCost: 126000, insuranceClaim: 0, avoidedCost: 137000, date: "2026-01-13" },
      { id: "INC-7008", name: "Security Incident 8", severity: "Low", totalCost: 1403000, responseCost: 210450, recoveryCost: 378810, legalCost: 84180, regulatoryCost: 210450, insuranceClaim: 771650, avoidedCost: 166000, date: "2026-04-22" },
      { id: "INC-7009", name: "Security Incident 9", severity: "Critical", totalCost: 1406000, responseCost: 309320, recoveryCost: 534280, legalCost: 267140, regulatoryCost: 140600, insuranceClaim: 0, avoidedCost: 195000, date: "2026-03-03" },
      { id: "INC-7010", name: "Security Incident 10", severity: "High", totalCost: 1409000, responseCost: 408610, recoveryCost: 394520, legalCost: 225440, regulatoryCost: 70450, insuranceClaim: 704500, avoidedCost: 224000, date: "2026-02-12" },
      { id: "INC-7011", name: "Security Incident 11", severity: "Medium", totalCost: 1412000, responseCost: 211800, recoveryCost: 550680, legalCost: 183560, regulatoryCost: 155320, insuranceClaim: 0, avoidedCost: 253000, date: "2026-01-21" },
      { id: "INC-7012", name: "Security Incident 12", severity: "Low", totalCost: 1415000, responseCost: 311300, recoveryCost: 410350, legalCost: 141500, regulatoryCost: 84900, insuranceClaim: 636750, avoidedCost: 282000, date: "2026-04-02" },
    ];
    const yearlyTrend = [
      { month: "Jan", incidents: 3, totalCost: "195k", avgCost: "27k", insured: 37 },
      { month: "Feb", incidents: 11, totalCost: "238k", avgCost: "74k", insured: 37 },
      { month: "Mar", incidents: 8, totalCost: "281k", avgCost: "121k", insured: 37 },
      { month: "Apr", incidents: 5, totalCost: "324k", avgCost: "168k", insured: 37 },
      { month: "May", incidents: 2, totalCost: "367k", avgCost: "34k", insured: 37 },
      { month: "Jun", incidents: 10, totalCost: "410k", avgCost: "81k", insured: 37 },
    ];
