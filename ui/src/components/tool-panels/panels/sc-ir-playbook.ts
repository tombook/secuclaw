...
@customElement('sc-ir-playbook')
export class ScIrPlaybook extends LitElement {
  static styles = css`
...
@state() private _tablePage = 1;
  ...
  @state() private _comments: CommentEntry[] = [
    { id: 'c1', author: 'soc-tier1', timestamp: '2026-04-23 04:00', text: 'Initial assessment started for Ir Playbook finding #1' },
  ...
  private _items: PanelItem[] = [
      {
            ...
            "category": "Security",
            "source": "SIEM",
            "description": "Steps: 8 | Current step: 4 (Containment) | Elapsed: 45min | Analyst: Li Wei",
      ...
      },
      {
            ...
            "category": "Network",
            "source": "EDR",
            "description": "Steps: 6 | Status: Ready | Trigger: Phishing alert + user report | Last activated: 3d ago",
      ...
      },
      {
            ...
            "category": "Access",
            "source": "Scanner",
            "description": "Steps: 7 | Status: Ready | Legal review: Complete | Regulatory: 72h GDPR notification",
      ...
      },
      {
            ...
            "category": "Compliance",
            "source": "Audit",
            "description": "Steps: 5 | Status: Ready | Cloudflare integration tested | Last drill: 30d ago",
      ...
      },
      {
            ...
            "category": "Operations",
            "source": "Manual",
            "description": "Steps: 9 | Status: Ready | Forensics toolkit: Updated | Legal liaison: On call",
      ...
      {
            "id": "ir-playbook-6",
            "title": "Ir Playbook Finding #6",
            "severity": "high",
            "status": "open",
            "category": "Security",
            "source": "SIEM",
            "description": "Automated ir playbook assessment finding detected during routine scan of infrastructure component. Review and remediation recommended.",
  ...
  private _history: HistoryEntry[] = [
    { timestamp: '2026-04-23 04:15', action: 'Auto-correlated', user: 'system', details: 'Grouped related findings from Ir Playbook analysis' },
    { timestamp: '2026-04-23 03:00', action: 'Created', user: 'scanner', details: 'New Ir Playbook finding detected by automated assessment' },
    ...
    { timestamp: '2026-04-22 22:00', action: 'Resolved', user: 'security-eng', details: 'Remediation applied and verified for Ir Playbook finding' },
    { timestamp: '2026-04-22 18:00', action: 'Created', user: 'audit', details: 'Compliance audit identified Ir Playbook gap requiring remediation' },
    ...
    { timestamp: '2026-04-21 16:00', action: 'Resolved', user: 'soc-tier1', details: 'False positive confirmed - benign activity flagged by Ir Playbook scan' },
    { timestamp: '2026-04-21 12:00', action: 'Exported', user: 'compliance', details: 'Exported Ir Playbook findings for Q1 compliance report' },
  ];
...
private _workflowTasks: WorkflowTask[] = [
    { id: 'wt-1', title: 'Review Ir Playbook finding #1', status: 'active', assignee: 'soc-tier1', blockedBy: [], slaDeadline: '2026-04-23T12:00:00Z', priority: 'p1', createdAt: '2026-04-23T00:00:00Z', completedAt: null },
    ...
    { id: 'wt-3', title: 'Update Ir Playbook detection rules', status: 'blocked', assignee: 'soc-tier2', blockedBy: ['wt-2'], slaDeadline: '2026-04-24T00:00:00Z', priority: 'p2', createdAt: '2026-04-23T03:00:00Z', completedAt: null },
    ...
    { id: 'wt-8', title: 'Archive resolved Ir Playbook findings', status: 'failed', assignee: 'soc-tier1', blockedBy: [], slaDeadline: '2026-04-23T08:00:00Z', priority: 'p4', createdAt: '2026-04-22T22:00:00Z', completedAt: null },
  ];

private _executionHistory: ExecutionRecord[] = [
    {
      id: 'exec-1', name: 'Ir Playbook Full Assessment Run', startedAt: '2026-04-22T10:00:00Z', completedAt: '2026-04-22T10:12:00Z', status: 'success',
      steps: [
    ...
    {
      id: 'exec-2', name: 'Ir Playbook Delta Scan', startedAt: '2026-04-23T02:00:00Z', completedAt: '2026-04-23T02:05:00Z', status: 'failed',
      steps: [
  ...
  private _runExecution() {
    ...
    const exec: ExecutionRecord = {
      id: 'exec-' + Date.now(), name: 'Ir Playbook Assessment ' + new Date().toISOString().slice(0, 10), startedAt: new Date().toISOString(), completedAt: null, status: 'running', steps,
    };
  ...
  private _renderExecutionPanel(): unknown {
    ...
    return html`
      <div class="form-section">
        ...
        ` : html`<div class="empty-state">Click "Run Assessment" to start the Ir Playbook analysis pipeline</div>`}
      </div>
      <div class="form-section">
        ...
        <div class="settings-card">
          ...
          <div style="display:flex;gap:8px;margin-top:4px;">
            <button class="btn primary" @click=${() => {
              ...
              const a = document.createElement('a'); a.href = url; a.download = 'ir-playbook-config.json'; a.click();
    ...
    return html`
      <div style="margin-top:12px;">
        ...
        <div style="display:flex;gap:6px;margin-top:8px;">
          <input class="search-box" type="text" placeholder="Add a comment..." style="min-width:auto;flex:1;" id="ir-playbook-comment-input" />
          <button class="btn primary" @click=${() => {
            const input = this.shadowRoot?.querySelector('#ir-playbook-comment-input') as HTMLInputElement;
  ...
  private _exportJSON() {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'sc-ir-playbook-data.json'; a.click();
    URL.revokeObjectURL(url);
  }
  ...
  private _exportCSV() {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'sc-ir-playbook-data.csv'; a.click();
    URL.revokeObjectURL(url);
  }


  // --- Domain Rules Engine ---
  @state() private _irpRules: { id: string; name: string; category: string; severity: Severity; enabled: boolean; lastEval: string; passRate: number }[] = [];
  private _initIrpRules() {
    const rules = [
    ...
    this._irpRules = rules;
  }
  private _evaluateIrpRules(): { passed: number; failed: number; skipped: number; total: number } {
    let passed = 0, failed = 0, skipped = 0;
    this._irpRules.forEach(r => { if (!r.enabled) { skipped++; } else if (r.passRate >= 80) { passed++; } else { failed++; } });
    return { passed, failed, skipped, total: this._irpRules.length };
  }

  // --- CVSS Scoring ---
  @state() private _irpcvssData: { itemId: string; vector: string; base: number; temporal: number; environmental: number; overall: number }[] = [];
  private _initIrpCvss() {
    ...
    this._irpcvssData = vectors.map((v, i) => {
  ...
  // --- Anomaly Detection ---
  @state() private _irpanomalies: { id: string; type: string; severity: Severity; description: string; detected: string; confidence: number; affected: string[] }[] = [];
  private _runIrpAnomalyDetection() {
    const types = [
    ...
    this._irpanomalies = types.map((a, i) => ({
  ...
  // --- Trend Prediction ---
  @state() private _irppredictions: { horizon: string; metric: string; current: number; predicted: number; direction: 'up' | 'down' | 'stable'; confidence: number }[] = [];
  private _generateIrpPredictions() {
    this._irppredictions = [
  ...
  // --- Approval Workflow ---
  @state() private _irpApprovals: { id: string; title: string; requester: string; status: 'pending' | 'approved' | 'rejected' | 'expired'; createdAt: string; priority: Priority; type: string }[] = [];
  private _initIrpApprovals() {
    this._irpApprovals = [
  ...
  private _approveIrpItem(id: string) { const item = this._irpApprovals.find(a => a.id === id); if (item) item.status = 'approved'; this.requestUpdate(); }
  private _rejectIrpItem(id: string) { const item = this._irpApprovals.find(a => a.id === id); if (item) item.status = 'rejected'; this.requestUpdate(); }

  // --- Activity Feed ---
  @state() private _irpActivity: { id: string; action: string; user: string; target: string; timestamp: string }[] = [];
  private _initIrpActivity() {
    const actions = [
    ...
    this._irpActivity = actions.map((a, i) => ({ id: 'ACT-' + String(i + 1).padStart(3, '0'), ...a, timestamp: new Date(Date.now() - i * 3600000).toISOString() }));
  }

  // --- Notification System ---
  @state() private _irpNotifications: { id: string; message: string; type: 'info' | 'warning' | 'error' | 'success'; read: boolean; timestamp: string }[] = [];
  private _initIrpNotifications() {
    this._irpNotifications = [
  ...
  private _markIrpNotifRead(id: string) { const n = this._irpNotifications.find(x => x.id === id); if (n) n.read = true; this.requestUpdate(); }

  // --- Panel Configuration ---
  @state() private _irpConfig: { layout: 'compact' | 'default' | 'expanded'; theme: 'dark' | 'midnight' | 'slate'; showAnomalies: boolean; showPredictions: boolean; autoRefresh: boolean; refreshInterval: number } = {
    layout: 'default', theme: 'dark', showAnomalies: true, showPredictions: true, autoRefresh: true, refreshInterval: 60,
  };
  private _irpPresets: { name: string; config: typeof this._irpConfig }[] = [
  ...
  private _applyIrpPreset(preset: typeof this._irpPresets[0]) { this._irpConfig = { ...preset.config }; this.requestUpdate(); }

  private _renderIrpTreemapSVG(): string {
    const categories = [
  ...
  private _renderIrpSankeySVG(): string {
  ...
  // --- Render: Rules Engine ---
  private _renderIrpRules(): any {
    const ev = this._evaluateIrpRules();
    return html`
  ...
  private _renderIrpAnomalies(): any {
  ...
  // --- Render: Approvals ---
  private _renderIrpApprovals(): any {
          ...
          $${this._irpApprovals.map(a => html`
            <div style="padding:6px 8px;background:#1f2937;border-radius:4px">
              ...
              $${a.status === 'pending' ? html`
                <div style="display:flex;gap:4px;margin-top:4px">
                  <button class="btn success" style="padding:2px 8px;font-size:9px" @click=$${() => this._approveIrpItem(a.id)}>Approve</button>
                  <button class="btn error" style="padding:2px 8px;font-size:9px" @click=$${() => this._rejectIrpItem(a.id)}>Reject</button>
                </div>` : nothing}
            </div>`)}
        </div>
      </div>`;
  ...
  private _renderIrpNotifications(): any {
    const tc = (t: string) => t === 'error' ? '#ef4444' : t === 'warning' ? '#eab308' : t === 'success' ? '#22c55e' : '#3b82f6';
    const unread = this._irpNotifications.filter(n => !n.read).length;
    return html`
      <div style="background:#1a1d27;border-radius:8px;padding:12px;margin-top:12px">
        ...
        <div style="display:flex;flex-direction:column;gap:4px;max-height:120px;overflow-y:auto">
          $${this._irpNotifications.map(n => html`
            <div style="display:flex;align-items:center;gap:8px;padding:4px 8px;background:$${n.read ? '#1f2937' : '#252a36'};border-radius:4px;font-size:10px;opacity:$${n.read ? '0.6' : '1'};cursor:pointer" @click=$${() => this._markIrpNotifRead(n.id)}>
      ...
      <div style="background:#1a1d27;border-radius:8px;padding:12px;margin-top:12px">
        ...
        <div style="display:flex;flex-direction:column;gap:6px;font-size:10px">
          <div style="display:flex;align-items:center;gap:8px">
            ...
            <select class="form-input" style="flex:1" @change=$${(e: Event) => { this._irpConfig.layout = (e.target as HTMLSelectElement).value as any; this.requestUpdate(); }}>
              <option value="compact" ?selected=$${this._irpConfig.layout === 'compact'}>Compact</option>
              <option value="default" ?selected=$${this._irpConfig.layout === 'default'}>Default</option>
              <option value="expanded" ?selected=$${this._irpConfig.layout === 'expanded'}>Expanded</option>
            </select>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:100px">Theme</span>
            <select class="form-input" style="flex:1" @change=$${(e: Event) => { this._irpConfig.theme = (e.target as HTMLSelectElement).value as any; this.requestUpdate(); }}>
              <option value="dark" ?selected=$${this._irpConfig.theme === 'dark'}>Dark</option>
              <option value="midnight" ?selected=$${this._irpConfig.theme === 'midnight'}>Midnight</option>
              <option value="slate" ?selected=$${this._irpConfig.theme === 'slate'}>Slate</option>
            </select>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:100px">Auto Refresh</span>
            <input type="checkbox" ?checked=$${this._irpConfig.autoRefresh} @change=$${() => { this._irpConfig.autoRefresh = !this._irpConfig.autoRefresh; this.requestUpdate(); }}/>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:100px">Show Anomalies</span>
            <input type="checkbox" ?checked=$${this._irpConfig.showAnomalies} @change=$${() => { this._irpConfig.showAnomalies = !this._irpConfig.showAnomalies; this.requestUpdate(); }}/>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:100px">Show Predictions</span>
            <input type="checkbox" ?checked=$${this._irpConfig.showPredictions} @change=$${() => { this._irpConfig.showPredictions = !this._irpConfig.showPredictions; this.requestUpdate(); }}/>
          </div>
          <div style="margin-top:6px;font-weight:600">Presets</div>
          <div style="display:flex;gap:4px">
            $${this._irpPresets.map(ps => html`<button class="btn" style="padding:2px 8px;font-size:9px" @click=$${() => this._applyIrpPreset(ps)}>$${ps.name}</button>`)}
          </div>
        </div>
      </div>`;
  }
...
@state() private _spmNetworkSegment9 = { id: "NET-009", name: "Segment 9", subnet: "10.9.0.0/16", criticality: "high", deviceCount: 37, vulnerabilityCount: 3, lastScan: "2026-01-19" } as any;
  ...
  @state() private _irPlaybookContainerScans: Array<{image:string;registry:string;critical:number;high:number;medium:number;scanDate:string;status:string}> = [
  ...
  @state() private _irPlaybookK8sPods: Array<{namespace:string;pod:string;securityContext:string;hostIPC:boolean;hostPID:boolean;privileged:boolean;riskLevel:string}> = [
  ...
  @state() private _irPlaybookServerlessRisk: Array<{function:string;runtime:string;timeout:number;iamPerms:string;externalCalls:number;riskScore:number}> = [
  ...
  @state() private _irPlaybookRuntimeAlerts: Array<{id:string;workload:string;alertType:string;severity:string;description:string;timestamp:string}> = [
  ...
  private _renderIrplaybookCloudWl(): TemplateResult {
    const containers = this._irPlaybookContainerScans;
    const pods = this._irPlaybookK8sPods;
    const alerts = this._irPlaybookRuntimeAlerts;
    return html`
  ...
  @state() private _irPlaybookZones: Array<{id:string;name:string;trustLevel:number;subnet:string;devices:number;policy:string;lastAudit:string}> = [
  ...
  @state() private _irPlaybookSegRules: Array<{id:string;source:string;dest:string;action:string;protocol:string;port:string;status:string;hits:number}> = [
  ...
  @state() private _irPlaybookCrossZoneTraffic: Array<{source:string;dest:string;bytes:number;sessions:number;violations:number}> = [
  ...
  @state() private _irPlaybookMicroSegGaps: Array<{id:string;zone:string;gapType:string;severity:string;recommendation:string}> = [
  ...
  private _renderIrplaybookNetworkSeg(): TemplateResult {
    const zones = this._irPlaybookZones;
    const gaps = this._irPlaybookMicroSegGaps;
    return html`
  ...
  @state() private _irPlaybookCorrRules: Array<{id:string;name:string;sources:string[];logic:string;severity:string;active:boolean;lastTriggered:string}> = [
  ...
  @state() private _irPlaybookEventTimeline: Array<{timestamp:string;source:string;eventType:string;details:string;correlated:boolean}> = [
  ...
  @state() private _irPlaybookFalsePosMetrics: {totalEvents:number;correlatedEvents:number;falsePositives:number;fpRate:number;topFpRules:string[]} = {
  ...
  @state() private _irPlaybookEventPatterns: Array<{id:string;pattern:string;frequency:number;firstSeen:string;lastSeen:string;status:string}> = [
  ...
  private _renderIrplaybookEventCorr(): TemplateResult {
    const rules = this._irPlaybookCorrRules;
    const timeline = this._irPlaybookEventTimeline;
    const fpMetrics = this._irPlaybookFalsePosMetrics;
    return html`
  ...
  @state() private _irPlaybookCourses: Array<{id:string;title:string;category:string;duration:number;enrolled:number;completed:number;difficulty:string;rating:number}> = [
  ...
  @state() private _irPlaybookLearningPaths: Array<{id:string;name:string;courseIds:string[];progress:number;enrolled:number}> = [
  ...
  @state() private _irPlaybookDeptCompliance: Array<{dept:string;trainedPct:number;targetPct:number;avgScore:number;certCount:number}> = [
  ...
  @state() private _irPlaybookSkillsGaps: Array<{skill:string;current:number;required:number;gap:number;priority:string}> = [
  ...
  private _renderIrplaybookTraining(): TemplateResult {
    const courses = this._irPlaybookCourses;
    const deptComp = this._irPlaybookDeptCompliance;
    return html`
  ...
  @state() private _irPlaybookThreatIntel: Array<{source:string;indicator:string;type:string;confidence:number;severity:string;observed:string;description:string}> = [
  ...
  @state() private _irPlaybookIntelMetrics: {totalIndicators:number;criticalCount:number;highCount:number;mediumCount:number;staleIndicators:number;feedUptime:number} = {
    totalIndicators: 45230, criticalCount: 234, highCount: 1892, mediumCount: 8765, staleIndicators: 567, feedUptime: 99.7
  };
  @state() private _irPlaybookIntelTrending: Array<{category:string;thisWeek:number;lastWeek:number;change:number}> = [
  ...
  private _renderIrplaybookThreatIntel(): TemplateResult {
    const intel = this._irPlaybookThreatIntel;
    const trending = this._irPlaybookIntelTrending;
    const metrics = this._irPlaybookIntelMetrics;
    return html`
  ...
  @state() private _complianceCalendarEvents: Array<{id: string; title: string; deadline: string; category: string; status: string; assignee: string; notes: string}> = [
    {id: 'cc-ir-playbook-001', title: 'SOX Section 404 Compliance Review', deadline: '2026-06-30', category: 'Regulatory', status: 'In Progress', assignee: 'Compliance Team', notes: 'Annual internal control assessment for financial reporting'},
    {id: 'cc-ir-playbook-002', title: 'GDPR Data Protection Impact Assessment', deadline: '2026-05-15', category: 'Privacy', status: 'Pending', assignee: 'DPO Office', notes: 'Quarterly DPIA for high-risk processing activities'},
    {id: 'cc-ir-playbook-003', title: 'ISO 27001 Surveillance Audit', deadline: '2026-07-20', category: 'Certification', status: 'Scheduled', assignee: 'QA Lead', notes: 'Stage 2 surveillance audit for ISMS certification renewal'},
    {id: 'cc-ir-playbook-004', title: 'PCI DSS v4.0 Gap Analysis', deadline: '2026-05-30', category: 'Payment', status: 'Not Started', assignee: 'Security Architect', notes: 'Assess current state against PCI DSS v4.0 requirements'},
    {id: 'cc-ir-playbook-005', title: 'NIST CSF Self-Assessment', deadline: '2026-08-15', category: 'Framework', status: 'Pending', assignee: 'CISO', notes: 'Biannual self-assessment against NIST Cybersecurity Framework'},
    {id: 'cc-ir-playbook-006', title: 'HIPAA Security Rule Audit', deadline: '2026-06-15', category: 'Healthcare', status: 'In Progress', assignee: 'Compliance Analyst', notes: 'Annual audit of administrative, physical, and technical safeguards'},
    {id: 'cc-ir-playbook-007', title: 'SOC 2 Type II Report Renewal', deadline: '2026-09-30', category: 'Audit', status: 'Scheduled', assignee: 'External Auditor', notes: 'Engage auditor for SOC 2 Type II examination period'},
    {id: 'cc-ir-playbook-008', title: 'FedRAMP Authorization Review', deadline: '2026-07-01', category: 'Government', status: 'Pending', assignee: 'FedRAMP PMO', notes: 'Review and update security authorization package'},
    {id: 'cc-ir-playbook-009', title: 'Annual Penetration Test', deadline: '2026-10-15', category: 'Testing', status: 'Not Started', assignee: 'Red Team Lead', notes: 'Comprehensive external and internal penetration testing engagement'},
    {id: 'cc-ir-playbook-010', title: 'Security Awareness Training Rollout', deadline: '2026-05-01', category: 'Training', status: 'Completed', assignee: 'Security Awareness Manager', notes: 'Q2 organization-wide security awareness training program'},
    {id: 'cc-ir-playbook-011', title: 'Vendor Security Review Cycle', deadline: '2026-06-01', category: 'Third Party', status: 'In Progress', assignee: 'Vendor Manager', notes: 'Quarterly review of critical vendor security posture and SLA compliance'},
    {id: 'cc-ir-playbook-012', title: 'Incident Response Plan Update', deadline: '2026-05-20', category: 'Operations', status: 'Pending', assignee: 'IR Team Lead', notes: 'Update IR procedures based on lessons learned from recent incidents'},
  ];
  ...
  @state() private _dataQualityMetrics: Array<{id: string; dataSource: string; completenessScore: number; accuracyScore: number; timelinessScore: number; consistencyScore: number; reliabilityRank: number; lastUpdated: string; issues: string[]; trend: string}> = [
    {id: 'dq-ir-playbook-001', dataSource: 'Vulnerability Scanner Feed', completenessScore: 94, accuracyScore: 91, timelinessScore: 88, consistencyScore: 95, reliabilityRank: 1, lastUpdated: '2026-04-23T08:00:00Z', issues: ['Minor delay in Nessus plugin updates'], trend: 'improving'},
    {id: 'dq-ir-playbook-002', dataSource: 'SIEM Event Logs', completenessScore: 87, accuracyScore: 93, timelinessScore: 96, consistencyScore: 89, reliabilityRank: 2, lastUpdated: '2026-04-23T07:30:00Z', issues: ['Some log sources showing gaps in off-hours'], trend: 'stable'},
    {id: 'dq-ir-playbook-003', dataSource: 'Asset Inventory Database', completenessScore: 82, accuracyScore: 78, timelinessScore: 75, consistencyScore: 84, reliabilityRank: 4, lastUpdated: '2026-04-22T18:00:00Z', issues: ['Shadow IT devices not fully cataloged', 'Decommissioned assets still listed'], trend: 'declining'},
    {id: 'dq-ir-playbook-004', dataSource: 'Threat Intelligence Platform', completenessScore: 91, accuracyScore: 89, timelinessScore: 92, consistencyScore: 87, reliabilityRank: 3, lastUpdated: '2026-04-23T06:00:00Z', issues: ['Some IOC sources lack confidence scoring'], trend: 'improving'},
    {id: 'dq-ir-playbook-005', dataSource: 'Identity Provider Logs', completenessScore: 96, accuracyScore: 97, timelinessScore: 98, consistencyScore: 96, reliabilityRank: 1, lastUpdated: '2026-04-23T09:00:00Z', issues: [], trend: 'stable'},
    {id: 'dq-ir-playbook-006', dataSource: 'Cloud Security Posture Data', completenessScore: 79, accuracyScore: 85, timelinessScore: 82, consistencyScore: 81, reliabilityRank: 5, lastUpdated: '2026-04-23T05:00:00Z', issues: ['Multi-cloud coverage gaps', 'API rate limiting affects scan frequency'], trend: 'declining'},
    {id: 'dq-ir-playbook-007', dataSource: 'Compliance Evidence Store', completenessScore: 88, accuracyScore: 86, timelinessScore: 80, consistencyScore: 83, reliabilityRank: 4, lastUpdated: '2026-04-21T14:00:00Z', issues: ['Manual evidence collection delays', 'Inconsistent formatting across frameworks'], trend: 'stable'},
    {id: 'dq-ir-playbook-008', dataSource: 'Network Flow Data', completenessScore: 92, accuracyScore: 90, timelinessScore: 94, consistencyScore: 91, reliabilityRank: 2, lastUpdated: '2026-04-23T08:30:00Z', issues: ['Encrypted traffic classification accuracy needs improvement'], trend: 'improving'},
  ];
  ...
  @state() private _workflowTemplates: Array<{id: string; name: string; description: string; category: string; steps: number; avgDuration: string; successRate: number; lastRun: string; manualInterventions: number; optimizationScore: number}> = [
    {id: 'wf-ir-playbook-001', name: 'Vulnerability Triage Pipeline', description: 'Automated vulnerability assessment, prioritization, and assignment workflow', category: 'Vulnerability Management', steps: 8, avgDuration: '12 min', successRate: 94.5, lastRun: '2026-04-23T06:00:00Z', manualInterventions: 2, optimizationScore: 87},
    {id: 'wf-ir-playbook-002', name: 'Incident Response Orchestration', description: 'End-to-end IR workflow from detection to containment and recovery', category: 'Incident Response', steps: 15, avgDuration: '45 min', successRate: 89.2, lastRun: '2026-04-22T14:30:00Z', manualInterventions: 5, optimizationScore: 72},
    {id: 'wf-ir-playbook-003', name: 'Access Request Approval Chain', description: 'Multi-level approval workflow for privileged access requests', category: 'Identity & Access', steps: 6, avgDuration: '4 hours', successRate: 97.1, lastRun: '2026-04-23T09:15:00Z', manualInterventions: 1, optimizationScore: 92},
    {id: 'wf-ir-playbook-004', name: 'Compliance Evidence Collection', description: 'Automated gathering and packaging of audit evidence artifacts', category: 'Compliance', steps: 10, avgDuration: '30 min', successRate: 91.8, lastRun: '2026-04-21T10:00:00Z', manualInterventions: 3, optimizationScore: 78},
    {id: 'wf-ir-playbook-005', name: 'Security Patch Deployment', description: 'Staged patch rollout with validation and rollback capability', category: 'Patch Management', steps: 12, avgDuration: '2 hours', successRate: 96.3, lastRun: '2026-04-20T22:00:00Z', manualInterventions: 1, optimizationScore: 85},
    {id: 'wf-ir-playbook-006', name: 'Threat Intelligence Enrichment', description: 'IOC enrichment and correlation with internal threat data', category: 'Threat Intelligence', steps: 7, avgDuration: '8 min', successRate: 93.7, lastRun: '2026-04-23T07:00:00Z', manualInterventions: 0, optimizationScore: 95},
    {id: 'wf-ir-playbook-007', name: 'Security Reporting Pipeline', description: 'Automated generation and distribution of security metrics reports', category: 'Reporting', steps: 5, avgDuration: '15 min', successRate: 98.9, lastRun: '2026-04-23T08:00:00Z', manualInterventions: 0, optimizationScore: 97},
    {id: 'wf-ir-playbook-008', name: 'Vendor Risk Assessment', description: 'Automated vendor security questionnaire distribution and scoring', category: 'Third Party', steps: 9, avgDuration: '3 days', successRate: 85.4, lastRun: '2026-04-19T11:00:00Z', manualInterventions: 4, optimizationScore: 65},
  ];
  ...
  @state() private _workflowExecutionHistory: Array<{workflowId: string; runDate: string; duration: string; status: string; trigger: string}> = [
    {workflowId: 'wf-ir-playbook-001', runDate: '2026-04-23T06:00:00Z', duration: '11 min 23 sec', status: 'Success', trigger: 'Scheduled'},
    {workflowId: 'wf-ir-playbook-002', runDate: '2026-04-22T14:30:00Z', duration: '42 min 15 sec', status: 'Success', trigger: 'Alert'},
    {workflowId: 'wf-ir-playbook-003', runDate: '2026-04-23T09:15:00Z', duration: '3h 45 min', status: 'Success', trigger: 'User Request'},
    {workflowId: 'wf-ir-playbook-001', runDate: '2026-04-22T06:00:00Z', duration: '13 min 08 sec', status: 'Partial', trigger: 'Scheduled'},
    {workflowId: 'wf-ir-playbook-004', runDate: '2026-04-21T10:00:00Z', duration: '28 min 42 sec', status: 'Success', trigger: 'Scheduled'},
  ];
  ...
  @state() private _knowledgeArticles: Array<{id: string; title: string; category: string; author: string; publishDate: string; views: number; rating: number; tags: string[]; summary: string; readTime: string}> = [
    ...
    {id: 'kb-ir-playbook-003', title: 'Cloud Security Best Practices for Multi-Cloud', category: 'Cloud', author: 'Cloud Security Team', publishDate: '2026-02-28', views: 1567, rating: 4.5, tags: ['cloud', 'aws', 'azure', 'gcp'], summary: 'Best practices for securing workloads across multiple cloud service providers', readTime: '18 min'},
    {id: 'kb-ir-playbook-004', title: 'API Security Testing Methodology', category: 'Testing', author: 'Red Team', publishDate: '2026-03-22', views: 987, rating: 4.6, tags: ['api', 'testing', 'owasp'], summary: 'Structured methodology for identifying and testing API security vulnerabilities', readTime: '10 min'},
    {id: 'kb-ir-playbook-005', title: 'Supply Chain Security Risk Management', category: 'Third Party', author: 'GRC Team', publishDate: '2026-01-20', views: 1234, rating: 4.3, tags: ['supply-chain', 'vendor', 'risk'], summary: 'Framework for managing security risks in the software supply chain', readTime: '14 min'},
    {id: 'kb-ir-playbook-006', title: 'Kubernetes Security Hardening Checklist', category: 'Container', author: 'Platform Security', publishDate: '2026-04-10', views: 2103, rating: 4.7, tags: ['kubernetes', 'container', 'hardening'], summary: 'Step-by-step hardening checklist for production Kubernetes clusters', readTime: '11 min'},
    {id: 'kb-ir-playbook-007', title: 'Security Metrics That Matter for Executives', category: 'Metrics', author: 'CISO Office', publishDate: '2026-03-05', views: 876, rating: 4.4, tags: ['metrics', 'kpi', 'reporting'], summary: 'Key security metrics and KPIs that resonate with board-level stakeholders', readTime: '8 min'},
    {id: 'kb-ir-playbook-008', title: 'Phishing Simulation Campaign Design', category: 'Awareness', author: 'Security Awareness Team', publishDate: '2026-02-15', views: 1456, rating: 4.5, tags: ['phishing', 'simulation', 'training'], summary: 'Designing effective phishing simulations that drive real behavioral change', readTime: '9 min'},
    ...
    {id: 'kb-ir-playbook-010', title: 'Security Automation with SOAR Platforms', category: 'Automation', author: 'SOC Engineering', publishDate: '2026-04-05', views: 1098, rating: 4.7, tags: ['soar', 'automation', 'soc'], summary: 'Building effective security automation playbooks using SOAR technology', readTime: '13 min'},
    {id: 'kb-ir-playbook-011', title: 'Third-Party Risk Assessment Framework', category: 'GRC', author: 'Vendor Management', publishDate: '2026-01-30', views: 1345, rating: 4.4, tags: ['vendor', 'risk', 'assessment'], summary: 'Structured framework for evaluating and monitoring third-party security risks', readTime: '12 min'},
    {id: 'kb-ir-playbook-012', title: 'Network Detection and Response Best Practices', category: 'Network', author: 'Network Security Team', publishDate: '2026-03-18', views: 1567, rating: 4.5, tags: ['ndr', 'network', 'detection'], summary: 'Implementing effective network detection and response capabilities', readTime: '14 min'},
    {id: 'kb-ir-playbook-013', title: 'Security Chaos Engineering Guide', category: 'Resilience', author: 'SRE Security', publishDate: '2026-04-12', views: 789, rating: 4.3, tags: ['chaos-engineering', 'resilience', 'testing'], summary: 'Applying chaos engineering principles to validate security controls', readTime: '10 min'},
    {id: 'kb-ir-playbook-014', title: 'Incident Communication Templates', category: 'Incident Response', author: 'IR Team', publishDate: '2026-02-22', views: 2234, rating: 4.8, tags: ['incident', 'communication', 'templates'], summary: 'Ready-to-use communication templates for security incident scenarios', readTime: '7 min'},
    {id: 'kb-ir-playbook-015', title: 'Endpoint Detection and Response Configuration', category: 'Endpoint', author: 'Endpoint Security', publishDate: '2026-03-08', views: 1345, rating: 4.5, tags: ['edr', 'endpoint', 'configuration'], summary: 'Optimal EDR configuration for maximum detection with minimal false positives', readTime: '11 min'},
    {id: 'kb-ir-playbook-016', title: 'Security Code Review Standards', category: 'Development', author: 'AppSec Team', publishDate: '2026-02-10', views: 1890, rating: 4.6, tags: ['code-review', 'secure-sdlc', 'standards'], summary: 'Standards and checklists for security-focused code reviews', readTime: '13 min'},
    {id: 'kb-ir-playbook-017', title: 'IoT Security Assessment Methodology', category: 'IoT', author: 'IoT Security Lab', publishDate: '2026-04-08', views: 654, rating: 4.2, tags: ['iot', 'embedded', 'assessment'], summary: 'Methodology for assessing security posture of IoT devices and ecosystems', readTime: '15 min'},
    {id: 'kb-ir-playbook-018', title: 'Passwordless Authentication Migration Guide', category: 'Identity', author: 'IAM Team', publishDate: '2026-03-25', views: 1678, rating: 4.7, tags: ['passwordless', 'authentication', 'mfa'], summary: 'Step-by-step migration plan from password-based to passwordless authentication', readTime: '12 min'},
    {id: 'kb-ir-playbook-019', title: 'Security Awareness Program Metrics', category: 'Awareness', author: 'Security Culture Team', publishDate: '2026-01-25', views: 1123, rating: 4.4, tags: ['awareness', 'metrics', 'culture'], summary: 'Measuring the effectiveness of your security awareness and training programs', readTime: '9 min'},
    {id: 'kb-ir-playbook-020', title: 'Deception Technology Deployment Guide', category: 'Detection', author: 'Blue Team', publishDate: '2026-04-15', views: 876, rating: 4.5, tags: ['deception', 'honeypot', 'detection'], summary: 'Planning and deploying deception technology for advanced threat detection', readTime: '11 min'},
  ];
  ...
  // --- Security Scenario Planning Methods (IRPL) ---
  private _initIrplScenarios(): void {
    const worstCaseScenarios = [
      { id: "irpl-sc-01", name: "Ransomware Attack on Critical Infrastructure", category: "Malware", impact: 9500000, probability: 0.12, recoveryDays: 21, affectedSystems: 47, dataLossGb: 320, businessImpact: "critical" },
      { id: "irpl-sc-02", name: "Insider Data Exfiltration via Authorized Channels", category: "Insider Threat", impact: 7200000, probability: 0.18, recoveryDays: 14, affectedSystems: 12, dataLossGb: 85, businessImpact: "high" },
      { id: "irpl-sc-03", name: "Supply Chain Compromise via Third-Party Library", category: "Supply Chain", impact: 6800000, probability: 0.09, recoveryDays: 30, affectedSystems: 63, dataLossGb: 0, businessImpact: "critical" },
      { id: "irpl-sc-04", name: "Cloud Misconfiguration Leading to Public Exposure", category: "Cloud", impact: 4100000, probability: 0.22, recoveryDays: 7, affectedSystems: 8, dataLossGb: 450, businessImpact: "high" },
      { id: "irpl-sc-05", name: "Zero-Day Exploit in Core Application Framework", category: "Vulnerability", impact: 8900000, probability: 0.05, recoveryDays: 45, affectedSystems: 120, dataLossGb: 0, businessImpact: "critical" },
      { id: "irpl-sc-06", name: "Distributed Denial of Service on Customer-Facing Services", category: "Availability", impact: 3400000, probability: 0.25, recoveryDays: 3, affectedSystems: 15, dataLossGb: 0, businessImpact: "medium" },
      { id: "irpl-sc-07", name: "Credential Stuffing and Account Takeover Wave", category: "Identity", impact: 2800000, probability: 0.30, recoveryDays: 5, affectedSystems: 6, dataLossGb: 12, businessImpact: "medium" },
      { id: "irpl-sc-08", name: "Physical Security Breach at Data Center Facility", category: "Physical", impact: 12000000, probability: 0.02, recoveryDays: 60, affectedSystems: 200, dataLossGb: 5000, businessImpact: "critical" },
    ];
    this._irplScenarios = worstCaseScenarios.map(s => ({ ...s, mitigationCost: Math.round(s.impact * 0.08), drillScheduled: false, lastDrillDate: null, drillScore: null, controlGaps: Math.floor(Math.random() * 5) + 1 }));
  }

  private _calculateIrplScenarioRisk(): void {
    this._irplScenarios.forEach(s => {
  ...
  private _scheduleIrplDrills(): void {
  ...
  private _irplScenarioComparison(): Record<string, unknown>[] {
    return this._irplScenarios.map(s => ({
  ...
  // --- Security Control Effectiveness Analytics (IRPL) ---
  private _initIrplControls(): void {
    const controls = [
      { id: "irpl-ct-01", name: "Network Segmentation", type: "preventive", maturity: "defined", score: 82, target: 90, failures: 3, lastTest: "2026-03-15" },
      { id: "irpl-ct-02", name: "Endpoint Detection and Response", type: "detective", maturity: "managed", score: 88, target: 92, failures: 1, lastTest: "2026-04-01" },
      { id: "irpl-ct-03", name: "Data Loss Prevention", type: "preventive", maturity: "repeatable", score: 71, target: 85, failures: 7, lastTest: "2026-03-22" },
      { id: "irpl-ct-04", name: "Security Information and Event Management", type: "detective", maturity: "managed", score: 85, target: 90, failures: 2, lastTest: "2026-04-05" },
      { id: "irpl-ct-05", name: "Identity and Access Management", type: "preventive", maturity: "defined", score: 76, target: 88, failures: 5, lastTest: "2026-03-28" },
      { id: "irpl-ct-06", name: "Vulnerability Management", type: "corrective", maturity: "managed", score: 80, target: 90, failures: 4, lastTest: "2026-04-02" },
      { id: "irpl-ct-07", name: "Email Security Gateway", type: "preventive", maturity: "managed", score: 90, target: 95, failures: 1, lastTest: "2026-04-08" },
      { id: "irpl-ct-08", name: "Web Application Firewall", type: "preventive", maturity: "managed", score: 87, target: 92, failures: 2, lastTest: "2026-03-30" },
      { id: "irpl-ct-09", name: "Patch Management", type: "corrective", maturity: "defined", score: 68, target: 85, failures: 9, lastTest: "2026-03-18" },
      { id: "irpl-ct-10", name: "Encryption at Rest", type: "preventive", maturity: "managed", score: 93, target: 95, failures: 0, lastTest: "2026-04-10" },
      { id: "irpl-ct-11", name: "Encryption in Transit", type: "preventive", maturity: "optimized", score: 96, target: 98, failures: 0, lastTest: "2026-04-12" },
      { id: "irpl-ct-12", name: "Privileged Access Management", type: "preventive", maturity: "defined", score: 74, target: 88, failures: 6, lastTest: "2026-03-25" },
      { id: "irpl-ct-13", name: "Security Awareness Training", type: "preventive", maturity: "repeatable", score: 65, target: 80, failures: 11, lastTest: "2026-03-20" },
      { id: "irpl-ct-14", name: "Incident Response Plan", type: "corrective", maturity: "managed", score: 78, target: 88, failures: 4, lastTest: "2026-04-03" },
      { id: "irpl-ct-15", name: "Backup and Recovery", type: "corrective", maturity: "managed", score: 84, target: 92, failures: 3, lastTest: "2026-04-06" },
      { id: "irpl-ct-16", name: "Multi-Factor Authentication", type: "preventive", maturity: "managed", score: 91, target: 95, failures: 1, lastTest: "2026-04-09" },
      { id: "irpl-ct-17", name: "Network Traffic Analysis", type: "detective", maturity: "defined", score: 72, target: 85, failures: 5, lastTest: "2026-03-27" },
      { id: "irpl-ct-18", name: "Cloud Security Posture Management", type: "detective", maturity: "repeatable", score: 69, target: 82, failures: 8, lastTest: "2026-03-24" },
      { id: "irpl-ct-19", name: "Container Security Scanning", type: "preventive", maturity: "defined", score: 75, target: 85, failures: 5, lastTest: "2026-04-04" },
      { id: "irpl-ct-20", name: "API Security Gateway", type: "preventive", maturity: "repeatable", score: 70, target: 82, failures: 7, lastTest: "2026-03-29" },
      { id: "irpl-ct-21", name: "Threat Intelligence Platform", type: "detective", maturity: "managed", score: 83, target: 90, failures: 2, lastTest: "2026-04-07" },
      { id: "irpl-ct-22", name: "Database Activity Monitoring", type: "detective", maturity: "defined", score: 67, target: 80, failures: 8, lastTest: "2026-03-21" },
      { id: "irpl-ct-23", name: "Deception Technology", type: "detective", maturity: "initial", score: 55, target: 75, failures: 12, lastTest: "2026-03-16" },
      { id: "irpl-ct-24", name: "Security Orchestration Automation", type: "corrective", maturity: "repeatable", score: 73, target: 85, failures: 6, lastTest: "2026-04-01" },
      { id: "irpl-ct-25", name: "Third-Party Risk Assessment", type: "preventive", maturity: "defined", score: 64, target: 78, failures: 9, lastTest: "2026-03-19" },
    ];
    this._irplControls = controls;
  }

  private _analyzeIrplControlEffectiveness(): void {
    const typeBreakdown: Record<string, { total: number; avgScore: number; avgFailures: number }> = {};
    this._irplControls.forEach(c => {
    ...
    this._irplControlTypeBreakdown = typeBreakdown;
  }

  private _irplControlOptimization(): Record<string, unknown>[] {
    return this._irplControls.filter(c => c.score < c.target).map(c => ({
  ...
  // --- Security Data Pipeline Health (IRPL) ---
  private _initIrplPipelines(): void {
    const pipelines = [
      { id: "irpl-pl-01", name: "SIEM Log Ingestion", status: "healthy", healthScore: 94, latencyMs: 120, freshnessMin: 2, errorRate: 0.02, throughputMbps: 450, recordsPerSec: 12000, backPressure: 0.05, uptime: 99.97 },
      { id: "irpl-pl-02", name: "Threat Feed Aggregation", status: "healthy", healthScore: 91, latencyMs: 300, freshnessMin: 15, errorRate: 0.05, throughputMbps: 85, recordsPerSec: 3200, backPressure: 0.08, uptime: 99.92 },
      { id: "irpl-pl-03", name: "Vulnerability Scan Results", status: "degraded", healthScore: 72, latencyMs: 2500, freshnessMin: 60, errorRate: 0.15, throughputMbps: 25, recordsPerSec: 800, backPressure: 0.35, uptime: 98.50 },
      { id: "irpl-pl-04", name: "Endpoint Telemetry Stream", status: "healthy", healthScore: 88, latencyMs: 450, freshnessMin: 5, errorRate: 0.08, throughputMbps: 280, recordsPerSec: 8500, backPressure: 0.12, uptime: 99.85 },
      { id: "irpl-pl-05", name: "Cloud Audit Log Pipeline", status: "healthy", healthScore: 86, latencyMs: 600, freshnessMin: 10, errorRate: 0.06, throughputMbps: 150, recordsPerSec: 4500, backPressure: 0.10, uptime: 99.80 },
      { id: "irpl-pl-06", name: "Identity Event Stream", status: "warning", healthScore: 78, latencyMs: 1200, freshnessMin: 20, errorRate: 0.12, throughputMbps: 45, recordsPerSec: 1500, backPressure: 0.22, uptime: 99.40 },
      { id: "irpl-pl-07", name: "Network Flow Collection", status: "healthy", healthScore: 82, latencyMs: 800, freshnessMin: 8, errorRate: 0.09, throughputMbps: 650, recordsPerSec: 18000, backPressure: 0.15, uptime: 99.70 },
      { id: "irpl-pl-08", name: "DLP Incident Pipeline", status: "degraded", healthScore: 68, latencyMs: 3500, freshnessMin: 45, errorRate: 0.20, throughputMbps: 18, recordsPerSec: 500, backPressure: 0.42, uptime: 97.80 },
    ];
    this._irplPipelines = pipelines.map(p => ({ ...p, dependencies: [], slaBreached: p.healthScore < 75, alertThreshold: p.errorRate > 0.15 }));
    this._irplPipelines[0].dependencies = ["irpl-pl-04", "irpl-pl-07"];
    this._irplPipelines[4].dependencies = ["irpl-pl-01"];
    this._irplPipelines[5].dependencies = ["irpl-pl-02"];
    this._irplPipelines[7].dependencies = ["irpl-pl-04", "irpl-pl-05"];
  }

  private _irplPipelineTrend(): Record<string, unknown>[] {
    const hours = Array.from({ length: 24 }, (_, i) => `H${String(i).padStart(2, "0")}`);
    return this._irplPipelines.map(p => ({
  ...
  // --- Security Stakeholder Report Generator (IRPL) ---
  private _initIrplReportTemplates(): void {
    this._irplReportTemplates = [
      { id: "irpl-rp-01", name: "Board Security Report", audience: "board", frequency: "quarterly", sections: 6, autoGenerate: true, lastGenerated: "2026-03-31", pages: 12 },
      { id: "irpl-rp-02", name: "Executive Risk Summary", audience: "executive", frequency: "monthly", sections: 8, autoGenerate: true, lastGenerated: "2026-04-01", pages: 8 },
      { id: "irpl-rp-03", name: "Technical Security Review", audience: "technical", frequency: "weekly", sections: 12, autoGenerate: true, lastGenerated: "2026-04-21", pages: 25 },
      { id: "irpl-rp-04", name: "Audit Compliance Report", audience: "audit", frequency: "quarterly", sections: 10, autoGenerate: false, lastGenerated: "2026-03-15", pages: 35 },
      { id: "irpl-rp-05", name: "Regulatory Filing Package", audience: "regulatory", frequency: "annual", sections: 15, autoGenerate: false, lastGenerated: "2025-12-31", pages: 48 },
    ];
  }

  private _generateIrplExecSummary(): string {
    const totalRisk = this._irplScenarios.reduce((sum: number, s: any) => sum + s.riskScore, 0);
    const avgControl = Math.round(this._irplControls.reduce((sum: number, c: any) => sum + c.score, 0) / this._irplControls.length);
    const degradedPipes = this._irplPipelines.filter((p: any) => p.status !== "healthy").length;
    return `Overall risk exposure: ${(totalRisk / 1000000).toFixed(1)}M. Average control effectiveness: {avgControl}%. {degradedPipes} pipeline(s) need attention. {this._irplScenarios.filter((s: any) => s.mitigationPriority === "immediate").length} scenarios require immediate action.`;
  }

  // --- Security Technology Radar (IRPL) ---
  private _initIrplTechRadar(): void {
    this._irplTechRadar = [
      { id: "irpl-tr-01", name: "AI-Powered Threat Detection", ring: "adopt", category: "Detection", maturity: 4, trend: "up", investment: "high", roi: 3.2, vendorCount: 8 },
      { id: "irpl-tr-02", name: "Zero Trust Architecture", ring: "adopt", category: "Architecture", maturity: 4, trend: "up", investment: "high", roi: 2.8, vendorCount: 12 },
      { id: "irpl-tr-03", name: "SASE/SSE Platform", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.4, vendorCount: 10 },
      { id: "irpl-tr-04", name: "CNAPP Cloud Security", ring: "trial", category: "Cloud", maturity: 3, trend: "up", investment: "medium", roi: 2.1, vendorCount: 7 },
      { id: "irpl-tr-05", name: "Extended Detection and Response", ring: "adopt", category: "Detection", maturity: 4, trend: "stable", investment: "high", roi: 2.9, vendorCount: 9 },
      { id: "irpl-tr-06", name: "Quantum-Safe Cryptography", ring: "assess", category: "Crypto", maturity: 2, trend: "up", investment: "low", roi: 0.8, vendorCount: 4 },
      { id: "irpl-tr-07", name: "Security Service Edge", ring: "trial", category: "Network", maturity: 3, trend: "up", investment: "medium", roi: 2.0, vendorCount: 6 },
      { id: "irpl-tr-08", name: "SOAR 2.0 Autonomous SOC", ring: "assess", category: "Operations", maturity: 2, trend: "up", investment: "low", roi: 1.2, vendorCount: 5 },
      { id: "irpl-tr-09", name: "Software Supply Chain Security", ring: "trial", category: "DevSecOps", maturity: 3, trend: "up", investment: "medium", roi: 2.3, vendorCount: 8 },
      { id: "irpl-tr-10", name: "Identity Threat Detection", ring: "adopt", category: "Identity", maturity: 3, trend: "up", investment: "high", roi: 2.7, vendorCount: 7 },
      { id: "irpl-tr-11", name: "Data Security Posture Management", ring: "trial", category: "Data", maturity: 3, trend: "up", investment: "medium", roi: 2.2, vendorCount: 9 },
      { id: "irpl-tr-12", name: "Decentralized Identity Standards", ring: "hold", category: "Identity", maturity: 1, trend: "stable", investment: "low", roi: 0.5, vendorCount: 3 },
    ];
  }

  private _irplRadarSummary(): Record<string, unknown> {
    const rings: Record<string, number> = { adopt: 0, trial: 0, assess: 0, hold: 0 };
    this._irplTechRadar.forEach(t => { if (rings[t.ring as keyof typeof rings] !== undefined) rings[t.ring as keyof typeof rings]++; });
    return { adopt: rings.adopt, trial: rings.trial, assess: rings.assess, hold: rings.hold, total: this._irplTechRadar.length, avgMaturity: 2.8, topInvestment: this._irplTechRadar.filter(t => t.investment === "high").map(t => t.name) };
  }

  // --- IRPL Security Compliance Mapping Matrix ---
  private _initIrplComplianceMatrix(): void {
    ...
    this._irplComplianceMatrix = frameworks.map(fw => ({
  ...
  private _irplComplianceTrend(): Record<string, unknown>[] {
    const quarters = ["Q1-2025", "Q2-2025", "Q3-2025", "Q4-2025", "Q1-2026", "Q2-2026"];
    return this._irplComplianceMatrix.map(fw => ({
  ...
  // --- IRPL Threat Intelligence Correlation Engine ---
  private _initIrplThreatIntel(): void {
    ...
    this._irplThreatIntel = threatActors.map((actor, i) => ({
  ...
  private _irplThreatCorrelation(): Record<string, unknown>[] {
    return this._irplThreatIntel.filter(t => t.riskScore > 70).map(t => ({
  ...
  // --- IRPL Security Metrics Deep Dive ---
  private _initIrplMetricsDeep(): void {
    ...
    this._irplMetricsDeep = metricCategories.map(cat => ({
  ...
  private _irplMetricsHeatmap(): number[][] {
    return this._irplMetricsDeep.map(cat =>
      cat.metrics.map(m => Math.round((m.value / m.target) * 100))
    );
  }

  // --- IRPL Incident Cost Modeling ---
  private _initIrplCostModel(): void {
    const incidentTypes = ["Data Breach", "Ransomware", "DDoS", "Insider Threat", "Phishing", "Supply Chain", "Cloud Misconfig", "Zero-Day"];
    this._irplCostModel = incidentTypes.map(inc => ({
    ...
    this._irplCostModel.forEach(m => { m.totalAnnualExposure = m.avgCost * m.frequency; });
  }

  private _irplCostProjection(): Record<string, unknown>[] {
    return this._irplCostModel.map(m => ({
  ...
  // --- IRPL Security Architecture Pattern Library ---
  private _initIrplArchPatterns(): void {
    ...
    this._irplArchPatterns = patterns.map((p, i) => ({
  ...
  private _irplArchPatternAnalysis(): Record<string, unknown> {
    const implemented = this._irplArchPatterns.filter(p => p.implementation > 75).length;
    const avgCoverage = Math.round(this._irplArchPatterns.reduce((s: number, p: any) => s + p.coverage, 0) / this._irplArchPatterns.length);
    return {
      totalPatterns: this._irplArchPatterns.length, fullyImplemented: implemented, avgCoverage,
      avgRiskReduction: Math.round(this._irplArchPatterns.reduce((s: number, p: any) => s + p.riskReduction, 0) / this._irplArchPatterns.length),
      gaps: this._irplArchPatterns.filter(p => p.implementation < 60).map(p => p.pattern),
      recommendations: this._irplArchPatterns.filter(p => p.implementation < 70).map(p => ({ pattern: p.pattern, current: p.implementation, target: 85, effort: p.costComplexity }))
    };
  }
  ...
  private _f67RiskAssessmentWorkflow = [
  ...
  private _f67TalentPositions = [
    ...
    {id: 'f67-H-002', title: 'Cloud Security Engineer', dept: 'Cloud Security', level: 'Mid-Senior', loc: 'SF/Remote', salary: '$155K-$180K', posted: '2026-03-20', applicants: 45, pipeline: 12, interviews: 6, offers: 2, urgency: 'high' as const, manager: 'Mike Johnson', skills: ['AWS/Azure/GCP', 'Terraform', 'Kubernetes', 'IAM', 'CSPM'], nice: ['CCSP', 'CKS', 'Serverless']},
    {id: 'f67-H-003', title: 'SOC Analyst L2', dept: 'Security Operations', level: 'Mid', loc: 'Austin TX', salary: '$105K-$130K', posted: '2026-04-01', applicants: 67, pipeline: 18, interviews: 8, offers: 0, urgency: 'medium' as const, manager: 'Lisa Park', skills: ['SIEM', 'Incident triage', 'Malware analysis', 'TCP/IP'], nice: ['GCIA', 'SOAR', 'Kill chain']},
    ...
    {id: 'f67-H-005', title: 'GRC Analyst', dept: 'GRC', level: 'Mid', loc: 'NY/Remote', salary: '$110K-$135K', posted: '2026-04-05', applicants: 34, pipeline: 9, interviews: 3, offers: 1, urgency: 'medium' as const, manager: 'Rachel Green', skills: ['ISO 27001', 'SOC 2', 'Risk assessment', 'Audit'], nice: ['CRISC', 'CISA', 'GDPR']},
    {id: 'f67-H-006', title: 'Threat Intel Analyst', dept: 'Threat Intel', level: 'Mid-Senior', loc: 'Remote US', salary: '$140K-$170K', posted: '2026-03-25', applicants: 19, pipeline: 5, interviews: 2, offers: 0, urgency: 'low' as const, manager: 'James Wilson', skills: ['MITRE ATT&CK', 'OSINT', 'Threat modeling', 'Reverse eng'], nice: ['CTIA', 'Nation-state', 'Dark web']},
    {id: 'f67-H-007', title: 'AppSec Engineer', dept: 'AppSec', level: 'Senior', loc: 'Seattle/Remote', salary: '$160K-$190K', posted: '2026-04-10', applicants: 28, pipeline: 7, interviews: 3, offers: 0, urgency: 'medium' as const, manager: 'Emily Zhang', skills: ['SAST/DAST/SCA', 'Secure SDLC', 'Code review', 'CI/CD'], nice: ['CSSLP', 'Mobile security']},
    {id: 'f67-H-008', title: 'IAM Engineer', dept: 'IAM', level: 'Mid', loc: 'Remote US', salary: '$135K-$160K', posted: '2026-04-08', applicants: 31, pipeline: 10, interviews: 5, offers: 1, urgency: 'medium' as const, manager: 'Chris Martinez', skills: ['Okta/Azure AD', 'SAML/OIDC', 'PAM', 'RBAC'], nice: ['CISSP', 'Zero trust IAM']}
  ];

  private _f67InterviewScorecard = [
  ...
  private _f67OnboardingSteps = [
    {id: 'f67-OB-01', item: 'Provision security-hardened laptop, monitors, peripherals, and configured workstation image with all required security tools', cat: 'IT Setup', day: 'Day 1', owner: 'IT Operations'},
    {id: 'f67-OB-02', item: 'Create AD account, email, Slack, VPN, SSO enrollment, and all required application access accounts', cat: 'Access', day: 'Day 1', owner: 'IAM Team'},
    {id: 'f67-OB-03', item: 'Grant security tooling access: SIEM console, EDR dashboard, vuln scanner, pentest platforms, SOAR', cat: 'Tools', day: 'Day 1-2', owner: 'SOC Manager'},
    {id: 'f67-OB-04', item: 'Complete MFA enrollment, security awareness onboarding course, and acceptable use policy acknowledgment', cat: 'Security', day: 'Day 1', owner: 'Awareness Team'},
    {id: 'f67-OB-05', item: 'Sign NDA, employment agreement, IP assignment, and code of conduct documents with HR', cat: 'HR', day: 'Day 1', owner: 'Human Resources'},
    {id: 'f67-OB-06', item: 'Introduction meetings with direct team members and key cross-functional stakeholders', cat: 'Integration', day: 'Week 1', owner: 'Hiring Manager'},
    {id: 'f67-OB-07', item: 'Security architecture overview, tooling walkthrough, and operational runbook review sessions', cat: 'Training', day: 'Week 1-2', owner: 'Security Architect'},
    {id: 'f67-OB-08', item: 'Shadow experienced team member on active incident response and threat hunting investigations', cat: 'Mentoring', day: 'Week 2-3', owner: 'SOC Lead'},
    {id: 'f67-OB-09', item: 'Set up individual certification study plan aligned with role requirements and career goals', cat: 'Development', day: 'Week 2', owner: 'Hiring Manager'},
    {id: 'f67-OB-10', item: 'Establish 30-60-90 day goals, define success metrics, and schedule first performance checkpoint', cat: 'Goals', day: 'Week 1', owner: 'Hiring Manager'}
  ];

  private _f67MetricsDashboard = [
  ...
  private _f67ArchReview = [
    {id: 'f67-AR-001', cat: 'Network', item: 'Network segmentation between trust zones with dedicated firewalls and granular ACLs', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Micro-segmentation via NSX across all production zones'},
    {id: 'f67-AR-002', cat: 'Network', item: 'DMZ architecture isolating public services from internal network', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Dual-firewall DMZ with WAF and DDoS mitigation'},
    {id: 'f67-AR-003', cat: 'Network', item: 'Least-privilege traffic flows with deny-by-default and quarterly review', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'Explicit allow rules with automated drift detection'},
    {id: 'f67-AR-004', cat: 'Network', item: 'Encrypted external communications with IPSec/TLS 1.3 and PFS', sev: 'high' as const, status: 'pass' as const, reviewer: 'Network Architect', notes: 'IKEv2 with PFS, TLS 1.3 for all external APIs'},
    {id: 'f67-AR-005', cat: 'Identity', item: 'Centralized IdP with SSO and automated SCIM provisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Okta SSO SAML 2.0 for 247 apps with SCIM'},
    {id: 'f67-AR-006', cat: 'Identity', item: 'RBAC with quarterly reviews and automated deprovisioning', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'Quarterly recertification with JIT admin access'},
    {id: 'f67-AR-007', cat: 'Identity', item: 'PAM controlling all admin and service accounts with session recording', sev: 'high' as const, status: 'fail' as const, reviewer: 'IAM Lead', notes: '3 legacy accounts pending CyberArk enrollment'},
    {id: 'f67-AR-008', cat: 'Identity', item: 'MFA enforced for all users, hardware tokens for privileged access', sev: 'critical' as const, status: 'pass' as const, reviewer: 'IAM Lead', notes: 'FIDO2 for admins, push MFA for users, 99.8% enrollment'},
    {id: 'f67-AR-009', cat: 'Data', item: 'Data classification enforced with automated labeling on all repos', sev: 'high' as const, status: 'partial' as const, reviewer: 'Data Protection Lead', notes: 'Framework deployed, gaps on 4 legacy file shares'},
    {id: 'f67-AR-010', cat: 'Data', item: 'AES-256 encryption at rest for all sensitive data stores', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'Full-disk, TDE on DBs, SSE-KMS on cloud storage'},
    {id: 'f67-AR-011', cat: 'Data', item: 'DLP monitoring and preventing unauthorized data exfiltration', sev: 'high' as const, status: 'pass' as const, reviewer: 'Data Protection Lead', notes: 'DLP active on email, endpoint, cloud, and network'},
    {id: 'f67-AR-012', cat: 'Data', item: 'Encrypted off-site backups with quarterly restore testing', sev: 'critical' as const, status: 'pass' as const, reviewer: 'Infrastructure Lead', notes: '3-2-1 strategy with immutable backups'},
    {id: 'f67-AR-013', cat: 'AppSec', item: 'Secure SDLC with SAST, DAST, SCA in all CI/CD pipelines', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: '34 pipelines with security gates and PR blocking'},
    {id: 'f67-AR-014', cat: 'AppSec', item: 'API gateway with auth, rate limiting, schema validation', sev: 'high' as const, status: 'pass' as const, reviewer: 'AppSec Lead', notes: 'Kong with OAuth2, rate limits, OpenAPI validation'},
    {id: 'f67-AR-015', cat: 'AppSec', item: 'Container security with image scanning and runtime protection', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Trivy scanning, Falco runtime in 80% of clusters'},
    {id: 'f67-AR-016', cat: 'Monitoring', item: 'SIEM collecting from all critical systems with correlation rules', sev: 'critical' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'Splunk 98% coverage with 1200+ correlation rules'},
    {id: 'f67-AR-017', cat: 'Monitoring', item: 'EDR with behavioral detection on all managed endpoints', sev: 'high' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'CrowdStrike on 99.2% endpoints with 24/7 MDR'},
    {id: 'f67-AR-018', cat: 'Monitoring', item: 'Executive dashboards with real-time posture visibility', sev: 'medium' as const, status: 'pass' as const, reviewer: 'SOC Manager', notes: 'PowerBI dashboards with weekly exec summaries'},
    {id: 'f67-AR-019', cat: 'Cloud', item: 'CSPM monitoring and remediating cloud misconfigurations', sev: 'high' as const, status: 'pass' as const, reviewer: 'Cloud Security Lead', notes: 'Prisma Cloud 450+ checks with auto-remediation'},
    {id: 'f67-AR-020', cat: 'Cloud', item: 'IaC templates scanned before deployment to production', sev: 'high' as const, status: 'partial' as const, reviewer: 'Cloud Security Lead', notes: 'Checkov for Terraform, cfn-nag for CloudFormation pending'}
  ];

  private _getf67RiskProgress(): number {
    return Math.round(this._f67RiskAssessmentWorkflow.filter(s => s.status === 'completed').length / this._f67RiskAssessmentWorkflow.length * 100);
  }

  private _getf67TreatmentSummary(): {total: number; approved: number; avgReduction: number; investment: string} {
    const a = this._f67TreatmentOptions.filter(r => r.status === 'approved').length;
    const avg = this._f67TreatmentOptions.reduce((s, r) => s + ((r.before - r.after) / r.before * 100), 0) / this._f67TreatmentOptions.length;
    return {total: this._f67TreatmentOptions.length, approved: a, avgReduction: Math.round(avg), investment: '$1.125M annually'};
  }

  private _getf67RuleSummary(): {total: number; passing: number; failing: number; avgRate: number; violations: number} {
    const p = this._f67ComplianceRules.filter(r => r.result === 'pass').length;
    const f = this._f67ComplianceRules.filter(r => r.result === 'fail').length;
    const avg = this._f67ComplianceRules.reduce((s, r) => s + r.passRate, 0) / this._f67ComplianceRules.length;
    const v = this._f67ComplianceRules.reduce((s, r) => s + r.violations, 0);
    return {total: this._f67ComplianceRules.length, passing: p, failing: f, avgRate: Math.round(avg * 10) / 10, violations: v};
  }

  private _getf67PipelineStats(): {positions: number; applicants: number; interviewing: number; offers: number; critical: number} {
    const t = this._f67TalentPositions.length;
    const a = this._f67TalentPositions.reduce((s, p) => s + p.applicants, 0);
    const i = this._f67TalentPositions.reduce((s, p) => s + p.interviews, 0);
    const o = this._f67TalentPositions.reduce((s, p) => s + p.offers, 0);
    const c = this._f67TalentPositions.filter(x => x.urgency === 'critical').length;
    return {positions: t, applicants: a, interviewing: i, offers: o, critical: c};
  }

  private _getf67MetricTrends(): {improving: number; stable: number; total: number} {
    const imp = this._f67MetricsDashboard.filter(m => m.trend === 'improving').length;
    const stb = this._f67MetricsDashboard.filter(m => m.trend === 'stable').length;
    return {improving: imp, stable: stb, total: this._f67MetricsDashboard.length};
  }

  private _getf67ArchScore(): {total: number; passed: number; failed: number; partial: number; score: number} {
    const p = this._f67ArchReview.filter(c => c.status === 'pass').length;
    const f = this._f67ArchReview.filter(c => c.status === 'fail').length;
    const pa = this._f67ArchReview.filter(c => c.status === 'partial').length;
    return {total: this._f67ArchReview.length, passed: p, failed: f, partial: pa, score: Math.round((p / this._f67ArchReview.length) * 100)};
  }


  private _zf67IncidentScenarios = [
    {id: 'zf67-IS-001', name: 'Ransomware Outbreak in Production Environment', category: 'Malware', likelihood: 'Medium', impact: 'Critical', riskScore: 15, playbooks: ['IR-Ransomware-Isolate', 'IR-Ransomware-Eradicate', 'IR-Ransomway-Recover'], estimatedMTTD: '15min', estimatedMTTR: '4h', involvedTeams: ['SOC', 'IR', 'Infrastructure', 'Legal', 'Comms'], keyIndicators: ['Mass file encryption events', 'Ransom note deployment', 'Lateral movement via SMB/RDP', 'Backup deletion attempts'], mitigationControls: ['EDR behavioral detection', 'Network segmentation', 'Immutable backups', 'Email filtering'], lessonsLearned: 'Ensure offline backups are tested quarterly and Ransomware-specific playbooks are updated with latest threat intelligence', lastDrillDate: '2026-03-15', nextDrillDate: '2026-06-15'},
    {id: 'zf67-IS-002', name: 'Cloud Misconfiguration Leading to Data Exposure', category: 'Cloud', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Cloud-Misconfig-Assess', 'IR-Cloud-Misconfig-Remediate', 'IR-Cloud-Misconfig-Notify'], estimatedMTTD: '2h', estimatedMTTR: '6h', involvedTeams: ['Cloud Security', 'SOC', 'Data Protection', 'Legal'], keyIndicators: ['Public S3 bucket exposure', 'Overly permissive IAM policies', 'Unencrypted database snapshots', 'Public API endpoint discovery'], mitigationControls: ['CSPM monitoring', 'IAM policy analyzer', 'Automated remediation', 'Cloud audit logging'], lessonsLearned: 'Implement CSPM with auto-remediation for common misconfigurations and conduct monthly cloud configuration reviews', lastDrillDate: '2026-02-20', nextDrillDate: '2026-05-20'},
    {id: 'zf67-IS-003', name: 'Insider Threat Data Exfiltration', category: 'Insider', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-Insider-Investigate', 'IR-Insider-Contain', 'IR-Insider-Escalate'], estimatedMTTD: '8h', estimatedMTTR: '24h', involvedTeams: ['SOC', 'IR', 'HR', 'Legal', 'IAM'], keyIndicators: ['Anomalous data access patterns', 'Large file downloads to USB or cloud', 'Off-hours access to sensitive systems', 'Accessing data outside normal job scope'], mitigationControls: ['UEBA analytics', 'DLP policies', 'PAM controls', 'Access reviews'], lessonsLearned: 'Strengthen UEBA baselines and implement automated alerts for data access pattern anomalies', lastDrillDate: '2026-01-10', nextDrillDate: '2026-04-10'},
    {id: 'zf67-IS-004', name: 'Supply Chain Attack via Compromised Vendor Software', category: 'Supply Chain', likelihood: 'Low', impact: 'Critical', riskScore: 12, playbooks: ['IR-SupplyChain-Detect', 'IR-SupplyChain-Isolate', 'IR-SupplyChain-Remediate'], estimatedMTTD: '24h', estimatedMTTR: '72h', involvedTeams: ['SOC', 'IR', 'Procurement', 'Vendor Mgmt', 'Legal'], keyIndicators: ['Unexpected network connections from vendor software', 'Modified binaries or dependencies', 'Anomalous behavior from trusted applications', 'SBOM discrepancy alerts'], mitigationControls: ['SCA scanning', 'SBOM monitoring', 'Vendor security assessments', 'Network segmentation'], lessonsLearned: 'Require SBOM from all vendors and implement continuous monitoring of third-party component behavior', lastDrillDate: '2026-03-01', nextDrillDate: '2026-06-01'},
    {id: 'zf67-IS-005', name: 'Credential Stuffing Attack on Customer Portal', category: 'External Attack', likelihood: 'High', impact: 'Medium', riskScore: 12, playbooks: ['IR-CredentialStuffing-Detect', 'IR-CredentialStuffing-Block', 'IR-CredentialStuffing-Notify'], estimatedMTTD: '30min', estimatedMTTR: '2h', involvedTeams: ['SOC', 'AppSec', 'Customer Support', 'Comms'], keyIndicators: ['High volume of failed login attempts', 'Login attempts from known compromised credential lists', 'Unusual geographic distribution of login attempts', 'Account takeover indicators'], mitigationControls: ['WAF rate limiting', 'CAPTCHA on login', 'MFA enforcement', 'Credential monitoring service'], lessonsLearned: 'Deploy advanced bot detection on login endpoints and implement proactive credential monitoring for customer accounts', lastDrillDate: '2026-04-01', nextDrillDate: '2026-07-01'},
    {id: 'zf67-IS-006', name: 'Phishing Campaign Targeting Executive Team', category: 'Social Engineering', likelihood: 'High', impact: 'High', riskScore: 16, playbooks: ['IR-Phishing-Contain', 'IR-Phishing-Investigate', 'IR-Phishing-Notify'], estimatedMTTD: '1h', estimatedMTTR: '4h', involvedTeams: ['SOC', 'Awareness', 'Executive Office', 'IT'], keyIndicators: ['Sophisticated spear-phishing emails to C-suite', 'Business email compromise indicators', 'Redirected MFA approval requests', 'Unauthorized wire transfer attempts'], mitigationControls: ['AI email filtering', 'Executive awareness training', 'Out-of-band verification policy', 'DMARC/DKIM/SPF'], lessonsLearned: 'Implement additional email security controls for executive mailboxes and conduct monthly spear-phishing simulations', lastDrillDate: '2026-02-15', nextDrillDate: '2026-05-15'}
  ];

  private _zf67SecurityFrameworks = [
  ...
  private _zf67ThirdPartyRiskData = [
  ...
  private _getzf67ScenarioSummary(): {total: number; critical: number; high: number; drilled: number; avgRiskScore: number} {
    const crit = this._zf67IncidentScenarios.filter(s => s.impact === 'Critical').length;
    const high = this._zf67IncidentScenarios.filter(s => s.impact === 'High').length;
    const avg = this._zf67IncidentScenarios.reduce((s, x) => s + x.riskScore, 0) / this._zf67IncidentScenarios.length;
    return {total: this._zf67IncidentScenarios.length, critical: crit, high: high, drilled: this._zf67IncidentScenarios.length, avgRiskScore: Math.round(avg)};
  }

  private _getzf67VendorRiskSummary(): {total: number; approved: number; conditional: number; remediation: number; avgScore: number} {
    const approved = this._zf67ThirdPartyRiskData.filter(v => v.status === 'approved').length;
    const cond = this._zf67ThirdPartyRiskData.filter(v => v.status === 'conditional').length;
    const rem = this._zf67ThirdPartyRiskData.filter(v => v.status === 'remediation_required').length;
    const avg = this._zf67ThirdPartyRiskData.reduce((s, v) => s + v.riskScore, 0) / this._zf67ThirdPartyRiskData.length;
    return {total: this._zf67ThirdPartyRiskData.length, approved, conditional: cond, remediation: rem, avgScore: Math.round(avg)};
  }

  private _getzf67TrainingSummary(): {total: number; avgCompletion: number; avgScore: number; overdueUpdates: number} {
    const avgComp = this._zf67SecurityTrainingCatalog.reduce((s, t) => s + t.completionRate, 0) / this._zf67SecurityTrainingCatalog.length;
    const avgScore = this._zf67SecurityTrainingCatalog.filter(t => t.avgScore > 0).reduce((s, t) => s + t.avgScore, 0) / this._zf67SecurityTrainingCatalog.filter(t => t.avgScore > 0).length;
    return {total: this._zf67SecurityTrainingCatalog.length, avgCompletion: Math.round(avgComp * 10) / 10, avgScore: Math.round(avgScore), overdueUpdates: 0};
  }

  private _getzf67ToolHealthSummary(): {total: number; healthy: number; degraded: number; annualCost: string} {
    const healthy = this._zf67SecurityToolInventory.filter(t => t.health === 'healthy').length;
    const degraded = this._zf67SecurityToolInventory.filter(t => t.health === 'degraded').length;
    const cost = this._zf67SecurityToolInventory.reduce((s, t) => s + parseInt(t.annualCost.replace(/[^0-9]/g, '')), 0);
    return {total: this._zf67SecurityToolInventory.length, healthy, degraded, annualCost: '$' + (cost / 1000).toFixed(0) + 'K'};
  }


  private _xf67SecurityZones = [
    {zoneId: 'xf67-Z-001', name: 'Internet DMZ', trustLevel: 'Untrusted', subnet: '10.0.0.0/24', segmentation: 'Dual-firewall', firewallRules: 45, monitoring: 'Full packet capture', assets: ['Web servers', 'Load balancers', 'WAF appliances', 'Reverse proxy'], riskLevel: 'high' as const, lastAssessment: '2026-04-15', nextAssessment: '2026-07-15'},
    {zoneId: 'xf67-Z-002', name: 'Corporate Network', trustLevel: 'Trusted', subnet: '10.1.0.0/16', segmentation: 'VLAN + micro-seg', firewallRules: 128, monitoring: 'NetFlow + IDS', assets: ['Workstations', 'Printers', 'Meeting room systems', 'VoIP phones'], riskLevel: 'medium' as const, lastAssessment: '2026-04-10', nextAssessment: '2026-07-10'},
    {zoneId: 'xf67-Z-003', name: 'Production Data Center', trustLevel: 'Restricted', subnet: '10.2.0.0/16', segmentation: 'Full micro-segmentation', firewallRules: 256, monitoring: 'Full packet capture + EDR', assets: ['Application servers', 'Database clusters', 'Message queues', 'API gateways'], riskLevel: 'low' as const, lastAssessment: '2026-04-12', nextAssessment: '2026-07-12'},
    {zoneId: 'xf67-Z-004', name: 'Management Network', trustLevel: 'Restricted', subnet: '10.3.0.0/24', segmentation: 'Isolated VLAN', firewallRules: 32, monitoring: 'Syslog + SNMP', assets: ['Hypervisors', 'Switch management', 'Storage controllers', 'Backup appliances'], riskLevel: 'low' as const, lastAssessment: '2026-04-08', nextAssessment: '2026-07-08'},
    {zoneId: 'xf67-Z-005', name: 'Cloud Production', trustLevel: 'Restricted', subnet: 'VPC: 172.16.0.0/12', segmentation: 'Security groups + NACLs', firewallRules: 189, monitoring: 'VPC Flow Logs + GuardDuty', assets: ['Kubernetes clusters', 'Managed databases', 'Object storage', 'Serverless functions'], riskLevel: 'medium' as const, lastAssessment: '2026-04-18', nextAssessment: '2026-07-18'}
  ];

  private _xf67SecurityAlerts = [
    {id: 'xf67-AL-001', severity: 'critical', title: 'Active ransomware encryption detected on PROD-DB-01', source: 'CrowdStrike', timestamp: '2026-04-22T13:45:00Z', status: 'investigating', assignee: 'SOC Analyst J. Smith', asset: 'PROD-DB-01 (10.2.1.15)', mitreTechnique: 'T1486 - Data Encrypted for Impact', playbook: 'IR-Ransomware-Isolate', sla: '15 minutes', elapsed: '12 minutes', notes: 'EDR detected mass file encryption event pattern. Automated isolation triggered. IR team paged.'},
    {id: 'xf67-AL-002', severity: 'high', title: 'Impossible travel detected for user admin@company.com', source: 'Okta', timestamp: '2026-04-22T12:30:00Z', status: 'triaged', assignee: 'SOC Analyst M. Johnson', asset: 'Okta - admin@company.com', mitreTechnique: 'T1078 - Valid Accounts', playbook: 'IR-ImpossibleTravel', sla: '30 minutes', elapsed: '87 minutes', notes: 'Login from Tokyo followed by login from New York within 30 minutes. Account temporarily locked. User confirmed via out-of-band channel they are in New York.'},
    {id: 'xf67-AL-003', severity: 'high', title: 'Public S3 bucket discovered with customer PII exposure', source: 'Prisma Cloud', timestamp: '2026-04-22T11:15:00Z', status: 'remediating', assignee: 'Cloud Security Lead A. Chen', asset: 'AWS S3: customer-data-archive-2024', mitreTechnique: 'N/A - Misconfiguration', playbook: 'IR-Cloud-Misconfig-Remediate', sla: '1 hour', elapsed: '2.5 hours', notes: 'Bucket containing archived customer records was publicly accessible. Bucket blocked and encrypted. Data exposure assessment in progress.'},
    {id: 'xf67-AL-004', severity: 'medium', title: 'Anomalous data download by user jane.developer@company.com', source: 'Microsoft DLP', timestamp: '2026-04-22T10:00:00Z', status: 'investigating', assignee: 'SOC Analyst R. Davis', asset: 'Endpoint: WS-DEV-042', mitreTechnique: 'T1567 - Exfil Over Web Service', playbook: 'IR-DLP-Investigation', sla: '4 hours', elapsed: '3.5 hours', notes: 'User downloaded 4.2GB of source code files to personal Google Drive. Interview scheduled with user and manager.'},
    {id: 'xf67-AL-005', severity: 'medium', title: 'Credential stuffing attack detected on customer portal', source: 'Cloudflare WAF', timestamp: '2026-04-22T09:30:00Z', status: 'contained', assignee: 'SOC Analyst K. Wilson', asset: 'Customer Portal (portal.company.com)', mitreTechnique: 'T1110 - Brute Force', playbook: 'IR-CredentialStuffing-Block', sla: '2 hours', elapsed: '4 hours', notes: 'Rate limiting and CAPTCHA automatically deployed. 12,000 failed login attempts from 800 unique IPs blocked. No successful account takeovers confirmed.'},
    {id: 'xf67-AL-006', severity: 'low', title: 'SSL certificate expiring in 14 days for api.company.com', source: 'Certificate monitoring', timestamp: '2026-04-22T08:00:00Z', status: 'remediated', assignee: 'Infrastructure Lead T. Brown', asset: 'api.company.com', mitreTechnique: 'N/A', playbook: 'Certificate Renewal', sla: '30 days', elapsed: '16 days', notes: 'Automated renewal triggered via ACME. New certificate deployed successfully. No service interruption.'}
  ];

  private _xf67ComplianceFrameworks = [
  ...
  private _xf67DataClassificationPolicy = [
  ...
  private _getxf67ZoneSummary(): {total: number; highRisk: number; mediumRisk: number; lowRisk: number; totalRules: number} {
    const high = this._xf67SecurityZones.filter(z => z.riskLevel === 'high').length;
    const med = this._xf67SecurityZones.filter(z => z.riskLevel === 'medium').length;
    const low = this._xf67SecurityZones.filter(z => z.riskLevel === 'low').length;
    const rules = this._xf67SecurityZones.reduce((s, z) => s + z.firewallRules, 0);
    return {total: this._xf67SecurityZones.length, highRisk: high, mediumRisk: med, lowRisk: low, totalRules: rules};
  }

  private _getxf67AlertSummary(): {total: number; critical: number; high: number; medium: number; low: number; open: number; contained: number} {
    const crit = this._xf67SecurityAlerts.filter(a => a.severity === 'critical').length;
    const high = this._xf67SecurityAlerts.filter(a => a.severity === 'high').length;
    const med = this._xf67SecurityAlerts.filter(a => a.severity === 'medium').length;
    const low = this._xf67SecurityAlerts.filter(a => a.severity === 'low').length;
    const open = this._xf67SecurityAlerts.filter(a => a.status === 'investigating' || a.status === 'triaged').length;
    const contained = this._xf67SecurityAlerts.filter(a => a.status === 'contained' || a.status === 'remediated').length;
    return {total: this._xf67SecurityAlerts.length, critical: crit, high, medium: med, low, open, contained};
  }

  private _getxf67ComplianceSummary(): {total: number; fullyCompliant: number; mostlyCompliant: number; avgComplianceRate: number} {
    const full = this._xf67ComplianceFrameworks.filter(f => f.status === 'compliant').length;
    const mostly = this._xf67ComplianceFrameworks.filter(f => f.status === 'mostly_compliant').length;
    const avgRate = this._xf67ComplianceFrameworks.reduce((s, f) => s + (f.compliant / f.requirements * 100), 0) / this._xf67ComplianceFrameworks.length;
    return {total: this._xf67ComplianceFrameworks.length, fullyCompliant: full, mostlyCompliant: mostly, avgComplianceRate: Math.round(avgRate * 10) / 10};
  }
  ...
  // === Security Risk Register (ir_playbook) ===
  private _ir_playbookRiskRegister = [
    { id: "RSK-IR_-0001", title: "Data breach from unpatched systems", owner: "CTO", category: "Financial", status: "mitigating", trend: "stable", severity: "low", likelihood: 10, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-19", nextReview: "2026-07-12" },
    { id: "RSK-IR_-0002", title: "Ransomware attack on critical infrastructure", owner: "CRO", category: "Strategic", status: "closed", trend: "decreasing", severity: "low", likelihood: 2, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-07", nextReview: "2026-07-01" },
    { id: "RSK-IR_-0003", title: "Insider threat data exfiltration", owner: "VP Eng", category: "Compliance", status: "accepted", trend: "increasing", severity: "low", likelihood: 1, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-03", nextReview: "2026-07-28" },
    { id: "RSK-IR_-0004", title: "Supply chain compromise", owner: "Security Lead", category: "Technical", status: "open", trend: "stable", severity: "low", likelihood: 9, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-15", nextReview: "2026-07-01" },
    { id: "RSK-IR_-0005", title: "Cloud misconfiguration exposure", owner: "Risk Mgr", category: "Reputational", status: "mitigating", trend: "decreasing", severity: "critical", likelihood: 7, impact: 9, treatment: "Monitor and review quarterly", lastReview: "2026-04-16", nextReview: "2026-07-16" },
    { id: "RSK-IR_-0006", title: "Phishing campaign success rate", owner: "Compliance Dir", category: "Legal", status: "closed", trend: "increasing", severity: "high", likelihood: 8, impact: 2, treatment: "Monitor and review quarterly", lastReview: "2026-04-22", nextReview: "2026-07-12" },
    { id: "RSK-IR_-0007", title: "Third-party vendor data breach", owner: "IT Dir", category: "Regulatory", status: "accepted", trend: "stable", severity: "critical", likelihood: 3, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-23", nextReview: "2026-07-19" },
    { id: "RSK-IR_-0008", title: "Regulatory non-compliance penalty", owner: "DevOps Lead", category: "Third-Party", status: "open", trend: "decreasing", severity: "low", likelihood: 5, impact: 10, treatment: "Monitor and review quarterly", lastReview: "2026-04-09", nextReview: "2026-07-12" },
    { id: "RSK-IR_-0009", title: "Zero-day exploit in production", owner: "Architect", category: "Human Capital", status: "mitigating", trend: "increasing", severity: "medium", likelihood: 6, impact: 3, treatment: "Monitor and review quarterly", lastReview: "2026-04-22", nextReview: "2026-07-09" },
    { id: "RSK-IR_-0010", title: "Insufficient access controls", owner: "CISO", category: "Operational", status: "closed", trend: "stable", severity: "critical", likelihood: 8, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-06", nextReview: "2026-07-15" },
    { id: "RSK-IR_-0011", title: "DDoS attack on services", owner: "CTO", category: "Financial", status: "accepted", trend: "decreasing", severity: "critical", likelihood: 1, impact: 8, treatment: "Monitor and review quarterly", lastReview: "2026-04-13", nextReview: "2026-07-18" },
    { id: "RSK-IR_-0012", title: "Social engineering attack", owner: "CRO", category: "Strategic", status: "open", trend: "increasing", severity: "critical", likelihood: 1, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-02", nextReview: "2026-07-20" },
    { id: "RSK-IR_-0013", title: "API security vulnerability", owner: "VP Eng", category: "Compliance", status: "mitigating", trend: "stable", severity: "high", likelihood: 9, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-14", nextReview: "2026-07-21" },
    { id: "RSK-IR_-0014", title: "Mobile device compromise", owner: "Security Lead", category: "Technical", status: "closed", trend: "decreasing", severity: "medium", likelihood: 4, impact: 1, treatment: "Monitor and review quarterly", lastReview: "2026-04-12", nextReview: "2026-07-19" },
    { id: "RSK-IR_-0015", title: "Physical security breach", owner: "Risk Mgr", category: "Reputational", status: "accepted", trend: "increasing", severity: "critical", likelihood: 8, impact: 4, treatment: "Monitor and review quarterly", lastReview: "2026-04-11", nextReview: "2026-07-04" },
    { id: "RSK-IR_-0016", title: "Password policy weakness", owner: "Compliance Dir", category: "Legal", status: "open", trend: "stable", severity: "critical", likelihood: 10, impact: 7, treatment: "Monitor and review quarterly", lastReview: "2026-04-05", nextReview: "2026-07-14" },
    { id: "RSK-IR_-0017", title: "Network segmentation gap", owner: "IT Dir", category: "Regulatory", status: "mitigating", trend: "decreasing", severity: "low", likelihood: 10, impact: 9, treatment: "Monitor and review quarterly", lastReview: "2026-04-20", nextReview: "2026-07-27" },
    { id: "RSK-IR_-0018", title: "Encryption key management failure", owner: "DevOps Lead", category: "Third-Party", status: "closed", trend: "increasing", severity: "low", likelihood: 2, impact: 4, treatment: "Monitor and review quarterly", lastReview: "2026-04-23", nextReview: "2026-07-10" },
    { id: "RSK-IR_-0019", title: "Audit trail tampering", owner: "Architect", category: "Human Capital", status: "accepted", trend: "stable", severity: "medium", likelihood: 7, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-07", nextReview: "2026-07-19" },
    { id: "RSK-IR_-0020", title: "Business email compromise", owner: "CISO", category: "Operational", status: "open", trend: "decreasing", severity: "critical", likelihood: 8, impact: 6, treatment: "Monitor and review quarterly", lastReview: "2026-04-02", nextReview: "2026-07-09" },
  ];
  private _ir_playbookRiskFilter: string = 'all';
  private _ir_playbookRiskSeverity: string = 'all';
  private _ir_playbookRiskStatus: string = 'all';
  private _ir_playbookExpandedRisk: string = '';
  private _ir_playbookFilterRisks() {
    const reg = this._ir_playbookRiskRegister;
    return reg.filter(r => {
      if (this._ir_playbookRiskFilter !== "all" && r.category !== this._ir_playbookRiskFilter) return false;
      if (this._ir_playbookRiskSeverity !== "all" && r.severity !== this._ir_playbookRiskSeverity) return false;
      if (this._ir_playbookRiskStatus !== "all" && r.status !== this._ir_playbookRiskStatus) return false;
      return true;
    });
  }
  private _ir_playbookGetRiskScore(r: any) { return Math.round(r.likelihood * r.impact * 1.5); }
  private _ir_playbookGetRiskTrendIcon(trend: string) {
    if (trend === "increasing") return "\u2191";
    if (trend === "decreasing") return "\u2193";
    return "\u2192";
  }
  private _ir_playbookGetRiskColor(sev: string) {
    if (sev === "critical") return "#dc2626";
    if (sev === "high") return "#ea580c";
    if (sev === "medium") return "#d97706";
    return "#16a34a";
  }
  private _ir_playbookGetRiskCounts() {
    const reg = this._ir_playbookRiskRegister;
  ...
  private _ir_playbookGetTreatmentProgress() {
    const reg = this._ir_playbookRiskRegister;
    const treated = reg.filter(r => r.status === "mitigating" || r.status === "closed").length;
    return Math.round((treated / Math.max(reg.length, 1)) * 100);
  }

  // === Security Metrics API Gateway (ir_playbook) ===
  private _ir_playbookApiEndpoints = [
    { method: "GET", path: "/api/v1/threats", name: "Threat Intelligence Feed", status: "active", avgLatency: 235.4, reqPerMin: 2255.0, errorRate: 2.27, uptime: round(97.60438816569898,2), version: "v1.2.10" },
    { method: "POST", path: "/api/v1/scans/start", name: "Vulnerability Scanner", status: "active", avgLatency: 19.7, reqPerMin: 1884.0, errorRate: 1.83, uptime: round(96.31399394985553,2), version: "v2.7.7" },
    { method: "GET", path: "/api/v1/assets", name: "Asset Inventory", status: "active", avgLatency: 284.2, reqPerMin: 67.0, errorRate: 1.48, uptime: round(98.94073559010191,2), version: "v3.5.1" },
    { method: "POST", path: "/api/v1/alerts", name: "Alert Management", status: "active", avgLatency: 158.1, reqPerMin: 1090.0, errorRate: 2.4, uptime: round(97.89487465684611,2), version: "v2.8.8" },
    { method: "GET", path: "/api/v1/compliance", name: "Compliance Status", status: "active", avgLatency: 159.2, reqPerMin: 248.0, errorRate: 1.23, uptime: round(95.23206404388384,2), version: "v1.5.14" },
    { method: "PUT", path: "/api/v1/policies", name: "Policy Engine", status: "active", avgLatency: 59.3, reqPerMin: 387.0, errorRate: 0.49, uptime: round(96.91603604384025,2), version: "v2.3.18" },
    { method: "GET", path: "/api/v1/incidents", name: "Incident Tracker", status: "active", avgLatency: 66.4, reqPerMin: 1012.0, errorRate: 2.42, uptime: round(98.19011992209141,2), version: "v2.6.16" },
    { method: "POST", path: "/api/v1/forensics", name: "Forensics Collector", status: "degraded", avgLatency: 85.9, reqPerMin: 475.0, errorRate: 0.3, uptime: round(97.73420404533907,2), version: "v1.4.14" },
    { method: "GET", path: "/api/v1/risk/assess", name: "Risk Assessment", status: "active", avgLatency: 374.3, reqPerMin: 2297.0, errorRate: 0.5, uptime: round(98.87259050327583,2), version: "v1.8.1" },
    { method: "POST", path: "/api/v1/auth/verify", name: "Authentication", status: "active", avgLatency: 384.1, reqPerMin: 417.0, errorRate: 1.79, uptime: round(99.5935170496502,2), version: "v1.0.13" },
    { method: "GET", path: "/api/v1/logs/audit", name: "Audit Log Query", status: "active", avgLatency: 111.7, reqPerMin: 974.0, errorRate: 0.99, uptime: round(96.97340623058524,2), version: "v2.9.4" },
    { method: "PUT", path: "/api/v1/users/roles", name: "Role Management", status: "active", avgLatency: 127.2, reqPerMin: 2236.0, errorRate: 0.37, uptime: round(97.10089066760254,2), version: "v3.5.3" },
    { method: "POST", path: "/api/v1/encrypt", name: "Encryption Service", status: "active", avgLatency: 170.1, reqPerMin: 1482.0, errorRate: 2.41, uptime: round(95.23484691704846,2), version: "v3.6.13" },
    { method: "GET", path: "/api/v1/network/topo", name: "Network Topology", status: "maintenance", avgLatency: 42.6, reqPerMin: 1095.0, errorRate: 0.67, uptime: round(99.84069533723175,2), version: "v2.6.15" },
    { method: "DELETE", path: "/api/v1/sessions", name: "Session Manager", status: "active", avgLatency: 149.6, reqPerMin: 888.0, errorRate: 2.46, uptime: round(99.32128024047783,2), version: "v1.4.19" },
  ];
  private _ir_playbookApiKeys = [
    { id: "ak-000001", name: "key-ir_p-001", created: "2026-09-07", lastUsed: "2026-04-14", status: "active", calls: 27159, rateLimit: 500 },
    { id: "ak-000002", name: "key-ir_p-002", created: "2026-02-16", lastUsed: "2026-04-21", status: "active", calls: 26108, rateLimit: 100 },
    { id: "ak-000003", name: "key-ir_p-003", created: "2026-09-06", lastUsed: "2026-04-02", status: "active", calls: 20450, rateLimit: 100 },
    { id: "ak-000004", name: "key-ir_p-004", created: "2026-05-10", lastUsed: "2026-04-21", status: "active", calls: 21431, rateLimit: 100 },
    { id: "ak-000005", name: "key-ir_p-005", created: "2026-04-05", lastUsed: "2026-04-21", status: "revoked", calls: 35256, rateLimit: 5000 },
    { id: "ak-000006", name: "key-ir_p-006", created: "2026-09-15", lastUsed: "2026-04-16", status: "revoked", calls: 7705, rateLimit: 5000 },
    { id: "ak-000007", name: "key-ir_p-007", created: "2026-02-25", lastUsed: "2026-04-20", status: "active", calls: 933, rateLimit: 5000 },
    { id: "ak-000008", name: "key-ir_p-008", created: "2026-03-10", lastUsed: "2026-04-12", status: "active", calls: 32478, rateLimit: 500 },
  ];
  private _ir_playbookApiHealthSummary() {
    const eps = this._ir_playbookApiEndpoints;
  ...
  private _ir_playbookGetApiByMethod(method: string) { return this._ir_playbookApiEndpoints.filter(e=>e.method===method); }
  private _ir_playbookGetSlowEndpoints() { return this._ir_playbookApiEndpoints.filter(e=>e.avgLatency>200).sort((a,b)=>b.avgLatency-a.avgLatency); }
  private _ir_playbookGetHighErrorEndpoints() { return this._ir_playbookApiEndpoints.filter(e=>e.errorRate>1.0).sort((a,b)=>b.errorRate-a.errorRate); }

  // === Security Training Effectiveness (ir_playbook) ===
  private _ir_playbookTrainingModules = [
    { id: "TRN-001", name: "Security Fundamentals", completionRate: 66.9, avgScore: 61.8, behaviorChange: 66.8, enrolled: 262, completed: 147, duration: "120min", category: "mandatory" },
    { id: "TRN-002", name: "Phishing Awareness", completionRate: 46.0, avgScore: 82.1, behaviorChange: 46.4, enrolled: 151, completed: 82, duration: "120min", category: "mandatory" },
    { id: "TRN-003", name: "Social Engineering Defense", completionRate: 93.6, avgScore: 84.2, behaviorChange: 47.6, enrolled: 275, completed: 26, duration: "114min", category: "optional" },
    { id: "TRN-004", name: "Password Hygiene", completionRate: 65.3, avgScore: 74.7, behaviorChange: 47.9, enrolled: 251, completed: 141, duration: "99min", category: "mandatory" },
    { id: "TRN-005", name: "Data Classification", completionRate: 58.5, avgScore: 86.9, behaviorChange: 55.7, enrolled: 456, completed: 132, duration: "99min", category: "mandatory" },
    { id: "TRN-006", name: "Incident Response Basics", completionRate: 80.0, avgScore: 65.7, behaviorChange: 82.9, enrolled: 213, completed: 37, duration: "44min", category: "mandatory" },
    { id: "TRN-007", name: "Secure Coding", completionRate: 72.1, avgScore: 59.3, behaviorChange: 89.1, enrolled: 318, completed: 221, duration: "94min", category: "optional" },
    { id: "TRN-008", name: "Cloud Security", completionRate: 71.8, avgScore: 74.4, behaviorChange: 78.8, enrolled: 297, completed: 312, duration: "101min", category: "mandatory" },
    { id: "TRN-009", name: "Mobile Device Security", completionRate: 97.8, avgScore: 81.2, behaviorChange: 43.7, enrolled: 428, completed: 385, duration: "98min", category: "mandatory" },
    { id: "TRN-010", name: "Network Security", completionRate: 68.3, avgScore: 82.1, behaviorChange: 77.9, enrolled: 64, completed: 399, duration: "38min", category: "mandatory" },
    { id: "TRN-011", name: "Physical Security", completionRate: 86.8, avgScore: 63.1, behaviorChange: 87.6, enrolled: 369, completed: 313, duration: "41min", category: "optional" },
    { id: "TRN-012", name: "Regulatory Compliance", completionRate: 79.3, avgScore: 62.6, behaviorChange: 66.4, enrolled: 93, completed: 185, duration: "63min", category: "mandatory" },
    { id: "TRN-013", name: "Risk Management", completionRate: 51.0, avgScore: 69.7, behaviorChange: 87.2, enrolled: 74, completed: 414, duration: "90min", category: "mandatory" },
    { id: "TRN-014", name: "Cryptography Basics", completionRate: 77.3, avgScore: 75.6, behaviorChange: 85.8, enrolled: 195, completed: 441, duration: "55min", category: "mandatory" },
    { id: "TRN-015", name: "Access Control", completionRate: 56.4, avgScore: 80.5, behaviorChange: 37.2, enrolled: 357, completed: 206, duration: "97min", category: "optional" },
    { id: "TRN-016", name: "Vendor Management", completionRate: 79.0, avgScore: 91.2, behaviorChange: 70.9, enrolled: 125, completed: 100, duration: "31min", category: "mandatory" },
  ];
  private _ir_playbookPhishingResults = [
    { month: "2026-01", sent: 687, clicked: 121, reported: 377, clickRate: round(17.612809315866084,1), reportRate: round(54.87627365356623,1) },
    { month: "2026-02", sent: 609, clicked: 108, reported: 105, clickRate: round(17.733990147783253,1), reportRate: round(17.24137931034483,1) },
    { month: "2026-03", sent: 882, clicked: 217, reported: 389, clickRate: round(24.6031746031746,1), reportRate: round(44.10430839002268,1) },
    { month: "2026-04", sent: 877, clicked: 70, reported: 44, clickRate: round(7.98175598631699,1), reportRate: round(5.017103762827822,1) },
    { month: "2026-05", sent: 905, clicked: 52, reported: 351, clickRate: round(5.745856353591161,1), reportRate: round(38.78453038674033,1) },
    { month: "2026-06", sent: 321, clicked: 71, reported: 32, clickRate: round(22.118380062305295,1), reportRate: round(9.968847352024921,1) },
  ];
  private _ir_playbookTrainingROI = { totalInvestment: 158116, avgCostPerEmployee: 627, riskReductionPct: round(random.uniform(15,45),1), incidentReductionPct: round(random.uniform(10,35),1), complianceScoreGain: round(random.uniform(5,25),1) };
  private _ir_playbookLearningPaths = [
    { name: "Beginner Security Analyst", totalModules: 8, completedModules: 16, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 12, enrolled: 177 },
    { name: "Advanced Threat Hunter", totalModules: 16, completedModules: 3, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 33, enrolled: 108 },
    { name: "Security Architect", totalModules: 9, completedModules: 8, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 10, enrolled: 127 },
    { name: "Incident Responder", totalModules: 13, completedModules: 3, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 72, enrolled: 118 },
    { name: "Compliance Specialist", totalModules: 15, completedModules: 7, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 25, enrolled: 137 },
    { name: "DevSecOps Engineer", totalModules: 17, completedModules: 14, progress: round(random.uniform(10,95),1), estimatedHoursLeft: 45, enrolled: 49 },
  ];
  private _ir_playbookGetOverallCompletion() {
    const mods = this._ir_playbookTrainingModules;
    return round(mods.reduce((s,m)=>s+m.completionRate,0)/mods.length,1);
  }
  private _ir_playbookGetTopPerformers() { return [...this._ir_playbookTrainingModules].sort((a,b)=>b.avgScore-a.avgScore).slice(0,5); }
  private _ir_playbookGetModulesNeedingAttention() { return this._ir_playbookTrainingModules.filter(m=>m.completionRate<70||m.avgScore<65); }

  // === Security Governance Framework (ir_playbook) ===
  private _ir_playbookGovBodies = [
    { name: "Security Steering Committee", members: 6, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-13", nextMeeting: "2026-05-07", quorum: 5 },
    { name: "Risk Management Board", members: 12, chair: "CTO", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-12", nextMeeting: "2026-05-26", quorum: 6 },
    { name: "Data Governance Council", members: 9, chair: "CRO", meetingFreq: "Monthly", lastMeeting: "2026-04-01", nextMeeting: "2026-05-10", quorum: 4 },
    { name: "Compliance Oversight Board", members: 7, chair: "CDO", meetingFreq: "Monthly", lastMeeting: "2026-04-18", nextMeeting: "2026-05-26", quorum: 4 },
    { name: "Architecture Review Board", members: 6, chair: "VP Eng", meetingFreq: "Bi-weekly", lastMeeting: "2026-04-18", nextMeeting: "2026-05-06", quorum: 4 },
    { name: "Change Advisory Board", members: 15, chair: "CISO", meetingFreq: "Weekly", lastMeeting: "2026-04-06", nextMeeting: "2026-05-05", quorum: 5 },
    { name: "Incident Review Board", members: 10, chair: "CIO", meetingFreq: "As needed", lastMeeting: "2026-04-03", nextMeeting: "2026-05-02", quorum: 3 },
    { name: "Vendor Risk Committee", members: 7, chair: "CFO", meetingFreq: "Quarterly", lastMeeting: "2026-04-02", nextMeeting: "2026-05-19", quorum: 6 },
  ];
  private _ir_playbookDecisions = [
    { id: "DEC-0001", title: "Adopt zero-trust architecture framework", date: "2026-01-19", status: "approved", owner: "CISO" },
    { id: "DEC-0002", title: "Migrate to SIEM 2.0 platform", date: "2026-04-05", status: "implemented", owner: "CTO" },
    { id: "DEC-0003", title: "Implement DLP across all endpoints", date: "2026-03-07", status: "in-progress", owner: "Security Lead" },
    { id: "DEC-0004", title: "Mandate MFA for all external access", date: "2026-04-25", status: "pending", owner: "CRO" },
    { id: "DEC-0005", title: "Establish bug bounty program", date: "2026-03-15", status: "approved", owner: "VP Eng" },
    { id: "DEC-0006", title: "Deploy EDR solution enterprise-wide", date: "2026-04-05", status: "approved", owner: "CISO" },
    { id: "DEC-0007", title: "Conduct annual penetration testing", date: "2026-01-04", status: "implemented", owner: "CTO" },
    { id: "DEC-0008", title: "Implement network micro-segmentation", date: "2026-03-02", status: "in-progress", owner: "Security Lead" },
    { id: "DEC-0009", title: "Establish security champion program", date: "2026-04-22", status: "pending", owner: "CRO" },
    { id: "DEC-0010", title: "Migrate to passwordless authentication", date: "2026-03-23", status: "approved", owner: "VP Eng" },
  ];
  private _ir_playbookPolicyLifecycle = [
    { name: "Information Security Policy", version: "v4.2", status: "active", lastUpdated: "2026-02-06", nextReview: "2026-9-27", owner: "CISO" },
    { name: "Acceptable Use Policy", version: "v2.2", status: "active", lastUpdated: "2026-02-12", nextReview: "2026-12-26", owner: "Legal" },
    { name: "Data Retention Policy", version: "v2.2", status: "under-review", lastUpdated: "2026-03-26", nextReview: "2026-12-16", owner: "DPO" },
    { name: "Access Control Policy", version: "v2.0", status: "active", lastUpdated: "2026-04-09", nextReview: "2026-9-14", owner: "IAM Lead" },
    { name: "Incident Response Policy", version: "v2.4", status: "draft", lastUpdated: "2026-04-09", nextReview: "2026-11-06", owner: "IR Lead" },
    { name: "Business Continuity Plan", version: "v2.4", status: "active", lastUpdated: "2026-03-26", nextReview: "2026-12-24", owner: "BCP Mgr" },
    { name: "Vendor Management Policy", version: "v1.4", status: "active", lastUpdated: "2026-01-11", nextReview: "2026-8-05", owner: "Procurement" },
    { name: "Encryption Standard", version: "v1.2", status: "under-review", lastUpdated: "2026-03-27", nextReview: "2026-12-14", owner: "Security Arch" },
  ];
  private _ir_playbookGovMaturityScore = { overall: 5.0, riskManagement: 2.5, compliance: 2.7, incidentResponse: 4.5, awareness: 2.2, technology: 4.9 };
  private _ir_playbookGetPendingDecisions() { return this._ir_playbookDecisions.filter(d=>d.status==='pending'||d.status==='in-progress'); }
  private _ir_playbookGetActivePolicies() { return this._ir_playbookPolicyLifecycle.filter(p=>p.status==='active'); }
  private _ir_playbookGetEscalationPath() { return ["L1 Analyst","L2 Senior","Security Lead","CISO","Board"]; }

  // === Security Innovation Lab (ir_playbook) ===
  private _ir_playbookInnoProjects = [
    { id: "INN-001", name: "AI-Powered Threat Detection", description: "Machine learning models for real-time threat identification", status: "active", progress: 72, startDate: "2026-03-26", teamSize: 2, budget: 90247, milestones: 6, completedMilestones: 6 },
    { id: "INN-002", name: "Quantum-Resistant Cryptography", description: "Post-quantum encryption algorithm prototyping", status: "research", progress: 35, startDate: "2026-01-24", teamSize: 5, budget: 180143, milestones: 5, completedMilestones: 5 },
    { id: "INN-003", name: "Automated Red Teaming", description: "Autonomous penetration testing framework", status: "active", progress: 58, startDate: "2026-01-14", teamSize: 4, budget: 111808, milestones: 5, completedMilestones: 9 },
    { id: "INN-004", name: "Zero-Knowledge Authentication", description: "Privacy-preserving identity verification", status: "poc", progress: 88, startDate: "2026-03-14", teamSize: 8, budget: 160006, milestones: 6, completedMilestones: 1 },
    { id: "INN-005", name: "Blockchain Audit Trail", description: "Immutable security event logging", status: "active", progress: 45, startDate: "2026-03-05", teamSize: 4, budget: 159169, milestones: 6, completedMilestones: 1 },
    { id: "INN-006", name: "Behavioral Biometrics", description: "Continuous authentication via user behavior patterns", status: "research", progress: 22, startDate: "2026-03-04", teamSize: 6, budget: 72068, milestones: 10, completedMilestones: 8 },
    { id: "INN-007", name: "Deception Grid 2.0", description: "Advanced honeypot network with adaptive responses", status: "active", progress: 65, startDate: "2026-03-03", teamSize: 2, budget: 169943, milestones: 9, completedMilestones: 9 },
    { id: "INN-008", name: "Secure Enclave Integration", description: "Hardware-backed security for critical workloads", status: "poc", progress: 40, startDate: "2026-01-20", teamSize: 5, budget: 67991, milestones: 9, completedMilestones: 8 },
  ];
  private _ir_playbookTechEvaluations = [
    { name: "Rust for Security Tools", status: "evaluating", score: 10.7, recommendation: "Adopt", vendor: "Open Source" },
    { name: "eBPF for Runtime Detection", status: "completed", score: 3.5, recommendation: "Adopt", vendor: "AWS" },
    { name: "Confidential Computing", status: "planned", score: 10.7, recommendation: "Investigate", vendor: "Azure" },
    { name: "Homomorphic Encryption", status: "evaluating", score: 9.4, recommendation: "Pilot", vendor: "GCP" },
    { name: "SASE Architecture", status: "completed", score: 9.1, recommendation: "Adopt", vendor: "Multiple" },
    { name: "SOAR Platform 3.0", status: "planned", score: 3.6, recommendation: "Monitor", vendor: "Splunk" },
  ];
  private _ir_playbookCollaborationPartners = [
  ...
  private _ir_playbookInnoMetrics = { totalProjects: 8, activeProjects: 4, avgTimeToValue: "110 days", pocSuccessRate: round(random.uniform(55,85),1), researchToProduction: round(random.uniform(20,50),1), innovationIndex: round(random.uniform(6.0,9.5),1) };
  private _ir_playbookGetProjectByStatus(status: string) { return this._ir_playbookInnoProjects.filter(p=>p.status===status); }
  private _ir_playbookGetTopEvaluations() { return [...this._ir_playbookTechEvaluations].sort((a,b)=>b.score-a.score).slice(0,3); }



  // === Compliance Dashboard Extension (ir_playbook) ===
  private _ir_playbookComplianceFrameworks = [
    { name: "SOC 2 Type II", description: "Trust Services Criteria", totalControls: 5, implementedControls: 4, status: "in-review", lastAudit: "2026-02-01", nextAudit: "2026-11-04", evidenceCount: 194 },
    { name: "ISO 27001", description: "Information Security Management", totalControls: 114, implementedControls: 84, status: "non-compliant", lastAudit: "2026-04-19", nextAudit: "2026-9-25", evidenceCount: 280 },
    { name: "PCI DSS 4.0", description: "Payment Card Industry", totalControls: 12, implementedControls: 11, status: "in-review", lastAudit: "2026-03-01", nextAudit: "2026-9-09", evidenceCount: 151 },
    { name: "HIPAA", description: "Health Insurance Portability", totalControls: 18, implementedControls: 9, status: "non-compliant", lastAudit: "2026-01-28", nextAudit: "2026-7-18", evidenceCount: 457 },
    { name: "GDPR", description: "Data Protection Regulation", totalControls: 99, implementedControls: 79, status: "compliant", lastAudit: "2026-01-12", nextAudit: "2026-10-16", evidenceCount: 314 },
    { name: "NIST CSF 2.0", description: "Cybersecurity Framework", totalControls: 6, implementedControls: 3, status: "compliant", lastAudit: "2026-03-21", nextAudit: "2026-9-08", evidenceCount: 120 },
    { name: "FedRAMP", description: "Federal Risk Authorization", totalControls: 15, implementedControls: 12, status: "compliant", lastAudit: "2026-04-22", nextAudit: "2026-9-16", evidenceCount: 450 },
    { name: "SOX", description: "Sarbanes-Oxley Compliance", totalControls: 8, implementedControls: 8, status: "in-review", lastAudit: "2026-04-14", nextAudit: "2026-9-28", evidenceCount: 327 },
    { name: "CIS Controls v8", description: "Center for Internet Security", totalControls: 18, implementedControls: 9, status: "compliant", lastAudit: "2026-03-17", nextAudit: "2026-10-09", evidenceCount: 212 },
    { name: "COBIT 2019", description: "IT Governance Framework", totalControls: 40, implementedControls: 36, status: "non-compliant", lastAudit: "2026-02-15", nextAudit: "2026-7-03", evidenceCount: 141 },
  ];
  private _ir_playbookGetComplianceScore() {
    const fw = this._ir_playbookComplianceFrameworks;
    return round(fw.reduce((s,f)=>s + (f.implementedControls/Math.max(f.totalControls,1))*100, 0) / fw.length, 1);
  }
  private _ir_playbookGetGaps() {
    return this._ir_playbookComplianceFrameworks.filter(f => f.status !== "compliant");
  }
  private _ir_playbookAuditTrail = [
    { id: "AUD-0001", action: "Control tested", auditor: "Internal Audit", date: "2026-04-01", result: "pass", findings: 0 },
    { id: "AUD-0002", action: "Evidence collected", auditor: "External Auditor", date: "2026-04-22", result: "pass", findings: 0 },
    { id: "AUD-0003", action: "Gap identified", auditor: "Security Team", date: "2026-04-12", result: "fail", findings: 5 },
    { id: "AUD-0004", action: "Remediation completed", auditor: "Compliance Officer", date: "2026-04-07", result: "pass", findings: 1 },
    { id: "AUD-0005", action: "Policy updated", auditor: "IT Audit", date: "2026-04-04", result: "pass", findings: 1 },
    { id: "AUD-0006", action: "Training verified", auditor: "Risk Team", date: "2026-04-12", result: "pass", findings: 3 },
    { id: "AUD-0007", action: "Access reviewed", auditor: "QA Team", date: "2026-04-04", result: "pass", findings: 0 },
    { id: "AUD-0008", action: "Exception approved", auditor: "CISO Office", date: "2026-04-08", result: "conditional", findings: 0 },
    { id: "AUD-0009", action: "Risk accepted", auditor: "Board Audit", date: "2026-04-17", result: "pass", findings: 1 },
    { id: "AUD-0010", action: "Control enhanced", auditor: "Third Party", date: "2026-04-18", result: "pass", findings: 2 },
  ];

  // === Threat Intelligence Feed Extension (ir_playbook) ===
  private _ir_playbookThreatActors = [
    { name: "APT-29", alias: "Cozy Bear", origin: "Russia", type: "Nation-State", severity: "high", lastActivity: "2026-04-14", targets: "Government", indicators: 282, ttps: 27 },
    { name: "APT-41", alias: "Double Dragon", origin: "China", type: "Nation-State", severity: "critical", lastActivity: "2026-04-05", targets: "Technology", indicators: 266, ttps: 49 },
    { name: "Lazarus Group", alias: "Hidden Cobra", origin: "North Korea", type: "Nation-State", severity: "critical", lastActivity: "2026-04-14", targets: "Technology", indicators: 473, ttps: 33 },
    { name: "FIN7", alias: "Carbanak", origin: "Eastern Europe", type: "Financial", severity: "high", lastActivity: "2026-04-11", targets: "Technology", indicators: 476, ttps: 24 },
    { name: "Conti", alias: "Wizard Spider", origin: "Russia", type: "Ransomware", severity: "critical", lastActivity: "2026-04-07", targets: "Technology", indicators: 125, ttps: 5 },
    { name: "LockBit", alias: "LockBit Gang", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-11", targets: "Technology", indicators: 346, ttps: 30 },
    { name: "Cl0p", alias: "Cl0p Team", origin: "Unknown", type: "Ransomware", severity: "high", lastActivity: "2026-04-09", targets: "Government", indicators: 378, ttps: 48 },
    { name: "Sandworm", alias: "Unit 74455", origin: "Russia", type: "Nation-State", severity: "critical", lastActivity: "2026-04-01", targets: "Energy", indicators: 202, ttps: 20 },
  ];
  private _ir_playbookIoCFeed = [
    { id: "ioc-000001", type: "ip", value: "109.133.165.191", confidence: 48, source: "Mandiant", firstSeen: "2026-04-18", lastSeen: "2026-04-04" },
    { id: "ioc-000002", type: "ip", value: "224.64.78.56", confidence: 93, source: "MISP", firstSeen: "2026-04-09", lastSeen: "2026-04-20" },
    { id: "ioc-000003", type: "ip", value: "198.238.44.161", confidence: 91, source: "STIX", firstSeen: "2026-04-16", lastSeen: "2026-04-21" },
    { id: "ioc-000004", type: "ip", value: "241.228.210.131", confidence: 82, source: "Mandiant", firstSeen: "2026-04-14", lastSeen: "2026-04-20" },
    { id: "ioc-000005", type: "ip", value: "177.23.61.56", confidence: 63, source: "CrowdStrike", firstSeen: "2026-04-20", lastSeen: "2026-04-17" },
    { id: "ioc-000006", type: "ip", value: "246.105.197.94", confidence: 85, source: "VirusTotal", firstSeen: "2026-04-12", lastSeen: "2026-04-16" },
    { id: "ioc-000007", type: "ip", value: "71.228.254.163", confidence: 51, source: "VirusTotal", firstSeen: "2026-04-01", lastSeen: "2026-04-19" },
    { id: "ioc-000008", type: "ip", value: "163.208.96.240", confidence: 62, source: "AlienVault", firstSeen: "2026-04-04", lastSeen: "2026-04-02" },
    { id: "ioc-000009", type: "ip", value: "60.4.231.89", confidence: 42, source: "VirusTotal", firstSeen: "2026-04-13", lastSeen: "2026-04-05" },
    { id: "ioc-000010", type: "ip", value: "83.33.213.36", confidence: 51, source: "CrowdStrike", firstSeen: "2026-04-02", lastSeen: "2026-04-21" },
    { id: "ioc-000011", type: "ip", value: "92.15.49.97", confidence: 51, source: "AlienVault", firstSeen: "2026-04-14", lastSeen: "2026-04-15" },
    { id: "ioc-000012", type: "ip", value: "63.86.7.47", confidence: 56, source: "Mandiant", firstSeen: "2026-04-04", lastSeen: "2026-04-14" },
    { id: "ioc-000013", type: "ip", value: "246.162.93.12", confidence: 91, source: "STIX", firstSeen: "2026-04-01", lastSeen: "2026-04-08" },
    { id: "ioc-000014", type: "ip", value: "64.60.184.48", confidence: 77, source: "AlienVault", firstSeen: "2026-04-17", lastSeen: "2026-04-02" },
    { id: "ioc-000015", type: "ip", value: "190.179.206.94", confidence: 67, source: "VirusTotal", firstSeen: "2026-04-18", lastSeen: "2026-04-11" },
  ];
  private _ir_playbookGetActiveThreats() { return this._ir_playbookThreatActors.filter(a => a.severity === 'critical'); }
  private _ir_playbookGetThreatSummary() {
    const actors = this._ir_playbookThreatActors;
  ...
  // === Incident Management Extension (ir_playbook) ===
  private _ir_playbookIncidents = [
    { id: "INC-20260001", title: "Unauthorized access detected", severity: "critical", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-05T11:07", affectedAssets: 2, rootCause: "Misconfiguration" },
    { id: "INC-20260002", title: "Malware outbreak on workstation", severity: "high", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-10T05:02", affectedAssets: 20, rootCause: "Credential compromise" },
    { id: "INC-20260003", title: "Data leak from S3 bucket", severity: "medium", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-01T13:06", affectedAssets: 10, rootCause: "Zero-day" },
    { id: "INC-20260004", title: "Phishing campaign targeting finance", severity: "low", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-10T19:54", affectedAssets: 18, rootCause: "Human error" },
    { id: "INC-20260005", title: "DDoS attack on web services", severity: "critical", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-20T08:24", affectedAssets: 45, rootCause: "Policy violation" },
    { id: "INC-20260006", title: "Ransomware encryption attempt", severity: "high", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-01T23:44", affectedAssets: 1, rootCause: "Unknown" },
    { id: "INC-20260007", title: "Insider data exfiltration", severity: "medium", status: "open", assignedTo: "SOC L1", detectedAt: "2026-04-20T05:07", affectedAssets: 33, rootCause: "Misconfiguration" },
    { id: "INC-20260008", title: "API key exposure in repo", severity: "low", status: "investigating", assignedTo: "SOC L2", detectedAt: "2026-04-10T03:05", affectedAssets: 20, rootCause: "Credential compromise" },
    { id: "INC-20260009", title: "SQL injection on portal", severity: "critical", status: "contained", assignedTo: "IR Lead", detectedAt: "2026-04-18T06:37", affectedAssets: 15, rootCause: "Zero-day" },
    { id: "INC-20260010", title: "Brute force on VPN", severity: "high", status: "eradicated", assignedTo: "CISO", detectedAt: "2026-04-21T08:47", affectedAssets: 23, rootCause: "Human error" },
    { id: "INC-20260011", title: "Supply chain alert from vendor", severity: "medium", status: "recovered", assignedTo: "Security Eng", detectedAt: "2026-04-10T01:26", affectedAssets: 39, rootCause: "Policy violation" },
    { id: "INC-20260012", title: "Suspicious lateral movement", severity: "low", status: "closed", assignedTo: "Forensics", detectedAt: "2026-04-09T13:17", affectedAssets: 45, rootCause: "Unknown" },
  ];
  private _ir_playbookGetIncidentStats() {
    const inc = this._ir_playbookIncidents;
    return { total: inc.length, open: inc.filter(i=>i.status==="open").length, investigating: inc.filter(i=>i.status==="investigating").length, mttd: 27, mttr: 34 };
  }
  private _ir_playbookGetSeverityDistribution() {
    const inc = this._ir_playbookIncidents;
...
<div class="tabs">
        ...
        ${this._activeTab === 'rules' ? html`
          ${this._renderIrpRules()}
          <div style="margin-top:12px;font-size:11px;font-weight:600;margin-bottom:6px">Category Treemap</div>
          <div style="background:#1a1d27;border-radius:8px;padding:12px">${this._renderIrpTreemapSVG()}</div>
          <div style="margin-top:12px;font-size:11px;font-weight:600;margin-bottom:6px">Data Flow (Sankey)</div>
          <div style="background:#1a1d27;border-radius:8px;padding:12px">${this._renderIrpSankeySVG()}</div>
          ${this._renderIrpActivity()}
        ` : nothing}
        ${this._activeTab === 'anomalies' ? html`
          ${this._renderIrpAnomalies()}
          ${this._renderIrpNotifications()}
        ` : nothing}
        ${this._activeTab === 'predictions' ? html`
          ${this._renderIrpPredictions()}
          ...
          <div style="background:#1a1d27;border-radius:8px;padding:12px">
            <table class="history-table"><thead><tr><th>ID</th><th>Base</th><th>Temporal</th><th>Environmental</th><th>Overall</th></tr></thead><tbody>
              ${this._irpcvssData.map(c => html`<tr><td>${c.itemId}</td><td style="color:${c.base >= 7 ? '#ef4444' : c.base >= 4 ? '#eab308' : '#22c55e'}">${c.base}</td><td>${c.temporal}</td><td>${c.environmental}</td><td style="font-weight:700;color:${c.overall >= 7 ? '#ef4444' : c.overall >= 4 ? '#eab308' : '#22c55e'}">${c.overall}</td></tr>`)}
            </tbody></table>
          </div>
        ` : nothing}
        ${this._activeTab === 'approvals' ? html`
          ${this._renderIrpApprovals()}
        ` : nothing}
        ${this._activeTab === 'config' ? html`
          ${this._renderIrpConfig()}
        ` : nothing}
        ${this._activeTab === 'new' ? html`
          <div class="form-section">
            <div class="form-title">Create New Ir Playbook Entry</div>
            <div class="form-grid">
  ...
  @state() private _ipScenarios: {id:string;name:string;attackType:string;target:string;method:string;impactLow:number;impactHigh:number;confidence:number;mitigation:string;status:string}[] = [];
  @state() private _ipScenarioForm: {attackType:string;target:string;method:string} = {attackType:'',target:'',method:''};
  @state() private _ipScenarioCompare: boolean = false;
  @state() private _ipScenarioSelected: string[] = [];

  private _ipInitScenarios(): void {
    const saved = localStorage.getItem('ip_scenarios');
    if (saved) { try { this._ipScenarios = JSON.parse(saved); } catch { /* ignore */ } }
    if (this._ipScenarios.length === 0) {
      this._ipScenarios = [
        {id:'ip-s1',name:'Baseline Threat',attackType:'Phishing',target:'Employees',method:'Spear Email',impactLow:45,impactHigh:78,confidence:72,mitigation:'Security awareness training + email filtering',status:'active'},
        {id:'ip-s2',name:'Escalated Attack',attackType:'Ransomware',target:'Endpoints',method:'Drive-by Download',impactLow:65,impactHigh:95,confidence:58,mitigation:'EDR deployment + network segmentation',status:'saved'},
        {id:'ip-s3',name:'Insider Threat',attackType:'Data Exfiltration',target:'Databases',method:'SQL Injection',impactLow:55,impactHigh:88,confidence:65,mitigation:'DLP policies + query monitoring',status:'draft'},
      ];
    }
  }

  private _ipSaveScenarios(): void {
    localStorage.setItem('ip_scenarios', JSON.stringify(this._ipScenarios));
  }

  private _ipAddScenario(): void {
    const f = this._ipScenarioForm;
    if (!f.attackType || !f.target) return;
    this._ipScenarios = [...this._ipScenarios, {
      id: 'ip-s' + (this._ipScenarios.length + 1),
    ...
    this._ipScenarioForm = {attackType:'',target:'',method:''};
    this._ipSaveScenarios();
  }

  private _ipRenderScenarioEngine(): any {
    ...
    return html`
      <div style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          ...
          <button class="tab" @click=${() => { this._ipScenarioCompare = !this._ipScenarioCompare; }}>${this._ipScenarioCompare ? 'List View' : 'Compare'}</button>
        </div>
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:10px">
          <select style="background:#1a1d2e;border:1px solid #2a2d3a;border-radius:4px;padding:4px 6px;color:#e2e8f0;font-size:11px" @change=${(e: any) => { this._ipScenarioForm = {...this._ipScenarioForm, attackType: (e.target as HTMLSelectElement).value}; }}>
            <option value="">Attack Type</option>
            ${attackTypes.map(a => html`<option value=${a} ${this._ipScenarioForm.attackType === a ? 'selected' : ''}>${a}</option>`)}
          </select>
          <select style="background:#1a1d2e;border:1px solid #2a2d3a;border-radius:4px;padding:4px 6px;color:#e2e8f0;font-size:11px" @change=${(e: any) => { this._ipScenarioForm = {...this._ipScenarioForm, target: (e.target as HTMLSelectElement).value}; }}>
            <option value="">Target</option>
            ${targets.map(t => html`<option value=${t} ${this._ipScenarioForm.target === t ? 'selected' : ''}>${t}</option>`)}
          </select>
          <select style="background:#1a1d2e;border:1px solid #2a2d3a;border-radius:4px;padding:4px 6px;color:#e2e8f0;font-size:11px" @change=${(e: any) => { this._ipScenarioForm = {...this._ipScenarioForm, method: (e.target as HTMLSelectElement).value}; }}>
            <option value="">Method</option>
            ${methods.map(m => html`<option value=${m} ${this._ipScenarioForm.method === m ? 'selected' : ''}>${m}</option>`)}
          </select>
        </div>
        <button style="background:#3b82f6;border:none;border-radius:4px;padding:4px 12px;color:#fff;font-size:11px;cursor:pointer" @click=${this._ipAddScenario}>Run Simulation</button>
      </div>
      ${this._ipScenarioCompare && this._ipScenarios.length >= 2 ? html`
        <div style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
          <div style="font-weight:600;font-size:12px;color:#e2e8f0;margin-bottom:8px">Side-by-Side Comparison</div>
          <div style="display:grid;grid-template-columns:repeat(${Math.min(3, this._ipScenarios.length)},1fr);gap:8px">
            ${this._ipScenarios.slice(0,3).map(s => html`
      ...
      <div style="background:#0f1117;border-radius:8px;padding:12px">
        <div style="font-weight:600;font-size:12px;color:#e2e8f0;margin-bottom:8px">Saved Scenarios (${this._ipScenarios.length})</div>
        ${this._ipScenarios.map(s => html`
  ...
  @state() private _ipTrendData: {day:number;value:number;anomaly:boolean}[] = [];
  @state() private _ipTrendZoom: {start:number;end:number} | null = null;
  @state() private _ipTrendMA: number = 7;

  private _ipInitTrendData(): void {
    ...
    this._ipTrendData = data;
  }

  private _ipCalcMA(window: number): number[] {
    const result: number[] = [];
    for (let i = 0; i < this._ipTrendData.length; i++) {
      const start = Math.max(0, i - window + 1);
      const slice = this._ipTrendData.slice(start, i + 1);
      result.push(slice.reduce((s, d) => s + d.value, 0) / slice.length);
    }
    return result;
  }

  private _ipGetStats(): {mean:number;median:number;stddev:number;trend:string} {
    const vals = this._ipTrendData.map(d => d.value);
    const n = vals.length;
  ...
  private _ipRenderTimeSeries(): any {
    const stats = this._ipGetStats();
    const filtered = this._ipTrendZoom ? this._ipTrendData.filter(d => d.day >= this._ipTrendZoom.start && d.day <= this._ipTrendZoom.end) : this._ipTrendData;
    ...
    return html`
      <div style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          ...
          <div style="display:flex;gap:4px">
            <button class="tab ${this._ipTrendMA === 7 ? 'active' : ''}" @click=${() => { this._ipTrendMA = 7; }}>7D MA</button>
            <button class="tab ${this._ipTrendMA === 30 ? 'active' : ''}" @click=${() => { this._ipTrendMA = 30; }}>30D MA</button>
            <button class="tab" @click=${() => { this._ipTrendZoom = null; }}>Reset Zoom</button>
          </div>
        </div>
        <div style="position:relative;height:120px;background:#1a1d2e;border-radius:6px;overflow:hidden;margin-bottom:8px;cursor:crosshair" @click=${(e: any) => {
          ...
          this._ipTrendZoom = { start: Math.max(0, center - 10), end: Math.min(89, center + 10) };
        }}>
  ...
  @state() private _ipRoles: string[] = ['Admin','Analyst','Operator','Viewer','Auditor'];
  @state() private _ipActions: string[] = ['Create','Read','Update','Delete','Export','Approve'];
  @state() private _ipPermissions: { [role: string]: { [action: string]: boolean } } = {};
  @state() private _ipPermAudit: {role:string;action:string;changedBy:string;timestamp:string;oldVal:boolean;newVal:boolean}[] = [];
  @state() private _ipPermCompare: string[] = [];

  private _ipInitPermissions(): void {
    ...
    for (const role of this._ipRoles) {
      perms[role] = {};
      this._ipActions.forEach((a, i) => { perms[role][a] = defaults[role]?.[i] ?? false; });
    }
    this._ipPermissions = perms;
  }

  private _ipTogglePermission(role: string, action: string): void {
    const old = this._ipPermissions[role][action];
    this._ipPermissions = {...this._ipPermissions, [role]: {...this._ipPermissions[role], [action]: !old}};
    this._ipPermAudit.unshift({role,action,changedBy:'current_user',timestamp:new Date().toISOString(),oldVal:old,newVal:!old});
  }

  private _ipRenderRBAC(): any {
    const compareRoles = this._ipPermCompare.map(r => this._ipPermissions[r]).filter(Boolean);
    return html`
      <div style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          <span style="font-weight:700;font-size:13px;color:#e2e8f0">RBAC Permission Matrix</span>
          <div style="display:flex;gap:4px">
            ${this._ipRoles.map(r => html`
              <button class="tab ${this._ipPermCompare.includes(r) ? 'active' : ''}" @click=${() => {
                this._ipPermCompare = this._ipPermCompare.includes(r) ? this._ipPermCompare.filter(x => x !== r) : [...this._ipPermCompare, r];
              }}>${r}</button>
            `)}
          </div>
        </div>
          ...
          <table style="width:100%;border-collapse:collapse;font-size:10px">
            <thead>
              <tr>
                ...
                ${this._ipActions.map(a => html`<th style="padding:6px;color:#6b7280;border-bottom:1px solid #2a2d3a;text-align:center">${a}</th>`)}
              </tr>
            </thead>
            <tbody>
              ${this._ipRoles.map(role => html`
                <tr style="border-bottom:1px solid #1a1d2e">
                  <td style="padding:6px;color:#e2e8f0;font-weight:600">${role}</td>
                  ${this._ipActions.map(action => html`
                    <td style="text-align:center;padding:6px">
                      <button style="width:28px;height:20px;border-radius:3px;border:1px solid #2a2d3a;background:${this._ipPermissions[role][action] ? '#22c55e' : '#1a1d2e'};cursor:pointer;color:#fff;font-size:10px" @click=${() => this._ipTogglePermission(role, action)}>${this._ipPermissions[role][action] ? 'Y' : 'N'}</button>
                    </td>
                  `)}
                </tr>
              `)}
        ...
        ${compareRoles.length >= 2 ? html`
          <div style="margin-top:10px;padding:8px;background:#1a1d2e;border-radius:6px">
            <div style="font-size:11px;font-weight:600;color:#e2e8f0;margin-bottom:6px">Role Diff: ${this._ipPermCompare.join(' vs ')}</div>
            ${this._ipActions.map(action => {
              ...
              return allSame ? nothing : html`
                <div style="display:flex;gap:8px;padding:3px 0;font-size:10px">
                  ...
                  ${compareRoles.map((r, i) => html`<span style="color:${r[action] ? '#22c55e' : '#ef4444'}">${this._ipPermCompare[i]}=${r[action] ? 'Y' : 'N'}</span>`)}
                </div>
              `;
            })}
          </div>
        ` : nothing}
        ${this._ipPermAudit.length > 0 ? html`
          <div style="margin-top:8px;font-size:9px;color:#6b7280">Recent: ${this._ipPermAudit.slice(0,3).map(a => html`<span style="margin-right:8px">${a.role}.${a.action}: ${a.oldVal ? 'Y' : 'N'}->${a.newVal ? 'Y' : 'N'}</span>`)}
        ` : nothing}
      </div>
    `;
  }
  ...
  // === REPORTING SUITE ===
  @state() private _ipReportTemplate: string = 'executive';
  @state() private _ipReportSchedule: string = 'weekly';
  @state() private _ipReportDistList: string[] = ['security-team@company.com','ciso@company.com'];
  @state() private _ipReportHistory: {id:string;template:string;generatedAt:string;status:string}[] = [];

  private _ipGenerateReport(): void {
    const id = 'rpt-' + Date.now();
    this._ipReportHistory.unshift({id,template:this._ipReportTemplate,generatedAt:new Date().toISOString(),status:'sent'});
  }

  private _ipRenderReporting(): any {
    ...
    return html`
      <div style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">
          <div>
            ...
            ${templates.map(t => html`
              <div style="padding:6px;background:${this._ipReportTemplate === t.key ? '#1e3a5f' : '#1a1d2e'};border:1px solid ${this._ipReportTemplate === t.key ? '#3b82f6' : '#2a2d3a'};border-radius:4px;margin-bottom:4px;cursor:pointer" @click=${() => { this._ipReportTemplate = t.key; }}>
          ...
          <div>
            ...
            <div style="display:flex;gap:4px;margin-bottom:8px">
              ${schedules.map(s => html`<button class="tab ${this._ipReportSchedule === s.key ? 'active' : ''}" @click=${() => { this._ipReportSchedule = s.key; }}>${s.label}</button>`)}
            </div>
            <div style="font-size:10px;color:#6b7280;margin-bottom:4px">Distribution</div>
            <div style="font-size:10px;color:#9ca3af">${this._ipReportDistList.map(d => html`<div>${d}</div>`)}</div>
            <div style="margin-top:6px;display:flex;gap:4px">
              <button style="background:#3b82f6;border:none;border-radius:4px;padding:4px 10px;color:#fff;font-size:10px;cursor:pointer" @click=${this._ipGenerateReport}>Generate</button>
        ...
        ${this._ipReportHistory.length > 0 ? html`
          <div style="border-top:1px solid #2a2d3a;padding-top:8px">
            ...
            ${this._ipReportHistory.slice(0,3).map(r => html`
  ...
  @state() private _ipHighContrast: boolean = false;
  @state() private _ipA11yAnnounce: string = '';
  @state() private _ipShortcutsVisible: boolean = false;
  @state() private _ipFocusTrap: boolean = false;

  private _ipShortcuts: Record<string, string> = {
  ...
  private _ipHandleKeydown(e: KeyboardEvent): void {
    if (e.key === 'Escape' && this._ipFocusTrap) { this._ipFocusTrap = false; this._ipAnnounce('Dialog closed'); }
    if (e.ctrlKey && e.shiftKey && e.key === 'H') { e.preventDefault(); this._ipHighContrast = !this._ipHighContrast; this._ipAnnounce('High contrast ' + (this._ipHighContrast ? 'enabled' : 'disabled')); }
    if (e.ctrlKey && e.shiftKey && e.key === 'A') { e.preventDefault(); this._ipShortcutsVisible = !this._ipShortcutsVisible; }
  }

  private _ipAnnounce(msg: string): void {
    this._ipA11yAnnounce = msg;
    setTimeout(() => { this._ipA11yAnnounce = ''; }, 2000);
  }

  private _ipRenderAccessibility(): any {
    return html`
      <div role="region" aria-label="Accessibility Controls" style="background:#0f1117;border-radius:8px;padding:12px;margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          ...
          <button class="tab ${this._ipShortcutsVisible ? 'active' : ''}" @click=${() => { this._ipShortcutsVisible = !this._ipShortcutsVisible; }} aria-expanded=${this._ipShortcutsVisible}>Shortcuts</button>
        </div>
        <div style="display:flex;gap:8px;margin-bottom:8px">
          <label style="display:flex;align-items:center;gap:6px;font-size:11px;color:#e2e8f0;cursor:pointer">
            <input type="checkbox" .checked=${this._ipHighContrast} @change=${() => { this._ipHighContrast = !this._ipHighContrast; }} aria-label="Toggle high contrast mode">
            High Contrast
          </label>
        </div>
        ${this._ipShortcutsVisible ? html`
          <div role="list" aria-label="Keyboard shortcuts" style="background:#1a1d2e;border-radius:6px;padding:8px">
            ${Object.entries(this._ipShortcuts).map(([key, desc]) => html`
        ...
        ` : nothing}
        <div role="status" aria-live="polite" aria-atomic="true" style="position:absolute;left:-9999px">${this._ipA11yAnnounce}</div>
      </div>
    `;
  }
  ...
  connectedCallback(): void {
    ...
    this._ipInitScenarios();
    this._ipInitTrendData();
    this._ipInitPermissions();
    document.addEventListener('keydown', this._ipHandleKeydown.bind(this));
  }

  disconnectedCallback(): void {
    super.disconnectedCallback();
    document.removeEventListener('keydown', this._ipHandleKeydown.bind(this));
  }

  // === TAB INTEGRATION FOR EXTENDED FEATURES ===
  @state() private _ipActiveSubTab: string = 'scenario';
  ...
  private _ipGetAllSubTabs(): {key:string;label:string}[] {
    return [
  ...
  private _ipRenderSubPanel(): any {
    switch (this._ipActiveSubTab) {
      case 'scenario': return this._ipRenderScenarioEngine();
      case 'timeseries': return this._ipRenderTimeSeries();
      case 'rbac': return this._ipRenderRBAC();
      case 'reporting': return this._ipRenderReporting();
      case 'a11y': return this._ipRenderAccessibility();
      default: return nothing;
    }
  }

  private _ipRenderTabBar(): any {
    return html`
      <div style="display:flex;gap:4px;margin-bottom:12px;border-bottom:1px solid #2a2d3a;padding-bottom:8px;flex-wrap:wrap" role="tablist" aria-label="Extended panel features">
        ${this._ipGetAllSubTabs().map(t => html`
          <button class="tab ${this._ipActiveSubTab === t.key ? 'active' : ''}" @click=${() => { this._ipActiveSubTab = t.key; }} role="tab" aria-selected=${this._ipActiveSubTab === t.key}>${t.label}</button>
        `)}
      </div>
      <div role="tabpanel" aria-labelledby="ip-tab-${this._ipActiveSubTab}">
        ${this._ipRenderSubPanel()}
      </div>
    `;
  }
  ...
  @state() private _soarPlaybooks: Array<{
    ...
    {
      id: 'pb-soar-001', name: 'Ir Playbook Auto Response', status: 'active',
      steps: [
    ...
    {
      id: 'pb-soar-002', name: 'Ir Playbook Investigation Workflow', status: 'active',
      steps: [
    ...
    {
      id: 'pb-soar-003', name: 'Ir Playbook Compliance Scan', status: 'draft',
      steps: [
  ...
  @state() private _ipAle: number = 0;
  @state() private _ipSroi: number = 0;
  @state() private _ipCpi: number = 0;
  @state() private _ipBudgetAlloc: number = 0;
  @state() private _ipCostBenefit: number = 0;

  // Security Economics Calculator
  private ipInitEconomics() {
    this._ipAle = Math.round(2850000 + Math.random() * 4500000);
    this._ipSroi = Math.round(180 + Math.random() * 320);
    this._ipCpi = Math.round(45000 + Math.random() * 120000);
    this._ipBudgetAlloc = Math.round(2500000 + Math.random() * 3000000);
    this._ipCostBenefit = Math.round(220 + Math.random() * 180);
  }

  private _ipCalcAle(): { annual: number; perIncident: number; byCategory: Array<{name: string; value: number}> } {
    const base = this._ipAle;
    const categories = [
  ...
  private _ipCalcSroi(): Array<{year: number; investment: number; savings: number; roi: number}> {
    const baseInv = this._ipSroi * 10000;
  ...
  private _ipGetBudgetAlloc(): Array<{category: string; amount: number; pct: number; trend: string}> {
    const total = this._ipBudgetAlloc;
    const items = [
  ...
  private _ipGetCostBenefit(): Array<{control: string; cost: number; benefit: number; ratio: number; priority: string}> {
    const base = this._ipCostBenefit;
    const controls = [
  ...
  private _ipRenderEconomics() {
    const ale = this._ipCalcAle();
    const roi = this._ipCalcSroi();
    const budget = this._ipGetBudgetAlloc();
    const cb = this._ipGetCostBenefit();
    const cpi = this._ipCpi;
  ...
  @state() private _ipThreatLevel: any = null;
  @state() private _ipEmergingThreats: any = null;
  @state() private _ipThreatTrends: any = null;
  @state() private _ipSectorRadar: any = null;
  @state() private _ipActorActivity: any = null;

  // Threat Landscape Intelligence
  private ipInitThreatIntel() {
    this._ipThreatLevel = {
  ...
  private _ipRenderThreatIntel() {
    const tl = this._ipThreatLevel;
    const et = this._ipEmergingThreats;
    const sr = this._ipSectorRadar;
    const aa = this._ipActorActivity;
  ...
  @state() private _ipPolicies: any = null;
  @state() private _ipExceptions: any = null;
  @state() private _ipRiskRegister: any = null;
  @state() private _ipMeetings: any = null;
  @state() private _ipDeadlines: any = null;

  // Security Governance Dashboard
  private ipInitGovernance() {
    this._ipPolicies = [
  ...
  private _ipRenderGovernance() {
    const policies = this._ipPolicies;
    const risks = this._ipRiskRegister;
    const deadlines = this._ipDeadlines;
  ...
  @state() private _ipCriticalAssets: any = null;
  @state() private _ipAssetDeps: any = null;
  @state() private _ipEolAssets: any = null;
  @state() private _ipAssetRisk: any = null;

  // Asset Intelligence
  private ipInitAssetIntel() {
    this._ipCriticalAssets = [
    ...
    this._ipAssetRisk = { critical: 7, high: 23, medium: 45, low: 128, total: 203 };
  }

  private _ipRenderAssetIntel() {
    const assets = this._ipCriticalAssets;
    const eol = this._ipEolAssets;
    const ar = this._ipAssetRisk;
  ...
  @state() private _ipUserBaseline: any = null;
  @state() private _ipAnomalyRules: any = null;
  @state() private _ipDataAccess: any = null;
  @state() private _ipInsiderRisk: any = null;
  @state() private _irplayr15Timeline: Array<{id:string;time:string;event:string;severity:string;actor:string}> = [
  ...
  @state() private _irplayr15RootCauses: Array<{why:string;answer:string}> = [
  ...
  @state() private _irplayr15ImpactMatrix: Array<{system:string;users:number;data:string;revenue:string;status:string}> = [
  ...
  @state() private _irplayr15Actions: Array<{id:string;item:string;owner:string;deadline:string;priority:string;status:string}> = [
  ...
  @state() private _irplayr15Lessons: Array<{id:string;lesson:string;category:string;severity:string;applies_to:string}> = [
  ...
  @state() private _irplayr15ActiveTab: string = 'timeline';
  @state() private _irplayr15Benchmarks: Array<{metric:string;current:number;industry:number;target:number;unit:string;source:string}> = [
  ...
  @state() private _irplayr15MaturityLevels: Array<{domain:string;current:number;target:number;description:string}> = [
  ...
  @state() private _irplayr15QuarterlyData: Array<{quarter:string;score:number;improvement:number}> = [
  ...
  @state() private _irplayr15SelectedDomain: string = 'all';
  @state() private _irplayr15Alerts: Array<{id:string;name:string;severity:number;confidence:number;assetCrit:number;score:number;enriched:boolean;group:string;status:string;enrichData:Array<{key:string;value:string}>}> = [
  ...
  @state() private _irplayr15QualityMetrics: {fpRate:number;enrichSuccess:number;avgTriageTime:number;enrichedCount:number;totalCount:number} = {fpRate:0.12, enrichSuccess:0.78, avgTriageTime:4.5, enrichedCount:3, totalCount:5};
  @state() private _irplayr15RoutingRules: Array<{name:string;condition:string;channel:string;active:boolean}> = [
  ...
  @state() private _irplayr15Shapes: Array<{id:string;type:string;label:string;controls:string[]}> = [
  ...
  @state() private _irplayr15TrustBoundaries: Array<{from:string;to:string;label:string;strength:string}> = [
  ...
  @state() private _irplayr15ADRs: Array<{id:string;title:string;status:string;date:string;decision:string}> = [
  ...
  @state() private _irplayr15SelectedShape: string = '';
  @state() private _irplayr15Gauges: Array<{name:string;value:number;max:number;unit:string;status:string;color:string}> = [
  ...
  @state() private _irplayr15Anomalies: Array<{id:string;time:string;description:string;severity:string;acknowledged:boolean}> = [
  ...
  @state() private _irplayr15Integrations: Array<{name:string;status:string;uptime:number;lastCheck:string;latency:number}> = [
  ...
  @state() private _irplayr15AlertFatigue: Array<{analyst:string;alertsPerDay:number;escalated:number;dismissed:number;avgResponseMin:number}> = [
  ...
  @state() private _irplayr15SlaTarget: number = 99.9;

  // Insider Threat Detection
  private ipInitInsiderThreat() {
    this._ipUserBaseline = [
    ...
    this._ipInsiderRisk = { totalUsers: 2847, monitored: 342, flagged: 18, investigated: 5, confirmed: 1 };
  }

  private _ipRenderInsiderThreat() {
    const baseline = this._ipUserBaseline;
    const rules = this._ipAnomalyRules;
    const ir = this._ipInsiderRisk;
    ...
    return html`<div class="card" style="padding:14px;margin-bottom:8px">
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:4px;margin-bottom:10px;flex-wrap:wrap">
            ${['timeline','rootcause','impact','actions','lessons'].map(t => html`
              <button class="btn ${this._irplayr15ActiveTab === t ? 'btn-primary' : ''}" style="font-size:10px;padding:3px 8px" @click=${() => { this._irplayr15ActiveTab = t; }}>${t.charAt(0).toUpperCase() + t.slice(1)}</button>
            `)}
          </div>
          ${this._irplayr15ActiveTab === 'timeline' ? html`
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._irplayr15Timeline.map(e => html`
          ...
          ` : this._irplayr15ActiveTab === 'rootcause' ? html`
            <div style="margin-bottom:8px">${{__html: this._irplayr15RenderFishbone()}}</div>
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._irplayr15RootCauses.map((rc, i) => html`
          ...
          ` : this._irplayr15ActiveTab === 'impact' ? html`
            <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:10px">
              ...
              <tbody>${this._irplayr15ImpactMatrix.map(imp => html`
          ...
          ` : this._irplayr15ActiveTab === 'actions' ? html`
            <div style="display:flex;gap:12px;margin-bottom:8px;font-size:10px;color:#888">
              ${Object.entries(this._irplayr15GetActionStats()).map(([k,v]) => html`<span>${k}: <b style="color:#ddd">${v}</b></span>`)}
            </div>
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._irplayr15Actions.map(a => html`
                <div style="display:flex;gap:8px;align-items:center;padding:6px;background:#1a1d27;border-radius:4px;cursor:pointer" @click=${() => this._irplayr15ToggleAction(a.id)}>
          ...
          ` : html`
            <div style="display:flex;flex-direction:column;gap:4px">
              ${this._irplayr15Lessons.map(l => html`
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:4px;margin-bottom:10px;flex-wrap:wrap">
            ${['all', ...this._irplayr15MaturityLevels.map(m => m.domain)].map(d => html`
              <button class="btn ${this._irplayr15SelectedDomain === d ? 'btn-primary' : ''}" style="font-size:9px;padding:2px 6px" @click=${() => { this._irplayr15SelectedDomain = d; }}>${d}</button>
            `)}
          </div>
          <div style="display:flex;gap:16px;margin-bottom:10px">
            <div style="text-align:center"><div style="color:#e0e0e0;font-size:20px;font-weight:bold">${this._irplayr15GetOverallMaturity()}/5</div><div style="color:#888;font-size:10px">Maturity Level</div></div>
            <div style="text-align:center"><div style="color:#4f4;font-size:20px;font-weight:bold">${this._irplayr15GetGapAnalysis().filter(g => g.isAbove).length}/${this._irplayr15Benchmarks.length}</div><div style="color:#888;font-size:10px">Above Industry</div></div>
          </div>
          <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:10px">
            ...
            <tbody>${this._irplayr15GetGapAnalysis().map(b => html`
          ...
          <div style="margin-top:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Maturity by Domain</div>
            ${this._irplayr15MaturityLevels.filter(m => this._irplayr15SelectedDomain === 'all' || m.domain === this._irplayr15SelectedDomain).map(m => html`
          ...
          <div style="margin-top:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Quarterly Trend</div>
            <div style="display:flex;height:40px;align-items:flex-end;gap:4px">
              ${this._irplayr15QuarterlyData.map(q => html`<div style="flex:1;text-align:center"><div style="background:#4a9eff;height:${q.score * 0.5}px;border-radius:2px 2px 0 0" title="${q.score}"></div><div style="color:#666;font-size:8px;margin-top:2px">${q.quarter}</div></div>`)}
            </div>
          </div>
        </div>
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:12px;margin-bottom:8px;font-size:10px">
            <div style="display:flex;gap:4px;flex-wrap:wrap">${this._irplayr15GroupAlerts().map(g => html`<span style="color:#888;padding:2px 6px;background:#1a1d27;border-radius:3px">${g.group}: ${g.count}</span>`)}</div>
          </div>
          <div style="display:flex;gap:8px;margin-bottom:8px;font-size:10px;color:#888">
            <span>FP Rate: <b style="color:${this._irplayr15QualityMetrics.fpRate > 0.15 ? '#f44' : '#4f4'}">${(this._irplayr15QualityMetrics.fpRate * 100).toFixed(1)}%</b></span>
            <span>Enrich: <b style="color:#4a9eff">${(this._irplayr15QualityMetrics.enrichSuccess * 100).toFixed(0)}%</b></span>
            <span>Avg Triage: <b style="color:#ddd">${this._irplayr15QualityMetrics.avgTriageTime}m</b></span>
          </div>
          <div style="display:flex;flex-direction:column;gap:4px;max-height:200px;overflow-y:auto">
            ${this._irplayr15Alerts.sort((a, b) => this._irplayr15CalcScore(b) - this._irplayr15CalcScore(a)).map(a => {
              const score = this._irplayr15CalcScore(a);
              return html`
                <div style="padding:6px;background:#1a1d27;border-radius:4px;border-left:3px solid ${a.severity >= 4 ? '#f44' : a.severity >= 3 ? '#fa0' : '#4a9eff'}">
                  <div style="display:flex;justify-content:space-between;align-items:center">
                    ...
                    <button class="btn" style="font-size:9px;padding:1px 6px;margin-left:4px" @click=${() => this._irplayr15EnrichAlert(a.id)}>${a.enriched ? 'Re-enrich' : 'Enrich'}</button>
                  </div>
          ...
          <div style="margin-top:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Routing Rules</div>
            ${this._irplayr15RoutingRules.map(r => html`
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap">
            ${this._irplayr15Shapes.map(s => {
              const v = this._irplayr15ValidateControl(s.id);
              return html`<div style="padding:4px 8px;background:#1a1d27;border-radius:3px;cursor:pointer;border:1px solid ${this._irplayr15SelectedShape === s.id ? '#4a9eff' : '#2a2d37'}" @click=${() => { this._irplayr15SelectedShape = this._irplayr15SelectedShape === s.id ? '' : s.id; }}>
          ...
          ${this._irplayr15SelectedShape ? html`
            ${(() => {
              const shape = this._irplayr15Shapes.find(s => s.id === this._irplayr15SelectedShape);
              const v = this._irplayr15ValidateControl(this._irplayr15SelectedShape);
              return shape ? html`
          ...
          <div style="margin-bottom:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Trust Boundaries</div>
            ${this._irplayr15TrustBoundaries.map(tb => html`
              <div style="display:flex;gap:6px;align-items:center;padding:3px;background:#1a1d27;border-radius:3px;margin-bottom:2px;font-size:9px">
                <span style="color:#ddd">${this._irplayr15Shapes.find(s => s.id === tb.from)?.label || tb.from}</span>
                <span style="color:#4a9eff">\u2194</span>
                <span style="color:#ddd">${this._irplayr15Shapes.find(s => s.id === tb.to)?.label || tb.to}</span>
          ...
          <div><div style="color:#888;font-size:10px;margin-bottom:4px">Architecture Decision Records</div>
            ${this._irplayr15ADRs.map(adr => html`
        ...
        <div class="card" style="padding:12px;margin-bottom:8px">
          ...
          <div style="display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap">
            ${this._irplayr15Gauges.map(g => html`
          ...
          <div style="display:flex;gap:8px;margin-bottom:8px;align-items:center">
            ...
            <span style="color:${this._irplayr15GetOverallHealth().score >= 99 ? '#4f4' : '#fa0'};font-size:12px;font-weight:bold">${this._irplayr15GetOverallHealth().score}%</span>
            <span style="color:#888;font-size:10px">(Target: ${this._irplayr15SlaTarget}%)</span>
            <span style="color:#4f4;font-size:10px">${this._irplayr15GetOverallHealth().healthy} healthy</span>
            <span style="color:#fa0;font-size:10px">${this._irplayr15GetOverallHealth().degraded} degraded</span>
          </div>
          <div style="margin-bottom:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Anomaly Stream</div>
            ${this._irplayr15Anomalies.slice(0, 3).map(a => html`
              <div style="display:flex;gap:6px;align-items:center;padding:4px;background:#1a1d27;border-radius:3px;margin-bottom:2px;opacity:${a.acknowledged ? 0.5 : 1}">
                ...
                ${!a.acknowledged ? html`<button class="btn" style="font-size:8px;padding:1px 4px" @click=${() => this._irplayr15AckAnomaly(a.id)}>ACK</button>` : html`<span style="color:#4f4;font-size:9px">ACK'd</span>`}
              </div>
            `)}
          </div>
          <div style="margin-bottom:8px"><div style="color:#888;font-size:10px;margin-bottom:4px">Integration Health</div>
            ${this._irplayr15Integrations.map(i => html`
              <div style="display:flex;gap:6px;align-items:center;padding:3px;background:#1a1d27;border-radius:3px;margin-bottom:2px;font-size:9px">
          ...
          <div><div style="color:#888;font-size:10px;margin-bottom:4px">Alert Fatigue Analysis</div>
            <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:9px">
              ...
              <tbody>${this._irplayr15AlertFatigue.map(af => html`
  ...
  private _irplayr15RenderFishbone(): string {
  ...
  private _irplayr15ToggleAction(id: string) {
    this._irplayr15Actions = this._irplayr15Actions.map(a => a.id === id ? {...a, status: a.status === 'pending' ? 'in_progress' : a.status === 'in_progress' ? 'completed' : 'pending'} : a);
  }

  private _irplayr15GetActionStats() {
    const total = this._irplayr15Actions.length;
    const done = this._irplayr15Actions.filter(a => a.status === 'completed').length;
    const inProg = this._irplayr15Actions.filter(a => a.status === 'in_progress').length;
    return { total, done, inProg, pending: total - done - inProg };
  }

  private _irplayr15GetOverallMaturity(): number {
    if (this._irplayr15SelectedDomain === 'all') {
      return Math.round(this._irplayr15MaturityLevels.reduce((s, m) => s + m.current, 0) / this._irplayr15MaturityLevels.length * 10) / 10;
    }
    const d = this._irplayr15MaturityLevels.find(m => m.domain === this._irplayr15SelectedDomain);
    return d ? d.current : 0;
  }

  private _irplayr15GetGapAnalysis() {
    return this._irplayr15Benchmarks.map(b => {
  ...
  private _irplayr15CalcScore(alert: any): number {
    return Math.round(alert.severity * alert.confidence * alert.assetCrit * 100) / 100;
  }

  private _irplayr15EnrichAlert(id: string) {
    this._irplayr15Alerts = this._irplayr15Alerts.map(a => {
      if (a.id !== id) return a;
      const score = this._irplayr15CalcScore(a);
  ...
  private _irplayr15GroupAlerts() {
    const groups: Record<string, number> = {};
    this._irplayr15Alerts.forEach(a => { groups[a.group] = (groups[a.group] || 0) + 1; });
    return Object.entries(groups).map(([g, c]) => ({group: g, count: c}));
  }

  private _irplayr15ValidateControl(shapeId: string): {valid:boolean;missing:string[]} {
    const shape = this._irplayr15Shapes.find(s => s.id === shapeId);
  ...
  private _irplayr15GetOverallHealth(): {healthy:number;degraded:number;down:number;score:number} {
    const online = this._irplayr15Integrations.filter(i => i.status === 'online').length;
    const degraded = this._irplayr15Integrations.filter(i => i.status === 'degraded').length;
    const total = this._irplayr15Integrations.length;
    return { healthy: online, degraded, down: total - online - degraded, score: Math.round(online / total * 100) };
  }

  private _irplayr15AckAnomaly(id: string) {
    this._irplayr15Anomalies = this._irplayr15Anomalies.map(a => a.id === id ? {...a, acknowledged: true} : a);
  }
...
// ===== SECURITY ROI DASHBOARD (R22) =====
  ...
  private _renderBudgetPlanning() {
    const budgetData = [
      { category: "Personnel & Training", planned: 3073000, actual: 4583000, utilization: 149.1, q1: "25%", q2: "30%", q3: "28%", q4: "20%" },
      { category: "Tooling & Licensing", planned: 3080000, actual: 4596000, utilization: 149.2, q1: "28%", q2: "35%", q3: "23%", q4: "21%" },
      { category: "Infrastructure Security", planned: 3087000, actual: 4609000, utilization: 149.3, q1: "15%", q2: "24%", q3: "18%", q4: "22%" },
      { category: "Compliance & Audit", planned: 3094000, actual: 4622000, utilization: 149.4, q1: "18%", q2: "29%", q3: "29%", q4: "23%" },
      { category: "Incident Response", planned: 3101000, actual: 4635000, utilization: 149.5, q1: "21%", q2: "34%", q3: "24%", q4: "24%" },
      { category: "Third-Party Assessments", planned: 3108000, actual: 4648000, utilization: 149.5, q1: "24%", q2: "23%", q3: "19%", q4: "25%" },
      { category: "Security Awareness", planned: 3115000, actual: 4661000, utilization: 149.6, q1: "27%", q2: "28%", q3: "30%", q4: "10%" },
      { category: "Research & Innovation", planned: 3122000, actual: 4674000, utilization: 149.7, q1: "30%", q2: "33%", q3: "25%", q4: "11%" },
    ];
    ...
    const headcount = [
      { team: "SOC Tier 1", current: 11, target: 5, gap: 4, avgSalary: "179k" },
      { team: "SOC Tier 2", current: 3, target: 4, gap: 3, avgSalary: "97k" },
      { team: "Threat Intel", current: 12, target: 3, gap: 2, avgSalary: "126k" },
      { team: "Red Team", current: 4, target: 22, gap: 1, avgSalary: "155k" },
      { team: "GRC", current: 13, target: 21, gap: 0, avgSalary: "184k" },
      { team: "AppSec", current: 5, target: 20, gap: 5, avgSalary: "102k" },
      { team: "Cloud Sec", current: 14, target: 19, gap: 4, avgSalary: "131k" },
      { team: "Identity & Access", current: 6, target: 18, gap: 3, avgSalary: "160k" },
    ];
    const vendorSpend = [
      { vendor: "CrowdStrike", annual: "199k", contractEnd: "2026-11", renewalRisk: "Low", satisfaction: 4 },
      { vendor: "Palo Alto", annual: "230k", contractEnd: "2026-12", renewalRisk: "Medium", satisfaction: 3 },
      { vendor: "Splunk", annual: "261k", contractEnd: "2026-01", renewalRisk: "High", satisfaction: 5 },
      { vendor: "Qualys", annual: "292k", contractEnd: "2026-02", renewalRisk: "Low", satisfaction: 4 },
      { vendor: "Rapid7", annual: "323k", contractEnd: "2026-03", renewalRisk: "Medium", satisfaction: 3 },
      { vendor: "Mandiant", annual: "354k", contractEnd: "2026-04", renewalRisk: "High", satisfaction: 5 },
      { vendor: "Zscaler", annual: "385k", contractEnd: "2026-05", renewalRisk: "Low", satisfaction: 4 },
      { vendor: "Duo Security", annual: "416k", contractEnd: "2026-06", renewalRisk: "Medium", satisfaction: 3 },
    ];
    const roiProjections = [
      { area: "Threat Detection", investment: "485k", projectedReturn: "847k", roiMultiple: "2.7x", confidence: 94 },
      { area: "Incident Reduction", investment: "528k", projectedReturn: "894k", roiMultiple: "2.6x", confidence: 75 },
      { area: "Compliance Savings", investment: "571k", projectedReturn: "941k", roiMultiple: "2.5x", confidence: 92 },
      { area: "Automation Gains", investment: "614k", projectedReturn: "988k", roiMultiple: "2.4x", confidence: 73 },
      { area: "Risk Avoidance", investment: "657k", projectedReturn: "1035k", roiMultiple: "2.3x", confidence: 90 },
    ];
    return html`
  ...
  private _renderMetricsNormalization() {
    const kpiCatalog = [
      { id: "kpi-1", name: "MTTD", owner: "SOC", unit: "minutes", target: 71, current: 94, benchmark: 70, collection: "auto", frequency: "realtime" },
      { id: "kpi-2", name: "MTTR", owner: "GRC", unit: "%", target: 74, current: 55, benchmark: 81, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-3", name: "MTTC", owner: "AppSec", unit: "score", target: 77, current: 62, benchmark: 92, collection: "manual", frequency: "weekly" },
      { id: "kpi-4", name: "Vuln SLA Compliance", owner: "Cloud Sec", unit: "count", target: 80, current: 69, benchmark: 69, collection: "auto", frequency: "monthly" },
      { id: "kpi-5", name: "Patch Coverage", owner: "Identity", unit: "days", target: 83, current: 76, benchmark: 80, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-6", name: "Phishing Click Rate", owner: "Threat Intel", unit: "minutes", target: 86, current: 83, benchmark: 91, collection: "manual", frequency: "daily" },
      { id: "kpi-7", name: "Training Completion", owner: "Security Ops", unit: "%", target: 89, current: 90, benchmark: 68, collection: "auto", frequency: "weekly" },
      { id: "kpi-8", name: "Escalation Rate", owner: "Risk Mgmt", unit: "score", target: 92, current: 97, benchmark: 79, collection: "semi-auto", frequency: "monthly" },
      { id: "kpi-9", name: "False Positive Rate", owner: "SOC", unit: "count", target: 95, current: 58, benchmark: 90, collection: "manual", frequency: "realtime" },
      { id: "kpi-10", name: "Threat Intel Actionability", owner: "GRC", unit: "days", target: 98, current: 65, benchmark: 67, collection: "auto", frequency: "daily" },
      { id: "kpi-11", name: "Endpoint Compliance", owner: "AppSec", unit: "minutes", target: 71, current: 72, benchmark: 78, collection: "semi-auto", frequency: "weekly" },
      { id: "kpi-12", name: "Cloud Misconfig Score", owner: "Cloud Sec", unit: "%", target: 74, current: 79, benchmark: 89, collection: "manual", frequency: "monthly" },
      { id: "kpi-13", name: "Identity Anomaly Rate", owner: "Identity", unit: "score", target: 77, current: 86, benchmark: 66, collection: "auto", frequency: "realtime" },
      { id: "kpi-14", name: "DLP Events", owner: "Threat Intel", unit: "count", target: 80, current: 93, benchmark: 77, collection: "semi-auto", frequency: "daily" },
      { id: "kpi-15", name: "Vendor Risk Avg", owner: "Security Ops", unit: "days", target: 83, current: 100, benchmark: 88, collection: "manual", frequency: "weekly" },
      { id: "kpi-16", name: "Compliance Audit Pass Rate", owner: "Risk Mgmt", unit: "minutes", target: 86, current: 61, benchmark: 65, collection: "auto", frequency: "monthly" },
      { id: "kpi-17", name: "Awareness Score", owner: "SOC", unit: "%", target: 89, current: 68, benchmark: 76, collection: "semi-auto", frequency: "realtime" },
      { id: "kpi-18", name: "SOC Utilization", owner: "GRC", unit: "score", target: 92, current: 75, benchmark: 87, collection: "manual", frequency: "daily" },
      { id: "kpi-19", name: "Automation Coverage", owner: "AppSec", unit: "count", target: 95, current: 82, benchmark: 98, collection: "auto", frequency: "weekly" },
      { id: "kpi-20", name: "Risk Register Currency", owner: "Cloud Sec", unit: "days", target: 98, current: 89, benchmark: 75, collection: "semi-auto", frequency: "monthly" },
    ];
    const benchmarkSources = [
      { source: "NIST CSF", mappedKPIs: 4, alignment: 70, lastReview: "2026-02-10", status: "aligned" },
      { source: "CIS Controls v8", mappedKPIs: 5, alignment: 87, lastReview: "2026-03-05", status: "partial" },
      { source: "ISO 27001:2022", mappedKPIs: 6, alignment: 65, lastReview: "2026-04-28", status: "reviewing" },
      { source: "PCI DSS 4.0", mappedKPIs: 7, alignment: 82, lastReview: "2026-05-23", status: "aligned" },
      { source: "SOC 2 Type II", mappedKPIs: 8, alignment: 60, lastReview: "2026-06-18", status: "partial" },
      { source: "MITRE ATT&CK", mappedKPIs: 3, alignment: 77, lastReview: "2026-01-13", status: "reviewing" },
      { source: "SANS Top 20", mappedKPIs: 4, alignment: 94, lastReview: "2026-02-08", status: "aligned" },
      { source: "OWASP Top 10", mappedKPIs: 5, alignment: 72, lastReview: "2026-03-03", status: "partial" },
    ];
  ...
  private _renderThreatHuntingCampaigns() {
    const campaigns = [
      { id: "HC-1001", name: "Lateral Movement Sweep", status: "active", hypothesis: "H1: Actors using pass-the-hash for lateral movement", leadHunter: "Alice Chen", findings: 41, startDate: "2026-02-02", endDate: null, effectiveness: 86 },
      { id: "HC-1002", name: "Credential Harvesting Hunt", status: "completed", hypothesis: "H2: Actors using web shells for persistence", leadHunter: "Bob Martinez", findings: 44, startDate: "2026-01-13", endDate: "2026-04-19", effectiveness: 46 },
      { id: "HC-1003", name: "Persistence Mechanism Audit", status: "planned", hypothesis: "H3: Actors using scheduled tasks for data theft", leadHunter: "Carol Wu", findings: 47, startDate: "2026-04-24", endDate: null, effectiveness: 65 },
      { id: "HC-1004", name: "C2 Beacon Detection", status: "in-review", hypothesis: "H4: Actors using DNS tunneling for C2 communication", leadHunter: "Dave Kim", findings: 2, startDate: "2026-03-07", endDate: null, effectiveness: 84 },
      { id: "HC-1005", name: "Data Exfiltration Patterns", status: "active", hypothesis: "H5: Actors using encrypted channels for privilege escalation", leadHunter: "Eve Johnson", findings: 5, startDate: "2026-02-18", endDate: null, effectiveness: 44 },
      { id: "HC-1006", name: "Privilege Escalation Scan", status: "completed", hypothesis: "H6: Actors using token impersonation for defense evasion", leadHunter: "Frank Liu", findings: 8, startDate: "2026-01-01", endDate: "2026-05-03", effectiveness: 63 },
      { id: "HC-1007", name: "Supply Chain Implant Hunt", status: "planned", hypothesis: "H7: Actors using poisoned images for initial access", leadHunter: "Grace Park", findings: 11, startDate: "2026-04-12", endDate: null, effectiveness: 82 },
      { id: "HC-1008", name: "Insider Threat Indicators", status: "in-review", hypothesis: "H8: Actors using legitimate tools for credential access", leadHunter: "Hector Silva", findings: 14, startDate: "2026-03-23", endDate: null, effectiveness: 42 },
      { id: "HC-1009", name: "Cloud Metadata Analysis", status: "active", hypothesis: "H9: Actors using API keys for command execution", leadHunter: "Alice Chen", findings: 17, startDate: "2026-02-06", endDate: null, effectiveness: 61 },
      { id: "HC-1010", name: "DNS Tunnel Detection", status: "completed", hypothesis: "H10: Actors using encoded subdomains for exfiltration", leadHunter: "Bob Martinez", findings: 20, startDate: "2026-01-17", endDate: "2026-06-15", effectiveness: 80 },
      { id: "HC-1011", name: "Fileless Malware Search", status: "planned", hypothesis: "H11: Actors using WMI providers for discovery", leadHunter: "Carol Wu", findings: 23, startDate: "2026-04-28", endDate: null, effectiveness: 40 },
      { id: "HC-1012", name: "Zero-Day Exploit Traces", status: "in-review", hypothesis: "H12: Actors using exploit kits for collection", leadHunter: "Dave Kim", findings: 26, startDate: "2026-03-11", endDate: null, effectiveness: 59 },
    ];
    const hunterLeaderboard = [
      { hunter: "Alice Chen", campaigns: 12, findings: 6, highSeverity: 9, avgScore: 60, streak: 2 },
      { hunter: "Bob Martinez", campaigns: 9, findings: 35, highSeverity: 14, avgScore: 97, streak: 3 },
      { hunter: "Carol Wu", campaigns: 6, findings: 64, highSeverity: 19, avgScore: 90, streak: 4 },
      { hunter: "Dave Kim", campaigns: 3, findings: 93, highSeverity: 24, avgScore: 83, streak: 5 },
      { hunter: "Eve Johnson", campaigns: 13, findings: 6, highSeverity: 3, avgScore: 76, streak: 6 },
      { hunter: "Frank Liu", campaigns: 10, findings: 35, highSeverity: 8, avgScore: 69, streak: 7 },
      { hunter: "Grace Park", campaigns: 7, findings: 64, highSeverity: 13, avgScore: 62, streak: 8 },
      { hunter: "Hector Silva", campaigns: 4, findings: 93, highSeverity: 18, avgScore: 55, streak: 1 },
    ];
    const mitreMapping = [
      { tactic: "Initial Access", techniques: 7, campaigns: 6, coverage: 43 },
      { tactic: "Execution", techniques: 6, campaigns: 5, coverage: 94 },
      { tactic: "Persistence", techniques: 5, campaigns: 4, coverage: 74 },
      { tactic: "Privilege Escalation", techniques: 4, campaigns: 3, coverage: 54 },
      { tactic: "Defense Evasion", techniques: 3, campaigns: 2, coverage: 34 },
      { tactic: "Credential Access", techniques: 2, campaigns: 1, coverage: 85 },
      { tactic: "Discovery", techniques: 12, campaigns: 6, coverage: 65 },
      { tactic: "Lateral Movement", techniques: 11, campaigns: 5, coverage: 45 },
      { tactic: "Collection", techniques: 10, campaigns: 4, coverage: 96 },
      { tactic: "Exfiltration", techniques: 9, campaigns: 3, coverage: 76 },
      { tactic: "Command & Control", techniques: 8, campaigns: 2, coverage: 56 },
      { tactic: "Impact", techniques: 7, campaigns: 1, coverage: 36 },
    ];
    return html`
  ...
  private _renderControlInventory() {
    const controls = [
      ...
      { id: "CTL-2002", name: "Network Segmentation", domain: "Network Security", status: "partial", effectiveness: 40, lastTest: "2026-01-07", nextReview: "2026-07-13", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2003", name: "EDR Deployment", domain: "Endpoint Protection", status: "planned", effectiveness: 47, lastTest: "2026-04-20", nextReview: "2026-08-04", owner: "IT Ops", risk: "High" },
      { id: "CTL-2004", name: "DLP Policy", domain: "Data Protection", status: "gap", effectiveness: 3, lastTest: "2026-03-05", nextReview: "2026-09-23", owner: "Data Gov", risk: "Critical" },
      ...
      { id: "CTL-2006", name: "SAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 17, lastTest: "2026-01-03", nextReview: "2026-11-05", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2007", name: "CSPM Scanning", domain: "Cloud Security", status: "planned", effectiveness: 24, lastTest: "2026-04-16", nextReview: "2026-12-24", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2008", name: "Badge Access", domain: "Physical Security", status: "gap", effectiveness: 31, lastTest: "2026-03-01", nextReview: "2026-05-15", owner: "Facilities", risk: "Critical" },
      ...
      { id: "CTL-2010", name: "Firewall Rules", domain: "Network Security", status: "partial", effectiveness: 45, lastTest: "2026-01-27", nextReview: "2026-07-25", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2011", name: "Disk Encryption", domain: "Endpoint Protection", status: "planned", effectiveness: 1, lastTest: "2026-04-12", nextReview: "2026-08-16", owner: "IT Ops", risk: "High" },
      { id: "CTL-2012", name: "Data Classification", domain: "Data Protection", status: "gap", effectiveness: 8, lastTest: "2026-03-25", nextReview: "2026-09-07", owner: "Data Gov", risk: "Critical" },
      ...
      { id: "CTL-2014", name: "DAST Pipeline", domain: "Application Security", status: "partial", effectiveness: 22, lastTest: "2026-01-23", nextReview: "2026-11-17", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2015", name: "IAM Policy Review", domain: "Cloud Security", status: "planned", effectiveness: 29, lastTest: "2026-04-08", nextReview: "2026-12-08", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2016", name: "Visitor Management", domain: "Physical Security", status: "gap", effectiveness: 36, lastTest: "2026-03-21", nextReview: "2026-05-27", owner: "Facilities", risk: "Critical" },
      ...
      { id: "CTL-2018", name: "IDS/IPS Tuning", domain: "Network Security", status: "partial", effectiveness: 50, lastTest: "2026-01-19", nextReview: "2026-07-09", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2019", name: "Patch Management", domain: "Endpoint Protection", status: "planned", effectiveness: 6, lastTest: "2026-04-04", nextReview: "2026-08-28", owner: "IT Ops", risk: "High" },
      { id: "CTL-2020", name: "Backup Encryption", domain: "Data Protection", status: "gap", effectiveness: 13, lastTest: "2026-03-17", nextReview: "2026-09-19", owner: "Data Gov", risk: "Critical" },
      ...
      { id: "CTL-2022", name: "Container Scanning", domain: "Application Security", status: "partial", effectiveness: 27, lastTest: "2026-01-15", nextReview: "2026-11-01", owner: "DevSecOps", risk: "Medium" },
      { id: "CTL-2023", name: "WAF Configuration", domain: "Cloud Security", status: "planned", effectiveness: 34, lastTest: "2026-04-28", nextReview: "2026-12-20", owner: "Cloud Ops", risk: "High" },
      { id: "CTL-2024", name: "CCTV Coverage", domain: "Physical Security", status: "gap", effectiveness: 41, lastTest: "2026-03-13", nextReview: "2026-05-11", owner: "Facilities", risk: "Critical" },
      ...
      { id: "CTL-2026", name: "VPN Management", domain: "Network Security", status: "partial", effectiveness: 4, lastTest: "2026-01-11", nextReview: "2026-07-21", owner: "Network Ops", risk: "Medium" },
      { id: "CTL-2027", name: "App Whitelisting", domain: "Endpoint Protection", status: "planned", effectiveness: 11, lastTest: "2026-04-24", nextReview: "2026-08-12", owner: "IT Ops", risk: "High" },
      { id: "CTL-2028", name: "Key Management", domain: "Data Protection", status: "gap", effectiveness: 18, lastTest: "2026-03-09", nextReview: "2026-09-03", owner: "Data Gov", risk: "Critical" },
    ];
    const gapAnalysis = [
      { gap: "Insufficient MFA coverage for legacy apps", severity: "High", remediationPlan: "Plan R3001", eta: "2026-Q2", estimatedCost: "50k" },
      { gap: "Missing network micro-segmentation", severity: "Medium", remediationPlan: "Plan R3002", eta: "2026-Q4", estimatedCost: "79k" },
      { gap: "Inconsistent EDR deployment", severity: "Medium", remediationPlan: "Plan R3003", eta: "2026-Q3", estimatedCost: "108k" },
      { gap: "DLP not covering cloud storage", severity: "Low", remediationPlan: "Plan R3004", eta: "2026-Q2", estimatedCost: "137k" },
      { gap: "SSO not integrated with all SaaS", severity: "High", remediationPlan: "Plan R3005", eta: "2026-Q4", estimatedCost: "166k" },
    ];
    return html`
  ...
  private _renderIncidentCostTracker() {
    const incidents = [
      { id: "INC-7001", name: "Security Incident 1", severity: "Critical", totalCost: 2062000, responseCost: 577360, recoveryCost: 680460, legalCost: 288680, regulatoryCost: 164960, insuranceClaim: 886660, avoidedCost: 126000, date: "2026-02-14" },
      { id: "INC-7002", name: "Security Incident 2", severity: "High", totalCost: 2065000, responseCost: 722750, recoveryCost: 474950, legalCost: 227150, regulatoryCost: 289100, insuranceClaim: 0, avoidedCost: 155000, date: "2026-01-23" },
      { id: "INC-7003", name: "Security Incident 3", severity: "Medium", totalCost: 2068000, responseCost: 434280, recoveryCost: 703120, legalCost: 165440, regulatoryCost: 186120, insuranceClaim: 785840, avoidedCost: 184000, date: "2026-04-04" },
      { id: "INC-7004", name: "Security Incident 4", severity: "Low", totalCost: 2071000, responseCost: 579880, recoveryCost: 497040, legalCost: 103550, regulatoryCost: 310650, insuranceClaim: 0, avoidedCost: 213000, date: "2026-03-13" },
      { id: "INC-7005", name: "Security Incident 5", severity: "Critical", totalCost: 2074000, responseCost: 725900, recoveryCost: 725900, legalCost: 373320, regulatoryCost: 207400, insuranceClaim: 684420, avoidedCost: 242000, date: "2026-02-22" },
      { id: "INC-7006", name: "Security Incident 6", severity: "High", totalCost: 2077000, responseCost: 436170, recoveryCost: 519250, legalCost: 311550, regulatoryCost: 103850, insuranceClaim: 0, avoidedCost: 271000, date: "2026-01-03" },
      { id: "INC-7007", name: "Security Incident 7", severity: "Medium", totalCost: 2080000, responseCost: 582400, recoveryCost: 748800, legalCost: 249600, regulatoryCost: 228800, insuranceClaim: 1643200, avoidedCost: 300000, date: "2026-04-12" },
      { id: "INC-7008", name: "Security Incident 8", severity: "Low", totalCost: 2083000, responseCost: 729050, recoveryCost: 541580, legalCost: 187470, regulatoryCost: 124980, insuranceClaim: 0, avoidedCost: 329000, date: "2026-03-21" },
      { id: "INC-7009", name: "Security Incident 9", severity: "Critical", totalCost: 2086000, responseCost: 438060, recoveryCost: 771820, legalCost: 125160, regulatoryCost: 250320, insuranceClaim: 1543640, avoidedCost: 358000, date: "2026-02-02" },
      { id: "INC-7010", name: "Security Incident 10", severity: "High", totalCost: 2089000, responseCost: 584920, recoveryCost: 564030, legalCost: 396910, regulatoryCost: 146230, insuranceClaim: 0, avoidedCost: 387000, date: "2026-01-11" },
      { id: "INC-7011", name: "Security Incident 11", severity: "Medium", totalCost: 2092000, responseCost: 732200, recoveryCost: 794960, legalCost: 334720, regulatoryCost: 271960, insuranceClaim: 1443480, avoidedCost: 416000, date: "2026-04-20" },
      { id: "INC-7012", name: "Security Incident 12", severity: "Low", totalCost: 2095000, responseCost: 439950, recoveryCost: 586600, legalCost: 272350, regulatoryCost: 167600, insuranceClaim: 0, avoidedCost: 445000, date: "2026-03-01" },
    ];
    const yearlyTrend = [
      { month: "Jan", incidents: 5, totalCost: "443k", avgCost: "145k", insured: 33 },
      { month: "Feb", incidents: 2, totalCost: "486k", avgCost: "192k", insured: 33 },
      { month: "Mar", incidents: 10, totalCost: "529k", avgCost: "58k", insured: 33 },
      { month: "Apr", incidents: 7, totalCost: "572k", avgCost: "105k", insured: 33 },
      { month: "May", incidents: 4, totalCost: "615k", avgCost: "152k", insured: 33 },
      { month: "Jun", incidents: 12, totalCost: "658k", avgCost: "199k", insured: 33 },
    ];
...
declare global { interface HTMLElementTagNameMap { 'sc-ir-playbook': ScIrPlaybook; } }
