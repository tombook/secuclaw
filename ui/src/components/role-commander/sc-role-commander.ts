    ...
    return html`
      <div class="zone zone-dark" style="background:linear-gradient(135deg,#0a0a0a 0%,#1a0a0a 100%);border:1px solid #3b1111;border-radius:8px;margin:0 14px 8px;">
        <div style="display:flex;align-items:center;gap:8px;padding:8px 12px;cursor:pointer;" @click=${() => { this._darkExpanded = !this._darkExpanded; }}>
          <span style="font-size:12px;font-weight:600;color:#ef4444;">⚔️ Dark Side 能力</span>
          <span style="font-size:10px;color:#64748b;">${caps.length} 项攻防模拟 · SKILL.md 定义</span>
        ...
        ${this._darkExpanded ? html`
          <div style="display:flex;flex-wrap:wrap;gap:6px;padding:0 12px 10px;">
            ${caps.map(c => html`
              <button
                style="display:inline-flex;align-items:center;gap:4px;padding:4px 10px;border-radius:6px;border:1px solid ${sevColors[c.severity]}33;background:${sevColors[c.severity]}11;color:#e2e8f0;font-size:11px;cursor:pointer;transition:all 0.15s;"
                @mouseenter=${(e: Event) => { (e.target as HTMLElement).style.borderColor = sevColors[c.severity]; }}
                @mouseleave=${(e: Event) => { (e.target as HTMLElement).style.borderColor = `${sevColors[c.severity]}33`; }}
                @click=${(e: Event) => { e.stopPropagation(); this._openToolPanel(c.toolId); }}
              >
                <span style="width:6px;height:6px;border-radius:50%;background:${sevColors[c.severity]};"></span>
                ${c.label}
                <span style="font-size:9px;color:${sevColors[c.severity]};margin-left:2px;">${sevLabels[c.severity]}</span>
              </button>
            `)}
          </div>
        ` : nothing}
