...
const TECHNIQUES: Record<string, TechCell[]> = {
  Recon: [
...
export interface MitreHeatmapProps {
  /** Role ID — determines which coverage data to show */
  roleId?: string;
  /** Role accent color used for cell shading */
  accentColor?: string;
  className?: string;
}

function coverageToOpacity(coverage: number): number {
  // Map 0-100 → 0.1-1.0
  return 0.1 + (coverage / 100) * 0.9;
}

// Role-specific MITRE coverage — each role emphasizes different tactics
const ROLE_COVERAGE: Record<string, Record<string, number>> = {
...
export const MitreHeatmap: React.FC<MitreHeatmapProps> = ({
  ...
  accentColor = 'var(--role-primary, #1e40af)',
  className = '',
}) => {
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <div className={`overflow-x-auto ${className}`}>
        {/* Tactic headers */}
        <div className="grid gap-1 mb-1 min-w-[900px]" style={{ gridTemplateColumns: `repeat(${TACTICS.length}, minmax(60px, 1fr))` }}>
          {TACTICS.map((tactic) => (
            <div
              key={tactic}
              className="text-[10px] font-semibold text-center py-1 px-0.5 truncate"
              style={{ color: 'var(--role-text-secondary, #94a3b8)' }}
            >
              {tactic}
            </div>
          ))}
        ...
        {Array.from({ length: maxRows }).map((_, rowIdx) => (
          <div
            key={rowIdx}
            className="grid gap-1 mb-1 min-w-[900px]"
            style={{ gridTemplateColumns: `repeat(${TACTICS.length}, minmax(60px, 1fr))` }}
          >
            {TACTICS.map((tactic) => {
              const cell = TECHNIQUES[tactic]?.[rowIdx];
              if (!cell) return <div key={tactic} />;
              return (
                <Tooltip key={`${tactic}-${cell.id}`}>
                  <TooltipTrigger asChild>
                    <div
                      className="rounded-sm flex items-center justify-center text-[10px] font-medium cursor-pointer transition-colors hover:ring-1 hover:ring-white/20"
                      style={{
                  ...
                  <TooltipContent
                    side="top"
                    className="bg-[#0f1525] border border-white/10 text-white text-xs"
                  >
                    <div className="font-semibold">{cell.name}</div>
                    <div className="text-white/60">Coverage: {roleCoverage[tactic] ?? 50}%</div>
                  </TooltipContent>
                </Tooltip>
              );
            })}
