...
interface NavItemRole extends NavItemBase {
  type: 'role';
  roleId: RoleId;
}
...
interface NavItemSep {
  type: 'separator';
}

type NavItem = NavItemRole | NavItemPage | NavItemSep;
...
export const AppSidebar: React.FC = () => {
  ...
  const isActive = (item: NavItem): boolean => {
    ...
    if (item.type === 'role' && item.roleId === currentRole) return true;
    ...
    <TooltipProvider delayDuration={0}>
      <aside
        className={cn(
          ...
          'bg-[#0f1525] border-r border-white/[0.06]',
      ...
      >
        ...
        <div
          className={cn(
            'flex items-center h-14 shrink-0 border-b border-white/[0.06] px-4',
            collapsed ? 'justify-center' : 'gap-3'
          )}
        >
          <div
            className="h-8 w-8 rounded-lg flex items-center justify-center text-lg font-bold shrink-0 transition-colors duration-500"
            style={{
              background: theme
          ...
          {NAV_ITEMS.map((item, idx) => {
            ...
            const inner = (
              <button
                ...
                className={cn(
                  'group relative flex items-center w-full rounded-lg text-left',
            ...
            if (collapsed) {
              return (
                <Tooltip key={item.id}>
                  ...
                  <TooltipContent side="right" className="bg-[#1a2035] text-slate-200 border-white/10 text-xs">
