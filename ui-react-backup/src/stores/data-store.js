...
// ── Initial Data ──
function createInitialDatabases() {
    return [
...
export const useDataStore = create()((set, get) => ({
    ...
    getQualityIssuesByDatabase(databaseId) {
        return get().qualityIssues.filter((qi) => qi.databaseId === databaseId);
    },
    fixQualityIssue(issueId) {
        set((state) => ({
            qualityIssues: state.qualityIssues.map((qi) => qi.id === issueId ? { ...qi, status: 'resolved' } : qi),
        }));
    },
    syncDataSource(sourceId) {
        set((state) => ({
            dataSources: state.dataSources.map((ds) => ds.id === sourceId ? { ...ds, lastSync: new Date(), status: 'connected' } : ds),
        }));
    },
}));
