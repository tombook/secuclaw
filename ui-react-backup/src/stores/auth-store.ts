...
export const useAuthStore = create<AuthStore>()(
  persist(
    (set, get) => ({
      ...
      // Actions
      async login(username: string, password: string, wsStore: WebSocketStore): Promise<User> {
        set({ loading: true });
        try {
          const result = await wsStore.request<{ token: string; user: { id: string; username: string; displayName: string; roleIds: string[] } }>(
...
function mapRoleIds(roleIds: string[]): RoleId {
