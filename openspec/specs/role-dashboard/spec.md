...
When a role is selected, dashboard SHALL show KPI cards relevant to that role's focus area. Additionally, dashboard SHALL display RACI task summary for the current role.
...
Alert panel SHALL filter to show alerts relevant to current role's domain.
...
When the global role context changes, dashboard SHALL immediately reflect the new role's configuration.
