...
- **THEN** SkillExecutor SHALL execute the skill specified in the workflow step's skillId
- **AND** execution result SHALL be recorded in the War Room event timeline

#### Scenario: Execute skill from AI suggestion
- **WHEN** user accepts an 'ai-suggest' workflow step's recommendation
- **THEN** SkillExecutor SHALL execute the suggested skill with the AI-provided parameters
- **AND** result SHALL be recorded in the workflow step status
